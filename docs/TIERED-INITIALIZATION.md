...
```typescript
// voice-agent-child.ts - Lightweight child entry
export default defineAgent({
  prewarm: async (proc) => {
    // Just mark ready - main process handles warming
    proc.userData.prewarmComplete = true;
  },
  
  entry: async (ctx) => {
    // Load session runner dynamically
    const { runFullVoiceAgentEntry } = await import('./voice-agent-entry.js');
    await runFullVoiceAgentEntry(ctx);
  },
});

// voice-agent-entry.ts - Lazy loading entry
async function loadVoiceDeps() {
  // Load dependencies sequentially with logging
