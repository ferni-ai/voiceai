...
function scanForDebt(): DebtItem[] {
  ...
  for (const marker of MARKERS) {
    try {
      ...
      const cmd = `grep -rn --include="*.ts" --include="*.tsx" --include="*.js" --include="*.md" ${ignoreArgs} --exclude="tech-debt.ts" "${marker.pattern}" src scripts frontend-typescript/src docs 2>/dev/null || true`;
