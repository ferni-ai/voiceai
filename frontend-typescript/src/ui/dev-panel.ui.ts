...
function createPanel(): HTMLElement {
  ...
  container.innerHTML = `
    ...
    <div class="dev-panel__content">
      ...
      <section class="dev-section">
        <h3 class="dev-section__title">${ICONS.sparkles} Brand Health</h3>
        <p class="dev-section__desc">Monitor brand compliance across touchpoints</p>

        <div class="dev-brand-health" id="dev-brand-health">
          <div class="dev-brand-health-loading">Loading brand health...</div>
        </div>

        <div class="dev-subsection">
          <span class="dev-label">Quick Validate</span>
          <div class="dev-expression-buttons">
            <button class="dev-expression-btn" data-brand="refresh" title="Refresh brand health metrics">🔄 Refresh</button>
            <button class="dev-expression-btn" data-brand="check" title="Validate test content">✓ Test Validate</button>
          </div>
        </div>

        <div class="dev-subsection">
          <span class="dev-label">Content Validator</span>
          <textarea id="dev-brand-content" class="dev-brand-textarea" placeholder="Paste content to validate against brand rules..."></textarea>
          <div class="dev-expression-buttons" style="margin-top: var(--space-2);">
            <button class="dev-expression-btn" data-brand="validate" title="Validate content">Validate Content</button>
          </div>
          <div id="dev-brand-result" class="dev-brand-result"></div>
        </div>
      </section>
  ...
  // 🌿 Brand health buttons
  container.querySelectorAll('[data-brand]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const action = (btn as HTMLElement).dataset.brand;
      handleBrandAction(action!);
    });
  });

  // Initialize brand health on panel open
  loadBrandHealth();
...
// ============================================================================
// BRAND HEALTH ACTIONS
// ============================================================================

async function loadBrandHealth(): Promise<void> {
  const container = document.getElementById('dev-brand-health');
  if (!container) return;

  try {
    const response = await fetch('/api/brand/health');
    if (!response.ok) {
      container.innerHTML = '<div class="dev-brand-health-error">Auth required</div>';
      return;
    }

    const data = await response.json();
    
    const complianceColor = data.complianceRate >= 0.9 ? 'var(--color-success)' : 
                           data.complianceRate >= 0.7 ? 'var(--color-warning)' : 
                           'var(--color-error)';

    container.innerHTML = `
      <div class="dev-brand-health-grid">
        <div class="dev-brand-metric">
          <span class="dev-brand-metric-value" style="color: ${complianceColor}">
            ${Math.round(data.complianceRate * 100)}%
          </span>
          <span class="dev-brand-metric-label">Compliance</span>
        </div>
        <div class="dev-brand-metric">
          <span class="dev-brand-metric-value">${Math.round(data.averageScore)}</span>
          <span class="dev-brand-metric-label">Avg Score</span>
        </div>
        <div class="dev-brand-metric">
          <span class="dev-brand-metric-value">${data.recentLearnings || 0}</span>
          <span class="dev-brand-metric-label">Learnings</span>
        </div>
        <div class="dev-brand-metric">
          <span class="dev-brand-metric-value">${(data.experimentVelocity || 0).toFixed(1)}</span>
          <span class="dev-brand-metric-label">Exp/Week</span>
        </div>
      </div>
      ${data.topViolations?.length > 0 ? `
        <div class="dev-brand-violations">
          <span class="dev-label">Top Violations:</span>
          <ul>
            ${data.topViolations.slice(0, 3).map((v: { text: string }) => `<li>${v.text}</li>`).join('')}
          </ul>
        </div>
      ` : ''}
    `;
  } catch (error) {
    container.innerHTML = '<div class="dev-brand-health-error">Failed to load</div>';
  }
}

async function handleBrandAction(action: string): Promise<void> {
  switch (action) {
    case 'refresh':
    ...
    case 'check': {
      ...
      const testContent = "As an AI assistant, I'm designed to help you 24/7!";
      await validateBrandContent(testContent);
      break;
    }

    case 'validate': {
      const textarea = document.getElementById('dev-brand-content') as HTMLTextAreaElement;
      if (textarea?.value) {
        await validateBrandContent(textarea.value);
      }
      break;
    }
  }
...
async function validateBrandContent(content: string): Promise<void> {
  const resultDiv = document.getElementById('dev-brand-result');
  if (!resultDiv) return;

  resultDiv.innerHTML = '<div class="dev-brand-loading">Validating...</div>';
  ...
  try {
    ...
    const statusColor = data.isCompliant ? 'var(--color-success)' : 'var(--color-error)';
    const statusIcon = data.isCompliant ? '✓' : '✗';

    resultDiv.innerHTML = `
      <div class="dev-brand-result-header" style="color: ${statusColor}">
        ${statusIcon} Score: ${data.score}/100
      </div>
      ${data.violations?.length > 0 ? `
        <div class="dev-brand-violations">
          ${data.violations.map((v: { suggestion: string }) => `
            <div class="dev-brand-violation">⚠ ${v.suggestion}</div>
          `).join('')}
        </div>
      ` : '<div class="dev-brand-success">Content is brand compliant!</div>'}
    `;
  } catch (error) {
    resultDiv.innerHTML = '<div class="dev-brand-error">Validation failed</div>';
  }
}
...
function injectStyles(): void {
  ...
  styleElement.textContent = `
    ...
    .dev-brand-metric-value {
      font-size: 1.2rem;
      font-weight: 600;
      color: var(--dev-accent-bright);
    }
    
    .dev-brand-metric-label {
      font-size: 0.65rem;
      color: var(--dev-text-muted);
      text-transform: uppercase;
      letter-spacing: 0.5px;
    ...
    .dev-brand-violations ul {
      margin: var(--space-1, 4px) 0 0 var(--space-3, 12px);
      padding: 0;
      color: rgba(255, 180, 180, 0.9);
    }
    
    .dev-brand-health-loading,
    .dev-brand-health-error {
      text-align: center;
      padding: var(--space-3, 12px);
      color: var(--dev-text-muted);
      font-size: 0.8rem;
    ...
    .dev-brand-result:empty {
      display: none;
    }
    
    .dev-brand-result-header {
      font-weight: 600;
    ...
    .dev-brand-violation:last-child {
      border-bottom: none;
    }
