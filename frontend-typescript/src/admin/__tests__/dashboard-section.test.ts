...
describe('Dashboard Section Rendering', () => {
  ...
  describe('Quick Actions', () => {
    ...
    it('should have data-action attributes', () => {
      ...
      expect(actionButtons.some((btn) => hasAttribute(btn, 'data-action', 'validate-agents'))).toBe(
        true
      );
      expect(actionButtons.some((btn) => hasAttribute(btn, 'data-action', 'run-evalops'))).toBe(
        true
      );
    });
  });
