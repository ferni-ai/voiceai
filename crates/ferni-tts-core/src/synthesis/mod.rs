...
//!
//! Abstraction layer for TTS synthesis (client trait, types, mock).
//! CosyVoice and other backends live in the ferni-tts HTTP service crate.
...
/// Create a synthesis client (core only supports Mock; use ferni-tts service for CosyVoice etc.)
pub async fn create_client(config: &SynthesisConfig) -> Result<Arc<dyn SynthesisClient>> {
    match config.backend {
        ...
        SynthesisBackend::CosyVoice | SynthesisBackend::Azure | SynthesisBackend::Google | SynthesisBackend::OpenAi => {
