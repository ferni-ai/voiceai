...
/**
 * Context for background task logging
 */
export interface BackgroundTaskContext {
  /** Name of the task (required) */
  task: string;
  /** Additional context to log on failure */
  [key: string]: unknown;
}
 ...
 * ```
 */
export function runBackgroundWithTimeout<T>(
  promise: Promise<T>,
  timeoutMs: number,
  ...
  promise
    .then(() => {
      ...
      if (timedOut) {
        getLogger().debug(
          { ...context, timeoutMs },
          `Background task completed after timeout: ${context.task}`
        );
      }
    })
    .catch((err) => {
