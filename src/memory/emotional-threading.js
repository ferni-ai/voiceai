...
export class EmotionalThreading {
    config;
    userData = new Map();
    constructor(config) {
    ...
    async recordSessionEnd(context) {
        const { userId, sessionId, dominantEmotion, endState, unresolvedTopics, intensity = 0.5, } = context;
    ...
    async getSessionContext(userId) {
        ...
        const suggestedApproach = this.determineSuggestedApproach(data.lastSessionEndState, activeThreads, trajectory);
        return {
     ...
     */
    async updateThread(userId, threadId, update) {
        const data = this.userData.get(userId);
        if (!data)
            return;
     ...
     */
    async resolveThread(userId, threadId, resolution) {
        const data = this.userData.get(userId);
        if (!data)
            return;
        const thread = data.threads.find((t) => t.id === threadId);
     ...
     */
    async createOrUpdateThread(userId, context) {
        const data = this.getOrCreateUserData(userId);
        const now = new Date();
        // Check if thread exists for this topic
        const existingThread = data.threads.find((t) => t.topic.toLowerCase() === context.topic.toLowerCase() && t.status !== 'resolved');
        if (existingThread) {
     ...
     */
    determineSuggestedApproach(lastEndState, activeThreads, trajectory) {
        // Determine opening tone
        let openingTone = 'warm';
        ...
        let checkInPriority = 'connection';
...
// SINGLETON
// ============================================================================
let defaultThreading = null;
export function getEmotionalThreading() {
    if (!defaultThreading) {
        defaultThreading = new EmotionalThreading();
    }
    return defaultThreading;
}
export function resetEmotionalThreading() {
...
//# sourceMappingURL=emotional-threading.js.map