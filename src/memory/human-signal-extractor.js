...
const DATE_PATTERNS = [
    // Birthdays
    { pattern: /my birthday is (on )?(\w+ \d+|\d+\/\d+)/i, type: 'birthday' },
    { pattern: /(?:i|we) (?:turn|turned) (\d+) (?:on|in) (\w+)/i, type: 'birthday' },
    { pattern: /born (?:on |in )(\w+ \d+|\d+\/\d+)/i, type: 'birthday' },
    // Anniversaries
    { pattern: /(?:our|my) anniversary is (on )?(\w+ \d+|\d+\/\d+)/i, type: 'anniversary' },
    { pattern: /(?:married|got married) (?:on |in )(\w+ \d+|\d{4})/i, type: 'anniversary' },
    { pattern: /(\d+) years? (?:married|together)/i, type: 'anniversary' },
    // Loss anniversaries
    ...
    // Milestones
    { pattern: /(\d+) years? sober/i, type: 'milestone' },
    {
        pattern: /quit (?:smoking|drinking) (\d+) (?:years?|months?) ago/i,
        type: 'milestone',
    },
    { pattern: /started (?:.*) (\d+) (?:years?|months?) ago/i, type: 'milestone' },
];
/**
 * Extract important dates from conversation
 */
function extractDates(turns, context) {
    const dates = [];
    const now = new Date();
            ...
            if (match) {
                ...
                if (parsed) {
                    dates.push({
                        ...
                        sentiment: type === 'loss_anniversary'
                            ? 'sensitive'
                            : type === 'birthday'
                                ? 'celebratory'
                                : 'neutral',
...
function isLikelyDateText(text) {
    ...
    if (monthNames.some((m) => lower.includes(m)))
        return true;
...
const DREAM_PATTERNS = [
    { pattern: /(?:i|we) (?:dream|hope|wish) (?:of|to) (.+)/i, category: 'other' },
    { pattern: /someday (?:i|we) (?:want to|hope to) (.+)/i, category: 'other' },
    { pattern: /(?:i|we) always wanted to (.+)/i, category: 'other' },
    { pattern: /my dream (?:job|career) (?:is|would be) (.+)/i, category: 'career' },
    { pattern: /(?:i|we) (?:want to|hope to) travel to (.+)/i, category: 'travel' },
    { pattern: /(?:i|we) (?:want to|hope to) learn (.+)/i, category: 'learning' },
    { pattern: /(?:i|we) (?:want to|hope to) write (.+)/i, category: 'creative' },
    { pattern: /(?:i|we) (?:want to|hope to) start (.+)/i, category: 'career' },
];
/**
 * Extract dreams from conversation
 */
function extractDreams(turns) {
    const dreams = [];
    const now = new Date();
    for (const turn of turns) {
        if (turn.role !== 'user')
 ...
 */
function extractStressTriggers(turns) {
    const triggers = [];
    const now = new Date();
    for (const turn of turns) {
        if (turn.role !== 'user')
 ...
 */
function extractGrowthMarkers(turns) {
    const markers = [];
    const now = new Date();
    for (const turn of turns) {
        if (turn.role !== 'user')
 ...
 */
function extractChallenges(turns) {
    const challenges = [];
    const now = new Date();
    for (const turn of turns) {
        if (turn.role !== 'user')
...
const COMFORT_PATTERNS = [
    { pattern: /(?:it helps|helps me) (?:when|to) (.+)/i, type: 'validation' },
    { pattern: /(?:i|we) (?:feel better|calm down) when (.+)/i, type: 'validation' },
    {
        pattern: /(?:just|i just) (?:need|want) someone to (?:listen|hear)/i,
        type: 'presence',
    },
    { pattern: /(?:talking|venting) (?:helps|makes me feel)/i, type: 'validation' },
    {
        pattern: /(?:i|we) (?:need|want) (?:a solution|to fix|to solve)/i,
        type: 'problem_solving',
    },
    { pattern: /(?:make|makes) me laugh/i, type: 'humor' },
    {
 ...
 */
function extractComfortPatterns(turns, context) {
    const patterns = [];
    const now = new Date();
    for (const turn of turns) {
        if (turn.role !== 'user')
 ...
 */
function detectInsideJokePotential(turns) {
    const jokes = [];
    const now = new Date();
    ...
    for (let i = 0; i < turns.length - 1; i++) {
        ...
        if (turn.role === 'user' && laughIndicators.test(turn.content)) {
            ...
            if (callbackIndicators.test(turn.content) ||
 ...
 */
function detectAvoidances(turns) {
    const avoidances = [];
    const now = new Date();
 ...
 */
export function extractHumanSignals(turns, context) {
 ...
 */
function isDuplicateTrigger(a, b) {
 ...
 */
export function mergeSignalsIntoMemory(existing, extracted) {
    const now = new Date();
    return {
        // Dedupe dates by month/day/type
        importantDates: dedupeArray(existing?.importantDates || [], extracted.importantDates, isDuplicateDate),
        // Dedupe inside jokes by reference text similarity
        insideJokes: [
            ...(existing?.insideJokes || []),
            ...extracted.insideJokes.filter((newJoke) => !(existing?.insideJokes || []).some((e) => e.reference.toLowerCase() === newJoke.reference.toLowerCase())),
        ],
        ...
        emotionalSignature: {
            ...
            comfortPatterns: dedupeArray(existing?.emotionalSignature?.comfortPatterns || [], extracted.comfortPatterns, isDuplicateComfort),
            tells: existing?.emotionalSignature?.tells || [],
            // Dedupe stress triggers by category + trigger text
            stressTriggers: dedupeArray(existing?.emotionalSignature?.stressTriggers || [], extracted.stressTriggers, isDuplicateTrigger),
            updatedAt: now,
        },
        growthArc: {
            // Growth markers are unique events, but dedupe by description similarity
            markers: dedupeArray(existing?.growthArc?.markers || [], extracted.growthMarkers, (a, b) => a.description.toLowerCase().includes(b.description.toLowerCase().slice(0, 50))),
            // Challenges dedupe by challenge text
            challenges: dedupeArray(existing?.growthArc?.challenges || [], extracted.challenges, (a, b) => a.challenge.toLowerCase().includes(b.challenge.toLowerCase().slice(0, 30))),
            updatedAt: now,
        },
        unspoken: {
 ...
 */
function mergeAvoidances(existing, newAvoidances) {
    const result = [...existing];
    for (const newAvoid of newAvoidances) {
        const existingIdx = result.findIndex((e) => e.topic.toLowerCase() === newAvoid.topic.toLowerCase());
        if (existingIdx >= 0) {
...
//# sourceMappingURL=human-signal-extractor.js.map