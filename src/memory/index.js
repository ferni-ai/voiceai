...
// Embeddings
export { cosineSimilarity, embed, embedBatch, euclideanDistance, findTopK, getEmbeddingProvider, GoogleEmbeddings, LocalEmbeddings, OpenAIEmbeddings, setEmbeddingProvider, VertexAIEmbeddings, } from './embeddings.js';
...
export { isVectorStore, } from './vector-store-interface.js';
...
// Semantic RAG
export { formatRAGContext, getRAGContext, hybridSearch, indexAllPersonaContent, indexConversationSummary, indexPersonaContent, ragLookup, semanticSearch, setActiveVectorStore, } from './semantic-rag.js';
// User Memory Indexer (comprehensive user data vectorization)
export { batchIndexUserMemories, getUserMemoryStats, indexUserMemories, removeUserMemories, } from './user-memory-indexer.js';
...
// Summarization
export { extractFollowUpItems, extractOpenQuestions, generateRollingSummary, summarizeConversation, summarizeWithLLM, } from './summarizer.js';
// History tracking
export { ConversationHistoryTracker, getActiveSessionIds, getHistoryTracker, removeHistoryTracker, } from './history.js';
// Key Moment Retrieval
export { clearCurrentSessionMomentsGetter, getKeyMomentRetrieval, KeyMomentRetrieval, setCurrentSessionMomentsGetter, } from './key-moment-retrieval.js';
// Advanced Memory Retrieval (semantic, temporal, emotional scoring)
export { buildMemoryIndex, clearMemoryIndex, computeMemoryEmbeddings, getConversationPrimingMemories, getIndexStats, getPersonRelatedMemories, retrieveMemories, searchMemoriesByTopic, } from './advanced-retrieval.js';
// ============================================================================
// NEW v2 MODULES
// ============================================================================
// Result Type Pattern (error handling)
export { all, allSettled, andThen, err, isErr, isOk, map, mapError, memoryError, ok, retry, tryAsync, trySync, unwrap, unwrapOr, } from './result.js';
// Embedding Cache (performance)
export { configureEmbeddingCacheMetrics, embedBatchCached, embedCached, EmbeddingCache, getEmbeddingCache, resetEmbeddingCache, } from './embedding-cache.js';
// Semantic Memory Cache (performance - "Better than Human" optimization)
export { configureSemanticCache, findSimilarCached, storeInSemanticCache, withSemanticCache, clearUserSemanticCache, clearAllSemanticCaches, invalidateSemanticCache, getSemanticCacheStats, resetSemanticCacheStats, getUserCacheInfo, } from './semantic-memory-cache.js';
// Predictive Cache Warming (80%+ cache hit rate for anticipated queries)
export { configurePredictiveWarming, configureMemoryRetrieval, setupMemoryFetcher, detectTimeSignals, predictQueries, warmCacheForSession, warmCacheForHandoff, } from './predictive-cache-warming.js';
// Memory Consolidation (long-term memory management)
export { getMemoryConsolidator, MemoryConsolidator, resetMemoryConsolidator, } from './memory-consolidator.js';
// Memory Decay (graceful forgetting)
export { getMemoryDecayManager, MemoryDecayManager, resetMemoryDecayManager, } from './memory-decay.js';
// Retrieval Explanations (natural language reasoning)
export { getRetrievalExplainer, resetRetrievalExplainer, RetrievalExplainer, } from './retrieval-explanations.js';
// Session Priming (cross-session continuity)
export { getSessionPrimer, resetSessionPrimer, SessionPrimer, } from './session-priming.js';
// Memory Deduplication (data quality)
export { getMemoryDeduplicator, MemoryDeduplicator, resetMemoryDeduplicator, } from './memory-deduplication.js';
// LSH Deduplication (O(n) approximate matching)
export { exactJaccardSimilarity, findDuplicatesLSH, isNativeLshAvailable, LSHIndex, } from './lsh-deduplication.js';
// ============================================================================
// RUST-ACCELERATED OPERATIONS (SIMD-optimized for batch processing)
// ============================================================================
export { 
// Euclidean distance (SIMD-accelerated for batches)
batchEuclideanDistance, batchEuclideanDistanceF32, euclideanDistanceF32, 
// Vector normalization (SIMD-accelerated)
normalizeVector, normalizeVectorF32, batchNormalizeVectorsF32, vectorNormF32, 
// Centroid computation (SIMD-accelerated)
computeCentroidF32, 
// Cosine similarity (already Rust-accelerated)
batchCosineSimilarity, 
// Native module availability check
isRustAvailable, } from './rust-accelerator.js';
// Memory Metrics (observability)
export { checkMemoryHealthAlerts, collectMemoryMetrics, getMemoryMetricsCollector, MemoryMetricsCollector, resetMemoryMetricsCollector, } from './memory-metrics.js';
// Tiered Memory Storage (Phase 2.3 optimization - 10x faster hot data retrieval)
export { clearAccessRecords, getFromHotTier, getMemoriesTiered, getMemoryTiered, getTieredMemoryConfig, getTieredMemoryStats, getUserAccessRecords, recordMemoryAccess, removeFromHotTier, resetTieredMemoryStats, runDemotionCheck, setTieredMemoryConfig, storeInHotTier, } from './tiered-memory-storage.js';
// ============================================================================
// UNIFIED EMOTIONAL MEMORY (coordinates user emotions + bonding)
// ============================================================================
export { areEmotionalMemoryEnginesConfigured, clearAllUnifiedEmotionalMemories, clearUnifiedEmotionalMemory, configureEmotionalMemoryEngines, getUnifiedEmotionalMemory, UnifiedEmotionalMemory, } from './emotional-memory-unified.js';
...
// Session state
saveSessionState, getSessionState, getRecentSessions, 
// Tool execution logs
logToolExecution, getToolExecutions, 
// Persona bonds
savePersonaBond, getPersonaBond, getAllPersonaBonds, 
// Voice profile
saveVoiceProfile, getVoiceProfile, 
// User intents
logUserIntent, getRecentIntents, 
// Superhuman cache
setCachedInsight, getCachedInsight, 
// Quality metrics
saveQualityMetrics, getQualityMetrics, 
// GDPR
deleteAllExtendedUserData, } from './firestore-extended-persistence.js';
...
export { 
// Storage
createEntity, getEntity, updateEntity, deleteEntity, findEntityByAlias, searchEntities, getAllEntities, getEntitiesByType, createMention, getMentionsForEntity, getRecentMentions, upsertRelationship, getRelationshipsForEntity, recordMention, hasEntityStore, getEntityStoreStats, 
// Resolver
resolvePerson, mergeEntities, whatDoWeKnowAbout, 
// Integration
isEntityStoreReady, initializeEntityStore, capturePersonEntity, captureMultiplePeople, findContactForTelephony, getAllContacts, getEntityStoreHealth, 
// Migration
migrateUser, migrateAllUsers, } from './entity-store/index.js';
// ============================================================================
...
export function detectStoreType() {
    // Explicit override
    const explicit = process.env.MEMORY_STORE_TYPE;
 ...
 */
export async function createStore(type, skipInit = false) {
 ...
 */
export async function rehydrateConversationEmbeddings(_store, _vectorStore) {
    getLogger().warn('⚠️ rehydrateConversationEmbeddings() is DEPRECATED and does nothing. ' +
 ...
 */
function validateEmbeddingDimensions(usePersistentVectors) {
    try {
        ...
        if (providerDimensions !== vectorStoreDimensions) {
            getLogger().warn({
                ...
                recommendation: providerDimensions === 1536
            ...
            getLogger().debug({ providerModel, dimensions: providerDimensions }, 'Embedding dimensions validated successfully');
        }
    }
    catch (error) {
...
async function doInitializeMemorySystem(config) {
    ...
    const usePersistentVectors = config?.usePersistentVectors ??
    ...
    if (config?.indexPersona !== false) {
        try {
            ...
            getLogger().warn(`Failed to index persona content (embeddings may not be available): ${error}`);
        }
    }
    ...
    try {
        ...
        getLogger().warn({ error: String(error) }, '⚠️ Entity store initialization failed (legacy memory will be used)');
    }
    getLogger().info(`Memory system initialized (store: ${storeType}, vectors: ${usePersistentVectors ? 'persistent' : 'ephemeral'}, redis: ${!!redisCache})`);
 ...
 */
export async function getMemorySystemHealth() {
