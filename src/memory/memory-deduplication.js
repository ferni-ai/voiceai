...
import { err, isOk, memoryError, ok } from './result.js';
...
export class MemoryDeduplicator {
    ...
    async checkDuplicate(newMemory, existingMemories) {
        ...
        try {
            ...
            if (maxSimilarity >= this.config.exactDuplicateThreshold) {
                ...
                log.debug({ similarity: maxSimilarity, threshold: this.config.exactDuplicateThreshold }, 'Exact duplicate detected');
            }
                ...
                if (this.shouldMerge(newMemory, mostSimilar)) {
                    ...
                    this.stats.duplicatesFound++;
                    log.debug({ similarity: maxSimilarity, threshold: this.config.mergeThreshold }, 'Merge candidate detected');
                }
            }
            const result = {
                isDuplicate,
            ...
            return err(memoryError('deduplication_failed', `Duplicate check failed: ${error}`, {
    ...
    async mergeMemories(existing, incoming) {
        try {
            ...
            const mergedTimestamp = new Date(Math.min(existing.timestamp.getTime(), incoming.timestamp.getTime()));
            ...
            return err(memoryError('consolidation_failed', `Memory merge failed: ${error}`, {
                cause: error instanceof Error ? error : undefined,
            }));
        }
    }
     ...
     */
    async findDuplicateClusters(memories) {
        ...
        log.info({ totalMemories: memories.length, clusters: clusters.size }, 'Found duplicate clusters');
        return clusters;
    }
    /**
     ...
     */
    shouldMerge(newMemory, existing) {
        // Check age difference
        const newTimestamp = this.getTimestamp(newMemory);
        const existingTimestamp = this.getTimestamp(existing);
        if (newTimestamp && existingTimestamp) {
            const ageDiffDays = Math.abs((newTimestamp.getTime() - existingTimestamp.getTime()) / (1000 * 60 * 60 * 24));
        ...
        if (!this.config.allowCrossTopicMerge) {
            ...
            if (newTopics.length > 0 && existingTopics.length > 0) {
                const hasOverlap = newTopics.some((t) => existingTopics.some((et) => t.toLowerCase().includes(et.toLowerCase()) ||
     ...
     */
    extractNewDetails(existingContent, newContent) {
        const newDetails = [];
        // Split into sentences
        const existingSentences = new Set(existingContent
            .split(/[.!?]+/)
...
// SINGLETON
// ============================================================================
let defaultDeduplicator = null;
/**
 * Get the default deduplicator
 */
export function getMemoryDeduplicator(config) {
 ...
 */
export function resetMemoryDeduplicator() {
...
//# sourceMappingURL=memory-deduplication.js.map