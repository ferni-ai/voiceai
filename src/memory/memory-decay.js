...
export class MemoryDecayManager {
    ...
    calculateStrength(memory) {
        const now = Date.now();
        const lastAccessed = 'lastAccessed' in memory ? memory.lastAccessed.getTime() : memory.timestamp.getTime();
        ...
        const reactivationFactor = 'reactivationCount' in memory ? 1 + memory.reactivationCount * 0.2 : 1;
     ...
     */
    simulateDecay(memory, daysToSimulate, stepDays = 7) {
        const results = [];
     ...
     */
    applyDecay(memory) {
        const baseImportance = typeof memory.initialStrength === 'number' ? this.clamp01(memory.initialStrength) : 0.5;
        const item = {
            id: memory.id,
...
// SINGLETON
// ============================================================================
let defaultDecayManager = null;
/**
 * Get the default decay manager
 */
export function getMemoryDecayManager(config) {
 ...
 */
export function resetMemoryDecayManager() {
