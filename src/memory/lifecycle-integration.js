 ...
 */
export async function saveMemory(userId, memory, strength = 1.0) {
    const db = getFirestoreDb();
    if (!db) {
 ...
 */
export async function updateMemoryStrength(userId, memoryId, strength, archived = false) {
    const db = getFirestoreDb();
    if (!db)
        return false;
    try {
 ...
 */
export async function reinforceMemory(userId, memoryId, boostFactor = 1.5) {
    const db = getFirestoreDb();
    if (!db) {
    ...
    try {
        ...
        log.debug({ userId, memoryId, previousStrength, newStrength, reactivationCount }, 'Memory reinforced');
 ...
 */
export async function saveConsolidatedMemory(userId, consolidated) {
    const db = getFirestoreDb();
    if (!db)
        return false;
    try {
        ...
        log.debug({ userId, consolidatedId: consolidated.id, sourceCount: consolidated.sourceMemoryIds.length }, 'Saved consolidated memory');
        return true;
    }
    catch (error) {
 ...
 */
export async function runLifecycleMaintenance(userId) {
    ...
    try {
        ...
        const strongMemories = decayingMemories.filter((m) => decayManager.calculateStrength(m).currentStrength > 0.6);
 ...
 */
export async function createLinksForNewMemory(userId, newMemory) {
    try {
        ...
        const graph = getMemoryGraph();
        const links = await graph.detectLinks(userId, newMemory.id, newMemory.content, existingForLinks);
        log.debug({ userId, memoryId: newMemory.id, linksCreated: links.length }, 'Created links for new memory');
        return links;
    }
    catch (error) {
...
// HELPERS
// ============================================================================
function convertToMemoryItem(doc) {
    return {
        id: doc.id,
        type: doc.type,
        content: doc.content,
...
function convertConsolidatedToMemoryItem(consolidated) {
    return {
        id: consolidated.id,
        type: 'topic',
