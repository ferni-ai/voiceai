...
export async function loadUserPreferences(userId) {
    ...
    try {
        ...
        const preferences = data.preferences.map((p) => ({
            ...p,
            lastUpdated: p.lastUpdated instanceof Date
                ? p.lastUpdated
                : new Date(p.lastUpdated?.toDate?.() || Date.now()),
            evidence: (p.evidence || []).map((e) => ({
                ...e,
                timestamp: e.timestamp instanceof Date
                    ? e.timestamp
                    : new Date(e.timestamp?.toDate?.() || Date.now()),
            })),
        }));
 ...
 */
function analyzeResponseSentiment(response) {
    ...
    if (response.length < 20)
        score -= 0.1; // Very short might be disengagement
    // Determine signal
    let signal;
    if (score > 0.2)
        signal = 'positive';
...
export class CommunicationPreferences {
    config;
    preferences = new Map(); // userId -> preferences (in-memory cache)
    loadingPromises = new Map(); // Prevent duplicate loads
    constructor(config) {
     ...
     */
    async getApproachGuidance(userId, context) {
     ...
     */
    determineOverallApproach(prefs, context) {
        const prefMap = new Map(prefs.map((p) => [p.dimension, p]));
        // Context overrides
        if (context.emotion) {
            const emotionLower = context.emotion.toLowerCase();
...
export function resetCommunicationPreferences() {
...
//# sourceMappingURL=communication-preferences.js.map