...
export class KeyMomentRetrieval {
     ...
     */
    async findRelevantMoments(profile, context) {
     ...
     */
    calculateRelevance(moment, context) {
        let score = 0;
        ...
        const daysSince = Math.floor((Date.now() - new Date(moment.timestamp).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSince < 7) {
            score += 0.2;
        }
     ...
     */
    matchEmotionToMomentType(emotion, momentType) {
        const emotionMomentMap = {
     ...
     */
    generateMomentReference(match, userName) {
        const { moment } = match;
     ...
     */
    shouldReferenceKeyMoment(turnCount) {
 ...
 */
export function getKeyMomentRetrieval() {
    if (!retrieval) {
 ...
 * Reset for testing
 */
export function resetKeyMomentRetrieval() {
    retrieval = null;
}
export default KeyMomentRetrieval;
//# sourceMappingURL=key-moment-retrieval.js.map