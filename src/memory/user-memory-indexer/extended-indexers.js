 ...
 */
export async function indexVoiceJournals(userId, journals, store) {
    let indexed = 0;
    for (const journal of journals) {
        // Skip if no transcript (nothing to embed)
        if (!journal.transcript?.trim())
            continue;
        const text = `Voice journal entry${journal.mood ? ` (mood: ${journal.mood})` : ''}: ${journal.transcript}. ${journal.themes?.length ? `Themes: ${journal.themes.join(', ')}. ` : ''}${journal.keyInsights?.length ? `Insights: ${journal.keyInsights.join('. ')}` : ''}`;
        const doc = {
 ...
 */
export async function indexCustomAgents(userId, agents, store) {
    let indexed = 0;
 ...
 */
async function indexAgentMemories(userId, agentId, memories, store) {
    let indexed = 0;
    // Index stories
    ...
    for (const moment of memories.sharedMoments || []) {
        const text = `Shared moment with agent: ${moment.description}. Emotion: ${moment.emotion}. ${moment.whatTheySaid ? `They said: "${moment.whatTheySaid}". ` : ''}${moment.whatILearned ? `I learned: ${moment.whatILearned}` : ''}`;
        const doc = {
 ...
 */
export async function indexContactNotes(userId, contacts, store) {
    let indexed = 0;
 ...
 */
export async function indexHabits(userId, habits, store) {
    let indexed = 0;
...
//# sourceMappingURL=extended-indexers.js.map