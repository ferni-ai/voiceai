 ...
 */
export async function indexImportantDates(userId, dates, store) {
    let indexed = 0;
    for (const date of dates) {
        const text = `Important date: ${date.label}. Type: ${date.type}. ${date.relatedPerson ? `Related to ${date.relatedPerson}. ` : ''}Occurs ${date.month}/${date.day}${date.year ? `/${date.year}` : ' annually'}. ${date.notes || ''}`;
        const doc = {
 ...
 */
export async function indexInsideJokes(userId, jokes, store) {
    let indexed = 0;
 ...
 */
export async function indexRunningThemes(userId, themes, store) {
    let indexed = 0;
    for (const theme of themes) {
        const keyMomentsSummary = theme.keyMoments
            ...
            .map((m) => m.summary)
            .join('; ');
        const text = `Running theme: ${theme.theme}. Comes up ${theme.frequency}. Sentiment: ${theme.sentiment}. ${keyMomentsSummary ? `Key moments: ${keyMomentsSummary}` : ''}`;
        const doc = {
 ...
 * Index core values
 */
export async function indexValues(userId, values, store) {
    let indexed = 0;
 ...
 */
export async function indexDreams(userId, dreams, store) {
    let indexed = 0;
 ...
 */
export async function indexFears(userId, fears, store) {
    let indexed = 0;
    for (const fear of fears) {
        const text = `Fear/Worry: ${fear.fear}. Frequency: ${fear.frequency}. ${fear.copingMechanisms?.length ? `Coping: ${fear.copingMechanisms.join(', ')}` : ''}`;
        const doc = {
 ...
 */
export async function indexGrowthMarkers(userId, markers, store) {
    let indexed = 0;
    for (const marker of markers) {
        const text = `Growth: ${marker.description}. Before: ${marker.before}. After: ${marker.after}. ${marker.acknowledged
 ...
 */
export async function indexChallenges(userId, challenges, store) {
    let indexed = 0;
    for (const challenge of challenges) {
        const milestonesSummary = challenge.milestones
            ...
            .map((m) => m.description)
            .join('; ');
        const text = `Challenge: ${challenge.challenge}. Status: ${challenge.status}. ${milestonesSummary ? `Progress: ${milestonesSummary}` : ''}`;
        const doc = {
 ...
 */
export async function indexAvoidances(userId, avoidances, store) {
    let indexed = 0;
    for (const avoidance of avoidances) {
        const text = `Avoids topic: ${avoidance.topic}. Style: ${avoidance.avoidanceStyle}. Observed ${avoidance.observations} times. ${avoidance.possibleReason ? `Possible reason: ${avoidance.possibleReason}` : ''}. Approach: ${avoidance.approach}`;
        const doc = {
 ...
 */
export async function indexTemporalPatterns(userId, patterns, store) {
    let indexed = 0;
    for (const pattern of patterns) {
        const text = `Seasonal pattern: ${pattern.pattern}. Timing: ${pattern.timing}${pattern.customTiming ? ` (${pattern.customTiming})` : ''}. Emotional tone: ${pattern.emotionalTone}. ${pattern.approach ? `Approach: ${pattern.approach}` : ''}`;
        const doc = {
 ...
 */
export async function indexComfortPatterns(userId, patterns, store) {
    let indexed = 0;
    for (const pattern of patterns) {
 ...
 */
export async function indexStressTriggers(userId, triggers, store) {
    let indexed = 0;
    for (const trigger of triggers) {
        const text = `Stress trigger: ${trigger.trigger}. Category: ${trigger.category}. Intensity: ${trigger.intensity}. ${trigger.helpfulResponses?.length ? `Helpful: ${trigger.helpfulResponses.join(', ')}` : ''}`;
        const doc = {
 ...
 */
export async function indexEmotionalTells(userId, tells, store) {
    let indexed = 0;
 ...
 */
export async function indexHumanMemory(userId, humanMemory, store) {
    const counts = {};
    // Important dates
...
//# sourceMappingURL=human-memory-indexers.js.map