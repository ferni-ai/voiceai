...
export class MemoryGraph {
  private linksCache = new Map<string, MemoryLink[]>(); // userId -> links
  ...
  async createLink(
    ...
    log.debug(
      { userId, linkId: link.id, linkType, source: sourceMemoryId, target: targetMemoryId },
      'Memory link created'
    );
   ...
   */
  async getLinks(userId: string): Promise<MemoryLink[]> {
    // Check cache
   ...
   */
  async getLinksForMemory(userId: string, memoryId: string): Promise<MemoryLink[]> {
  ...
  async spreadActivation(
    ...
    const adjacency = new Map<string, MemoryLink[]>();
    ...
    const activated = new Map<string, SpreadingActivationResult>();
    const queue: Array<{ memoryId: string; activation: number; depth: number; path: string[] }> =
      [];
    ...
    while (queue.length > 0) {
      ...
      if (current.activation < opts.minActivation) continue;
      ...
      for (const link of neighbors) {
        ...
        if (existing && existing.activationLevel >= current.activation) continue;

        // Calculate activation with decay and link weight
        const linkWeight = opts.linkTypeWeights?.[link.linkType] ?? 0.5;
        const nextActivation =
          current.activation * (1 - opts.decayPerHop) * linkWeight * link.strength;
   ...
   */
  async recordActivation(userId: string, linkId: string): Promise<void> {
    const links = await this.getLinks(userId);
  ...
  async detectLinks(
    ...
    for (const existing of existingMemories) {
      ...
      for (const pattern of personPatterns) {
        const newMatches: string[] = contentLower.match(pattern) || [];
        const existingMatches: string[] = existingLower.match(pattern) || [];
        const overlap = newMatches.filter((m: string) => existingMatches.includes(m));
  ...
  private describePath(path: string[], lastLinkType: LinkType): string {
    if (path.length === 2) {
  ...
  private async persistLink(userId: string, link: MemoryLink): Promise<void> {
  ...
  private async loadLinks(userId: string): Promise<MemoryLink[]> {
    const db = getFirestoreDb();
    if (!db) {
      return [];
    }
