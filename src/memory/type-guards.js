...
function isDateLike(value) {
    ...
    if (isObject(value) && '_seconds' in value)
        return true;
    return false;
}
/**
 ...
 */
export function isValidUserProfile(data) {
    if (!isObject(data))
        return false;
    // Required fields
    if (!isString(data.id))
    ...
    if ('name' in data && data.name != null && !isString(data.name))
        return false;
    if ('email' in data && data.email != null && !isString(data.email))
        return false;
    if ('phone' in data && data.phone != null && !isString(data.phone))
        return false;
    ...
    for (const field of dateFields) {
        if (field in data && data[field] != null && !isDateLike(data[field]))
            return false;
    }
    return true;
}
 ...
 */
export function isValidConversationSummary(data) {
    if (!isObject(data))
        return false;
    // Required fields
    if (!isString(data.id))
    ...
    if ('mainTopics' in data && !isStringArray(data.mainTopics))
        return false;
    if ('keyPoints' in data && !isStringArray(data.keyPoints))
        return false;
    // String fields
 ...
 */
export function parseConversationSummary(data) {
 ...
 */
export function isValidKeyMoment(data) {
    if (!isObject(data))
        return false;
    // Required fields
    ...
    if ('id' in data && !isString(data.id))
        return false;
    if ('timestamp' in data && !isDateLike(data.timestamp))
        return false;
    if ('importance' in data && !isNumber(data.importance))
        return false;
    return true;
}
/**
 ...
 */
export function parseKeyMoment(data) {
 ...
 */
export function isValidFinancialGoal(data) {
    if (!isObject(data))
        return false;
    // Required fields
    if (!isString(data.id))
 ...
 */
export function parseFinancialGoal(data) {
 ...
 */
export function safeParse(data, validator, hydrateFunc) {
    if (!validator(data)) {
        return null;
    }
    if (hydrateFunc) {
 ...
 */
export function safeParseArray(items, validator, hydrateFunc) {
    return items
