 ...
 */
export function configureSemanticCache(options) {
 ...
 */
export async function findSimilarCached(userId, query) {
 ...
 */
export async function storeInSemanticCache(userId, query, result, embedding) {
    // Track access time for stale user cleanup
    userLastAccess.set(userId, Date.now());
    ...
    if (config.debug) {
        log.debug({ userId, query: query.slice(0, 50), cacheSize: userCache.length }, '💾 Stored in semantic cache');
    }
}
/**
 ...
 */
export async function withSemanticCache(userId, query, queryFn) {
    // Try cache first
    const cached = await findSimilarCached(userId, query);
 ...
 */
function pruneExpiredEntries(userId) {
    const userCache = userCaches.get(userId);
    if (!userCache)
        return;
    const now = Date.now();
    ...
    if (validEntries.length < userCache.length) {
        ...
        if (config.debug) {
            log.debug({ userId, pruned: userCache.length - validEntries.length }, '🧹 Pruned expired cache entries');
        }
    }
}
/**
 ...
 */
export function clearUserSemanticCache(userId) {
    userCaches.delete(userId);
    userLastAccess.delete(userId);
 ...
 */
export function clearAllSemanticCaches() {
    userCaches.clear();
 ...
 */
export function invalidateSemanticCache(userId, pattern) {
    const userCache = userCaches.get(userId);
    if (!userCache)
        return 0;
    if (!pattern) {
 ...
 */
export function getUserCacheInfo(userId) {
    const userCache = userCaches.get(userId);
    if (!userCache || userCache.length === 0) {
...
//# sourceMappingURL=semantic-memory-cache.js.map