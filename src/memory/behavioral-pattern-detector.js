...
export class BehavioralPatternDetectorImpl {
    patterns = new Map(); // userId -> patterns
    /**
     * Analyze conversation turns for behavioral patterns
     */
    async analyzeForPatterns(turns, existingPatterns) {
        ...
        for (const [patternType, accumulator] of accumulators.entries()) {
            ...
            if (accumulator.matches.length >= def.detectPatterns.minMatches) {
                ...
                if (existing) {
                    ...
                    existing.examples.push(...accumulator.matches.slice(0, 3).map((m) => ({
                        context: m.context,
                        behavior: m.behavior,
     ...
     */
    async getActivePatternGuidance(userId, currentContext) {
        ...
        for (const pattern of patterns.sort((a, b) => b.confidence - a.confidence)) {
            ...
            const isActive = def.detectPatterns.indicators.some((indicator) => indicator.test(currentContext));
...
export function getBehavioralPatternDetector() {
    if (!defaultDetector) {
 ...
 */
export async function loadPatternsFromPersistence(userId) {
 ...
 */
export async function savePatternsToPeristence(userId) {
    try {
        const { getFirestoreMemoryPersistence } = await import('./firestore-memory-persistence.js');
        const persistence = await getFirestoreMemoryPersistence();
        if (persistence.isAvailable()) {
...
export function resetBehavioralPatternDetector() {
...
//# sourceMappingURL=behavioral-pattern-detector.js.map