 ...
 */
function detectEmotionalArc(turns) {
    ...
    function detectEmotion(turnSubset) {
 ...
 */
function generateTextSummary(turns, topics, keyPoints) {
 ...
 */
export async function summarizeConversation(sessionId, turns, options) {
    const now = new Date();
    const opts = {
        maxLength: 500,
    ...
    if (turns.length > 0 && turns[0].timestamp && turns[turns.length - 1].timestamp) {
        duration = Math.floor((turns[turns.length - 1].timestamp.getTime() - turns[0].timestamp.getTime()) / 1000);
    }
    else {
 ...
 */
export async function summarizeWithLLM(sessionId, turns, llmCall, options) {
    const now = new Date();
    const opts = {
        maxLength: 500,
        includeEmotionalArc: true,
 ...
 */
export async function generateRollingSummary(turns, previousSummary) {
    // Get last N turns
 ...
 */
export function extractOpenQuestions(turns) {
    const openQuestions = [];
 ...
 */
export function extractFollowUpItems(turns) {
