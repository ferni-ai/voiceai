...
export class SpreadingActivationEngine {
     ...
     */
    async spreadFromMemory(userId, sourceMemoryId, initialActivation = 1.0) {
        ...
        while (queue.length > 0) {
            ...
            if (current.activation < this.config.minActivation)
                continue;
            // Skip if too deep
            if (current.distance > this.config.maxDepth)
                continue;
            ...
            for (const { targetId, linkType, strength } of neighbors) {
                ...
                const spreadActivation = current.activation * this.config.decayFactor * linkWeight * strength;
     ...
     */
    async spreadFromMultiple(userId, sourceMemoryIds, weights) {
     ...
     */
    async activateByTopic(userId, seedMemoryIds, topK = 10) {
     ...
     * (Reverse spreading)
     */
    async findActivators(userId, targetMemoryId, maxSources = 5) {
        const memoryGraph = getMemoryGraph();
        const allLinks = await memoryGraph.getLinks(userId);
    ...
    buildAdjacencyMap(links) {
        ...
        for (const link of links) {
            ...
            const bidirectionalTypes = ['narrative', 'emotion', 'topic'];
    ...
    buildReverseAdjacencyMap(links) {
        const adj = new Map();
        for (const link of links) {
...
// SINGLETON
// ============================================================================
let spreadingActivationInstance = null;
...
export function resetSpreadingActivation() {
