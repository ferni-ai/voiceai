...
// ============================================================================
const memoryIndex = new Map(); // userId -> memories
// ============================================================================
// MEMORY ITEM BUILDERS (helpers for buildMemoryIndex)
// ============================================================================
function buildFollowUpMemories(userId, followUps) {
...
function buildFamilyMemories(userId, profile) {
...
function buildTopicMemories(userId, profile) {
...
function buildThreadMemories(userId, threads) {
...
function getEmotionalWeight(significance) {
    switch (significance) {
        case 'life_changing':
            return 1.0;
        case 'major':
...
function buildLifeEventMemories(userId, events) {
 ...
 */
export function buildMemoryIndex(userId, profile) {
    const memories = [];
 ...
 */
export async function retrieveMemories(userId, context, config = {}) {
    ...
    const cacheResult = await findSimilarCached(userId, context.query);
    ...
    for (const memory of memories) {
        ...
        const halfLife = cfg.temporalDecayHalfLifeDays * (1 + memory.emotionalWeight * cfg.emotionalDecayResistance);
        ...
        if (recentTopics && recentTopics.length > 0 && memoryTopics && memoryTopics.length > 0) {
            const overlap = memoryTopics.filter((t) => recentTopics.some((rt) => rt.toLowerCase().includes(t.toLowerCase()) || t.toLowerCase().includes(rt.toLowerCase()))).length;
            contextScore += overlap * 0.3;
        }
        // Current topic match
        const { currentTopic } = context;
        if (currentTopic && memoryTopics && memoryTopics.length > 0) {
            if (memoryTopics.some((t) => t.toLowerCase().includes(currentTopic.toLowerCase()) ||
        ...
        let score = breakdown.semantic * cfg.semanticWeight +
 ...
 */
export function getConversationPrimingMemories(userId, _personaId, options = {}) {
    const memories = memoryIndex.get(userId);
    if (!memories)
        return [];
    const { maxMemories = 5, includeCommitments = true, includeRecentTopics = true, sessionCount = 0, } = options;
    const result = [];
 ...
 */
export function getPersonRelatedMemories(userId, personName) {
    const memories = memoryIndex.get(userId);
    if (!memories)
        return [];
    const personLower = personName.toLowerCase();
    return memories
        .filter((m) => m.personMentioned?.toLowerCase() === personLower ||
 ...
 */
export async function searchMemoriesByTopic(userId, topic) {
 ...
 */
export async function computeMemoryEmbeddings(userId) {
    const memories = memoryIndex.get(userId);
    if (!memories)
        return 0;
    let computed = 0;
 ...
 */
export function clearMemoryIndex(userId) {
 ...
 */
function summaryToMemoryItem(summary) {
    const content = [
    ...
    function getEmotionalWeightFromArc(arc) {
        if (!arc)
            return 0.4;
        if (arc.includes('heavy'))
            return 0.8;
        if (arc.includes('vulnerable'))
            return 0.7;
        if (arc.includes('emotional'))
            return 0.6;
        return 0.4;
    }
 ...
 */
function momentToMemoryItem(moment) {
    const emotionalWeightMap = {
        light: 0.3,
        medium: 0.6,
        heavy: 0.9,
    };
 ...
 */
function generateRetrievalReason(memory, breakdown, _context) {
    const reasons = [];
...
//# sourceMappingURL=advanced-retrieval.js.map