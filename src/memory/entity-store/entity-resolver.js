...
import { createEntity, getEntity, updateEntity, findEntityByAlias, searchEntities, getAllEntities, } from './storage.js';
 ...
 */
function isPersonAttributes(attrs) {
 ...
 */
function getSpecificRelation(entity) {
    if (isPersonAttributes(entity.attributes)) {
 ...
 */
function getRelationshipCategory(entity) {
    if (isPersonAttributes(entity.attributes)) {
 ...
 */
function normalizeRelationship(term) {
 ...
 * 5. Create new entity
 */
export async function resolvePerson(userId, input) {
    ...
    if (name) {
        ...
        if (exactMatch) {
            ...
            if (phone || email) {
                const currentAttrs = exactMatch.attributes;
    ...
    if (relationship) {
        ...
        if (normalizedRel?.specific) {
            ...
            if (existingByRelation) {
                log.debug({ entityId: existingByRelation.id, relation: normalizedRel.specific }, 'Found by relationship');
                ...
                if (phone || email) {
                    const currentAttrs = existingByRelation.attributes;
                    updates.attributes = {
                        ...currentAttrs,
                        phone: phone || currentAttrs.phone,
                        email: email || currentAttrs.email,
    ...
    log.info({ entityId: newEntity.id, name: newEntity.canonicalName, relationship: getSpecificRelation(newEntity) }, '✨ Created new person entity');
    return {
        entity: newEntity,
 ...
 */
function buildAliases(name, relationship) {
    const aliases = [];
    if (name) {
 ...
 */
async function findBySpecificRelation(userId, specificRelation) {
 ...
 */
export async function mergeEntities(userId, primaryEntityId, secondaryEntityIds) {
    ...
    const maxEmotionalWeight = Math.max(primary.emotionalWeight, ...secondaries.map((s) => s.emotionalWeight));
    // Merge source conversations
    const allSourceConversations = new Set(primary.sourceConversations);
    for (const secondary of secondaries) {
        secondary.sourceConversations?.forEach((c) => allSourceConversations.add(c));
    }
 ...
 */
export async function whatDoWeKnowAbout(userId, query) {
    ...
    for (const rel of relationships) {
        if (rel.fromEntity !== entity.id)
            relatedEntityIds.add(rel.fromEntity);
        if (rel.toEntity !== entity.id)
            relatedEntityIds.add(rel.toEntity);
    }
 ...
 */
async function resolveMentionImpl(userId, mention) {
    const input = mention;
    ...
    log.debug({ userId, input: nameOrText, entityId: result.entity.id, confidence: result.confidence }, 'Resolved mention to entity');
    return result.entity;
}
/**
 * Add a relationship between two entities - FULL IMPLEMENTATION
 */
async function addRelationshipImpl(userId, fromEntityId, toEntityId, relationshipType) {
    ...
    log.info({ userId, fromEntityId, toEntityId, relationshipType }, 'Added relationship between entities');
}
/**
 * Map string to EdgeType (handles various relationship strings)
 */
function mapToEdgeType(rel) {
    const mapping = {
        // Person-to-person
        family: 'family_of',
        family_of: 'family_of',
 ...
 */
async function resolveImpl(userId, query) {
    ...
    if (queryObj.name) {
        return findEntityByAlias(userId, queryObj.name, queryObj.type);
    }
    return null;
}
/**
 ...
 */
async function getEntityImpl(userId, entityId) {
    return getEntity(userId, entityId);
}
/**
 * Get entities by type - FULL IMPLEMENTATION
 */
async function getEntitiesByTypeImpl(userId, type, limit = 200) {
    return getAllEntities(userId, { types: [type], topK: limit });
}
/**
 * Get the entity resolver singleton
 */
export function getEntityResolver() {
