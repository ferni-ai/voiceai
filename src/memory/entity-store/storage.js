...
import { getCachedEntityList, cacheEntityList, getCachedEntity, cacheEntity, getCachedEntityIdByAlias, cacheAliasMapping, invalidateUserCache, invalidateEntity, recordCacheHit, recordCacheMiss, } from './entity-cache.js';
 ...
 */
async function getEntitiesRef(userId) {
 ...
 * Create a new entity
 */
export async function createEntity(userId, entity) {
    ...
    log.info({ userId, entityId: fullEntity.id, name: fullEntity.canonicalName, type: fullEntity.type }, '✨ Created entity');
    return fullEntity;
}
/**
 * Get an entity by ID
 ...
 * Update an entity
 */
export async function updateEntity(userId, entityId, updates) {
    const ref = await getEntitiesRef(userId);
 ...
 */
export async function findEntityByAlias(userId, alias, type) {
    // Check cache first
 ...
 */
export async function searchEntities(userId, searchText, options = {}) {
    const ref = await getEntitiesRef(userId);
 ...
 */
export async function getAllEntities(userId, options = {}) {
    // Check cache first
 ...
 */
export async function getEntitiesByType(userId, type, limit = 50) {
    const ref = await getEntitiesRef(userId);
 ...
 */
async function getMentionsRef(userId) {
    const db = await getFirestore();
 ...
 * Create a mention
 */
export async function createMention(userId, mention) {
 ...
 */
export async function getMentionsForEntity(userId, entityId, limit = 50) {
    const ref = await getMentionsRef(userId);
    const snapshot = await ref
 ...
 */
export async function getRecentMentions(userId, limit = 20) {
    const ref = await getMentionsRef(userId);
 ...
 */
async function getRelationshipsRef(userId) {
    const db = await getFirestore();
 ...
 */
export async function upsertRelationship(userId, relationship) {
    ...
    if (!existing.empty) {
        ...
        await docRef.update(cleanForFirestore({
            ...
            reinforcementCount: (existing.docs[0].data().reinforcementCount || 0) + 1,
        }));
        return { ...existing.docs[0].data(), id: existing.docs[0].id };
    }
    // Create new
    const docRef = ref.doc();
    const now = new Date();
 ...
 */
export async function getRelationshipsForEntity(userId, entityId) {
    const ref = await getRelationshipsRef(userId);
    ...
    for (const doc of fromSnapshot.docs) {
        relationships.push({ ...doc.data(), id: doc.id });
    }
 ...
 */
export async function recordMention(userId, entityId, mentionData) {
    const ref = await getEntitiesRef(userId);
    const docRef = ref.doc(entityId);
    const updates = {
    ...
    if (mentionData?.sentiment !== undefined) {
        ...
        if (Math.abs(mentionData.sentiment) > 0.5) {
            updates.emotionalWeight = (await import('@google-cloud/firestore')).FieldValue.increment(0.05);
        }
    }
 ...
 */
export async function hasEntityStore(userId) {
    try {
        const ref = await getEntitiesRef(userId);
 ...
 */
export async function getEntityStoreStats(userId) {
