...
import { createMention, recordMention } from './storage.js';
 ...
 */
export function isEntityStoreReady() {
 ...
 */
export async function capturePersonEntity(userId, input, context) {
    if (!isEntityStoreReady()) {
        throw new Error('Entity store not ready');
    }
    log.debug({ userId, name: input.name, relationship: input.relationship }, 'Capturing person entity');
 ...
 */
function inferMentionType(transcript, input) {
 ...
 */
function extractFacts(input, context) {
    const facts = [];
    if (input.phone) {
        facts.push({
            type: 'attribute',
 ...
 */
export async function captureMultiplePeople(userId, inputs, context) {
    const results = [];
    for (const input of inputs) {
        try {
            ...
            log.warn({ userId, name: input.name, error: String(error) }, 'Failed to capture person in bulk import');
        }
    }
    log.info({ userId, total: inputs.length, captured: results.length }, 'Bulk person capture complete');
    return results;
}
// ============================================================================
// QUERY HELPERS
 ...
 */
export async function findContactForTelephony(userId, query) {
    if (!isEntityStoreReady()) {
 ...
 */
export async function getEntityStoreHealth(userId) {
    if (!isEntityStoreReady()) {
        return {
            ready: false,
 ...
 */
export async function retrieveMemoriesUnified(userId, query, options = {}) {
    if (!isEntityStoreReady()) {
        ...
        const results = entities.map((entity, index) => ({
            entity: {
                ...
                lastSeen: entity.lastMentionedAt
                    ? (typeof entity.lastMentionedAt.toDate === 'function'
                        ? entity.lastMentionedAt.toDate().toISOString()
                        : entity.lastMentionedAt.toISOString())
 ...
 */
export async function checkProactiveSurfacing(userId, currentTurn, options) {
    if (!isEntityStoreReady()) {
        return [];
    }
    try {
        ...
        log.debug({ userId, turnNumber: options.turnNumber, opportunitiesFound: opportunities.length }, 'Proactive surfacing check complete');
 ...
 */
export async function captureCommitmentEntity(userId, data, context) {
    if (!isEntityStoreReady()) {
