...
export class CorrelationEngine {
     ...
     */
    async getCorrelations(userId, options) {
        await this.initialize();
        if (!this.db)
            return [];
        let query = this.db
        ...
        if (options?.entityIds) {
            correlations = correlations.filter((c) => options.entityIds.some((id) => c.entityIds.includes(id)));
        }
     ...
     */
    generateDescription(correlation, entityNames) {
    ...
    // ============================================================================
    detectTemporalPatterns(userId, observations) {
        const correlations = [];
        const entityTimePatterns = new Map();
        for (const obs of observations) {
            if (!obs.context.timeOfDay)
                continue;
    ...
    detectEmotionalPatterns(userId, observations) {
        const correlations = [];
        const entityEmotionPatterns = new Map();
        for (const obs of observations) {
            if (!obs.context.emotion)
                continue;
            for (const entityId of obs.entityIds) {
        ...
        for (const [entityId, emotionMap] of entityEmotionPatterns) {
            ...
            if (totalCount < MIN_OBSERVATIONS)
                continue;
    ...
    detectSocialPatterns(userId, observations) {
        const correlations = [];
        const cooccurrences = new Map();
        for (const obs of observations) {
            if (obs.entityIds.length < 2)
                continue;
    ...
    detectCyclicalPatterns(userId, observations) {
        const correlations = [];
        const entityDayPatterns = new Map();
        for (const obs of observations) {
            if (!obs.context.dayOfWeek)
                continue;
            for (const entityId of obs.entityIds) {
    ...
    async loadRecentObservations(userId, days) {
        if (!this.db)
            return [];
    ...
    async findExistingCorrelation(userId, key) {
        if (!this.db)
            return null;
        const snapshot = await this.db
            .collection(CORRELATIONS_COLLECTION)
