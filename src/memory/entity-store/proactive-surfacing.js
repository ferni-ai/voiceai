...
export class ProactiveSurfacingEngine {
     ...
     */
    async findEntityMentions(context) {
        const store = getEntityStore();
        // Get potential entity matches via Graph-RAG
        const results = await graphRAGRetrieve(context.userId, context.currentTurn, {
     ...
     */
    async getRelatedWorthSurfacing(entity, context) {
        const store = getEntityStore();
     ...
     */
    shouldSurfaceRelated(source, related, relationship, context) {
        ...
        const daysSinceReinforced = (Date.now() - relationship.lastReinforced.getTime()) / (24 * 60 * 60 * 1000);
        if (daysSinceReinforced > 30)
            return false;
        // Emotional entities are worth surfacing
        if (related.emotionalWeight > 0.7)
            return true;
        // Commitments related to people are worth surfacing
        if (source.type === 'person' && related.type === 'commitment')
            return true;
        // Events involving people are worth surfacing
        if (source.type === 'person' && related.type === 'event')
            return true;
        return false;
    }
    /**
     ...
     */
    async checkTemporalTriggers(userId) {
        const store = getEntityStore();
        const triggers = [];
        ...
        for (const person of people) {
            ...
            if (attrs._type !== 'person' || !attrs.birthday)
                continue;
        ...
        for (const event of events) {
            ...
            if (attrs._type !== 'event' || !attrs.date)
                continue;
     ...
     */
    generatePatternPhrase(pattern, context) {
        const attrs = pattern.attributes;
        if (attrs._type !== 'pattern')
            return '';
        const softOpeners = [
     ...
     */
    async checkCommitmentOpportunities(context) {
        const store = getEntityStore();
        ...
        for (const commitment of commitments) {
            ...
            if (attrs._type !== 'commitment')
                continue;
            // Skip completed or abandoned
            if (attrs.status === 'completed' || attrs.status === 'abandoned')
                continue;
            // Check if target date is soon
            if (attrs.targetDate) {
                const daysUntil = Math.ceil((new Date(attrs.targetDate).getTime() - today.getTime()) / (24 * 60 * 60 * 1000));
                if (daysUntil <= 0) {
                    opportunities.push({
                        entity: commitment,
                        timing: 'soon',
            ...
            if (attrs.lastCheckIn) {
                const daysSinceCheckin = Math.ceil((today.getTime() - new Date(attrs.lastCheckIn).getTime()) / (24 * 60 * 60 * 1000));
     ...
     */
    assessReceptivity(context, content) {
        let receptivity = 0.5;
     ...
     */
    generateEntityContextPhrase(entity, related) {
     ...
     */
    prioritizeOpportunities(opportunities, context) {
        // Sort by receptivity score
        const sorted = opportunities.sort((a, b) => (b.receptivityScore ?? 0) - (a.receptivityScore ?? 0));
        // Take top 2-3 opportunities
        const maxOpportunities = Math.max(1, this.config.maxSurfacingsPerSession - context.surfacingCountThisSession);
        return sorted.slice(0, maxOpportunities);
    }
    // ═══════════════════════════════════════════════════════════════════════════
    // DATE HELPERS
    // ═══════════════════════════════════════════════════════════════════════════
    isTomorrow(month, day) {
        const tomorrow = new Date();
        tomorrow.setDate(tomorrow.getDate() + 1);
        return tomorrow.getMonth() + 1 === month && tomorrow.getDate() === day;
    }
    isThisWeek(month, day) {
        const today = new Date();
        for (let i = 0; i <= 7; i++) {
...
export function getProactiveSurfacingEngine() {
    if (!engineInstance) {
        engineInstance = new ProactiveSurfacingEngine();
    }
    return engineInstance;
}
//# sourceMappingURL=proactive-surfacing.js.map