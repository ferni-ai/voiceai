 ...
 */
export function configurePredictiveWarming(options) {
 ...
 */
export function configureMemoryRetrieval(fn) {
    retrieveMemoryFn = fn;
 ...
 */
export function detectTimeSignals() {
    const now = new Date();
 ...
 */
export function predictQueries(signals) {
    const predictions = [];
 ...
 */
export async function warmCacheForSession(userId, signals) {
 ...
 */
export async function warmCacheForHandoff(userId, fromPersona, toPersona) {
 ...
 */
async function warmQuery(userId, prediction) {
    if (!retrieveMemoryFn)
        return false;
    try {
        ...
        if (config.debug) {
            log.debug({ userId, query: prediction.query, category: prediction.category }, 'Pre-warmed query in semantic cache');
        }
        return true;
    }
    catch (error) {
...
//# sourceMappingURL=predictive-cache-warming.js.map