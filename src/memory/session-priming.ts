...
export interface RelationshipContext {
  ...
  lastSessionGap: number; // days since last conversation
...
const DEFAULT_CONFIG: SessionPrimingConfig = {
  maxOpenThreads: 3,
  maxFollowUps: 2,
...
export class SessionPrimer {
  private config: SessionPrimingConfig;
  ...
  async generatePrimingContext(
    ...
    const suggestedOpener = this.generateOpener(
      relationshipContext,
      emotionalContext,
      openThreads,
      pendingFollowUps,
  ...
  private extractOpenThreads(
    ...
    for (const memory of emotionalMemories) {
      ...
      if (
        threads.some(
          (t) => t.topic === memory.topics?.[0] || t.context.includes(memory.content.slice(0, 30))
        )
      ) {
        continue;
      }
   ...
   */
  private extractPendingFollowUps(profile: UserProfile, memories: MemoryItem[]): PendingFollowUp[] {
  ...
  private analyzeEmotionalContext(
    lastSummary: ConversationSummary | undefined,
    profile: UserProfile
  ): EmotionalContext {
    if (!lastSummary) {
      return {
    ...
    let sessionEndState: EmotionalContext['sessionEndState'] = 'neutral';
  ...
  private buildRelationshipContext(
    ...
    let relationshipStage: RelationshipContext['relationshipStage'] = 'new';
    if (sessionCount >= 20) {
   ...
   */
  private selectSalientMemories(memories: MemoryItem[]): MemoryItem[] {
  ...
  private calculateUrgency(dueDate: Date, now: Date): PendingFollowUp['urgency'] {
  ...
  private generateEmotionalOpener(memory: MemoryItem): string {
  ...
  private generateFollowUpPrompt(memory: MemoryItem): string {
...
let defaultPrimer: SessionPrimer | null = null;

/**
 * Get the default session primer
 */
export function getSessionPrimer(config?: Partial<SessionPrimingConfig>): SessionPrimer {
  if (!defaultPrimer) {
 ...
 */
export function resetSessionPrimer(): void {
  defaultPrimer = null;
}
