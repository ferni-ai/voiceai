...
vi.mock('../../services/twilio-sms.js', () => ({ sendSMS: vi.fn().mockResolvedValue('mock_sms_id') }));
...
vi.mock('../../services/calendar/calendar-service.js', () => ({
  ...
  createEvent: vi.fn().mockResolvedValue({ id: 'mock_event_id', htmlLink: 'https://calendar.google.com/event/mock' }),
  deleteEvent: vi.fn().mockResolvedValue(true),
}));

import { checkActionPermission, getTrustProfile, createPendingAction, getPendingActions, resolvePendingAction, markActionExecuted, ACTION_TYPES } from '../../services/automation/trust-level-system.js';
...
import { logAction, markApproved as auditMarkApproved, markExecuted as auditMarkExecuted, queryAuditLog } from '../../services/automation/audit-log.js';
...
describe('AGI Full Flow E2E', () => {
  ...
  describe('Messaging Full Flow', () => {
    it('should complete SMS flow: check → create → approve → execute → audit', async () => {
      const preview = createMockActionPreview({ title: 'Send SMS to Mom', summary: 'Happy Birthday!', canUndo: false });
      ...
      const auditEntry = await logAction({
        ...
        description: 'Send birthday message', status: 'pending', requestedBy: 'ferni',
      ...
      const sendResult = await sendMessageOnBehalf({
        userId: testUserId, channel: 'sms', recipient: { name: 'Mom', phone: '+1234567890' }, message: 'Happy Birthday!',
      });
      expect(sendResult.success).toBe(true);

      if (checkResult.pendingActionId) await markActionExecuted(testUserId, checkResult.pendingActionId);
  ...
  describe('Calendar Full Flow', () => {
    it('should complete calendar event flow', async () => {
      ...
      const preview = createMockActionPreview({ title: 'Team Standup', summary: 'Daily meeting', canUndo: true });
      ...
      const auditEntry = await logAction({
        ...
        description: 'Team Standup', status: 'pending', requestedBy: 'ferni',
        approvalRequired: checkResult.requiresApproval, canUndo: true,
      });

      await auditMarkApproved(testUserId, auditEntry.id, 'ui');
      const createResult = await createEventOnBehalf({ userId: testUserId, title: 'Team Standup', startTime: tomorrow });
  ...
  describe('Booking Full Flow', () => {
    it('should complete restaurant booking flow', async () => {
      const preview = createMockActionPreview({ title: 'Book table', summary: '4 people Saturday', canUndo: true, estimatedCost: 80 });
      const checkResult = await checkActionPermission(testUserId, 'book_restaurant', preview);
      expect(checkResult.success).toBe(true);
      expect(checkResult.requiresApproval).toBe(true);

      const auditEntry = await logAction({
        userId: testUserId, category: 'booking', actionType: 'book_restaurant',
        description: 'Book table', status: 'pending', requestedBy: 'ferni',
        approvalRequired: true, canUndo: true, monetaryValue: 80, currency: 'USD',
      });

      const bookResult = await bookReservation({
        userId: testUserId,
        restaurant: { id: 'rest_1', name: 'Italian Place', address: '123 Main St', city: 'SF', provider: 'google' },
        date: '2025-02-15', time: '19:00', partySize: 4,
        guestName: 'Test User', guestPhone: '+1234567890', guestEmail: 'test@test.com',
      });
    ...
    it('should complete rideshare booking flow', async () => {
      const preview = createMockActionPreview({ title: 'Book Uber', summary: 'To Airport', canUndo: true, estimatedCost: 35 });
  ...
  describe('Trust Progression Flow', () => {
    it('should track approval history', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (result.pendingActionId) await resolvePendingAction(testUserId, result.pendingActionId, true);
      }
    });
  });
  ...
  describe('Audit Trail Flow', () => {
    it('should maintain complete audit trail', async () => {
      const auditEntry = await logAction({
        userId: testUserId, category: 'messaging', actionType: 'send_sms',
        description: 'Send message', status: 'pending', requestedBy: 'ferni',
        approvalRequired: true, canUndo: false, personaId: 'ferni', triggerSource: 'calendar_insight',
      });
  ...
  describe('Multi-Action Flow', () => {
    it('should handle multiple concurrent actions', async () => {
      ...
      const results = await Promise.all(
        actions.map(action => checkActionPermission(testUserId, action.type, createMockActionPreview({ title: action.title })))
      );
