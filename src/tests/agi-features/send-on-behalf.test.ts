...
import { sendMessageOnBehalf, getMessageHistory, type SendMessageRequest } from '../../services/automation/send-on-behalf.js';
...
describe('Send on Behalf', () => {
  ...
  describe('Request Validation', () => {
    it('should reject request without userId', async () => {
      const request: SendMessageRequest = { userId: '', channel: 'sms', recipient: { name: 'John', phone: '+1234567890' }, message: 'Hello' };
    ...
    it('should reject SMS without phone number', async () => {
      const request: SendMessageRequest = { userId: testUserId, channel: 'sms', recipient: { name: 'John' }, message: 'Hello' };
      const result = await sendMessageOnBehalf(request);
      expect(result.success).toBe(false);
    ...
    it('should reject email without email address', async () => {
      const request: SendMessageRequest = { userId: testUserId, channel: 'email', recipient: { name: 'John' }, message: 'Hello' };
      const result = await sendMessageOnBehalf(request);
      expect(result.success).toBe(false);
    ...
    it('should reject empty message', async () => {
      const request: SendMessageRequest = { userId: testUserId, channel: 'sms', recipient: { name: 'John', phone: '+1234567890' }, message: '' };
      const result = await sendMessageOnBehalf(request);
      expect(result.success).toBe(false);
    ...
    it('should accept valid SMS request', async () => {
      const request: SendMessageRequest = { userId: testUserId, channel: 'sms', recipient: { name: 'John', phone: '+1234567890' }, message: 'Hello' };
      const result = await sendMessageOnBehalf(request);
      expect(result.success).toBe(true);
    });

    it('should accept valid email request', async () => {
      const request: SendMessageRequest = { userId: testUserId, channel: 'email', recipient: { name: 'Jane', email: 'jane@test.com' }, message: 'Hello' };
      const result = await sendMessageOnBehalf(request);
      expect(result.success).toBe(true);
    });
  });
  ...
  describe('Approval Flow', () => {
    it('should require approval for new users', async () => {
      const request: SendMessageRequest = { userId: testUserId, channel: 'sms', recipient: { name: 'John', phone: '+1234567890' }, message: 'Test' };
      const result = await sendMessageOnBehalf(request);
      expect(result.success).toBe(true);
