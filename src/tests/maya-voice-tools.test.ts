/**
 * Maya Voice-First Tools Tests
 *
 * Tests for Maya Santos's voice-first habit coaching tools:
 * - quickHabitCheck
 * - microCommitNow
...
describe('Maya Voice-First Tools', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('Tool Definitions', () => {
    it('should export all four voice-first tools', async () => {
      const { mayaVoiceTools } = await import('../tools/domains/habits/maya-voice-tools.js');

      expect(mayaVoiceTools).toHaveLength(4);

      const toolIds = mayaVoiceTools.map((t) => t.id);
    ...
    it('should have correct domains for all tools', async () => {
      ...
      for (const tool of mayaVoiceTools) {
        expect(tool.domain).toBe('habits');
        expect(tool.tags).toContain('voice');
        expect(tool.tags).toContain('maya');
      }
    });
  });
  ...
  describe('weeklyHabitReview Definition', () => {
    it('should have correct metadata', async () => {
      const { mayaVoiceTools } = await import('../tools/domains/habits/maya-voice-tools.js');
      const weeklyReview = mayaVoiceTools.find((t) => t.id === 'weeklyHabitReview');
...
describe('Maya Voice Tools - Voice-First Design', () => {
