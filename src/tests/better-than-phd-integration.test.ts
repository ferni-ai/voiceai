...
describe('Better-Than-PhD Integration', () => {
  describe('Context Builder Pipeline', () => {
    ...
    it('should produce cognitive distortion context for concerning statements', async () => {
      ...
      const hasCognitiveContent = injections.some(i => 
    ...
    it('should produce somatic context for high-distress situations', async () => {
      ...
      const hasCriticalOrStandard = injections.some(i => 
        i.priority === 'critical' || i.priority === 'standard'
      );
      expect(hasCriticalOrStandard).toBe(true);
      
      // Should have somatic or breathing-related content
      const hasSomaticContent = injections.some(i => 
    ...
    it('should produce behavioral economics context for goal discussions', async () => {
      ...
      const mockInput = {
        userText: "I want to start exercising but I just can't seem to make it happen. What's wrong with me?",
        analysis: {
    ...
    it('should produce therapeutic framework context for established relationships', async () => {
      ...
      const mockInput = {
        userText: "I want to change but I keep falling back into old patterns. Part of me wants to, part of me doesn't.",
        analysis: {
      ...
      const hasChangeOrTherapeutic = injections.some(i => 
  ...
  describe('Context Formatting', () => {
    it('should format multiple injections correctly', async () => {
      const { formatContextForPrompt, createStandardInjection, createHintInjection, createCriticalInjection } = await import(
    ...
    it('should respect max length limits', async () => {
      const { formatContextForPrompt, createStandardInjection } = await import(
        '../intelligence/context-builders/index.js'
      );
      ...
      for (let i = 0; i < 20; i++) {
        injections.push(
          createStandardInjection(`test_${i}`, 'A'.repeat(500))
        );
      }
