...
describe('Behavior System E2E', () => {
  ...
  describe('AliveOrchestrator → Behavior Mode', () => {
    it('should map alive events to behavior modes', async () => {
      const { AliveOrchestrator } = await import('../services/alive-orchestrator.js');

      const sendDataMessageMock = vi.fn().mockResolvedValue(undefined);

      const orchestrator = new AliveOrchestrator({
        userId: 'test-user',
        personaId: 'ferni',
        speakCallback: vi.fn(),
        sendDataMessage: sendDataMessageMock,
      });

      // Simulate a game milestone event
      const event = orchestrator.onGameEvent({
        gameType: 'name-that-tune' as const,
        eventType: 'correct',
        gameMemory: {
          totalGamesPlayed: 10,
          highScore: 5,
        ...
        currentStreak: 8, // High streak triggers intensity change
      });
    ...
    it('should not emit behavior signals when callback not provided', async () => {
      const { AliveOrchestrator } = await import('../services/alive-orchestrator.js');

      // Create without sendDataMessage
      const orchestrator = new AliveOrchestrator({
        userId: 'test-user',
        personaId: 'ferni',
        speakCallback: vi.fn(),
      ...
      const result = await orchestrator.onUserTurn({
        userMessage: 'test',
        turnCount: 5,
      });
