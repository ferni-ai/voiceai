...
import { createCalculatorTools, calculateCompoundGrowth, calculateFeeImpact } from '../tools/calculators.js';
...
describe('Integration Tests: Tools → Memory → Services', () => {
  describe('Calculator Tools → Service Integration', () => {
    ...
    const sessionId = `integration-calc-${Date.now()}`;
  ...
  describe('Emotion Detection → Context Flow', () => {
    let services: SessionServices;
    const sessionId = `integration-emotion-${Date.now()}`;
  ...
  describe('State Machine → Context Integration', () => {
    it('should progress through conversation phases', () => {
      ...
      stateMachine.processTurn({
        ...
        topics: ['retirement'],
        emotion: { primary: 'neutral', valence: 'neutral', distressLevel: 0, intensity: 0.3, confidence: 0.7, markers: [], suggestedTone: 'friendly' },
        intent: { primary: 'seeking_advice', secondary: [], confidence: 0.8, urgency: 'medium', requiresAction: false, requiresEmpathy: false, suggestedApproach: 'provide advice', markers: [] },
      });
  ...
  describe('Full Conversation Cycle', () => {
    let services: SessionServices;
    const sessionId = `integration-full-${Date.now()}`;
    ...
    it('should handle complete conversation flow', async () => {
      ...
      // Turn 4: Response
      services.addTurn('assistant', "Great question! Let me explain the basics of retirement investing...");
    ...
    it('should generate SSML for responses', () => {
      ...
      const text = "Index funds are a great choice for most investors. They offer low costs and broad diversification.";
