...
describe('Agent Orchestration Tests', () => {
  describe('Session Services', () => {
    ...
    beforeEach(async () => {
      // Initialize services (with minimal setup for testing)
  ...
  describe('Context Manager', () => {
    ...
    const createMockGuidance = (phase: 'greeting' | 'warming_up' | 'exploring' | 'advising' | 'supporting' | 'wrapping_up' | 'follow_up' = 'greeting') => ({
      phase,
    ...
    it('should track conversation turns', () => {
      ...
      const context = contextManager.buildPromptContext(
        { phase: 'greeting', currentMood: 'neutral', distressLevel: 0, turnCount: 2, userNeedsSupport: false, topicsDiscussed: [], currentTopic: null, topicsToCircleBack: [], startedAt: new Date(), lastActivityAt: new Date(), greetingComplete: false, nameObtained: false, emotionalStateKnown: false, primaryConcernIdentified: false, emotionalTrend: 'stable', userWantsToEnd: false, userIsReturning: false },
        createMockGuidance(),
        { primary: 'neutral', intensity: 0.3, valence: 'neutral', distressLevel: 0, confidence: 0.5, markers: [], suggestedTone: 'friendly' }
      );
    ...
    it('should detect returning user status', () => {
      const context = contextManager.buildPromptContext(
        { phase: 'greeting', currentMood: 'neutral', distressLevel: 0, turnCount: 1, userNeedsSupport: false, topicsDiscussed: [], currentTopic: null, topicsToCircleBack: [], startedAt: new Date(), lastActivityAt: new Date(), greetingComplete: false, nameObtained: false, emotionalStateKnown: false, primaryConcernIdentified: false, emotionalTrend: 'stable', userWantsToEnd: false, userIsReturning: false },
        createMockGuidance(),
        { primary: 'neutral', intensity: 0.3, valence: 'neutral', distressLevel: 0, confidence: 0.5, markers: [], suggestedTone: 'friendly' }
      );
    ...
    it('should format context for prompt injection', () => {
      contextManager.addTurn({ role: 'user', content: 'Tell me about retirement', timestamp: new Date() });
      
      const context = contextManager.buildPromptContext(
        { phase: 'exploring', currentMood: 'curious', distressLevel: 0, turnCount: 1, userNeedsSupport: false, topicsDiscussed: ['retirement'], currentTopic: 'retirement', topicsToCircleBack: [], startedAt: new Date(), lastActivityAt: new Date(), greetingComplete: true, nameObtained: false, emotionalStateKnown: false, primaryConcernIdentified: false, emotionalTrend: 'stable', userWantsToEnd: false, userIsReturning: false },
        createMockGuidance('exploring'),
        { primary: 'anticipation', intensity: 0.5, valence: 'positive', distressLevel: 0, confidence: 0.7, markers: [], suggestedTone: 'friendly' }
      );
    ...
    it('should handle new user without profile', () => {
      ...
      const context = newContextManager.buildPromptContext(
        { phase: 'greeting', currentMood: 'neutral', distressLevel: 0, turnCount: 0, userNeedsSupport: false, topicsDiscussed: [], currentTopic: null, topicsToCircleBack: [], startedAt: new Date(), lastActivityAt: new Date(), greetingComplete: false, nameObtained: false, emotionalStateKnown: false, primaryConcernIdentified: false, emotionalTrend: 'stable', userWantsToEnd: false, userIsReturning: false },
        createMockGuidance(),
        { primary: 'neutral', intensity: 0.3, valence: 'neutral', distressLevel: 0, confidence: 0.5, markers: [], suggestedTone: 'friendly' }
      );
  ...
  describe('Conversation Analyzer Integration', () => {
    ...
    it('should detect distress requiring support', () => {
      ...
      const distressed = detector.detect("I'm desperate and can't cope, I feel helpless about losing everything");
      expect(distressed.distressLevel).toBeGreaterThan(0.5);
      // High distress should suggest supportive tones
      expect(['gentle', 'reassuring', 'calm', 'measured', 'informative']).toContain(distressed.suggestedTone);
    });
  ...
  describe('Session Lifecycle', () => {
    ...
    it('should handle multiple concurrent sessions', async () => {
      ...
      const sessions = await Promise.all(
        sessionIds.map(id => createSessionServices(id))
      );
