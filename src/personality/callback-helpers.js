 ...
 */
export function createCallbackKeyMoment(what, options = {}) {
    return {
 ...
 */
export function extractCallbackKeyMoments(userMessage) {
    const keyMoments = [];
    // Upcoming events
    const eventPatterns = [
    ...
    for (const pattern of eventPatterns) {
        if (pattern.test(userMessage)) {
            keyMoments.push(createCallbackKeyMoment(userMessage.slice(0, 100), {
                type: 'milestone',
 ...
 */
export function getPendingCallbacksFromProfile(profile) {
    ...
    pending.sort((a, b) => {
        ...
        const weightDiff = weightOrder[a.moment.emotionalWeight] - weightOrder[b.moment.emotionalWeight];
        if (weightDiff !== 0)
            return weightDiff;
 ...
 */
export function formatCallbackForPrompt(callback) {
    const transitions = [
