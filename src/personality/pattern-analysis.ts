 ...
 */
export function analyzeForPatterns(userId: string): void {
  if (!hasEnoughHistoryForPatterns(userId)) return;
    ...
    if (relevantPoints.length >= PATTERN_CONFIG.MIN_RELEVANT_POINTS) {
      ...
      if (correlation >= PATTERN_CONFIG.CORRELATION_THRESHOLD) {
        ...
        patterns.push({
          ...
          deliveryTiming:
            correlation >= PATTERN_CONFIG.HIGH_CORRELATION_THRESHOLD ? 'now' : 'when_relevant',
          confidence: correlation,
          detectedAt: new Date(),
          lastUpdated: new Date(),
          surfacedToUser: false,
  ...
  if (history.length >= minPointsForTrend) {
    ...
    if (
      recentNegative.length >= PATTERN_CONFIG.MIN_NEGATIVE_FOR_DECLINE &&
