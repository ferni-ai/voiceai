...
export class TimingCalculator {
     ...
     */
    analyzeMessageTiming(message, metadata) {
        ...
        for (const [intent, signals] of Object.entries(INTENT_SIGNALS)) {
            for (const signal of signals) {
                if (signal.test(message, meta)) {
                    ...
                    scores.set(intent, current + signal.weight);
                }
            }
        }
     ...
     */
    shouldSharePersonalMoment(message, momentRelevance, emotionalState, relationshipDepth, metadata) {
     ...
     */
    shouldBringUpCallback(message, callbackUrgency, emotionalState, metadata) {
        const timing = this.analyzeMessageTiming(message, metadata);
     ...
     */
    shouldSurfacePatternInsight(message, patternConfidence, emotionalState, relationshipDepth, metadata) {
        const timing = this.analyzeMessageTiming(message, metadata);
        // Crisis = never
        if (emotionalState.isCrisisLevel) {
...
//# sourceMappingURL=timing-calculator.js.map