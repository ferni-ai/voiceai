 ...
 */
export function recordGrowthEvidence(userId, area, evidence, isProgress) {
    ...
    if (areaGrowth) {
        if (isProgress) {
            ...
            const daysSincePast = Math.floor((Date.now() - new Date(areaGrowth.pastDate).getTime()) / (1000 * 60 * 60 * 24));
            if (daysSincePast >= 7) {
 ...
 */
function generateGrowthCelebration(growth) {
    const daysSince = Math.floor((growth.currentDate.getTime() - growth.pastDate.getTime()) / (1000 * 60 * 60 * 24));
    const timePhrase = daysSince > 60
        ? 'a few months ago'
        : daysSince > 30
            ? 'last month'
            : daysSince > 14
 ...
 */
export function getGrowthCelebrations(userId, options = {}) {
 ...
 */
export function markGrowthSurfaced(growthId, userId) {
    const moments = growthMoments.get(userId);
    if (!moments)
        return;
 ...
 */
export function clearUserGrowthMoments(userId) {
 ...
 */
export function clearAllGrowthMoments() {
    growthMoments.clear();
...
//# sourceMappingURL=growth-tracking.js.map