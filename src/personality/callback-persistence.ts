 ...
 */
export async function extractAndSaveCallbacks(
  userId: string,
  userMessage: string
): Promise<CallbackSaveResult> {
  const result: CallbackSaveResult = {
    saved: 0,
    skipped: 0,
    errors: 0,
    momentIds: [],
  ...
  if (result.saved > 0) {
    log.info(
      { userId, saved: result.saved, types: callbacks.map((c) => c.type) },
      '✅ Callbacks persisted'
    );
  }
 ...
 */
export async function saveKeyMoment(userId: string, moment: KeyMoment): Promise<boolean> {
  if (!userId || !moment) {
    return false;
  }
 ...
 */
export async function markCallbackComplete(userId: string, momentId: string): Promise<boolean> {
  if (!userId || !momentId) {
    return false;
  }
