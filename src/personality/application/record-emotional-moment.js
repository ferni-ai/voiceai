...
export class RecordEmotionalMoment {
    ...
    emotionDetector;
    voiceAnalyzer;
    vulnerabilityScorer = new VulnerabilityScorer();
    constructor(repository, emotionDetector, voiceAnalyzer) {
    ...
    async execute(input) {
        ...
        if (input.voiceFeatures && this.voiceAnalyzer) {
            ...
            const voiceState = EmotionalState.fromVoiceAnalysis(voiceEmotion.emotion, voiceEmotion.granular, voiceEmotion.confidence, voiceEmotion.confidence);
        ...
        if (input.topics && input.topics.length > 0) {
            ...
            profile.recordPatternEvidence('topic_emotion', `${input.topics[0]} → ${detectedState.primary}`, input.topics, patternEvidence);
            // Check for person-related patterns
            if (input.mentionedPeople && input.mentionedPeople.length > 0) {
                profile.recordPatternEvidence('person_related', `${input.mentionedPeople[0]} → ${detectedState.primary}`, input.mentionedPeople, patternEvidence);
            }
            ...
            if (detectedState.isNegative && detectedState.intensity > 0.5) {
                profile.recordPatternEvidence('temporal', `${currentDay} ${timeOfDay} → ${detectedState.primary}`, [`${currentDay} ${timeOfDay}`, currentDay ?? '', timeOfDay], patternEvidence);
            }
            patternEvidenceRecorded = true;
        }
        // 8. Update trust signal based on acknowledgment quality
        if (input.acknowledgmentQuality) {
            const trustDelta = input.acknowledgmentQuality === 'positive' ? 1 :
     ...
     */
    async detectEmotion(message, topics) {
     ...
     */
    simpleEmotionDetection(message, topics) {
     ...
     */
    generateSummary(message, category) {
...
//# sourceMappingURL=record-emotional-moment.js.map