...
const CRISIS_PATTERNS = [
  { pattern: /\b(kill myself|want to die|end it all)\b/i, severity: 'critical' as const },
  { pattern: /\b(suicidal|self.?harm)\b/i, severity: 'critical' as const },
  { pattern: /\b(don't want to live|no point living)\b/i, severity: 'high' as const },
  { pattern: /\b(can't go on|can't do this anymore)\b/i, severity: 'high' as const },
  { pattern: /\b(hopeless|nothing matters|give up)\b/i, severity: 'moderate' as const },
  { pattern: /\b(panic attack|can't breathe)\b/i, severity: 'moderate' as const },
];

/**
 * EmotionDetectorAdapter - Text-based emotion detection
 */
export class EmotionDetectorAdapter implements EmotionDetector {
  /**
  ...
  async detectContradiction(
    ...
    const positiveEmotions: PrimaryEmotion[] = ['joy', 'trust', 'anticipation'];
    const negativeEmotions: PrimaryEmotion[] = ['sadness', 'anger', 'fear', 'disgust'];
   ...
   */
  async analyzeTrajectory(emotionalHistory: EmotionalState[]): Promise<TrajectoryResult> {
    ...
    const valenceScores: number[] = recentStates.map((state) => {
      if (state.isPositive) return 1;
      if (state.isNegative) return -1;
      return 0;
    });
   ...
   */
  async detectFirstTimeVulnerability(
    text: string,
    _userId: string
    ...
    if (text.match(/\b(trauma|abuse|secret|shame)\b/i)) vulnerabilityLevel = 5;
    else if (text.match(/\b(scared|afraid|struggling)\b/i)) vulnerabilityLevel = 4;
    else if (text.match(/\b(worried|anxious|hard)\b/i)) vulnerabilityLevel = 3;
   ...
   */
  async extractEmotionalTopics(text: string): Promise<{
    topics: string[];
    topicEmotionPairs: Array<{ topic: string; emotion: PrimaryEmotion; confidence: number }>;
  }> {
    const topicPatterns: Array<{ pattern: RegExp; topic: string; emotion?: PrimaryEmotion }> = [
   ...
   */
  async detectVagueEmotions(text: string): Promise<{
    vagueTerms: string[];
    suggestedPreciseEmotions: Map<string, GranularEmotion[]>;
    clarifyingQuestions: string[];
  }> {
    const vaguePatterns: Array<{
      pattern: RegExp;
      term: string;
      suggestions: GranularEmotion[];
      question: string;
    }> = [
      {
    ...
    const suggestedPreciseEmotions = new Map<string, GranularEmotion[]>();
 ...
 */
let instance: EmotionDetectorAdapter | null = null;

export function getEmotionDetectorAdapter(): EmotionDetectorAdapter {
  if (!instance) {
