 ...
 */
export class VoiceAnalyzerAdapter implements VoiceAnalyzer {
  /**
   * Analyze voice for emotional content
   */
  async analyzeEmotion(features: VoiceFeatures): Promise<VoiceEmotionResult> {
    try {
   ...
   */
  async analyzeStress(features: VoiceFeatures): Promise<StressAnalysisResult> {
   ...
   */
  async analyzeBreath(features: VoiceFeatures): Promise<BreathAnalysisResult> {
    // Infer breath pattern from pauses and voice quality
    let pattern: BreathAnalysisResult['pattern'] = 'normal';
    let emotionalIndicator: string | undefined;
   ...
   */
  async detectVoiceBreaking(features: VoiceFeatures): Promise<{
    isBreaking: boolean;
    confidence: number;
    ...
    if (features.jitter && features.jitter > 0.03) breakingScore += 0.3;
    if (features.shimmer && features.shimmer > 0.08) breakingScore += 0.3;

    // Low voice quality
    if (features.voiceQuality && features.voiceQuality < 0.5) breakingScore += 0.2;

    // High pitch variation in short bursts
    if (features.pitchVariation && features.pitchVariation > 80) breakingScore += 0.2;

    const isBreaking = breakingScore >= 0.4;
    const severity: 'mild' | 'moderate' | 'severe' =
      breakingScore >= 0.8 ? 'severe' : breakingScore >= 0.6 ? 'moderate' : 'mild';
   ...
   */
  async classifyTone(features: VoiceFeatures): Promise<VoiceTone> {
    // Check for breaking first
    const breaking = await this.detectVoiceBreaking(features);
    if (breaking.isBreaking) return 'breaking';
  ...
  async compareToBaseline(
    ...
    if (currentFeatures.pitchMean && baselineFeatures.pitchMean) {
      ...
      if (pitchDiff > 30) {
        deviations.push(
          currentFeatures.pitchMean > baselineFeatures.pitchMean
    ...
    if (currentFeatures.speakingRate && baselineFeatures.speakingRate) {
      ...
      if (rateDiff > 30) {
        deviations.push(
          currentFeatures.speakingRate > baselineFeatures.speakingRate
    ...
    if (currentFeatures.energyLevel && baselineFeatures.energyLevel) {
      ...
      if (energyDiff > 10) {
        deviations.push(
          currentFeatures.energyLevel > baselineFeatures.energyLevel
  ...
  private estimateArousal(features: VoiceFeatures): number {
    let arousal = 0.5; // Default moderate

    // Speaking rate affects arousal
    if (features.speakingRate) {
      if (features.speakingRate > 180) arousal += 0.2;
      else if (features.speakingRate > 150) arousal += 0.1;
      else if (features.speakingRate < 100) arousal -= 0.2;
      else if (features.speakingRate < 120) arousal -= 0.1;
    }
    ...
    if (features.pitchVariation) {
      if (features.pitchVariation > 50) arousal += 0.15;
      else if (features.pitchVariation < 20) arousal -= 0.1;
    }

    return Math.max(0, Math.min(1, arousal));
  }

  private estimateValence(features: VoiceFeatures): number {
    ...
    if (features.jitter && features.jitter > 0.02) valence -= 0.2;
    if (features.shimmer && features.shimmer > 0.05) valence -= 0.15;
  ...
  private calculateConfidence(features: VoiceFeatures): number {
    ...
    if (features.pitchMean !== undefined) featureCount++;
    if (features.pitchVariation !== undefined) featureCount++;
    if (features.speakingRate !== undefined) featureCount++;
    if (features.energyLevel !== undefined) featureCount++;
    if (features.jitter !== undefined) featureCount++;
    if (features.shimmer !== undefined) featureCount++;
    if (features.voiceQuality !== undefined) featureCount++;

    // More features = higher confidence, max 0.9
    return Math.min(0.9, 0.3 + featureCount * 0.1);
  }

  private classifyPace(features: VoiceFeatures): 'rapid' | 'fast' | 'normal' | 'slow' | 'hesitant' {
    if (!features.speakingRate) return 'normal';

    if (features.speakingRate > 200) return 'rapid';
    if (features.speakingRate > 170) return 'fast';
    if (features.speakingRate < 100) return 'hesitant';
    if (features.speakingRate < 130) return 'slow';
    return 'normal';
  }
}
 ...
 */
let instance: VoiceAnalyzerAdapter | null = null;

export function getVoiceAnalyzerAdapter(): VoiceAnalyzerAdapter {
  if (!instance) {
