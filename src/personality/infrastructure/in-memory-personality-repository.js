 ...
 */
function profileKey(userId, personaId) {
 ...
 * ```
 */
export class InMemoryPersonalityRepository {
    storage = {
        profiles: new Map(),
        patterns: new Map(),
    ...
    async saveProfile(profile) {
        ...
        this.storage.profiles.set(key, profileData);
    ...
    async loadPatterns(userId, options) {
        ...
        if (!userPatterns)
            return [];
        let patterns = Array.from(userPatterns.values()).map((data) => EmotionalPattern.fromPersistence(data));
        // Apply filters
    ...
    async loadVulnerabilities(userId, options) {
        ...
        if (!userVulns)
            return [];
        let vulnerabilities = Array.from(userVulns.values()).map((data) => VulnerabilityDeposit.fromPersistence(data));
        // Apply filters
        if (options?.openOnly) {
    ...
    async loadMilestones(userId, options) {
        ...
        if (!userMilestones)
            return [];
        let milestones = Array.from(userMilestones.values()).map((data) => GrowthMilestone.fromPersistence(data));
        // Apply filters
...
//# sourceMappingURL=in-memory-personality-repository.js.map