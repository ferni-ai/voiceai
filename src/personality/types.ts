 ...
 */
export type PersonalMomentTopic =
  // Life experiences
  | 'morning_routine'
  | 'creative_struggle'
  | 'family_life'
 ...
 */
export type UserMomentCategory =
  | 'upcoming_event' // "I have a recital Friday"
  | 'ongoing_situation' // "I'm dealing with my mom's health"
  | 'decision_point' // "I'm thinking about quitting"
  | 'relationship_moment' // "My sister and I had a fight"
  | 'achievement' // "I finally finished the project"
  | 'struggle' // "I've been having a hard time"
  | 'goal' // "I want to start exercising"
  | 'life_update'; // General updates worth remembering
...
export interface PersonalityRelationshipDoc {
  ...
  sharedMoments: Array<
    Omit<SharedMomentRecord, 'sharedAt'> & {
      sharedAt: string;
    }
  >;
  userMoments: Array<
    Omit<UserMomentRecord, 'sharedAt' | 'followUpAfter' | 'followedUpAt'> & {
      sharedAt: string;
