...
import type { MemoryRetrievalResult, TrustContextResult } from './types.js';
 ...
 */
export async function wireContextIntegrations(
  contextManager: ContextManager,
  options: ContextIntegrationOptions = {}
): Promise<void> {
  const {
    userId,
    enableTrust = true,
    ...
    try {
      ...
      contextManager.setTrustContextBuilder((uid, userText, context) => {
        try {
          ...
          const result: TrustContextResult = {
            unsaidSignals: trustResult.unsaidSignals?.map(
              (s) => `${s.type}: ${s.underlying} (${s.observation})`
            ),
            ...
            needsSupport: trustResult.unsaidSignals?.some(
              (s) => s.type === 'emotional_mismatch' || s.type === 'minimizing_pain'
            ),
            raw: trustResult,
          };
 ...
 */
export async function getIntegratedContextManager(
  sessionId: string,
  options: ContextIntegrationOptions & {
