 ...
 */
import { formatForPrompt, injectSharedContent, } from '../personas/shared/index.js';
// ============================================================================
// RELATIONSHIP CONTEXT
// ============================================================================
export function buildRelationshipContext(userProfile) {
    if (!userProfile) {
...
// EMOTIONAL CONTEXT
// ============================================================================
export function buildEmotionalContext(userProfile, emotion, state) {
    const sections = [];
    if (emotion) {
        sections.push(`Current mood: ${emotion.primary} (intensity: ${emotion.intensity.toFixed(1)})`);
        if (emotion.distressLevel > 0.5) {
            sections.push(`⚠️ DISTRESS DETECTED (${emotion.distressLevel.toFixed(2)}) - prioritize emotional support`);
        }
    }
...
// TOPIC CONTEXT
// ============================================================================
export function buildTopicContext(userProfile, state) {
    const parts = [
...
function maybeAvoidTopics(userProfile) {
 ...
 */
export function buildHandoffChainDescription(handoffHistory, currentPersona) {
    const firstHandoff = handoffHistory[0];
    if (!firstHandoff) {
        return currentPersona || '';
    }
    const parts = [firstHandoff.fromPersona];
 ...
 */
export function getInvolvedPersonas(currentPersona, handoffHistory) {
...
// ============================================================================
export function buildSharedContent(options, injectionOptions) {
    ...
    let recentLifeMilestones;
    if (userProfile?.lifeEvents && userProfile.lifeEvents.length > 0) {
        const activeEvents = userProfile.lifeEvents.filter((e) => e.status === 'planning' || e.status === 'upcoming' || e.status === 'in_progress');
        ...
        const recentCompletions = userProfile.lifeEvents.filter((e) => {
            if (e.status !== 'completed' || !e.completedAt)
                return false;
            const daysSinceCompletion = Math.floor((Date.now() - new Date(e.completedAt).getTime()) / (1000 * 60 * 60 * 24));
    ...
    const sharedContext = {
        ...
        handoffChain: handoffHistory && handoffHistory.length > 0
            ? buildHandoffChainDescription(handoffHistory, currentPersona)
            : undefined,
        involvedPersonas: handoffHistory && handoffHistory.length > 0
...
export function getFormattedSharedContent(options, injectionOptions) {
