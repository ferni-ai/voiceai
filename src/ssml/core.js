 ...
 * @module ssml/core
 */
import { applyThinkingTimeSSML, calculateThinkingTime, } from '../conversation/thinking-time-injector.js';
...
import { containsSsmlNative, stripSsmlNative, isNativeSsmlAvailable, } from './native-ssml-processor.js';
// =============================================================================
 ...
 */
function handleUrlsAndEmails(text) {
    let result = text;
    // Replace URLs with spoken description
    result = result.replace(/https?:\/\/(?:www\.)?([a-zA-Z0-9-]+(?:\.[a-zA-Z]{2,})+)(?:\/[^\s)]*)?/gi, (_, domain) => {
 ...
 */
export function hasSsmlTags(text) {
    ...
    return (/<(speed|volume|emotion|break|spell)\b/.test(text) ||
 ...
 * Falls back to JS regex when native module not loaded.
 */
export function stripSsmlTags(text) {
 ...
 */
export function sanitizeSsml(text) {
    let result = text;
    // ================================================
    ...
    result = result.replace(/(?<!\[)\b(chuckl(?:es?|ing)|laugh(?:s|ing)?|giggl(?:es?|ing))\s*(softly|gently|quietly|to himself|to herself|briefly|warmly)?\s*\.?\s*/gi, '[laughter] ');
    result = result.replace(/(?<!\[)\b(he|she|jack|ferni|i)\s+(chuckl(?:es?|ing)|laugh(?:s|ing)?|giggl(?:es?|ing))\s*(softly|gently|quietly|briefly|warmly)?\.?\s*/gi, '[laughter] ');
    // Catch any remaining standalone forms (chuckle, chuckles, chuckling, laugh, laughs, laughing, giggle, giggles, giggling)
    result = result.replace(/(?<!\[)\b(chuckl(?:es?|ing)|laugh(?:s|ing)?|giggl(?:es?|ing))\b[,.!?:;—–-]?\s*/gi, '[laughter] ');
    ...
    result = result.replace(new RegExp(LAUGHTER_PLACEHOLDER.replace(/[\\^$.*+?()[\]{}|]/g, '\\$&'), 'g'), '[laughter]');
    // Remove common standalone stage direction phrases
    result = result.replace(/\b(deep breath|long pause|brief pause|sighs heavily|clears throat|takes a breath|taking a breath)\b/gi, '');
    // Remove non-audio action verbs with modifiers
    result = result.replace(/\b(sighs?|smiles?|grins?|nods?|pauses?|winks?|exhales?|inhales?|shifts?|leans?|settles?)\s*(softly|gently|quietly|to himself|to herself|briefly|warmly|in|down|up|back)?\s*\.?\s*/gi, '');
    result = result.replace(/\b(he|she|jack|ferni|maya|jordan|alex|peter|i)\s+(sighs?|smiles?|grins?|pauses?|nods?|exhales?|shifts?|leans?|settles?)\s*(softly|gently|quietly|briefly|warmly)?\.?\s*/gi, '');
    // Remove "with a [adjective] smile/tone/voice" phrases
    result = result.replace(/\bwith\s+a\s+(?:warm|gentle|soft|teasing|playful|knowing|mischievous|affectionate|slight|small|big|wide)?\s*(?:smile|grin|smirk|tone|voice|look|expression)\b[,.!?:;—–-]?\s*/gi, '');
    // Remove "[adjective] smile/tone" at start of sentence or standalone
    result = result.replace(/\b(?:teasing|playful|knowing|mischievous|warm|gentle|affectionate)\s+(?:smile|grin|smirk|tone|voice)\b[,.!?:;—–-]?\s*/gi, '');
    ...
    result = result.replace(/\b(?:playfully|warmly|gently|softly|quietly|teasingly|knowingly|mockingly|dryly|wryly|ironically)?\s*(?:sarcastic|teasing|playful|mischievous|knowing|wry|dry|ironic|deadpan|mock)\b[,.!?:;—–-]?\s*/gi, '');
    ...
    result = result.replace(/\*(?:eyes?|accent|rapid|efficient|excitedly?|delighted|enthusiastic)[^*]*\*/gi, '');
    // ================================================
 ...
 */
export function tagTextWithSsmlPersonaAware(text, optionsOrPersonaId) {
    // Extract options
    const options = typeof optionsOrPersonaId === 'string'
        ? { personaId: optionsOrPersonaId }
        : (optionsOrPersonaId ?? {});
    const { personaId, naturalFillers = true, breathGroupPacing = true, fillerConfig, breathConfig, thinkingTime = false, thinkingContext, thinkingInjection: providedThinkingInjection, } = options;
    ...
    if (thinkingTime && (providedThinkingInjection || thinkingContext)) {
        const injection = providedThinkingInjection ||
...
function addBasicPauses(text) {
    ...
    result = result.replace(/(\.\s+)(First|Second|Third|Also|Another|Additionally|Furthermore|Moreover|Finally)\b/gi, `$1<break time="${listItemPause}"/>$2`);
    // Slow down slightly before important words
    result = result.replace(/\b(importantly|crucially|key|essential|main|critical)\b/gi, '<break time="150ms"/>$1');
