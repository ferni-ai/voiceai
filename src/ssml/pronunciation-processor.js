...
function categorizePatterns() {
    const categories = [
        ...
        },
        {
            ...
            quickCheck: (text) => /wabi|ikigai|kaizen|shinrin|kintsugi|ganbatte|shoganai|mono no aware|omoiyari|natsukashii|gaman|mottainai|yugen|ichi.?go|satori|sensei|sake|umami|tsunami|bonsai|tatami|dojo|futon|ramen|matcha|shoyu|miso|wasabi|edamame|kombucha|tofu/i.test(text),
            patterns: [],
        },
        {
            name: 'mindfulness_meditation',
            // Sanskrit, Tibetan, Zen, and meditation terms
            quickCheck: (text) => /namaste|prana|chakra|mantra|asana|savasana|dharma|karma|sangha|mudra|kundalini|vinyasa|samadhi|dhyana|ayurveda|dosha|vata|pitta|kapha|satsang|ahimsa|bardo|tonglen|metta|vipassana|dukkha|nirvana|bodhisattva|qi|chi|tai chi|qigong|feng shui|yin|yang|dao|tao|zen|koan|zazen|kinhin/i.test(text),
            patterns: [],
        },
        {
            name: 'coaching_psychology',
            // Coaching frameworks and psychology terms
            quickCheck: (text) => /MBTI|Myers.?Briggs|enneagram|StrengthsFinder|CliftonStrengths|DiSC|eustress|somatic|polyvagal|amygdala|hippocampus|prefrontal|neuroplasticity|cortisol|dopamine|serotonin|oxytocin|endorphin|BJ Fogg|Gretchen Rubin|Upholder|Questioner|Obliger|Rebel|dichotomy|premeditatio|amor fati|memento mori|apatheia|ataraxia|eudaimonia|Seneca|Epictetus|Marcus Aurelius/i.test(text),
            patterns: [],
        },
        {
            name: 'western_places',
            // Western US place names
            quickCheck: (text) => /Teton|Cheyenne|Laramie|Shoshone|Dubois|Absaroka|Bighorn|Thermopolis|Cody|Jackson Hole|Missoula|Bozeman|Helena|Butte|Glacier|Flathead|Tucson|Prescott|Tempe|Sedona|Saguaro|Mogollon|Albuquerque|Santa Fe|Taos|Rio Grande|Nevada|Tahoe|Tonopah|Ouray|Salida|Gunnison|Sangre|Alamosa|Pueblo|Saguache|Limon|Tooele|Duchesne|Moab|Uinta|Wasatch|Hurricane|Nephi|Lehi|Kanab|Weber|Escalante|Heber|Panguitch|Parowan|Sanpete|Sevier|Timpanogos|Bonneville|Zion|Coeur d.?Alene|Boise|Sequoia|Yosemite|Willamette|Spokane|Puyallup|Cascade/i.test(text),
            patterns: [],
        },
    ];
    ...
    for (const entry of ALL_PRONUNCIATIONS) {
        ...
        const hasUppercaseReq = /\[A-Z\]|[A-Z]{2,}/.test(patternStr) || /\\b[A-Z]{2,}\\b/.test(patternStr);
        const hasSymbols = /[&%$@#\/]/.test(patternStr);
        const isJapanese = /wabi|ikigai|kaizen|shinrin|kintsugi|ganbatte|shoganai|omoiyari|natsukashii|gaman|mottainai|yugen|satori|sensei|sake|umami|tsunami|bonsai|tatami|dojo|futon|ramen|matcha|shoyu|miso|wasabi|edamame|kombucha|tofu/i.test(patternStr);
        const isMindfulness = /namaste|prana|chakra|mantra|asana|savasana|shavasana|dharma|karma|sangha|mudra|kundalini|vinyasa|samadhi|dhyana|ayurveda|dosha|vata|pitta|kapha|satsang|ahimsa|bardo|tonglen|metta|vipassana|dukkha|nirvana|bodhisattva|qi|chi|qigong|feng.?shui|yin|yang|dao|tao|zen|koan|zazen|kinhin/i.test(patternStr);
        const isCoachingPsych = /MBTI|Myers|enneagram|Strengths|CliftonStrengths|DiSC|eustress|somatic|polyvagal|amygdala|hippocampus|prefrontal|neuroplasticity|cortisol|dopamine|serotonin|oxytocin|endorphin|Fogg|Rubin|Upholder|Questioner|Obliger|Rebel|dichotomy|premeditatio|amor|memento|apatheia|ataraxia|eudaimonia|Seneca|Epictetus|Aurelius/i.test(patternStr);
        const isWesternPlace = /Teton|Cheyenne|Laramie|Shoshone|Dubois|Absaroka|Bighorn|Thermopolis|Cody|Jackson|Missoula|Bozeman|Helena|Butte|Glacier|Flathead|Tucson|Prescott|Tempe|Sedona|Saguaro|Mogollon|Albuquerque|Santa.?Fe|Taos|Rio.?Grande|Nevada|Tahoe|Tonopah|Ouray|Salida|Gunnison|Sangre|Alamosa|Pueblo|Saguache|Limon|Tooele|Duchesne|Moab|Uinta|Wasatch|Hurricane|Nephi|Lehi|Kanab|Weber|Escalante|Heber|Panguitch|Parowan|Sanpete|Sevier|Timpanogos|Bonneville|Zion|Coeur|Boise|Sequoia|Yosemite|Willamette|Spokane|Puyallup|Cascade/i.test(patternStr);
 ...
 */
function getCategorizedPatterns() {
 ...
 */
export function applyPronunciationsOptimized(text) {
 ...
 */
export function estimatePatternChecks(text) {
    const categories = getCategorizedPatterns();
    let checked = 0;
    let skipped = 0;
    const categoryStats = [];
 ...
 */
export function resetPronunciationCache() {
    categorizedPatterns = null;
}
//# sourceMappingURL=pronunciation-processor.js.map