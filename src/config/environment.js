...
export function loadConfig() {
    ...
    if (process.env.MEMORY_STORE_TYPE) {
        storageType = process.env.MEMORY_STORE_TYPE;
    }
    ...
    const config = {
        ...
        integrations: {
            ...
            delivery: process.env.DOORDASH_API_KEY || process.env.UBER_CLIENT_ID
                ? {
                ...
                : undefined,
            sip: process.env.SIP_TRUNK_ID && process.env.SIP_DOMAIN
                ? {
...
export function printConfigSummary(config) {
    ...
    logger.info('├─────────────────────────────────────────────────┤');
    logger.info(`│ LiveKit:      ${(config.apis.livekitUrl ? '✓ Configured' : '✗ Missing').padEnd(33)}│`);
    logger.info(`│ Google AI:    ${(config.apis.googleApiKey ? '✓ Configured' : '✗ Missing').padEnd(33)}│`);
    logger.info(`│ Cartesia:     ${(config.apis.cartesiaApiKey ? '✓ Configured' : '✗ Missing').padEnd(33)}│`);
    logger.info(`│ Market Data:  ${(config.integrations.alphaVantage ? '✓ Configured' : '○ Optional').padEnd(33)}│`);
    logger.info(`│ Spotify:      ${(config.integrations.spotify ? '✓ Configured' : '○ Optional').padEnd(33)}│`);
    logger.info(`│ Yelp:         ${(config.integrations.yelp ? '✓ Configured' : '○ Optional').padEnd(33)}│`);
    logger.info(`│ Twilio:       ${(config.integrations.twilio ? '✓ Configured' : '○ Optional').padEnd(33)}│`);
    logger.info(`│ Delivery:     ${(config.integrations.delivery ? '✓ Configured' : '○ Optional').padEnd(33)}│`);
    logger.info('└─────────────────────────────────────────────────┘');
}
/**
