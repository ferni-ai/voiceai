...
function applyEnvironmentOverrides() {
    ...
    for (const flag of boolFlags) {
        ...
        if (envValue !== undefined) {
            overrides[flag] = envValue === 'true' || envValue === '1';
        }
    }
    // Rollout percentage
 ...
 */
export function isEnabledForSession(sessionId) {
    const flags = getFlags();
 ...
 */
export function updateFlags(updates) {
 ...
 */
export const voiceHumanizationFlags = new Proxy({}, {
    get(_target, prop) {
        return getFlags()[prop];
    },
});
//# sourceMappingURL=voice-humanization-flags.js.map