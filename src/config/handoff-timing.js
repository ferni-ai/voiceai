 ...
 *
 * These values are intentionally duplicated from apps/web/src/config/animation-constants.ts
 * because this backend config cannot import from the frontend package.
 *
 * IMPORTANT: If you change these values, also update:
 * - apps/web/src/config/animation-constants.ts
 * - design-system/tokens/animation.json
 *
 * @see apps/web/src/config/animation-constants.ts for the canonical source
 ...
 */
export function getTransitionDelay(style = 'standard', isUserInitiated = false, isFirstMeeting = false, isReturningToCoach = false) {
 ...
 */
export function getPostSoundPause(style = 'standard', isFirstMeeting = false) {
 ...
 */
export function isHandoffAllowed(lastHandoffTime) {
    const now = Date.now();
 ...
 */
export function getRateLimitCooldown(lastHandoffTime) {
    const now = Date.now();
...
//# sourceMappingURL=handoff-timing.js.map