 ...
 */
export async function handleAdminDiagnosticsRoutes(
  req: IncomingMessage,
  res: ServerResponse,
  pathname: string,
  _parsedUrl: URL
 ...
 */
export function recordHandoffEvent(event: Omit<HandoffEvent, 'id' | 'timestamp'>): void {
...
async function checkCartesiaHealth(): Promise<ServiceHealth> {
  try {
