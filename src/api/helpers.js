...
import { getCorsHeaders as getSecureCorsHeaders, getAPISecurityHeaders, isProduction, } from './security-headers.js';
// ============================================================================
// REQUEST PARSING
// ============================================================================
import { z } from 'zod';
 ...
 */
export async function parseRawBody(req, options = {}) {
 ...
 */
export async function parseBodyWithSchema(req, schema) {
    try {
 ...
 * @returns Validation result with typed data or error message
 */
export function validateQueryParams(parsedUrl, schema) {
    const params = {};
 ...
 */
export function getUserId(req, parsedUrl) {
    ...
    if (typeof fromLegacyHeader === 'string' && fromLegacyHeader)
        return fromLegacyHeader;
    // Dev mode bypass - allows testing without authentication
    // SECURITY: Only works in development environment
    const isDev = process.env.NODE_ENV !== 'production';
    const adminKey = parsedUrl.searchParams.get('admin_key') || req.headers['x-admin-key'];
 ...
 */
export function requireUserId(req, res, parsedUrl) {
 ...
 */
export function getSecureResponseHeaders(origin) {
    return {
 ...
 */
export function sendJSONCached(res, data, maxAge = 60, status = 200) {
 ...
 * @param status - HTTP status code
 */
export function sendJSONEdgeCached(res, data, maxAge = 3600, status = 200) {
 ...
 */
export function handleCorsPreflightIfNeeded(req, res) {
 ...
 */
export function parsePositiveInt(value, defaultValue, max) {
    if (!value)
        return defaultValue;
 ...
 */
export function getSecureUserId(auth, requestedUserId) {
 ...
 */
export function verifyAdminAccess(req, allowDevMode = false) {
 ...
 * @returns true if request has valid admin credentials
 */
export function verifyAdminAccessFromUrl(req, parsedUrl, allowDevMode = false) {
