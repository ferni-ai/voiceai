...
import { detectAnomalies, isLockedOut, recordSecurityEvent, recordSuccessfulAuth, trackFailedAuth, } from '../services/security-events.js';
...
export function authenticate(req) {
    ...
    if (apiKey) {
        ...
        void trackFailedAuth(`apikey:${apiKey.substring(0, 8)}`, ip, 'invalid_api_key').catch((e) => log.error({ error: String(e) }, 'Failed to track auth failure'));
    ...
    if (legacyUserId && !getHeader(req, 'Authorization') && !getHeader(req, 'X-API-Key')) {
        log.warn({ attemptedUserId: legacyUserId, ip }, 'SECURITY: Blocked legacy X-User-Id auth attempt - must use Firebase token or API key');
    }
    return null;
}
/**
...
async function tryFirebaseAuth(req) {
    const token = req._pendingFirebaseToken;
    if (!token)
        return null;
    const ip = getClientIP(req);
    const userAgent = getHeader(req, 'User-Agent');
    try {
        ...
        if (!verified) {
            ...
            void trackFailedAuth(`firebase:${ip}`, ip, 'firebase_token_invalid').catch((e) => log.error({ error: String(e) }, 'Failed to track auth failure'));
            return null;
        }
        return {
            userId: verified.uid,
 ...
 */
export async function requireAuth(req, res, config = {}) {
 ...
 */
export function requireAuthSync(req, res, config = {}) {
    const { optional = false, requireAdmin = false, allowDevMode = true } = config;
    const ip = getClientIP(req);
    const userAgent = getHeader(req, 'User-Agent');
 ...
 */
export function requireAdminSync(req, res) {
 ...
 */
export async function optionalAuthAsync(req) {
    let auth = authenticate(req);
    if (!auth) {
        auth = await tryFirebaseAuth(req);
    }
 ...
 */
export function getAuthenticatedUserIdSync(req, res) {
 ...
 */
export async function checkRateLimitAsync(key, maxRequests, windowMs) {
    return rateLimiter.check(key, maxRequests, windowMs);
}
/**
 * Get rate limit tier based on auth context
 */
export function getRateLimitTier(auth) {
    if (!auth)
...
export function rateLimit(req, res, options = {}) {
    ...
    const { maxRequests = options.tier?.maxRequests ?? defaultTier.maxRequests, windowMs = options.tier?.windowMs ?? defaultTier.windowMs, keyGenerator = (r) => getHeader(r, 'X-Forwarded-For') || r.socket.remoteAddress || 'unknown', keyPrefix = '', } = options;
 ...
 */
export function rateLimitExpensive(req, res, operationName) {
