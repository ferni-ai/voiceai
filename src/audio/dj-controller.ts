...
export type DJCommand =
  | { type: 'PLAY_TRACK'; track: MusicTrack; isAmbient?: boolean }
  | { type: 'STOP' }
  | { type: 'PAUSE' }
  | { type: 'RESUME' }
  | { type: 'SKIP' }
 ...
 */
export type DJEvent =
  | { type: 'state_changed'; from: DJState; to: DJState; track: MusicTrack | null }
  | { type: 'track_started'; track: MusicTrack; isAmbient: boolean }
  | { type: 'track_ended'; track: MusicTrack; wasSkipped: boolean; wasAmbient: boolean }
  | { type: 'ducking_started'; reason: DuckReason }
  | { type: 'ducking_ended' }
  | { type: 'fading_started'; track: MusicTrack }
  | { type: 'should_speak_intro'; track: MusicTrack }
  | { type: 'should_speak_outro'; track: MusicTrack }
  | { type: 'should_interject'; track: MusicTrack; momentType: 'buildup' | 'drop' | 'appreciation' }
  | { type: 'error'; message: string; command?: DJCommand };
 ...
 */
const VALID_TRANSITIONS: Record<DJState, Set<DJState>> = {
...
export class DJController extends EventEmitter {
  ...
  initialize(config: DJControllerConfig): void {
    ...
    if (this._state.sessionId && this._state.sessionId !== config.sessionId) {
      log.warn(
        { oldSession: this._state.sessionId, newSession: config.sessionId },
        'Session change detected - resetting DJ controller'
      );
      this.reset();
    }
   ...
   * Get initial state
   */
  private getInitialState(): DJControllerState {
    return {
      state: 'idle',
      currentTrack: null,
      isAmbient: false,
  ...
  private handlePlayTrack(track: MusicTrack, isAmbient: boolean): void {
    const prevState = this._state.state;
  ...
  private handleStop(explicit: boolean): void {
    const prevState = this._state.state;
  ...
  private handleDuck(reason: DuckReason): void {
  ...
  private handleTrackNearEnd(): void {
    if (this._state.state !== 'playing' && this._state.state !== 'ducking') {
      return;
    }
  ...
  private transitionTo(newState: DJState): void {
    const prevState = this._state.state;
  ...
  private emitEvent(event: DJEvent): void {
  ...
  getTrackElapsedTime(): number {
    if (!this._state.trackStartTime) return 0;
  ...
  getTrackRemainingTime(): number {
    if (!this._state.trackStartTime || !this._state.trackDuration) return 0;
...
let djControllerInstance: DJController | null = null;

/**
 * Get the singleton DJ Controller instance
 */
export function getDJController(): DJController {
 ...
 */
export function resetDJController(): void {
  if (djControllerInstance) {
 ...
 */
export function isDJControllerInitialized(): boolean {
