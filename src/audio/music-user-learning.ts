 ...
 */
const userProfiles = new Map<string, UserTransitionProfile>();
 ...
 */
export function getUserProfile(userId: string): UserTransitionProfile {
 ...
 */
function createDefaultProfile(userId: string): UserTransitionProfile {
  const now = Date.now();
 ...
 */
function jStat_gammaSample(shape: number): number {
 ...
 */
export function selectTransitionWithLearning(
  userId: string,
  availableTypes: TransitionType[],
  context?: {
    startReason?: MusicStartReason;
    emotionalTone?: 'heavy' | 'light' | 'neutral' | 'crisis';
    isLateNight?: boolean;
  }
): { selectedType: TransitionType; explorationRate: number } {
  ...
  let bestType: TransitionType = availableTypes[0];
  let bestSample = -1;
 ...
 */
export function updateUserLearning(
  userId: string,
 ...
 */
export function addMusicMemory(userId: string, entry: Omit<MusicMemoryEntry, 'timestamp'>): void {
  const profile = getUserProfile(userId);
 ...
 */
export function findRelevantMusicMemories(
  userId: string,
  emotionalContext?: string,
  topicContext?: string
): MusicMemoryEntry[] {
 ...
 */
export function getUserPreferredTransition(
  userId: string,
  context: {
    startReason?: MusicStartReason;
    emotionalTone?: 'heavy' | 'light' | 'neutral' | 'crisis';
  ...
  for (const [type, arm] of Object.entries(profile.transitionArms)) {
    if (arm.pulls >= 3) {
      ...
      if (expected > bestExpected) {
        ...
        bestType = type as TransitionType;
      }
    }
  }
 ...
 */
export function exportUserProfile(userId: string): UserTransitionProfile | null {
 ...
 */
export function importUserProfile(profile: UserTransitionProfile): void {
...
export function getUserLearningStats(userId: string): {
  totalTransitions: number;
  topTransitionTypes: Array<{ type: TransitionType; expectedValue: number; pulls: number }>;
  ...
  return {
    ...
    hasContextPreferences:
      Object.keys(profile.contextPreferences.byStartReason).length > 0 ||
