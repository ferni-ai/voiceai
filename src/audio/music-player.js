...
export class CallMusicPlayer {
    ...
    onMusicStateChangeCallback = null;
    // 🎤 Callback for "Wait for it..." mid-song moments
    onMidSongMomentCallback = null;
     ...
     */
    async initialize(room, agentSession, sessionId) {
        ...
        if (this.sessionId && sessionId && this.sessionId !== sessionId) {
            log.warn({ oldSession: this.sessionId, newSession: sessionId }, '🎵 Singleton pollution detected - resetting player for new session');
            await this.dispose();
        }
     ...
     */
    setOnMusicStateChangeCallback(callback) {
        // 🐛 FIX: Remove existing callback before adding new one to prevent listener accumulation
     ...
     */
    setOnMidSongMomentCallback(callback) {
        // 🐛 FIX: Remove existing callback before adding new one to prevent listener accumulation
     ...
     */
    addToSessionHistory(track, wasRequested) {
        ...
        log.debug({ track: track.name, historySize: this.sessionHistory.length }, '🎧 Added to session music history');
    }
    /**
     * Mark the most recent track as fully played
     */
    markCurrentTrackCompleted() {
     ...
     */
    getRecentTracks(count = 5) {
        return this.sessionHistory.slice(-count);
    }
    /**
     * Check if we've played music from this artist before in this session
     */
    hasPlayedArtist(artist) {
        return this.sessionHistory.some((entry) => entry.track.artist.toLowerCase() === artist.toLowerCase());
    }
    /**
    ...
    scheduleMidSongMoment(track) {
        ...
        log.debug({ track: track.name, momentTime: Math.round(momentTime / 1000) }, '🎤 Mid-song moment scheduled');
    }
    /**
     ...
     */
    async playFromUrl(url, track, isAmbient = false) {
        if (DEBUG_MUSIC)
        ...
        log.info({ url: url.slice(0, 50), track: track.name, artist: track.artist, isAmbient }, 'Playing from URL');
        ...
        try {
            ...
            log.info({ track: track.name, wasExplicitlyStoppedBefore: this.state.wasExplicitlyStopped }, '🎵 [PLAY-TRACE] Step 1: About to call internal stop() to clear current playback');
            this.stop();
            ...
            log.info({ track: track.name, url: url.slice(0, 60) }, '🎵 [PLAY-TRACE] Step 3: Starting download...');
            ...
            try {
                this.currentPlayHandle = this.backgroundPlayer.play({ source: audioPath, volume }, false // Don't loop previews
                );
            }
            catch (playError) {
                log.error({
            ...
            if (DEBUG_MUSIC)
                log.debug('BackgroundAudioPlayer.play() called successfully');
            log.info({ track: track.name, artist: track.artist, volume, isAmbient }, '🎵 Music playback started');
            ...
            const handleTrackEnd = (source) => {
                const elapsedMs = Date.now() - playbackStartTime;
                // Prevent double-firing
                if (this.trackEndHandled) {
                    log.debug({ source, track: track.name, elapsedMs }, '🎧 Track end already handled, skipping');
                    return;
                }
                this.trackEndHandled = true;
                // Clear both timers
            ...
            if (this.currentPlayHandle) {
                void this.currentPlayHandle
                    ...
                    if (this.state.currentTrack?.name !== track.name) {
                        log.debug({ expectedTrack: track.name, currentTrack: this.state.currentTrack?.name }, '🎧 waitForPlayout fired for stale track - ignoring');
                        return;
                    }
                    ...
                    const elapsedMs = Date.now() - playbackStartTime;
                    log.warn({ track: track.name, error: String(err), elapsedMs }, '🎧 waitForPlayout error - attempting immediate track end handling');
     ...
     */
    async crossfadeTo(url, track, isAmbient = false) {
        ...
        if (this.state.isChangingTrack) {
            log.warn({ currentTrack: this.state.currentTrack?.name, requestedTrack: track.name }, '🎧 Crossfade already in progress - rejecting concurrent request');
            return { success: false, previousTrack: this.state.currentTrack };
        }
        ...
        if (!downloadResult) {
            ...
            log.warn({ fromTrack: previousTrack.name, toTrack: track.name }, '🎧 Crossfade download failed - keeping current track');
        ...
        log.info({ track: track.name, artist: track.artist, volume, isAmbient }, '🎧 Crossfade complete - new track playing!');
        ...
        const handleTrackEnd = (source) => {
            ...
            if (source === 'backupTimer') {
                log.warn(timingInfo, '🎧 Track ended via BACKUP TIMER (crossfade) - waitForPlayout unreliable');
            }
            else {
        ...
        if (this.currentPlayHandle) {
            void this.currentPlayHandle
                ...
                // 🐛 FIX BUG-004: Check track name before handling to prevent stale handler from firing
                if (this.state.currentTrack?.name !== track.name) {
                    log.debug({ expectedTrack: track.name, currentTrack: this.state.currentTrack?.name }, '🎧 waitForPlayout fired for stale track in crossfade - ignoring');
                    return;
                }
                handleTrackEnd('waitForPlayout');
            })
                ...
                const elapsedMs = Date.now() - playbackStartTime;
                log.warn({ track: track.name, error: String(err), elapsedMs }, '🎧 waitForPlayout error in crossfade - attempting immediate track end handling');
                // Only handle if we haven't already (backup timer might have fired)
                if (!this.trackEndHandled && this.state.currentTrack?.name === track.name) {
                    handleTrackEnd('waitForPlayoutError');
                }
        ...
        log.debug({ track: track.name, fadeOutTime, backupDelay }, '🎧 Scheduled track timers (crossfade)');
     ...
     */
    async downloadAudio(url, trackName, hintDurationMs) {
        try {
            let buffer;
            ...
            if (!fs.existsSync(this.tempDir)) {
                try {
                    ...
                    log.error({ tempDir: this.tempDir, error: String(mkdirErr) }, '🎵 Failed to create temp directory - audio download will fail');
                    return null;
                }
            }
            // Write to temp file
            ...
            let durationSource = hintDurationMs != null ? 'hint' : 'default';
            if (this.ffmpegAvailable) {
                try {
                    ...
                    if (!isNaN(actualDurationSec) && actualDurationSec > 0) {
                        ...
                        log.debug({ actualDurationMs, actualDurationSec, trackName }, '🎧 Detected actual audio duration via ffprobe');
                    }
                }
                catch (probeErr) {
                    log.warn({ error: String(probeErr), hintDurationMs, trackName }, '🎧 ffprobe duration detection failed, using fallback');
                }
            }
            else {
                log.debug({ durationSource, actualDurationMs, trackName }, '🎧 ffprobe not available, using fallback duration');
            }
     ...
     */
    async applyAudioFadeOut(inputPath, actualDurationMs) {
        ...
        if (this.state.queue.length > 0) {
            ...
            if (nextTrack.previewUrl) {
                ...
                this.playFromUrl(nextTrack.previewUrl, nextTrack)
                    ...
                    .catch((err) => {
                    log.error({ track: nextTrack.name, error: String(err) }, '🎧 Queue playback error - trying next');
                    this.onTrackEnded();
                });
            }
            else {
            ...
            if (this.onMusicStateChangeCallback) {
                ...
                log.warn('🎧 No onMusicStateChangeCallback set - frontend will not be notified of track end!');
            }
        }
    }
    /**
    ...
    async resume() {
        if (this.state.currentTrack && this.state.currentTrack.previewUrl) {
            log.debug({ track: this.state.currentTrack.name }, 'Resuming playback (replaying from start)');
    ...
    setVolume(volume) {
        ...
        log.debug({ volume: Math.round(this.state.volume * 100) }, 'Volume set (applies to next track)');
    }
    /**
    ...
    wasExplicitlyStopped() {
        ...
        if (this.state.explicitStopTime &&
    ...
    async dispose() {
        ...
        if (this.state.isPlaying && this.onMusicStateChangeCallback) {
            const stoppedTrack = this.state.currentTrack;
            const wasAmbient = this.state.isAmbientMode;
            log.info({ track: stoppedTrack?.name, wasAmbient }, '🎧 [DISPOSE] Sending final stopped state before cleanup');
            try {
        ...
        if (this.backgroundPlayer) {
            try {
                ...
                log.debug({ error: String(err) }, 'Background player close error (expected during room cleanup)');
            }
 ...
 */
export async function resetMusicPlayer() {
    log.warn({
        timestamp: new Date().toISOString(),
 ...
 */
export async function initializeMusicPlayer(room, agentSession, sessionId) {
...
export function isMusicAvailable() {
    ...
    if (!musicPlayerInstance.isInitialized()) {
        log.info({ sessionId: musicPlayerInstance.getSessionId() }, '🎵 [DIAG] isMusicAvailable: singleton exists but NOT initialized');
        return {
            available: false,
    ...
    if (!state.isInitialized) {
        log.info({ sessionId: musicPlayerInstance.getSessionId(), state }, '🎵 [DIAG] isMusicAvailable: player was DISPOSED');
        return {
            available: false,
    ...
    if (!musicPlayerInstance.isRoomConnected()) {
        log.info({ sessionId: musicPlayerInstance.getSessionId() }, '🎵 [DIAG] isMusicAvailable: LiveKit room DISCONNECTED');
        return {
            available: false,
            reason: 'LiveKit room disconnected - reconnect to enable music playback',
        };
    }
    log.info({ sessionId: musicPlayerInstance.getSessionId() }, '🎵 [DIAG] isMusicAvailable: ALL CHECKS PASSED - music IS available');
