 ...
 */
function calculateProportionSignificance(p1, n1, p2, n2) {
    // Handle edge cases
    if (n1 === 0 || n2 === 0) {
        return {
            isSignificant: false,
...
class TransitionAnalyticsStore {
    ...
    globalStats = new Map();
    activeTests = new Map();
    userAssignments = new Map(); // userId -> testName -> variant
     ...
     */
    updateStatsMap(statsMap, event, signals) {
        ...
        if (signals.emotionalResponse) {
            const positiveCount = stats.positiveResponseRate * (stats.count - 1) +
        ...
        if (signals.continuedTopic !== undefined) {
            const continuedCount = stats.topicContinuationRate * (stats.count - 1) + (signals.continuedTopic ? 1 : 0);
     ...
     */
    getStats(transitionType) {
        return this.globalStats.get(transitionType) || null;
    }
    /**
     * Get stats for a user
     */
    getUserStats(userId, transitionType) {
     ...
     */
    getVariantAssignment(userId, testName) {
     ...
     */
    getVariantConfig(userId, testName) {
        ...
        for (const event of this.recentEvents) {
            if (event.experimentVariant && this.activeTests.has(testName)) {
                ...
                if (engagement) {
                    ...
                    this.updateStatsMap(new Map([[event.transitionType, results[event.experimentVariant]]]), event, engagement);
                }
            }
        }
        return results;
     ...
     */
    getTestResultsWithSignificance(testName) {
        const test = this.activeTests.get(testName);
        if (!test)
            return null;
        ...
        const significance = calculateProportionSignificance(control.positiveResponseRate, control.count, treatment.positiveResponseRate, treatment.count);
        ...
        if (!sampleSizeAdequate) {
            ...
            const lift = Math.abs(((treatment.positiveResponseRate - control.positiveResponseRate) /
        ...
        return {
            ...
            comparison: {
                ...
                relativeLift: control.positiveResponseRate > 0
    ...
    export() {
        return {
            ...
            globalStats: Object.fromEntries(this.globalStats),
        };
    }
    /**
    ...
    import(data) {
        ...
        if (data.globalStats) {
            for (const [type, stats] of Object.entries(data.globalStats)) {
                this.globalStats.set(type, stats);
            }
        }
        log.info({
 ...
 */
export function getTransitionAnalytics() {
    if (!analyticsStore) {
 ...
 */
export function resetTransitionAnalytics() {
    if (analyticsStore) {
 ...
 */
export function createTransitionEvent(sessionId, userId, personaId, musicContext, result, experimentVariant) {
    return {
 ...
 */
export function recordTransitionWithAnalytics(sessionId, userId, personaId, musicContext, result, experimentVariant) {
    const event = createTransitionEvent(sessionId, userId, personaId, musicContext, result, experimentVariant);
 ...
 */
export function recordEngagementSignals(eventId, signals) {
 ...
 */
export function getBestTransitionType(startReason, userId) {
 ...
 */
export async function loadAnalyticsFromFirestore() {
    if (analyticsLoaded)
        return;
    try {
        ...
        if (doc.exists) {
            ...
            if (data?.globalStats) {
                ...
                log.info({ statsCount: Object.keys(data.globalStats).length }, '📊 Analytics loaded from Firestore');
            }
        }
 ...
 */
export async function persistAnalyticsToFirestore() {
    try {
        const analytics = getTransitionAnalytics();
 ...
 */
export function startAnalyticsPersistence() {
 ...
 */
export async function stopAnalyticsPersistence() {
    if (persistenceInterval) {
...
//# sourceMappingURL=music-transition-analytics.js.map