...
import { importUserProfile, exportUserProfile, } from './music-user-learning.js';
import { exportUserMusicMemories, importUserMusicMemories, } from './music-memory-integration.js';
...
function serializeMemories(userId, data) {
    return {
        userId,
...
function deserializeMemories(data) {
    return {
        memories: data.memories.map((m) => ({
            id: m.id,
            userId: data.userId,
...
function getMemoryPersistence() {
    if (!memoryPersistence) {
        memoryPersistence = createPersistenceStore({
            collection: 'music_learning',
            documentId: 'memories',
...
export async function ensureProfileLoaded(userId) {
    if (loadedProfileUsers.has(userId))
        return;
    try {
        ...
        if (data) {
            ...
            log.debug({ userId, totalTransitions: profile.totalTransitions }, 'Loaded music profile from persistence');
        }
 ...
 */
export async function ensureMemoriesLoaded(userId) {
    if (loadedMemoryUsers.has(userId))
        return;
    try {
        ...
        if (data) {
            ...
            log.debug({ userId, memoryCount: memories.memories.length }, 'Loaded music memories from persistence');
        }
 ...
 */
export async function ensureMusicLearningLoaded(userId) {
 ...
 */
export function saveMemories(userId) {
    try {
        ...
        if (memories.memories.length > 0 || memories.preferences.length > 0) {
            ...
            log.debug({ userId, count: memories.memories.length }, 'Music memories queued for persistence');
        }
    }
    catch (error) {
 ...
 */
export function saveMusicLearning(userId) {
    saveProfile(userId);
    saveMemories(userId);
}
/**
 ...
 */
export async function flushMusicLearning(userId) {
    saveProfile(userId);
    saveMemories(userId);
    await Promise.all([
 ...
 */
export async function flushAllMusicLearning() {
 ...
 */
export async function shutdownMusicLearningPersistence() {
 ...
 */
export function onTransitionFeedbackRecorded(userId) {
 ...
 */
export function onMusicMemoryStored(userId) {
    saveMemories(userId);
}
// ============================================================================
// EXPORTS
...
//# sourceMappingURL=music-learning-persistence.js.map