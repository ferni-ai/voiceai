...
let cachedSpotifyAmbientTracks = null;
 ...
 */
export async function fetchSpotifyAmbientTracks() {
    // Check if cache is valid
    const now = Date.now();
    const cacheValid = cachedSpotifyAmbientTracks !== null &&
        cacheTimestamp !== null &&
    ...
    try {
        ...
        getLogger().debug({ error }, 'Could not fetch Spotify ambient tracks (Spotify may not be connected)');
 ...
 */
export function clearAmbientTracksCache() {
 ...
 */
export async function getAmbientTracksAsync() {
    const envTracks = getEnvAmbientTracks();
    const builtInTracks = BUILT_IN_AMBIENT_TRACKS.filter((t) => t.previewUrl);
 ...
 */
export function getRandomAmbientTrack() {
...
export async function playAmbientMusic() {
    ...
    if (tracksWithPreviews.length === 0) {
        getLogger().warn({ totalTracks: tracks.length }, 'No ambient tracks have preview URLs - cannot play ambient music');
        return false;
    }
 ...
 */
export function getAmbientMusicEndedPhrase(personaId) {
    ...
    const phrases = personaId && personaPhrases[personaId] ? personaPhrases[personaId] : defaultPhrases;
 ...
 */
export function getDJOutroPhrase(trackName, artistName, personaId) {
    ...
    const defaultOutros = (track, artist) => [
    ...
    const getOutros = personaId && personaOutros[personaId] ? personaOutros[personaId] : defaultOutros;
 ...
 * @param personaId - The persona speaking (for voice variation)
 */
export function getDJTrackChangePhrase(currentTrack, newTrackName, personaId) {
 ...
 */
export function getDJDropPhrase(trackName, artistName, personaId) {
 ...
 * @param personaId - The persona speaking
 */
export function getMidSongMomentPhrase(momentType, trackName, personaId) {
    ...
    const personaSet = personaId && personaPhrases[personaId] ? personaPhrases[personaId] : defaultPhrases;
 ...
 * @param personaId - The persona speaking
 */
export function getMoodAwareMusicOffer(mood, personaId) {
 ...
 * @param personaId - The persona speaking
 */
export function getSessionCallbackPhrase(sessionVibe, personaId) {
    if (sessionVibe.artists.length === 0)
        return null;
 ...
 */
export function getMusicStoppedPhrase(personaId, wasPaused = false) {
    ...
    const phrases = personaId && stoppedPhrases[personaId] ? stoppedPhrases[personaId] : defaultStopped;
    return phrases[Math.floor(Math.random() * phrases.length)];
}
export default {
    playAmbientMusic,
...
//# sourceMappingURL=ambient-music.js.map