 ...
 */
const userMusicMemories = new Map();
const userMusicPreferences = new Map();
// ============================================================================
// MEMORY CREATION
// ============================================================================
/**
 ...
 */
export function detectEmotionalContext(musicContext, userMessage) {
    // Default to general
    let state = 'general';
    let intensity = 0.5;
    let topic;
 ...
 */
export function detectMusicHelped(userResponse, voiceTone, sessionContinued = true) {
    let helpedScore = 0;
 ...
 */
export function storeMusicHelpedMemory(userId, musicContext, effectiveTransition, evidence) {
    // Check if music actually helped
    const helpCheck = detectMusicHelped(evidence.userResponse, evidence.voiceTone, evidence.continuedSession);
    if (!helpCheck.helped) {
        log.debug({ userId, confidence: helpCheck.confidence }, '🎵 Music did not help enough to store memory');
        return null;
    }
    // Create the memory
    const memory = {
        ...
        userId,
        createdAt: Date.now(),
        emotionalContext: detectEmotionalContext(musicContext, musicContext?.lastUserMessageBeforeMusic),
        music: {
 ...
 */
function generateTags(musicContext) {
    const tags = [];
    if (musicContext) {
        if (musicContext.startReason)
            tags.push(musicContext.startReason);
        if (musicContext.emotionalToneBeforeMusic)
            tags.push(musicContext.emotionalToneBeforeMusic);
        if (musicContext.wasAmbient)
            tags.push('ambient');
        if (musicContext.trackArtist)
            tags.push(musicContext.trackArtist.toLowerCase());
 ...
 */
function updatePreferencesFromMemory(userId, memory) {
    ...
    const existingPref = prefs.find((p) => p.context.emotionalState === memory.emotionalContext.state);
    if (existingPref) {
 ...
 */
export function findRelevantMemories(userId, context) {
 ...
 */
export function getMusicPreferences(userId, context) {
 ...
 */
export function generateMusicCallback(userId, personaId, currentContext) {
 ...
 */
function generateCallbackPhrases(memory, personaId) {
    const phrases = [];
    ...
    switch (memory.emotionalContext.state) {
        case 'stressed':
            if (hasArtist) {
                phrases.push(`<break time="300ms"/>Last time you were stressed, some ${memory.music.artist} seemed to help...`, `<break time="300ms"/>Remember when we played ${memory.music.artist}? That seemed to take the edge off.`);
            }
            else {
                phrases.push(`<break time="300ms"/>Music helped last time you were feeling like this...`, `<break time="300ms"/>I remember music made a difference before when things felt heavy.`);
            }
            break;
        case 'sad':
            phrases.push(`<break time="300ms"/>I remember music helped before. Want me to put something on?`, `<break time="300ms"/>Last time you were down, we listened together. Want to do that again?`);
            break;
        case 'thinking':
            if (memory.music.wasAmbient) {
                phrases.push(`<break time="300ms"/>Some background music helped you think last time...`, `<break time="300ms"/>Want some thinking music? It seemed to help before.`);
            }
            break;
        case 'celebrating':
            if (hasTrack) {
                phrases.push(`<break time="300ms"/>Remember "${memory.music.trackName}"? That was a good moment.`);
            }
            break;
    }
    ...
    if (personaId === 'nayan-patel') {
        ...
        return phrases.map((p) => p.replace('Last time', 'I recall when').replace('I remember', 'There was a time when'));
    }
    else if (personaId === 'jordan-taylor') {
        // Jordan is more energetic
        return phrases.map((p) => p.replace('seemed to help', 'was great!').replace('made a difference', 'really worked!'));
    }
    return phrases;
}
/**
 ...
 */
export function shouldMentionMusicMemory(userId, lastMentionTimestamp) {
 ...
 */
export function exportUserMusicMemories(userId) {
    return {
 ...
 */
export function importUserMusicMemories(userId, data) {
 ...
 */
export function clearAllMusicMemories() {
 ...
 */
export function getUserMusicMemoryStats(userId) {
    const memories = userMusicMemories.get(userId) || [];
    const prefs = userMusicPreferences.get(userId) || [];
    if (memories.length === 0) {
        return {
...
//# sourceMappingURL=music-memory-integration.js.map