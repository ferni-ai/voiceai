 ...
 */
export const PERSONA_DJ_STYLES = {
    ferni: {
        personaId: 'ferni',
 ...
 */
export function shouldDuck(context) {
    const { state } = context;
 ...
 */
export function shouldSpeakIntro(context) {
    ...
    const interjectionProbability = DJ_PROBABILITIES.TRACK_START_INTERJECTION * style.interjectionMultiplier;
 ...
 */
export function shouldSpeakOutro(context) {
 ...
 */
export function shouldInterject(context, momentType) {
 ...
 */
export function calculateScheduledMoments(track, personaId) {
 ...
 */
export function isDeadAirDetectionActive(state) {
    // Dead air detection is INACTIVE when music is active
    const musicActive = state.state === 'playing' ||
        state.state === 'ducking' ||
        state.state === 'fading' ||
 ...
 */
export function getDuckTiming(reason) {
    switch (reason) {
        case 'agent_speaking':
 ...
 */
export function shouldSkipThinkingMusic(state, thresholdMs = 30000) {
    // Music is already playing
...
//# sourceMappingURL=dj-decision-engine.js.map