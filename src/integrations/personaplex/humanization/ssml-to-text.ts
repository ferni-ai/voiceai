 ...
 * that PersonaPlex can interpret and embody in its speech.
 *
 * This is how we preserve ALL humanization investment when moving to PersonaPlex:
...
import { getProsodyProfile, type ProsodyProfile } from '../../../speech/prosody/prosody-profiles.js';
import { getEmotionalProsody, type EmotionalProsody } from '../../../speech/prosody/emotional-prosody.js';
import { getBackchannelResponse, type BackchannelContext } from '../../../speech/backchanneling/backchannel-engine.js';
 ...
 */
function translateAnticipation(
  topics: string[],
  turnCount: number,
  trustLevel: number
): string {
 ...
 */
function translateNaturalSpeech(
  personaId: string,
  userEmotion: string
): string {
  ...
  if (patterns.fillers && patterns.fillers.length > 0) {
    guidance.push(`Use natural fillers occasionally: "${patterns.fillers.slice(0, 3).join('", "')}"`);
  }
...
function generateTimingGuidance(
  ...
  // Pacing matching
  if (userSpeakingFast) {
    guidance.push('They\'re speaking quickly. Match their pace to show engagement, but stay grounded.');
  }
...
/**
 * Translate all SSML/prosody guidance to text-based instructions for PersonaPlex
 */
export async function translateSSMLToText(
  ...
  const emotionGuidance = translateEmotionalExpression(
    userEmotion,
    intensity,
    trustLevel
  );

  const backchannelResult = translateBackchanneling(
    { userEmotion, turnCount },
    turnCount
  );
  ...
  const timingGuidance = generateTimingGuidance(
    isSensitiveTopic,
    userSpeakingFast,
    userEmotion
  );
...
${backchannelResult.suggestedPhrases.slice(0, 3).map((p) => `• "${p}"`).join('\n')}
`.trim();
 ...
 */
export function getTimeBasedVoiceGuidance(
  hour: number
): string {
  if (hour >= 5 && hour < 9) {
