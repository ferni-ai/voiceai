...
import { isNativeAudioAvailable, convertI16ToF32, getOrCreateNativeProcessor, processNativeFrame, getNativeFullFeatures, resetNativeProcessor, } from './native-analyzer.js';
import { mapToEmotionalDimensions, classifyEmotion, calculateStressLevel, detectAnxietyMarkers, smoothFeatures, } from './emotion-mapping.js';
...
export class AudioProsodyAnalyzer {
    buffers = [];
    maxBufferMs = 5000; // Keep last 5 seconds
    minSamplesForAnalysis = 4410; // ~100ms at 44.1kHz
    // Baseline calibration (personalized over time)
    baselinePitch = 150; // Hz (average human)
    baselineEnergy = -20; // dB
    baselineRate = 4; // syllables/sec
    calibrated = false;
    ...
    processAudioFrame(frame) {
        ...
        try {
            ...
            if (this.useNativeProcessor && this.sessionId) {
                processNativeFrame(this.sessionId, frame.data, now);
    ...
    analyze() {
        ...
        const dimensions = mapToEmotionalDimensions(smoothed, { pitch: this.baselinePitch, energy: this.baselineEnergy, rate: this.baselineRate }, this.calibrated);
        // Classify emotion
