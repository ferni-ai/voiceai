 ...
 */
const EMOTION_PROTOTYPES: Record<VoiceEmotion, { v: number; a: number; d: number }> = {
  neutral: { v: 0, a: 0, d: 0 },
 ...
 */
const CONTOUR_SCORES: Record<ProsodyFeatures['pitchContour'], number> = {
  rising: 0.3,
  dynamic: 0.1,
  flat: -0.1,
  falling: -0.3,
 ...
 */
export function mapToEmotionalDimensions(
  prosody: ProsodyFeatures,
  baseline: { pitch: number; energy: number; rate: number },
  calibrated: boolean
): EmotionalDimensions {
 ...
 */
export function detectAnxietyMarkers(prosody: ProsodyFeatures): boolean {
 ...
 */
export function smoothFeatures(history: ProsodyFeatures[]): ProsodyFeatures {
