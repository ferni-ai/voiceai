 ...
 */
function analyzeProsodic(prosody) {
    let score = 0;
 ...
 */
function analyzeTemporalPatterns(bursts, currentDuration) {
    if (bursts.length < 2) {
    ...
    const validIntervals = intervals.filter((i) => i >= CONFIG.BURST_INTERVAL_MIN && i <= CONFIG.BURST_INTERVAL_MAX);
    ...
    const intervalVariance = intervals.reduce((acc, i) => acc + Math.pow(i - avgInterval, 2), 0) / intervals.length;
 ...
 */
function analyzeContext(recentAgentText, emotionalArc, conversationPhase) {
 ...
 */
function classifyLaughterType(prosodic, temporal, duration) {
    // Short, quiet = chuckle
 ...
 */
function determineSocialFunction(laughType, contextScore, emotionalArc) {
 ...
 */
function suggestResponse(laughType, socialFunction, confidence) {
...
export class MultiSignalLaughterDetector {
     ...
     */
    detect(prosody, spectral, duration = 0) {
        ...
        const spectralScore = spectral ? this.analyzeSpectral(spectral) : prosodicScore * 0.8; // Fallback estimate
        const recentBursts = this.getRecentBursts();
        const temporalAnalysis = analyzeTemporalPatterns(recentBursts, duration);
        const contextualScore = analyzeContext(this.recentAgentText, this.emotionalArc, this.conversationPhase);
        // Weighted combination
        const confidence = CONFIG.WEIGHT_PROSODIC * prosodicScore +
        ...
        let laughType = 'unknown';
        let socialFunction = 'unknown';
        if (isLaughter) {
     ...
     * Record energy burst
     */
    recordBurst(intensity, duration) {
...
const laughterDetectorRegistry = createSessionRegistry((sessionId) => new MultiSignalLaughterDetector(sessionId), { name: 'LaughterDetector', cleanup: (detector) => detector.reset(), verbose: false });
registerGlobalRegistry(laughterDetectorRegistry);
export function getMultiSignalLaughterDetector(sessionId) {
    return laughterDetectorRegistry.get(sessionId);
}
export function resetMultiSignalLaughterDetector(sessionId) {
    laughterDetectorRegistry.reset(sessionId);
    log.debug({ sessionId }, '😂 Multi-signal laughter detector reset');
}
export function getActiveLaughterDetectorCount() {
    return laughterDetectorRegistry.getActiveCount();
}
//# sourceMappingURL=multi-signal-laughter.js.map