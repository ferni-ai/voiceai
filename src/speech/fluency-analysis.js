...
export class FluencyAnalyzer {
    ...
    analyze(text) {
        ...
        while ((prolongMatch = prolongRegex.exec(text)) !== null) {
            ...
            if (!['yeah', 'nooo', 'wooo', 'yay'].some((w) => prolongMatch[0].toLowerCase().includes(w))) {
        ...
        // Calculate totals
        const totalDisfluencies = counts.repetitions +
            counts.prolongations +
            counts.blocks +
            counts.revisions +
            counts.interjections +
        ...
        if (pattern !== 'normal') {
            log.debug({ pattern, fluency: overallFluency.toFixed(2), disfluencyRate: disfluencyRate.toFixed(1) }, '🗣️ Fluency pattern detected');
        }
        return result;
    }
    /**
    ...
    // PRIVATE HELPERS
    // ==========================================================================
    getContext(text, position, radius = 20) {
        const start = Math.max(0, position - radius);
        const end = Math.min(text.length, position + radius);
        return text.slice(start, end);
    }
    determinePattern(counts, wordCount) {
    ...
    generateInterpretation(pattern, counts) {
        switch (pattern) {
            case 'word_finding':
    ...
    generateGuidance(pattern, fluency) {
        switch (pattern) {
            case 'word_finding':
...
const fluencyAnalyzerRegistry = createSessionRegistry((sessionId) => new FluencyAnalyzer(), { name: 'FluencyAnalyzer', cleanup: (analyzer) => analyzer.reset(), verbose: false });
registerGlobalRegistry(fluencyAnalyzerRegistry);
export function getFluencyAnalyzer(sessionId) {
    return fluencyAnalyzerRegistry.get(sessionId);
}
export function resetFluencyAnalyzer(sessionId) {
    fluencyAnalyzerRegistry.reset(sessionId);
}
export function resetAllFluencyAnalyzers() {
    fluencyAnalyzerRegistry.resetAll();
}
export function getActiveFluencyAnalyzerCount() {
    return fluencyAnalyzerRegistry.getActiveCount();
}
export default FluencyAnalyzer;
//# sourceMappingURL=fluency-analysis.js.map