...
export class BackchannelingSystem {
    ...
    backchannelCount = 0;
    MIN_INTERVAL_MS = 5000; // Don't backchannel more than every 5 seconds
    /**
     * Determine if agent should backchannel (verbal nod)
     */
    shouldBackchannel(context) {
        const { userHasBeenSpeaking, userPausedBriefly, userEmotion, topicWeight, lastBackchannelTime, personaId, } = context;
     ...
     * Now uses persona-specific backchannels for distinct personalities!
     */
    getBackchannel(emotion, topicWeight, personaId) {
 ...
 */
const sessionBackchannelingSystems = new Map();
/**
 * Get or create a backchanneling system for a specific session
 */
export function getSessionBackchannelingSystem(sessionId) {
 ...
 */
export function resetSessionBackchannelingSystem(sessionId) {
    sessionBackchannelingSystems.delete(sessionId);
}
//# sourceMappingURL=backchanneling.js.map