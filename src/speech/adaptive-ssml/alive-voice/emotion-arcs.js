 ...
 */
export const EMOTION_ARC_PATTERNS = [
    // ==========================================================================
    ...
    {
        pattern: /\b(I'm so (?:excited|happy|thrilled) for you)([^.!?]*?)(just (?:make sure|remember|be careful))/gi,
    ...
    {
        pattern: /\b(congratulations|you did it|that's incredible|I'm so proud)([^.!?]*?)((?:now|next|going forward))/gi,
    ...
    {
        pattern: /\b(I'm (?:so |really )?happy|this is wonderful)([^.!?]*?)(thank (?:you|goodness)|I appreciate)/gi,
    ...
    {
        pattern: /\b(I (?:hear|understand|get) (?:you|that|what you're saying))([^.!?]*?)((?:let's|here's what|the thing is))/gi,
    ...
    {
        pattern: /\b(that (?:sounds|must be) (?:really |so )?(?:hard|difficult|tough))([^.!?]*?)(but (?:you|I (?:believe|know)))/gi,
    ...
    {
        pattern: /\b(I'm (?:so )?sorry|that's (?:really )?(?:sad|hard))([^.!?]*?)(but (?:remember|know that|you've))/gi,
        replacement: '<emotion value="sympathetic"/>$1$2<emotion value="affectionate"/>$3',
        name: 'sad_to_hope',
    },
    // Concern → reassurance (worried → calming)
    {
        pattern: /\b(I'm (?:a bit )?(?:worried|concerned) about)([^.!?]*?)(but (?:I think|it'll be|you can))/gi,
    ...
    // Skepticism → openness
    {
        pattern: /\b(I wasn't sure|I had my doubts)([^.!?]*?)(but (?:actually|I have to admit|it turns out))/gi,
    ...
    {
        pattern: /\b(I remember|back when|it reminds me of)([^.!?]*?)(and now|these days|looking back)/gi,
    ...
    {
        pattern: /\b(I've been thinking|when I reflect on|looking at my life)([^.!?]*?)(I (?:realize|understand|see))/gi,
    ...
    // Teasing → genuine
    {
        pattern: /\b(I'm (?:just )?(?:kidding|teasing|joking))([^.!?]*?)(but (?:seriously|really|honestly))/gi,
        replacement: '<emotion value="joking"/>$1$2<emotion value="affectionate"/>$3',
        name: 'teasing_to_genuine',
    },
    // Sarcasm → earnest
    {
        pattern: /\b(oh (?:sure|great|wonderful))([^.!?]*?)((?:but|no) (?:really|seriously|actually))/gi,
        replacement: '<emotion value="sarcastic"/>$1$2<emotion value="curious"/>$3',
        name: 'sarcasm_to_earnest',
    },
    // Playful banter → sincere
    {
        pattern: /\b(you're (?:ridiculous|too much|hilarious))([^.!?]*?)(but I (?:love|appreciate|adore))/gi,
        replacement: '<emotion value="joking"/>$1$2<emotion value="affectionate"/>$3',
    ...
    // Hurt → healing
    {
        pattern: /\b(it (?:really )?hurt|I felt (?:so )?(?:hurt|rejected))([^.!?]*?)(but (?:I'm|I've|time))/gi,
        replacement: '<emotion value="hurt"/>$1$2<emotion value="content"/>$3',
        name: 'hurt_to_healing',
    },
    // Fear → courage
    {
        pattern: /\b(I was (?:so )?(?:scared|afraid|terrified))([^.!?]*?)(but (?:I did it|I faced|I pushed through))/gi,
        replacement: '<emotion value="scared"/>$1$2<emotion value="triumphant"/>$3',
        name: 'fear_to_courage',
    },
    // Guilt → self-compassion
    {
        pattern: /\b(I felt (?:so )?guilty|I blamed myself)([^.!?]*?)(but (?:I've learned|I realize|I'm))/gi,
        replacement: '<emotion value="guilty"/>$1$2<emotion value="peaceful"/>$3',
        name: 'guilt_to_compassion',
    },
    // Disappointment → resilience
    {
        pattern: /\b(I was (?:so )?disappointed|it didn't work out)([^.!?]*?)(but (?:I'll|there's|next time))/gi,
    ...
    {
        pattern: /\b(I can't wait|I'm (?:so )?looking forward)([^.!?]*?)(it's going to be|I know it'll)/gi,
 ...
 */
export function applyEmotionArcs(text, _context) {
...
//# sourceMappingURL=emotion-arcs.js.map