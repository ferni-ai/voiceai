...
import { getEnergyMatchedPacing, getLateNightPacing, selectLaughterResponseSync, } from '../../humanization/behavior-loader.js';
import { getEmotionProfile } from '../../voice-manager/config.js';
import { applyPersonaSpeechTraitsSync, hasCustomSpeechTraits, } from '../persona-speech-traits-loader.js';
 ...
 */
export function applyPersonaFingerprint(text, context) {
    ...
    if (context.userEnergy && context.userEnergy !== 'neutral') {
        ...
        if (energyPacing) {
            ...
            log.debug({ personaId, energy: context.userEnergy, multiplier: energyPacing.speedMultiplier }, 'Applied energy matching');
        }
    }
    // =========================================================================
    ...
    if (Math.random() < fingerprint.thinkingSoundProbability &&
        ...
        const sound = fingerprint.thinkingSounds[Math.floor(Math.random() * fingerprint.thinkingSounds.length)];
