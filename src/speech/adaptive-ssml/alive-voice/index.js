...
// Nonverbals
export { NONVERBAL_CONFIG, getNonverbal, isNonverbalSupported, } from './nonverbals.js';
// Context Detection
 ...
 */
export function makeVoiceAlive(text, context = {}) {
    ...
    if (context.enableLaughter !== false) {
        ...
        const mapEnergyTo3Level = (energy) => {
            if (!energy)
                return undefined;
            if (energy === 'very_low' || energy === 'low')
                return 'low';
            if (energy === 'elevated' || energy === 'high')
                return 'high';
            return 'medium';
        };
        const laughResult = addContextualLaughter(result, {
