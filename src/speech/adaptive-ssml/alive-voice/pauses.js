 ...
 * Heavier topics get longer pauses for processing and presence.
 */
export function applyDynamicPauses(text, context) {
    ...
    if (!result.includes('<break')) {
        ...
        if (weight === 'heavy') {
            result = result.replace(/\b(important|crucial|matter|care|feel|hard|difficult|loss|grief)\b/gi, `<break time="${pauses.emphasis}ms"/>$1`);
        }
    }
    return result;
}
