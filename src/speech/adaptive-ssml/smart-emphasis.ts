...
// ============================================================================
// SMART EMPHASIS FUNCTION (Cartesia-compatible)
// ============================================================================
 ...
 */
export function applySmartEmphasis(text: string, options: EmphasisOptions = {}): string {
  ...
  for (const { pattern, pauseBefore } of EMPHASIS_PHRASE_PATTERNS) {
    ...
    if (pattern.test(result)) {
      ...
      result = result.replace(pauseRegex, (match) => {
        if (replaced || emphasisCount >= maxEmphasis) return match;
        replaced = true;
        emphasisCount++;
