 ...
 */
export function getAnticipatoryComfortSound(contentType) {
 ...
 */
export function detectHeavyContentType(text) {
    const lowerText = text.toLowerCase();
    // Grief signals
    if (/\b(died|passed away|lost|gone|funeral|grieving|miss them|miss her|miss him)\b/i.test(lowerText)) {
        return 'grief';
    }
    // Fear signals
    if (/\b(scared|terrified|afraid|anxious|panic|worried sick|can't stop thinking)\b/i.test(lowerText)) {
        return 'fear';
    }
    // Frustration signals
    if (/\b(so frustrated|can't believe|fed up|sick of|tired of|keeps happening)\b/i.test(lowerText)) {
        return 'frustration';
    }
    // General heavy content
    if (/\b(struggling|hard to|difficult|tough|breaking down|falling apart|don't know what)\b/i.test(lowerText)) {
        return 'heavyContent';
    }
    return null;
}
//# sourceMappingURL=anticipatory-comfort.js.map