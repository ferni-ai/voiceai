...
const AGENT_HUMOR_PATTERNS = [
  // Self-aware jokes
  { pattern: /\b(just kidding|jk|I'm kidding|kidding)\b/i, type: 'full' as const, confidence: 0.9 },
  {
    pattern: /\b(okay,? that was (bad|terrible|cheesy|corny))\b/i,
    type: 'self-deprecating' as const,
    confidence: 0.85,
  },
  { pattern: /\b(don't judge( me)?|no judgment)\b/i, type: 'soft' as const, confidence: 0.7 },
  {
  ...
  { pattern: /\b(that's (so |very )?you)\b/i, type: 'warm' as const, confidence: 0.65 },
  ...
  { pattern: /\b(shocking|what a surprise|who knew)\b/i, type: 'soft' as const, confidence: 0.55 },
  ...
  // After a punchline
  { pattern: /[.!?]\s*\.\.\.\s*$/i, type: 'soft' as const, confidence: 0.5 }, // Trailing "..."
  { pattern: /[!?]{2,}\s*$/i, type: 'full' as const, confidence: 0.6 }, // Multiple punctuation
];
 ...
 */
function recordLaugh(sessionId: string, turnCount: number): void {
 ...
 */
export function resetLaughterSession(sessionId: string): void {
...
export function decideLaughter(context: LaughterContext, sessionId = 'default'): LaughterDecision {
  ...
  const noLaugh: LaughterDecision = {
    shouldLaugh: false,
    laughType: 'none',
    placement: 'end',
    laughText: '',
  ...
  let bestMatch: {
    type: LaughterDecision['laughType'];
    confidence: number;
    position: number;
    reason: string;
  } | null = null;
    ...
    for (const { pattern, type, confidence } of AGENT_HUMOR_PATTERNS) {
      ...
      if (match && match.index !== undefined) {
        const adjustedConfidence =
          confidence * personaStyle.laughProbabilityBase * (comfortLevel + 0.5);
  ...
  const laughTypeConfig = LAUGH_TYPES[bestMatch.type as keyof typeof LAUGH_TYPES];
  ...
  const typeConfig = LAUGH_TYPES[bestMatch.type as keyof typeof LAUGH_TYPES] || LAUGH_TYPES.soft;
  ...
  // Determine placement
  let placement: LaughterDecision['placement'];
  ...
  return {
    shouldLaugh: true,
    laughType: bestMatch.type as LaughterDecision['laughType'],
    placement,
    laughText,
 ...
 */
export function applyLaughter(text: string, decision: LaughterDecision): string {
