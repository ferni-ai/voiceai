...
import { humanizeSpeech, quickHumanizeSync, preloadAllSpeechProfiles, } from '../humanization/index.js';
 ...
 */
async function loadPersonaTraits(personaId) {
    // Check cache first
 ...
 */
export async function preloadAllTraits() {
    if (preloaded)
        return;
    const personas = [
        'ferni',
 ...
 */
export function getPersonaTraitsSync(personaId) {
 ...
 */
export async function applyPersonaSpeechTraits(text, personaId, context = {}) {
    ...
    if (config) {
        try {
            ...
            log.debug({ personaId, originalLength: text.length, resultLength: result.length }, 'Applied persona speech traits (layer 1: hardcoded)');
        }
        catch (error) {
    ...
    try {
        ...
        if (humanized.wasHumanized) {
            ...
            log.debug({ personaId, features: humanized.features }, 'Applied persona speech traits (layer 2: JSON behaviors)');
        }
    }
    catch (error) {
 ...
 * @returns Enhanced text with persona speech patterns
 */
export function applyPersonaSpeechTraitsSync(text, personaId, context = {}) {
    ...
    if (config) {
        try {
            result = config.apply(result, emotion, baseSpeed, laughterCount);
            log.debug({ personaId, originalLength: text.length, resultLength: result.length }, 'Applied persona speech traits sync (layer 1: hardcoded)');
        }
        catch (error) {
 ...
 */
export function hasCustomSpeechTraits(personaId) {
    const withTraits = [
        'ferni',
        'peter-john',
        'maya-santos',
 ...
 */
export function getPersonasWithSpeechTraits() {
    return ['ferni', 'peter-john', 'maya-santos', 'alex-chen', 'jordan-taylor', 'nayan-patel'];
}
/**
 * Clear the trait registry (for testing).
 */
export function clearTraitRegistry() {
    traitRegistry.clear();
    preloaded = false;
}
//# sourceMappingURL=persona-speech-traits-loader.js.map