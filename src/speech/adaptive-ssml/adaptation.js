...
import { makeVoiceAlive } from './alive-voice.js';
import { applySuperhmanVoice, getLastEmotion, updateSuperhmanVoiceSession, } from './superhuman-voice.js';
// ============================================================================
// HELPERS
// ============================================================================
/**
 ...
 */
export function tagTextWithSsmlAdaptive(text, context, personaId) {
    ...
    getLogger().debug(`Persona-aware SSML (${effectivePersonaId}): speed=${context.baseSpeed.toFixed(2)}, energy=${context.energyMultiplier.toFixed(2)}`);
    // =========================================================================
    ...
    if (aliveResult.appliedFeatures.length > 0) {
        getLogger().debug({ features: aliveResult.appliedFeatures, personaId }, 'Applied alive voice enhancements');
    }
 ...
 */
export function tagTextWithSsmlSuperhuman(text, context, personaId) {
    if (!text || text.trim().length === 0) {
        return text;
    }
 ...
 */
function hasMalformedSsml(text) {
 ...
 */
function adjustExistingSsml(text, context) {
    ...
    try {
        ...
        result = result.replace(/<speed\s+ratio="([\d.]+)"\s*\/?>/gi, (match, ratio) => {
            ...
            if (isNaN(original))
                return match; // Skip if can't parse
 ...
 */
function applySpeedAdaptation(text, context) {
 ...
 */
function applyPauseAdaptation(text, context) {
 ...
 */
function applyWarmthAdjustment(text, context) {
