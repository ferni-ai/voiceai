...
const ECHO_PATTERNS = [
    // Time references (powerful for showing you caught the timeline)
    {
        ...
        extractor: (m) => `${capitalize(m[1])} ${m[2].trim()}${m[0].match(/years?|months?|weeks?|days?/)?.[0] || ''}...`,
    ...
    {
        pattern: /\b(my|your)\s+(mom|dad|mother|father|sister|brother|wife|husband|partner|friend|boss|coworker)\b/i,
 ...
 */
function extractEchoPhrase(userMessage) {
 ...
 */
function buildEchoSSML(phrase, emotion = 'affectionate') {
 ...
 */
function calculateOpeningPause(context) {
 ...
 */
function getEnergyMatchedOpener(personaId, energy) {
 ...
 */
export function addActivePresence(context) {
    const { sessionId, userMessage, agentResponse, userEnergy = 'medium', topicWeight = 'medium', turnCount = 0, personaId = 'ferni', } = context;
...
export function resetActivePresenceSession(sessionId) {
    sessionState.delete(sessionId);
}
export function resetAllActivePresenceSessions() {
    sessionState.clear();
}
// ============================================================================
// EXPORTS
...
//# sourceMappingURL=active-presence.js.map