 ...
 */
function getBreathStressContribution(breathPattern) {
 ...
 */
function getConcernStressContribution(concernLevel) {
    switch (concernLevel) {
        case 'crisis':
            return 1.0;
        case 'high':
 ...
 */
export function getStressAdaptationEngine(sessionId) {
 ...
 */
export function resetStressAdaptationEngine(sessionId) {
 ...
 */
export function getActiveStressAdaptationCount() {
    return engines.size;
}
// ============================================================================
 ...
 */
export function recordStressReading(sessionId, reading) {
 ...
 */
function calculateWeightedStress(reading) {
    ...
    return (reading.stressLevel * WEIGHTS.stressLevel +
 ...
 */
function calculateTargetAdaptation(engine) {
    ...
    if (averageStress < STRESS_ADAPTATION_CONFIG.STRESS_MILD) {
        ...
        const t = (averageStress - STRESS_ADAPTATION_CONFIG.STRESS_MILD) /
        ...
        const t = (averageStress - STRESS_ADAPTATION_CONFIG.STRESS_MODERATE) /
 ...
 */
export function calculateStressAdaptation(sessionId) {
    const engine = getStressAdaptationEngine(sessionId);
    ...
    if (Math.abs(delta) > STRESS_ADAPTATION_CONFIG.HYSTERESIS_THRESHOLD) {
        ...
        if (turnsSinceChange >= STRESS_ADAPTATION_CONFIG.COOLDOWN_TURNS) {
            ...
            if (delta > 0) {
                ...
                newLevel = Math.min(targetLevel, engine.adaptationLevel + STRESS_ADAPTATION_CONFIG.MAX_RAMP_UP_PER_TURN);
            }
            else {
                // Ramping down (stress decreasing)
                newLevel = Math.max(targetLevel, engine.adaptationLevel - STRESS_ADAPTATION_CONFIG.MAX_RAMP_DOWN_PER_TURN);
            }
 ...
 */
function adaptationLevelToParams(level) {
 ...
 */
export function applyStressAdaptationSsml(text, adaptation) {
 ...
 */
export function getStressAdaptationState(sessionId) {
    const engine = engines.get(sessionId);
    if (!engine) {
        return null;
    }
...
//# sourceMappingURL=stress-adaptation.js.map