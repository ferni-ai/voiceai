 ...
 */
export function applyPhasePersonality(text, phase, _context) {
    switch (phase) {
        case 'greeting':
 ...
 */
export function tagGreetingWithPersonality(text, options) {
    ...
    const tagged = text.replace(/\b(Hi|Hello|Good morning|Good afternoon|Good evening)\b/i, `$1<break time="150ms"/>`);
 ...
 */
export function tagSupportWithPersonality(text, options) {
 ...
 */
export function tagAdviceWithPersonality(text, options) {
    const speed = options.speedRatio || 0.82;
    const volume = options.volumeRatio || 0.95;
    // Add thoughtful pauses before key points
    const tagged = text.replace(/\b(Here's what I think|Let me tell you|The truth is|You know what)\b/i, `<break time="300ms"/>$1`);
    return `<speed ratio="${speed}"><volume ratio="${volume}">${tagged}</volume></speed>`;
}
/**
 * Tag wrap-up with personality - warm, affectionate farewell
 */
export function tagWrapUpWithPersonality(text, options) {
    const speed = options.speedRatio || 0.85;
    const emotion = options.emotion || 'affectionate';
    // Add warm pauses
    const tagged = text.replace(/\b(Take care|Good luck|Until next time|God bless)\b/i, `$1<break time="300ms"/>`);
    return `<emotion value="${emotion}"><speed ratio="${speed}">${tagged}</speed></emotion>`;
}
//# sourceMappingURL=phase-personality.js.map