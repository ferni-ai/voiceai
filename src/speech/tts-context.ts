...
export class TtsContextService {
  private contexts = new Map<string, TtsContextState>();
  ...
  getOrCreateContext(sessionId: string, personaId: string): TtsContextState {
    ...
    if (!this.contexts.has(key)) {
      ...
      getLogger().debug(
        { sessionId, personaId, contextId: context.contextId },
        '🎤 Created new TTS context for prosody continuity'
      );
    }
  ...
  getProsodyGuidance(sessionId: string, personaId: string): ProsodyGuidance {
    ...
    const guidance: ProsodyGuidance = {
      openingPause: false,
   ...
   */
  private updateEmotionalArc(context: TtsContextState): void {
...
let ttsContextInstance: TtsContextService | null = null;

export function getTtsContextService(): TtsContextService {
...
export function resetTtsContextService(): void {
