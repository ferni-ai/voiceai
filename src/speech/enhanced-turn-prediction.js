 ...
 */
export function detectPhraseBoundary(prosody, previousProsody) {
 ...
 */
export function estimateSyntacticCompleteness(text) {
...
export class EnhancedTurnPredictionService {
    ...
    lastProsody = null;
    userTypicalTurnDuration = CONFIG.DEFAULT_TURN_DURATION;
     ...
     */
    predict(prosody, transcriptSoFar, silenceDuration) {
        // Track turn start
     ...
     */
    recordTurnComplete(duration, hadFinalFall, wasQuestion, silenceBeforeNext) {
...
const enhancedTurnPredictorRegistry = createSessionRegistry((sessionId) => new EnhancedTurnPredictionService(sessionId), { name: 'EnhancedTurnPredictor', cleanup: (service) => service.reset(), verbose: false });
registerGlobalRegistry(enhancedTurnPredictorRegistry);
export function getEnhancedTurnPredictor(sessionId) {
    return enhancedTurnPredictorRegistry.get(sessionId);
}
export function resetEnhancedTurnPredictor(sessionId) {
    enhancedTurnPredictorRegistry.reset(sessionId);
    log.debug({ sessionId }, '🎯 Enhanced turn predictor reset');
}
export function getActiveEnhancedTurnPredictorCount() {
    return enhancedTurnPredictorRegistry.getActiveCount();
}
//# sourceMappingURL=enhanced-turn-prediction.js.map