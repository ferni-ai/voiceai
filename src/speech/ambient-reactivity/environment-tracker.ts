...
import type {
  AudioSnapshot,
  EnvironmentAcknowledgment,
  EnvironmentEvent,
  EnvironmentEventType,
  EnvironmentTrackerState,
  EnvironmentTtsAdjustment,
  EventSeverity,
} from './types.js';
...
const trackers = new Map<string, EnvironmentTracker>();

/**
 * Get or create environment tracker for a session
 */
export function getEnvironmentTracker(sessionId: string): EnvironmentTracker {
 ...
 */
export function getActiveEnvironmentTrackerCount(): number {
...
export class EnvironmentTracker {
   ...
   */
  private updateBaseline(snapshot: AudioSnapshot): void {
    const now = Date.now();

    // Remove old baseline samples
    this.baselineSamples = this.baselineSamples.filter(
      (s) => now - s.timestamp < ENVIRONMENT_CONFIG.BASELINE_MAX_AGE_MS
    );
   ...
   */
  private getBaselineNoiseDb(): number {
   ...
   */
  private detectEvent(snapshot: AudioSnapshot, now: number): EnvironmentEvent | null {
    ...
    if (
      snapshot.bandEnergies.brilliance > ENVIRONMENT_CONFIG.SPECTRAL.DOORBELL_HIGH_FREQ_THRESHOLD &&
   ...
   */
  private canEmitEvent(type: EnvironmentEventType, now: number): boolean {
   ...
   */
  private recordEvent(event: EnvironmentEvent): void {
   ...
   */
  private generateAcknowledgment(event: EnvironmentEvent): EnvironmentAcknowledgment | null {
  ...
  private cleanupOldEvents(now: number): void {
    this.recentEvents = this.recentEvents.filter(
      (e) => now - e.timestamp < ENVIRONMENT_CONFIG.EVENT_RETENTION_MS
    );
  }
  ...
  getTtsAdjustments(): EnvironmentTtsAdjustment {
    ...
    let adjustment: EnvironmentTtsAdjustment;
