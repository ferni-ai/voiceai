 ...
 */
export const SILENCE_PRESENCE_PHRASES = {
    ferni: [
 ...
 */
export function getPersonaSilencePresencePhrase(personaId) {
 ...
 */
export function getPersonaAnticipatoryComfortSound(personaId, contentType) {
    const normalized = normalizePersonaId(personaId);
    const personaSounds = ANTICIPATORY_COMFORT_SOUNDS[normalized] || ANTICIPATORY_COMFORT_SOUNDS.ferni;
 ...
 */
export function getPersonaEmotionalTransitionBridge(personaId, fromEmotion, toEmotion) {
    if (!fromEmotion || !toEmotion || fromEmotion === toEmotion)
        return null;
    const normalized = normalizePersonaId(personaId);
    const personaBridges = EMOTIONAL_TRANSITION_BRIDGES[normalized] || EMOTIONAL_TRANSITION_BRIDGES.ferni;
    const fromBridges = personaBridges[fromEmotion];
    if (!fromBridges)
        return null;
    return fromBridges[toEmotion] || null;
}
//# sourceMappingURL=superhuman-voice.js.map