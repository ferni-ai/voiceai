...
import { StreamStateMachine, StreamState, type TransitionResult } from './stream-state-machine.js';
...
const sessionStates = new Map<string, SanitizerStateSync>();

/**
 * Initialize sanitizer integration for a session.
 * Call this when creating the sanitizer transform stream.
 */
export function initializeSanitizerIntegration(sessionId: string): SanitizerStateSync {
  // Clean up existing if any
  cleanupSanitizerIntegration(sessionId);

  const state: SanitizerStateSync = {
 ...
 */
export function getSanitizerIntegration(sessionId: string): SanitizerStateSync | null {
 ...
 */
export function cleanupSanitizerIntegration(sessionId: string): void {
 ...
 */
export function notifyJsonStart(sessionId: string): void {
  const state = sessionStates.get(sessionId);
 ...
 */
export function notifyJsonComplete(sessionId: string, toolName: string): void {
  const state = sessionStates.get(sessionId);
  if (!state?.active) return;
  ...
  log.debug(
    { sessionId, toolName, state: state.stateMachine.getState() },
    'JSON complete notified'
  );
}
 ...
 */
export function notifyToolCompleted(sessionId: string, toolName: string, _success: boolean): void {
  const state = sessionStates.get(sessionId);
  if (!state?.active) return;
 ...
 */
export function notifySentenceBoundary(sessionId: string): void {
  const state = sessionStates.get(sessionId);
  if (!state?.active) return;
 ...
 */
export function notifyLeakageDetected(sessionId: string, pattern: string): void {
  const state = sessionStates.get(sessionId);
  if (!state?.active) return;
 ...
 */
export function shouldSuppressOutput(sessionId: string): boolean {
  const state = sessionStates.get(sessionId);
  if (!state?.active) return false;

  const currentState = state.stateMachine.getState();
  return (
    currentState === StreamState.EXECUTING_TOOL ||
 ...
 */
export function getCurrentState(sessionId: string): StreamState | null {
  const state = sessionStates.get(sessionId);
...
export function getStateMetrics(
  sessionId: string
): { state: StreamState; activeToolCount: number; isActive: boolean } | null {
  const state = sessionStates.get(sessionId);
 ...
 */
export function processChunkWithStateMachine(
  sessionId: string,
  chunk: string,
  _options: {
    isJsonStart?: boolean;
  ...
  const result: TransitionResult = state.stateMachine.processChunk(chunk);
