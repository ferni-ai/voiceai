 ...
 */
export async function loadUserAcknowledgmentPreferences(userId) {
    try {
        ...
        if (stored) {
            userPreferences.set(userId, {
                userId,
                preferredCategories: stored.preferredCategories,
            ...
            log.debug({ userId, sampleCount: stored.sampleCount }, 'Loaded user acknowledgment preferences');
        }
    }
    catch {
 ...
 */
export function recordAcknowledgmentFeedback(userId, phrase, category, wasPositive) {
    ...
    const avgLikedLength = prefs.preferredPhrases.reduce((sum, p) => sum + p.length, 0) /
    ...
    try {
        ...
        const { updateAcknowledgmentPreferences } = require('./acknowledgment-persistence.js');
    ...
    log.debug({ userId, phrase, wasPositive, sampleCount: prefs.sampleCount }, 'Recorded ack feedback');
}
// ============================================================================
 ...
 */
export function generateAcknowledgment(context) {
 ...
 */
function getPersonaAcknowledgments(personaId) {
 ...
 */
export function shouldAcknowledge(estimatedWaitMs, userId) {
...
//# sourceMappingURL=persona-acknowledgments.js.map