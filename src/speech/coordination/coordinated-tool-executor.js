...
import { generateAcknowledgment, shouldAcknowledge, getToolCategory, } from './persona-acknowledgments.js';
const log = createLogger({ module: 'coordinated-tool-executor' });
const toolTimings = new Map();
const toolDurations = new Map(); // For calculating percentiles
 ...
 */
export function getEstimatedDuration(toolId) {
 ...
 */
export function recordToolDuration(toolId, durationMs) {
 ...
 */
export async function executeToolWithCoordination(request, executor) {
    ...
    try {
        ...
        if (result.success && result.result && session) {
            ...
            if (result.speakDirectly) {
                ...
                log.info({ toolId, accepted: speakResult.accepted }, '🎤 Tool result queued for direct speech');
            }
            else {
 ...
 */
async function speakToolResultViaLLM(session, toolId, resultText) {
    try {
        const { safeGenerateReply, formatToolResult } = await import('../../agents/shared/safe-generate-reply.js');
 ...
 */
export function isSlowTool(toolId) {
 ...
 */
export function getToolTimingStats() {
    return new Map(toolTimings);
}
//# sourceMappingURL=coordinated-tool-executor.js.map