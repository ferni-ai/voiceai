...
export class PronunciationMemoryService {
     ...
     */
    handlePronunciationCorrection(message) {
        // Pattern: "It's pronounced X" or "[Name], pronounced X"
        const correctionMatch = message.match(/([A-Z][a-z]+)?\s*(?:it's|its)?\s*pronounced\s+(?:like\s+)?["']?([^"'\n.!?]+)["']?/i);
        if (correctionMatch) {
     ...
     */
    learnFromContext(term, phonetic, context, confidence = 0.7) {
        const entry = {
            text: term,
            phonetic,
            phoneticType: 'sounds_like',
    ...
    applyToText(text) {
        ...
        const sortedEntries = [...this.state.entries.values()].sort((a, b) => b.text.length - a.text.length);
    ...
    importState(state) {
        ...
        log.debug({ entryCount: state.entries.length, hasUserName: !!state.userName }, '🗣️ Imported pronunciation memory state');
    }
    /**
     * Reset the memory
     */
 ...
 */
export function analyzePronunciationNeeds(text) {
