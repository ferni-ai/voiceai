 ...
 */
export const MICRO_REACTIONS: Record<MicroReactionType, MicroReaction> = {
  gasp: {
    type: 'gasp',
 ...
 */
const CONTEXT_PATTERNS: Record<MicroReactionContext, RegExp[]> = {
 ...
 */
export function detectContext(text: string): MicroReactionContext | null {
  for (const [context, patterns] of Object.entries(CONTEXT_PATTERNS)) {
    for (const pattern of patterns) {
      if (pattern.test(text)) {
        return context as MicroReactionContext;
      }
    }
  }
  return null;
 ...
 */
export function detectContexts(text: string): MicroReactionContext[] {
  const contexts: MicroReactionContext[] = [];

  for (const [context, patterns] of Object.entries(CONTEXT_PATTERNS)) {
    for (const pattern of patterns) {
      if (pattern.test(text)) {
        contexts.push(context as MicroReactionContext);
 ...
 */
export function getReactionsForContext(context: MicroReactionContext): MicroReaction[] {
 ...
 * would feel forced.
 */
export function selectMicroReaction(
  text: string,
 ...
 */
export function getMicroReaction(type: MicroReactionType): MicroReaction {
...
const sessions = new Map<string, MicroReactionSession>();

/**
 * Get or create session
 */
export function getMicroReactionSession(sessionId: string): MicroReactionSession {
 ...
 */
export function recordReaction(sessionId: string, type: MicroReactionType): void {
 ...
 */
export function shouldUseReaction(sessionId: string): boolean {
 ...
 * Reset session
 */
export function resetMicroReactionSession(sessionId: string): void {
 ...
 */
export function getActiveMicroReactionSessionCount(): number {
