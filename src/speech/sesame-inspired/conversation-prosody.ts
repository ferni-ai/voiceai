 ...
 */
const EMOTION_INTENSITY: Record<CartesiaEmotion, number> = {
  // High intensity
  excited: 0.9,
  enthusiastic: 0.85,
  elated: 0.9,
 ...
 */
const HEAVY_EMOTIONS: CartesiaEmotion[] = [
  'sad',
  'dejected',
  'hurt',
  'guilty',
 ...
 */
export function getProsodyRecommendation(
  ...
  const recommendation: ConversationProsodyRecommendation = {
    baseSpeed: speed,
    baseVolume: volume,
 ...
 */
function buildReasonString(state: ConversationEmotionalState): string {
...
const sessions = new Map<string, ConversationEmotionalState>();

/**
 * Get current emotional state for a session
 */
export function getConversationState(sessionId: string): ConversationEmotionalState {
