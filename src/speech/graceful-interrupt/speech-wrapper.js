 ...
 */
const PERSONA_RECOVERY_OPENINGS = {
    ferni: {
        hard: [
 ...
 */
function getPersonaRecoveryOpening(personaId, interruptType) {
 ...
 * ```
 */
export function wrapSpeechWithInterruptAwareness(text, context) {
 ...
 */
export function markRecoveryComplete(sessionId) {
    endRecovery(sessionId);
}
/**
 * Check if we're currently in recovery phase
 */
export function isInRecoveryPhase(sessionId) {
    const state = getInterruptState(sessionId);
 ...
 */
export function createInterruptAwareTransform(context) {
...
//# sourceMappingURL=speech-wrapper.js.map