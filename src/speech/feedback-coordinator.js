 ...
 */
function createEmptyBudget(turnNumber) {
    return {
 ...
 */
export function canAddFeedback(sessionId, type, turnNumber) {
    ...
    if (budget.feedbackCount >= MAX_FEEDBACK_PER_TURN) {
        log.debug({ type, feedbackCount: budget.feedbackCount, max: MAX_FEEDBACK_PER_TURN }, 'Feedback blocked: turn limit reached');
        return false;
    }
    ...
    if (budget.lastFeedbackTime > 0 && timeSinceLast < MIN_FEEDBACK_INTERVAL_MS) {
        log.debug({ type, timeSinceLast, minInterval: MIN_FEEDBACK_INTERVAL_MS }, 'Feedback blocked: too soon');
        return false;
    }
 ...
 */
export function recordFeedback(sessionId, type) {
    const state = getSessionState(sessionId);
    const budget = state.currentTurn;
    ...
    log.debug({ type, feedbackCount: budget.feedbackCount, totalBackchannels: state.totalBackchannels }, 'Feedback recorded');
}
/**
 ...
 */
export function advanceTurn(sessionId, turnNumber) {
    const state = getSessionState(sessionId);
    state.currentTurn = createEmptyBudget(turnNumber);
 ...
 * @param sessionId - Session ID
 */
export function resetFeedbackCoordinator(sessionId) {
 ...
 */
export function resetAllFeedbackCoordinators() {
    sessionStates.clear();
 ...
 */
export function getFeedbackStats(sessionId) {
    const state = getSessionState(sessionId);
    return {
...
// HELPERS
// ============================================================================
function hasFeedbackThisTurn(budget, type) {
    switch (type) {
        case 'backchannel':
...
//# sourceMappingURL=feedback-coordinator.js.map