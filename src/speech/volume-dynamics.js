...
export class VolumeDynamicsTracker {
     ...
     */
    recordFromAudioSamples(samples, sampleRate, textSnippet) {
    ...
    computeState() {
        ...
        const onSensitiveTopic = currentRelativeVolume < 0.7 && acrossUtterancesTrend === 'getting_quieter';
        // Detect intensity increase
        const intensityIncreasing = currentRelativeVolume > 1.2 && acrossUtterancesTrend === 'getting_louder';
        // Generate interpretation
        const interpretation = this.generateInterpretation(currentLevel, acrossUtterancesTrend, onSensitiveTopic, intensityIncreasing);
        // Suggested agent volume
        let suggestedAgentVolume = 'normal';
        ...
        const confidence = Math.min(1, this.observations.length / 5) * (this.calibrationSamples >= 5 ? 1 : 0.7);
        const state = {
    ...
    determineWithinUtteranceTrend(obs) {
    ...
    determineAcrossUtterancesTrend() {
        if (this.observations.length < 3)
            return 'stable';
    ...
    generateInterpretation(level, trend, onSensitive, intensityIncreasing) {
        if (onSensitive) {
...
const volumeDynamicsRegistry = createSessionRegistry((sessionId) => new VolumeDynamicsTracker(), { name: 'VolumeDynamics', cleanup: (tracker) => tracker.reset(), verbose: false });
registerGlobalRegistry(volumeDynamicsRegistry);
export function getVolumeDynamicsTracker(sessionId) {
    return volumeDynamicsRegistry.get(sessionId);
}
export function resetVolumeDynamicsTracker(sessionId) {
    volumeDynamicsRegistry.reset(sessionId);
}
export function resetAllVolumeDynamicsTrackers() {
    volumeDynamicsRegistry.resetAll();
}
export function getActiveVolumeDynamicsCount() {
    return volumeDynamicsRegistry.getActiveCount();
}
export default VolumeDynamicsTracker;
//# sourceMappingURL=volume-dynamics.js.map