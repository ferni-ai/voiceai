...
import { CARTESIA_MODEL, createCartesiaTTS, DEFAULT_VOICE_IDS, getDefaultTTSProvider, } from './cartesia-core.js';
import { createBTCWTTS, getBTCWVoiceIdForPersona } from './btcw-core.js';
export const DEFAULT_ACCENT = 'american';
export const SUPPORTED_ACCENTS = ['american', 'british', 'australian', 'indian'];
// ============================================================================
// LAZY-LOADED LOGGER
// ============================================================================
...
async function getLogger() {
    if (!_logger) {
        try {
            ...
            _logger = {
                info: (data, msg) => console.log(`[PersonaAwareTTS] ${msg}`, data),
                debug: (data, msg) => console.debug(`[PersonaAwareTTS] ${msg}`, data),
                warn: (data, msg) => console.warn(`[PersonaAwareTTS] ${msg}`, data),
                error: (data, msg) => console.error(`[PersonaAwareTTS] ${msg}`, data),
            };
        }
    }
    return _logger;
...
export class PersonaAwareTTS extends tts.TTS {
    ...
    constructor(personaName, voiceConfig) {
        ...
        this.provider = voiceConfig.provider || getDefaultTTSProvider();
        ...
        if (this.provider === 'btcw') {
            ...
            this.personaTTS = createBTCWTTS(btcwVoice, {
                ...
                defaultEmotion: voiceConfig.btcwDefaultEmotion || 'warm',
            });
        }
        else {
     ...
     */
    switchVoice(newPersonaName, newVoiceId, newAccent) {
     ...
     */
    switchAccent(newAccent, personaId) {
        if (this.accent === newAccent) {
            log('debug', { accent: newAccent }, 'Already using this accent');
            return;
        }
        log('warn', { from: this.accent, to: newAccent, persona: this.personaName }, '⚠️ switchAccent() is deprecated - calling switchToLocalizedAccent() instead');
     ...
     */
    async switchToLocalizedAccent(newAccent, personaId) {
        if (this.accent === newAccent) {
            log('debug', { accent: newAccent }, 'Already using this accent');
            return true;
        }
        ...
        try {
            ...
            const { getLocalizedVoiceId } = await import('../../services/voice/cartesia-voice-localization.js');
     ...
     */
    performVoiceSwitch(newPersonaName, newVoiceId, newAccent) {
        if (this.isSwitching) {
        ...
        log('info', { from: oldPersona, to: newPersonaName, oldVoiceId, newVoiceId, provider: this.provider }, '🔄 Voice switched');
    ...
    synthesize(text) {
        ...
        log('debug', { persona: this.personaName, voiceId: this.voiceId, hadSsml: cleanText !== text }, 'TTS synthesize');
    ...
    stream() {
        ...
        const underlyingStream = this.trackStream(() => this.personaTTS.stream());
 ...
 * ```
 */
export function createPersonaAwareTTS(personaName, voiceConfig) {
...
//# sourceMappingURL=persona-aware.js.map