 ...
 */
export const DEFAULT_BTCW_ENDPOINT = process.env.BTCW_ENDPOINT || 'https://btcw-cosyvoice-1031920444452.us-central1.run.app';
/**
...
export class BTCWTTS {
     ...
     */
    stream() {
        return new BTCWSynthesizeStream(this.#endpoint, this.#voiceId, this.#defaultEmotion, this.sampleRate, this.numChannels, this.#superhumanOptions, this.#apiKey, this.#idToken, this.#getIdToken);
    }
    /**
    ...
    async synthesize(text) {
        ...
        return new BTCWChunkedStream(this.#endpoint, this.#voiceId, text, this.#defaultEmotion, this.sampleRate, this.numChannels, this.#superhumanOptions, authHeaders);
    }
    /**
     * Health check
     */
 ...
 */
export class BTCWSynthesizeStream {
    #endpoint;
    #voiceId;
    #emotion;
    #sampleRate;
    ...
    constructor(endpoint, voiceId, emotion, sampleRate, numChannels, superhumanOptions, apiKey, idToken, getIdToken) {
     ...
     */
    updateInputStream(textOrStream) {
        ...
        if (textOrStream && typeof textOrStream === 'object' && 'getReader' in textOrStream) {
            ...
            this.#consumeTextStream(textOrStream);
            return;
        }
        // From here on, treat as string
        let text = textOrStream;
        // Validate text before processing
        if (text === undefined || text === null) {
        ...
        while (this.#textQueue.length > 0) {
            ...
            try {
                ...
                for (let i = 0; i < allAudioData.length; i += CHUNK_SIZE_SAMPLES) {
                    ...
                    const frame = new AudioFrame(chunkData, this.#sampleRate, this.#numChannels, samplesPerChannel);
                    frames.push(frame);
                }
 ...
 * Yields SynthesizedAudio objects that LiveKit expects
 */
export class BTCWChunkedStream {
    #endpoint;
    #voiceId;
    #text;
    #emotion;
    ...
    #requestId = generateId();
    #segmentId = generateId();
    constructor(endpoint, voiceId, text, emotion, sampleRate, numChannels, superhumanOptions, authHeaders = { 'Content-Type': 'application/json' }) {
        this.#endpoint = endpoint;
        this.#voiceId = voiceId;
        this.#text = text;
        this.#emotion = emotion;
 ...
 */
export function createBTCWTTS(voiceId, options) {
    return new BTCWTTS({
        voice: voiceId,
        ...options,
    });
 ...
 */
export function createBTCWTTSFromEnv(voiceId) {
 ...
 */
export async function prewarmBTCWTTS(voiceId = 'ferni') {
 ...
 */
export function isBTCWTTSPrewarmed() {
    return _prewarmState !== null;
}
/**
 * Get prewarmed BTCW TTS instance (consumes it)
 */
export function getPrewarmedBTCWTTS() {
    if (_prewarmState) {
 ...
 */
export function clearPrewarmedBTCWTTS() {
    _prewarmState = null;
 ...
 */
export function cartesiaVoiceToBTCW(cartesiaVoiceId) {
    // Import the voice IDs from config to match
    const CARTESIA_TO_BTCW = {
 ...
 */
export function getBTCWVoiceIdForPersona(personaId) {
    const normalized = personaId.toLowerCase();
    const PERSONA_TO_BTCW = {
        ferni: 'ferni',
        'jack-b': 'ferni',
        coach: 'ferni',
        'life-coach': 'ferni',
...
//# sourceMappingURL=btcw-core.js.map