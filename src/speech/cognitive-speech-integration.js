...
import { calculateCognitiveSpeechAdjustments, applyCognitiveAdjustments, buildPauseSSML, getCognitiveThinkingSound, } from './cognitive-speech.js';
 ...
 */
export function applyCognitiveSpeechAdjustments(input, sessionId) {
    const sessionState = getOrCreateSessionState(sessionId);
    sessionState.totalTurns++;
    const { speechContext, baseCharacteristics, cognitiveGuidance, emotionalWeight, inReasoningChain = false, chainStep, chainTotal, } = input;
    // Build cognitive speech context
    const cognitiveSpeechContext = {
    ...
    const adjustments = calculateCognitiveSpeechAdjustments(baseCharacteristics, cognitiveSpeechContext);
    ...
    let thinkingSound;
    const shouldUseThinkingSound = cognitiveGuidance.showReasoning &&
    ...
    if (shouldUseThinkingSound) {
        thinkingSound = getCognitiveThinkingSound(cognitiveGuidance.recommendedApproach, cognitiveGuidance.confidenceLevel);
 ...
 */
export function buildCognitiveSSML(text, cognitiveResult) {
    let result = text;
 ...
 */
export function getCognitiveSpeechStats(sessionId) {
 ...
 */
export function clearCognitiveSpeechState(sessionId) {
 ...
 */
export function getReasoningStyleSpeechPreset(style) {
    switch (style) {
        case 'analytical':
            return {
...
//# sourceMappingURL=cognitive-speech-integration.js.map