...
import { getResponseAnticipationService, generatePrefetchContext, } from './response-anticipation.js';
...
export class RealtimePreemptiveProcessor {
     ...
     */
    processPartialTranscript(event) {
        if (!this.enabled) {
            return {
                type: 'none',
                anticipation: null,
        ...
        // Check for partial anticipation (context preparation)
            ...
            const prefetchContext = generatePrefetchContext(this.recentUserMessages, this.emotionalState, this.currentTopic);
            action = {
     ...
     */
    processWithProsody(event, prosody, silenceDuration) {
 ...
 * ```
 */
export function hookRealtimePreemptive(sessionId, session, // voice.AgentSession - avoid importing
callback) {
    ...
    sessionAny.on('user_input_transcribed', (event) => {
        const ev = event;
...
const realtimePreemptiveRegistry = createSessionRegistry((sessionId) => new RealtimePreemptiveProcessor(sessionId), { name: 'RealtimePreemptive', cleanup: (processor) => processor.reset(), verbose: false });
registerGlobalRegistry(realtimePreemptiveRegistry);
export function getRealtimePreemptiveProcessor(sessionId) {
    return realtimePreemptiveRegistry.get(sessionId);
}
export function resetRealtimePreemptiveProcessor(sessionId) {
    realtimePreemptiveRegistry.reset(sessionId);
    log.debug({ sessionId }, '⚡ Realtime preemptive processor reset');
}
export function getActiveRealtimePreemptiveCount() {
    return realtimePreemptiveRegistry.getActiveCount();
}
//# sourceMappingURL=realtime-preemptive-patch.js.map