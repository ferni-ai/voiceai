...
import { detectPitchBatch, calculateRms, calculateMean, calculateVariance, calculateStdDev, isNativeAudioDspAvailable, } from './audio-dsp/index.js';
// ============================================================================
// RUST ACCELERATION (VIA AUDIO DSP MODULE)
// ============================================================================
/** Check if Rust acceleration is available for voice tremor detection */
export function isNativeTremorDetectionAvailable() {
...
export class VoiceTremorDetector {
    ...
    analyzeAudio(samples, sampleRate) {
        ...
        const emotionalIndicator = this.interpretEmotionalState(primaryType, intensity, possibleTears, possibleAnxiety);
    ...
    getStabilityProfile() {
        ...
        if (avgStability > 0.8)
            ...
            overallStability = 'slightly_unstable';
        else if (avgStability > 0.4)
            overallStability = 'unstable';
        else
            overallStability = 'very_unstable';
        // Determine trend
    ...
    // PRIVATE HELPERS
    // ==========================================================================
    detectTremorEvents(samples, sampleRate) {
        const events = [];
        ...
        const pitches = pitchResults.map((r) => r.pitchHz);
        const energies = [];
        ...
        for (let i = 3; i < pitches.length - 3; i++) {
            ...
            if (stdDev > TREMOR_THRESHOLDS.MIN_PITCH_VARIANCE) {
                ...
                if (tremorFreq >= TREMOR_THRESHOLDS.TREMOR_FREQ_MIN &&
            ...
            if (mean > TREMOR_THRESHOLDS.STRAIN_PITCH_THRESHOLD &&
    ...
    aggregateEvents(events) {
        ...
        if (intensityCounts.get('pronounced') >= 1)
            intensity = 'pronounced';
        else if (intensityCounts.get('noticeable') >= 2)
            intensity = 'noticeable';
        else if (intensityCounts.get('noticeable') >= 1)
            intensity = 'noticeable';
        return { primaryType, intensity };
    }
    detectPossibleTears(events) {
    ...
    detectPossibleAnxiety(events) {
        // Indicators: high-frequency tremor, strain
        const highFreqTremors = events.filter((e) => e.type === 'tremor' && e.tremorFrequency && e.tremorFrequency > 6).length;
        const strains = events.filter((e) => e.type === 'strain').length;
        return highFreqTremors >= 2 || strains >= 2 || (highFreqTremors >= 1 && strains >= 1);
    }
    interpretEmotionalState(type, intensity, tears, anxiety) {
        if (tears) {
    ...
    generateResponse(type, tears, anxiety) {
        if (tears) {
...
const voiceTremorRegistry = createSessionRegistry((sessionId) => new VoiceTremorDetector(), { name: 'VoiceTremor', cleanup: (detector) => detector.reset(), verbose: false });
registerGlobalRegistry(voiceTremorRegistry);
export function getVoiceTremorDetector(sessionId) {
    return voiceTremorRegistry.get(sessionId);
}
export function resetVoiceTremorDetector(sessionId) {
    voiceTremorRegistry.reset(sessionId);
}
export function resetAllVoiceTremorDetectors() {
    voiceTremorRegistry.resetAll();
}
export function getActiveVoiceTremorCount() {
    return voiceTremorRegistry.getActiveCount();
}
export default VoiceTremorDetector;
//# sourceMappingURL=voice-tremor.js.map