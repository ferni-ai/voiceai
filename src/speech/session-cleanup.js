...
import { resetAnticipatorySession, resetConversationState, resetDisfluencySession, resetMicroReactionSession, resetSesamePipeline, } from './sesame-inspired/index.js';
 ...
 */
const cleaningUpSessions = new Set();
/**
 ...
 */
export function registerSpeechSession(sessionId) {
 ...
 */
export function getActiveSpeechSessionCount() {
    return activeSessions.size;
}
/**
 * Get all active session IDs (for debugging)
 */
export function getActiveSpeechSessions() {
 ...
 */
export function cleanupSpeechSession(sessionId, options = {}) {
    ...
    if (cleanedAt) {
        ...
        if (ageMs < CLEANED_SESSION_TTL_MS) {
            log.debug({ sessionId, cleanedAgoMs: ageMs }, '⏭️ Speech session already cleaned recently, skipping');
            return;
        }
 ...
 */
export function cleanupAllSpeechSessions(reason = 'shutdown') {
 ...
 */
export async function emergencySpeechCleanup() {
    ...
    const safeClearAll = async (name, fn) => {
        try {
            await fn();
    ...
    await Promise.all([
        safeClearAll('audioProsody', async () => {
            ...
            if ('resetAllAudioProsodyAnalyzers' in m) {
                m.resetAllAudioProsodyAnalyzers();
            }
        }),
    ...
    log.warn({ successCount, totalCount }, `⚠️ Emergency speech cleanup complete (${successCount}/${totalCount} services cleared)`);
}
// ============================================================================
// EXPORTS
// ============================================================================
