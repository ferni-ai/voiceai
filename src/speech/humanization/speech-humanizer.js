...
import { loadSpeechProfile, selectImperfection, selectThinkingSound, selectBackchannel, selectBreathSound, getInjectionConfig, 
// Sync accessors
areSpeechProfilesPreloaded, selectThinkingSoundSync, selectImperfectionSync, selectBreathSoundSync, 
// New humanization features
selectLaughterResponse, selectLaughterResponseSync, isLateNightHours, getLateNightPacing, } from './behavior-loader.js';
 ...
 */
export async function humanizeSpeech(text, context) {
    ...
    try {
        ...
        if (context.userText && context.conversationCount !== undefined) {
            ...
            if (triggers.length > 0) {
                const callback = selectCallback(triggers, personaId, context.conversationCount, context.usedCallbacks);
                if (callback) {
        ...
        for (const tryBehavior of behaviorsToTry) {
            if (injectedCount >= config.maxBehaviorsPerResponse)
                break;
        ...
        if (appliedBehaviors.length > 0) {
            log.debug({ personaId, count: appliedBehaviors.length, features }, 'Applied speech humanization');
        }
        return {
            text: result,
 ...
 */
function injectBehavior(text, behavior, minCharsBetween) {
    ...
    switch (position) {
        ...
        case 'inline':
            ...
            if (breakPoints.length > 0) {
                ...
                const validBreaks = breakPoints.filter((bp) => bp > minCharsBetween && bp < text.length - minCharsBetween);
 ...
 */
function shouldAddThinkingSound(context) {
    // More likely when:
    ...
    if (context.content.isQuestion)
        return true;
    if (context.emotional.agentTone === 'curious')
        return true;
    if (context.emotional.userEmotion === 'reflective')
        return true;
 ...
 */
function shouldAddBreathSound(context) {
    ...
    if (context.emotional.userEmotion === 'distressed')
        return Math.random() < 0.5;
    if (context.emotional.userEmotion === 'anxious')
        return Math.random() < 0.5;
 ...
 */
export async function quickHumanize(text, personaId, turnNumber) {
    const context = {
        personaId,
        emotional: {},
        content: {},
 ...
 */
export function quickHumanizeSync(text, personaId, context) {
    ...
    try {
        ...
        if (context?.isCelebration) {
            const extendedContext = selectionContext;
 ...
 */
function mapEmotionToUserEmotion(emotion) {
    if (!emotion)
        return undefined;
 ...
 */
function mapEmotionToAgentTone(emotion) {
    if (!emotion)
        return undefined;
    switch (emotion.toLowerCase()) {
        case 'sympathetic':
 ...
 */
export async function getAvailableCategories(personaId) {
...
//# sourceMappingURL=speech-humanizer.js.map