 ...
 */
async function loadBehaviorFile(personaId, fileName) {
    try {
 ...
 */
export async function loadSpeechProfile(personaId) {
    // Check cache
    ...
    const [imperfections, thinkingSounds, backchannels, breathSounds, lateNightPresence, callbacks, laughterContagion, energyMatching, celebrations, catchphrases, anticipation,] = await Promise.all([
        loadBehaviorFile(personaId, 'speech-imperfections.json'),
        loadBehaviorFile(personaId, 'thinking-sounds.json'),
        loadBehaviorFile(personaId, 'backchannels.json'),
        loadBehaviorFile(personaId, 'breath-sounds.json'),
        loadBehaviorFile(personaId, 'late-night-presence.json'),
        loadBehaviorFile(personaId, 'callbacks.json'),
        loadBehaviorFile(personaId, 'laughter-contagion.json'),
        loadBehaviorFile(personaId, 'energy-matching.json'),
        loadBehaviorFile(personaId, 'celebrations.json'),
        loadBehaviorFile(personaId, 'catchphrases.json'),
        loadBehaviorFile(personaId, 'anticipation.json'),
    ]);
    const profile = {
        personaId,
        imperfections,
 ...
 */
export function clearSpeechProfileCache(personaId) {
    if (personaId) {
 ...
 */
function getRandomPhrase(phrases, seed) {
 ...
 */
function matchesContext(usageRules, context) {
    if (!usageRules) {
 ...
 */
export async function selectImperfection(personaId, category, context) {
    const profile = await loadSpeechProfile(personaId);
    if (!profile.imperfections) {
        return null;
    }
    // Get phrases for the category
    const phrases = profile.imperfections[category];
 ...
 */
export async function selectThinkingSound(personaId, context) {
    const profile = await loadSpeechProfile(personaId);
 ...
 */
export async function selectBackchannel(personaId, context) {
    const profile = await loadSpeechProfile(personaId);
 ...
 */
export function getInjectionConfig(personaId) {
 ...
 */
export async function preloadAllSpeechProfiles() {
    const personas = [
        'ferni',
        'maya-santos',
        'jordan-taylor',
 ...
 */
export function getSpeechProfileSync(personaId) {
    const cached = profileCache.get(personaId);
    if (cached && isCacheValid(cached)) {
        return cached.profile;
    }
 ...
 */
export function areSpeechProfilesPreloaded() {
    return profileCache.size > 0;
}
/**
 * Select a thinking sound synchronously (uses cached profile)
 */
export function selectThinkingSoundSync(personaId, context) {
 ...
 */
export function selectImperfectionSync(personaId, category, context) {
    const profile = getSpeechProfileSync(personaId);
    if (!profile?.imperfections) {
        return null;
    }
 ...
 */
export async function selectBreathSound(personaId, context) {
    const profile = await loadSpeechProfile(personaId);
    ...
    const phrases = profile.breathSounds[category];
    if (!Array.isArray(phrases)) {
        return null;
    }
    const phrase = getRandomPhrase(phrases, context.randomSeed);
 ...
 */
export function selectBreathSoundSync(personaId, context) {
    const profile = getSpeechProfileSync(personaId);
    if (!profile?.breathSounds) {
        return null;
    }
 ...
 */
export async function selectLaughterResponse(personaId, context) {
    const profile = await loadSpeechProfile(personaId);
 ...
 */
export function selectLaughterResponseSync(personaId, context) {
    const profile = getSpeechProfileSync(personaId);
    if (!profile?.laughterContagion) {
        return null;
    }
 ...
 */
export function getLateNightPacing(personaId) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function getLateNightGreeting(personaId, seed) {
    const profile = getSpeechProfileSync(personaId);
    if (!profile?.lateNightPresence || !isLateNightHours()) {
        return null;
    }
 ...
 */
export function getEnergyMatchedPacing(personaId, userEnergyLevel) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function shouldUseCallback(personaId, callbackId, conversationCount) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
function selectBreathCategory(personaId, context) {
    const { emotional, content } = context;
    // Persona-specific category selection
    switch (personaId) {
        case 'ferni':
            if (emotional.isVulnerable)
                return 'holding_space';
            if (content.isComforting)
                return 'after_user_shares';
            if (emotional.isLateNight)
                return 'grounding';
            return 'contemplative_breath';
        case 'maya-santos':
            if (content.isCelebration)
                return 'before_celebration';
            if (emotional.isVulnerable)
                return 'empathetic_sigh';
            if (content.isComforting)
                return 'grounding';
        ...
        case 'nayan-patel':
            if (emotional.isVulnerable)
                return 'holding_space';
            if (emotional.isLateNight)
                return 'meditative_breath';
            return 'before_wisdom';
        case 'jordan-taylor':
            if (content.isCelebration)
                return 'celebration_build';
            if (emotional.isVulnerable)
                return 'grounding';
 ...
 */
export function detectCelebrationIntensity(userText) {
 ...
 */
export function selectCelebration(personaId, intensity, seed) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function selectCelebrationSync(personaId, context) {
    if (!context.userText)
        return null;
 ...
 */
export const CATCHPHRASE_TRIGGERS = {
 ...
 */
export function selectCatchphrase(personaId, userText, conversationCount, usedThisSession = new Set()) {
    const profile = getSpeechProfileSync(personaId);
    ...
    if (secondary_signatures?.phrases) {
        for (const sig of secondary_signatures.phrases) {
            if (usedThisSession.has(sig.phrase))
                continue;
 ...
 */
export function getPowerfulQuestion(personaId, seed) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function getPartnershipPhrase(personaId, seed) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function getSessionOpeningPhrase(personaId, context = {}, seed) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function getTopicCallbackPhrase(personaId, seed) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function getFutureLookingPhrase(personaId, type = 'curiosity', seed) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function getContinuityMarker(personaId, type = 'growth', seed) {
    const profile = getSpeechProfileSync(personaId);
 ...
 */
export function getPendingItemPhrase(personaId, type = 'goal', seed) {
    const profile = getSpeechProfileSync(personaId);
...
//# sourceMappingURL=behavior-loader.js.map