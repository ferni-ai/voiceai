 ...
 */
export function mapContextToEmotion(context) {
    ...
    switch (agentIntent) {
        ...
        case 'thinking':
            return 'curious';
        case 'explaining':
            if (relationshipStage === 'trusted_advisor')
                return 'calm';
            return 'content';
        case 'celebrating':
        ...
        case 'joking':
            if (relationshipStage === 'stranger')
                return 'content'; // Don't overdo it
            return 'comedic';
        case 'remembering':
            return 'nostalgic';
        case 'questioning':
 ...
 */
export function getEmotionTransition(fromEmotion, toEmotion) {
    ...
    for (const [group, emotions] of Object.entries(emotionGroups)) {
        if (emotions.includes(fromEmotion))
            fromGroup = group;
        if (emotions.includes(toEmotion))
            toGroup = group;
    }
