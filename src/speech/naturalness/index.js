...
import { getStressAdaptationEngine, resetStressAdaptationEngine, recordStressReading, calculateStressAdaptation, } from '../adaptive-ssml/stress-adaptation.js';
import { getVoicePatternEngine, resetVoicePatternEngine, recordVoiceObservation, getRecommendedAgentWpm, getRecommendedTurnGap, initializeVoicePatterns, persistVoicePatterns, } from '../../conversation/humanization/voice-pattern-learning.js';
...
import { combineAdjustments, mergeContextInjections, } from './combine-adjustments.js';
 ...
 */
export function getNaturalnessEngine(sessionId, userId) {
 ...
 */
export function resetNaturalnessEngine(sessionId) {
    const engine = engines.get(sessionId);
    if (engine) {
        log.debug({ sessionId, turnsProcessed: engine.turnsProcessed }, '🌊 Resetting NaturalnessEngine');
    }
 ...
 */
export function getActiveNaturalnessEngineCount() {
    return engines.size;
}
// ============================================================================
// INITIALIZATION
 ...
 */
export async function initializeNaturalnessEngine(sessionId, userId) {
 ...
 */
export async function persistNaturalnessData(sessionId) {
    try {
 ...
 */
export function processTurn(sessionId, input) {
    const engine = engines.get(sessionId);
    if (!engine) {
    ...
    if (input.audio) {
        ...
        if (stressAdaptation.adaptationLevel > 0.1) {
            ...
            sources.push({
                ...
                warmthLevel: stressAdaptation.warmthLevel === 'high'
                    ? 'very_warm'
    ...
    if (rapportScore.level === 'needs_attention' ||
 ...
 */
function buildStressReading(audio) {
    return {
        timestamp: Date.now(),
 ...
 */
function mapBreathPattern(pattern) {
    switch (pattern) {
        case 'shallow':
            return 'gasp';
        case 'deep':
 ...
 */
function mapConcernLevel(level) {
 ...
 */
function buildVoiceObservation(context) {
    return {
 ...
 */
function buildRapportObservation(context) {
 ...
 */
export function getNaturalnessEngineState(sessionId) {
    const engine = engines.get(sessionId);
    if (!engine)
        return null;
 ...
 */
export function getLastNaturalnessResult(sessionId) {
 ...
 */
export function processAudioFrame(sessionId, data, sampleRate, isSpeech) {
    const ambientService = getAmbientAwarenessService(sessionId);
