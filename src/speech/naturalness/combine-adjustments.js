 ...
 */
export function combineAdjustments(sources) {
    if (sources.length === 0) {
        return {
    ...
    // Max for volume
    const volumeBoost = clamp(Math.max(0, ...sources.map((s) => s.volumeBoost ?? 0)), COMBINE_CONFIG.VOLUME_BOOST.MIN, COMBINE_CONFIG.VOLUME_BOOST.MAX);
    // OR for clarity
    const clarityMode = sources.some((s) => s.clarityMode === true);
    // Max for pause
    const extraPauseMs = clamp(Math.max(0, ...sources.map((s) => s.extraPauseMs ?? 0)), COMBINE_CONFIG.EXTRA_PAUSE.MIN, COMBINE_CONFIG.EXTRA_PAUSE.MAX);
    // Highest priority with warmth wins
    let warmthLevel = 'neutral';
 ...
 */
export function mergeContextInjections(injections) {
...
//# sourceMappingURL=combine-adjustments.js.map