 ...
 */
export async function analyzeAudio(sessionId, context) {
    // If we have raw audio samples, use the full analyzers
    if (context.audioSamples && context.sampleRate) {
        const breath = getBreathDetector(sessionId).analyzeAudio(context.audioSamples, context.sampleRate);
        const tremor = getVoiceTremorDetector(sessionId).analyzeAudio(context.audioSamples, context.sampleRate);
        const volumeDynamics = getVolumeDynamicsTracker(sessionId).recordFromAudioSamples(context.audioSamples, context.sampleRate, context.text.slice(0, 50));
        const energyDynamics = getEnergyDynamicsTracker(sessionId).analyzeFromAudio(context.audioSamples, context.sampleRate, context.text);
 ...
 */
function deriveAudioFromProsody(prosody, _text) {
 ...
 */
export async function analyzeText(sessionId, context) {
    const cognitiveLoad = getCognitiveLoadDetector(sessionId).analyzeUtterance(context.text, context.durationMs ?? 0, context.pauseInfo);
 ...
 */
export async function analyzeConversation(sessionId, context) {
 ...
 */
export function resetAllAnalyzers(sessionId) {
    // Audio
