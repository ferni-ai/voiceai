...
import { calculateOverallConfidence, calculateSsmlSuggestions, determinePossibleDistress, determineShouldGiveSpace, determineShouldSlowDown, generateAgentGuidance, generateOverallAssessment, identifyPrioritySignals, synthesizeEmotionalUndercurrent, } from './synthesis.js';
...
export class HumanListeningPipeline {
    ...
    async analyze(context) {
        ...
        const overallAssessment = generateOverallAssessment(audio, text, conversation, emotionalUndercurrent);
     ...
     */
    quickAnalyze(text, turnNumber) {
        const cognitiveLoad = getCognitiveLoadDetector(this.sessionId).analyzeUtterance(text, turnNumber);
        ...
        const shouldSlowDown = cognitiveLoad.level === 'high' ||
