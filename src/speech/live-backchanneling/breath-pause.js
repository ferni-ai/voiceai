...
export class BreathPauseDetector {
    ...
    recentEnergyLevels = [];
    adaptiveSilenceThreshold = BREATH_PAUSE_CONFIG.SILENCE_THRESHOLD;
    adaptiveSpeechThreshold = BREATH_PAUSE_CONFIG.SPEECH_THRESHOLD;
    log = getLogger().child({ module: 'BreathPauseDetector' });
    /**
    ...
    updateSpeechState(now) {
        ...
        if (this.highEnergyFrameCount >= BREATH_PAUSE_CONFIG.SPEECH_CONFIRMATION_FRAMES) {
            ...
            if (this.inPause && this.pauseStartTime) {
                ...
                this.log.debug({ pauseDuration, wasBreathPause: pauseDuration >= 100 && pauseDuration <= 400 }, '▶️ Speech resumed');
            }
            if (!this.isSpeaking) {
     ...
     */
    getAveragePauseLength() {
        if (this.pauseHistory.length < 3)
            return undefined;
...
//# sourceMappingURL=breath-pause.js.map