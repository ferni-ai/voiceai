...
const TRAJECTORY_MICRO_REACTIONS = {
    rising_excitement: [
...
export class EmotionPredictor {
    ...
    predict(text, tone) {
        ...
        let bestTrajectory = 'stable_neutral';
        ...
        if (tone && bestConfidence > 0) {
            if ((tone === 'excited' && bestTrajectory === 'rising_excitement') ||
        ...
        if (bestConfidence >= 0.5) {
            ...
            log.debug({ trajectory: bestTrajectory, confidence: bestConfidence.toFixed(2), emotion }, '🎭 Emotion predicted');
        }
        return prediction;
    }
    /**
     ...
     */
    calculateConfidence(text, pattern, tone) {
...
//# sourceMappingURL=emotion-predictor.js.map