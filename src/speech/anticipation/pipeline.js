...
export class AnticipationPipeline {
    ...
    lastResult = null;
    lastUpdateTime = 0;
    updateThrottleMs = 100; // Don't update more than 10x/second
    ...
    process(context) {
        ...
        const combinedConfidence = intentPrediction.confidence * preferenceWeight.intent +
        ...
        const actionableReason = this.getActionableReason(isActionable, intentPrediction.confidence, emotionPrediction.confidence);
     ...
     */
    shouldUseMicroReaction() {
        if (!this.options.enableMicroReactions)
            return false;
        if (!this.lastResult || !this.lastResult.isActionable)
            return false;
        // Only use micro-reactions for high-confidence emotional trajectories
        return (this.lastResult.emotion.confidence >= 0.6 &&
    ...
    // HELPERS
    // ==========================================================================
    getActionableReason(isActionable, intentConfidence, emotionConfidence) {
        if (isActionable) {
 ...
 */
export function getAnticipationPipeline(sessionId, options) {
 ...
 */
export function resetAnticipationPipeline(sessionId) {
 ...
 * Reset all pipelines
 */
export function resetAllAnticipationPipelines() {
 ...
 */
export function getActiveAnticipationPipelineCount() {
