...
function isValidName(name) {
    ...
    if (NOT_NAMES.has(name) || NOT_NAMES.has(titleCase))
        return false;
    // Reject core persona names (Ferni, Maya, etc.) but NOT common names like John
    if (CORE_PERSONA_NAMES.has(name))
        return false;
    // Reject persona last names
    if (PERSONA_EXTENDED.has(name))
        return false;
...
//# sourceMappingURL=small-details.js.map