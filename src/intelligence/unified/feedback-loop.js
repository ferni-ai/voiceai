...
export class NaturalnessFeedbackLoop {
    ...
    recordResponseContext(context) {
        ...
        log.debug({ turnId: context.turnId, buildersUsed: context.buildersUsed.length }, '📝 Recorded response context');
    }
    /**
     * Record user reaction to a response
     */
    recordUserReaction(turnId, reaction) {
    ...
    getRecommendations() {
        ...
        const underperforming = effectiveness.filter((e) => e.usageCount >= 10 && e.avgNaturalnessScore < 0.5);
        if (underperforming.length > 0) {
            recommendations.push(`Consider reviewing these builders (low naturalness scores): ${underperforming.map((e) => e.builderName).join(', ')}`);
        }
        // Find high-performing builders that could be used more
        const highPerforming = effectiveness.filter((e) => e.usageCount >= 5 && e.avgNaturalnessScore > 0.7);
        if (highPerforming.length > 0) {
            recommendations.push(`These builders are working well: ${highPerforming.map((e) => e.builderName).join(', ')}`);
        }
        ...
        if (mismatchSignals.length > 0) {
            const avgMismatchScore = mismatchSignals.reduce((sum, s) => sum + s.naturalnessScore, 0) / mismatchSignals.length;
    ...
    // PRIVATE METHODS
    // ============================================================================
    computeNaturalnessScore(context, reaction) {
        ...
        if (reaction.askedFollowUp)
            score += 0.1;
        if (reaction.responseLength === 'long')
            score += 0.05;
        // Negative signals
        if (!reaction.continuedConversation)
            score -= 0.2;
        if (reaction.emotionalShift === 'declined')
            score -= 0.2;
        if (reaction.topicShift && !context.analysis.context.isTopicShift)
            score -= 0.1;
        if (reaction.responseLength === 'short')
            score -= 0.1;
    ...
    analyzeSignals(context, reaction) {
        const positive = [];
        const negative = [];
        if (reaction.continuedConversation) {
    ...
    updateBuilderStats(signal) {
        for (const builderName of signal.context.buildersUsed) {
            ...
            if (signal.reaction.continuedConversation &&
