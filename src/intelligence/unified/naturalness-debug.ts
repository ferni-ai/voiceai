 ...
 */
export function generateNaturalnessReport(
  ...
  for (const issue of issues) {
    if (issue.severity === 'high') score -= 0.2;
    else if (issue.severity === 'medium') score -= 0.1;
    else score -= 0.05;
  }

  // Bonus for good signals
  if (analysis.mismatch.detected && analysis.mismatch.shouldSurface) score += 0.1;
  if (analysis.emotion.source === 'combined') score += 0.1;
  if (humanization.activeListening.length > 0) score += 0.05;

  score = Math.max(0, Math.min(1, score));

  const report: NaturalnessReport = {
    score,
    signals: {
 ...
 */
export function logAnalysisSummary(analysis: UnifiedAnalysisResult): void {
  log.info(
    {
 ...
 */
export function checkNaturalnessIssues(analysis: UnifiedAnalysisResult): string[] {
