...
export class VoiceTextMismatchDetector {
     ...
     */
    detect(userText, voiceEmotion, textEmotion) {
        ...
        if (isMaskingPhrase && this.isNegativeVoice(voiceEmotion)) {
            ...
            log.info({ type: result.type, confidence: result.confidence.toFixed(2) }, '🎯 Voice-text mismatch detected: masking negative');
            return result;
        }
        ...
        if (this.isPositiveText(textValence, textPrimary) && this.isNegativeVoice(voiceEmotion)) {
            ...
            log.info({ type: result.type, confidence: result.confidence.toFixed(2) }, '🎯 Voice-text mismatch detected: contradicting');
            return result;
        }
        // Type 3: Understating positive emotions
        if (this.isNeutralText(textValence, textPrimary) &&
            ...
            log.info({ type: result.type, confidence: result.confidence.toFixed(2) }, '🎯 Voice-text mismatch detected: understating positive');
            return result;
        }
        ...
        if (this.isNeutralText(textValence, textPrimary) && (voiceEmotion.stressLevel || 0) > 0.6) {
            ...
            log.info({ type: result.type, confidence: result.confidence.toFixed(2) }, '🎯 Voice-text mismatch detected: suppressing stress');
            return result;
        }
        ...
        if (voiceEmotion.anxietyMarkers &&
            ...
            log.info({ type: result.type, confidence: result.confidence.toFixed(2) }, '🎯 Voice-text mismatch detected: anxiety markers');
            return result;
        }
    ...
    buildGuidance(mismatch) {
        ...
        lines.push(``, `DON'T: Call out the mismatch directly ("I can tell you're not fine")`, `DO: Create space and show you're present ("I'm here if there's more you want to share")`);
        return {
 ...
 */
export function detectMismatch(userText, voiceEmotion, textEmotion) {
...
//# sourceMappingURL=mismatch-detector.js.map