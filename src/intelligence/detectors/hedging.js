...
export class HedgingDetector {
    ...
    analyze(text) {
        ...
        for (const [category, patterns] of Object.entries(HEDGING_PATTERNS)) {
        ...
        const shouldProbe = elevated || (dominantCategory === 'protecting' && byCategory.protecting >= 1);
        // Generate probe approach if needed
        let probeApproach;
        if (shouldProbe && dominantCategory !== null) {
            const category = dominantCategory;
            const suggestions = PROBE_SUGGESTIONS[category];
    ...
    // PRIVATE HELPERS
    // ==========================================================================
    getContext(text, position, radius = 25) {
        const start = Math.max(0, position - radius);
        const end = Math.min(text.length, position + radius);
        return text.slice(start, end);
    }
    generateInterpretation(byCategory, elevated, dominant) {
        if (!elevated && !dominant) {
            return 'Hedging within normal conversational range.';
        }
        const interpretations = {
            uncertainty: 'User is expressing significant uncertainty. They may need help clarifying their thoughts or feelings.',
            minimizing: 'User is consistently minimizing. They may be downplaying something important or protecting themselves.',
            distancing: 'User is distancing from their statements. They may be uncertain or avoiding ownership of feelings.',
            protecting: 'User is using protective language. They may be afraid of judgment or vulnerability.',
            qualifying: 'User is heavily qualifying statements. They may be unsure or reluctant to commit to their position.',
            softening: 'User is softening their language. They may be more certain than they sound but hesitant to be direct.',
        };
        if (dominant) {
...
const hedgingDetectorRegistry = createSessionRegistry((sessionId) => new HedgingDetector(), { name: 'HedgingDetector', cleanup: (detector) => detector.reset(), verbose: false });
registerGlobalRegistry(hedgingDetectorRegistry);
export function getHedgingDetector(sessionId) {
    return hedgingDetectorRegistry.get(sessionId);
}
export function resetHedgingDetector(sessionId) {
    hedgingDetectorRegistry.reset(sessionId);
}
export function resetAllHedgingDetectors() {
    hedgingDetectorRegistry.resetAll();
}
export function getActiveHedgingDetectorCount() {
