 ...
 */
export const DISTRESS_GUIDANCE: Record<DistressLevel, DistressGuidance> = {
  CRISIS: {
    level: 'CRISIS',
 ...
 */
export function getDistressCategory(level: number): DistressLevel {
  if (level >= DISTRESS.CRISIS) return 'CRISIS';
  if (level >= DISTRESS.HIGH) return 'HIGH';
  if (level >= DISTRESS.MODERATE) return 'MODERATE';
  if (level >= DISTRESS.ELEVATED) return 'ELEVATED';
  if (level >= DISTRESS.MILD) return 'MILD';
  return 'LOW';
}
 ...
 */
export function getDistressGuidance(level: number): DistressGuidance {
 ...
 */
export function needsEmotionalSupport(level: number): boolean {
 ...
 */
export function isCrisis(level: number): boolean {
 ...
 */
export function shouldBeGentle(level: number): boolean {
 ...
 */
export function getSuggestedTone(level: number): DistressGuidance['tone'] {
 ...
 */
export function formatDistressForPrompt(level: number): string {
