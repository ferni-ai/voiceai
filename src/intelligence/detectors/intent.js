...
export class IntentClassifier {
    ...
    classify(text) {
        ...
        getLogger().debug(`Classified intent: ${primary} (confidence: ${result.confidence.toFixed(2)})`);
        return result;
    }
    /**
     * Determine urgency level
     */
    determineUrgency(primary, secondary, text) {
        const highUrgencyIntents = ['seeking_support', 'making_decision', 'market_concern'];
        const mediumUrgencyIntents = ['seeking_advice', 'confiding', 'venting'];
        const lowUrgencyIntents = ['expressing_concern'];
        // Check for urgency keywords first (most important)
        if (/urgent|immediately|asap|crisis|emergency|panic|lost my job/i.test(text))
            return 'high';
        // Check for panic indicators in text
        if (/panicking|crashing!/i.test(text))
            return 'high';
        ...
        if (lowUrgencyIntents.includes(primary)) {
            ...
            if (secondary.some((i) => highUrgencyIntents.includes(i)))
                return 'medium';
            return 'low';
        }
        if (highUrgencyIntents.includes(primary))
            return 'high';
        if (secondary.some((i) => highUrgencyIntents.includes(i)))
            return 'high';
        if (mediumUrgencyIntents.includes(primary))
            return 'medium';
        // Check for medium urgency keywords
        if (/soon|quickly|when you can|should probably/i.test(text))
            return 'medium';
        return 'low';
    }
    /**
     ...
     */
    requiresAction(primary, secondary) {
        const actionIntents = [
            'seeking_advice',
            'asking_question',
            'requesting_info',
        ...
        const noActionIntents = ['greeting', 'gratitude', 'farewell', 'small_talk'];
        if (noActionIntents.includes(primary))
            return false;
     ...
     */
    requiresEmpathy(primary, secondary, text) {
     ...
     */
    getSuggestedApproach(primary, requiresEmpathy) {
        if (requiresEmpathy) {
