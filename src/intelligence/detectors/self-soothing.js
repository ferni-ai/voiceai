...
export class SelfSoothingDetector {
    ...
    analyze(text) {
        ...
        const interpretation = this.generateInterpretation(instances, dominantCategory, possibleDistress);
    ...
    getPatterns() {
        ...
        const frequency = detectedCount < 2 ? 'rare' : detectedCount < 5 ? 'occasional' : 'frequent';
        ...
        const concernLevel = distressCount >= 3 ? 'high' : distressCount >= 1 ? 'moderate' : 'low';
    ...
    buildContextForPrompt() {
        ...
        const lines = [];
        lines.push(`[SELF-SOOTHING DETECTED] User is saying things like "${recent.instances[0]?.phrase}"`);
    ...
    findDominantCategory(instances) {
        if (instances.length === 0)
            return null;
    ...
    determineUnderlyingState(instances, dominant) {
        if (instances.length === 0) {
    ...
    checkDistress(instances) {
        ...
        return instances.some((i) => distressIndicators.some((d) => i.phrase.toLowerCase().includes(d.toLowerCase())));
    }
    generateInterpretation(instances, dominant, distress) {
        if (instances.length === 0) {
        ...
        const categoryDescriptions = {
            ...
            dismissive: 'User is being dismissive, possibly avoiding vulnerability or expressing frustration.',
    ...
    generateApproach(dominant, distress) {
        if (!dominant) {
            return 'Continue listening attentively.';
        }
        const approaches = {
            reassurance: "Don't challenge their reassurance directly. Create space: 'How are you really feeling about this?'",
            minimizing: "Gently acknowledge what they're minimizing without pushing: 'That sounds like it might be meaningful.'",
            dismissive: "Don't chase what they dismissed, but leave the door open: 'I'm here if you want to come back to that.'",
            normalizing: "Validate their experience even if it's common: 'Even if others feel this, your experience matters.'",
            deflecting: "Don't accept the deflection but don't push: 'I don't see it as a big deal that you're sharing.'",
            convincing: "Notice the self-talk: 'You're working hard to convince yourself. What's the other voice saying?'",
        };
...
const selfSoothingRegistry = createSessionRegistry((sessionId) => new SelfSoothingDetector(), { name: 'SelfSoothing', cleanup: (detector) => detector.reset(), verbose: false });
registerGlobalRegistry(selfSoothingRegistry);
export function getSelfSoothingDetector(sessionId) {
    return selfSoothingRegistry.get(sessionId);
}
export function resetSelfSoothingDetector(sessionId) {
    selfSoothingRegistry.reset(sessionId);
}
export function resetAllSelfSoothingDetectors() {
    selfSoothingRegistry.resetAll();
}
export function getActiveSelfSoothingCount() {
    return selfSoothingRegistry.getActiveCount();
}
export default SelfSoothingDetector;
//# sourceMappingURL=self-soothing.js.map