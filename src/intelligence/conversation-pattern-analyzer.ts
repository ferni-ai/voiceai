...
export class ConversationPatternAnalyzer {
  ...
  recordTopic(topic: string, engagement: number): void {
    ...
    if (!this.currentSession.topicSequence!.includes(topic)) {
      this.currentSession.topicSequence!.push(topic);
    }
    
    if (engagement > 0.7 && !this.currentSession.peakEngagementTopics!.includes(topic)) {
      this.currentSession.peakEngagementTopics!.push(topic);
    }
  }
  ...
  recordEngagement(minutesIn: number, totalExpectedMinutes: number, engagement: number): void {
    ...
    const current = this.currentSession.engagementCurve![quarter];
    this.currentSession.engagementCurve![quarter] = (current + engagement) / 2;
  }
  ...
  endSession(options: {
    ...
    const durationMinutes = Math.round(
      (now.getTime() - this.currentSession.startedAt!.getTime()) / 60000
    );
  ...
  analyzePatterns(): LearnedConversationPatterns {
    ...
    const typicalOpening = Object.entries(openingCounts)
      .sort((a, b) => b[1] - a[1])[0]?.[0] as OpeningStyle || 'social_greeting';
    
    // Calculate preferred duration
    const preferredDuration = Object.entries(durationCounts)
      .sort((a, b) => b[1] - a[1])[0]?.[0] as DurationBucket || 'medium';
    ...
    for (let i = 1; i < sortedSessions.length; i++) {
      const gap = (sortedSessions[i].startedAt.getTime() - 
                   sortedSessions[i-1].endedAt.getTime()) / (1000 * 60 * 60 * 24);
      if (gap < 30) {  // Only count gaps < 30 days
        totalGaps += gap;
        gapCount++;
      }
    }
    ...
    const smallTalkOpenings = (openingCounts['social_greeting'] || 0) + 
                              (openingCounts['casual_chat'] || 0);
    ...
    return {
      ...
      preferredFirstTopic: Object.entries(firstTopicCounts)
