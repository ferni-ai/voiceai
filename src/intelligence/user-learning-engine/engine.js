...
import { extractSmallDetails, generateFarewellSummary, } from '../conversation-quality.js';
// ============================================================================
...
export class UserLearningEngine {
    ...
    voiceEmotionAccuracy = 0.5; // Start neutral, adjust based on validation
    /**
     * Process a user turn and extract learning opportunities
     */
    processUserTurn(message, analysis, profile) {
        ...
        // 3. Capture topics
        if (analysis.state.currentTopic &&
     ...
     */
    recordVoiceEmotion(emotion, confidence) {
     ...
     */
    validateVoiceEmotionPrediction(textEmotion) {
     ...
     */
    detectKeyMoment(message, analysis) {
        const messageLower = message.toLowerCase();
     ...
     */
    generateMomentSummary(message, type) {
    ...
    learnPreferences(profile) {
        ...
        if (preferences.responseLength !== 'unknown') {
            this.sessionInsights.push({
                type: 'preference',
                key: 'verbosity',
                value: preferences.responseLength === 'brief'
                    ? 'concise'
     ...
     */
    extractExplicitInsights(message, intent) {
        const messageLower = message.toLowerCase();
        // Goal mentions
        const goalPatterns = [
    ...
    finalizeSession(profile) {
        ...
        if (this.conversationHistory.length > 2) {
            ...
            farewellSummary = generateFarewellSummary(this.conversationHistory, this.topicsDiscussed, profile, { start: startEmotion, end: endEmotion });
        }
        ...
        for (const insight of this.sessionInsights) {
            if (insight.type === 'preference' && insight.confidence > 0.6) {
                if (insight.key === 'verbosity' && typeof insight.value === 'string') {
                    updates.responseLength = insight.value;
                }
                if (insight.key === 'storyAppetite') {
                    updates.storyAppetite = insight.value;
                }
                if (insight.key === 'humorAppreciation') {
                    updates.humorReceptivity = insight.value;
                }
            }
        }
        return updates;
     ...
     */
    static applyLearningToProfile(profile, learning) {
        ...
        if (learning.preferenceUpdates.responseLength &&
            learning.preferenceUpdates.responseLength !== 'unknown') {
            updated.preferences = {
                ...updated.preferences,
                verbosity: learning.preferenceUpdates.responseLength === 'brief'
                    ? 'concise'
        ...
        if (learning.preferenceUpdates.humorReceptivity &&
     ...
     */
    captureExternalInsight(insight) {
        this.sessionInsights.push({
            ...insight,
            capturedAt: new Date(),
        });
     ...
     */
    getProactiveInsight(profile, turnCount) {
        // Only suggest proactively after warmup (turns 4-8) or periodically
        if (turnCount < 4)
            return null;
        if (turnCount > 8 && turnCount % 6 !== 0)
            return null;
        ...
        const shallowTopics = this.topicsDiscussed.filter((topic) => {
            const mentions = this.conversationHistory.filter((m) => m.content.toLowerCase().includes(topic.toLowerCase())).length;
            return mentions <= 2;
        });
     ...
     */
    contributeToCommunnityLearning(personaId, sessionData) {
        const communityInsights = getCommunityInsights();
        try {
            // 1. Contribute response effectiveness signals
            const avgEngagement = sessionData.engagementScores.length > 0
            ...
            for (let i = 0; i < this.conversationHistory.length - 1; i += 2) {
                ...
                if (userMsg?.role === 'user' && assistantMsg?.role === 'assistant') {
                    ...
                    communityInsights.recordResponseSignal({
                        ...
                        strategy: {
                            type: responseType,
                            hadPersonalShare: /\bi\s+(remember|recall|had|was|felt|think)\b/i.test(assistantMsg.content),
            ...
            if (sessionData.breakthoughQuestions) {
                for (const q of sessionData.breakthoughQuestions) {
                    communityInsights.recordBreakthroughQuestion(q.question, personaId, this.topicsDiscussed[0] || 'general', 'conversation', q.engagementLift);
                }
            }
            ...
            for (const moment of this.sessionKeyMoments) {
                if (moment.type === 'breakthrough') {
                    ...
                    getLogger().debug({ personaId, momentType: moment.type }, 'Breakthrough moment contributed to community learning');
                }
            }
            // 4. Contribute story effectiveness
            for (const story of this.sessionStoriesTold) {
                // Find user reaction after story was told
                const storyIndex = this.conversationHistory.findIndex((m) => m.role === 'assistant' && m.content.includes(story.theme));
                if (storyIndex >= 0 && storyIndex + 1 < this.conversationHistory.length) {
                    ...
                    communityInsights.recordStoryUsage(story.storyId, personaId, {
                        topic: story.theme,
                        relationshipStage: 'acquaintance',
                        userEmotion: 'neutral',
                    }, reaction, sessionData.engagementScores[Math.floor(storyIndex / 2)] || avgEngagement);
                }
            }
            getLogger().info({
                personaId,
     ...
     */
    getCommunityContext(personaId, context) {
        const result = {};
        try {
            const communityInsights = getCommunityInsights();
    ...
    detectResponseType(content) {
        ...
        if (/\b(i remember|when i|back in|years ago|let me tell you|there was a time)\b/.test(lower) &&
            content.length > 150) {
            return 'story';
        }
    ...
    detectStoryReaction(userResponse) {
        const lower = userResponse.toLowerCase();
        if (/\b(wow|amazing|incredible|beautiful|touching)\b/.test(lower))
            return 'moved';
        if (/\b(inspired|motivat|encourage|excit)\b/.test(lower))
            return 'inspired';
        if (/\b(me too|same|i also|i remember when|my|mine)\b/.test(lower))
            return 'connected';
