...
export class ProactiveInsightEngine {
    ...
    if (month >= 9 && month <= 11) {
      ...
      if (!this.deliveredInsightIds.has(yearEndId)) {
        insights.push({
          ...
          message:
            "The year's winding down - it's a good time to review your portfolio and max out any retirement contributions. Want to do a quick year-end check?",
  ...
  markDelivered(
    insightId: string,
    reaction?: 'positive' | 'neutral' | 'dismissed' | 'negative'
  ): void {
  ...
  getStats(): {
    ...
    const negative = delivered.filter(
      (i) => i.userReaction === 'negative' || i.userReaction === 'dismissed'
    ).length;
