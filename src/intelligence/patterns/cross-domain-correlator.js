...
export function recordDomainSignal(userId, signal) {
    ...
    log.debug({ userId, domain: signal.domain, metric: signal.metric, direction: signal.direction }, '📊 Domain signal recorded');
 ...
 */
async function analyzeCorrelations(userId) {
    const state = getOrCreateState(userId);
    ...
    for (const windowSignals of windows) {
        ...
        for (let i = 0; i < windowSignals.length; i++) {
            for (let j = i + 1; j < windowSignals.length; j++) {
                ...
                if (signalA.domain === signalB.domain)
                    continue;
    ...
    log.debug({ userId, correlationCount: state.correlations.length }, '🔗 Correlation analysis complete');
}
/**
 * Group signals into time windows for co-occurrence detection.
 */
function groupSignalsByTimeWindow(signals) {
    const windows = [];
 ...
 */
function updateOrCreateCorrelation(userId, state, signalA, signalB) {
    ...
    const existing = state.correlations.find((c) => c.domainA.domain === first.domain &&
 ...
 */
function calculateStrength(correlation, signalA, signalB) {
 ...
 */
function getDirectionScore(dirA, dirB) {
 ...
 */
function isRecent(correlation) {
    const daysSinceObserved = (Date.now() - new Date(correlation.lastObserved).getTime()) / (1000 * 60 * 60 * 24);
    return daysSinceObserved < 30;
}
/**
 * Generate human-readable insight and suggestion.
 */
function generateInsightAndSuggestion(correlation) {
...
function getInsightTemplates() {
    return {
        ...
        energy_exercise: {
            insight: (a, b, s) => s > 0 ? `${b} exercise correlates with ${a} energy` : `${b} might be affecting your energy`,
 ...
 */
export function getCorrelations(userId, options) {
    ...
    if (options?.domains?.length) {
        correlations = correlations.filter((c) => options.domains.includes(c.domainA.domain) ||
 ...
 */
export function getRelevantCorrelations(userId, context) {
    ...
    const scored = all.map((corr) => {
        ...
        if (context.currentTopics) {
            for (const topic of context.currentTopics) {
                ...
                if (corr.domainA.pattern.toLowerCase().includes(topicLower) ||
        ...
        if (context.currentDomains) {
            if (context.currentDomains.includes(corr.domainA.domain) ||
                context.currentDomains.includes(corr.domainB.domain)) {
                relevance += 0.3;
            }
        }
        // Confidence boost
        if (corr.confidence === 'confirmed')
            relevance += 0.2;
        // Recency boost
        ...
        if (corr.lastSurfaced) {
            const daysSinceSurfaced = (Date.now() - new Date(corr.lastSurfaced).getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
export function markCorrelationSurfaced(userId, correlationId) {
    const state = userStates.get(userId);
    if (!state)
        return;
...
export function formatCorrelationsForPrompt(correlations) {
    ...
    for (const corr of correlations) {
        const confIcon = corr.confidence === 'confirmed' ? '✓' : corr.confidence === 'likely' ? '~' : '?';
 ...
 */
export function clearCorrelatorState(userId) {
    if (userId) {
...
export function getCrossCorrelator() {
    return singleton;
}
export const crossDomainCorrelator = singleton;
//# sourceMappingURL=cross-domain-correlator.js.map