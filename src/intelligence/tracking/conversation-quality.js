 ...
 */
export function generateFarewellSummary(conversationHistory, topicsDiscussed, userProfile, emotionalArc) {
    ...
    for (const topic of topicsDiscussed) {
        ...
        const topicMentions = conversationHistory.filter((m) => m.content.toLowerCase().includes(topic.toLowerCase())).length;
    ...
    const nextTimeGreeting = generateNextTimeGreeting(userName, topicsDiscussed, endingMood, emotionalArc);
    // Build relationship notes
    const relationshipNotes = buildRelationshipNotes(conversationHistory.length, topicsDiscussed, emotionalArc);
    // Extract follow-ups
...
function generateNextTimeGreeting(userName, topics, endingMood, emotionalArc) {
...
function buildRelationshipNotes(turnCount, topics, emotionalArc) {
    const notes = [];
    if (turnCount > 20) {
 ...
 */
export function extractSmallDetails(text) {
    const details = [];
    const now = new Date();
 ...
 */
export function getDetailCallback(detail) {
    switch (detail.type) {
        case 'pet_name':
 ...
 */
export function extractFollowUps(conversationHistory, topics) {
    const followUps = [];
    const now = new Date();
 ...
 */
export function getFollowUpSuggestion(followUp) {
    const daysUntil = Math.ceil((followUp.suggestedDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
    if (daysUntil <= 7) {
 ...
 */
export function getPersonaPhysicalState(hour, conversationMinutes, turnCount, personaId) {
 ...
 */
export function getPhysicalStateInterjection(state) {
 ...
 */
export function calculatePacingScore(recentMessages, turnCount, topicsDiscussed, emotionalMoments, goalsReached) {
    const factors = {
        engagement: 50,
        depth: 50,
        rapport: 50,
    ...
    const personalTopics = topicsDiscussed.filter((t) => ['family', 'fear', 'dreams', 'regret', 'loss', 'hope', 'love'].includes(t.toLowerCase()));
    ...
    if (factors.progress < 40 && turnCount > 15) {
        suggestions.push('Conversation feels unfocused - consider summarizing or asking about next steps');
    }
    // ===== OVERALL SCORE =====
    const overallScore = Math.round(factors.engagement * 0.3 +
        factors.depth * 0.25 +
 ...
 */
export function getGracefulErrorResponse(errorType, context) {
...
//# sourceMappingURL=conversation-quality.js.map