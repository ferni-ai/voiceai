...
export class ResponseQualityTracker {
    signals = [];
    userId;
    constructor(userId, existingSignals) {
        this.userId = userId;
        if (existingSignals) {
    ...
    analyzeResponse(response) {
        ...
        const hadStory = /\b(when i|i remember|back in|years ago|one time|there was a)\b/i.test(response);
        const hadHumor = /\b(haha|joke|kidding|funny[laughter]😄|😂)\b/i.test(response) || /\!.*\!/i.test(response);
        const hadQuestion = response.includes('?');
        const hadAdvice = /\b(should|recommend|suggest|consider|try|important|make sure)\b/i.test(response);
        ...
        let type = 'mixed';
        if (hadStory && wordCount > 50)
            type = 'story';
        else if (hadEmpathy && !hadAdvice)
            type = 'empathy';
        else if (hadAdvice && !hadStory)
            type = 'advice';
        else if (hadQuestion && wordCount < 30)
            type = 'question';
        else if (hadHumor && wordCount < 40)
            type = 'humor';
        else if (hadExplanation && wordCount > 40)
            type = 'explanation';
        // Determine length
        let length = 'moderate';
        if (wordCount < 30)
     ...
     */
    analyzeUserReaction(userResponse, previousTopic, emotion) {
        // 🦀 Rust-accelerated word counting
        const wordCount = RUST_COUNTING_AVAILABLE
        ...
        // Detect signals
        const askedFollowUp = userResponse.includes('?') ||
        ...
        const changedTopic = /\b(anyway|actually|by the way|different|speaking of|what about)\b/i.test(userResponse);
        const shortResponse = wordCount < 5;
        const longResponse = wordCount > 30;
        const enthusiastic = /!/.test(userResponse) || /\b(yes|definitely|absolutely|exactly)\b/i.test(userResponse);
        ...
        const confused = /\b(what do you mean|don't understand|confused|huh|unclear)\b/i.test(userResponse);
        // Determine reaction
     ...
     */
    recordSignal(jackResponse, userResponse, topic, conversationPhase, emotion) {
    ...
    calculatePreferences() {
        ...
        for (const signal of this.signals) {
            ...
            // Topic engagement
            if (!topicScores[signal.topic])
                topicScores[signal.topic] = [];
            topicScores[signal.topic].push(signal.engagementScore);
        }
        // Calculate averages
        const avgScore = (scores) => scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : 0.5;
        ...
        const preferredLength = Object.entries(lengthAvgs).sort((a, b) => b[1] - a[1])[0][0];
    ...
    getQualityData() {
        ...
        return {
            ...
            preferences: prefs,
            avgEngagementScore: this.signals.length > 0
...
export function getResponseQualityTracker(userId, existingSignals) {
...
export function removeResponseQualityTracker(userId) {
    trackers.delete(userId);
}
export default ResponseQualityTracker;
//# sourceMappingURL=response-quality.js.map