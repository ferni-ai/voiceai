...
export class ConversationPatternAnalyzer {
    ...
    userId;
    currentSession = null;
    constructor(userId, existingSessions) {
        this.userId = userId;
        if (existingSessions) {
    ...
    detectOpeningStyle(firstUserMessage) {
     ...
     */
    recordEngagement(minutesIn, totalExpectedMinutes, engagement) {
        if (!this.currentSession)
            return;
    ...
    analyzePatterns() {
        ...
        const openingCounts = {};
        const firstTopicCounts = {};
        const topicSequences = [];
        let totalDuration = 0;
        let rushCount = 0;
        const engagementCurves = [];
        ...
        const typicalOpening = Object.entries(openingCounts).sort((a, b) => b[1] - a[1])[0]?.[0] ||
            'social_greeting';
        ...
        const peakTime = peakIndex <= 1 ? 'early' : peakIndex === 2 ? 'middle' : 'late';
        ...
        let totalGaps = 0;
        let gapCount = 0;
        const sortedSessions = [...this.sessions].sort((a, b) => a.startedAt.getTime() - b.startedAt.getTime());
        for (let i = 1; i < sortedSessions.length; i++) {
            const gap = (sortedSessions[i].startedAt.getTime() - sortedSessions[i - 1].endedAt.getTime()) /
                (1000 * 60 * 60 * 24);
            if (gap < 30) {
        ...
        const smallTalkOpenings = (openingCounts['social_greeting'] || 0) + (openingCounts['casual_chat'] || 0);
        ...
        return {
            ...
            preferredFirstTopic: Object.entries(firstTopicCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || 'general',
            ...
            prefersQuickConversations: preferredDuration === 'short' || preferredDuration === 'very_short',
    ...
    getPatternGuidance() {
        ...
        if (patterns.topicsThatLeadToEngagement.length > 0) {
            lines.push(`• High-engagement topics: ${patterns.topicsThatLeadToEngagement.slice(0, 3).join(', ')}`);
        }
...
export function getConversationPatternAnalyzer(userId, existingSessions) {
...
export function removeConversationPatternAnalyzer(userId) {
    analyzers.delete(userId);
}
export default ConversationPatternAnalyzer;
//# sourceMappingURL=conversation-patterns.js.map