...
export class MemoryEngagementEngine {
     ...
     */
    generateCallbacks(userId, profile, personaId) {
        const callbacks = [];
        if (!profile)
            return callbacks;
        // 1. Anniversary callbacks
        const anniversaryCallback = this.checkAnniversary(profile, personaId);
        if (anniversaryCallback)
            callbacks.push(anniversaryCallback);
        ...
        if (emotionalCallback)
            callbacks.push(emotionalCallback);
        ...
        if (this.shouldMirrorGrowth(profile)) {
            ...
            if (growthCallback)
                callbacks.push(growthCallback);
        }
        // Sort by priority
        callbacks.sort((a, b) => b.priority - a.priority);
        // Store for this user
        this.pendingCallbacks.set(userId, callbacks);
        getLogger().debug({ userId, personaId, callbackCount: callbacks.length }, 'Generated memory callbacks');
        return callbacks;
    }
    /**
     * Check for relationship anniversary
     */
    checkAnniversary(profile, personaId) {
        ...
        for (const { days, key } of milestones) {
            ...
            if (Math.abs(daysSinceCreation - days) <= 1) {
                ...
                const alreadyAcknowledged = existingMilestones.some((m) => m.type === key && m.acknowledged);
                if (!alreadyAcknowledged) {
                    const templates = CALLBACK_TEMPLATES.anniversary[key];
                    if (templates) {
                        return {
                            type: 'anniversary',
     ...
     */
    generateProgressCallbacks(profile, personaId) {
        const callbacks = [];
        if (!profile.keyMoments || profile.keyMoments.length < 2)
            return callbacks;
        // Find moments worth celebrating
        const celebratableMoments = profile.keyMoments.filter((km) => {
            const daysSince = Math.floor((Date.now() - new Date(km.timestamp).getTime()) / (1000 * 60 * 60 * 24));
        ...
        for (const struggle of struggles.slice(0, 1)) {
            ...
            const relatedBreakthrough = breakthroughs.find((bt) => bt.topics?.some((t) => struggle.topics?.includes(t)));
            if (relatedBreakthrough) {
                const templates = CALLBACK_TEMPLATES.progress[personaId];
                if (templates) {
     ...
     */
    checkEmotionalContinuity(profile, personaId) {
        if (!profile.emotionalPatterns || profile.emotionalPatterns.length < 2)
            return null;
     ...
     */
    generateDetailCallbacks(profile, personaId) {
        const callbacks = [];
        ...
        if (profile.familyMembers && profile.familyMembers.length > 0) {
            ...
            if (member?.name) {
                ...
                callbacks.push({
                    type: 'small_detail',
                    content: templates[Math.floor(Math.random() * templates.length)].replace('{name}', member.name),
                    personaId,
                    priority: 4,
                });
            }
     ...
     */
    shouldMirrorGrowth(profile) {
        // Only mirror growth for established relationships
        if (profile.relationshipStage === 'new_acquaintance')
            return false;
        // Minimum conversation count
        if ((profile.totalConversations || 0) < 10)
            return false;
     ...
     */
    generateGrowthMirror(profile, personaId) {
        if (!profile.keyMoments || profile.keyMoments.length < 3)
            return null;
        // Find earliest and latest moments about similar topics
        const moments = [...profile.keyMoments].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
        const earliest = moments[0];
        const latest = moments[moments.length - 1];
        if (!earliest.topics || earliest.topics.length === 0)
            return null;
     ...
     */
    markDelivered(userId, callbackType) {
        const callbacks = this.pendingCallbacks.get(userId);
        if (!callbacks)
            return;
     ...
     */
    registerPrediction(userId, prediction, checkDate) {
        const tracker = {
     ...
     */
    recordPredictionOutcome(predictionId, outcome) {
     ...
     */
    generateThinkingAbout(profile, topic) {
     ...
     */
    generateStreakAlert(streakType, days) {
     ...
     */
    generateGoalProximity(goal, percentage) {
    ...
    formatTimeAgo(timestamp) {
        ...
        if (diffDays < 365)
            return `about ${Math.floor(diffDays / 30)} months ago`;
        return 'over a year ago';
    }
    summarizeMoment(moment) {
 ...
 */
export function buildMemoryEngagementContext(profile, personaId, turnCount) {
    if (!profile)
        return '';
