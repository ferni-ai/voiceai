...
export class FinancialJourneyTracker {
    ...
    snapshots = [];
    milestones = [];
    constructor(userId, existingSnapshots, existingMilestones) {
        this.userId = userId;
     ...
     */
    createSnapshot(profile, type, jackNotes) {
        // Determine emergency fund status
        let emergencyFundStatus = 'none';
        let emergencyFundMonths;
        ...
        const activeGoalCount = profile.goals.filter((g) => g.status === 'active' || g.status === 'on_track').length;
        const goalsAchieved = profile.goals.filter((g) => g.status === 'achieved').length;
        // Infer confidence from conversation patterns
        let financialConfidence = 'moderate';
        ...
        getLogger().info({ type, goalsAchieved, hasInvestments: profile.hasInvestments }, 'Financial snapshot created');
        return snapshot;
    }
    /**
     ...
     */
    detectMilestones(profile, previousSnapshot) {
        const newMilestones = [];
        const current = this.getCurrentState();
        // First investment milestone
        if (profile.hasInvestments && !previousSnapshot?.hasInvestments) {
            newMilestones.push(this.createMilestone('started_investing', 'Started Investing Journey', "You've taken the first step into investing. This is huge - most people never get this far!", 'major'));
        }
        // Debt-free milestone
        if (!profile.financialSituation?.hasDebt && previousSnapshot?.hasDebt) {
            newMilestones.push(this.createMilestone('debt_free', 'Debt Free!', "You've paid off your debt! That's incredible discipline. This is a moment to really celebrate.", 'major'));
        }
        // Emergency fund milestone
        if (profile.financialSituation?.hasEmergencyFund && current?.emergencyFundStatus === 'none') {
            newMilestones.push(this.createMilestone('emergency_fund_complete', 'Emergency Fund Established', "You now have an emergency fund! That's one of the most important foundations of financial security.", 'major'));
        }
        // First goal achieved
        ...
        if (achievedGoals.length === 1 && (!previousSnapshot || previousSnapshot.goalsAchieved === 0)) {
            newMilestones.push(this.createMilestone('first_goal_achieved', 'First Goal Achieved!', `You did it! You achieved your first financial goal: ${achievedGoals[0].name}. This proves you can do it.`, 'major'));
        }
        // Anniversary milestones
        const firstContact = new Date(profile.firstContact);
        const daysSinceStart = Math.floor((Date.now() - firstContact.getTime()) / (1000 * 60 * 60 * 24));
        ...
        for (const days of anniversaryDays) {
            if (daysSinceStart >= days && daysSinceStart < days + 7) {
                ...
                const existingAnniversary = this.milestones.find((m) => m.type === 'anniversary' && m.title.includes(`${years} Year`));
                if (!existingAnniversary) {
                    newMilestones.push(this.createMilestone('anniversary', `${years} Year${years > 1 ? 's' : ''} on the Journey`, `Can you believe it? We've been on this financial journey together for ${years} year${years > 1 ? 's' : ''}. I'm honored to be part of it.`, years >= 3 ? 'major' : 'moderate'));
                }
            }
        }
     ...
     */
    createMilestone(type, title, description, significance) {
        return {
    ...
    analyzeTrends() {
        if (this.snapshots.length < 2)
            return [];
        const trends = [];
        const sorted = [...this.snapshots].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        // Confidence trend
        ...
        if (confidenceTrend !== 'stable') {
            trends.push({
                ...
                summary: confidenceTrend === 'improving'
     ...
     */
    generateProgressNarrative(profile) {
        ...
        const sections = [];
        // Opening
        const daysSinceStart = Math.floor((Date.now() - new Date(starting.date).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSinceStart > 30) {
            const monthsOrYears = daysSinceStart > 365
     ...
     */
    generateJourneySummary(profile) {
...
export function getFinancialJourneyTracker(userId, existingSnapshots, existingMilestones) {
...
export function removeFinancialJourneyTracker(userId) {
    trackers.delete(userId);
}
export default FinancialJourneyTracker;
//# sourceMappingURL=financial-journey.js.map