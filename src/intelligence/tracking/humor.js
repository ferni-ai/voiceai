...
export class HumorCalibrationEngine {
     ...
     */
    recordHumorAttempt(content, context, type) {
     ...
     */
    analyzeReaction(userResponse, userLaughed // From voice detection
    ) {
    ...
    calculatePreferences() {
        ...
        const goodContexts = new Set();
        const badContexts = new Set();
        let positiveCount = 0;
        let totalReacted = 0;
        ...
        for (const [type, attempts] of Object.entries(typeAttempts)) {
            if (attempts.length >= 2) {
                ...
                if (reactedAttempts.length > 0) {
                    const avgScore = reactedAttempts.reduce((sum, a) => sum + this.reactionToScore(a.reaction), 0) /
        ...
        let preferredFrequency = 'moderate';
        ...
        const recommendedTypes = Object.entries(typeScores)
     ...
     */
    getHumorGuidance(currentContext, currentEmotion, turnCount) {
    ...
    getSuggestedApproach(type, context) {
        switch (type) {
            case 'callbacks':
...
export function getHumorCalibration(userId) {
    if (!engines.has(userId)) {
        engines.set(userId, new HumorCalibrationEngine());
    }
    return engines.get(userId);
}
export function removeHumorCalibration(userId) {
    engines.delete(userId);
}
export function resetAllHumorCalibration() {
    for (const engine of engines.values()) {
        engine.reset();
    }
}
//# sourceMappingURL=humor.js.map