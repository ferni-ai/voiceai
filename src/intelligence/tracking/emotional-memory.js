...
export class EmotionalMemoryEngine {
     ...
     */
    recordMoment(emotion, topic, trigger, userStatement, intensity = 'moderate') {
        // Only capture significant emotions (not neutral)
        if (emotion === 'neutral')
            return '';
        // Avoid duplicates in quick succession
        const recentSimilar = this.moments.find((m) => m.topic === topic && m.emotion === emotion && Date.now() - m.timestamp.getTime() < 60000 // 1 minute
        );
        if (recentSimilar)
            return recentSimilar.id;
        const moment = {
        ...
        for (const [topic, data] of topicEmotions.entries()) {
            if (data.emotions.length >= 2) {
                ...
                if (data.emotions.length >= 4) {
                    ...
                    if (newRatio > oldRatio + 0.2)
                        trend = 'improving';
                    else if (newRatio < oldRatio - 0.2)
                        trend = 'worsening';
                    else
                        trend = 'stable';
                }
    ...
    getCheckInSuggestions() {
        ...
        const unresolvedConcerns = this.moments.filter((m) => !m.resolved &&
            !m.followedUp &&
    ...
    generateFollowUpOpener(moment) {
        ...
        const options = templates[moment.emotion] || [
            `How are things with ${moment.topic}? You mentioned it last time.`,
        ];
        return options[Math.floor(Math.random() * options.length)];
    }
    generateCelebrationOpener(moment) {
        const templates = [
    ...
    buildEmotionalContext() {
        ...
        const celebratableWins = this.moments
            .filter((m) => ['joy', 'anticipation'].includes(m.emotion) &&
            m.intensity !== 'mild' &&
