...
import { getCapabilityEffectiveness, clearCapabilityTracking, markCapabilityEngaged, markCapabilityToolUsed, } from '../context-builders/awareness/domain-awareness.js';
 ...
 */
export function getRecentlySurfacedDomains(sessionKey) {
 ...
 */
export async function finalizeSessionLearning(sessionKey, userId) {
...
async function updateAggregatePattern(record) {
    ...
    if (record.userEmotion) {
        const emotionEntry = existing.bestEmotionalContexts.find((e) => e.emotion === record.userEmotion);
        if (emotionEntry) {
...
function createEmptyPattern(domain) {
    return {
        domain,
        surfaceCount: 0,
        engagementCount: 0,
 ...
 */
export function onUserEngagedWithCapability(sessionKey, domain) {
 ...
 */
export function onToolUsedInDomain(sessionKey, domain) {
 ...
 */
export function onToolExecuted(sessionKey, toolName) {
 ...
 */
export function getMostEffectiveDomains(limit = 10) {
 ...
 */
export function getBestEmotionalContext(domain) {
    const pattern = aggregatePatterns.get(domain);
    if (!pattern || pattern.bestEmotionalContexts.length === 0)
        return null;
 ...
 */
export function getBestPersonaForDomain(domain) {
    const pattern = aggregatePatterns.get(domain);
    if (!pattern || pattern.bestPersonas.length === 0)
        return null;
 ...
 */
export function getDomainEngagementRate(domain) {
    const pattern = aggregatePatterns.get(domain);
    ...
    try {
        ...
        for (const [domain, pattern] of Object.entries(patternsObj)) {
            aggregatePatterns.set(cleanForFirestore(domain), {
                ...
                lastUpdated: pattern.lastUpdated instanceof Date
                    ? pattern.lastUpdated
 ...
 */
export async function initializeCapabilityLearning() {
    await loadPatterns();
