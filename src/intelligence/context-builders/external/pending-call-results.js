 ...
 */
export async function getPendingCallResults(userId) {
    try {
        const { getFirestoreDb } = await import('../../../services/superhuman/firestore-utils.js').catch(() => ({ getFirestoreDb: null }));
 ...
 */
export async function markCallResultsDelivered(userId, callIds) {
    try {
        const { getFirestoreDb } = await import('../../../services/superhuman/firestore-utils.js').catch(() => ({ getFirestoreDb: null }));
        const db = getFirestoreDb ? getFirestoreDb() : null;
        if (!db || callIds.length === 0)
            return;
 ...
 */
export async function buildPendingCallResultsContext(userId) {
 ...
 */
export async function buildPendingBackgroundResultsContext(userId) {
    try {
 ...
 */
export async function buildAllPendingResultsContext(userId) {
...
//# sourceMappingURL=pending-call-results.js.map