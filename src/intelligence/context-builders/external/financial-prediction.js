 ...
 */
import { detectAnomalies, generateFinancialInsight, generateSuperhumanMoment, predictCashFlow, } from '../../../services/finance/prediction.js';
 ...
 */
export const financialPredictionBuilder = {
    ...
    build: async (input) => {
        ...
        try {
            ...
            if (insight) {
                ...
                const priority = insight.severity === 'alert'
                    ? 'high'
                ...
                log.debug({ userId, type: insight.type, severity: insight.severity }, 'Financial prediction context injected');
            }
        }
        catch (error) {
        ...
        const isFinancialTopic = analysis.topics.detected.some((t) => /money|budget|spend|save|bill|debt|finance|afford/.test(t.toLowerCase())) ||
 ...
 */
async function syncFinancialData(userId) {
    try {
        await Promise.all([
...
//# sourceMappingURL=financial-prediction.js.map