 ...
 */
export const familyAwarenessContextBuilder = {
    ...
    build: async (input) => {
        ...
        try {
            ...
            const { getPendingContexts, markContextsDelivered } = await import('../../../services/family/family-context-sharing.js');
            ...
            if (explicitShares.length > 0) {
                ...
                explicitShares.forEach((ctx) => {
                    contextParts.push(`- From your ${formatRelationship(ctx.fromRelationship)} ${ctx.fromName}: "${ctx.summary}"`);
                });
                contextParts.push('');
            }
            ...
            if (emotionalHints.length > 0) {
                ...
                emotionalHints.forEach((ctx) => {
                    contextParts.push(`- Your ${formatRelationship(ctx.fromRelationship)} ${ctx.fromName}: ${ctx.summary}`);
                });
                contextParts.push('');
            }
            ...
            injections.push(createStandardInjection('family_awareness', `[FAMILY AWARENESS - PRIVACY PROTECTED]
...
//# sourceMappingURL=family-awareness-context.js.map