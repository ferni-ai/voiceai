 ...
 */
export function setInboundCallContext(sessionId, context) {
 ...
 */
export function getInboundCallContext(sessionId) {
    return inboundCallContexts.get(sessionId);
}
/**
 * Clear inbound call context after call completes.
 */
export function clearInboundCallContext(sessionId) {
...
// CONTEXT BUILDER
// ============================================================================
export const inboundCallContextBuilder = {
    name: 'inbound-call-context',
    description: 'Injects caller identity and context for inbound phone calls, especially for sponsored family members',
    ...
    build: async (input) => {
        ...
        injections.push(createStandardInjection('inbound_call_identity', identityContent, {
        ...
        if (callContext.isKnownCaller && !callContext.isVoiceEnrolled) {
            ...
            injections.push(createStandardInjection('inbound_call_verification', verificationContent, {
                category: 'inbound-call',
                confidence: 0.9,
            }));
        }
        ...
        if (callContext.sponsoredIdentityId && callContext.sponsorUserId) {
            ...
            injections.push(createStandardInjection('inbound_call_sponsored', sponsoredContent, {
                category: 'inbound-call',
                confidence: 1.0,
            }));
        }
        ...
        if (!callContext.isKnownCaller) {
            ...
            injections.push(createStandardInjection('inbound_call_unknown', unknownContent, {
                category: 'inbound-call',
                confidence: 0.95,
            }));
        }
        ...
        if (callContext.accessLevel && callContext.accessLevel !== 'full') {
            ...
            injections.push(createStandardInjection('inbound_call_restrictions', restrictionContent, {
...
// INJECTION BUILDERS
// ============================================================================
function buildCallerIdentityInjection(context) {
...
function buildVerificationGuidance(context) {
    return `
...
function buildSponsoredIdentityContext(context) {
    return `
...
function buildUnknownCallerGuidance() {
    return `
...
function buildAccessRestrictions(context) {
    const restrictions = [];
    if (context.accessLevel === 'limited') {
        restrictions.push('This caller has LIMITED access - keep conversations supportive but general.');
        if (context.allowedPersonas && !context.allowedPersonas.includes('*')) {
            restrictions.push(`They can only access these team members: ${context.allowedPersonas.join(', ')}`);
        }
    }
    if (context.accessLevel === 'supervised') {
        restrictions.push('This caller is on SUPERVISED access - their sponsor receives notifications about calls.');
...
//# sourceMappingURL=inbound-call-context.js.map