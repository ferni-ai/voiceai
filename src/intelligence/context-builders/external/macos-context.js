 ...
 */
export function buildMacOSContext(ctx) {
    const parts = [];
    // Active app context
    ...
    if (ctx.selectedText && ctx.selectedText.trim().length > 0) {
        const truncated = ctx.selectedText.length > 500 ? ctx.selectedText.substring(0, 500) + '...' : ctx.selectedText;
        ...
        if (ctx.helpMeWithThis) {
            parts.push('⚡ They pressed "Help me with this" - IMMEDIATELY focus on helping them with this specific text. Do not ask clarifying questions first.');
        }
    }
    // Current meeting
    if (ctx.currentMeeting) {
        parts.push(`User is currently in a meeting: "${ctx.currentMeeting.title}" (${ctx.currentMeeting.remainingMinutes} minutes remaining)`);
    ...
    if (ctx.upcomingEvent) {
        ...
        if (attendees && attendees.length > 0) {
            const attendeeList = attendees.length > 3
    ...
    if (ctx.needsBreak) {
        ...
        if (sessionMins > 60) {
            parts.push(`💆 User has been in the current app for ${sessionMins} minutes. Consider gently suggesting a break if the moment feels right.`);
        }
    }
 ...
 */
export function parseMacOSContextMessage(data) {
    try {
 ...
 */
export function generateContextInsights(ctx) {
    const insights = [];
    // Meeting prep opportunity
    if (ctx.upcomingEvent && ctx.upcomingEvent.inMinutes <= 15 && ctx.upcomingEvent.inMinutes > 5) {
        insights.push(`You have "${ctx.upcomingEvent.title}" coming up - would you like help preparing?`);
    }
    // Long time in same app
    if (ctx.topApp && ctx.topApp.minutesToday > 180) {
        insights.push(`You've been focused on ${ctx.topApp.name} for a while. Would you like a quick break or to talk through what you're working on?`);
    }
 ...
 */
export function classifyWorkContext(activeApp) {
 ...
 */
export function getSuggestedPersona(workContext) {
    switch (workContext) {
        case 'communication':
        case 'email':
 ...
 */
export const macOSContextBuilder = {
    name: 'macos-context',
    ...
    build: async (input) => {
        ...
        const macOSContext = input.userData?.macOS;
        if (!macOSContext) {
...
//# sourceMappingURL=macos-context.js.map