...
import { hasCalendarConnected, generateAnticipationInsights, generateSuperhumanMoment, getUpcomingEvents, getCurrentLocation, } from '../../../services/context-awareness/location-calendar.js';
...
const anticipationCache = new Map();
/**
 * Load anticipation content for a persona
 */
async function loadAnticipationContent(personaId) {
    if (anticipationCache.has(personaId)) {
        return anticipationCache.get(personaId) || null;
    }
    try {
        const content = await loadPersonaContent(personaId, 'anticipation');
 ...
 */
export const anticipationBuilder = {
    name: 'anticipation',
    ...
    build: async (input) => {
            ...
            if (!shouldSkipDueToNeverWhen(usageRules?.never_when, triggerContext)) {
                ...
                if (matchedTrigger) {
                    ...
                    if (Math.random() < adjustedProbability) {
                        injections.push(createHighInjection('anticipation_dynamic_trigger', `[🔮 BETTER-THAN-HUMAN ANTICIPATION: ${matchedTrigger.triggerName}]\n\n` +
                            `Condition detected: ${matchedTrigger.trigger}\n\n` +
                            `Suggested behavior: ${matchedTrigger.behavior}\n\n` +
                            `You can see around corners. Use this anticipation naturally.`, { category: 'anticipation', confidence: matchedTrigger.confidence }));
                        log.info({
                            userId,
                            personaId,
        ...
        try {
            ...
            for (const insight of insights) {
                const priority = insight.urgency === 'high' ? 'high' : insight.urgency === 'medium' ? 'standard' : 'hint';
                injections.push({
