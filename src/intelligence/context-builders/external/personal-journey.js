 ...
 */
import { isPersonalJourneyEnabled, isPersonalJourneyFeatureEnabled, isUserInPersonalJourneyRollout, } from '../../../config/feature-flags.js';
import { getCurrentChapterSummary, getJourneySnapshot, getRhythmStats, recordDelivery, selectMomentForTurn, } from '../../../services/personal-journey/index.js';
 ...
 */
function buildMomentInjection(moment) {
 ...
 */
function shouldInjectJourneyContext(turnCount, userText) {
 ...
 */
function buildBackgroundJourneyContext(userId) {
    try {
        ...
        if (stats.daysKnown > 30) {
            ...
            parts.push(`You've known this person for ${months} month${months === 1 ? '' : 's'} (${stats.totalConversations} conversations).`);
        }
        else if (stats.daysKnown > 7) {
            parts.push(`You've been talking for ${stats.daysKnown} days (${stats.totalConversations} conversations).`);
        }
        ...
        if (chapter.hasChapter && chapter.theme) {
            parts.push(`Current life focus: ${chapter.theme}${chapter.isInTransition ? ' (in transition)' : ''}.`);
        }
        if (parts.length === 0) {
            return null;
        }
 ...
 */
async function buildPersonalJourneyContext(input) {
    const injections = [];
    ...
    try {
        ...
        if (backgroundContext) {
            injections.push(createHintInjection('journey_background', `[RELATIONSHIP CONTEXT: ${backgroundContext}]`, { confidence: 0.3 } // Low priority - just background
            ));
        }
...
registerContextBuilder({
    ...
    description: 'Personal Journey Awareness - milestones, seasonal memory, life chapters, growth mirrors. Better Than Human remembers YOUR journey.',
    priority: 50,
 ...
 */
export async function getJourneyGreetingEnhancement(userId) {
    try {
        // Check feature flag
 ...
 */
export function getJourneyStatsForDisplay(userId) {
    try {
...
//# sourceMappingURL=personal-journey.js.map