...
/** Different types of conversational imperfections */
interface ImperfectionType {
  name: string;
  description: string;
  examples: string[];
  triggers: string[];
  frequency: number; // 0-1, how often to use when triggered
}

const IMPERFECTION_TYPES: ImperfectionType[] = [
  {
...
interface SessionImperfections {
  lastUsed: Map<string, number>; // imperfection type -> turn number
  totalUsed: number;
  turnCount: number;
}

const sessionState = new Map<string, SessionImperfections>();

// ============================================================================
// CONTEXT BUILDER
// ============================================================================

export const conversationalImperfectionsBuilder: ContextBuilder = {
  ...
  build: async (input: ContextBuilderInput): Promise<ContextInjection[]> => {
    ...
    const triggered: ImperfectionType[] = [];
    ...
    for (const imperfection of IMPERFECTION_TYPES) {
      ...
      const triggerMatches = imperfection.triggers.some((trigger) => {
        ...
        if (trigger === 'emotion' && emotion !== 'neutral') return true;
        if (trigger === 'sensitive' && isVulnerable) return true;
        if (trigger === 'complex' && isComplex) return true;
...
CONSTRAINT: Only use this imperfection ONCE. Don't overdo it.`;
    ...
    injections.push(
      createHintInjection('natural_speech', content, {
        category: 'humanizing',
      })
    );
 ...
 */
export function clearSessionImperfections(sessionId: string): void {
 ...
 */
export function getImperfectionStats(
  sessionId: string
): { totalUsed: number; types: string[] } | null {
  const state = sessionState.get(sessionId);
