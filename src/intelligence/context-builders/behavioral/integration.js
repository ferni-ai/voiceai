...
import { buildBehavioralContext } from './orchestrator.js';
 ...
 */
function detectHighEmotionMode(analysis) {
    if (!analysis?.emotion)
        return false;
 ...
 */
export async function buildIntegratedContext(input) {
 ...
 */
export async function getSystemPromptContext(input) {
    const result = await buildIntegratedContext(input);
