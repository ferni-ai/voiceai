 ...
 */
export async function buildAwarenessFacts(input) {
    ...
    const userTimezone = userProfile?.contact?.timezone ||
        userData?.timezone ||
        undefined;
    // Create locale options with user's timezone if available
    const localeOptions = userTimezone ? { timeZone: userTimezone } : {};
 ...
 */
export function formatAwarenessFacts(facts) {
    const lines = [];
 ...
 */
export function formatAwarenessCompact(facts) {
    const parts = [];
    if (facts.currentDate || facts.currentTime) {
        let dateTime = '';
        if (facts.currentDate) {
 ...
 */
export function getTimeOfDayBehavior(facts) {
    if (!facts.timeOfDay)
        return null;
