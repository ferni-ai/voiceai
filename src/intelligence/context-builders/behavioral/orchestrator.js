...
import { aggregateBehavior, formatBehavioralDirective, formatForSystemPrompt, } from './aggregator.js';
const log = createLogger({ module: 'behavioral:orchestrator' });
// ============================================================================
// BUILDER REGISTRY
// ============================================================================
const behavioralBuilders = new Map();
/**
 * Register a behavioral builder
 */
export function registerBehavioralBuilder(builder) {
 ...
 */
export function getBehavioralBuilders() {
 ...
 */
export async function buildBehavioralContext(input, options = {}) {
     ...
     */
    const runBuilder = async (builder) => {
    ...
    log.debug({
        ...
        modes: Object.keys(behavior.modes).filter((k) => behavior.modes[k]),
 ...
 */
export async function getBehavioralDirective(input) {
 ...
 */
export async function getCompactBehavioralDirective(input) {
    const result = await buildBehavioralContext(input);
 ...
 */
export async function buildHybridBehavioralContext(input, legacyInjections) {
    // Run behavioral builders
    const behavioralResult = await buildBehavioralContext(input);
    // Translate legacy injections to signals
    const legacySignals = translateContextsToSignals(legacyInjections.map((inj) => ({
        content: inj.content,
        source: inj.source,
        priority: inj.priority === 'critical'
            ? 90
            : inj.priority === 'high'
                ? 70
