...
import { checkPerformanceIssues, getMetricsSummary, recordBuilderMetrics, recordTurnMetrics, } from './metrics.js';
...
const contextOutputCache = new Map();
 ...
 */
function generateContextCacheKey(input) {
    const keyParts = [
 ...
 */
function isContextCacheValid(entry, inputHash) {
    // Check TTL
 ...
 */
function getCachedContextOutput(input) {
 ...
 */
function cacheContextOutput(input, injections) {
    const cacheKey = generateContextCacheKey(input);
    const inputHash = generateInputHash(input);
 ...
 */
export function clearContextOutputCache() {
 ...
 */
export function registerContextBuilder(builderOrName, buildFn) {
    let builder;
    ...
    if (builder.dependsOn) {
        for (const dep of builder.dependsOn) {
            if (!builders.has(dep) && !BUILDER_CATEGORIES[dep]) {
                log.debug({ builder: builder.name, dependency: dep }, 'Builder depends on unregistered builder (may load later)');
            }
        }
    }
 ...
 */
export function getRegisteredBuilders() {
 ...
 */
export function getBuildersByCategory(category) {
    // Check cache first
 ...
 */
export function isBuilderRegistered(name) {
...
// ============================================================================
let counter = 0;
export function createInjection(source, content, priority, options) {
    return {
        id: `${source}_${++counter}`,
        source,
        content,
...
export function createCriticalInjection(source, content, options) {
 ...
 */
export function createHighInjection(source, content, options) {
    return createInjection(source, content, 'high', options);
}
export function createStandardInjection(source, content, options) {
    return createInjection(source, content, 'standard', options);
}
export function createHintInjection(source, content, options) {
 ...
 */
export function formatContextForPrompt(injections, options) {
 ...
 */
export function shouldUseHighEmotionMode(analysis) {
    ...
    return Boolean(analysis.emotion.needsSupport ||
 ...
 */
const CORE_CATEGORIES = [BUILDER_CATEGORIES.SAFETY, BUILDER_CATEGORIES.CONTEXT];
...
import { prioritizeBuilders, recordPrioritizationResult, getPrioritizationConfig, } from './builder-prioritization.js';
/**
 ...
 */
export function determineActiveCategories(input) {
    ...
    if (analysis?.emotion?.needsSupport ||
    ...
    if (analysis?.intent?.primary === 'seeking_advice' ||
    ...
    if (analysis?.emotion?.mentalHealthSignals?.length ||
        analysis?.intent?.primary === 'seeking_advice' ||
        analysis?.topics?.detected?.some((t) => ['decision', 'problem', 'stuck', 'confused', 'anxiety'].includes(t.toLowerCase()))) {
    ...
    if (analysis?.emotion?.valence === 'positive' ||
        ...
        analysis?.topics?.detected?.some((t) => ['music', 'game', 'fun', 'story', 'celebration'].includes(t.toLowerCase()))) {
        categories.add(BUILDER_CATEGORIES.ENGAGEMENT);
    }
    // TEAM - When handoff signals detected or team mentioned
    if (analysis?.intent?.primary === 'handoff_request' ||
    ...
    if (turnCount % 10 === 0 ||
        analysis?.topics?.detected?.some((t) => ['weather', 'calendar', 'health', 'finance', 'biometric'].includes(t.toLowerCase()))) {
 ...
 */
export function getBuildersByActiveCategories(activeCategories) {
...
export async function buildConversationContext(input) {
    ...
    if (input.analysis.emotion.needsSupport ||
        (input.analysis.emotion.distressLevel &&
            input.analysis.emotion.distressLevel >= DISTRESS.MODERATE)) {
        injections.push(createCriticalInjection('emotional', `User seems to be going through something difficult. Be extra supportive and empathetic.`, { category: 'emotional' }));
    }
    // User name context
    const userName = input.userData.userName || input.userData.name || input.userProfile?.name;
    if (userName) {
        injections.push(createHintInjection('personalization', `User's name is ${userName}. Use it occasionally but naturally.`, { category: 'personalization' }));
    }
    ...
    const results = await Promise.allSettled(buildersToRun.map(async (builder) => {
        ...
        try {
            ...
            recordBuilderMetrics(builder.name, durationMs, 0, error);
            builderResults.push({
                name: builder.name,
                durationMs,
                injectionCount: 0,
    ...
    results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
            ...
            log.warn({ builder: buildersToRun[index].name, error: result.reason }, 'Context builder failed');
        }
    });
 ...
 */
export async function buildConversationContextWithMetrics(input) {
    const start = Date.now();
    const injections = await buildConversationContext(input);
...
export { areBuildersLoaded, BUILDER_MANIFEST, ensureBuildersLoaded, getAllBuilderModules, getBuilderModulesByCategory, getLastLoadReport, getLoadingStatus, reloadBuilders, } from './core/loader.js';
...
// ============================================================================
export { createBuilderRng, createSimpleRng } from './core/rng-utils.js';
// ============================================================================
 ...
 */
export async function cleanupContextBuilderSession(sessionId) {
 ...
 */
export async function cleanupAllContextBuilderSessions() {
    ...
    try {
        const { clearAllSuperhumanInsightsSessions } = await import('./superhuman/superhuman-insights.js');
