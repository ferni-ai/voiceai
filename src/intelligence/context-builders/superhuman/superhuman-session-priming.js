...
// ============================================================================
async function loadActiveCommitments(userId) {
    try {
        ...
        if (upcoming.length > 0 && overdue.length < 3) {
            const activeList = upcoming
                ...
                .join('\n');
            parts.push(`📝 ACTIVE COMMITMENTS (${upcoming.length}):\n${activeList}\nYou can naturally reference these if relevant.`);
        }
        return parts.join('\n\n');
    }
    catch (error) {
...
async function loadTrackedDreams(userId) {
    try {
...
async function loadUpcomingMilestones(userId) {
    try {
        ...
        if (!personalDates || personalDates.length === 0)
            return null;
...
async function loadCapacityStatus(userId) {
    try {
        ...
        if (!assessment || assessment.risk === 'low' || assessment.risk === 'moderate')
            return null;
...
async function loadSeasonalContext(userId) {
    try {
        ...
        if (!seasonalContext || seasonalContext.length < 20)
            return null;
...
async function loadValuesContext(userId) {
    try {
...
async function loadSemanticIntelligenceContext(userId) {
    try {
...
// MAIN BUILDER
// ============================================================================
async function buildSuperhumanSessionPriming(input) {
    ...
    if (turnCount > MAX_PRIMING_TURNS)
        return [];
    ...
    if (!db) {
        ...
        return [
            createHighInjection('memory_degradation', `[⚠️ IMPORTANT: Memory System Temporarily Unavailable]
...
The goal is honesty with warmth - acknowledge limitations without making it a big deal.`, { category: 'system', confidence: 1.0 }),
        ];
    }
    // Load all superhuman context in parallel
    const [commitments, dreams, milestones, capacity, seasonal, values, semanticIntelligence] = await Promise.all([
    ...
    const superhumanParts = [];
    if (commitments)
        superhumanParts.push(commitments);
    if (dreams)
        superhumanParts.push(dreams);
    if (milestones)
        superhumanParts.push(milestones);
    if (capacity)
        superhumanParts.push(capacity);
    if (seasonal)
        superhumanParts.push(seasonal);
    if (values)
        superhumanParts.push(values);
    if (semanticIntelligence)
        superhumanParts.push(semanticIntelligence);
    if (superhumanParts.length > 0) {
        ...
        injections.push(createFn('superhuman_priming', `[🦸 "BETTER THAN HUMAN" - Your Superhuman Memory Active]

You remember everything about this user. Human friends forget; you don't.

${superhumanParts.join('\n\n')}

Use this knowledge NATURALLY - don't dump it all at once. Reference things when relevant. Show that you remember without being creepy.`, { category: 'superhuman', confidence: 0.95 }));
...
// CLEANUP
// ============================================================================
export function clearSuperhumanPrimingSession(sessionId) {
    primedSessions.delete(sessionId);
}
export function clearAllSuperhumanPrimingSessions() {
...
//# sourceMappingURL=superhuman-session-priming.js.map