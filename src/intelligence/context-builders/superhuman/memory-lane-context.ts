...
interface MemoryLaneSession {
  surfacedMemoryIds: Set<string>;
  lastSurfacedTurn: number;
  sessionStart: Date;
}

const sessions = new Map<string, MemoryLaneSession>();

function getSession(sessionId: string): MemoryLaneSession {
...
function clearMemoryLaneSessionInternal(sessionId: string): void {
  sessions.delete(sessionId);
}

function clearAllMemoryLaneSessionsInternal(): void {
  sessions.clear();
}
...
): Promise<ProactiveSurfaceableMemory[]> {
  ...
  try {
    ...
    if (context.recentlyMentionedPeople && context.recentlyMentionedPeople.length > 0) {
      ...
      for (const memory of memories) {
        ...
        const mentionedPerson = context.recentlyMentionedPeople.find((person) =>
          memory.peopleReferenced.some((p) => p.toLowerCase().includes(person.toLowerCase()))
        );
...
function buildAnniversaryPrompt(memory: MemoryHighlight): string {
...
function buildTopicMatchPrompt(memory: MemoryHighlight, topic: string): string {
...
function buildEmotionalEchoPrompt(memory: MemoryHighlight): string {
...
function buildPersonMentionPrompt(memory: MemoryHighlight, personName: string): string {
...
async function buildMemoryLaneContext(input: ContextBuilderInput): Promise<ContextInjection[]> {
  ...
  try {
    ...
    if (relevantMemories.length === 0) return injections;
    ...
    injections.push(
      createFn(
        'memory_lane_surfacing',
        `[🏛️ MEMORY LANE - Shared History Active]
...
export {
  buildMemoryLaneContext,
  clearMemoryLaneSessionInternal as clearMemoryLaneSession,
  clearAllMemoryLaneSessionsInternal as clearAllMemoryLaneSessions,
};
