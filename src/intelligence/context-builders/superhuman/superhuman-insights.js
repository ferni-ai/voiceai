 ...
 */
export function clearSuperhumanInsightsSession(sessionId) {
    sessions.delete(sessionId);
}
/**
 * Clear all session data (for shutdown).
 */
export function clearAllSuperhumanInsightsSessions() {
    sessions.clear();
}
/**
 * Generate time-based insights from user profile
 */
function generateTimeBasedInsights(profile, currentTopics) {
    if (!profile)
        return [];
    const insights = [];
    const now = new Date();
    ...
    const goals = profile.customData?.goals;
    if (goals) {
    ...
    const topicHistory = profile.customData?.topicHistory;
    if (topicHistory) {
 ...
 */
async function buildSuperhumanInsights(input) {
    ...
    try {
        ...
        if (turnCount >= 2 &&
            ...
            if (reflectionMoments.length > 0) {
                const reflection = selectBestReflection(reflectionMoments, currentTopics, currentEmotion, turnCount);
                if (reflection && reflection.appropriateness > 0.5) {
                    injections.push(createHighInjection('superhuman_reflection', `[CROSS-SESSION REFLECTION]\n` +
                        `Something from our previous conversation to naturally weave in:\n` +
                        `→ "${reflection.phrase}"\n` +
                        `(Only mention if it feels natural, don't force it)`, { category: 'superhuman', confidence: reflection.appropriateness }));
        ...
        if (turnCount >= 3 && turnCount <= 6 && !session.surfacedInsights.has('time_based')) {
            ...
            if (timeInsights.length > 0) {
                ...
                if (turnCount - lastReflectionTurn >= 2) {
                    const createInjection = best.priority === 'high'
                        ? createHighInjection
                        : best.priority === 'standard'
                            ? createStandardInjection
                            : createHintInjection;
                    injections.push(createInjection('superhuman_time', `[TIME-BASED INSIGHT: ${best.type.toUpperCase()}]\n` +
                        `${best.content}\n` +
                        `→ Consider: "${best.suggestedPhrase}"`, { category: 'superhuman', confidence: best.relevance }));
        ...
        if (turnCount >= 5 && !session.surfacedInsights.has('subconscious')) {
            ...
            if (subconscious) {
                ...
                const lastInsightTurn = Math.max(session.lastSurfacedTurn['reflection'] || 0, session.lastSurfacedTurn['time_based'] || 0);
                if (turnCount - lastInsightTurn >= 2) {
                    injections.push(createHintInjection('superhuman_subconscious', subconscious + '\n(Surface gently when relevant, not as an accusation)', { category: 'superhuman' }));
        ...
        if (turnCount === 1 && userData.isReturningUser && !session.surfacedInsights.has('rhythm')) {
            ...
            if (rhythmPrediction.confidence > 0.6 &&
                ...
                if (combinedHint && rhythmPrediction.reasons.length > 0) {
                    injections.push(createHintInjection('superhuman_rhythm', `[LIFE RHYTHM AWARENESS]\n` +
                        `Based on patterns, ${combinedHint}.\n` +
                        `Reason: ${rhythmPrediction.reasons[0]}\n` +
                        `(Don't state this directly - just be attuned to it)`, { category: 'superhuman', confidence: rhythmPrediction.confidence }));
...
registerContextBuilder({
    ...
    description: 'Time-based memory, cross-session reflection, pattern recognition - the "better than human" moments',
 ...
 */
export function detectLinguisticPatterns(currentMessage, recentHistory = []) {
 ...
 */
export function detectRepeatedTopics(topics, topicHistory = {}) {
    // Need at least 3 topics to detect patterns
    if (topics.length < 3) {
        return null;
    }
    // Count current topics
    const counts = { ...topicHistory };
 ...
 */
export function analyzeEmotionalWeather(sessionCount, emotions) {
 ...
 */
export function detectAnticipatoryCues(text, voiceStressLevel) {
...
export { buildSuperhumanInsights, generateTimeBasedInsights, };
export default {
...
//# sourceMappingURL=superhuman-insights.js.map