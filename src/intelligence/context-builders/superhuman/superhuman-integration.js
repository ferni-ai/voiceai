...
function getCacheKey(userId, capability) {
    return capability ? `superhuman:${userId}:${capability}` : `superhuman:${userId}`;
}
function getTieredValue(userId, capability) {
    ...
    if (!cached)
        return null;
    const ttl = tier === 'stable' ? CACHE_TTL.STABLE : tier === 'normal' ? CACHE_TTL.NORMAL : CACHE_TTL.FRESH;
...
function setTieredValue(userId, capability, value) {
    const tier = CAPABILITY_TIERS[capability] ?? 'normal';
    const cacheMap = tieredCache[tier];
    const key = getCacheKey(userId, capability);
 ...
 */
function getPartialCachedContext(userId) {
    const partial = {};
    for (const capability of Object.keys(CAPABILITY_TIERS)) {
 ...
 */
export async function getSuperhuman(userId, persona, options) {
 ...
 */
function formatForPersona(context, persona) {
    ...
    for (const capability of config.priorityOrder) {
        if (totalLength >= config.maxTokens)
            break;
 ...
 */
export async function getCommitmentContext(userId) {
    try {
        const { buildCommitmentContext } = await import('../../../services/superhuman/commitment-keeper.js');
 ...
 */
export async function getPredictiveContext(userId, sessionId) {
    const sections = [];
    try {
        // 1. Core predictive coaching context
        const { buildPredictiveContextString } = await import('../../../services/superhuman/predictive-coaching.js');
    ...
    try {
        ...
        if (sessionId) {
            const { getPredictiveContextForTurn } = await import('../../../agents/integrations/predictive-intelligence-integration.js');
    ...
    try {
        ...
        const { getPatternsToSurface, generatePatternSurfacingQuestion } = await import('../../coaching-patterns.js');
 ...
 */
export async function getNarrativeContext(userId) {
    try {
        const { buildNarrativeContextString } = await import('../../../services/superhuman/life-narrative.js');
 ...
 */
export async function getValuesContext(userId) {
    try {
 ...
 */
export async function getCapacityContext(userId) {
    try {
 ...
 */
export async function getDreamContext(userId) {
    try {
 ...
 */
export async function getNetworkContext(userId) {
    try {
        const { buildNetworkContext } = await import('../../../services/superhuman/relationship-network.js');
 ...
 */
export async function getSeasonalContext(userId) {
    try {
 ...
 */
export function clearSuperhumanCache(userId) {
 ...
 */
export function clearAllSuperhumanCache() {
 ...
 */
export async function warmupSuperhumanCache(userId) {
    try {
...
export function getPerformanceStats() {
    ...
    const slowest = performanceLog.reduce((max, e) => (e.durationMs > (max?.durationMs || 0) ? e : max), null);
    return {
        totalCalls,
 ...
 */
export function withPerformanceTracking(builderName, fn, meta) {
    const startTime = Date.now();
    return fn().finally(() => {
        const duration = Date.now() - startTime;
        recordPerformance({
