 ...
 */
// Phase 1 imports
import { captureJoke, findRelevantJoke, formatJokeForPrompt, } from '../../../conversation/superhuman/inside-jokes.js';
import { detectMicroWin, formatMicroWinForPrompt, } from '../../../conversation/superhuman/micro-celebrations.js';
import { formatNaturalSpeechGuidance } from '../../../conversation/superhuman/natural-speech.js';
import { extractNameFromMessage, formatNamingGuidance, setUserName, updateEndearmentLevel, } from '../../../conversation/superhuman/nicknames.js';
import { captureQuote, findRelevantQuote, formatQuoteForPrompt, markQuoteSurfaced, } from '../../../conversation/superhuman/quote-memory.js';
import { acknowledgeMilestone, checkMilestones, formatMilestoneForPrompt, recordConversation, recordLaugh, } from '../../../conversation/superhuman/relationship-milestones.js';
import { extractPerson, findPeopleToAskAbout, formatFollowUpForPrompt, getOrCreatePerson, } from '../../../conversation/superhuman/story-continuity.js';
// Phase 2 imports
import { formatRitualGuidance } from '../../../conversation/superhuman/conversational-rituals.js';
import { formatForecastGuidance, shouldMentionForecast, } from '../../../conversation/superhuman/emotional-forecasting.js';
...
import { formatPresenceGuidance, shouldAvoidAdvice, } from '../../../conversation/superhuman/presence-mode.js';
import { extractSharedLanguage, formatSharedLanguageGuidance, } from '../../../conversation/superhuman/shared-language.js';
 ...
 */
export function clearSuperpowersSession(sessionId) {
    sessionData.delete(sessionId);
}
/**
 * Clear all session data (for shutdown).
 */
export function clearAllSuperpowersSessions() {
    sessionData.clear();
}
// ============================================================================
// CONTEXT BUILDER
// ============================================================================
async function buildConversationalSuperpowers(input) {
    ...
    const relationshipStage = calculateRelationshipStage(services?.userProfile?.totalConversations || 0);
    ...
    if (!data.quoteSurfacedThisSession && turnCount >= 2) {
        ...
        if (relevantQuote && relevantQuote.relevanceScore > 15) {
            injections.push(createHintInjection('conversational_quote_callback', formatQuoteForPrompt(relevantQuote), {
                category: 'superhuman',
            }));
            markQuoteSurfaced(userId, relevantQuote.quote.id);
            data.quoteSurfacedThisSession = true;
            log.info({ userId, quoteId: relevantQuote.quote.id, score: relevantQuote.relevanceScore }, '💬 BETTER THAN HUMAN: Quote callback surfaced - "Last time you said..."');
        }
    }
    ...
    if (turnCount === 1 && relationshipStage !== 'stranger' && !data.sessionContinuationDone) {
        ...
        if (continuationQuote && continuationQuote.relevanceScore > 10) {
            injections.push(createHintInjection('session_continuation', `[SESSION CONTINUATION - BETTER THAN HUMAN]
...
Don't force it - only bring it up if it flows naturally from what they say.`, { category: 'superhuman' }));
    ...
    if (!data.milestoneSurfacedThisSession && turnCount <= 2) {
        ...
        if (milestones.length > 0) {
            ...
            injections.push(createHintInjection('conversational_milestone', formatMilestoneForPrompt(milestone), {
                category: 'superhuman',
            }));
    ...
    if (!data.jokeSurfacedThisSession &&
        turnCount >= 4 &&
        currentEmotion !== 'sad' &&
        ...
        if (relevantJoke) {
            injections.push(createHintInjection('conversational_inside_joke', formatJokeForPrompt(relevantJoke), {
                category: 'superhuman',
            }));
    ...
    if (!data.personFollowUpThisSession && turnCount >= 5) {
        ...
        if (followUp) {
            injections.push(createHintInjection('conversational_person_followup', formatFollowUpForPrompt(followUp), {
                category: 'superhuman',
            }));
    ...
    if (microWin) {
        injections.push(createHintInjection('conversational_micro_win', formatMicroWinForPrompt(microWin), {
            category: 'superhuman',
        }));
    ...
    injections.push(createHintInjection('conversational_natural_speech', speechGuidance, { category: 'persona' }));
    // ============================================================================
    // 5. NAMING GUIDANCE
    // ============================================================================
    const namingContext = {
    ...
    if (namingGuidance) {
        injections.push(createHintInjection('conversational_naming', namingGuidance, { category: 'superhuman' }));
    }
    // ============================================================================
    ...
    if (!data.vulnerabilityGuidanceGiven && isPersonalSharing) {
        ...
        if (vulnGuidance) {
            injections.push(createHintInjection('conversational_vulnerability', vulnGuidance, {
                category: 'superhuman',
            }));
    ...
    if (!data.reflectionGivenThisSession && isPersonalSharing && turnCount >= 2) {
        ...
        if (reflectionGuidance) {
            injections.push(createHintInjection('conversational_reflection', reflectionGuidance, {
                category: 'superhuman',
            }));
    ...
    if (!data.presenceGuidanceGiven) {
        ...
        if (presenceGuidance) {
            injections.push(createHintInjection('conversational_presence', presenceGuidance, {
                category: 'superhuman',
            }));
    ...
    if (!data.sharedLanguageSurfaced && turnCount >= 5) {
        ...
        if (langGuidance) {
            injections.push(createHintInjection('conversational_shared_language', langGuidance, {
                category: 'superhuman',
            }));
    ...
    if (!data.ritualSuggestedThisSession) {
        ...
        if (ritualGuidance) {
            injections.push(createHintInjection('conversational_ritual', ritualGuidance, { category: 'superhuman' }));
    ...
    if (!data.forecastGivenThisSession && turnCount >= 8 && data.hadDeepSharing) {
        ...
        if (shouldMentionForecast(forecastContext)) {
            ...
            if (forecastGuidance) {
                injections.push(createHintInjection('conversational_forecast', forecastGuidance, {
                    category: 'superhuman',
                }));
    ...
    if (!data.challengeGivenThisSession && !inPresenceMode && turnCount >= 5) {
        ...
        if (challengeGuidance) {
            injections.push(createHintInjection('conversational_challenge', challengeGuidance, {
                category: 'superhuman',
            }));
    ...
    if (!data.metaMomentThisSession && turnCount >= 7) {
        const moodShift = data.sessionStartMood && data.sessionStartMood !== currentEmotion
        ...
        if (metaGuidance) {
            injections.push(createHintInjection('conversational_meta_moment', metaGuidance, { category: 'superhuman' }));
...
// HELPERS
// ============================================================================
function calculateRelationshipStage(conversationCount) {
...
function getPersonaStyle(personaId) {
    const styleMap = {
        ferni: 'warm',
...
//# sourceMappingURL=conversational-superpowers.js.map