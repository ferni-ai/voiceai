...
import { wasHighImpactActionExecuted, getHumanReadableSummary, } from '../../../agents/shared/action-history.js';
 ...
 */
function detectCapabilityQuestion(transcript) {
    ...
    for (const [actionType, patterns] of Object.entries(CAPABILITY_QUESTION_PATTERNS)) {
        for (const pattern of patterns) {
            if (pattern.test(transcript)) {
                ...
                log.debug({ actionType, contact, confidence, transcript: transcript.slice(0, 50) }, '🔍 Capability question detected');
                return {
 ...
 */
export function buildHonestyContext(sessionId, userTranscript) {
 ...
 */
export function getHonestyInjection(sessionId, userTranscript) {
 ...
 */
export function getSessionActionSummary(sessionId) {
...
//# sourceMappingURL=honesty-guardrail.js.map