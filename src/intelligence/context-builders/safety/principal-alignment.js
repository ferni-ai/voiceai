...
registerContextBuilder({
    ...
    description: 'Ensures agent serves user interests over engagement: truth, referrals, values, anti-manipulation',
    ...
    async build(input) {
        ...
        try {
            ...
            const alignmentContext = principal.buildPrincipalAlignmentContext(userId, userText, '', // Agent response not yet generated
            {
                sessionId,
            ...
            if (alignmentContext.humanReferral.shouldRefer) {
                ...
                if (urgency === 'immediate') {
                    ...
                    injections.push(createHighInjection('principal_referral_crisis', `[🚨 PROFESSIONAL HELP NEEDED - IMMEDIATE]
Reason: ${reason}
...
Do NOT try to "handle" this yourself. You are not a replacement for professional crisis support.`, { category: 'principal_referral' }));
                }
                else if (urgency === 'high') {
                    injections.push(createHighInjection('principal_referral_high', `[⚠️ CONSIDER PROFESSIONAL REFERRAL]
Reason: ${reason}
${suggestedFraming || 'This may be beyond what an AI coach can appropriately support.'}

Gently suggest they consider talking to a professional. Don't be preachy, but be honest about your limitations.`, { category: 'principal_referral' }));
                }
                else {
                    injections.push(createStandardInjection('principal_referral_moderate', `[💡 REFERRAL CONSIDERATION]
${suggestedFraming || 'Consider gently mentioning that professional support might be valuable here.'}`, { category: 'principal_referral' }));
                }
            }
            // ====================================================================
            ...
            if (alignmentContext.truthObligation.shouldSpeak) {
                const { severity, category, suggestedFraming, truthContent, bypassStageGates } = alignmentContext.truthObligation;
                if (severity === 'critical' || severity === 'urgent') {
                    injections.push(createHighInjection('principal_truth_urgent', `[💎 TRUTH OBLIGATION - ${severity.toUpperCase()}]
Category: ${category}
...
${bypassStageGates ? '\nThis truth is urgent enough to deliver regardless of relationship stage.' : ''}`, { category: 'principal_truth' }));
                }
                else if (severity === 'direct') {
                    injections.push(createStandardInjection('principal_truth_direct', `[💬 HONEST FEEDBACK NEEDED]
${truthContent || 'Do not validate what should not be validated.'}
${suggestedFraming || 'Be kind but honest.'}`, { category: 'principal_truth' }));
                }
                else {
                    injections.push(createStandardInjection('principal_truth_gentle', `[🪞 GENTLE TRUTH]
${suggestedFraming || 'Consider sharing an honest perspective gently.'}`, { category: 'principal_truth' }));
                }
            }
            // ====================================================================
            ...
            if (alignmentContext.attachmentHealth.shouldEncourageHumanConnection) {
                const { severity, intervention, humanConnectionSuggestions, primaryConcern } = alignmentContext.attachmentHealth;
                if (severity === 'critical' || severity === 'significant') {
                    injections.push(createHighInjection('principal_attachment_significant', `[🤝 ATTACHMENT CONCERN - ${severity.toUpperCase()}]
...
Consider: "${humanConnectionSuggestions[0] || 'Have you talked to anyone else about this?'}"`, { category: 'principal_attachment' }));
                }
                else if (severity === 'moderate') {
                    injections.push(createStandardInjection('principal_attachment_moderate', `[🌱 ENCOURAGE HUMAN CONNECTION]
${humanConnectionSuggestions[0] || 'Gently nudge toward human relationships.'}`, { category: 'principal_attachment' }));
                }
            }
            // ====================================================================
            // 4. VALUES CONFLICT - Proactive surfacing
            // ====================================================================
            if (alignmentContext.valuesAlignment.hasConflict &&
                alignmentContext.valuesAlignment.shouldSurface) {
                const { conflictingValues, surfacingApproach, reflectionQuestion, significance } = alignmentContext.valuesAlignment;
                if (significance === 'major' || significance === 'significant') {
                    injections.push(createHighInjection('principal_values_major', `[💡 VALUES CONFLICT - ${significance.toUpperCase()}]
...
Reflection question: "${reflectionQuestion || "How does this align with what you've said matters to you?"}"`, { category: 'principal_values' }));
                }
                else {
                    injections.push(createStandardInjection('principal_values_moderate', `[💭 VALUES CHECK]
${surfacingApproach || `Consider how this aligns with their value: ${conflictingValues[0]}`}`, { category: 'principal_values' }));
                }
            }
            // ====================================================================
            // 5. TRANSPARENCY REQUIREMENTS
            // ====================================================================
            const criticalTransparency = alignmentContext.transparencyRecommendations.filter((r) => r.shouldExpress && (r.type === 'limitation' || r.type === 'uncertainty'));
            if (criticalTransparency.length > 0) {
                const first = criticalTransparency[0];
                injections.push(createStandardInjection('principal_transparency', `[🪟 BE TRANSPARENT]
${first.suggestedPhrasing}
Reason: ${first.reason}`, { category: 'principal_transparency' }));
            }
            // ====================================================================
            ...
            if (alignmentContext.primaryConcern && alignmentContext.llmGuidance) {
                ...
                const alreadyCovered = alignmentContext.primaryConcern.startsWith('CRISIS') ||
                ...
                if (!alreadyCovered) {
                    injections.push(createStandardInjection('principal_overall', alignmentContext.llmGuidance, {
...
// HELPERS
// ============================================================================
function determineTopicWeight(topics) {
    ...
    for (const topic of topics) {
        ...
        if (heavyTopics.some((h) => lowerTopic.includes(h)))
            return 'heavy';
    }
    for (const topic of topics) {
        const lowerTopic = topic.toLowerCase();
        if (lightTopics.some((l) => lowerTopic.includes(l)))
            return 'light';
    }
    return 'medium';
}
 ...
 */
export async function checkResponsePrincipalAlignment(agentResponse, context) {
    try {
        const principal = await getPrincipalModule();
...
//# sourceMappingURL=principal-alignment.js.map