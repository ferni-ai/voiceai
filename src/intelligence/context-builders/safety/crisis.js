...
import { createCriticalInjection, createStandardInjection, registerContextBuilder, } from '../index.js';
...
function mapCrisisType(type) {
    if (type.includes('market') || type.includes('financial'))
        return 'financial';
    if (type.includes('grief') || type.includes('loss'))
        return 'emotional';
...
async function recordCrisisEpisode(userId, type, severity, description, _distressLevel) {
    if (!userId)
        return;
    try {
        const episodeData = {
 ...
 */
const MARKET_PANIC_PATTERNS = /\b(sell everything|get out of the market|cash out|panic|market crash|losing everything|should i sell|pull out my money|move to cash|can't take it anymore|scared of market|market is tanking)\b/i;
/**
 * Grief patterns with loss type categorization
 */
const GRIEF_PATTERNS = [
    {
        pattern: /\b(passed away|died|death|funeral|lost my|mourning|grieving|miss them|miss him|miss her)\b/i,
        lossType: 'person',
    },
...
const LIFE_EVENT_PATTERNS = [
    ...
    {
        pattern: /\b(cancer|heart attack|diagnosis|sick|illness|hospital|surgery|medical|health crisis)\b/i,
 ...
 */
function buildCrisisContext(input) {
    ...
    // -----------------------------------------------
    if (MARKET_PANIC_PATTERNS.test(userText) &&
        ...
        void recordCrisisEpisode(input.services?.userId, 'market_panic', distressLevel >= DISTRESS.HIGH ? 'critical' : 'high', 'Market panic detected - user expressing fear about selling investments', distressLevel);
        injections.push(createCriticalInjection('market_panic', `[MARKET PANIC DETECTED - CRITICAL INTERVENTION]
...
DO NOT: Promise the market will go up. DO: Promise you'll be here to talk.`, { category: 'crisis' }));
    }
    // -----------------------------------------------
    // GRIEF DETECTION
    ...
    for (const { pattern, lossType } of GRIEF_PATTERNS) {
        if (pattern.test(userText) && distressLevel >= DISTRESS.MODERATE) {
            ...
            void recordCrisisEpisode(input.services?.userId, `grief_${lossType}`, distressLevel >= DISTRESS.HIGH ? 'high' : 'moderate', `Grief detected - ${lossType} loss`, distressLevel);
            injections.push(createCriticalInjection('grief', `[GRIEF DETECTED - ${lossType.toUpperCase()}]
...
DO SAY:
  ...
  - [Silence is okay. Even preferred.]`, { category: 'crisis' }));
    ...
    for (const { pattern, eventType } of LIFE_EVENT_PATTERNS) {
        if (pattern.test(userText)) {
            ...
            if (guidance) {
                ...
                injections.push(createStandardInjection('life_event', `[LIFE EVENT: ${eventType}] Lead with empathy, not advice.`, { category: 'crisis' }));
            }
