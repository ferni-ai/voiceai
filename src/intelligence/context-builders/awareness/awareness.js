 ...
 */
import { decideTangent, } from '../../../conversation/mid-response-tangents.js';
import { getMomentumTracker, } from '../../../conversation/momentum-tracker.js';
import { getSelfAwarenessTracker, } from '../../../conversation/self-awareness-loop.js';
import { calculateThinkingTime, } from '../../../conversation/thinking-time-injector.js';
...
const DEFAULT_AWARENESS_PROFILE = {
...
function detectResponseType(text) {
    ...
    if (/\?$/.test(text.trim()))
        return 'question';
    if (/\b(I think|in my opinion|I believe|my take)\b/i.test(lowerText))
        return 'advice';
    if (/\b(I hear|sounds like|it seems|you('re| are) feeling)\b/i.test(lowerText))
        return 'reflection';
    if (/\b(that makes sense|I understand|valid|of course)\b/i.test(lowerText))
        return 'validation';
    if (/\b(remember when|there was this time|story|once)\b/i.test(lowerText))
        return 'story';
    if (/\b(but have you|what if|consider|challenge)\b/i.test(lowerText))
        return 'challenge';
    if (/\b(haha|lol|funny|joke)\b/i.test(lowerText))
        return 'humor';
    return 'information';
}
// ============================================================================
// MAIN BUILDER
// ============================================================================
async function buildAwarenessContext(input) {
    ...
    if (Math.random() < profile.momentumGuidanceProbability) {
        ...
        if (suggestions.length > 0 && suggestions[0].confidence > 0.6) {
            ...
            switch (suggestion.type) {
                case 'lean_in':
                    guidance =
                        profile.phrases.feeling_energy[Math.floor(Math.random() * profile.phrases.feeling_energy.length)];
                    injections.push(createHintInjection('awareness_momentum', `[AWARENESS: ${guidance}]`));
                    break;
                case 'acknowledge_depth':
                    guidance =
                        profile.phrases.sensing_depth[Math.floor(Math.random() * profile.phrases.sensing_depth.length)];
                    injections.push(createHighInjection('awareness_depth', `[AWARENESS: ${guidance}]`));
                    break;
                case 'gently_pivot':
                    injections.push(createStandardInjection('awareness_pivot', '[AWARENESS: Conversation needs new energy. Try a gentle topic shift or ask an open question.]'));
                    break;
                case 'give_space':
                    guidance =
                        profile.phrases.moment_of_presence[Math.floor(Math.random() * profile.phrases.moment_of_presence.length)];
                    injections.push(createHintInjection('awareness_space', `[AWARENESS: ${guidance}]`));
                    break;
                case 'wrap_opportunity':
                    injections.push(createHintInjection('awareness_wrap', '[AWARENESS: Natural ending approaching. Offer a graceful exit or invite them to continue.]'));
                    break;
            }
        }
    }
    ...
    // 3. TANGENT AWARENESS
    // =========================================================================
    const tangentDecision = decideTangent(sessionId, personaId, userText, turnCount, relationshipDepth);
    ...
    if (momentum.phase === 'opening' && turnCount <= 2) {
        injections.push(createHintInjection('awareness_opening', '[AWARENESS: Early in conversation. Focus on warmth and understanding before depth.]'));
    }
    else if (momentum.phase === 'deep' && momentum.topicDepth > 5) {
        injections.push(createHintInjection('awareness_deep', "[AWARENESS: Deep in topic. User trusts you with this. Don't rush or pivot prematurely.]"));
    }
    // =========================================================================
    // 6. EMOTIONAL INTENSITY AWARENESS
    // =========================================================================
    if (emotionalIntensity > 0.7) {
        const phrase = profile.phrases.sensing_depth[Math.floor(Math.random() * profile.phrases.sensing_depth.length)];
        injections.push(createHighInjection('awareness_emotion', `[EMOTIONAL AWARENESS: High intensity detected. ${phrase}]`));
    }
    log.debug({
        sessionId,
        personaId,
...
registerContextBuilder({
    name: 'awareness',
    description: 'Integrates momentum, thinking time, tangents, and self-awareness for human-like presence',
