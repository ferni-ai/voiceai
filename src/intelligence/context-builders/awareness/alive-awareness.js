...
import { getTeamContext, formatCrossAgentContextForPrompt, } from '../../../services/cross-agent-awareness.js';
import { updateSessionState, getPhysicalStateComment, getMetacognitiveComment, getTemporalAnchor, } from '../../../services/embodied-awareness.js';
import { processMoodDrift, getMoodExpression } from '../../../services/mood-drift.js';
function getWorldContext() {
    ...
    if (month === 11 && date >= 20 && date <= 25)
        specialDay = 'holiday_season';
    if (month === 0 && date === 1)
        specialDay = 'new_years';
    if (month === 1 && date === 14)
        specialDay = 'valentines';
    // Add more as needed
    return { timeOfDay, dayOfWeek, isWeekend, season, specialDay };
}
function formatWorldContextForPrompt(world) {
    const parts = [];
    ...
    if (world.specialDay) {
        const specialDayNotes = {
            holiday_season: '[Holiday season - be mindful that this can be both joyful and stressful for people]',
            new_years: "[New Year's Day - people often reflect on fresh starts and resolutions]",
            valentines: "[Valentine's Day - be sensitive to relationship status and feelings of loneliness]",
        };
...
function selectCuriosityQuestion(personaId, turnCount, relationshipStage) {
    // Ask curiosity questions frequently - Ferni should be genuinely curious!
    // Increased from 35% to 55% based on user feedback that Ferni wasn't asking enough questions
    // The genuine-curiosity.json file has even more questions that can be loaded via behaviors
    if (turnCount < 2 || Math.random() > 0.55)
        return null;
...
// MAIN BUILDER
// ============================================================================
export async function buildAliveAwarenessContext(input) {
    const log = getLogger();
    const injections = [];
    const summaryParts = [];
 ...
 */
export function formatAliveAwarenessForPrompt(result) {
    if (result.injections.length === 0) {
        return '';
    }
    const lines = ['', '=== ALIVE AWARENESS ==='];
 ...
 */
async function buildAliveAwarenessContextWrapper(input) {
...
//# sourceMappingURL=alive-awareness.js.map