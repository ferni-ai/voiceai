...
import { hasLinkedInConnected, initializeLinkedIn, generateLinkedInInsight, getUpcomingMilestones, getCurrentPosition, getLinkedInProfile, syncLinkedInData, } from '../../../services/linkedin/index.js';
 ...
 */
export const linkedInAwarenessBuilder = {
    ...
    build: async (input) => {
        ...
        if (insight && canInject) {
            if (insight.priority === 'high') {
                injections.push(createHighInjection('linkedin_milestone', insight.message, {
                    category: 'career-intelligence',
                }));
            }
            else if (insight.priority === 'low') {
                injections.push(createHintInjection('linkedin_awareness', insight.message, {
                    category: 'career-intelligence',
                }));
            }
            else {
                injections.push(createStandardInjection('linkedin_awareness', insight.message, {
                    category: 'career-intelligence',
                }));
            }
        ...
        const isCareerConversation = userText.includes('work') ||
            userText.includes('job') ||
            userText.includes('career') ||
            userText.includes('boss') ||
            userText.includes('coworker') ||
            userText.includes('promotion') ||
            analysis?.topics?.detected?.some((t) => ['career', 'work', 'job', 'professional'].includes(t.toLowerCase()));
        if (isCareerConversation && canInject) {
            ...
            if (currentPosition) {
                ...
                if (yearsInRole > 0) {
                    injections.push(createHintInjection('linkedin_context', `[CONTEXT: User has been ${currentPosition.title} at ${currentPosition.companyName} for ~${yearsInRole} year${yearsInRole > 1 ? 's' : ''}. You can reference this naturally if relevant.]`, {
                        category: 'career-intelligence',
                    }));
                }
            }
            ...
            if (milestones.length > 0 && !insight) {
                ...
                const daysUntil = Math.ceil((nextMilestone.date.getTime() - Date.now()) / (24 * 60 * 60 * 1000));
                if (daysUntil <= 14) {
                    injections.push(createStandardInjection('linkedin_upcoming', `[MILESTONE: ${nextMilestone.description} coming up in ${daysUntil} days. Could acknowledge if appropriate.]`, {
                        category: 'career-intelligence',
                    }));
                }
            }
...
//# sourceMappingURL=linkedin-awareness.js.map