...
import { getActiveWorkflows, } from '../../../services/stores/workflow-store.js';
...
const ROUTINE_AWARE_PERSONAS = ['ferni', 'maya-santos'];
 ...
 */
function formatTrigger(workflow) {
 ...
 */
function workflowToSummary(workflow) {
    // Map workflow status to our expected type
    const status = workflow.status === 'active' ? 'active' :
 ...
 */
async function buildRoutineAwareness(input) {
 ...
 */
function buildContextInjection(routines, recentlyRun, needsAttention) {
    const parts = [];
    // Overview
...
//# sourceMappingURL=routine-awareness.js.map