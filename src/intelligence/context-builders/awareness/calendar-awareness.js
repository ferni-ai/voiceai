...
import { detectCalendarAlerts, } from '../../../services/calendar/calendar-intelligence.js';
// Better Than Human calendar services
import { getAmbientCalendarContext, generateAmbientContextInjection, shouldInterruptForCalendar, } from '../../../services/calendar/ambient-calendar-awareness.js';
import { getCalendarLoadFactors, getCalendarLoadSummary, } from '../../../services/calendar/calendar-load-service.js';
import { detectRecoveryNeeds, buildRecoveryContext, } from '../../../services/calendar/recovery-protection.js';
import { registerContextBuilder, createStandardInjection, createHighInjection, } from '../index.js';
import { BuilderCategory } from '../core/categories.js';
const log = createLogger({ module: 'context:calendar-awareness' });
const CALENDAR_DETAIL_LEVELS = {
...
const BTH_FULL_PERSONAS = ['alex-chen'];
// Personas that get lite BTH features (load factors, recovery suggestions)
const BTH_LITE_PERSONAS = ['ferni', 'maya-patel', 'nayan-kumar'];
// ============================================================================
// MAIN BUILDER
// ============================================================================
/**
 ...
 */
export async function buildCalendarAwarenessContext(userId, personaId) {
    ...
    try {
        ...
        const contextInjection = buildContextInjectionForPersona(personaId, todayOverview, alerts, nextMeetingIn);
        // =========================================================================
        ...
        let loadFactors;
        let recoveryNeeds;
        let betterThanHumanInjection = null;
        const isFullBTH = BTH_FULL_PERSONAS.includes(personaId);
        const isLiteBTH = BTH_LITE_PERSONAS.includes(personaId);
        ...
        log.debug({ userId, personaId, hasContext: !!contextInjection, hasBTH: !!betterThanHumanInjection }, 'Calendar awareness context built');
        return {
            isConnected: true,
            todayOverview,
            alerts,
 ...
 */
function buildContextInjectionForPersona(personaId, overview, alerts, nextMeetingIn) {
 ...
 */
function buildJordanCalendarContext(overview, alerts) {
    const parts = [];
    ...
    const eventAlerts = alerts.filter((a) => a.message.toLowerCase().includes('birthday') ||
 ...
 */
function buildFerniCalendarContext(overview, nextMeetingIn) {
 ...
 */
function buildMayaCalendarContext(overview, nextMeetingIn) {
    const parts = [];
 ...
 */
function buildPeterCalendarContext(overview, nextMeetingIn) {
    const parts = [];
 ...
 */
function buildNayanCalendarContext(overview, alerts) {
    const parts = [];
    ...
    const stressAlerts = alerts.filter((a) => a.severity === 'warning' ||
 ...
 */
function buildContextInjectionText(overview, alerts, nextMeetingIn) {
    const parts = [];
    // Calendar snapshot
    if (overview.totalMeetings > 0) {
 ...
 */
export function formatCalendarContextForSpeech(context) {
 ...
 */
export const calendarAwarenessBuilder = {
    ...
    build: async (input) => {
        ...
        if (context.contextInjection) {
            injections.push(createStandardInjection('calendar_awareness', context.contextInjection, {
                category: 'calendar',
            }));
        }
        ...
        if (context.betterThanHumanInjection) {
            ...
            if (context.ambientContext && shouldInterruptForCalendar(context.ambientContext)) {
                injections.push(createHighInjection('calendar_urgent', context.betterThanHumanInjection, {
                    category: 'calendar-urgent',
                }));
            }
            else {
                injections.push(createStandardInjection('calendar_bth', context.betterThanHumanInjection, {
                    category: 'calendar-better-than-human',
                }));
            }
        }
        if (injections.length > 0) {
            log.debug({ userId, personaId, connected: context.isConnected, injectionCount: injections.length }, 'Calendar awareness context injected');
        }
        return injections;
    },
};
...
//# sourceMappingURL=calendar-awareness.js.map