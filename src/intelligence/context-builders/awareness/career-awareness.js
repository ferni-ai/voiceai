 ...
 */
function detectCareerTopic(text) {
 ...
 */
function analyzeCareerSentiment(text) {
    const lowerText = text.toLowerCase();
    let positiveScore = 0;
    let negativeScore = 0;
 ...
 */
function calculateTrend(mentions) {
    ...
    const avgFirst = firstHalf.reduce((sum, m) => sum + (m.sentiment === 'positive' ? 1 : m.sentiment === 'negative' ? -1 : 0), 0) / firstHalf.length;
    const avgSecond = secondHalf.reduce((sum, m) => sum + (m.sentiment === 'positive' ? 1 : m.sentiment === 'negative' ? -1 : 0), 0) / secondHalf.length;
 ...
 */
function updateCareerProfile(userId, mention) {
    ...
    // Recalculate metrics
    const sentimentScores = profile.mentions.map((m) => m.sentiment === 'positive' ? 1 : m.sentiment === 'negative' ? -1 : 0);
 ...
 */
function getCareerProfile(userId) {
    return careerProfiles.get(userId) || null;
}
/**
 * Generate career insight from profile
 */
function generateCareerInsight(profile) {
    const thirtyDaysAgo = Date.now() - 30 * 24 * 60 * 60 * 1000;
    const recentNegative = profile.mentions.filter((m) => m.timestamp > thirtyDaysAgo && m.sentiment === 'negative').length;
    const recentPositive = profile.mentions.filter((m) => m.timestamp > thirtyDaysAgo && m.sentiment === 'positive').length;
    ...
    const daysSinceCareerMention = profile.mentions.length > 0
        ? Math.floor((Date.now() - profile.mentions[profile.mentions.length - 1].timestamp) /
            (24 * 60 * 60 * 1000))
        : 999;
 ...
 */
export const careerAwarenessBuilder = {
    ...
    build: async (input) => {
        ...
        if (careerDetection.isCareerRelated) {
            ...
            log.debug({ userId, sentiment: mention.sentiment, topics: careerDetection.topics }, 'Career mention detected and tracked');
            ...
            if (profile.mentions.length >= 3) {
                ...
                if (insight && insight.priority !== 'low') {
                    if (insight.priority === 'high') {
                        injections.push(createHighInjection('career_awareness', insight.message, {
                            category: 'career-intelligence',
                        }));
                    }
                    else {
                        injections.push(createStandardInjection('career_awareness', insight.message, {
                            category: 'career-intelligence',
                        }));
                    }
                }
            ...
            if (profile && profile.trendDirection === 'declining') {
                ...
                if (recentNegative >= 3) {
                    injections.push(createStandardInjection('career_pattern', `[AWARENESS: User has shown declining career sentiment recently (${recentNegative} concerns in past month). If work comes up naturally, approach with extra care and curiosity.]`, {
                        category: 'career-intelligence',
                    }));
                }
            }
...
//# sourceMappingURL=career-awareness.js.map