 ...
 */
function stageToTrustLevel(stage) {
    switch (stage) {
        case 'stranger':
            return 0.1;
        case 'acquaintance':
...
// BUILDER
// ============================================================================
export const revelationAwarenessBuilder = {
    ...
    build: async (input) => {
        ...
        const { getAntiSurveillanceGuidance } = await import('../../../services/revelation-moments/anti-surveillance.js');
        const { getThrottleState, getAvailableRevelations } = await import('../../../services/revelation-moments/throttling.js');
        const { getPermissionGuidance } = await import('../../../services/revelation-moments/permission-prompts.js');
        // =========================================================================
        ...
        if (turnCount <= 2) {
            ...
            injections.push(createStandardInjection('anti_surveillance', antiSurveillanceGuidance, {
                category: 'revelation',
                confidence: 1.0,
            }));
        }
            ...
            injections.push(createHintInjection('anti_surveillance_reminder', '[LANGUAGE] Say "I noticed" not "Our records show". Observations, not data.', {
                category: 'revelation',
                confidence: 0.9,
            }));
        }
        ...
        try {
            ...
            if (throttleState.usedThisSession.length > 0) {
                ...
                if (throttleState.shouldHoldBack) {
                    injections.push(createStandardInjection('throttle_holdback', `[HOLD BACK]
...
Just be present and warm. Don't show off.`, {
                    ...
                    injections.push(createHintInjection('throttle_notice', `[NOTE] Already used: ${usedList} this session. Space out capabilities.`, {
                        category: 'revelation',
                        confidence: 0.8,
                    }));
                }
            ...
            if (throttleState.blockedCategories.length > 0) {
                log.debug({ userId, blocked: throttleState.blockedCategories }, 'Some capabilities blocked by throttle');
            }
        }
        catch (error) {
        ...
        try {
            ...
            if (availableRevelations.length > 0 && sessionNumber >= 3) {
                ...
                if (permissionGuidance) {
                    injections.push(createHintInjection('permission_guidance', permissionGuidance, {
                        category: 'revelation',
                        confidence: 0.85,
                    }));
                }
        ...
        // =========================================================================
        if (sessionNumber <= 2) {
            injections.push(createHintInjection('first_meeting_restraint', `[EARLY RELATIONSHIP]
...
Impressive features come later when trust is earned.`, {
        ...
        log.debug({ userId, sessionNumber, injectionCount: injections.length }, '🎭 Revelation awareness guidance generated');
        return injections;
    },
};
 ...
 */
export async function checkBeforeReveal(userId, sessionId, capability, context) {
    try {
 ...
 */
export async function afterReveal(userId, sessionId, capability, personaId, context) {
    try {
...
//# sourceMappingURL=revelation-awareness.js.map