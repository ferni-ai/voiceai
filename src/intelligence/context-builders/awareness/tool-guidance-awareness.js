...
// CONTEXT BUILDER
// ============================================================================
export const dynamicToolGuidanceBuilder = {
    ...
    build: async (input) => {
        ...
        for (const trigger of TOOL_TRIGGERS) {
            ...
            if (matchedTools.has(trigger.toolName))
                continue;
            // Check if any pattern matches
            const matches = trigger.patterns.some((pattern) => pattern.test(userText));
            if (matches) {
                matchedTools.add(trigger.toolName);
                log.debug({ tool: trigger.toolName, text: userText.slice(0, 50) }, '🎯 Dynamic tool guidance triggered');
                injections.push(createInjection(`tool_hint_${trigger.toolName}`, trigger.guidance, trigger.priority, // 'high' or 'normal'
...
//# sourceMappingURL=tool-guidance-awareness.js.map