...
const sessionDeviceContext = new Map<string, DeviceContext>();
 ...
 */
export function updateDeviceContext(sessionId: string, payload: Partial<DeviceContext>): void {
  const existing = sessionDeviceContext.get(sessionId);

  const updated: DeviceContext = {
  ...
  log.debug(
    { sessionId, deviceType: updated.deviceType, platform: updated.platform },
 ...
 */
export function getDeviceContext(sessionId: string): DeviceContext | null {
 ...
 */
export function clearDeviceContext(sessionId: string): void {
 ...
 */
export function inferDeviceFromUserAgent(userAgent?: string): Partial<DeviceContext> {
...
interface DeviceInsight {
  type: 'adaptation' | 'awareness' | 'suggestion';
  message: string;
 ...
 */
function generateDeviceInsight(context: DeviceContext, hour: number): DeviceInsight | null {
  ...
  if (context.inMotion) {
    insights.push({
      type: 'adaptation',
      message:
        'User appears to be in motion (walking/commuting). Keep responses brief and conversational. Avoid suggesting actions that require hands.',
      priority: 'high',
    });
  }
  ...
  if (context.deviceType === 'mobile' && context.hasHeadphones === false) {
    insights.push({
      type: 'awareness',
      message:
        'User is on mobile without headphones - someone nearby might hear. Be mindful of sensitive topics.',
      priority: 'normal',
    });
  }
  ...
  if (context.privacyMode) {
    insights.push({
      type: 'adaptation',
      message:
        'User has indicated privacy mode - someone else may be present. Keep responses appropriate for others to hear.',
      priority: 'high',
    });
  }
 ...
 */
export const deviceAwarenessBuilder: ContextBuilder = {
  ...
  build: async (input: ContextBuilderInput): Promise<ContextInjection[]> => {
    ...
    const userAgent = (userData as { userAgent?: string })?.userAgent;
    ...
    if (insight) {
      if (insight.priority === 'high') {
        injections.push(
          createHighInjection('device_awareness', insight.message, {
            category: 'device-context',
          })
        );
      } else {
        injections.push(
          createStandardInjection('device_awareness', insight.message, {
            category: 'device-context',
          })
        );
      }
    ...
    if (context.deviceType === 'mobile' && !insight) {
      injections.push(
        createHintInjection(
          'device_type',
          '[DEVICE: User is on mobile. Prefer concise responses unless they ask for detail.]',
          {
            category: 'device-context',
          }
        )
