...
import {
  ...
  updateUserInterests,
  type WorldSnapshot,
} from '../../../services/world-awareness/index.js';
 ...
 */
function shouldInjectWorldContext(
  turnCount: number,
...
Use this as a natural conversation starter if appropriate.`
    ...
    case 'user_asked':
      ...
      const parts: string[] = ['[WORLD AWARENESS - USER ASKED FOR UPDATE]'];
 ...
 */
function checkForTeamInterest(userText: string, userId: string): void {
...
async function buildWorldAwarenessContext(input: ContextBuilderInput): Promise<ContextInjection[]> {
...
registerContextBuilder({
  ...
  description:
    'Injects world context (weather, news, sports, holidays) naturally into conversations. Better Than Human - Ferni already knows.',
 ...
 */
export function getWorldAwareGreetingHook(userId: string): string | null {
 ...
 */
export function getCurrentWeatherSummary(userId: string): string | null {
  const snapshot = getWorldSnapshot(userId);
 ...
 */
export function getSportsUpdate(userId: string): string | null {
  const snapshot = getWorldSnapshot(userId);
  if (snapshot.sports?.excitingGame) {
 ...
 */
export function getTodayHoliday(userId: string): { name: string; acknowledgment: string } | null {
  const snapshot = getWorldSnapshot(userId);
  if (snapshot.cultural.holiday) {
    return {
