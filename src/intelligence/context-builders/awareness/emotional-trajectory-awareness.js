...
// TRAJECTORY LOADER
// ============================================================================
async function loadEmotionalTrajectory(userId) {
    try {
        ...
        let trajectory = null;
        try {
            const { getEmotionalTrajectory } = await import('../../../services/superhuman/semantic-intelligence/emotional-semantics.js');
...
// CONTEXT FORMATTER
// ============================================================================
function formatTrajectoryContext(trajectory) {
    const parts = [];
    // Current state
    parts.push(`Current: ${trajectory.currentMood}`);
    // Trend (most important for "Better Than Human")
    if (trajectory.trend !== 'stable') {
        const trendEmoji = trajectory.trend === 'improving' ? '📈' : trajectory.trend === 'declining' ? '📉' : '〰️';
...
// CONTEXT BUILDER
// ============================================================================
async function buildEmotionalTrajectoryAwareness(input) {
    ...
    if (trajectory.trend === 'declining' || trajectory.distressLevel > 0.7) {
        const urgency = trajectory.distressLevel > 0.7
        ...
        injections.push(createHighInjection('emotional_trajectory_alert', urgency, {
...
${trajectory.trend === 'improving' ? "They're on an upswing - celebrate small wins!" : 'You notice patterns they might not see.'}`;
        injections.push(createHintInjection('emotional_trajectory_hint', hint, {
...
// REGISTRATION
// ============================================================================
export const emotionalTrajectoryAwarenessBuilder = {
...
//# sourceMappingURL=emotional-trajectory-awareness.js.map