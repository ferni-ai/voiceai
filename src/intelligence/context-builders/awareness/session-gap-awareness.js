...
async function loadSessionGapData(userId) {
    ...
    try {
        ...
        const lastSessionDate = lastSessionTimestamp instanceof Date
...
// GUIDANCE GENERATORS
// ============================================================================
function getWarmAcknowledgmentGuidance(days) {
...
function getCheckInGuidance(days, lastMood) {
...
function getReconnectionGuidance(days, lastMood) {
    const moodContext = lastMood
...
// CONTEXT BUILDER
// ============================================================================
async function buildSessionGapAwareness(input) {
    ...
    try {
        ...
        if (daysSinceLastSession >= config.reconnectionDays) {
            ...
            injections.push(createHighInjection('session_gap_reconnection', guidance, {
                category: 'session_awareness',
            }));
            log.debug({ userId, days: daysSinceLastSession, tier: 'reconnection' }, '🔄 Session gap awareness: reconnection');
        }
        // 6-14 days - check-in guidance
            ...
            injections.push(createHighInjection('session_gap_checkin', guidance, {
                category: 'session_awareness',
            }));
            log.debug({ userId, days: daysSinceLastSession, tier: 'check-in' }, '🔄 Session gap awareness: check-in');
        }
        // 3-5 days - warm acknowledgment
            ...
            injections.push(createHintInjection('session_gap_warm', guidance, {
                category: 'session_awareness',
            }));
            log.debug({ userId, days: daysSinceLastSession, tier: 'warm' }, '🔄 Session gap awareness: warm acknowledgment');
        }
        return injections;
    }
    catch (error) {
...
// REGISTRATION
// ============================================================================
export const sessionGapAwarenessBuilder = {
...
//# sourceMappingURL=session-gap-awareness.js.map