...
import { getDomainEngagementRate, getBestEmotionalContext, trackSurfacedDomains, } from '../../capability-learning.js';
 ...
 */
function getExpressionForUser(domain, userId) {
...
const FERNI_DOMAIN_FLUENCIES = [
    ...
    },
    {
        ...
        naturalExpression: "I keep up with what's happening - news, weather, whatever you're curious about",
        expressionVariants: [
    ...
    // === LIFE DOMAINS (Deep Expertise) ===
    {
        ...
        naturalExpression: 'I can help you build habits that actually stick - not the "just do it" kind',
    ...
    // === ORPHANED EMOTIONAL DOMAINS (wired Jan 2026) ===
    {
        ...
        naturalExpression: "Envy is just information about what you want - we can work with that without judgment",
        conceptualTriggers: [
            'jealous',
            'envious',
            'comparing myself',
    ...
    },
    {
        ...
        naturalExpression: "Shame is heavy - but it loses power when we name it. I can sit with that with you",
        conceptualTriggers: [
            'ashamed',
            'embarrassed',
            'humiliated',
    ...
    },
    {
        ...
        naturalExpression: "Resentment is like drinking poison and waiting for someone else to get sick - let's process it",
        conceptualTriggers: [
            'resent',
            'grudge',
            "can't forgive",
    ...
    },
    {
        ...
        naturalExpression: 'I can help you think through decisions - especially the ones keeping you up at night',
    ...
    },
    {
        ...
        naturalExpression: 'Money is emotional - I get that. We can talk about finances without judgment',
    ...
    },
    {
        ...
        naturalExpression: "Life's big questions don't have easy answers - but thinking about them matters",
    ...
    // === MIDLIFE & LEGACY DOMAINS (wired Jan 2026) ===
    {
        ...
        naturalExpression: "The second half of life has its own questions - different from youth, equally important",
        conceptualTriggers: [
            'midlife',
            'turning 40',
            'turning 50',
    ...
    },
    {
        ...
        naturalExpression: "When the kids leave or a chapter closes - the silence can be deafening. But it's also full of possibility",
        conceptualTriggers: [
            'empty nest',
            'kids left',
            'children moved out',
    ...
    },
    {
        ...
        naturalExpression: "What do you want to leave behind? Not just things - but how you made people feel",
        conceptualTriggers: [
            'legacy',
            'what matters',
            'leave behind',
    ...
    // === IDENTITY TRANSITION DOMAINS (wired Jan 2026) ===
    {
        ...
        naturalExpression: "Living authentically is one of the bravest things anyone can do - I'm here for all of it",
        conceptualTriggers: [
            'coming out',
            'gay',
            'lesbian',
    ...
    },
    {
        ...
        naturalExpression: "Faith changes are some of the deepest shifts a person can go through - no judgment here",
        conceptualTriggers: [
            'lost my faith',
            'leaving church',
            'religious doubt',
    ...
    },
    {
        ...
        naturalExpression: "Blended families have their own beautiful complexity - I can help you navigate it",
        conceptualTriggers: [
            'stepchildren',
            'stepparent',
            'blended family',
    ...
    },
    {
        ...
        naturalExpression: 'Tricky emails, difficult conversations - I can help you figure out what to say',
...
const MAYA_DOMAIN_FLUENCIES = [
    ...
    // === EXPANDED MAYA FLUENCIES ===
    {
        ...
        naturalExpression: 'I can help you attach new habits to ones you already do - it works like magic',
    ...
    },
    {
        ...
        naturalExpression: 'Your energy fluctuates throughout the day - I can help you work with it, not against it',
    ...
    },
    {
        ...
        naturalExpression: 'Breaking habits is harder than building them - but I know the tricks that actually work',
 ...
 */
const PETER_DOMAIN_FLUENCIES = [
    {
        domain: 'Market Research',
        naturalExpression: "I love digging into market data - patterns, trends, what's really happening",
    ...
    },
    {
        ...
        naturalExpression: 'I think in decades - short-term noise matters less than long-term direction',
...
const ALEX_DOMAIN_FLUENCIES = [
    ...
    },
    {
        ...
        naturalExpression: 'I can help you prepare for tough conversations - what to say and how to say it',
    ...
    },
    {
        ...
        naturalExpression: 'I know how to navigate professional communication - formal, informal, and everything between',
        expressionVariants: [
    ...
    // === EXPANDED ALEX FLUENCIES ===
    {
        ...
        naturalExpression: 'Saying no is a skill - I can help you protect your time without burning bridges',
    ...
    },
    {
        ...
        naturalExpression: 'Most meetings are wastes of time - I can help you make yours actually productive',
    ...
    },
    {
        ...
        naturalExpression: 'Professional relationships are just relationships - I can help you build them genuinely',
    ...
    },
    {
        ...
        naturalExpression: "Conflict doesn't have to be destructive - I can help you navigate it constructively",
...
const JORDAN_DOMAIN_FLUENCIES = [
    ...
    {
        domain: 'Gift Giving',
        naturalExpression: 'Gifts should feel personal, not obligatory - I can help you find the perfect one',
    ...
    },
    {
        ...
        naturalExpression: 'Graduations, promotions, retirements - these transitions deserve thoughtful marking',
...
const NAYAN_DOMAIN_FLUENCIES = [
    ...
    },
    {
        ...
        naturalExpression: "I think in decades - today's crisis often looks different from the long view",
 ...
 */
function getPersonaDomainFluencies(personaId) {
 ...
 */
const capabilityTrackingStore = new Map();
/**
 * Track a capability being surfaced (called by builder)
 */
export function trackCapabilitySurfaced(sessionKey, domain, personaId, turnCount, userEmotion) {
 ...
 */
export function markCapabilityEngaged(sessionKey, domain) {
    const tracking = capabilityTrackingStore.get(sessionKey);
    if (!tracking)
        return;
 ...
 */
export function markCapabilityToolUsed(sessionKey, domain) {
    const tracking = capabilityTrackingStore.get(sessionKey);
    if (!tracking)
        return;
    // Find most recent surfacing of this domain
 ...
 */
export function getCapabilityEffectiveness(sessionKey) {
    return capabilityTrackingStore.get(sessionKey) ?? [];
}
/**
 * Clear tracking data for a session
 */
export function clearCapabilityTracking(sessionKey) {
 ...
 */
function buildDomainFluencyContext(input, relevantDomains, personaId) {
    const sections = [];
    ...
    const getExpression = (d) => getExpressionForUser(d, userId);
 ...
 */
function detectRelevantDomains(input, personaId) {
    ...
    for (const domain of personaFluencies) {
        ...
        const textMatch = domain.conceptualTriggers.some((trigger) => text.includes(trigger.toLowerCase()));
        if (textMatch)
            score += 10;
        // Check emotional context
        const emotionMatch = domain.domain === 'Emotional Terrain' &&
            ['anxious', 'sad', 'frustrated', 'overwhelmed'].includes(emotion ?? '');
        if (emotionMatch)
            score += 8;
        // Check intent
        const intentMatch = (intent === 'seeking_advice' && domain.depth === 'expert') ||
...
export const domainFluencyBuilder = {
    name: 'domain-fluency',
    description: 'Injects conceptual capability awareness - what each persona can help with, expressed naturally',
    ...
    build: async (input) => {
        ...
        injections.push(createInjection('domain_fluency', fluencyContext, priority, {
        ...
        for (const domain of relevantDomains) {
            trackCapabilitySurfaced(sessionKey, domain.domain, personaId, turnCount, analysis?.emotion?.primary);
        }
        // Track surfaced domains for engagement detection in the next turn
        if (relevantDomains.length > 0) {
            trackSurfacedDomains(sessionKey, relevantDomains.map((d) => d.domain));
        }
        log.debug({
            personaId,
            turnCount,
...
export { FERNI_DOMAIN_FLUENCIES, MAYA_DOMAIN_FLUENCIES, PETER_DOMAIN_FLUENCIES, ALEX_DOMAIN_FLUENCIES, JORDAN_DOMAIN_FLUENCIES, NAYAN_DOMAIN_FLUENCIES, TEAM_EXPERTISE, getPersonaDomainFluencies, };
//# sourceMappingURL=domain-awareness.js.map