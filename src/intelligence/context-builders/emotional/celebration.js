 ...
 */
import { getCelebration } from '../../../personas/theatrical.js';
 ...
 */
function getPersonaCelebrationExample(personaId, type) {
    if (!personaId)
        return '';
    try {
 ...
 * Good news patterns
 */
const GOOD_NEWS_PATTERNS = /\b(great news|good news|exciting news|guess what|you won't believe|i did it|it worked|finally|so happy|thrilled|over the moon)\b/i;
/**
 * Decision/commitment patterns
 */
const DECISION_PATTERNS = /\b(i('ve| have)? decided|made (up my mind|a decision)|going to (do|start|stop)|i('m| am) (committed|doing)|let's do it|i('m| am) in|i'll (do|take|start))\b/i;
/**
 * Response quality tracking patterns
 */
const POSITIVE_REACTION = /\b(thanks|thank you|that helps|makes sense|exactly|perfect|great|helpful|appreciate|love that|so true)\b/i;
const NEGATIVE_REACTION = /\b(not what i asked|confused|don't understand|that's not|you're wrong|no|actually|but i said|already told you)\b/i;
// ============================================================================
// CELEBRATION CONTEXT BUILDER
// ============================================================================
/**
 ...
 * Now includes PERSONA-SPECIFIC celebration responses!
 */
function buildCelebrationContext(input) {
    ...
    for (const pattern of MILESTONE_PATTERNS) {
        if (pattern.test(userText) && analysis.emotion.valence === 'positive') {
            ...
            injections.push(createStandardInjection('milestone', `[MILESTONE DETECTED - CELEBRATE!]
...
DO NOT:
  ...
  - Make it about the numbers instead of their effort`));
    ...
    // GOOD NEWS DETECTION
    // -----------------------------------------------
    if (GOOD_NEWS_PATTERNS.test(userText) &&
        ...
        injections.push(createStandardInjection('good_news', `[GOOD NEWS - CELEBRATE WITH THEM!]
...
Let them bask in it before moving on.`));
    ...
    // -----------------------------------------------
    const AHA_PATTERNS = /\b(oh!|ohhh|ahh|aha|that makes sense|now i get it|i see|finally understand|clicked|light bulb|never thought of it|wow|mind blown)\b/i;
    const REALIZATION_PATTERNS = /\b(so (that's|what) (why|you mean)|wait( so)?|hold on|really\?|seriously\?|i didn't (know|realize)|i never knew)\b/i;
    if ((AHA_PATTERNS.test(userText) || REALIZATION_PATTERNS.test(userText)) &&
        ...
        injections.push(createStandardInjection('aha_moment', `[AHA MOMENT - ANCHOR THIS INSIGHT!]
...
DO NOT:
  ...
  - Move on too quickly`));
    ...
    if (DECISION_PATTERNS.test(userText)) {
        ...
        injections.push(createStandardInjection('achievement', `[DECISION MADE - CELEBRATE THIS COMMITMENT!]
...
DO NOT:
  ...
  - List downsides right after they've decided`));
    ...
    if (POSITIVE_REACTION.test(userText)) {
        ...
        injections.push(createHintInjection('quality_positive', `[RESPONSE QUALITY: User reacted positively. Keep this style.]`));
    }
    else if (NEGATIVE_REACTION.test(userText)) {
        extServices.trackResponseQuality?.(userText, 'negative');
        // Important hint to adjust
        injections.push(createStandardInjection('quality_negative', `[RESPONSE QUALITY: User seems confused or corrected you. Acknowledge the correction gracefully and adjust your approach.]`));
    }
    return injections;
}
// ============================================================================
