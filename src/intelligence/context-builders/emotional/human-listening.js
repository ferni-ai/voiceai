 ...
 */
export function setHumanListeningResult(sessionId, result) {
    sessionResults.set(sessionId, result);
}
/**
 * Get the latest analysis result for a session.
 */
export function getHumanListeningResult(sessionId) {
    return sessionResults.get(sessionId) ?? null;
}
/**
 * Clear stored result for a session.
 */
export function clearHumanListeningResult(sessionId) {
    sessionResults.delete(sessionId);
}
// ============================================================================
// CONTEXT BUILDER
// ============================================================================
const humanListeningBuilder = {
        ...
        try {
            ...
            if (!result) {
                ...
                if (contextFromPipeline) {
                    injections.push(createStandardInjection('human-listening', contextFromPipeline, {
                        category: 'listening',
                        confidence: 0.7,
                    }));
                }
            ...
            if (result.possibleDistress) {
                injections.push(createCriticalInjection('human-listening', `[🎧 DISTRESS SIGNALS DETECTED]
${result.agentGuidance}
Priority: ${result.prioritySignals.join('; ')}`, { category: 'distress' }));
            }
            // Should slow down is important
            if (result.shouldSlowDown && !result.possibleDistress) {
                injections.push(createStandardInjection('human-listening', `[🎧 NEEDS SLOWER PACE]
${result.overallAssessment}
${result.agentGuidance}`, { category: 'pacing', confidence: result.confidence }));
            }
            // Emotional undercurrent if detected
            if (result.emotionalUndercurrent.evidence.length > 0 &&
                result.emotionalUndercurrent.possiblyMasked) {
                injections.push(createStandardInjection('human-listening', `[🎧 EMOTIONAL UNDERCURRENT]
...
Their words may be masking how they really feel.`, { category: 'emotional', confidence: result.emotionalUndercurrent.confidence }));
            }
            // Give space hint
            if (result.shouldGiveSpace && !result.possibleDistress && !result.shouldSlowDown) {
                injections.push(createHintInjection('human-listening', `[🎧 Consider giving more space in your response - user may be processing something sensitive]`, { category: 'pacing', confidence: 0.6 }));
            }
            // Voice-specific signals (if audio analysis was done)
            if (result.audio.tremor?.detected) {
                injections.push(createStandardInjection('human-listening', `[🎧 VOICE SIGNAL: ${result.audio.tremor.suggestedResponse}]`, { category: 'voice', confidence: result.audio.tremor.confidence }));
            }
            if (result.audio.breath?.needsSpace) {
                injections.push(createHintInjection('human-listening', `[🎧 Breath pattern: ${result.audio.breath.guidance}]`, { category: 'voice', confidence: result.audio.breath.confidence }));
            }
            // =================================================================
            ...
            try {
                ...
                if (userId) {
                    ...
                    const { detectDeviations } = await import('../../../services/memory/human-listening-memory.js');
                    ...
                    if (deviationReport.hasDeviation && deviationReport.confidence > 0.5) {
                        const deviationDescriptions = deviationReport.deviations
                            ...
                            .join('\n');
                        injections.push(createStandardInjection('human-listening', `[🎧 SPEECH PATTERN DEVIATION - "Something's different"]
...
Or weave in naturally: "I'm picking up on something..."`, { category: 'deviation', confidence: deviationReport.confidence }));
                        log.info({
                            userId,
                    ...
                    const voiceSignal = result.audio?.tremor?.detected
                        ? {
                            ...
                            characteristics: {
                                ...
                                energy: result.overallAssessment?.includes('low') ||
                                    result.overallAssessment?.includes('tired')
                                    ? 'low'
                                    : 'normal',
                                // Infer pace from shouldSlowDown flag
                                pace: result.shouldSlowDown ? 'rushed' : 'normal',
                            },
                        }
                        : null;
                    if (voiceSignal) {
                        const { detectVoiceDeviation } = await import('../../../services/trust-systems/voice-emotion-integration.js');
                        const voiceDeviation = detectVoiceDeviation(userId, voiceSignal);
                        if (voiceDeviation.deviates && voiceDeviation.significance > 0.4) {
                            injections.push(createStandardInjection('human-listening', `[🎧 VOICE DEVIATION - "${voiceDeviation.deviation}"]
...
Consider a gentle check-in: "You sound a bit different today. How are you really doing?"`, { category: 'voice-deviation', confidence: voiceDeviation.significance }));
                            log.info({
                                userId,
...
//# sourceMappingURL=human-listening.js.map