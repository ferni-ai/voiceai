...
import { getCelebrationEngine, } from '../../../services/celebration-engine.js';
import { getGrowthVisibilityEngine, } from '../../../services/growth-visibility-engine.js';
 ...
 */
async function buildCelebrationContext(input) {
    ...
    if (trigger) {
        ...
        injections.push(createStandardInjection('celebration', formatCelebrationForPrompt(trigger, response), {
            category: 'celebration',
            confidence: 0.9,
        }));
        log.debug({ type: trigger.type, intensity: trigger.intensity }, '🎉 Celebration opportunity detected');
        // Track telemetry
        const telemetry = getBetterThanHumanTelemetry();
        const celebrationType = trigger.type === 'goal_completed'
            ? 'goal_completed'
 ...
 */
function formatCelebrationForPrompt(trigger, response) {
    const emoji = {
        subtle: '✨',
        warm: '🌟',
        enthusiastic: '🎉',
 ...
 */
async function buildGrowthContext(input) {
    const { userProfile, userData, analysis, services, userText } = input;
    const injections = [];
    if (!userProfile || !services.userId) {
        return injections;
    ...
    const isGoodMoment = (turnCount >= 2 && turnCount <= 4) || (turnCount > 0 && turnCount % 10 === 0);
    if (!isGoodMoment) {
        return injections;
    }
    // Get growth engine
    ...
    growthEngine.recordTurn({
        ...
        hadInsight: analysis.intent?.primary === 'realization' || /i (just )?realized/i.test(userText || ''),
    });
    ...
    if (reflection) {
        ...
        injections.push(createHintInjection('growth_reflection', formatGrowthForPrompt(reflection), {
            category: 'growth',
            confidence: reflection.insight.confidence,
        }));
        log.debug({ type: reflection.insight.type, area: reflection.insight.area }, '🌱 Growth insight ready to surface');
        // Track telemetry
        const telemetry = getBetterThanHumanTelemetry();
        telemetry.trackGrowthInsightSurfaced(services.userId, services.personaId || 'ferni', reflection.insight.id);
    }
    return injections;
}
/**
 ...
 */
function formatGrowthForPrompt(reflection) {
 ...
 */
async function buildCelebrationAndGrowthContext(input) {
...
//# sourceMappingURL=celebration-growth.js.map