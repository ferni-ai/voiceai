 ...
 */
function detectTextEnergy(text) {
    const lower = text.toLowerCase();
    const wordCount = text.split(/\s+/).length;
    // Exhausted signals
    if (lower.includes('so tired') ||
 ...
 */
function detectEmotionEnergy(analysis) {
 ...
 */
function combineEnergySignals(signals) {
    const levels = [signals.textEnergy, signals.emotionEnergy];
    if (signals.voiceEnergy)
        levels.push(signals.voiceEnergy);
    ...
    if (counts.exhausted > 0)
        return 'exhausted';
    if (counts.excited > 0 && counts.excited >= counts.low)
        return 'excited';
    if (counts.low > counts.engaged)
        return 'low';
    if (counts.engaged > 0)
        return 'engaged';
    return 'neutral';
 ...
 */
async function buildEnergyMirroringContext(input) {
    ...
    injections.push(createHintInjection('energy_mirroring', lines.join('\n'), { category: 'humanizing' }));
    log.debug({
        energyLevel,
...
//# sourceMappingURL=energy-mirroring.js.map