...
import { registerContextBuilder, createHighInjection, createStandardInjection, BuilderCategory, } from '../index.js';
...
function detectEmotionState(input) {
    ...
    if (rawIntensity > 0.8)
        intensity = 'intense';
    else if (rawIntensity > 0.6)
        intensity = 'high';
    else if (rawIntensity < 0.3)
        intensity = 'low';
    // Check for vulnerability markers
    const lower = userText.toLowerCase();
    const isVulnerable = emotion.needsSupport === true ||
    ...
    const isCelebration = ['joy', 'excitement', 'proud', 'happy', 'thrilled'].includes(primary) &&
        rawIntensity > 0.5;
    // Check for distress
    const isDistress = ['fear', 'anxiety', 'panic', 'overwhelmed', 'despair'].includes(primary) ||
...
// TIMING CALCULATOR
// ============================================================================
function calculateContagionTiming(state) {
    ...
    else if (['sadness', 'grief', 'loss', 'hurt'].includes(emotion))
        phraseCategory = 'sadness';
    ...
    else if (['joy', 'happy', 'content', 'grateful'].includes(emotion))
        phraseCategory = 'joy';
    else if (intensity === 'intense' || intensity === 'high')
        phraseCategory = 'heavy';
    ...
    if (intensity === 'intense')
        ...
        mirrorIntensity = 0.85;
    else if (intensity === 'moderate')
        mirrorIntensity = 0.8;
    else if (intensity === 'low')
        mirrorIntensity = 0.9; // Can match more closely on subtle
    ...
    if (!REFLECTION_GUIDANCE[guidanceKey])
        guidanceKey = 'default';
...
export function clearContagionSession(sessionId) {
    sessionStates.delete(sessionId);
}
// ============================================================================
// CONTEXT BUILDER
// ============================================================================
export const emotionalContagionTimingBuilder = {
    ...
    build: async (input) => {
        ...
        if (emotionalState.isDistress || emotionalState.isVulnerable) {
            injections.push(createHighInjection('emotional_contagion', lines.join('\n'), {
                category: 'emotional',
            }));
        }
        else {
            injections.push(createStandardInjection('emotional_contagion', lines.join('\n'), {
                category: 'emotional',
            }));
        }
...
export { detectEmotionState, calculateContagionTiming, };
//# sourceMappingURL=emotional-contagion-timing.js.map