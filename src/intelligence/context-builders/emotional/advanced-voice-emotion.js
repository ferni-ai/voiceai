...
import { getEmotionTimeline, getLastEmotion, generateEmotionInsight, detectSuppression, distinguishNegativeEmotion, } from '../../../services/emotion-analysis/hume.js';
import { registerContextBuilder, } from '../index.js';
const log = createLogger({ module: 'context:advanced-voice-emotion' });
const sessionEmotionalArcs = new Map();
/**
 * Advanced Voice Emotion Context Builder
 *
 * Priority: 25 (early - informs how to approach the conversation)
 */
export const advancedVoiceEmotionBuilder = {
        ...
        if (latestEmotion.valence > 0.3 && latestEmotion.primary === 'joy' && points.length > 1) {
            ...
            if (previousPoint.valence <= 0) {
                injections.push({
                    ...
                    source: 'advanced-voice-emotion',
                    content: "VOICE BRIGHTENED: User's voice just lifted - they mentioned something that makes them happy. Follow up: 'Your voice just brightened when you mentioned that - tell me more.'",
                    priority: 'hint',
                    category: 'voice-emotion',
                });
            }
 ...
 */
function updateEmotionalArc(sessionId, timeline, latestEmotion, turnNumber) {
...
function isNegativeEmotion(emotion) {
    return ['sadness', 'anxiety', 'anger', 'fear', 'disgust', 'tiredness', 'distress'].includes(emotion);
}
 ...
 */
export function clearEmotionalArc(sessionId) {
...
//# sourceMappingURL=advanced-voice-emotion.js.map