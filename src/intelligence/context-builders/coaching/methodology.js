...
const worldClassCoachingCache = new Map();
const cacheLoadTimes = new Map();
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes
/**
 * Get methodology with caching
 */
async function getCachedMethodology(personaId) {
    const now = Date.now();
    ...
    try {
        ...
        const methodology = behaviors.methodology ?? null;
 ...
 */
async function getCachedWorldClassCoaching(personaId) {
    ...
    try {
        ...
        const behaviors = await bundle.getBehaviors();
        const worldClassCoaching = behaviors.world_class_coaching ?? null;
 ...
 */
function detectMethodologyTriggers(userText, topics) {
    const triggers = [];
    const lowerText = userText.toLowerCase();
    const lowerTopics = topics.map((t) => t.toLowerCase());
    for (const [domain, keywords] of Object.entries(TOPIC_TRIGGERS)) {
        const matched = keywords.some((keyword) => lowerText.includes(keyword) || lowerTopics.some((t) => t.includes(keyword)));
        if (matched) {
 ...
 */
function formatMethodologyInjection(methodology, triggers) {
    const parts = [];
 ...
 */
function formatInterventionTechnique(techniqueName, technique) {
    const parts = [];
 ...
 */
function matchEliteCoachFramework(userText) {
    const lowerText = userText.toLowerCase();
 ...
 */
function formatWorldClassCoachingInjection(coaching, matchedCoach, situationalMatch) {
    const parts = [];
    ...
    if (matchedCoach && coaching.elite_coaches_framework) {
        const coachData = coaching.elite_coaches_framework[matchedCoach];
        if (coachData) {
 ...
 */
function matchSituation(userText) {
    const lowerText = userText.toLowerCase();
    const situationTriggers = {
...
registerContextBuilder({
    name: 'methodology',
    description: 'Evidence-based coaching frameworks from methodology.json and world-class-coaching.json',
    ...
    async build(input) {
        ...
        if (methodologyTriggers.length > 0) {
            ...
            if (methodology) {
                log.debug({ personaId: persona.id, triggers: methodologyTriggers, topics: detectedTopics }, 'Methodology triggers detected');
                // Inject main methodology context
                const methodologyContext = formatMethodologyInjection(methodology, methodologyTriggers);
                injections.push(createStandardInjection('methodology', methodologyContext, {
                    category: 'methodology',
                    confidence: 0.8,
                }));
                // Check for specific intervention opportunities
                if (methodology.intervention_techniques) {
                    const techniques = methodology.intervention_techniques;
                    for (const [techniqueName, technique] of Object.entries(techniques)) {
                        const techniqueKeywords = techniqueName.toLowerCase().split('_');
                        const isRelevant = methodologyTriggers.some((trigger) => techniqueKeywords.some((keyword) => trigger.includes(keyword) || keyword.includes(trigger)));
                        if (isRelevant) {
                            const techniqueInjection = formatInterventionTechnique(techniqueName, technique);
                            injections.push(createHintInjection('intervention', techniqueInjection, {
        ...
        if (matchedCoach || matchedSituation) {
            ...
            if (worldClassCoaching) {
                log.debug({ personaId: persona.id, matchedCoach, matchedSituation }, 'World-class coaching triggers detected');
                const coachingContext = formatWorldClassCoachingInjection(worldClassCoaching, matchedCoach, matchedSituation);
                if (coachingContext) {
                    injections.push(createHintInjection('world_class_coaching', coachingContext, {
