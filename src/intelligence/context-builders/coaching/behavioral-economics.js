...
import { buildBehavioralEconomicsContext, getImplementationIntentions, getActiveCommitments, } from '../../../services/behavioral-economics/index.js';
 ...
 */
function detectGoalDiscussion(text) {
 ...
 */
async function buildBehavioralEconContext(input) {
    ...
    const beContext = buildBehavioralEconomicsContext(userId, {
        ...
        hasIntention: intentions.some((i) => goalDetection.goal && i.goal.toLowerCase().includes(goalDetection.goal.toLowerCase())),
        hasCommitment: commitments.some((c) => goalDetection.goal && c.commitment.toLowerCase().includes(goalDetection.goal.toLowerCase())),
    });
    if (beContext) {
        injections.push(createHintInjection('behavioral_economics', beContext, { category: 'coaching' }));
        log.debug({
            userId,
            goal: goalDetection.goal,
...
//# sourceMappingURL=behavioral-economics.js.map