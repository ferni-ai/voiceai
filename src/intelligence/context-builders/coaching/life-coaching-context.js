...
import { registerContextBuilder, createHintInjection, createHighInjection, } from '../index.js';
...
function detectLifeCoachingDomains(userText, emotion, topics) {
...
// PHRASE SELECTION
// ============================================================================
async function getSecondChancesPhrases(personaId, context) {
...
async function getConnectionPhrases(_personaId, context) {
    // IMPORTANT: Don't return literal phrases - the LLM copies them verbatim
...
async function getDifficultConversationsPhrases(_personaId, context) {
    // IMPORTANT: Don't return literal phrases - the LLM copies them verbatim
    // Instead, describe the coaching approach to take
    const phrases = [];
...
async function getLifeTransitionsPhrases(_personaId, context) {
    // IMPORTANT: Don't return literal phrases - the LLM copies them verbatim
...
async function getQuietGrowthPhrases(personaId, context) {
    // IMPORTANT: Don't return literal phrases - the LLM copies them verbatim
...
// CONTEXT BUILDER
// ============================================================================
async function buildLifeCoachingContext(input) {
    ...
    for (const detected of detectedDomains.slice(0, 2)) {
        ...
        switch (detected.domain) {
            case 'secondChances': {
                ...
                phrases = await getSecondChancesPhrases(personaId, {
                    isHopeless: textLower.includes('hopeless') ||
                        textLower.includes("can't") ||
                        textLower.includes('give up'),
                    isGrieving: textLower.includes('lost') || textLower.includes('grief') || textLower.includes('miss'),
                    hasWin: textLower.includes('did it') ||
            ...
            case 'connection': {
                ...
                phrases = await getConnectionPhrases(personaId, {
                    isLonely: detected.triggers.some((t) => ['lonely', 'alone', 'isolated', 'no friends'].includes(t)),
            ...
            case 'difficultConversations': {
                ...
                phrases = await getDifficultConversationsPhrases(personaId, {
                    isPreparing: textLower.includes('need to talk') || textLower.includes('have a conversation'),
                    needsPractice: textLower.includes('practice') ||
                        textLower.includes('role play') ||
                        textLower.includes('how to say'),
                    needsBoundary: textLower.includes('boundary') ||
                    ...
                    stage: textLower.includes('ending') || textLower.includes('leaving')
                        ...
                        : textLower.includes('between') || textLower.includes('in-between')
                            ...
                            : textLower.includes('beginning') || textLower.includes('starting')
                                ? 'beginning'
                                : undefined,
                    hasDualEmotions: (textLower.includes('happy') && textLower.includes('sad')) ||
            ...
            case 'quietGrowth': {
                ...
                phrases = await getQuietGrowthPhrases(personaId, {
                    isExhausted: textLower.includes('tired') ||
                        textLower.includes('exhausted') ||
                        textLower.includes('burned out'),
                    isPlateau: textLower.includes('plateau') ||
                        textLower.includes('stuck') ||
                        textLower.includes('not progressing'),
                    isComparing: textLower.includes('everyone else') ||
        ...
        if (phrases.length > 0) {
            ...
            injections.push(isHighPriority
...
registerContextBuilder({
    ...
    description: 'Better-than-human life coaching: second chances, connection, difficult conversations, transitions, quiet growth',
    build: buildLifeCoachingContext,
});
export { buildLifeCoachingContext, detectLifeCoachingDomains };
//# sourceMappingURL=life-coaching-context.js.map