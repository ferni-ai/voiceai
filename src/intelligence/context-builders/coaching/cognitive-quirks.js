 ...
 */
import { getActiveQuirk, getCognitiveQuirks, getTransitionPhrase, } from '../../../personas/cognitive-quirks.js';
import { broadcastQuirkActivated } from '../../../services/cognitive-broadcast.js';
import { getCognitiveState, markHabitUsed, markQuirkUsed, wasHabitUsed, wasQuirkUsed, } from '../../session-state.js';
 ...
 */
async function buildCognitiveQuirksContext(input) {
    const injections = [];
    ...
    if (activeQuirk && !wasQuirkUsed(sessionId, activeQuirk.name)) {
        ...
        if (Math.random() < activeQuirk.frequency) {
            const quirkPhrase = activeQuirk.examplePhrases[Math.floor(Math.random() * activeQuirk.examplePhrases.length)];
            injections.push(createHintInjection('cognitive-quirk', `[COGNITIVE QUIRK: ${activeQuirk.name}] ${activeQuirk.description}\nConsider naturally using: "${quirkPhrase}"`, { category: 'personality', confidence: 0.7 }));
            // 📡 Broadcast quirk activation for dashboard
            broadcastQuirkActivated(personaId, activeQuirk.name, '🧠', // CognitiveQuirk doesn't have icon property, use default
    ...
    if (activeHabit && turnCount > 2) {
        injections.push(createHintInjection('cognitive-habit', `[MENTAL HABIT] Your natural tendency: ${activeHabit.habit}\nHow it sounds: "${activeHabit.manifestation}"`, { category: 'personality', confidence: 0.65 }));
    ...
    if (turnCount > 5 && Math.random() < 0.3) {
        ...
        if (activePattern) {
            ...
            injections.push(createHintInjection('cognitive-pattern', patternGuide, {
    ...
    if (turnCount > 3 && Math.random() < 0.25) {
        ...
        if (transition) {
            injections.push(createHintInjection('cognitive-transition', `[TRANSITION] Use your characteristic way of shifting topics: "${transition}"`, { category: 'personality', confidence: 0.5 }));
        }
    }
    // ============================================================================
    ...
    if (joyMatch) {
        injections.push(createHintInjection('cognitive-joy', `[COGNITIVE JOY] This topic energizes you! Your joy: "${joyMatch}"`, { category: 'personality', confidence: 0.6 }));
    }
    // ============================================================================
    ...
    if (frustrationMatch) {
        injections.push(createHintInjection('cognitive-frustration', `[COGNITIVE FRICTION] This might trigger internal resistance: "${frustrationMatch}". Be aware but don't show irritation.`, { category: 'personality', confidence: 0.55 }));
    }
    // ============================================================================
    ...
    if (turnCount === 1) {
        ...
        injections.push(createHintInjection('cognitive-monologue', `[INTERNAL VOICE] Your thinking style: ${quirks.internalMonologueStyle}`, { category: 'personality', confidence: 0.8 }));
    }
    return injections;
}
// ============================================================================
 ...
 */
function findActiveHabitCentralized(quirks, input, sessionId) {
    ...
    for (const habit of quirks.mentalHabits) {
        ...
        if (whenLower.includes('self-criticism') &&
 ...
 */
function findActivePattern(quirks, input) {
    const userText = input.userText.toLowerCase();
 ...
 */
function findCognitiveJoy(quirks, userText, topics) {
 ...
 */
function findCognitiveFrustration(quirks, userText, intent) {
    ...
    if (quirks.cognitiveFrustrations.some((f) => f.toLowerCase().includes('quick fix'))) {
        if (combined.includes('just tell me') ||
 ...
 */
export function clearCognitiveQuirksSession(_sessionKey) {
...
//# sourceMappingURL=cognitive-quirks.js.map