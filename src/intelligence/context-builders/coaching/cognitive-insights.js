 ...
 */
async function buildCognitiveInsightsContext(input) {
    const injections = [];
    const personaId = input.persona?.id;
    const sessionId = input.services.sessionId || `${personaId}_${input.services.userId || 'anonymous'}`;
    ...
    for (const insight of insights) {
        ...
        if (wasInsightShared(sessionId, insightKey))
            continue;
        // Check cooldown (centralized state)
        if (isInsightOnCooldown(sessionId, insightKey, turnCount, INSIGHT_COOLDOWN_TURNS))
            continue;
        ...
        if (insight.confidence < 0.6)
            continue;
        // Share the insight
        injections.push(createHintInjection('cognitive-insight-share', `[SHAREABLE INSIGHT] You can transparently share: "${insight.shareablePhrase}"\n(Internal: ${insight.message})`, { category: 'cognitive-insight', confidence: insight.confidence }));
        // 📡 Broadcast insight generation for dashboard
        broadcastInsightGenerated(personaId, insight.type, insight.shareablePhrase, true // This is a shared insight
        );
 ...
 */
function generateInsights(input, profile, userStyle, personaId) {
    const insights = [];
    if (!profile)
        return insights;
    // ============================================================================
...
// ============================================================================
function getStyleMatchPhrase(personaId, userStyle) {
    const phrases = {
        ferni: {
...
function getStyleAdaptPhrase(personaId, userStyle, personaStyle) {
    const adapts = {
...
function getVoiceResponsePhrase(personaId, emotion) {
    const responses = {
...
function getEmpathyShiftPhrase(personaId) {
    const shifts = {
...
function getLearningPhrase(personaId) {
    const learnings = {
 ...
 */
export function clearCognitiveInsightsSession(_sessionKey) {
...
//# sourceMappingURL=cognitive-insights.js.map