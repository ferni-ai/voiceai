 ...
 */
import { detectQuestionComplexity, detectUserExpertise, getCognitiveEngine, getCognitiveProfile, } from '../../../personas/cognitive-index.js';
import { createHintInjection, createStandardInjection, registerContextBuilder, } from '../index.js';
// Advanced cognitive features
import { buildCognitiveGrowthContext, buildReasoningChain, detectCognitiveConflict, detectUserCognitiveStyle, getCognitiveGrowthProfile, getCognitiveLearningTracker, getKnowledgeStateTracker, getReasoningChainGuidance, } from '../../../personas/cognitive-advanced.js';
...
import { broadcastApproachUsed, broadcastCognitiveMode, broadcastConfidence, broadcastUserStyle, } from '../../../services/cognitive-broadcast.js';
// Cognitive metrics for performance tracking
import { cognitiveMetrics, maybeBroadcastMetrics, recordTurnMetrics, } from '../../../utils/cognitive-metrics.js';
// Centralized distress levels
import { DISTRESS } from '../../distress-levels.js';
// Centralized session state
import { addReasoningApproach, addUserMessageForStyleDetection, getActiveReasoningChain, getCognitiveState, setActiveReasoningChain, updateUserCognitiveStyle, } from '../../session-state.js';
/**
 * Build cognitive intelligence context
 *
 * Uses centralized SessionStateManager for session tracking.
 */
async function buildCognitiveContext(input) {
    ...
    const previousApproaches = cognitiveState.reasoningHistory;
    ...
    const learnedRecommendation = learningTracker.getRecommendedApproach(userId, personaId, profile.reasoningStyle);
    ...
    if (learnedRecommendation.confidence > 0.7 && learnedRecommendation.approach !== finalApproach) {
        ...
        injections.push(createHintInjection('cognitive-learning', `[LEARNED] ${learnedRecommendation.reason}`, {
            category: 'cognitive',
    ...
    broadcastCognitiveMode(personaId, finalApproach, learnedRecommendation.confidence > 0.7 ? 'learned preference' : 'context-driven');
    // 📡 Broadcast approach used for history tracking
    broadcastApproachUsed(personaId, finalApproach, cognitiveContext.currentTopic, learnedRecommendation.confidence > 0.5 ? learnedRecommendation.confidence : 0.5);
    // ============================================================================
    ...
    injections.push(createStandardInjection('cognitive-reasoning', reasoningPrompt, {
        category: 'cognitive',
        confidence: 0.9,
    }));
    // ============================================================================
    ...
    if (userCognitiveStyle.primary !== 'unknown' && userCognitiveStyle.confidence > 0.5) {
        ...
        injections.push(createHintInjection('cognitive-user-style', styleGuidance, {
            category: 'cognitive',
        ...
        const signalsAsRecord = userCognitiveStyle.signals
            ? {
            ...
            : {};
        broadcastUserStyle(userId, userCognitiveStyle.primary, userCognitiveStyle.confidence, signalsAsRecord);
    }
    // ============================================================================
    ...
    if (conflict?.detected && conflict.severity !== 'mild') {
        injections.push(createStandardInjection('cognitive-conflict', `[COGNITIVE TENSION] Your ${conflict.personaStyle} style may not match their need for ${conflict.userNeed}. ${conflict.phrase || 'Consider adapting.'}`, { category: 'cognitive', confidence: 0.85 }));
    }
    // ============================================================================
    // 4. MULTI-STEP REASONING CHAINS (for complex situations)
    // Uses centralized session state
    // ============================================================================
    let activeChain = getActiveReasoningChain(sessionId);
    ...
    if (activeChain) {
        ...
        if (chainGuidance) {
            injections.push(createStandardInjection('cognitive-chain', chainGuidance, {
                category: 'cognitive',
                confidence: 0.9,
            }));
        }
    ...
    if (topicGuidance.depth !== 'moderate') {
        injections.push(createHintInjection('cognitive-knowledge', `[KNOWLEDGE] ${topicGuidance.note}`, {
            category: 'cognitive',
            confidence: 0.8,
        }));
    }
    ...
    const growthProfile = getCognitiveGrowthProfile(relationshipStage, sessionCount);
    ...
    if (growthContext) {
        injections.push(createHintInjection('cognitive-growth', growthContext, {
            category: 'cognitive',
            confidence: 0.75,
        }));
    }
    ...
    if (guidance.attentionCues.length > 0) {
        ...
        injections.push(createHintInjection('cognitive-attention', attentionContent, {
            category: 'cognitive',
            confidence: 0.8,
        }));
    }
    ...
    if (guidance.biasAlerts.length > 0) {
        ...
        injections.push(createHintInjection('cognitive-bias', biasContent, { category: 'cognitive', confidence: 0.7 }));
    }
    // ============================================================================
    // 9. CONFIDENCE SIGNALING - Express appropriate uncertainty
    // ============================================================================
    // 📡 Broadcast confidence level for dashboard
    broadcastConfidence(personaId, guidance.confidenceLevel, questionComplexity === 'complex' ? 'complex question' : cognitiveContext.currentTopic);
    if (guidance.confidenceLevel < 0.6) {
        ...
        injections.push(createHintInjection('cognitive-confidence', confidenceContent, {
            category: 'cognitive',
            confidence: 0.85,
        }));
    }
    ...
    if (turnCount > 2 && Math.random() < 0.4) {
        ...
        if (question) {
            injections.push(createHintInjection('cognitive-question', `[COGNITIVE QUESTION] Consider asking a ${question.style} question: "${question.text}" (${question.purpose})`, { category: 'cognitive', confidence: 0.7 }));
        }
    }
    // ============================================================================
    ...
    if (turnCount > 5 && Math.random() < 0.15) {
        ...
        const tier = input.userProfile?.subscription?.tier || 'free';
        const unlockedMemberIds = getUnlockedTeamMemberIds(input.userProfile, tier);
        const teamCommentary = generateTeamCommentary(personaId, cognitiveContext.currentTopic, 'reflection', unlockedMemberIds // Only generate commentary about unlocked members
        );
        if (teamCommentary.length > 0) {
            injections.push(createHintInjection('cognitive-team', `[TEAM PERSPECTIVE] ${teamCommentary[0]}`, {
                category: 'cognitive',
                confidence: 0.6,
            }));
        }
    ...
    if (profile.signatureThinkingPhrases.length > 0 && Math.random() < 0.25) {
        const phrase = profile.signatureThinkingPhrases[Math.floor(Math.random() * profile.signatureThinkingPhrases.length)];
        injections.push(createHintInjection('cognitive-signature', `[SIGNATURE] Consider using your characteristic phrase: "${phrase}"`, { category: 'cognitive', confidence: 0.6 }));
    }
 ...
 */
function buildUserStyleGuidance(userStyle, personaStyle) {
 ...
 */
function determineRequestType(input) {
 ...
 */
function determineUserNeed(input, userStyle) {
    const requestType = determineRequestType(input);
    if (requestType === 'venting')
        return 'support';
    if (requestType === 'seeking_advice')
        return 'decision';
    if (userStyle === 'analytical')
        return 'information';
    if (userStyle === 'narrative' || userStyle === 'intuitive')
        return 'exploration';
    if (input.userText.includes('?'))
        return 'information';
    return 'exploration';
 ...
 */
function detectUserExpertiseFromContext(input) {
 ...
 */
function calculateEmotionalWeight(input) {
    const { emotion } = input.analysis;
 ...
 */
function buildReasoningPrompt(approach, primaryStyle) {
 ...
 */
function buildConfidencePrompt(confidence, suggestedPhrases) {
