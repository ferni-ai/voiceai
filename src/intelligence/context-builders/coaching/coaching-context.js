    ...
    async build(input) {
        ...
        try {
            ...
            if (safetyCheck.crisisDetected) {
                ...
                injections.push(createHighInjection('safety', `[🚨 SAFETY ALERT - ${severity.toUpperCase()}]
${safetyCheck.contextInjection || 'User may be in distress. Please respond with extra care and warmth.'}

CRITICAL: Respond with warmth and care. Never abandon. Validate first, then gently offer resources.`, { category: 'crisis' }));
            ...
            const currentPersona = persona.id;
            const coachingContext = coaching.getCoachingContextForLLM(userId, {
                currentPersona,
                userMessage: userText,
            });
            if (coachingContext) {
                injections.push(createStandardInjection('coaching', coachingContext, {
                    category: 'coaching',
                    confidence: 0.9,
                }));
            }
            ...
            if (coachingAnalysis.hasGoalStatement) {
                injections.push(createHighInjection('goal_detection', `[🎯 GOAL DETECTED]
...
Don't force it - let it emerge naturally.`, { category: 'goal', confidence: 0.8 }));
            }
            // Obstacle detection
            if (coachingAnalysis.hasObstacle && coachingAnalysis.obstacleType) {
                const obstacleSupport = coaching.getObstacleSupport(coachingAnalysis.obstacleType);
                injections.push(createHighInjection('obstacle_support', `[🚧 OBSTACLE: ${coachingAnalysis.obstacleType.replace('_', ' ').toUpperCase()}]
...
Acknowledge the obstacle with compassion, then gently explore.`, { category: 'obstacle', confidence: 0.7 }));
            }
            // Vague emotion expansion
            if (coachingAnalysis.hasVagueEmotion && coachingAnalysis.emotionExpansion) {
                injections.push(createHintInjection('emotional_granularity', `[💭 EMOTIONAL VOCABULARY OPPORTUNITY]
User used vague emotional language.
Consider gently asking: "${coachingAnalysis.emotionExpansion}"
This helps them develop emotional granularity.`, { category: 'emotional_vocabulary', confidence: 0.6 }));
            }
            // Handoff opportunity
            if (coachingAnalysis.suggestedHandoff && coachingAnalysis.handoffTarget) {
                ...
                injections.push(createHintInjection('team_handoff', `[🤝 TEAM OPPORTUNITY]
This topic might be perfect for ${coachingAnalysis.handoffTarget}.
Natural intro: "${intro.intro}"

Only suggest if it feels right. Don't force handoffs.`, { category: 'handoff', confidence: 0.5 }));
            }
            // Values suggestion
            if (coachingAnalysis.suggestedValues && coachingAnalysis.suggestedValues.length > 0) {
                ...
                injections.push(createHintInjection('values_opportunity', `[💎 VALUES HINT]
User's message touches on the value of "${topValue.value}" (${topValue.domain}).
If relevant, connect to values: "It sounds like ${topValue.value.toLowerCase()} matters to you."`, { category: 'values', confidence: topValue.confidence }));
            }
            // ====================================================================
            ...
            if (milestone) {
                injections.push(createHighInjection('journey_milestone', `[🎉 JOURNEY MILESTONE: ${milestone.title}]
${milestone.description}

Consider acknowledging this milestone warmly!`, { category: 'milestone', confidence: 1.0 }));
            }
        }
        catch (error) {
...
// ============================================================================
export default {};
//# sourceMappingURL=coaching-context.js.map