...
let counter = 0;
function createInjection(source, content, priority) {
    return {
 ...
 */
export async function buildScientificCoachingContext(input) {
    const { userId, userMessage, personaId, topic, emotionalState, emotionalIntensity = 0.5, conversationPhase = 'exploring', turnNumber = 1, enableDistortionDetection = true, enableWellbeingTracking = true, enableNudges = true, enableWisdom = true, } = input;
    const result = {
        injections: [],
        detectedDistortions: [],
        wellbeingSignals: {},
    ...
    try {
        ...
        if (enableDistortionDetection && userMessage.length > 20) {
            try {
                const { detectDistortions, getDistortionContextInjection, recordANT } = await import('../../../services/cognitive-intelligence/index.js');
                ...
                if (distortions.length > 0) {
                    ...
                    if (injection) {
                        const priority = distortions[0].confidence > 0.7 ? 'high' : 'standard';
                        result.injections.push(createInjection('cognitive_intelligence', injection, priority));
                    }
                    log.debug({ userId, distortions: result.detectedDistortions }, 'Cognitive distortions detected');
                }
            }
            catch (error) {
        ...
        if (enableWellbeingTracking && userMessage.length > 15) {
            try {
                const { detectWellbeing, recordSnapshot, getWellbeingProfile } = await import('../../../services/wellbeing-tracking/index.js');
                const { checkWarnings, getWarningContextInjection } = await import('../../../services/wellbeing-tracking/early-warning.js');
                ...
                if (Object.keys(detected.dimensions).length > 0) {
                    result.wellbeingSignals = detected.dimensions;
                    // Record snapshot
        ...
        if (enableNudges && shouldOfferNudge(userMessage, topic, conversationPhase)) {
            try {
                const { selectNudges, getNudgeContextInjection } = await import('../../../services/behavioral-economics/nudge-engine.js');
        ...
        if (enableWisdom && turnNumber >= 3) {
            try {
                const { getWisdomContextInjection } = await import('../../../services/wisdom-synthesis/index.js');
        ...
        // =========================================================================
        try {
            const { getEndpointingRecommendation } = await import('../../../conversation/adaptive-endpointing.js');
...
// HELPER FUNCTIONS
// ============================================================================
function shouldOfferNudge(message, topic, phase) {
    // Don't nudge in opening or if message is too short
    if (phase === 'opening' || message.length < 30)
        return false;
...
function inferGoalType(topic, message) {
    const text = `${topic || ''} ${message || ''}`.toLowerCase();
    if (/health|exercise|sleep|eat|weight|fitness/.test(text))
        return 'health';
    if (/work|productivity|focus|procrastin|task/.test(text))
        return 'productivity';
    if (/relationship|partner|friend|family|dating/.test(text))
        return 'relationship';
    if (/money|budget|save|spend|debt|financial/.test(text))
        return 'financial';
    if (/habit|routine|daily|morning|evening/.test(text))
        return 'habit';
    if (/grow|learn|improve|better|develop/.test(text))
        return 'growth';
    return null;
}
function inferChangeStage(message) {
    const lower = message.toLowerCase();
    if (/thinking about|considering|might|maybe|wonder if/.test(lower))
        return 'considering';
    if (/going to|plan to|will start|ready to|decided/.test(lower))
        return 'planning';
    if (/started|trying|doing|working on|in the middle/.test(lower))
        return 'acting';
    if (/been doing|for \d+ (days|weeks|months)|keeping up/.test(lower))
        return 'maintaining';
    return null;
}
function inferSituationType(topic, emotionalState, message) {
...
function getPersonaScientificFocus(personaId) {
    const focuses = {
...
//# sourceMappingURL=scientific-coaching.js.map