 ...
 */
function configureUnifiedMemoryOrchestrator(newConfig) {
 ...
 */
async function buildUnifiedMemoryContext(input) {
    ...
    try {
        ...
        if (memory.formattedContext && memory.formattedContext.trim().length > 0) {
            ...
            injections.push(createStandardInjection('unified_memory', contextContent, {
                category: 'memory',
                confidence: 0.9,
            }));
            log.debug({
        ...
        if (turnCount >= config.minTurnForCallbacks && memory.callbacks.length > 0) {
            const callbackHints = memory.callbacks
                ...
                .join('\n');
            injections.push(createHintInjection('memory_callback_hints', `[MEMORY CALLBACKS - Weave in naturally if appropriate]\n${callbackHints}`, { category: 'memory', confidence: 0.7 }));
        }
        ...
        if (config.enableBehavioralPatterns && memory.activePatterns.length > 0) {
            const patternContext = memory.activePatterns
                ...
                .join('\n');
            injections.push(createHintInjection('behavioral_patterns', `[BEHAVIORAL AWARENESS - Patterns you've noticed]\n${patternContext}`, { category: 'cognitive', confidence: memory.activePatterns[0]?.confidence || 0.5 }));
        }
        ...
        if (config.enableApproachGuidance && memory.emotional.approachGuidance) {
            ...
            if (guidanceParts.length > 0) {
                injections.push(createHintInjection('approach_guidance', `[HOW TO APPROACH - Based on past interactions]\n${guidanceParts.join('\n')}`, { category: 'personalization', confidence: 0.6 }));
            }
        }
        ...
        if (memory.emotional.threads.length > 0) {
            ...
            injections.push(createHintInjection('emotional_thread', threadContext, {
                category: 'emotional',
                confidence: 0.7,
            }));
        }
        ...
        if (isSessionStart && memory.priming) {
            ...
            if (priming.suggestedOpener) {
                injections.push(createHintInjection('session_opener', `[SUGGESTED OPENER]\n${priming.suggestedOpener}`, {
                    category: 'engagement',
                    confidence: 0.8,
                }));
            }
            ...
            if (priming.sensitiveTopics.length > 0) {
                injections.push(createHintInjection('sensitive_topics', `[SENSITIVE TOPICS - Be gentle with these]\n${priming.sensitiveTopics.join(', ')}`, { category: 'emotional', confidence: 0.9 }));
            }
        }
        return injections;
    }
    catch (error) {
        log.warn({ error: error instanceof Error ? error.message : String(error), userId }, 'Unified memory orchestrator failed');
        return [];
    }
}
// ============================================================================
 ...
 */
export const unifiedMemoryOrchestratorBuilder = {
...
// EXPORTS
// ============================================================================
export { buildUnifiedMemoryContext, configureUnifiedMemoryOrchestrator };
export default unifiedMemoryOrchestratorBuilder;
//# sourceMappingURL=unified-memory-orchestrator.js.map