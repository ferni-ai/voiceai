...
import { BuilderCategory, createHintInjection, createHighInjection, createStandardInjection, registerContextBuilder, } from '../index.js';
// Import memory systems
import { getBestInsight } from '../../../services/trust-systems/tonal-memory.js';
import { getFollowUpOpportunity, detectPassingMentions, recordPassingMention, } from '../../../services/trust-systems/curiosity-memory.js';
import { getThinkingMomentToSurface, detectThinkingWorthy, recordThinkingMoment, } from '../../../services/trust-systems/between-session-thinking.js';
import { getGrowthMomentToShare, detectGrowthOpportunity, recordPersonaGrowth, } from '../../../services/trust-systems/persona-growth.js';
import { recordToneSignal, recordDepthSignal, recordTopics, detectTone, detectDepth, compareToUsual, getRecentTextureSummary, } from '../../../services/trust-systems/conversation-texture.js';
const log = createLogger({ module: 'MemoryEnhancementBuilder' });
const sessionGates = new Map();
function getGateState(sessionId) {
...
// MEMORY ENHANCEMENT CONTEXT BUILDER
// ============================================================================
export const memoryEnhancementBuilder = {
    ...
    build: async (input) => {
        ...
        // ========================================================================
        if (userMessage) {
            const relationshipStage = analysis?.state?.trustLevel && analysis.state.trustLevel > 0.7 ? 'established' : 'familiar';
            ...
            if (growthResult.detected && growthResult.growthType) {
                ...
                log.debug({ userId, growthType: growthResult.growthType }, '🌱 Recorded persona growth opportunity');
            }
        }
        // ========================================================================
        ...
        if (userMessage) {
            ...
            const detectedDepth = detectDepth({
                ...
                isPersonal: /\b(my|me|i'm|i've|i am)\b.*\b(feel|afraid|hope|dream|love|hate)\b/i.test(userMessage),
                turnCount,
            });
...
// INJECTION BUILDERS
// ============================================================================
function buildThinkingInjection(moment) {
...
function buildCuriosityInjection(followUp) {
...
function buildTonalInjection(insight) {
...
function buildGrowthInjection(moment, personaId) {
...
function buildTextureInjection(comparison, _personaId) {
...
function buildRecentTextureInjection(summary, _personaId) {
