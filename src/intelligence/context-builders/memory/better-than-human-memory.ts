...
const DEFAULT_CONFIG: BetterThanHumanConfig = {
...
export function configureBetterThanHumanMemory(newConfig: Partial<BetterThanHumanConfig>): void {
...
function formatReactiveMemories(
  memories: Array<{ item: { content: string }; connectionStrength?: string }>
): string | null {
...
function formatCallbackSuggestions(callbacks: string[]): string | null {
...
function buildGreeting(sessionCount: number): string {
  if (sessionCount > 10) {
...
async function buildSessionStartContext(
  userId: string,
  userProfile?: { name?: string; totalConversations?: number }
...
async function buildProactiveSurfacing(
...
async function buildReactiveMemories(
  recallContext: SimpleRecallContext,
  currentInjectionCount: number
  ...
  if (recallResult.primaryMemories.length > 0) {
    ...
    if (reactiveContext !== null) {
      injections.push(
        createStandardInjection('reactive_memory', reactiveContext, {
          category: BuilderCategory.MEMORY,
        })
      );
    }
  ...
  if (hasCallbacks && hasRoom) {
    const callbackContext = formatCallbackSuggestions(
      recallResult.callbacks.map((c) => c.suggestedReference)
    );
    if (callbackContext !== null) {
      injections.push(
        createHintInjection('memory_callback', callbackContext, {
          category: BuilderCategory.MEMORY,
        })
      );
    }
...
function buildSurfacingContext(
...
async function buildBetterThanHumanMemoryContext(
  input: ContextBuilderInput
): Promise<ContextInjection[]> {
