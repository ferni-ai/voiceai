...
import { getProactiveMemories, shouldSurfaceMemory, } from '../../../services/memory/memory-management.js';
// Track when we last surfaced a memory (per session)
const lastMemorySurfacedTurn = new Map();
/**
 ...
 */
async function buildProactiveMemoryContext(input) {
    ...
    try {
        ...
        for (const memory of memories) {
            if (shouldSurfaceMemory(memory, turnCount, lastTurn)) {
                ...
                if (memory.priority === 'high') {
                    injections.push(createStandardInjection('proactive_memory', formattedMemory, {
                        category: 'memory',
                        confidence: memory.relevanceScore,
                    }));
                }
                else {
                    injections.push(createHintInjection('proactive_memory', formattedMemory, {
                        category: 'memory',
                        confidence: memory.relevanceScore,
                    }));
                }
        ...
        if (highPriorityCount > 0 && injections.length === 0 && turnCount <= 3) {
            injections.push(createHintInjection('pending_memories', `[📌 ${highPriorityCount} important thing(s) to follow up on when the conversation warms up]`, { category: 'memory' }));
        }
    }
    catch (error) {
 ...
 */
async function buildVoiceRecognitionContext(input) {
    ...
    try {
        const { generateVoiceRecognitionGreeting } = await import('../../../services/memory/memory-management.js');
        // If we have high confidence in voice match, suggest greeting
        if (userProfile.voiceSketch.confidence > 0.8) {
            const greeting = generateVoiceRecognitionGreeting(userProfile.voiceSketch.confidence, userProfile.name || userProfile.preferredName);
            if (greeting) {
                injections.push(createHintInjection('voice_recognition', `[🎤 VOICE RECOGNIZED: ${greeting}]`, {
 ...
 */
async function buildProactiveContext(input) {
...
//# sourceMappingURL=proactive-memory.js.map