...
import { buildMemoryIndex, retrieveMemories, getConversationPrimingMemories, computeMemoryEmbeddings, } from '../../../memory/advanced-retrieval.js';
 ...
 */
async function ensureMemoryIndex(userId, profile, turnCount) {
    if (!profile)
        return false;
    ...
    if (shouldRebuild) {
        try {
            ...
            void computeMemoryEmbeddings(userId).catch((e) => log.debug({ error: e }, 'Background embedding computation failed'));
 ...
 */
async function retrieveRelevantMemories(userId, userText, input) {
    try {
 ...
 */
async function getPrimingMemories(userId, personaId, sessionCount) {
    try {
 ...
 */
function formatMemoriesForContext(memories, isSessionStart) {
 ...
 */
function generateCallbackSuggestions(memories) {
    const suggestions = [];
 ...
 */
async function buildAdvancedMemoryContext(input) {
    ...
    if (memories.some((m) => m.score > 0.6)) {
        ...
        if (suggestions.length > 0) {
            injections.push(createHintInjection('memory_callbacks', `[💭 NATURAL CALLBACK OPTIONS]\n${suggestions.join('\n')}`, { category: 'memory' }));
        }
    }
    // Log for debugging
    log.debug({
