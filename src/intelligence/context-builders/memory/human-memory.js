 ...
 */
function getUpcomingDates(humanMemory, lookAheadDays = 7) {
 ...
 */
function buildDateContext(humanMemory) {
    ...
    return createStandardInjection('human_memory_dates', `[IMPORTANT DATES - Remember these!]\n${lines.join('\n')}`);
}
// ============================================================================
 ...
 */
function buildEmotionalSignatureContext(humanMemory, currentEmotion) {
    const sig = humanMemory.emotionalSignature;
    if (!sig)
        return null;
    const lines = [];
    // If user seems stressed, surface comfort patterns
    const isStressed = currentEmotion &&
    ...
    if (lines.length === 0)
        return null;
    return createHintInjection('human_memory_emotional', `[USER'S EMOTIONAL SIGNATURE - How to support them]\n${lines.join('\n')}`);
}
// ============================================================================
// AVOIDANCE CONTEXT
// ============================================================================
 ...
 */
function buildAvoidanceContext(humanMemory) {
    ...
    if (strongAvoidances.length === 0)
        return null;
    const lines = strongAvoidances.slice(0, 3).map((a) => {
        const approachHint = a.approach === 'never_raise'
            ? '(never bring up)'
    ...
    return createHintInjection('human_memory_avoidances', `[TOPICS THIS USER AVOIDS - Respect their boundaries]\n${lines.join('\n')}`);
}
// ============================================================================
 ...
 */
function getUnacknowledgedGrowth(humanMemory) {
 ...
 */
function buildGrowthContext(humanMemory) {
    ...
    return createHintInjection('human_memory_growth', `[GROWTH OPPORTUNITY - Celebrate their progress!]\n${lines.join('\n')}`);
}
// ============================================================================
 ...
 */
function getActiveInsideJokes(humanMemory) {
 ...
 */
function buildInsideJokesContext(humanMemory) {
    ...
    return createHintInjection('human_memory_jokes', `[INSIDE JOKES - Use naturally if relevant]\n${lines.join('\n')}\nDon't force it - only reference if it fits!`);
}
// ============================================================================
 ...
 */
function buildIdentityContext(humanMemory) {
    ...
    if (lines.length === 0)
        return null;
    return createHintInjection('human_memory_identity', `[WHO THEY ARE - Values & aspirations]\n${lines.join('\n')}`);
}
// ============================================================================
 ...
 */
function buildRunningThemesContext(humanMemory) {
    ...
    const lines = activeThemes.map((t) => {
        const sentimentHint = t.sentiment === 'positive'
            ? '(going well)'
    ...
    return createHintInjection('human_memory_themes', `[ONGOING THEMES IN THEIR LIFE]\n${lines.join('\n')}\nAsk for updates naturally!`);
}
// ============================================================================
// CHALLENGES CONTEXT
// ============================================================================
 ...
 */
function buildChallengesContext(humanMemory) {
    const challenges = humanMemory.growthArc?.challenges;
    if (!challenges?.length)
        return null;
    // Get active challenges
    const activeChallenges = challenges.filter((c) => c.status !== 'resolved' && c.status !== 'breakthrough');
    if (activeChallenges.length === 0)
        return null;
    const lines = activeChallenges.slice(0, 2).map((c) => {
        const statusHint = c.status === 'struggling'
            ? '(needs support)'
    ...
    return createHintInjection('human_memory_challenges', `[CHALLENGES THEY'RE WORKING THROUGH]\n${lines.join('\n')}`);
}
// ============================================================================
 ...
 */
function buildTemporalContext(humanMemory) {
    const temporal = humanMemory.temporal;
    if (!temporal?.seasonal?.length)
        return null;
    // Check if current season/time matches any pattern
    const now = new Date();
    const month = now.getMonth();
    // Map months to seasons
    const currentSeason = month >= 2 && month <= 4
        ? 'spring'
        : month >= 5 && month <= 7
            ? 'summer'
            : month >= 8 && month <= 10
    ...
    const matchingPatterns = temporal.seasonal.filter((p) => {
        if (p.timing === currentSeason)
            return true;
        if (p.timing === 'tax_season' && isTaxSeason)
            return true;
        if (p.timing === 'holidays' && isHolidaySeason)
            return true;
        return false;
    });
    ...
    const lines = matchingPatterns.slice(0, 2).map((p) => {
        const tone = p.emotionalTone === 'challenging' ? '⚠️' : p.emotionalTone === 'positive' ? '✨' : '';
        return `${tone} ${p.pattern} ${p.approach ? `- ${p.approach}` : ''}`;
    });
    return createHintInjection('human_memory_temporal', `[SEASONAL PATTERN - This time of year...]\n${lines.join('\n')}`);
}
// ============================================================================
 ...
 */
async function buildHumanMemoryContext(input) {
...
//# sourceMappingURL=human-memory.js.map