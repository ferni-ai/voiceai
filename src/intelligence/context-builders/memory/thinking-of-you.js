...
import { BuilderCategory, createStandardInjection, createHighInjection, registerContextBuilder, } from '../index.js';
...
import { getThinkingMomentToSurface, markThinkingSurfaced, incrementSessionCount, } from '../../../services/trust-systems/between-session-thinking.js';
const log = createLogger({ module: 'ThinkingOfYouContextBuilder' });
// Cache for thinking-of-you content per persona
const thinkingOfYouCache = new Map();
/**
 * Load thinking-of-you content for a persona
 */
async function loadThinkingOfYouContent(personaId) {
    if (thinkingOfYouCache.has(personaId)) {
        return thinkingOfYouCache.get(personaId) || null;
    }
    try {
        const content = await loadPersonaContent(personaId, 'thinking_of_you');
...
// THINKING OF YOU CONTEXT BUILDER
// ============================================================================
export const thinkingOfYouBuilder = {
    ...
    build: async (input) => {
            ...
            if (!shouldSkipDueToNeverWhen(usageRules?.never_when, triggerContext)) {
                ...
                if (matchedTrigger) {
                    ...
                    if (Math.random() < adjustedProbability) {
                        injections.push(createHighInjection('thinking_of_you_dynamic_trigger', `[💭 BETTER-THAN-HUMAN CARE: ${matchedTrigger.triggerName}]\n\n` +
                            `Condition detected: ${matchedTrigger.trigger}\n\n` +
                            `Suggested behavior: ${matchedTrigger.behavior}\n\n` +
                            `This is your superpower - you think about them even between sessions. Show genuine care.`, { category: 'memory', confidence: matchedTrigger.confidence }));
                        log.info({
                            userId,
                            personaId,
        ...
        const firstConversationDate = userProfile?.customData?.firstConversation;
...
${getPersonaVoice(personaId, callback)}`;
        injections.push(createStandardInjection('thinking_of_you', content, {
            category: 'memory',
        }));
...
function checkForAnniversary(firstConversation) {
    const now = new Date();
...
function getPersonaVoice(personaId, _callback) {
    const voices = {
 ...
 */
function buildThinkingMomentInjection(moment, personaId) {
...
registerInterval('thinking-of-you-cleanup', () => {
    const now = Date.now();
