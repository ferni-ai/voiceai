 ...
 */
function getMemoryCallback(topics, userName) {
    if (topics.length === 0)
        return null;
 ...
 */
function getCrossSessionMemory(services, userName) {
    ...
    if (profile.keyMoments && profile.keyMoments.length > 0) {
        ...
        references.push(`I've been thinking about what you shared last time—about ${moment.summary}...`);
    }
 ...
 */
function getTimeSinceContext(lastContact) {
    const daysSince = Math.floor((Date.now() - lastContact.getTime()) / (1000 * 60 * 60 * 24));
    if (daysSince === 0)
        return null;
    if (daysSince === 1)
        return "It's good to hear from you again! Just yesterday we were talking...";
    if (daysSince <= 3)
        return "Good to talk again! It's only been a few days.";
    if (daysSince <= 7)
        return 'A week already! Time flies. Good to reconnect.';
    if (daysSince <= 14)
        return "It's been a couple weeks—I was wondering how you were doing.";
    if (daysSince <= 30)
        return "About a month since we last talked. I'm glad you're back.";
    if (daysSince <= 90)
        return "It's been a while! A few months. How have things been?";
    return "It's been quite some time! I'm genuinely happy to hear from you again.";
}
/**
 * Get emotional continuity check
 */
function getEmotionalContinuity(profile) {
    if (!profile.emotionalPatterns || profile.emotionalPatterns.length === 0)
        return null;
    // Find most recent distress pattern
    const recentDistress = profile.emotionalPatterns.find((p) => p.intensity &&
        p.intensity > 0.5 &&
 ...
 */
function getIntelligentFollowUp(services) {
    const profile = services.userProfile;
    if (!profile)
        return null;
    // Follow up on goals
...
async function buildMemoryContext(input) {
    ...
    if (topics.length > 0 && turnCount > 3 && turnCount % 5 === 0) {
        ...
        injections.push(createHintInjection('memory_callback', `[MEMORY CALLBACK: Earlier in this conversation, ${userData.name ? userData.name : 'they'} mentioned ${topicToReference}. If natural, circle back to show you were listening.]`));
    }
    // -----------------------------------------------
    ...
    if (turnCount === 3 && !alreadyReferencedLastConversation && Math.random() < 0.4) {
        ...
        if (hasLastConversation) {
            injections.push(createHintInjection('cross_session_memory', `[CROSS-SESSION MEMORY: You remember previous conversations with ${userData.name || 'this person'}. If natural, reference something from your shared history. Only mention once, don't repeat.]`));
        }
    }
    // -----------------------------------------------
    ...
    if (turnCount >= 6 && turnCount <= 10 && Math.random() < 0.3) {
        ...
        if (intelligentFollowUp) {
            ...
            injections.push(createHintInjection('intelligent_followup', `[INTELLIGENT FOLLOW-UP: Based on your history, there's an opportunity to ask about ${intelligentFollowUp.context}. Ask in your own words if it feels natural.]`));
        }
    }
    // -----------------------------------------------
    ...
    if (turnCount <= 2 && userProfile?.lastContact) {
        const daysSince = Math.floor((Date.now() - new Date(userProfile.lastContact).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSince > 1) {
            // Note: Don't inject literal phrases - just provide context
            const timeFrame = daysSince > 30 ? 'over a month' : daysSince > 7 ? 'over a week' : 'a few days';
            injections.push(createHintInjection('time_since', `[TIME AWARENESS: It's been ${timeFrame} since you last talked. Acknowledge the time gap naturally if it fits.]`));
        }
    }
    // -----------------------------------------------
    ...
    if (turnCount <= 3 && userProfile?.emotionalPatterns) {
        ...
        const hasRecentDistress = Array.isArray(userProfile.emotionalPatterns) &&
            userProfile.emotionalPatterns.some((p) => p.emotion === 'distressed' || p.emotion === 'anxious' || p.emotion === 'sad');
        if (hasRecentDistress) {
            // Note: Don't inject literal phrases - just provide context
            injections.push(createStandardInjection('emotional_continuity', `[EMOTIONAL CONTINUITY: ${userData.name || 'They'} seemed distressed in recent conversations. Check in on how they're doing - in your own words, with genuine care.]`));
        }
    }
    // -----------------------------------------------
        ...
        try {
            ...
            if (keyMomentRetrieval.shouldReferenceKeyMoment(turnCount)) {
                ...
                if (relevantMoment) {
                    ...
                    const momentDesc = 'moment' in relevantMoment && relevantMoment.moment
                        ? String(relevantMoment.moment)
                        : 'something meaningful from your history';
                    injections.push(createStandardInjection('key_moment', `[KEY MOMENT: You remember a significant moment - ${momentDesc}. Reference it naturally if relevant, in your own words.]`));
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        if (enhancedContext && enhancedContext.trim().length > 0) {
            injections.push(createHintInjection('learning_context', `[PERSONALIZATION - What You've Learned About This User]\n${enhancedContext}`));
        }
    }
    catch (e) {
    ...
    // -----------------------------------------------
    try {
        const proactiveInsight = extServices.learningEngine?.getProactiveInsight(userProfile ?? null, turnCount);
        if (proactiveInsight) {
            // Note: Don't inject literal phrases - describe what insight is available
            injections.push(createHintInjection('proactive_insight', `[PROACTIVE MEMORY: You have an insight about ${userData.name || 'this person'} that may be relevant. Weave it naturally into conversation if it fits.]`));
        }
    }
    catch {
        // Non-blocking
    ...
    if (analysis.topics.detected.length > 0 && Math.random() < 0.3) {
        try {
            ...
            if (pastContext) {
                injections.push(createHintInjection('past_conversation', `[MEMORY RETRIEVAL: ${pastContext}]`));
            }
        }
        catch {
            // Non-blocking
