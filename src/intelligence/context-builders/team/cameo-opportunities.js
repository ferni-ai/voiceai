...
function getCameoCandidate(input) {
    const text = `${input.userText} ${(input.analysis.topics?.detected || []).join(' ')}`.toLowerCase();
...
// Builder
// ----------------------------------------------------------------------------
async function buildCameoOpportunitiesContext(input) {
    const injections = [];
    ...
    const isAdviceMoment = intent === 'seeking_advice' ||
    ...
    injections.push(createHintInjection('cameo_opportunity', `[🎬 CAMEO OPPORTUNITY]
...
Only use if it genuinely adds value - cameos are special moments.`, { category: 'team' }));
    logger.debug({ memberId: candidate.memberId, reason: candidate.reason, turnCount, intent }, 'Cameo opportunity identified');
    return injections;
}
// ----------------------------------------------------------------------------
// Register
...
//# sourceMappingURL=cameo-opportunities.js.map