 ...
 */
import { getTeamUnlockState, TEAM_MEMBERS, } from '../../../services/team-unlocks.js';
 ...
 */
function detectTopicMatch(text, memberId) {
 ...
 */
function getEligibleMembers(state, introducedThisSession) {
    ...
    for (const member of TEAM_MEMBERS) {
        ...
        if (introducedThisSession.has(member.memberId))
            continue;
 ...
 */
function checkFallbackEligibility(state, totalConversations, memberId) {
 ...
 */
export function findCameoUnlockCandidate(input, state, introducedThisSession) {
 ...
 */
export function markIntroduced(memberId) {
 ...
 */
export function wasIntroducedThisSession(memberId) {
    return introducedThisSession.has(memberId);
}
// ============================================================================
// CONTEXT BUILDER
// ============================================================================
async function buildCameoUnlockContext(input) {
    const injections = [];
    ...
    const topicContext = candidate.matchedTopics.length > 0
        ? `The user mentioned ${candidate.matchedTopics.join(', ')} - perfect timing!`
        : "You've been wanting to introduce them for a while now.";
    injections.push(createHintInjection('cameo_unlock_introduction', `[🎭 ${introType} OPPORTUNITY - ${candidate.displayName}]
...
- After introducing, you can offer to connect them for a chat`, { category: 'team' }));
    log.info({
...
//# sourceMappingURL=cameo-unlock.js.map