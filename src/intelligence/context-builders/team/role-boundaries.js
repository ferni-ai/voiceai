...
import { createCriticalInjection, registerContextBuilder, } from '../index.js';
 ...
 */
function detectBoundaryViolation(userText, currentPersonaId) {
    const text = userText.toLowerCase();
    for (const domain of DOMAIN_OWNERSHIP) {
        // Skip if this IS the current persona's domain
        if (domain.owner === currentPersonaId)
            continue;
...
YOU ARE ${currentPersonaName}. You have teammates who could help with this, but the user hasn't met them yet. Don't name specific people they haven't been introduced to.`;
        ...
        injections.push(createCriticalInjection('role_boundary_violation', boundaryGuidance, {
            category: 'boundary',
...
export { detectBoundaryViolation, DOMAIN_OWNERSHIP, roleBoundaryBuilder };
//# sourceMappingURL=role-boundaries.js.map