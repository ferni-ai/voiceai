 ...
 */
import { registerContextBuilder, createHintInjection, createCriticalInjection, } from '../index.js';
...
import { getCurrentAgent, suggestHandoff, getAgentContext, getLastHandoff, formatHandoffContextForAgent, getHandoffContextNew as getHandoffContext, // Use executor version which includes cognitiveContext
...
// ============================================================================
let cachedTriggerMap = null;
let triggerMapPromise = null;
/**
 * Get trigger map, loading from bundles if needed
 */
async function getTriggerMap() {
    if (cachedTriggerMap)
        return cachedTriggerMap;
    // Avoid multiple concurrent loads
    if (triggerMapPromise)
 ...
 */
function detectWakeWord(userText) {
 ...
 */
function getAgentInfo(canonicalId) {
    const displayName = getPersonaDisplayName(canonicalId);
 ...
 */
function legacyToCanonical(agentId) {
    const mapping = {
 ...
 */
async function buildHandoffContext(input) {
    ...
    if (wakeWord.isWakeWord && wakeWord.targetAgent !== currentAgent) {
        ...
        if (targetUnlocked) {
            ...
            injections.push(createCriticalInjection('wake_word_handoff', `[CRITICAL - IMMEDIATE HANDOFF REQUIRED]
...
This is NON-NEGOTIABLE. The user explicitly asked for ${wakeWord.targetName}.`));
            ...
            injections.push(createHintInjection('locked_wake_word', `[TEAM MEMBER NOT YET AVAILABLE]
...
Respond warmly: "I have a friend who'd be perfect for that, but we need to get to know each other a bit better first. In the meantime, I'm here - how can I help?"`));
    ...
    const semanticHandoff = detectHandoffEnhanced(userId, userText, currentCanonicalId);
    if (semanticHandoff.shouldHandoff && semanticHandoff.candidate) {
        ...
        if (targetAgentInfo && targetUnlocked && candidateId !== currentAgent) {
            ...
            if (confidence >= 0.7) {
                injections.push(createHintInjection('handoff_suggestion', `[🎯 STRONG HANDOFF MATCH: ${targetAgentInfo.name}]
...
Tool: ${targetAgentInfo.tool}`));
                ...
                injections.push(createHintInjection('handoff_suggestion', `[HANDOFF OPPORTUNITY: ${targetAgentInfo.name}]
...
Consider mentioning: "${warmIntro}"`));
        ...
        if (handoffSuggestion.suggest && handoffSuggestion.to !== currentAgent) {
            ...
            if (targetAgentInfo && targetUnlocked) {
                injections.push(createHintInjection('handoff_suggestion', `[HANDOFF SUGGESTION: User's request about "${handoffSuggestion.reason}" matches ${targetAgentInfo.name}'s specialty (${targetAgentInfo.specialty}).
...
Acknowledge the transition naturally and continue the conversation.]`;
        ...
        try {
            ...
            if (DEBUG_HANDOFF)
                log.debug({ error: trustErr }, 'Trust handoff context unavailable');
        }
    ...
    if (currentCanonical === 'ferni') {
        ...
        if (specializedTopics.length >= 2) {
            injections.push(createHintInjection('team_awareness', `[TEAM AWARENESS: The conversation is getting into specialized territory (${specializedTopics.join(', ')}).
