...
import { createStandardInjection, registerContextBuilder, BuilderCategory, } from '../index.js';
 ...
 */
export function buildCapabilityAwarenessContext(personaId) {
    const profile = TEAM_CAPABILITIES[personaId];
    if (!profile) {
...
${deferEntries
            .map(([topic, { persona, reason }]) => `- **${topic}** → Suggest ${persona.charAt(0).toUpperCase() + persona.slice(1)}. ${reason}`)
            .join('\n')}
...
${sharedEntries
            .map(([domain, personas]) => `- **${domain}**: You share this with ${personas.map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join(', ')}`)
            .join('\n')}
 ...
 */
export function getCapabilityProfile(personaId) {
    return TEAM_CAPABILITIES[personaId] || null;
}
/**
 * Find the best persona for a given topic.
 * Returns null if no clear specialist.
 */
export function findSpecialistFor(topic) {
 ...
 */
async function buildCapabilityAwarenessContextBuilder(input) {
    const { persona, userData } = input;
    const injections = [];
    // Get persona ID (handle both direct id and nested identity)
    const personaId = persona?.id || persona?.identity?.id;
    if (!personaId) {
    ...
    const capabilityContext = buildCapabilityAwarenessContext(personaId);
    if (capabilityContext) {
        injections.push(createStandardInjection('capability_awareness', capabilityContext, { category: 'team', confidence: 1.0 }));
        log.debug({ personaId, turnCount, contextLength: capabilityContext.length }, 'Injected capability awareness context');
    }
    // On first turn, also inject team overview
    if (turnCount === 0 || turnCount === 1) {
        const teamOverview = buildTeamOverview();
        injections.push(createStandardInjection('team_overview', teamOverview, { category: 'team', confidence: 1.0 }));
    }
    return injections;
}
// ============================================================================
...
//# sourceMappingURL=capability-awareness.js.map