 ...
 */
function detectTeamMemberMention(text) {
 ...
 */
function detectExpertiseMatch(text, topics) {
    const combinedText = `${text} ${topics.join(' ')}`.toLowerCase();
    // Score each team member by keyword matches
    let bestMatch = null;
    let bestScore = 0;
    for (const member of TEAM_MEMBERS) {
        const score = member.expertiseKeywords.filter((kw) => combinedText.includes(kw.toLowerCase())).length;
 ...
 */
function normalizeTeamMemberId(id) {
    return id.replace(/-/g, '_');
}
// ============================================================================
// CONTEXT BUILDER
// ============================================================================
async function buildTeamDynamicsContext(input) {
    ...
    if (mentionedMember && mentionedMember.id !== currentPersonaId) {
        ...
        if (bundleRuntime) {
            ...
            if (dynamic) {
                ...
                log.debug({ personaId: persona.id, mentioned: mentionedMember.id, unlocked: memberUnlocked }, 'Team member mentioned - injecting dynamics');
            }
        }
        return injections;
    }
    ...
    if (expertMatch && expertMatch.id !== currentPersonaId) {
        ...
        if (rng.fork('handoff').chance(0.25) && bundleRuntime) {
            ...
            if (dynamic) {
                ...
                log.debug({ personaId: persona.id, expertMatch: expertMatch.id, reason: 'expertise' }, 'Team expertise match - suggesting natural reference');
            }
        }
        return injections;
    }
    ...
    if (turnCount > 5 && organicRng.chance(0.1) && bundleRuntime) {
        ...
        if (teamMembers.length > 0) {
            ...
            const otherMembers = teamMembers.filter((id) => {
                if (id === currentPersonaId || id === persona.id)
                    return false;
            ...
            if (otherMembers.length > 0) {
                ...
                if (dynamic?.whatIAdmire) {
                    ...
                    injections.push(createHintInjection('team_organic', `[TEAM COLOR: If natural, you might briefly reference your colleague ${memberDesc} - speak about them warmly as a friend, only if it fits.]`));
                }
            }
        }
    }
