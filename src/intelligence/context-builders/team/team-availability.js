 ...
 */
import { getContextualUnlockTeaser, getTeamUnlockState, TEAM_MEMBERS, } from '../../../services/team-unlocks.js';
 ...
 */
export function isCoreTeamMember(agentId) {
    ...
    return TEAM_MEMBERS.some((m) => {
        ...
        return (normalizedMemberId === normalizedId ||
 ...
 */
function buildAvailableTeamSection(state) {
 ...
 */
function buildLockedTeamSection(state) {
    const lockedMembers = TEAM_MEMBERS.filter((m) => !state.unlockedMembers.includes(m.memberId) && m.memberId !== 'ferni');
 ...
 */
function buildNextUnlockHint(state) {
    if (!state.nextUnlock)
        return '';
...
// CONTEXT BUILDER
// ============================================================================
async function buildTeamAvailabilityContext(input) {
    ...
    injections.push(createStandardInjection('team_availability', availableSection, {
        category: 'team',
    }));
    // Inject locked team section if there are locked members
    const lockedSection = buildLockedTeamSection(state);
    if (lockedSection) {
        injections.push(createStandardInjection('team_locked', lockedSection, {
            category: 'team',
        }));
    }
    ...
    if (nextUnlockHint) {
        injections.push(createHintInjection('team_next_unlock', nextUnlockHint, {
            category: 'team',
        }));
    }
    ...
    if (teaser) {
        ...
        injections.push(createHintInjection('team_teaser', `[NATURAL MENTION: You have team members who could help with topics like this. If it fits naturally, you can mention you have colleagues with relevant expertise.]`, { category: 'team' }));
    }
    return injections;
}
// ============================================================================
 ...
 */
export function getUnlockedTeamMemberIds(userProfile, tier = 'free') {
    const state = getTeamUnlockState(userProfile, tier);
 ...
 */
export function isTeamMemberUnlocked(memberId, userProfile, tier = 'free') {
 ...
 */
export function getLockedMemberTeaser(memberId) {
