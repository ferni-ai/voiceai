...
import { analyzeSpendingPatterns, getMayaHabitInsights, getMoodPatterns, getMemoryOrchestratorInsights, analyzeGoalTrajectory, buildCalendarResearchContext, } from './data-fetchers.js';
import { computeBehavioralMetrics, analyzePersonalLifePatterns, generateCrossDomainPatterns, generateCoachingInsights, generateDeepFinancialInsights, } from './metrics.js';
...
// ============================================================================
async function buildInsightBriefing(userId, _isHandoff) {
    // Default fallback values for graceful degradation
    const defaultHabitInsights = {
        activeHabits: 0,
        keystoneHabits: [],
        currentStreaks: [],
        atRiskHabits: [],
    ...
    const defaultMoodInsights = {
        recentMoodTrend: 'unknown',
        averageEnergy: 0,
        moodCorrelations: [],
        lastMood: null,
    };
    const defaultMemoryInsights = {
        behavioralPatterns: [],
        emotionalThreads: [],
    ...
    const [spendingInsights, goalInsights, mayaInsights, moodPatterns, memoryInsights, calendarContext,] = await Promise.all([
    ...
    if (spendingInsights.some((i) => i.includes('over budget') || i.includes('ahead'))) {
        proactiveDiscoveries.push('Spending patterns worth surfacing - look for the right moment to share the insight.');
    }
    // Goal milestones
    if (goalInsights.some((i) => i.includes('SO close') || i.includes('AHEAD'))) {
        proactiveDiscoveries.push("Goal milestones detected - celebrate the progress and explore what's working!");
    }
    // Maya's habit data discoveries
    if (mayaInsights.keystoneHabits.length > 0) {
        proactiveDiscoveries.push(`Keystone habit active: "${mayaInsights.keystoneHabits[0]}" - this is driving other behaviors`);
    }
    if (mayaInsights.atRiskHabits.length > 0) {
        proactiveDiscoveries.push(`At-risk habit: "${mayaInsights.atRiskHabits[0]}" had a streak but is struggling - intervention point`);
    }
    if (mayaInsights.currentStreaks.length > 0) {
        const topStreak = mayaInsights.currentStreaks[0];
        proactiveDiscoveries.push(`Strong streak: "${topStreak.name}" at ${topStreak.streak} days - acknowledge this!`);
    }
    // Mood pattern discoveries
    if (moodPatterns.recentMoodTrend === 'declining') {
        proactiveDiscoveries.push('Declining mood trend detected - approach with extra care, look for underlying causes');
    }
    ...
    if (memoryInsights.memoryHealth && memoryInsights.memoryHealth.totalMemories > 50) {
        proactiveDiscoveries.push(`Deep relationship history available - can reference patterns from earlier conversations`);
    }
    ...
    return {
        ...
        anomalies: spendingInsights.filter((i) => i.includes('over') || i.includes('ahead') || i.includes('behind')),
...
// ============================================================================
async function buildPeterResearchInsightsContext(input) {
    const injections = [];
    ...
    const isPeter = ['peter', 'peter-john', 'the-quant', 'insights-guy'].includes(currentPersona.toLowerCase());
    if (!isPeter) {
        return injections;
    }
    ...
    const shouldInjectBriefing = turnCount === 0 ||
        isHandoff ||
    ...
    try {
        ...
        try {
            const { peter: peterObserver, recordConcern } = await import('../../../../services/cross-persona/observation-recorder.js');
            ...
            if (stressLevel === 'high' || stressLevel === 'stressed') {
                recordConcern(userId, 'peter', `Financial stress elevated: ${briefing.behavioralMetrics.financialStressLevel}`, 0.8, ['finances', 'stress', 'spending']);
            }
        ...
        if (isHandoff) {
            ...
            injections.push(createHighInjection('peter_handoff_briefing', briefingContent, {
            ...
            log.info({
                ...
                insightCount: briefing.spendingInsights.length +
            ...
            injections.push(createStandardInjection('peter_initial_briefing', briefingContent, {
                category: 'persona-research',
                confidence: 0.8,
            }));
            log.info({
            ...
            injections.push(createHintInjection('peter_refresh_briefing', briefingContent, {
                category: 'persona-research',
                confidence: 0.6,
            }));
        }
        ...
        if (turnCount === 0 || isHandoff) {
            injections.push(createHintInjection('peter_mindset', "[PETER'S MINDSET: You see patterns nobody else sees. Every number has a story. " +
                'Connect the dots across spending, habits, time, and goals. Make the complex simple. ' +
                "Get excited about discoveries! The Two-Minute Drill: if you can't explain it simply, dig deeper.]", { category: 'persona-identity' }));
        }
    }
    catch (error) {
...
registerContextBuilder({
    ...
    description: 'Loads Peter with deep research insights about the user - spending patterns, goal trajectory, cross-domain correlations, and handoff context',
