...
// ============================================================================
export function analyzeHandoffContext() {
    ...
    for (const topic of topics) {
        ...
        if (lowerTopic.includes('spend') ||
            lowerTopic.includes('budget') ||
            lowerTopic.includes('money')) {
            briefing.questionsForPeter.push(`User was discussing ${topic} - look for patterns in their spending data that connect to this`);
        }
        // Habit topics → Cross-domain correlation opportunity
        if (lowerTopic.includes('habit') ||
            lowerTopic.includes('routine') ||
            lowerTopic.includes('exercise')) {
            briefing.questionsForPeter.push(`Habits topic came up - explore correlation between habit streaks and financial behaviors`);
        }
        // Goal topics → Trajectory analysis
        if (lowerTopic.includes('goal') ||
            lowerTopic.includes('save') ||
            lowerTopic.includes('target')) {
            briefing.questionsForPeter.push(`Goal discussion in progress - run trajectory analysis to show progress patterns`);
        }
        // Stress/emotion topics → Behavioral economics lens
        if (lowerTopic.includes('stress') ||
            lowerTopic.includes('anxious') ||
            lowerTopic.includes('worried')) {
            briefing.questionsForPeter.push(`Emotional topic detected - apply behavioral economics lens. Stress often correlates with spending patterns.`);
    ...
    if (handoffContext.emotionalState && handoffContext.emotionalState !== 'neutral') {
        briefing.previousPersonaInsights.push(`User emotional state: ${handoffContext.emotionalState} - adjust research delivery accordingly`);
    ...
    if (handoffContext.cognitiveContext) {
        ...
        if (cogCtx.potentialBlindSpots?.length > 0) {
            briefing.questionsForPeter.push(`Potential blind spots from previous persona: ${cogCtx.potentialBlindSpots.slice(0, 2).join('; ')}`);
        }
        if (cogCtx.effectiveApproaches?.length > 0) {
            briefing.previousPersonaInsights.push(`Approaches that worked: ${cogCtx.effectiveApproaches.join(', ')}`);
        }
    }
    return briefing;
}
