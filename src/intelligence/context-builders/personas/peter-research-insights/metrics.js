...
// ============================================================================
export function computeBehavioralMetrics(mayaInsights, moodPatterns, spendingInsights) {
    const patterns = [];
    // Decision Quality Index (0-100)
    // Based on: energy level, mood trend, time of day, streak consistency
    let dqi = 70; // baseline
    if (moodPatterns.averageEnergy >= 3.5)
        dqi += 15;
    else if (moodPatterns.averageEnergy < 2.5)
        dqi -= 20;
    if (moodPatterns.recentMoodTrend === 'improving')
        dqi += 10;
    else if (moodPatterns.recentMoodTrend === 'declining')
        dqi -= 15;
    if (mayaInsights.averageSuccessRate > 0.7)
        dqi += 10;
    dqi = Math.max(0, Math.min(100, dqi));
    // Habit Formation Velocity
    let habitVelocity = 'unknown';
    if (mayaInsights.activeHabits > 0) {
        if (mayaInsights.averageSuccessRate > 0.8)
            habitVelocity = 'fast (21-30 days)';
        else if (mayaInsights.averageSuccessRate > 0.5)
            habitVelocity = 'moderate (30-66 days)';
        else
            habitVelocity = 'slow (66+ days)';
    }
    ...
    if (mayaInsights.currentStreaks.length >= 3) {
        patterns.push(`Multiple active streaks (${mayaInsights.currentStreaks.length}) = momentum building`);
    }
    return {
...
export function analyzeTimePatterns() {
    ...
    if (dayOfWeek === 0 && hour >= 17) {
        insights.push('Sunday evening - classic "Sunday scaries" time. Research shows impulse spending often spikes now. Worth watching.');
    }
    // End of month crunch
    if (dayOfMonth >= 28) {
        insights.push('End of month - budget psychology shifts. People either panic-cut spending or give up until next month. Which pattern does this user follow?');
    }
    // Monday morning correlation
    if (dayOfWeek === 1 && hour >= 7 && hour <= 10) {
        insights.push('Monday morning - decision quality tends to be high. Good time to tackle important financial decisions.');
    }
    // Friday afternoon
    if (dayOfWeek === 5 && hour >= 14) {
        insights.push('Friday afternoon - end-of-week impulse spending risk. The "I deserve this" pattern often kicks in.');
    }
    // Pre-market hours (if before 9:30 AM on weekday)
    if (dayOfWeek >= 1 && dayOfWeek <= 5 && hour >= 6 && hour < 9) {
        insights.push('Pre-market hours - if user has investments, they might be checking overnight news. Good time for research discussions.');
    }
    return insights;
}
// ============================================================================
...
// ============================================================================
export function analyzePersonalLifePatterns() {
    const insights = [];
    const now = new Date();
    const hour = now.getHours();
    const dayOfWeek = now.getDay();
    ...
    if (hour >= 9 && hour <= 11) {
        insights.push('Peak decision-making window (9-11 AM). Decision Quality Index is typically highest now. Good time for important discussions.');
    }
    else if (hour >= 14 && hour <= 15) {
        insights.push('Post-lunch energy dip. Decision quality often drops 30-40% in this window. Keep discussions light or energy-focused.');
    }
    else if (hour >= 21) {
        insights.push('Evening wind-down. Good time for reflection, not major decisions. Ask about how their day went.');
    }
    // Behavioral economics lens based on day
    if (dayOfWeek === 0) {
        insights.push('Sunday: "Sunday scaries" window. Anxiety and control-seeking spending often spike. Look for stress signals.');
    }
    else if (dayOfWeek === 1) {
        insights.push('Monday: Fresh start energy. Good day for goal-setting discussions. Motivation is typically higher.');
    }
    else if (dayOfWeek === 5 && hour >= 14) {
        insights.push('Friday afternoon: "I deserve this" psychology kicks in. Watch for rationalized spending or impulsive decisions.');
    }
    return insights;
}
// ============================================================================
...
// ============================================================================
export function generateCrossDomainPatterns() {
...
// ============================================================================
export function generateDeepFinancialInsights() {
    return [
