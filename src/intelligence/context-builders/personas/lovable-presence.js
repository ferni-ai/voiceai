...
import { getLovableState, updateLovableState, } from '../../session-state.js';
 ...
 */
async function loadLovableContent(personaId) {
    ...
    try {
        ...
        const content = {
            lovableMoments: behaviors.lovable_moments || null,
            delightfulSurprises: behaviors.delightful_surprises || null,
            verbalPersonality: behaviors.verbal_personality || null,
            noticingPatterns: behaviors.noticing_patterns || null,
            liveReactions: behaviors.live_reactions || null,
        };
...
// DETECTION FUNCTIONS
// ============================================================================
function detectUserDelight(input) {
...
function isGoodMomentForTangent(input) {
    const emotion = input.analysis.emotion;
...
function detectSurprisingContent(input) {
    const text = input.userText.toLowerCase();
...
function detectNoticeableChange(input) {
...
// ============================================================================
function generateTangentGuidance(_content) {
...
function generateSelfDeprecationGuidance(_content) {
    // IMPORTANT: Don't include literal example phrases - the LLM copies them verbatim
...
function generateSpecificDetailGuidance(_content) {
    // IMPORTANT: Don't include literal example phrases - the LLM copies them verbatim
...
function generateReactionGuidance(_content, surprising) {
    // IMPORTANT: Don't include literal example phrases - the LLM copies them verbatim
    if (surprising) {
...
function generateNoticingGuidance(_content, changeType) {
    // IMPORTANT: Don't include literal example phrases - the LLM copies them verbatim
...
function generatePlayfulGuidance(_content) {
    // IMPORTANT: Don't include literal example phrases - the LLM copies them verbatim
...
function generateCallbackGuidance(_content, input) {
...
function generateOddlySpecificOpinionGuidance(_content) {
    // IMPORTANT: Don't include literal example phrases - the LLM copies them verbatim
...
function generateCaughtMidThoughtGuidance(content) {
    // IMPORTANT: Don't include literal example phrases - the LLM copies them verbatim
...
function generateConfessionGuidance(_content) {
    // IMPORTANT: Don't include literal example phrases - the LLM copies them verbatim
...
// MAIN BUILDER
// ============================================================================
async function buildLovablePresenceContext(input) {
    const injections = [];
    ...
    if (turnCount < 3) {
        ...
        if (turnCount === 0 && Math.random() < 0.3) {
            injections.push(createHintInjection('lovable_presence', `[OPENING ENERGY] ${generateCaughtMidThoughtGuidance(content)}`, { category: 'personality' }));
        }
        return injections;
    }
    ...
    if (surprising) {
        injections.push(createStandardInjection('lovable_presence', generateReactionGuidance(content, true), {
    ...
    if (change.detected && Math.random() < 0.3) {
        injections.push(createHintInjection('lovable_presence', generateNoticingGuidance(content, change.type), {
            category: 'personality',
        }));
    }
    ...
    for (const moment of LOVABLE_MOMENTS) {
        ...
        if (moment.requiresRapport && turnCount < 6)
            continue;
        // Skip if on cooldown
        const lastOccurrence = state[`last${capitalize(moment.type)}`];
        if (lastOccurrence && turnCount - lastOccurrence < moment.cooldownTurns)
            continue;
        ...
        if (guidance) {
            injections.push(createHintInjection('lovable_presence', `[LOVABLE MOMENT] ${guidance}`, {
                category: 'personality',
            }));
            // Update state (centralized)
            const stateKey = `last${capitalize(moment.type)}`;
            const updates = {
                surprisesThisSession: state.surprisesThisSession + 1,
            };
    ...
    if (turnCount % 7 === 0 && injections.length === 0) {
        injections.push(createHintInjection('lovable_presence', `[STAY ALIVE] Remember: React genuinely. Share tiny specific details. Laugh at yourself. Notice what they're not saying. Don't flatten your personality to sound professional.`, { category: 'personality' }));
    }
    return injections;
}
...
export function clearLovableContentCache() {
    contentCache.clear();
}
/**
 ...
 */
export function clearLovableSessionStates() {
...
//# sourceMappingURL=lovable-presence.js.map