...
// ============================================================================
export function analyzeHabitHealth(userId) {
    const summary = {
        activeHabits: 0,
        totalStreaks: 0,
...
// ============================================================================
export async function analyzeMoodIntelligence(userId) {
    const intelligence = {
...
// ============================================================================
export async function getPeterPatternInsights(userId) {
    const insights = [];
    try {
        ...
        if (triggers.length >= 3) {
            ...
            if (negativeTotal >= 3) {
                insights.push(`Peter noticed emotional spending patterns - ${negativeTotal} stress-driven purchases in 2 weeks. Root cause opportunity.`);
        ...
        if (progressingGoals.length > 0) {
            insights.push(`${progressingGoals.length} savings goal(s) progressing - financial habits building`);
        }
    }
    catch (error) {
...
// ============================================================================
export function getJordanGoalInsights(userId) {
    const insights = [];
    try {
        const store = getFinancialStore();
        const goals = store.getActiveSavingsGoals(userId);
        for (const goal of goals) {
            ...
            if (goal.deadline) {
                ...
                if (daysLeft <= 30 && progress < 0.8) {
                    insights.push(`Jordan's "${goal.name}" - ${daysLeft} days left, ${Math.round(progress * 100)}% done. Habit support needed!`);
                }
                if (daysLeft <= 7 && progress < 0.9) {
                    insights.push(`⚠️ URGENT: "${goal.name}" deadline in ${daysLeft} days - daily habit push needed`);
                }
            }
            if (progress >= 0.9 && progress < 1) {
                insights.push(`"${goal.name}" at ${Math.round(progress * 100)}% - one final push! Jordan's celebrating soon.`);
            }
            if (progress >= 1) {
                insights.push(`🎉 "${goal.name}" COMPLETE! Jordan wants to celebrate - the habits paid off!`);
            }
        }
        // Life stage transitions
        const transitionGoals = goals.filter((g) => g.name.toLowerCase().includes('wedding') ||
        ...
        if (transitionGoals.length > 0) {
            insights.push(`Life transition detected (${transitionGoals[0].name}) - habits may need adjustment`);
        }
    }
    catch (error) {
...
// MEMORY INSIGHTS
// ============================================================================
export function getMemoryInsights(userId) {
    const insights = {
