...
import { analyzeHabitHealth, analyzeMoodIntelligence, getPeterPatternInsights, getJordanGoalInsights, getMemoryInsights, } from './data-fetchers.js';
...
// BUILD BRIEFING
// ============================================================================
async function buildMayaBriefing(userId) {
    // Default fallback values for graceful degradation
    const defaultHabitHealth = {
        activeHabits: 0,
        totalStreaks: 0,
    ...
    const [habitHealth, moodIntelligence, peterInsights, jordanInsights, memoryInsights, calendarInsights,] = await Promise.all([
    ...
    const winsToCelebrate = [];
    if (habitHealth.longestStreak && habitHealth.longestStreak.days >= 7) {
        winsToCelebrate.push(`${habitHealth.longestStreak.name} - ${habitHealth.longestStreak.days} day streak!`);
    }
    ...
    if (habitHealth.averageSuccessRate > 0.7) {
        winsToCelebrate.push(`${Math.round(habitHealth.averageSuccessRate * 100)}% overall success rate - excellent!`);
    }
    if (coachingMetrics.momentumScore > 70) {
        winsToCelebrate.push('Momentum is strong - energy is building');
    }
    // Identify struggles
    const strugglesToAddress = habitHealth.recentSetbacks.map((name) => `"${name}" streak broke - needs gentle restart`);
...
// MAIN BUILDER
// ============================================================================
async function buildMayaCoachingInsightsContext(input) {
    const injections = [];
    const { services, userData } = input;
    // Only for Maya
    const currentPersona = services?.personaId || '';
    const isMaya = ['maya', 'maya-santos', 'habits-coach', 'life-coach'].includes(currentPersona.toLowerCase());
    if (!isMaya)
        return injections;
    ...
    try {
        ...
        try {
            const { maya: mayaObserver } = await import('../../../../services/cross-persona/observation-recorder.js');
            // Record concerning patterns
            if (briefing.habitHealth.atRiskCount > 0) {
                mayaObserver.concern(userId, `${briefing.habitHealth.atRiskCount} habits at risk of breaking streak`, 0.8, ['habits', 'streak', 'motivation']);
            }
            ...
            if (briefing.coachingMetrics.momentumScore > 70) {
                mayaObserver.pattern(userId, `Strong momentum score (${briefing.coachingMetrics.momentumScore}/100)`, 0.8, ['habits', 'momentum', 'progress']);
            }
            // Record milestones
            if (briefing.habitHealth.longestStreak && briefing.habitHealth.longestStreak.days >= 7) {
                mayaObserver.milestone(userId, `${briefing.habitHealth.longestStreak.days}-day streak on ${briefing.habitHealth.longestStreak.name}`, 0.9, ['habits', 'streak', 'achievement']);
            }
            ...
            if (briefing.proactiveTriggers.length > 0) {
                ...
                mayaObserver.opportunity(userId, topTrigger.message || 'Coaching opportunity detected', confidenceMap[topTrigger.priority] || 0.7, undefined, // suggestedAction not available on this ProactiveTrigger type
                ['habits', 'coaching']);
            }
        }
        catch (err) {
        ...
        if (isHandoff) {
            injections.push(createHighInjection('maya_handoff_briefing', content, {
            ...
            injections.push(createStandardInjection('maya_initial_briefing', content, {
                category: 'persona-coaching',
                confidence: 0.8,
            }));
            log.info({
            ...
            injections.push(createHintInjection('maya_refresh_briefing', content, {
                category: 'persona-coaching',
            }));
        }
        ...
        if (turnCount === 0 || isHandoff) {
            injections.push(createHintInjection('maya_mindset', "[MAYA'S HEART: You believe in them before they believe in themselves. " +
                'Celebrate every tiny step. Meet setbacks with compassion, not criticism. ' +
                "Progress isn't linear - and that's okay. Start small. Stay patient. Trust the process.]", { category: 'persona-identity' }));
        }
    }
    catch (error) {
...
registerContextBuilder({
    ...
    description: 'Loads Maya with deep coaching insights - computed metrics, mood intelligence, proactive triggers, and cross-team patterns',
    priority: 45,
