...
import { recordEmotionalDataPoint, } from '../../../personality/emotional-patterns.js';
import { extractCallbackKeyMoments, findRelevantMomentSemantic, formatCallbackForPrompt, getPendingCallbacksFromProfile, warmUpPersonaEmbeddings, } from '../../../personality/memory-adapter.js';
import { analyzeMessageTiming, shouldSharePersonalMoment, } from '../../../personality/timing-intelligence.js';
 ...
 */
function calculateRelationshipStage(totalConversations, sharedVulnerabilities = 0) {
    // Stranger: 0-2 conversations
    ...
    if (totalConversations > 25 && sharedVulnerabilities >= 3)
        return 'trusted';
    // Friend: 11+ conversations OR has shared vulnerabilities
    if (totalConversations > 10 || sharedVulnerabilities > 0)
        return 'friend';
    return 'acquaintance';
}
// ============================================================================
 ...
 */
async function buildHumanPersonalityContext(input) {
    const { userText, persona, userData, services, analysis } = input;
    const injections = [];
    // Get user data
    const personalityData = userData;
    ...
    const vulnerabilityCount = services?.userProfile?.keyMoments?.filter((m) => m.type === 'shared_vulnerability').length || 0;
    const relationshipStage = personalityData.relationshipStage ||
    ...
    recordEmotionalDataPoint(userId, detectedEmotion, emotionalIntensity, detectedTopics, userText.slice(0, 100));
    ...
    if (isCallbackWindow &&
        ...
        if (pendingCallbacks.length > 0) {
            ...
            injections.push(createHintInjection('human_personality_callback', callbackInjection, {
                category: 'personality',
            }));
            // Mark as surfaced
            if (userData) {
                userData.callbackSurfacedThisSession = true;
            }
            log.info({
        ...
        try {
            ...
            if (relevantMoment && relevantMoment.relevanceScore > 0.4) {
                ...
                if (timingCheck.should) {
                    ...
                    const depthDesc = relevantMoment.moment.depth === 'surface'
                    ...
                    const momentInjection = [
                        ...
                        .filter(Boolean)
                        .join('\n');
                    injections.push(createHintInjection('human_personality_moment', momentInjection, {
                        category: 'personality',
                    }));
                    if (userData) {
                        userData.momentSharedThisSession = true;
                    }
                    log.debug({
                        personaId: persona.id,
                    ...
                    log.debug({ reason: timingCheck.reason }, '⏱️ Personal moment skipped - timing not right');
                }
            }
        }
        catch (error) {
    ...
    if (callbackMoments.length > 0) {
        ...
        if (callbackMoments.some((m) => m.emotionalWeight === 'heavy')) {
            injections.push(createHintInjection('human_personality_remember', [
                '[📝 REMEMBER THIS]',
                '',
 ...
 */
async function warmUpHumanPersonality(personaId) {
    try {
...
//# sourceMappingURL=human-personality.js.map