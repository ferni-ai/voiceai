...
// ============================================================================
export function computePlanningMetrics(goalsOverview, peterInsights, mayaInsights, moodPatterns) {
    const patterns = [];
    // Planning Velocity Index (0-100)
    // Based on: goal progress rate, financial capacity, habit support
    let planningVelocity = 50;
    if (peterInsights.savingsVelocity === 'rapid')
        planningVelocity += 20;
    else if (peterInsights.savingsVelocity === 'steady')
        planningVelocity += 10;
    if (goalsOverview.nearingCompletion.length > 0)
        planningVelocity += 15;
    if (goalsOverview.atRisk.length > 0)
        planningVelocity -= 15;
    if (mayaInsights.planningRelatedHabits.length > 0)
        planningVelocity += 10;
    ...
    if (moodPatterns.celebrationReadiness === 'high')
        celebrationReadiness += 25;
    else if (moodPatterns.celebrationReadiness === 'low')
        celebrationReadiness -= 20;
    if (peterInsights.budgetHealth === 'excellent')
        celebrationReadiness += 15;
    else if (peterInsights.budgetHealth === 'stressed')
        celebrationReadiness -= 15;
    if (moodPatterns.recentMoodTrend === 'improving')
        celebrationReadiness += 10;
    ...
    if (goalsOverview.activeGoals >= 3)
        lifeStageMomentum += 15;
    if (goalsOverview.milestoneDates.length > 0)
        lifeStageMomentum += 15;
    if (goalsOverview.recentlyAchieved.length > 0)
        lifeStageMomentum += 20;
    if (mayaInsights.momentumScore > 70)
        lifeStageMomentum += 10;
    lifeStageMomentum = Math.max(0, Math.min(100, lifeStageMomentum));
    // Event Success Predictor (0-100)
    // Composite of all factors
    const eventSuccessPredictor = Math.round(planningVelocity * 0.3 +
...
// LIFE STAGE CONTEXT
// ============================================================================
export function analyzeLifeStageContext(goalsOverview, memoryInsights) {
    const context = {
    ...
    if (goalsOverview.biggestGoal) {
        ...
        if (name.includes('house') || name.includes('home')) {
            ...
            context.stageSpecificAdvice.push('Home buying chapter! Focus on: neighborhood research, pre-approval timeline, inspection checklist');
            ...
            context.stageSpecificAdvice.push('Wedding planning! Key decisions: venue → date → guest list → vendors. Timeline is everything.');
            ...
            context.stageSpecificAdvice.push('Growing family chapter! Consider: childcare plans, life insurance, estate basics');
            ...
            context.stageSpecificAdvice.push('Retirement chapter! Focus on: income sources, healthcare coverage, purpose discovery');
            ...
            context.stageSpecificAdvice.push('Education milestone! Consider: celebration plans, next chapter preparation');
        }
    }
    ...
    context.stageSpecificAdvice.push('Remember: Hard chapters deserve presence, not positivity. Celebrate ALL progress.');
    return context;
}
// ============================================================================
// SEASONAL AWARENESS
...
export function analyzeSeasonalContext() {
    ...
    if (month >= 4 && month <= 5) {
        ...
        context.seasonalOpportunities.push('🎓 GRADUATION SEASON! I cry at graduations. Any graduations. Happy tears.');
        ...
        context.seasonalOpportunities.push('💒 WEDDING SEASON! Peak planning energy. Love is in the air.');
