...
// ============================================================================
export function formatJordanBriefing(briefing, handoffBriefing, turnCount) {
    const sections = [];
    ...
    if (handoffBriefing) {
        ...
        const excitementEmoji = handoffBriefing.excitementLevel === 'high'
            ? '🎉'
        ...
        if (handoffBriefing.emotionalWeight > 0.5) {
            sections.push('NOTE: User may be emotionally charged. Start with validation before diving into planning.');
        }
    }
    ...
    if (goalsOverview.biggestGoal) {
        sections.push(`• Biggest goal: "${goalsOverview.biggestGoal.name}" (${goalsOverview.biggestGoal.progress}% complete)`);
    }
    ...
    if (briefing.lifeStageContext.currentStage !== 'active-planning') {
        ...
        briefing.lifeStageContext.transitionSignals.forEach((signal) => sections.push(`• 🔄 ${signal}`));
    }
    ...
    if (briefing.moodPatterns.lastMood || briefing.moodPatterns.recentMoodTrend !== 'unknown') {
        ...
        if (briefing.moodPatterns.lastMood) {
            sections.push(`• Last mood: ${briefing.moodPatterns.lastMood.mood} mood, ${briefing.moodPatterns.lastMood.energy} energy`);
        }
