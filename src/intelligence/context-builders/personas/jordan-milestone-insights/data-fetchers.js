...
// ============================================================================
export async function analyzeGoalsOverview(userId) {
    const overview = {
        activeGoals: 0,
        nearingCompletion: [],
        atRisk: [],
    ...
    try {
        ...
        for (const goal of goals) {
            ...
            if (goal.deadline) {
                ...
                const totalDays = Math.ceil((deadline.getTime() - new Date(goal.createdAt || now).getTime()) / (1000 * 60 * 60 * 24));
                const daysElapsed = Math.ceil((now.getTime() - new Date(goal.createdAt || now).getTime()) / (1000 * 60 * 60 * 24));
...
// ============================================================================
export async function getPeterFinancialInsights(userId) {
    const insights = {
        budgetHealth: 'good',
    ...
    try {
        ...
        if (budget) {
            ...
            if (percentUsed < 50) {
                ...
                insights.financialReadiness.push(`Peter confirms: Budget healthy at ${Math.round(percentUsed)}% - excellent runway for milestone planning!`);
            }
            else if (percentUsed < 75) {
                insights.budgetHealth = 'good';
                insights.financialReadiness.push(`Peter notes: Budget at ${Math.round(percentUsed)}% - solid foundation for planning`);
            }
            else if (percentUsed < 90) {
                insights.budgetHealth = 'tight';
                insights.financialReadiness.push(`Peter flags: Budget at ${Math.round(percentUsed)}% - timeline planning should account for this`);
            }
            else {
                insights.budgetHealth = 'stressed';
                insights.financialReadiness.push(`Peter warns: Budget stretched at ${Math.round(percentUsed)}% - may need to adjust milestone timelines`);
            }
            ...
            if (goals.length > 0) {
                ...
                if (savingsCapacity > 0 && totalNeeded > 0) {
                    ...
                    if (monthsNeeded < 6) {
                        ...
                        insights.financialReadiness.push(`🚀 Peter calculates: ALL goals achievable in ~${Math.ceil(monthsNeeded)} months at current pace!`);
                    }
                    else if (monthsNeeded < 12) {
                        insights.savingsVelocity = 'steady';
                        insights.financialReadiness.push(`Peter projects: Goals on track for completion within ${Math.ceil(monthsNeeded)} months`);
                    }
                    else {
                        insights.savingsVelocity = 'gradual';
                        insights.financialReadiness.push(`Peter notes: Current pace suggests ${Math.ceil(monthsNeeded)} months to goal completion - may need acceleration`);
                    }
                }
            }
        }
        ...
        const celebrationSpending = triggers.filter((t) => t.emotion === 'celebratory' || t.emotion === 'happy');
        if (celebrationSpending.length >= 2) {
            insights.financialReadiness.push(`🎊 Peter noticed ${celebrationSpending.length} celebration purchases recently - someone's hitting milestones!`);
        }
    }
    catch (error) {
...
export async function getMayaHabitInsights(userId) {
    ...
    try {
        ...
        let momentum = 50;
        if (insights.keystoneHabits.length > 0)
            momentum += 15;
        if (insights.averageSuccessRate > 0.7)
            momentum += 15;
        if (insights.currentStreaks.length >= 3)
            momentum += 10;
        if (insights.atRiskHabits.length > 2)
            momentum -= 15;
...
export async function getMemoryOrchestratorInsights(userId) {
    ...
    try {
        ...
        if (health.totalMemories > 10) {
            insights.milestoneMentions.push(`${health.totalMemories} memories available - can reference past conversations`);
        }
        if (health.totalMemories > 20) {
            insights.relationshipMilestones.push(`Rich history (${health.totalMemories} memories) - can reference past conversations about life events`);
        }
        if (health.recentMemories > 5) {
            insights.milestoneMentions.push(`${health.recentMemories} recent memories - active relationship`);
        }
        if (health.emotionalMemories > 3) {
            insights.pastCelebrations.push(`${health.emotionalMemories} emotionally significant memories - deep context available`);
        }
        if (health.commitments > 0) {
            insights.familyContext.push(`${health.commitments} active commitments tracked - accountability context available`);
        }
    }
    catch (error) {
