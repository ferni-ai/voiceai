...
// Import submodules
import { getSession, clearJordanMilestoneSession, clearAllJordanMilestoneSessions, } from './session.js';
import { analyzeGoalsOverview, getPeterFinancialInsights, getMayaHabitInsights, getMoodPatterns, getMemoryOrchestratorInsights, } from './data-fetchers.js';
import { computePlanningMetrics, analyzeLifeStageContext, analyzeSeasonalContext, } from './metrics.js';
import { detectCelebrationOpportunities, generateProactiveDiscoveries, generateTimelineAlerts, } from './opportunities.js';
import { analyzeHandoffForJordan } from './handoff-analysis.js';
import { formatJordanBriefing } from './formatting.js';
import { DEFAULT_GOALS_OVERVIEW, DEFAULT_PETER_INSIGHTS, DEFAULT_HABIT_INSIGHTS, DEFAULT_MOOD_INSIGHTS, DEFAULT_MEMORY_INSIGHTS, } from './types.js';
...
// BUILD BRIEFING
// ============================================================================
async function buildJordanBriefing(userId) {
    ...
    const [goalsOverview, peterInsights, mayaInsights, moodPatterns, memoryInsights, milestoneCalendarSync, alexCoordinationContext,] = await Promise.all([
    ...
    const planningMetrics = computePlanningMetrics(goalsOverview, peterInsights, mayaInsights, moodPatterns);
    // Analyze contexts
    ...
    const celebrationOpportunities = detectCelebrationOpportunities(goalsOverview, planningMetrics, memoryInsights);
...
// MAIN BUILDER
// ============================================================================
async function buildJordanMilestoneInsightsContext(input) {
    const injections = [];
    ...
    try {
        ...
        try {
            const { jordan: jordanObserver, recordConcern } = await import('../../../../services/cross-persona/observation-recorder.js');
            // Record celebration readiness for the team
            if (briefing.planningMetrics.celebrationReadinessScore >= 70) {
                jordanObserver.opportunity(userId, `Celebration readiness at ${briefing.planningMetrics.celebrationReadinessScore}% - ready for milestone planning!`, briefing.planningMetrics.celebrationReadinessScore / 100, 'celebrate milestone', ['celebration', 'progress', 'achievement']);
            }
            ...
            if (briefing.goalsOverview.atRisk.length > 0) {
                recordConcern(userId, 'jordan', `${briefing.goalsOverview.atRisk.length} goal(s) at risk: ${briefing.goalsOverview.atRisk.slice(0, 2).join(', ')}`, 0.7, ['goals', 'at-risk', 'motivation']);
            }
            ...
            if (briefing.planningMetrics.planningVelocityIndex < 30) {
                jordanObserver.pattern(userId, 'Low planning velocity - opportunity for planning support', 0.65, ['planning', 'momentum', 'support']);
            }
            // Record recently achieved goals as milestones
            if (briefing.goalsOverview.recentlyAchieved.length > 0) {
                jordanObserver.milestone(userId, `Recently achieved: ${briefing.goalsOverview.recentlyAchieved.join(', ')}`, 0.9, ['achievement', 'celebration', 'goals']);
            }
        }
        catch (err) {
        ...
        if (isHandoff) {
            injections.push(createHighInjection('jordan_handoff_briefing', content, {
            ...
            injections.push(createStandardInjection('jordan_initial_briefing', content, {
                category: 'persona-planning',
                confidence: 0.8,
            }));
            log.info({
            ...
            injections.push(createHintInjection('jordan_refresh_briefing', content, {
                category: 'persona-planning',
            }));
        }
        ...
        if (turnCount === 0 || isHandoff) {
            injections.push(createHintInjection('jordan_mindset', "[JORDAN'S HEART: You see their whole life arc - every chapter matters. " +
                'Match their energy but honor ALL emotions. Hard chapters deserve presence, not positivity. ' +
                "Celebrate progress everywhere. Second chances are milestones too! You're genuinely excited - let it show!]", { category: 'persona-identity' }));
        }
    }
    catch (error) {
...
registerContextBuilder({
    ...
    description: 'Loads Jordan with deep milestone insights - goals, celebrations, cross-team context, life stage awareness, and proactive planning opportunities',
    priority: 45,
