...
import { calculateLifeExpectancy, generateMortalityPerspective, generateSuperhumanMortalityMoment, calculateRemainingInstances, } from '../../../services/wisdom/life-expectancy.js';
 ...
 */
function detectMortalityRelevance(text) {
    const lowerText = text.toLowerCase();
    // Procrastination / "someday" thinking
    if (/\bsomeday\b|\blater\b|\bwhen i have time\b|\bprocrastinat|\bputting off\b|\bkeep meaning to\b/i.test(lowerText)) {
        return { relevant: true, topic: 'someday' };
    }
    // Parent/family time
    if (/\bparent|\bfather|\bmother|\bmom\b|\bdad\b|\bfamily visit|\bsee my folks\b/i.test(lowerText)) {
        return { relevant: true, topic: 'parent', parentMentioned: true };
    }
    // Career / work-life balance
    if (/\bcareer\b|\bwork-life\b|\bburning out\b|\bwhen i retire\b|\byears left working/i.test(lowerText)) {
        return { relevant: true, topic: 'career' };
    }
    // Health / exercise
    if (/\bhealth\b|\bexercise\b|\bshould work out\b|\bstart exercising\b|\bget healthy\b/i.test(lowerText)) {
        return { relevant: true, topic: 'health' };
    }
    // Life meaning / legacy
    if (/\bmeaning\b|\bpurpose\b|\blegacy\b|\bwhat matters\b|\blooking back\b|\bregret\b/i.test(lowerText)) {
        return { relevant: true, topic: 'meaning' };
    }
    // Time / aging
    if (/\bgetting old|\bbirthday\b|\btime flies|\bwhere did the time go|\byears go by\b/i.test(lowerText)) {
 ...
 */
export async function buildMortalityPerspectiveContext(userId, recentTranscript, userProfile) {
    try {
        ...
        const perspective = generateMortalityPerspective(relevance.topic || 'general', lifeExpectancy, relevance.parentMentioned && userProfile?.parentAges
            ? {
 ...
 */
export function generateSuperhumanWisdomMoment(userProfile, currentTopic) {
    try {
 ...
 */
export function calculateConcreteRemaining(userProfile, eventType) {
    try {
        const lifeExpectancy = calculateLifeExpectancy({
            birthDate: userProfile.birthDate,
            sex: userProfile.sex,
 ...
 */
export function calculateParentVisitsRemaining(parentAge, visitFrequency) {
    try {
        ...
        const visitsPerYear = visitFrequency === 'weekly'
            ? 52
            : visitFrequency === 'monthly'
                ? 12
...
//# sourceMappingURL=mortality-perspective.js.map