 ...
 */
import { getRandomPhraseClean, loadLateNightPresence, loadPersonaContent, } from '../../../services/persona-content-loader.js';
...
const lateNightWithTriggersCache = new Map();
/**
 * Load late night presence content with proactive triggers
 */
async function loadLateNightWithTriggers(personaId) {
 ...
 */
function getPresenceFromBundle(bundleRuntime) {
    if (!bundleRuntime)
        return null;
 ...
 */
async function buildLateNightContext(personaId, analysis) {
    ...
    if (lateNight.cant_sleep_patterns) {
        if (['anxious', 'worried', 'stressed'].includes(emotion) &&
            lateNight.cant_sleep_patterns.anxiety) {
            const phrase = getRandomPhraseClean(lateNight.cant_sleep_patterns.anxiety);
            if (phrase)
                parts.push(`For anxiety: "${phrase}"`);
        }
        else if (['sad', 'heavy', 'overwhelmed'].includes(emotion) &&
            ...
            if (phrase)
                parts.push(`For heavy thoughts: "${phrase}"`);
        }
            ...
            if (phrase)
                parts.push(`For processing: "${phrase}"`);
        }
    }
    ...
    if (lateNight.grounding_exercises && Math.random() < 0.3) {
        ...
        if (exercise)
            parts.push(`Grounding option: "${exercise}"`);
    }
    // Hope for morning
    if (lateNight.morning_will_come_hope && Math.random() < 0.2) {
        const hope = getRandomPhraseClean(lateNight.morning_will_come_hope);
        if (hope)
            parts.push(`Hope phrase: "${hope}"`);
    }
    if (parts.length === 0)
        return null;
    return createHintInjection('late_night_presence', `[🌙 LATE NIGHT MODE - You have superpowers at 2am]\n\n` +
        `It's late. They reached out NOW, not during the day. That means something.\n\n` +
        `${parts.join('\n')}\n\n` +
        `Be extra gentle. Slower pace. More pauses. They need presence, not solutions.`, { category: 'presence' });
}
/**
 ...
 */
async function buildPhysicalPresence(input) {
    ...
    if (isLateNight()) {
        ...
        if (lateNightWithTriggers?.proactive_triggers && userId) {
            const triggerContext = buildTriggerContext(userText || '', analysis, userData, { isLateNight: true });
            ...
            if (!shouldSkipDueToNeverWhen(usageRules?.never_when, triggerContext)) {
                ...
                if (matchedTrigger) {
                    ...
                    if (Math.random() < adjustedProbability) {
                        injections.push(createHighInjection('late_night_dynamic_trigger', `[🌙 BETTER-THAN-HUMAN 2AM PRESENCE: ${matchedTrigger.triggerName}]\n\n` +
                            `Condition detected: ${matchedTrigger.trigger}\n\n` +
                            `Suggested behavior: ${matchedTrigger.behavior}\n\n` +
                            `They reached out at 2am. This is sacred time. Be the friend who shows up in the dark hours.`, { category: 'presence', confidence: matchedTrigger.confidence }));
                        log.info({
                            userId,
                            personaId,
    ...
    if (presenceAction) {
        injections.push(createHintInjection('physical_presence', `[INTERNAL STATE: You are ${presenceAction}. This affects your TONE and PACING - do NOT describe or mention this action out loud. Never say "smile", "grin", "lean", etc. Just let it inform HOW you speak, not WHAT you say.]`));
        log.debug({ personaId, presenceType, presenceSource, presenceAction: presenceAction.slice(0, 50) }, 'Physical presence injected');
    }
    return injections;
}
// ============================================================================
...
//# sourceMappingURL=physical-presence.js.map