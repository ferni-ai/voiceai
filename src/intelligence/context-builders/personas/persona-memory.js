...
import { getFerniMemories, getBogleMemories, getPeterMemories, getMayaMemories, getJordanMemories, getAlexMemories, } from '../../../services/memory/persona-memories.js';
function normalizePersonaId(id) {
    if (!id)
        return null;
    const lower = id.toLowerCase();
    // Ferni aliases
    if (['ferni', 'jack-b', 'jackie', 'life-coach'].includes(lower))
        return 'ferni';
    // Bogle aliases
    if (['nayan-patel', 'bogle', 'index-investor'].includes(lower))
        return 'bogle';
    // Peter aliases
    if (['peter-john', 'peter', 'stock-picker'].includes(lower))
        return 'peter';
    // Maya aliases
    if (['maya', 'spend-save', 'maya-santos', 'budgeting'].includes(lower))
        return 'maya';
    // Jordan aliases
    if (['jordan', 'event-planner', 'jordan-taylor', 'lifes-firsts'].includes(lower))
        return 'jordan';
    // Alex aliases
    if (['alex', 'comm-specialist', 'alex-chen', 'communications'].includes(lower))
        return 'alex';
    return null;
}
// ============================================================================
...
// ============================================================================
function formatFerniMemories(memories, userName) {
...
function formatBogleMemories(memories, userName) {
    if (memories.length === 0)
        return '';
    const sections = [];
    const name = userName || 'they';
    ...
    if (funds.length > 0) {
        const fundList = funds
            ...
            if (f.expenseRatio !== undefined)
                desc += ` - ${f.expenseRatio}% ER`;
    ...
    if (philosophies.length > 0) {
        sections.push(`**${name}'s investing principles:** "${philosophies.map((p) => p.name).join('"; "')}"`);
    }
    return sections.join('\n');
}
function formatPeterMemories(memories, userName) {
    if (memories.length === 0)
        return '';
    const sections = [];
    const name = userName || 'they';
    ...
    if (tenBaggers.length > 0) {
        sections.push(`**🚀 Ten-bagger candidates:** ${tenBaggers.map((t) => t.ticker || t.name).join(', ')}`);
    }
    return sections.join('\n');
}
function formatMayaMemories(memories, userName) {
    if (memories.length === 0)
        return '';
    const sections = [];
    const name = userName || 'they';
    ...
    if (triggers.length > 0) {
        sections.push(`**⚠️ Spending triggers to be gentle about:** ${triggers.map((t) => t.name).join(', ')}`);
    }
    if (merchants.length > 0) {
        const problemSpots = merchants.filter((m) => m.sentiment === 'negative');
        if (problemSpots.length > 0) {
            sections.push(`**Stores ${name} struggles with:** ${problemSpots.map((m) => m.name).join(', ')}`);
        }
    }
    if (bills.length > 0) {
        const upcoming = bills.filter((b) => b.dueDate && b.dueDate <= new Date().getDate() + 7);
        if (upcoming.length > 0) {
            sections.push(`**Bills coming up:** ${upcoming.map((b) => `${b.name}${b.amount ? ` ($${b.amount})` : ''}`).join(', ')}`);
        }
    }
    return sections.join('\n');
}
function formatJordanMemories(memories, userName) {
    if (memories.length === 0)
        return '';
    const sections = [];
    const name = userName || 'they';
    ...
    if (dates.length > 0) {
        ...
        if (upcomingDates.length > 0) {
            sections.push(`**📅 UPCOMING (mention this!):** ${upcomingDates.map((d) => `${d.name} on ${d.date}${d.person ? ` (${d.person})` : ''}`).join('; ')}`);
        }
        // All important dates
        sections.push(`**Important dates ${name} wants to remember:** ${dates.map((d) => `${d.name} - ${d.date}`).join(', ')}`);
    }
    if (venues.length > 0) {
        const favorites = venues.filter((v) => v.sentiment === 'positive');
        if (favorites.length > 0) {
            sections.push(`**Favorite venues:** ${favorites.map((v) => `${v.name}${v.rating ? ` (${v.rating}/5)` : ''}`).join(', ')}`);
        }
    }
...
function formatAlexMemories(memories, userName) {
    if (memories.length === 0)
        return '';
    const sections = [];
    const name = userName || 'they';
    ...
    if (commPrefs.length > 0) {
        sections.push(`**📱 Communication preferences:** ${commPrefs.map((p) => `${p.name}${p.preferredChannel ? ` (prefers ${p.preferredChannel})` : ''}`).join('; ')}`);
    }
    if (schedNotes.length > 0) {
        sections.push(`**📅 Scheduling notes:** ${schedNotes.map((s) => `${s.name}${s.recurring ? ' (recurring)' : ''}`).join('; ')}`);
    }
    if (contactNotes.length > 0) {
        sections.push(`**👥 People notes:** ${contactNotes.map((c) => `${c.contactName || c.name}${c.relationship ? ` (${c.relationship})` : ''} - ${c.details || c.name}`).join('; ')}`);
    }
 ...
 */
function filterMemoriesByRelationshipStage(memories, relationshipStage) {
...
async function getPersonaMemories(userId, personaId, userName, relationshipStage) {
    try {
        switch (personaId) {
            case 'ferni': {
                ...
                return {
                    personaId,
                    memories,
                    formatted: formatFerniMemories(memories, userName),
            ...
            case 'bogle': {
                ...
                return {
                    personaId,
                    memories,
                    formatted: formatBogleMemories(memories, userName),
                    count: memories.length,
                };
            }
            case 'peter': {
                ...
                return {
                    personaId,
                    memories,
                    formatted: formatPeterMemories(memories, userName),
                    count: memories.length,
                };
            }
            case 'maya': {
                ...
                return {
                    personaId,
                    memories,
                    formatted: formatMayaMemories(memories, userName),
                    count: memories.length,
                };
            }
            case 'jordan': {
                ...
                return {
                    personaId,
                    memories,
                    formatted: formatJordanMemories(memories, userName),
                    count: memories.length,
                };
            }
            case 'alex': {
                ...
                return {
                    personaId,
                    memories,
                    formatted: formatAlexMemories(memories, userName),
                    count: memories.length,
                };
            }
            default:
 ...
 */
function getProactiveMemoryCallback(result, turnCount, userName) {
    ...
    switch (result.personaId) {
        case 'ferni': {
            ...
            const wins = result.memories.filter((m) => m.type === 'win');
            const insideJokes = result.memories.filter((m) => m.type === 'inside_joke');
            const preferences = result.memories.filter((m) => m.type === 'preference');
        ...
        case 'bogle': {
            ...
            const funds = result.memories.filter((m) => m.type === 'fund');
            const philosophies = result.memories.filter((m) => m.type === 'philosophy');
        ...
        case 'peter': {
            ...
            const watchlist = result.memories.filter((m) => m.type === 'watchlist');
            const companies = result.memories.filter((m) => m.type === 'company');
            const tenBaggers = result.memories.filter((m) => m.type === 'ten_bagger');
        ...
        case 'maya': {
            ...
            const goals = result.memories.filter((m) => m.type === 'savings_goal');
            const triggers = result.memories.filter((m) => m.type === 'trigger');
            const merchants = result.memories.filter((m) => m.type === 'merchant');
            ...
            const positiveSpots = merchants.filter((m) => m.sentiment === 'positive');
            if (positiveSpots.length > 0 && Math.random() < 0.15) {
                const spot = positiveSpots[0];
        ...
        case 'jordan': {
            ...
            const dates = result.memories.filter((m) => m.type === 'date');
            const destinations = result.memories.filter((m) => m.type === 'destination');
            const venues = result.memories.filter((m) => m.type === 'venue');
            ...
            if (venues.length > 0 && Math.random() < 0.15) {
                const faves = venues.filter((v) => v.sentiment === 'positive');
                if (faves.length > 0) {
                    const venue = faves[0];
        ...
        case 'alex': {
            ...
            const commPrefs = result.memories.filter((m) => m.type === 'communication_preference');
            const schedNotes = result.memories.filter((m) => m.type === 'scheduling_note');
            // Communication preference callbacks
            if (commPrefs.length > 0 && Math.random() < 0.25) {
                const pref = commPrefs[Math.floor(Math.random() * commPrefs.length)];
            ...
            if (schedNotes.length > 0 && Math.random() < 0.2) {
                const note = schedNotes[Math.floor(Math.random() * schedNotes.length)];
 ...
 */
function getMemoryAcknowledgmentPhrases(personaId) {
    switch (personaId) {
        case 'ferni':
            return [
 ...
 */
export function getRandomAcknowledgmentPhrase(personaId) {
...
// CONTEXT BUILDER
// ============================================================================
async function buildPersonaMemoryContext(input) {
    const { services, userProfile, userData } = input;
    const injections = [];
    const turnCount = userData.turnCount || 0;
    // Get persona ID from services
    const rawPersonaId = services.personaId;
    ...
    const memoryResult = await getPersonaMemories(userProfile.id, personaId, userData.name || userProfile.name, relationshipStage);
    if (!memoryResult || memoryResult.count === 0) {
        // No memories yet - hint to build relationship
        if (turnCount > 5 && Math.random() < 0.2) {
            injections.push(createHintInjection('no_memories_hint', "[RELATIONSHIP BUILDING: You don't have specific memories stored for this user yet. Consider asking what they'd like you to remember about them.]"));
        }
        return injections;
    }
    // Inject the formatted memories as context
    if (memoryResult.formatted.length > 0) {
        injections.push(createStandardInjection('persona_memories', `[WHAT YOU REMEMBER ABOUT THIS USER - Reference these naturally!]\n${memoryResult.formatted}`));
    }
    // Add proactive callback suggestion
    const callback = getProactiveMemoryCallback(memoryResult, turnCount, userData.name);
    if (callback) {
        injections.push(createHintInjection('memory_callback_suggestion', `[MEMORY CALLBACK: ${callback}]`));
    }
...
// EXPORTS
// ============================================================================
export { buildPersonaMemoryContext, getPersonaMemories, normalizePersonaId, formatFerniMemories, formatBogleMemories, formatPeterMemories, formatMayaMemories, formatJordanMemories, filterMemoriesByRelationshipStage, getProactiveMemoryCallback, };
//# sourceMappingURL=persona-memory.js.map