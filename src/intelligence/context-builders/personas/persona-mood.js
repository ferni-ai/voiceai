 ...
 */
export function selectPersonaMood(persona, context) {
    ...
    const phrases = MOOD_PHRASES[personaId]?.[selectedMood] || MOOD_PHRASES['ferni'][selectedMood] || [];
    return {
        state: selectedMood,
        ...definition,
        moodPhrases: phrases,
 ...
 * Calculate mood probabilities based on context
 */
function calculateMoodProbabilities(context, persona) {
    const probs = new Map();
    // Base probabilities
 ...
 */
export function shouldMoodShift(currentMood, userEmotion, topicWeight) {
    // Check shift triggers
    for (const trigger of currentMood.shiftTriggers) {
        if (trigger.includes('heavy') && topicWeight === 'heavy')
            return true;
        ...
        if (trigger.includes('user') && trigger.includes('excited') && userEmotion === 'joy')
            return true;
        if (trigger.includes('serious') && topicWeight === 'heavy')
            return true;
        if (trigger.includes('vulnerability') && topicWeight === 'heavy')
            return true;
    }
    return false;
}
 ...
 */
export function getMoodShift(currentMood, reason) {
 ...
 */
export function getMoodContext(recentConversationCount = 0, lastMood) {
    const now = new Date();
...
//# sourceMappingURL=persona-mood.js.map