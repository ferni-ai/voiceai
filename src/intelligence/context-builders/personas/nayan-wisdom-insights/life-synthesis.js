...
// ============================================================================
export async function synthesizeLifeContext(userId) {
    const synthesis = {
        lifeChapter: 'unknown',
        dominantTheme: null,
    ...
    try {
        ...
        const seasonName = month >= 2 && month <= 4
            ? 'spring'
            : month >= 5 && month <= 7
                ? 'summer'
                : month >= 8 && month <= 10
        ...
        if (goalsWithDeadlines.length > 0) {
            const furthestDeadline = Math.max(...goalsWithDeadlines.map((g) => new Date(g.deadline).getTime()));
...
//# sourceMappingURL=life-synthesis.js.map