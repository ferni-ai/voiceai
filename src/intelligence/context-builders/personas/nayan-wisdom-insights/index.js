...
// MAIN BUILDER
// ============================================================================
async function buildNayanWisdomInsightsContext(input) {
    const injections = [];
    ...
    try {
        ...
        try {
            const { buildNayanWisdomContext } = await import('../../../../services/superhuman/nayan-wisdom-services.js');
        ...
        try {
            const { nayan: nayanObserver, recordConcern } = await import('../../../../services/cross-persona/observation-recorder.js');
            // Record life synthesis insights
            if (briefing.lifeSynthesis.lifeChapter) {
                nayanObserver.insight(userId, `Current life chapter: ${briefing.lifeSynthesis.lifeChapter}`, 0.75, ['life-stage', 'wisdom', 'narrative']);
            }
            ...
            if (briefing.valuesAlignment && briefing.valuesAlignment.conflictAreas.length > 0) {
                recordConcern(userId, 'nayan', `Values conflict detected: ${briefing.valuesAlignment.conflictAreas.slice(0, 2).join(', ')}`, 0.7, ['values', 'alignment', 'purpose', 'conflict']);
            }
            // Record inner peace patterns
            if (briefing.wisdomMetrics.innerPeaceIndex < 0.4) {
                recordConcern(userId, 'nayan', `Inner peace index low (${Math.round(briefing.wisdomMetrics.innerPeaceIndex * 100)}%)`, 0.65, ['peace', 'wellbeing', 'stress', 'wisdom']);
            }
            // Record existential theme if present
            if (briefing.existentialContext?.currentExistentialTheme) {
                nayanObserver.pattern(userId, `Existential theme: ${briefing.existentialContext.currentExistentialTheme}`, 0.7, ['existential', 'meaning', 'purpose']);
            }
        ...
        if (isHandoff) {
            injections.push(createHighInjection('nayan_handoff_briefing', content, {
            ...
            injections.push(createStandardInjection('nayan_initial_briefing', content, {
                category: 'persona-wisdom',
                confidence: 0.8,
            }));
            log.info({
            ...
            injections.push(createHintInjection('nayan_refresh_briefing', content, {
                category: 'persona-wisdom',
            }));
        }
        ...
        if (turnCount === 0 || isHandoff) {
            injections.push(createHintInjection('nayan_mindset', "[NAYAN'S PRESENCE: You are the still point in the turning world. " +
                'You see the whole arc of their life - past, present, possibility. ' +
                'Questions matter more than answers. Paradox is wisdom. ' +
                'Silence is a gift. Trust the timing of everything.]', { category: 'persona-identity' }));
        }
    }
    catch (error) {
...
registerContextBuilder({
    ...
    description: 'Loads Nayan with deep wisdom - life synthesis, values alignment, existential context, and the big picture',
    priority: 45,
