...
// ============================================================================
export function detectWisdomOpportunities(lifeSynthesis, teamSynthesis, wisdomMetrics) {
    const opportunities = [];
    // Growth pattern wisdom
    if (lifeSynthesis.growthPattern === 'striving') {
        opportunities.push('🌿 Pattern: Striving mode detected. "You cannot force a flower to bloom by pulling on its petals."');
    }
    else if (lifeSynthesis.growthPattern === 'resting') {
        opportunities.push('🌙 Pattern: Fallow season. The seed in darkness is not lost - it is becoming.');
    }
    ...
    if (lifeSynthesis.lifeChapter === 'freedom-seeking') {
        opportunities.push("🦅 Chapter: Seeking freedom. Ask: 'What is the freedom for, not just from?'");
    }
    else if (lifeSynthesis.lifeChapter === 'creation') {
        opportunities.push('🌱 Chapter: Creation mode. They are bringing something new into the world.');
    }
    else if (lifeSynthesis.lifeChapter === 'foundation-building') {
        opportunities.push('🏗️ Chapter: Building foundations. Solid ground takes time. Honor the patience required.');
    }
    // Values wisdom
    if (lifeSynthesis.valuesRevealed.length >= 3) {
        opportunities.push(`✨ Values emerging: ${lifeSynthesis.valuesRevealed.slice(0, 3).join(', ')}. Their life speaks.`);
    }
    // Compounding wisdom
    if (lifeSynthesis.compoundingAreas.length > 0) {
        opportunities.push('📈 Compounding detected. The eighth wonder of the world applies to habits too, not just money.');
    }
    ...
    if (wisdomMetrics.innerPeaceIndex > 70) {
        opportunities.push("🕊️ Inner peace present. This is rare. Name it. Honor it. It didn't come by accident.");
    }
...
//# sourceMappingURL=wisdom-opportunities.js.map