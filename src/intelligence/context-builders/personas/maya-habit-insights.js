...
import { checkStreaksAtRisk, checkMilestonesToCelebrate, } from '../../../services/outreach/maya-habit-outreach.js';
...
export function clearMayaInsightSession(sessionId) {
...
async function getHabitInsights(userId) {
    try {
        ...
        const completionRateThisWeek = expectedCompletions > 0 ? actualCompletions / expectedCompletions : 0;
        // Find longest streak
        const longestStreak = currentStreaks.length > 0
            ? currentStreaks.reduce((max, s) => (s.days > max.days ? s : max))
            : null;
        // Analyze by day of week
        const habitsByDayOfWeek = {
            Sunday: 0,
            Monday: 0,
            Tuesday: 0,
            Wednesday: 0,
...
// INSIGHT GENERATION
// ============================================================================
function generatePatternInsight(data) {
    // Find weakest day
...
function generateStreakProtectionInsight(data) {
...
function generatePredictiveCareInsight(data) {
...
function generateCelebrationInsight(data) {
 ...
 */
async function generateHabitAwareGreetingContext(userId) {
    try {
...
// CONTEXT BUILDER
// ============================================================================
export const mayaHabitInsightsBuilder = {
    ...
    build: async (input) => {
        ...
        if (insight) {
            ...
            injections.push(createHighInjection('maya_habit_insight', `MAYA SUPERHUMAN INSIGHT - Find a natural moment to share this observation (don't force it): "${insight}"`, { category: 'coaching' }));
        }
        // Always inject Maya's coaching context
        injections.push(createHintInjection('maya_coaching_style', `Remember Maya's superpowers: (1) Perfect memory of habit history (2) Pattern detection (3) Zero judgment (4) Infinite patience. Use "quickHabitCheck" for voice check-ins, "microCommitNow" for immediate action, "implementationIntention" for when-then planning, "weeklyHabitReview" for weekly reflection.`, { category: 'coaching' }));
        ...
        if (turnCount <= 2) {
            ...
            if (greetingContext) {
                injections.push(createStandardInjection('maya_habit_greeting_context', greetingContext, {
                    category: 'greeting',
                }));
            }
        }
        ...
        if (habitData.activeHabits > 0) {
            const streakSummary = habitData.currentStreaks.length > 0
                ? habitData.currentStreaks
                    .slice(0, 3)
                    .map((s) => `${s.name}: ${s.days}d`)
                    .join(', ')
                : 'none active';
            injections.push(createStandardInjection('maya_habit_context', `User habit status: ${habitData.activeHabits} active habits, week completion: ${Math.round(habitData.completionRateThisWeek * 100)}%, streaks: ${streakSummary}${habitData.atRiskHabits.length > 0 ? `, AT RISK: ${habitData.atRiskHabits.join(', ')}` : ''}`, { category: 'context' }));
        }
        return injections;
    },
};
...
//# sourceMappingURL=maya-habit-insights.js.map