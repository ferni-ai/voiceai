...
// USER STATE SNAPSHOT
// ============================================================================
export async function getUserStateSnapshot(userId) {
    const snapshot = {
        stressLevel: 'unknown',
        stressSignals: [],
        energyLevel: 'unknown',
    ...
    try {
        ...
        if (triggers.length > 0) {
            const stressTriggers = triggers.filter((t) => ['stressed', 'anxious', 'overwhelmed', 'tired'].includes(t.emotion?.toLowerCase() || ''));
        ...
        if (activeHabits.length > 0) {
            ...
            const avgSuccessRate = activeHabits.reduce((sum, h) => sum + (h.successRate || 0), 0) / activeHabits.length;
...
export function getTimeOfDayContext() {
    ...
    if (hour >= 6 && hour < 12)
        return 'Morning - peak energy for difficult conversations';
    if (hour >= 12 && hour < 17)
        return 'Afternoon - good for routine coordination';
    if (hour >= 17 && hour < 21)
        return 'Evening - best for reflective conversations';
    return 'Late night - emotional conversations common, handle with care';
}
// ============================================================================
// UPCOMING PRIORITIES
// ============================================================================
export function getUpcomingPriorities(userId) {
    const priorities = [];
    try {
        ...
        const enhancedHabits = userData?.enhancedHabits || [];
        const activeHabits = enhancedHabits.filter((h) => h.isActive && !h.isPaused);
        const strugglingHabits = activeHabits.filter((h) => (h.longestStreak || 0) >= 7 && (h.currentStreak || 0) <= 1);
        ...
        const milestonePriorities = activeHabits.filter((h) => h.currentStreak === 7 || h.currentStreak === 30 || h.currentStreak === 100);
    ...
    try {
        ...
        for (const reflection of reflections.slice(-5)) {
            if (reflection.challenges) {
                ...
                const commChallenges = reflection.challenges.filter((c) => c.toLowerCase().includes('conversation') ||
            ...
            if (reflection.wins) {
                ...
                const commWins = reflection.wins.filter((w) => w.toLowerCase().includes('conversation') ||
