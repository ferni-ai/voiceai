...
import { buildSuperhumanCommunicationContext, buildQuickCommunicationContext, communicationArchaeology, relationshipTemperature, unsaidWordsDetector, communicationDebt, strategicSilence, unspokenNeeds, } from '../../../../tools/domains/communication/superhuman-tools/index.js';
 ...
 */
export async function buildAlexSuperhumanContext(userId, conversationContext) {
    ...
    try {
        ...
        const hasUrgentItems = debts.some((d) => d.priority === 'urgent' || d.priority === 'high') ||
 ...
 */
export async function buildAlexQuickSuperhumanContext(userId) {
    try {
 ...
 */
export async function processTranscriptForSuperhuman(userId, transcript, context) {
    const results = {};
    try {
        ...
        if (unsaidDetection.topicMentioned) {
            ...
            // Track in session
            unsaidWordsDetector.track(userId, unsaidDetection.topicMentioned, unsaidDetection.wasDeflected, transcript.slice(0, 100));
        }
        ...
        if (commMention.detected) {
            ...
            if (commMention.contactName && commMention.contactName !== 'Unknown') {
                ...
                void relationshipTemperature.update(userId, commMention.contactName, commMention.sentiment || 0, transcript.slice(0, 100));
            }
        }
    }
    catch (error) {
 ...
 */
export async function endSuperhumanSession(userId) {
    try {
...
//# sourceMappingURL=superhuman-context.js.map