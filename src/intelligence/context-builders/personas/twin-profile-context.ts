...
async function getTwinProfile(userId: string): Promise<TwinProfile | null> {
  ...
  try {
    ...
    const profile = doc.data() as TwinProfile;
 ...
 */
function buildBackgroundContext(profile: TwinProfile): string {
  const parts: string[] = [];
 ...
 */
function buildCommunicationContext(profile: TwinProfile): string {
  const style = profile.communicationStyle;
  const parts: string[] = ['**Communication Preferences:**'];
  ...
  // Preferences
  const prefs: string[] = [];
  if (style.storytelling) prefs.push('enjoys storytelling');
  if (style.usesMetaphors) prefs.push('uses metaphors and analogies');
  if (style.askingQuestions) prefs.push('asks questions to understand');
  if (style.givingAdvice) prefs.push('comfortable giving advice');
  if (prefs.length > 0) {
 ...
 */
function buildMannerismsContext(profile: TwinProfile): string {
  const parts: string[] = ['**Voice & Mannerisms:**'];
 ...
 */
function buildValuesContext(profile: TwinProfile): string {
  const parts: string[] = [];
 ...
 */
export async function buildTwinProfileContext(userId: string): Promise<string | null> {
 ...
 */
export async function getTwinGreetingSummary(userId: string): Promise<{
  greeting?: string;
  name?: string;
  interests?: string[];
} | null> {
 ...
 */
export function clearTwinProfileCache(userId: string): void {
 ...
 */
export function clearAllTwinProfileCache(): void {
  profileCache.clear();
}
 ...
 */
export const twinProfileContextBuilder: ContextBuilder = {
