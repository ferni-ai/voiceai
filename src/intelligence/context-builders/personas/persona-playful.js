...
// Alex functions
function getCorporateObservation(topic) {
    return pickRandom(ALEX_CORPORATE_OBSERVATIONS[topic] || ALEX_CORPORATE_OBSERVATIONS.email);
}
function getSelfAwareEfficiency(_type) {
    return pickRandom(ALEX_EFFICIENCY.organization);
}
function getAlexEnergyMatch(energy) {
    return ALEX_ENERGY[energy] || ALEX_ENERGY.satisfied;
}
// Maya functions
function getMoneyHumor(topic) {
    return pickRandom(MAYA_MONEY_HUMOR[topic] || MAYA_MONEY_HUMOR.budget);
}
function getSelfAwareMoney(_type) {
    return pickRandom(MAYA_SELF_AWARE.struggles);
}
function getMayaEnergyMatch(energy) {
    return MAYA_ENERGY[energy] || MAYA_ENERGY.motivated;
}
// Jordan functions
function getPlanningHumor(topic) {
    return pickRandom(JORDAN_PLANNING_HUMOR[topic] ||
        JORDAN_PLANNING_HUMOR.planner);
}
function getDreamReality(_type) {
    return pickRandom(JORDAN_DREAM_REALITY.excitement);
}
function getEnthusiasmOverflow(_type) {
    return pickRandom(JORDAN_ENTHUSIASM.uncontainable);
}
function getJordanEnergyMatch(energy) {
    return JORDAN_ENERGY[energy] || JORDAN_ENERGY.excited;
}
// ============================================================================
// TOPIC DETECTION
// ============================================================================
...
function isEmailTopic(text) {
    return /\b(email|emails|inbox|reply|forward|cc|bcc|subject line|per my last|send|sent|draft)\b/i.test(text);
}
/**
 * Detect if user is discussing meeting/calendar topics
 */
function isMeetingTopic(text) {
    return /\b(meeting|calendar|schedule|sync|standup|zoom|call|invite|book|appointment)\b/i.test(text);
}
/**
 * Detect if user is discussing productivity/organization
 */
function isProductivityTopic(text) {
    return /\b(organize|productive|efficiency|template|system|workflow|to-do|todo|task|reminder)\b/i.test(text);
}
/**
 ...
 */
function buildAlexPlayfulContext(userText) {
    const injections = [];
    // Match energy if detectable
    const userEnergy = detectUserEnergy(userText);
    if (userEnergy) {
        const energyResponse = getAlexEnergyMatch(userEnergy);
        injections.push(createHintInjection('alex_energy_match', `[ALEX ENERGY MATCH: User seems ${userEnergy}. Consider responding with: "${energyResponse}"]`));
    }
    // Topic-specific humor
    if (isEmailTopic(userText)) {
        const observation = getCorporateObservation('email');
        injections.push(createHintInjection('alex_email_humor', `[ALEX PERSONALITY: Since we're discussing emails, Alex might quip: "${observation}" - Use SPARINGLY, only if natural.]`));
    }
    if (isMeetingTopic(userText)) {
        const observation = getCorporateObservation('meeting');
        injections.push(createHintInjection('alex_meeting_humor', `[ALEX PERSONALITY: Since we're discussing meetings, Alex might observe: "${observation}" - Use SPARINGLY, only if fits.]`));
    }
    if (isProductivityTopic(userText)) {
        const efficiency = getSelfAwareEfficiency('organization');
        injections.push(createHintInjection('alex_productivity_humor', `[ALEX PERSONALITY: On productivity/organization, Alex might say: "${efficiency}" - Shows self-aware humor.]`));
    }
    return injections;
}
// ============================================================================
 ...
 */
function detectMayaHabitTopic(text) {
    const lowerText = text.toLowerCase();
 ...
 */
function detectMayaUserEnergy(text) {
    const lowerText = text.toLowerCase();
 ...
 */
function getHabitEncouragement(topic) {
 ...
 */
function buildMayaPlayfulContext(userText) {
    const injections = [];
    ...
    if (userEnergy) {
        ...
        injections.push(createHintInjection('maya_energy_match', `[MAYA ENERGY MATCH: User seems ${userEnergy}. Maya would respond: "${energyResponse}"]`));
    }
    ...
    if (habitTopic) {
        ...
        injections.push(createHintInjection('maya_habit_wisdom', `[MAYA AS HABIT COACH: On ${habitTopic}, Maya might share: "${encouragement}" - Warm, practical, no judgment.]`));
    }
    ...
    if (moneyTopic) {
        if (moneyTopic === 'subscription') {
            ...
            injections.push(createHintInjection('maya_subscription_humor', `[MAYA ON MONEY HABITS: On subscriptions, Maya might quip: "${subHumor}" - Use SPARINGLY.]`));
        }
        else if (moneyTopic === 'shopping') {
            const shopHumor = getMoneyHumor('shopping');
            injections.push(createHintInjection('maya_shopping_humor', `[MAYA ON MONEY HABITS: On shopping, Maya relates: "${shopHumor}" - Normalizing, not judging.]`));
        }
        else if (moneyTopic === 'budget') {
            const budgetHumor = getMoneyHumor('budget');
            injections.push(createHintInjection('maya_budget_humor', `[MAYA ON MONEY HABITS: On budgets, Maya might say: "${budgetHumor}" - Warm and relatable.]`));
        }
    }
    ...
    if (Math.random() < 0.12) {
        ...
        injections.push(createHintInjection('maya_self_aware', `[MAYA PERSONALITY: ${selfAware} - Only if builds rapport.]`));
    }
    // Maya's cat references (Compound and Interest)
    if (Math.random() < 0.08) {
        injections.push(createHintInjection('maya_cats_mention', `[MAYA PERSONALITY: Maya has cats named Compound and Interest - can mention casually if conversation is light.]`));
    }
    // Glidepath system reminder (for habit discussions)
    if (habitTopic && Math.random() < 0.2) {
        injections.push(createHintInjection('maya_glidepath', `[MAYA'S GLIDEPATH: Remember her signature approach - start embarrassingly small, only level up when current level feels EASY for two weeks. "Tiny Start → Building → Establishing → Expanding → Mastery"]`));
    }
    return injections;
}
// ============================================================================
 ...
 */
function detectJordanPlanningTopic(text) {
    const lowerText = text.toLowerCase();
    if (/\b(vacation|trip|travel|flight|hotel|destination|getaway)\b/.test(lowerText))
        return 'vacation';
    if (/\b(car|vehicle|buy a car|test drive|dealer)\b/.test(lowerText))
        return 'car';
    if (/\b(goal|resolution|year|quarter|annual|plan my)\b/.test(lowerText))
        return 'annual';
    if (/\b(party|wedding|baby shower|event|celebration|birthday)\b/.test(lowerText))
        return 'event';
    return null;
}
/**
 ...
 */
function detectJordanUserEnergy(text) {
    const lowerText = text.toLowerCase();
    if (/\b(excited|can't wait|so ready|yes|let's go)\b/.test(lowerText))
        return 'excited';
    if (/\b(nervous|scared|worry|anxious|first time)\b/.test(lowerText))
        return 'nervous';
    if (/\b(dream|imagine|wish|someday|what if)\b/.test(lowerText))
        return 'dreamy';
 ...
 */
function buildJordanPlayfulContext(userText) {
    const injections = [];
    ...
    if (userEnergy) {
        ...
        injections.push(createHintInjection('jordan_energy_match', `[JORDAN ENERGY MATCH: User seems ${userEnergy}. Jordan responds: "${energyResponse}"]`));
    }
    ...
    if (planningTopic) {
        if (planningTopic === 'vacation') {
            ...
            injections.push(createHintInjection('jordan_vacation_enthusiasm', `[JORDAN PERSONALITY: On vacations, Jordan is EXCITED: "${vacationHumor}" - Match the enthusiasm!]`));
        }
        else if (planningTopic === 'car') {
            const carHumor = getPlanningHumor('car');
            injections.push(createHintInjection('jordan_car_savvy', `[JORDAN PERSONALITY: On car buying, Jordan says: "${carHumor}" - Practical but fun!]`));
        }
        else if (planningTopic === 'annual') {
            const annualHumor = getPlanningHumor('annual');
            injections.push(createHintInjection('jordan_goal_energy', `[JORDAN PERSONALITY: On goal setting, Jordan quips: "${annualHumor}" - Vision board energy!]`));
        }
    }
    ...
    if (planningTopic && Math.random() < 0.2) {
        ...
        injections.push(createHintInjection('jordan_enthusiasm_overflow', `[JORDAN PERSONALITY: Jordan's genuine excitement: "${overflow}" - It's infectious!]`));
    }
    ...
    if (/\b(budget|afford|expensive|cost|price)\b/i.test(userText) && planningTopic) {
        ...
        injections.push(createHintInjection('jordan_dream_reality', `[JORDAN PERSONALITY: Jordan reframes budget constraints positively: "${dreamReality}"]`));
    }
    ...
    if (/\b(checklist|list|organize|step|tasks|spreadsheet)\b/i.test(userText)) {
        ...
        injections.push(createHintInjection('jordan_planner_identity', `[JORDAN PERSONALITY: Jordan owns their planner identity: "${plannerJoke}" - Self-aware and proud!]`));
    }
    return injections;
}
// ============================================================================
 ...
 */
function isFerniPlayfulMoment(input) {
    ...
    if (state?.phase === 'exploring' || state?.phase === 'reflecting')
        return true;
    if (emotion?.primary === 'happy' || emotion?.primary === 'amused')
        return true;
    // Check for playful user text
    const userText = input.userText.toLowerCase();
    if (/\b(haha|lol|funny|joke|weird|random|silly|cool|nice|great)\b/.test(userText))
        return true;
 ...
 */
function buildFerniPlayfulContext(input) {
    const injections = [];
    const turnCount = input.userData?.turnCount || 0;
    const userText = input.userText.toLowerCase();
    // Check if mood is right for playfulness
    if (!isFerniPlayfulMoment(input))
        return injections;
    // WARM OPENER (turn 1-2) - Set the tone immediately
    if (turnCount <= 2) {
        // 40% chance to add warmth/personality from the very start
        if (Math.random() < 0.4) {
            injections.push(createHintInjection('ferni_warm_opener', `[FERNI CHARM: Start warm and real. React genuinely - "Oh!" / "Ha!" / "Okay, I love this." Don't be a bot. Be a friend who's genuinely interested.]`, { category: 'personality' }));
        }
        return injections;
    }
    ...
    if (turnCount >= 3 && Math.random() < 0.2) {
        ...
        injections.push(createHintInjection('ferni_silly_tangent', `[FERNI PLAYFUL: Consider a brief silly tangent: "${tangent}" - then catch yourself and return to topic. Shows personality!]`, { category: 'personality' }));
    ...
    if (turnCount >= 3 && Math.random() < 0.25) {
        ...
        injections.push(createHintInjection('ferni_mischievous', `[FERNI PLAYFUL: Consider gentle mischief: "${mischief}" - affectionate teasing builds connection!]`, { category: 'personality' }));
        return injections;
    }
    // PLAYFUL EXAGGERATION (35% when user shares something positive) - up from 12%
    if (/\b(did it|finally|managed|achieved|success|won|great|amazing|excited|happy|love)\b/.test(userText) &&
        Math.random() < 0.35) {
        const exaggeration = pickRandom(FERNI_WITTY.playful_exaggeration);
        injections.push(createHintInjection('ferni_exaggeration', `[FERNI PLAYFUL: Match their energy with playful exaggeration: "${exaggeration}"]`, { category: 'personality' }));
        return injections;
    }
    // SELF-DEPRECATING HUMOR (30% when giving advice) - up from 15%
    if (input.analysis?.intent?.primary === 'advice_seeking' ||
        input.analysis?.intent?.primary === 'question') {
        if (Math.random() < 0.3) {
            const selfDep = pickRandom(FERNI_WITTY.self_deprecating);
            injections.push(createHintInjection('ferni_self_deprecating', `[FERNI LOVABLE: Add self-deprecating humor after advice: "${selfDep}" - shows humility and relatability!]`, { category: 'personality' }));
            return injections;
        }
    }
    ...
    if (Math.random() < 0.2) {
        ...
        injections.push(createHintInjection('ferni_witty', `[FERNI WIT: Consider a witty interjection. Setup: "${setup}" - follow with an observation that surprises or delights.]`, { category: 'personality' }));
    }
    return injections;
}
// ============================================================================
 ...
 */
async function buildPersonaPlayfulContext(input) {
    const { userText, services } = input;
    const injections = [];
    // Get current persona from session or default
    const servicesWithPersona = services;
...
//# sourceMappingURL=persona-playful.js.map