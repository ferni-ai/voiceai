...
// CONTEXT BUILDER
// ============================================================================
async function buildCommunityLearningContext(input) {
    ...
    try {
        ...
        if (bestStrategy && bestStrategy.confidence > 0.6) {
            injections.push(createHintInjection('community_strategy', `[COMMUNITY LEARNING: "${bestStrategy.strategy}" responses work well here ` +
        ...
        if (effectiveQuestions.length > 0 && Math.random() < 0.3) {
            ...
            injections.push(createHintInjection('community_question', `[BREAKTHROUGH QUESTION: "${question.question}" has ` +
        ...
        if (turnCount > 5 && Math.random() < 0.2) {
            // 20% chance
            const recommendedStories = evolution.getRecommendedStories(personaId, { topic, relationshipStage, userEmotion }, 2);
            if (recommendedStories.length > 0) {
                const storyRec = recommendedStories[0];
                injections.push(createHintInjection('story_recommendation', `[STORY SUGGESTION: "${storyRec.storyId}" scores ${(storyRec.score * 100).toFixed(0)}% ` +
                    `effectiveness here - ${storyRec.reason}. Consider if a story fits.]`));
            }
        }
        if (injections.length > 0) {
            getLogger().debug({ personaId, injectionCount: injections.length, topic, emotion: userEmotion }, 'Community learning context injected');
        }
    }
    catch (error) {
...
export function recordStoryUsage(params) {
    try {
        const insights = getCommunityInsights();
        insights.recordStoryUsage(params.storyId, params.personaId, {
            topic: params.topic,
            relationshipStage: params.relationshipStage,
            userEmotion: params.userEmotion,
        }, params.reaction, params.engagementScore);
    }
    catch (error) {
...
export function recordBreakthroughQuestion(params) {
    try {
        const insights = getCommunityInsights();
        insights.recordBreakthroughQuestion(params.questionPattern, params.personaId, params.topic, params.context, params.engagementLift);
    }
    catch (error) {
...
//# sourceMappingURL=community-learning.js.map