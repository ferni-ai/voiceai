...
import { getWisdomContextInjection, getPopulationInsights, } from '../../../services/wisdom-synthesis/index.js';
...
function detectSituation(input) {
    ...
    if (emotion.includes('anxious') ||
        ...
        if (userTextLower.includes('panic') ||
    ...
    if (emotion.includes('sad') ||
        ...
        if (userTextLower.includes('stuck') ||
    ...
    if (topics.some((t) => t.includes('relationship') || t.includes('partner') || t.includes('family')) ||
        ...
        if (userTextLower.includes('always') ||
    ...
    if (userTextLower.includes('stupid') ||
    ...
    if (userTextLower.includes('decide') ||
    ...
    build: async (input) => {
        ...
        try {
            ...
            if (wisdomContext && wisdomContext.length > 0) {
                injections.push(createStandardInjection('wisdom-synthesis', wisdomContext, {
                    category: 'wisdom',
                    confidence: 0.8,
                }));
            ...
            if (insights.topApproaches.length > 0) {
                ...
                if (topApproach) {
                    injections.push(createHintInjection('wisdom-synthesis', `[Population insight: "${topApproach.technique}" works for ${Math.round(topApproach.helpfulRate * 100)}% of people in similar situations]`, { category: 'wisdom-meta', confidence: 0.7 }));
                }
            }
        }
        catch (error) {
...
//# sourceMappingURL=wisdom-synthesis.js.map