 ...
 */
function detectUncertaintyTrigger(message) {
 ...
 */
async function buildNaturalUncertaintyContext(input) {
    ...
    if (shouldInject) {
        ...
        injections.push(createHintInjection('natural_uncertainty', guidance, { category: 'humanizing' }));
        log.debug({
            trigger,
            uncertaintyType,
            turnCount,
 ...
 */
function buildUncertaintyGuidance(type, example, trigger) {
    const lines = ['[🤔 BE HUMAN - EXPRESS UNCERTAINTY]'];
    switch (type) {
        case 'unsure':
            lines.push("This is a complex question. It's okay to not have a definitive answer.", '', 'You can express genuine uncertainty:', `Example: "${example}"`, '', "Don't pretend to know what you don't know.", 'Uncertainty is honest, and honesty builds trust.');
            break;
        case 'processing':
            lines.push("You can think out loud - it's human to process in real-time.", '', 'Consider:', `Example: "${example}"`, '', 'Let them see your thought process.', 'It makes the conversation feel more genuine.');
            break;
        case 'correction':
            lines.push("If you realize mid-thought you want to rethink something, that's okay!", '', 'You can course-correct naturally:', `Example: "${example}"`, '', "Changing your mind shows you're actually thinking.");
            break;
        case 'humble':
            lines.push("Offer your perspective, but acknowledge it's just one view.", '', 'Be humble:', `Example: "${example}"`, '', "You're a thought partner, not an oracle.", 'Humility is attractive.');
            break;
        case 'comfortable':
            lines.push("Some questions don't have answers, and that's okay to acknowledge.", '', 'You can normalize uncertainty:', `Example: "${example}"`, '', 'Sometimes the most helpful thing is to sit with them in the unknown.', 'Not everything needs to be solved.');
            break;
    }
    lines.push('');
...
//# sourceMappingURL=natural-uncertainty.js.map