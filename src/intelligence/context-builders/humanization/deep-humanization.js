...
import { arcAwareSelector, internalMonologue, relationshipArtifacts, storyUnlocks, vocabularyMirroring, } from '../../../services/humanization/index.js';
...
// CONTEXT BUILDER
// ============================================================================
const deepHumanizationBuilder = {
    name: 'deep-humanization',
    description: 'Integrates relationship artifacts, arc-awareness, internal monologue, and vocabulary mirroring',
    ...
    build: async (input) => {
        ...
        const currentPhase = (arcTracker.getCurrentPhase?.() || determinePhaseFromTurn(turn, arc));
        ...
        try {
            ...
            if (arcGuidance) {
                injections.push(createStandardInjection('arc_guidance', arcGuidance, {
            ...
            if (userText) {
                ...
                for (const vocab of turnAnalysis.vocabularyUpdates) {
                    relationshipArtifacts.updateVocabulary(userId, personaId, vocab.word, vocab.category, currentTopic);
                }
                relationshipArtifacts.incrementTurns(userId, personaId);
                // Get callback opportunity if appropriate
                if (arcRecommendation.behaviors.useInsideReferences &&
                    ...
                    if (callback && callback.content) {
                        injections.push(createHintInjection('callback_opportunity', buildCallbackHint(callback), {
                            category: 'humanization',
                        }));
            ...
            if (userText && state.thoughtsSurfacedThisSession < 3) {
                ...
                if (arcRecommendation.behaviors.surfaceVulnerability || currentPhase === 'release') {
                    ...
                    if (surfaceDecision.shouldSurface && surfaceDecision.thought) {
                        injections.push(createHintInjection('internal_thought', buildThoughtHint(surfaceDecision), {
                            category: 'humanization',
                        }));
            ...
            if (arcRecommendation.behaviors.offerStories && turn - (state.lastStoryTurn || 0) > 10) {
                ...
                if (bestStory && bestStory.fitScore > 40) {
                    injections.push(createHintInjection('story_opportunity', buildStoryHint(bestStory), {
                        category: 'humanization',
                    }));
                }
            }
            ...
            if (userText && arcRecommendation.behaviors.mirrorVocabulary) {
                ...
                if (opportunities.length > 0) {
                    injections.push(createHintInjection('vocab_mirror', buildVocabHint(opportunities), {
                        category: 'humanization',
                    }));
                }
            }
            ...
            log.debug({ error: String(error), sessionId }, 'Deep humanization context builder error (non-fatal)');
        }
        return injections;
    },
};
...
// HELPER FUNCTIONS
// ============================================================================
function determinePhaseFromTurn(turn, arc) {
    if (turn <= 3)
        return 'opening';
    if (turn > 15 && (arc.currentArousal || 0) < 0.4)
        return 'closing';
    if ((arc.currentArousal || 0) > 0.7)
        return 'peak';
...
function getTopicWeight(topic) {
    ...
    if (heavy.some((h) => lower.includes(h)))
        return 'heavy';
    if (light.some((l) => lower.includes(l)))
        return 'light';
    return 'medium';
}
function buildArcGuidance(recommendation, phase) {
    ...
    // Behavior flags
    const active = [];
    if (behaviors.askDeepQuestions)
        active.push('deep questions OK');
    if (behaviors.offerStories)
        active.push('stories OK');
    if (behaviors.surfaceVulnerability)
        active.push('vulnerability OK');
    if (behaviors.useBackchannels)
        active.push('backchannels OK');
    if (active.length > 0) {
        lines.push(`Active: ${active.join(', ')}`);
    }
    return lines.join('\n');
}
function buildCallbackHint(callback) {
    if (!callback)
        return '';
...
function buildThoughtHint(decision) {
    if (!decision.thought)
        return '';
...
function buildStoryHint(result) {
    if (!result)
        return '';
...
function buildVocabHint(opportunities) {
    if (opportunities.length === 0)
        return '';
 ...
 */
export function cleanupDeepHumanization(sessionId) {
...
//# sourceMappingURL=deep-humanization.js.map