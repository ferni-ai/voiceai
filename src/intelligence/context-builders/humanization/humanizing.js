...
import { analyzeVoiceEmotionIntelligence, formatVoiceIntelligenceForPrompt, } from '../emotional/voice-emotion-intelligence.js';
import { findInnerWorldInjections, formatInnerWorldForPrompt, shouldInject, } from '../intelligence/inner-world-injector.js';
import { selectSpontaneousShare, formatSpontaneousShareForPrompt, getShareTags, } from '../personas/spontaneous-vulnerability.js';
import { selectPersonaMood, formatMoodForPrompt, getMoodContext, shouldMoodShift, getMoodShift, } from '../personas/persona-mood.js';
import { getRelationshipBehaviors, calculateRelationshipStage, getRelationshipTransitionAnnouncement, mapUserProfileStageToHumanizing, mapHumanizingStageToUserProfile, getRelationshipStageFromProfile, } from '../relationship/relationship-behaviors.js';
// ============================================================================
// MAIN BUILDER
// ============================================================================
/**
 ...
 */
export function buildHumanizingContext(ctx) {
    const injections = [];
    const usedTags = ctx.usedShareTags || [];
    const summaryParts = [];
    ...
    if (ctx.userProfileRelationshipStage) {
        ...
        relationshipStage = calculateRelationshipStage(ctx.totalTurns || ctx.turnCount, ctx.sessionCount, ctx.sharedVulnerabilities, ctx.celebratedTogether, ctx.difficultConversations);
    }
    ...
    if (ctx.previousRelationshipStage && ctx.previousRelationshipStage !== relationshipStage) {
        ...
        const announcement = getRelationshipTransitionAnnouncement(ctx.previousRelationshipStage, relationshipStage, ctx.userName);
        if (announcement) {
...
This should feel organic, not announced.`,
    ...
    let voiceIntelligence;
    if (ctx.voiceEmotion && ctx.voiceEmotion.confidence > 0.4) {
        voiceIntelligence = analyzeVoiceEmotionIntelligence(ctx.voiceEmotion, ctx.textEmotion || null, ctx.turnCount);
    ...
    const shareContext = {
        ...
        conversationFlow: ctx.isVulnerableMoment ? 'deep' : 'flowing',
 ...
 */
export function formatHumanizingForPrompt(result) {
 ...
 */
export function getHumanizingSummary(result) {
...
//# sourceMappingURL=humanizing.js.map