...
const CONFIG = {
    ...
    allowedMoods: ['positive', 'neutral'],
    // Only for established/deep relationships
    allowedRelationshipStages: ['established', 'deep'],
 ...
 */
function hasNaturalTrigger(userText, topics) {
 ...
 */
export async function buildReferralPromptInjection(ctx) {
    ...
    if (!CONFIG.allowedRelationshipStages.includes(relationshipStage)) {
        return { shouldInject: false, reason: `Relationship stage: ${relationshipStage}` };
    }
    // Must be in positive/neutral mood
    if (userMood && !CONFIG.allowedMoods.includes(userMood)) {
 ...
 */
export function buildReferralConversationContext() {
