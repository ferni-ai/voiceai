 ...
 */
import { getDailyRitualsService, PERSONA_RITUALS, RITUAL_PROMPTS, } from '../../../services/daily-rituals.js';
...
function getPersonaRitualId(personaId) {
    const ritualMap = {
...
// DAILY RITUALS CONTEXT BUILDER
// ============================================================================
export const dailyRitualsBuilder = {
    name: 'daily-rituals',
    ...
    build: async (input) => {
        ...
        if (userId) {
            ...
            if ('openings' in prompts) {
                ...
                const wisdomDrops = prompts.wisdomDrops;
...
- Don't rush to fill the space`;
            ...
            injections.push(createStandardInjection('daily_ritual_opportunity', content, {
                category: 'engagement',
            }));
            log.debug({ personaId, ritualId, currentStreak, timeOfDay }, 'Daily ritual opportunity injected');
        }
        return injections;
    },
};
