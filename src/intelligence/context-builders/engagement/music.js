...
// MUSIC PATTERNS
// ============================================================================
const STOP_MUSIC_PATTERNS = /\b(stop|quit|enough|turn it off|no more music|that's enough|silence|shut up|quiet)\b/i;
// ============================================================================
// MUSIC CONTEXT BUILDER
// ============================================================================
/**
 ...
 */
async function buildMusicContext(input) {
    ...
    try {
        ...
        if (musicState.isPlaying && musicState.currentTrack) {
            ...
            const explicitMusicStop = /\b(stop|turn off|pause|no more)\s+(the\s+)?music\b/i.test(userText) ||
                /\bmusic.*(stop|off|quiet|pause)\b/i.test(userText);
            if (explicitMusicStop) {
                injections.push(createStandardInjection('music_stop_requested', `[USER WANTS MUSIC STOPPED: The user explicitly asked to stop the music.
...
Then smoothly transition to a new topic.]`));
                ...
                injections.push(createStandardInjection('music_playing', `[MUSIC PLAYING: "${track.name}" by ${track.artist} is playing in the background. 
