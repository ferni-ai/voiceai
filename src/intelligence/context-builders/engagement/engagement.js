 ...
 */
function getConversationDepth(turnCount, topicsDiscussed, emotionalMomentCount) {
...
function buildEngagementContext(input) {
    ...
    // -----------------------------------------------
    const shouldShowCuriosity = turnCount > 2 &&
    ...
    if (shouldShowCuriosity) {
        ...
        if (topics.length > 0) {
            injections.push(createStandardInjection('curiosity', `[CURIOSITY MOMENT - Keep the conversation moving!]
...
BETTER THAN HUMAN: Your best friend might let this topic slide. You don't.`));
            ...
            injections.push(createHintInjection('curiosity_emotional', `[CURIOSITY: They seem to have something on their mind. Ask them to share more. Don't let this fizzle.]`));
        }
    }
    // -----------------------------------------------
    // CONVERSATION DEPTH AWARENESS
    ...
    if (depth === 'deep') {
        injections.push(createHintInjection('depth_deep', `[DEPTH: This is a DEEP conversation. You've built trust. You can be more direct, personal, and vulnerable. Share wisdom from your heart.]`));
    }
    else if (depth === 'surface' && turnCount > 3) {
        injections.push(createHintInjection('depth_surface', `[DEPTH: Still at SURFACE level. Focus on building connection before diving into advice. Ask about THEM.]`));
    }
    // -----------------------------------------------
    // USER ENGAGEMENT DETECTION
    // -----------------------------------------------
    ...
    if (engagement.level === 'checked_out' || engagement.level === 'disengaged') {
        injections.push(createStandardInjection('engagement_low', `[ENGAGEMENT: User seems ${engagement.level}. ${engagement.suggestions.join(' ')}`));
    }
    else if (engagement.level === 'highly_engaged') {
        injections.push(createHintInjection('engagement_high', `[ENGAGEMENT: User is highly engaged! They're invested in this conversation. Match their energy.]`));
    }
    // -----------------------------------------------
    // RUNNING JOKES WITH RETURNING USERS
    ...
    if (userProfile && userProfile.totalConversations >= 2) {
        ...
        if (joke) {
            ...
            injections.push(createHintInjection('running_joke', `[RUNNING JOKE (${jokeType}): Consider weaving this in naturally: "${joke.joke}"]`));
        }
    }
    return injections;
}
