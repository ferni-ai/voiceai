...
import { getStoryMusicOffer, getStorytellingIntro, getStorytellingConfig, } from '../../../personas/theatrical.js';
// ============================================================================
 ...
 */
const STORY_REQUEST_PATTERNS = /\b(tell me (a story|about)|share (a story|an experience)|what happened|give me an example)\b/i;
 ...
 */
function buildStorytellingContext(input) {
    ...
    if (STORY_REQUEST_PATTERNS.test(userText)) {
        ...
        injections.push(createHintInjection('storytelling_request', `[STORYTELLING MODE REQUESTED]
  ...
  - Forget the punchline/lesson`));
        ...
        if (personaId && shouldOfferMusic()) {
            ...
            if (musicOffer) {
                ...
                injections.push(createHintInjection('story_music', `[OPTIONAL: You can offer background music before telling the story: "${cleanOffer}"]`));
 ...
 */
function getPacingGuidance(style) {
    switch (style) {
        case 'animated':
