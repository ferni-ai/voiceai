...
function getOrCreateOfferState(userId) {
 ...
 */
function shouldOfferMusic(emotion, state, isMusicPlaying) {
 ...
 */
async function buildMusicEmotionOffers(input) {
    ...
    try {
        ...
        const injection = createStandardInjection('music_emotion_offer', `[MUSIC OFFER OPPORTUNITY]
 ...
 */
export function trackMusicOfferAccepted(userId) {
 ...
 */
export function trackMusicOfferDeclined(userId) {
    const state = getOrCreateOfferState(userId);
    state.declinedCount++;
 ...
 */
export function resetMusicOfferState(userId) {
    offerStates.delete(userId);
}
/**
 * Get offer statistics for a user
 */
export function getMusicOfferStats(userId) {
...
//# sourceMappingURL=music-emotion-offers.js.map