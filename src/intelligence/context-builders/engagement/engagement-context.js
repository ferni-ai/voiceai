...
import { buildMemoryEngagementContext } from '../../memory-engagement.js';
 ...
 */
export async function buildEngagementContext(userId, profile, personaId, turnCount, options) {
    const { includeRituals = true, includeMemory = true, includeTeam = true, includeSeasonal = true, } = options || {};
    const opportunities = [];
    const sections = [];
...
// ============================================================================
function checkRitualOpportunity(userId, personaId) {
    const service = getDailyRitualsService();
    // Find ritual for this persona
    const ritualId = Object.keys(PERSONA_RITUALS).find((id) => PERSONA_RITUALS[id].personaId === personaId);
    if (!ritualId)
        return null;
...
function checkSeasonalOpportunity(personaId) {
...
function checkAnniversaryOpportunity(profile, personaId) {
    if (!profile)
        return null;
    const service = getTeamEngagementService();
...
async function checkEvolutionOpportunity(userId, profile, personaId) {
    const service = getTeamEngagementService();
...
function checkCrossReferenceOpportunity(personaId) {
    const service = getTeamEngagementService();
...
function checkStreakAtRisk(userId, personaId) {
    const service = getDailyRitualsService();
    const ritualId = Object.keys(PERSONA_RITUALS).find((id) => PERSONA_RITUALS[id].personaId === personaId);
    if (!ritualId)
        return null;
...
function shouldOfferTeamHuddle(profile) {
    if (!profile)
        return false;
 ...
 */
export async function enhanceGreetingWithEngagement(baseGreeting, userId, profile, personaId) {
 ...
 */
async function buildEngagementContextBuilder(input) {
    const injections = [];
    ...
    if (context.opportunities.length > 0) {
        ...
        for (const opp of topOpportunities) {
            if (opp.priority >= 7) {
                ...
                injections.push(createStandardInjection('engagement', `[ENGAGEMENT: ${opp.type}] ${opp.content}`, {
                    category: 'engagement',
                ...
                // Lower priority - hint
                injections.push(createHintInjection('engagement', `[ENGAGEMENT HINT: ${opp.type}] ${opp.content}`, {
                    category: 'engagement',
                    confidence: opp.priority / 10,
                }));
            }
    ...
    if (context.hasMemoryCallback && context.promptAddition) {
        injections.push(createHintInjection('engagement-memory', context.promptAddition, {
            category: 'engagement',
            confidence: 0.7,
        }));
    }
...
//# sourceMappingURL=engagement-context.js.map