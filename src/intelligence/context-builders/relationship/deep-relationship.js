 ...
 */
import { createStandardInjection, createHighInjection, createHintInjection, registerContextBuilder, } from '../index.js';
...
const relationshipCache = new EdgeCache({
    maxSize: 100,
...
// DATA LOADING
// ============================================================================
async function getRelationshipHistory(userId) {
    ...
    try {
        ...
        const sharedMoments = jokesSnapshot.docs.map((doc) => doc.data());
        const runningJokes = sharedMoments
            .filter((m) => m.type === 'running_gag')
            .map((m) => ({
            id: m.id,
            content: m.content,
            origin: m.origin?.whatTheySaid || '',
            timesReferenced: m.callbackCount || 0,
            lastReferenced: m.lastCallback?.toDate?.() || undefined,
            reception: m.callbackReception || 'neutral',
            triggers: m.triggers || [],
        }));
        const sharedVocabulary = sharedMoments
            .filter((m) => m.type === 'phrase')
            .map((m) => ({
            phrase: m.content,
            origin: m.origin?.whatTheySaid || '',
            usageCount: m.callbackCount || 0,
            lastUsed: m.lastCallback?.toDate?.() || new Date(),
            meaning: m.meaning,
        }));
        const result = {
            userId,
            firstConversation: userData?.createdAt?.toDate?.() || undefined,
            totalConversations: userData?.totalConversations || userData?.turnCount || 0,
            currentStreak: userData?.currentStreak || 0,
            longestStreak: userData?.longestStreak || 0,
            sharedVocabulary,
            runningJokes,
            milestonesCelebrated: userData?.milestonesCelebrated || [],
        };
...
// ============================================================================
function findCallbackOpportunity(userText, history) {
    const textLower = userText.toLowerCase();
    // Check running jokes
    for (const joke of history.runningJokes || []) {
        if (joke.reception === 'negative')
            continue;
        for (const trigger of joke.triggers) {
            if (textLower.includes(trigger.toLowerCase())) {
                ...
                if (joke.lastReferenced) {
                    ...
                    if (daysSince < 7)
                        continue; // At least a week between callbacks
                }
                return joke;
            }
        }
...
// BUILDER
// ============================================================================
async function buildDeepRelationshipContext(input) {
    ...
    for (const milestone of milestones) {
        injections.push(createHighInjection(`milestone_${milestone.type}`, `[RELATIONSHIP MILESTONE] This is conversation #${history.totalConversations}. ${milestone.celebration} Consider acknowledging this moment naturally. Don't force it, but it's worth marking.`, { category: 'celebration' }));
        // Mark as celebrated (async, don't await)
        void markMilestoneCelebrated(userId, `${milestone.type}_${milestone.value}`);
        log.info({ userId, milestone: milestone.type, value: milestone.value }, '🎉 BETTER-THAN-HUMAN: Relationship milestone detected');
    }
    ...
    if (callback) {
        ...
        if (isJoke) {
            injections.push(createStandardInjection('running_joke_callback', `[SHARED HISTORY] This reminds me of an inside joke we have: "${callback.content}". Consider a natural callback if it fits. Don't force it. Origin: "${callback.origin}"`, { category: 'trust' }));
        }
        else {
            injections.push(createHintInjection('shared_vocabulary', `[SHARED VOCABULARY] They used a phrase we share: "${callback.phrase}". This is "our" word. You might acknowledge the shared meaning.`, { category: 'trust' }));
        }
        log.info({ userId, callbackType: isJoke ? 'joke' : 'vocabulary' }, '💬 BETTER-THAN-HUMAN: Shared history callback opportunity');
    }
    ...
    if (history.totalConversations && history.totalConversations > 5) {
        ...
        if (context) {
            injections.push(createHintInjection('relationship_depth', context, { category: 'awareness' }));
        }
    }
    ...
    if (turnCount === 0 && history.totalConversations && history.totalConversations > 1) {
        const daysSinceFirst = history.firstConversation
            ...
            : 0;
        if (daysSinceFirst > 0) {
            injections.push(createHintInjection('relationship_timeline', `[RELATIONSHIP TIMELINE] You've known them for ${daysSinceFirst} days. This is conversation #${history.totalConversations}. You have real history together.`, { category: 'awareness' }));
        }
    }
    return injections;
}
...
// HELPERS
// ============================================================================
function buildRelationshipContextSummary(history) {
    const parts = [];
...
async function markMilestoneCelebrated(userId, milestoneKey) {
    try {
        const db = await getFirestoreDb();
        if (!db)
            return;
 ...
 */
async function recordSharedMomentInternal(userId, moment) {
    try {
        const db = await getFirestoreDb();
        if (!db)
            return;
        ...
        log.info({ userId, type: moment.type, content: moment.content.slice(0, 50) }, '💫 Shared moment recorded');
    }
    catch (err) {
...
//# sourceMappingURL=deep-relationship.js.map