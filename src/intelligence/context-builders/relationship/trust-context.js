...
import { 
// Core trust context
buildTrustContext, formatGuidanceForLLM, 
// Phase 27: Learning Style
formatLearningGuidanceForLLM, generateCelebrations, generateSeasonalContextForLLM, 
// Phase 15: Response Tuning
generateTuningGuidance, generateVoiceContext, getEventsNeedingReminders, 
// Phase 24: Voice Prosody
getFamiliarityScore, 
// Phase 12: Relationship Health
getHealthScore, getLearningProfile, 
// Phase 16: Celebration Momentum
getMomentumSummary, getStageName, 
// Phase 14: Life Events
getUpcomingEvents, 
// Trust Signal Emitter (Frontend UI Bridge)
processContextForSignals, } from '../../../services/trust-systems/index.js';
import { createLogger } from '../../../utils/safe-logger.js';
import { checkDynamicTriggers, calculateProbabilityBoost, shouldSkipDueToNeverWhen, buildTriggerContext, } from '../dynamic-trigger-utils.js';
 ...
 */
async function buildTrustAwareContext(input) {
    ...
    // ============================================================================
    const triggerContext = buildTriggerContext(userText, analysis, userData);
    const proactiveTriggers = cachedTrustPhrases
        ?.proactive_triggers;
    const usageRules = cachedTrustPhrases?.usage_rules;
    ...
    if (!shouldSkipDueToNeverWhen(usageRules?.never_when, triggerContext)) {
        ...
        if (matchedTrigger) {
            ...
            const probabilityBoost = calculateProbabilityBoost(usageRules?.more_likely_when, triggerContext, matchedTrigger);
            ...
            if (Math.random() < adjustedProbability) {
                injections.push(createHighInjection('trust_dynamic_trigger', `[🎯 BETTER-THAN-HUMAN TRIGGER: ${matchedTrigger.triggerName}]\n\n` +
                    ...
                    `Apply this guidance naturally in your voice.`, { category: 'trust', confidence: matchedTrigger.confidence }));
                log.info({
                    userId,
    ...
    if (trustContext.topicsToAvoid.length > 0) {
        injections.push(createHintInjection('boundary_awareness', formatBoundaryWarnings(trustContext.topicsToAvoid), { category: 'trust' }));
    }
    ...
    const { isGoodMomentForGrowth, generateEarlyGrowthReflection, generateGrowthReflection } = await import('../../../services/trust-systems/index.js');
    ...
    if (trustContext.growthReflection) {
        const growthInjection = formatGrowthReflection(trustContext.growthReflection, growthMoment.reason === 'milestone_turn');
        if (growthInjection) {
            injections.push(growthInjection);
            log.info({ userId, turnCount, reason: growthMoment.reason }, '🌱 BETTER-THAN-HUMAN: Surfacing growth reflection');
        }
    }
        ...
        if (reflection) {
            const isMilestone = growthMoment.reason === 'milestone_turn' || growthMoment.reason === 'time_milestone';
    ...
    if (healthScore) {
        ...
        if (healthContext) {
            injections.push(createHintInjection('relationship_health', healthContext, { category: 'trust' }));
        }
    }
    // ============================================================================
    // PHASE 14: LIFE EVENTS
    ...
    if (eventsNeedingReminders.length > 0) {
        ...
        if (reminderContext) {
            injections.push(createHintInjection('event_reminders', reminderContext, { category: 'trust' }));
        }
    }
    // ============================================================================
    // PHASE 16: CELEBRATION MOMENTUM
    ...
    if (momentumSummary) {
        injections.push(createHintInjection('celebration_momentum', `[🔥 MOMENTUM]\n${momentumSummary}`, {
            category: 'trust',
        }));
    }
    ...
    if (celebrations.length > 0) {
        const celebrationText = celebrations
            ...
            .map((c) => `• ${c.message}`)
            .join('\n');
        injections.push(createHintInjection('momentum_celebration', `[🎉 MOMENTUM CELEBRATION]\n${celebrationText}\n\nShare naturally when appropriate.`, { category: 'trust' }));
    }
    // ============================================================================
    // PHASE 24: VOICE PROSODY
    // ============================================================================
    ...
    if (familiarityResult.score > 0) {
        ...
        if (familiarityContext) {
            injections.push(createHintInjection('voice_familiarity', familiarityContext, { category: 'trust' }));
        }
    }
    // ============================================================================
    ...
    if (seasonalContext) {
        injections.push(createHintInjection('seasonal_awareness', seasonalContext, { category: 'trust' }));
    }
    // ============================================================================
    // PHASE 27: LEARNING STYLE
    // ============================================================================
 ...
 */
function buildResponseTuningContext(userId, analysis, userData) {
    const healthScore = getHealthScore(userId);
    // Determine relationship stage from health score
    let relationshipStage = 'building';
    if (healthScore) {
        relationshipStage = healthScore.stage;
    }
    // Detect vulnerable share from emotion signals
    const isVulnerableShare = Boolean(analysis?.emotion?.needsSupport ||
        analysis?.emotion?.isProcessing ||
        (analysis?.emotion?.intensity && analysis.emotion.intensity > 0.7));
    // Detect crisis from emotion signals
    const isCrisis = Boolean(analysis?.emotion?.needsSupport &&
        analysis?.emotion?.intensity &&
        analysis.emotion.intensity > 0.9);
    // Detect advice-seeking from intent
    const isAskingForAdvice = Boolean(analysis?.intent?.primary === 'advice' || analysis?.intent?.requiresAction);
    // Use userData to enhance context with user preferences and history
    const recentTopics = userData?.recentTopics || [];
    const hasRecentVulnerableShare = recentTopics.some((topic) => topic.includes('personal') || topic.includes('emotional'));
 ...
 */
function formatRelationshipHealth(health) {
 ...
 */
function formatLifeEventsContext(events) {
...
// ============================================================================
import { getRandomPhraseClean, loadTrustPhrases, } from '../../../services/persona-content-loader.js';
/**
 * Cached trust phrases per persona
 * Each persona has their own voice for trust phrases
 */
const trustPhrasesCache = new Map();
let currentPersonaId = null;
let cachedTrustPhrases = null;
/**
 ...
 */
async function ensureTrustPhrasesLoaded(personaId = 'ferni') {
 ...
 */
function getThinkingOfYouPhrase(context) {
    ...
    switch (context) {
        ...
        case 'harddate':
            return getRandomPhraseClean(cachedTrustPhrases.thinking_of_you_proactive.anticipating_hard_date);
        default:
            return null;
    }
}
 ...
 */
function getCallbackPhrase(context) {
    if (!cachedTrustPhrases?.inside_jokes_callbacks) {
        return null;
    }
    switch (context) {
        case 'shared_moment':
            return getRandomPhraseClean(cachedTrustPhrases.inside_jokes_callbacks.referencing_shared_moment);
        case 'continuity':
 ...
 */
function formatUnsaidSignals(signals) {
    if (signals.length === 0)
        return null;
    ...
    lines.push('');
    lines.push(hasEmotionalMismatch
 ...
 */
function formatBoundaryWarnings(topics) {
    const lines = [
        '[🚫 BOUNDARY AWARENESS]',
 ...
 */
function formatGrowthReflection(reflection, isMilestone = false) {
    const lines = [
        isMilestone
 ...
 */
function formatCallbackOpportunity(opportunity) {
 ...
 */
function formatCelebrationOpportunity(opportunity) {
 ...
 */
function formatEventsNeedingReminders(events) {
    if (events.length === 0)
        return null;
    const lines = ['[⏰ EVENTS NEEDING FOLLOW-UP]'];
    for (const event of events.slice(0, 3)) {
        const daysAgo = Math.floor((Date.now() - new Date(event.date).getTime()) / (24 * 60 * 60 * 1000));
 ...
 */
function formatFamiliarityContext(score) {
    if (score < 0.3)
        return null; // Not familiar enough to mention
    const lines = ['[🤝 VOICE FAMILIARITY]'];
    if (score > 0.8) {
...
//# sourceMappingURL=trust-context.js.map