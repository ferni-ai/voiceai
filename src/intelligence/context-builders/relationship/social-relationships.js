...
import { recordMention, extractNames, generateSocialInsights, generateSuperhumanMoment, getImportantPeople, } from '../../../services/social-graph/index.js';
 ...
 */
export const socialRelationshipsBuilder = {
    ...
    build: async (input) => {
        ...
        for (const { name, context } of extractedNames) {
            ...
            if (mentionedThisSession.has(name.toLowerCase()))
                continue;
            mentionedThisSession.add(name.toLowerCase());
            // Estimate sentiment from conversation analysis
            const sentiment = analysis.emotion.primary === 'joy'
                ? 0.5
                : analysis.emotion.primary === 'sadness'
                    ...
                    : analysis.emotion.primary === 'anger'
                        ...
                        : analysis.emotion.primary === 'anxiety'
                            ...
                            : 0;
            // Record the mention
            recordMention(userId, name, context, sentiment, analysis.topics.detected, analysis.emotion.intensity || 0.5);
        ...
        for (const person of importantPeople.slice(0, 3)) {
            if (mentionedInMessage.includes(person.name.toLowerCase())) {
                ...
                const sentimentDesc = person.averageSentiment > 0.3
                    ? 'generally positive'
 ...
 */
export function clearSessionMentions(sessionId) {
...
//# sourceMappingURL=social-relationships.js.map