 ...
 */
export function getRelationshipBehaviors(context) {
 ...
 */
function buildPromptInjection(context, behaviors) {
    const sections = [];
    // Header
 ...
 */
export function isBehaviorAllowed(behavior, stage) {
 ...
 */
export function getChallengePhrase(stage) {
    const phrases = RELATIONSHIP_BEHAVIORS[stage].unlockedPhrases.filter((p) => p.toLowerCase().includes('push') ||
 ...
 */
export function getDeepQuestion(stage) {
 ...
 */
export function calculateRelationshipStage(turnCount, sessionCount, sharedVulnerabilities = 0, celebratedTogether = 0, difficultConversations = 0) {
    // Bonus turns from meaningful interactions
    const bonusTurns = sharedVulnerabilities * 20 + celebratedTogether * 10 + difficultConversations * 30;
 ...
 */
export function getRelationshipTransitionAnnouncement(fromStage, toStage, userName) {
    const transitions = {
 ...
 */
export function mapUserProfileStageToHumanizing(stage) {
    const mapping = {
 ...
 */
export function mapHumanizingStageToUserProfile(stage) {
    const mapping = {
...
//# sourceMappingURL=relationship-behaviors.js.map