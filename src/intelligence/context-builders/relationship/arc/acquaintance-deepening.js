...
import { loadRelationshipArcData, canMakeFirstWordsCallbackAsync, markFirstWordsCallbackMade, } from './storage.js';
 ...
 */
async function isAcquaintanceStage(input) {
    ...
    if (arcData?.currentStage === 'acquaintance')
        return true;
 ...
 */
function getSharedHistoryGuidance(firstWords, keyMoments) {
    const parts = [];
    if (firstWords) {
        parts.push(`Their very first words to you: "${firstWords.slice(0, 80)}${firstWords.length > 80 ? '...' : ''}"`);
    }
 ...
 */
function getPatternObservationGuidance(totalSessions, vulnerabilityCount) {
    const observations = [];
    if (totalSessions >= 3) {
 ...
 */
function getTrustBuildingGuidance() {
 ...
 */
function getFirstWordsCallbackForAcquaintance(firstWords) {
...
// BUILDER
// ============================================================================
export const acquaintanceDeepeningBuilder = {
    name: 'acquaintance-deepening',
    description: 'Trust-building behaviors for the acquaintance stage: shared history, gentle patterns',
    ...
    build: async (input) => {
        ...
        injections.push(createStandardInjection('acquaintance_trust', getTrustBuildingGuidance(), {
            category: 'relationship-arc',
            confidence: 0.95,
        }));
        // Shared history guidance
        const historyGuidance = getSharedHistoryGuidance(arcData.firstMeeting?.firstWords, arcData.keyMoments.slice(-3));
        if (historyGuidance) {
            injections.push(createStandardInjection('acquaintance_history', historyGuidance, {
                category: 'relationship-arc',
                confidence: 0.9,
            }));
        }
        ...
        if (arcData.totalSessions >= 3) {
            const patternGuidance = getPatternObservationGuidance(arcData.totalSessions, arcData.vulnerabilityCount);
            if (patternGuidance) {
                injections.push(createHintInjection('acquaintance_patterns', patternGuidance, {
                    category: 'relationship-arc',
                    confidence: 0.8,
                }));
            }
        ...
        if (await canMakeFirstWordsCallbackAsync(userId)) {
            if (arcData.firstMeeting?.firstWords) {
                injections.push(createHintInjection('acquaintance_first_words', getFirstWordsCallbackForAcquaintance(arcData.firstMeeting.firstWords), {
                    category: 'relationship-arc',
                    confidence: 0.85,
                }));
...
//# sourceMappingURL=acquaintance-deepening.js.map