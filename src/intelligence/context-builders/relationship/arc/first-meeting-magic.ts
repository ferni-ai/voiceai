 ...
 */
export function detectUserEnergy(
  userText: string,
  ...
  if (voiceEmotion) {
    ...
    if (primary === 'anxious' || primary === 'nervous') return 'anxious';
    if (primary === 'excited' || primary === 'happy') return 'excited';
    if (primary === 'sad' || primary === 'down') return 'low';
    if (intensity < 0.3) return 'guarded';
  }

  // Check speech rate
  if (speechRate && speechRate > 180) return 'rushed';
  if (speechRate && speechRate < 100) return 'low';
  ...
  if (rushedIndicators.some((i) => text.includes(i))) return 'rushed';
  if (anxiousIndicators.some((i) => text.includes(i))) return 'anxious';
  if (excitedIndicators.some((i) => text.includes(i))) return 'excited';
  if (lowIndicators.some((i) => text.includes(i))) return 'low';
  if (guardedIndicators.some((i) => text.includes(i))) return 'guarded';
 ...
 */
function isFirstMeeting(input: ContextBuilderInput): boolean {
  ...
  if (userData?.isReturningUser) return false;
  ...
  if (totalConversations > 1) return false;
 ...
 */
export function checkIsFirstMeeting(input: ContextBuilderInput): boolean {
 ...
 */
function getEnergyGuidance(energy: DetectedEnergy): string {
  switch (energy) {
    case 'rushed':
 ...
 */
function getNoticingGuidance(firstWords: string | undefined): string {
 ...
 */
function getModelVulnerabilityGuidance(): string {
  return `
 ...
 */
function getFirstWordsCallbackGuidance(firstWords: string | undefined): string {
  if (!firstWords || firstWords.length < 10) return '';
 ...
 */
function getAntiExplainingGuidance(): string {
  return `
[ZERO EXPLAINING]
NEVER say:
...
export const firstMeetingMagicBuilder: ContextBuilder = {
  name: 'first-meeting-magic',
  description:
    'Better-than-human first meeting behaviors: energy matching, vulnerability modeling, noticing gifts',
...
Not because you're performing interest—because you ARE interested.`;
      ...
      injections.push(
        createHighInjection('first_meeting_core', coreGuidance, {
    ...
    if (turnCount >= 1 && turnCount <= 3) {
      ...
      if (noticingGuidance) {
        injections.push(
          createStandardInjection('first_meeting_noticing', noticingGuidance, {
            category: 'first-meeting',
            confidence: 0.9,
          })
        );
      ...
      if (turnCount === 1 && userText) {
        ...
        if (callbackGuidance) {
          injections.push(
            createStandardInjection('first_meeting_callback', callbackGuidance, {
              category: 'first-meeting',
              confidence: 0.85,
            })
          );
...
${getEnergyGuidance(detectedEnergy)}`;
      ...
      injections.push(
        createStandardInjection('first_meeting_energy', energyOnlyGuidance, {
          category: 'first-meeting',
          confidence: 0.9,
        })
      );
 ...
 */
function estimateSpeechRate(text: string, durationMs: number): number | undefined {
  if (!text || durationMs < 1000) return undefined;
  ...
  // Sanity check
  if (wordsPerMinute < 50 || wordsPerMinute > 300) return undefined;
