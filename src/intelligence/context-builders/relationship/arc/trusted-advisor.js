 ...
 */
async function isTrustedAdvisorStage(input) {
    ...
    if (arcData?.currentStage === 'trusted_advisor')
        return true;
 ...
 */
function getLifeArcGuidance(keyMoments, stageTransitions, totalSessions, firstSessionDate) {
 ...
 */
function getValuesAccountabilityGuidance(keyMoments) {
    // Look for stated values in breakthrough/vulnerability moments
    const valueMoments = keyMoments.filter((m) => m.type === 'breakthrough' || (m.summary && m.summary.toLowerCase().includes('want')));
 ...
 */
function getCrossDomainSynthesisGuidance(keyMoments) {
 ...
 */
function getDirectChallengeGuidance() {
 ...
 */
function getInstitutionalMemoryGuidance(keyMoments, firstSessionDate) {
...
// BUILDER
// ============================================================================
export const trustedAdvisorBuilder = {
    name: 'trusted-advisor',
    description: 'Deep partnership behaviors: life arc, values accountability, cross-domain synthesis',
    priority: 26,
    ...
    build: async (input) => {
        ...
        const lifeArcGuidance = getLifeArcGuidance(arcData.keyMoments, arcData.stageTransitions, arcData.totalSessions, arcData.firstSessionDate);
        injections.push(createStandardInjection('advisor_life_arc', lifeArcGuidance, {
            category: 'relationship-arc',
            confidence: 0.95,
        }));
        // Values accountability
        const valuesGuidance = getValuesAccountabilityGuidance(arcData.keyMoments);
        injections.push(createStandardInjection('advisor_values', valuesGuidance, {
            category: 'relationship-arc',
            confidence: 0.9,
        }));
        // Cross-domain synthesis
        injections.push(createStandardInjection('advisor_synthesis', getCrossDomainSynthesisGuidance(arcData.keyMoments), {
            category: 'relationship-arc',
            confidence: 0.9,
        }));
        // Direct challenge (fully unlocked)
        injections.push(createHintInjection('advisor_challenge', getDirectChallengeGuidance(), {
            category: 'relationship-arc',
            confidence: 0.85,
        }));
        // Institutional memory
        const memoryGuidance = getInstitutionalMemoryGuidance(arcData.keyMoments, arcData.firstSessionDate);
        injections.push(createHintInjection('advisor_memory', memoryGuidance, {
            category: 'relationship-arc',
            confidence: 0.85,
        }));
        log.debug({
...
//# sourceMappingURL=trusted-advisor.js.map