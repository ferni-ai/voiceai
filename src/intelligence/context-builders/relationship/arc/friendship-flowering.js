 ...
 */
async function isFriendshipStage(input) {
    ...
    if (arcData?.currentStage === 'friend')
        return true;
 ...
 */
function getGrowthReflectionGuidance(keyMoments, vulnerabilityCount, breakthroughCount) {
    const parts = [];
 ...
 */
function getInsideJokeGuidance(sharedVocabulary, keyMoments) {
    const parts = [];
    // Shared vocabulary
    if (sharedVocabulary.length > 0) {
        const topTerms = sharedVocabulary.slice(0, 3);
        parts.push(`Shared vocabulary: ${topTerms.map((v) => `"${v.term}" (${v.meaning})`).join(', ')}`);
    }
    // Humor moments
 ...
 */
function getGentleChallengeGuidance() {
 ...
 */
function getAnticipationGuidance(keyMoments) {
...
// BUILDER
// ============================================================================
export const friendshipFloweringBuilder = {
    ...
    build: async (input) => {
        ...
        const growthGuidance = getGrowthReflectionGuidance(arcData.keyMoments, arcData.vulnerabilityCount, arcData.breakthroughCount);
        injections.push(createStandardInjection('friendship_growth', growthGuidance, {
        ...
        injections.push(createStandardInjection('friendship_callbacks', insideJokeGuidance, {
            category: 'relationship-arc',
            confidence: 0.9,
        }));
        // Gentle challenging (unlocked at friendship level)
        injections.push(createHintInjection('friendship_challenge', getGentleChallengeGuidance(), {
            category: 'relationship-arc',
            confidence: 0.85,
        }));
        // Anticipation
        const anticipationGuidance = getAnticipationGuidance(arcData.keyMoments);
        injections.push(createHintInjection('friendship_anticipation', anticipationGuidance, {
            category: 'relationship-arc',
            confidence: 0.8,
        }));
        ...
        if (unreferenced.length > 0) {
            ...
            injections.push(createHintInjection('friendship_unreferenced', `[MOMENTS TO POTENTIALLY REFERENCE]
...
//# sourceMappingURL=friendship-flowering.js.map