 ...
 */
export async function buildSocialGraphContext(params) {
    ...
    try {
        const { detectWithdrawal, generateSocialInsights, getUpcomingDates, detectSentimentPatterns } = await import('../../../services/social-graph/index.js');
        ...
        if (highPriorityWithdrawal) {
            injections.push(createStandardInjection('social_graph_withdrawal', `[🔍 SOCIAL INTELLIGENCE - "Better Than Human"]
...
Don't force it - only mention if conversation leads there naturally.`));
        ...
        if (immediateDates.length > 0) {
            const dateContext = immediateDates
                ...
                .join('\n');
            injections.push(createStandardInjection('social_graph_dates', `[📅 IMPORTANT DATES - "Better Than Human"]
${dateContext}
...
"By the way, ${immediateDates[0].personName}'s ${immediateDates[0].type} is ${immediateDates[0].daysUntil === 0 ? 'today' : 'tomorrow'} - how are you feeling about it?"`));
        ...
        if (significantPattern && Math.random() < 0.3) {
            ...
            injections.push(createHintInjection('social_graph_pattern', `[💡 RELATIONSHIP PATTERN - "Better Than Human"]
...
Only surface if it adds value to the conversation.`));
        ...
        if (turnCount % 10 === 0) {
            ...
            if (topInsight) {
                injections.push(createHintInjection('social_graph_insight', `[🌟 SOCIAL INSIGHT - "Better Than Human"]
${topInsight.insight}
...
// REGISTER BUILDER
// ============================================================================
registerContextBuilder('social_graph', async (input) => {
...
//# sourceMappingURL=social-graph-context.js.map