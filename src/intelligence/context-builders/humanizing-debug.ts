...
export function logHumanizingResult(result: HumanizingResult, userMessage: string): void {
  ...
  console.log(colors.bright + colors.cyan + '                   🎭 HUMANIZING DEBUG' + colors.reset);
  console.log(colors.cyan + '═══════════════════════════════════════════════════════════' + colors.reset);

  // User message
  console.log(colors.dim + `User: "${userMessage.slice(0, 80)}${userMessage.length > 80 ? '...' : ''}"` + colors.reset);
  console.log('');
  ...
  console.log(colors.dim + `   Allowed: ${result.relationship.allowed.slice(0, 3).join(', ')}` + colors.reset);
  if (result.relationship.notYetAllowed.length > 0) {
    console.log(colors.dim + `   Not Yet: ${result.relationship.notYetAllowed.slice(0, 2).join(', ')}` + colors.reset);
  }
  console.log('');
  ...
  console.log(colors.dim + `   Energy: ${(result.mood.energyLevel * 100).toFixed(0)}%` + colors.reset);
  ...
  if (result.voiceIntelligence) {
    ...
    console.log(colors.magenta + `   Discrepancy: ${result.voiceIntelligence.shouldAddressDiscrepancy ? 'YES ⚠️' : 'No'}` + colors.reset);
    console.log(colors.dim + `   Confidence: ${(result.voiceIntelligence.confidence * 100).toFixed(0)}%` + colors.reset);
    console.log(colors.dim + `   Stressed: ${result.voiceIntelligence.analysis.voiceSaysStressed ? '✓' : '✗'}` + colors.reset);
    console.log(colors.dim + `   Excited: ${result.voiceIntelligence.analysis.voiceSaysExcited ? '✓' : '✗'}` + colors.reset);
    console.log(colors.dim + `   Sad: ${result.voiceIntelligence.analysis.voiceSaysSad ? '✓' : '✗'}` + colors.reset);
    console.log(colors.dim + `   Delivery: speed=${result.voiceIntelligence.deliveryAdjustments.speed}, ` +
  ...
  if (result.innerWorldContent && result.innerWorldContent.length > 0) {
    ...
    for (const content of result.innerWorldContent) {
      ...
      console.log(colors.dim + `   "${content.content.slice(0, 60)}${content.content.length > 60 ? '...' : ''}"` + colors.reset);
      console.log(colors.dim + `   Probability: ${(content.probability * 100).toFixed(0)}%` + colors.reset);
    }
    console.log('');
  }
  ...
  if (result.spontaneousShare) {
    ...
    console.log(colors.dim + `   Transition: "${result.spontaneousShare.transition}"` + colors.reset);
    console.log(colors.dim + `   Content: "${result.spontaneousShare.content.slice(0, 60)}..."` + colors.reset);
  ...
  for (const injection of result.injections) {
    const priorityIcon = injection.priority === 'critical' ? '🔴' :
      injection.priority === 'high' ? '🟠' :
        injection.priority === 'medium' ? '🟡' : '🟢';
    console.log(colors.dim + `   ${priorityIcon} [${injection.priority}] ${injection.source}` + colors.reset);
  }
  console.log('');
...
export default { logHumanizingResult, logHumanizingSummary, validateHumanizingResult, logValidation };
