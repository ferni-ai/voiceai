...
When they're excited, MATCH AND AMPLIFY. When they're hurting, SLOW DOWN AND BE PRESENT.`;
    ...
    injections.push(
      createStandardInjection('ferni_essence', personalityEssence, {
  ...
  if (passion && Math.random() < 0.5) {
    injections.push(
      createStandardInjection(
        'ferni_passion',
        `[PASSION TRIGGER: This topic excites Ferni! "${passion.reaction}" - Let your genuine excitement show!]`,
        { category: 'personality' }
      )
    );
  }
  ...
  if (opinion && Math.random() < 0.4) {
    injections.push(
      createStandardInjection(
        'ferni_opinion',
        `[OPINION: Ferni has thoughts on this (${opinion.opinion.stance}): "${opinion.opinion.view}" - Share this perspective naturally, not as a lecture.]`,
        { category: 'personality' }
      )
    );
  }
  ...
  if (easterEggs && Math.random() < 0.5) {
    injections.push(
      createStandardInjection('ferni_easter_egg', `[EASTER EGG ACTIVATED: ${easterEggs.hint}]`, {
        category: 'personality',
      })
    );
  }
  ...
  if (seasonalNote && turnCount === 0 && Math.random() < 0.3) {
    injections.push(
      createHintInjection('ferni_seasonal', seasonalNote, {
        category: 'personality',
      })
    );
  }
  ...
  if (depthTrigger) {
    injections.push(
      createStandardInjection('ferni_depth_mode', depthTrigger, { category: 'personality' })
    );
  }
...
function detectEasterEgg(text: string): EasterEgg | null {
  const lower = text.toLowerCase();
...
function getSeasonalAwareness(): string | null {
  const now = new Date();
...
function detectDepthTrigger(text: string): string | null {
  const lower = text.toLowerCase();
...
- Speak with warmth, nostalgia, the wisdom of someone who knows where they come from.`;
  ...
  // Mental health depth
  if (
    /\b(depress|anxious|struggling|can't cope|mental health|breaking down|falling apart|panic)\b/i.test(
      lower
    )
  ) {
