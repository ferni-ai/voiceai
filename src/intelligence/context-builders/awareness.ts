...
import { getMomentumTracker, type MomentumState_Full } from '../../conversation/momentum-tracker.js';
import {
  calculateThinkingTime,
  type ThinkingContext,
  type ThinkingInjection,
} from '../../conversation/thinking-time-injector.js';
import { decideTangent, type TangentDecision } from '../../conversation/mid-response-tangents.js';
import { getSelfAwarenessTracker, type ResponseType } from '../../conversation/self-awareness-loop.js';
...
function detectResponseType(text: string): ResponseType {
  ...
  if (/\b(I hear|sounds like|it seems|you('re| are) feeling)\b/i.test(lowerText)) return 'reflection';
...
async function buildAwarenessContext(input: ContextBuilderInput): Promise<ContextInjection[]> {
  ...
  if (Math.random() < profile.thinkingPauseProbability) {
    ...
    if (thinkingTime.openingPauseMs > 300 || thinkingTime.thinkingSound) {
      ...
      thinkingGuidance +=
        thinkingTime.reasoning.length > 0 ? `(${thinkingTime.reasoning[0]})` : '';
      thinkingGuidance += ']';
