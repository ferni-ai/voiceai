 ...
 */
import { registerContextBuilder, createHintInjection, createHighInjection, createStandardInjection, } from '../index.js';
...
import { detectTrustSignals } from '../../../services/coaching/semantic-trust.js';
...
function detectCalendarIntent(message) {
    ...
    if (calendarResult.type !== 'none' && calendarResult.confidence >= 0.4) {
        ...
        if (calendarResult.extractedInfo) {
            ...
            if (info.timeReference)
                fullGuidance += ` Time: "${info.timeReference}"`;
            if (info.eventType)
                fullGuidance += ` Event: ${info.eventType}`;
            if (info.person)
                fullGuidance += ` With: ${info.person}`;
        }
...
function detectMusicIntent(message) {
    const patterns = [
        {
            pattern: /\b(play|put on|queue|listen to)\s*(some|a|the)?\s*(music|song|track|jazz|rock|lo-?fi|chill)/i,
            intent: 'music_play',
            confidence: 0.9,
        },
        {
 ...
 */
function detectCrisisIntent(message) {
...
// ============================================================================
async function buildSemanticIntentContext(input) {
    ...
    // -------------------------------------------------------------------------
    try {
        const handoffDecision = detectHandoffSemanticly(userId, userText, currentPersona);
        if (handoffDecision.shouldHandoff && handoffDecision.candidate) {
            ...
            if (confidence >= 0.7 && isUnlocked) {
                ...
                injections.push(createHighInjection('semantic_handoff_strong', `[🤝 TEAM HANDOFF OPPORTUNITY - HIGH CONFIDENCE]
...
Only suggest if it feels natural. Don't force it.`, { category: 'handoff', confidence }));
            }
            else if (confidence >= 0.5 && isUnlocked) {
                // MEDIUM CONFIDENCE → Hint
                injections.push(createHintInjection('semantic_handoff_hint', `[🤝 POSSIBLE TEAM FIT]
${targetName} specializes in ${specialty}.
If the conversation goes deeper into this area, consider introducing them.
Confidence: ${(confidence * 100).toFixed(0)}%`, { category: 'handoff', confidence }));
            }
            else if (confidence >= 0.5 && !isUnlocked) {
                // MEDIUM+ CONFIDENCE but NOT UNLOCKED → Plant seed
                injections.push(createStandardInjection('semantic_handoff_locked', `[👥 TEAM MEMBER NOT YET MET]
This topic relates to a team member's specialty, but user hasn't met them yet.
Don't mention specific names. You can say:
"I have a friend on my team who's really good at this - we'll meet them as we get to know each other better."`, { category: 'handoff', confidence: confidence * 0.8 }));
            }
        }
    }
    catch (err) {
 ...
 */
function formatTrustGuidance(primarySignal, allSignals) {
    const lines = [];
...
//# sourceMappingURL=semantic-intent-guidance.js.map