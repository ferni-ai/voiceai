...
import { generateSECInsight, getInsiderTradingSummary, getCompanyFilings, } from '../../../services/finance/sec-edgar.js';
 ...
 */
export async function buildSECIntelligenceContext(userId, recentTranscript, watchlistTickers = []) {
    try {
        ...
        for (const ticker of allTickers.slice(0, 3)) {
            ...
            if (filingsResult.success && filingsResult.data) {
                for (const filing of filingsResult.data) {
                    const daysAgo = Math.floor((Date.now() - new Date(filing.filingDate).getTime()) / (1000 * 60 * 60 * 24));
                    if (daysAgo <= 14) {
                        recentFilings.push({
                            ticker,
                            form: filing.form,
        ...
        log.debug({ userId, tickers: allTickers, insightCount: insights.length }, 'Built SEC intelligence context');
        return {
            insights,
            recentFilings,
            insiderSentiment,
 ...
 */
export async function generateSuperhumanSECMoment(ticker) {
...
//# sourceMappingURL=sec-intelligence.js.map