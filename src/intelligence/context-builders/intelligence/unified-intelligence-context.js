...
import { getIntelligenceForTurn, } from '../../unified-intelligence-api.js';
 ...
 */
export const unifiedIntelligenceBuilder = {
    ...
    async build(input) {
        ...
        try {
            ...
            const moment = turnCount === 1 ? 'session_start' : turnCount <= 3 ? 'natural_pause' : 'topic_relevant';
            ...
            if (intelligence.context.capacity.bandwidth === 'low') {
                injections.push({
                    ...
                    content: '[CAPACITY] Low bandwidth detected. Keep responses focused and avoid overwhelming.',
                    source: 'unified-intelligence',
                    priority: 'high',
                    category: 'capacity',
                });
            ...
            if (intelligence.proactiveInsights.length > 0 && turnCount >= 2) {
                ...
                if (insight.priority <= 5 || moment === 'session_start') {
                    injections.push({
                        ...
                        source: 'unified-intelligence',
                        priority: (insight.priority <= 2 ? 'critical' : 'high'),
                        category: 'proactive',
                    });
                }
            }
 ...
 */
export async function getUnifiedIntelligenceData(userId, options = {}) {
 ...
 */
export function formatUnifiedIntelligenceForPrompt(data) {
    const sections = [];
    // Context summary
    const ctx = data.context;
    sections.push(`[UNIFIED INTELLIGENCE]`, `Time: ${ctx.immediate.timeOfDay} ${ctx.immediate.dayOfWeek}${ctx.immediate.isLateNight ? ' (late night)' : ''}`);
...
//# sourceMappingURL=unified-intelligence-context.js.map