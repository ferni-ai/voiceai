...
import { getBestInsight as getBestTonalInsight, markInsightSurfaced as markTonalInsightSurfaced, recordTonalObservation, } from '../../../services/trust-systems/tonal-memory.js';
 ...
 */
const lastNoticeSurfaced = new Map(); // userId -> turnCount
let cachedPhrases = null;
async function loadINoticePhrases(personaId) {
    if (cachedPhrases)
        return cachedPhrases;
    try {
        const content = await loadPersonaContent(personaId, 'i-notice-power');
        if (content) {
 ...
 */
function detectFrequencyPattern(userText, userData, analysis) {
 ...
 */
function detectContradictionPattern(userText, analysis) {
 ...
 */
function detectDeflectionPattern(userText) {
    const lowerText = userText.toLowerCase();
 ...
 */
function buildNoticingPhrase(pattern, phrases, relationshipStage) {
    ...
    switch (pattern.type) {
        ...
        case 'frequency':
            ...
            break;
        case 'deflection':
            const unsaidPhrases = phrases.what_they_didnt_say?.phrases || phrases.surfacing_phrases.avoidance_patterns || [];
 ...
 */
function checkDynamicTriggers(triggers, context) {
    if (!triggers)
        return null;
    ...
    for (const [name, trigger] of Object.entries(triggers)) {
        ...
        if (triggerLower.includes('distress') &&
            context.emotion &&
        ...
        if ((triggerLower.includes('should') || triggerLower.includes('self-criticism')) &&
            (lowerText.match(/i should/gi) || []).length >= 2) {
            return { matched: true, triggerName: name, behavior: trigger.behavior };
        }
        // Check for comparison language
        if (triggerLower.includes('comparison') &&
        ...
        if (triggerLower.includes('avoidance') &&
        ...
        if (triggerLower.includes('meaning') || triggerLower.includes('point')) {
            if (lowerText.includes("what's the point") ||
...
// ============================================================================
async function buildProactiveNoticingContext(input) {
    const { userText, analysis, userData, services, persona } = input;
    const userId = services?.userId;
    const turnCount = userData?.turnCount || 0;
    const personaId = persona?.id || 'ferni';
    const relationshipStage = userData?.relationshipStage || 'stranger';
    ...
    const emotion = analysis?.emotion?.primary;
    if (neverWhen.includes('first_few_turns') && turnCount < 4)
        return [];
    if (neverWhen.includes('user_distressed') &&
        emotion &&
        ['distressed', 'crisis', 'panic'].includes(emotion))
        return [];
    if (neverWhen.includes('user_in_crisis') && emotion === 'crisis')
        return [];
    // Check cooldown
    ...
    if (tonalInsight && tonalInsight.confidence > 0.6 && tonalInsight.occurrences >= 3) {
        patterns.push({
            type: 'energy', // Using 'energy' type for tonal patterns
    ...
    if (patterns.length === 0 && !dynamicTrigger)
        return [];
    ...
    if (dynamicTrigger)
        adjustedProbability *= 1.5; // Boost when dynamic trigger matches
    // Apply probability gate
    if (Math.random() > Math.min(adjustedProbability, 0.4)) {
        // Cap at 40%
        log.debug({ userId, pattern: bestPattern.type, confidence: bestPattern.confidence }, 'Pattern detected but probability gate not passed');
        return [];
    }
    ...
    return [
        createHighInjection('proactive_noticing', `[BETTER-THAN-HUMAN MOMENT] You noticed something worth surfacing. Consider saying something like: "${noticingPhrase}" - but make it natural and in your own voice. This is a "they really see me" moment. Offer an exit ramp.${patternGuidance}`, { category: 'trust', confidence: bestPattern.confidence }),
    ];
}
// ============================================================================
// REGISTRATION
...
//# sourceMappingURL=proactive-intelligence.js.map