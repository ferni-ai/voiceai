 ...
 */
import { extractPersonalThemes } from '../../../types/personal-themes.js';
 ...
 */
function userAskedAboutTheme(userMessage, theme) {
 ...
 */
export function findInnerWorldInjections(context, bundleRuntime) {
    if (!bundleRuntime) {
        return [];
    }
    const injections = [];
    const combinedText = `${context.userMessage} ${context.recentTopics.join(' ')}`;
    const mentionedThemes = context.mentionedThemes || new Set();
            ...
            for (const path of trigger.innerWorldPaths) {
                ...
                if (content) {
                    ...
                    if (alreadyMentionedThemes.length > 0) {
                        const userAsked = alreadyMentionedThemes.some((theme) => userAskedAboutTheme(context.userMessage, theme));
                        if (!userAsked) {
 ...
 */
function getInnerWorldContent(bundleRuntime, path, context) {
    try {
        ...
        if (path.includes('sense_memories') || path.includes('embodied_memories')) {
            ...
            if (randomMemory)
                return randomMemory.memory;
        }
        if (path.includes('what_they_tell_themselves_when_struggling') ||
        ...
        if (path.includes('contradiction') || path.includes('belief_vs_behavior')) {
            ...
            if (contradiction)
                return `${contradiction.belief}, but ${contradiction.but}`;
        }
        ...
        if (path.includes('mentor')) {
            ...
            if (mentor)
                return `${mentor.mentor} once told me: "${mentor.quote}"`;
        }
 ...
 */
function getRequiredRelationship(depth) {
    switch (depth) {
        case 'surface':
            return 'stranger';
        case 'medium':
 ...
 */
function calculateInjectionProbability(context, depth) {
 ...
 */
function getRelationshipDepthScore(stage) {
    const scores = {
        stranger: 1,
        acquaintance: 2,
        friend: 3,
...
function getTypeFromPath(path) {
    if (path.includes('sense_memories') || path.includes('embodied'))
        return 'embodied_memory';
    if (path.includes('sensory'))
        return 'sensory_memory';
    if (path.includes('contradiction'))
        return 'contradiction';
    if (path.includes('flashpoint'))
        return 'emotional_flashpoint';
    if (path.includes('unfinished'))
        return 'unfinished_business';
    if (path.includes('secret'))
        return 'secret_self';
    if (path.includes('dream'))
        return 'dream_still_chasing';
    if (path.includes('mortality'))
        return 'mortality_awareness';
    if (path.includes('voice_fingerprint'))
 ...
 */
export function formatInnerWorldForPrompt(injections, relationshipStage) {
    ...
    for (const injection of validInjections.slice(0, 2)) {
        ...
        const depthDesc = injection.depth === 'surface'
 ...
 */
export function shouldInject(injection) {
    return Math.random() < injection.probability;
}
export default findInnerWorldInjections;
//# sourceMappingURL=inner-world-injector.js.map