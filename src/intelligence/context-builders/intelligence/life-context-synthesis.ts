 ...
 */
const lastLifeContextSurfaced = new Map<string, number>();
 ...
 */
function shouldSurfaceThisTurn(sessionId: string, turnCount: number): boolean {
 ...
 */
function formatTriggerGuidance(trigger: SynthesisTrigger): string {
  const parts: string[] = [];
 ...
 */
function formatPatternGuidance(patterns: LifeContextSnapshot['patterns']): string {
 ...
 */
function formatLifeContextSummary(snapshot: LifeContextSnapshot): string {
  const parts: string[] = [];
...
export const lifeContextSynthesisBuilder = {
  ...
  build: async (input: ContextBuilderInput): Promise<ContextInjection[]> => {
    ...
    const lifeContext = userData?.lifeContextSnapshot as LifeContextSnapshot | undefined;
    ...
    if (!shouldSurfaceThisTurn(sessionId, turnCount)) {
      ...
      if (lifeContext.overallLoadScore > HIGH_LOAD_THRESHOLD) {
        injections.push(
          createStandardInjection(
            'life_context_background',
            `[Background Awareness] User's life load is elevated (${(lifeContext.overallLoadScore * 100).toFixed(0)}%). ` +
              'Be extra gentle and supportive. Avoid adding pressure.',
    ...
    injections.push(
      createStandardInjection('life_context_summary', formatLifeContextSummary(lifeContext), {
        category: 'life-context',
      })
    );
    ...
    if (lifeContext.patterns.length > 0) {
      ...
      if (patternGuidance) {
        injections.push(
          createStandardInjection('life_context_patterns', patternGuidance, {
            category: 'life-context',
          })
        );
      }
 ...
 */
export function clearLifeContextSurfacingHistory(): void {
