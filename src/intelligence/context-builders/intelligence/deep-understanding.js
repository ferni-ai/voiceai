...
import { formatPredictionForPrompt, predictUserState, recordConversationObservation, } from '../../life-rhythm-prediction.js';
import { detectUnspokenTension, extractPersonMentions, formatRelationalInsightsForPrompt, recordPersonMention, } from '../../relational-network.js';
...
import { detectMisunderstanding, formatRepairForPrompt, generateRepair, recordAIResponse, } from '../../repair-intelligence.js';
...
import { BuilderCategory, createCriticalInjection, createHighInjection, createHintInjection, createStandardInjection, registerContextBuilder, } from '../index.js';
 ...
 */
export function clearDeepUnderstandingSession(sessionId) {
    sessions.delete(sessionId);
}
/**
 * Clear all session data (for shutdown).
 */
export function clearAllDeepUnderstandingSessions() {
    sessions.clear();
}
// ============================================================================
 ...
 */
async function buildDeepUnderstanding(input) {
    ...
    const voiceData = input.voiceEmotion ?? null;
    // Calculate shifts
    ...
    try {
        ...
        if (session.turnCount > 1 && session.lastAIResponse) {
            const misunderstanding = detectMisunderstanding(userId, sessionId, userText, emotionShift, 0 // engagement shift - would need more context
            );
            if (misunderstanding.detected) {
                const repair = generateRepair(misunderstanding);
                injections.push(createCriticalInjection('deep_repair', formatRepairForPrompt(misunderstanding, repair)));
        ...
        if (energy.physical.level === 'depleted' ||
            energy.physical.level === 'low' ||
            energy.mental.capacity === 'overwhelmed' ||
            energy.mental.capacity === 'limited') {
            injections.push(createStandardInjection('deep_energy', formatEnergyForPrompt(energy), {
                category: 'awareness',
            }));
        }
        // ========================================================================
        ...
        // ========================================================================
        const hope = analyzeHope(userId, sessionId, userText, currentTopics, [currentEmotion], voiceData?.stressLevel || 0);
        ...
        // ========================================================================
        const flow = analyzeFlow(userId, sessionId, userText, session.turnCount, currentEmotionIntensity, voiceData
            ? {
        ...
        if (flow.state.recommendedDirection !== 'maintain') {
            injections.push(createStandardInjection('deep_flow', formatFlowForPrompt(flow), {
                category: 'guidance',
            }));
        }
        else {
            // Even when "maintaining", remind to ask questions - this is core to Ferni's personality
            injections.push(createHintInjection('deep_flow_curiosity', `[STAY CURIOUS] Conversation is flowing well. Keep asking questions to show genuine interest. Consider: "What's that like for you?" or "How does that feel?" or "What's underneath that?" - end your response with a follow-up question when natural.`, { category: 'guidance' }));
        }
        // ========================================================================
        // 5. RESISTANCE DETECTION
        // ========================================================================
        const resistance = analyzeResistance(userId, userText, currentEmotion, currentEmotionIntensity, currentTopics, session.turnCount > 1 ? currentTopics[0] : undefined);
        // Inject if significant resistance detected
        if (resistance.resistanceLevel > 0.4 || resistance.defensesDetected.length > 0) {
            injections.push(createHintInjection('deep_resistance', formatResistanceForPrompt(resistance), {
                category: 'awareness',
            }));
        }
        // ========================================================================
        ...
        // ========================================================================
        const subconscious = analyzeSubconscious(userId, userText, currentTopics, currentEmotionIntensity);
        ...
        if (surfaceOpp.shouldSurface && surfaceOpp.phrase && surfaceOpp.desire) {
            ...
            if (desire.signalCount >= 3 && desire.confidence >= 0.6) {
                ...
                injections.push(createStandardInjection('deep_subconscious', `[SUBCONSCIOUS PATTERN DETECTED]\n` +
                ...
                injections.push(createHintInjection('deep_subconscious_emerging', `[EMERGING PATTERN: ${desire.goal}]\n` +
                    `This theme is recurring (${desire.signalCount} signals, ${Math.round(desire.confidence * 100)}% confidence).\n` +
                    `Keep listening for it - may be worth exploring when confidence grows.`, { category: 'insight' }));
            }
        }
        ...
        if (subconscious.reinforcedDesires.length > 0) {
            ...
            injections.push(createHintInjection('deep_subconscious_reinforced', `[PATTERN REINFORCED] "${reinforced.goal}" (signal #${reinforced.signalCount}).\n` +
                `This user keeps circling back to this theme.`, { category: 'insight' }));
        }
        // Fantasy detection (what-if statements reveal true desires)
        if (subconscious.fantasyDetected && subconscious.fantasyContent) {
            injections.push(createHintInjection('deep_fantasy', `[FANTASY DETECTED] User shared a "what if" scenario:\n` +
                `"${subconscious.fantasyContent.slice(0, 100)}..."\n` +
                `Fantasies often reveal unspoken desires. Explore gently.`, { category: 'insight' }));
        }
        // ========================================================================
        // 7. LIFE CHAPTER
        // ========================================================================
        ...
        if (chapter.chapter.current.confidence > 0.5 &&
            ...
            injections.push(createHintInjection('deep_chapter', formatChapterForPrompt(chapter), {
                category: 'context',
            }));
        }
        // ========================================================================
        ...
        if (personMentions.length > 0) {
            ...
            if (relationalInsight) {
                injections.push(createHintInjection('deep_relational', relationalInsight, {
                    category: 'context',
                }));
            }
        }
        ...
        if (rhythmPrediction.reasons.length > 0 && rhythmPrediction.confidence > 0.4) {
            injections.push(createHintInjection('deep_rhythm', formatPredictionForPrompt(rhythmPrediction), {
                category: 'context',
            }));
        }
        // ========================================================================
        ...
        const actualSilenceAnalysis = userData
            .lastSilenceAnalysis;
        if (actualSilenceAnalysis && actualSilenceAnalysis.confidence > 0.4) {
            ...
            injections.push(createStandardInjection('deep_silence', formatSilenceForPrompt(actualSilenceAnalysis), {
                category: 'presence',
        ...
        // Fallback: detect hesitation markers in text if no voice silence
            ...
            const silence = analyzeSilence(2000, // Estimated duration from text hesitation
            userText, currentEmotion, currentEmotionIntensity, currentTopics, userText.includes('?'));
            if (silence.type !== 'unknown' && silence.confidence > 0.5) {
                injections.push(createHintInjection('deep_silence', formatSilenceForPrompt(silence), {
                    category: 'presence',
                }));
            }
        }
        ...
        if (voiceData && voiceData.confidence > 0.4) {
            ...
            if (mismatch.hasMismatch && mismatch.confidence > 0.5) {
                ...
                if (guidance) {
                    ...
                    injections.push(createHighInjection('deep_mismatch', guidance, {
        ...
        if (energy.physical.level === 'depleted' ||
            energy.physical.level === 'low' ||
            energy.mental.capacity === 'overwhelmed') {
            injections.push(createHintInjection('deep_energy_response', `[ENERGY AWARENESS] User appears ${energy.physical.level === 'depleted' ? 'exhausted' : 'low energy'}. ` +
                `Keep your response brief and gentle. No complex questions or advice. ` +
                `Prioritize presence over problem-solving. Consider: "That sounds exhausting" over lengthy responses.`, { category: 'guidance' }));
        }
        // ========================================================================
        ...
        if (resistance.resistanceLevel > 0.6 || resistance.defensesDetected.length >= 2) {
            injections.push(createStandardInjection('deep_resistance_pacing', `[RESISTANCE AWARENESS] Strong resistance detected (level: ${resistance.resistanceLevel.toFixed(2)}). ` +
                `Slow down. Don't push. Offer an exit: "We don't have to talk about this if you'd rather not." ` +
                `Match their pace. Let them lead. Defenses detected: ${resistance.defensesDetected.join(', ')}`, { category: 'guidance' }));
        }
        // ========================================================================
...
// ============================================================================
export function recordResponse(sessionId, response) {
    const session = getSession(sessionId);
...
registerContextBuilder({
    ...
    description: 'Superhuman understanding: silence, rhythm, resistance, energy, goals, flow, repair, hope, chapters, voice-text mismatch',
...
//# sourceMappingURL=deep-understanding.js.map