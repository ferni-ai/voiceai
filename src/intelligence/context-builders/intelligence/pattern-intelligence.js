...
function getPatternState(userId) {
 ...
 */
function recordObservation(userId, data) {
    ...
    for (const [intention, intentionData] of state.statedIntentions) {
        ...
        for (const [action, actionList] of state.reportedActions) {
            ...
            if (overlap.length >= 1 && actionList.length >= 2) {
                ...
                const daysSinceIntention = Math.floor((Date.now() - intentionData.statedAt.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
function getPatternToSurface(userId, relationshipStage, turnCount, emotionalContext) {
    const state = getPatternState(userId);
    // Don't surface too frequently - but earlier is okay for acquaintances
    const minTurnsBetween = relationshipStage === 'stranger' ? 20 : relationshipStage === 'acquaintance' ? 10 : 8;
    ...
    const eligible = patterns.filter((p) => {
        ...
        if (p.sensitivity > sensitivityThreshold)
            return false;
        return true;
    });
 ...
 */
function generatePatternPhrasing(pattern) {
    const phrases = {
        emotional: [
...
function getTopicFromPattern(pattern) {
    const match = pattern.pattern.match(/(?:discussing|about|on) ([^,]+)/);
    return match?.[1] || 'this topic';
}
function getEmotionFromPattern(pattern) {
    const match = pattern.pattern.match(/feel (\w+)/);
    return match?.[1] || 'certain';
}
function getDayFromPattern(pattern) {
    const match = pattern.pattern.match(/on (\w+)s/);
    return match?.[1] || 'certain day';
}
function getWordFromPattern(pattern) {
    const match = pattern.pattern.match(/"([^"]+)"/);
    return match?.[1] || 'this word';
}
function getIntentionFromPattern(pattern) {
    // Try multiple patterns for extracting intention
    const match = pattern.pattern.match(/wanting to "([^"]+)"/) || pattern.pattern.match(/tends? to (\w+ \w+)/);
    return match?.[1] || 'do this';
}
function getActionFromPattern(pattern) {
    // Try multiple patterns for extracting action
    const match = pattern.pattern.match(/reported "([^"]+)"/) || pattern.pattern.match(/now (?:you )?(\w+ \w+)/);
 ...
 */
async function buildPatternSurfacingContext(input) {
    ...
    if (patternContext) {
        injections.push(createHintInjection('pattern_surfacing', formatPatternForPrompt(patternContext), {
            category: 'insight',
            confidence: patternContext.pattern.confidence,
        }));
        log.debug({ type: patternContext.pattern.type, confidence: patternContext.pattern.confidence }, '🔍 Pattern ready to surface');
        // Track telemetry
        const telemetry = getBetterThanHumanTelemetry();
        telemetry.trackPatternSurfaced(services.userId, services.personaId || 'ferni', patternContext.pattern.id);
    }
    return injections;
}
/**
 ...
 */
function formatPatternForPrompt(context) {
