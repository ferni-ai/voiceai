...
import { processCommitments, generateFollowUpPhrase, recordFollowUp, } from '../../../services/trust-systems/commitment-tracking.js';
...
const sessionFollowUps = new Map(); // sessionId -> followUp count
// ============================================================================
// BUILDER
// ============================================================================
async function buildCommitmentFollowUpContext(input) {
    ...
    const { newCommitments, progressUpdates, followUpsDue } = await processCommitments(userId, userText, {
    ...
    if (newCommitments.length > 0) {
        ...
        injections.push(createStandardInjection('commitment_detected', `[USER COMMITMENT] They just committed to: "${commitment.content}". You're now tracking this. Don't make a big deal of it, but acknowledge naturally in your response. Something like "I'll remember that" or "That's a meaningful commitment."`, { category: 'trust' }));
        log.info({ userId, commitment: commitment.content.slice(0, 50) }, '💫 BETTER-THAN-HUMAN: New commitment tracked');
    }
    // 2. If progress was detected, celebrate it
    if (progressUpdates.length > 0) {
        const update = progressUpdates[0];
        const celebration = update.type === 'completed'
    ...
    if (followUpsDue.length > 0 &&
        ...
        if (followUp) {
            ...
            injections.push(createStandardInjection('commitment_follow_up', `[FOLLOW-UP OPPORTUNITY] You've been tracking that they said they'd "${followUp.content}". Consider naturally checking in with something like: "${phrase}" - but only if it fits the conversation. If they seem stressed or the topic doesn't fit, skip it. They haven't mentioned it in a while.`, { category: 'trust', confidence: 0.7 }));
 ...
 */
function prioritizeFollowUp(commitments) {
...
//# sourceMappingURL=commitment-follow-up.js.map