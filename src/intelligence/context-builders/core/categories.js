 ...
 */
export function getBuilderCategory(builderName) {
 ...
 */
export function getBuildersInCategory(category) {
 ...
 */
export function validateBuilderPriorities(builders) {
    const warnings = [];
    for (const builder of builders) {
        const category = getBuilderCategory(builder.name);
        const metadata = getCategoryMetadata(category);
        if (builder.priority < metadata.priorityRange.min) {
            warnings.push(`Builder "${builder.name}" priority ${builder.priority} is below category "${category}" range (${metadata.priorityRange.min}-${metadata.priorityRange.max})`);
        }
        else if (builder.priority > metadata.priorityRange.max) {
            warnings.push(`Builder "${builder.name}" priority ${builder.priority} is above category "${category}" range (${metadata.priorityRange.min}-${metadata.priorityRange.max})`);
        }
    }
    return warnings;
}
