 ...
 */
export function configureConditionalLoading(config) {
 ...
 */
export function getAllBuilderModules() {
    return Object.values(BUILDER_MANIFEST).flat();
}
/**
 * Get builder modules by category
 */
export function getBuilderModulesByCategory(category) {
    return BUILDER_MANIFEST[category] || [];
}
/**
 * Load a single builder module
 */
async function loadBuilderModule(moduleName) {
    const importer = BUILDER_IMPORTS[moduleName];
    if (!importer) {
        log.warn({ module: moduleName }, 'Failed to load builder module (missing from import registry)');
        return false;
    }
    try {
        await importer();
 ...
 */
async function loadBuildersByCategory() {
    const failed = [];
    const skipped = [];
    let loaded = 0;
 ...
 */
async function loadBuildersInternal() {
 ...
 */
export async function ensureBuildersLoaded() {
 ...
 */
export async function reloadBuilders() {
 ...
 */
export function areBuildersLoaded() {
 ...
 */
export function prewarmBuildersInBackground() {
 ...
 */
export function prewarmBuildersWithConfig(config) {
    if (!isFeatureEnabled('contextBuilderPrewarm')) {
        log.debug('Context builder pre-warm disabled by feature flag');
        return;
    }
