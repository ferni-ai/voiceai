...
// ============================================================================
// ANALYTICS TRACKING
// In-memory stats that can be queried for the debug panel
// ============================================================================
...
interface TriggerAnalytics {
  ...
  byTriggerName: Map<string, { checked: number; matched: number; fired: number }>;
  byBuilderSource: Map<string, { checked: number; matched: number; fired: number }>;
 ...
 */
export function recordTriggerMatch(
  triggerName: string,
  builderSource: string,
  confidence: number,
  userId?: string
 ...
 */
export function recordTriggerFired(triggerName: string, builderSource: string): void {
  ...
  const recentMatch = analytics.recentActivations.find(
    (a) => a.triggerName === triggerName && a.builderSource === builderSource && !a.fired
  );
  if (recentMatch) {
 ...
 */
export function resetTriggerAnalytics(): void {
  analytics.totalTriggersChecked = 0;
  analytics.totalTriggersMatched = 0;
 ...
 */
function matchesTextPattern(text: string, patternKey: keyof typeof TEXT_PATTERNS): boolean {
 ...
 */
export function checkDynamicTriggers(
  ...
  log.debug(
    { triggerName: best.triggerName, confidence: best.confidence, matchCount: matches.length },
    'Dynamic trigger matched'
  );
 ...
 */
export function calculateProbabilityBoost(
  moreLikelyWhen: string[] | undefined,
  context: TriggerContext,
  matchedTrigger: MatchedTrigger | null
): number {
  if (!moreLikelyWhen) return 1.0;
 ...
 */
export function shouldSkipDueToNeverWhen(
  neverWhen: string[] | undefined,
  context: TriggerContext
): boolean {
  if (!neverWhen) return false;
 ...
 */
export async function checkTriggersHybrid(
  triggers: Record<string, ProactiveTrigger> | undefined,
  context: TriggerContext,
  personaId: string,
  options: {
  ...
  if (enableSemantic) {
    try {
      ...
      const result = await matchTriggersHybrid(
        ...
        triggers,
        personaId,
        { semanticThreshold, patternThreshold }
      );
