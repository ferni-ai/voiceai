...
import { getActiveThread, buildAgentContext, } from '../../../services/conversation-thread/thread-manager.js';
 ...
 */
export async function buildThreadContext(userId, agentId, options) {
    try {
        // ─────────────────────────────────────────────────────────────────────────
 ...
 */
export async function buildContinuationHint(userId, agentId) {
    try {
        const bridgeContext = await getConversationBridgeContext(userId);
        if (bridgeContext && bridgeContext.isDirectResponse) {
 ...
 */
export function registerThreadContextBuilder() {
