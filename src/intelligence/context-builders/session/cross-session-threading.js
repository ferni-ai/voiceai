...
// CONTEXT BUILDER
// ============================================================================
function buildCrossSessionThreadingContext(input) {
    ...
    try {
        ...
        if (turnCount === 0 || turnCount === 1) {
            ...
            if (conversationStarter) {
                injections.push(createStandardInjection('cross_session_starter', `[CONVERSATION CONTINUITY]
...
Only use if it feels natural - don't force it if user has something urgent.`, { category: 'memory' }));
            }
        }
        // -----------------------------------------------
        ...
        if (openThreads.length > 0 && turnCount >= 2 && turnCount <= 6) {
            ...
            if (topThread.priority === 'high' || topThread.emotionalWeight === 'heavy') {
                injections.push(createStandardInjection('open_thread', `[OPEN THREAD FROM LAST SESSION]
...
Handle this with care - the user trusted you with this.`, { category: 'memory' }));
            }
            else if (openThreads.length > 1) {
                // Lower priority threads get a hint
                injections.push(createHintInjection('open_threads_hint', `[You have ${openThreads.length} open threads from previous sessions. ` +
                    `Main topic: "${topThread.topic}". Consider circling back naturally.]`, { category: 'memory' }));
            }
        }
        // -----------------------------------------------
        ...
        if (pendingFollowUps.length > 0 && turnCount >= 3) {
            ...
            injections.push(createHintInjection('promised_followup', `[PROMISED FOLLOW-UP]
You promised: "${topFollowUp.description}"
Type: ${topFollowUp.type}
${topFollowUp.targetTimeframe ? `When: ${topFollowUp.targetTimeframe}` : ''}

Deliver on this promise when natural. Breaking promises damages trust.`, { category: 'memory' }));
        }
        // -----------------------------------------------
        ...
        if (heavyThreads.length > 0 && turnCount >= 4 && turnCount <= 8) {
            ...
            const shouldCheckIn = userEmotion === 'neutral' || userEmotion === 'trust' || userEmotion === 'anticipation';
            if (shouldCheckIn) {
                injections.push(createHintInjection('emotional_checkin', `[EMOTIONAL CHECK-IN OPPORTUNITY]
...
Example: "I've been thinking about what you shared last time..."`, { category: 'emotional' }));
            }
        }
...
// HELPERS
// ============================================================================
function formatThreadReason(reason) {
    const reasonMap = {
