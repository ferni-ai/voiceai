 ...
 */
function getPersonalDetailCallback(profile) {
    const callbacks = [];
    // Family references
    if (profile.familyMembers && profile.familyMembers.length > 0) {
        const spouse = profile.familyMembers.find((f) => f.relationship === 'spouse');
        if (spouse) {
            callbacks.push(`How is ${spouse.name || 'your spouse'} doing?`);
        }
        const child = profile.familyMembers.find((f) => f.relationship === 'daughter' || f.relationship === 'son' || f.relationship === 'child');
        if (child) {
 ...
 */
function buildPersonalContext(input) {
    ...
    if (userName && turnCount - lastNameTurn >= 4 && Math.random() < 0.4) {
        injections.push(createHintInjection('name_usage', `[NAME: Use their name "${userName}" naturally in this response to build connection.]`));
        if (userData)
            userData.lastNameUsed = turnCount;
    }
    // -----------------------------------------------
    ...
    if (extractedDetails.length > 0) {
        ...
        if (userData) {
            ...
            userData.extractedDetails = [...(userData.extractedDetails || []), ...simpleDetails].slice(-20); // Keep last 20 details
        }
    ...
    if (userData?.extractedDetails && userData.extractedDetails.length > 0 && Math.random() < 0.2) {
        const storedDetail = userData.extractedDetails[Math.floor(Math.random() * userData.extractedDetails.length)];
        if (storedDetail) {
            ...
            if (callback) {
                injections.push(createHintInjection('detail_callback', `[REMEMBERED DETAIL: You know about their ${storedDetail.type} "${storedDetail.value}". Consider: "${callback}"]`));
            }
        }
    }
    // -----------------------------------------------
    ...
    if (turnCount > 5 && turnCount % 7 === 0 && userProfile) {
        ...
        if (personalCallback) {
            injections.push(createHintInjection('personal_callback', `[PERSONAL DETAIL: You know specific things about this person. Consider naturally mentioning: "${personalCallback}"]`));
        }
    }
    return injections;
}
