 ...
 */
function getAcknowledgmentBeforeAdvice(emotion) {
    ...
    const phrases = acknowledgments[emotion] || acknowledgments.neutral;
...
function buildIntentContext(input) {
    ...
    if (primaryIntent === 'seeking_advice') {
        ...
        injections.push(createStandardInjection('intent_advice', `[INTENT: User seeking advice.
...
DO NOT: Lecture. DO: Have a conversation.]`));
    ...
    if (analysis.intent.requiresEmpathy) {
        ...
        injections.push(createStandardInjection('intent_empathy', `[INTENT: User needs empathy.
...
After they finish, consider: "${transition}"]`));
    ...
    if (primaryIntent === 'seeking_advice' &&
        ...
        injections.push(createStandardInjection('acknowledgment', `[ACKNOWLEDGMENT: Before giving advice, acknowledge their emotion first: "${ack}"]`));
    }
    // -----------------------------------------------
    // PHASE-AWARE GUIDANCE
    // -----------------------------------------------
    if (promptContext.phase) {
        injections.push(createHintInjection('phase', `[CONVERSATION PHASE: ${promptContext.phase} - Focus: ${promptContext.topicContext || 'building rapport'}]`));
    }
    // -----------------------------------------------
    // RELATIONSHIP CONTEXT
    // -----------------------------------------------
    if (promptContext.relationshipContext && promptContext.isReturning) {
        injections.push(createHintInjection('relationship', `[RELATIONSHIP: ${promptContext.relationshipContext}]`));
    }
    return injections;
}
// ============================================================================
