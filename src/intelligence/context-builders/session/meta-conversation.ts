 ...
 */
function detectQuieterThanUsual(
  currentTurnCount: number,
  avgWordsThisSession: number,
  profile: UserProfile | null
): ConversationPattern | null {
  if (!profile || currentTurnCount < 4) return null;
  ...
  if (!historicalAvgWPM && !historicalPace) return null;
 ...
 */
function detectEnergyChange(
  currentEnergy: 'high' | 'medium' | 'low' | undefined,
  profile: UserProfile | null
): ConversationPattern | null {
  if (!profile || !currentEnergy) return null;

  const historicalEnergy = profile.currentEnergyLevel;
  if (!historicalEnergy) return null;
 ...
 */
function detectTopicRepetition(
  ...
  const historicalTopics = new Map<string, number>();
...
function detectOpeningUp(
  ...
  if (emotionalPatterns.length < 3) return null;

  const avgHistoricalIntensity =
    emotionalPatterns.reduce((sum, p) => sum + (p.intensity || 0), 0) / emotionalPatterns.length;
 ...
 */
const metaConversationBuilder: ContextBuilder = {
  ...
  build: async (input: ContextBuilderInput): Promise<ContextInjection[]> => {
    ...
    if (quieterPattern) patterns.push(quieterPattern);
    ...
    // 4. Opening up
    const openingPattern = detectOpeningUp(
      analysis.emotion.intensity || 0,
    ...
    const emotionalShiftPattern = detectEmotionalShift(
      analysis.emotion.primary,
      sessionEmotionalHistory
    );
    if (emotionalShiftPattern) patterns.push(emotionalShiftPattern);
    ...
    if (patterns.length > 0) {
      ...
      if (selectedPattern.confidence > 0.65 && Math.random() < 0.4) {
        injections.push(
          createHintInjection(
            'meta_conversation',
            `[META-AWARENESS: ${selectedPattern.observation}. Consider naturally saying: "${selectedPattern.reflection}"]`
          )
        );
