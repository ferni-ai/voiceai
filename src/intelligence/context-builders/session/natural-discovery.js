...
const VALUES_DISCOVERY_PROMPTS = [
    {
        topic: 'values',
...
const CAPACITY_DISCOVERY_PROMPTS = [
    {
        topic: 'capacity',
...
const RELATIONSHIP_DISCOVERY_PROMPTS = [
    {
        topic: 'relationships',
...
const GROWTH_CELEBRATION_PROMPTS = [
    {
        topic: 'growth',
...
// ============================================================================
function getDiscoveryState(input) {
    ...
    if (profile) {
        ...
        state.hasDreams = !!(humanMemory?.dreams && Array.isArray(humanMemory.dreams) && humanMemory.dreams.length > 0);
        // Values - check humanMemory
        state.hasValues = !!(humanMemory?.values && Array.isArray(humanMemory.values) && humanMemory.values.length > 0);
...
// PROMPT SELECTOR
// ============================================================================
function selectDiscoveryPrompts(state) {
    const prompts = [];
    ...
    if (!state.hasDreams) {
        const dreamPrompts = DREAM_DISCOVERY_PROMPTS.filter(p => {
            if (p.timing === 'early')
                return isEarly;
            if (p.timing === 'rapport_built')
                return rapportBuilt;
            if (p.timing === 'deep_conversation')
...
// FORMAT OUTPUT
// ============================================================================
function formatDiscoveryContext(prompts, state) {
    if (prompts.length === 0)
        return '';
    const lines = [
    ...
    const missing = [];
    if (!state.hasDreams)
        missing.push('what they dream about');
    if (!state.hasValues)
        missing.push('what they value most');
    if (!state.hasGoals)
        missing.push('what they\'re working toward');
    if (!state.hasCapacityData)
        missing.push('how full their plate is');
    if (!state.hasRelationshipData)
        missing.push('who the important people in their life are');
    if (missing.length > 0) {
...
// CONTEXT BUILDER
// ============================================================================
async function buildNaturalDiscoveryContext(input) {
    ...
    try {
        ...
        log.debug({ userId, promptCount: prompts.length, topics: prompts.map(p => p.topic) }, 'Natural discovery context built');
        return [{
...
//# sourceMappingURL=natural-discovery.js.map