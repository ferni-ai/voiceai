...
import { BuilderCategory, createHintInjection, createStandardInjection, registerContextBuilder, } from '../index.js';
...
// HELPERS
// ============================================================================
function userSharedSomethingExploreable(message) {
    return SHARING_PATTERNS.some((pattern) => pattern.test(message));
}
function isShortResponse(message) {
    const words = message.trim().split(/\s+/);
    return words.length <= 5 || SHORT_RESPONSE_PATTERNS.some((p) => p.test(message.trim()));
}
function hasOpenThread(topics) {
    return topics.some((topic) => CALLBACK_WORTHY_TOPICS.some((worthy) => topic.toLowerCase().includes(worthy)));
}
// ============================================================================
// CONVERSATION FORWARD BUILDER
// ============================================================================
async function buildConversationForwardContext(input) {
...
Don't let this topic drop without understanding it better.`;
    ...
    if (isShortResponse(userText) && turnCount > 3) {
        injections.push(createHintInjection('conversation_forward_short', `[SHORT RESPONSE - RE-ENGAGE]
...
Don't accept surface-level unless they clearly want to keep it light.`));
    ...
    if (turnCount >= 5 && turnCount % 5 === 0 && hasOpenThread(topics)) {
        injections.push(createHintInjection('conversation_forward_callback', `[CALLBACK OPPORTUNITY]
...
This shows you're paying attention across the whole conversation.`));
    ...
    if (turnCount >= 2 && Math.random() < 0.65) {
        ...
        if (endingGuidance) {
            injections.push(createHintInjection('conversation_forward_ending', endingGuidance, { category: 'guidance' }));
        }
    }
    // ============================================================================
    ...
    if (seemsWrappingUp && conversationWasShallow && !analysis?.intent?.primary?.includes('close')) {
        injections.push(createHintInjection('conversation_forward_prevent_fizzle', `[PREVENT EARLY END]
...
// ============================================================================
function getResponseEndingGuidance(intent, emotion, intensity) {
...
// CLEANUP
// ============================================================================
export function cleanupConversationForward() {
...
//# sourceMappingURL=conversation-forward.js.map