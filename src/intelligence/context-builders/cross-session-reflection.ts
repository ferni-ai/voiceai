...
} from '../cross-session-reflection.js';
 ...
 * Cross-Session Reflection Context Builder
 */
const crossSessionReflectionBuilder: ContextBuilder = {
  ...
  build: async (input: ContextBuilderInput): Promise<ContextInjection[]> => {
    ...
    if (userProfile && turnCount > 2) {
      const newMoment = detectReflectionMoment(
        userText,
        analysis.topics.primary || analysis.topics.detected[0] || 'this',
      ...
      if (newMoment) {
        ...
        log.debug(
          { type: newMoment.type, topic: newMoment.topic },
          'Detected reflection-worthy moment'
        );
      }
    }
    ...
    if (reflection) {
      injections.push(
        createStandardInjection(
          ...
          `[CROSS-SESSION REFLECTION: Something from your last conversation has stayed with you. Consider naturally weaving in: "${reflection.phrase}"]`
        )
      );
      ...
      log.info(
        ...
        'Injected cross-session reflection'
      );
    }
