 ...
 */
export function findCelebratableGrowth(humanMemory, currentTopic) {
    ...
    if (humanMemory.growthArc.challenges) {
        for (const challenge of humanMemory.growthArc.challenges) {
            if (challenge.status === 'breakthrough' || challenge.status === 'resolved') {
                const isRelevant = currentTopic && challenge.challenge.toLowerCase().includes(currentTopic.toLowerCase());
                insights.push({
 ...
 */
function generateGrowthPhrase(marker) {
    const phrases = [
...
//# sourceMappingURL=growth-celebration.js.map