 ...
 */
export function buildSuperhumanContext(profile, options = {}) {
    ...
    const comfortGuidance = getComfortGuidance(humanMemory, options.detectedEmotion, options.detectedStressLevel || 0);
    // Detect topic absences
    const topicAbsences = detectTopicAbsences(humanMemory, options.recentTopics || [], options.sessionCount || 0);
    // Get temporal context
    const temporalContext = getTemporalContext(humanMemory);
    // Build prompt injection
    const promptInjection = buildPromptInjection(allInsights, comfortGuidance, topicAbsences, temporalContext);
    return {
 ...
 */
function buildPromptInjection(insights, comfortGuidance, topicAbsences, temporalContext) {
    const sections = [];
