...
class VoiceEmotionOrchestratorImpl {
  ...
  analyze(
    sessionId: string,
    voiceInput: VoiceEmotionInput | undefined,
    textInput: TextEmotionInput,
    userText: string
    ...
    const { primary, source, confidence } = this.determinePrimaryEmotion(
      voiceInput,
      textInput,
      stressIndicators
    );
  ...
  private detectStressIndicators(
    voice: VoiceEmotionInput | undefined,
    userText: string
  ): VoiceEmotionAnalysis['stressIndicators'] {
    return {
   ...
   */
  private detectTremor(voice?: VoiceEmotionInput): boolean {
    if (!voice?.pitch) return false;
   ...
   */
  private detectRushed(voice?: VoiceEmotionInput, text?: string): boolean {
    // Voice-based detection
    if (voice?.speechRate && voice.speechRate > 200) return true;

    // Text-based detection
    if (text) {
      const rushPatterns =
        /\b(gotta go|quick question|running late|no time|hurry|briefly|short on time)\b/i;
      if (rushPatterns.test(text)) return true;
    }
   ...
   */
  private detectHesitation(text: string): boolean {
    const patterns = [
      /\.\.\./, // Ellipses
      /\bum+\b/i, // "um"
      /\buh+\b/i, // "uh"
  ...
  private detectAnxietyMarkers(voice?: VoiceEmotionInput, text?: string): boolean {
    ...
    if (text) {
      const anxietyPatterns =
        /\b(worried|anxious|nervous|stressed|panic|freaking out|can't stop thinking)\b/i;
      if (anxietyPatterns.test(text)) return true;
    }
  ...
  private mergeDistress(
    ...
    indicators: VoiceEmotionAnalysis['stressIndicators']
  ): number {
  ...
  private determinePrimaryEmotion(
    voice: VoiceEmotionInput | undefined,
    text: TextEmotionInput,
    _indicators: VoiceEmotionAnalysis['stressIndicators']
  ): { primary: string; source: 'voice' | 'text' | 'merged'; confidence: number } {
   ...
   */
  private mapVoiceEmotion(voiceEmotion: string): string {
  ...
  private generateGuidance(
    ...
    voice: VoiceEmotionInput | undefined,
    trend: 'improving' | 'stable' | 'declining'
  ): VoiceEmotionGuidance {
    const messages: string[] = [];
    const guidance: VoiceEmotionGuidance = {
      slowDown: false,
      addPauses: false,
      softenTone: false,
    ...
    if (indicators.hasTremor) {
      ...
      guidance.softenTone = true;
      messages.push(
        'Voice tremor detected - user may be emotionally affected. Slow down and be present.'
      );
    }
 ...
 */
export function detectEmotionSuppression(
  voice: VoiceEmotionInput | undefined,
  text: TextEmotionInput
): SuppressionResult {
 ...
 */
export function formatVoiceEmotionForPrompt(analysis: VoiceEmotionAnalysis): string {
