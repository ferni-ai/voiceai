...
export class FinancialJourneyTracker {
  ...
  createSnapshot(
    ...
    getLogger().info({ type, goalsAchieved, hasInvestments: profile.hasInvestments }, 
  ...
  getStartingPoint(): FinancialSnapshot | null {
    return this.snapshots.find(s => s.type === 'starting_point') || 
           this.snapshots[0] || 
           null;
  }
  ...
  detectMilestones(
    ...
    if (profile.hasInvestments && !previousSnapshot?.hasInvestments) {
      newMilestones.push(this.createMilestone(
        'started_investing',
    ...
    if (profile.financialSituation?.hasEmergencyFund && 
        current?.emergencyFundStatus === 'none') {
      newMilestones.push(this.createMilestone(
        'emergency_fund_complete',
    ...
    if (achievedGoals.length === 1 && 
        (!previousSnapshot || previousSnapshot.goalsAchieved === 0)) {
      newMilestones.push(this.createMilestone(
        'first_goal_achieved',
        'First Goal Achieved!',
