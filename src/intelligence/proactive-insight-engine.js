...
export class ProactiveInsightEngine {
    userId;
    generatedInsights = [];
    deliveredInsightIds = new Set();
    constructor(userId, existingInsights) {
        this.userId = userId;
        if (existingInsights) {
     ...
     */
    generateInsights(profile, patterns, responsePrefs) {
        const insights = [];
     ...
     */
    checkOverdueConversation(profile, patterns) {
        const daysSinceContact = Math.floor((Date.now() - new Date(profile.lastContact).getTime()) / (1000 * 60 * 60 * 24));
    ...
    checkGoals(profile) {
        ...
        for (const goal of profile.goals) {
            ...
            if (goal.targetDate) {
                const daysToTarget = Math.floor((new Date(goal.targetDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
            ...
            if (goal.status === 'achieved' && goal.updatedAt) {
                const daysSinceAchieved = Math.floor((Date.now() - new Date(goal.updatedAt).getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    checkKeyMoments(profile) {
        const insights = [];
        for (const moment of profile.keyMoments) {
            if (!moment.followUpNeeded)
                continue;
            const daysSinceMoment = Math.floor((Date.now() - new Date(moment.timestamp).getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    checkPatterns(profile, patterns) {
        const insights = [];
        ...
        if (patterns.topicsThatLeadToEngagement.length > 0) {
            ...
            const daysSinceLastContact = Math.floor((Date.now() - new Date(profile.lastContact).getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    checkSeasonalOpportunities(profile) {
        const insights = [];
        const now = new Date();
     ...
     */
    checkRelationshipMilestones(profile) {
        const firstContact = new Date(profile.firstContact);
        const monthsSinceFirst = Math.floor((Date.now() - firstContact.getTime()) / (1000 * 60 * 60 * 24 * 30));
        // Celebrate milestone months (6, 12, 24, etc.)
        const milestones = [6, 12, 24, 36, 48, 60];
        const milestone = milestones.find((m) => monthsSinceFirst >= m && monthsSinceFirst < m + 1);
        if (milestone) {
            const milestoneId = `relationship_${milestone}mo_${profile.id}`;
            if (!this.deliveredInsightIds.has(milestoneId)) {
                const years = milestone >= 12
                    ? `${Math.floor(milestone / 12)} year${milestone >= 24 ? 's' : ''}`
                    : null;
     ...
     */
    checkBehaviorPatterns(profile, responsePrefs) {
        const insights = [];
        // Only generate if we have enough data
        if (responsePrefs.totalSignals < 10)
            return insights;
        // Notice high-engagement topics the user might not realize
        if (responsePrefs.highEngagementTopics.length > 0) {
            const unexploredTopic = responsePrefs.highEngagementTopics.find((t) => !profile.preferredTopics.includes(t));
            if (unexploredTopic) {
                insights.push({
     ...
     */
    markDelivered(insightId, reaction) {
    ...
    getNextInsight() {
        ...
        const valid = undelivered.filter((i) => !i.expiresAt || new Date(i.expiresAt) > new Date());
        // Sort by priority
        valid.sort((a, b) => {
            const priorityOrder = { high: 0, medium: 1, low: 2 };
            return priorityOrder[a.priority] - priorityOrder[b.priority];
    ...
    getStats() {
        ...
        const negative = delivered.filter((i) => i.userReaction === 'negative' || i.userReaction === 'dismissed').length;
        return {
...
export function getProactiveInsightEngine(userId, existingInsights) {
...
export function removeProactiveInsightEngine(userId) {
    engines.delete(userId);
}
export default ProactiveInsightEngine;
//# sourceMappingURL=proactive-insight-engine.js.map