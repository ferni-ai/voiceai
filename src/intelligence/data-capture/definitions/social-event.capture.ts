...
const EVENT_TYPE_MAP: Record<string, { type: string; baseDuration: number }> = {
  // Large gatherings
...
export const socialEventCaptureDefinition: DataCaptureDefinition = {
  ...
  handler: async (
    ...
    let eventMapping: { type: string; baseDuration: number } | undefined;
    ...
    if (durationStr) {
      ...
      if (durationStr.includes('all day')) durationMinutes = 480;
      if (durationStr.includes('all evening')) durationMinutes = 240;
      if (durationStr.includes('all afternoon')) durationMinutes = 240;
    }
    ...
    try {
      ...
      log.info(
        { type: eventMapping.type, duration: durationMinutes, userId: context.userId },
        'Captured social event from conversation'
      );
