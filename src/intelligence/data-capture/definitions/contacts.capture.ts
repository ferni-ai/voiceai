...
const RELATIONSHIP_MAP: Record<string, { relationship: string; label: string }> = {
...
export const contactCaptureDefinition: DataCaptureDefinition = {
  ...
  handler: async (
    ...
    const { nameOrRelation, phone, email } = extractedArgs as {
    ...
    try {
      ...
      const existingResult = existingResults.find(
        (r) => r.contact.displayName?.toLowerCase() === displayName.toLowerCase()
      );
      ...
      if (existingResult) {
        ...
        if (Object.keys(updates).length > 0) {
          ...
          log.info(
            { contactId: existingResult.contact.id, updates, userId: context.userId },
            'Updated existing contact with new info'
          );
      ...
      log.info(
        { contactId: newContact.id, name: displayName, userId: context.userId },
        'Created new contact from passive capture'
      );
