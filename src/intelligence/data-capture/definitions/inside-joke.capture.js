...
export const insideJokeCaptureDefinition = {
    ...
    handler: async (extractedArgs, context) => {
        ...
        else if (lowerTranscript.includes("there you go again") ||
        ...
        // Check for callbacks
        else if (momentTypeHint.includes('remember') ||
        ...
        // Check for laughter
        else if (lowerTranscript.includes('haha') ||
        ...
        try {
            const { recordSharedMoment } = await import('../../../services/superhuman/inside-joke-memory.js');
            await recordSharedMoment(context.userId, momentType, essence.slice(0, 150), transcript.slice(0, 300), triggerKeywords, callbackPhrase);
            log.info({ type: momentType, essence: essence.slice(0, 50), userId: context.userId }, 'Captured shared moment from conversation');
...
//# sourceMappingURL=inside-joke.capture.js.map