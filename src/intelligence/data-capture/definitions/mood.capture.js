...
export const moodCaptureDefinition = {
    id: 'capture_mood',
    name: 'Mood Capture',
    ...
    handler: async (extractedArgs, context) => {
        const rawMood = String(extractedArgs.mood || '')
            .toLowerCase()
            .trim();
        const hasIntensifier = String(extractedArgs.intensity || '')
            .toLowerCase()
            .includes('so') ||
        ...
        try {
            ...
            await recordMoodEntry(context.userId, moodMapping.mood, intensity, moodContext || undefined, rawMood ? [rawMood] : undefined);
            log.info({ mood: moodMapping.mood, intensity, userId: context.userId }, 'Captured mood entry from conversation');
...
//# sourceMappingURL=mood.capture.js.map