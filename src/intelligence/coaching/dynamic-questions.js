...
import { PERSONA_TRAIT_PROFILES } from '../../personas/dynamic-responses.js';
const log = getLogger();
// ============================================================================
 ...
 */
export function clearQuestionHistory(sessionId) {
 ...
 */
function getPersonaFilteredQuestion(context, type) {
    ...
    return {
        ...
        intent: {
            ...
            buildsOnContext: context.recentTopics.length > 0 ? `Relates to: ${context.recentTopics[0]}` : undefined,
        },
        followUpStrategy: {
 ...
 */
function buildQuestionGenerationPrompt(context, type, cognitiveDiff) {
    const traits = PERSONA_TRAIT_PROFILES[context.personaId];
    ...
    if (questioningStyle) {
        ...
        if (questioningStyle.questionStarters.length > 0) {
            voiceConstraints.push(`Use starters like: "${questioningStyle.questionStarters.slice(0, 3).join('", "')}"`);
        }
        if (questioningStyle.avoidQuestions.length > 0) {
            voiceConstraints.push(`NEVER ask questions like: "${questioningStyle.avoidQuestions.join('", "')}"`);
        }
    }
    if (traits) {
        if (traits.warmth > 0.7)
            voiceConstraints.push('Use warm, caring tone');
        if (traits.directness > 0.6)
            voiceConstraints.push('Be direct, not overly soft');
        if (traits.formality > 0.6)
            voiceConstraints.push('Keep professional tone');
        if (traits.formality < 0.4)
            voiceConstraints.push('Keep casual, conversational');
    }
    ...
    if (context.emotionalState) {
        contextLines.push(`Emotional state: ${context.emotionalState.primary} (intensity: ${context.emotionalState.intensity})`);
    }
 ...
 */
export async function generateQuestion(context, type, options = {}) {
 ...
 */
export async function getSilenceQuestion(context, options) {
    // Determine the best question type based on silence context
    let type = 'silence_break';
 ...
 */
export async function getCheckInQuestion(context, options) {
    return generateQuestion(context, 'checking_in', options);
}
/**
 * Convenience function: Get a deepening question
 */
export async function getDeepeningQuestion(context, options) {
 ...
 */
export function getPersonaQuestionCapabilities(personaId) {
