 ...
 */
export async function loadCoachingMemories(userId, personaId) {
    const memories = [];
    const now = new Date();
    try {
        ...
        if (voiceMemory) {
            ...
            for (const topic of voiceMemory.topics.slice(0, 10)) {
                const daysAgo = Math.floor((now.getTime() - topic.lastMentioned.getTime()) / (1000 * 60 * 60 * 24));
                memories.push({
                    topic: topic.topic,
                    daysAgo,
            ...
            for (const milestone of voiceMemory.milestones.slice(0, 3)) {
                const daysAgo = Math.floor((now.getTime() - milestone.date.getTime()) / (1000 * 60 * 60 * 24));
                memories.push({
    ...
    try {
        ...
        if (personaMemories.length > 0) {
            for (const mem of personaMemories.slice(0, 10)) {
                const daysAgo = Math.floor((now.getTime() - mem.createdAt.getTime()) / (1000 * 60 * 60 * 24));
                memories.push({
                    topic: mem.name,
                    daysAgo,
 ...
 */
export async function getMemoriesForTopic(userId, topic) {
    ...
    return context.memories.filter((m) => {
        ...
        return (memTopic.includes(topicLower) ||
 ...
 */
export async function getSuggestedFollowUps(userId, currentTopic) {
    const context = await loadCoachingMemories(userId, 'ferni');
    const suggestions = [];
...
async function loadVoiceConversationMemory(userId) {
    try {
        // Dynamic import to avoid circular dependencies
        const { getUserMemory, getConversationContext } = await import('../../services/memory/voice-conversation-memory.js');
        ...
        return {
            ...
            milestones: memory.relationshipMilestones?.map((m) => ({
                name: m.milestone,
                date: m.date,
            })) || [],
...
async function loadPersonaMemories(userId, _personaId) {
    try {
        // Dynamic import to avoid circular dependencies
