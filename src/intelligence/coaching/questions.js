...
import { generateQuestion, } from './dynamic-questions.js';
const log = getLogger();
// ============================================================================
// PATTERN TEMPLATES
// ============================================================================
...
const PATTERN_TEMPLATES = [
    ...
    },
    {
        ...
        detector: (ctx) => ctx.recentTopics.length > 2 &&
 ...
 */
export async function generateMemoryGroundedQuestion(context, memories, options) {
    log.info({ personaId: context.personaId, memoryCount: memories.length }, '🧠 COACHING: Generating memory-grounded question');
 ...
 */
export function detectPatterns(context) {
    const patterns = [];
 ...
 */
export async function generatePatternQuestion(context, options) {
    ...
    log.info({ personaId: context.personaId, pattern: pattern.pattern }, '🧠 COACHING: Generating pattern-surfacing question');
 ...
 */
export function generateMirror(transcript) {
 ...
 */
export async function getCoachingQuestion(context, options = {}) {
    log.info({
...
// HELPERS
// ============================================================================
function findRelevantMemory(context, memories) {
    if (memories.length === 0)
        return null;
    // Check if any memory relates to current topics
    for (const memory of memories) {
        for (const topic of context.recentTopics) {
            if (memory.topic.toLowerCase().includes(topic.toLowerCase()) ||
