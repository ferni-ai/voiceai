 ...
 */
export function detectPatternsInTranscript(transcript, topic, emotion) {
    const detected = [];
    ...
    // Deflection patterns
    if (lowerTranscript.includes('haha') ||
    ...
    if (lowerTranscript.includes("i'm busy") ||
 ...
 */
function extractMentionedPeople(transcript) {
    const people = [];
 ...
 */
export async function recordPattern(observation) {
 ...
 */
export async function getPatternsToSurface(userId) {
    ...
    return allPatterns.filter((p) => {
        ...
        if (p.surfacedToUser && p.surfacedAt && p.surfacedAt > oneWeekAgo)
            return false;
        // Don't resurface dismissed patterns
        if (p.userReaction === 'dismissed')
            return false;
        return true;
    });
}
 ...
 */
export async function markPatternSurfaced(patternId, reaction) {
    const db = getFirestore();
    if (!db)
        return;
    try {
 ...
 */
export function generatePatternSurfacingQuestion(pattern) {
    const questions = {
        recurring_topic: (p) => [
 ...
 */
export async function processTranscriptForPatterns(userId, transcript, topic, emotion) {
 ...
 */
export async function getPatternForSilence(userId) {
