...
let PERSONALITY_CONNECTORS = null;
async function loadPersonalityBuildingBlocks() {
    if (PERSONALITY_CONNECTORS)
        return;
    try {
        ...
        if ('CONNECTORS' in personality) {
            PERSONALITY_CONNECTORS = personality.CONNECTORS;
 ...
 */
function getTimeMultiplier(hourOfDay) {
    if (hourOfDay === undefined)
        return 1.0;
    if (hourOfDay >= 23 || hourOfDay < 5)
        return 1.4; // Late night: 40% slower
    if (hourOfDay >= 6 && hourOfDay < 9)
        return 1.1; // Early morning: 10% slower
    return 1.0;
}
/**
 ...
 */
function selectPhrase(type, weight) {
 ...
 */
function getSSMLWrapper(emotionalState) {
    if (!emotionalState)
        return '';
 ...
 */
function calculatePauses(weight, hourOfDay, relationshipStage) {
 ...
 * ```
 */
export function composeProcessingExpression(ctx) {
 ...
 */
export function formatProcessingAsSSML(result) {
 ...
 */
export function getToolCallProcessing(toolName, weight = 'light') {
 ...
 */
export function getEmotionalProcessing(emotionalState, relationshipStage) {
    // Determine weight based on intensity
    const weight = emotionalState.intensity > 0.7 ? 'heavy' : emotionalState.intensity > 0.4 ? 'medium' : 'light';
    return composeProcessingExpression({
        trigger: 'emotional',
        weight,
        emotionalState,
 ...
 */
export function getThinkingProcessing(weight = 'medium', hourOfDay) {
    return composeProcessingExpression({
        trigger: 'thinking',
        weight,
 ...
 */
export function getMemoryRecallProcessing(weight = 'medium') {
    return composeProcessingExpression({
 ...
 */
export function getAfterToolResultProcessing(weight = 'light', personaId) {
    if (personaId) {
 ...
 */
export function getContextLoadingProcessing(weight = 'light') {
    return composeProcessingExpression({
 ...
 */
export function composePersonaProcessingExpression(ctx) {
 ...
 */
export function getProcessingPhrase(type, weight) {
 ...
 */
export function getProcessingPhraseWithSSML(type, weight, options) {
    const result = composeProcessingExpression({
        trigger: type,
        weight,
...
//# sourceMappingURL=processing-intelligence.js.map