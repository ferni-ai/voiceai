 ...
 */
async function runPatternRecomputation() {
 ...
 * Run every hour
 */
async function runEvolutionUpdate() {
    const startTime = Date.now();
    try {
        const insights = getCommunityInsights();
 ...
 * Run every hour
 */
async function runStoryAnalysis() {
    const startTime = Date.now();
    try {
        const insights = getCommunityInsights();
 ...
 */
export function startCollectiveLearningScheduler() {
    if (isRunning) {
    ...
    registerInterval('collective-learning-pattern-recompute', () => {
        void runPatternRecomputation();
    }, 15 * 60 * 1000);
    recomputeIntervalId = 1; // Marker
    // Firestore save: every 10 minutes
    registerInterval('collective-learning-firestore-save', () => {
        void saveCommunityInsightsToFirestore();
    }, 10 * 60 * 1000);
    saveIntervalId = 1; // Marker
    // Story analysis: every hour
    registerInterval('collective-learning-story-analysis', () => {
        void runStoryAnalysis();
    }, 60 * 60 * 1000);
    // Evolution update: every hour
    registerInterval('collective-learning-evolution-update', () => {
 ...
 */
export function stopCollectiveLearningScheduler() {
    if (!isRunning) {
        return;
    }
 ...
 */
export async function forceRunAllJobs() {
