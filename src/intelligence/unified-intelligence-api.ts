...
import {
  ...
  markInsightSurfaced as markSurfaced,
  recordInsightReaction as recordReaction,
  wasInsightSurfaced as wasSurfaced,
  getInsightReaction as getReaction,
...
export async function getIntelligenceForTurn(
  ...
  const correlations = getRelevantCorrelations(userId, {
    ...
    currentDomains: context.activeDomains as CorrelationDomain[],
  });
  ...
  if (correlationStr) sections.push(correlationStr);
  ...
  if (proactiveResult.insights.length > 0) {
    ...
    sections.push(
      `[PROACTIVE] Ready to surface: ${topInsight.category} (priority ${topInsight.priority})`
    );
  }
 ...
 */
export function getAllCorrelations(userId: string): CrossDomainCorrelation[] {
 ...
 */
export function clearIntelligenceCaches(userId?: string): void {
  clearContextCache(userId);
 ...
 */
export async function getTopProactiveInsight(
  userId: string,
  moment: SurfaceMoment
): Promise<ProactiveIntelligenceInsight | null> {
  const context = await assembleContext({ userId });
  const result = checkProactiveTriggers(userId, context, moment);
