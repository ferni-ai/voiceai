...
export type GoalCategory =
  | 'career' // Work, job, professional growth
  | 'relationship' // Love, connection, family
  | 'self' // Identity, values, personal growth
  | 'health' // Physical, mental, spiritual wellbeing
  | 'financial' // Money, security, freedom
  | 'creative' // Expression, art, purpose
...
const profiles = new Map<string, SubconsciousProfile>();

/**
 * Get or create subconscious profile
 */
export function getSubconsciousProfile(userId: string): SubconsciousProfile {
 ...
 */
export function analyzeSubconscious(
  userId: string,
  text: string,
  topics: string[],
  emotionIntensity: number
  ...
  for (const { patterns, category, goalTemplate } of DESIRE_PATTERNS) {
    for (const pattern of patterns) {
      ...
      if (match) {
        ...
        const existingDesire = profile.emergingDesires.find(
          (d) => d.category === category && calculateSimilarity(d.goal, goalTemplate) > 0.5
        );
        ...
        if (existingDesire) {
          ...
          // New desire detected
          const newDesire: EmergingDesire = {
  ...
  for (const pattern of FANTASY_PATTERNS) {
    ...
    if (match) {
      ...
      // Determine category
      let category: GoalCategory = 'lifestyle';
      if (/job|work|career/i.test(text)) category = 'career';
      if (/money|rich|afford/i.test(text)) category = 'financial';
      if (/love|relationship|partner/i.test(text)) category = 'relationship';
  ...
  if (questionMatch) {
    ...
    const existing = profile.unresolvedQuestions.find(
      (q) => calculateSimilarity(q.question, questionText) > 0.7
    );
  ...
  profile.metadata.confidence = Math.min(
    0.9,
    profile.emergingDesires.length * 0.1 +
 ...
 */
function refineGoal(template: string, fullText: string, match: string): string {
 ...
 */
function inferThemeMeaning(theme: string): string {
 ...
 */
function inferQuestionMeaning(question: string): string {
 ...
 */
function checkSurfaceOpportunity(
  profile: SubconsciousProfile,
  currentTopics: string[],
  emotionIntensity: number
): SubconsciousAnalysis['surfaceOpportunity'] {
  // Look for desires ready to surface
  const readyDesires = profile.emergingDesires.filter(
    (d) =>
      d.signalCount >= 3 &&
      d.confidence > 0.6 &&
  ...
  const matchingDesire = readyDesires.find((d) =>
    currentTopics.some(
      (t) =>
        d.category.toLowerCase().includes(t.toLowerCase()) ||
 ...
 */
function generateSurfacePhrase(desire: EmergingDesire): string {
 ...
 */
export function recordSurfaceReaction(
  userId: string,
  desireId: string,
  reaction: 'resonated' | 'rejected' | 'considered'
): void {
  const profile = getSubconsciousProfile(userId);
 ...
 */
export function formatSubconsciousForPrompt(
  userId: string,
 ...
 */
export function getSubconsciousSummary(userId: string): string | null {
  const profile = getSubconsciousProfile(userId);
