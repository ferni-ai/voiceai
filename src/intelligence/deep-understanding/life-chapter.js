 ...
 */
export function getChapterProfile(userId) {
 ...
 */
export function analyzeChapter(userId, text, topics, emotions) {
    ...
    for (const [chapterType, signals] of Object.entries(CHAPTER_SIGNALS)) {
        ...
        const matchingEmotions = signals.emotionalSignatures.filter((e) => emotions.some((ue) => ue.toLowerCase().includes(e.toLowerCase())));
 ...
 */
function updateChapterAssessment(profile) {
    ...
    if (sortedChapters.length > 0) {
        ...
        profile.chapter.current.chapter = topChapter;
        ...
        if (previousChapter !== topChapter && previousChapter !== 'unknown') {
            profile.chapter.transition.leaving = previousChapter;
            profile.chapter.transition.entering = topChapter;
    ...
    const currentChapterStart = profile.history.find((h) => h.chapter === profile.chapter.current.chapter && !h.exited)?.entered;
    if (currentChapterStart) {
        profile.chapter.current.durationEstimate = Math.floor((Date.now() - currentChapterStart.getTime()) / (1000 * 60 * 60 * 24 * 30));
    }
}
/**
 * Detect transition state
 */
function detectTransitionState(profile, text) {
 ...
 */
function identifyChapterNeeds(profile, text, emotions) {
 ...
 */
function buildChapterGuidance(profile) {
    const { chapter } = profile;
    const chapterApproaches = {
 ...
 */
function generateNarrativeInsight(profile, newEvidence) {
    // Only generate insight if we have enough confidence and evidence
    if (profile.chapter.current.confidence < 0.6 ||
 ...
 */
export function formatChapterForPrompt(analysis) {
 ...
 */
export function importChapterProfile(profile) {
 ...
 */
export function resetLifeChapterAwareness() {
    profiles.clear();
}
// ============================================================================
// EXPORTS
...
//# sourceMappingURL=life-chapter.js.map