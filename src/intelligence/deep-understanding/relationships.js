 ...
 */
export function getRelationalNetwork(userId) {
...
function createEmptyNetwork(userId) {
    return {
        userId,
        people: new Map(),
        triangulations: [],
 ...
 */
export function extractPersonMentions(text, emotion, emotionIntensity) {
    const mentions = [];
    ...
    while ((nameMatch = namePattern.exec(text)) !== null) {
        ...
        if (['Monday', 'Tuesday', 'God', 'Lord'].includes(name))
            continue;
 ...
 */
export function recordPersonMention(userId, mention) {
    ...
    const toneDiff = Math.abs(getEmotionValence(mention.emotionalTone) - getEmotionValence(person.emotionalTone.typical));
    ...
    log.debug({ userId, personName: person.name, role: person.specificRole }, '👤 Person mention recorded');
    return person;
}
/**
 * Find person by name or role
 */
function findPersonByNameOrRole(network, name, role) {
 ...
 */
function updateRelationshipQuality(person, mention) {
    ...
    if (getEmotionValence(person.emotionalTone.typical) < -0.3 &&
 ...
 */
export function detectUnspokenTension(userId, text, mentionedPerson, topics) {
    const hasTensionPattern = TENSION_PATTERNS.some((p) => p.test(text));
    if (!hasTensionPattern || !mentionedPerson)
        return null;
    const network = getRelationalNetwork(userId);
    // Check if we already have this tension
    const existing = network.unspokenTensions.find((t) => t.personId === mentionedPerson.id && topics.some((topic) => t.topic.includes(topic)));
    if (existing) {
    ...
    log.info({ userId, personName: mentionedPerson.name, topic: tension.topic }, '🔒 Unspoken tension detected');
    return tension;
}
/**
 * Generate suggestion for approaching a tension
 */
function generateApproachSuggestion(person, topic) {
 ...
 */
export function analyzeSupportNetwork(userId) {
    const network = getRelationalNetwork(userId);
    // Reset
    ...
    const hasCloseFamily = network.supportNetwork.innerCircle.some((id) => {
    ...
    const qualityBonus = [...network.people.values()].filter((p) => p.quality === 'supportive').length * 0.05;
    network.supportNetwork.health.score = Math.max(0, Math.min(1, innerCircleScore + outerCircleScore - gapPenalty + qualityBonus));
    ...
    if (network.supportNetwork.gaps.length > 0) {
        network.supportNetwork.health.vulnerabilities.push(...network.supportNetwork.gaps.map((g) => g.role));
    }
 ...
 */
export function generateRelationalInsights(userId) {
    const network = getRelationalNetwork(userId);
    const insights = [];
    // Pattern: Frequently mentioned but complicated
    for (const person of network.people.values()) {
        if (person.mentionFrequency > 0.3 &&
 ...
 */
export function formatRelationalInsightsForPrompt(userId, currentPersonMentioned) {
 ...
 */
export function importRelationalNetwork(network) {
 ...
 */
export function resetRelationalNetwork() {
    networks.clear();
}
// ============================================================================
// EXPORTS
