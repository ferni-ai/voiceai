 ...
 */
export function getHopeProfile(userId) {
 ...
 */
export function analyzeHope(userId, sessionId, text, topics, emotions, stressLevel) {
    ...
    const hopeScore = indicators.futureOrientation * 0.25 +
    ...
    log.debug({ userId, hopeScore: hopeScore.toFixed(2), direction: profile.trajectory.trend.direction }, '🌱 Hope trajectory updated');
    return {
        observation,
 ...
 */
function calculateIndicatorScore(text, patterns) {
    let matches = 0;
 ...
 */
function calculateCopingScore(text) {
 ...
 */
function calculatePerspectiveScore(text) {
 ...
 */
function updateTrajectory(profile, observation) {
    ...
    const resilienceScore = observation.resilience.bounceBackSpeed * 0.4 +
    ...
    if (observations.length >= 3) {
        ...
        if (older.length > 0) {
            ...
            if (change > 0.1)
                trajectory.trend.direction = 'improving';
            else if (change < -0.1)
                trajectory.trend.direction = 'declining';
            else
                trajectory.trend.direction = 'stable';
            // Calculate volatility
            const variance = recent.reduce((s, o) => s + Math.pow(o.hopeScore - recentAvg, 2), 0) / recent.length;
 ...
 * Update intervention guidance
 */
function updateInterventionGuidance(trajectory, observation) {
    ...
    else if (trajectory.trend.direction === 'declining' ||
 ...
 * Update hope anchors
 */
function updateAnchors(profile, observation) {
    ...
    if (observation.hopeScore > 0.6) {
        for (const topic of observation.context.topicsDuring) {
            const existing = anchors.sources.find((s) => s.description.toLowerCase().includes(topic.toLowerCase()));
            if (existing) {
    ...
    if (observation.hopeScore < 0.4) {
        for (const topic of observation.context.topicsDuring) {
            const existing = anchors.threats.find((t) => t.description.toLowerCase().includes(topic.toLowerCase()));
            if (existing) {
 ...
 */
function updatePatterns(profile, observation) {
    const alpha = 0.1;
    // Update baseline
 ...
 */
function generateAlerts(profile, observation) {
    const alerts = [];
    ...
    if (profile.trajectory.trend.direction === 'declining' &&
 ...
 */
function buildHopeGuidance(profile, observation, alerts) {
    ...
    if (profile.trajectory.anchors.sources.length > 0) {
        reinforce.push(`Hope sources: ${profile.trajectory.anchors.sources.map((s) => s.description).join(', ')}`);
    }
 ...
 */
export function formatHopeForPrompt(analysis) {
    const lines = ['[HOPE TRAJECTORY AWARENESS]'];
    // Current state
    lines.push(`Hope level: ${(analysis.trajectory.current.hopeLevel * 100).toFixed(0)}% | Trend: ${analysis.trajectory.trend.direction}`);
    // Urgency
 ...
 */
export function importHopeProfile(profile) {
 ...
 */
export function resetHopeTrajectory() {
    profiles.clear();
}
// ============================================================================
// EXPORTS
