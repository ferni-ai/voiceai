 ...
 */
export function analyzeSilence(durationMs, precedingText, precedingEmotion, precedingEmotionIntensity, precedingTopics, wasQuestion, userPatterns) {
    // Detect context flags
    const isVulnerable = VULNERABLE_PATTERNS.some((p) => p.test(precedingText));
    const isHeavy = HEAVY_TOPICS.some((t) => precedingTopics.some((pt) => pt.toLowerCase().includes(t))) ||
    ...
    if (userPatterns && userPatterns.observationCount > 5) {
        ...
        if (Math.abs(durationMs - userPatterns.avgProcessingDuration) <
    ...
    // Determine response
    const { response, waitDuration, prompt } = determineSilenceResponse(type, durationMs, isVulnerable, isHeavy, wasQuestion, userPatterns);
    // Build guidance
 ...
 */
function determineSilenceResponse(type, durationMs, isVulnerable, isHeavy, wasQuestion, userPatterns) {
 ...
 */
function buildSilenceGuidance(type, precedingText, topics, isVulnerable) {
    const guidance = [];
    switch (type) {
        case 'processing':
 ...
 */
export function getSilencePattern(userId) {
    return userPatterns.get(userId);
}
/**
 * Record a silence observation
 */
export function recordSilence(userId, analysis, howBroken) {
    ...
    // Track heavy topics
    if (analysis.precedingContent.wasHeavy &&
 ...
 */
export function formatSilenceForPrompt(analysis) {
 ...
 */
export function importSilencePattern(pattern) {
 ...
 */
export function resetSilenceIntelligence() {
    userPatterns.clear();
}
// ============================================================================
// EXPORTS
