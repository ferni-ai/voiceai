 ...
 */
export function getResistanceProfile(userId) {
...
function createEmptyProfile(userId) {
    ...
    const patterns = {};
    for (const pattern of Object.keys(DEFENSE_PATTERNS)) {
        patterns[pattern] = {
            frequency: 0,
            contexts: [],
            effectiveness: 0.5,
 ...
 */
export function analyzeResistance(userId, text, emotion, emotionIntensity, currentTopics, previousTopic) {
    const profile = getResistanceProfile(userId);
    const defensesDetected = [];
    ...
    let avoidedTopic = null;
    let isDeflecting = false;
    let deflectingFrom = null;
    ...
    for (const trigger of AVOIDANCE_TRIGGERS) {
        if (trigger.pattern.test(text)) {
            const { topic } = trigger;
            const existing = profile.avoidedTopics.find((t) => t.topic.toLowerCase().includes(topic.toLowerCase()));
    ...
    // Determine approach
    const approach = determineApproach(resistanceLevel, defensesDetected.map((d) => d.pattern), readinessSignals.length > 0, emotionIntensity);
    // Update metadata
 ...
 */
function findOrCreateAvoidedTopic(profile, topic) {
    let existing = profile.avoidedTopics.find((t) => t.topic.toLowerCase().includes(topic.toLowerCase()) ||
 ...
 */
function updateProfile(profile, defenses, contexts, resistanceLevel) {
    const alpha = 0.15;
 ...
 */
function determineApproach(resistanceLevel, defenses, hasReadinessSignals, emotionIntensity) {
 ...
 */
export function identifyGrowthEdges(userId) {
    const profile = getResistanceProfile(userId);
    const edges = [];
 ...
 */
function generateEntryPoint(topic) {
    const entryPoints = {
 ...
 */
function generateAvoidPhrases(topic) {
    const baseAvoid = [
 ...
 */
export function formatResistanceForPrompt(analysis) {
 ...
 */
export function getResistanceSummary(userId) {
    const profile = getResistanceProfile(userId);
 ...
 */
export function importResistanceProfile(profile) {
 ...
 */
export function resetResistanceDetection() {
    profiles.clear();
}
// ============================================================================
// EXPORTS
