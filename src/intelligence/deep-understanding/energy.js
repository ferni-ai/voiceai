 ...
 */
export function getEnergyPattern(userId) {
 ...
 */
function getExpectedEnergy(hour) {
    if (hour >= 6 && hour < 10)
        return 'moderate'; // Morning ramp up
    if (hour >= 10 && hour < 12)
        return 'high'; // Morning peak
    if (hour >= 12 && hour < 14)
        return 'moderate'; // Post-lunch dip
    if (hour >= 14 && hour < 17)
        return 'moderate'; // Afternoon
    if (hour >= 17 && hour < 20)
        return 'moderate'; // Early evening
    if (hour >= 20 && hour < 23)
 ...
 */
export function assessEnergyState(userId, text, voiceData, currentTopics, sessionTurnCount) {
    ...
    if (hour < 12) {
        ...
        if (physicalScore < 0.3)
            sleepQuality = 'poor';
        else if (physicalScore < 0.5)
            sleepQuality = 'fair';
        else
            sleepQuality = 'good';
    }
    ...
    else if (mentalScore < 0.4)
        capacity = 'limited';
    else if (mentalScore > 0.7)
        capacity = 'sharp';
    else if (mentalScore > 0.85 && engagementSignals.length >= 2)
        capacity = 'flow';
    // Emotional reserve
    const emotionalReserve = calculateEmotionalReserve(physicalScore, mentalScore, currentTopics, pattern);
    const mental = {
        capacity,
        decisionFatigue: {
    ...
    const overall = {
        ...
        optimalResponseLength: overallScore < 0.35
            ? 'brief'
            : overallScore > 0.65
                ? 'detailed'
                : 'normal',
 ...
 */
function calculateEmotionalReserve(physicalScore, mentalScore, currentTopics, pattern) {
    let reserve = (physicalScore + mentalScore) / 2;
    // Check if current topics are draining
    const drainingTopicsPresent = currentTopics.filter((t) => pattern.drainingTopics.some((d) => t.toLowerCase().includes(d.toLowerCase())));
    reserve -= drainingTopicsPresent.length * 0.1;
    // Check if current topics are energizing
    const energizingTopicsPresent = currentTopics.filter((t) => pattern.energizingTopics.some((e) => t.toLowerCase().includes(e.toLowerCase())));
 ...
 */
function generateApproach(physicalLevel, mentalCapacity, canHandleHeavy) {
 ...
 */
function recordEnergyObservation(userId, energyScore, hour, dayOfWeek, topics) {
    const pattern = getEnergyPattern(userId);
    const alpha = 0.2;
    // Update time pattern
 ...
 */
export function markTopicEnergy(userId, topic, effect) {
    const pattern = getEnergyPattern(userId);
    if (effect === 'draining') {
        if (!pattern.drainingTopics.includes(topic)) {
            pattern.drainingTopics.push(topic);
            if (pattern.drainingTopics.length > 10)
                pattern.drainingTopics.shift();
        }
    }
    else {
        if (!pattern.energizingTopics.includes(topic)) {
            pattern.energizingTopics.push(topic);
            if (pattern.energizingTopics.length > 10)
                pattern.energizingTopics.shift();
        }
    }
}
// ============================================================================
 ...
 */
export function formatEnergyForPrompt(assessment) {
 ...
 */
export function importEnergyPattern(pattern) {
 ...
 */
export function resetEnergyStateInference() {
    userPatterns.clear();
}
// ============================================================================
// EXPORTS
