 ...
 */
export function getLifeRhythmProfile(userId) {
 ...
 */
export function predictUserState(userId, targetTime = new Date()) {
    ...
    const likelyMood = moodScore < 0.35 ? 'low' : moodScore > 0.65 ? 'elevated' : 'neutral';
    const likelyEnergy = energyScore < 0.35 ? 'depleted' : energyScore > 0.65 ? 'high' : 'normal';
    const supportNeedLevel = supportNeed > 0.6 ? 'proactive' : supportNeed > 0.35 ? 'available' : 'minimal';
    ...
    if (supportNeed > 0.6)
        ...
        tone = 'celebrating';
    else if (supportNeed > 0.4)
        tone = 'checking_in';
    // Generate opener
    const suggestedOpener = generateOpener(reasons, tone, profile);
    // Calculate confidence based on data quality
    const confidence = Math.min(profile.dataQuality.weeklyConfidence, profile.dataQuality.monthlyConfidence * 0.8, 0.9);
    return {
        targetTime,
        prediction: {
            likelyMood,
 ...
 */
function findBestTime(profile, aroundTime) {
 ...
 */
function generateOpener(reasons, tone, profile) {
    // Find the most specific reason
    const anniversaryReason = reasons.find((r) => r.includes('anniversary'));
    const dayReason = reasons.find((r) => r.includes('Monday') || r.includes('Sunday') || r.includes('weekend'));
    const seasonalReason = reasons.find((r) => r.includes('seasonal') || r.includes('Holiday') || r.includes('winter'));
 ...
 */
export function recordConversationObservation(userId, observation) {
    const profile = getLifeRhythmProfile(userId);
    ...
    if (dayOfWeek === 0 || dayOfWeek === 6) {
        ...
        if (observation.topics.some((t) => /family|parent|sibling/i.test(t)) &&
    ...
    log.debug({ userId, day: dayOfWeek, mood: observation.mood }, '📅 Life rhythm observation recorded');
}
/**
 * Add an anniversary date
 */
export function addAnniversary(userId, anniversary) {
    const profile = getLifeRhythmProfile(userId);
    // Check for duplicate
    const exists = profile.anniversaries.some((a) => a.date.month === anniversary.date.month && a.date.day === anniversary.date.day);
    if (!exists) {
        profile.anniversaries.push(anniversary);
        log.info({ userId, type: anniversary.type, description: anniversary.description }, '📆 Anniversary recorded');
    }
}
/**
 * Format prediction for prompt injection
 */
export function formatPredictionForPrompt(prediction) {
 ...
 */
export function importLifeRhythmProfile(profile) {
 ...
 */
export function resetLifeRhythmPrediction() {
    userProfiles.clear();
}
// ============================================================================
// EXPORTS
...
//# sourceMappingURL=life-rhythm.js.map