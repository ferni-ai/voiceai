 ...
 */
export function getFlowProfile(userId) {
 ...
 */
export function analyzeFlow(userId, sessionId, text, turnCount, emotionIntensity, voiceData) {
    ...
    const { direction, reasoning } = determineDirection(state, signals, indicators, profile, turnCount);
 ...
 */
function calculateDepthIndicators(text, voiceData) {
    const words = text.split(/\s+/);
    const sentences = text.split(/[.!?]+/).filter(Boolean);
    // Content analysis
    const personalPronouns = (text.match(/\b(i|me|my|myself|i'm|i've|i'll|i'd)\b/gi) || []).length / words.length;
    const emotionalWords = (text.match(/\b(feel|felt|feeling|happy|sad|angry|scared|anxious|excited|hurt|love|hate|afraid|worried|stressed)\b/gi) || []).length / words.length;
    const vulnerableTopics = VULNERABLE_TOPIC_PATTERNS.some((p) => p.test(text));
    // Specificity (presence of specific details)
    const hasSpecifics = /\b(yesterday|last\s+\w+|on\s+\w+day|\d+\s+(years?|months?|days?|hours?)|when\s+i\s+was)\b/i.test(text);
 ...
 */
function detectUserSignals(text, indicators, emotionIntensity) {
    const signals = [];
    // Opening up
    if (OPENING_UP_PATTERNS.some((p) => p.test(text)) ||
 ...
 */
function assessCurrentDepth(indicators, emotionIntensity) {
    const depthScore = indicators.content.personalPronouns * 0.2 +
    ...
    if (depthScore > 0.7 || indicators.content.vulnerableTopics)
        return 'vulnerable';
    if (depthScore > 0.45)
        return 'deep';
    if (depthScore > 0.2)
 ...
 */
function determineDirection(state, signals, indicators, profile, turnCount) {
 ...
 */
function findExitPoints(text, currentDepth) {
    const exits = [];
 ...
 */
function buildTransition(direction, currentDepth, signals) {
    switch (direction) {
        case 'deepen':
            return {
                timing: 'when_natural',
 ...
 * Build response guidance
 */
function buildResponseGuidance(state, signals, indicators) {
    ...
    if (signals.includes('opening_up'))
        focusOn = 'validation and space';
    if (signals.includes('reaching_capacity'))
        focusOn = 'grounding and presence';
    if (signals.includes('seeking_closeness'))
        focusOn = 'connection and warmth';
    return { depth, length, tone, focusOn };
}
/**
 * Update flow profile based on observation
 */
function updateFlowProfile(profile, state, indicators) {
    const alpha = 0.1;
    ...
    if (newTypicalNumeric < 1.5)
        profile.typicalDepth = 'surface';
    else if (newTypicalNumeric < 2.5)
        profile.typicalDepth = 'medium';
    else if (newTypicalNumeric < 3.5)
        profile.typicalDepth = 'deep';
    else
        profile.typicalDepth = 'vulnerable';
 ...
 */
export function formatFlowForPrompt(analysis) {
    ...
    if (analysis.transition.phrases.length > 0 &&
    ...
    lines.push(`Response: ${analysis.responseGuidance.length} length, ${analysis.responseGuidance.tone} tone, focus on ${analysis.responseGuidance.focusOn}`);
    return lines.join('\n');
}
// ============================================================================
 ...
 */
export function importFlowProfile(profile) {
 ...
 */
export function resetConversationalFlow() {
    userProfiles.clear();
    sessionStates.clear();
}
// ============================================================================
