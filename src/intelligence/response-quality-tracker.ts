...
export class ResponseQualityTracker {
  ...
  analyzeResponse(response: string): {
    ...
    const hadHumor = /\b(haha|joke|kidding|funny|laugh|😄|😂)\b/i.test(response) || 
                     /\!.*\!/i.test(response);
  ...
  analyzeUserReaction(
    ...
    const showedEmotion = emotion ? emotion.intensity > 0.5 : 
  ...
  calculatePreferences(): LearnedResponsePreferences {
    ...
    const typeScores: Record<ResponseType, number[]> = {
      story: [], advice: [], question: [], humor: [], empathy: [],
      explanation: [], encouragement: [], reflection: [], factual: [], mixed: []
    };
    
    const lengthScores: Record<string, number[]> = {
      brief: [], moderate: [], lengthy: []
    };
    ...
    const preferredLength = Object.entries(lengthAvgs)
      .sort((a, b) => b[1] - a[1])[0][0] as 'brief' | 'moderate' | 'lengthy';
    ...
    const highTopics = topicAvgs.filter(t => t.avg > 0.7).map(t => t.topic).slice(0, 10);
    const lowTopics = topicAvgs.filter(t => t.avg < 0.4).map(t => t.topic).slice(0, 5);
