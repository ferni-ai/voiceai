...
const supportTriggerTemplates = [
    ...
    },
    {
        ...
        condition: (snapshot) => {
            const { calendar } = snapshot.domains;
            const { relationships } = snapshot.domains;
            const matches = !!(calendar?.isOverloaded && relationships?.relationshipHealth === 'strained');
            return {
                matches,
                confidence: matches ? 0.7 : 0,
    ...
    // Peter support trigger for financial decision support
    {
        ...
        condition: (snapshot) => {
            const { finance } = snapshot.domains;
            const matches = !!(finance?.pendingDecision.exists &&
 ...
 */
const celebrationTriggerTemplates = [
    {
    ...
    },
    {
        ...
        condition: (snapshot) => {
            const { relationships } = snapshot.domains;
            const matches = !!(relationships?.relationshipHealth === 'thriving' &&
...
const nuancedTriggerTemplates = [
    ...
    },
    {
        ...
        condition: (snapshot) => {
            ...
            const matches = !!(goals?.motivationLevel === 'medium' &&
                goals.overallProgress !== 'behind' &&
                habits &&
                !habits.inSlump);
            return {
    ...
    // ===========================================================================
    {
        ...
        condition: (snapshot) => {
            ...
            const matches = !!(calendar?.isOverloaded &&
                goals?.overallProgress !== 'behind' &&
    ...
    },
    {
        ...
        condition: (snapshot) => {
            ...
            const matches = snapshot.overallLoadScore >= 0.4 &&
    ...
    },
    {
        ...
        condition: (snapshot) => {
            ...
            const totalDomains = Object.values(snapshot.domains).filter((d) => d !== null).length;
            const matches = totalDomains <= 2 && snapshot.overallLoadScore < 0.4 && snapshot.wellbeingScore > 0.5;
            return {
                matches,
    ...
    // ===========================================================================
    {
        ...
        condition: (snapshot) => {
            ...
            const matches = !!(goals?.overallProgress === 'on_track' && goals.motivationLevel === 'medium');
            return {
                matches,
                confidence: matches ? 0.65 : 0,
    ...
    },
    {
        ...
        condition: (snapshot) => {
            ...
            const matches = !!(goals?.overallProgress === 'behind' &&
    ...
    },
    {
        ...
        condition: (snapshot) => {
            ...
            const matches = !!(relationships?.existentialThemes.includes('purpose') && goals?.motivationLevel === 'medium');
            return {
                matches,
                confidence: matches ? 0.7 : 0,
...
const warningTriggerTemplates = [
    {
        ...
        condition: (snapshot) => {
            const { calendar } = snapshot.domains;
            const { goals } = snapshot.domains;
            const matches = !!(calendar?.upcomingDeadline.exists &&
    ...
    },
    {
        ...
        condition: (snapshot) => {
            const { relationships } = snapshot.domains;
            const { goals } = snapshot.domains;
            const matches = !!(relationships?.existentialThemes.includes('nihilism') && goals?.motivationLevel === 'low');
            return {
                matches,
                confidence: matches ? 0.8 : 0,
 ...
 */
export function generateSynthesisTriggers(snapshot, config = {}) {
 ...
 */
export function populateSynthesisTriggers(snapshot, config = {}) {
 ...
 */
export function getMostImportantTrigger(snapshot) {
    if (snapshot.synthesizedTriggers.length === 0)
        return null;
 ...
 */
export function getTriggersByCategory(snapshot, category) {
    return snapshot.synthesizedTriggers.filter((t) => t.category === category);
}
/**
 * Get triggers recommended for a specific persona
 */
export function getTriggersForPersona(snapshot, personaId) {
 ...
 */
export function recordSynthesisTriggers(triggers) {
 ...
 */
export function getSynthesisAnalytics() {
    ...
    return {
        ...
        byCategory: analyticsData.categoryBreakdown,
        byPriority: analyticsData.priorityBreakdown,
        byPersona: analyticsData.personaBreakdown,
        averageConfidence: analyticsData.totalGenerated > 0
 ...
 */
export function resetSynthesisAnalytics() {
    analyticsData = {
        triggerCounts: {},
        totalGenerated: 0,
        categoryBreakdown: {},
...
//# sourceMappingURL=synthesis-trigger-generator.js.map