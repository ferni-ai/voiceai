 ...
 */
export function daysUntilRecurringDate(date, fromDate = new Date()) {
 ...
 */
export function daysSinceRecurringDate(date, fromDate = new Date()) {
    const parts = date.date.split('-');
 ...
 */
function calculateConfidence(observed, total, expected) {
    if (total < 3)
        return 0;
 ...
 */
export function createTriggerFiringEvent(triggerName, triggerCategory, outcome = 'unknown', sessionId, significantDates) {
    const now = new Date();
 ...
 */
export function recordFiringEvent(profile, event, config = DEFAULT_TEMPORAL_CONFIG) {
 ...
 */
export function analyzeDayOfWeekPatterns(events, config = DEFAULT_TEMPORAL_CONFIG) {
    if (events.length < config.minObservations)
        return [];
    const days = [
        'monday',
        'tuesday',
 ...
 */
export function analyzeTimeOfDayPatterns(events, config = DEFAULT_TEMPORAL_CONFIG) {
    if (events.length < config.minObservations)
        return [];
    const timeBuckets = [
        'late_night',
 ...
 */
export function analyzeRecurringDatePatterns(events, significantDates, config = DEFAULT_TEMPORAL_CONFIG) {
    if (events.length < config.minObservations || significantDates.length === 0)
        return [];
    const patterns = [];
    // For each significant date, analyze events near it
    for (const sigDate of significantDates) {
        if (!sigDate.isRecurring)
            continue;
        // Find events that have proximity to this date
        const nearbyEvents = events.filter((e) => e.dateProximity && e.dateProximity.dateId === sigDate.id);
        if (nearbyEvents.length < config.minObservations)
            continue;
        ...
        let approachBehavior = 'neutral';
        const emotionalCategories = ['emotional', 'grief', 'anxiety'];
        const hasEmotionalSpike = elevatedCategories.some((c) => emotionalCategories.includes(c.category) && c.multiplier > 0.3);
 ...
 */
export function analyzeTemporalPatterns(profile, config = DEFAULT_TEMPORAL_CONFIG) {
    const intelligence = profile.temporalIntelligence ?? { ...DEFAULT_TEMPORAL_INTELLIGENCE };
 ...
 */
export function calculateTemporalBoost(profile, currentTime = new Date()) {
    const result = {
    ...
    for (const datePattern of intelligence.datePatterns) {
        ...
        if (!sigDate || !sigDate.isRecurring)
            continue;
 ...
 */
export function recordTemporalBoost(result, dayOfWeek, timeOfDay, processingMs) {
 ...
 */
export function recordFiringEventAnalytics(event) {
 ...
 */
export function getTemporalAnalytics() {
    return {
        ...temporalAnalytics,
 ...
 */
export function resetTemporalAnalytics() {
    temporalAnalytics = {
        totalBoostCalculations: 0,
        nearSignificantDateBoosts: 0,
        averageMultiplier: 0,
...
//# sourceMappingURL=temporal-pattern-detector.js.map