 ...
 */
function computeSleepStress(data) {
 ...
 */
function computeCalendarStress(data) {
    if (data.confidence < 0.3)
        return null;
    let stressLevel = 0;
    const reasons = [];
 ...
 */
function computeFinanceStress(data) {
    if (data.confidence < 0.3)
        return null;
    let stressLevel = 0;
    const reasons = [];
 ...
 */
function computeGoalsStress(data) {
    if (data.confidence < 0.3)
        return null;
    let stressLevel = 0;
    const reasons = [];
 ...
 */
function computeRelationshipStress(data) {
    if (data.confidence < 0.3)
        return null;
    let stressLevel = 0;
    const reasons = [];
 ...
 */
function computeHabitsStress(data) {
    if (data.confidence < 0.3)
        return null;
    let stressLevel = 0;
    const reasons = [];
 ...
 */
function detectCrossDomainPatterns(domains, stressIndicators) {
    const patterns = [];
    ...
    if (domains.sleep && domains.habits) {
        if (domains.sleep.averageSleepHours >= 7 &&
    ...
    if (domains.relationships) {
        if (domains.relationships.relationshipHealth === 'thriving' &&
 ...
 */
function calculateOverallLoadScore(stressIndicators) {
 ...
 */
function calculateWellbeingScore(domains, loadScore, patterns) {
 ...
 */
export async function aggregateLifeContext(userId, config = {}) {
    ...
    const domains = {};
    if (domainData.sleep)
        domains.sleep = domainData.sleep;
    if (domainData.calendar)
        domains.calendar = domainData.calendar;
    if (domainData.finance)
        domains.finance = domainData.finance;
    if (domainData.goals)
        domains.goals = domainData.goals;
    if (domainData.relationships)
        domains.relationships = domainData.relationships;
    if (domainData.habits)
        domains.habits = domainData.habits;
 ...
 */
export function summarizeLifeContext(snapshot) {
    const parts = [];
...
//# sourceMappingURL=life-context-aggregator.js.map