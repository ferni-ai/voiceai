 ...
 */
function getCachedData(userId, domain, windowDays) {
 ...
 * Store data in cache
 */
function setCachedData(userId, domain, windowDays, data) {
    const cacheKey = `${userId}:${domain}`;
 ...
 */
export function clearDomainCache(userId, domain) {
    if (domain) {
        domainCache.delete(`${userId}:${domain}`);
    }
    else {
        // Clear all domains for user
        for (const d of Object.keys(CACHE_TTL_MS)) {
 ...
 */
export function clearAllDomainCaches() {
    domainCache.clear();
 ...
 */
async function getRecentConversationContext(userId, _windowDays) {
    try {
        ...
        if (typeof semanticRag.semanticSearch === 'function') {
            const results = await semanticRag.semanticSearch('recent conversations mood feelings concerns', { topK: 10, minScore: 0.3 });
 ...
 */
export const sleepDataCollector = {
    domain: 'sleep',
    sourcePersona: 'maya',
    async collect(userId, windowDays) {
        try {
            ...
            if (recentContext) {
                ...
                mentionedFatigue = fatigueKeywords.some((keyword) => recentContext.toLowerCase().includes(keyword));
            }
            ...
            if (summariesWithSleep.length > 0) {
                ...
                const poorNights = summariesWithSleep.filter((s) => (s.sleepHours && s.sleepHours < 6) || s.sleepQuality === 'poor').length;
...
export const calendarDataCollector = {
    ...
    async collect(userId, windowDays) {
        try {
            ...
            const hasOverloadAlerts = calendarAlerts.some((a) => a.type === 'overload' || a.type === 'back_to_back');
 ...
 */
export const financeDataCollector = {
    domain: 'finance',
    sourcePersona: 'peter',
    async collect(userId, windowDays) {
        try {
            ...
            const concernTopics = [];
            if (contextLower.includes('market'))
                concernTopics.push('market volatility');
            if (contextLower.includes('bill'))
                concernTopics.push('bills');
            if (contextLower.includes('debt'))
                concernTopics.push('debt');
            if (contextLower.includes('retire'))
                concernTopics.push('retirement');
            return {
                checkFrequency,
                expressedAnxiety,
                stressLevel,
 ...
 */
export const goalsDataCollector = {
    domain: 'goals',
                ...
                return {
                    ...
                    upcomingMilestone: upcomingMilestone
                        ? {
                            ...
                            daysUntil: Math.ceil((new Date(upcomingMilestone.targetDate).getTime() - now.getTime()) /
                                (24 * 60 * 60 * 1000)),
                        }
                        : { exists: false },
                    overallProgress,
 ...
 */
export const relationshipDataCollector = {
    ...
    async collect(userId, windowDays) {
        try {
            ...
            if (habitCoachData && habitCoachData.enhancedHabits.length > 0) {
                ...
                const adherencePercent = habits.length > 0 ? Math.round((activeHabits.length / habits.length) * 100) : 50;
 ...
 */
async function collectWithCache(userId, windowDays, domain, collector) {
    // Check cache first
    const cached = getCachedData(userId, domain, windowDays);
    if (cached !== null) {
 ...
 */
export async function collectAllDomainData(userId, windowDays = 7, options = {}) {
...
//# sourceMappingURL=domain-data-collectors.js.map