...
export const DEFAULT_SIGNAL_LEARNER_CONFIG: SignalLearnerConfig = {
  minObservations: 3,
  minProbability: 0.6,
 ...
 */
export const COMMON_ANTICIPATORY_PHRASES: Array<{
  phrases: string[];
 ...
 */
export function detectAnticipatorySignals(
  partialInput: string,
  ...
  config: SignalLearnerConfig = DEFAULT_SIGNAL_LEARNER_CONFIG
  ...
  const matchedSignals: SignalDetectionResult['signals'] = [];

  // Check user's learned signals
  for (const signal of intelligence.signals) {
    if (signal.observations < config.minObservations) continue;
    if (signal.probability < config.minProbability) continue;
    ...
    for (const common of COMMON_ANTICIPATORY_PHRASES) {
      for (const phrase of common.phrases) {
        if (inputLower.includes(phrase)) {
          ...
          const tempSignal: AnticipatorySignal = {
          ...
          const combinedConfidence =
            config.textSignalWeight * common.baselineProbability +
            config.voiceProsodyWeight * voiceConfidence;
  ...
  let anticipatedOutcome: AnticipatedOutcomeType | null = null;
  ...
  if (matchedSignals.length > 0) {
    ...
    const agreeingSignals = matchedSignals.filter(
      (s) => s.signal.anticipatedOutcome === anticipatedOutcome
    );
    overallConfidence = Math.min(
      matchedSignals[0].combinedConfidence * (1 + 0.1 * (agreeingSignals.length - 1)),
      1.0
    );
  }
 ...
 */
function matchSignalPhrase(input: string, signal: AnticipatorySignal): string | null {
  if (signal.isRegex) {
    try {
...
function calculateVoiceConfidence(
  detectedCues: VoiceProsodyCue[],
  expectedCues: VoiceProsodyCue[]
): number {
  if (expectedCues.length === 0) return 0;

  let matchScore = 0;
  let totalWeight = 0;

  for (const expected of expectedCues) {
    const matching = detectedCues.find(
      (d) =>
...
function checkSafeguards(
  safeguards: AnticipatoryIntelligence['safeguards'],
  input: string,
  sessionContext?: {
    anticipationsThisSession: number;
    lastAnticipationTime?: Date;
  ...
  if (sessionContext) {
    ...
    for (const timeWindow of safeguards.disabledTimes) {
      if (
        sessionContext.currentHour >= timeWindow.startHour &&
...
export function learnFromUtterance(
  ...
  if (!foundMatch && input.actualOutcome !== 'unknown') {
    ...
    if (bestPhrase && intelligence.signals.length < config.maxSignalsPerUser) {
      ...
      if (!isPhraseTooGeneric(bestPhrase)) {
        const newSignal: AnticipatorySignal = {
  ...
  for (const cue of input.voiceCues) {
    const existingCue = intelligence.voiceCues.find(
 ...
 */
function isPhraseTooGeneric(phrase: string): boolean {
 ...
 */
export function recordAnticipationEvent(
  profile: UserTriggerProfile,
 ...
 */
export function getAnticipatoryAnalytics(
  intelligence: AnticipatoryIntelligence,
  config: SignalLearnerConfig = DEFAULT_SIGNAL_LEARNER_CONFIG
): AnticipatoryAnalytics {
  const activeSignals = intelligence.signals.filter(
    (s) => s.observations >= config.minObservations && s.probability >= config.minProbability
  );

  const outcomeDistribution: Record<AnticipatedOutcomeType, number> = {
    vulnerability: 0,
    distress: 0,
    request: 0,
    celebration: 0,
  ...
  const reactionDistribution: Record<AnticipationEvent['userReaction'], number> = {
    appreciated: 0,
    continued: 0,
    ignored: 0,
    corrected: 0,
