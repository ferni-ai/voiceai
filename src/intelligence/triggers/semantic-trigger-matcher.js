...
import { checkDynamicTriggers, calculateProbabilityBoost, shouldSkipDueToNeverWhen, recordTriggerCheck, recordTriggerMatch, } from '../context-builders/dynamic-trigger-utils.js';
 ...
 */
function calculatePatternScore(userText, triggerText, context) {
    ...
    for (const [category, { patterns, weight }] of Object.entries(PATTERN_KEYWORDS)) {
        ...
        if (!isCategoryRelevantToTrigger(category, lowerTrigger))
            continue;
    ...
    if (context.daysSinceLastSession &&
    ...
    const normalizedScore = matchCount > 0 ? Math.min(1, totalScore / Math.max(1, matchCount * 0.8)) : 0;
    return {
 ...
 */
function isCategoryRelevantToTrigger(category, triggerText) {
    const relevanceMap = {
 ...
 */
export async function matchTriggersHybrid(userText, context, triggers, personaId, config = {}) {
 ...
 */
export async function getSemanticSimilarity(userText, triggerText) {
    const service = getTriggerEmbeddingService();
    // Get embeddings
 ...
 */
export function shouldSkipTriggers(neverWhen, context) {
    return shouldSkipDueToNeverWhen(neverWhen, context);
}
/**
 * Calculate probability boost from more_likely_when
 */
export function getTriggerProbabilityBoost(moreLikelyWhen, context, match) {
    if (!match)
        return 1.0;
 ...
 */
export function recordSemanticMatch(result) {
    matchCount++;
 ...
 */
export function getSemanticAnalytics() {
    return {
        ...semanticAnalytics,
        byCategoryArray: Array.from(semanticAnalytics.byCategory.entries()).map(([category, stats]) => ({ category, ...stats })),
    };
}
/**
 * Reset analytics
 */
export function resetSemanticAnalytics() {
...
//# sourceMappingURL=semantic-trigger-matcher.js.map