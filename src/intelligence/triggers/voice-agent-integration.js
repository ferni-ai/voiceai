...
import { generatePersonalContextBoost, } from './personal-context-integrator.js';
import { calculateTemporalBoost, createTriggerFiringEvent, recordFiringEvent, analyzeTemporalPatterns, recordTemporalBoost, recordFiringEventAnalytics, getDayOfWeek, getTimeOfDayBucket, } from './temporal-pattern-detector.js';
import { analyzeUserEffectiveness, getEffectivenessMultiplier, recordEffectivenessAnalytics, } from './effectiveness-calculator.js';
 ...
 */
export async function loadUserTriggerContext(userId, sessionId) {
    ...
    try {
        ...
        const personalBoost = generatePersonalContextBoost('', // No user text at session start
        analyzedProfile, triggerContext, {
            enabled: true,
        ...
        // Record analytics
        const now = new Date();
        recordTemporalBoost(temporalBoost, getDayOfWeek(now), getTimeOfDayBucket(now.getHours()), Date.now() - startTime);
        log.info({
            userId,
            sessionId,
 ...
 */
export function getSessionTriggerContext(sessionId) {
 ...
 */
export function recordTriggerOutcome(sessionId, triggerName, triggerCategory, outcome) {
    ...
    // Create firing event
    const event = createTriggerFiringEvent(triggerName, triggerCategory, outcome, sessionId, context.profile.significantDates);
 ...
 */
export async function saveUserTriggerContext(sessionId) {
    const context = sessionContexts.get(sessionId);
    if (!context) {
 ...
 */
export function getCombinedTriggerBoost(sessionId, triggerName, triggerCategory) {
    const context = sessionContexts.get(sessionId);
    if (!context) {
        return {
            multiplier: 1.0,
    ...
    const contextNotes = [...context.temporalBoost.contextNotes];
    ...
    const triggerAdjustment = context.temporalBoost.triggerAdjustments.find((a) => a.triggerName === triggerName);
    ...
    const triggerBoostEntry = context.personalBoost.triggerBoosts.find((t) => t.triggerName === triggerName);
 ...
 */
export function getApproachingSignificantDates(sessionId, withinDays = 7) {
    const context = sessionContexts.get(sessionId);
    if (!context)
        return [];
    ...
    return context.profile.significantDates.filter((date) => {
        ...
        const daysUntil = Math.round((nextOccurrence.getTime() - now.getTime()) / (24 * 60 * 60 * 1000));
