 ...
 */
export function generatePersonalContextBoost(
  text: string,
  profile: UserTriggerProfile,
  triggerContext: TriggerContext,
  config: PersonalContextConfig = DEFAULT_PERSONAL_CONTEXT_CONFIG
): PersonalContextBoost {
  ...
  const triggerBoosts: Array<{ triggerName: string; boost: number; reason: string }> = [];
  const appliedContext = {
    upcomingDates: [] as SignificantDate[],
    mentionedRelationships: [] as Relationship[],
    detectedPatterns: [] as string[],
    temporalFlags: [] as string[],
  };
  ...
  for (const date of upcomingDates) {
    ...
    for (const category of date.triggerCategories) {
      categoryBoosts[category] = Math.max(
        categoryBoosts[category] || 0,
        date.emotionalWeight * config.relationshipBoostWeight
      );
    }
    ...
    for (const rel of extracted.relationships) {
      ...
      if (profileRel) {
        ...
        for (const category of profileRel.triggerCategories) {
          categoryBoosts[category] = Math.max(
            categoryBoosts[category] || 0,
            valenceBoost * config.relationshipBoostWeight
          );
        }
        ...
        if (
          profileRel.emotionalValence === 'complicated' ||
  ...
  for (const category of patternResult.detectedCategories) {
    ...
    if (category === 'distress' || category === 'hopelessness') {
      categoryBoosts['emotional'] = Math.max(
        categoryBoosts['emotional'] || 0,
        0.5 * config.patternBoostWeight
      );
      triggerBoosts.push({
    ...
    if (category === 'deflection' || category === 'avoidance') {
      categoryBoosts['behavioral'] = Math.max(
        categoryBoosts['behavioral'] || 0,
        0.35 * config.patternBoostWeight
      );
      triggerBoosts.push({
  ...
  if (hour >= 0 && hour < 5) {
    ...
    categoryBoosts['temporal'] = Math.max(
      categoryBoosts['temporal'] || 0,
      0.3 * config.temporalBoostWeight
    );
    triggerBoosts.push({
 ...
 */
export function applyPersonalContextBoost(
  matchResult: HybridMatchResult,
  contextBoost: PersonalContextBoost
): HybridMatchResult {
  ...
  if (boostedBestMatch) {
    ...
    const specificBoost = contextBoost.triggerBoosts.find(
      (tb) => tb.triggerName === boostedBestMatch!.triggerName
    );
  ...
  const boostedAllMatches = matchResult.allMatches.map((match) => {
    const specificBoost = contextBoost.triggerBoosts.find(
      (tb) => tb.triggerName === match.triggerName
    );
...
function createEmptyBoost(profile: UserTriggerProfile, startTime: number): PersonalContextBoost {
  return {
...
function findUpcomingDates(dates: SignificantDate[], daysAhead: number): SignificantDate[] {
  const now = new Date();
  const upcoming: SignificantDate[] = [];
...
function getValenceBoost(valence: Relationship['emotionalValence']): number {
  switch (valence) {
    case 'very_positive':
      return 0.15;
    case 'positive':
