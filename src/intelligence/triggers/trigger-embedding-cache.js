...
export class TriggerEmbeddingCache {
     ...
     */
    generateTriggerId(personaId, triggerName) {
        return `${personaId}:${triggerName}`;
    }
    /**
     * Generate content hash for cache validation
     */
    hashTriggerText(triggerText) {
     ...
     */
    async get(personaId, triggerName, triggerText) {
        ...
        if (memoryCached) {
            ...
            if (this.hashTriggerText(memoryCached.triggerText) === textHash &&
     ...
     */
    async set(personaId, triggerName, triggerText, embedding, model) {
        await this.initFirestore();
        const triggerId = this.generateTriggerId(personaId, triggerName);
        const cached = {
            triggerId,
     ...
     * Set in Firestore
     */
    async setInFirestore(triggerId, cached) {
        if (!this.firestoreDb)
            return;
        try {
     ...
     */
    async bulkSave(embeddings, model) {
        await this.initFirestore();
        if (!this.firestoreDb) {
     ...
     */
    async invalidate(personaId, triggerName) {
        const triggerId = this.generateTriggerId(personaId, triggerName);
        this.memoryCache.delete(triggerId);
        if (this.firestoreDb) {
            try {
     ...
     */
    async invalidatePersona(personaId) {
        let count = 0;
        // Remove from memory
        for (const [id, cached] of this.memoryCache.entries()) {
    ...
    getStats() {
        ...
        return {
            ...
            hitRate: totalAccesses > 0 ? (this.stats.memoryHits + this.stats.firestoreHits) / totalAccesses : 0,
 ...
 */
export function getTriggerEmbeddingCache(config) {
    if (!defaultCache) {
 ...
 */
export function resetTriggerEmbeddingCache() {
    if (defaultCache) {
...
//# sourceMappingURL=trigger-embedding-cache.js.map