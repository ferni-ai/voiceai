...
import { detectAnticipatorySignals, recordAnticipationEvent, DEFAULT_SIGNAL_LEARNER_CONFIG, } from './anticipatory-signal-learner.js';
 ...
 */
export function processPartialInput(sessionId, partialInput, intelligence, voiceProsody, currentTopic, config = DEFAULT_ENGINE_CONFIG) {
    ...
    const detection = detectAnticipatorySignals(partialInput, intelligence, voiceProsody, {
        sessionId,
 ...
 */
export function checkPendingAnticipation(sessionId, pauseDurationMs, config = DEFAULT_ENGINE_CONFIG) {
 ...
 */
export function recordAnticipatoryOutcome(profile, sessionId, detection, userReaction, responseType, voiceProsodyScore = 0, predictionCorrect = true) {
 ...
 */
function selectVerbalResponse(template, _outcome) {
 ...
 */
export function getPersonaResponseTemplates(_personaId) {
 ...
 */
export function clearAnticipatorySession(sessionId) {
    sessionStates.delete(sessionId);
}
/**
 * Clear all session states (for testing).
 */
export function clearAllAnticipatorySessions() {
    sessionStates.clear();
}
/**
 * Get session stats (for monitoring).
 */
export function getAnticipatorySessionStats(sessionId) {
    const state = sessionStates.get(sessionId);
    if (!state)
        return null;
    return {
 ...
 */
export function recordAnticipationFiring() {
 ...
 */
export function getAnticipatoryEngineAnalytics() {
 ...
 */
export function resetAnticipatoryEngineAnalytics() {
    analyticsData = {
        totalAnticipationsFired: 0,
        lastFiringTime: null,
    };
}
//# sourceMappingURL=anticipatory-trigger-engine.js.map