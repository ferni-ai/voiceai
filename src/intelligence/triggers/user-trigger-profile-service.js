...
export class UserTriggerProfileService {
     ...
     */
    async addSignificantDate(userId, date) {
        const profile = await this.loadProfile(userId);
        // Check for duplicates (same date, type, and person)
        const existing = profile.significantDates.find((d) => d.date === date.date && d.type === date.type && d.relatedPerson === date.relatedPerson);
        if (existing) {
    ...
    async addRelationship(userId, relationship) {
        ...
        const existing = profile.relationships.find((r) => r.name.toLowerCase() === relationship.name.toLowerCase() ||
     ...
     */
    async updateCommunicationPatterns(userId, patterns) {
        const profile = await this.loadProfile(userId);
        // Merge patterns
     ...
     */
    async recordTriggerEffectiveness(userId, triggerName, outcome) {
        const profile = await this.loadProfile(userId);
     ...
     */
    async mergeExtractionResult(userId, extraction) {
        const profile = await this.loadProfile(userId);
        ...
        if (extraction.patternUpdates.deflectionPhrases.length > 0) {
            profile.communicationPatterns.deflectionPhrases = this.mergePhraseLists(profile.communicationPatterns.deflectionPhrases, extraction.patternUpdates.deflectionPhrases);
        }
        if (extraction.patternUpdates.vulnerabilitySignals.length > 0) {
            profile.communicationPatterns.vulnerabilitySignals = this.mergePhraseLists(profile.communicationPatterns.vulnerabilitySignals, extraction.patternUpdates.vulnerabilitySignals);
        }
        if (extraction.patternUpdates.sensitiveTopics.length > 0) {
            profile.communicationPatterns.sensitiveTopics = this.mergeSensitiveTopics(profile.communicationPatterns.sensitiveTopics, extraction.patternUpdates.sensitiveTopics);
        }
     ...
     */
    async generateContextBoost(userId, context) {
        const profile = await this.loadProfile(userId);
        ...
        const recentlyMentioned = profile.relationships.filter((r) => r.lastMentioned && Date.now() - r.lastMentioned.getTime() < 24 * 60 * 60 * 1000);
    ...
    mergeRelationship(existing, updated) {
        return {
            ...existing,
            // Merge aliases
            aliases: [...new Set([...existing.aliases, ...updated.aliases])],
            // Take higher confidence updates
            emotionalValence: updated.confidence > existing.confidence
            ...
            mentionFrequency: (existing.mentionFrequency * existing.confidence +
    ...
    mergePatterns(existing, updates) {
        return {
    ...
    mergePhraseLists(existing, updates) {
    ...
    mergeSensitiveTopics(existing, updates) {
        const merged = [...existing];
        for (const update of updates) {
    ...
    mergeTemporalPatterns(existing, updates) {
        const merged = [...existing];
        for (const update of updates) {
            // Match on timeOfDay (new format) or type+value (legacy format)
            const idx = merged.findIndex((p) => p.timeOfDay === update.timeOfDay ||
    ...
    calculateProfileConfidence(profile) {
        ...
        const patternCount = (profile.communicationPatterns.deflectionPhrases?.length || 0) +
    ...
    formatDateContext(date, daysUntil) {
 ...
 */
export function getUserTriggerProfileService(config) {
    if (!instance) {
 ...
 */
export function resetUserTriggerProfileService() {
    if (instance) {
        instance.clearAllCache();
    }
    instance = null;
}
//# sourceMappingURL=user-trigger-profile-service.js.map