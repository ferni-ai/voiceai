...
export type EngagementSignal =
  | 'longer_response' // User gave a longer than average response
  | 'deeper_topic' // User went deeper into the topic
  | 'emotional_expression' // User expressed emotion
  | 'question_asked' // User asked a follow-up question
  | 'gratitude_expressed' // User thanked or appreciated
  | 'vulnerability_shared' // User shared something vulnerable
  | 'continuation_requested'; // User asked to continue

/**
 * Signals indicating deflection or disengagement
 */
export type DeflectionSignal =
  | 'topic_change' // User changed the topic
  | 'short_response' // User gave a very short response
  | 'minimization' // User minimized ("it's fine", "no big deal")
  | 'deflection_phrase' // User used deflection phrase ("anyway", "moving on")
  | 'dismissive_tone' // Response had dismissive tone
  | 'closed_body_language' // For future: detected closed posture
  | 'session_ended'; // User ended session shortly after
...
const sessionOutcomes = new Map<string, TriggerOutcomeEvent[]>();
 ...
 */
export function getSessionOutcomes(sessionId: string): TriggerOutcomeEvent[] {
 ...
 */
export function clearSessionOutcomes(sessionId: string): void {
 ...
 */
export function detectEngagementSignals(
  userResponse: string,
 ...
 */
export function detectDeflectionSignals(
  userResponse: string,
  averageResponseLength: number,
  previousTopic: string,
  currentTopic: string,
  sessionEndedWithin: number | null // minutes until session ended, or null if ongoing
): DeflectionSignal[] {
  const signals: DeflectionSignal[] = [];
  const responseLower = userResponse.toLowerCase();
 ...
 */
export function calculateEffectivenessFromEvents(
  triggerName: string,
  events: TriggerOutcomeEvent[],
  ...
  // Weighted score
  const rawScore =
    config.engagementWeight * engagementRate +
 ...
 */
export function calculateEffectivenessFromRecord(
  record: TriggerEffectiveness,
  config: EffectivenessConfig = DEFAULT_EFFECTIVENESS_CONFIG
): EffectivenessResult {
 ...
 */
export function analyzeUserEffectiveness(
  profile: UserTriggerProfile,
  config: EffectivenessConfig = DEFAULT_EFFECTIVENESS_CONFIG
): UserEffectivenessAnalysis {
  const triggerResults: EffectivenessResult[] = [];
 ...
 */
export function applyEffectivenessToScore(
  originalScore: number,
  triggerName: string,
  profile: UserTriggerProfile,
  config: EffectivenessConfig = DEFAULT_EFFECTIVENESS_CONFIG
): { adjustedScore: number; wasExplored: boolean; multiplierApplied: number } {
  const { multiplier, shouldExplore, confidence } = getEffectivenessMultiplier(
    triggerName,
    profile,
    config
  );
...
export interface EffectivenessAnalytics {
  ...
  topPerformingTriggers: Array<{ name: string; score: number; observations: number }>;
  worstPerformingTriggers: Array<{ name: string; score: number; observations: number }>;
}

// In-memory analytics (reset on restart)
let effectivenessAnalytics: EffectivenessAnalytics = {
 ...
 */
export function recordEffectivenessAnalytics(analysis: UserEffectivenessAnalysis): void {
 ...
 */
export function getEffectivenessAnalytics(): EffectivenessAnalytics {
 ...
 */
export function resetEffectivenessAnalytics(): void {
