...
export type TriggerCategory =
  | 'emotional' // Distress, grief, worry, etc.
  | 'behavioral' // Deflection, avoidance, etc.
  | 'temporal' // Late night, returning user, etc.
  | 'domain' // Habits, finance, planning, etc.
  | 'relational' // Relationship dynamics
  | 'existential' // Meaning, purpose, life questions
  | 'growth'; // Progress, change, development
...
export interface SemanticMatch {
  ...
  patternScore: number; // Pattern matching score (0-1)
...
export interface CachedTriggerEmbedding {
  triggerId: string; // personaId:triggerName
  embedding: number[];
  triggerText: string;
  personaId: string;
  model: string;
...
export interface SemanticMatchAnalytics {
  ...
  byCategory: Map<TriggerCategory, { matched: number; avgScore: number }>;
  recentMatches: Array<{
    timestamp: Date;
    triggerName: string;
 ...
 */
export const DEFAULT_HYBRID_CONFIG: HybridMatchConfig = {
