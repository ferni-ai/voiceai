...
const DATE_PATTERNS = [
    // Birthday patterns
    {
        pattern: new RegExp(`(?:my|his|her|their)\\s+(?:mom'?s?|mother'?s?|dad'?s?|father'?s?)\\s+birthday\\s+is\\s+(?:on\\s+)?(?:the\\s+)?(${ORDINAL_PATTERN})\\s+(?:of\\s+)?(${MONTH_PATTERN})`, 'i'),
        type: 'birthday',
        ...
        extractPerson: (match) => {
            if (/mom|mother/i.test(match[0]))
                return 'Mom';
            if (/dad|father/i.test(match[0]))
                return 'Dad';
            return null;
        },
    },
    {
        pattern: new RegExp(`(?:my|his|her|their)\\s+(\\w+)'?s?\\s+birthday\\s+is\\s+(?:on\\s+)?(?:the\\s+)?(${ORDINAL_PATTERN})\\s+(?:of\\s+)?(${MONTH_PATTERN})`, 'i'),
        type: 'birthday',
    ...
    {
        pattern: new RegExp(`my\\s+birthday\\s+is\\s+(?:on\\s+)?(?:the\\s+)?(${ORDINAL_PATTERN})\\s+(?:of\\s+)?(${MONTH_PATTERN})`, 'i'),
        type: 'birthday',
    ...
    {
        pattern: new RegExp(`(?:our|my)\\s+(?:wedding\\s+)?anniversary\\s+is\\s+(?:on\\s+)?(?:the\\s+)?(${ORDINAL_PATTERN})\\s+(?:of\\s+)?(${MONTH_PATTERN})`, 'i'),
        type: 'anniversary',
...
// ============================================================================
const TYPE_TO_TRIGGER_CATEGORIES = {
 ...
 */
export function extractSignificantDates(text, options = {}) {
    ...
    for (const patternDef of DATE_PATTERNS) {
        ...
        if (!dateStr && !patternDef.extractPerson)
            continue;
    ...
    log.info({ datesFound: extractedDates.length, processingTimeMs }, 'Significant date extraction complete');
    return { dates: extractedDates, processingTimeMs };
}
/**
 * Calculate confidence score for an extraction
 */
function calculateConfidence(match, patternDef, text) {
 ...
 */
function buildDescription(match, type, person) {
    switch (type) {
        case 'birthday':
        ...
        case 'milestone':
            if (/graduat/i.test(match[0]))
                return 'Graduation';
            if (/promot/i.test(match[0]))
                return 'Promotion anniversary';
            if (/sober|clean/i.test(match[0]))
                return 'Sobriety anniversary';
            return 'Personal milestone';
        case 'medical':
            if (/diagnos/i.test(match[0]))
                return 'Diagnosis date';
            if (/surgery|operation/i.test(match[0]))
                return 'Surgery date';
 ...
 */
export function hasDateMentions(text) {
    const quickPatterns = [
        /birthday/i,
        /anniversary/i,
...
//# sourceMappingURL=significant-date-extractor.js.map