 ...
 */
export function extractRelationships(text, options = {}) {
    ...
    log.info({ relationshipsFound: extractedRelationships.length, processingTimeMs }, 'Relationship extraction complete');
    return { relationships: extractedRelationships, processingTimeMs };
}
/**
 * Determine the emotional valence of a relationship
 */
function determineValence(text, name) {
    ...
    if (positiveScore >= 0.8 && negativeScore < 0.3)
        return 'very_positive';
    if (positiveScore >= 0.4 && negativeScore < 0.3)
        return 'positive';
    if (negativeScore >= 0.8 && positiveScore < 0.3)
        return 'very_negative';
    if (negativeScore >= 0.4 && positiveScore < 0.3)
        return 'negative';
 ...
 */
function checkDeceased(text, name) {
 ...
 */
function checkEstranged(text, name) {
    const nameContext = extractContext(text, name, 150);
 ...
 */
function extractContext(text, name, radius) {
 ...
 */
function calculateConfidence(match, text) {
 ...
 */
function determineTriggerCategories(type, valence, isDeceased) {
    const categories = new Set(['relational']);
    // Add based on type
    if (type === 'family')
 ...
 */
export function hasRelationshipMentions(text) {
    const quickPatterns = [
