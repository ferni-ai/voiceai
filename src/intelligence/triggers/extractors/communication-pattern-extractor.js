 ...
 */
export function extractCommunicationPatterns(text, options = {}) {
    const startTime = Date.now();
    const { minConfidence = 0.4, contextTime, existingPatterns } = options;
    const phrasePatterns = existingPatterns?.phrasePatterns
        ? [...existingPatterns.phrasePatterns]
        : [];
    const temporalPatterns = existingPatterns?.temporalPatterns
    ...
    for (const patternDef of PHRASE_PATTERN_DEFS) {
        for (const pattern of patternDef.patterns) {
            ...
            const existingIndex = phrasePatterns.findIndex((p) => p.phrase.toLowerCase() === match[0].toLowerCase());
            if (existingIndex >= 0) {
                // Update frequency
            ...
            log.debug({ category: patternDef.category, phrase: match[0], confidence }, 'Detected phrase pattern');
        }
    }
 ...
 */
export function hasDistressSignals(text) {
    const distressPatterns = PHRASE_PATTERN_DEFS.filter((p) => p.category === 'distress' || p.category === 'hopelessness');
    return distressPatterns.some((patternDef) => patternDef.patterns.some((p) => p.test(text)));
}
/**
 * Detect if text contains deflection signals
 */
export function hasDeflectionSignals(text) {
    const deflectionPatterns = PHRASE_PATTERN_DEFS.filter((p) => p.category === 'deflection' || p.category === 'avoidance');
...
// HELPER FUNCTIONS
// ============================================================================
function calculatePhraseConfidence(match, text) {
    let confidence = 0.6;
...
function extractPhraseContext(text, matchIndex) {
    const radius = 50;
...
//# sourceMappingURL=communication-pattern-extractor.js.map