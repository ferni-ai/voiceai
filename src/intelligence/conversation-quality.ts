...
export interface JackPhysicalState {
...
/**
 * Get Jack's physical state based on time and conversation length
 */
export function getJackPhysicalState(
  hour: number,
  ...
  if (hour >= 5 && hour < 8) {
    return {
      ...
      alertness: 'normal',
      mood: 'mellow',
      physicalNote: conversationMinutes > 15 
        ? "I'm still waking up. Coffee's helping though." 
        : "Early riser, huh? Me too. Old habits.",
    };
  }
  ...
  if (hour >= 8 && hour < 12) {
    return {
      ...
      alertness: 'sharp',
      mood: 'upbeat',
      physicalNote: turnCount > 15 
        ? "I'm enjoying this conversation. Morning energy, you know?" 
        : null,
    };
  }
  ...
  if (hour >= 12 && hour < 15) {
    ...
    return {
      ...
      mood: 'mellow',
      physicalNote: postLunch && conversationMinutes > 10 
        ? "Just had lunch. Give me a second to gather my thoughts." 
        : null,
    };
  }
  ...
  if (hour >= 15 && hour < 18) {
    return {
      ...
      alertness: 'normal',
      mood: 'reflective',
      physicalNote: conversationMinutes > 20 
        ? "Afternoon conversations always feel different. More thoughtful somehow." 
        : null,
    };
  }
  ...
  if (hour >= 18 && hour < 21) {
    return {
      ...
      alertness: 'normal',
      mood: 'mellow',
      physicalNote: turnCount > 20 
        ? "Evening talks are my favorite. No rush." 
        : null,
    };
  }
  ...
  if (hour >= 21 || hour < 1) {
    return {
      ...
      alertness: 'tired',
      mood: 'reflective',
      physicalNote: conversationMinutes > 10 
        ? "Getting late for an old man. But I'm enjoying this." 
        : "Late night conversations are often the most honest.",
    };
  }
  ...
  return {
    ...
    alertness: 'tired',
    mood: 'sleepy',
    physicalNote: "Can't sleep? Me neither. Let's talk.",
  };
}

/**
 * Get a physical state interjection for Jack
 */
export function getPhysicalStateInterjection(state: JackPhysicalState): string | null {
  // Only occasionally mention physical state
  if (Math.random() > 0.15) return null;
  
  if (state.energyLevel === 'low' && state.alertness === 'tired') {
    const phrases = [
      "Let me sit back a moment... <break time=\"300ms\"/>",
      "These old bones... <break time=\"200ms\"/>Where were we?",
      "<volume level=\"soft\">Getting tired, but this is important.</volume>",
    ];
  ...
  if (state.energyLevel === 'high' && state.alertness === 'sharp') {
    const phrases = [
      "I'm feeling sharp today! <break time=\"150ms\"/>",
