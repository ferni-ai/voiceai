 ...
 */
export function detectReflectionMoment(userText, topic, emotionPrimary, emotionIntensity, sessionId, personaId) {
 ...
 */
export function generateReflection(moment, userName) {
 ...
 */
function calculateAppropiateness(moment) {
    let score = 0.5;
    // Heavier moments are more appropriate to reflect on
    if (moment.emotionalWeight === 'heavy')
        score += 0.2;
    if (moment.emotionalWeight === 'light')
        score -= 0.1;
    ...
    if (daysSince < 1)
        score -= 0.2; // Same day - too soon
    if (daysSince > 7)
        score += 0.1; // Week+ - good timing
    if (daysSince > 30)
        score -= 0.1; // Month+ - getting stale
    // Already reflected on - less appropriate
    if (moment.reflectedOn)
        score -= 0.3;
    // Certain types are more appropriate
    if (moment.type === 'vulnerability_shared')
        score += 0.15;
    if (moment.type === 'goal_commitment')
        score += 0.1;
 ...
 */
export function selectBestReflection(moments, currentTopics, currentEmotion, turnCount) {
 ...
 */
export function getReflectionMoments(profile) {
    if (!profile?.customData?.reflectionMoments)
        return [];
    return profile.customData.reflectionMoments;
}
/**
 * Save a reflection moment to user profile
 */
export function saveReflectionMoment(profile, moment) {
    if (!profile.customData) {
        profile.customData = {};
    }
    const moments = profile.customData.reflectionMoments || [];
 ...
 */
export function markMomentReflectedOn(profile, momentId) {
