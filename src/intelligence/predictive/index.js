...
import { recordTransition, recordSecondOrderTransition, predictNextStates, extractStatesFromTurn, } from './markov-sequence-predictor.js';
import { recordObservation, forecast, findOptimalTimes, analyzeTrend, } from './time-series-forecaster.js';
import { fusePrediction, recordPredictionOutcome, } from './multi-signal-fusion.js';
import { trackPrediction, recordAction, recordOutcome, recordReward, getLearningStats, getCalibratedConfidence, getBestOutreachTimes, getMostAccurateSignals, cleanupOldPredictions, } from './reinforcement-learner.js';
// LLM Deep Analysis (Tier 3 - Batch, scheduled)
import { runDeepAnalysis, getLatestDeepAnalysis, getDeepAnalysisContextForTurn, runBatchDeepAnalysis, recordDeepAnalysisFeedback, } from './llm-deep-analysis.js';
// Persistence layer for ML models
import { initializePersistence, shutdownPersistence, forceFlushUser, } from './persistence.js';
...
// ============================================================================
import { avoidancePrediction, buildAvoidanceContext, } from './avoidance-prediction.js';
import { breakthroughProximity, buildBreakthroughContext, } from './breakthrough-proximity.js';
import { preTrajectoryDetection, buildPreTrajectoryContext, } from './pre-trajectory-detection.js';
import { conversationPreparation, buildConversationPrepContext, } from './conversation-preparation.js';
import { cognitiveFingerprint, buildFingerprintContext, } from './cognitive-fingerprint.js';
import { rippleEffectPrediction, buildRippleContext, } from './ripple-effect-prediction.js';
import { lifePhasePrediction, buildPhaseContext, } from './life-phase-prediction.js';
import { interventionTiming, buildInterventionTimingContext, } from './intervention-timing.js';
// Signal Integration
 ...
 */
export async function processConversationForLearning(userId, params) {
    ...
    try {
        ...
        if (params.previousEmotion || params.previousTopic) {
            const previousStates = extractStatesFromTurn('', params.previousEmotion, params.previousTopic, now);
        ...
        log.debug({ userId, stateCount: states.length, topic: params.topic, emotion: params.emotion }, '🧠 Processed conversation for learning');
    }
    catch (error) {
 ...
 */
export async function getPrediction(userId, target, context = {}) {
 ...
 */
export async function getAllPredictions(userId, context = {}) {
    const targets = [
        'needs_support_now',
        'will_struggle_soon',
        'ready_for_challenge',
 ...
 */
export async function findBestOutreachTime(userId, daysAhead = 3) {
    const results = [];
    ...
    for (const [_, scores] of timeScores.entries()) {
        ...
        const reasons = [];
        if (scores.moodScore > 0.6)
            reasons.push('predicted positive mood');
        if (scores.energyScore > 0.6)
            reasons.push('predicted good energy');
        if (scores.learnedScore > 0.6)
            reasons.push('historically good engagement');
        results.push({
            time: scores.time,
 ...
 */
export function getLearningSummary(userId) {
    ...
    if (stats.calibration.length >= 3) {
        const avgError = stats.calibration.reduce((sum, b) => sum + b.calibrationError, 0) / stats.calibration.length;
 ...
 */
export async function initializePredictiveIntelligence() {
    if (initialized)
        return;
    log.info('🧠 Initializing Predictive Intelligence System');
    // Initialize persistence layer with data getters
    initializePersistence(getMarkovDataForPersistence, getTimeSeriesDataForPersistence, getReinforcementDataForPersistence);
 ...
 */
export async function shutdownPredictiveIntelligence() {
    if (!initialized)
        return;
    log.info('🧠 Shutting down Predictive Intelligence System');
    // Flush all pending state
    await shutdownPersistence(getMarkovDataForPersistence, getTimeSeriesDataForPersistence, getReinforcementDataForPersistence);
    initialized = false;
 ...
 */
export async function flushUserMLState(userId) {
    await forceFlushUser(userId, getMarkovDataForPersistence, getTimeSeriesDataForPersistence, getReinforcementDataForPersistence);
}
// ============================================================================
 ...
 */
export async function getPredictiveIntelligenceContext(userId, context = {}) {
    const sections = [];
    try {
        ...
        if (needsSupport && needsSupport.confidence > 0.6) {
            highConfidencePredictions.push(`• May need extra support: ${needsSupport.explanation.slice(0, 100)}`);
        }
        if (burnoutRisk && burnoutRisk.confidence > 0.5) {
            highConfidencePredictions.push(`• Burnout risk detected: ${burnoutRisk.explanation.slice(0, 100)}`);
        }
        if (readyForChallenge && readyForChallenge.confidence > 0.7) {
            highConfidencePredictions.push(`• Seems ready for growth challenge: ${readyForChallenge.explanation.slice(0, 100)}`);
        }
 ...
 */
export function getSuperhumanPredictiveContext(userId, context = {}) {
    const sections = [];
    try {
...
export { 
// Markov
recordTransition, recordSecondOrderTransition, predictNextStates, extractStatesFromTurn, 
// Time Series
recordObservation, forecast, findOptimalTimes, analyzeTrend, 
// Multi-Signal
fusePrediction, recordPredictionOutcome, 
// Reinforcement
trackPrediction, recordAction, recordOutcome, recordReward, getLearningStats, getCalibratedConfidence, getBestOutreachTimes, getMostAccurateSignals, cleanupOldPredictions, 
// Deep Analysis (LLM-powered)
runDeepAnalysis, getLatestDeepAnalysis, getDeepAnalysisContextForTurn, runBatchDeepAnalysis, recordDeepAnalysisFeedback, 
// ============================================================================
// BETTER THAN HUMAN v4 - SUPERHUMAN CAPABILITIES
// ============================================================================
// Avoidance Prediction
avoidancePrediction, buildAvoidanceContext, 
// Breakthrough Proximity
breakthroughProximity, buildBreakthroughContext, 
// Pre-Trajectory Detection
preTrajectoryDetection, buildPreTrajectoryContext, 
// Conversation Preparation
conversationPreparation, buildConversationPrepContext, 
// Cognitive Fingerprint
cognitiveFingerprint, buildFingerprintContext, 
// Ripple Effect Prediction
rippleEffectPrediction, buildRippleContext, 
// Life Phase Prediction
lifePhasePrediction, buildPhaseContext, 
// Intervention Timing
interventionTiming, buildInterventionTimingContext, 
// Signal Integration
signalIntegration, processTurnForSuperhumanLearning, recordInterventionFromTurn, recordBreakthroughMoment, recordLifeEvent, recordDecisionMade, recordStressObserved, recordVulnerabilityMoment, processSessionStart, processSessionEnd, 
// Superhuman Persistence
superhumanPersistence, markSuperhumanDirty, flushSuperhumanState, };
