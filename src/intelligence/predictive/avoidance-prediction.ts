...
export type AvoidableTopic =
  // Relationships
  | 'relationship:parent_mother'
  | 'relationship:parent_father'
  | 'relationship:partner'
  | 'relationship:ex'
  | 'relationship:sibling'
  | 'relationship:friend_specific'
  | 'relationship:boss'
  | 'relationship:colleague'
  // Life areas
  | 'area:career_dissatisfaction'
  | 'area:financial_stress'
  | 'area:health_concern'
  | 'area:mental_health'
  | 'area:addiction'
  | 'area:body_image'
  ...
  | 'area:loneliness'
  | 'area:failure'
  | 'area:dreams_abandoned'
  // Decisions
  | 'decision:pending_breakup'
  | 'decision:job_change'
  | 'decision:relocation'
  | 'decision:major_purchase'
  | 'decision:family_planning'
  // Emotions
  | 'emotion:anger'
  | 'emotion:grief'
  | 'emotion:shame'
...
interface UserAvoidanceProfile {
  userId: string;
  avoidedTopics: Map<AvoidableTopic, AvoidedTopic>;
  /** Topics they've successfully processed (no longer avoided) */
  resolvedTopics: AvoidableTopic[];
  /** Overall avoidance tendency (0-1) */
  avoidanceTendency: number;
  /** How pressure builds over time for them */
  pressureBuildupRate: number;
  /** Global deflection preferences */
  preferredDeflectionStyles: DeflectionStyle[];
  lastUpdated: number;
}
...
const userProfiles = new Map<string, UserAvoidanceProfile>();
 ...
 */
export function recordDeflection(
  userId: string,
  topic: AvoidableTopic,
  style: DeflectionStyle,
  context: {
    triggerTopic?: string;
    emotionalState?: string;
 ...
 */
export function recordTopicEngagement(
  userId: string,
  topic: AvoidableTopic,
  depth: number,
  context: {
  ...
  if (avoided) {
    ...
    if (depth > 0.8) {
      log.info(
        { userId, topic, depth },
        '🎯 Deep engagement with avoided topic - potential resolution'
      );
    }
  }
 ...
 */
export function detectAvoidanceFromConversation(
  userId: string,
  analysis: {
 ...
 */
export function predictSurfacing(
  userId: string,
  topic: AvoidableTopic
): AvoidanceSurfacingPrediction | null {
  ...
  const deflectionPressure = Math.min(
    1,
    avoided.deflectionCount * CONFIG.DEFLECTION_PRESSURE_INCREASE
  );
  ...
  let pressureLevel =
    (timePressure * 0.4 + deflectionPressure * 0.6 + recentDeflectionBoost) *
  ...
  // Determine timeframe
  let expectedTimeframe: AvoidanceSurfacingPrediction['expectedTimeframe'];
  ...
  let sensitivityLevel: AvoidanceSurfacingPrediction['sensitivityLevel'];
  if (
    avoided.emotionalStateOnDeflection.some((e) =>
      ['distressed', 'tearful', 'angry', 'shutdown'].includes(e)
    )
  ) {
  ...
  const confidence =
    avoided.deflectionCount >= CONFIG.MIN_DEFLECTIONS_FOR_PATTERN
      ? Math.min(0.9, 0.5 + avoided.deflectionCount * 0.05)
      : 0.3;

  // Identify surfacing triggers
  const surfacingTriggers: string[] = [];
  if (pressureLevel > 0.5) surfacingTriggers.push('accumulated emotional pressure');
  if (avoided.triggerTopics.length > 0)
    surfacingTriggers.push(`adjacent topics: ${avoided.triggerTopics.slice(0, 3).join(', ')}`);
  if (daysSinceLastMention > 30) surfacingTriggers.push('long suppression period');
...
export function getAllAvoidancePredictions(
  ...
  const predictions: AvoidanceSurfacingPrediction[] = [];
 ...
 */
export function getImminentTopics(
  userId: string,
  threshold: number = 0.4
): AvoidanceSurfacingPrediction[] {
  return getAllAvoidancePredictions(userId).filter(
    (p) => p.surfacingProbability >= threshold || p.expectedTimeframe === 'imminent'
  );
}
 ...
 */
export function buildAvoidanceContext(userId: string): string {
...
function getOrCreateProfile(userId: string): UserAvoidanceProfile {
...
function mapToAvoidableTopic(topic: string): AvoidableTopic | null {
  const mapping: Record<string, AvoidableTopic> = {
...
function determineOptimalApproach(
  avoided: AvoidedTopic,
  profile: UserAvoidanceProfile
): AvoidanceSurfacingPrediction['optimalApproach'] {
  // Determine timing
