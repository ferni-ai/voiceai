...
import { loadTimeSeriesState, markDirty, isUserLoaded, markUserLoaded, } from './persistence.js';
import { isTimeSeriesForecastingAvailable, calculateStatisticsF32, calculateLinearTrendF32, exponentialSmoothingF32, } from '../../memory/rust-accelerator.js';
 ...
 */
export function recordObservation(userId, series, value, timestamp = new Date(), metadata) {
    ...
    markDirty(userId);
    log.debug({ userId, series, value, totalPoints: profile.series[series].length }, '📊 Recorded time series observation');
}
/**
 ...
 */
export function forecast(userId, series, targetTime = new Date()) {
 ...
 */
export function findOptimalTimes(userId, series, daysAhead = 7) {
    const results = [];
    const now = new Date();
    ...
    results.sort((a, b) => isNegativeMetric
 ...
 */
export function analyzeTrend(userId, series) {
    const profile = userProfiles.get(userId);
    ...
    const previousPoints = data.filter((p) => p.timestamp > now - 2 * weekMs && p.timestamp <= now - weekMs);
 ...
 */
async function loadTimeSeriesProfileFromFirestore(userId) {
    try {
        ...
        if (data) {
            ...
            if (profile) {
                // Restore series data
                if (data.mood)
                    profile.series.mood = data.mood;
                if (data.energy)
                    profile.series.energy = data.energy;
                if (data.engagement)
                    profile.series.engagement = data.engagement;
                if (data.stress)
                    profile.series.stress = data.stress;
                // Restore seasonality
                if (data.seasonality) {
                ...
                // Recalculate stats
                for (const seriesType of ['mood', 'energy', 'engagement', 'stress']) {
                ...
                const totalPoints = profile.series.mood.length +
...
function createDefaultSeasonality() {
    return {
...
function createInsufficientDataForecast(targetTime) {
    return {
        targetTime,
...
function calculateSeasonality(data) {
    ...
    const seasonalDow = dayOfWeek.map((sum, i) => dayOfWeekCounts[i] > 2 ? 1 + sum / dayOfWeekCounts[i] / overallMean : 1);
    const seasonalHod = hourOfDay.map((sum, i) => hourOfDayCounts[i] > 2 ? 1 + sum / hourOfDayCounts[i] / overallMean : 1);
    const seasonalWom = weekOfMonth.map((sum, i) => weekOfMonthCounts[i] > 2 ? 1 + sum / weekOfMonthCounts[i] / overallMean : 1);
    return {
...
function calculateSmoothedTrend(data) {
    if (data.length === 0)
        return { level: 0.5, trend: 0 };
    if (data.length === 1)
        return { level: data[0].value, trend: 0 };
...
function getSeasonalAdjustment(seasonality, targetTime) {
...
function calculateForecastError(data, level, trend, seasonality) {
    if (data.length < 3)
...
function determineReliability(dataPoints, forecastError, hoursAhead) {
    if (dataPoints < 7)
        return 'low';
    if (forecastError > 0.25 || hoursAhead > 168)
        return 'low'; // 1 week
    if (dataPoints >= 30 && forecastError < 0.15 && hoursAhead < 48)
        return 'high';
    return 'medium';
}
function detectAnomaly(value, stats) {
    if (stats.stdDev === 0)
        return undefined;
 ...
 */
export function getTimeSeriesDataForPersistence(userId) {
    const profile = userProfiles.get(userId);
    if (!profile)
        return null;
    return {
...
//# sourceMappingURL=time-series-forecaster.js.map