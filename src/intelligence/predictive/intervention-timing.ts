...
/** User's timing profile */
interface UserTimingProfile {
  userId: string;
  /** Learned patterns per intervention type */
  patterns: Map<InterventionType, TimingPattern>;
  /** Outcome history */
...
export function recordInterventionOutcome(
  ...
  const fullOutcome: InterventionOutcome = {
    ...outcome,
    timestamp: now,
  };
 ...
 */
export function recordQuickOutcome(
  userId: string,
  interventionType: InterventionType,
  success: boolean,
  context: {
...
export function getTimingRecommendation(
  ...
  // Check time of day
  let currentTimeOfDay: 'morning' | 'afternoon' | 'evening' | 'night';
  if (hour < 12) currentTimeOfDay = 'morning';
  else if (hour < 17) currentTimeOfDay = 'afternoon';
  ...
  let riskLevel: TimingRecommendation['riskLevel'] = 'low';
  ...
  let alternative: TimingRecommendation['alternative'] | undefined;
  let betterTiming: string | undefined;
 ...
 */
export function getAllTimingRecommendations(
  userId: string,
  context: {
    emotionalState?: string;
    topic?: string;
 ...
 */
export function buildInterventionTimingContext(
  userId: string,
  context: {
    emotionalState?: string;
    topic?: string;
  ...
  if (topRecommended.length === 0 && toAvoid.length === 0) return '';
...
function updatePattern(profile: UserTimingProfile, outcome: InterventionOutcome): void {
  ...
  if (outcome.outcome === 'positive') {
    ...
    const timeOfDay = outcome.conditions.timeOfDay as 'morning' | 'afternoon' | 'evening' | 'night';
...
function evaluateIntervention(
  profile: UserTimingProfile | undefined,
  interventionType: InterventionType,
  context: {
    emotionalState?: string;
    topic?: string;
