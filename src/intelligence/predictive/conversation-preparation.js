 ...
 */
export function recordTopicDiscussion(userId, entry) {
 ...
 */
export function recordConversationNeed(userId, need, context) {
    const profile = getOrCreateProfile(userId);
    const now = new Date();
 ...
 */
export function recordConversationOutcome(userId, outcome) {
    const profile = getOrCreateProfile(userId);
 ...
 */
export function recordTemporalPattern(userId, pattern) {
    const profile = getOrCreateProfile(userId);
    // Check if similar pattern exists
    const existingIdx = profile.temporalPatterns.findIndex((p) => p.dayOfWeek === pattern.dayOfWeek &&
 ...
 */
export function prepareForConversation(userId, context = {}) {
    const profile = getOrCreateProfile(userId);
    ...
    // Generate opening
    const suggestedOpening = generateOpening(predictedTopics, predictedEmotionalState, predictedNeeds, context);
    ...
    const overallConfidence = calculateOverallConfidence(profile, predictedTopics, predictedEmotionalState, predictedNeeds);
    const preparation = {
        userId,
        generatedAt: now,
        predictedTopics,
 ...
 */
export function buildConversationPrepContext(userId) {
...
function updateRecurringPattern(profile, topic, entry) {
...
function predictTopics(profile, targetTime, context) {
    const predictions = [];
    const now = Date.now();
    ...
    const matchingTemporalPatterns = profile.temporalPatterns.filter((p) => p.dayOfWeek === undefined || p.dayOfWeek === dayOfWeek);
...
function predictEmotionalState(profile, targetTime, context) {
...
function predictNeeds(profile, targetTime, context) {
    const needs = [];
    const dayOfWeek = targetTime.getDay();
    ...
    for (const entry of recentNeeds) {
        ...
        const weight = (entry.dayOfWeek === dayOfWeek ? 1.5 : 1) *
...
function generateOpening(predictedTopics, emotionalState, needs, context) {
...
function identifyProactiveTopics(profile, predictedTopics, context) {
    const proactive = [];
...
function identifyWarmupTopics(profile, predictedTopics) {
    const warmup = [];
...
function identifyTopicsToAvoid(profile, context) {
    const avoid = [];
...
function gatherRelevantContext(profile, predictedTopics) {
    const context = [];
...
function determinePacing(emotionalState, needs, context) {
    const primaryNeed = needs[0]?.need;
    const emotion = emotionalState.primaryEmotion;
    let recommendedLength = 'normal';
    let energyLevel = 'moderate';
    let depthLevel = 'moderate';
...
function calculateOverallConfidence(profile, topics, emotionalState, needs) {
    ...
    if (profile.outcomes.length > 5) {
        const recentAccuracy = profile.outcomes.slice(-5).reduce((sum, o) => sum + o.predictedTopicsHit.length /
            Math.max(1, o.predictedTopicsHit.length + o.unexpectedTopics.length), 0) / 5;
...
//# sourceMappingURL=conversation-preparation.js.map