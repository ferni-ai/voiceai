        ...
        case 'person':
            ...
            if (entity.attributes && '_type' in entity.attributes) {
                ...
                if (attrs.relationship)
                    parts.push(`relationship: ${attrs.relationship}`);
                if (attrs.context)
                    parts.push(attrs.context);
            }
            break;
        case 'event':
 ...
 */
export async function generateEntityEmbedding(entity) {
    const text = buildEntityEmbeddingText(entity);
    const embedding = await embed(text);
    log.debug({ entityId: entity.id, entityName: entity.canonicalName }, '🔄 Generated entity embedding');
    return embedding;
}
/**
 * Batch generate embeddings for multiple entities
 */
export async function generateEntityEmbeddingsBatch(entities) {
 ...
 */
export async function feedEntityMentionToTrajectory(userId, mention) {
    ...
    if (domain) {
        ...
        log.debug({ userId, entityName: mention.entityName, domain }, '🔄 Fed entity mention to ripple space');
    }
}
/**
 * Link entity to avoidance pattern if relevant
 */
export async function checkEntityForAvoidanceLink(userId, entity) {
    ...
    const result = await semanticAvoidance.isNearAvoidedTerritory(userId, entity.canonicalName, buildEntityEmbeddingText(entity));
    if (result.isNear) {
        log.info({ userId, entityName: entity.canonicalName, distance: result.distance }, '⚠️ Entity linked to avoidance pattern');
        return true;
    }
    return false;
}
 ...
 */
export async function recordEntityInBreakthrough(userId, entityIds, breakthroughId, entities) {
    // Store the link for future reference
    // This could be used to identify which entities are catalysts for breakthroughs
    log.debug({ userId, entityCount: entityIds.length, breakthroughId }, '🔄 Recorded entities in breakthrough');
 ...
 */
export async function syncEntityToDomainSpace(userId, entity) {
    const { rippleEmbeddingSpace } = await import('./ripple-embedding-space.js');
 ...
 */
export async function suggestEntitiesFromPatterns(userId) {
    const suggestions = [];
 ...
 */
export async function updateEntitySalienceFromPatterns(userId, entityId, factor) {
...
function entityTypeToDomain(entityType) {
    const mapping = {
...
function inferTargetDomain(entity) {
...
function parseEmotionalValence(emotionalContext) {
 ...
 */
export async function syncAllEntitiesToPredictive(userId) {
    ...
    try {
        ...
        log.info({ userId, entitiesProcessed, avoidanceLinksFound, domainUpdates }, '🔄 Full entity sync completed');
    }
    catch (error) {
...
//# sourceMappingURL=entity-embedding-sync.js.map