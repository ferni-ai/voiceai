 ...
 */
export function startTrajectory(sessionId, userId) {
    const trajectory = {
        sessionId,
        userId,
        startTime: Date.now(),
 ...
 */
export async function recordTurn(sessionId, turn, avoidedTopicEmbeddings) {
 ...
 */
export function analyzeTrajectory(sessionId) {
    const trajectory = activeTrajectories.get(sessionId);
    if (!trajectory || trajectory.turns.length < 3)
        return null;
    ...
    // Determine depth
    const avgDepth = trajectory.turns.reduce((sum, t) => sum + t.topicDepth, 0) / trajectory.turns.length;
    const depth = avgDepth > 0.7 ? 'deep' :
        avgDepth > 0.4 ? 'moderate' : 'surface';
    // Determine coherence
    const coherence = metrics.topicCoherence > 0.7 ? 'focused' :
        metrics.topicCoherence > 0.4 ? 'moderate' : 'scattered';
    ...
    const emotionalDirection = emotionVolatility > 0.4 ? 'volatile' :
        emotionTrend > 0.1 ? 'improving' :
            emotionTrend < -0.1 ? 'declining' : 'stable';
    // Generate recommendations
    const recommendations = generateRecommendations(pattern, depth, coherence, emotionalDirection, metrics);
    return {
        pattern,
        depth,
        coherence,
 ...
 */
export async function checkAvoidanceApproach(sessionId, avoidedTopicEmbeddings) {
    const trajectory = activeTrajectories.get(sessionId);
    ...
    for (let i = 0; i < avoidedTopicEmbeddings.length; i++) {
        ...
        const currentDistance = 1 - cosineSimilarity(recentTurns[recentTurns.length - 1].embedding, avoidedEmb);
    ...
    if (recentTurns.length >= 2 && nearestTopic >= 0) {
        const previousDistance = 1 - cosineSimilarity(recentTurns[recentTurns.length - 2].embedding, avoidedTopicEmbeddings[nearestTopic]);
        const direction = minCurrentDistance < previousDistance - 0.05 ? 'toward' :
            minCurrentDistance > previousDistance + 0.05 ? 'away' : 'stable';
        return {
 ...
 */
export function getSemanticDistance(sessionId, turnA, turnB) {
    const trajectory = activeTrajectories.get(sessionId);
    if (!trajectory)
        return null;
 ...
 */
export function endTrajectory(sessionId) {
    const trajectory = activeTrajectories.get(sessionId);
    activeTrajectories.delete(sessionId);
    if (trajectory) {
        log.debug({ sessionId, turns: trajectory.turns.length, drift: trajectory.metrics.semanticDrift }, '📊 Ended conversation trajectory');
    }
...
// METRIC UPDATES
// ============================================================================
async function updateMetrics(trajectory, avoidedTopicEmbeddings) {
...
function detectPivotPoints(trajectory) {
    ...
    if (currentTurn.topicDepth > previousTurn.topicDepth + 0.2 &&
    ...
    if (Math.abs(currentTurn.emotionalValence) > 0.7 &&
...
function estimateDepth(text) {
    ...
    const deepIndicators = [
        'i realize', 'i feel', 'i think', 'i believe', 'i need', 'i want',
        'scared', 'afraid', 'anxious', 'sad', 'angry', 'frustrated',
        'love', 'hate', 'hurt', 'vulnerable', 'ashamed', 'guilty',
        'always', 'never', 'everything', 'nothing',
        'why', 'how do i', 'what if', 'i wonder',
        'my father', 'my mother', 'my childhood', 'growing up',
    ];
    // Surface indicators
    const surfaceIndicators = [
        'yeah', 'okay', 'sure', 'fine', 'good', 'great',
        'weather', 'work', 'weekend', 'plan', 'schedule',
    ];
    let deepScore = 0;
    let surfaceScore = 0;
...
function generateRecommendations(pattern, depth, coherence, emotionalDirection, metrics) {
    const recs = [];
    if (pattern === 'circling') {
...
//# sourceMappingURL=conversation-trajectory.js.map