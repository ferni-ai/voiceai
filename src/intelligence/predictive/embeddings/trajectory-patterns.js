...
// STORAGE
// ============================================================================
const userPatternLibrary = new Map();
// ============================================================================
// CORE FUNCTIONS
// ============================================================================
/**
 ...
 */
export async function recordTrajectoryPattern(userId, pattern) {
    ...
    log.info({ userId, trajectory: pattern.trajectory, severity: pattern.severity }, '📈 Recorded trajectory pattern');
    return fullPattern;
}
/**
 * Find similar patterns from history
 */
export async function findSimilarPatterns(userId, currentState, k = 5) {
    const patterns = userPatternLibrary.get(userId) || [];
    if (patterns.length === 0)
        return [];
    // Embed current state
    const precursorText = buildPrecursorText(currentState.signals.map((s) => ({
        signal: s.signal,
        value: s.value,
        daysBeforeOnset: 0,
    })));
...
export async function predictTrajectoryFromPatterns(userId, currentState) {
    ...
    for (const match of matches) {
        ...
        const avgLeadTime = match.pattern.precursorSignals.reduce((sum, s) => sum + s.daysBeforeOnset, 0) / (match.pattern.precursorSignals.length || 1);
 ...
 */
export async function recordTrajectoryOutcome(userId, patternId, outcome) {
    const patterns = userPatternLibrary.get(userId) || [];
    const pattern = patterns.find((p) => p.id === patternId);
    if (pattern) {
        pattern.resolution = outcome.resolution;
        if (outcome.actualSeverity !== undefined)
            pattern.severity = outcome.actualSeverity;
        if (outcome.actualDuration !== undefined)
            pattern.duration = outcome.actualDuration;
        if (outcome.helpfulInterventions)
            pattern.helpfulInterventions = outcome.helpfulInterventions;
 ...
 */
export function getTrajectoryStats(userId) {
    const patterns = userPatternLibrary.get(userId) || [];
    const byTrajectory = {};
    let totalSeverity = 0;
    const interventions = [];
...
// HELPERS
// ============================================================================
function buildTrajectoryText(pattern) {
    const parts = [
...
function buildPrecursorText(signals) {
    if (signals.length === 0)
        return 'no precursor signals';
    return signals
        .map((s) => `${s.signal.replace(/_/g, ' ')}: ${s.value.toFixed(2)} (${s.daysBeforeOnset}d before)`)
        .join(', ');
}
function generateImplication(pattern, similarity) {
...
function formatOnset(daysFromNow) {
    if (daysFromNow < 1)
        return 'within 24 hours';
    if (daysFromNow < 2)
        return 'within 1-2 days';
    if (daysFromNow < 4)
        return 'within 2-4 days';
    if (daysFromNow < 7)
 ...
 */
export async function buildTrajectoryPatternContext(userId, currentState) {
    const prediction = await predictTrajectoryFromPatterns(userId, currentState);
    if (!prediction || prediction.probability < 0.4)
        return '';
    const sections = ['[TRAJECTORY PATTERN INTELLIGENCE]'];
...
//# sourceMappingURL=trajectory-patterns.js.map