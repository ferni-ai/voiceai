...
const userSituationLibrary = new Map<string, SituationEmbedding[]>();
 ...
 */
export async function recordSituationOutcome(
  userId: string,
  situation: {
    transcript: string;
    emotionalState: string;
...
export async function getInterventionRecommendations(
  ...
  for (const [intervention, stats] of Object.entries(interventionStats)) {
    ...
    recommendations.push({
      ...
      successRate,
      sampleSize,
      reasoning: generateReasoning(intervention as InterventionType, stats.situations, successRate),
      antiPatterns,
    });
  }
 ...
 */
export async function getBestIntervention(
  userId: string,
  currentSituation: CurrentSituation
 ...
 */
export async function getInterventionSuccessRate(
  userId: string,
  intervention: InterventionType,
  currentSituation: CurrentSituation
 ...
 */
export function getSuccessfulSituations(
  userId: string,
  intervention: InterventionType,
  limit = 5
): SituationEmbedding[] {
...
function describeAntiPattern(situation: SituationEmbedding): string {
  const parts = [];
...
function describeConditions(situation: SituationEmbedding): string {
  const parts = [];
 ...
 */
export async function buildInterventionMatchingContext(
  userId: string,
  currentSituation: CurrentSituation
): Promise<string> {
