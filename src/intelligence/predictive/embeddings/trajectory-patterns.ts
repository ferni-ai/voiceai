...
export async function recordTrajectoryPattern(
  ...
  pattern: Omit<
    TrajectoryPattern,
    'id' | 'trajectoryEmbedding' | 'precursorEmbedding' | 'contextEmbedding'
  >
...
export async function findSimilarPatterns(
  ...
  const [currentPrecursorEmb, currentContextEmb] = await embedBatch([precursorText, contextText]);
...
export async function predictTrajectoryFromPatterns(
  ...
  const trajectoryVotes: Record<
    string,
    { count: number; totalWeight: number; severities: number[]; onsets: number[] }
  > = {};
  ...
  for (const match of matches) {
    ...
    const avgLeadTime =
      match.pattern.precursorSignals.reduce((sum, s) => sum + s.daysBeforeOnset, 0) /
...
export async function recordTrajectoryOutcome(
  ...
  if (pattern) {
    ...
    log.info(
      { userId, patternId, resolution: outcome.resolution },
...
function buildPrecursorText(
  ...
  return signals
    .map(
      (s) => `${s.signal.replace(/_/g, ' ')}: ${s.value.toFixed(2)} (${s.daysBeforeOnset}d before)`
    )
    .join(', ');
}
...
function generateImplication(pattern: TrajectoryPattern, similarity: number): string {
  const confidenceWord =
    similarity > 0.8 ? 'strongly' : similarity > 0.6 ? 'moderately' : 'somewhat';
...
export async function buildTrajectoryPatternContext(
  ...
  sections.push(
    `• Similar signals in past led to: ${prediction.likelyTrajectory.replace(/_/g, ' ')}`
  );
