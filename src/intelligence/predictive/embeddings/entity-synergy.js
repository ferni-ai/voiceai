 ...
 */
export async function findEntitiesRelatedToAvoidance(userId, avoidedTopicEmbedding, entityTypes) {
    try {
 ...
 */
export async function getEntityContextForTrajectory(userId, trajectoryDescription, lifeDomains) {
    try {
        const store = await getEntityStore();
        ...
        const domainRelevantEntities = sortedEntities.filter((entity) => lifeDomains.some((domain) => isEntityRelevantToDomain(entity, domain)));
 ...
 */
export async function findEntitiesForBreakthrough(userId, breakthroughTopic, insightEmbedding) {
    try {
        const store = await getEntityStore();
 ...
 */
export async function getEntitiesForDomain(userId, domain) {
    try {
        const store = await getEntityStore();
        // Get all entities
        const allEntities = await store.getUserEntities(userId, { limit: 100 });
 ...
 */
export async function findPeopleInSituation(userId, situationTranscript) {
    try {
        const store = await getEntityStore();
        // Get person entities
 ...
 */
export async function enrichAvoidanceWithEntities(userId, avoidedTopics) {
    try {
        const store = await getEntityStore();
        const allEntities = await store.getUserEntities(userId, { limit: 50 });
        const enriched = [];
...
// HELPERS
// ============================================================================
function inferAvoidanceReasons(entity) {
    const reasons = [];
...
function isEntityRelevantToDomain(entity, domain) {
...
function inferInfluenceType(entity, description) {
...
function describeEntityConnection(entity, topic) {
    const name = getEntityName(entity);
    switch (entity.type) {
        case 'person':
 ...
 */
export async function buildEntityEnrichedContext(userId, currentTopic) {
    try {
        const store = await getEntityStore();
        const sections = [];
        ...
        if (currentTopic) {
            ...
            const topicEntities = sortedEntities.filter((e) => getEntitySearchText(e).includes(topicLower));
...
//# sourceMappingURL=entity-synergy.js.map