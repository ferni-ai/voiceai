...
export interface BreakthroughEmbedding {
  ...
  impact: number; // 0-1, how transformative

  // Embeddings
  contextEmbedding: number[]; // Conversation context leading up
  indicatorEmbedding: number[]; // What indicators were present
  catalystEmbedding: number[]; // What triggered the breakthrough
  insightEmbedding: number[]; // The insight itself
...
const userBreakthroughLibrary = new Map<string, BreakthroughEmbedding[]>();
 ...
 */
export async function recordBreakthroughWithEmbeddings(
  userId: string,
  breakthrough: {
    topic: string;
    type: BreakthroughType;
    insightSummary: string;
    impact: number;
    indicators: Array<{ type: IndicatorType; strength: number; content: string }>;
  ...
  const indicatorText = breakthrough.indicators.map((i) => `${i.type}: ${i.content}`).join('. ');
  ...
  // Generate embeddings
  const [contextEmbedding, indicatorEmbedding, catalystEmbedding, insightEmbedding] =
    await embedBatch([contextText, indicatorText, catalystText, insightText]);

  const record: BreakthroughEmbedding = {
  ...
  log.info(
    ...
    '💡 Recorded breakthrough embedding'
  );
 ...
 */
export async function findSimilarBreakthroughs(
  userId: string,
  currentState: {
    topic: string;
    conversationContext: string;
  ...
  const indicatorText = currentState.indicators.map((i) => `${i.type}: ${i.content}`).join('. ');
  ...
  const scored: BreakthroughMatch[] = [];
  ...
  for (const breakthrough of library) {
    ...
    const overallSimilarity =
      indicatorSimilarity * 0.4 + contextSimilarity * 0.3 + topicSimilarity * 0.3;
 ...
 */
export async function predictBreakthroughReadiness(
  userId: string,
  currentState: Parameters<typeof findSimilarBreakthroughs>[1]
): Promise<BreakthroughPrediction | null> {
  ...
  for (const match of matches) {
    typeCounts[match.breakthrough.type] =
      (typeCounts[match.breakthrough.type] || 0) + match.likelihood;
  }
  const likelyType =
    (Object.entries(typeCounts).sort((a, b) => b[1] - a[1])[0]?.[0] as BreakthroughType) ||
    'self_understanding';

  // Find optimal catalysts
  const catalystSuccess: Record<string, { count: number; totalImpact: number }> = {};
  for (const match of matches) {
 ...
 */
export function getOptimalCatalysts(userId: string): Array<{
  type: string;
  successRate: number;
  ...
  const catalystStats: Record<string, { count: number; totalImpact: number }> = {};
 ...
 */
export async function getBreakthroughsByTopic(
  userId: string,
  topic: string,
  k = 5
): Promise<BreakthroughEmbedding[]> {
...
function generateGuidance(breakthrough: BreakthroughEmbedding, similarity: number): string {
    ...
    timeAgo === 0
      ...
      : timeAgo === 1
        ...
        : timeAgo < 7
          ? `${timeAgo} days ago`
          : timeAgo < 30
            ? `${Math.round(timeAgo / 7)} weeks ago`
...
function getCatalystDescription(type: string): string {
...
function generateRecommendation(
  matches: BreakthroughMatch[],
  optimalCatalysts: Array<{ type: string; description: string; historicalSuccess: number }>
): string {
  if (matches.length === 0) return 'Continue building trust and exploring.';
 ...
 */
export async function buildBreakthroughEmbeddingContext(
  userId: string,
  currentState: Parameters<typeof findSimilarBreakthroughs>[1]
): Promise<string> {
  const prediction = await predictBreakthroughReadiness(userId, currentState);

  if (!prediction || prediction.readiness < 0.4) return '';

  const sections: string[] = ['[BREAKTHROUGH PATTERN INTELLIGENCE]'];
  ...
  if (prediction.optimalCatalysts.length > 0) {
    ...
    for (const catalyst of prediction.optimalCatalysts) {
      sections.push(
        `• ${catalyst.type}: ${catalyst.description} (${Math.round(catalyst.historicalSuccess * 100)}% success)`
      );
    }
  }
 ...
 */
export function hydrateFromPersistence(userId: string, data: BreakthroughPersistenceData): void {
