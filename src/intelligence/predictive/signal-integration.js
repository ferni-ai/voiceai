 ...
 */
export async function processTurnForSuperhumanLearning(userId, turnData) {
    try {
        // =========================================
        ...
        if (deflectionAnalysis.detected && deflectionAnalysis.topic && deflectionAnalysis.style) {
            avoidancePrediction.recordDeflection(userId, deflectionAnalysis.topic, deflectionAnalysis.style, {
 ...
 */
export function recordInterventionFromTurn(userId, intervention, outcome) {
 ...
 */
export function recordBreakthroughMoment(userId, breakthrough) {
    breakthroughProximity.recordBreakthrough(userId, breakthrough.topic, breakthrough.type, breakthrough.catalyst);
 ...
 */
export function recordLifeEvent(userId, event) {
 ...
 */
export function recordDecisionMade(userId, decision) {
 ...
 */
export function recordStressObserved(userId, response) {
 ...
 */
export function recordVulnerabilityMoment(userId, vulnerability) {
 ...
 */
export async function processSessionStart(userId, sessionData) {
 ...
 */
export async function processSessionEnd(userId, sessionSummary) {
    ...
    if (sessionSummary.emotionalArc) {
        ...
        if (mentalHealthChange > 0) {
            rippleEffectPrediction.updateDomainHealth(userId, 'mental_health', 0.6 + mentalHealthChange // Relative update
            );
        }
    }
...
// HELPER FUNCTIONS
// ============================================================================
function detectSelfTalkValence(message) {
...
function detectFutureOrientation(message) {
    const lower = message.toLowerCase();
...
function detectSocialMentions(message) {
    const lower = message.toLowerCase();
    // Social references
    const socialWords = [
...
function mapToTopicCategory(category) {
    if (!category)
        return 'emotions';
    const mapping = {
        career: 'work',
...
//# sourceMappingURL=signal-integration.js.map