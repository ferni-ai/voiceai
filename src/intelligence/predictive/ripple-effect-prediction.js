 ...
 */
export function recordDomainEvent(userId, event) {
 ...
 */
export function recordObservedRipple(userId, sourceEvent, observedEffect) {
    const profile = userProfiles.get(userId);
    if (!profile)
        return;
    // Find or create pattern
    let pattern = profile.influencePatterns.find((p) => p.sourceDomain === sourceEvent.domain &&
 ...
 */
export function updateDomainHealth(userId, domain, health) {
    const profile = getOrCreateProfile(userId);
 ...
 * @returns Ripple prediction
 */
function predictRipples(profile, event) {
    const ripples = [];
    ...
    for (const targetDomain of allDomains) {
        if (targetDomain === event.domain)
            continue;
        // Check for learned pattern
        const learnedPattern = profile.influencePatterns.find((p) => p.sourceDomain === event.domain &&
            p.targetDomain === targetDomain &&
            p.eventType === event.eventType);
        // Check default influence
        const defaultInfluence = CONFIG.DEFAULT_INFLUENCES.find((i) => i.from === event.domain && i.to === targetDomain);
 ...
 */
export function getRippleStatus(userId) {
    const profile = userProfiles.get(userId);
    if (!profile) {
        return {
            domainStates: [],
 ...
 */
export function simulateRipples(userId, hypotheticalEvent) {
    const profile = getOrCreateProfile(userId);
    const event = {
        ...hypotheticalEvent,
        timestamp: Date.now(),
 ...
 */
export function buildRippleContext(userId) {
    const status = getRippleStatus(userId);
    if (status.domainStates.length === 0)
        return '';
    const sections = [];
...
function updateDomainState(profile, event) {
...
function determineEffect(event, targetDomain, magnitude) {
    // Common patterns
    if (magnitude < -0.3) {
        if (targetDomain === 'energy')
            return 'energy_drain';
        if (targetDomain === 'mental_health')
            return 'increased_stress';
        if (targetDomain === 'sleep')
            return 'decline';
        if (targetDomain === 'relationships')
            return 'conflict_spillover';
        if (targetDomain === 'self_care')
            return 'neglect';
        return 'decline';
    }
    if (magnitude > 0.3) {
...
function determineTimeframe(pattern, event) {
    if (pattern?.typicalDelay) {
        if (pattern.typicalDelay < 24)
            return 'immediate';
        if (pattern.typicalDelay < 72)
            return 'days';
        if (pattern.typicalDelay < 168)
            return 'weeks';
        return 'months';
    }
    // Default based on event magnitude
    if (Math.abs(event.magnitude) > 0.7)
        return 'immediate';
    if (Math.abs(event.magnitude) > 0.4)
        return 'days';
    return 'weeks';
}
function determineMitigation(event, targetDomain, effect) {
...
function generateReasoning(event, targetDomain, effect, pattern) {
...
function determineCascadeRisk(ripples, profile) {
    ...
    const riskScore = highProbRipples.length * 0.2 +
        negativeRipples.length * 0.15 +
        weakDomains.length * 0.1;
    if (riskScore >= CONFIG.CASCADE_RISK_THRESHOLDS.critical)
        return 'critical';
    if (riskScore >= CONFIG.CASCADE_RISK_THRESHOLDS.high)
        return 'high';
    if (riskScore >= CONFIG.CASCADE_RISK_THRESHOLDS.moderate)
        return 'moderate';
    return 'low';
}
function identifyLeveragePoints(profile, ripples) {
    const points = [];
...
//# sourceMappingURL=ripple-effect-prediction.js.map