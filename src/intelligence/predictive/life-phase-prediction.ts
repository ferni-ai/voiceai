...
interface UserPhaseProfile {
  ...
  /** Phase tendency scores */
  phaseTendencies: Map<LifePhase, number>;
  lastUpdated: number;
}
...
const CONFIG = {
  ...
  } as Record<LifePhase, { min: number; max: number }>,
  ...
  ] as Array<{ from: LifePhase; to: LifePhase; probability: number }>,
...
const userProfiles = new Map<string, UserPhaseProfile>();
 ...
 */
export function recordPhaseSignal(
  userId: string,
  signal: PhaseSignal,
  strength: number,
  context?: string
 ...
 */
export function predictPhase(userId: string): PhasePrediction | null {
  const profile = userProfiles.get(userId);
  if (!profile || profile.observations.length < 5) return null;
 ...
 */
export function getPhaseInfo(userId: string): {
  phase: LifePhase;
  confidence: number;
  daysInPhase: number;
  summary: string;
  ...
  const summaries: Record<LifePhase, string> = {
 ...
 */
export function buildPhaseContext(userId: string): string {
  const prediction = predictPhase(userId);
...
function getOrCreateProfile(userId: string): UserPhaseProfile {
...
function transitionPhase(profile: UserPhaseProfile, newPhase: LifePhase): void {
  const now = Date.now();
  ...
  if (oldDuration) {
    profile.phasePatterns.typicalDurations.set(
      profile.currentPhase,
      oldDuration * 0.8 + durationDays * 0.2
    );
  } else {
  ...
  const existingSeq = profile.phasePatterns.commonSequences.find(
    (s) => s.from === profile.currentPhase && s.to === newPhase
  );
  if (existingSeq) {
...
function calculatePhaseScores(profile: UserPhaseProfile): Map<LifePhase, number> {
  const scores = new Map<LifePhase, number>();
  const now = Date.now();
  const decayDays = 14;

  // Initialize scores
  for (const phase of Object.keys(CONFIG.PHASE_SIGNALS) as LifePhase[]) {
    scores.set(phase, 0);
  }
  ...
  for (const obs of profile.observations) {
    ...
    for (const [phase, signals] of Object.entries(CONFIG.PHASE_SIGNALS)) {
      const typedPhase = phase as LifePhase;
...
function getActiveSignals(
  profile: UserPhaseProfile,
  phase: LifePhase
): PhasePrediction['activeSignals'] {
  const now = Date.now();
  const recentDays = 7;
  const signals: PhasePrediction['activeSignals'] = [];

  const recentObs = profile.observations.filter(
    (o) => now - o.timestamp < recentDays * 24 * 60 * 60 * 1000
  );
...
function getPhaseNeeds(phase: LifePhase): PhasePrediction['phaseNeeds'] {
  const needs: Record<LifePhase, PhasePrediction['phaseNeeds']> = {
    expansion: {
...
function calculatePhaseHealth(
  profile: UserPhaseProfile,
  phase: LifePhase,
  activeSignals: PhasePrediction['activeSignals']
): PhasePrediction['phaseHealth'] {
...
function getCommonTriggers(phase: LifePhase): string[] {
  const triggers: Record<LifePhase, string[]> = {
