...
import { predictNextStates, } from './markov-sequence-predictor.js';
import { forecast, analyzeTrend } from './time-series-forecaster.js';
import { predictOptimalTiming, } from '../../services/trust-systems/outreach-timing-ml.js';
 ...
 */
export async function fusePrediction(userId, target, context = {}) {
...
// SIGNAL COLLECTORS
// ============================================================================
async function collectMarkovSignal(userId, context, target) {
    try {
        ...
        if (context.currentEmotion) {
            currentState = `emotion:${context.currentEmotion.toLowerCase()}`;
        }
        else if (context.currentTopic) {
            currentState = `topic:${context.currentTopic.toLowerCase()}`;
        }
        if (!currentState)
            return null;
...
async function collectTimeSeriesSignal(userId, target, now) {
    try {
        ...
        const isNegativeTarget = ['burnout_risk', 'will_struggle_soon', 'habit_slip_likely'].includes(target);
...
function collectTemporalSignal(userId, target, now) {
    ...
    switch (target) {
        ...
        case 'will_struggle_soon':
            ...
            if (dayOfWeek === 0 && hour >= 18)
                value = 0.7;
            else if (dayOfWeek === 1 && hour >= 6 && hour <= 10)
                value = 0.65;
            else if (hour >= 22 || hour <= 5)
                value = 0.6; // Late night
            break;
        ...
        case 'ready_for_challenge':
            ...
            if (hour >= 8 && hour <= 12 && dayOfWeek >= 1 && dayOfWeek <= 3)
                value = 0.7;
            break;
        case 'burnout_risk':
...
async function collectTimingSignal(userId, target) {
    if (target !== 'optimal_outreach_window')
        return null;
    try {
...
function collectTrendSignal(userId, target) {
    try {
        const series = mapTargetToSeries(target);
        if (!series)
            return null;
...
function collectRecencySignal(userId, target, now) {
...
// BAYESIAN FUSION
// ============================================================================
function fuseSignalsBayesian(signals, profile) {
    if (signals.length === 0) {
...
function getCorrelation(profile, signal1, signal2) {
 ...
 */
export function recordPredictionOutcome(userId, outcome, signals) {
    const profile = getOrCreateProfile(userId);
    for (const signal of signals) {
        ...
        if (accuracy.total >= 5) {
            ...
            const newWeight = currentWeight + CONFIG.WEIGHT_LEARNING_RATE * (targetWeight - currentWeight);
...
function updateSignalCorrelations(profile, signals) {
...
function getRelevantStatesForTarget(target) {
    const stateMap = {
        needs_support_now: [
            'emotion:stressed',
            'emotion:anxious',
...
function mapTargetToSeries(target) {
    switch (target) {
        case 'needs_support_now':
        case 'will_struggle_soon':
            return 'mood';
...
function generateExplanation(signals, probability, target) {
    if (signals.length === 0) {
    ...
    const probabilityDesc = probability > 0.7
        ? 'high'
        : probability > 0.5
            ? 'moderate'
            : probability > 0.3
...
function determineSuggestedAction(target, probability, confidence, now) {
...
//# sourceMappingURL=multi-signal-fusion.js.map