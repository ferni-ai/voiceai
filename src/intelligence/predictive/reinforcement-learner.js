...
import { loadReinforcementState, markDirty, isUserLoaded, markUserLoaded, } from './persistence.js';
 ...
 * @returns Tracking ID
 */
export function trackPrediction(userId, prediction, expectedOutcome = 'engaged') {
    ...
    markDirty(userId);
    log.debug({ userId, predictionId: id, target: prediction.target, confidence: prediction.confidence }, '📊 Tracking prediction for outcome');
    return id;
}
/**
 ...
 */
export function recordAction(predictionId, action) {
 ...
 */
export function recordOutcome(predictionId, outcome, feedback) {
 ...
 */
export function recordReward(reward) {
...
// LEARNING & STATISTICS
// ============================================================================
function updateLearningStats(userId, prediction, outcome) {
    ...
    for (const signal of prediction.prediction.signals) {
        ...
        if (signalPredictedPositive === isPositive)
            signalStats.correct++;
    ...
    if (prediction.actionTaken && isPositive) {
        ...
        let timeSlot = stats.bestOutreachTimes.find((t) => t.hour === hour && t.dayOfWeek === dayOfWeek);
        if (!timeSlot) {
...
function updateCalibration(stats, confidence, wasPositive) {
...
function isOutcomePositive(outcome) {
 ...
 */
export function getCalibratedConfidence(userId, confidence) {
    const stats = learningStats.get(userId);
    if (!stats || stats.totalWithOutcome < CONFIG.MIN_PREDICTIONS_FOR_STATS) {
        return confidence; // Not enough data to calibrate
    }
    // Find the calibration bucket
    const bucketStart = Math.floor(confidence / CONFIG.CONFIDENCE_BUCKET_WIDTH) * CONFIG.CONFIDENCE_BUCKET_WIDTH;
 ...
 */
export function getBestOutreachTimes(userId, limit = 5) {
    const stats = learningStats.get(userId);
    if (!stats)
        return [];
 ...
 */
export function getMostAccurateSignals(userId) {
    const stats = learningStats.get(userId);
    if (!stats)
        return [];
    const results = [];
 ...
 */
export function getReinforcementDataForPersistence(userId) {
    ...
    const calibration = stats?.calibration?.map((bucket) => ({
 ...
 */
export async function loadReinforcementFromFirestore(userId) {
    ...
    try {
        ...
        if (data) {
            ...
            log.debug({ userId, signalCount: Object.keys(data.signalAccuracy || {}).length }, 'Loaded reinforcement state from Firestore');
        }
 ...
 */
function initializeLearningStats(userId) {
    return {
        userId,
        totalPredictions: 0,
        totalWithOutcome: 0,
...
//# sourceMappingURL=reinforcement-learner.js.map