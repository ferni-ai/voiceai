...
/** User's trajectory profile */
interface UserTrajectoryProfile {
  userId: string;
  /** Learned precursor patterns */
  patterns: Map<EmotionalTrajectory, PrecursorPattern>;
  /** Current baseline values for each signal */
  baselines: Map<PrecursorSignal, BaselineStats>;
  /** Recent observations */
  recentObservations: PrecursorObservation[];
  /** Past trajectory events (for learning) */
  trajectoryHistory: Array<{
    trajectory: EmotionalTrajectory;
    startedAt: number;
    precursorsObserved: PrecursorSignal[];
    leadTimeMs: number;
    severity: number;
    duration: number;
  }>;
  /** Personal vulnerability factors */
  vulnerabilities: Map<EmotionalTrajectory, number>;
  lastUpdated: number;
}
...
const userProfiles = new Map<string, UserTrajectoryProfile>();
 ...
 */
export function recordPrecursorObservation(
  userId: string,
  signal: PrecursorSignal,
  value: number,
...
export function recordTrajectoryEvent(
  ...
  const precedingObservations = profile.recentObservations.filter(
    (obs) => now - obs.timestamp < lookbackMs && Math.abs(obs.deviation) >= 1
  );
...
export function recordConversationSignals(
  ...
  if (analysis.responseLatency !== undefined) {
    recordPrecursorObservation(
      userId,
      'response_latency',
      Math.min(1, analysis.responseLatency),
      'conversation_analysis'
    );
  }
  ...
  if (analysis.futureOrientation !== undefined) {
    recordPrecursorObservation(
      userId,
      'future_focus_change',
      (analysis.futureOrientation + 1) / 2,
      'conversation_analysis'
    );
  }
  ...
  if (analysis.socialMentions !== undefined) {
    recordPrecursorObservation(
      userId,
      'social_pattern_change',
      Math.min(1, analysis.socialMentions),
      'conversation_analysis'
    );
  }
 ...
 */
export function predictTrajectories(userId: string): TrajectoryPrediction[] {
 ...
 */
export function getTrajectoryAlerts(userId: string): TrajectoryPrediction[] {
  return predictTrajectories(userId).filter(
 ...
 */
export function predictSpecificTrajectory(
  userId: string,
  trajectory: EmotionalTrajectory
...
function getOrCreateProfile(userId: string): UserTrajectoryProfile {
...
function evaluatePattern(
  ...
  const activePrecursors: TrajectoryPrediction['activePrecursors'] = [];
  let totalWeight = 0;
  let weightedSum = 0;
  ...
  for (const patternSignal of pattern.signals) {
    ...
    if (recentObs.length === 0) continue;
    ...
    const matchesDirection =
      (patternSignal.direction === 'increase' && latestObs.deviation > 0) ||
  ...
  if (activePrecursors.length === 0) return null;
  ...
  // Determine severity
  let severity: TrajectoryPrediction['severity'];
  if (probability >= 0.8) severity = 'alert';
  else if (probability >= 0.6) severity = 'warning';
  else if (probability >= 0.4) severity = 'caution';
  else severity = 'watch';

  // Determine expected duration
  const pastEvents = profile.trajectoryHistory.filter((t) => t.trajectory === trajectory);
  let expectedDuration: TrajectoryPrediction['expectedDuration'] = 'unknown';
  if (pastEvents.length > 0) {
    const avgDuration = pastEvents.reduce((s, e) => s + e.duration, 0) / pastEvents.length;
    if (avgDuration < 6 * 60 * 60 * 1000) expectedDuration = 'hours';
    else if (avgDuration < 2 * 24 * 60 * 60 * 1000) expectedDuration = 'days';
    else if (avgDuration < 7 * 24 * 60 * 60 * 1000) expectedDuration = 'week';
    else expectedDuration = 'weeks';
  }
  ...
  const confidence = Math.min(
    0.9,
    0.3 + pattern.observationCount * 0.05 + activePrecursors.length * 0.1
  );
...
function evaluateGenericPatterns(profile: UserTrajectoryProfile): TrajectoryPrediction[] {
  const predictions: TrajectoryPrediction[] = [];
  const now = Date.now();
  ...
  for (const [trajectory, count] of trajectoryCounts) {
    ...
    let severity: TrajectoryPrediction['severity'];
    if (adjustedProbability >= 0.8) severity = 'alert';
    else if (adjustedProbability >= 0.6) severity = 'warning';
    else if (adjustedProbability >= 0.4) severity = 'caution';
    else severity = 'watch';
...
function evaluateGenericTrajectory(
  profile: UserTrajectoryProfile,
  trajectory: EmotionalTrajectory
): TrajectoryPrediction | null {
  ...
  if (recentDeviations.length === 0 && vulnerability < 0.6) return null;
...
function generateLikelyTriggers(
  activePrecursors: TrajectoryPrediction['activePrecursors']
): string[] {
...
function generatePreventiveActions(
  trajectory: EmotionalTrajectory,
  activePrecursors: TrajectoryPrediction['activePrecursors']
): TrajectoryPrediction['preventiveActions'] {
  const actions: TrajectoryPrediction['preventiveActions'] = [];
...
function initializeCommonPatterns(profile: UserTrajectoryProfile): void {
  // Initialize with population-level patterns
  const commonPatterns: Array<{
    trajectory: EmotionalTrajectory;
    signals: PrecursorPattern['signals'];
  }> = [
    {
