...
interface UserPatternProfile {
  ...
  lastUpdated: number;
  priorStrength: number; // How much to weight community patterns
}
...
const userProfiles = new Map<string, UserPatternProfile>();
const communityPatterns: FirstOrderChain = new Map();
...
export function recordTransition(
  ...
  from: ObservableState,
  to: ObservableState,
  context?: TransitionObservation['context']
): void {
  ...
  log.debug(
    { userId, from, to, totalObs: profile.totalObservations },
 ...
 */
export function recordSecondOrderTransition(
  userId: string,
  previous: ObservableState,
  current: ObservableState,
  next: ObservableState
): void {
  const profile = getOrCreateProfile(userId);
  const now = Date.now();
...
export function predictNextStates(
  ...
  const predictions: SequencePrediction['predictions'] = [];
  let source: SequencePrediction['source'] = 'prior';
...
function getOrCreateProfile(userId: string): UserPatternProfile {
 ...
 */
async function loadUserProfileFromFirestore(userId: string): Promise<void> {
  try {
    ...
    if (data && data.firstOrder) {
      ...
      if (profile) {
        ...
        for (const [from, transitions] of Object.entries(data.firstOrder)) {
          const transitionMap = new Map<ObservableState, TransitionProbability>();
          for (const [to, prob] of Object.entries(transitions)) {
            transitionMap.set(to as ObservableState, prob);
          }
          profile.firstOrder.set(from as ObservableState, transitionMap);
        }
...
function calculateConfidence(observations: number): TransitionProbability['confidence'] {
  if (observations >= 20) return 'very_high';
  if (observations >= 10) return 'high';
  if (observations >= CONFIG.MIN_OBSERVATIONS_FOR_CONFIDENCE) return 'medium';
  return 'low';
}

function recalculateProbabilities(
  transitions: Map<ObservableState, TransitionProbability>,
...
function updateCommunityPatterns(from: ObservableState, to: ObservableState): void {
 ...
 */
export function extractStatesFromTurn(
  text: string,
  emotion?: string,
  topic?: string,
  timestamp?: Date
): ObservableState[] {
  const states: ObservableState[] = [];
  ...
  if (hour >= 5 && hour < 12) states.push('temporal:morning');
  else if (hour >= 12 && hour < 17) states.push('temporal:afternoon');
  else if (hour >= 17 && hour < 22) states.push('temporal:evening');
  else states.push('temporal:late_night');

  if (day === 0 || day === 6) states.push('temporal:weekend');
  else states.push('temporal:weekday');

  // Add emotion state
  if (emotion) {
    const emotionMap: Record<string, ObservableState> = {
  ...
  // Add topic state
  if (topic) {
    const topicMap: Record<string, ObservableState> = {
      work: 'topic:work',
 ...
 */
export async function saveUserProfile(userId: string): Promise<void> {
  const profile = userProfiles.get(userId);
  if (!profile) return;

  // Convert Maps to plain objects for Firestore
  const data: MarkovPersistenceData = {
 ...
 */
export async function loadUserProfile(userId: string): Promise<UserPatternProfile | null> {
 ...
 */
export function getMarkovDataForPersistence(userId: string): MarkovPersistenceData | null {
  const profile = userProfiles.get(userId);
