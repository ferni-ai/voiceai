...
function diagnoseHabitFailure(
  ...
  const diagnoses: Record<string, HabitDiagnosis> = {
    never_start: {
      ...
      explanation:
        "When we never even start, it usually means the habit feels overwhelming or there's no clear trigger to remind us.",
      science:
        'Behavior science shows that tiny habits (2 min or less) have dramatically higher success rates than ambitious ones.',
      fixes: [
      ...
      reframe:
        'Instead of "I want to meditate", try "After I sit down with morning coffee, I will take 3 breaths"',
    ...
    start_then_stop: {
      ...
      explanation:
        "We start strong then stop because there's no immediate payoff. The long-term benefits don't motivate in the moment.",
      science:
        "Habits form when there's an immediate reward. Without one, the brain doesn't encode the behavior as valuable.",
      fixes: [
      ...
      reframe:
        'Add a celebration right after the habit. Even a mental "nice!" starts wiring the reward.',
    ...
    inconsistent: {
      ...
      explanation:
        "Inconsistency usually means we're relying on motivation instead of systems. Motivation is unreliable.",
      science:
        "The most successful habit builders don't rely on motivation - they rely on consistent cues and environment design.",
      fixes: [
      ...
      reframe:
        'Find an "anchor moment" - something you do every day at the same time. Stack onto that.',
    ...
    hate_it: {
      ...
      explanation:
        'Not all habits work for all people. If you hate it, either the habit or the approach needs to change.',
      science:
        "Sustainable habits come from finding the version that works FOR YOU, not forcing someone else's approach.",
      fixes: [
      ...
      reframe:
        "You don't have to do the textbook version. What's a version of this you might actually enjoy?",
      nextStep:
        'What would this habit look like if it were actually fun? Brainstorm 3 alternatives.',
    },
    forget: {
      issue: 'Cue is invisible or not connected to existing behavior',
      explanation:
        "Forgetting means the habit isn't triggered by anything in your environment or routine.",
      science:
        'The most reliable habits are anchored to existing behaviors or made visually obvious in the environment.',
      fixes: [
      ...
      nextStep:
        "Where could you put a visual reminder that you'd definitely see at the right time?",
    },
  };
  ...
  if (
    !currentCue ||
    currentCue.toLowerCase().includes('when i feel') ||
    currentCue.toLowerCase().includes('when i want')
  ) {
    diagnosis.fixes.unshift(
      'Your cue depends on feeling motivated. Replace with a time-based or action-based cue.'
    );
  }
...
function analyzeMoodPatterns(logs: MoodLog[]): MoodPatterns {
  ...
  const morningPositive =
    morningMoods.filter((l) => ['great', 'good'].includes(l.mood)).length /
    (morningMoods.length || 1);
  const eveningPositive =
    eveningMoods.filter((l) => ['great', 'good'].includes(l.mood)).length /
    (eveningMoods.length || 1);
  ...
  const withHabitsPositive =
    logsWithHabits.filter((l) => ['great', 'good'].includes(l.mood)).length /
    (logsWithHabits.length || 1);
  const withoutHabitsPositive =
    logsWithoutHabits.filter((l) => ['great', 'good'].includes(l.mood)).length /
    (logsWithoutHabits.length || 1);

  if (withHabitsPositive > withoutHabitsPositive + 0.15) {
    insights.push(
      'Your mood tends to be better on days you complete habits. The habits are working!'
    );
...
export const LIFE_TRANSITION_SUPPORT: Record<string, LifeTransitionSupport> = {
  new_job: {
    name: 'New Job',
    validation:
      "Starting a new job is exciting AND exhausting. Your routines will be disrupted, and that's okay.",
    expectations: [
    ...
    selfCareNote:
      "Learning a new job IS the work right now. Don't add pressure to maintain every habit perfectly.",
  },
  job_loss: {
    name: 'Job Loss',
    ...
    habitsToAdd: [
      ...
      'Free stress relief (walks, library)',
    ],
    priorityOrder: [
      'Basic self-care',
      'Daily structure',
    ...
    selfCareNote:
      'Your worth is not your job. Take care of yourself first, then tackle the search.',
  },
  new_baby: {
    name: 'New Baby',
    validation:
      "A new baby changes EVERYTHING. Survival mode is not only acceptable, it's necessary.",
    expectations: [
  ...
  new_relationship: {
    ...
    selfCareNote:
      "Don't lose yourself. The best relationships allow both people to keep growing individually.",
  },
  breakup: {
    name: 'Breakup',
    validation:
      "Breakups hurt, even when they're right. Your feelings are valid, whatever they are.",
    expectations: [
  ...
  moving: {
    name: 'Moving',
    validation:
      'Moving is in the top 5 most stressful life events. It disrupts everything, including habits.',
    expectations: [
    ...
    selfCareNote:
      "Your old habits won't work in the new space. That's okay. You'll build new ones.",
  },
  empty_nest: {
    name: 'Empty Nest',
...
- User is frustrated with lack of progress`,
      ...
      execute: async (
        { habit, attempts, failurePoint, currentCue, currentReward, timeOfDay },
        { ctx }
      ) => {
        const userData = ctx.userData as { userId?: string };
        const userId = userData.userId || 'anonymous';
...
- "I feel tired/energized/stressed"`,
      ...
      execute: async ({ mood, energy, timeOfDay, habitsCompletedToday, notes }, { ctx }) => {
        ...
        const recentLogs = moodLogs.filter(
          (l) => Date.now() - new Date(l.date).getTime() < 14 * 24 * 60 * 60 * 1000
        );
