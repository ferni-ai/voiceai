...
function detectFeedbackType(message, context) {
 ...
 */
function extractRequestedCapability(message) {
...
export class FeedbackCollector {
     ...
     */
    processFeedback(message, context, lastToolId) {
     ...
     */
    recordRating(toolId, rating, // 1-5 or -1/1 for thumbs
    context) {
        // Normalize to -1 to 1
    ...
    updateStats(record) {
        ...
        if (record.sentiment === 'positive')
            ...
            stats.neutral++;
        if (record.type === 'implicit_retry')
            stats.retries++;
        if (record.type === 'implicit_abandon')
            stats.abandons++;
    }
    /**
     * Track feature requests
     */
    trackFeatureRequest(capability, message) {
        if (!capability)
            return;
        // Normalize capability
        const normalizedCapability = capability.toLowerCase().trim();
        // Find existing or create new
        const existing = this.featureRequests.find((fr) => fr.capability.toLowerCase().includes(normalizedCapability) ||
     ...
     */
    getProblematicTools() {
        return this.getAllFeedback().filter((summary) => summary.averageScore < -0.2 || summary.retryRate > 0.3 || summary.abandonRate > 0.2);
    }
    /**
