#!/usr/bin/env npx tsx
/* eslint-disable no-console -- CLI code generator outputs to console */
/**
...
// ============================================================================
// TYPES (mirror tool-tiers.json structure)
// ============================================================================
...
interface ToolTiersConfig {
  ...
  tiers: {
    ...
    preloaded: {
      description: string;
      domains: string[];
      conditionalDomains: Record<string, string[]>;
    };
    predictive: {
      description: string;
      rules: PredictiveRule[];
    };
    onDemand: {
      description: string;
      domains: string[];
  ...
  metrics: {
    targetTier0LoadTime: number;
    targetTier1LoadTime: number;
    targetTier2LoadTime: number;
    targetTier3LoadTime: number;
  };
}
 ...
 *
 * In 2026, we use a lightweight local model or pre-computed vectors.
 * For now, we use a simple bag-of-words approach that's FAST.
 * You can replace this with real embeddings from your embedding service.
 */
function computeTriggerSignature(triggers: string[]): number[] {
  ...
  // Convert to fixed-size vector (26 letters + 100 common bigrams)
...
function generateTypeScript(config: ToolTiersConfig): string {
  const lines: string[] = [];
 ...
 * Generator: src/tools/gateway/codegen/generate-tiers.ts
 *
...
export const PREDICTIVE_RULES: readonly PredictiveRule[] = [`);
  ...
  for (const rule of config.tiers.predictive.rules) {
    ...
    lines.push(`  {
    id: '${rule.id}',
    triggers: [${rule.triggers.map((t) => `'${t}'`).join(', ')}],
    triggerSet: new Set([${rule.triggers.map((t) => `'${t.toLowerCase()}'`).join(', ')}]),
    ${rule.timeCondition ? `timeCondition: { hours: [${rule.timeCondition.hours.join(', ')}] },` : ''}
    domains: [${rule.domains.map((d) => `'${d}'`).join(', ')}] as const,
...
${(() => {
  ...
  return Array.from(triggerMap.entries())
    .map(([trigger, ruleIds]) => {
      return `  ['${trigger}', PREDICTIVE_RULES.filter(r => [${ruleIds.map((id) => `'${id}'`).join(', ')}].includes(r.id))],`;
    })
    .join('\n');
})()}
]);
...
${[...new Set(config.tiers.predictive.rules.flatMap((r) => r.triggers.map((t) => t.toLowerCase())))]
  .map((t) => `  '${t}',`)
  .join('\n')}
]);
...
export function getDomainTier(domain: ToolDomain): 0 | 1 | 2 | 3 | -1 {
  ...
  // Tier 2 is dynamic (predictive), check if it's on-demand
...
export function getSessionStartDomains(): readonly ToolDomain[] {
  // Return pre-computed combined array
  return SESSION_START_DOMAINS;
}

/** Pre-computed session start domains (Tier 0 + Tier 1) */
const SESSION_START_DOMAINS: readonly ToolDomain[] = [
  ...TIER_0_DOMAINS,
  ...TIER_1_DOMAINS,
] as const;
...
function main() {
  const configPath = join(__dirname, '..', 'tool-tiers.json');
  const outputPath = join(__dirname, '..', 'tool-tiers.generated.ts');
  ...
  const config = JSON.parse(configText) as ToolTiersConfig;
