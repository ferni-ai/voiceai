 ...
 */
export function getDomainInterpretation(personaId: string, domain: string): string | null {
  const profile = getCognitiveProfile(personaId);
...
function shouldShowUncertainty(profile: CognitiveProfile, context: ToolResultContext): boolean {
  ...
  return (
    Math.random() < (isComplexDomain ? baseUncertaintyChance * 1.5 : baseUncertaintyChance * 0.5)
  );
}
