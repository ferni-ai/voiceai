...
export interface LifeGoal {
    id: string;
    category: 'health' | 'career' | 'financial' | 'relationships' | 'personal' | 'travel' | 'learning' | 'other';
    description: string;
...
export type EventType = 'birthday' | 'anniversary' | 'wedding' | 'graduation' | 'baby-shower' | 'retirement' | 'holiday' | 'corporate' | 'dinner-party' | 'other';
...
export interface PersistedEvent extends Omit<Event, 'date' | 'createdAt' | 'checklist'> {
    date: string;
    createdAt: string;
    checklist: Array<Omit<ChecklistItem, 'dueDate'> & {
        dueDate?: string;
    }>;
}
...
export interface PersistedVacation extends Omit<Vacation, 'startDate' | 'endDate' | 'createdAt' | 'itinerary' | 'bookings'> {
    startDate?: string;
    endDate?: string;
    createdAt: string;
    itinerary: Array<Omit<ItineraryDay, 'date'> & {
        date?: string;
    }>;
    bookings: Array<Omit<Booking, 'date'> & {
        date?: string;
    }>;
}
export interface PersistedAnnualPlan extends Omit<AnnualPlan, 'createdAt' | 'goals'> {
    createdAt: string;
    goals: Array<Omit<LifeGoal, 'deadline'> & {
        deadline?: string;
    }>;
}
