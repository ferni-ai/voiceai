 ...
 */
function updateMetrics(sourceId: string, latencyMs: number, success: boolean): void {
 ...
 */
function getEffectivePriority(source: SourceConfig): number {
 ...
 */
export async function fetchWithPriority<T>(options: FetchWithPriorityOptions<T>): Promise<{
  results: T[];
  sources: string[];
  latency: number;
  complete: boolean;
  ...
  const {
    ...
    slowThreshold = 2000,
    transformResult = (data) => (Array.isArray(data) ? data : [data]) as T[],
  } = options;
  ...
  return new Promise((resolve) => {
    ...
    const timeoutId = setTimeout(() => {
      if (!resolved) {
        resolved = true;
        log.debug(
          { elapsed: Date.now() - startTime, resultsCount: results.length },
    ...
    const fetchFromSource = async (source: SourceConfig) => {
      ...
      try {
        ...
        log.debug(
          { sourceId: source.id, error: String(error), latency: sourceLatency },
          'Source fetch failed'
        );
      }
    };
...
export function createSource(
  ...
  options?: Partial<Omit<SourceConfig, 'id' | 'name' | 'fetch'>>
): SourceConfig {
  return {
    id,
    name,
