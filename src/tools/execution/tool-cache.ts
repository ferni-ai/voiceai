...
import { type CachedToolResult, type CacheConfig, DEFAULT_CACHE_CONFIGS } from './types.js';
...
class ToolResultCache {
  private cache = new Map<string, CachedToolResult<unknown>>();
   ...
   */
  private makeKey(toolName: string, query: string): string {
   ...
   */
  private getMaxStaleAge(category: string): number {
  ...
  private getCategoryFromKey(key: string): string {
    ...
    if (toolName.toLowerCase().includes('news')) return 'news';
    if (toolName.toLowerCase().includes('weather')) return 'weather';
    if (toolName.toLowerCase().includes('stock')) return 'stocks';
    if (toolName.toLowerCase().includes('calendar')) return 'calendar';
  ...
  set<T>(toolName: string, query: string, data: T, category?: string): void {
    ...
    const entry: CachedToolResult<T> = {
      data,
      timestamp: Date.now(),
      source: toolName,
      key,
  ...
  get<T>(toolName: string, query: string): CachedToolResult<T> | null {
    const key = this.makeKey(toolName, query);
    const entry = this.cache.get(key) as CachedToolResult<T> | undefined;
