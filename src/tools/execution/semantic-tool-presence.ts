...
export interface PresenceFeedback {
  ...
  emotion?: string; // Cartesia emotion tag
  speedRatio?: number;
  shouldSpeak: boolean;
  reason: string;
}
...
export interface ToolTimingContext {
  ...
  durationMs: number;
  wasLong: boolean; // > 2s
  wasVeryLong: boolean; // > 5s
  userWasPatient: boolean;
  framingHint: string; // Hint for LLM to frame result naturally
}
 ...
 */
const PRESENCE_PATTERNS: Record<
  EmotionalContext,
  {
    initial: string[];
    stillHere: string[];
 ...
 */
const TIME_MODIFIERS: Record<string, { speedAdjust: number; pauseMultiplier: number }> = {
 ...
 */
const TOOL_SEMANTICS: Record<string, { action: string; domain: string }> = {
 ...
 */
export function selectPresenceFeedback(
  phase: 'initial' | 'stillHere' | 'completion',
...
function mapEmotionToCartesia(
  ...
  const mapping: Record<EmotionalContext, string> = {
    anxious: 'caring',
    stressed: 'calm',
    excited: 'excited',
    curious: 'curious',
 ...
 */
export function emitToolPresence(event: ToolPresenceEvent): void {
...
// ============================================================================
// SESSION-SCOPED PRESENCE MANAGER
// ============================================================================
...
const activeExecutions = new Map<string, ActiveToolExecution>();

/**
 * Start tracking a tool execution for semantic presence.
 */
export function startToolPresence(
  context: ToolExecutionContext,
  onProgress?: (feedback: PresenceFeedback) => void
): void {
  ...
  log.debug(
    { tool: context.toolName, session: context.sessionId },
    'Started tool presence tracking'
  );
}
 ...
 */
export function stopToolPresence(sessionId: string, toolName: string): ToolTimingContext | null {
 ...
 */
export function cleanupSessionToolPresence(sessionId: string): void {
 ...
 * This helps the LLM frame its response naturally.
 */
export function generateToolContextInjection(timingContexts: ToolTimingContext[]): string {
  if (timingContexts.length === 0) {
    return '';
  }

  const parts: string[] = ['## Tool Execution Context'];
