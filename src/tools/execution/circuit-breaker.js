...
class ServiceCircuitBreaker {
    ...
    recordSuccess(service) {
        ...
        if (circuit.state === 'half-open') {
            ...
            circuit.failures = 0;
            log.info({ service, successRate: circuit.successRate }, 'Circuit closed after successful test');
        }
    ...
    recordFailure(service) {
        ...
        if (circuit.state === 'closed' && circuit.failures >= this.config.failureThreshold) {
            circuit.state = 'open';
            log.warn({ service, failures: circuit.failures, threshold: this.config.failureThreshold }, 'Circuit opened due to failures');
        }
    }
    /**
    ...
    recordLatency(service, latencyMs) {
        ...
        if (latencyMs > this.config.slowThreshold) {
            log.debug({ service, latencyMs, threshold: this.config.slowThreshold }, 'Slow response detected');
            ...
            if (circuit.avgLatency > this.config.slowThreshold &&
                circuit.failures >= this.config.slowThreshold_count) {
                if (circuit.state === 'closed') {
                    circuit.state = 'open';
                    circuit.lastFailure = Date.now();
                    log.warn({ service, avgLatency: circuit.avgLatency, threshold: this.config.slowThreshold }, 'Circuit opened due to slow responses');
                }
            }
        }
    }
