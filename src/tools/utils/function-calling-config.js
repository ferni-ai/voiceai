 ...
 */
export function requiresConfirmation(toolId) {
    return HIGH_STAKES_TOOLS.has(toolId);
}
/**
 * Check if a tool requires extra confirmation (critical action)
 */
export function requiresCriticalConfirmation(toolId) {
 ...
 */
export function getFunctionCallingConfig(environment) {
    switch (environment) {
        case 'testing':
...
export function buildToolConfig(options) {
    const { environment = 'production', modeOverride, additionalTools = [], excludeTools = [], isCrisis = false, isNewUser = false, } = options;
    ...
    if (excludeTools.length > 0 && config.functionCallingConfig.allowedFunctionNames) {
        config.functionCallingConfig.allowedFunctionNames =
            config.functionCallingConfig.allowedFunctionNames.filter((tool) => !excludeTools.includes(tool));
    }
    log.debug({
        environment: baseEnv,
 ...
 */
export function getThoughtSignatureProtocol(personaId) {
...
//# sourceMappingURL=function-calling-config.js.map