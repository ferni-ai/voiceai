...
async function initAnomalyDetection() {
    if (anomalyMonitor)
        return;
    try {
        const { recordLatency, recordSuccessRate } = await import('../../services/self-healing/anomaly-detection.js');
...
initAnomalyDetection().catch((err) => {
    log.warn({ error: String(err) }, 'Anomaly detection init failed (non-critical)');
});
// Track success/failure for anomaly detection per tool
const toolStats = new Map();
const STATS_REPORT_INTERVAL_MS = 30000; // Report every 30 seconds
function recordToolExecution(toolId, success, durationMs) {
 ...
 */
export function failure(error, code, metadata) {
 ...
 */
export function wrapToolExecute(toolId, domain, originalExecute, ctx, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    return async (params, execContext) => {
        const startTime = Date.now();
        const tracker = opts.enableAnalytics && isLifeCoachAnalyticsEnabled()
        ...
        try {
                ...
                if (opts.sanitizeFields) {
                    for (const field of opts.sanitizeFields) {
                        if (typeof params[field] === 'string') {
                            params[field] = sanitizePlainText(params[field]);
                        }
                    }
                }
            }
            ...
            if (opts.enableResilience) {
                ...
                result = await withResilience(() => originalExecute(params, execContext), {
                    ...
                    onRetry: (attempt, error, delay) => {
                        log.debug({ toolId, domain, attempt, error: error.message, delay }, 'Tool execution retry');
                    },
                });
            }
            else {
            ...
            try {
                const { onAfterToolCall } = await import('../../personas/bundles/extensibility-integration.js');
...
export function wrapToolDefinition(definition, options = {}) {
    ...
    if (opts.enableDeprecationWarnings && definition.deprecated) {
        log.warn({ toolId: definition.id, message: definition.deprecationMessage }, 'Using deprecated tool');
    }
    return {
        ...definition,
        create: (ctx) => {
 ...
 */
export function enhanceDomainTools(domainTools, domainOptions = {}) {
