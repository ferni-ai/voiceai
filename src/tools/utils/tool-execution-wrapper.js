...
import { requiresConfirmation as isHighStakes, requiresCriticalConfirmation, } from './function-calling-config.js';
import { failure, formatForLLM, fromLegacyResponse, pending, success, } from './tool-response.js';
const log = getLogger();
 ...
 */
function getConfirmationKey(userId, sessionId, toolId) {
    return `${userId}:${sessionId}:${toolId}`;
}
/**
 * Store a pending confirmation
 */
export function storePendingConfirmation(userId, sessionId, toolId, params, description, ttlMs = 60000) {
 ...
 */
export function getPendingConfirmation(userId, sessionId, toolId) {
    const key = getConfirmationKey(userId, sessionId, toolId);
 ...
 */
export function clearPendingConfirmation(userId, sessionId, toolId) {
    const key = getConfirmationKey(userId, sessionId, toolId);
    pendingConfirmations.delete(key);
}
/**
 ...
 */
export function hasPendingConfirmation(userId, sessionId, toolId) {
 ...
 */
export async function processConfirmation(userId, sessionId, toolId, confirmed, executeTool) {
    const pending = getPendingConfirmation(userId, sessionId, toolId);
    if (!pending) {
        return failure('No pending confirmation found', "I don't have a pending action to confirm. Could you repeat what you'd like me to do?", { errorCode: 'NOT_FOUND', recoverable: true });
    }
    ...
    try {
        ...
        return failure(String(error), `I tried to ${pending.description} but ran into an issue. Would you like me to try again?`, { errorCode: 'INTERNAL_ERROR', recoverable: true });
    }
}
// ============================================================================
// INPUT VALIDATION
 ...
 */
function getMaxLength(paramName) {
    const lowerName = paramName.toLowerCase();
    // Check exact match
    if (FIELD_LIMITS[paramName])
        return FIELD_LIMITS[paramName];
 ...
 */
function validateParams(toolId, params, customValidator) {
    // Sanitize inputs
    ...
    for (const [key, value] of Object.entries(sanitized)) {
        if (typeof value === 'string') {
            ...
            if (value.length > maxLength) {
                log.warn({ toolId, param: key, length: value.length, maxLength }, 'Parameter exceeds length limit');
                return {
                    valid: false,
 ...
 */
export function wrapToolExecution(toolId, originalExecute, ctx, options = {}) {
    ...
    return async (params, context) => {
        ...
        try {
            ...
            if (opts.enableValidation) {
                ...
                if (!validation.valid) {
                    const response = failure(validation.error || 'Validation failed', "I couldn't process that request. Could you try rephrasing?", { errorCode: 'VALIDATION_ERROR', recoverable: true });
 ...
 */
function getActionDescription(toolId, params) {
    // Map common tools to friendly descriptions
    const descriptions = {
        sendMessage: (p) => `send a message to ${p.recipient || 'them'}${p.subject ? ` about "${p.subject}"` : ''}`,
        sendEmail: (p) => `send an email to ${p.recipient || p.to || 'them'}${p.subject ? ` about "${p.subject}"` : ''}`,
        sendSMS: (p) => `send a text to ${p.recipient || p.phoneNumber || 'them'}`,
        createAppointment: (p) => `create an appointment${p.title ? ` called "${p.title}"` : ''}${p.date ? ` on ${p.date}` : ''}`,
        scheduleAppointment: (p) => `schedule an appointment${p.with ? ` with ${p.with}` : ''}${p.date ? ` on ${p.date}` : ''}`,
        ...
        createPayment: (p) => `make a payment${p.amount ? ` of $${p.amount}` : ''}${p.to ? ` to ${p.to}` : ''}`,
        transferMoney: (p) => `transfer ${p.amount ? `$${p.amount}` : 'money'}${p.to ? ` to ${p.to}` : ''}`,
        deleteMemory: (p) => `forget ${p.topic || p.fact || 'that information'}`,
        forgetMemory: (p) => `remove ${p.topic || p.what || 'that'} from my memory`,
        setThermostat: (p) => `set the thermostat to ${p.temperature || p.temp}${p.mode ? ` on ${p.mode} mode` : ''}`,
 ...
 */
export function wrapToolDefinition(definition, options) {
 ...
 */
export function detectConfirmationIntent(userInput) {
...
//# sourceMappingURL=tool-execution-wrapper.js.map