...
function validatePlanName(name: unknown): { valid: boolean; sanitized?: string; error?: string } {
...
function validateMonthlyIncome(amount: unknown): {
  valid: boolean;
  sanitized?: number;
  error?: string;
} {
...
export type RetirementStyle =
  | 'early-retirement' // FIRE movement, retire before 50
  | 'traditional' // 65-67 retirement
  | 'semi-retirement' // Part-time work, gradual transition
  | 'encore-career' // New career/passion after "retirement"
  | 'flexible'; // Undecided, exploring options

export type RetirementPhase =
  | 'dreaming' // Just starting to think about it
  | 'planning' // Actively planning
  | 'accumulating' // Building retirement savings
  | 'pre-retirement' // 5-10 years out
  | 'transitioning' // 1-2 years out
...
export interface RetirementPlan {
  ...
  // Financial Goals (coordinate with Maya)
  monthlyIncomeGoal: number; // Desired monthly income in retirement
  currentSavings: number;
  monthlySavingsTarget: number;
  savingsProgress: number; // Percentage towards goal
...
export interface RetirementVisionItem {
  ...
  estimatedCost?: number; // Annual or one-time
  notes?: string;
}
...
export interface RetirementChecklistItem {
  ...
  yearsBeforeRetirement?: number; // When to complete (e.g., 10, 5, 1, 0)
  completed: boolean;
  notes?: string;
}
...
const VISION_PROMPTS: Record<RetirementVisionItem['category'], string[]> = {
  location: [
...
// HELPER FUNCTIONS
// ============================================================================
function calculateYearsToRetirement(currentAge: number, targetAge: number): number {
  return Math.max(0, targetAge - currentAge);
}

function getPhaseFromYears(yearsToRetirement: number): RetirementPhase {
  if (yearsToRetirement <= 0) return 'retired';
  if (yearsToRetirement <= 2) return 'transitioning';
  if (yearsToRetirement <= 10) return 'pre-retirement';
  return 'accumulating';
}

function buildChecklist(yearsToRetirement: number): RetirementChecklistItem[] {
  const checklist: RetirementChecklistItem[] = [];
  let itemIndex = 0;
...
export function createRetirementPlan(
  ...
  getLogger().info(
    { planId: id, targetAge: validatedTargetAge, yearsToRetirement },
...
export function getRetirementPlan(userId: string): RetirementPlan | undefined {
  return Array.from(retirementPlans.values()).find((p) => p.userId === userId);
}

export function updateRetirementSavings(
  planId: string,
  currentSavings: number,
  monthlySavingsTarget?: number
...
export function addVisionItem(
  planId: string,
  category: RetirementVisionItem['category'],
  description: string,
  priority: RetirementVisionItem['priority'] = 'nice-to-have',
  estimatedCost?: number
): RetirementVisionItem | undefined {
  ...
  const item: RetirementVisionItem = {
...
Categories: location, activities, travel, family, health, legacy, work`,
    ...
    updateRetirementSavings: llm.tool({
      ...
      execute: async ({ currentSavings, monthlySavingsTarget, userId }) => {
        ...
        if (!updated) return `Couldn't update savings.`;
    ...
    completeRetirementTask: llm.tool({
      ...
      execute: async ({ taskDescription, userId }) => {
        ...
        if (!plan) return `No retirement plan found.`;

        const task = plan.checklist.find(
          (t) => t.task.toLowerCase().includes(taskDescription.toLowerCase()) && !t.completed
        );
