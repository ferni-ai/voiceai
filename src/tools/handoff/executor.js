 ...
 */
import { getLockedMemberTeaser, isCoreTeamMember, isTeamMemberUnlocked, } from '../../intelligence/context-builders/team/team-availability.js';
import { detectUserMoodFromContext, getAliveEntranceForHandoff, } from '../../personas/alive-entrances.js';
...
import { buildCognitiveHandoffContext, formatCognitiveHandoffForPrompt, } from './cognitive-handoff.js';
import { buildInsightBriefingForHandoff, formatInsightBriefingForPrompt, } from '../../services/cross-persona-insights.js';
...
import { getCurrentAgent, handoffEvents, isHandoffAllowed, isSameAgent, // Use the shared EventEmitter from state.ts
recordHandoff as recordHandoffToState, resetHandoffState as resetStateHandoffState, setCurrentAgent, } from './state.js';
// FIX: Import session-scoped state for concurrent session isolation
import { getSessionState, hasSessionState, getCurrentAgent as getSessionCurrentAgent, setCurrentAgent as setSessionCurrentAgent, } from './session-state.js';
let conversationContext = null;
/**
 * Capture context before a handoff
 */
export function captureHandoffContext(context) {
    conversationContext = {
 ...
 */
export function captureHandoffContextWithCognition(context, sessionId, previousPersonaId, targetPersonaId) {
    // Calculate emotional weight from emotional state
    const emotionalWeight = context.emotionalState === 'distressed'
        ? 0.9
 ...
 */
export async function executeHandoff(targetAgentId, reason, options = {}) {
    ...
    if (!options.skipUnlockCheck && !isCoach(canonicalTargetId)) {
        ...
        if (isCoreTeamMember(canonicalTargetId)) {
            ...
            if (!isUnlocked) {
                ...
                getLogger().info({ targetAgent: canonicalTargetId, tier, hasProfile: !!options.userProfile }, `🔒 Handoff blocked - ${targetName} is not yet unlocked for this user`);
                return {
                    success: false,
                    error: teaser ||
            ...
            if (!fullTeamUnlocked) {
                getLogger().info({ targetAgent: canonicalTargetId, tier, hasProfile: !!options.userProfile }, `🔒 Handoff blocked - marketplace agent ${targetName} requires full team to be unlocked`);
                return {
                    success: false,
    ...
    recordHandoffToState(previousAgent, canonicalTargetId, reason);
    ...
    if (sessionState) {
        setSessionCurrentAgent(sessionState, canonicalTargetId);
    }
    setCurrentAgent(canonicalTargetId);
    // Generate greeting
    ...
    getLogger().info({ from: previousAgent, to: canonicalTargetId, reason }, `🔄 HANDOFF: ${previousAgent} → ${targetAgentName}`);
    ...
    const handlerCompletePromise = new Promise((resolve) => {
        ...
        const timeoutId = setTimeout(() => {
            getLogger().warn({ targetId: canonicalTargetId }, '⚠️ Handoff handler timeout - proceeding without confirmation');
            if (!listenerRemoved) {
                handoffEvents.off('handoffHandlerComplete', onComplete);
                listenerRemoved = true;
            }
    ...
    getLogger().info({ targetId: canonicalTargetId }, '✅ voiceSwitch event emitted, waiting for handler...');
    ...
    if (handlerResult.success) {
        ...
        try {
            ...
            getLogger().debug({ from: previousAgent, to: canonicalTargetId }, '🧠 Recorded handoff for cross-persona intelligence');
        }
        catch (intelligenceErr) {
            // Non-critical, don't fail the handoff
            getLogger().debug({ error: String(intelligenceErr) }, 'Could not record handoff for intelligence layer');
        }
        ...
        try {
            if (options.userId) {
                ...
                getLogger().debug({ from: previousAgent, to: canonicalTargetId, userId: options.userId }, '💕 Recorded persona affinity for handoff');
            }
        }
        catch (affinityErr) {
            // Non-critical, don't fail the handoff
 ...
 */
async function generateHandoffGreeting(targetAgentId, reason, previousAgent) {
 ...
 */
function getCognitiveCollaborationGreeting(targetAgentId, previousAgentId) {
 ...
 */
function getReasonAcknowledgment(reason) {
 ...
 */
function buildContextContinuation(reason, recentMessages) {
    const parts = [];
    ...
    if (reason && reason.length >= 10) {
        ...
        if (reason.toLowerCase().includes('stock'))
            topics.push('stocks');
        if (reason.toLowerCase().includes('invest'))
            topics.push('investing');
        if (reason.toLowerCase().includes('budget'))
            topics.push('budgeting');
        if (reason.toLowerCase().includes('save'))
            topics.push('savings');
        if (reason.toLowerCase().includes('calendar'))
            topics.push('calendar');
        if (reason.toLowerCase().includes('goal'))
            topics.push('goals');
        if (reason.toLowerCase().includes('plan'))
    ...
    if (recentMessages && recentMessages.length > 0) {
        ...
        const conversationSummary = lastMessages
            .map((m) => `${m.role === 'user' ? 'User' : 'Previous Agent'}: ${m.content.slice(0, 150)}${m.content.length > 150 ? '...' : ''}`)
            .join('\n');
        parts.push(`\n[RECENT CONVERSATION]\n${conversationSummary}`);
    }
    return parts.join(' ');
 ...
 * Build instructions for the new agent
 */
function buildHandoffInstructions(targetAgentId, reason, contextContinuation) {
    const targetName = getPersonaDisplayName(targetAgentId);
