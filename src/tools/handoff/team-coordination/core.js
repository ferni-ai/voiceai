 ...
 */
export async function createSharedGoal(userId, title, category, financialTarget, timeline) {
    // Validate inputs
    ...
    getLogger().info({ goalId: goal.id, title: sanitizedTitle }, '🤝 Shared goal created (persisted)');
    return goal;
}
// ============================================================================
 ...
 */
export async function createTeamHandoff(userId, fromMember, toMember, reason, handoffContext) {
    const store = getLifeDataStore();
    ...
    getLogger().info({ handoffId: handoff.id, from: fromMember, to: toMember }, '🔄 Team handoff created (persisted)');
    return handoff;
}
// ============================================================================
 ...
 */
export async function linkMilestoneToTeam(userId, jordanMilestoneId, name, targetDate, mayaBudgetId) {
    const store = getLifeDataStore();
    const context = await getOrCreateTeamContext(userId);
    const milestone = {
