 ...
 */
export function getExpectedPreviousEvent(eventType) {
...
export class EventSequencer {
    ...
    bufferTimeouts = new Map();
    constructor(sessionId, options) {
        this.sessionId = sessionId;
        if (options?.maxBufferSize)
            this.maxBufferSize = options.maxBufferSize;
        if (options?.bufferTimeoutMs)
            this.bufferTimeoutMs = options.bufferTimeoutMs;
        log.debug({ sessionId, maxBufferSize: this.maxBufferSize, bufferTimeoutMs: this.bufferTimeoutMs }, '📬 Created event sequencer');
    }
    /**
    ...
    async processEvent(event) {
        log.debug({ sessionId: this.sessionId, type: event.type, seq: event.seq }, '⚡ Processing event');
        ...
        for (const handler of handlers) {
            try {
                ...
                log.error({ sessionId: this.sessionId, type: event.type, error: String(err) }, '❌ Event handler error');
            }
        }
        // If terminal event, reset for next handoff
        if (isTerminalEvent(event.type)) {
            log.debug({ sessionId: this.sessionId, type: event.type }, '🏁 Terminal event - handoff complete');
        }
    }
    /**
    ...
    bufferEvent(event) {
        ...
        if (this.buffer.size >= this.maxBufferSize) {
            log.warn({ sessionId: this.sessionId, bufferSize: this.buffer.size }, '⚠️ Event buffer full - dropping oldest');
        ...
        const timeout = setTimeout(() => {
            if (this.buffer.has(event.seq)) {
                log.warn({ sessionId: this.sessionId, type: event.type, seq: event.seq }, '⏰ Buffer timeout - processing despite gap');
     ...
     */
    async forceProcessBuffered(seq) {
    ...
    getState() {
        return {
            ...
            handlers: new Map(Array.from(this.handlers.entries()).map(([type, handlers]) => [type, [...handlers]])),
        };
    }
    /**
 ...
 */
export function createEventSequencer(sessionId, options) {
