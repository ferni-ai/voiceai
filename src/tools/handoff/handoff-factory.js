 ...
 */
function generateHandoffTool(agent, _coordinator) {
 ...
 */
function generateReturnToCoordinatorTool(coordinator) {
    // Sanitize the first name to remove non-alphanumeric characters
 ...
 */
export async function createHandoffTools(currentAgentId) {
    const now = Date.now();
 ...
 */
function filterToolsForAgent(toolSet, currentAgentId) {
    ...
    const filteredByAgentId = new Map();
    for (const tool of filteredTools) {
        filteredByName.set(tool.name.toLowerCase(), tool);
        filteredByAgentId.set(tool.agentId, tool);
    }
    getLogger().debug({ currentAgentId, availableHandoffs: filteredTools.map((t) => t.name) }, 'Filtered handoff tools for agent');
    return {
        tools: filteredTools,
 ...
 */
export async function getHandoffTool(toolName) {
 ...
 */
export async function getHandoffToolForAgent(agentId) {
    const toolSet = await createHandoffTools();
 ...
 */
export async function findHandoffTarget(userMessage) {
    const toolSet = await createHandoffTools();
    ...
    for (const tool of toolSet.tools) {
        for (const trigger of tool.handoffTriggers) {
            if (messageLower.includes(trigger.toLowerCase())) {
                ...
                if (agent) {
                    getLogger().debug({ trigger, agentId: tool.agentId }, 'Found handoff target from trigger');
                    return agent;
                }
            }
        }
 ...
 */
export async function getHandoffToolNames(currentAgentId) {
 ...
 */
export function clearHandoffToolCache() {
    toolSetCache = null;
    lastCacheTime = 0;
 ...
 */
export function isHandoffToolName(toolName) {
 ...
 */
export function getAgentNameFromToolName(toolName) {
 ...
 */
export async function buildHandoffTools(currentAgentIdOrOptions) {
    // Handle both old (string) and new (options) signatures
    const options = typeof currentAgentIdOrOptions === 'string'
    ...
    for (const def of toolSet.tools) {
        ...
        const tool = llm.tool({
            ...
            execute: async ({ reason, context_summary }, runContext) => {
                ...
                const runtimeTier = runtimeUserProfile?.subscription?.tier ||
                    subscriptionTier;
    ...
    tools.softTeamIntro = llm.tool({
        ...
        parameters: z.object({
            teammate_specialty: z
                .string()
                .describe('What specialty area to introduce (e.g., "habits", "research", "planning", "communication")'),
        ...
        execute: async ({ teammate_specialty, context }, runContext) => {
            ...
            const runtimeUserProfile = runContext
            ...
            const lockedTeammates = TEAM_MEMBERS.filter((m) => !isCoach(m.memberId) && !isTeamMemberUnlocked(m.memberId, runtimeUserProfile, runtimeTier));
            ...
            let matchedTeammate = lockedTeammates.find((m) => {
                ...
                return (roleLower.includes(specialtyLower) ||
...
Do NOT try to transfer to them. This was just a quick hello.`,
    ...
    tools.introduceMember = llm.tool({
        ...
        parameters: z.object({
            memberId: z
                .string()
                .describe('The team member ID to introduce (e.g., "maya-santos", "peter-john", "alex-chen", "jordan-taylor", "nayan-patel")'),
            spoken_intro: z
                .string()
                .optional()
        ...
        execute: async ({ memberId, spoken_intro }, _runContext) => {
            ...
            try {
                const { markIntroduced } = await import('../../intelligence/context-builders/team/cameo-unlock.js');
            ...
            getLogger().info({ memberId: member.memberId, displayName: member.displayName }, '🎭 Cameo unlock: Visual reveal triggered!');
            return {
                success: true,
                memberId: member.memberId,
                displayName: member.displayName,
        ...
        execute: async (_params, runContext) => {
            ...
            for (const agent of allAgents) {
                ...
                if (isUnlocked) {
                    ...
                    const memberInfo = TEAM_MEMBERS.find((m) => m.memberId.replace(/-/g, '_') === agent.id.replace(/-/g, '_') ||
                        m.memberId === agent.id ||
            ...
            const lockedCount = allAgents.filter((a) => !a.isCoordinator &&
                a.enabled &&
                !isTeamMemberUnlocked(a.id, runtimeUserProfile, runtimeTier)).length;
            if (lockedCount > 0) {
 ...
 */
async function getAgentEmoji(agentId) {
 ...
 */
export async function getHandoffToolsForAgent(currentAgentId, options) {
    const { tools } = await buildHandoffTools({
        currentAgentId,
...
//# sourceMappingURL=handoff-factory.js.map