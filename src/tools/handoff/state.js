...
export { getActiveSessionIds, getSessionState, hasSessionState, removeSessionState, } from './session-state.js';
// ============================================================================
// HANDOFF EVENTS
// ============================================================================
/**
...
let currentAgent = 'ferni'; // Start with coordinator (canonical ID)
// Handoff history
...
export function setCurrentAgent(agent) {
    ...
    if (cachedAgentContext && cachedAgentContext.agentId !== canonical) {
        getLogger().debug({ previousAgent, newAgent: canonical, cachedAgent: cachedAgentContext.agentId }, 'Clearing stale agent context cache on handoff');
 ...
 */
export function isCurrentAgent(agentId) {
 ...
 */
export function recordHandoffRecord(record) {
 ...
 */
export function recordHandoff(from, to, reason) {
    const now = Date.now();
 ...
 */
export function formatHandoffContextForAgent() {
    if (!handoffContext) {
        return '';
    }
    const parts = [];
 ...
 */
export function getHandoffAnalytics() {
    const bySource = {};
    const byTarget = {};
    const pairCounts = {};
    let totalDuration = 0;
 ...
 */
export function logHandoffAnalytics() {
...
let perPersonaMeetingCount = new Map();
let perPersonaLastTopic = new Map();
// Persistence callbacks (for cross-session state)
let persistMeetingCountsCallback = null;
let persistLastTopicsCallback = null;
/**
...
export function initializeHandoffContext(context) {
    ...
    if (context.meetingCounts) {
        ...
        getLogger().info({ count: perPersonaMeetingCount.size }, '📊 Loaded per-persona meeting counts from profile');
    }
    ...
    if (context.lastTopics) {
        ...
        getLogger().debug({ topics: Object.keys(context.lastTopics) }, '📝 Loaded per-persona last topics');
    }
 ...
 */
export function getLastTopicForPersona(personaId) {
 ...
 */
export function incrementMeetingCount(personaId) {
 ...
 */
export function getMeetingCount(personaId) {
...
export function getAgentDisplayName(agentId) {
    ...
    AgentDirectory.getDisplayName(canonical)
        ...
        .catch((err) => {
        getLogger().debug({ error: String(err), agentId: canonical }, 'Failed to cache agent display name');
    });
 ...
 */
export async function getAgentDisplayNameAsync(agentId) {
...
export function getAgentContext() {
    ...
    getLogger().debug({ currentAgent, cachedAgent: cachedAgentContext?.agentId, fallback: true }, 'Agent context cache miss - returning fallback identity (async fetch started)');
 ...
 */
export async function getAgentContextAsync() {
 ...
 */
async function fetchAgentContextAsync(agentId, retryCount = 0) {
    const MAX_RETRIES = 2;
    ...
    try {
        ...
        if (retryCount < MAX_RETRIES) {
            getLogger().debug({ error: String(error), agentId, attempt: retryCount + 1 }, 'Agent context fetch failed, retrying...');
            await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY_MS * (retryCount + 1)));
            return fetchAgentContextAsync(agentId, retryCount + 1);
        }
        getLogger().warn({ error: String(error), agentId, attempts: MAX_RETRIES + 1 }, 'Failed to fetch agent context from manifest after retries');
    }
}
/**
 * Normalize any agent ID to canonical form for internal tracking.
 ...
 */
export function suggestHandoff(userInput) {
...
let cachedTeamForHandoff = null;
let teamCachePromise = null;
/**
 ...
 */
export function getTeamForHandoff() {
    ...
    return [{ id: 'ferni', name: 'Ferni', specialty: 'team coordinator' }];
}
/**
 * Get all available team members for handoff (async version).
 * Preferred over sync version for new code.
 */
export async function getTeamForHandoffAsync() {
