...
import { activateVibe, getVibeState, getAvailablePresets, VIBE_PRESETS, } from '../../../services/vibe/index.js';
...
const setVibeDef = {
    id: 'setVibe',
    name: 'Set Vibe',
    description: 'Set the environment vibe - adjusts music, lights, and temperature together for a unified atmosphere.',
    domain: 'smart-home',
    additionalDomains: ['entertainment', 'wellness'],
    tags: ['vibe', 'environment', 'ambiance', 'mood', 'atmosphere'],
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('setVibe') ||
        ...
        This controls music, lights, and temperature together for a unified experience.`,
            parameters: z.object({
                vibe: z
                    ...
                    .describe('The vibe to set. focus=work mode, relax=wind down, energize=get moving, sleep=bedtime, social=hosting, morning=wake up, romantic=date night, workout=exercise'),
            }),
            execute: async ({ vibe }) => {
...
const getEnvironmentStatusDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('getEnvironmentStatus') ||
...
const adjustLightsDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('adjustLights') ||
            ...
            parameters: z.object({
                action: z
                    ...
                    .describe('What to do: on, off, dim (reduce by 25%), brighten (increase by 25%), or set to specific level'),
                brightness: z
                    .number()
                    .min(0)
                    .max(100)
...
const listVibesDef = {
    ...
    create: (_ctx) => {
        return llm.tool({
            description: getToolDescription('listVibes') ||
...
//# sourceMappingURL=vibe-tools.js.map