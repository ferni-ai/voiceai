...
const MAINTENANCE_INTERVALS: MaintenanceItem[] = [
...
function saveUserVehicles(userId: string, vehicles: Vehicle[]): void {
...
const addVehicleDef: ToolDefinition = {
  id: 'addVehicle',
  name: 'Add Vehicle',
...
const logMileageDef: ToolDefinition = {
  id: 'logMileage',
  name: 'Log Mileage',
...
const getMaintenanceScheduleDef: ToolDefinition = {
...
const trackServiceHistoryDef: ToolDefinition = {
...
const setRegistrationExpiryDef: ToolDefinition = {
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async (params: { expiryDate: string }) => {
        const userId = ctx.userId;
        if (!userId) {
...
const setInsuranceRenewalDef: ToolDefinition = {
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('vehicle', [
  addVehicleDef,
  logMileageDef,
  getMaintenanceScheduleDef,
  trackServiceHistoryDef,
  setRegistrationExpiryDef,
  setInsuranceRenewalDef,
]);
