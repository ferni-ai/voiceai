...
// PATTERN DETECTIVE
// ============================================================================
export const patternDetectiveDef = {
    id: 'patternDetective',
...
// WEEKLY PREDICTION
// ============================================================================
export const weeklyPredictionDef = {
    id: 'weeklyPrediction',
        ...
        return llm.tool({
            ...
            execute: async ({ action, predictions, actuals, predictionId }) => {
                ...
                if (action === 'save-predictions' && predictions) {
                    ...
                    return {
                        response: `Predictions locked in! <break time="300ms"/>` +
                ...
                if (action === 'get-pending') {
                    ...
                    if (pending.length === 0) {
                        return {
                            response: `No pending predictions to resolve. <break time="200ms"/>` +
                    ...
                    return {
                        response: `You have ${pending.length} prediction${pending.length > 1 ? 's' : ''} to resolve! <break time="200ms"/>` +
                ...
                if (action === 'record-actuals' && actuals) {
                    ...
                    if (!targetId) {
                        return {
                            ...
                            response: `I don't see any predictions waiting to be resolved. <break time="200ms"/>` +
                                `Want to make some predictions for this week?`,
                        };
                    }
                    ...
                    getLogger().info({ userId, predictionId: targetId, accuracy: result.accuracy }, '📊 Weekly prediction resolved');
                    // Generate analysis
                    const analysis = [];
                    for (const [cat, actual] of Object.entries(actuals)) {
                        analysis.push(`• ${cat}: ${actual}`);
                    }
                    const accuracyResponse = result.accuracy >= 75
                    ...
                    return {
                        response: `Actuals recorded! <break time="300ms"/>\n\n` +
                            `${analysis.join('\n')}\n\n${accuracyResponse}`,
...
// EXPORTS
// ============================================================================
export const analyticsGameDefinitions = [
    patternDetectiveDef,
    weeklyPredictionDef,
];
//# sourceMappingURL=analytics-games.js.map