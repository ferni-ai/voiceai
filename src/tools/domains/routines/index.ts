...
const RESPONSES = {
  ...
  routinesList: (count: number) =>
    count === 1
      ? "Here's the one thing I'm taking care of for you:"
      : `Here are the ${count} things I take care of for you:`,
  routineCreated: (name: string) => `Done! I'll ${name.toLowerCase()} for you now.`,
  routinePaused: (name: string) => `Okay, I'll put "${name}" on hold for now.`,
  routineResumed: (name: string) => `Got it, I'll start ${name.toLowerCase()} again.`,
...
function formatRoutineForVoice(routine: Workflow): string {
 ...
 */
async function createRoutineFromVoice(
  ctx: ToolContext,
  params: {
    name: string;
 ...
 */
function parseTimeToCron(timeStr: string): string {
  try {
...
async function runRoutine(
  ...
  try {
    const data = await getWorkflowData(userId);
    const routine = data.workflows.find(
      (w) => w.name.toLowerCase() === params.routineName.toLowerCase()
    );
...
async function toggleRoutine(
  ctx: ToolContext,
  params: { routineName: string; action: 'pause' | 'resume' }
): Promise<string> {
  const userId = ctx.userId;
  if (!userId) {
    return "I need to know who you are first.";
 ...
 */
async function suggestRoutines(ctx: ToolContext): Promise<string> {
  try {
...
const routineToolDefinitions: ToolDefinition[] = [
  {
    id: 'listRoutines',
