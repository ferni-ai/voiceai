...
export function detectProactiveTriggers(context) {
    ...
    if (context.lastActivity) {
        const daysSinceActivity = Math.floor((now.getTime() - context.lastActivity.getTime()) / (1000 * 60 * 60 * 24));
    ...
    if (context.tendency === 'obliger') {
        ...
        if (context.lastActivity) {
            const daysSince = Math.floor((now.getTime() - context.lastActivity.getTime()) / (1000 * 60 * 60 * 24));
            if (daysSince >= 2) {
...
// MESSAGE GENERATORS
// ============================================================================
function generateSilenceMessage(days, tendency) {
    const messages = [
        {
...
function generateComebackMessage(days, tendency) {
    return {
...
function generateStreakAtRiskMessage(habitName, streak, tendency) {
...
function generateStreakMilestoneMessage(habitName, streak) {
...
function generateChallengeMilestoneMessage(day, completed, challengeType) {
    if (day === 7) {
        return {
...
function generateLevelUpReadyMessage(habitName, currentLevel) {
    const levelNames = [
        'Tiny Start',
        'Mini Habit',
        'Emerging Practice',
...
function generateEncouragementMessage(tendency) {
    return {
        opener: 'I see you.',
...
function generateWeeklyReflectionMessage() {
    return {
...
function generateAccountabilityMessage(days) {
    return {
 ...
 */
export function generateLonelinessCheckInMessage() {
    const messages = [
        {
 ...
 */
export function generateSocialWinCelebrationMessage(winType) {
    const messages = {
        reached_out: {
 ...
 */
export function generateConversationFollowUpMessage(conversationType) {
    return {
 ...
 */
export function generateBoundaryCheckInMessage(boundaryName) {
    return {
 ...
 */
export function generateRebuildingMilestoneMessage(milestone, journeyType) {
    const milestoneMessages = {
        first_step: {
 ...
 */
export function generateFreshStartAnniversaryMessage(duration) {
 ...
 */
export function generateTransitionStageShiftMessage(fromStage, toStage, transitionType) {
 ...
 */
export function generateLifeTransitionCheckInMessage(transitionType) {
    return {
 ...
 */
export function generateBelongingMilestoneMessage(milestone) {
    const milestoneMessages = {
        found_community: {
 ...
 */
export function generateRestPermissionMessage(signType) {
    const messages = {
        overwork: {
 ...
 */
export function generatePlateauCelebrationMessage(plateauType) {
    const messages = {
        maintaining: {
 ...
 */
export function generateSeasonalTransitionMessage(season) {
    const messages = {
        spring: {
 ...
 */
export function generateEnoughForTodayMessage() {
    const variations = [
        {
 ...
 */
export function generateGentlePaceCheckMessage(paceType) {
    const messages = {
        rushing: {
...
// HELPER FUNCTIONS
// ============================================================================
function createTrigger(type, userId, priority, data) {
    return {
    ...
    return {
        ...
        checkForProactiveOpportunities: llm.tool({
            ...
            execute: async (_, { ctx }) => {
                ...
                const tendency = store.getUserPreference(userId, 'fourTendency');
        ...
        scheduleFollowUp: llm.tool({
            ...
            execute: async ({ reason, timing, habitId, priority = 'medium' }, { ctx }) => {
                ...
                const followUps = store.getUserPreference(userId, 'scheduledFollowUps') || [];
                const newFollowUp = {
        ...
        getPendingFollowUps: llm.tool({
            ...
            execute: async (_, { ctx }) => {
                ...
                return {
                    ...
                    suggestion: due.length > 0 ? `Follow up on: "${due[0].reason}"` : 'No follow-ups due right now.',
                };
            },
        }),
        /**
...
//# sourceMappingURL=proactive-coaching.js.map