...
import { recordObservation, generatePredictions, buildPredictiveContext, } from '../../../services/superhuman/predictive-coaching.js';
import { buildNarrativeContextString, buildNarrativeContext, } from '../../../services/superhuman/life-narrative.js';
            ...
            execute: async ({ filter = 'active', includeFollowUps = true }) => {
                ...
                try {
                    ...
                    const filtered = filter === 'all'
                        ? commitments
                        : commitments.filter((c) => filter === 'active' ? c.status === 'active' : c.status === 'completed');
        ...
        return llm.tool({
            ...
            execute: async ({ commitmentSummary, howItWent }) => {
                ...
                try {
                    ...
                    const commitments = await loadUserCommitments(ctx.userId);
                    const matching = commitments.find((c) => c.status === 'active' &&
        ...
        return llm.tool({
            ...
            execute: async ({ focus = 'themes' }) => {
                ...
                try {
                    ...
                    if (narrativeContext.currentChapter?.keyThemes &&
...
// ============================================================================
const generateProactiveMessageDef = {
...
// ============================================================================
const backgroundCallDef = {
    id: 'backgroundCall',
    name: 'Background Call',
    description: 'Make a phone call on the user\'s behalf in the background. Perfect for: "Call my mom while I\'m away", "Check on that appointment", "Make a call for me".',
    domain: 'proactive',
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ contactName, contactPhone, objective, context, script }) => {
                ...
                try {
                    const { queueCall } = await import('../../../services/background-agents/executors/call-executor.js');
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('proactive', proactiveTools);
