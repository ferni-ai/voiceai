/**
 * Nayan's Superhuman Wisdom Tools - Execution Tests
 *
 * Tests the actual execution of Nayan's "Better Than Human" wisdom capabilities:
 ...
 *
 * Run with: pnpm vitest run src/tools/domains/nayan-wisdom/__tests__/nayan-wisdom-execution.test.ts
 */
...
import * as nayanServices from '../../../../services/superhuman/nayan-wisdom-services.js';
...
describe('Nayan Wisdom Tools - Execution', () => {
  ...
  describe('holdParadox execution', () => {
    it('should record a paradox and return wisdom response', async () => {
      ...
      expect(nayanServices.recordParadox).toHaveBeenCalledWith('test-user-123', expect.objectContaining({
        desire1: 'stability and security',
        desire2: 'adventure and freedom',
      }));
    });
    ...
    it('should reference existing similar paradoxes', async () => {
      vi.mocked(nayanServices.getParadoxes).mockResolvedValueOnce([
        {
          desire1: 'stability',
          desire2: 'adventure',
  ...
  describe('trackEnough execution', () => {
    it('should record new enough statement', async () => {
      ...
      expect(nayanServices.recordEnoughStatement).toHaveBeenCalledWith('test-user-123', expect.objectContaining({
        domain: 'money',
    ...
    it('should check past enough statements', async () => {
      vi.mocked(nayanServices.getEnoughStatements).mockResolvedValueOnce([
        {
          domain: 'career',
  ...
  describe('trackWisdomIncubation execution', () => {
    it('should record question to sit with', async () => {
      ...
      expect(nayanServices.recordIncubatingWisdom).toHaveBeenCalledWith('test-user-123', expect.objectContaining({
        question: 'Should I leave my stable job to pursue my passion?',
        suggestedDuration: 'weeks',
        status: 'incubating',
      }));
    ...
    it('should check on incubating items', async () => {
      vi.mocked(nayanServices.getIncubatingWisdom).mockResolvedValueOnce([
        {
  ...
  describe('Edge Cases', () => {
    it('should handle anonymous user gracefully', async () => {
      ...
      expect(result).toBeDefined();
      expect(nayanServices.recordParadox).toHaveBeenCalledWith('anonymous', expect.any(Object));
    });

    it('should propagate persistence failures (to be handled by caller)', async () => {
      vi.mocked(nayanServices.recordParadox).mockRejectedValueOnce(new Error('Firestore unavailable'));
