...
import { getHomeAssistantClient, getHueClient, getLifxClient, } from '../../../services/self-healing/index.js';
...
// ============================================================================
async function homeAssistantRequest(endpoint, method = 'GET', body) {
    ...
    if (method === 'POST') {
        const { data, error } = await haClient.post(url, body, {
        ...
        const { data, error } = await haClient.get(url, {
            headers: { Authorization: `Bearer ${HOME_ASSISTANT_TOKEN}` },
        });
        if (error) {
            getLogger().warn({ endpoint, error: error.message }, 'Home Assistant request failed');
...
async function getHomeAssistantDevices() {
    ...
    return states
        ...
        return ['light', 'switch', 'climate', 'lock', 'fan', 'cover', 'media_player'].includes(domain || '');
    })
        .map((s) => {
...
async function callHomeAssistantService(domain, service, entityId, data) {
...
async function getHueLights(credentials) {
    ...
    const { data: lights, error } = await hueClient.get(url);
    if (error || !lights) {
        if (error) {
...
async function setHueLight(credentials, lightId, on, brightness, colorTemp) {
    const hueConfig = credentials.hue;
    if (!hueConfig) {
        return false;
    }
...
async function getLifxLights(credentials) {
    ...
    const { data: lights, error } = await lifxClient.get('https://api.lifx.com/v1/lights/all', { headers: { Authorization: `Bearer ${lifxConfig.token}` } });
    if (error || !lights) {
        if (error) {
...
async function setLifxLight(credentials, lightId, power, brightness, colorTemp) {
    const lifxConfig = credentials.lifx;
    if (!lifxConfig) {
        return false;
    }
    ...
    if (power)
        body.power = power;
    if (brightness !== undefined)
        body.brightness = brightness / 100;
    if (colorTemp !== undefined)
        body.color = `kelvin:${colorTemp}`;
...
// ============================================================================
async function getSonosDevices(credentials) {
...
async function controlSonos(credentials, groupId, action, value) {
    const sonosConfig = credentials.sonos;
    if (!sonosConfig) {
        return false;
    }
...
// ============================================================================
async function getHomeKitDevices(userId) {
    try {
...
async function controlHomeKit(userId, deviceId, changes) {
...
// ECOBEE API
// ============================================================================
async function getEcobeeDevices(userId) {
    if (!userId)
        return [];
...
async function controlEcobee(userId, temperature) {
    const configured = await isEcobeeConfigured(userId);
    if (!configured) {
        return false;
    }
...
export async function controlDevice(deviceNameOrId, action, value, userId) {
    ...
    const device = devices.find((d) => d.id === deviceNameOrId || d.name.toLowerCase().includes(deviceNameOrId.toLowerCase()));
    if (!device) {
        if (devices.length === 0) {
    ...
    switch (device.platform) {
        case 'home_assistant':
            if (device.type === 'light' || device.type === 'switch') {
                ...
                success = await callHomeAssistantService('lock', action === 'on' ? 'lock' : 'unlock', device.id);
                newState = action === 'on' ? 'locked' : 'unlocked';
            }
            break;
        case 'hue':
            if (device.type === 'light') {
                const turnOn = action === 'on' || action === 'set' || (action === 'toggle' && device.state === 'off');
                success = await setHueLight(credentials, device.id, turnOn, typeof value === 'number' ? value : undefined);
            }
            break;
        case 'lifx':
            if (device.type === 'light') {
                const lifxPower = action === 'on' || action === 'set' ? 'on' : action === 'off' ? 'off' : undefined;
                success = await setLifxLight(credentials, device.id, lifxPower, typeof value === 'number' ? value : undefined);
            }
            break;
        case 'sonos':
 ...
 */
export async function setLightsForVibe(userId, brightness, colorTemperature) {
    ...
    for (const light of lights) {
        ...
        switch (light.platform) {
            case 'hue':
                ...
                break;
            case 'lifx':
                success = await setLifxLight(credentials, light.id, brightness > 0 ? 'on' : 'off', brightness, colorTemperature);
                break;
            case 'homekit':
            ...
            case 'home_assistant':
                ...
                if (HOME_ASSISTANT_TOKEN && HOME_ASSISTANT_URL) {
                    try {
                        ...
                        const serviceData = { entity_id: light.id };
                        if (brightness > 0) {
                        ...
                        await homeAssistantRequest(`/api/services/light/${service}`, 'POST', serviceData);
                        success = true;
                    }
                    catch {
                        success = false;
 ...
 */
export async function playVibeMusic(userId, vibe, volume) {
    const credentials = await getCredentials(userId);
...
// TOOL DEFINITIONS
// ============================================================================
export function createSmartHomeTools(context) {
    ...
    return {
        ...
        controlLock: llm.tool({
            ...
            execute: async ({ lock, action }) => {
                if (action === 'status') {
                    const devices = await getAllDevices(userId);
                    const lockDevice = devices.find((d) => d.type === 'lock' && d.name.toLowerCase().includes(lock.toLowerCase()));
                    if (lockDevice) {
        ...
        listDevices: llm.tool({
            ...
            execute: async ({ type }) => {
                ...
                const filtered = type === 'all'
                    ? devices
                    : devices.filter((d) => d.type === type.replace(/s$/, ''));
