...
export function createConversationTools() {
    return {
        ...
        rememberName: llm.tool({
            description: "CALL silently when user shares their name. Execute without announcing - DO NOT say 'let me remember that'. Respond naturally after. CRITICAL: Do NOT call this with persona names (Ferni, Maya, Peter, Alex, Jordan, Nayan) - those are YOUR team members, not the user!",
            parameters: z.object({
                name: z
                    .string()
                    .describe("The user's name (NOT a persona name like Ferni, Maya, Peter, Alex, Jordan, or Nayan)"),
            }),
            execute: async ({ name }, { ctx }) => {
                ...
                if (userData.services?.userProfile) {
                    ...
                    try {
                        ...
                        getLogger().info({ name, userId: userData.services.userProfile.id }, '🎉 User name persisted to Firestore!');
                    }
                    catch (saveError) {
                        getLogger().error({ error: String(saveError), name }, 'Failed to persist name to Firestore - will be saved at session end');
                    }
                }
        ...
        noteEmotionalState: llm.tool({
            description: "CALL silently to note emotional state. Execute without announcing - DO NOT say 'I notice you seem...' before calling. Respond with empathy after.",
            parameters: z.object({
                state: z
                    .string()
                    .describe('The emotional state (e.g., anxious, excited, worried, hopeful, frustrated, calm)'),
            ...
            execute: async ({ state, context }, { ctx }) => {
                ...
                ctx.userData.emotionalState = state;
        ...
        shareStory: llm.tool({
            description: "CALL immediately to get a relevant story. Do not say 'let me share a story' - just call this and speak the returned story naturally.",
            parameters: z.object({
                theme: z
                    .string()
                    .describe('The theme or topic for the story, e.g., "failure", "patience", "love", "health", "fees", "starting over", "uncertainty", "contentment"'),
            }),
            execute: async ({ theme }, { ctx }) => {
                ...
                if (themeLower.includes('patience') ||
                ...
                if (themeLower.includes('uncertain') ||
        ...
        checkIn: llm.tool({
            ...
            parameters: z.object({
                reason: z
                    .string()
                    .describe('Why checking in (e.g., "heavy topic", "they seemed quiet", "long conversation")'),
            }),
        ...
        setReminder: llm.tool({
            ...
            parameters: z.object({
                ...
                timeframe: z
                    .string()
                    .describe('When they want to do it (e.g., "next week", "in a month", "before retirement")'),
            }),
