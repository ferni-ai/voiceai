...
import { recordEventPattern, getEventPatterns, recordGuestProfile, getGuestProfiles, recordMilestoneDetection, getDetectedMilestones, recordEventMeaning, getEventMeanings, recordCelebration, getCelebrationBalance, recordTransitionSignal, getAnticipatedTransitions, checkPlanningReadiness, } from '../../../services/superhuman/jordan-planning-services.js';
const log = createLogger({ module: 'tools:jordan-planning' });
// ============================================================================
// EVENT PATTERN MEMORY - Perfect recall across all events
// ============================================================================
const recallEventPatternsDef = {
    id: 'recallEventPatterns',
    name: 'Recall Event Patterns',
    description: 'Perfect memory for patterns across ALL events - budget tendencies, guest dynamics, emotional patterns, vendor preferences.',
    domain: 'jordan-planning',
...
// ============================================================================
const getGuestInsightsDef = {
    id: 'getGuestInsights',
    name: 'Get Guest Insights',
    description: 'Permanent guest profiles - dietary needs, accessibility, attendance patterns, relationship dynamics. Never forget a guest.',
    domain: 'jordan-planning',
        ...
        return llm.tool({
            ...
            execute: async ({ guestName, updateGuest, listAll }) => {
                ...
                if (updateGuest) {
                    ...
                    if (updateGuest.dietary)
                        response += `🍽️ Dietary: ${updateGuest.dietary}\n`;
                    if (updateGuest.accessibility)
                        response += `♿ Accessibility: ${updateGuest.accessibility}\n`;
                    if (updateGuest.note)
                        response += `📝 Note: ${updateGuest.note}\n`;
                ...
                if (guestName && guests.length === 1) {
                    ...
                    if (g.dietary)
                        response += `🍽️ Dietary: ${g.dietary}\n`;
                    if (g.accessibility)
                        response += `♿ Accessibility: ${g.accessibility}\n`;
                    if (g.note)
                        response += `📝 Note: ${g.note}\n`;
...
// ============================================================================
const detectMilestonesDef = {
    id: 'detectMilestones',
    name: 'Detect Milestones',
    description: 'Detect celebrations humans forget - work anniversaries, friendship milestones, quiet win streaks, approaching transitions.',
    domain: 'jordan-planning',
        ...
        return llm.tool({
            ...
            execute: async ({ recordMilestone, getUpcoming }) => {
                ...
                if (recordMilestone) {
                    ...
                    if (recordMilestone.recurring)
                        response += `🔄 Recurring annually\n`;
...
// ============================================================================
const captureEventMeaningDef = {
    id: 'captureEventMeaning',
    name: 'Capture Event Meaning',
    description: 'Remember what events MEANT, not just logistics - the emotional journey, meaningful speeches, lessons learned.',
    domain: 'jordan-planning',
            ...
            execute: async ({ eventName, meaning, memorableMoment, lessonLearned, recallEvent }) => {
                ...
                if (recallEvent) {
                    ...
                    for (const m of meanings) {
                        if (m.meaning)
                            response += `💭 **Meaning:** ${m.meaning}\n\n`;
                        if (m.memorableMoment)
                            response += `✨ **Moment:** ${m.memorableMoment}\n\n`;
                        if (m.lessonLearned)
                            response += `📚 **Lesson:** ${m.lessonLearned}\n\n`;
                    }
                    return response;
                }
                ...
                if (memorableMoment)
                    response += `✨ Moment: ${memorableMoment}\n`;
                if (lessonLearned)
                    response += `📚 Lesson: ${lessonLearned}\n`;
...
// ============================================================================
const checkCelebrationHealthDef = {
    id: 'checkCelebrationHealth',
    name: 'Check Celebration Health',
    description: 'Track objectively: Are they celebrating enough? Too much? Always for others? Notice joy gaps and celebration fatigue.',
    domain: 'jordan-planning',
...
// ============================================================================
const anticipateTransitionDef = {
    id: 'anticipateTransition',
    name: 'Anticipate Transition',
    description: 'See life transitions coming from conversation patterns - empty nest, retirement, career change. Suggest planning before they ask.',
    domain: 'jordan-planning',
...
// ============================================================================
const checkPlanningReadinessDef = {
    id: 'checkPlanningReadiness',
    name: 'Check Planning Readiness',
    description: 'Cross-team readiness check before major planning - financial health, calendar capacity, energy levels, emotional readiness.',
    domain: 'jordan-planning',
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ eventType, budget, timeframe }) => {
                ...
                const overallEmoji = readiness.overall === 'green' ? '🟢' : readiness.overall === 'yellow' ? '🟡' : '🔴';
                ...
                for (const area of areas) {
                    const statusEmoji = area.status === 'green' ? '✅' : area.status === 'yellow' ? '⚠️' : '❌';
...
// ============================================================================
const backgroundReservationDef = {
    id: 'backgroundReservation',
    name: 'Background Reservation',
    description: 'Make reservations in the background that continue even when the user disconnects. Perfect for: "Book dinner at Nobu for Saturday", "Reserve a table while I\'m away", "Get us a hotel room". Results are delivered when the user reconnects.',
    domain: 'jordan-planning',
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ venueName, reservationType, dateTime, partySize = 2, specialRequests, alternateOptions, urgency = 'when_ready', }) => {
                try {
                    const { queueReservationTask } = await import('../../../services/background-agents/index.js');
                    const userId = ctx.userId || 'anonymous';
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('jordan-planning', jordanPlanningTools);
