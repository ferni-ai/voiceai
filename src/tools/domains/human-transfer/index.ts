...
import {
  ...
  type EscalationType,
} from '../../../services/human-transfer/index.js';
...
const evaluateHumanTransferDef: ToolDefinition = {
...
`;
          ...
          const services = humanTransfer.getAvailableServices(decision.type as EscalationType);
...
const connectToHumanExpertDef: ToolDefinition = {
...
Use after evaluateHumanTransfer indicates transfer is appropriate.`,
      ...
      execute: async ({ transferType, userConsent, keyTopics }) => {
        log.info(
          { agentId: ctx.agentId, transferType, consent: userConsent },
          'Initiating human transfer'
        );

        try {
          // Get resources for this transfer type
          const services = humanTransfer.getAvailableServices(transferType as EscalationType);
...
const quickCrisisResourcesDef: ToolDefinition = {
...
const humanTransferTools: ToolDefinition[] = [
