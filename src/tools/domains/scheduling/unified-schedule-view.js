...
import { getEventsForDay, } from '../../../services/calendar/calendar-service.js';
import { getPendingReminders, } from '../../../services/scheduling/reminder-scheduler.js';
 ...
 */
function calendarToUnified(event) {
    return {
        id: event.id,
        type: 'event',
        title: event.title,
 ...
 */
function reminderToUnified(reminder) {
    const typeIcons = {
        sms: '📱',
        email: '📧',
        call: '📞',
    ...
    return {
        id: reminder.id,
        type: (typeLabels[reminder.deliveryMethod] || 'reminder'),
 ...
 */
function appointmentToUnified(apt) {
    const typeIcons = {
        restaurant: '🍽️',
        doctor: '🩺',
        dentist: '🦷',
 ...
 */
function detectItemConflicts(items) {
    const conflicts = [];
 ...
 */
function formatScheduleForSpeech(schedule) {
    ...
    if (morning.length > 0) {
        parts.push(`Morning: ${morning.map((i) => `${i.icon} ${i.title} at ${formatTimeForSpeech(i.scheduledFor)}`).join(', ')}.`);
    }
    if (afternoon.length > 0) {
        parts.push(`Afternoon: ${afternoon.map((i) => `${i.icon} ${i.title} at ${formatTimeForSpeech(i.scheduledFor)}`).join(', ')}.`);
    }
    if (evening.length > 0) {
        parts.push(`Evening: ${evening.map((i) => `${i.icon} ${i.title} at ${formatTimeForSpeech(i.scheduledFor)}`).join(', ')}.`);
    }
    // Mention conflicts
    if (schedule.conflicts.length > 0) {
        parts.push(`⚠️ Warning: ${schedule.conflicts.length} scheduling conflict${schedule.conflicts.length !== 1 ? 's' : ''} detected.`);
    }
    return parts.join(' ');
}
// ============================================================================
...
const checkScheduleConflictsDef = {
...
//# sourceMappingURL=unified-schedule-view.js.map