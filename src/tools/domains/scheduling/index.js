...
import { scheduleText, scheduleEmail, scheduleCall, textUser, getUserContactInfo, setUserContactInfo, } from '../proactive/outreach/index.js';
import { getPendingReminders, cancelReminder, parseNaturalTime, } from '../../../services/scheduling/reminder-scheduler.js';
// Import intelligent timing ML
import { getTimingRecommendation, } from '../../../services/contacts/optimal-timing.js';
const log = getLogger();
// ============================================================================
...
const scheduleMessageTool = {
    ...
    description: 'Schedule a text message to be sent at a specific time. Great for reminders, follow-ups, or time-sensitive messages.',
    domain: 'scheduling',
...
If no recipient specified, sends to the user themselves.`,
        parameters: z.object({
            ...
            when: z
                .string()
                .describe('When to send - natural language like "tomorrow at 9am", "in 2 hours", "next Monday"'),
            recipient: z
                .string()
                .optional()
                .describe('Who to send to - defaults to the user if not specified. Can be a name or "me/myself"'),
        }),
        execute: async (params) => {
            const userId = ctx.userId || 'unknown';
            const personaId = ctx.agentId || 'ferni';
            log.info({ userId, when: params.when, hasRecipient: !!params.recipient }, '📱 Scheduling text message');
            // Parse the time
...
The call will play a voice message when answered.`,
        parameters: z.object({
            ...
            when: z
                .string()
                .describe('When to call - natural language like "at 2pm", "tomorrow morning", "in 30 minutes"'),
        }),
        execute: async (params) => {
            const userId = ctx.userId || 'unknown';
            const personaId = ctx.agentId || 'ferni';
...
- "Send an email reminder about the deadline"`,
        parameters: z.object({
            ...
            when: z
                .string()
                .describe('When to send - natural language like "Friday morning", "next week", "in 3 days"'),
        }),
        execute: async (params) => {
            const userId = ctx.userId || 'unknown';
            const personaId = ctx.agentId || 'ferni';
            ...
            const result = await scheduleEmail(userId, params.subject, params.message, scheduledFor, personaId);
            if (result.success) {
                const timeStr = scheduledFor.toLocaleString('en-US', {
                    weekday: 'long',
                    month: 'short',
...
- "What messages are queued up?"`,
        ...
        execute: async () => {
            ...
            const items = pending.map((r, i) => {
                ...
                const typeEmoji = r.deliveryMethod === 'sms' ? '📱' : r.deliveryMethod === 'call' ? '📞' : '📧';
...
If multiple items exist, asks which one to cancel.`,
        ...
        execute: async (params) => {
            ...
            if (params.which) {
                ...
                const match = pending.find((r) => r.message.toLowerCase().includes(params.which.toLowerCase()) ||
...
// ============================================================================
const getOptimalSendTimeTool = {
    id: 'getOptimalSendTime',
    name: 'Get Optimal Send Time',
    description: 'Get ML-recommended best time to reach a contact based on learned response patterns.',
    domain: 'scheduling',
...
Returns the ML-learned optimal time slot and day with confidence level.`,
        ...
        execute: async (params) => {
            ...
            try {
                ...
                const contactId = params.contactId || `contact_${params.contactName.toLowerCase().replace(/\s+/g, '_')}`;
                const recommendation = await getTimingRecommendation(userId, contactId, params.contactName);
                ...
                const responseRateText = recommendation.expectedResponseRate > 0
...
// ============================================================================
const scheduleAtBestTimeTool = {
    id: 'scheduleAtBestTime',
    name: 'Schedule At Best Time',
    description: 'Schedule a message, call, or email at the ML-recommended optimal time for the recipient.',
    domain: 'scheduling',
...
Automatically picks the best time based on learned response patterns.`,
        ...
        execute: async (params) => {
            const userId = ctx.userId || 'unknown';
            const personaId = ctx.agentId || 'ferni';
            log.info({ userId, contactName: params.contactName, channel: params.channel }, '🧠 Scheduling at optimal time');
            try {
                // Get optimal timing
                const contactId = params.contactId || `contact_${params.contactName.toLowerCase().replace(/\s+/g, '_')}`;
                const recommendation = await getTimingRecommendation(userId, contactId, params.contactName);
                ...
                switch (params.channel) {
                    case 'email':
                        ...
                        result = await scheduleEmail(userId, params.subject, params.message, scheduledFor, personaId, contactOptions);
                        break;
                    case 'call':
                        result = await scheduleCall(userId, params.message, scheduledFor, personaId, contactOptions);
                        break;
                    case 'text':
                    default:
                        result = await scheduleText(userId, params.message, scheduledFor, personaId, contactOptions);
                        break;
                }
                if (result.success) {
                    const timeStr = scheduledFor.toLocaleString('en-US', {
                    ...
                    const channelEmoji = params.channel === 'text' ? '📱' : params.channel === 'email' ? '📧' : '📞';
                    const confidenceNote = recommendation.confidenceLevel === 'high'
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('scheduling', schedulingTools);
