 ...
 */
const triggerWebhookDef: ToolDefinition = {
  id: 'triggerWebhook',
        ...
        try {
          ...
          if (result.success) {
            log.info(
              ...
              'Webhook triggered via voice'
            );
            return `Done! I triggered your "${webhook.name}" automation.`;
          } else {
            log.warn(
              { userId, webhookId: webhook.id, error: result.error },
 ...
 */
const listWebhooksDef: ToolDefinition = {
  id: 'listWebhooks',
  name: 'List Webhooks',
      ...
      execute: async ({ includeDisabled }) => {
        ...
        try {
          ...
          const filtered = includeDisabled
            ? webhooks
            : webhooks.filter((w: WebhookConfig) => w.enabled);
          ...
          // Format the list
          const lines = filtered.map((w: WebhookConfig) => {
 ...
 */
const getWebhookStatusDef: ToolDefinition = {
  id: 'getWebhookStatus',
