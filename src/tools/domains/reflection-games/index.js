...
// ============================================================================
const threeWordDayStates = new Map();
const valuesCardSortStates = new Map();
const headlineWriterStates = new Map();
// ============================================================================
// TOOL DEFINITIONS
// ============================================================================
const startReflectionGameDef = {
        ...
        return llm.tool({
            ...
            execute: async ({ game, topic }) => {
                ...
                const gameDescriptions = {
                    two_truths_dream: "Let's play Two Truths and a Dream! Tell me two things that have actually happened in your life, and one thing you dream of doing. I'll try to guess which is the dream!",
                    values_auction: 'Imagine you have $100 to bid on different life values - things like adventure, security, creativity, connection, achievement. What would you spend the most on?',
                    rose_thorn_bud: "Let's do a Rose, Thorn, Bud reflection. Share something positive (rose), something challenging (thorn), and something you're looking forward to (bud).",
                    gratitude_chain: "Let's build a gratitude chain! I'll start with something I appreciate, then you add something that connects to it. We'll see where the chain takes us.",
                    life_bingo: "Let's play Life Bingo! I'll share some experiences and you tell me if you've done them. We'll celebrate the ones you have and maybe set intentions for new ones!",
                    finish_sentence: "I'll give you the start of a sentence, and you finish it with whatever comes to mind. Ready?",
                    would_you_rather: "Let's play Would You Rather with some deeper questions that might reveal what matters to you!",
                };
                const prompt = topic
...
const threeWordDayDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Start a Three Word Day reflection game. User describes their day, mood, or experience in exactly 3 words, then we explore what each word means. Great for daily check-ins and emotional awareness.',
            parameters: z.object({
                promptType: z
...
const valuesCardSortDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Start a Values Card Sort game. User sorts through 30 value cards to discover their top 5 core values. Based on ACT (Acceptance and Commitment Therapy) values clarification. Great for decision-making clarity and life direction.',
            parameters: z.object({}),
            execute: async () => {
                const log = getLogger();
...
const valuesCardSortRespondDef = {
...
const headlineWriterDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Start a Headline Writer game. User writes newspaper headlines about their life - past, present, or future. Based on narrative therapy techniques. Great for perspective-taking, life review, and goal visualization.',
            parameters: z.object({
                timeframe: z
...
const headlineWriterRespondDef = {
