...
function buildContextualAwareness(timeOfDay, isWeekend, isFriday, isMonday, season) {
    const insights = [];
    // Time-based insights
...
// ============================================================================
const getConversationAwarenessDef = {
        ...
        return llm.tool({
            ...
            execute: async ({ includeHistorical = false, includeQuirky = false }) => {
                ...
                if (majorHolidays[key]) {
                    significance.holidays.push(majorHolidays[key]);
                }
                // Generate contextual openers
                const openers = [];
                if (significance.holidays.length > 0) {
                    openers.push(`Today is ${significance.holidays[0]} - a great day to discuss...`);
                }
                // Day-of-week openers
                const dayOpeners = {
                    Monday: [
        ...
        return llm.tool({
            ...
            execute: async ({ focus = 'general' }, { ctx: toolCtx }) => {
                ...
                const response = {
                    ...
                    insights,
                    suggestions,
                    guidance: 'Be proactive but not pushy. Offer, don\'t insist. "I noticed..." or "Would it be helpful to..." works better than assuming.',
                };
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('awareness', awarenessTools);
