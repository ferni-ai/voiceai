...
async function getUserHabitsToday(userId) {
...
function getMicroAction(habitName) {
...
function getCelebration(streakDays) {
    if (streakDays >= 3) {
...
const intentionsCache = new Map();
// ============================================================================
// TOOL 1: QUICK HABIT CHECK
// ============================================================================
export const quickHabitCheckDefinition = {
    id: 'quickHabitCheck',
    name: 'Quick Habit Check',
    description: 'Quick 60-second voice check-in on habits. Natural conversation, not a data dump. ' +
        'Use for morning plans, midday check-ins, or end-of-day reviews.',
    domain: 'habits',
    tags: ['habits', 'voice', 'checkin', 'quick'],
    create: (_ctx) => {
        return llm.tool({
            description: 'Quick voice check-in on habit progress. Natural, conversational, under 60 seconds. ' +
            ...
            execute: async ({ context, focusHabit }, { ctx }) => {
                try {
                    ...
                    if (focusHabit) {
                        const habit = habits.find((h) => h.name.toLowerCase().includes(focusHabit.toLowerCase()));
                        if (habit) {
...
// ============================================================================
export const microCommitNowDefinition = {
    id: 'microCommitNow',
    name: 'Micro Commit Now',
    description: 'Prompt for immediate 2-minute action. Perfect for low energy, procrastination, ' +
        'or building momentum. Gives a specific tiny action to do RIGHT NOW.',
    domain: 'habits',
    tags: ['habits', 'voice', 'action', 'micro', 'tiny'],
    create: (_ctx) => {
        return llm.tool({
            description: 'Get a 2-minute micro action to do RIGHT NOW. Perfect for building momentum when energy ' +
...
// ============================================================================
export const implementationIntentionDefinition = {
    id: 'implementationIntention',
    name: 'Implementation Intention',
    description: 'Create "When X, I will Y" plans that double follow-through. Based on behavioral ' +
    ...
    create: (_ctx) => {
        return llm.tool({
            description: 'Create "When X, I will Y" implementation intentions for habits. Research shows these ' +
            ...
            execute: async ({ habit, cue, obstacle }, { ctx }) => {
                try {
                    ...
                    const existingHabit = habits.find((h) => h.name.toLowerCase().includes(habitName.toLowerCase()));
 ...
 */
const weeklyHabitReviewDefinition = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Generate a reflective weekly habit review. Perfect for Sunday check-ins ' +
            ...
            execute: async ({ tone, focusArea }) => {
                try {
                    ...
                    const { totalHabits, completedThisWeek, missedThisWeek, completionRate, bestStreak, improvingHabits, strugglingHabits, } = reviewData;
...
//# sourceMappingURL=habit-voice-tools.js.map