...
function habitDataToHabit(data, userId) {
    return {
        id: data.id,
        userId,
        name: data.name,
...
function habitToHabitData(habit) {
    return {
        id: habit.id,
        name: habit.name,
...
const habitLogsCache = new Map();
const loadedUsers = new Set();
async function ensureUserHabitsLoaded(userId) {
...
function persistHabit(userId, habit) {
    try {
        const store = getProductivityStore();
...
function persistHabitLog(userId, log) {
    try {
        const store = getProductivityStore();
...
function getTodayLog(habitId) {
    ...
    return (Array.from(habitLogsCache.values()).find((l) => {
...
function shouldTrackOnDay(habit, date) {
...
function getDueHabits(userId) {
    const today = new Date();
    return getUserHabitsFromCache(userId).filter((habit) => {
        if (!shouldTrackOnDay(habit, today))
            return false;
    ...
    return {
        ...
        logHabit: llm.tool({
            ...
            execute: async ({ habitName, count, notes }, { ctx }) => {
                ...
                const habit = userHabits.find((h) => h.name.toLowerCase().includes(habitName.toLowerCase()));
                if (!habit) {
