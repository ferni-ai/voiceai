 ...
 */
async function resolveContact(contactQuery, userId) {
    try {
        // =========================================================================
        // PRIMARY: Try unified entity store first
        // =========================================================================
        try {
            const { findContactForTelephony, isEntityStoreReady } = await import('../../../memory/entity-store/integration.js');
            if (isEntityStoreReady()) {
                const entityResult = await findContactForTelephony(userId, contactQuery);
                if (entityResult) {
                    log.info({ contactQuery, name: entityResult.name, source: 'entity_store' }, '📇 Contact resolved from unified entity store');
                    return {
            ...
            log.debug({ error: String(entityError), contactQuery }, 'Entity store lookup failed, falling back to legacy');
        }
        // =========================================================================
        // FALLBACK: Legacy contact_relationships collection
        // =========================================================================
        const { searchContacts } = await import('../../../services/contacts/contact-relationship-service.js');
        ...
        log.info({ contactQuery, name: contact.name, source: 'legacy_contact_relationships' }, '📇 Contact resolved from legacy collection');
        return {
            id: contact.id,
 ...
 */
function inferCallType(contact, purpose) {
...
function inferObjective(purpose) {
    const purposeLower = purpose.toLowerCase();
    if (purposeLower.includes('reschedule'))
        return 'reschedule';
    if (purposeLower.includes('cancel'))
        return 'cancel';
    if (purposeLower.includes('appointment') || purposeLower.includes('schedule'))
        return 'new_appointment';
    if (purposeLower.includes('reservation') || purposeLower.includes('book'))
        return 'reservation';
    if (purposeLower.includes('check') && purposeLower.includes('on'))
        return 'check_in';
    if (purposeLower.includes('tell') || purposeLower.includes('message'))
        return 'deliver_message';
    if (purposeLower.includes('ask') || purposeLower.includes('question'))
        return 'inquiry';
    return 'general';
}
// ============================================================================
 ...
 */
async function initiateOnBehalfCall(request) {
    // Dynamic import to avoid circular dependencies
    const { getOnBehalfCallOrchestrator } = await import('../../../services/outreach/on-behalf-call-orchestrator.js');
 ...
 */
function inferRelationshipFromQuery(query) {
 ...
 */
async function saveContactForFuture(userId, contactQuery, phone) {
    try {
 ...
 */
export function createCallOnBehalfTool(ctx) {
    return llm.tool({
        description: 'Call a third party (doctor, restaurant, family member, etc.) on behalf of the user. ' +
        ...
        execute: async (params) => {
            ...
            log.info({ userId: ctx.userId, contactQuery, phoneNumber: phoneNumber ? '***' : undefined, purpose }, 'Initiating call on behalf of user');
            try {
                ...
                const request = {
                    ...
                    requiresHIPAA: callType === 'business' &&
                        ['doctor', 'dentist', 'therapist', 'pharmacy', 'clinic', 'hospital'].some((kw) => (contact.relationship || '').toLowerCase().includes(kw)),
                };
...
//# sourceMappingURL=call-on-behalf.js.map