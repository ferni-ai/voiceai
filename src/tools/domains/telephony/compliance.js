 ...
 */
export function checkCallCompliance(request, config = DEFAULT_COMPLIANCE_CONFIG) {
    const issues = [];
    const warnings = [];
    const disclosures = [];
    log.debug({ callType: request.callType, requiresHIPAA: request.requiresHIPAA }, 'Checking call compliance');
    // -------------------------------------------------------------------------
    ...
    if (request.requiresHIPAA && config.requireHIPAADisclosure) {
        disclosures.push('State that the user has authorized you to call regarding their healthcare. ' +
    ...
    if (request.callType === 'business') {
        disclosures.push(`State that you are an AI assistant calling on behalf of ${request.userName}.`);
    }
    // -------------------------------------------------------------------------
 ...
 */
export function generateComplianceScript(request, compliance) {
    const parts = [];
    // Greeting
 ...
 */
export function requiresTwoPartyConsent(stateCode, config = DEFAULT_COMPLIANCE_CONFIG) {
    return config.twoPartyConsentStates.includes(stateCode.toUpperCase());
}
/**
 * Get compliance requirements for a specific call type
 */
export function getCallTypeRequirements(callType) {
    switch (callType) {
        case 'emergency':
            return {
                mustDo: [
