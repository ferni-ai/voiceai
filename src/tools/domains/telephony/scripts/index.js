 ...
 */
export function selectScript(contact, purpose) {
    ...
    if (restaurantKeywords.some((kw) => relationship.includes(kw)) ||
 ...
 */
export function buildCallScript(template, params) {
    ...
    parts.push(template.identityDisclosure.replace('{agentName}', agentName).replace('{userName}', userName));
 ...
 */
export function getScriptSummary(type) {
    switch (type) {
        case 'healthcare':
