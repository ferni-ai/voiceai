...
import { persistKeyMoment, persistTrackedItem, } from '../shared/persistence.js';
// ============================================================================
        ...
        return llm.tool({
            ...
            execute: async ({ activityType, activityName, durationMinutes, intensity, howTheyFeel, notes }, { ctx: toolCtx }) => {
                ...
                try {
                    ...
                    if (howTheyFeel)
                        response += `**How you feel:** ${howTheyFeel}\n`;
                    if (notes)
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ energyLevel, availableMinutes, preference, goal, equipment }) => {
                ...
                const energyKey = energyLevel === 'low'
                    ? 'low_energy'
                    : energyLevel === 'high'
                        ? 'high_energy'
                        : 'moderate_energy';
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ symptom, severity, location, duration, possibleTriggers, notes }) => {
                ...
                if (possibleTriggers)
                    response += `**Possible triggers:** ${possibleTriggers}\n`;
                if (notes)
                    response += `**Notes:** ${notes}\n`;
                response += `\n---\n\n`;
...
const prepareForDoctorVisitDef = {
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('health', healthTools);
