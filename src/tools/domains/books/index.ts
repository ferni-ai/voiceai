...
import {
  ...
  type Book,
  type BookGenre,
} from '../../../services/books/google-books.js';
...
} from '../../../services/books/reading-list-store.js';
...
function formatBookForSpeech(book: Book): string {
...
function formatBookList(books: Book[], limit = 5): string {
  const limited = books.slice(0, limit);
  if (limited.length === 0) return 'No books found.';
...
function formatReadingListEntry(entry: ReadingListEntry): string {
...
function formatReadingList(entries: ReadingListEntry[]): string {
  if (entries.length === 0) return 'Your reading list is empty.';
...
const searchBooksTool: ToolDefinition = {
  id: 'searchBooks',
  name: 'Search Books',
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        'Search for books by title, author, or topic. Returns book details including ratings and descriptions.',
      parameters: z.object({
...
const getBookRecommendationsTool: ToolDefinition = {
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        'Get book recommendations based on interests or genre. Great for discovering new reads.',
      parameters: z.object({
        interests: z
          .array(z.string())
...
const getBookDetailsTool: ToolDefinition = {
  id: 'getBookDetails',
  ...
  create: (_ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        'Get detailed information about a specific book including description, page count, and preview link.',
      parameters: z.object({
...
const getPopularBooksTool: ToolDefinition = {
  id: 'getPopularBooks',
  ...
  create: (_ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        'Get popular and bestselling books. Can filter by genre like fiction, nonfiction, mystery, romance, self_help, business, etc.',
      parameters: z.object({
        genre: z
          .enum([
            'fiction',
      ...
      execute: async ({ genre, limit = 5 }) => {
        ...
        try {
          const result = await getPopularBooks(genre as BookGenre, limit);
...
const addToReadingListTool: ToolDefinition = {
  id: 'addToReadingList',
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        "Add a book to the user's reading list for later. Can set priority and add notes.",
      parameters: z.object({
        bookId: z.string().describe('The Google Books ID'),
      ...
      execute: async ({ bookId, title, authors, imageUrl, pageCount, priority, notes }) => {
        ...
        try {
          const result = await addToReadingList(
            ctx.userId,
            { bookId, title, authors, imageUrl, pageCount },
            { priority, notes }
          );
...
const getReadingListTool: ToolDefinition = {
  id: 'getReadingList',
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        "Get the user's reading list. Can filter by status (want_to_read, reading, completed).",
      parameters: z.object({
        status: z
...
const updateReadingProgressTool: ToolDefinition = {
    ...
    return llm.tool({
      ...
      execute: async ({ entryId, status, currentPage, notes }) => {
        ...
        try {
          ...
          const updates = [];
          if (status) updates.push(`status to "${status.replace('_', ' ')}"`);
          if (currentPage) updates.push(`progress to page ${currentPage}`);
...
const markBookReadTool: ToolDefinition = {
  id: 'markBookRead',
  name: 'Mark Book Read',
...
const removeFromReadingListTool: ToolDefinition = {
...
const getReadingStatsTool: ToolDefinition = {
  id: 'getReadingStats',
...
const bookTools: ToolDefinition[] = [
  // Discovery
  searchBooksTool,
