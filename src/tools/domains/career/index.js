...
// ============================================================================
const assessCareerSatisfactionDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ currentRole, targetRole, currentSkills }) => {
                getLogger().info({ agentId: ctx.agentId, currentRole, targetRole }, 'Exploring growth areas');
...
const suggestJobSearchStrategyDef = {
        ...
        return llm.tool({
            ...
            execute: async ({ interviewType, role, mode, previousAnswer }) => {
                ...
                if (mode === 'feedback' && previousAnswer) {
                    ...
                    const questions = INTERVIEW_QUESTIONS[interviewType === 'behavioral'
                        ? 'behavioral'
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ role, location, yearsExperience, currentSalary }) => {
                ...
                if (yearsExperience)
                    response += `Experience: ${yearsExperience} years\n`;
                response += `\n`;
            ...
            execute: async ({ scenario, theirOffer, theirTarget }) => {
                ...
                if (scenario === 'tips-only') {
                    ...
                    if (theirOffer) {
                        ...
                        if (theirTarget)
                            response += `Your target: $${theirTarget.toLocaleString()}\n`;
                    }
                    response += `\n---\n\n`;
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ skill, currentLevel, timeAvailable, learningStyle }) => {
                ...
                if (learningStyle)
                    response += `Learning style: ${learningStyle}\n`;
                response += `\n---\n\n`;
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ symptoms, duration, workHours }) => {
                ...
                response += `\nDuration: ${duration}\n`;
                if (workHours)
                    response += `Work hours: ${workHours}/week\n`;
                response += `\n---\n\n`;
                // Determine level
                let level;
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ transitionType, currentState, targetState, timeline }) => {
                ...
                if (currentState)
                    response += `From: ${currentState}\n`;
                if (targetState)
                    response += `To: ${targetState}\n`;
                if (timeline)
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('career', careerTools);
