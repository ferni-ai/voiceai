...
function billDataToBill(data, userId) {
    return {
        id: data.id,
        userId,
        name: data.name,
...
function paymentToPaymentData(payment) {
    return {
        id: payment.id,
...
async function ensureUserBillsLoaded(userId) {
...
function persistBill(userId, bill) {
    try {
        const store = getProductivityStore();
...
function persistPayment(userId, payment) {
    try {
        const store = getProductivityStore();
...
// HELPER FUNCTIONS
// ============================================================================
function calculateNextDueDate(dueDay, frequency, fromDate) {
...
function calculateMonthlyTotal(userId) {
...
function formatBillForSpeech(bill) {
    const now = new Date();
...
export async function addBill(params) {
    ...
    try {
        ...
        getLogger().warn({ error: String(calendarError), billId: bill.id }, 'Failed to sync bill to calendar');
    }
    return bill;
}
export async function recordPayment(params) {
    ...
    getLogger().info({ billId: bill.id, paymentId: payment.id, amount: payment.amount }, 'Payment recorded');
    ...
    try {
        ...
        getLogger().warn({ error: String(calendarError), billId: bill.id }, 'Failed to update bill calendar event');
    }
    return { payment, bill };
}
...
export async function deactivateBill(billId) {
    ...
    try {
        ...
        getLogger().warn({ error: String(calendarError), billId }, 'Failed to remove bill from calendar');
    }
    return true;
}
// ============================================================================
...
export function createBillTools() {
    return {
        addBill: llm.tool({
            ...
            execute: async ({ name, payee, amount, dueDay, frequency, category, isAutoPay, reminderDays }, { ctx }) => {
        ...
        payBill: llm.tool({
            ...
            execute: async ({ billName, amount, confirmationNumber }, { ctx }) => {
                ...
                if (!result)
                    return `Couldn't record that payment. Want to try again?`;
        ...
        getBillSummary: llm.tool({
            ...
            execute: async (_, { ctx }) => {
                ...
                if (upcoming.length > 0) {
                    ...
                    const daysUntil = Math.ceil((nextBill.nextDueDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
