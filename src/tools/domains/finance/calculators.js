 ...
 */
export function calculateCompoundGrowth(principal, monthlyContribution, years, annualReturn) {
 ...
 */
export function calculateFeeImpact(initialInvestment, years, returnRate, lowFee, highFee) {
 ...
 */
export function calculateRetirementProjection(currentAge, retirementAge, currentSavings, monthlyContribution, annualReturn) {
 ...
 */
export function calculateEmergencyFund(monthlyExpenses, monthsCoverage = 6) {
 ...
 */
export function calculateSavingsRate(income, savings) {
 ...
 */
export function calculateYearsToDouble(annualReturn) {
    ...
    return {
        calculateCompoundGrowth: llm.tool({
            ...
            execute: async ({ principal, annualReturn, years, monthlyContribution = 0 }) => {
                getLogger().info(`Calculating compound growth: $${principal} at ${annualReturn}% for ${years} years`);
        ...
        calculateFeeImpact: llm.tool({
            ...
            execute: async ({ investment, years, highFeePercent = 1, lowFeePercent = 0.03, returnPercent = 7, }) => {
                getLogger().info(`Calculating fee impact: $${investment} over ${years} years`);
                const result = calculateFeeImpact(investment, years, returnPercent, lowFeePercent, highFeePercent);
        ...
        calculateRetirementProjection: llm.tool({
            ...
            execute: async ({ currentAge, retirementAge, currentSavings, monthlyContribution, annualReturn = 7, }) => {
                getLogger().info(`Calculating retirement projection: age ${currentAge} to ${retirementAge}`);
        ...
        calculateMortgage: llm.tool({
            ...
            execute: async ({ homePrice, downPaymentPercent = 20, interestRate, termYears = 30 }) => {
                ...
                getLogger().info(`Calculating mortgage: $${loanAmount} at ${interestRate}% for ${termYears} years`);
        ...
        calculateEmergencyFund: llm.tool({
            ...
            execute: async ({ monthlyExpenses, monthsCoverage = 6 }) => {
                ...
                getLogger().info(`Calculating emergency fund: ${monthsCoverage} months of $${monthlyExpenses}`);
