 ...
 */
export async function getAccountBalances(accessToken) {
    const result = await plaidRequest('/accounts/balance/get', {
 ...
 */
export async function getTransactions(accessToken, startDate, endDate, count = 100) {
    const result = await plaidRequest('/transactions/get', {
        access_token: accessToken,
...
export function analyzeSpending(transactions) {
    ...
    const recurringExpenses = Object.entries(merchantCounts)
        ...
        const frequency = data.amounts.length >= 4 ? 'weekly' : data.amounts.length >= 2 ? 'monthly' : 'occasional';
    ...
    return {
        ...
        largestExpenses,
        averageTransactionSize: spendingTxns.length > 0 ? Math.round((totalSpending / spendingTxns.length) * 100) / 100 : 0,
        recurringExpenses,
    };
}
/**
 ...
 */
export function formatBalancesForSpeech(accounts) {
    ...
    for (const account of accounts) {
        ...
        if (account.type === 'depository') {
            ...
            if (account.balances.limit) {
                ...
                lines.push(`• ${name}${lastFour}: $${balance.toLocaleString()} balance (${utilization}% of $${account.balances.limit.toLocaleString()} limit)`);
            }
            else {
 ...
 */
export function formatSpendingForSpeech(analysis, period) {
    ...
    if (sortedCategories.length > 0) {
        ...
        for (const [category, data] of sortedCategories) {
            ...
            lines.push(`• ${category}: $${data.total.toLocaleString()} (${pct}%) - ${data.count} transactions`);
        }
        lines.push('');
    }
    // Largest expenses
    ...
    return {
        checkBankLinkStatus: llm.tool({
            ...
            execute: async ({ userId }) => {
                ...
                getLogger().info({ userId, hasAccounts, institution: tokenData?.institution?.name }, 'Checking bank link status');
        ...
        linkBankAccount: llm.tool({
            ...
            execute: async ({ userId, deliveryMethod, contact }) => {
                ...
                getLogger().info({ userId, deliveryMethod, sendResult, elapsed }, '<<< TOOL: linkBankAccount COMPLETED');
                if (sendResult === 'sent') {
        ...
        getAccountBalances: llm.tool({
            ...
            execute: async ({ userId }) => {
                ...
                const elapsed = Date.now() - startTime;
                getLogger().info({ userId, accountCount: accounts.length, elapsed }, '<<< TOOL: getAccountBalances COMPLETED');
        ...
        getSpendingAnalysis: llm.tool({
            ...
            execute: async ({ userId, period = 'month' }) => {
                ...
                const transactions = await getTransactions(accessToken, startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0], 250);
                ...
                const formatted = formatSpendingForSpeech(analysis, period === 'week'
                    ? 'the past week'
                    : period === 'month'
                        ? 'the past month'
                        : 'the past 3 months');
                ...
                const jackComment = analysis.totalSpending > 0
        ...
        getRecentTransactions: llm.tool({
            ...
            execute: async ({ userId, merchantFilter, categoryFilter, limit = 10 }) => {
                const startTime = Date.now();
                getLogger().info({ userId, merchantFilter, categoryFilter, limit }, '>>> TOOL: getRecentTransactions STARTED');
                const accessToken = getStoredAccessToken(userId);
                if (!accessToken) {
                ...
                const startDate = new Date();
                startDate.setMonth(startDate.getMonth() - 1);
                let transactions = await getTransactions(accessToken, startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0], 100);
                // Apply filters
                if (merchantFilter) {
                    const filter = merchantFilter.toLowerCase();
                    transactions = transactions.filter((t) => t.merchant_name?.toLowerCase().includes(filter) ||
                        t.name.toLowerCase().includes(filter));
                }
                if (categoryFilter) {
                    const filter = categoryFilter.toLowerCase();
                    transactions = transactions.filter((t) => t.category?.some((c) => c.toLowerCase().includes(filter)));
                }
                // Take top N
                ...
                const elapsed = Date.now() - startTime;
                getLogger().info({ userId, resultCount: transactions.length, elapsed }, '<<< TOOL: getRecentTransactions COMPLETED');
        ...
        checkFinancialHealth: llm.tool({
            ...
            execute: async ({ userId }) => {
                ...
                const creditUtilization = totalCreditLimit > 0 ? Math.round((totalDebt / totalCreditLimit) * 100) : 0;
                // Build health report
                const lines = [];
                ...
                const monthsOfExpenses = spending.totalSpending > 0
                    ? Math.round((totalCash / spending.totalSpending) * 10) / 10
                    : 0;
                let emergencyFundAssessment;
