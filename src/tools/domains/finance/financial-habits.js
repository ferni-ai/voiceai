...
import { hasLinkedAccounts, getStoredAccessToken, getTransactions, analyzeSpending as analyzeRealSpending, formatSpendingForSpeech, } from './plaid.js';
    ...
    return {
        ...
        createBudget: llm.tool({
            ...
            execute: async ({ category, monthlyLimit, isEssential }, { ctx }) => {
                ...
                const existingIndex = budget.categories.findIndex((c) => c.name.toLowerCase() === category.toLowerCase());
                if (existingIndex >= 0) {
        ...
        createSavingsGoal: llm.tool({
            ...
            execute: async ({ name, targetAmount, deadline, monthlyContribution, priority, isEmergencyFund }, { ctx }) => {
                const userId = getUserId({ ctx });
                const store = getFinancialStore();
                await store.loadUserData(userId);
                const now = new Date().toISOString();
        ...
        getActualSpending: llm.tool({
            ...
            execute: async ({ userId, period }) => {
                ...
                let mayaComment = '\n\n';
                const topCategory = Object.entries(analysis.byCategory).sort((a, b) => b[1].total - a[1].total)[0];
                if (topCategory) {
        ...
        compareDebtStrategies: llm.tool({
            ...
            execute: async ({ debts, extraMonthlyPayment }) => {
                ...
                const calculateTotalInterest = (orderedDebts) => {
                    let totalInterest = 0;
        ...
        analyzeCashFlow: llm.tool({
            ...
            execute: async ({ payFrequency, payDays, majorBills }) => {
                ...
                const sumBills = (bills) => bills.reduce((sum, b) => sum + b.amount, 0);
        ...
        weeklyCheckIn: llm.tool({
            ...
            execute: async ({ weekNumber }) => {
                const week = weekNumber ||
                    Math.ceil((new Date().getTime() - new Date(new Date().getFullYear(), 0, 1).getTime()) /
                        (7 * 24 * 60 * 60 * 1000));
        ...
        getSpendingTriggerPatterns: llm.tool({
            ...
            execute: async (_, { ctx }) => {
                ...
                const byEmotion = triggers.reduce((acc, t) => {
                    if (!acc[t.emotion])
                        acc[t.emotion] = { count: 0, total: 0 };
                    acc[t.emotion].count++;
        ...
        recommendSavingsAccounts: llm.tool({
            ...
            execute: async ({ savingsGoal, amount, timeframe, riskTolerance = 'none' }) => {
                ...
                if (timeframe)
                    response += `**Timeframe:** ${timeframe.replace('-', ' ')}\n\n`;
                ...
                if (['short-term', 'vacation', 'general'].includes(savingsGoal) ||
                    timeframe === 'immediate' ||
        ...
        setSpendingLimit: llm.tool({
            ...
            execute: async ({ category, weeklyLimit, monthlyLimit, alertAt = 80 }, { ctx }) => {
                ...
                if (weeklyLimit)
                    response += `**Weekly limit:** $${weeklyLimit}\n`;
                if (monthlyLimit)
            ...
            execute: async ({ category }, { ctx }) => {
                ...
                for (const limit of toCheck) {
                    ...
                    if (limit.weeklyLimit) {
                        ...
                        const weekStatus = weekPercent >= 100 ? '🔴' : weekPercent >= limit.alertAtPercent ? '🟡' : '🟢';
                    ...
                    if (limit.monthlyLimit) {
                        ...
                        const monthStatus = monthPercent >= 100 ? '🔴' : monthPercent >= limit.alertAtPercent ? '🟡' : '🟢';
        ...
        logSpendingAgainstLimit: llm.tool({
            ...
            execute: async ({ category, amount, description }, { ctx }) => {
                ...
                if (updatedLimit.weeklyLimit) {
                    const weekPercent = Math.round((updatedLimit.currentWeekSpend / updatedLimit.weeklyLimit) * 100);
                ...
                if (updatedLimit.monthlyLimit) {
                    const monthPercent = Math.round((updatedLimit.currentMonthSpend / updatedLimit.monthlyLimit) * 100);
...
//# sourceMappingURL=financial-habits.js.map