    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ creation, howItFeels }) => {
                ...
                if (howItFeels)
                    response += `**How it feels:** ${howItFeels}\n`;
                response += `\n---\n\n`;
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ currentInterests, constraints, seeking }) => {
                ...
                if (seeking)
                    response += `**Seeking:** ${seeking}\n`;
                if (constraints?.length)
                    response += `**Constraints:** ${constraints.join(', ')}\n`;
                response += `\n---\n\n`;
...
const suggestHobbyBasedOnInterestsDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ preferences, budget, timePerWeek }) => {
                ...
                if (timePerWeek)
                    response += `• Time: ${timePerWeek}/week\n`;
                response += `\n---\n\n`;
...
// ============================================================================
const navigateCreativeBlockDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ blockType, project }) => {
                ...
                if (project)
                    response += `_Project: ${project}_\n`;
                response += `\n---\n\n`;
                const advice = {
                    'blank-page': `**The Blank Page Terror**\n\n` +
                    ...
                    perfectionism: `**Perfectionism: The Creativity Killer**\n\n` +
                    ...
                    'fear-of-judgment': `**Fear of What Others Will Think**\n\n` +
                    ...
                    'stuck-on-project': `**Stuck on a Specific Project**\n\n` +
                    ...
                    general: `**General Creative Block**\n\n` +
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ medium, currentPractice, desiredPractice }) => {
                ...
                if (currentPractice)
                    response += `**Currently:** ${currentPractice}\n`;
                if (desiredPractice)
                    response += `**Goal:** ${desiredPractice}\n`;
                response += `\n---\n\n`;
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('creativity', creativityTools);
