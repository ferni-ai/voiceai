...
import { getCompanyFundamentals, getEconomicDashboard, getYieldCurve } from './external-apis.js';
 ...
 */
export async function generateFIREInsights(
  userId: string,
  profile: FinancialProfile,
  currentSnapshot: FIRESnapshot,
  previousSnapshot: FIRESnapshot | null
): Promise<QuantInsight[]> {
  const insights: QuantInsight[] = [];
  const firestore = getQuantFirestore();
 ...
 */
export async function generateEconomicInsights(userId: string): Promise<QuantInsight[]> {
  const insights: QuantInsight[] = [];
  const firestore = getQuantFirestore();
 ...
 */
export async function generateAllInsights(userId: string): Promise<InsightGenerationResult> {
  const firestore = getQuantFirestore();
  const allInsights: QuantInsight[] = [];
  const errors: string[] = [];
  ...
  try {
    ...
    if (profile && fireHistory.length > 0) {
      try {
        const insights = await generateFIREInsights(
          userId,
          profile,
          fireHistory[0],
          fireHistory[1] || null
