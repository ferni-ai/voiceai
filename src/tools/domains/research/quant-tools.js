 ...
 */
async function fetchHistoricalPrices(symbol, days = 100) {
    return withRateLimit('yahoo-finance', async () => {
        ...
        return closes.filter((c) => c !== null).reverse();
    }, []);
}
/**
 * Calculate risk metrics for a stock
 */
function calculateRiskMetrics(prices, benchmarkPrices) {
    if (prices.length < 20) {
...
function calculateNetWorth(snapshot) {
    const totalAssets = snapshot.assets.cash +
        snapshot.assets.investments +
        snapshot.assets.property +
        snapshot.assets.other;
    const totalLiabilities = snapshot.liabilities.mortgage +
...
function calculateFIRENumber(annualExpenses, withdrawalRate = 4) {
...
function calculateRetirementReadiness(currentAge, targetRetirementAge, currentSavings, monthlyContribution, monthlyExpenses, expectedReturn = 7) {
...
function calculateBehavioralScore(behavior) {
    ...
    if (emotionalControl >= 80)
        ...
        improvements.push('Work on staying calm during market drops');
    if (discipline >= 80)
        strengths.push('Strong financial discipline');
    else if (discipline < 60)
        improvements.push('Focus on sticking to your budget');
    if (patience >= 80)
        strengths.push('Consistent saver - the key to building wealth');
    else if (patience < 60)
        improvements.push('Automate savings to improve consistency');
    if (behavior.panicSells === 0)
        strengths.push("You haven't panic sold - that's rare and valuable!");
    if (behavior.impulsePurchases === 0)
        strengths.push('No impulse purchases - great self-control');
    return {
        overallScore,
        emotionalControl,
        discipline,
...
function calculatePeerComparison(userMetrics, ageGroup) {
    ...
    const overallPercentile = Math.round((savingsRatePercentile + netWorthPercentile + debtPercentile + emergencyFundPercentile) / 4);
    const standoutAreas = [];
    if (savingsRatePercentile >= 75)
        standoutAreas.push(`Savings rate in top 25% for your age group`);
    if (netWorthPercentile >= 75)
        standoutAreas.push(`Net worth in top 25% for your age group`);
    if (debtPercentile >= 75)
        standoutAreas.push(`Debt management in top 25% for your age group`);
...
export function createQuantTools() {
    return {
        ...
        technicalIndicators: llm.tool({
            description: 'Calculate technical indicators for any stock: RSI (overbought/oversold), MACD (momentum), moving averages (trend), Bollinger Bands (volatility). Great for timing analysis.',
            parameters: z.object({
            ...
            execute: async ({ symbol, indicators = ['all'] }) => {
                ...
                const results = [`Technical Analysis for ${symbol.toUpperCase()}:`];
                if (showAll || indicators.includes('rsi')) {
                    const rsi = calculateRSI(prices);
                    const rsiSignal = rsi > 70
                ...
                if (showAll || indicators.includes('sma')) {
                    ...
                    const currentPrice = prices[0];
                    const trend = currentPrice > sma20 && sma20 > sma50
                        ? 'UPTREND'
        ...
        riskAnalysis: llm.tool({
            description: 'Calculate risk metrics for stocks: Beta (market sensitivity), Sharpe Ratio (risk-adjusted returns), Volatility, Max Drawdown, and Value at Risk (VaR). Essential for understanding risk.',
            parameters: z.object({
        ...
        analyzeSavingsRate: llm.tool({
            description: "Calculate and analyze your savings rate. Shows where you stand, how much you're saving annually, and personalized advice to improve.",
            parameters: z.object({
        ...
        calculateFIRE: llm.tool({
            description: 'Calculate your Financial Independence number - how much you need to retire early. Shows regular FIRE, Lean FIRE, Fat FIRE, and Coast FIRE numbers.',
            parameters: z.object({
        ...
        retirementReadiness: llm.tool({
            description: 'Calculate your retirement readiness score. Projects your savings at retirement, estimates years of retirement income, and gives personalized recommendations.',
            parameters: z.object({
            ...
            execute: async ({ currentAge, targetRetirementAge, currentSavings, monthlyContribution, monthlyExpenses, expectedReturn = 7, }) => {
                log.info({ currentAge, targetRetirementAge, currentSavings }, 'Calculating retirement readiness');
                const readiness = calculateRetirementReadiness(currentAge, targetRetirementAge, currentSavings, monthlyContribution, monthlyExpenses, expectedReturn);
                const scoreEmoji = readiness.score >= 100
                    ? '🎉'
                    : readiness.score >= 80
                        ? '👍'
                        : readiness.score >= 60
        ...
        behavioralScore: llm.tool({
            description: 'Analyze your financial behavior patterns. Scores emotional control, discipline, and patience. Identifies strengths and areas for improvement.',
            parameters: z.object({
            ...
            execute: async (behavior) => {
                ...
                const overallEmoji = score.overallScore >= 80
                    ? '🏆'
        ...
        peerComparison: llm.tool({
            description: 'See how you compare to others in your age group. Shows percentiles for savings rate, net worth, debt management, and emergency fund.',
            parameters: z.object({
            ...
            execute: async ({ ageGroup, savingsRate, netWorth, debtToIncome, emergencyFundMonths }) => {
                ...
                const comparison = calculatePeerComparison({ savingsRate, netWorth, debtToIncome, emergencyFundMonths }, ageGroup);
                const percentileEmoji = (p) => p >= 75 ? '🏆' : p >= 50 ? '👍' : p >= 25 ? '📈' : '💪';
                return [
...
export function createPersistentQuantTools() {
    return {
        ...
        saveFinancialProfile: llm.tool({
            description: 'Save or update your financial profile. Stores income, expenses, age, retirement goals, and risk tolerance for personalized analysis.',
            parameters: z.object({
            ...
            execute: async ({ monthlyIncome, monthlyExpenses, currentAge, targetRetirementAge, currentRetirementSavings, riskTolerance, }, { ctx }) => {
                const userId = getUserIdFromContext(ctx);
                if (!userId)
                    return 'I need to know who you are to save your profile. Please try again.';
        ...
        addToPortfolio: llm.tool({
            ...
            execute: async ({ symbol, shares, costBasis, accountType }, { ctx }) => {
                const userId = getUserIdFromContext(ctx);
                if (!userId)
                    return 'I need to know who you are to track your portfolio.';
                const { getQuantFirestore } = await import('./quant-firestore.js');
                const firestore = getQuantFirestore();
        ...
        viewPortfolio: llm.tool({
            ...
            execute: async (_params, { ctx }) => {
                const userId = getUserIdFromContext(ctx);
                if (!userId)
                    return 'I need to know who you are to show your portfolio.';
                const { getQuantFirestore } = await import('./quant-firestore.js');
                const firestore = getQuantFirestore();
        ...
        getDailyBriefing: llm.tool({
            ...
            execute: async (_params, { ctx }) => {
                const userId = getUserIdFromContext(ctx);
                if (!userId)
                    return 'I need to know who you are to generate your briefing.';
        ...
        recordFIREProgress: llm.tool({
            ...
            execute: async ({ netWorth, monthlyPassiveIncome = 0 }, { ctx }) => {
                const userId = getUserIdFromContext(ctx);
                if (!userId)
                    return 'I need to know who you are to track your progress.';
                ...
                const progressEmoji = percentToFire >= 100
                    ? '🎉'
                    : percentToFire >= 75
                        ? '🔥'
                        : percentToFire >= 50
...
function getUserIdFromContext(ctx) {
    ...
    if ('userId' in ctx && typeof ctx.userId === 'string') {
        return ctx.userId;
    }
...
//# sourceMappingURL=quant-tools.js.map