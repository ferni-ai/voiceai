...
export async function loadAllTheses(userId: string): Promise<InvestmentThesis[]> {
  try {
    const firestore = await getFirestore();
...
): Promise<{ goal: FinancialGoal; newMilestones: GoalMilestone[] } | null> {
  try {
    const firestore = await getFirestore();
    const docRef = firestore
      .collection(USERS_COLLECTION)
...
function deserializeGoal(data: Record<string, unknown>): FinancialGoal {
  ...
  return {
    ...data,
    target: {
      amount: target.amount as number,
      date: target.date ? new Date(target.date as string) : undefined,
    },
    current: {
      amount: current.amount as number,
      lastUpdated: new Date(current.lastUpdated as string),
    },
    progress: {
      percentage: progress.percentage as number,
      projectedCompletion: progress.projectedCompletion
        ? new Date(progress.projectedCompletion as string)
        : undefined,
      onTrack: progress.onTrack as boolean,
      monthlyRequired: progress.monthlyRequired as number | undefined,
    },
    milestones: milestones.map((m) => ({
      percentage: m.percentage as number,
      celebratedAt: m.celebratedAt ? new Date(m.celebratedAt as string) : undefined,
      celebrationMessage: m.celebrationMessage as string | undefined,
    })),
    createdAt: new Date(data.createdAt as string),
  } as FinancialGoal;
}

function calculateOnTrack(goal: FinancialGoal): boolean {
  if (!goal.target.date) return true;
...
export async function saveLifeEvent(userId: string, event: LifeEvent): Promise<void> {
  try {
    const firestore = await getFirestore();
    const docRef = firestore
      .collection(USERS_COLLECTION)
...
export async function loadLifeEvents(userId: string, limit = 50): Promise<LifeEvent[]> {
  try {
    const firestore = await getFirestore();
    const snapshot = await firestore
      .collection(USERS_COLLECTION)
...
export async function getRecentLifeEvents(userId: string, daysBack = 90): Promise<LifeEvent[]> {
  try {
    const firestore = await getFirestore();
...
export async function saveQuestion(userId: string, record: QuestionRecord): Promise<void> {
  try {
    const firestore = await getFirestore();
    const docRef = firestore
      .collection(USERS_COLLECTION)
...
export async function loadQuestionHistory(userId: string, limit = 100): Promise<QuestionRecord[]> {
  try {
    const firestore = await getFirestore();
    const snapshot = await firestore
      .collection(USERS_COLLECTION)
...
export async function hasAskedAbout(userId: string, topic: string): Promise<boolean> {
  try {
    const firestore = await getFirestore();
    const snapshot = await firestore
      .collection(USERS_COLLECTION)
...
export async function getConceptsExplained(userId: string): Promise<string[]> {
  try {
...
export async function recordEffectiveExplanation(
  userId: string,
  topic: string,
  approachUsed: string,
...
function createDefaultLearningPreferences(userId: string): LearningPreferences {
  return {
    userId,
...
export async function loadRiskEvents(userId: string): Promise<RiskEvent[]> {
  try {
    const firestore = await getFirestore();
    const snapshot = await firestore
      .collection(USERS_COLLECTION)
...
export async function getUserCrisisHistory(userId: string): Promise<{
  totalEvents: number;
...
export async function loadKnowledgeProfile(userId: string): Promise<KnowledgeProfile | null> {
  try {
    ...
    return {
      ...data,
      gaps: (data.gaps || []).map((g: Record<string, unknown>) => ({
        ...g,
        identifiedAt: new Date(g.identifiedAt as string),
        addressedAt: g.addressedAt ? new Date(g.addressedAt as string) : undefined,
      })),
...
export async function identifyKnowledgeGap(userId: string, gap: KnowledgeGap): Promise<void> {
  try {
...
export async function markGapAddressed(userId: string, topic: string): Promise<void> {
  try {
    const profile = await loadKnowledgeProfile(userId);
    if (!profile) return;
...
export async function getNextLearningTopic(userId: string): Promise<string | null> {
  try {
    const profile = await loadKnowledgeProfile(userId);
