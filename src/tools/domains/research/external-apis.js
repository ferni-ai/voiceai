 ...
 */
export async function getCompanyFundamentals(symbol) {
    const apiKey = process.env.ALPHA_VANTAGE_API_KEY;
    if (!apiKey) {
        log.warn('ALPHA_VANTAGE_API_KEY not set, using mock data');
        return getMockFundamentals(symbol);
    }
    return withRateLimit(`alpha-vantage-fundamentals-${symbol}`, async () => {
        try {
 ...
 */
export async function getEarningsHistory(symbol, limit = 4) {
    const apiKey = process.env.ALPHA_VANTAGE_API_KEY;
    if (!apiKey) {
        return getMockEarnings(symbol, limit);
    }
    return withRateLimit(`alpha-vantage-earnings-${symbol}`, async () => {
        try {
 ...
 */
export async function getEconomicIndicator(indicatorKey) {
    ...
    return withRateLimit(`fred-${indicatorKey}`, async () => {
        try {
...
export async function getEconomicDashboard() {
    ...
    const validIndicators = indicators.filter((i) => i !== null);
    // Generate summary
...
function getMockEarnings(symbol, limit) {
...
function getMockEconomicIndicator(indicatorKey) {
    const config = FRED_SERIES[indicatorKey] || {
