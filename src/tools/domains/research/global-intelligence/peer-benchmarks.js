...
export class PeerBenchmarks {
     ...
     */
    static calculatePercentile(value, metric) {
        if (value <= metric.p25) {
            // 0-25th percentile
    ...
    static getPeerComparison(userProfile) {
        ...
        const savingsPercentile = PeerBenchmarks.calculatePercentile(userProfile.savingsRate, benchmark.savingsRate);
        const netWorthPercentile = PeerBenchmarks.calculatePercentile(userProfile.netWorth, benchmark.netWorth);
        const behavioralPercentile = userProfile.behavioralScore >= benchmark.behavioralScore.topQuartile
            ? 85
            : userProfile.behavioralScore >= benchmark.behavioralScore.median
                ? 55
                : 30;
        const firePercentile = Math.min(Math.round((userProfile.fireProgress / benchmark.fireProgress.medianPercentage) * 50), 95);
        const overallPercentile = Math.round((savingsPercentile + netWorthPercentile + behavioralPercentile + firePercentile) / 4);
        const getComparison = (percentile) => {
            if (percentile >= 75)
                return 'top';
            if (percentile >= 50)
                return 'above';
        ...
        if (savingsPercentile >= 75) {
            insights.push(`Your ${userProfile.savingsRate}% savings rate puts you in the top quartile of your peers!`);
        }
        else if (savingsPercentile < 25) {
            insights.push(`Most peers in your demographic save around ${benchmark.savingsRate.median}% - there's room to grow.`);
        }
        ...
        if (userProfile.hasEmergencyFund && benchmark.characteristics.emergencyFundRate < 0.5) {
            insights.push(`Having an emergency fund puts you ahead of ${Math.round((1 - benchmark.characteristics.emergencyFundRate) * 100)}% of your peers.`);
        }
        if (userProfile.hasAutomatedSavings) {
            insights.push(`Automating savings is a key habit - only ${Math.round(benchmark.characteristics.automatedSavingsRate * 100)}% of your peers do this.`);
        }
     ...
     */
    static getIncomeBracket(annualIncome) {
...
//# sourceMappingURL=peer-benchmarks.js.map