...
const experimentStore = new Map<string, PersonalExperiment[]>();
const beliefStore = new Map<string, BeliefTracker[]>();
const hypothesisStore = new Map<string, HypothesisTracker>();
...
export const designExperiment = llm.tool({
  ...
  execute: async (
    params: { hypothesis: string; intervention: string; metric: string; duration: number },
    { ctx }: { ctx: unknown }
  ) => {
...
export const recordExperimentData = llm.tool({
  ...
  execute: async (
    params: { value: number; condition: 'control' | 'treatment'; notes?: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
...
export const updateBelief = llm.tool({
  ...
  execute: async (
    params: { beliefKeyword: string; evidence: string; direction: string; strength: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
...
export const updateHypothesis = llm.tool({
  ...
  execute: async (
    params: { hypothesisKeyword: string; evidence?: string; newStatus?: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
    ...
    if (params.newStatus) {
      ...
      else if (oldStatus === 'refuted') tracker.summary.refuted--;
      else if (oldStatus === 'inconclusive') tracker.summary.inconclusive--;

      if (params.newStatus === 'confirmed') tracker.summary.confirmed++;
      else if (params.newStatus === 'refuted') tracker.summary.refuted++;
      else if (params.newStatus === 'inconclusive') tracker.summary.inconclusive++;
    }
...
export const calculateEffectSize = llm.tool({
  ...
  execute: async (
    params: { beforeValues: number[]; afterValues: number[]; context: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
