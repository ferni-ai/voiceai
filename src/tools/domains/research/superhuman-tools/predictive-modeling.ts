...
// ============================================================================
// LOCAL TYPES (for goal records not exported from persistence)
// ============================================================================
...
interface GoalRecord {
  ...
  milestones: { date: Date; progress: number }[];
...
function calculateGoalVelocity(milestones: { date: Date; progress: number }[]): number {
...
export const recordGoalProgress = llm.tool({
  description:
    'Record progress on a goal. Over time, Peter will learn to predict your goal completion probability.',
  parameters: z.object({
  ...
  execute: async (
    params: { goalName: string; currentProgress: number; notes?: string },
    { ctx }: { ctx: unknown }
  ) => {
...
export const predictGoalSuccess = llm.tool({
  ...
  execute: async (params: { goalName: string }, { ctx }: { ctx: unknown }) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
    ...
    if (velocity > 0) {
      ...
      if (velocity > 2) successProbability += 10; // Strong velocity
    } else {
    ...
    if (velocity <= 0) riskFactors.push('Stalled momentum - no recent progress');
    if (goal.milestones.length < 5) riskFactors.push('Limited data for accurate prediction');
    if (expectedProgress && goal.currentProgress < expectedProgress - 10) {
      riskFactors.push('Behind schedule');
    }

    const successFactors: string[] = [];
    if (velocity > 0) successFactors.push('Positive momentum');
    if (goal.currentProgress > 50) successFactors.push('Past the halfway point');
    if (historicalSuccessRate > 60) successFactors.push('Strong track record');
...
export const recordHabit = llm.tool({
  description:
    'Record a habit completion or break. Over time, Peter will predict which habits will survive.',
  parameters: z.object({
  ...
  execute: async (
    params: { habitName: string; completed: boolean; notes?: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
...
export const analyzeHabitSurvival = llm.tool({
  ...
  execute: async (params: { habitName: string }, { ctx }: { ctx: unknown }) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
    ...
    const riskPeriods: { period: string; risk: string; reason: string }[] = [];
    if (daysSinceStart < 7) {
...
export const analyzeCounterfactual = llm.tool({
  description:
    "What would have happened if you'd made a different choice? Backtest your decisions against alternatives.",
  parameters: z.object({
  ...
  execute: async (
    params: { decision: string; alternative: string; domain: string; timeframe: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
...
export const predictLifeEventImpact = llm.tool({
  ...
  execute: async (
    params: { event: string; eventType: string; magnitude: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
