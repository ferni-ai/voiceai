...
function getSimulatedSECFiling(symbol) {
    const filings = {
        AAPL: {
            symbol: 'AAPL',
            filingType: '10-K',
...
export const analyzeSECFiling = llm.tool({
    description: "Analyze SEC filings (10-K, 10-Q) for any stock. Extract key changes, risk factors, red flags, and opportunities that most investors miss.",
    parameters: z.object({
...
// ============================================================================
function getSimulatedInsiderActivity(symbol) {
    const activities = {
        AAPL: {
            symbol: 'AAPL',
            period: 'Last 90 days',
...
export const trackInsiderTrading = llm.tool({
    description: "Track insider buying and selling. When executives buy with their own money, pay attention. Cluster buying is especially significant.",
    parameters: z.object({
        symbol: z.string().describe('Stock ticker symbol'),
    }),
    execute: async (params, { ctx }) => {
...
// ============================================================================
function getSimulatedOptionsFlow(symbol) {
    const flows = {
        SPY: {
            symbol: 'SPY',
            unusualActivity: [
...
export const analyzeOptionsFlow = llm.tool({
    description: "Analyze unusual options activity. Large, unusual options trades often precede major moves. See what smart money might be positioning for.",
    parameters: z.object({
        symbol: z.string().describe('Stock ticker symbol'),
    }),
    execute: async (params, { ctx }) => {
...
export const bridgeMacroToPersonal = llm.tool({
    description: "Connect macro economic events to YOUR personal finances. When the Fed raises rates, what should YOU do? Make macro relevant.",
    parameters: z.object({
        macroEvent: z.enum([
            'fed_rate_hike',
            'fed_rate_cut',
...
//# sourceMappingURL=financial-research.js.map