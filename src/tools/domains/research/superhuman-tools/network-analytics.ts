...
// ============================================================================
// LOCAL TYPES AND CACHES (sync with Firestore on read/write)
// ============================================================================
...
interface RelationshipRecord {
  ...
  interactionHistory: { date: Date; type: string; quality: number; topic: string }[];
...
export const trackRelationship = llm.tool({
  ...
  execute: async (
    ...
    userRelationships.push({
      ...
      supportProvided: [],
      supportReceived: [],
      energyImpact: params.energyImpact as RelationshipRecord['energyImpact'],
      influenceDomains: params.influenceDomains || [],
    });
...
export const logInteraction = llm.tool({
  ...
  execute: async (
    params: { name: string; type: string; quality: number; topic?: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
...
export const scoreRelationshipHealth = llm.tool({
  ...
  execute: async (params: { name: string }, { ctx }: { ctx: unknown }) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
    ...
    // Recency factor
    if (daysSinceContact < 7) healthScore += 20;
    else if (daysSinceContact < 30) healthScore += 10;
    else if (daysSinceContact > 90) healthScore -= 20;
    else if (daysSinceContact > 60) healthScore -= 10;

    // Quality factor
    healthScore += (avgQuality - 5) * 5;

    // Frequency factor
    if (interactions.length >= 10) healthScore += 10;
    else if (interactions.length >= 5) healthScore += 5;

    // Energy factor
    if (relationship.energyImpact === 'energizing') healthScore += 10;
    else if (relationship.energyImpact === 'draining') healthScore -= 10;
    ...
    if (daysSinceContact > 60) warnings.push('Extended period without contact');
    if (avgQuality < 5) warnings.push('Low interaction quality');
    if (trend === 'declining') warnings.push('Quality trending down');
    ...
    if (avgQuality >= 7) strengths.push('High quality interactions');
    if (daysSinceContact < 7) strengths.push('Recently connected');
    if (relationship.energyImpact === 'energizing') strengths.push('Energizing presence');
...
export const mapInfluence = llm.tool({
  ...
  execute: async (_params: Record<string, never>, { ctx }: { ctx: unknown }) => {
    ...
    const byDomain: Record<string, RelationshipRecord[]> = {};
...
export const analyzeNetworkGaps = llm.tool({
  ...
  execute: async (params: { goals: string[] }, { ctx }: { ctx: unknown }) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
...
function getNetworkSuggestions(area: string): string[] {
