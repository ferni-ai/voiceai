...
// HELPER FUNCTIONS
// ============================================================================
function findRelevantResearch(query) {
    ...
    return researchDatabase
        ...
        let score = 0;
        // Topic match
        if (entry.topic.includes(queryLower))
            score += 10;
        // Keyword matches
        for (const kw of keywords) {
            if (entry.keywords.some((k) => k.includes(kw)))
                score += 2;
            if (entry.topic.includes(kw))
                score += 3;
        }
        return { entry, score };
    })
...
export const scoreEvidenceQuality = llm.tool({
    description: "Score the evidence quality behind any claim. Get instant meta-analysis: study count, sample size, effect size, consistency. Know if advice is well-supported or just popular.",
    parameters: z.object({
    ...
    execute: async (params, { ctx }) => {
        ...
        if (totalStudies >= 30)
            score += 30;
        else if (totalStudies >= 15)
            score += 20;
        else if (totalStudies >= 5)
            score += 10;
        if (totalParticipants >= 10000)
            score += 25;
        else if (totalParticipants >= 5000)
            score += 15;
        else if (totalParticipants >= 1000)
            score += 10;
        if (topMatch.consensus === 'strong')
            score += 25;
        else if (topMatch.consensus === 'moderate')
            score += 15;
        else if (topMatch.consensus === 'weak')
            score += 5;
        if (topMatch.effectSize === 'large')
            score += 20;
        else if (topMatch.effectSize === 'medium')
            score += 10;
        else
            score += 5;
...
export const synthesizeResearch = llm.tool({
    description: "Get an instant synthesis of research on any life topic. What would take you weeks to read, I can summarize in seconds.",
    parameters: z.object({
    ...
    execute: async (params, { ctx }) => {
        ...
        if (params.depth !== 'quick') {
            output.push('', `═══════════════════════════════════`, `⚠️ **LIMITATIONS & CAVEATS**`, `═══════════════════════════════════`, ...allLimitations.slice(0, 5).map((l) => `• ${l}`));
        }
        output.push('', `═══════════════════════════════════`, `💡 **PRACTICAL IMPLICATIONS**`, `═══════════════════════════════════`, ...relevantResearch.slice(0, 3).map((r) => `• ${r.practicalTakeaway}`));
        if (params.depth === 'comprehensive') {
            output.push('', `═══════════════════════════════════`, `📖 **RESEARCH AREAS CONSULTED**`, `═══════════════════════════════════`, ...relevantResearch.map((r) => `• ${r.topic} (${r.studyCount} studies)`));
        }
        output.push('', `═══════════════════════════════════`, `🎯 **BOTTOM LINE**`, `═══════════════════════════════════`, overallConsensus === 'strong'
...
export const findCounterArguments = llm.tool({
    description: "Find the strongest arguments AGAINST something you believe. Combat confirmation bias by seeing the other side. No human friend will do this for you.",
    parameters: z.object({
    ...
    execute: async (params, { ctx }) => {
        ...
        const counterArgumentTemplates = {
            finance: [
                {
                    ...
                    strength: 'strong',
                    evidence: "You hear about winners, not the 90% who failed. The full picture may be less optimistic.",
                },
                {
            ...
            health: [
                ...
                },
                {
                    ...
                    strength: 'moderate',
                    evidence: 'Believing something works can create real effects even if the intervention itself is neutral.',
                },
                {
            ...
            general: [
                {
                    ...
                    strength: 'strong',
                    evidence: 'The human brain finds patterns even in random data. Is this correlation or causation?',
                },
                {
                ...
                },
                {
                    ...
                    strength: 'moderate',
                    evidence: 'Extreme situations naturally tend back toward average regardless of intervention.',
                },
            ],
        };
        ...
        return [
            ...
            `═══════════════════════════════════`,
            '',
            ...counterArgs.map((ca, i) => `**${i + 1}. ${ca.argument}** (${ca.strength})\n${ca.evidence}\n`),
            `═══════════════════════════════════`,
            `💭 **THE STEEL MAN**`,
            `═══════════════════════════════════`,
            '',
...
export const verifyClaim = llm.tool({
    description: "Fact-check any life advice or claim against peer-reviewed research. Know if that productivity tip is backed by science or just popular.",
    parameters: z.object({
    ...
    execute: async (params, { ctx }) => {
        ...
        const myths = {
            '21 days': {
                ...
                reality: 'The 21-day habit myth comes from a misquoted 1960s book. Research shows 66 days on average, ranging from 18-254 days.',
            },
            'multitask': {
                verdict: 'MYTH',
                reality: "True multitasking is neurologically impossible for complex tasks. What we call 'multitasking' is task-switching, which costs 20-40% productivity.",
            },
            'willpower muscle': {
                verdict: 'CONTESTED',
                reality: 'The "ego depletion" theory has failed to replicate in recent studies. Willpower may be more about beliefs and motivation than a depletable resource.',
            },
            'morning person': {
                verdict: 'NUANCED',
                reality: 'Chronotypes are real and partially genetic. Night owls forced into morning schedules show health impacts. Work WITH your biology.',
            },
            '10000 hours': {
                verdict: 'OVERSIMPLIFIED',
                reality: "Deliberate practice matters, but 10,000 hours is an average, not a rule. Quality of practice and domain matter more than raw hours.",
            },
            'right brain left brain': {
                verdict: 'MYTH',
                reality: 'No evidence that people are "right-brained" or "left-brained." Both hemispheres work together on virtually all tasks.',
            },
        };
        ...
        if (relevantResearch.length > 0) {
            const topMatch = relevantResearch[0];
            const verdict = topMatch.consensus === 'strong'
                ? 'SUPPORTED'
...
export const getBaseRate = llm.tool({
    description: "Combat probability blindness. Get the actual base rate for any claim. Your gut estimate is probably wrong - here's the data.",
    parameters: z.object({
...
//# sourceMappingURL=research-synthesis.js.map