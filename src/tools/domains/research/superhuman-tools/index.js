...
export const saveInvestmentThesis = llm.tool({
    description: 'Save your investment thesis for a stock. Peter will remind you of this during market volatility.',
    parameters: z.object({
    ...
    execute: async (params, { ctx }) => {
        ...
        if (!userId)
            return 'I need to know who you are to save this.';
...
export const remindThesis = llm.tool({
    description: 'Remind yourself why you bought a stock. Essential during market drops when emotions run high.',
    parameters: z.object({
        symbol: z.string().describe('Stock ticker symbol'),
    }),
    execute: async (params, { ctx }) => {
        ...
        const daysSincePurchase = Math.floor((Date.now() - thesis.purchaseDate.getTime()) / (1000 * 60 * 60 * 24));
        const lines = [
...
export const createFinancialGoal = llm.tool({
    description: 'Create a financial goal. Peter will track progress and celebrate milestones with you.',
    parameters: z.object({
...
export const updateGoalProgress = llm.tool({
    ...
    execute: async (params, { ctx }) => {
        ...
        if (!result)
            return 'Failed to update goal progress.';
...
export const predictBehavior = llm.tool({
    description: 'Predict how you might react to future market events based on your history and patterns from similar investors.',
    parameters: z.object({
        scenario: z.enum([
            'market_drop_10',
            'market_drop_20',
    ...
    execute: async (params, { ctx }) => {
        ...
        if (probability > 50) {
            lines.push('Write down your investment theses NOW. When the drop comes, read them before making any decisions.');
...
export const recordLifeEvent = llm.tool({
    description: 'Record a significant life event. Peter will adjust advice based on your life circumstances.',
    parameters: z.object({
        type: z.enum([
            'career',
            'family',
    ...
    execute: async (params, { ctx }) => {
        ...
        if (!userId)
            return 'I need to know who you are.';
        const direction = (incomeChange || 0) - (expenseChange || 0) + (oneTimeImpact || 0) > 0
            ? 'positive'
...
export const getNextLesson = llm.tool({
    description: 'Find out what financial topic you should learn next based on your goals and knowledge gaps.',
    parameters: z.object({}),
