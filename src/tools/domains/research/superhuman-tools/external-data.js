...
import { getUserIdFromContext, saveSpendingRecord, } from './firestore-persistence.js';
const log = getLogger();
...
function getLocalEconomicData(location) {
...
export const getLocalEconomics = llm.tool({
    description: "Get local economic conditions for your area. Housing market, job market, cost of living - contextualized for your location.",
    parameters: z.object({
...
export const synthesizeIndustryTrends = llm.tool({
    description: "Get a synthesis of trends in any industry. Key developments, disruption risks, opportunities, and leading companies.",
    parameters: z.object({
...
// ============================================================================
function getNewsSentiment(topic) {
...
export const analyzeNewsSentiment = llm.tool({
    description: "Analyze news sentiment on any topic. See if coverage is positive/negative and trending up/down. Useful for timing decisions.",
    parameters: z.object({
...
export const recordSpending = llm.tool({
    description: "Record spending data to calculate your personal inflation rate. Track how YOUR costs compare to official CPI.",
    parameters: z.object({
        category: z.enum([
...
export const calculatePersonalInflation = llm.tool({
    description: "Calculate YOUR personal inflation rate vs official CPI. See how your actual cost increases compare to government statistics.",
    parameters: z.object({
    ...
    execute: async (params, { ctx }) => {
        ...
        const recentSpending = userSpending.filter(s => Date.now() - s.date.getTime() < 30 * 24 * 60 * 60 * 1000);
        ...
        return [
            ...
            `═══════════════════════════════════`,
            '',
            ...categoryBreakdown.map(c => `**${c.category.charAt(0).toUpperCase() + c.category.slice(1)}**\n` +
            ...
            `═══════════════════════════════════`,
            '',
            ...categoryBreakdown.slice(0, 3).map((c, i) => `${i + 1}. ${c.category} (${c.contribution.toFixed(1)}% contribution)`),
            '',
            `═══════════════════════════════════`,
            `💡 **HEDGING STRATEGIES**`,
            `═══════════════════════════════════`,
