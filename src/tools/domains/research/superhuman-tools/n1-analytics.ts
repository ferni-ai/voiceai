...
export const recordDecisionOutcome = llm.tool({
  ...
  execute: async (
    params: { decisionKeywords: string; wasReversed: boolean; satisfaction: number },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';

    let userDecisions: DecisionRecord[];
    try {
    ...
    const matchingDecision = userDecisions.find((d) =>
      keywords.some((k) => d.decision.toLowerCase().includes(k))
    );
    ...
    const daysSince = Math.floor(
      (Date.now() - new Date(matchingDecision.timestamp).getTime()) / (1000 * 60 * 60 * 24)
    );
...
export const analyzeDecisionQuality = llm.tool({
  ...
  execute: async (params: { domain: string }, { ctx }: { ctx: unknown }) => {
    ...
    // Calculate patterns
    const byHour: Record<number, { total: number; satisfaction: number; reversals: number }> = {};
    const byDay: Record<number, { total: number; satisfaction: number; reversals: number }> = {};

    for (const d of filtered) {
      const hour = d.context.timeOfDay;
      const day = d.context.dayOfWeek;

      if (!byHour[hour]) byHour[hour] = { total: 0, satisfaction: 0, reversals: 0 };
      if (!byDay[day]) byDay[day] = { total: 0, satisfaction: 0, reversals: 0 };
    ...
    const avgSatisfaction =
      filtered.reduce((sum, d) => sum + (d.outcome?.satisfaction || 0), 0) / filtered.length;
...
export const analyzeSleepCorrelations = llm.tool({
  description:
    "Analyze how your sleep correlates with spending, decisions, and productivity. This reveals patterns you'd never notice yourself.",
  parameters: z.object({}),
  execute: async (_params: Record<string, never>, { ctx }: { ctx: unknown }) => {
    ...
    if (withQuality.length >= 7) {
      const byHours: Record<number, number[]> = {};
      for (const s of withQuality) {
        const rounded = Math.round(s.hours);
        if (!byHours[rounded]) byHours[rounded] = [];
...
export const predictEnergy = llm.tool({
  ...
  execute: async (
    params: { sleepLastNight?: number; calendarLoad?: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
    ...
    if (params.calendarLoad) {
      ...
      calendarModifier = modifiers[params.calendarLoad as keyof typeof modifiers] || 0;
    }
...
export const analyzePeakPerformance = llm.tool({
  ...
  execute: async (_params: Record<string, never>, { ctx }: { ctx: unknown }) => {
    ...
    const decisionsByHour: Record<number, { total: number; satisfaction: number }> = {};
    for (const d of decisions.filter((d) => d.outcome)) {
      const hour = d.context.timeOfDay;
      if (!decisionsByHour[hour]) decisionsByHour[hour] = { total: 0, satisfaction: 0 };
...
export const calculateLifestyleImpact = llm.tool({
  ...
  execute: async (
    params: { change: string; type: string; magnitude: string },
    { ctx }: { ctx: unknown }
  ) => {
    const userId = getUserIdFromContext(ctx);
    if (!userId) return 'I need to know who you are.';
    ...
    const impactMatrix: Record<string, { domain: string; impact: number; direction: string }[]> = {
      habit_add: [
    ...
    const multiplier = magnitudeMultiplier[params.magnitude as keyof typeof magnitudeMultiplier];
    ...
    const netScore = impacts.reduce(
      (sum, i) => sum + (i.direction === 'negative' ? -i.impact : i.impact),
      0
    );
    ...
    return [
      ...
      `═══════════════════════════════════`,
      ...positiveImpacts.map(
        (i) => `• ${i.domain.charAt(0).toUpperCase() + i.domain.slice(1)}: +${i.impact}%`
      ),
      '',
      `═══════════════════════════════════`,
      `🔴 **COSTS/TRADE-OFFS**`,
