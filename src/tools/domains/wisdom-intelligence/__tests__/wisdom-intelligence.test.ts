/**
 * Wisdom Intelligence Domain Tests
 *
 * Tests the "Better Than Human" wisdom capabilities.
 */
...
describe('wisdom-intelligence domain', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should have all tools in wisdom-intelligence domain', async () => {
    const tools = await getToolDefinitions();

    for (const tool of tools) {
      expect(tool.domain).toBe('wisdom-intelligence');
    }
  });

  it('should have superhuman-wisdom tag on all tools', async () => {
    const tools = await getToolDefinitions();

    for (const tool of tools) {
      expect(tool.tags).toContain('superhuman-wisdom');
    }
  });
