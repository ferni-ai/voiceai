...
// ============================================================================
const setSpokenLanguageDef = {
    id: 'setSpokenLanguage',
    name: 'Set Spoken Language',
    description: 'Changes the spoken language for the conversation. Call this when the user asks to speak in a different language.',
    domain: 'settings',
...
After calling this tool, you should respond in the new language to confirm the switch.`,
            parameters: z.object({
                language: z
                    .string()
                    .describe('The language to switch to. Can be a language name (Spanish, French) or code (es, fr)'),
            }),
            execute: async (params) => {
                const { language } = params;
                try {
                    // Dynamically import to avoid circular dependencies
                    const { parseLanguageName, setSessionLanguage, getLanguageConfig, persistUserLanguagePreference, } = await import('../../../services/language/index.js');
                    ...
                    log.error({ error: String(error), userId: ctx.userId, language }, '🌍 Failed to switch language');
...
// ============================================================================
const listSupportedLanguagesDef = {
    id: 'listSupportedLanguages',
    name: 'List Supported Languages',
    description: 'Lists all languages supported by Ferni. Call this when the user asks what languages you can speak.',
    domain: 'settings',
...
// ============================================================================
const getCurrentLanguageDef = {
    id: 'getCurrentLanguage',
    name: 'Get Current Language',
    description: 'Gets the current spoken language for the conversation. Call this when the user asks what language you are speaking.',
    domain: 'settings',
...
- "What's my language set to?"`,
            ...
            execute: async () => {
                try {
                    const { getSessionLanguageState, getLanguageConfig } = await import('../../../services/language/index.js');
                    ...
                    const autoDetectedNote = state.autoDetected
                        ...
                        : '';
                    log.info({ userId: ctx.userId, language: state.currentLanguage }, '🌍 Reporting current language');
                    return `We're currently speaking in ${config.displayName}${autoDetectedNote}. Would you like to switch to a different language?`;
                }
                catch (error) {
                    log.error({ error: String(error), userId: ctx.userId }, '🌍 Failed to get current language');
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('settings', languageToolDefinitions);
