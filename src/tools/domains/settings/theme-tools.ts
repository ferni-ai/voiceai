 ...
 */
function parseTheme(input: string): Theme {
...
const setThemeDef: ToolDefinition = {
  id: 'setTheme',
  name: 'Set Theme',
  description:
    'Changes the visual theme (dark/light/auto) for the app. Call this when the user wants to change the app appearance.',
  domain: 'settings',
...
The theme parameter should be what the user said (e.g., "dark mode", "lighter", "system").`,
      ...
      execute: async ({ theme }) => {
        try {
          ...
          log.info(
            { userId: ctx.userId, theme: normalizedTheme, requested: theme },
            '🎨 Theme switched via voice'
          );

          // Human-friendly confirmations
          const confirmations: Record<Theme, string> = {
...
- "What's my current theme?"`,
      ...
      execute: async () => {
        try {
          ...
          const theme: Theme = ctx.userId ? await getThemePreference(ctx.userId) : 'auto';
