...
import { getEventsForDay, getEventsForWeek, createEvent, updateEvent, deleteEvent, findFreeTimeSlots, isTimeSlotAvailable, getDayOverview, getWeekOverview, } from '../../../services/calendar/index.js';
// Keep formatting helpers from old service for now
import { formatEventForSpeech, formatDayOverviewForSpeech, } from '../../../services/calendar/calendar-service.js';
import { generateDailyBriefing, suggestMeetingTimes, detectCalendarAlerts, } from '../../../services/calendar/calendar-intelligence.js';
import { parseNaturalDate, } from '../../../services/calendar/natural-date-parser.js';
import { parseEventRequest, } from '../../../services/calendar/event-confirmation.js';
import { getUpcomingBriefings, getPostMeetingFollowUps, analyzeConflicts, findBestTimeFor, } from '../../../services/calendar/proactive-calendar.js';
import { detectRecoveryNeeds, findRecoveryOpportunities, } from '../../../services/calendar/recovery-protection.js';
import { getCalendarLoadFactors } from '../../../services/calendar/calendar-load-service.js';
import { enrichPreMeetingBriefing, recordMeetingInteraction, } from '../../../services/calendar/meeting-memory-service.js';
const log = getLogger();
// ============================================================================
// GET CALENDAR TODAY
// ============================================================================
const getCalendarTodayDef = {
    id: 'getCalendarToday',
...
const scheduleEventNaturalDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Schedule an event using natural language time expressions. Use when user says things like "schedule a meeting tomorrow at 3" or "add dentist appointment next Tuesday". Returns confirmation or asks for clarification.',
        parameters: z.object({
...
// ============================================================================
const getPreMeetingBriefingDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Get enriched briefings for upcoming meetings including prep tips, relationship history, past topics, and open commitments. Use when user asks "What should I know before my next meeting?" or is about to join a meeting.',
        parameters: z.object({
        ...
        execute: async (params) => {
            ...
            if (enrichedBriefing?.relationshipContext &&
            ...
            if (enrichedBriefing?.suggestedAgendaItems &&
...
// ============================================================================
const getPostMeetingFollowUpDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Get follow-up prompts after a recent meeting to capture action items. Use when user just finished a meeting or says "my meeting just ended".',
        parameters: z.object({
            windowMinutes: z
                .number()
                .optional()
...
const recordMeetingOutcomeDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Record the outcome of a meeting including topics discussed, commitments made, and action items. This information will be used to enrich future pre-meeting briefings. Use after user shares meeting details.',
        parameters: z.object({
        ...
        execute: async (params) => {
            ...
            try {
                ...
                log.info({ userId, attendee: params.attendeeEmail, topicCount: params.topics.length }, 'Recorded meeting outcome');
                return response;
            }
            catch (error) {
...
const checkConflictsDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Check if a proposed time conflicts with existing events and get alternatives. Use when user proposes a time and you want to verify availability.',
        parameters: z.object({
            time: z
                .string()
...
const smartRescheduleDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Intelligently reschedule an event by finding the optimal new time based on calendar patterns, energy levels, and meeting context. Use when user wants to move a meeting or when a conflict is detected.',
        parameters: z.object({
        ...
        execute: async (params) => {
            ...
            try {
                ...
                const event = weekEvents.find((e) => e.id === params.eventIdOrTitle ||
                ...
                const scoredSlots = freeSlots
                    ...
                    else if (params.preferredTimeOfDay === 'afternoon' &&
                        slotHour >= 12 &&
                        slotHour < 17) {
                        score += 20;
                    }
                ...
                log.info({ userId, eventId: event.id, optionsFound: topSlots.length }, 'Smart reschedule options generated');
                return response;
            }
            catch (error) {
...
const findBestTimeDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Find the best time for a new event based on user preferences, learned patterns, and energy levels. Uses "Better Than Human" energy-aware scheduling to suggest times when the user is typically most productive. Use when user asks "when should I schedule this?" or needs help finding a good time.',
        parameters: z.object({
...
const detectRecoveryNeedsDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Detect if the user needs recovery time based on their calendar load and patterns. Use this proactively when you sense the user might be overwhelmed or when discussing workload.',
        parameters: z.object({}),
        execute: async () => {
            const { userId } = ctx;
            if (!userId) {
            ...
            let response = "I've been looking at your calendar, and I want to share some observations:\n\n";
            // Group by urgency
            ...
            log.info({ userId, needsCount: needs.length, immediateUrgency: immediateUrgency.length }, 'Recovery needs detected');
            return response;
        },
    }),
};
...
// ============================================================================
const findRecoveryOpportunitiesDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Find optimal time slots for recovery, breaks, or deep rest. Use when user needs to find time to recharge or when proactively suggesting rest.',
        parameters: z.object({
            minDurationMinutes: z
                .number()
                .optional()
        ...
        execute: async (params) => {
            ...
            const opportunities = await findRecoveryOpportunities(userId, params.minDurationMinutes || 60);
            ...
            log.info({ userId, opportunityCount: opportunities.length }, 'Recovery opportunities found');
            return response;
        },
    }),
};
...
const blockRecoveryTimeDef = {
    ...
    create: (ctx) => llm.tool({
        description: 'Block time on the calendar for rest, deep work, or personal recovery. Use when user agrees to take a break or when proactively protecting their energy.',
        parameters: z.object({
            durationMinutes: z
                .number()
            ...
            startTime: z
                .string()
                .optional()
                .describe('When to start the recovery block (ISO format or natural time like "2pm tomorrow"). If not specified, will find the next available slot.'),
        }),
        execute: async (params) => {
            const { userId } = ctx;
            if (!userId) {
            ...
            const eventInput = {
                title,
                description: 'Time blocked for rest, deep work, or personal recovery. Protected by Ferni.',
...
// ============================================================================
const getCalendarLoadSummaryDef = {
    ...
    create: (ctx) => llm.tool({
        description: "Get an analysis of the user's calendar load, including meeting hours, focus time, and patterns. Use when discussing workload, productivity, or when the user mentions feeling overwhelmed.",
        parameters: z.object({}),
        execute: async () => {
            const { userId } = ctx;
            if (!userId) {
            ...
            log.info({ userId, weeklyMeetingHours: load.weeklyMeetingHours }, 'Calendar load summary generated');
            return response;
        },
    }),
};
...
// EXPORTS
// ============================================================================
export const smartCalendarTools = [
    // Core calendar tools
    getCalendarTodayDef,
    getCalendarWeekDef,
...
//# sourceMappingURL=smart-calendar-tools.js.map