...
import { createAppointmentTools, createContactsTools, createDeliveryTools, createPlacesTools, } from '../../scheduling.js';
// ============================================================================
...
// ============================================================================
function getAppointmentToolDefinitions() {
    const log = getLogger();
    ...
    return [
        {
            ...
            description: 'Schedule, confirm, cancel, or check status of appointments. Works for doctors, dentists, salons, and other services.',
            domain: 'calendar',
            ...
            create: (_ctx) => llm.tool({
                ...
                execute: async (params) => {
                    log.info({ action: params.action, business: params.businessName }, '📅 Appointment tool called');
                    try {
                        switch (params.action) {
                            ...
                            case 'status':
                                ...
                                return await legacyTools.getAppointmentStatus.execute({ includeCompleted: false }, STUB_CONTEXT);
                            default:
                                return 'Please specify an action: schedule, confirm, cancel, or status.';
                        }
                    }
                    catch (error) {
                        log.error({ action: params.action, error: String(error) }, '📅 Appointment tool error');
            ...
            create: (_ctx) => llm.tool({
                ...
                execute: async (params) => {
                    ...
                    try {
                        ...
                        log.error({ action: params.action, error: String(error) }, '🍽️ Restaurant tool error');
...
// ============================================================================
function getDeliveryToolDefinitions() {
    const log = getLogger();
    ...
    return [
        {
            id: 'foodDelivery',
            name: 'Food Delivery',
            description: 'Order food for delivery - search restaurants, start orders, add items, checkout.',
            domain: 'calendar',
            ...
            create: (_ctx) => llm.tool({
                ...
                execute: async (params) => {
                    log.info({ action: params.action, query: params.query }, '🚚 Food delivery tool called');
                    try {
                        switch (params.action) {
                            case 'search':
                            ...
                            case 'start':
                                return await legacyTools.startFoodOrder.execute({
                                    restaurantName: params.restaurantName || params.query || '',
                                    platform: (params.platform === 'both' ? 'doordash' : params.platform) || 'doordash',
                                }, STUB_CONTEXT);
                            case 'add':
                            ...
                            case 'checkout':
                                ...
                                return await legacyTools.checkoutOrder.execute({ orderId: params.orderId, tip: params.tip }, STUB_CONTEXT);
                            case 'status':
                                if (!params.orderId) {
                                    return 'I need the order ID to check status.';
                                }
                                return await legacyTools.getOrderStatus.execute({ orderId: params.orderId }, STUB_CONTEXT);
                            case 'quick':
                        ...
                        log.error({ action: params.action, error: String(error) }, '🚚 Food delivery tool error');
...
// ============================================================================
function getPlacesToolDefinitions() {
    const log = getLogger();
    ...
    return [
        {
            id: 'findBusiness',
            name: 'Find Business',
            description: 'Search for businesses by name, type, or location. Get details including phone, hours, address, and reviews.',
            domain: 'calendar',
...
// ============================================================================
function getContactsToolDefinitions() {
    const log = getLogger();
    ...
    return [
        {
            ...
            create: (_ctx) => llm.tool({
                ...
                execute: async (params) => {
                    log.info({ action: params.action, name: params.name || params.query }, '📇 Contacts tool called');
                    try {
                        switch (params.action) {
                            case 'add':
                                if (!params.name) {
                            ...
                            case 'find':
                                return await legacyTools.findMyContact.execute({ query: params.query || params.name || '' }, STUB_CONTEXT);
                            case 'update':
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('calendar', calendarTools);
