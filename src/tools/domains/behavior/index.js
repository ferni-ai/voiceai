...
import { z } from 'zod';
import { createModeShiftSignal, createPacingChangeSignal, createHoldSpaceSignal, createProcessingSignal, } from '../../../services/behavior/index.js';
import { composeProcessingExpression, formatProcessingAsSSML, } from '../../../intelligence/processing-intelligence.js';
...
// TOOL DEFINITIONS
// ============================================================================
export const shiftModeDef = {
    id: 'shiftMode',
    name: 'Shift Mode',
    description: 'Change your presence mode. Modes affect HOW you speak: presence (minimal words), ' +
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Change your presence mode. Use presence for minimal words, ' +
...
export const adjustPacingDef = {
    id: 'adjustPacing',
    name: 'Adjust Pacing',
    description: 'Control your speech rhythm. Use slower/faster speed and shorter/longer pauses ' +
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Control your speech rhythm. Adjust speed (slower/normal/faster) and ' +
...
export const processingDef = {
    id: 'processing',
    name: 'Processing',
    description: 'Take visible thinking time. Types: thinking (general), emotional (processing feelings), ' +
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Take visible thinking time. Use thinking for general thought, ' +
...
export const holdSpaceDef = {
    id: 'holdSpace',
    name: 'Hold Space',
    description: 'Create intentional meaningful silence. Use after heavy content to let things land. ' +
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Create intentional meaningful silence. Use brief (3s) for short pauses, ' +
                'medium (5s) after emotional content, extended (8s) after heavy shares.',
            parameters: holdSpaceSchema,
            execute: async ({ duration, reason }) => {
                const durationMs = SILENCE_DURATIONS_MS[duration];
...
export const expressPresenceDef = {
    id: 'expressPresence',
    name: 'Express Presence',
    description: 'Show non-verbal presence. Types: breath (audible breath), hum (soft hum), ' +
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Show non-verbal presence. Use breath for audible breathing, hum for soft "mmm", ' +
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('behavior', behaviorTools);
