 ...
 */
function createLeaveMessageTool(ctx) {
    return {
...
- emotionalContext (string, optional): Brief note about the caller's emotional state (e.g., "sounded happy", "seemed worried")`,
        ...
        execute: async (args) => {
            ...
            try {
                ...
                const { getInboundCallContext } = await import('../../../intelligence/context-builders/external/inbound-call-context.js');
                const callContext = getInboundCallContext(sessionId);
                if (!callContext?.sponsoredIdentityId || !callContext?.sponsorUserId) {
                    return {
                        success: false,
                        message: "I can only pass along messages from family members I know. Once you're set up, you'll be able to leave messages.",
                    };
                }
                // Get the sponsored identity for caller info
                const { getSponsoredIdentity } = await import('../../../services/identity/sponsored-identity.js');
 ...
 */
function createCheckMessagesForMeTool(ctx) {
    return {
...
- "Any messages from mom?"`,
        ...
        execute: async (args) => {
            ...
            try {
                const { getPendingMessages, formatMessagesForDelivery, markMessagesDelivered } = await import('../../../services/family/family-messages.js');
                ...
                if (args.fromName && messages.length > 0) {
                    ...
                    messages = messages.filter((m) => m.fromName.toLowerCase().includes(nameLower) ||
 ...
 */
function createCoordinatedReminderTool(ctx) {
    return {
        ...
        execute: async (args) => {
            ...
            try {
                ...
                if (!callContext?.sponsoredIdentityId || !callContext?.sponsorUserId) {
                    return {
                        success: false,
                        message: "I can only set reminders from family members I know. Once you're set up, you'll be able to create reminders.",
                    };
                }
                // Get the sponsored identity for caller info
                const { getSponsoredIdentity } = await import('../../../services/identity/sponsored-identity.js');
...
// TOOL DEFINITIONS
// ============================================================================
export const leaveMessageToolDef = {
...
export const checkMessagesToolDef = {
...
export const coordinatedReminderToolDef = {
...
//# sourceMappingURL=leave-message-tool.js.map