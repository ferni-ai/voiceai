...
import { persistKeyMoment } from '../shared/persistence.js';
import { z } from 'zod';
...
// ============================================================================
const coachParentingChallengeDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ challenge, childAgeGroup, attempts, urgency }) => {
                ...
                if (attempts)
                    response += `**What you've tried:** ${attempts}\n`;
                response += `\n---\n\n`;
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ situation, childAge, currentApproach }) => {
                ...
                if (childAge)
                    response += `**Child's age:** ${childAge}\n`;
                if (currentApproach)
                    response += `**Current approach:** ${currentApproach}\n`;
                response += `\n---\n\n`;
...
const suggestAgeAppropriateActivityDef = {
        ...
        return llm.tool({
            ...
            execute: async ({ childName, milestone, date, reaction }, { ctx: toolCtx }) => {
                ...
                if (date)
                    response += `**When:** ${date}\n`;
                if (reaction)
                    response += `**Their reaction:** ${reaction}\n`;
                response += `\n---\n\n`;
...
const celebrateFamilyMomentDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ moment, whoWasInvolved, whyItMatters }) => {
                ...
                if (whoWasInvolved?.length)
                    response += `**Who was there:** ${whoWasInvolved.join(', ')}\n`;
                if (whyItMatters)
...
// ============================================================================
const supportFamilyTransitionDef = {
        ...
        return llm.tool({
            ...
            execute: async ({ transition, childrenAges, stage, customTransition }) => {
                ...
                if (childrenAges?.length)
                    response += `**Children's ages:** ${childrenAges.join(', ')}\n`;
                if (stage)
...
const navigateFamilyConflictDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ conflictType, parties, coreIssue }) => {
                ...
                if (parties?.length)
                    response += `**Involved:** ${parties.join(' & ')}\n`;
                if (coreIssue)
                    response += `**Core issue:** ${coreIssue}\n`;
                response += `\n---\n\n`;
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ topic, attendees, goal }) => {
                ...
                if (attendees?.length)
                    response += `**Attendees:** ${attendees.join(', ')}\n`;
                if (goal)
                    response += `**Goal:** ${goal}\n`;
                response += `\n---\n\n`;
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ situation, concerns }) => {
                ...
                response += `**Situation:** ${situation}\n`;
                if (concerns?.length)
                    response += `**Concerns:** ${concerns.join(', ')}\n`;
                response += `\n---\n\n`;
...
// ============================================================================
const createFamilyTraditionDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ occasion, values, existingTraditions }) => {
                ...
                if (values?.length)
                    response += `**Values to incorporate:** ${values.join(', ')}\n`;
                response += `\n---\n\n`;
...
import { leaveMessageToolDef, checkMessagesToolDef, coordinatedReminderToolDef, } from './leave-message-tool.js';
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('family', familyTools);
