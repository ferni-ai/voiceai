 ...
 */
function createShareWithFamilyTool(ctx: ToolContext): Tool {
  return {
      ...
      try {
        ...
        if (!context) {
          return {
            success: false,
            message:
              "I couldn't share that message - it might contain something too personal to pass along. Want to try rephrasing it?",
          };
        }
 ...
 */
function createRequestCheckInTool(ctx: ToolContext): Tool {
  return {
...
export const shareWithFamilyToolDef: ToolDefinition = {
  id: 'shareWithFamily',
