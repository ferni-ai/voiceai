...
// ============================================================================
const recallVisualMemoryDef = {
    id: 'recallVisualMemory',
    name: 'Recall Visual Memory',
    description: 'Search for photos or images the user has shared based on a description. Better than human memory.',
    domain: 'memory',
            ...
            execute: async ({ query, limit }) => {
                try {
                    ...
                    const descriptions = results
                        .map((r, i) => `${i + 1}. ${r.memory.aiDescription || r.memory.detectedLabels?.slice(0, 3).join(', ') || 'An image'}${r.memory.userDescription ? ` - "${r.memory.userDescription}"` : ''}`)
                        .join('\n');
...
// ============================================================================
const describeSharedPhotoDef = {
    id: 'describeSharedPhoto',
    name: 'Describe Shared Photo',
    description: 'Get a detailed description of a photo the user has shared. Uses AI vision analysis.',
    domain: 'memory',
                ...
                try {
                    ...
                    if (memory.aiDescription) {
                        ...
                        if (memory.visionAnalysis.labels?.length) {
                            parts.push(`I see: ${memory.visionAnalysis.labels
                                .slice(0, 5)
                                .map((l) => l.name)
                                .join(', ')}`);
                        }
                        if (memory.visionAnalysis.dominantColors?.length) {
                            parts.push(`The dominant colors are ${memory.visionAnalysis.dominantColors
                                .slice(0, 3)
                                .map((c) => c.hex)
                                .join(', ')}`);
                        }
...
// ============================================================================
const recallEmotionalContextDef = {
        ...
        return llm.tool({
            ...
            execute: async ({ query }) => {
                try {
                    ...
                    const emotions = [];
                    if (memory.detectedLabels?.some((l) => ['smile', 'happy', 'celebration'].includes(l.toLowerCase()))) {
                        emotions.push('joy');
                    }
                    if (memory.detectedLabels?.some((l) => ['family', 'people', 'group'].includes(l.toLowerCase()))) {
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('memory', // Part of memory domain
visualMemoryTools);
