...
const findVolunteerOpportunityDef: ToolDefinition = {
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ interests, skills, timeAvailable, preference }) => {
        ...
        if (interests?.length) response += `**Interests:** ${interests.join(', ')}\n`;
        if (skills?.length) response += `**Skills:** ${skills.join(', ')}\n`;
        if (timeAvailable) response += `**Time:** ${timeAvailable}\n`;
        if (preference) response += `**Preference:** ${preference}\n`;
        response += `\n---\n\n`;
...
const trackVolunteerHoursDef: ToolDefinition = {
...
const planCharitableGivingDef: ToolDefinition = {
    ...
    return llm.tool({
      ...
      execute: async ({ currentGiving, budget, interests, approach }) => {
        ...
        if (approach && approach !== 'exploring') {
          ...
          if (fw) response += `**Approach:** ${fw.name}\n`;
        }
        response += `\n---\n\n`;
...
const alignGivingWithValuesDef: ToolDefinition = {
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ values, currentGiving }) => {
        ...
        if (currentGiving) response += `**Current giving:** ${currentGiving}\n`;
        response += `\n---\n\n`;
...
const findCommunityGroupDef: ToolDefinition = {
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ interests, goal, format }) => {
        ...
        if (format) response += `**Format:** ${format}\n`;
        response += `\n---\n\n`;
...
const engageCivicallyDef: ToolDefinition = {
  id: 'engageCivically',
...
const trackImpactDef: ToolDefinition = {
  id: 'trackImpact',
  name: 'Track Impact',
...
const reflectOnContributionDef: ToolDefinition = {
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ contribution }) => {
        ...
        if (contribution) response += `_Focusing on: ${contribution}_\n\n`;
        response += `---\n\n`;
