...
function routineDataToRoutine(data, userId) {
    return {
        id: data.id,
        userId,
        name: data.name,
...
function routineToRoutineData(routine) {
    return {
        id: routine.id,
        name: routine.name,
        type: routine.type,
...
function completionToCompletionData(completion) {
    return {
        id: completion.id,
...
// ============================================================================
const routinesCache = new Map();
const completionsCache = new Map();
const loadedUsers = new Set();
async function ensureUserRoutinesLoaded(userId) {
...
function persistRoutine(userId, routine) {
    try {
        const store = getProductivityStore();
...
function persistCompletion(userId, completion) {
    try {
        const store = getProductivityStore();
...
function getRoutineCompletions(routineId, days = 30) {
...
function getTodayCompletion(routineId) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return (Array.from(completionsCache.values()).find((c) => {
        if (c.routineId !== routineId)
            return false;
        const compDate = new Date(c.date);
        compDate.setHours(0, 0, 0, 0);
...
export function createRoutine(params) {
    ...
    getLogger().info({ routineId: routine.id, name: routine.name, steps: steps.length }, '🌅 Routine created');
    return routine;
}
...
export function completeRoutineStep(routineId, stepId, userId) {
    const routine = routinesCache.get(routineId);
    if (!routine)
        return null;
    ...
    const completedRequired = requiredSteps.filter((s) => completion.completedSteps.includes(s.id)).length;
...
export function skipRoutineStep(routineId, stepId) {
    const routine = routinesCache.get(routineId);
    if (!routine)
        return null;
    const orderedSteps = [...routine.steps].sort((a, b) => a.order - b.order);
    ...
    return {
        ...
        startRoutine: llm.tool({
            ...
            execute: async ({ routineType, routineName }, { ctx }) => {
                ...
                if (routineName) {
                    userRoutines = userRoutines.filter((r) => r.name.toLowerCase().includes(routineName.toLowerCase()));
                }
                else if (routineType) {
                ...
                if (!result)
                    return `Had trouble starting that routine. Give it another try?`;
        ...
        routineStepDone: llm.tool({
            ...
            execute: async ({ routineName, stepTitle }, { ctx }) => {
                ...
                if (!result)
                    return `Couldn't mark that step done. Try again?`;
                ...
                if (result.nextStep) {
                    ...
                    const remaining = orderedSteps.filter((s) => !result.completion?.completedSteps.includes(s.id) && s.id !== result.nextStep.id).length;
        ...
        skipRoutineStep: llm.tool({
            ...
            execute: async ({ routineName }, { ctx }) => {
                ...
                const orderedSteps = [...routine.steps].sort((a, b) => a.order - b.order);
                // Find current step
                const currentStep = orderedSteps.find((s) => !todayCompletion?.completedSteps.includes(s.id));
                if (!currentStep) {
