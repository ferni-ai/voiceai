...
function noteDataToNote(data, userId) {
    return {
        id: data.id,
        userId,
        type: data.type,
...
function journalDataToJournal(data, userId) {
    return {
        id: data.id,
        userId,
...
function journalToJournalData(journal) {
    return {
        id: journal.id,
...
async function ensureUserNotesLoaded(userId) {
...
function persistNote(userId, note) {
    try {
        const store = getProductivityStore();
...
function persistJournal(userId, journal) {
    try {
        const store = getProductivityStore();
...
// HELPER FUNCTIONS
// ============================================================================
function getUserNotes(userId, type) {
    return Array.from(notesCache.values())
        .filter((n) => n.userId === userId && (!type || n.type === type))
        .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
}
function searchUserNotes(userId, query) {
    const lower = query.toLowerCase();
    return getUserNotes(userId).filter((n) => n.content.toLowerCase().includes(lower) ||
        n.title?.toLowerCase().includes(lower) ||
        n.tags.some((t) => t.toLowerCase().includes(lower)));
}
function getTodayJournal(userId) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return (Array.from(journalsCache.values()).find((j) => {
        if (j.userId !== userId)
            return false;
...
function getRecentJournals(userId, days = 7) {
...
function getJournalStreak(userId) {
...
export function createNote(params) {
    ...
    getLogger().info({ noteId: note.id, type: note.type, length: note.content.length }, '📝 Note created');
    return note;
}
export function createJournalEntry(params) {
    ...
    if (existing) {
        // Update existing
        if (params.gratitudes)
            existing.gratitudes = params.gratitudes;
        if (params.highlight)
            existing.highlight = params.highlight;
        if (params.challenge)
            existing.challenge = params.challenge;
        if (params.learnings)
            existing.learnings = params.learnings;
        if (params.tomorrowIntention)
            existing.tomorrowIntention = params.tomorrowIntention;
        if (params.mood)
            existing.mood = params.mood;
        if (params.notes)
    ...
    return {
        ...
        startJournal: llm.tool({
            ...
            execute: async ({ type }, { ctx }) => {
                ...
                const prompts = JOURNAL_PROMPTS[type === 'evening' ? 'reflection' : type === 'morning' ? 'morning' : 'gratitude'];
        ...
        completeJournal: llm.tool({
            ...
            execute: async ({ highlight, challenge, learnings, tomorrowIntention, mood }, { ctx }) => {
                ...
                if (learnings)
                    response += `📚 **Learned:** ${learnings}\n`;
                if (tomorrowIntention)
                    response += `🎯 **Tomorrow:** ${tomorrowIntention}\n`;
                const streak = getJournalStreak(userId);
