...
import { createReminder, getPendingReminders, cancelReminder, parseNaturalTime, } from '../../../services/scheduling/reminder-scheduler.js';
const log = getLogger();
// ============================================================================
// SET REMINDER TOOL
// ============================================================================
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                message: z
                    .string()
                    .describe('What to remind the user about (e.g., "call mom", "take medication", "check the oven")'),
                when: z
                    .string()
                    .describe('When to send the reminder in natural language (e.g., "in 30 minutes", "tomorrow at 2pm", "next Monday")'),
                deliveryMethod: z
                    .enum(['sms', 'email', 'voice_message'])
                    .optional()
                    .describe('How to deliver the reminder. Defaults to voice_message (will remind in next conversation)'),
                deliveryAddress: z
                    .string()
                    .optional()
            ...
            execute: async (params) => {
                ...
                try {
                    ...
                    const methodStr = deliveryMethod === 'voice_message'
                        ? "I'll remind you"
                        : deliveryMethod === 'sms'
                            ? "I'll text you"
                            : "I'll email you";
                    log.info({ reminderId: reminder.id, scheduledFor: scheduledFor.toISOString() }, 'Reminder created');
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                reminderQuery: z
                    .string()
                    .describe('The reminder to cancel - either the message content or number from the list (e.g., "call mom" or "1")'),
            }),
            execute: async (params) => {
...
// EXPORTS
// ============================================================================
export const reminderTools = [setReminderDef, getRemindersDef, cancelReminderDef];
