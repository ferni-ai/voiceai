...
function taskToTaskData(task: Task): TaskData {
  return {
    id: task.id,
    title: task.title,
...
function persistTask(userId: string, task: Task): void {
  try {
    const store = getProductivityStore();
...
function parseNaturalDueDate(expression: string): Date | null {
  const now = new Date();
...
function formatTaskForSpeech(task: Task): string {
...
function createNextRecurringInstance(task: Task): Task | null {
...
export function updateTask(
  ...
  if (updates.title) task.title = sanitizePlainText(updates.title, 200);
...
export function createTaskTools() {
  return {
    addTask: llm.tool({
      ...
      execute: async (
        { title, dueDate, priority, category, isRecurring, recurrencePattern },
        { ctx }
      ) => {
