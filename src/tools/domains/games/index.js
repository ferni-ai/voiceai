 ...
 */
function transformStateForFrontend(gameType, gameData) {
 ...
 */
async function broadcastGameStarted(gameType, gameData) {
    try {
        ...
        if (publisher.isConnected()) {
            ...
            await publisher.sendGameState(normalizedType, 'active', frontendState);
 ...
 */
async function broadcastGameState(gameType, status, gameData) {
    try {
        const { getFrontendPublisher } = await import('../../../agents/realtime/frontend-publisher.js');
        const publisher = getFrontendPublisher();
        if (publisher.isConnected()) {
            ...
            const frontendState = transformStateForFrontend(normalizedType, gameData);
            await publisher.sendGameState(normalizedType, status, frontendState);
 ...
 */
async function broadcastGameEnded(gameType, result) {
    try {
        const { getFrontendPublisher } = await import('../../../agents/realtime/frontend-publisher.js');
        const publisher = getFrontendPublisher();
        if (publisher.isConnected()) {
...
// ============================================================================
function createGameToolDefinitions() {
    return [
        // ========================================
        // GAME MANAGEMENT
        // ========================================
            ...
            create: (ctx) => llm.tool({
                ...
                execute: async ({ gameType, rounds }) => {
                    try {
                        ...
                        const welcomeMessage = await gameEngine.startGame(gameType, config);
                        // Broadcast game started to frontend
                        const state = gameEngine.getState();
                        await broadcastGameStarted(gameType, { ...state.gameData });
            ...
            create: (ctx) => llm.tool({
                ...
                execute: async ({ answer }) => {
                    ...
                    try {
                        ...
                        await broadcastGameState(gameType, status, { ...state.gameData });
            ...
            create: (ctx) => llm.tool({
                ...
                execute: async ({ gameType, userGoesFirst, difficulty }) => {
                    try {
                        ...
                        const config = {};
                        if (userGoesFirst !== undefined)
                            config.userGoesFirst = userGoesFirst;
                        if (difficulty)
                            config.difficulty = difficulty;
                        const result = await textGameEngine.startGame(gameType, config);
...
// DOMAIN EXPORT
// ============================================================================
const gamesTools = createGameToolDefinitions();
