/**
 * Peter's Superhuman Analytics Tools Tests
 *
 * Tests the "Better Than Human" analytics capabilities unique to Peter.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';

// Mock services BEFORE importing the module
vi.mock('../../../../services/superhuman/peter-analytics-services.js', () => ({
...
describe('peter-analytics domain', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should export domain as peter-analytics', () => {
    expect(domain).toBe('peter-analytics');
  });
  ...
  it('should have all tools in peter-analytics domain', () => {
    for (const tool of definitions) {
      expect(tool.domain).toBe('peter-analytics');
    }
  });

  it('should have peter-specialty tag on all tools', () => {
    for (const tool of definitions) {
      expect(tool.tags).toContain('peter-specialty');
    }
  });
