...
async function aggregateWeeklySummary(userId: string): Promise<WeeklySummaryData | null> {
  try {
    ...
    // Calculate metrics
    const totalRituals = (streaks || []).reduce(
      (sum, s) => sum + ((s as { totalCompletions?: number }).totalCompletions || 0),
      0
    );

    const currentStreak = Math.max(
      ...(streaks || []).map((s) => (s as { currentStreak?: number }).currentStreak || 0),
      0
    );

    const longestStreak = Math.max(
      ...(streaks || []).map((s) => (s as { longestStreak?: number }).longestStreak || 0),
      0
    );

    // Calculate completion rate (rituals completed this week vs expected)
    const activeRituals = (profile as { activeRituals?: string[] })?.activeRituals?.length || 0;
    const expectedWeeklyCompletions = activeRituals * 7;
    const weeklyCompletions = (streaks || []).filter((s) => {
      const lastCompleted = (s as { lastCompletedAt?: string }).lastCompletedAt;
    ...
    const ritualCompletionRate =
      expectedWeeklyCompletions > 0 ? (weeklyCompletions / expectedWeeklyCompletions) * 100 : 0;
    ...
    const moodScores = (weatherHistory || []).map((w) => {
      const weather = (w as { weather?: { primary?: string } | string }).weather;
      const primary = typeof weather === 'object' ? weather?.primary : weather;
      return moodMap[primary as string] || 3;
    });
    ...
    let moodTrend: WeeklySummaryData['moodTrend'] = 'unknown';
    ...
    let moodHighlight: string | null = null;
    if (averageMood >= 4) {
    ...
    const sortedStreaks = [...(streaks || [])].sort(
      (a, b) =>
        ((b as { longestStreak?: number }).longestStreak || 0) -
        ((a as { longestStreak?: number }).longestStreak || 0)
    );
    const topRitual = sortedStreaks[0] as { ritualId?: string } | undefined;
 ...
 */
function formatRitualName(ritualId: string | null): string | null {
 ...
 */
function generateNoDataResponse(personaId: string): string {
  switch (personaId) {
    case 'maya':
...
const getWeeklySummaryDef: ToolDefinition = {
  id: 'getWeeklySummary',
  name: 'Get Weekly Summary',
  description:
    'Provides a conversational summary of how the user is doing - habits, mood, streaks, wins, and areas of attention.',
  domain: 'insights',
