    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ subject, motivation, currentLevel, targetOutcome, timeframe }) => {
                ...
                if (motivation)
                    response += `**Why:** ${motivation}\n`;
                if (currentLevel)
                    response += `**Current level:** ${currentLevel}\n`;
                if (targetOutcome)
                    response += `**Success looks like:** ${targetOutcome}\n`;
                if (timeframe)
...
const trackLearningProgressDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ subject, update, hoursLogged, milestoneReached }) => {
                ...
                if (hoursLogged)
                    response += `**Hours logged:** ${hoursLogged}\n`;
                if (milestoneReached)
                    response += `**Milestone reached:** ${milestoneReached} 🎉\n`;
                response += `\n---\n\n`;
                if (milestoneReached) {
...
const scheduleSpacedRepetitionDef = {
...
// ============================================================================
const overcomeLearningBlockDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ block, subject }) => {
                ...
                if (subject)
                    response += `_While learning: ${subject}_\n`;
                response += `\n---\n\n`;
                const advice = {
                    procrastination: `**Procrastination usually isn't laziness - it's emotion management.**\n\n` +
                        `**Strategies:**\n` +
                    ...
                    overwhelm: `**Overwhelm means too much, not too hard.**\n\n` +
                        `**Strategies:**\n` +
                    ...
                    confusion: `**Confusion is part of learning - don't run from it.**\n\n` +
                        `**Strategies:**\n` +
                    ...
                    boredom: `**Boredom signals a mismatch - too easy, too hard, or wrong approach.**\n\n` +
                        `**Strategies:**\n` +
                    ...
                    'lack-of-time': `**Time is made, not found.**\n\n` +
                        `**Strategies:**\n` +
                    ...
                    'imposter-syndrome': `**Imposter syndrome often hits the most capable learners.**\n\n` +
                        `**Reframes:**\n` +
                    ...
                    plateau: `**Plateaus are normal - they often precede breakthroughs.**\n\n` +
                        `**Strategies:**\n` +
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('learning', learningTools);
