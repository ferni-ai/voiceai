...
import { getWorkflowEngine, WORKFLOW_TEMPLATES, } from '../../../services/workflows/workflow-engine.js';
            ...
            create: (ctx) => {
                return llm.tool({
                    ...
                    execute: async (params) => {
                        ...
                        const data = await getWorkflowData(userId);
                        const workflow = data.workflows.find((w) => w.name.toLowerCase().includes(params.name.toLowerCase()));
                        if (!workflow) {
...
function describeTrigger(trigger) {
    switch (trigger.type) {
        ...
        case 'phrase':
            const phraseTrigger = trigger;
            return `When you say "${phraseTrigger.phrases[0]}"`;
        case 'event':
            return `When ${trigger.eventType || 'event'} happens`;
        case 'location':
