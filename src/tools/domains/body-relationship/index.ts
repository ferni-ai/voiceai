...
const bodyImageExploreDef: ToolDefinition = {
  id: 'bodyImageExplore',
...
const bodyNeutralityDef: ToolDefinition = {
  id: 'bodyNeutrality',
...
const bodyGratitudeDef: ToolDefinition = {
  id: 'bodyGratitude',
  name: 'Body Gratitude',
...
const joyfulMovementDef: ToolDefinition = {
  id: 'joyfulMovement',
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ currentRelationship }) => {
        ...
        const relationshipResponses: Record<string, string> = {
          'hate-exercise':
            "If you hate exercise, you probably haven't found YOUR movement yet. Gyms aren't for everyone. Running isn't for everyone. But moving your body in some way? There's something for everyone.",
          'do-to-burn':
            "If you're exercising to \"burn off\" food, that's a transactional relationship with your body. Movement becomes more joyful when it's for how you FEEL, not numbers.",
          'used-to-enjoy':
            "You can find that joy again. What did you love about movement before? The feeling? The community? The freedom? Let's reconnect to that.",
          'never-liked':
            "Maybe you've never been shown movement that fits YOU. School PE traumatized a lot of us. Let's explore what movement could feel good in your body.",
          injured:
            'Movement looks different with injury/disability. Adapted movement is still movement. What CAN your body do? Start there with compassion.',
        };
...
const innerCriticBodyDef: ToolDefinition = {
  id: 'innerCriticBody',
...
const emotionalEatingDef: ToolDefinition = {
  id: 'emotionalEating',
...
const mirrorWorkDef: ToolDefinition = {
  id: 'mirrorWork',
  name: 'Mirror Work',
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ currentReaction }) => {
        ...
        // Address reaction
        const reactionResponses: Record<string, string> = {
          avoid:
            "Avoiding mirrors suggests the reflection feels threatening. We can gently rebuild that relationship. You don't have to love what you see - just be able to look.",
          criticize:
            "If looking in the mirror triggers criticism, you've been trained to be your own bully. We can untrain that response.",
          obsess:
            'Obsessive mirror checking often comes from anxiety about appearance. The reassurance never lasts. We can work on tolerance instead.',
          neutral: 'Neutral is great! Mirror work can deepen that into compassion.',
          depends:
            'It making sense that it depends. Some days are harder. We can build resilience for the hard days.',
        };
...
const bodyCheckingDef: ToolDefinition = {
...
const bodyRelationshipTools: ToolDefinition[] = [
  bodyImageExploreDef,
  bodyNeutralityDef,
  bodyGratitudeDef,
  joyfulMovementDef,
