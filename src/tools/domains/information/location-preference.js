 ...
 */
export function setCurrentActiveSession(userId, location, sessionId) {
    currentActiveSession = { userId, location, sessionId };
    log.info({ userId, hasLocation: !!location, sessionId }, '📍 Current active session set (for native tools)');
}
/**
 * Clear the current active session.
 * Called when a voice session ends.
 */
export function clearCurrentActiveSession() {
 ...
 */
export function getCurrentSessionLocation() {
    ...
    if (currentActiveSession.location) {
        log.debug({ location: currentActiveSession.location, source: 'active-session' }, '📍 Using active session location');
 ...
 */
export function setSessionLocation(userId, city, regionCode, _countryCode) {
    if (!city) {
    ...
    if (existing?.source === 'explicit') {
        log.debug({ userId, existing: existing.location, new: location }, '📍 Keeping explicit location');
        return;
    }
 ...
 */
export function getUserLocationPreference(userId) {
    // Check in-memory cache
    const cached = locationCache.get(userId);
    if (cached && Date.now() - cached.timestamp < CACHE_TTL_MS) {
        log.debug({ userId, location: cached.location, source: cached.source }, '📍 Using cached location');
 ...
 */
export function setUserLocationPreference(userId, location) {
 ...
 */
export function clearLocationCache(userId) {
    if (userId) {
        locationCache.delete(userId);
...
//# sourceMappingURL=location-preference.js.map