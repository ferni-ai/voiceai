...
import { getUserPreferences, addFavoriteTeam, removeFavoriteTeam, getFavoriteTeams, addToWatchlist, removeFromWatchlist, getWatchlist, saveLocation, setAllergies, addNewsInterest, addAvoidTopic, updateUserPreferences, } from './storage.js';
...
// TOOL DEFINITIONS
// ============================================================================
export const preferencesToolDefinitions = [
    // ────────────────────────────────────────────────────────────────────────
    // FAVORITE TEAMS
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Add a team to the user\'s favorites. Use when user says "I\'m an Eagles fan", "The Phillies are my team", or "Remember I follow the Lakers". Enables zero-param team updates later.',
            parameters: z.object({
            ...
            execute: async ({ teamName, league, isPrimary }) => {
                ...
                log.info({ userId: ctx.userId, team: teamName, league: normalizedLeague }, '🏈 Added favorite team');
    ...
    },
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Remove a team from favorites. Use when user says "I don\'t follow the Jets anymore" or "Remove the Lakers from my favorites".',
            parameters: z.object({
    ...
    },
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Get scores for ALL the user\'s favorite teams. Use when user asks "how are my teams doing?", "any sports updates?", or "did my teams win?". NO PARAMETERS NEEDED - uses saved favorites.',
            parameters: z.object({}),
            execute: async () => {
                if (!ctx.userId) {
                ...
                log.info({ userId: ctx.userId, teamCount: favorites.length }, '🏈 Getting favorite teams scores');
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Add a stock to the user\'s watchlist. Use when user says "I want to follow Apple stock", "add Tesla to my watchlist", or "remind me about NVDA".',
            parameters: z.object({
    ...
    },
    {
        ...
        tags: ['information', 'finance', 'stocks', 'preferences'],
        create: (ctx) => llm.tool({
            description: 'Remove a stock from watchlist. Use when user says "remove Apple from my watchlist" or "I don\'t care about Tesla anymore".',
            parameters: z.object({
    ...
    },
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Get updates for all stocks on the user\'s watchlist. Use when user asks "how\'s my portfolio?", "any stock news?", or "how are my stocks doing?". NO PARAMETERS NEEDED.',
            parameters: z.object({}),
            execute: async () => {
                if (!ctx.userId) {
                ...
                log.info({ userId: ctx.userId, stockCount: watchlist.length }, '📈 Getting watchlist updates');
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Save a location for personalized updates. Use when user says "my home is in Philadelphia", "I work in Center City", or "save my gym location". Enables zero-param commute checks.',
            parameters: z.object({
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Save user\'s allergy information. Use when user mentions "I have pollen allergies", "I\'m allergic to grass", or asks for allergy-aware weather. Enables proactive pollen alerts.',
            parameters: z.object({
                allergies: z
                    .array(z.string())
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Set news preferences. Use when user says "I want more tech news", "less politics please", or "I\'m interested in climate news". Personalizes news delivery.',
            parameters: z.object({
            ...
            execute: async ({ interests, avoid, frequency }) => {
                ...
                log.info({ userId: ctx.userId, interests, avoid, frequency }, '📰 Updated news preferences');
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        create: (ctx) => llm.tool({
            description: "Get all the user's learned preferences - sports teams, stocks, news interests, locations, allergies. " +
            ...
            execute: async () => {
                ...
                if (preferences.favoriteTeams && preferences.favoriteTeams.length > 0) {
                    const teams = preferences.favoriteTeams.map((t) => t.league !== 'Unknown' ? `${t.name} (${t.league})` : t.name);
 ...
 */
export function getPreferencesToolDefinitions() {
