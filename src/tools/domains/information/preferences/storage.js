 ...
 */
function getPreferencesPath(userId) {
 ...
 */
export async function updateUserPreferences(userId, update) {
    try {
 ...
 * Add a favorite team
 */
export async function addFavoriteTeam(userId, team) {
...
export async function removeFavoriteTeam(userId, teamName) {
    const prefs = await getUserPreferences(userId);
    const updated = prefs.favoriteTeams.filter((t) => t.name.toLowerCase() !== teamName.toLowerCase());
 ...
 */
export async function getSavedLocation(userId, name) {
    const prefs = await getUserPreferences(userId);
    const normalized = name.toLowerCase();
    if (normalized === 'home')
        return prefs.homeLocation || null;
    if (normalized === 'work')
        return prefs.workLocation || null;
    return prefs.savedLocations.find((l) => l.name.toLowerCase() === normalized) || null;
}
/**
 * Add a news interest
 */
export async function addNewsInterest(userId, topic) {
    const prefs = await getUserPreferences(userId);
 ...
 */
export async function addAvoidTopic(userId, topic) {
    const prefs = await getUserPreferences(userId);
    const normalized = topic.toLowerCase();
 ...
 */
export async function setAllergies(userId, allergies, mode = 'replace') {
    if (mode === 'add') {
        const prefs = await getUserPreferences(userId);
