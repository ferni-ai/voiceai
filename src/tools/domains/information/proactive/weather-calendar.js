 ...
 */
function parseWeatherConditions(weatherText) {
 ...
 */
export async function checkWeatherCalendarConflicts(context) {
    const alerts = [];
    ...
    try {
        ...
        for (const event of calendarEvents) {
            const eventAlerts = await checkEventWeatherConflict(event, currentConditions, forecast, locationStr, userId);
 ...
 */
async function checkEventWeatherConflict(event, currentConditions, forecast, location, userId) {
    const alerts = [];
    const now = new Date();
    ...
    if (forecastConditions.hasRain && isLikelyOutdoor) {
        alerts.push(createWeatherAlert(userId, 'rain_during_event', event, `Heads up - rain is expected around ${formatEventTime(eventStart)} during your ${event.title}. Want to move it indoors or reschedule?`, forecastConditions.hasSevereWeather ? 'high' : 'medium'));
    }
    if (forecastConditions.hasSevereWeather) {
        alerts.push(createWeatherAlert(userId, 'severe_weather', event, `Weather alert: Severe weather expected this afternoon. Your ${event.title} at ${formatEventTime(eventStart)} might be affected. Stay safe!`, 'high'));
    }
    if (forecastConditions.isHot && isLikelyOutdoor) {
        alerts.push(createWeatherAlert(userId, 'extreme_heat', event, `It's going to be hot (${forecastConditions.temperature}°F) during your ${event.title}. Stay hydrated and maybe find some shade!`, 'medium'));
    }
    if (forecastConditions.isCold && isLikelyOutdoor) {
        alerts.push(createWeatherAlert(userId, 'extreme_cold', event, `It'll be cold (${forecastConditions.temperature}°F) during your ${event.title}. Bundle up!`, 'low'));
    }
    return alerts;
}
/**
 ...
 */
function detectOutdoorEvent(event) {
    if (event.isOutdoor === true)
        return true;
    if (event.isOutdoor === false)
        return false;
 ...
 */
function createWeatherAlert(userId, type, event, message, urgency) {
    const now = new Date();
    const eventStart = new Date(event.startTime);
    return {
 ...
 */
export async function checkWeatherAlerts(context) {
    const alerts = [];
...
//# sourceMappingURL=weather-calendar.js.map