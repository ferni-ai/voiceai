...
function getPendingAlerts(userId) {
    ...
    return alerts.filter((a) => !a.acknowledged && new Date(a.expiresAt) > now && new Date(a.relevantAt) <= now);
}
/**
 * Acknowledge an alert
 */
function acknowledgeAlert(userId, alertId, wasHelpful) {
    const alerts = userAlerts.get(userId);
    if (!alerts)
        return false;
 ...
 */
function setUserAlertPreferences(userId, prefs) {
 ...
 */
export async function checkAllAlerts(context) {
...
// TOOL DEFINITIONS
// ============================================================================
export const proactiveToolDefinitions = [
    {
        id: 'getMyAlerts',
        name: 'Get My Alerts',
        ...
        create: (ctx) => llm.tool({
            description: 'Get any pending alerts for the user. Use when user asks "any alerts?", "what should I know?", or at the start of a session to check for important notifications.',
            parameters: z.object({}),
            execute: async () => {
                if (!ctx.userId) {
                ...
                const formatted = alerts.map((a) => {
                    const urgencyEmoji = a.urgency === 'critical' ? '🚨' : a.urgency === 'high' ? '⚠️' : 'ℹ️';
    ...
    },
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Mark an alert as acknowledged. Use when user says "got it", "thanks for letting me know", or dismisses an alert.',
            parameters: z.object({
    ...
    },
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Configure alert preferences. Use when user says "fewer alerts", "only important alerts", "no alerts after 10pm", or "I want weather alerts".',
            parameters: z.object({
                enabledCategories: z
                    .array(z.enum([
                    'weather',
    ...
    },
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Check for new proactive alerts based on current conditions. Use at session start or when user asks to check for updates.',
            parameters: z.object({}),
            execute: async () => {
                if (!ctx.userId) {
 ...
 */
export function getProactiveToolDefinitions() {
