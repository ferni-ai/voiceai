...
import { executeWithProgressiveFeedback, fetchWithPriority, toolCache, createSource, } from '../../execution/index.js';
 ...
 */
function detectHeadlineWeight(headline) {
 ...
 */
function formatNewsWithSSML(headlines, intro, isStale = false) {
    // Filter out undefined headlines
    const validHeadlines = headlines.filter((h) => h !== undefined && h !== null && h.trim().length > 0);
 ...
 */
function formatSingleNewsResult(content) {
 ...
 */
function createNewsSources(topic) {
    const sources = [];
    const encodedTopic = encodeURIComponent(topic);
    // Primary: NewsData.io (usually fast, good coverage)
    if (NEWSDATA_KEY) {
        sources.push(createSource('newsdata', 'NewsData.io', async () => {
    ...
    if (GNEWS_KEY) {
        sources.push(createSource('gnews', 'GNews', async () => {
    ...
    sources.push(createSource('duckduckgo', 'DuckDuckGo', async () => {
        const searchQuery = `${topic} news today`;
        const response = await fetch(`https://api.duckduckgo.com/?q=${encodeURIComponent(searchQuery)}&format=json&no_html=1&skip_disambig=1`, { signal: AbortSignal.timeout(5000) });
        if (!response.ok)
        ...
        // Try related topics
        return (data.RelatedTopics?.slice(0, 4)
            .map((t) => t.Text)
 ...
 */
export async function searchNewsByTopic(topic) {
    const logger = getLogger();
    const startTime = Date.now();
    const cacheKey = `topic:${topic}`;
    logger.info({ topic }, '🔍 [DIAG] searchNewsByTopic START (progressive)');
    // Check cache first for instant response
    const cached = toolCache.getWithStaleness('searchNewsByTopic', cacheKey);
    if (cached?.freshness === 'fresh') {
        logger.info({ topic, cacheAge: Date.now() - cached.timestamp }, '🔍 [DIAG] Returning fresh cache');
    ...
    try {
        ...
        const { results, sources: usedSources, latency, complete, } = await fetchWithPriority({
            sources,
            query: topic,
            minResults: 3,
            maxWait: 8000,
        ...
        if (results.length > 0) {
            ...
            logger.info({ topic, elapsed: latency, count: results.length, sources: usedSources, complete }, '🔍 [DIAG] searchNewsByTopic SUCCESS');
            return formatNewsWithSSML(results, `Here's what's happening with ${topic}`);
        }
        // No live results - try stale cache
        if (cached) {
            logger.info({ topic, freshness: cached.freshness, cacheAge: Date.now() - cached.timestamp }, '🔍 [DIAG] Returning stale cache as fallback');
            return formatNewsWithSSML(cached.data, `Here's what I found about ${topic}`, true);
        }
        // Final fallback: general news
        logger.info({ topic, elapsed: Date.now() - startTime }, '🔍 [DIAG] All sources failed, trying general news');
        ...
        logger.warn({ topic, error: String(error), elapsed: Date.now() - startTime }, '🔍 [DIAG] searchNewsByTopic FAILED');
 ...
 */
export async function getFinancialNews(category = 'general') {
    const startTime = Date.now();
    const logger = getLogger();
    ...
    // Check cache first
    const cached = toolCache.getWithStaleness('getFinancialNews', cacheKey);
    if (cached?.freshness === 'fresh') {
    ...
    const result = await executeWithProgressiveFeedback('getFinancialNews', async () => {
    ...
    if (result.success && result.data && result.data.length > 0) {
        ...
        logger.info({ elapsed: result.latency, count: result.data.length, source: result.source }, '📰 [DIAG] getFinancialNews SUCCESS');
        return formatNewsWithSSML(result.data, "Here's what's moving in the markets");
    }
    // Try stale cache as fallback
    if (cached) {
        logger.info({ freshness: cached.freshness, cacheAge: Date.now() - cached.timestamp }, '📰 [DIAG] Returning stale cache');
    ...
    logger.warn({ elapsed: Date.now() - startTime, error: result.error }, '📰 [DIAG] getFinancialNews FAILED');
...
export async function getGeneralNews() {
    ...
    // Check cache first
    const cached = toolCache.getWithStaleness('getGeneralNews', cacheKey);
    if (cached?.freshness === 'fresh') {
        logger.info({ cacheAge: Date.now() - cached.timestamp }, '📰 [DIAG] Returning fresh cache');
    ...
    try {
        ...
        if (results.length > 0) {
            ...
            logger.info({ elapsed: latency, count: results.length, sources: usedSources, complete }, '📰 [DIAG] getGeneralNews SUCCESS');
        ...
        logger.warn({ elapsed: Date.now() - startTime }, '📰 [DIAG] getGeneralNews: All sources failed');
        return "I couldn't fetch the latest news right now. Check back later.";
    }
    catch (error) {
        logger.warn({ error: String(error), elapsed: Date.now() - startTime }, '📰 [DIAG] getGeneralNews FAILED');
        // Return stale cache if available
        if (cached) {
            return formatNewsWithSSML(cached.data, "Here's the news", true);
        }
 ...
 */
function extractRSSHeadlines(xml, limit) {
    const headlines = [];
...
export function createNewsTools() {
    return {
        getFinancialNews: llm.tool({
            description: "EXECUTE SILENTLY to get financial news. DO NOT say 'let me check the news' - call and share the headlines naturally.",
            parameters: z.object({
                category: z
