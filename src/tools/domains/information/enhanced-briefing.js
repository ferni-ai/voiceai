...
// Preferences
import { getUserPreferences, getFavoriteTeams } from './preferences/storage.js';
// Proactive alerts
import { checkAllAlerts } from './proactive/index.js';
// Sports
...
function getRandomQuote(timeOfDay) {
 ...
 */
async function generateEnhancedBriefing(options) {
    const { userId, location, includeWeather = true, includeSports = true, includeAlerts = true, includeEnvironmental = true, } = options;
    const parts = [];
    ...
    if (includeAlerts) {
        try {
            ...
            const importantAlerts = alerts.filter((a) => a.urgency === 'high' || a.urgency === 'critical');
    ...
    try {
        ...
        if (birthdayInsights.length > 0) {
            ...
            const upcomingBirthdays = birthdayInsights.filter((i) => i.type === 'birthday_upcoming' && i.priority >= 70);
    ...
    if (timeOfDay === 'morning') {
        try {
            ...
            const urgentReminders = contactReminders.filter((i) => i.context?.urgency === 'urgent' || i.priority >= 70);
...
// TOOL DEFINITIONS
// ============================================================================
export const enhancedBriefingToolDefinitions = [
    {
        id: 'getSmartBriefing',
        name: 'Get Smart Briefing',
        description: 'Enhanced daily briefing with environmental health, proactive alerts, and personalized sports updates.',
        domain: 'information',
        tags: ['information', 'briefing', 'morning', 'personalized', 'better-than-human'],
        create: (ctx) => llm.tool({
            description: 'Get a personalized daily briefing that includes weather, environmental conditions (air quality, UV, pollen if notable), proactive alerts, and updates on favorite teams. Use when user asks "what should I know today?", "morning briefing", "give me the rundown", or at session start.',
            parameters: z.object({
    ...
    },
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Get an ultra-brief status. Use when user asks "anything I need to know?", "quick check", or wants just the highlights without a full briefing.',
            parameters: z.object({}),
            execute: async () => {
                if (!ctx.userId) {
    ...
    },
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Get an evening reflection. Use when user asks "how was today?", "end of day", or in evening conversations. Focuses on wins, tomorrow prep, and rest.',
            parameters: z.object({}),
            execute: async () => {
                const parts = [];
                // Evening greeting
                ...
                if (ctx.userLocation?.city) {
                    try {
                        ...
                        if (forecast && !forecast.includes("couldn't")) {
                            ...
                            parts.push(`Tomorrow's looking like: ${forecast.split('|')[0] || forecast.slice(0, 100)}`);
                        }
                    }
                    catch {
                        // Skip weather
