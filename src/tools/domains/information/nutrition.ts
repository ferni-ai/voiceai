...
export async function compareNutrition(food1: string, food2: string): Promise<string> {
  ...
  if (n1.protein !== n2.protein) {
    ...
    comparison.push(
      `${higher} has more protein (${Math.max(n1.protein, n2.protein)}g vs ${Math.min(n1.protein, n2.protein)}g)`
    );
  }
...
export function createNutritionTools() {
  ...
  return {
    getNutritionInfo: llm.tool({
      description:
        'Get nutritional information for a food including calories, protein, carbs, and fat. Use when user asks about nutrition, calories, or macros of a food.',
      parameters: z.object({
    ...
    compareNutrition: llm.tool({
      description:
        'Compare nutritional information between two foods. Use when user asks which food is healthier or wants to compare calories/macros.',
      parameters: z.object({
