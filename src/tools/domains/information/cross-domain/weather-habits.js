...
import { getUserHabits } from '../../habits/habits.js';
// ============================================================================
 ...
 */
async function getUserOutdoorHabits(userId) {
    try {
...
const log = getLogger();
function parseWeatherConditions(weatherResponse) {
    ...
    return {
        ...
        isNice: /nice|pleasant|beautiful|perfect|gorgeous|lovely/.test(lower) ||
...
function parseUVResponse(response) {
    const lower = response.toLowerCase();
 ...
 */
export async function getWeatherHabitInsights(location, userHabitsOrUserId = []) {
    ...
    if (typeof userHabitsOrUserId === 'string') {
        ...
        log.info({ location, userId, outdoorHabitCount: userHabits.length }, '🌤️→💪 Fetched user outdoor habits');
    }
    else {
        userHabits = userHabitsOrUserId;
    }
    log.info({ location, habitCount: userHabits.length }, '🌤️→💪 Analyzing weather-habit connections');
    const insights = [];
    try {
        // Fetch weather and environmental data in parallel
        const [weatherResponse, airQualityResponse, pollenResponse, uvResponse] = await Promise.allSettled([
        ...
        const weatherData = weatherResponse.status === 'fulfilled' ? parseWeatherConditions(weatherResponse.value) : null;
        ...
        const airQuality = airQualityResponse.status === 'fulfilled'
            ...
            : null;
        const pollenHigh = pollenResponse.status === 'fulfilled' ? parsePollenResponse(pollenResponse.value) : false;
        ...
        if (weatherData?.isRainy || weatherData?.isStormy) {
            ...
            if (mapping) {
                const affectedUserHabits = userHabits.filter((h) => mapping.affectedHabits.some((a) => h.toLowerCase().includes(a.replace('_', ' ')) || a.includes(h.toLowerCase())));
                insights.push({
                    ...
                    message: weatherData.isStormy
                        ...
                        : mapping.suggestion,
                    suggestion: affectedUserHabits.length > 0
        ...
        log.error({ location, error: String(error) }, '🌤️→💪 Failed to generate weather-habit insights');
    }
    return insights;
}
/**
 ...
 */
export async function getHabitRecommendation(location, habitName, habitType = 'any') {
    ...
    try {
        ...
        if (habitType === 'outdoor' && insights.length > 0) {
            const relevantInsight = insights.find((i) => i.connectionType === 'weather_habit' || i.connectionType === 'environmental_wellness');
            if (relevantInsight) {
...
export function createWeatherHabitsTools() {
    return {
        getWeatherHabitInsights: llm.tool({
            description: 'Analyze weather and environmental conditions to suggest habit adjustments. ' +
        ...
        getHabitRecommendation: llm.tool({
            description: 'Get a personalized recommendation for a specific habit based on current weather ' +
...
//# sourceMappingURL=weather-habits.js.map