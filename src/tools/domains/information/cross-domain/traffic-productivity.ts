...
// ============================================================================
// COMMUTE SUGGESTIONS DATABASE
// ============================================================================

interface SuggestionTemplate {
  type: CommuteSuggestion['type'];
  conditions: {
    minCommute?: number; // minutes
    maxCommute?: number;
    trafficSeverity?: TrafficProductivityContext['trafficSeverity'][];
...
function matchesSuggestion(
  ...
  context: TrafficProductivityContext & { timeOfDay: string; hasUpcomingMeeting: boolean }
): boolean {
  ...
  if (
    ...
    !conditions.timeOfDay.includes(context.timeOfDay as 'morning' | 'afternoon' | 'evening')
  ) {
    return false;
  }
  if (
    conditions.hasUpcomingMeeting !== undefined &&
 ...
 */
export function generateCommuteSuggestions(
  commuteTime: number,
  trafficSeverity: TrafficProductivityContext['trafficSeverity'],
  ...
  const matchedTemplates = SUGGESTION_TEMPLATES.filter((t) => matchesSuggestion(t, context)).sort(
 ...
 */
export async function getTrafficProductivityInsights(
  commuteTime: number,
  trafficSeverity: TrafficProductivityContext['trafficSeverity'],
  hasUpcomingMeeting = false
): Promise<CrossDomainInsight[]> {
  log.info(
    { commuteTime, trafficSeverity, hasUpcomingMeeting },
    '🚗→💼 Analyzing traffic-productivity connections'
  );
 ...
 */
export function formatCommuteSuggestions(suggestions: CommuteSuggestion[]): string {
  if (suggestions.length === 0) {
...
export function createTrafficProductivityTools() {
  return {
    getCommuteSuggestions: llm.tool({
      description:
        'Get personalized suggestions for making commute time productive or enjoyable. ' +
      ...
      execute: async ({ commuteTime, trafficSeverity, hasUpcomingMeeting }) => {
        const suggestions = generateCommuteSuggestions(
          commuteTime,
          trafficSeverity,
          hasUpcomingMeeting || false
        );
    ...
    getTrafficProductivityInsights: llm.tool({
      ...
      execute: async ({ commuteTime, trafficSeverity, hasUpcomingMeeting }) => {
        const insights = await getTrafficProductivityInsights(
          commuteTime,
          trafficSeverity,
          hasUpcomingMeeting || false
        );
