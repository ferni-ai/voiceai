 ...
 */
export function getCrossDomainToolDefinitions() {
    const tools = createCrossDomainTools();
    return [
        {
            id: 'getWeatherHabitInsights',
            name: 'Get Weather-Habit Insights',
            description: 'Analyze weather and environmental conditions to suggest habit adjustments. "Better than human" - suggests indoor workout when rainy, outdoor activities when nice.',
            domain: 'information',
            tags: ['cross-domain', 'weather', 'habits', 'insights'],
            create: (_ctx) => tools.getWeatherHabitInsights,
        },
        {
            id: 'getHabitRecommendation',
            name: 'Get Habit Recommendation',
            description: 'Get a personalized recommendation for a specific habit based on current weather and environmental conditions.',
            domain: 'information',
            tags: ['cross-domain', 'weather', 'habits', 'recommendation'],
            create: (_ctx) => tools.getHabitRecommendation,
        },
        {
            id: 'analyzeNewsMoodImpact',
            name: 'Analyze News Mood Impact',
            description: 'Analyze news headlines for emotional impact and determine the best way to deliver them based on content heaviness and user mood. "Better than human" - protects user from overwhelming news.',
            domain: 'information',
            tags: ['cross-domain', 'news', 'mood', 'emotional'],
            create: (_ctx) => tools.analyzeNewsMoodImpact,
        },
        {
            id: 'getPositiveNewsOnly',
            name: 'Get Positive News Only',
            description: 'Filter news to show only positive, uplifting stories. Use when user needs a mood boost or wants to avoid heavy news.',
            domain: 'information',
            tags: ['cross-domain', 'news', 'positive', 'mood'],
            create: (_ctx) => tools.getPositiveNewsOnly,
        },
        {
            id: 'shouldSkipNews',
            name: 'Should Skip News',
            description: 'Determine if news should be skipped based on user mood and news content. Returns a recommendation and reason.',
            domain: 'information',
            tags: ['cross-domain', 'news', 'mood', 'recommendation'],
            create: (_ctx) => tools.shouldSkipNews,
        },
        {
            id: 'getCommuteSuggestions',
            name: 'Get Commute Suggestions',
            description: 'Get personalized suggestions for making commute time productive or enjoyable. Considers commute length, traffic severity, and upcoming meetings.',
            domain: 'information',
            tags: ['cross-domain', 'traffic', 'productivity', 'commute'],
            create: (_ctx) => tools.getCommuteSuggestions,
        },
        {
            ...
            create: (_ctx) => tools.getTrafficProductivityInsights,
        },
        {
            ...
            create: (_ctx) => tools.suggestPreMeetingPepTalk,
        },
    ];
}
