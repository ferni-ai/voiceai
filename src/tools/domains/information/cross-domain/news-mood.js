 ...
 */
function analyzeNewsContent(headlines) {
 ...
 */
export async function analyzeNewsMoodImpact(newsHeadlines, userMood) {
    log.info({ headlineCount: newsHeadlines.length, userMood: userMood?.currentMood }, '📰→😊 Analyzing news mood impact');
    const { heavyCount, positiveCount, heavyTopics, overallSentiment } = analyzeNewsContent(newsHeadlines);
 ...
 */
export function generateNewsMoodIntro(analysis, userMood) {
    const { recommendation, reason, overallSentiment, heavyTopics } = analysis;
    switch (recommendation) {
        case 'skip':
            return (`I was going to share the news, but ${reason} ` +
 ...
 */
export async function getNewsMoodInsights(newsHeadlines, userMood) {
    ...
    if (analysis.recommendation !== 'proceed') {
        insights.push({
            ...
            suggestion: analysis.recommendation === 'skip'
 ...
 */
export function filterPositiveNews(headlines) {
    return headlines.filter((headline) => {
        const lower = headline.toLowerCase();
        return (POSITIVE_TOPICS.some((topic) => lower.includes(topic)) &&
 ...
 */
export function generateUpliftingNewsSummary(positiveHeadlines) {
    if (positiveHeadlines.length === 0) {
        return 'Looking for some good news... Let me find something uplifting for you.';
    }
    return ("Here's some good news to brighten your day:\n" +
        positiveHeadlines
            .slice(0, 3)
            .map((h) => `• ${h}`)
            .join('\n'));
...
export function createNewsMoodTools() {
    return {
        analyzeNewsMoodImpact: llm.tool({
            description: 'Analyze news headlines for emotional impact and determine the best way to ' +
        ...
        getPositiveNewsOnly: llm.tool({
            description: 'Filter news to show only positive, uplifting stories. ' +
        ...
        shouldSkipNews: llm.tool({
            description: 'Determine if news should be skipped based on user mood and news content. ' +
...
//# sourceMappingURL=news-mood.js.map