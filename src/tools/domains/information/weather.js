...
import { geocodeLocation, geocodeWithGoogle, formatLocationName, } from './utils/geocoding.js';
 ...
 */
async function tryGoogleWeather(location) {
    const log = getLogger();
    ...
    try {
        ...
        const weatherStr = `Right now in ${locationName}: ${Math.round(temp || 0)}°F with ${condition.toLowerCase()}${feelsLikeStr}. ` +
...
export async function getCurrentWeather(location) {
    ...
    if (googleWeather) {
        log.info({ location, elapsed: Date.now() - startTime, source: 'GoogleWeather' }, '🌤️ Weather from Google');
    ...
    if (!geo) {
        log.warn({ location, elapsed: Date.now() - startTime }, '🌤️ [DIAG] getCurrentWeather: Geocoding FAILED for Open-Meteo');
    ...
    try {
        ...
        if (!response.ok) {
            log.warn({ location, status: response.status, elapsed: Date.now() - startTime }, '🌤️ [DIAG] Open-Meteo API returned non-OK');
        ...
        if (!current) {
            log.warn({ location, elapsed: Date.now() - startTime }, '🌤️ [DIAG] Open-Meteo returned no current data');
        ...
        return (`Right now in ${locationName}: ${Math.round(current.temperature_2m)}°F with ${condition}${feelsLike}. ` +
 ...
 */
export async function getSunriseSunset(location) {
    const log = getLogger();
...
export function createWeatherTools() {
     ...
     */
    function resolveLocation(argLocation) {
        ...
        if (sessionLocation) {
            logger.info({ argLocation, sessionLocation, source: 'session-fallback' }, '🌤️ Using session location as fallback');
        ...
        getWeather: llm.tool({
            ...
            parameters: z.object({
                location: z
                    .string()
                    .optional()
                    .describe('City name (e.g., "Philadelphia", "New York"). Optional - defaults to user\'s detected location.'),
            }),
            execute: async ({ location: argLocation }) => {
                ...
                try {
                    ...
                    const elapsed = Date.now() - startTime;
                    logger.info({ location, elapsed, resultLength: result.length }, '🌤️ Weather result returned');
                    return result;
                }
                catch (error) {
        ...
        getWeatherForecast: llm.tool({
            ...
            execute: async ({ location: argLocation, days = 5 }) => {
                ...
                try {
                    ...
                    const elapsed = Date.now() - startTime;
                    logger.info({ location, days, elapsed, resultLength: result.length }, '📅 Weather forecast returned');
                    return result;
                }
                catch (error) {
        ...
        getSunriseSunset: llm.tool({
            ...
            execute: async ({ location }) => {
                ...
                try {
                    ...
                    const elapsed = Date.now() - startTime;
                    logger.info({ location, elapsed, resultLength: result.length }, '🌅 Sunrise/sunset returned');
                    return result;
                }
                catch (error) {
