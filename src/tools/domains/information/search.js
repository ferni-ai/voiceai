...
export async function searchWeb(query) {
    try {
        const response = await fetch(`https://api.duckduckgo.com/?q=${encodeURIComponent(query)}&format=json&no_html=1&skip_disambig=1`, { signal: AbortSignal.timeout(10000) });
        if (!response.ok) {
 ...
 */
export async function searchRecipes(dish) {
    try {
        // Use DuckDuckGo to search for recipes
        const response = await fetch(`https://api.duckduckgo.com/?q=${encodeURIComponent(`${dish} recipe`)}&format=json&no_html=1&skip_disambig=1`, { signal: AbortSignal.timeout(10000) });
        if (!response.ok) {
        ...
        const recipeTopics = data.RelatedTopics?.filter((topic) => Boolean(topic.Text) &&
...
export function createSearchTools() {
    return {
        searchWeb: llm.tool({
            description: "EXECUTE SILENTLY to search web. DO NOT say 'let me search' - call and share results naturally.",
            parameters: z.object({
