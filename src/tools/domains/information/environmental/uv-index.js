...
import { SKIN_TYPE_BURN_TIMES } from './types.js';
const log = getLogger();
// ============================================================================
// CONSTANTS
// ============================================================================
const UV_MESSAGES = {
    ...
    very_high: 'Very high UV! Take full precautions: sunscreen SPF 30+, hat, sunglasses, and limit sun exposure.',
    extreme: 'Extreme UV levels! Avoid sun exposure between 10am-4pm if possible. Full protection essential.',
};
 ...
 */
function findPeakUVTime(hourly) {
 ...
 */
function getExposureLimit(category) {
    const limits = {
 ...
 */
function calculateBurnTime(uvIndex, skinType) {
 ...
 */
export async function getUVIndex(location, skinType) {
 ...
 */
function formatUVResponse(location, data, skinType) {
    const parts = [];
    // Main UV report
    ...
    if (skinType && data.uvIndex > 2) {
        ...
        parts.push(`With your ${formatSkinType(skinType)} skin, you could start burning in about ${burnTime} minutes without protection.`);
    }
 ...
 */
function getUVDescriptor(category) {
    const descriptors = {
        low: 'low',
        moderate: 'moderate',
        high: 'high',
 ...
 */
export async function getUVSummary(location) {
    const geo = await geocodeLocation(location);
    if (!geo)
        return null;
    const data = await fetchUVFromOpenMeteo(geo.latitude, geo.longitude);
 ...
 */
export async function isUVSafeForOutdoors(location, duration = 60, // minutes
skinType = 'medium') {
    const geo = await geocodeLocation(location);
    if (!geo) {
