...
const POLLEN_ADVICE = {
    ...
    very_high: 'Very high pollen! Allergy sufferers should stay indoors if possible. Keep windows closed.',
};
const POLLEN_TYPE_NAMES = {
    tree: 'tree pollen',
    grass: 'grass pollen',
 ...
 */
async function fetchPollenFromOpenMeteo(lat, lon) {
    try {
        ...
        const treePollen = Math.max(data.current.alder_pollen || 0, data.current.birch_pollen || 0, data.current.olive_pollen || 0);
 ...
 */
function getPollenLevel(count, type) {
 ...
 */
function getDominantPollenType(tree, grass, weed) {
    if (tree === 0 && grass === 0 && weed === 0) {
        return undefined;
    }
    if (tree >= grass && tree >= weed)
        return 'tree';
 ...
 */
export async function getPollenForecast(location, userAllergies) {
 ...
 */
function formatPollenResponse(location, data, userAllergies) {
    const parts = [];
    // Main pollen report
    ...
    if (userAllergies && userAllergies.length > 0 && data.allergyAlert) {
        const relevantAllergies = userAllergies.filter((a) => a.toLowerCase().includes('pollen') ||
            a.toLowerCase().includes('grass') ||
            a.toLowerCase().includes('tree') ||
            a.toLowerCase().includes('ragweed'));
        if (relevantAllergies.length > 0) {
            parts.push(`Since you've mentioned ${relevantAllergies[0]} allergies, you might want to take extra precautions today.`);
        }
    }
    return parts.join(' ');
}
 ...
 */
function getPollenDescriptor(level) {
    const descriptors = {
        none: 'very low',
        low: 'low',
        moderate: 'moderate',
 ...
 */
export async function getPollenSummary(location) {
    const geo = await geocodeLocation(location);
    if (!geo)
        return null;
    const data = await fetchPollenFromOpenMeteo(geo.latitude, geo.longitude);
 ...
 */
export async function isPollenSafeForAllergies(location, allergyTypes) {
    const geo = await geocodeLocation(location);
    if (!geo) {
