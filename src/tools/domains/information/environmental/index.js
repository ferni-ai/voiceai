...
// TOOL DEFINITIONS
// ============================================================================
export const environmentalToolDefinitions = [
    // ────────────────────────────────────────────────────────────────────────
    // AIR QUALITY
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        description: 'Get current air quality index (AQI) with health recommendations. Includes PM2.5, ozone, and other pollutants.',
        domain: 'information',
        tags: ['information', 'health', 'air-quality', 'environment', 'aqi'],
        create: (ctx) => llm.tool({
            description: "Get air quality for a location. Use when user asks about air quality, AQI, pollution, smog, or if it's safe to exercise outside. Returns AQI level, dominant pollutant, and health recommendations.",
            parameters: z.object({
                location: z
                    .string()
                    .optional()
                    .describe('City name (e.g., "Los Angeles", "Beijing"). Uses detected location if not provided.'),
            }),
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        id: 'getUVIndex',
        name: 'Get UV Index',
        description: 'Get UV index with sunscreen recommendations and estimated burn time based on skin type.',
        domain: 'information',
        tags: ['information', 'health', 'uv', 'sun', 'sunscreen', 'environment'],
        create: (ctx) => llm.tool({
            description: 'Get UV index for a location. Use when user asks about UV, sunburn risk, sunscreen needs, or mentions spending time in the sun. Can provide personalized burn time estimates based on skin type.',
            parameters: z.object({
                location: z
                    .string()
                    .optional()
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Get pollen forecast for a location. Use when user asks about pollen, allergies, hay fever, or mentions allergy symptoms. Returns levels for tree, grass, and weed pollen with health recommendations.',
            parameters: z.object({
                location: z
                    .string()
                    .optional()
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        description: 'Get comprehensive advice for outdoor activities combining weather, air quality, UV, and pollen.',
        domain: 'information',
        tags: ['information', 'health', 'outdoor', 'exercise', 'environment', 'activity'],
        create: (ctx) => llm.tool({
            description: 'Get comprehensive outdoor activity advice. Use when user asks if conditions are good for running, cycling, hiking, or general outdoor activities. Combines air quality, UV, and pollen into a single recommendation with precautions.',
            parameters: z.object({
                location: z
                    .string()
                    .optional()
            ...
            execute: async ({ location, activity, duration, hasAllergies, hasAsthma }) => {
                ...
                log.info({ location: effectiveLocation, activity, duration }, '🏃 Outdoor advice tool called');
                try {
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
        ...
        create: (ctx) => llm.tool({
            description: 'Quick check if it\'s good to exercise outside. Use when user asks "should I run outside?" or "is it good to exercise outdoors today?" Returns a simple yes/no with brief reason.',
            parameters: z.object({
                location: z
                    .string()
                    .optional()
            ...
            execute: async ({ location, hasAllergies = false, hasAsthma = false }) => {
                ...
                try {
                    const { recommended, reason } = await shouldExerciseOutside(effectiveLocation, hasAllergies, hasAsthma);
                    return reason;
                }
                catch (error) {
 ...
 */
export function getEnvironmentalToolDefinitions() {
    return environmentalToolDefinitions;
}
/**
 * Get brief environmental summary for morning briefing
 */
export async function getEnvironmentalBriefing(location) {
    const summaries = [];
    ...
    if (aqSummary)
        summaries.push(aqSummary);
    if (uvSummary)
        summaries.push(uvSummary);
    if (pollenSummary)
        summaries.push(pollenSummary);
