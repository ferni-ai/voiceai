...
const AQI_HEALTH_MESSAGES = {
    ...
    unhealthy_sensitive: 'Members of sensitive groups may experience health effects. Consider reducing prolonged outdoor exertion.',
    unhealthy: 'Everyone may begin to experience health effects. Sensitive groups should avoid prolonged outdoor exertion.',
    very_unhealthy: 'Health alert: everyone may experience more serious health effects. Avoid prolonged outdoor activities.',
 ...
 */
function formatAirQualityResponse(location, data) {
    const parts = [];
    // Main AQI report
 ...
 */
function getAQIDescriptor(category) {
    const descriptors = {
        good: 'excellent',
        moderate: 'moderate',
 ...
 */
export async function isAirQualitySafeForExercise(location, hasRespiratoryCondition = false) {
    const geo = await geocodeLocation(location);
    if (!geo) {
 ...
 */
export async function getAirQualitySummary(location) {
    const geo = await geocodeLocation(location);
    if (!geo)
        return null;
    const data = await fetchAirQualityFromOpenMeteo(geo.latitude, geo.longitude);
