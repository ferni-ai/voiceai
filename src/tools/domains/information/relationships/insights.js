...
import { DEFAULT_BIRTHDAY_REMINDER, TEAM_UPDATE_MESSAGES, INTEREST_GIFT_MAPPINGS, GENERIC_GIFT_SUGGESTIONS, } from './types.js';
import { getRelationships, getUpcomingBirthdays, getRelationshipsNeedingContact, getRelationshipsByTeam, } from './storage.js';
...
export async function getBirthdayInsights(userId) {
    ...
    for (const { relationship, daysUntil } of upcomingBirthdays) {
        ...
        if (daysUntil === 0) {
            // Birthday is today!
            message = formatBirthdayMessage(getRandomMessage(DEFAULT_BIRTHDAY_REMINDER.messageTemplates.today), relationship.name, 0);
            type = 'birthday_today';
            priority = 100;
        }
        else {
            // Birthday is upcoming
            message = formatBirthdayMessage(getRandomMessage(DEFAULT_BIRTHDAY_REMINDER.messageTemplates.upcoming), relationship.name, daysUntil);
        ...
        insights.push({
            ...
            message,
            suggestion: daysUntil <= 3 && relationship.interests.length > 0
 ...
 */
export async function getTeamInsights(userId, teamUpdates) {
 ...
 */
export async function checkFriendsTeamResults(userId) {
    ...
    const insights = [];
    const now = new Date();
    const checkedTeams = new Set(); // Avoid duplicate API calls
    for (const relationship of relationshipsWithTeams) {
        for (const teamName of relationship.favoriteTeams) {
            ...
            if (checkedTeams.has(teamName.toLowerCase()))
                continue;
 ...
 */
export async function getContactReminderInsights(userId) {
 ...
 */
export function generateGiftSuggestions(relationship, occasion, budget) {
    const suggestions = [];
    ...
    const filteredSuggestions = suggestions.filter((s) => !pastGifts.some((pg) => pg.includes(s.suggestion.toLowerCase())));
 ...
 */
export async function getGiftSuggestionInsight(userId, relationshipId, occasion) {
    const relationships = await getRelationships(userId);
 ...
 */
export async function getAllRelationshipInsights(userId) {
 ...
 */
export async function getTopRelationshipInsight(userId) {
