 ...
 */
async function initFirestore(): Promise<FirebaseFirestore.Firestore | null> {
  if (db) return db;
...
// In-memory fallback
const inMemoryRelationships = new Map<string, Map<string, Relationship>>();
 ...
 */
export async function findRelationshipByName(
  userId: string,
  name: string
): Promise<Relationship | null> {
  ...
  return (
    relationships.find(
      (r) => r.name.toLowerCase() === lowerName || r.nickname?.toLowerCase() === lowerName
    ) || null
  );
}
...
export async function saveRelationship(
  userId: string,
  relationship: Omit<Relationship, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }
 ...
 */
export async function deleteRelationship(userId: string, relationshipId: string): Promise<boolean> {
  const firestore = await initFirestore();
 ...
 * Add a gift record
 */
export async function addGiftRecord(
  userId: string,
  relationshipId: string,
  gift: Omit<GiftRecord, 'id'>
): Promise<void> {
  const relationship = await getRelationship(userId, relationshipId);
...
export async function getUpcomingBirthdays(
  userId: string,
  daysAhead = 30
): Promise<Array<{ relationship: Relationship; daysUntil: number }>> {
  const relationships = await getRelationships(userId);
  const today = new Date();

  const results: Array<{ relationship: Relationship; daysUntil: number }> = [];
 ...
 */
export async function getRelationshipsNeedingContact(userId: string): Promise<
  Array<{
...
> {
  ...
  for (const rel of relationships) {
    if (!rel.lastContact || !rel.targetContactFrequency) continue;

    const daysSince = Math.floor(
      (today.getTime() - rel.lastContact.getTime()) / (1000 * 60 * 60 * 24)
    );

    if (daysSince > rel.targetContactFrequency) {
      const overdueDays = daysSince - rel.targetContactFrequency;
      let urgency: 'gentle' | 'moderate' | 'urgent';
 ...
 */
export async function getRelationshipsByTeam(
  userId: string,
  teamName: string
): Promise<Relationship[]> {
  const relationships = await getRelationships(userId);
  ...
  return relationships.filter((rel) =>
    rel.favoriteTeams.some((t) => t.toLowerCase().includes(lowerTeam))
  );
}
