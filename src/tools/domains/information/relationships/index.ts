...
export function createRelationshipTools() {
  return {
    addRelationship: llm.tool({
      description:
        'Add a new relationship/contact to track. Use this when the user mentions a friend, ' +
      ...
      execute: async ({
        ...
        const relationship = await saveRelationship(userId, {
          ...
          nickname,
          birthday:
            birthdayMonth && birthdayDay ? { month: birthdayMonth, day: birthdayDay } : undefined,
          interests: interests || [],
    ...
    getRelationshipInfo: llm.tool({
      ...
      execute: async ({ userId, name }) => {
        ...
        if (relationship.nickname) info += ` (${relationship.nickname})`;
        ...
        if (relationship.lastContact) {
          const days = Math.floor(
            (Date.now() - relationship.lastContact.getTime()) / (1000 * 60 * 60 * 24)
          );
    ...
    recordContact: llm.tool({
      description:
        'Record that the user had contact with someone. Use this when they mention ' +
          ...
          switch (type) {
            case 'family':
              filtered = relationships.filter(
                (r) => r.relationshipType.startsWith('family') || r.relationshipType === 'spouse'
              );
              break;
            case 'friends':
    ...
    getUpcomingBirthdays: llm.tool({
      description:
        'Get upcoming birthdays from tracked relationships. ' +
    ...
    getContactReminders: llm.tool({
      description:
        "Get reminders about people the user hasn't contacted in a while. " +
    ...
    getGiftSuggestions: llm.tool({
      description:
        'Get gift suggestions for a person based on their interests. ' +
      ...
      execute: async ({ userId, name, occasion, budget }) => {
        ...
        if (suggestions.length === 0) {
          return (
            `I don't have enough information about ${name}'s interests to suggest gifts. ` +
    ...
    getRelationshipInsights: llm.tool({
      description:
        'Get all relationship insights - birthdays, contact reminders, team updates. ' +
