 ...
 */
async function getSportScoreboard(sport) {
    const sportPath = ESPN_SPORTS[sport.toLowerCase()];
    if (!sportPath)
        return null;
    return withRateLimit('espn', async () => {
 ...
 */
function findTeamGames(data, teamName) {
    ...
    return (data.events || []).filter((event) => {
        ...
        return (name.includes(searchTerm) ||
            ...
            teams.some((t) => t.team?.displayName?.toLowerCase().includes(searchTerm) ||
 ...
 */
export async function getSportScores(sport) {
 ...
 */
export async function getTeamScore(teamName) {
 ...
 */
export async function getPhilliesScore() {
 ...
 */
export async function getEaglesScore() {
        ...
        getSportScores: llm.tool({
            ...
            parameters: z.object({
                sport: z
                    ...
                    .describe('Sport league: mlb (baseball), nfl (football), nba (basketball), nhl (hockey), mls (soccer), epl (Premier League), ncaaf (college football), ncaab (college basketball)'),
            }),
            execute: async ({ sport }) => {
