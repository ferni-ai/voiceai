 ...
 */
async function getTrafficFromGoogle(origin, destination) {
 ...
 */
async function getDirectionsFromGoogle(origin, destination, mode = 'driving') {
    if (!GOOGLE_MAPS_API_KEY)
        return null;
    try {
        const params = new URLSearchParams({
...
// ============================================================================
async function getTrafficFromHERE(origin, destination) {
    if (!HERE_API_KEY)
        return null;
    try {
...
async function getHereRoute(origin, destination) {
    const params = new URLSearchParams({
        transportMode: 'car',
    ...
    if (durationInTrafficSeconds <= 0)
        return null;
    const distanceText = lengthMeters > 0
        ? lengthMeters >= 1000
 ...
 */
export async function getTrafficTime(origin, destination) {
...
function formatDirections(info, mode) {
    const modeEmoji = {
        driving: '🚗',
        walking: '🚶',
        bicycling: '🚴',
    ...
    if (info.steps.length > 0) {
        ...
        const importantSteps = info.steps.length <= 5
            ? info.steps
...
export function setSavedLocation(userId, name, address) {
    // In production, save to database
    defaultLocations[`${userId}:${name.toLowerCase()}`] = { name, address };
}
export function getSavedLocation(userId, name) {
