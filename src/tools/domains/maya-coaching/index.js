...
import { recordHabitDNA, getHabitDNA, recordFrictionPoint, getFrictionPoints, recordTendencySignal, getTendencyProfile, recordKeystoneObservation, getKeystoneHabits, recordIdentityStatement, getIdentityEvolution, recordSetbackPattern, getSetbackPatterns, recordHabitAutopsy, getHabitAutopsies, } from '../../../services/superhuman/maya-coaching-services.js';
const log = createLogger({ module: 'tools:maya-coaching' });
// ============================================================================
// HABIT DNA TRACKER - Complete genetic profile of habits
// ============================================================================
const trackHabitDNADef = {
    id: 'trackHabitDNA',
    name: 'Track Habit DNA',
    description: 'Build a complete genetic profile of a habit: what makes it live, what kills it, optimal conditions for survival.',
    domain: 'maya-coaching',
    tags: ['habits', 'patterns', 'memory', 'maya-specialty'],
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                ...
                triggerOrBarrier: z.string().optional().describe('What triggered success or caused failure'),
            ...
            execute: async ({ habitName, event, context, triggerOrBarrier, emotionalState, timeOfDay, checkHistory }) => {
...
// ============================================================================
const mapFrictionDef = {
    id: 'mapFriction',
    name: 'Map Friction',
    description: 'Track exactly where, when, and why habits encounter friction. Build a map of your willpower failure points.',
    domain: 'maya-coaching',
    tags: ['habits', 'friction', 'barriers', 'maya-specialty'],
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                ...
                intensity: z.enum(['minor', 'moderate', 'major']).optional().describe('How much it derailed you'),
                viewMap: z.boolean().default(false).describe('True to view friction map instead of recording'),
            }),
            execute: async ({ habitName, frictionType, description, intensity, viewMap }) => {
                ...
                if (viewMap) {
                    ...
                    for (const fp of frictionPoints) {
                        if (!byType[fp.frictionType])
                            byType[fp.frictionType] = [];
...
// ============================================================================
const assessTendencyDef = {
    id: 'assessTendency',
    name: 'Assess Tendency',
    description: 'Dynamically assess and track their Four Tendency (Upholder/Questioner/Obliger/Rebel) from behavior patterns.',
    domain: 'maya-coaching',
    tags: ['habits', 'tendencies', 'personality', 'maya-specialty'],
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ signal, context, getProfile }) => {
                ...
                if (getProfile) {
                    ...
                    const tendencyDescriptions = {
                        Upholder: 'You meet both inner AND outer expectations. Rules work for you, whether self-imposed or external.',
                        Questioner: 'You need to understand WHY before committing. External rules only work if they make sense to you.',
                        Obliger: 'You meet outer expectations but struggle with inner ones. You need external accountability to thrive.',
                        Rebel: 'You resist ALL expectations. You need to feel like choices are yours. Identity framing works best.',
                    };
...
// ============================================================================
const detectKeystoneDef = {
    id: 'detectKeystone',
    name: 'Detect Keystone',
    description: 'Identify keystone habits that cascade into other positive behaviors. Find your highest-leverage habits.',
    domain: 'maya-coaching',
    tags: ['habits', 'keystone', 'leverage', 'maya-specialty'],
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                ...
                affectedHabits: z.array(z.string()).optional().describe('Habits that improve when primary habit happens'),
            ...
            execute: async ({ observation, primaryHabit, affectedHabits, viewKeystones }) => {
                ...
                if (viewKeystones) {
                    ...
                    const topKeystone = keystones.sort((a, b) => b.affectedHabits.length - a.affectedHabits.length)[0];
...
// ============================================================================
const trackIdentityShiftDef = {
    id: 'trackIdentityShift',
    name: 'Track Identity Shift',
    description: 'Track the evolution of identity statements over time. "I am someone who..." is the deepest form of habit change.',
    domain: 'maya-coaching',
    tags: ['habits', 'identity', 'transformation', 'maya-specialty'],
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('trackIdentityShift'),
            parameters: z.object({
                statement: z.string().optional().describe('Identity statement (e.g., "I am someone who exercises")'),
                domain: z
                    .enum(['health', 'productivity', 'relationships', 'finances', 'creativity', 'learning', 'other'])
                    .optional()
                    .describe('Life domain'),
                confidence: z.enum(['aspiring', 'emerging', 'established', 'core']).optional().describe('How true it feels'),
...
// ============================================================================
const analyzeSetbackPatternDef = {
    id: 'analyzeSetbackPattern',
    name: 'Analyze Setback Pattern',
    description: 'Deep pattern analysis of setbacks across years. Find the common threads in every time you\'ve fallen off.',
    domain: 'maya-coaching',
    tags: ['habits', 'setbacks', 'patterns', 'maya-specialty'],
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                ...
                whenItHappened: z.string().optional().describe('Time context (morning, weekend, stressful period)'),
                emotionalTrigger: z.string().optional().describe('Emotional state before the setback'),
                viewPatterns: z.boolean().default(false).describe('True to analyze patterns across all setbacks'),
            }),
            execute: async ({ habitName, whatHappened, whenItHappened, emotionalTrigger, viewPatterns }) => {
                const userId = ctx.userId || 'anonymous';
                ...
                if (viewPatterns) {
                    ...
                    for (const [trigger, count] of Object.entries(emotionalTriggers).sort((a, b) => b[1] - a[1]).slice(0, 3)) {
                        response += `• ${trigger}: ${count} setbacks\n`;
                    }
                    response += '\n';
                    if (Object.keys(timeTriggers).length > 0) {
                        response += `**Time Patterns:**\n`;
                        for (const [time, count] of Object.entries(timeTriggers).sort((a, b) => b[1] - a[1]).slice(0, 3)) {
                            response += `• ${time}: ${count} setbacks\n`;
                        }
                        response += '\n';
                    }
                    response += `**Most Vulnerable Habits:**\n`;
                    for (const [habit, count] of Object.entries(habitCounts).sort((a, b) => b[1] - a[1]).slice(0, 3)) {
                ...
                if (whenItHappened)
                    response += `When: ${whenItHappened}\n`;
                if (emotionalTrigger)
                    response += `Emotional state: ${emotionalTrigger}\n`;
                response += '\n';
...
// ============================================================================
const conductHabitAutopsyDef = {
    id: 'conductHabitAutopsy',
    name: 'Conduct Habit Autopsy',
    description: 'Deep post-mortem analysis for habits that died. Learn from every failure to prevent future deaths.',
    domain: 'maya-coaching',
    tags: ['habits', 'autopsy', 'learning', 'maya-specialty'],
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ habitName, howLongItLasted, causeOfDeath, lastRites, lessonsLearned, willResurrect, viewPastAutopsies, }) => {
                const userId = ctx.userId || 'anonymous';
                ...
                if (viewPastAutopsies) {
                    ...
                    for (const [cause, count] of Object.entries(causes).sort((a, b) => b[1] - a[1]).slice(0, 5)) {
                    ...
                    for (const autopsy of autopsies.slice(-5)) {
                        ...
                        if (autopsy.lessonsLearned)
                            response += `  Lesson: ${autopsy.lessonsLearned}\n`;
                        response += '\n';
                    }
                ...
                if (howLongItLasted)
                    response += `Survived: ${howLongItLasted}\n`;
                response += `Cause of death: ${causeOfDeath}\n`;
                if (lastRites)
                    response += `Last performed: ${lastRites}\n`;
                response += '\n';
                if (lessonsLearned) {
...
// ============================================================================
const backgroundHabitReminderDef = {
    id: 'backgroundHabitReminder',
    name: 'Background Habit Reminder',
    description: 'Schedule a habit reminder that will be delivered even when the user disconnects. Perfect for: "Remind me about my morning routine tomorrow", "Check on my meditation habit later", "Nudge me about exercise".',
    domain: 'maya-coaching',
    tags: ['background', 'async', 'habits', 'while-you-were-away', 'maya-specialty'],
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                ...
                scheduledTime: z.string().optional().describe('When to send (e.g., "tomorrow morning", "in 2 hours")'),
            }),
            execute: async ({ habitName, reminderType, currentStreak, context, scheduledTime }) => {
                ...
                try {
                    const { queueHabitReminder } = await import('../../../services/background-agents/executors/habit-reminder-executor.js');
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('maya-coaching', mayaCoachingTools);
