 ...
 */
export function isTravelApiConfigured() {
    return {
...
async function searchFlightsReal(params) {
    ...
    if (AMADEUS_API_KEY && AMADEUS_API_SECRET) {
        try {
            ...
            const searchResponse = await fetch(`https://api.amadeus.com/v2/shopping/flight-offers?${searchParams}`, {
            ...
            if (searchResponse.ok) {
                ...
                return (data.data || []).map((offer) => ({
                    ...
                    departureTime: new Date(offer.itineraries[0]?.segments[0]?.departure.at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
                    arrivalTime: new Date(offer.itineraries[0]?.segments[offer.itineraries[0].segments.length - 1]?.arrival.at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
...
async function searchFlightsMock(params) {
    ...
    getLogger().info({ origin: params.origin, destination: params.destination }, '✈️ Searching flights (mock - set AMADEUS_API_KEY for real results)');
...
async function searchHotelsReal(params) {
    ...
    if (BOOKING_AFFILIATE_ID) {
        try {
            ...
            const response = await fetch(`https://distribution-xml.booking.com/2.0/json/hotels?${searchParams}`, {
                headers: {
...
async function searchHotelsMock(params) {
    ...
    getLogger().info({ destination: params.destination }, '🏨 Searching hotels (mock - set BOOKING_AFFILIATE_ID for real results)');
    const nights = Math.ceil((params.checkOut.getTime() - params.checkIn.getTime()) / (1000 * 60 * 60 * 24));
    const hotelNames = [
        'Marriott',
        'Hilton',
...
export function createTravelTools() {
    return {
        searchFlights: llm.tool({
            ...
            execute: async ({ origin, destination, departureDate, returnDate, passengers, cabinClass }, { ctx }) => {
