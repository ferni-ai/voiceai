...
async function searchGooglePlaces(params) {
    ...
    try {
        ...
        return (data.results || []).slice(0, params.limit || 5).map((place) => ({
            ...
            cuisine: (place.types || []).filter((t) => !['restaurant', 'food', 'point_of_interest', 'establishment'].includes(t)),
...
function formatRestaurantResults(restaurants, params) {
    let response = `**Restaurants`;
    if (params.cuisine)
        response += ` - ${params.cuisine}`;
 ...
 */
export function getReservationHelp(restaurantName, partySize, date, time) {
