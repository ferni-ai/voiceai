 ...
 */
function createStartEnrollmentTool(_ctx) {
    return {
...
- identityId (string): The sponsored identity ID from the inbound call context`,
        ...
        execute: async (args) => {
            ...
            if (!identityId) {
                return {
                    success: false,
                    message: "I don't have your identity linked yet. Let's try again later when I know who you are.",
                };
            }
            try {
                const { startPhoneVoiceEnrollment } = await import('../../../services/identity/sponsored-identity.js');
                ...
                if (!result.success) {
                    ...
                    return {
                        success: false,
                        message: result.error === 'Voice already enrolled'
 ...
 */
function createRecordSampleTool(_ctx) {
    return {
...
- sampleQuality (string): Either 'good' or 'retry'`,
        ...
        execute: async (args) => {
            ...
            const { hasActiveEnrollment } = await import('../../../services/identity/sponsored-identity.js');
 ...
 */
function createFinishEnrollmentTool(_ctx) {
    return {
...
- reason (string, optional): Reason for cancellation`,
        ...
        execute: async (args) => {
            ...
            const { cancelPhoneVoiceEnrollment, hasActiveEnrollment } = await import('../../../services/identity/sponsored-identity.js');
            if (!hasActiveEnrollment(identityId)) {
                return {
                    success: true,
            ...
            if (action === 'cancel') {
                ...
                return {
                    success: true,
                    message: "No problem! We can set up voice recognition another time. I'll still recognize you by your phone number.",
                };
            }
            return {
                success: true,
                message: "I'm still learning your voice. Let's chat a bit more so I can get to know how you sound.",
            };
        },
    };
}
...
// TOOL DEFINITIONS
// ============================================================================
const startPhoneEnrollmentDef = {
    id: 'startPhoneVoiceEnrollment',
    name: 'Start Phone Voice Enrollment',
    description: 'Start voice enrollment for the current phone caller who is recognized but not voice-enrolled',
    domain: 'voice-enrollment',
    category: 'core',
    tags: ['voice', 'enrollment', 'phone', 'identity'],
    create: createStartEnrollmentTool,
};
const recordVoiceSampleDef = {
...
const finishPhoneEnrollmentDef = {
...
//# sourceMappingURL=phone-enrollment-tool.js.map