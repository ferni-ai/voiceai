...
import { searchRestaurants as searchGooglePlaces, getPlaceDetails as getGooglePlaceDetails, isGooglePlacesConfigured, } from '../../../services/google-places.js';
// Yelp (fallback + explicit)
import { searchBusinesses as searchYelp, getBusinessDetails as getYelpDetails, getBusinessReviews as getYelpReviews, getBusinessByPhone as yelpPhoneLookup, formatBusinessForSpeech as formatYelpBusiness, formatReviewForSpeech as formatYelpReview, isYelpConfigured, } from '../../../services/yelp.js';
const log = getLogger();
// ============================================================================
 ...
 */
function detectExplicitSource(query) {
    const lower = query.toLowerCase();
    if (lower.includes('yelp') || lower.includes('on yelp'))
        return 'yelp';
    if (lower.includes('google') || lower.includes('on google'))
        return 'google';
    return 'auto';
}
/**
 ...
 */
function googleToUnified(place) {
    const priceLevels = ['', '$', '$$', '$$$', '$$$$'];
    return {
        name: place.name,
 ...
 */
function formatResults(results, query, location) {
    if (results.length === 0) {
 ...
 */
async function unifiedSearch(query, location, options) {
 ...
 */
const searchLocalBusinessesDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Search for local businesses like restaurants, coffee shops, gyms, salons. ' +
...
const findRestaurantsDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Find restaurants in a specific area. Great for dinner recommendations. ' +
                'Say "on Yelp" or "on Google" to use a specific source.',
            parameters: z.object({
                location: z.string().describe('Where to search (city, neighborhood, or address)'),
            ...
            execute: async ({ location, cuisine, openNow, priceLevel }) => {
                try {
                    ...
                    const cleanCuisine = (cuisine || '')
                        .replace(/\s+(on\s+)?(yelp|google)/gi, '')
                        .trim();
                    const results = await unifiedSearch(cleanCuisine ? `${cleanCuisine} restaurant` : 'restaurant', location, { openNow, priceLevel, explicitSource });
                    if (results.length === 0) {
...
const getBusinessInfoDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Get detailed information about a specific business including hours, phone, and reviews.',
            parameters: z.object({
            ...
            execute: async ({ businessId, source }) => {
                try {
                    ...
                    if (source === 'google' && isGooglePlacesConfigured()) {
                        ...
                        if (!details)
                            return "I couldn't find details for that business.";
                    ...
                    if (source === 'yelp' && isYelpConfigured()) {
                        ...
                        if (details.display_phone)
                            response += `📞 ${details.display_phone}\n`;
                        // Get reviews
...
const lookupByPhoneDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Look up a business by its phone number. Useful for identifying mystery calls or finding business info.',
            parameters: z.object({
 ...
 */
const checkLocalSearchStatusDef = {
