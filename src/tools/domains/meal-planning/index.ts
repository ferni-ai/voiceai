...
const planWeeklyMealsDef: ToolDefinition = {
  id: 'planWeeklyMeals',
...
const addRecipeDef: ToolDefinition = {
  id: 'addRecipe',
  name: 'Add Recipe',
    ...
    return llm.tool({
      ...
      execute: async (params: { query: string }) => {
        ...
        for (const recipe of results.slice(0, 5)) {
          ...
          if (recipe.rating) response += ` (${recipe.rating}⭐)`;
          if (recipe.isFavorite) response += ' ❤️';
          response += `\n`;
...
const suggestMealsDef: ToolDefinition = {
  id: 'suggestMeals',
  name: 'Suggest Meals',
...
const generateShoppingListDef: ToolDefinition = {
...
const trackDietaryPreferencesDef: ToolDefinition = {
...
const markRecipeCookedDef: ToolDefinition = {
  id: 'markRecipeCooked',
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('meal-planning', [
  planWeeklyMealsDef,
  addRecipeDef,
  searchRecipesDef,
  suggestMealsDef,
  generateShoppingListDef,
  trackDietaryPreferencesDef,
  markRecipeCookedDef,
]);
