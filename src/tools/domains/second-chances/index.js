...
// ASSESSMENT TOOLS
// ============================================================================
const assessReadinessForChangeDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ changeType, whatHappened, howLongAgo, currentFeeling }) => {
                getLogger().info({ agentId: ctx.agentId, changeType, currentFeeling }, 'Assessing readiness for change');
                const typeInfo = SECOND_CHANCE_TYPES[changeType] ||
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ currentStory, stuckPoint }) => {
                getLogger().info({ agentId: ctx.agentId, hasStuckPoint: !!stuckPoint }, 'Reframing narrative');
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ comebackType, startingPoint, destination, resources, constraints }) => {
                ...
                if (destination)
                    response += `**Heading toward:** ${destination}\n`;
                response += `\n`;
                ...
                // Add inspiring story
                const storyCategory = comebackType === 'financial'
                    ? 'financial'
                    : comebackType === 'career'
                        ? 'career'
                        : 'personal';
                const stories = COMEBACK_STORIES[storyCategory] ||
...
// ============================================================================
const processGriefForWhatWasDef = {
...
const buildCourageForWhatNextDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ fearOf, whatCourageLooksLike, pastCourage }) => {
                getLogger().info({ agentId: ctx.agentId, hasPastCourage: !!pastCourage }, 'Building courage');
...
// WISDOM TOOLS
// ============================================================================
const shareSecondChanceWisdomDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ context, wantStory }) => {
                getLogger().info({ agentId: ctx.agentId, context, wantStory }, 'Sharing second chance wisdom');
                ...
                const wisdomToShare = matchingWisdom.length > 0 ? matchingWisdom : SECOND_CHANCE_WISDOM.slice(0, 3);
                ...
                if (wantStory) {
                    ...
                    const storyCategories = Object.keys(COMEBACK_STORIES);
                    const category = context === 'financial'
                        ? 'financial'
                        : ['career', 'failure'].includes(context)
                            ? 'career'
                            : 'personal';
                    const stories = COMEBACK_STORIES[category] || COMEBACK_STORIES.career;
                    const story = stories[Math.floor(Math.random() * stories.length)];
                    response += `**${story.name}**\n`;
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('second-chances', secondChanceTools);
