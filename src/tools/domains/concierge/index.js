...
import { createConciergeRouter, getTaskTracker, PhoneCaller, registerNotifier, } from '../../../services/concierge/index.js';
...
const requestHotelQuotesDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Request hotel rate quotes by calling multiple hotels. Use when the user wants to find hotel prices or book accommodations.',
            parameters: z.object({
            ...
            execute: async (params) => {
                try {
                    log.info({ destination: params.destination, userId: ctx.userId }, 'Requesting hotel quotes');
                    ...
                    const result = await router.routeRequest(`Find hotel rates in ${params.destination}`, requirements, { maxTargets: 5, preferredChannel: 'phone' });
                    if (!result.success) {
                        return `I couldn't start the hotel search: ${result.error}`;
                    }
                    // Start outreach in background
                    startConciergeOutreach(result.requestId, ctx.userId).catch((e) => log.error({ error: String(e) }, 'Outreach failed'));
...
// ============================================================================
const makeRestaurantReservationDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Make a restaurant reservation by calling restaurants. Use when the user wants to book a table.',
            parameters: z.object({
            ...
            execute: async (params) => {
                try {
                    log.info({ location: params.location, partySize: params.partySize }, 'Making restaurant reservation');
                    const router = createConciergeRouter({
                        userId: ctx.userId,
                        sessionId: undefined,
                    });
...
const scheduleAppointmentDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Schedule a healthcare appointment by calling medical offices. Use when the user needs to see a doctor, dentist, or specialist.',
            parameters: z.object({
                providerType: z
                    .string()
            ...
            execute: async (params) => {
                try {
                    log.info({ providerType: params.providerType, location: params.location }, 'Scheduling appointment');
                    const router = createConciergeRouter({
                        userId: ctx.userId,
                        sessionId: undefined,
                    });
                    ...
                    const result = await router.routeRequest(`Schedule a ${params.providerType} appointment`, requirements, { maxTargets: 3, preferredChannel: 'phone' });
                    if (!result.success) {
                        return `I couldn't start the appointment search: ${result.error}`;
                    }
                    startConciergeOutreach(result.requestId, ctx.userId).catch((e) => log.error({ error: String(e) }, 'Outreach failed'));
                    const urgencyText = params.urgency === 'urgent'
...
const getServiceQuotesDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Get quotes from local service providers. Use when the user needs a plumber, electrician, cleaner, or other home service.',
            parameters: z.object({
                serviceType: z
                    .string()
            ...
            execute: async (params) => {
                try {
                    log.info({ serviceType: params.serviceType, location: params.location }, 'Getting service quotes');
                    const router = createConciergeRouter({
                        userId: ctx.userId,
                        sessionId: undefined,
                    });
                    ...
                    const result = await router.routeRequest(`Get quotes for ${params.serviceType}`, requirements, { maxTargets: 5, preferredChannel: 'phone' });
                    if (!result.success) {
...
const checkConciergeStatusDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Check the status of outreach requests. Use when the user asks about pending hotel quotes, reservations, or appointments.',
            parameters: z.object({
 ...
 */
async function startConciergeOutreach(requestId, userId) {
    const tracker = getTaskTracker();
 ...
 */
function formatStatusUpdate(request) {
    ...
    switch (request.status) {
        ...
        case 'awaiting_user':
            ...
            break;
        case 'completed':
            message += `All done! Got ${request.results.filter((r) => r.success).length} successful responses.`;
            break;
        case 'failed':
...
// ============================================================================
const prepareForUpcomingEventDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: 'Get comprehensive preparation for an upcoming calendar event - context about attendees, suggested talking points, travel considerations, and more.',
            parameters: z.object({
...
const proactiveConciergeCheckInDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: "Proactively review what's coming up and identify where concierge services could help.",
            parameters: z.object({}),
            execute: async () => {
                try {
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('concierge', conciergeTools);
