...
// REMEMBER ABOUT USER
// ============================================================================
export const rememberAboutUserDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                fact: z
                    .string()
                    .describe('The important fact to remember (e.g., "has two kids", "retiring next year", "worried about healthcare costs")'),
                category: z
            ...
            execute: async ({ fact, category, importance }, { ctx: toolCtx }) => {
                getLogger().info({ agentId: ctx.agentId, fact, category, importance }, 'Remembering about user');
...
// RECALL FROM MEMORY
// ============================================================================
export const recallFromMemoryDef = {
    id: 'recallFromMemory',
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                topic: z
                    .string()
                    .describe('What you\'re trying to recall (e.g., "their retirement goals", "family situation", "what worried them")'),
            }),
...
// ============================================================================
export const recallPreviousConversationDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                query: z
                    .string()
                    .describe('What to search for in past conversations (e.g., "retirement planning", "market concerns", "family goals")'),
            }),
...
// ============================================================================
export const rememberImportantFactDef = {
        ...
        return llm.tool({
            ...
            execute: async ({ fact, type, emotionalWeight }, { ctx: toolCtx }) => {
                ...
                if (services?.learningEngine) {
                    const keyMomentType = type === 'life_event'
                        ? 'milestone'
                        : type === 'decision'
                            ? 'decision'
                            : type === 'breakthrough'
...
// ============================================================================
export const getRelationshipSummaryDef = {
            ...
            execute: async (_, { ctx: toolCtx }) => {
                ...
                if (services?.userProfile) {
                    ...
                    if (profile.preferredTopics.length > 0) {
                        sections.push(`They tend to discuss: ${profile.preferredTopics.slice(0, 3).join(', ')}.`);
                    }
                    if (profile.goals.length > 0) {
                        const activeGoals = profile.goals.filter((g) => g.status === 'active');
                        if (activeGoals.length > 0) {
                            sections.push(`They're working toward: ${activeGoals.map((g) => g.name).join(', ')}.`);
                        }
                    }
                    if (profile.lastConversationSummary) {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ originalFact, updatedFact, reason }, { ctx: toolCtx }) => {
                getLogger().info({ agentId: ctx.agentId, originalFact, updatedFact, reason }, 'Updating memory');
                const userData = toolCtx.userData;
                const { services } = userData;
                // Update in session memory
                if (userData.keyMoments) {
                    const index = userData.keyMoments.findIndex((m) => m.toLowerCase().includes(originalFact.toLowerCase()));
                    if (index !== -1) {
                ...
                if (services?.captureInsight) {
                    services.captureInsight('correction', 'memory_update', `Updated: "${originalFact}" → "${updatedFact}"${reason ? ` (${reason})` : ''}`, 0.9);
                }
                // Return empty string - agent should continue naturally without reading any acknowledgment
                return '';
            },
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ whatToForget, confirmDeletion }, { ctx: toolCtx }) => {
                getLogger().info({ agentId: ctx.agentId, whatToForget, confirmDeletion }, 'Forgetting memory');
                if (!confirmDeletion) {
                ...
                if (userData.keyMoments) {
                    userData.keyMoments = userData.keyMoments.filter((m) => !m.toLowerCase().includes(whatToForget.toLowerCase()));
                }
                // Log the deletion for audit purposes (but don't persist the deleted content)
                if (services?.captureInsight) {
                    services.captureInsight('user_action', 'memory_deletion', `User requested deletion of memory related to: [redacted]`, 1.0);
                }
                // Return empty string - agent should continue naturally without reading any acknowledgment
                return '';
            },
...
// ============================================================================
export const surfaceRelevantMemoryDef = {
    id: 'surfaceRelevantMemory',
    name: 'Surface Relevant Memory',
    description: 'Proactively surface a relevant memory when context connects to something from past conversations. Use this to show continuity and deep understanding.',
    domain: 'memory',
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                context: z
                    .string()
                    .describe('Current conversation context that triggered this memory (e.g., "user mentioned Sarah")'),
                memoryToSurface: z
                    .string()
                    .describe('The relevant memory to bring up (e.g., "Sarah was going through a tough time")'),
                connectionReason: z
                    .string()
            ...
            execute: async ({ context, memoryToSurface, connectionReason }, { ctx: toolCtx }) => {
                getLogger().info({ agentId: ctx.agentId, context, memoryToSurface, connectionReason }, '🧠 [BETTER-THAN-HUMAN] Surfacing relevant memory');
                const userData = toolCtx.userData;
                const { services } = userData;
                ...
                // Check session memory
                if (userData.keyMoments && userData.keyMoments.length > 0) {
                    const relevant = userData.keyMoments.find((m) => m.toLowerCase().includes(memoryToSurface.toLowerCase().split(' ')[0]));
                    if (relevant) {
                        return relevant;
                    }
                }
...
// ============================================================================
export const predictUserNeedDef = {
    id: 'predictUserNeed',
    name: 'Predict User Need',
    description: 'Anticipate what the user might need based on context, time, patterns, or upcoming events. Use this proactively to offer help before they ask.',
    domain: 'memory',
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ context, prediction, confidence, suggestedAction }, { ctx: toolCtx }) => {
                getLogger().info({ agentId: ctx.agentId, context, prediction, confidence, suggestedAction }, '🔮 [BETTER-THAN-HUMAN] Predicting user need');
                const userData = toolCtx.userData;
                const { services } = userData;
                // Log the prediction for learning
                if (services?.captureInsight) {
                    services.captureInsight('anticipatory_prediction', `predict_${confidence}`, `Context: ${context} | Prediction: ${prediction}`, confidence === 'high' ? 0.9 : confidence === 'medium' ? 0.7 : 0.5);
                }
                ...
                if (services?.userProfile) {
                    ...
                    if (profile.preferredTopics && profile.preferredTopics.length > 0) {
                        const relevantTopic = profile.preferredTopics.find((t) => prediction.toLowerCase().includes(t.toLowerCase()));
                        if (relevantTopic) {
                    ...
                    if (profile.goals && profile.goals.length > 0) {
                        const activeGoal = profile.goals.find((g) => g.status === 'active' && prediction.toLowerCase().includes(g.name.toLowerCase()));
                        if (activeGoal) {
