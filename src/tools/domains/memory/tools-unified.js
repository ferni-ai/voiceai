...
// ============================================================================
export const recallFromMemoryUnifiedDef = {
    id: 'recallFromMemory',
    ...
    create: (ctx) => {
        ...
        return llm.tool({
            ...
            parameters: z.object({
                topic: z
                    .string()
                    .describe('What you\'re trying to recall (e.g., "their retirement goals", "family situation")'),
            }),
...
// ============================================================================
export const recallPreviousConversationUnifiedDef = {
    ...
    create: (ctx) => {
        ...
        return llm.tool({
            ...
            parameters: z.object({
                query: z
                    .string()
                    .describe('What to search for in past conversations (e.g., "retirement planning", "market concerns")'),
            }),
...
// ============================================================================
export const rememberAboutUserUnifiedDef = {
    ...
    create: (ctx) => {
        ...
        return llm.tool({
            ...
            execute: async ({ fact, category, importance }, { ctx: toolCtx }) => {
                log.info({ agentId: ctx.agentId, fact, category, importance }, '🧠 [UNIFIED] Remember about user');
                const userData = toolCtx.userData;
                const userId = userData.userId || ctx.userId;
                if (!userId) {
...
// ============================================================================
export const rememberImportantFactUnifiedDef = {
    ...
    create: (ctx) => {
        ...
        return llm.tool({
            ...
            execute: async ({ fact, type, emotionalWeight }, { ctx: toolCtx }) => {
                log.info({ agentId: ctx.agentId, fact, type, emotionalWeight }, '🧠 [UNIFIED] Remember important fact');
                const userData = toolCtx.userData;
                const userId = userData.userId || ctx.userId;
                if (!userId) {
                    log.warn('No userId available for memory write');
                ...
                const memoryType = type === 'life_event' || type === 'milestone'
                    ? 'milestone'
                    : type === 'concern'
                        ? 'emotion'
                        : type === 'decision'
                ...
                const result = await memoryService.write({
                    ...
                    content: fact,
                    type: memoryType,
                    importance: emotionalWeight === 'heavy'
                        ? 'critical'
...
// ============================================================================
export const surfaceRelevantMemoryUnifiedDef = {
    id: 'surfaceRelevantMemory',
    name: 'Surface Relevant Memory',
    description: 'Proactively surface a relevant memory when context connects to something from past conversations.',
    domain: 'memory',
    ...
    create: (ctx) => {
        ...
        return llm.tool({
            ...
            execute: async ({ context, memoryToSurface, connectionReason }, { ctx: toolCtx }) => {
                log.info({ agentId: ctx.agentId, context, connectionReason }, '🧠 [BETTER-THAN-HUMAN] Surface relevant memory');
                const userData = toolCtx.userData;
                const userId = userData.userId || ctx.userId;
                if (!userId) {
...
// ============================================================================
export const predictUserNeedUnifiedDef = {
    id: 'predictUserNeed',
    name: 'Predict User Need',
    description: 'Anticipate what the user might need based on context, time, patterns, or upcoming events.',
    domain: 'memory',
    ...
    create: (ctx) => {
        ...
        return llm.tool({
            ...
            execute: async ({ context, prediction, confidence, suggestedAction }, { ctx: toolCtx }) => {
                log.info({ agentId: ctx.agentId, context, prediction, confidence }, '🔮 [BETTER-THAN-HUMAN] Predict user need');
                const userData = toolCtx.userData;
                const userId = userData.userId || ctx.userId;
                if (userId) {
...
//# sourceMappingURL=tools-unified.js.map