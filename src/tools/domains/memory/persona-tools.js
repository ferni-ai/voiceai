...
import { getUserId, ordinal, } from '../../shared/persona-memory-factory.js';
import { 
// Core functions
recall, remember, findMemory, updateMemory, forget, formatMemoryForSpeech, touchMemory, 
// Ferni helpers
rememberPreference, rememberWin, getFerniMemories, 
// Bogle helpers
rememberFund, rememberInvestingPhilosophy, getBogleMemories, 
// Peter helpers
addToWatchlist, rememberCompany, getWatchlist, getPeterMemories, markAsTenBagger, 
// Maya helpers
rememberMerchant, rememberBill, rememberSavingsGoal, rememberSpendingTrigger, getMayaMemories, 
// Jordan helpers
rememberDate, rememberVenue, rememberDestination, getImportantDates, getJordanMemories, } from '../../../services/memory/persona-memories.js';
// ============================================================================
        ...
        whatDoYouKnowAboutMe: llm.tool({
            ...
            execute: async ({ type }, { ctx }) => {
                ...
                const filtered = type === 'all'
                    ? memories
                    : memories.filter((m) => type === 'wins' ? m.type === 'win' : m.type === 'preference');
                const list = filtered
                    .slice(0, 10)
    ...
    return {
        ...
        rememberBill: llm.tool({
            ...
            execute: async ({ name, amount, dueDate, isAutoPay }, { ctx }) => {
                ...
                if (amount)
                    response += ` ($${amount}/month)`;
                if (dueDate)
                    response += ` due on the ${ordinal(dueDate)}`;
                if (isAutoPay)
                    response += ` (autopay ✓)`;
