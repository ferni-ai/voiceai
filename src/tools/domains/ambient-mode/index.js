...
// GET AMBIENT CONTEXT
// ============================================================================
const getAmbientContextDef = {
    id: 'getAmbientContext',
    name: 'Get Ambient Context',
    description: 'Get the current ambient context - location type, time of day, recent activity. Better than human awareness.',
    domain: 'awareness',
            ...
            execute: async ({ nudgeType }) => {
                try {
                    ...
                    if (evaluation && evaluation.shouldSend) {
                        log.info({ userId: ctx.userId, nudgeType: evaluation.type, reason: evaluation.reason }, 'Nudge suggested');
...
// ============================================================================
const getAmbientPreferencesDef = {
        ...
        return llm.tool({
            ...
            execute: async () => {
                try {
                    ...
                    if (prefs.enabled) {
                        if (prefs.allowLocation)
                            parts.push('location tracking is on');
                        if (prefs.allowPushNudges)
                            parts.push('nudges are enabled');
...
const getEnergyWaveAwarenessDef = {
...
const getFullAmbientIntelligenceDef = {
            ...
            execute: async () => {
                try {
                    ...
                    if (ambientContext?.hasAmbientData) {
                        ...
                        if (ambientContext.locationAwareness)
                            sections.push(ambientContext.locationAwareness);
                        if (ambientContext.timeAwareness)
                            sections.push(ambientContext.timeAwareness);
                        if (ambientContext.activityAwareness)
                            sections.push(ambientContext.activityAwareness);
                    }
                    if (energyContext) {
                        sections.push(`\n**Energy:**\n${energyContext}`);
                    }
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('awareness', // Part of awareness domain
ambientModeTools);
