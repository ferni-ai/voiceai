...
import {
  ...
  type PodcastShow,
  type PodcastEpisode,
} from '../../../services/podcasts/index.js';
...
function formatPodcastForSpeech(show: PodcastShow): string {
...
function formatEpisodeForSpeech(episode: PodcastEpisode): string {
  const duration = episode.durationMs ? ` (${Math.round(episode.durationMs / 60000)} minutes)` : '';
  return `"${episode.title}"${duration}`;
}

function formatPodcastList(shows: PodcastShow[], limit = 5): string {
  const limited = shows.slice(0, limit);
  if (limited.length === 0) return 'No podcasts found.';
...
function formatEpisodeList(episodes: PodcastEpisode[], limit = 5): string {
  const limited = episodes.slice(0, limit);
  if (limited.length === 0) return 'No episodes found.';
...
const searchPodcastsTool: ToolDefinition = {
  id: 'searchPodcasts',
  ...
  create: (_ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        'Search for podcasts by topic, name, guest, or keyword. Returns podcast shows with descriptions and episode counts.',
      parameters: z.object({
...
const getPodcastRecommendationsTool: ToolDefinition = {
  ...
  create: (_ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        'Get podcast recommendations based on user interests or mood. Great for discovering new shows.',
      parameters: z.object({
        interests: z
          .array(z.string())
...
const getPodcastEpisodesTool: ToolDefinition = {
  ...
  create: (_ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        'Get recent episodes from a specific podcast. Use after finding a podcast with searchPodcasts.',
      parameters: z.object({
...
const getTopPodcastsTool: ToolDefinition = {
  id: 'getTopPodcasts',
  ...
  create: (_ctx: ToolContext): Tool => {
    return llm.tool({
      description:
        'Get popular and trending podcasts. Optionally filter by genre like comedy, news, true_crime, business, health, technology, education, sports, or music.',
      parameters: z.object({
        genre: z
          .string()
          .optional()
          .describe(
            'Genre to filter by (comedy, news, true_crime, business, health, technology, education, sports, music)'
          ),
        limit: z
          .number()
          .min(1)
...
const podcastTools: ToolDefinition[] = [
  searchPodcastsTool,
