...
function listDataToList(data, userId) {
    return {
        id: data.id,
        userId,
        name: data.name,
...
async function ensureUserListsLoaded(userId) {
...
function persistList(userId, list) {
    try {
        const store = getProductivityStore();
...
function getOrCreateList(userId, type = 'groceries') {
...
function parseItemWithQuantity(input) {
...
function formatItemForSpeech(item) {
...
export function addToList(params) {
    ...
    for (const itemStr of params.items) {
        ...
        const existing = list.items.find((i) => i.name.toLowerCase() === parsed.name.toLowerCase() && !i.isChecked);
        if (existing) {
...
export function checkOffItem(userId, itemName, listType) {
    const list = getOrCreateList(userId, listType || 'groceries');
    const item = list.items.find((i) => i.name.toLowerCase().includes(itemName.toLowerCase()) && !i.isChecked);
    if (item) {
        item.isChecked = true;
        list.updatedAt = new Date();
        listsCache.set(list.id, list);
...
export function removeItem(userId, itemName, listType) {
    const list = getOrCreateList(userId, listType || 'groceries');
...
export function clearCheckedItems(userId, listType) {
    const list = getOrCreateList(userId, listType || 'groceries');
...
export function clearList(userId, listType) {
    const list = getOrCreateList(userId, listType || 'groceries');
    list.items = [];
    list.updatedAt = new Date();
    listsCache.set(list.id, list);
