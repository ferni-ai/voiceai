...
function detectCarrier(trackingNumber) {
    ...
    if (/^1Z[A-Z0-9]{16}$/i.test(tracking))
        return 'ups';
    // FedEx: 12-22 digits
    if (/^\d{12,22}$/.test(tracking))
        return 'fedex';
    // USPS: Various formats
    if (/^(94|93|92|91|90)[0-9]{18,22}$/.test(tracking))
        return 'usps';
    if (/^[A-Z]{2}[0-9]{9}US$/i.test(tracking))
        return 'usps';
    // Amazon: TBA followed by digits
    if (/^TBA\d+$/i.test(tracking))
        return 'amazon';
    // DHL: 10 digits or JD followed by 18 digits
    if (/^\d{10}$/.test(tracking) || /^JD\d{18}$/i.test(tracking))
        return 'dhl';
    return 'other';
}
 ...
 */
export function isTrackingApiConfigured() {
    return !!(AFTERSHIP_API_KEY || UPS_ACCESS_KEY || FEDEX_API_KEY);
}
/**
 * Fetch real tracking info from AfterShip API
 */
async function fetchTrackingFromAfterShip(trackingNumber, carrier) {
...
function getUserPackages(userId) {
    return Array.from(packages.values())
        ...
        if (a.status === 'delivered' && b.status !== 'delivered')
            return 1;
        if (b.status === 'delivered' && a.status !== 'delivered')
            return -1;
...
function getActivePackages(userId) {
    return getUserPackages(userId).filter((p) => p.status !== 'delivered');
}
function formatPackageForSpeech(pkg) {
    const statusEmoji = {
        label_created: '📦',
        in_transit: '🚚',
        out_for_delivery: '🏃',
    ...
    if (pkg.status === 'delivered') {
        ...
        const now = new Date();
        const daysUntil = Math.ceil((pkg.expectedDelivery.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        if (daysUntil === 0) {
...
async function fetchTrackingInfo(trackingNumber, carrier) {
    ...
    if (realEvents && realEvents.length > 0) {
        getLogger().info({ trackingNumber, carrier, eventCount: realEvents.length }, '📦 Fetched real tracking info from AfterShip');
        // Determine current status from latest event
        const latestEvent = realEvents[realEvents.length - 1];
        // Estimate delivery date
        const expectedDelivery = latestEvent.status === 'out_for_delivery'
            ? new Date()
    ...
    getLogger().info({ trackingNumber, carrier }, '📦 Fetching tracking info (mock - set AFTERSHIP_API_KEY for real tracking)');
    // Simulate API call
...
export async function addPackage(params) {
    ...
    getLogger().info({ pkgId: pkg.id, tracking: pkg.trackingNumber, carrier: pkg.carrier }, '📦 Package added');
    return pkg;
}
export async function refreshPackage(pkgId) {
...
export function markDelivered(pkgId) {
    const pkg = packages.get(pkgId);
    if (!pkg)
        return null;
...
export function archivePackage(pkgId) {
    const pkg = packages.get(pkgId);
    if (!pkg)
        return false;
    pkg.isActive = false;
    ...
    return {
        ...
        checkPackageStatus: llm.tool({
            ...
            execute: async ({ packageDescription }, { ctx }) => {
                ...
                const pkg = userPackages.find((p) => p.description.toLowerCase().includes(packageDescription.toLowerCase()));
                if (!pkg) {
                ...
                if (!refreshed)
                    return `Couldn't check that package right now. Want to try again?`;
        ...
        getDeliveryExpectations: llm.tool({
            ...
            execute: async (_, { ctx }) => {
                ...
                const later = active.filter((p) => !outForDelivery.includes(p) &&
