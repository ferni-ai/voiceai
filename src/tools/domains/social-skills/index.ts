...
import { generateAdaptiveResponse } from '../life-coaching-shared/adaptive-response.js';
import { SOCIAL_FRAMEWORKS } from '../life-coaching-shared/content-databases.js';
import type { ResponseContext } from '../life-coaching-shared/types.js';
...
const makeFriendsAsAdultDef: ToolDefinition = {
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ situation, currentCircle, barrier }) => {
        ...
        const situationResponses: Record<string, string> = {
          'new-city':
            "Starting over in a new place is genuinely hard. You're building from scratch, and that takes time.",
          'lost-friends':
            'Friendships fading is painful - and common. Life changes, people drift. It says nothing about your worth.',
          'work-only':
            'When work is your only social outlet, it can feel hollow. You deserve friendships that exist because people like you, not because you share a Slack channel.',
          'general-lonely':
            "Loneliness is an epidemic - you're not alone in feeling alone. And it's okay to want more connection.",
          'want-more-friends':
            'Wanting deeper or more friendships is healthy. It means you value connection.',
        };
...
const startConversationDef: ToolDefinition = {
...
const navigateSocialAnxietyDef: ToolDefinition = {
...
const deepenAcquaintanceDef: ToolDefinition = {
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ personDescription, currentLevel, barrier }) => {
        ...
        const levelAdvice: Record<string, string[]> = {
          'just-met': [
        ...
        if (barrier) {
          ...
          if (barrier.toLowerCase().includes('busy') || barrier.toLowerCase().includes('time')) {
            ...
            barrier.toLowerCase().includes('weird') ||
            ...
            barrier.toLowerCase().includes('reject') ||
...
const handleSocialRejectionDef: ToolDefinition = {
    ...
    return llm.tool({
      ...
      execute: async ({ struggle, friendCount }) => {
        ...
        const struggleAdvice: Record<string, string> = {
          'too-busy':
            "**When you're too busy:** The truth is, we make time for what we prioritize. Block 'friend time' in your calendar like any other appointment. A 15-minute call > silence for months.",
          forget:
            "**When you forget to reach out:** Set recurring reminders. Create a 'friend check-in' list and rotate through it. Use birthdays/holidays as anchors. Technology can compensate for memory.",
          'one-sided':
            '**When it feels one-sided:** First, check - are you initiating enough? Some people are bad at reaching out but wonderful when you do. If truly one-sided after effort, it may be time to accept the drift.',
          'drift-apart':
            "**When you drift apart:** Not all friendships are meant to last forever. But before accepting the drift, try: a direct acknowledgment ('I miss our friendship'), a specific invitation, or asking what changed.",
          'long-distance':
            "**When distance is the barrier:** Schedule regular video calls. Send voice notes. Share memes. Visit when possible. Accept it will be different, but different isn't dead.",
        };
...
const joinNewGroupsDef: ToolDefinition = {
  id: 'joinNewGroups',
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ interests, barrier }) => {
        ...
        if (barrier) {
          const barrierAdvice: Record<string, string> = {
            ...
            'scared-to-show-up':
              "First time is always the hardest. Remember: everyone was new once. Set a low goal (just stay 30 minutes). Bring a 'wingman' if possible.",
            'tried-didnt-work':
              "One group not clicking doesn't mean they all won't. Try different vibes - some groups are cliquey, others welcoming. Give any group 3 visits before judging.",
            'nothing-fits':
              "Maybe nothing perfect exists yet. Could you start something? Even posting 'Anyone want to [activity]?' on a local Facebook group can attract your people.",
          };
...
const networkAuthenticallyDef: ToolDefinition = {
...
const smallTalkMasteryDef: ToolDefinition = {
  id: 'smallTalkMastery',
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({ struggle }) => {
        ...
        const struggleResponses: Record<string, string> = {
          'hate-it':
            "Hating small talk is fair. It can feel pointless. But here's the thing: small talk is the gateway to big talk. You can't skip it entirely.",
          'run-out-of-things':
            "Running out of things to say usually means you're too focused on yourself. Shift focus to them: ask questions, be curious, listen.",
          'feels-fake':
            "'How's the weather' IS fake. But small talk doesn't have to be. You can ask real questions in casual contexts.",
          anxious:
            "Anxiety in small talk is common. The pressure to 'perform' is exhausting. Reframe: you're not performing, you're connecting.",
          'boring-to-me':
            "If it's boring, you're probably not going deep enough. 'What do you do?' is boring. 'What got you into that?' is interesting.",
        };
...
const socialSkillsTools: ToolDefinition[] = [
