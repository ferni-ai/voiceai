 ...
 */
const acknowledgeTransitionDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ emotion1, emotion2, situation }) => {
                ...
                const matchingPair = DUAL_EMOTIONS_WISDOM.find((w) => w.pair.toLowerCase().includes(emotion1.toLowerCase()) ||
 ...
 */
const navigateAmbiguousLossDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ focus, specificQuestion }) => {
                const questions = IDENTITY_QUESTIONS[focus === 'past'
                    ? 'honoring_past'
                    : focus === 'present'
                        ? 'exploring_present'
                        : 'discovering_future'];
 ...
 */
const findMeaningInTransitionDef = {
 ...
 */
const createTransitionRitualDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ ritualType, theTransition, elements }) => {
                const rituals = TRANSITION_RITUALS[ritualType === 'ending'
                    ? 'endings'
 ...
 */
function identifyTransitionType(description) {
 ...
 */
function identifyTransitionStage(experiencing) {
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('life-transitions', lifeTransitionsTools);
