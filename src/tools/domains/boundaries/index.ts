...
import {
  ...
  adaptForTendency,
  TENDENCY_FRAMINGS,
  detectPeoplePleasing,
  recognizeProgress,
} from '../life-coaching-shared/adaptive-response.js';
import {
  BOUNDARY_SCRIPTS,
  getScriptForCategory,
  getAdaptedScript,
  TENDENCY_STRATEGIES,
  REFLECTION_QUESTIONS,
} from '../life-coaching-shared/content-databases.js';
import type { ResponseContext, FourTendency } from '../life-coaching-shared/types.js';
...
const identifyBoundaryNeedsDef: ToolDefinition = {
...
const setBoundaryDef: ToolDefinition = {
  id: 'setBoundary',
  name: 'Set a Boundary',
  ...
  create: (ctx: ToolContext): Tool => {
    return llm.tool({
      ...
      execute: async ({
        ...
        let scriptCategory: string = boundaryType;
...
const maintainBoundaryDef: ToolDefinition = {
  id: 'maintainBoundary',
    ...
    return llm.tool({
      ...
      execute: async ({ boundarySet, howTested, personReaction, feelingAboutIt }) => {
        ...
        const reactionResponses: Record<string, string> = {
          anger:
            "Anger is sometimes how people respond when they lose access to something they felt entitled to. That doesn't make your boundary wrong.",
          'guilt-trip':
            'Guilt trips are manipulation, even when unintentional. If your boundary is reasonable, their guilt-tripping is the problem, not your limit.',
          'silent-treatment':
            'Silent treatment is itself a boundary violation - using withdrawal as punishment. You can acknowledge it without caving.',
          pushback:
            'Some pushback is normal. People resist change. Your job is to hold firm, not convince them.',
          ignore:
            "If they're ignoring your boundary, you may need consequences. Boundaries without enforcement teach people to ignore you.",
        };
...
const healFromBoundaryViolationDef: ToolDefinition = {
...
const recoverFromPeoplePleasingDef: ToolDefinition = {
    ...
    return llm.tool({
      ...
      execute: async ({ awareness, specificPattern }) => {
        ...
        const stageResponses = {
          'just-realized':
            'Recognizing people-pleasing patterns takes courage. This awareness is the first step, not something to feel bad about.',
          'working-on-it':
            "Working on people-pleasing is a process. You're doing the hard work of rewiring patterns that took years to develop.",
          struggling:
            "Recovery from people-pleasing isn't linear. Struggling doesn't mean you're failing - it means you're in the thick of change.",
          'making-progress':
            "Progress in this area is worth celebrating. Every time you choose authenticity over approval, you're healing.",
        };
...
const boundaryInventoryDef: ToolDefinition = {
...
const boundariesTools: ToolDefinition[] = [
