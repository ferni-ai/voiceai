/**
 * Habit Intelligence Domain Tests
 *
 * Tests the "Better Than Human" habit coaching capabilities.
 */
...
vi.mock('../../../../services/superhuman/habit-intelligence-services.js', () => ({
...
describe('habit-intelligence domain', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should have all tools in habit-intelligence domain', async () => {
    const tools = await getToolDefinitions();

    for (const tool of tools) {
      expect(tool.domain).toBe('habit-intelligence');
    }
  });

  it('should have superhuman-habits tag on all tools', async () => {
    const tools = await getToolDefinitions();

    for (const tool of tools) {
      expect(tool.tags).toContain('superhuman-habits');
    }
  });
