 ...
 */
export async function loadUtilityPreferences(userId) {
 ...
 */
export async function saveUtilityPreferences(userId, preferences) {
    const db = await getFirestore();
    if (!db) {
 ...
 */
export async function updateTimerPreferences(userId, timerData) {
    const prefs = await loadUtilityPreferences(userId);
    // Find or create timer entry
    const existing = prefs.timers.usual.find((t) => Math.abs(t.minutes - timerData.minutes) < 0.5 && t.timeOfDay === timerData.timeOfDay);
    if (existing) {
        existing.count++;
        if (timerData.label)
            existing.label = timerData.label;
    }
    else {
 ...
 */
export async function updateTipPreferences(userId, tipPercent) {
    const prefs = await loadUtilityPreferences(userId);
 ...
 */
export async function updateTimezonePreferences(userId, city) {
    const prefs = await loadUtilityPreferences(userId);
 ...
 */
export async function trackCountdown(userId, event, targetDate, notifyMilestones = true) {
    const prefs = await loadUtilityPreferences(userId);
 ...
 */
export async function incrementDecisionCount(userId, type, topics) {
    const prefs = await loadUtilityPreferences(userId);
    if (type === 'coinFlip') {
 ...
 */
export async function syncPatternsToFirestore(userId, patterns) {
    ...
    const updates = {
        timers: {
            usual: patterns.patterns.commonTimerDurations.map((t) => ({
                minutes: t.minutes,
                label: t.label || 'Timer',
                timeOfDay: (t.usualTime || timeOfDay),
                count: t.count,
            })),
 ...
 */
export async function loadPatternsFromFirestore(userId) {
    const prefs = await loadUtilityPreferences(userId);
    return {
