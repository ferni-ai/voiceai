 ...
 */
export function registerVoiceCallbackHandler(handler) {
 ...
 */
export async function triggerVoiceCallback(callback) {
 ...
 */
export async function onTimerComplete(userId, label, durationMinutes) {
    // Build contextual follow-up based on timer type
    let message;
    let followUpQuestion;
    let sound = 'timer-ding';
    ...
    else if (labelLower.includes('cook') ||
    ...
    else if (labelLower.includes('focus') ||
    ...
    else if (labelLower.includes('exercise') ||
 ...
 */
export async function onCountdownMilestone(userId, event, daysRemaining, targetDate) {
    let message;
    let sound = 'soft-ping';
    let priority = 'normal';
    if (daysRemaining === 0) {
 ...
 */
export async function onProactiveSuggestion(userId, suggestion, context) {
    await triggerVoiceCallback({
 ...
 */
export async function onPatternInsight(userId, insight, context) {
    await triggerVoiceCallback({
 ...
 */
export function toVoiceResponse(text, options) {
    let ssml = text;
    // Add natural pauses
 ...
 */
export function speakNumber(num, type = 'cardinal') {
    switch (type) {
        case 'currency':
 ...
 */
export function speakDuration(minutes, seconds = 0) {
    const parts = [];
    if (minutes > 0) {
...
//# sourceMappingURL=voice-callbacks.js.map