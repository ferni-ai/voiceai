 ...
 */
export async function loadLifeContext(userId) {
    const context = {
        upcomingEvents: [],
        activeGoals: [],
        importantPeople: [],
...
async function loadLifePlanningContext(userId, context) {
    try {
...
async function loadMemoryContext(userId, context) {
    try {
        ...
        for (const result of peopleResults) {
            ...
            const personMatch = result.content.match(/(?:my\s+)?(?:friend|partner|spouse|husband|wife|brother|sister|mom|dad|mother|father|parent|child|son|daughter|colleague|boss)\s+(\w+)/i);
            if (personMatch) {
        ...
        for (const result of travelResults) {
            const destMatch = result.content.match(/(?:trip|travel|going|flying|visiting)\s+(?:to\s+)?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/);
            if (destMatch) {
...
async function loadHabitsContext(userId, context) {
    try {
        // Dynamic import
 ...
 */
export function enrichCountdownWithContext(daysUntil, targetDate, lifeContext) {
    // Check if this date is near any known events
    const nearbyEvents = lifeContext.upcomingEvents.filter((event) => {
        const eventDays = Math.ceil((event.date.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
        return Math.abs(eventDays - daysUntil) <= 3; // Within 3 days
    });
    if (nearbyEvents.length > 0) {
        const event = nearbyEvents[0];
        const daysDiff = Math.ceil((targetDate.getTime() - event.date.getTime()) / (1000 * 60 * 60 * 24));
        if (daysDiff === 0) {
    ...
    const nearbyGoals = lifeContext.activeGoals.filter((goal) => {
        if (!goal.targetDate)
            return false;
        const goalDays = Math.ceil((goal.targetDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export function enrichTimezoneWithContext(city, lifeContext) {
    ...
    for (const trip of lifeContext.travelPlans) {
        if (trip.destination.toLowerCase().includes(cityLower)) {
            const daysUntil = Math.ceil((trip.startDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export function enrichTimerWithContext(minutes, label, lifeContext) {
 ...
 */
export function getUpcomingBirthdays(lifeContext, withinDays = 30) {
    const now = new Date();
 ...
 */
export function getUpcomingAnniversaries(lifeContext, withinDays = 30) {
    const now = new Date();
    const currentYear = now.getFullYear();
    const anniversaries = [];
    for (const person of lifeContext.importantPeople) {
...
//# sourceMappingURL=context-integration.js.map