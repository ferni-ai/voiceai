...
// ============================================================================
// JOKE DATABASE (Better than human: personalized, never repeats)
// ============================================================================

interface Joke {
  id: string;
  category: 'dad' | 'pun' | 'one-liner' | 'wholesome' | 'clever' | 'absurd';
  setup: string;
  punchline: string;
}
...
const JOKES: Joke[] = [
  // Dad jokes
  { id: 'dad1', category: 'dad', setup: "Why don't scientists trust atoms?", punchline: "Because they make up everything!" },
  { id: 'dad2', category: 'dad', setup: "What do you call a fake noodle?", punchline: "An impasta!" },
  { id: 'dad3', category: 'dad', setup: "Why did the scarecrow win an award?", punchline: "Because he was outstanding in his field!" },
  { id: 'dad4', category: 'dad', setup: "What do you call a bear with no teeth?", punchline: "A gummy bear!" },
  { id: 'dad5', category: 'dad', setup: "Why don't eggs tell jokes?", punchline: "They'd crack each other up!" },
  { id: 'dad6', category: 'dad', setup: "What do you call a fish without eyes?", punchline: "A fsh!" },
  { id: 'dad7', category: 'dad', setup: "Why did the bicycle fall over?", punchline: "Because it was two-tired!" },
  { id: 'dad8', category: 'dad', setup: "What do you call a sleeping dinosaur?", punchline: "A dino-snore!" },

  // Puns
  { id: 'pun1', category: 'pun', setup: "I'm reading a book about anti-gravity.", punchline: "It's impossible to put down!" },
  { id: 'pun2', category: 'pun', setup: "I used to hate facial hair...", punchline: "But then it grew on me." },
  { id: 'pun3', category: 'pun', setup: "What do you call a parade of rabbits hopping backwards?", punchline: "A receding hare-line!" },
  { id: 'pun4', category: 'pun', setup: "I told my wife she was drawing her eyebrows too high.", punchline: "She looked surprised." },
  { id: 'pun5', category: 'pun', setup: "What do you call a can opener that doesn't work?", punchline: "A can't opener!" },

  // One-liners
  { id: 'one1', category: 'one-liner', setup: "I told my computer I needed a break...", punchline: "Now it won't stop sending me vacation ads." },
  { id: 'one2', category: 'one-liner', setup: "I'm on a seafood diet.", punchline: "I see food and I eat it." },
  { id: 'one3', category: 'one-liner', setup: "Parallel lines have so much in common.", punchline: "It's a shame they'll never meet." },
  { id: 'one4', category: 'one-liner', setup: "I would tell you a joke about pizza...", punchline: "But it's too cheesy." },

  // Wholesome
  { id: 'whole1', category: 'wholesome', setup: "What did the ocean say to the beach?", punchline: "Nothing, it just waved!" },
  { id: 'whole2', category: 'wholesome', setup: "Why do bees have sticky hair?", punchline: "Because they use honeycombs!" },
  { id: 'whole3', category: 'wholesome', setup: "What do you call a happy campfire?", punchline: "A s'more-gasm of joy!" },
  { id: 'whole4', category: 'wholesome', setup: "Why did the teddy bear say no to dessert?", punchline: "Because she was already stuffed!" },

  // Clever
  { id: 'clever1', category: 'clever', setup: "Why do we tell actors to 'break a leg'?", punchline: "Because every play has a cast!" },
  { id: 'clever2', category: 'clever', setup: "I have a joke about chemistry...", punchline: "But I don't think it will get a reaction." },
  { id: 'clever3', category: 'clever', setup: "What's the best thing about Switzerland?", punchline: "I don't know, but the flag is a big plus!" },

  // Absurd
  { id: 'absurd1', category: 'absurd', setup: "What do you call a fish with a bow tie?", punchline: "So-fish-ticated!" },
  { id: 'absurd2', category: 'absurd', setup: "Why did the invisible man turn down the job offer?", punchline: "He couldn't see himself doing it!" },
  { id: 'absurd3', category: 'absurd', setup: "What do you call a dinosaur that crashes their car?", punchline: "Tyrannosaurus Wrecks!" },
];

// Track which jokes each user has heard (better than human memory)
// In-memory cache backed by Firestore
const userJokeHistoryCache = new Map<string, Set<string>>();
const userFactHistoryCache = new Map<string, Set<string>>();
const userStoryHistoryCache = new Map<string, Set<string>>();

// Load humor history from Firestore
async function loadHumorHistory(userId: string, type: 'jokes' | 'facts' | 'stories'): Promise<Set<string>> {
  const cacheMap = type === 'jokes' ? userJokeHistoryCache : type === 'facts' ? userFactHistoryCache : userStoryHistoryCache;
  ...
  try {
    ...
    const doc = await db.collection('bogle_users').doc(userId).collection('humor_history').doc(type).get();
    if (doc.exists) {
      const data = doc.data() as { heard?: string[] } | undefined;
      const heardArray = data?.heard ?? [];
      const heard = new Set<string>(heardArray);
...
async function saveHumorHistory(userId: string, type: 'jokes' | 'facts' | 'stories', heard: Set<string>): Promise<void> {
  const cacheMap = type === 'jokes' ? userJokeHistoryCache : type === 'facts' ? userFactHistoryCache : userStoryHistoryCache;
  cacheMap.set(userId, heard);
...
async function getUnheardJoke(userId: string, category?: Joke['category']): Promise<Joke | null> {
  const heard = await loadHumorHistory(userId, 'jokes');
  
  const available = JOKES.filter(j => 
    !heard.has(j.id) && 
...
interface FunFact {
  id: string;
  category: 'science' | 'history' | 'nature' | 'space' | 'human-body' | 'food' | 'random';
  fact: string;
  source?: string;
}
...
const FUN_FACTS: FunFact[] = [
  // Science
  { id: 'sci1', category: 'science', fact: "Honey never spoils. Archaeologists have found 3,000-year-old honey in Egyptian tombs that was still edible!" },
  { id: 'sci2', category: 'science', fact: "A single bolt of lightning contains enough energy to toast 100,000 slices of bread." },
  { id: 'sci3', category: 'science', fact: "Octopuses have three hearts and blue blood." },
  { id: 'sci4', category: 'science', fact: "Bananas are slightly radioactive because they contain potassium-40." },
  { id: 'sci5', category: 'science', fact: "Water can boil and freeze at the same time. It's called the 'triple point'." },

  // History
  { id: 'hist1', category: 'history', fact: "Cleopatra lived closer in time to the Moon landing than to the construction of the Great Pyramid." },
  { id: 'hist2', category: 'history', fact: "Oxford University is older than the Aztec Empire." },
  { id: 'hist3', category: 'history', fact: "The fax machine was invented the same year people were still traveling the Oregon Trail (1843)." },
  { id: 'hist4', category: 'history', fact: "Nintendo was founded in 1889 - they originally made playing cards!" },

  // Nature
  { id: 'nat1', category: 'nature', fact: "A group of flamingos is called a 'flamboyance'." },
  { id: 'nat2', category: 'nature', fact: "Cows have best friends and get stressed when separated from them." },
  ...
  // Space
  { id: 'space1', category: 'space', fact: "There are more stars in the universe than grains of sand on all of Earth's beaches." },
  { id: 'space2', category: 'space', fact: "A day on Venus is longer than a year on Venus." },
  { id: 'space3', category: 'space', fact: "Neutron stars are so dense that a teaspoon would weigh about 6 billion tons." },
  { id: 'space4', category: 'space', fact: "Space smells like seared steak and raspberries, according to astronauts." },

  // Human body
  { id: 'body1', category: 'human-body', fact: "Your brain uses about 20% of your body's total energy, even though it's only about 2% of your weight." },
  { id: 'body2', category: 'human-body', fact: "You're taller in the morning than at night - gravity compresses your spine throughout the day." },
  { id: 'body3', category: 'human-body', fact: "Your nose can remember 50,000 different scents." },
  { id: 'body4', category: 'human-body', fact: "The human body contains about 37.2 trillion cells." },
  ...
  { id: 'rand2', category: 'random', fact: "A jiffy is an actual unit of time - it's 1/100th of a second." },
  { id: 'rand3', category: 'random', fact: "The opposite sides of a dice always add up to seven." },
  { id: 'rand4', category: 'random', fact: "The shortest war in history lasted 38 to 45 minutes (Britain vs Zanzibar, 1896)." },
];

async function getUnheardFact(userId: string, category?: FunFact['category']): Promise<FunFact | null> {
...
interface MiniStory {
  id: string;
  category: 'adventure' | 'funny' | 'heartwarming' | 'mysterious' | 'wisdom';
  title: string;
  story: string;
}
...
async function getUnheardStory(userId: string, category?: MiniStory['category']): Promise<MiniStory | null> {
  const heard = await loadHumorHistory(userId, 'stories');
  
  const available = MINI_STORIES.filter(s => 
    !heard.has(s.id) && 
...
const tellJokeDef: ToolDefinition = {
  id: 'tellJoke',
  name: 'Tell Joke',
...
const getFunFactDef: ToolDefinition = {
  id: 'getFunFact',
  name: 'Get Fun Fact',
...
const tellMiniStoryDef: ToolDefinition = {
  id: 'tellMiniStory',
...
export const humorToolDefinitions: ToolDefinition[] = [
  tellJokeDef,
  getFunFactDef,
  tellMiniStoryDef,
];
