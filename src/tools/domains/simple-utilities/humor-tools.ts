...
async function saveHumorHistory(
  userId: string,
  type: 'jokes' | 'facts' | 'stories',
  heard: Set<string>
): Promise<void> {
...
async function getUnheardJoke(userId: string, category?: Joke['category']): Promise<Joke | null> {
  ...
  const available = JOKES.filter(
    (j) => !heard.has(j.id) && (category ? j.category === category : true)
  );
...
async function getUnheardFact(
  userId: string,
  category?: FunFact['category']
): Promise<FunFact | null> {
  const heard = await loadHumorHistory(userId, 'facts');

  const available = FUN_FACTS.filter(
    (f) => !heard.has(f.id) && (category ? f.category === category : true)
  );
...
async function getUnheardStory(
  userId: string,
  category?: MiniStory['category']
): Promise<MiniStory | null> {
  const heard = await loadHumorHistory(userId, 'stories');

  const available = MINI_STORIES.filter(
    (s) => !heard.has(s.id) && (category ? s.category === category : true)
  );
