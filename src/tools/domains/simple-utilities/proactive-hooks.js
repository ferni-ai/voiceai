...
export async function evaluateProactiveHooks(context) {
    ...
    if (context.conversationTopics || context.recentUserInput) {
        const conversionOffer = evaluateConversionHook(context.conversationTopics || [], context.recentUserInput);
    ...
    for (const milestone of milestones) {
        ...
        if (countdownOffer && !recentOfferMessages.has(countdownOffer.message)) {
            ...
            if (milestone.daysRemaining <= 1) {
                await onCountdownMilestone(userId, milestone.event, milestone.daysRemaining, milestone.targetDate);
            }
        }
    }
 ...
 */
async function evaluateTimerHook(userId, currentTime) {
    ...
    const usualTimer = prefs.timers.usual.find((t) => t.timeOfDay === timeOfDay && t.count >= 3) ||
 ...
 */
function evaluateConversionHook(topics, recentInput) {
 ...
 */
function evaluateTimezoneHook(userId, travelPlans) {
    const now = new Date();
 ...
 */
function evaluateLifeEventHooks(events, currentTime) {
    const offers = [];
    for (const event of events) {
        const daysUntil = Math.ceil((event.date.getTime() - currentTime.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export async function getProactiveOpener(userId, context) {
    const fullContext = {
        userId,
    ...
    getLogger().info({ userId, offerType: topOffer.type, priority: topOffer.priority }, 'Proactive utility suggestion');
    return topOffer.message;
}
/**
 * Check if we should inject a proactive suggestion mid-conversation
 */
export async function shouldInjectProactiveSuggestion(userId, conversationTurnCount, lastActivityMinutes) {
 ...
 */
export async function runDailyProactiveChecks(userIds) {
...
//# sourceMappingURL=proactive-hooks.js.map