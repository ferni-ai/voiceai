...
function normalizeCurrency(input: string): string | null {
  ...
  // Direct match
  if (CURRENCY_INFO[upper]) return upper;
  ...
  for (const [code, info] of Object.entries(CURRENCY_INFO)) {
    if (info.name.toUpperCase().includes(upper)) return code;
  }
...
const convertCurrencyDef: ToolDefinition = {
  id: 'convertCurrency',
...
const getExchangeRateDef: ToolDefinition = {
  id: 'getExchangeRate',
...
const listCurrenciesDef: ToolDefinition = {
  id: 'listCurrencies',
...
export const currencyToolDefinitions: ToolDefinition[] = [
  convertCurrencyDef,
  getExchangeRateDef,
  listCurrenciesDef,
];
