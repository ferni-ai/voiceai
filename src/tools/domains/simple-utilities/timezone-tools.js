    ...
    create: (_ctx) => {
        return llm.tool({
            ...
            execute: async ({ city }, { ctx: toolCtx }) => {
                ...
                updateTimezonePreferences(userId, cityLower).catch((err) => getLogger().debug({ err }, 'Failed to persist timezone preference'));
                return response;
            },
        });
    },
};
const bestTimeToCallDef = {
    id: 'bestTimeToCall',
            ...
            execute: async ({ theirCity, yourTime }) => {
                ...
                if (yourTime) {
                    ...
                    if (match) {
                        ...
                        const theirHour = new Date(yourDateTime.toLocaleString('en-US', { timeZone: timezone })).getHours();
                        let assessment = '';
                ...
                for (let yourHour = 8; yourHour <= 20; yourHour++) {
                    ...
                    if (theirHour >= 9 && theirHour <= 21) {
                        const yourTimeStr = yourHour <= 12 ? `${yourHour}${yourHour < 12 ? 'am' : 'pm'}` : `${yourHour - 12}pm`;
...
// EXPORTS
// ============================================================================
export const timezoneToolDefinitions = [timeInCityDef, bestTimeToCallDef];
export { bestTimeToCallDef, timeInCityDef };
//# sourceMappingURL=timezone-tools.js.map