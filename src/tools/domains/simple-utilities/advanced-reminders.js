...
// ============================================================================
const locationReminders = new Map();
const recurringReminders = new Map();
async function loadLocationReminders(userId) {
...
async function saveLocationReminder(userId, reminder) {
    // Update cache
...
async function loadRecurringReminders(userId) {
...
async function saveRecurringReminder(userId, reminder) {
    // Update cache
...
const locationReminderDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('locationReminder') ||
...
// ============================================================================
const listLocationRemindersDef = {
...
const recurringReminderDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('recurringReminder') ||
...
// ============================================================================
const listRecurringRemindersDef = {
        ...
        return llm.tool({
            ...
            execute: async ({ type, searchTerm }) => {
                ...
                if (type === 'location') {
                    ...
                    const match = reminders.find((r) => r.active && r.message.toLowerCase().includes(searchLower));
                    if (!match) {
...
function calculateNextOccurrence(pattern, time) {
    const [hours, minutes] = time.split(':').map(Number);
    const now = new Date();
    const next = new Date();
...
function formatNextOccurrence(timestamp) {
    const date = new Date(timestamp);
    const now = new Date();
    const tomorrow = new Date(now);
    tomorrow.setDate(tomorrow.getDate() + 1);
    const timeStr = formatTime(`${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}`);
...
// EXPORTS
// ============================================================================
export const advancedReminderDefinitions = [
    locationReminderDef,
...
//# sourceMappingURL=advanced-reminders.js.map