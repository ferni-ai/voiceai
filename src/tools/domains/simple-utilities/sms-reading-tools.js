 ...
 */
async function fetchRecentMessages(userId, options = {}) {
...
async function getConversations(userId, options = {}) {
    ...
    for (const [phoneNumber, msgs] of conversationMap) {
        const sortedMsgs = msgs.sort((a, b) => new Date(b.dateSent).getTime() - new Date(a.dateSent).getTime());
                ...
                try {
                    if (contact) {
                        ...
                        const formatted = conversations
                            ...
                            const preview = lastMsg.body.length > 50 ? lastMsg.body.slice(0, 47) + '...' : lastMsg.body;
 ...
 */
export const checkNewMessagesDef = {
    id: 'checkNewMessages',
 ...
 * Search messages
 */
export const searchMessagesDef = {
    id: 'searchMessages',
        ...
        return llm.tool({
            ...
            execute: async ({ query, limit = 10 }) => {
                ...
                try {
                    ...
                    const matches = messages.filter((m) => m.body.toLowerCase().includes(query.toLowerCase()));
                    if (matches.length === 0) {
...
// EXPORTS
// ============================================================================
export const smsReadingToolDefinitions = [
    readSMSDef,
    checkNewMessagesDef,
    searchMessagesDef,
];
export default smsReadingToolDefinitions;
//# sourceMappingURL=sms-reading-tools.js.map