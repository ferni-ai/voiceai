...
import { registerVoiceCallbackHandler } from './voice-callbacks.js';
 ...
 */
export async function initializeUtilitiesForSession(userId, options) {
    const log = getLogger();
 ...
 */
export async function endUtilitiesSession(userId) {
    const log = getLogger();
    try {
 ...
 */
export function isSessionInitialized(userId) {
 ...
 */
export async function onConversationStart(userId, voiceHandler) {
 ...
 */
export async function onConversationEnd(userId) {
    await endUtilitiesSession(userId);
}
/**
 * Hook for periodic proactive checks during long conversations
 */
export async function onConversationTick(userId, turnCount, lastActivityMinutes) {
