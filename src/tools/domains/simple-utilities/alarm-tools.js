...
async function saveAlarm(userId, alarm) {
    try {
        const db = getFirestoreDb();
        if (db) {
 ...
 */
function parseRepeatPattern(input) {
    const normalized = input.toLowerCase().trim();
    // Simple patterns
 ...
 */
function formatRepeatForSpeech(alarm) {
    switch (alarm.repeat) {
        case 'once':
            return 'one time';
        case 'daily':
        ...
        return llm.tool({
            ...
            parameters: z.object({
                ...
                repeat: z
                    .string()
                    .optional()
                    .describe('How often to repeat: "once", "daily", "weekdays", "weekends", or specific days like "Monday, Wednesday, Friday"'),
                label: z
                    .string()
                    .optional()
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            parameters: z.object({
                query: z
                    .string()
                    .describe('Which alarm to delete - can be a number from the list, a time like "7am", or a label like "morning"'),
            }),
            execute: async ({ query }) => {
                const alarms = await loadAlarms(ctx.userId);
                if (alarms.length === 0) {
                ...
                const match = alarms.find((a) => a.label.toLowerCase().includes(queryLower) || queryLower.includes(a.label.toLowerCase()));
                if (match) {
                    await deleteAlarm(ctx.userId, match.id);
...
// EXPORTS
// ============================================================================
export const alarmToolDefinitions = [
    setAlarmDef,
    getAlarmsDef,
    deleteAlarmDef,
    snoozeAlarmDef,
...
//# sourceMappingURL=alarm-tools.js.map