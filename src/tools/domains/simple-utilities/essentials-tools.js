                        ...
                        if (history && history.sessions && history.sessions.length > 0) {
                            ...
                            for (const session of history.sessions) {
                                if (session.topicsDiscussed) {
                                    session.topicsDiscussed.forEach((t) => allTopics.add(cleanForFirestore(t)));
                                }
                                if (session.insights) {
                            ...
                            if (topic) {
                                ...
                                const relevantSessions = history.sessions.filter((s) => s.topicsDiscussed?.some((t) => t.toLowerCase().includes(topic.toLowerCase())) ||
                                    s.insights?.some((i) => i.toLowerCase().includes(topic.toLowerCase())));
            ...
            execute: async ({ preferenceType, type, value, customKey }) => {
                ...
                switch (prefType) {
                    case 'temperature':
                        ...
                        break;
                    case 'distance':
                        const distUnit = value.toLowerCase().includes('k') || value.toLowerCase().includes('metric')
                            ? 'kilometers'
                            : 'miles';
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async () => {
                ...
                if (prefs.nickname)
                    response += `• **Name:** ${prefs.nickname}\n`;
                if (prefs.temperatureUnit)
                    response += `• **Temperature:** ${prefs.temperatureUnit}\n`;
                if (prefs.distanceUnit)
                    response += `• **Distance:** ${prefs.distanceUnit}\n`;
                if (prefs.timeFormat)
                    response += `• **Time format:** ${prefs.timeFormat}\n`;
                if (prefs.timezone)
                    response += `• **Timezone:** ${prefs.timezone}\n`;
                if (prefs.language)
                    response += `• **Language:** ${prefs.language}\n`;
                if (prefs.voiceSpeed)
                    response += `• **Voice speed:** ${prefs.voiceSpeed}\n`;
...
// EXPORTS
// ============================================================================
export const essentialsToolDefinitions = [
    whatCanYouDoDef,
    quickCaptureDef,
    recentContextDef,
    setPreferenceDef,
...
//# sourceMappingURL=essentials-tools.js.map