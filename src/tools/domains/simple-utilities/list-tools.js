...
async function loadList(userId, listId) {
    try {
        const db = getFirestoreDb();
        if (db) {
            const doc = await db
...
async function findListByName(userId, name) {
    const lists = await loadLists(userId);
    const nameLower = name.toLowerCase().trim();
    return (lists.find((l) => l.name.toLowerCase() === nameLower || l.name.toLowerCase().includes(nameLower)) || null);
}
// ============================================================================
// HELPERS
// ============================================================================
...
function normalizeListType(input) {
...
function getListEmoji(type) {
    return LIST_TEMPLATES[type]?.emoji || '📝';
}
function formatListForSpeech(list) {
    const emoji = getListEmoji(list.type);
    const unchecked = list.items.filter((i) => !i.checked);
    const checked = list.items.filter((i) => i.checked);
    const lines = [`${emoji} **${list.name}**`];
        ...
        return llm.tool({
            ...
            parameters: z.object({
                ...
                type: z
                    .string()
                    .optional()
                    .describe('The type of list: packing, bucket, guest, reading, movies, gifts, restaurants, or custom'),
    ...
    create: (ctx) => {
        return llm.tool({
            ...
            execute: async ({ listName, item }) => {
                ...
                // Try to find by text
                if (!targetItem) {
                    targetItem = list.items.find((i) => !i.checked && i.text.toLowerCase().includes(itemLower));
                }
                if (!targetItem) {
