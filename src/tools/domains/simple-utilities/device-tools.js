...
async function ringPhone(userId, deviceId) {
...
const findMyPhoneDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('findMyPhone') ||
            ...
            execute: async ({ deviceName }) => {
                ...
                let targetDevice;
                if (deviceName) {
                    targetDevice = phones.find((p) => p.name.toLowerCase().includes(deviceName.toLowerCase()));
                    if (!targetDevice) {
...
const checkBatteryDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('checkBattery') ||
...
const doNotDisturbDef = {
    ...
    create: (ctx) => {
        return llm.tool({
            description: getToolDescription('doNotDisturb') ||
