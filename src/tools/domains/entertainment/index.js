 ...
 */
function createMusicControlTool(legacyTools) {
    return llm.tool({
        description: getToolDescription('musicControl') ||
 ...
 */
function createMusicInfoTool(legacyTools) {
    return llm.tool({
        description: getToolDescription('musicInfo') ||
        ...
        execute: async ({ action, mood }) => {
            ...
            try {
                ...
                if (action === 'suggest') {
                    ...
                    const result = await legacyTools.suggestMusic.execute({ mood: moodText }, {});
                    return result;
                }
...
function getUnifiedMusicToolDefinitions() {
    ...
    log.info({ toolCount: toolNames.length, tools: toolNames }, '🎵 [DIAG] Legacy music tools created');
    // Consolidated: playMusic is main entry, musicControl for pause/resume/stop/volume,
    // musicInfo for status/suggestions
    const definitions = [
        wrapLegacyTool('playMusic', 'Play Music', 'Play music by song, artist, genre, or mood. Searches music catalog and starts playback. Returns track information and playback status.', legacyTools.playMusic, { tags: ['playback', 'primary', 'search', 'suggestions'] }),
        // FIXED: musicControl now properly dispatches based on action parameter
        {
            id: 'musicControl',
            name: 'Music Control',
            description: 'Control music playback: pause, resume, stop, skip, or adjust volume. Actions: "pause", "resume", "stop", "skip", or "volume" (with level 0-100).',
            domain: 'entertainment',
            tags: ['entertainment', 'music', 'playback', 'pause', 'resume', 'stop', 'volume'],
            create: (_ctx) => createMusicControlTool(legacyTools),
        },
        // FIXED: musicInfo now properly dispatches based on action parameter
        {
            id: 'musicInfo',
            name: 'Music Info',
            description: 'Get info about current music: what\'s playing now, or get suggestions for a mood/activity. Actions: "playing" (current track info) or "suggest" (recommendations based on mood).',
            domain: 'entertainment',
            tags: ['entertainment', 'music', 'status', 'suggestions', 'info'],
            create: (_ctx) => createMusicInfoTool(legacyTools),
        },
        wrapLegacyTool('musicProvider', 'Music Provider', 'Switch between music providers. Actions: "spotify" (requires Premium account for full songs) or "free" (30-second previews, works for everyone).', legacyTools.useSpotify, { tags: ['provider', 'spotify', 'free'] }),
    ];
    log.info({ definitionCount: definitions.length, ids: definitions.map((d) => d.id) }, '🎵 [DIAG] Unified music tool definitions created');
    return definitions;
}
// ============================================================================
...
// ============================================================================
function getSpotifyToolDefinitions() {
    log.info('🎵 [DIAG] getSpotifyToolDefinitions() called');
    const legacyTools = createSpotifyTools();
    const toolNames = Object.keys(legacyTools);
    log.info({ toolCount: toolNames.length, tools: toolNames }, '🎵 [DIAG] Legacy Spotify tools created');
    // Consolidated: spotifyAdvanced for transfer/search/details, callMusic for in-call audio,
    // spotifyStatus for health/status
    const definitions = [
        wrapLegacyTool('spotifyAdvanced', 'Spotify Advanced', 'Advanced Spotify features: transfer playback to another device, search Spotify library, skip songs, or get detailed info about current song/artist/album. Actions: "transfer", "search", "skip", or "details". Requires linked Spotify Premium.', legacyTools.transferMusic, { tags: ['spotify', 'transfer', 'search', 'skip', 'details'], requiredServices: ['spotify'] }),
        wrapLegacyTool('callMusic', 'Call Music', 'Play background music during voice calls: play preview clips, pause, resume, stop, or adjust volume. Actions: "play", "pause", "resume", "stop", or "volume". Great for hold music or ambiance.', legacyTools.playPreview, { tags: ['call', 'preview', 'background', 'ambient'] }),
        wrapLegacyTool('spotifyStatus', 'Spotify Status', 'Check Spotify connection status and health. Returns current playback state, linked account info, and connection health.', legacyTools.checkSpotifyHealth, { tags: ['spotify', 'status', 'health'], requiredServices: ['spotify'] }),
    ];
    log.info({ definitionCount: definitions.length, ids: definitions.map((d) => d.id) }, '🎵 [DIAG] Spotify tool definitions created');
    return definitions;
}
// ============================================================================
// SONOS MUSIC TOOLS
// ============================================================================
function getSonosMusicToolDefinitions() {
    return [
        {
            id: 'playSonosMusic',
            name: 'Play Music on Sonos',
            description: 'Play music on Sonos speakers by searching favorites. Use when user says "play jazz on Sonos", "play music in living room", etc.',
            domain: 'entertainment',
...
// MOVIE TOOLS
// ============================================================================
function getMovieToolDefinitions() {
    const legacyTools = createMovieTools();
    return [
        wrapLegacyTool('getMovieInfo', 'Get Movie Info', 'Get information about a specific movie including rating, runtime, genres, and description.', legacyTools.getMovieInfo, { tags: ['movie', 'film', 'info'] }),
        wrapLegacyTool('getMoviesNowPlaying', 'Movies Now Playing', 'Get a list of movies currently playing in theaters.', legacyTools.getMoviesNowPlaying, { tags: ['movie', 'theater', 'now playing'] }),
        wrapLegacyTool('getUpcomingMovies', 'Upcoming Movies', 'Get a list of upcoming movies coming to theaters soon.', legacyTools.getUpcomingMovies, { tags: ['movie', 'upcoming', 'coming soon'] }),
        wrapLegacyTool('getMovieShowtimes', 'Movie Showtimes', 'Get showtime information for a movie in a specific location.', legacyTools.getMovieShowtimes, { tags: ['movie', 'showtimes', 'theater'] }),
    ];
}
// ============================================================================
...
// EXPORTS
// ============================================================================
export const { getToolDefinitions, domain, definitions } = createDomainExport('entertainment', entertainmentTools);
