...
import {
  ...
  setLastUsedRoom,
  searchFavorites,
  type SonosRoomConfig,
} from '../../../services/smart-home/sonos-music.js';
import type { SonosCredentials } from '../../../services/smart-home/sonos.js';
 ...
 */
async function getSonosCredentials(userId: string): Promise<SonosCredentials | null> {
  try {
 ...
 */
export function createSonosMusicTools(userId: string) {
  return {
    /**
    ...
    setSonosRoom: llm.tool({
      description:
        'Set the default Sonos room for music playback. Future "play on Sonos" commands will use this room unless another is specified.',
      parameters: z.object({
      ...
      execute: async ({ room }) => {
        ...
        const match = rooms.find(
          (r) =>
            r.groupName.toLowerCase().includes(normalizedRoom) ||
    ...
    listSonosRooms: llm.tool({
      description:
        'List all available Sonos rooms/speakers. Use this when the user asks about their Sonos setup or which rooms are available.',
      parameters: z.object({}),
      execute: async () => {
        const credentials = await getSonosCredentials(userId);
        if (!credentials) {
