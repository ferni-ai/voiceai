...
import {
  ...
  deleteRoom,
  getRoomConfig,
} from '../../../services/identity/spotify-room-service.js';
...
const playMusicInRoomDef: ToolDefinition = {
  id: 'playMusicInRoom',
  name: 'Play Music in Room',
  description:
    'Play music in a specific room or on "everywhere". Routes to the correct Spotify Connect device(s).',
  ...
  create: (ctx: ToolContext) => {
    return llm.tool({
      description:
        getToolDescription('playMusicInRoom') ||
      ...
      parameters: z.object({
        room: z
          .string()
          .describe(
            'Room name (e.g., "living room", "kitchen", "bedroom", "everywhere", or a group name)'
          ),
...
const transferToRoomDef: ToolDefinition = {
  id: 'transferToRoom',
  ...
  create: (ctx: ToolContext) => {
    return llm.tool({
      description:
        getToolDescription('transferToRoom') ||
...
const setRoomVolumeDef: ToolDefinition = {
  id: 'setRoomVolume',
  ...
  create: (ctx: ToolContext) => {
    return llm.tool({
      description:
        getToolDescription('setRoomVolume') ||
...
const listRoomsDef: ToolDefinition = {
  id: 'listMusicRooms',
  ...
  create: (ctx: ToolContext) => {
    return llm.tool({
      description:
        getToolDescription('listMusicRooms') || 'List your configured music rooms and room groups.',
      parameters: z.object({}),
      execute: async () => {
...
const syncRoomsDef: ToolDefinition = {
  id: 'syncMusicRooms',
  ...
  create: (ctx: ToolContext) => {
    return llm.tool({
      description:
        getToolDescription('syncMusicRooms') ||
...
const discoverDevicesDef: ToolDefinition = {
  ...
  create: (ctx: ToolContext) => {
    return llm.tool({
      description:
        getToolDescription('discoverSpotifyDevices') ||
      ...
      execute: async ({ showUnassignedOnly }) => {
        ...
        if (!result.success || !result.data) {
          return (
            result.error || "Couldn't discover devices. Is Spotify running on any of your devices?"
          );
        }
...
export const spotifyConnectTools: ToolDefinition[] = [
  playMusicInRoomDef,
  transferToRoomDef,
  setRoomVolumeDef,
  listRoomsDef,
