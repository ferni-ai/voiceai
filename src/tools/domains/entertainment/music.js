...
import { getMusicPlayer, isMusicAvailable, getDJController, } from '../../../audio/index.js';
...
import { getSpotifyAccessToken, isSpotifyConfigured, } from '../../../services/identity/spotify-auth.js';
import { extractRoomFromQuery, mentionsSonos, } from '../../../services/music/music-provider.js';
import { getAirDJMoment, getDancingComment, getExcitedMusicReaction, getFunDJMoment, getGenreReaction, getMoodMusicReaction, getMusicReaction, getPlayfulMusicComment, getPlayfulMusicIntro, shouldReactToMusic, } from '../../../speech/music-reactions.js';
...
let musicConfig = { ...DEFAULT_MUSIC_CONFIG };
/**
 ...
 */
export function detectMusicIntent(query) {
 ...
 */
export async function playMusicUnified(query) {
    const log = getLogger();
    ...
    if (!musicAvailability.available) {
        log.warn({ query, reason: musicAvailability.reason }, '🎵 Music playback NOT available for this session');
 ...
 */
async function playAmbientMusic(query) {
    const log = getLogger();
    ...
    const musicAvailability = isMusicAvailable();
    if (!musicAvailability.available) {
        log.warn({ query, reason: musicAvailability.reason }, '🎧 Music player not available for ambient mode');
 ...
 */
async function playViaItunesWithListeningFallback(query) {
    const log = getLogger();
 ...
 */
export async function playViaItunes(query, personaId) {
    const log = getLogger();
    ...
    try {
        ...
        if (!track.previewUrl) {
            log.warn({ track: track.name, artist: track.artist }, '🎵 [iTunes] Track found but no preview URL available!');
        ...
        if (!currentMusicPlayer.isInitialized()) {
            ...
            if (!initialized) {
                ...
                if (sessionId === null) {
                    ...
                    log.error('🎵 [DIAG] CRITICAL: Music player sessionId is NULL - dispose() was likely called!');
            ...
            log.info({ elapsed: Date.now() - startTime }, '🎵 [DIAG] Music player initialization complete, proceeding with playback');
        }
        ...
        if (wasPlaying && previousTrack) {
            ...
            log.info({ from: previousTrack.name, to: track.name }, '🎧 DJ Crossfade: Switching tracks smoothly');
 ...
 */
async function playViaSpotify(query) {
    ...
    try {
        ...
        const playMusicTool = tools.playMusic;
 ...
 */
export async function suggestAndPlayMusic(mood) {
    const log = getLogger();
    // 🐛 FIX: Check if music is enabled FIRST before doing anything
    if (!isMusicEnabled()) {
    ...
    return {
        playMusic: llm.tool({
            ...
            execute: async ({ query }) => {
                ...
                try {
                    ...
                    log.error({ query, error: String(error), stack: error.stack?.slice(0, 300) }, '🎵 [DIAG] playMusic EXCEPTION');
        ...
        suggestMusic: llm.tool({
            ...
            parameters: z.object({
                mood: z
                    .string()
                    .describe('Mood or activity: focus, relaxing, energizing, stressed, celebrating, workout, sleep'),
            }),
            execute: async ({ mood }) => {
        ...
        myMusicPreferences: llm.tool({
            description: "Get the user's learned music preferences including favorite genres, artists, and dislikes. " +
