...
async function getAppleMusicService() {
  try {
    const {
      ...
      getAppleMusicTrack,
    } = await import('../../../services/apple/musickit.js');
    return {
      isAppleMusicAvailable,
      searchAppleMusic,
      formatSearchResultsForVoice,
...
export const appleMusicTools: ToolDefinition[] = [
  {
    id: 'searchAppleMusic',
  ...
  },
  {
    ...
    create: (_ctx: ToolContext) =>
      llm.tool({
        description:
          'Play a 30-second preview of a song from Apple Music. Search first to get track info.',
        parameters: z.object({
        ...
        execute: async ({ query }) => {
          ...
          try {
            ...
            log.info(
              { track: track.name, artist: track.artistName },
              '🍎 Apple Music preview playing'
            );
