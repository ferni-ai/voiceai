...
import { getSpotifyAccessToken, getSpotifyHealthStatus, getSpotifyTokenStatus, isSpotifyConfigured, logSpotifyDiagnostics, recordSpotifyError, startAutoRefresh, stopAutoRefresh, } from '../../../services/identity/spotify-auth.js';
 ...
 */
function recordSuccessfulDevice(deviceId, deviceName) {
...
async function getAvailableDevices() {
    ...
    try {
        ...
        getLogger().warn({ error: error instanceof Error ? error.message : error }, '🎵 Failed to get Spotify devices - user may need to open Spotify app');
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (targetDevice?.is_active) {
                getLogger().info({ deviceId, deviceName: targetDevice.name, attempts: attempt }, '🎵 ✅ Device woke up successfully');
                return true;
            }
 ...
 */
async function wakeAnyDevice(devices) {
    ...
    if (wokenDevice) {
        getLogger().info({ deviceId: wokenDevice.id, deviceName: wokenDevice.name }, '🎵 ✅ Successfully woke a device');
...
async function checkPremiumStatus() {
    ...
    try {
        ...
        if (!hasPremium) {
            getLogger().info('🎵 Spotify account is not Premium - will use iTunes previews for in-call music');
        }
        else {
 ...
 */
function getPremiumStatusCached() {
    if (hasPremium !== null && Date.now() - premiumCheckTimestamp < PREMIUM_CHECK_CACHE_TTL_MS) {
        return hasPremium;
    }
    return null;
 ...
 */
async function getBestPlaybackDevice() {
    ...
    if (webDeviceId) {
        ...
        if (isPremium) {
            ...
            getLogger().info({ webDeviceId, otherDevicesCount: otherDevices.length }, '🎵 Using web player (where user is talking)');
    ...
    if (lastUsedDeviceId) {
        ...
        if (lastDevice) {
            ...
            getLogger().info({ deviceId: lastUsedDeviceId, deviceName: lastDevice.name }, '🎵 Using last-used device');
            return {
    ...
    if (devices.length > 0) {
        ...
        getLogger().info({ deviceId: firstDevice.id, deviceName: firstDevice.name }, '🎵 No device responded to wake, trying first available anyway');
        return {
 ...
 */
async function spotifyRequest(endpoint, method = 'GET', body, options) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (response.ok) {
                ...
                getLogger().debug({ dataKeys: Object.keys(data) }, '🎵 Spotify API response data');
                return data;
            }
            ...
            if (response.status >= 500) {
                ...
                getLogger().warn({ status: response.status, backoff, attempt }, '🎵 Server error, backing off...');
                await sleep(backoff);
                lastError = new Error(`Server error: ${response.status}`);
                continue;
            }
            // Other errors - don't retry (404, 400, etc.)
            getLogger().error({ status: response.status, error: errorText.slice(0, 200) }, '🎵 ❌ Spotify API error');
            ...
            if (lastError.message.includes('fetch') || lastError.message.includes('network')) {
                ...
                getLogger().warn({ error: lastError.message, backoff, attempt }, '🎵 Network error, retrying...');
                await sleep(backoff);
                continue;
            }
            // Re-throw non-retryable errors
            if (!lastError.message.includes('Token expired') &&
...
async function searchTracks(query, limit = 5) {
    try {
        const result = (await spotifyRequest(`/search?q=${encodeURIComponent(query)}&type=track&limit=${limit}`));
 ...
 */
export async function searchTracksWithPreviews(query, limit = 5) {
    try {
        const result = (await spotifyRequest(`/search?q=${encodeURIComponent(query)}&type=track&limit=${limit * 2}` // Fetch more to filter
        ));
        const tracks = result.tracks?.items || [];
 ...
 */
async function playViaItunesFallback(query) {
    const log = getLogger();
 ...
 */
async function playMusic(query, streamIntoCallOverride) {
    const log = getLogger();
    ...
    const isPremium = await checkPremiumStatus();
    try {
        getLogger().info({ shouldStreamIntoCall }, `🎵 Mode: ${shouldStreamIntoCall ? 'STREAM INTO CALL' : 'SPOTIFY CONNECT'}`);
        ...
        if (shouldStreamIntoCall) {
            ...
            if (!musicPlayer.isInitialized()) {
                ...
                const initialized = await musicPlayer.waitForInitialization(5000);
                if (!initialized) {
                    getLogger().error('🎵 [Spotify] Music player not initialized after waiting! Cannot stream preview.');
 ...
 */
export function setStreamIntoCall(enabled) {
 ...
 */
function getJackMusicSuggestion(mood) {
    const suggestions = {
        focus: [
    ...
    if (moodLower.includes('work') || moodLower.includes('concentrate'))
        moodKey = 'focus';
    ...
    if (moodLower.includes('old') || moodLower.includes('memory') || moodLower.includes('remember'))
        moodKey = 'nostalgic';
    if (moodLower.includes('deep') || moodLower.includes('philosoph'))
        moodKey = 'thinking';
...
export function createSpotifyTools() {
    ...
    if (isSpotifyConfigured()) {
        ...
        getLogger().warn('🎵 Spotify not configured at startup.\n' +
            '   To enable Spotify:\n' +
    ...
    const buildErrorMessage = (context) => {
        const health = getSpotifyHealthStatus();
    ...
    return {
        ...
        suggestMusic: llm.tool({
            ...
            parameters: z.object({
                mood: z
                    .string()
                    .describe('Current mood or activity (e.g., "focus", "relaxing", "energizing", "stressed", "celebrating")'),
            }),
            execute: async ({ mood }) => {
        ...
        playPreview: llm.tool({
            ...
            execute: async ({ query }) => {
                if (!checkConfigured())
                    return "Spotify isn't connected for previews.";
        ...
        transferMusic: llm.tool({
            ...
            parameters: z.object({
                deviceName: z
                    .string()
                    .optional()
                    .describe('Target device name (e.g., "car", "phone", "speaker"). If empty, will list available devices.'),
            }),
            execute: async ({ deviceName }) => {
                ...
                try {
                    ...
                    const targetDevice = devices.find((d) => d.name.toLowerCase().includes(deviceName.toLowerCase()) ||
                    ...
                    getLogger().info({ targetDevice: targetDevice.name, deviceId: targetDevice.id }, '🎵 Transferred music to device');
 ...
 */
export function shutdownSpotify() {
    stopAutoRefresh();
