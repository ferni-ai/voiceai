...
import { analyzeMessage, saveDraft, getPendingDrafts, getDraftsReadyForReview, approveDraft, discardDraft, getTimeUntilReview, formatAnalysisForSpeech, } from '../../../services/message-validation/message-validation-service.js';
 ...
 */
export function getMessageValidationToolDefinitions() {
    return [
        // =========================================================================
            ...
            create: (ctx) => {
                return llm.tool({
                    ...
                    execute: async (params) => {
                        ...
                        const pending = await getPendingDrafts(userId);
                        const draft = pending.find((d) => d.recipient.toLowerCase().includes(params.recipient.toLowerCase()));
                        if (!draft) {
...
//# sourceMappingURL=message-validation-tools.js.map