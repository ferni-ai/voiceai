...
registerInterval('batch-outreach-cleanup', () => {
    const now = Date.now();
...
// HELPER FUNCTIONS
// ============================================================================
function occasionToIntent(occasion) {
    switch (occasion) {
        case 'christmas':
        case 'new_year':
        case 'thanksgiving':
...
function occasionToPurpose(occasion) {
    switch (occasion) {
        case 'christmas':
...
async function craftBatchMessage(contact, occasion, customMessage, personaId, channel) {
    if (customMessage) {
...
// ============================================================================
export function createPreviewBatchTool(ctx) {
    return llm.tool({
...
before I send, and you can edit any message.`,
        ...
        execute: async (params) => {
            ...
            try {
                ...
                const group = groups.find((g) => g.name.toLowerCase() === groupOrContacts.toLowerCase() ||
            ...
            if (contacts.length === 0) {
                return (`I couldn't find any contacts matching "${groupOrContacts}". ` +
            ...
            for (const contact of contacts) {
                ...
                const message = await craftBatchMessage(contact, occasion, customMessage, personaId, useChannel);
                previews.push({
...
// SEND BATCH MESSAGES
// ============================================================================
export function createSendBatchTool(ctx) {
    return llm.tool({
...
// ============================================================================
export function createOutreachSuggestionsTool(ctx) {
    return llm.tool({
            ...
            try {
                ...
                for (const contact of contacts) {
                    ...
                    const daysSince = contact.lastInteraction
                        ? Math.floor((now.getTime() - contact.lastInteraction.getTime()) / (1000 * 60 * 60 * 24))
                        : 999;
                    // Pending follow-up
...
// ============================================================================
export function getBatchOutreachDefinitions() {
    return [
        {
...
//# sourceMappingURL=batch-outreach.js.map