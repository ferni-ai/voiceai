...
// ============================================================================
export async function craftPersonalizedMessage(ctx) {
    const { contactName, purpose, intent, recentTopics, daysSinceLastContact, relationshipStrength, pendingFollowUp, personaId, channel = 'text', } = ctx;
    const firstName = contactName.split(' ')[0];
    // Build context for the LLM
    const relationshipDescription = relationshipStrength >= 80
        ? 'very close'
        : relationshipStrength >= 60
            ...
            : relationshipStrength >= 40
                ...
                : 'casual contacts';
    const timeSince = daysSinceLastContact === 0
        ? 'talked today'
...
function useFallbackTemplate(name, intent, purpose) {
...
function extractOccasion(purpose) {
    const p = purpose.toLowerCase();
    if (p.includes('birthday'))
        return 'birthday';
    if (p.includes('new year'))
        return 'New Year';
    if (p.includes('christmas'))
        return 'Christmas';
    if (p.includes('thanksgiving'))
        return 'Thanksgiving';
    if (p.includes('anniversary'))
        return 'anniversary';
    if (p.includes('holiday'))
        return 'holidays';
    return 'day';
}
 ...
 */
export async function craftConversationOpener(ctx) {
 ...
 */
export async function craftFollowUpMessage(ctx, originalTopic) {
    return craftPersonalizedMessage({
        ...ctx,
        intent: 'follow_up',
 ...
 */
export async function craftThinkingOfYouMessage(ctx) {
...
//# sourceMappingURL=message-crafting.js.map