...
import { searchContacts, recordInteraction, } from '../../../../services/contacts/contact-relationship-service.js';
...
// INTENT DETECTION
// ============================================================================
function detectIntent(purpose) {
    const p = purpose.toLowerCase();
    // Wish well patterns
    if (/\b(happy birthday|birthday|congratulat|good (morning|afternoon|evening)|wish(ing)?|great day|best wishes|get well|feel better|thinking of you)\b/.test(p)) {
        return 'wish_well';
    }
    // Thank you
    if (/\b(thank|grateful|appreciate|gratitude)\b/.test(p))
        return 'thank_you';
    // Apology
    if (/\b(sorry|apologize|apology|forgive)\b/.test(p))
        return 'apology';
    // Important/urgent
    if (/\b(urgent|important|emergency|asap|need to|must)\b/.test(p))
        return 'important';
    // Question
    if (/\b(ask|question|wondering|know if|find out)\b/.test(p))
        return 'ask_question';
    // Follow up
    if (/\b(follow up|following up|check(ing)? (back|in)|how did|how was)\b/.test(p))
        return 'follow_up';
    // Schedule
    if (/\b(schedule|plan|set up|arrange|meet|appointment|calendar)\b/.test(p))
        return 'schedule';
    // Share
    if (/\b(share|tell|let .* know|update|news)\b/.test(p))
        return 'share_news';
    // Reminder
    if (/\b(remind|reminder|don't forget|remember)\b/.test(p))
        return 'reminder';
    // Check in
    if (/\b(check in|checking in|see how|how (are|is)|doing)\b/.test(p))
        return 'check_in';
    return 'just_because';
}
// ============================================================================
...
// ============================================================================
function needsConversation(intent, purpose) {
...
function selectOptimalChannel(ctx) {
    ...
    const available = channelList.filter((ch) => {
        if (ch === 'email')
            return hasEmail;
        if (['call', 'conversation', 'voicemail', 'text'].includes(ch))
            return hasPhone;
        return true;
    });
...
// ============================================================================
async function gatherOutreachContext(userId, contact, purpose, personaId) {
    const now = new Date();
    ...
    const pendingFollowUp = contact.pendingFollowUp?.completed === false ? contact.pendingFollowUp.reason : undefined;
    return {
        contact,
...
// OUTREACH EXECUTION
// ============================================================================
async function executeOutreach(ctx, decision, customMessage) {
    ...
    try {
        ...
        if (channel === 'conversation') {
            ...
            const callContext = {
                ...
                user: {
                    ...
                    name: contact.name,
                    phone: contact.phone,
                    relationshipStage: ctx.relationshipStrength >= 80
                        ? 'deep'
            ...
            if (result.status === 'initiating' || result.status === 'ringing') {
                ...
                }).catch((err) => log.debug({ error: String(err), contactId: contact.id }, 'Failed to record call interaction (non-fatal)'));
                return {
                    success: true,
                    channel,
        ...
        if (channel === 'call' || channel === 'voicemail') {
            ...
            if (result.success) {
                ...
                }).catch((err) => log.debug({ error: String(err), contactId: contact.id }, 'Failed to record voice message interaction (non-fatal)'));
                return {
                    success: true,
                    channel,
        ...
        if (channel === 'text') {
            ...
            if (result.success) {
                ...
                }).catch((err) => log.debug({ error: String(err), contactId: contact.id }, 'Failed to record text interaction (non-fatal)'));
                return {
                    success: true,
                    channel,
        ...
        if (channel === 'email') {
            ...
            if (result.success) {
                ...
                }).catch((err) => log.debug({ error: String(err), contactId: contact.id }, 'Failed to record email interaction (non-fatal)'));
                return {
                    success: true,
                    channel,
...
function generateSubject(intent, purpose, name) {
    const firstName = name.split(' ')[0];
    switch (intent) {
        case 'wish_well':
            if (purpose.toLowerCase().includes('birthday'))
...
// MAIN TOOL CREATOR
// ============================================================================
export function createUnifiedOutreachTool(ctx) {
    return llm.tool({
...
Just tell me WHO and WHY. I'll handle the rest.`,
        parameters: z.object({
            ...
            purpose: z
                .string()
                .describe('Why you want to reach out - be natural: "wish her happy birthday", "see how he\'s doing"'),
            preferredChannel: z
        ...
        execute: async (params) => {
            ...
            if (!userId)
                return 'I need to know who you are to reach out on your behalf.';
            ...
            if (!contactInfo) {
                return (`I don't have ${contact} in your contacts. ` +
                    `Can you give me their phone number or email, or should I save them as a new contact?`);
            }
            if (!contactInfo.phone && !contactInfo.email) {
                return (`I found ${contactInfo.name}, but I don't have their phone or email. ` +
...
// ============================================================================
export function getUnifiedOutreachDefinition() {
    return {
        id: 'reachOut',
        name: 'Reach Out',
        description: "Reach out to someone - I'll choose the best channel, craft a personalized message, and handle the delivery.",
...
//# sourceMappingURL=unified-outreach.js.map