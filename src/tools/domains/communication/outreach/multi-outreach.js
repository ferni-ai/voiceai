 ...
 */
async function resolveContact(userId, target) {
    const contactQuery = target.contact.toLowerCase().trim();
    // 1. Try as a group first
    try {
        const groups = await getGroups(userId);
        const matchedGroup = groups.find((g) => g.name.toLowerCase() === contactQuery ||
 ...
 */
async function resolveAllContacts(userId, targets) {
 ...
 */
function selectChannel(contact, preferredChannel, purpose) {
    // Explicit preference
    ...
    if (purposeLower.includes('check in') ||
...
function detectIntent(purpose) {
    ...
    if (/\b(thank|grateful|appreciate)\b/.test(p))
        return 'thank_you';
    if (/\b(sorry|apologize|apology)\b/.test(p))
        return 'apology';
    if (/\b(urgent|important|emergency)\b/.test(p))
        return 'important';
    if (/\b(ask|question|wondering)\b/.test(p))
        return 'ask_question';
    if (/\b(follow up|following up|check back)\b/.test(p))
        return 'follow_up';
    if (/\b(schedule|plan|meet|appointment)\b/.test(p))
        return 'schedule';
    if (/\b(share|tell|let .* know|update)\b/.test(p))
        return 'share_news';
    if (/\b(remind|reminder)\b/.test(p))
        return 'reminder';
    if (/\b(check in|checking in|see how|how are)\b/.test(p))
        return 'check_in';
    return 'just_because';
}
// ============================================================================
 ...
 */
async function executeOutreach(userId, personaId, contact, target, defaultPurpose) {
    ...
    if (!message) {
        ...
        const ctx = {
            ...
            daysSinceLastContact: Math.floor((Date.now() - (contact.lastInteraction?.getTime() || Date.now())) / (1000 * 60 * 60 * 24)),
    ...
    log.info({ userId, contact: contact.name, channel, purpose }, '🤝 Executing multi-outreach target');
    try {
...
function generateSubject(purpose, name) {
 ...
 */
async function executeMultiOutreach(userId, personaId, targets, defaultPurpose) {
    const results = [];
    ...
    const immediateResults = await Promise.all(immediate.map(({ target, contact }) => executeOutreach(userId, personaId, contact, target, defaultPurpose)));
...
// TOOL CREATOR
// ============================================================================
export function createMultiOutreachTool(ctx) {
    return llm.tool({
...
- "Text Sarah now, call Mom in an hour" → mixed timing`,
        ...
        execute: async (params) => {
            ...
            log.info({ userId, targetCount: targets.length, defaultPurpose }, '🤝 Multi-outreach initiated');
...
// ============================================================================
export function getMultiOutreachDefinition() {
    return {
        id: 'multiOutreach',
...
//# sourceMappingURL=multi-outreach.js.map