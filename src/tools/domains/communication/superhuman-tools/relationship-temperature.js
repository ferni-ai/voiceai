 ...
 */
function calculateTemperature(sentiment, daysSinceLastMention, recentSentiments) {
    ...
    const avgRecent = recentSentiments.length > 0
 ...
 */
export async function updateTemperature(userId, contactName, sentiment, event) {
    ...
    try {
        ...
        if (previousTemp - newTemp >= ALERT_THRESHOLD_COOLING && trend === 'cooling') {
            const existingCoolingAlert = alerts.find((a) => a.type === 'cooling' && Date.now() - a.createdAt < 7 * 24 * 60 * 60 * 1000);
        ...
        if (history.length >= 10 &&
            ...
            trend === 'cooling') {
            const existingDriftAlert = alerts.find((a) => a.type === 'drift' && Date.now() - a.createdAt < 14 * 24 * 60 * 60 * 1000);
 ...
 */
export async function getTemperature(userId, contactName) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return null;
        ...
        data.daysSinceLastInteraction = Math.floor((Date.now() - data.lastInteraction) / (24 * 60 * 60 * 1000));
        // Add neglect alert if needed
        if (data.daysSinceLastInteraction >= ALERT_THRESHOLD_NEGLECT &&
...
export async function getRelationshipsNeedingAttention(userId) {
    try {
        ...
        return allTemps.filter((t) => {
            // Cooling trend
            if (t.trend === 'cooling' && t.trendStrength > 0.3)
                return true;
            // Low temperature
            if (t.currentTemperature < 40)
                return true;
            // Neglected
            if (t.daysSinceLastInteraction >= ALERT_THRESHOLD_NEGLECT)
                return true;
            // Has active alerts
 ...
 */
export async function buildTemperatureContext(userId) {
    ...
    const moderate = needingAttention.filter((t) => !urgent.includes(t) && t.alerts.some((a) => a.severity === 'medium'));
 ...
 */
export async function buildContactTemperatureContext(userId, contactName) {
    const temp = await getTemperature(userId, contactName);
    if (!temp)
        return '';
    const sections = [`[RELATIONSHIP TEMPERATURE - ${contactName}]`];
    // Temperature reading
 ...
 */
export async function processEventForTemperature(userId, event) {
...
//# sourceMappingURL=relationship-temperature.js.map