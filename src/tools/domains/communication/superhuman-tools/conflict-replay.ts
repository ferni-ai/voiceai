...
export async function recordConflict(
  userId: string,
  conflict: Omit<ConflictRecord, 'id' | 'recordedAt'>
...
export async function getConflictHistory(
  ...
  try {
    ...
    return snapshot.docs.map((doc) => doc.data() as ConflictRecord);
  } catch (error) {
 ...
 */
export async function getAllConflicts(userId: string): Promise<ConflictRecord[]> {
  try {
    const db = getFirestoreDb();
    if (!db) return [];
 ...
 */
export function analyzeForEscalation(transcript: string): {
  escalationPoints: EscalationPoint[];
  deEscalationMoments: Array<{ phrase: string; effect: string }>;
  overallRisk: 'low' | 'medium' | 'high';
  suggestions: string[];
} {
  const escalationPoints: EscalationPoint[] = [];
  const deEscalationMoments: Array<{ phrase: string; effect: string }> = [];
  ...
  if (adjustedRisk > 6) overallRisk = 'high';
  else if (adjustedRisk > 3) overallRisk = 'medium';
 ...
 */
export function reconstructConflict(
  description: string,
  userPhrases: string[],
  otherPhrases: string[]
): {
  ...
  const highRiskPoints = timeline.filter(
    (t) => t.speaker === 'user' && t.escalationRisk && t.escalationRisk >= 6
  );
 ...
 */
export async function analyzeConflictPatterns(
  userId: string,
  contactName: string
): Promise<{
  totalConflicts: number;
  recurringTriggers: Array<{ trigger: string; count: number }>;
  ...
  if (recurringTriggers.length > 0) {
    recommendations.push(
      `Your conflicts with ${contactName} often involve "${recurringTriggers[0].trigger}". Working on this pattern could reduce friction.`
    );
  }

  if (resolutionRate < 0.5) {
    recommendations.push(
      `Only ${Math.round(resolutionRate * 100)}% of conflicts with ${contactName} reach resolution. Consider new approaches.`
    );
  }

  if (commonTopics.length > 0) {
    recommendations.push(
      `${commonTopics[0]} is a recurring conflict topic. Is there an underlying issue that needs addressing?`
    );
  }
...
export async function buildConflictContext(userId: string, contactName?: string): Promise<string> {
  ...
  // Group by contact
  const byContact = new Map<string, ConflictRecord[]>();
 ...
 */
export function generateReplayPrompt(conflictSummary: string): string {
