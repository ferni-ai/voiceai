...
export function createSuperhumanCommunicationTools() {
    return {
        ...
        recallConversation: llm.tool({
            description: getToolDescription('recallConversation') ||
        ...
        checkRelationshipHealth: llm.tool({
            description: getToolDescription('checkRelationshipHealth') ||
            ...
            execute: async ({ userId, contactName }) => {
                ...
                const tempEmoji = temp.currentTemperature >= 70 ? '🔥' : temp.currentTemperature >= 40 ? '😐' : '🥶';
        ...
        getRelationshipsNeedingAttention: llm.tool({
            description: getToolDescription('getRelationshipsNeedingAttention') ||
        ...
        predictMessageReception: llm.tool({
            description: getToolDescription('predictMessageReception') ||
            ...
            execute: async ({ userId, message, contactName }) => {
                ...
                const receptionEmoji = prediction.predictedReception === 'positive'
                    ? '✅'
        ...
        getApologyAdvice: llm.tool({
            description: getToolDescription('getApologyAdvice') ||
        ...
        analyzeConflict: llm.tool({
            description: getToolDescription('analyzeConflict') ||
            ...
            execute: async ({ userId, description, thingsTheySaid, thingsOtherSaid }) => {
                ...
                if (thingsTheySaid && thingsTheySaid.length > 0) {
                    const reconstruction = conflictReplay.reconstruct(description, thingsTheySaid, thingsOtherSaid || []);
        ...
        getCommunicationDebts: llm.tool({
            description: getToolDescription('getCommunicationDebts') ||
        ...
        markCommunicationDone: llm.tool({
            description: getToolDescription('markCommunicationDone') ||
        ...
        getObjectivePerspective: llm.tool({
            description: getToolDescription('getObjectivePerspective') ||
        ...
        shouldISendThis: llm.tool({
            description: getToolDescription('shouldISendThis') ||
            ...
            execute: async ({ userId, situation, contactName }) => {
                ...
                const recommendation = await strategicSilence.getRecommendation(userId, situation, contactName);
                const emoji = recommendation.recommendation === 'respond_now'
                    ? '✅'
        ...
        holdMessageForLater: llm.tool({
            description: getToolDescription('holdMessageForLater') ||
...
Sometimes the best messages are the ones we don't send. 💭`;
        ...
        translateMyNeed: llm.tool({
            description: getToolDescription('translateMyNeed') ||
        ...
        whatAmIAvoiding: llm.tool({
            description: getToolDescription('whatAmIAvoiding') ||
...
export function getToolDefinitions() {
    ...
    return Object.entries(rawTools).map(([key, rawTool]) => ({
        ...
        domain: 'superhuman-communication',
...
export const definitions = {
    domain,
    getToolDefinitions,
    description: '10 superhuman communication capabilities: recall past conversations, track relationship health, ' +
