...
export async function recordCommunicationEvent(
  userId: string,
  event: Omit<CommunicationEvent, 'id' | 'mentionedAt'>
  ...
  try {
    ...
    if (db) {
      ...
      log.info(
        { userId, eventId: id, type: event.type, contact: event.contactName },
...
export async function getConversationHistory(
  ...
  try {
    ...
    let events = snapshot.docs.map((doc) => doc.data() as CommunicationEvent);
 ...
 */
export async function getRecentCommunicationEvents(
  userId: string,
  ...
  try {
    ...
    return snapshot.docs.map((doc) => doc.data() as CommunicationEvent);
  } catch (error) {
...
export async function updateContactProfile(
  userId: string,
  contactName: string,
  update: Partial<Omit<ContactCommunicationProfile, 'contactId' | 'userId' | 'name'>>
): Promise<void> {
  try {
    const db = getFirestoreDb();
    if (!db) return;
    ...
    const existingData = existing.data() as ContactCommunicationProfile | undefined;

    const profile: ContactCommunicationProfile = {
      contactId,
      userId,
      name: contactName,
      ...
      } as ContactCommunicationProfile['responsePatterns'],
      effectiveApproaches: mergeArrays(
        existingData?.effectiveApproaches || [],
        update.effectiveApproaches || []
      ),
      ineffectiveApproaches: mergeArrays(
        existingData?.ineffectiveApproaches || [],
        update.ineffectiveApproaches || []
      ),
...
export async function getContactProfile(
  userId: string,
  contactName: string
): Promise<ContactCommunicationProfile | null> {
  try {
    const db = getFirestoreDb();
    if (!db) return null;
    ...
    return doc.exists ? (doc.data() as ContactCommunicationProfile) : null;
  } catch (error) {
 ...
 */
export function detectCommunicationMention(
  transcript: string,
 ...
 */
export async function learnFromConversationOutcome(
  userId: string,
  contactName: string,
  outcome: {
    whatWorked?: string;
  ...
  const update: Partial<ContactCommunicationProfile> = {};
 ...
 */
export async function buildArchaeologyContext(
  userId: string,
  contactName: string,
  currentTopic?: string
): Promise<string> {
  ...
  if (history.length > 0) {
    ...
    for (const event of history.slice(0, 3)) {
      ...
      const outcomeEmoji =
        event.outcome === 'positive' ? '✅' : event.outcome === 'negative' ? '❌' : '•';

      sections.push(
        `${outcomeEmoji} ${daysAgo} days ago: ${event.summary}` +
 ...
 */
export async function buildGeneralArchaeologyContext(userId: string): Promise<string> {
  ...
  // Group by contact
  const byContact = new Map<string, CommunicationEvent[]>();
...
function mergeArrays(existing: string[], incoming: string[]): string[] {
  const combined = new Set<string>([...existing, ...incoming]);
  return [...combined].slice(0, 10); // Keep max 10 items
}

function extractContactName(match: RegExpMatchArray): string {
  // Find the captured name in the match groups
  for (let i = match.length - 1; i >= 1; i--) {
    const group = match[i];
    if (
      group &&
      !['to', 'with', 'me', 'that', 'talked', 'spoke', 'said', 'told', 'a', 'the'].includes(
        group.toLowerCase()
      )
    ) {
...
function extractTopics(transcript: string, currentTopic?: string): string[] {
  const topics: string[] = [];
...
function detectSentiment(transcript: string): number {
  const lower = transcript.toLowerCase();
