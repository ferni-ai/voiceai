...
const sessionMentions = new Map<string, TopicMention[]>();

/**
 * Detect if a topic was mentioned and whether it was deflected.
 */
export function detectTopicAndDeflection(
  transcript: string,
  ...
  let deflectionType: string | undefined;
  ...
  let topicMentioned: string | undefined;
 ...
 */
export function trackTopicMention(
  userId: string,
  topic: string,
  wasDeflected: boolean,
  context: string
 ...
 */
export function analyzeSessionForUnsaidTopics(userId: string): UnsaidTopic[] {
  const unsaidTopics: UnsaidTopic[] = [];
  ...
  for (const [key, mentions] of Array.from(sessionMentions.entries())) {
    ...
    if (mentions.length >= MIN_MENTIONS_TO_DETECT && deflectionRatio >= DEFLECTION_THRESHOLD) {
      // Determine category
      let category: UnsaidTopic['category'] = 'other';
      for (const [cat, keywords] of Object.entries(TOPIC_CATEGORIES)) {
        if (keywords.some((k) => topic.includes(k))) {
          category = cat as UnsaidTopic['category'];
          break;
        }
      }
...
export async function saveUnsaidTopic(
  ...
  try {
    ...
    log.info(
      { userId, topic: topic.topic, deflectionRatio: topic.deflectionRatio },
      '🔇 Unsaid topic detected and saved'
    );
  } catch (error) {
 ...
 */
export async function getUnsaidTopics(userId: string): Promise<UnsaidTopic[]> {
  try {
    const db = getFirestoreDb();
    if (!db) return [];
    ...
    return snapshot.docs.map((doc) => doc.data() as UnsaidTopic);
  } catch (error) {
 ...
 */
export async function markTopicSurfaced(userId: string, topicId: string): Promise<void> {
  try {
    const db = getFirestoreDb();
    if (!db) return;
 ...
 */
export async function markTopicResolved(userId: string, topicId: string): Promise<void> {
  try {
    const db = getFirestoreDb();
    if (!db) return;
 ...
 */
export async function buildUnsaidWordsContext(userId: string): Promise<string> {
  ...
  for (const topic of allTopics.slice(0, 5)) {
    ...
    sections.push(
      `• **${topic.topic}** - mentioned ${topic.timesMentioned}x, deflected ${deflectPercent}% of the time`
    );
 ...
 */
export function generateSurfacingPrompt(topic: UnsaidTopic): string {
  const prompts: Record<UnsaidTopic['category'], string[]> = {
    person: [
 ...
 */
export async function endSessionAndPersist(userId: string): Promise<void> {
