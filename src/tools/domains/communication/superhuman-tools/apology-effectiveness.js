 ...
 */
export function detectApologyStyle(message) {
    const styles = [];
 ...
 */
export async function recordApology(userId, record) {
 ...
 */
export async function getApologyHistory(userId, contactName) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return [];
 ...
 */
export async function getAllApologyRecords(userId) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return [];
 ...
 */
export async function analyzeApologyEffectiveness(userId, contactName) {
    ...
    for (const record of history) {
        ...
        const wasSuccessful = record.reception === 'well-received' || record.relationshipAfter === 'improved';
    ...
    for (const [style, stats] of Object.entries(styleStats)) {
        if (stats.total >= 2) {
            ...
            if (successRate >= 0.6) {
                effective.push({ style: style, successRate });
            }
    ...
    if (effective.length > 0) {
        insights.push(`${contactName} responds best to ${effective[0].style} apologies (${Math.round(effective[0].successRate * 100)}% success rate).`);
    }
    if (ineffective.length > 0) {
        insights.push(`Avoid ${ineffective[0].style} apologies with ${contactName} - they don't land well.`);
    }
    if (whatWorked.length > 0) {
        insights.push(`What's worked: ${whatWorked.slice(0, 2).join(', ')}`);
    }
    // Determine best/worst approach
    const bestApproach = effective.length > 0
        ? generateApologyTemplate(effective[0].style, contactName)
        : 'Be genuine and specific about what you did wrong.';
    const worstApproach = ineffective.length > 0
 ...
 */
export async function getApologyRecommendation(userId, contactName) {
...
// TEMPLATES
// ============================================================================
function generateApologyTemplate(style, contactName) {
    const templates = {
 ...
 */
export async function buildApologyContext(userId, contactName) {
    if (contactName) {
    ...
    for (const [name, records] of Array.from(byContact.entries())) {
        if (records.length < 2)
            continue;
        const successful = records.filter((r) => r.reception === 'well-received' || r.relationshipAfter === 'improved');
        ...
        sections.push(`• ${name}: ${Math.round(successRate * 100)}% success ${emoji} (prefers ${dominantStyle} apologies)`);
    }
...
//# sourceMappingURL=apology-effectiveness.js.map