 ...
 */
function getPersonSpecificTriggers(profile) {
    if (!profile)
        return [];
    const triggers = [];
 ...
 */
function getPersonSpecificPositives(profile) {
    if (!profile)
        return [];
    const positives = [];
 ...
 */
export async function predictReception(userId, message, contactName) {
    ...
    log.debug({ userId, contactName, score, predictedReception, warningCount: warningFlags.length }, '🎯 Reception prediction generated');
    return {
 ...
 */
export function predictGeneralReception(message) {
    let score = 0.5;
    const warningFlags = [];
 ...
 */
export function generateSoftenedVersion(message) {
 ...
 */
export function generateAlternatives(triggerPhrase) {
 ...
 */
export function buildReceptionPredictorContext() {
...
function detectMessageTone(message) {
    const lower = message.toLowerCase();
    // Formal indicators
    if (/\b(dear|sincerely|regards|respectfully|kindly)\b/.test(lower) ||
        /\b(would you be so kind|i would appreciate)\b/.test(lower)) {
        return 'formal';
    }
    // Direct indicators
    if (/\b(i need|i want|please do|you must)\b/.test(lower) ||
        message.split('.').length <= 2) {
        return 'direct';
    }
    // Warm indicators
    if (/\b(love|care|appreciate|thinking of you|miss you)\b/.test(lower) ||
        /❤️|😊|💕|🤗/.test(message)) {
        return 'warm';
    }
    // Indirect indicators
...
//# sourceMappingURL=reception-predictor.js.map