...
export {
  createSuperhumanCommunicationTools,
  getToolDefinitions,
  domain as llmToolsDomain,
  definitions as llmToolsDefinitions,
} from './llm-tools.js';
 ...
 * for LLM injection.
 */
export async function buildSuperhumanCommunicationContext(
  userId: string,
  options?: {
    includeAll?: boolean;
    contactName?: string; // For contact-specific context
    maxLength?: number;
  }
): Promise<string> {
  ...
  try {
    ...
    if (temperatureCtx) sections.push(temperatureCtx);
    if (debtCtx) sections.push(debtCtx);
    if (archaeologyCtx) sections.push(archaeologyCtx);
    ...
    if (includeAll) {
      ...
      if (unsaidCtx) sections.push(unsaidCtx);
      if (silenceCtx) sections.push(silenceCtx);
      if (needsCtx) sections.push(needsCtx);
      if (conflictCtx) sections.push(conflictCtx);
      if (apologyCtx) sections.push(apologyCtx);
    ...
    if (contactName) {
      ...
      if (archCtx) sections.push(archCtx);
      if (tempCtx) sections.push(tempCtx);

      if (apologyProfile) {
        sections.push(
          `[APOLOGY PATTERNS - ${contactName}]\n` +
 ...
 */
export async function buildQuickCommunicationContext(userId: string): Promise<string> {
  try {
 ...
 */
export function getSuperhumanCapabilitiesSummary(): string {
