...
const COGNITIVE_BIASES = {
  ...
  mind_reading: {
    ...
    check: (story: string) =>
      /they (obviously|clearly|definitely) (wanted|meant|were trying) to/i.test(story) ||
      /i (know|can tell) (they|he|she) (thinks|feels|wants)/i.test(story),
    counter:
      'Have you actually asked them what they intended? How might you be wrong about their motives?',
  },
  black_and_white: {
...
export function generatePerspective(
  ...
  context?: { relationship?: string; history?: string }
): ThirdPartyPerspective {
  const lower = userStory.toLowerCase();

  // Detect biases
  const detectedBiases: Array<{ name: string; counter: string }> = [];
  ...
  if (blindSpots.length === 0) {
    blindSpots.push(
      `What might ${otherPersonName} say if they were telling this story?`,
      'What part of this situation are you most uncomfortable examining?'
    );
  }
...
function extractValidPoints(story: string, perspective: 'user' | 'other'): string[] {
  const points: string[] = [];
...
function generateOtherPerspective(story: string, otherName: string): string[] {
  const points: string[] = [];
  ...
  if (points.length < 2) {
    points.push(
      `${otherName} likely has their own story about what happened`,
      `${otherName} may be acting from their own hurt or fear`
    );
  }
...
function generateNeutralSummary(
  story: string,
  otherName: string,
  biases: Array<{ name: string; counter: string }>
): string {
  ...
  if (biases.some((b) => b.name === 'Victimhood Narrative')) {
    summaryParts.push(
      `Two people in conflict, each feeling hurt and misunderstood. Neither is purely a villain or victim.`
    );
  } else if (biases.some((b) => b.name === 'Fundamental Attribution Error')) {
    summaryParts.push(
      `Two people reacting to difficult circumstances, each doing their best with limited information.`
    );
  } else if (biases.some((b) => b.name === 'Mind Reading')) {
    summaryParts.push(
      `A misunderstanding that may be based on assumptions rather than actual intentions.`
    );
  } else {
    summaryParts.push(
      `A conflict between two people who both have legitimate feelings and perspectives.`
    );
  }

  // Add key observation
  summaryParts.push(
    `Both you and ${otherName} probably feel justified. That's usually how conflicts work.`
  );
...
function generatePathForward(
  ...
  if (biases.some((b) => b.name === 'Fundamental Attribution Error')) {
    paths.push(
      `Consider what circumstances might have led to their behavior - just as you'd want others to consider yours.`
    );
  }
  ...
  if (paths.length === 0) {
    paths.push(
      `Lead with curiosity rather than accusation. Ask "help me understand" instead of "why did you."`,
      `Acknowledge their experience before asserting yours. People listen better when they feel heard.`
    );
  }
 ...
 */
export function generatePerspectiveQuestions(situation: string, otherName: string): string[] {
  return [
 ...
 */
export function buildPerspectiveContext(): string {
