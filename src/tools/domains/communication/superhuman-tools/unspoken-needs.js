 ...
 */
export function detectUnderlyingNeed(complaint) {
    ...
    const betterExpression = categoryInfo.betterExpressions[Math.floor(Math.random() * categoryInfo.betterExpressions.length)];
    return {
 ...
 */
export function translateToNeed(complaint, targetPerson) {
...
function generateWhyItMatters(category, complaint) {
    const explanations = {
        belonging: 'When you name the need to belong instead of the specific incident, it opens a conversation about the relationship, not just one event.',
        autonomy: 'Asking for autonomy directly is more likely to get a positive response than pushing back against control.',
        ...
        meaning: 'Expressing a need for meaning invites deeper conversation than expressing frustration.',
        connection: 'Asking for connection is vulnerable but more likely to create it than complaining about distance.',
 ...
 */
export async function saveUnspokenNeed(userId, need) {
    ...
    try {
        ...
        if (db) {
            ...
            log.info({ userId, needCategory: need.needCategory, surfaceComplaint: need.surfaceComplaint.slice(0, 50) }, '💭 Unspoken need saved');
        }
    }
    catch (error) {
 ...
 */
export async function getUnspokenNeeds(userId) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return [];
 ...
 */
export async function markNeedSurfaced(userId, needId) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return;
 ...
 */
export async function markNeedAddressed(userId, needId) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return;
 ...
 */
export async function analyzeNeedPatterns(userId) {
 ...
 */
export async function buildNeedsContext(userId) {
 ...
 */
export function generateTranslationPrompt(complaint) {
...
//# sourceMappingURL=unspoken-needs.js.map