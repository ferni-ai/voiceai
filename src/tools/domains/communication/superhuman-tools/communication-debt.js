 ...
 */
export function detectCommunicationDebt(transcript) {
    const debts = [];
 ...
 */
export async function recordDebt(userId, debt) {
 ...
 */
export async function getPendingDebts(userId) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return [];
 ...
 */
export async function markDebtAddressed(userId, debtId) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return;
 ...
 */
export async function dismissDebt(userId, debtId, reason) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return;
...
function calculatePriority(debt) {
    ...
    if (adjustedDays >= thresholds.urgent)
        return 'urgent';
    if (adjustedDays >= thresholds.high)
        return 'high';
    if (adjustedDays >= thresholds.medium)
        return 'medium';
    return 'low';
}
// ============================================================================
...
export async function generateDashboard(userId) {
    ...
    const sections = ['📋 **Communication Debt Dashboard**\n'];
    // Group by priority
 ...
 */
export async function buildDebtContext(userId) {
    const debts = await getPendingDebts(userId);
    if (debts.length === 0) {
        return '';
    }
 ...
 */
export async function getDebtsForContact(userId, contactName) {
...
//# sourceMappingURL=communication-debt.js.map