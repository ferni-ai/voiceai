 ...
 */
export async function recordResponseTiming(userId, record) {
 ...
 */
export async function getTimingHistory(userId, contactName) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return [];
 ...
 */
export async function getAllTimingRecords(userId) {
    try {
        const db = getFirestoreDb();
        if (!db)
            return [];
 ...
 */
export async function analyzeTimingPatterns(userId, contactName) {
    ...
    const successfulDelays = history
        ...
        .map((r) => r.delayHours);
    const optimalDelay = successfulDelays.length > 0
 ...
 */
export async function getTimingRecommendation(userId, situation, contactName) {
    ...
    if (contactPattern) {
        ...
        if (contactPattern.recommendation === 'wait_24h' ||
            contactPattern.recommendation === 'wait_longer') {
            return {
                recommendation: 'wait',
                reason: `Your delayed responses to ${contactName} work ${Math.round((contactPattern.delayedOutcomes.positive /
                    (contactPattern.delayedOutcomes.positive +
                        contactPattern.delayedOutcomes.negative +
                        contactPattern.delayedOutcomes.neutral || 1)) *
                    100)}% better than immediate ones.`,
 ...
 */
export function holdMessage(userId, contactName, message, holdHours = 24) {
 ...
 */
export function getReadyMessages(userId) {
    const now = Date.now();
    const ready = [];
 ...
 */
export function discardMessage(messageId) {
    const held = heldMessages.get(messageId);
    if (held && held.status === 'held') {
        held.status = 'discarded';
        log.info({ messageId, contactName: held.contactName }, '🗑️ User wisely discarded message after cooling off');
        return true;
    }
    return false;
}
 ...
 */
export function generateCoolingPrompt(contactName, situation) {
    const prompts = [
...
//# sourceMappingURL=strategic-silence.js.map