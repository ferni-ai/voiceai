...
export function createCommunicationCoachingTools() {
    return {
        ...
        draftDifficultMessage: llm.tool({
            description: getToolDescription('draftDifficultMessage'),
            parameters: z.object({
                conversationType: z
                    .enum([
                    'asking_for_raise',
            ...
            execute: async ({ conversationType, recipient, context, keyPoints, desiredOutcome, format, tone, }) => {
                getLogger().info({ conversationType, recipient, format }, '💬 Alex: Coaching on difficult message');
        ...
        reviewMessage: llm.tool({
            description: getToolDescription('reviewMessage'),
            parameters: z.object({
        ...
        planCommunicationStrategy: llm.tool({
            description: getToolDescription('planCommunicationStrategy'),
            parameters: z.object({
        ...
        // ========== QUICK TONE CHECK ==========
        checkTone: llm.tool({
            description: getToolDescription('checkTone'),
            parameters: z.object({
        ...
        transformTone: llm.tool({
            description: getToolDescription('transformTone'),
            parameters: z.object({
        ...
        buildAssertiveResponse: llm.tool({
            description: getToolDescription('buildAssertiveResponse'),
            parameters: z.object({
        ...
        planFollowUp: llm.tool({
            description: getToolDescription('planFollowUp'),
            parameters: z.object({
            ...
            execute: async ({ originalRequest, recipient, daysSinceOriginal, previousFollowUps, urgency, }) => {
                ...
                const recommendation = generateFollowUpStrategy(daysSinceOriginal, previousFollowUps, urgency);
        ...
        analyzeIncomingMessage: llm.tool({
            description: getToolDescription('analyzeIncomingMessage'),
            parameters: z.object({
        ...
        analyzeCommPattern: llm.tool({
            description: getToolDescription('analyzeCommPattern'),
            parameters: z.object({
                sampleMessages: z
                    .string()
                ...
                context: z
                    .string()
                    .describe('What kind of communications are these? Work, personal, specific relationship?'),
...
// ============================================================================
function analyzeIncomingContent(message) {
    ...
    else if (lower.includes('unfortunately') ||
    ...
    if (lower.includes('following up') ||
    ...
    else if (lower.includes('when you get a chance') ||
...
function generateResponseToMessage(incoming, sender, userGoal, analysis) {
...
function transformMessage(message, targetTone) {
...
function addressConcern(message, concern) {
...
function generateAssertiveScript(situation, whatUserWants) {
...
function generateFollowUpStrategy(daysSince, previousFollowUps, urgency) {
    if (previousFollowUps === 0 && daysSince >= 2) {
        return {
            action: 'Send first follow-up',
            script: "Hi! Wanted to make sure this didn't get buried. Let me know if you need anything from me to move forward.",
            timing: 'Send today',
    ...
    if (previousFollowUps >= 2) {
        return {
            ...
            script: "I know things get busy. I'll assume the timing isn't right now. Feel free to reach out when it works better—happy to help whenever.",
...
//# sourceMappingURL=communication-coaching.js.map