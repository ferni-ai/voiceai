...
import { recordGift as persistGift, getGiftHistory, generateGiftSuggestions as getAISuggestions, getUpcomingGiftOccasions, } from '../../../services/contacts/gift-tracking-service.js';
    ...
    return {
        ...
        logGiftReceived: llm.tool({
            ...
            execute: async ({ from, description, eventName, estimatedValue, notes }, { ctx }) => {
                ...
                const contactId = matches.length > 0
                    ? matches[0].contactId
        ...
        logGiftGiven: llm.tool({
            ...
            execute: async ({ to, description, occasion, price, reaction, notes }, { ctx }) => {
                ...
                const reactionEmoji = reaction === 'loved'
                    ? '😍'
                    : reaction === 'liked'
                        ? '😊'
                        : reaction === 'neutral'
        ...
        suggestGiftIdeas: llm.tool({
            ...
            execute: async ({ personName, occasion, budget, mood }, { ctx }) => {
                ...
                const contact = matches[0];
                try {
                    const suggestions = await getAISuggestions(userId, contact.contactId, occasion || 'general', budget ? { min: budget * 0.5, max: budget * 1.5 } : { min: 20, max: 200 });
        ...
        getGiftIdeas: llm.tool({
            ...
            execute: async ({ eventType, category }) => {
                ...
                if (category) {
                    categories = ideas.filter((c) => c.category.toLowerCase().includes(category.toLowerCase()));
                }
        ...
        addToRegistry: llm.tool({
            ...
            execute: async ({ registryName, itemName, price, category, priority, url }) => {
                const registry = Array.from(registries.values()).find((r) => r.eventName.toLowerCase().includes(registryName.toLowerCase()));
                if (!registry) {
                ...
                registries.set(registry.id, registry);
                const priorityEmoji = priority === 'must-have' ? '✅' : priority === 'nice-to-have' ? '☑️' : '⭐';
        ...
        viewRegistry: llm.tool({
            ...
            execute: async ({ registryName }) => {
                ...
                const byCategory = registry.items.reduce((acc, item) => {
...
//# sourceMappingURL=gift-registry.js.map