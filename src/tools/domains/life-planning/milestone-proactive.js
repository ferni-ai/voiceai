...
import { getUserMilestones } from './life-firsts-tracker.js';
 ...
 */
export async function analyzeUserMilestones(userId) {
 ...
 */
export function getProactiveCheckIn(milestone) {
    ...
    // Countdown
    const messageSet = COUNTDOWN_MESSAGES[milestone.category] || COUNTDOWN_MESSAGES.default;
    parts.push(formatMessage(randomFrom(messageSet[urgency]), { days: daysUntil, event: milestone.name }));
    // Progress
    parts.push(formatMessage(randomFrom(PROGRESS_MESSAGES[progressLevel]), { percent: progressPercent }));
 ...
 */
export async function getMostUrgentMilestone(userId) {
    const urgencies = await analyzeUserMilestones(userId);
    // Find the most urgent that's not on track
    const needsAttention = urgencies.find((u) => (u.urgency === 'critical' || u.urgency === 'urgent') && u.progressPercent < 80);
    if (needsAttention)
        return needsAttention;
 ...
 */
export async function getMilestonesSummary(userId) {
    const urgencies = await analyzeUserMilestones(userId);
    ...
    for (const u of urgencies.slice(0, 3)) {
        ...
        parts.push(`${urgencyEmoji} **${u.milestoneName}** - ${u.daysUntil} days (${u.progressPercent}% ready)`);
    }
    ...
    return {
        ...
        getProactiveCheckIn: llm.tool({
            ...
            execute: async ({ milestoneName, userId }) => {
                const milestones = await getUserMilestones(userId);
                const milestone = milestones.find((m) => m.name.toLowerCase().includes(milestoneName.toLowerCase()));
                if (!milestone) {
...
I can help you plan gifts, parties, or just set reminders so you don't forget!`;
        ...
        checkMilestoneAnniversaries: llm.tool({
            ...
            execute: async ({ userId, daysAhead = 30 }) => {
                ...
                const completedMilestones = milestones.filter((m) => m.status === 'completed' && m.completedDate);
                ...
                for (const milestone of completedMilestones) {
                    ...
                    const thisYearAnniversary = new Date(today.getFullYear(), completedDate.getMonth(), completedDate.getDate());
...
//# sourceMappingURL=milestone-proactive.js.map