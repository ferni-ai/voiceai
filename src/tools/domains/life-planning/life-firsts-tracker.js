...
function validateMilestoneName(name) {
...
const userMilestones = new Map(); // userId -> milestone IDs
// ============================================================================
// HELPER FUNCTIONS
// ============================================================================
...
// CORE FUNCTIONS
// ============================================================================
export async function createMilestone(userId, category, name, targetDate, budget, culturalType) {
    // Validate inputs
    ...
    if (targetDate) {
        try {
            ...
            getLogger().warn({ error: calendarError, milestoneId: id }, 'Failed to sync milestone to calendar');
        }
    }
...
export async function getUserMilestones(userId) {
    ...
    if (cachedIds.length > 0) {
        return cachedIds.map((id) => milestones.get(id)).filter(Boolean);
    }
    ...
    return ids.map((id) => milestones.get(id)).filter(Boolean);
}
export function updateMilestoneChecklist(milestoneId, taskId, completed) {
...
export function addMilestoneMemory(milestoneId, type, content) {
    const milestone = milestones.get(milestoneId);
    if (!milestone)
        return undefined;
...
export function createLifeFirstsTools() {
    return {
        ...
        createLifeMilestone: llm.tool({
            description: getToolDescription('createLifeMilestone'),
            parameters: z.object({
                category: z
                    .enum([
                    'first-home',
            ...
            execute: async ({ category, name, targetDate, budget, culturalType, userId }) => {
                ...
                const milestone = await createMilestone(userId, category, name, parsedDate, budget, culturalType);
                const template = MILESTONE_TEMPLATES[category];
        ...
        viewLifeMilestones: llm.tool({
            description: getToolDescription('viewLifeMilestones'),
            parameters: z.object({
                status: z
        ...
        updateMilestoneTask: llm.tool({
            description: getToolDescription('updateMilestoneTask'),
            parameters: z.object({
            ...
            execute: async ({ milestoneName, taskDescription, completed, userId }) => {
                ...
                const milestone = userMilestoneList.find((m) => m.name.toLowerCase().includes(milestoneName.toLowerCase()));
                if (!milestone) {
                    return `Couldn't find a milestone matching "${milestoneName}". Your milestones: ${userMilestoneList.map((m) => m.name).join(', ')}`;
                }
                const task = milestone.checklist.find((t) => t.task.toLowerCase().includes(taskDescription.toLowerCase()));
                if (!task) {
        ...
        addMilestoneNote: llm.tool({
            description: getToolDescription('addMilestoneNote'),
            parameters: z.object({
                milestoneName: z.string().describe('Name of the milestone'),
                noteType: z
        ...
        getMilestoneTips: llm.tool({
            description: getToolDescription('getMilestoneTips'),
            parameters: z.object({
                category: z
                    .enum([
                    'first-home',
        ...
        getMilestoneCountdown: llm.tool({
            description: getToolDescription('getMilestoneCountdown'),
            parameters: z.object({
                milestoneName: z
                    .string()
                    .optional()
            ...
            execute: async ({ milestoneName, userId }) => {
                ...
                if (milestoneName) {
                    userMilestoneList = userMilestoneList.filter((m) => m.name.toLowerCase().includes(milestoneName.toLowerCase()));
                }
...
//# sourceMappingURL=life-firsts-tracker.js.map