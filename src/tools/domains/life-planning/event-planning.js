...
import { events, majorPurchases, vacations, annualPlans, ensureUserLoaded, persistEventPlanningData, BEST_TIMES_TO_BUY, DESTINATION_DATABASE, venueDatabase, } from '../../../tools/event-planning/storage.js';
...
// EVENT MANAGEMENT
// ============================================================================
async function createEventWithPersistence(userId, name, type, date, guestCount, budget, theme) {
...
function generateDefaultChecklist(type) {
    ...
    switch (type) {
        case 'birthday':
            baseItems.push({ id: 'cl_b1', task: 'Order birthday cake', category: 'catering', completed: false }, { id: 'cl_b2', task: 'Plan games/activities', category: 'entertainment', completed: false }, { id: 'cl_b3', task: 'Arrange gift table', category: 'decor', completed: false });
            break;
        case 'wedding':
            baseItems.push({ id: 'cl_w1', task: 'Book photographer', category: 'other', completed: false }, { id: 'cl_w2', task: 'Arrange flowers', category: 'decor', completed: false }, { id: 'cl_w3', task: 'Plan ceremony timeline', category: 'logistics', completed: false }, { id: 'cl_w4', task: 'Arrange transportation', category: 'logistics', completed: false });
            break;
        case 'corporate':
            baseItems.push({ id: 'cl_c1', task: 'Set up AV equipment', category: 'logistics', completed: false }, {
                id: 'cl_c2',
            ...
            break;
        case 'dinner-party':
            baseItems.push({ id: 'cl_d1', task: 'Plan menu', category: 'catering', completed: false }, { id: 'cl_d2', task: 'Select wine pairings', category: 'catering', completed: false }, { id: 'cl_d3', task: 'Set table arrangement', category: 'decor', completed: false });
            break;
        default:
            break;
    }
    ...
    return {
        ...
        createEvent: llm.tool({
            ...
            execute: async ({ name, type, date, guestCount, budget, theme }, context) => {
                ...
                const event = await createEventWithPersistence(userId, name, type, eventDate, guestCount, budget, theme);
        ...
        searchVenues: llm.tool({
            ...
            execute: async ({ guestCount, eventType, budgetLevel, venueType }) => {
                ...
                if (venueType !== 'both') {
                    matches = matches.filter((v) => venueType === 'outdoor' ? v.type === 'outdoor' : v.type !== 'outdoor');
                }
                if (matches.length === 0) {
        ...
        completeTask: llm.tool({
            ...
            execute: async ({ taskName, eventId, notes }, context) => {
                ...
                const task = event.checklist.find((c) => c.task.toLowerCase().includes(taskName.toLowerCase()) && !c.completed);
                if (!task) {
        ...
        suggestDestinations: llm.tool({
            ...
            execute: async ({ tripType, budget }) => {
                const matches = DESTINATION_DATABASE.filter((d) => d.type.includes(tripType) && d.budget === budget);
        ...
        getAnnualPlanStatus: llm.tool({
            ...
            execute: async ({ year }) => {
                ...
                for (const goal of plan.goals) {
                    const icon = goal.status === 'completed' ? '✅' : goal.status === 'in-progress' ? '🔄' : '☐';
                    response += `${icon} ${goal.description}\n`;
                }
                if (plan.experiences.length > 0) {
