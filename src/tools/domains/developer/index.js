 ...
 */
function summarizeForVoice(output, context) {
    // Strip ANSI codes
...
function startBackgroundJob(cmd, args) {
            ...
            execute: async () => {
                try {
                    ...
                    if (fileNames.length > 0) {
                        ...
                        if (lines.length > 3)
                            summary += ` and ${lines.length - 3} more.`;
                    }
                    return summary;
                }
                catch (error) {
        ...
        return llm.tool({
            ...
            execute: async ({ file, staged }) => {
                try {
                    ...
                    if (inserts > 0)
                        summary += `, ${inserts} line${inserts > 1 ? 's' : ''} added`;
                    if (deletes > 0)
                        summary += `, ${deletes} line${deletes > 1 ? 's' : ''} removed`;
                    summary += '.';
                    // Add file names
...
export const { getToolDefinitions, domain, definitions } = createDomainExport('developer', developerToolDefinitions);
