...
import { searchRestaurants, bookReservation, isReservationServiceConfigured, formatRestaurantForSpeech, } from '../../services/restaurant-reservations.js';
import { getAppointmentFollowUpService } from '../../services/scheduling/appointment-followup.js';
import { createAppointmentRequest, updateAppointmentStatus, makeAppointmentCall, getAppointment, getUserAppointments, } from './appointment-core.js';
...
export function createAppointmentTools() {
    return {
        ...
        makeReservation: llm.tool({
            ...
            execute: async ({ restaurantName, restaurantPhone, location, dateTime, partySize, guestName, guestPhone, guestEmail, specialRequests, }, { ctx }) => {
                ...
                if (isReservationServiceConfigured()) {
                    getLogger().info({ restaurant: restaurantName, location }, '🍽️ Searching for online booking');
                    const restaurants = await searchRestaurants(restaurantName, location || 'nearby', dateForApi, timeForApi, partySize);
                    // Find best match
                    const match = restaurants.find((r) => r.name.toLowerCase().includes(restaurantName.toLowerCase()) ||
                        restaurantName.toLowerCase().includes(r.name.toLowerCase())) || restaurants[0];
                    if (match && match.reservationProvider !== 'phone_only') {
                        ...
                        if (match.phone) {
                            restaurantPhone = match.phone;
                            getLogger().info({ restaurant: match.name, phone: match.phone }, 'Found restaurant, will call');
                        }
                    }
                }
        ...
        searchRestaurantsNearby: llm.tool({
            ...
            execute: async ({ query, location, date, partySize }) => {
                ...
                const restaurants = await searchRestaurants(query, location, dateForApi, '19:00', partySize);
        ...
        scheduleAppointment: llm.tool({
            ...
            execute: async ({ appointmentType, businessName, businessPhone, dateTime, forPerson, reason, specialRequests, }, { ctx }) => {
                const requestedTime = parseNaturalTime(dateTime);
                if (!requestedTime) {
        ...
        getAppointmentStatus: llm.tool({
            ...
            execute: async ({ includeCompleted }, { ctx }) => {
                ...
                for (const apt of userAppts) {
                    ...
                    const dateStr = (apt.confirmedDateTime || apt.requestedDateTime).toLocaleDateString('en-US', {
                        weekday: 'short',
                        month: 'short',
                        day: 'numeric',
                    });
                    const timeStr = (apt.confirmedDateTime || apt.requestedDateTime).toLocaleTimeString('en-US', {
                        hour: 'numeric',
                        minute: '2-digit',
                    });
        ...
        scheduleLifeEventAppointment: llm.tool({
            ...
            execute: async ({ eventName, milestoneId, appointmentType, businessName, businessPhone, preferredDate, specialRequests, }, { ctx }) => {
        ...
        quickCall: llm.tool({
            ...
            execute: async ({ phoneNumber, businessName, purpose }) => {
                ...
                getLogger().info({
                    phone: sanitizePhoneForLog(validation.sanitized),
                    business: businessName,
                    purpose,
                }, '📞 Quick call');
        ...
        markAppointmentConfirmed: llm.tool({
            ...
            execute: async ({ businessName, confirmationNumber, confirmedTime, notes }, { ctx }) => {
                ...
                const apt = await updateAppointmentStatus(foundApt.id, 'confirmed', notes ? `Manually confirmed by user: ${notes}` : 'Manually confirmed by user', confirmedDateTime, confirmationNumber);
                if (!apt) {
        ...
        retryAppointmentCall: llm.tool({
            ...
            execute: async ({ businessName }, { ctx }) => {
                const userData = ctx?.userData;
                const userId = userData?.userId || 'unknown';
                const apt = getUserAppointments(userId).find((a) => a.businessName.toLowerCase().includes(businessName.toLowerCase()) &&
        ...
        setAppointmentReminder: llm.tool({
            ...
            execute: async ({ appointmentDescription, appointmentDateTime, reminderTime, deliveryMethod, contact }, { ctx }) => {
