...
import { createContact, searchContacts, findContact, getUserContacts, getRecentContacts, getFavoriteContacts, markContacted, formatPhoneForDisplay, formatContactForSpeech, updateContact, deleteContact, addNickname, toggleFavorite, importFromVCard, importFromCSV, } from '../../services/contacts.js';
    ...
    return {
        ...
        addContact: llm.tool({
            ...
            execute: async ({ name, phone, email, nickname, relationship, company, notes }, { ctx }) => {
                ...
                if (phone)
                    response += ` - ${formatPhoneForDisplay(phone)}`;
                if (email)
                    response += ` - ${email}`;
                if (nickname)
            ...
            execute: async ({ query }, { ctx }) => {
                ...
                const list = results
                    ...
                    if (c.phones.length > 0)
                        line += ` - ${formatPhoneForDisplay(c.phones[0].number)}`;
                    return line;
                })
                    .join('\n');
                ...
                switch (filter) {
                    ...
                    case 'group':
                        if (group) {
                            ...
                            contacts = allContacts.filter((c) => c.groups.some((g) => g.toLowerCase() === group.toLowerCase()));
                        }
                        else {
                            contacts = [];
                        }
        ...
        importContacts: llm.tool({
            ...
            execute: async ({ source, data }, { ctx }) => {
                ...
                switch (source) {
                    case 'google':
                        return (`To import from Google Contacts:\n\n` +
                    ...
                    case 'vcard':
                        if (data) {
                            ...
                            return `✅ Imported ${result.imported} contact${result.imported !== 1 ? 's' : ''} from vCard!${result.errors > 0 ? ` (${result.errors} couldn't be imported)` : ''}`;
                        }
                        return (`To import from vCard:\n\n` +
                    ...
                    case 'csv':
                        if (data) {
                            ...
                            return `✅ Imported ${result.imported} contact${result.imported !== 1 ? 's' : ''} from CSV!${result.errors > 0 ? ` (${result.errors} couldn't be imported)` : ''}`;
                        }
                        return (`To import from CSV:\n\n` +
