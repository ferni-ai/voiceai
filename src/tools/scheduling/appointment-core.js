...
import { syncAppointmentToCalendar, updateCalendarSyncedItem, removeCalendarSyncedItem, } from '../../services/calendar/calendar-bridge.js';
// Twilio credentials
...
export async function createAppointmentRequest(params) {
    ...
    getLogger().info({ id, type: params.type, business: params.businessName }, 'Appointment request created');
    ...
    try {
        ...
        const calendarResult = await syncAppointmentToCalendar(params.userId, id, appointmentTitle, params.requestedDateTime, {
        ...
        getLogger().warn({ error: String(calendarError), appointmentId: id }, '⚠️ Failed to sync appointment to calendar');
    }
    return appointment;
}
/**
 ...
 */
function getAppointmentTitle(type, businessName) {
    const typeLabels = {
 ...
 */
function buildAppointmentDescription(apt) {
    const parts = [];
    if (apt.forPerson) {
 ...
 */
export async function updateAppointmentStatus(id, status, note, confirmedDateTime, confirmationNumber) {
    ...
    if (note)
        apt.notes.push(`[${new Date().toISOString()}] ${note}`);
    ...
    try {
        ...
        getLogger().warn({ error: String(calendarError), appointmentId: id }, '⚠️ Failed to update calendar for appointment status change');
    }
    return apt;
}
/**
 ...
 */
export function generateCallScript(apt) {
 ...
 */
export async function makeAppointmentCall(apt) {
...
//# sourceMappingURL=appointment-core.js.map