...
export class AutoToolOptimizer {
    ...
    start() {
        ...
        this.analysisTimer = setInterval(() => this.runOptimizationCycle(), this.config.analysisIntervalMs);
    }
    /**
     ...
     */
    processUserMessage(message, context, lastToolId) {
        // Collect feedback
     ...
     */
    recordToolExecution(sessionId, toolId, success, latencyMs) {
    ...
    async runOptimizationCycle() {
        ...
        try {
            ...
            if (cycle.feedbackProcessed < this.config.minDataPoints) {
                getLogger().info({ current: cycle.feedbackProcessed, required: this.config.minDataPoints }, '⏳ Not enough data for optimization');
     ...
     */
    async autoCreateExperiments() {
        // Check current experiment count
        const activeExperiments = abTestingService.getActiveExperiments();
        if (activeExperiments.length >= this.config.maxConcurrentExperiments) {
            getLogger().debug({ current: activeExperiments.length, max: this.config.maxConcurrentExperiments }, 'Max concurrent experiments reached');
            return 0;
        }
     ...
     */
    async processExperimentResults() {
        const activeExperiments = abTestingService.getActiveExperiments();
        for (const exp of activeExperiments) {
    ...
    analyzeExperimentResults(exp, results) {
