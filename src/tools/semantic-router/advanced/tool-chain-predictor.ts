...
interface ChainPattern {
  ...
  chain: string[]; // Tool IDs in order
  conditionals?: Array<{
    afterStep: number;
    condition: string;
...
export class ToolChainPredictor {
  ...
  private patterns: ChainPattern[] = [];

  // Learned co-occurrence statistics
  private coOccurrence = new Map<string, CoOccurrenceStats[]>();

  // Per-user chain history
  private userHistory = new Map<string, UserChainHistory>();
  ...
  async predict(
    intent: string,
    primaryTool: ToolMatch,
    availableTools: SemanticToolDefinition[],
    userId?: string
    ...
    if (patternChain) {
      log.debug(
        { intent, pattern: patternChain.steps.map((s) => s.toolId) },
        'Matched explicit pattern'
      );
      return patternChain;
    }
    ...
    if (likelyNext.length > 0 || userPatterns.length > 0) {
      const chain = this.buildChainFromSignals(
        primaryTool.toolId,
        likelyNext,
        userPatterns,
        availableTools
  ...
  private matchPattern(intent: string): ToolChain | null {
  ...
  private updateCoOccurrence(from: string, to: string): void {
    const key = from;
  ...
  private updateUserHistory(userId: string, toolId: string): void {
  ...
  private getUserPatterns(userId: string, currentTool: string): string[] {
    ...
    const matchingChains = history.chains.filter(
  ...
  private buildChainFromSignals(
    primaryTool: string,
    coOccurrence: Array<{ toolId: string; probability: number }>,
    userPatterns: string[],
    availableTools: SemanticToolDefinition[]
  ): ToolChain | null {
    const steps: ToolChain['steps'] = [{ toolId: primaryTool, dependsOn: [] }];
...
let predictorInstance: ToolChainPredictor | null = null;

export function getChainPredictor(): ToolChainPredictor {
