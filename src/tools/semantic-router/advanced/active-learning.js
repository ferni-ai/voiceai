...
import { getCalibrator } from './uncertainty.js';
...
export class ActiveLearningEngine {
     ...
     */
    recordSuccess(userId, query, toolId, confidence) {
    ...
    getMetrics() {
        ...
        const correctionRate = recentWindow.length > 0
        ...
        const avgConfidenceGap = this.corrections.length > 0
     ...
     */
    recordTestMetric(testId, variant, metric, value) {
    ...
    // PRIVATE METHODS
    // ============================================================================
    calculateInformationGain(event) {
    ...
    getConfusionCount(from, to) {
        return this.confusionMatrix.get(from)?.get(to) || 0;
    }
    updateAccuracyWindow(wasCorrect) {
    ...
    getWindowedAccuracy(startIdx, endIdx) {
        ...
        const slice = this.accuracyWindows.slice(Math.max(0, actualStart), Math.min(len, actualEnd + 1));
        if (slice.length === 0) {
            return 0.5;
        }
        return slice.reduce((sum, w) => sum + w.accuracy, 0) / slice.length;
    }
    async checkRetrainTrigger() {
        ...
        const shouldRetrain = this.pendingExamples.length >= 50 ||
    ...
    async triggerRetrain() {
        ...
        try {
            ...
            for (const example of this.pendingExamples) {
                await retriever.addCorrection(example.query, '__predicted__', // We don't track original prediction here
                example.toolId);
            }
...
export function getActiveLearningEngine() {
    if (!engineInstance) {
 ...
 */
export async function recordCorrection(userId, query, predictedTool, actualTool, confidence, context = []) {
 ...
 */
export function recordSuccess(userId, query, toolId, confidence) {
    getActiveLearningEngine().recordSuccess(userId, query, toolId, confidence);
}
//# sourceMappingURL=active-learning.js.map