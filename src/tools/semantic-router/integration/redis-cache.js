 ...
 */
export function configureCacheLayer(newConfig) {
...
export class SemanticRouterCache {
    redis = null;
    memoryCache = new MemoryCache(config.maxMemoryCacheSize, config.embeddingTTLSeconds * 1000);
    initialized = false;
    initPromise = null;
    // Stats
    hits = 0;
    ...
    async getEmbedding(text, model) {
        ...
        if (this.redis?.isConnected()) {
            try {
                const cached = await this.redis.getCompressed(key);
    ...
    async setEmbedding(text, model, vector) {
        const key = `${KEY_PREFIX}emb:${model}:${this.hashKey(text)}`;
        const value = {
            vector,
            model,
    ...
    async setScores(query, scores, context) {
        const contextHash = context ? this.hashKey(context) : 'default';
        const key = `${KEY_PREFIX}scores:${contextHash}:${this.hashKey(query)}`;
        const value = {
            scores,
     ...
     */
    async getToolIndex(toolId, version) {
     ...
     */
    async getAllToolIndices(version) {
...
export function getSemanticRouterCache() {
    if (!cacheInstance) {
