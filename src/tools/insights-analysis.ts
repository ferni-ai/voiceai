...
function generateSampleSpendingSignals(): SpendingSignal[] {
  return [
...
function generateSampleTimeSignal(): TimeSignal {
  return {
...
function generateSampleHabitSignal(): HabitSignal {
  return {
    activeHabits: 4,
    averageStreak: 12,
...
function generateSampleGoalSignal(): GoalSignal {
  return {
    activeGoals: 5,
    onTrackGoals: 2,
    stalledGoals: 2,
...
function generateSampleWealthSignal(): WealthSignal {
  return {
 ...
 */
function findCorrelations(snapshot: CrossDomainSnapshot): CorrelationInsight[] {
 ...
 */
function generateProactiveInsights(snapshot: CrossDomainSnapshot): ProactiveInsight[] {
...
export function createInsightsAnalysisTools() {
  return {
    ...
    synthesizeInsights: llm.tool({
      description: `Peter's signature capability: Synthesize insights across ALL domains.
Analyzes spending, time, habits, goals, and wealth data to find:
- Hidden correlations
- Behavioral patterns
- Proactive warnings
...
- "Connect the dots for me"`,
    ...
    spotAnomalies: llm.tool({
      description: `Find unusual patterns or anomalies in user data.
Peter's pattern-detection capability. Looks for:
- Spending spikes or drops
- Habit streak breaks
- Calendar density changes
- Goal trajectory shifts
...
- "What's different?"`,
    ...
    findCorrelation: llm.tool({
      description: `Find specific correlations between two domains.
Peter's cross-pollination capability. Connects:
- Spending × Time (calendar affects spending?)
- Habits × Goals (which habits drive which goals?)
- Time × Habits (calendar killing habits?)
- Spending × Habits (emotional spending patterns?)
- Goals × Wealth (financial trajectory toward goals?)
...
Tell me more about what's happening in both areas. The more specific you are, the better I can find the real connection.`;
    ...
    projectTrends: llm.tool({
      description: `Project future trends based on current patterns.
Peter's predictive capability. Uses historical patterns to forecast:
- Where spending is headed
- When habits might break
- Goal completion likelihood
- Upcoming stress points
- Milestone readiness
...
- "What should I watch for?"`,
    ...
    generateBehavioralInsight: llm.tool({
      description: `Generate a behavioral economics insight.
Peter's behavioral analysis capability. Identifies:
- Which cognitive biases are operating
- What emotional patterns drive decisions
- Why rational advice hasn't worked
- What would actually change behavior

Use when:
- User is stuck in a pattern they can't break
- Logical solutions haven't worked
- User asks "Why do I keep doing X?"`,
      parameters: z.object({
    ...
    createInsightBriefing: llm.tool({
      description: `Create a focused insight briefing on a specific topic.
Peter's research summary capability. Good for:
- Preparing for a decision
- Understanding a specific area
- Getting Peter's take on something
- Quick insight on a topic

Use when user wants:
- "What should I know about X?"
- "Break down X for me"
- "Give me your analysis of X"`,
      parameters: z.object({
    ...
    findTheLever: llm.tool({
      description: `Find the ONE highest-impact change that would improve everything.
Peter's leverage analysis. Identifies:
- The keystone change that cascades
- Why other changes haven't worked
- The minimum viable intervention

Use when user is:
- Overwhelmed with things to fix
...
- Looking for the "one thing"`,
    ...
    runProactiveInsightScan: llm.tool({
      description: `Run Peter's proactive insight scan across all domains.
This is Peter's signature move: surfacing insights before anyone asks.
Scans for:
...
- Periodic check-ins
- User asks "anything I should know?"`,
      parameters: z.object({
        scanDepth: z
      ...
      execute: async ({ scanDepth }, { ctx }) => {
        ...
        const insights: Array<{
          type: 'warning' | 'opportunity' | 'milestone' | 'pattern' | 'anomaly';
    ...
    logPatternObservation: llm.tool({
      description: `Log a pattern observation to Peter's Pattern Journal.
This helps users develop their own pattern recognition skills.
Peter has done this for 40 years - three observations per day.

Use when:
- User notices something interesting
- User wants to track a pattern
- User says "I noticed..."`,
      parameters: z.object({
    ...
    detectBehavioralBias: llm.tool({
      description: `Detect potential cognitive biases in a decision or behavior.
Peter's behavioral economics analysis capability.
Identifies which biases might be operating and how to work with them.

Use when:
- User is making a decision
- User asks "why do I keep doing X?"
- User seems stuck in a pattern
- User is rationalizing a choice`,
      parameters: z.object({
      ...
      execute: async ({ situation, context, recentHistory }) => {
        ...
        if (recentHistory) response += `**History:** ${recentHistory}\n`;
        response += `\n---\n\n`;
    ...
    generateInsightsDashboard: llm.tool({
      description: `Generate a cross-domain insights dashboard.
Peter's at-a-glance view of all domains and their connections.
Shows status, trends, and key metrics across the user's life.

Use when:
- User wants a big-picture view
