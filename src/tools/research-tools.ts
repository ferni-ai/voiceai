...
const CATEGORY_DESCRIPTIONS: Record<StockCategory, string> = {
  ...
  asset_play:
    'Companies with hidden assets worth more than the stock price. The market missed something!',
};
 ...
 */
async function getRealStockQuote(symbol: string): Promise<{
  price: number;
  change: number;
...
export function createResearchTools() {
  return {
    analyzeStock: llm.tool({
      description: `Analyze a stock Peter John style. Looks at P/E ratio, PEG ratio, growth rate, 
...
Use when user asks about a specific stock.`,
    ...
    findStockCategory: llm.tool({
      description: `Find stocks by Peter John's categories:
...
Peter John's favorite valuation metric! PEG under 1.0 suggests a stock might be undervalued.`,
      parameters: z.object({
...
Use when user asks about finding big winners or growth stocks.`,
    ...
    explainStockCategory: llm.tool({
      description: `Explain one of Peter John's six stock categories in detail.
