...
export function createGamificationToolsV2() {
  ...
  return {
    ...
    getGamificationProfileV2: llm.tool({
      description: `Get the user's complete gamification profile with XP, level, title, badge count, and stats.
Uses Firestore subcollections for fast access.`,
      parameters: z.object({}),
      execute: async (_, { ctx }) => {
     ...
     */
    awardXPV2: llm.tool({
      description: `Award XP to the user. Automatically handles level-ups and leaderboard updates.`,
      parameters: z.object({
    ...
    awardBadgeV2: llm.tool({
      description: `Award a specific badge to the user. Checks if already earned.`,
      parameters: z.object({
    ...
    setLeaderboardPrivacy: llm.tool({
      description: `Configure privacy settings for the leaderboard.`,
      parameters: z.object({
    ...
    exportGamificationData: llm.tool({
      description: `Export all gamification data for backup. Returns a JSON structure that can be imported later.`,
      parameters: z.object({}),
      execute: async (_, { ctx }) => {
        const userData = ctx.userData as { userId?: string };
        const userId = userData.userId || 'anonymous';
    ...
    celebrateProgressV2: llm.tool({
      description: `Generate a celebration of the user's overall gamification progress.`,
      parameters: z.object({}),
      execute: async (_, { ctx }) => {
        const userData = ctx.userData as { userId?: string };
        const userId = userData.userId || 'anonymous';
