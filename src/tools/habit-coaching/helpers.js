 ...
 */
export function generateFrictionTips(badHabit) {
    const lowerHabit = badHabit.toLowerCase();
    if (lowerHabit.includes('phone') ||
 ...
 */
export function detectSetbackPattern(setbacks) {
    if (setbacks.length < 2)
        return null;
    const triggers = setbacks.map((s) => s.trigger.toLowerCase());
    // Check for common patterns
    if (triggers.filter((t) => t.includes('stress') || t.includes('tired')).length >
        triggers.length / 2) {
        return 'stress or tiredness';
    }
    if (triggers.filter((t) => t.includes('weekend') || t.includes('saturday') || t.includes('sunday'))
        .length >
        triggers.length / 2) {
        return 'weekends (less structure)';
    }
    if (triggers.filter((t) => t.includes('evening') || t.includes('night')).length >
        triggers.length / 2) {
        return 'evenings (willpower depletion)';
    }
    if (triggers.filter((t) => t.includes('alone') || t.includes('bored')).length >
        triggers.length / 2) {
 ...
 */
export function diagnoseHabitFailure(failurePoint, currentCue, currentReward) {
    const diagnoses = {
        never_start: {
            issue: 'The habit is too big or intimidating',
            explanation: "When we can't even start, it usually means the habit feels overwhelming. Your brain is protecting you from perceived effort.",
            science: 'The brain resists tasks that seem effortful. Making the habit tiny bypasses this resistance.',
            fixes: [
        ...
        start_then_stop: {
            ...
            explanation: "You can start but can't sustain because the reward isn't immediate or satisfying enough.",
            science: "Habits need immediate rewards to stick. Delayed benefits (health, success) don't motivate the habit-forming part of your brain.",
            fixes: [
        ...
        inconsistent: {
            ...
            explanation: "You can do the habit but don't do it reliably because there's no strong trigger.",
            science: 'Habits are cue-dependent. Without a consistent trigger, the behavior stays in conscious effort territory.',
            fixes: [
        ...
        hate_it: {
            ...
            explanation: "You're forcing yourself to do something that doesn't align with who you are or how you work.",
            science: "Habits that conflict with identity are unsustainable. We need to feel like 'the kind of person who...'",
            fixes: [
        ...
        forget: {
            ...
            explanation: "You want to do the habit but it's not on your mental radar. Out of sight, out of mind.",
 ...
 */
export function getMotivationalContent(type, context, _struggle) {
    const content = {
        science_fact: [
            {
 ...
 */
export function analyzeMoodPatterns(moodLogs) {
    const insights = [];
...
function calculateMoodScore(logs) {
    const moodScores = {
        great: 5,
        good: 4,
        okay: 3,
 ...
 */
export function getEncouragement(avgStreak, wins) {
 ...
 */
export function getMoodBasedTip(mood, energy, _timeOfDay) {
 ...
 */
export function getChallengeDayEncouragement(day) {
    if (day === 1)
        return 'Day 1! The hardest part is starting. You did it!';
    if (day === 7)
        return "ONE WEEK! Most people don't make it this far. You're different.";
    if (day === 14)
        return "Two weeks! You're building something real now.";
    if (day === 21)
        return 'THREE WEEKS! Research says this is when habits start to stick.';
    if (day === 30)
        return '30 DAYS! You did it! You transformed yourself!';
    if (day % 7 === 0)
        return `Week ${day / 7} complete! Keep going!`;
    return "Another day, another vote for who you're becoming.";
}
/**
 * Check for challenge milestones
 */
export function checkChallengeMilestones(day, completedDays) {
    if (day === 7 && completedDays >= 5)
        return 'First Week Champion! 5+ days completed!';
    if (day === 14 && completedDays >= 10)
        return 'Two Week Warrior! 10+ days completed!';
    if (day === 21 && completedDays >= 15)
        return 'Three Week Titan! Habit forming!';
    if (day === 30 && completedDays >= 25)
        return '30-Day Master! 25+ days! Incredible!';
    if (completedDays === 7)
        return '7-day completion streak!';
    if (completedDays === 14)
        return '14-day completion streak!';
    if (completedDays === 21)
        return "21-day streak! You're unstoppable!";
    return null;
}
