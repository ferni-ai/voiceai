...
import { LIFE_DOMAINS, LIFE_STAGES, FOUR_TENDENCIES_STRATEGIES, GLIDEPATH_LEVELS, SELF_COMPASSION_MESSAGES, ACCOUNTABILITY_TIPS, ENVIRONMENT_BUILD_STRATEGIES, ENVIRONMENT_BREAK_STRATEGIES, } from './constants.js';
...
import { generateFrictionTips, detectSetbackPattern, diagnoseHabitFailure, getMotivationalContent, analyzeMoodPatterns, getMoodBasedTip, getChallengeDayEncouragement, checkChallengeMilestones, } from './helpers.js';
import { getUserCoachData, saveUserCoachProfile, saveEnhancedHabit, saveHabitStack, saveWeeklyReflection, } from './storage.js';
// ============================================================================
// TOOL CREATOR
// ============================================================================
/**
    ...
    return {
        ...
        recommendHabits: llm.tool({
            ...
            execute: async ({ domain, goal: _goal, difficulty = 'beginner' }, { ctx }) => {
                ...
                const relevant = HABIT_TEMPLATES.filter((t) => t.domain === domain && t.difficulty === difficulty && t.goodFor.includes(data.lifeStage));
        ...
        logHabitCompletion: llm.tool({
            ...
            execute: async ({ habitId, completed, notes: _notes, feelingAfter: _feelingAfter }, { ctx }) => {
                const userData = ctx.userData;
                const userId = userData.userId || 'anonymous';
                const data = getUserCoachData(userId);
        ...
        setLifeStage: llm.tool({
            ...
            execute: async ({ stage }, { ctx }) => {
                ...
                return {
                    ...
                    priorities: stageInfo.priorities.map((p) => LIFE_DOMAINS[p].name),
        ...
        createIdentityShift: llm.tool({
            ...
            execute: async ({ currentBelief, desiredIdentity, domain, smallProofs }, { ctx }) => {
                ...
                const identityShifts = store.getUserPreference(userId, 'identityShifts') || [];
                const newShift = {
        ...
        breakBadHabit: llm.tool({
            ...
            execute: async ({ badHabit, currentCue, actualReward, replacementRoutine, frictionStrategies = [] }, { ctx }) => {
                ...
                const habitBreaks = store.getUserPreference(userId, 'habitBreaks') || [];
                const plan = {
                    id: `break_${Date.now()}`,
                    badHabit,
                    cue: currentCue,
                ...
                return {
                    ...
                    frictionTips: frictionStrategies.length > 0 ? frictionStrategies : generateFrictionTips(badHabit),
        ...
        designEnvironment: llm.tool({
            ...
            execute: async ({ habit, habitType, currentEnvironment, suggestedChanges }, { ctx }) => {
                ...
                const envDesigns = store.getUserPreference(userId, 'environmentDesigns') || [];
                const design = {
                    id: `env_${Date.now()}`,
                    habit,
                    type: habitType,
                ...
                const strategies = habitType === 'build' ? ENVIRONMENT_BUILD_STRATEGIES : ENVIRONMENT_BREAK_STRATEGIES;
        ...
        createTemptationBundle: llm.tool({
            ...
            execute: async ({ needToDo, wantToDo, bundleRule }, { ctx }) => {
                ...
                const bundles = store.getUserPreference(userId, 'temptationBundles') || [];
                const bundle = {
                    id: `bundle_${Date.now()}`,
                    needToDo,
                    wantToDo,
        ...
        processSetback: llm.tool({
            ...
            execute: async ({ habit, whatHappened, currentFeeling, lessonsLearned }, { ctx }) => {
                ...
                const setbacks = store.getUserPreference(userId, 'setbackLog') || [];
                const setback = {
                ...
                // Detect patterns
                const recentSetbacks = setbacks.filter((s) => s.habit === habit && Date.now() - new Date(s.date).getTime() < 30 * 24 * 60 * 60 * 1000);
        ...
        setupAccountability: llm.tool({
            ...
            execute: async ({ habit, accountabilityType, partnerName, checkInSchedule, consequences }, { ctx }) => {
                ...
                const accountability = store.getUserPreference(userId, 'accountabilitySystems') ||
                    [];
                const system = {
        ...
        getTodaysChallengeAction: llm.tool({
            ...
            execute: async ({ challengeId }, { ctx }) => {
                ...
                const userId = userData.userId || 'anonymous';
                const store = getProductivityStore();
                const activeId = challengeId || store.getUserPreference(userId, 'activeChallenge');
                if (!activeId) {
                ...
                for (const type of challenges) {
                    const data = store.getUserPreference(userId, `challenge_${type}`);
                ...
                const challenge = THIRTY_DAY_CHALLENGES[challengeData.type];
        ...
        logChallengeDay: llm.tool({
            ...
            execute: async ({ completed, notes, difficulty }, { ctx }) => {
                ...
                const userId = userData.userId || 'anonymous';
                const store = getProductivityStore();
                const activeId = store.getUserPreference(userId, 'activeChallenge');
                if (!activeId) {
        ...
        getHabitBundle: llm.tool({
            ...
            execute: async ({ bundleType, currentWakeTime: _currentWakeTime, availableMinutes }, { ctx }) => {
                const userData = ctx.userData;
                const userId = userData.userId || 'anonymous';
        ...
        troubleshootHabit: llm.tool({
            ...
            execute: async ({ habit, attempts, failurePoint, currentCue, currentReward, timeOfDay: _timeOfDay }, { ctx }) => {
                const userData = ctx.userData;
                const userId = userData.userId || 'anonymous';
        ...
        logMoodEnergy: llm.tool({
            ...
            execute: async ({ mood, energy, timeOfDay, habitsCompletedToday, notes }, { ctx }) => {
                ...
                const store = getProductivityStore();
                // Store mood log
                const moodLogs = store.getUserPreference(userId, 'moodLogs') || [];
                const log = {
                    id: `mood_${Date.now()}`,
                    date: new Date().toISOString(),
                    mood,
        ...
        supportLifeTransition: llm.tool({
            ...
            execute: async ({ transition, currentHabitStatus, biggestChallenge: _biggestChallenge }, { ctx }) => {
                const userData = ctx.userData;
                const userId = userData.userId || 'anonymous';
...
// HELPER
// ============================================================================
function getEncouragement(avgStreak, wins) {
    if (avgStreak > 30 && wins > 5)
        return "You're absolutely crushing it! Your consistency is inspiring.";
    if (avgStreak > 14)
        return "Incredible momentum! You're building real staying power.";
    if (avgStreak > 7)
        return "You're on a roll! Each day is strengthening these habits.";
    if (wins > 0)
        return 'Love seeing those wins! Every small victory matters.';
