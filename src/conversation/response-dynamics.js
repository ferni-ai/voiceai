...
export class ResponseDynamicsEngine {
     ...
     */
    recordMessage(role, text, topics) {
     ...
     */
    getResponseLengthRecommendation() {
        ...
        const avgUserWords = recentUserMessages.reduce((sum, m) => sum + m.wordCount, 0) / recentUserMessages.length;
     ...
     */
    getTopicTransition(fromTopic, toTopic, transitionType) {
    ...
    getEngagementMetrics() {
        ...
        const questionsAsked = userMessages.filter((m) => this.messageHistory.find((msg) => msg === m)).length; // Simplified - would need actual text
    ...
    generateLengthRationale(avgUserWords, trend, interruptionRate) {
        const parts = [];
        if (avgUserWords < 15) {
    ...
    determineTransitionType(fromTopic, toTopic) {
    ...
    generateTransitionPhrase(type, fromTopic, toTopic) {
        switch (type) {
            case 'smooth':
                if (toTopic) {
                    const phrases = [
...
// ============================================================================
let instance = null;
export function getResponseDynamicsEngine() {
    if (!instance) {
        instance = new ResponseDynamicsEngine();
    }
    return instance;
}
export function resetResponseDynamicsEngine() {
    if (instance) {
        instance.reset();
    }
    instance = null;
...
//# sourceMappingURL=response-dynamics.js.map