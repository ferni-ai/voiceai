...
const SEEKING_PERMISSION_PATTERNS = [
...
const RHETORICAL_DISTANCE_PATTERNS = [
...
const INDIRECT_ADMISSION_PATTERNS = [
...
export class SubtextDetectionEngine {
    ...
    detect(context) {
        ...
        const bestDetection = detections.reduce((best, current) => current.confidence > best.confidence ? current : best);
        ...
        const shouldAct = this.shouldActOnDetection(bestDetection.type, bestDetection.confidence, turnCount, relationshipDepth);
        // Get gentle probe
        const probes = GENTLE_PROBES[bestDetection.type];
        const gentleProbe = shouldAct && probes.length > 0
     ...
     */
    checkPatterns(message, patterns) {
        const matches = [];
     ...
     */
    shouldActOnDetection(type, confidence, turnCount, relationshipDepth) {
     ...
     */
    generateInferredMeaning(type, originalMessage) {
        switch (type) {
            case 'deflection':
...
const subtextDetectionRegistry = createSessionRegistry((sessionId) => new SubtextDetectionEngine(), { name: 'SubtextDetection', cleanup: (engine) => engine.reset(), verbose: false });
registerGlobalRegistry(subtextDetectionRegistry);
export function getSubtextDetectionEngine(sessionId) {
    return subtextDetectionRegistry.get(sessionId);
}
export function resetSubtextDetectionEngine(sessionId) {
    const engine = subtextDetectionRegistry.get(sessionId);
    engine.reset();
}
export function clearSubtextDetectionEngine(sessionId) {
    subtextDetectionRegistry.reset(sessionId);
}
export function getActiveSubtextDetectionCount() {
    return subtextDetectionRegistry.getActiveCount();
}
export default SubtextDetectionEngine;
//# sourceMappingURL=subtext-detection.js.map