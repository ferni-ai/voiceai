...
import { analyzeTurnBoundary, countWordsRust, isTurnAnalysisAvailable, isTokenCountingAvailable, } from '../memory/rust-accelerator.js';
 ...
 */
export function analyzeTranscriptCompleteness(transcript) {
    ...
    const hasVerb = /\b(is|are|was|were|have|has|had|do|does|did|will|would|can|could|should|might|'m|'re|'s|'ve|'d|'ll)\b/i.test(trimmed);
...
export class TurnPredictionService {
    ...
    predict(ctx) {
        ...
        if (ctx.silenceDurationMs > 0) {
            const silenceConfidenceBoost = this.getSilenceConfidenceBoost(ctx.silenceDurationMs, ctx.topicWeight, ctx.emotionIntensity);
        ...
        const suggestedWaitMs = this.calculateSuggestedWait(isComplete, confidence, ctx.topicWeight, ctx.emotionIntensity);
     ...
     */
    recordTurnCompletion(wordCount, pauseBeforeResponseMs) {
     ...
     */
    getSilenceConfidenceBoost(silenceMs, topicWeight, emotionIntensity) {
     ...
     */
    calculateSuggestedWait(isComplete, confidence, topicWeight, emotionIntensity) {
        if (!isComplete) {
 ...
 */
export function decidePreemptiveGeneration(prediction, currentLatencyMs) {
...
const turnPredictionRegistry = createSessionRegistry((sessionId) => new TurnPredictionService(), { name: 'TurnPrediction', cleanup: (service) => service.reset(), verbose: false });
registerGlobalRegistry(turnPredictionRegistry);
export function getTurnPredictionService(sessionId) {
    return turnPredictionRegistry.get(sessionId);
}
export function resetTurnPredictionService(sessionId) {
    turnPredictionRegistry.reset(sessionId);
}
export function resetAllTurnPrediction() {
    turnPredictionRegistry.resetAll();
}
export function getActiveTurnPredictionCount() {
    return turnPredictionRegistry.getActiveCount();
}
//# sourceMappingURL=turn-prediction.js.map