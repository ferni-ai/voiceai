 ...
 */
export function getHumanizingConfig() {
    return currentConfig;
}
/**
 * Update humanizing configuration (deep merge)
 */
export function updateHumanizingConfig(updates) {
 ...
 */
export function resetHumanizingConfig() {
    currentConfig = { ...DEFAULT_CONFIG };
}
/**
 * Get effective frequency/probability considering warmup
 */
export function getEffectiveRate(baseRate, turnNumber) {
    if (!currentConfig.global.enabled)
        return 0;
 ...
 */
export function shouldApplyFeature(featureKey, probability, turnNumber) {
    if (!currentConfig.global.enabled)
        return false;
...
export const HUMANIZING_PRESETS = {
     ...
     */
    conversational: {
        disfluency: { frequency: 0.18, personaStyle: 'conversational' },
    ...
    therapeutic: {
        ...
        hedging: { adviceHedgingRate: 0.35, defaultStrength: 'soft' },
     ...
     */
    warm: {
        disfluency: { frequency: 0.14, personaStyle: 'natural' },
        hedging: { adviceHedgingRate: 0.22, defaultStrength: 'soft' },
 ...
 */
export function getRecommendedPreset(personaId) {
    const presetMap = {
 ...
 */
const personaConfigs = new Map();
/**
 * Register humanization config from a persona bundle.
 * Called by the bundle adapter when loading a persona.
 */
export function registerBundleHumanization(personaId, bundleConfig) {
    const config = {};
    // Apply preset first
    if (bundleConfig.preset &&
        HUMANIZING_PRESETS[bundleConfig.preset]) {
        Object.assign(config, HUMANIZING_PRESETS[bundleConfig.preset]);
    }
    // Apply overrides
    ...
    if (bundleConfig.context_modifiers) {
        ...
        config._contextModifiers = bundleConfig.context_modifiers;
    }
 ...
 */
export function getPersonaHumanizingConfig(personaId) {
 ...
 */
export function getPersonaContextModifiers(personaId) {
    const personaOverrides = personaConfigs.get(personaId);
 ...
 */
export function clearPersonaConfigs() {
...
//# sourceMappingURL=humanizing-config.js.map