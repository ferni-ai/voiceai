...
export class EmotionalArcTracker {
     ...
     */
    recordEmotion(textEmotion, voiceEmotion) {
     ...
     */
    getTransitionPhrase() {
        if (!this.hasSuddenShift() || this.history.length < 2)
            return null;
        const current = this.history[this.history.length - 1];
        const previous = this.history[this.history.length - 2];
    ...
    updateNarrativePhase(snapshot) {
        ...
        if (previousPhase !== this.currentPhase) {
            this.signalEmittedThisPhase = false;
            log.debug({ from: previousPhase, to: this.currentPhase, turn: turnCount }, 'Narrative phase transition');
    ...
    createSnapshot(textEmotion, voiceEmotion) {
    ...
    computeArc() {
        ...
        const needsSupport = current.combinedValence < -0.4 ||
            this.distressCount > 2 ||
            (current.combinedArousal > 0.7 && current.combinedValence < 0);
        const stabilizing = trajectory.type === 'improving' ||
            (Math.abs(valenceMomentum) < 0.1 && Math.abs(arousalMomentum) < 0.1);
        const suddenShift = this.hasSuddenShift();
        const turnsSincePeak = this.history.length - this.history.findIndex((s) => s.combinedArousal === this.peakArousal);
    ...
    computeSmoothedValue(field) {
        if (this.history.length === 0)
            return 0;
        if (this.history.length === 1)
            return this.history[0][field];
