...
const BACKCHANNELS: Record<Backchannel['type'], Array<Omit<Backchannel, 'type'>>> = {
  ...
  encouragement: [
    ...
    { verbal: "I'd like to hear more.", ssml: "<break time=\"100ms\"/>I'd like to hear more.", energy: 'medium' },
  ],
  empathy: [
    ...
    { verbal: 'That makes sense.', ssml: "<break time=\"150ms\"/>That makes sense.", energy: 'medium' },
    { verbal: "I can imagine.", ssml: "<break time=\"150ms\"/>I can imagine.", energy: 'low' },
    { verbal: "That sounds hard.", ssml: "<volume level=\"soft\"/><break time=\"200ms\"/>That sounds hard.", energy: 'low' },
  ],
  curiosity: [
...
const PERSONA_BACKCHANNEL_STYLES: Record<string, {
  preferred: Backchannel['type'][];
  ...
  'nayan-patel': {
    ...
    energyBias: 'medium',
    uniquePhrases: [
      { phrase: 'Stay the course.', type: 'agreement', ssml: '<break time=\"100ms\"/>Stay the course.' },
  ...
  'ferni': {
    ...
    uniquePhrases: [
      ...
      { phrase: "That's powerful.", type: 'curiosity', ssml: "<break time=\"100ms\"/>That's powerful." },
      { phrase: 'Sit with that.', type: 'encouragement', ssml: '<break time=\"200ms\"/>Sit with that.' },
    ],
  },
  'peter-john': {
    ...
    uniquePhrases: [
      ...
      { phrase: 'Tell me more about that!', type: 'encouragement', ssml: 'Tell me more about that!' },
  ...
  'maya-santos': {
    ...
    uniquePhrases: [
      ...
      { phrase: "Keep going, you're doing great.", type: 'encouragement', ssml: "<break time=\"100ms\"/>Keep going, you're doing great." },
    ],
  },
  'alex-chen': {
...
export class ActiveListeningEngine {
  ...
  private extractedUserVocabulary: Set<string> = new Set();
  ...
  getBackchannelCooldownMs(): number {
    switch (this.userBackchannelPreference) {
      ...
      case 'moderate': return 3500;
      case 'minimal': return 6000;
    }
  }
  ...
  generateEmotionalEcho(
    ...
    const echoes: Record<string, Record<string, string[]>> = {
      worried: {
        ...
        high: ['This is really weighing on you, isn\'t it?', 'I can feel how much this is troubling you.'],
      },
      excited: {
  ...
  generateClarifyingQuestion(
    ...
    const questions: Record<ClarifyingQuestion['type'], Array<{ q: string; ssml: string }>> = {
      understanding: [
        { q: 'Let me make sure I understand...', ssml: '<break time=\"100ms\"/>Let me make sure I understand...' },
        { q: "So what you're saying is...", ssml: "<break time=\"100ms\"/>So what you're saying is..." },
      ...
      elaboration: [
        ...
        { q: "What's behind that thought?", ssml: "<break time=\"100ms\"/>What's behind that thought?" },
      ...
      confirmation: [
        ...
        { q: "Does that match what you're thinking?", ssml: "Does that match what you're thinking?" },
        { q: "Am I understanding correctly?", ssml: "Am I understanding correctly?" },
      ],
      emotion: [
        { q: "How does that make you feel?", ssml: "<break time=\"150ms\"/>How does that make you feel?" },
        { q: "What's the emotion behind that?", ssml: "<break time=\"100ms\"/>What's the emotion behind that?" },
  ...
  evaluateSilence(
    ...
    if (silenceDurationMs < 4000) {
      return { comfortable: false, action: 'backchannel', reason: 'Extended pause - light acknowledgment' };
    }

    return { comfortable: false, action: 'gentle_prompt', reason: 'Long silence - gentle re-engagement' };
  }
  ...
  getSilenceBackchannel(
    ...
    const silenceBackchannels: Array<Omit<Backchannel, 'type'> & { type: Backchannel['type'] }> = [
      { verbal: 'Mm-hmm.', ssml: '<volume level="soft"/><break time="100ms"/>Mm-hmm.', type: 'acknowledgment', energy: 'low' },
      { verbal: "I'm here.", ssml: '<volume level="soft"/><break time="150ms"/>I\'m here.', type: 'empathy', energy: 'low' },
      { verbal: 'Take your time.', ssml: '<volume level="soft"/><break time="200ms"/>Take your time.', type: 'encouragement', energy: 'low' },
      { verbal: 'No rush.', ssml: '<volume level="soft"/><break time="150ms"/>No rush.', type: 'encouragement', energy: 'low' },
    ];

    // Add emotion-specific options
    if (context.lastUserEmotion === 'sad' || context.lastUserEmotion === 'overwhelmed') {
      silenceBackchannels.push(
        { verbal: "I'm with you.", ssml: '<volume level="soft"/><break time="200ms"/>I\'m with you.', type: 'empathy', energy: 'low' },
        { verbal: "It's okay.", ssml: '<volume level="soft"/><break time="200ms"/>It\'s okay.', type: 'empathy', energy: 'low' }
      );
    }
    ...
    if (personaStyle) {
      ...
      if (personaId === 'ferni') {
        silenceBackchannels.push(
          { verbal: 'Sit with it.', ssml: '<volume level="soft"/><break time="200ms"/>Sit with it.', type: 'encouragement', energy: 'low' }
        );
      }
      // Jack-style (patient, wise)
      if (personaId === 'nayan-patel') {
        silenceBackchannels.push(
          { verbal: "Thinking is good.", ssml: '<volume level="soft"/><break time="150ms"/>Thinking is good.', type: 'acknowledgment', energy: 'low' }
        );
      }
    }
    ...
    getLogger().debug({ persona: personaId, backchannel: selected.verbal, silenceMs: context.silenceDurationMs }, 
  ...
  private extractNotableWords(text: string): string[] {
    ...
    const commonWords = new Set([
      ...
      'not', 'no', 'yes', 'just', 'also', 'very', 'too', 'quite', 'rather',
      'about', 'like', 'really', 'think', 'know', 'get', 'got', 'want', 'going',
      'because', 'when', 'if', 'then', 'than', 'some', 'any', 'all', 'each',
    ]);
