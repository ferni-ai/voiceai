...
export function evaluateConversationQuality(input) {
    ...
    const isSupportMoment = input.wasPersonalSharing === true ||
        (input.userEmotion
    ...
    const validationHits = validationSignals.reduce((acc, r) => acc + (r.test(input.responseText) ? 1 : 0), 0);
    ...
    const userNeedsRepair = /\b(what do you mean|i don't understand|that (doesn't|does not) make sense|no, that's not|you misunderstood)\b/i.test(input.userMessage);
    const repairSignals = /\b(let me rephrase|sorry—|sorry, i|i might have|that came out wrong|what i mean is)\b/i;
    ...
    const userAskedQuestion = /\?\s*$/.test(input.userMessage.trim()) || input.userMessage.includes('?');
    let followUp = 0.5;
