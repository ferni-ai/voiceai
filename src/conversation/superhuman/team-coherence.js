...
export class TeamCoherenceEngine {
     ...
     */
    recordHandoffNote(fromPersona, toPersona, type, content, topic) {
        const note = {
            fromPersona,
            toPersona,
            timestamp: new Date(),
        ...
        logger.debug({ userId: this.userId, from: fromPersona, to: toPersona, type }, '📝 Team handoff note recorded');
    }
    /**
     * Record topic discussed by a persona
     */
    recordTopicDiscussion(personaId, topic) {
     ...
     */
    addSharedObservation(observation) {
     ...
     */
    checkForTeamAwareness(currentPersona, context) {
     ...
     */
    generateHandoffSummary(fromPersona, toPersona, conversationContext) {
        ...
        // Record the handoff
        this.recordHandoffNote(fromPersona, toPersona, 'context', parts.join(' '), conversationContext.topics[0]);
        return parts.join(' ');
    }
    // ==========================================================================
    // HELPERS
    // ==========================================================================
    getRelevantHandoffNote(currentPersona, topic) {
        // Find notes addressed to current persona
        const relevantNotes = this.coherence.handoffNotes.filter((n) => n.toPersona === currentPersona || n.toPersona === 'all');
    ...
    findPersonaWhoDiscussedTopic(currentPersona, topic) {
    ...
    generateColleagueContextPhrase(personaId, topic) {
