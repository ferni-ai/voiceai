...
export class EmotionalMemoryEngine {
    ...
    personaId;
    lastPhraseType = null;
    phrasesUsedThisSession = new Set();
    constructor(userId, existingBond, personaId = 'ferni') {
        this.userId = userId;
        ...
        logger.debug({ userId, personaId, warmth: this.bond.warmth, trust: this.bond.trust }, '💝 EmotionalMemory initialized');
    }
    /**
    ...
    recordEvent(event, context) {
        ...
        for (const [dimension, value] of Object.entries(modifiers)) {
            ...
            if (typeof this.bond[key] === 'number') {
                this.bond[key] = Math.min(1, this.bond[key] + value * intensity);
            }
        }
    ...
    // EMOTIONAL SNAPSHOTS
    // ==========================================================================
    recordEmotionalSnapshot(emotion, trigger, topic) {
        const snapshot = {
            date: new Date(),
            emotion,
            trigger,
    ...
    recordRelationshipPeak(event, description) {
    ...
    getGreetingModifier() {
        ...
        if (stage === 'new_acquaintance')
            return null;
        // 30% chance to add warmth to greeting
        if (!seededChance(`${Date.now()}:1`, 0.3))
            return null;
        const modifiers = {
            getting_to_know: [
     ...
     */
    getEmotionalMemoryCallback(currentTopic) {
        if (this.bond.memorableEmotions.length === 0)
            return null;
        // 15% chance to reference a past emotional moment
        if (!seededChance(`${Date.now()}:2`, 0.15))
            return null;
        ...
        if (currentTopic) {
            relevantMemory =
                this.bond.memorableEmotions.find((m) => m.topic?.toLowerCase().includes(currentTopic.toLowerCase())) || null;
        }
        ...
        const daysSince = Math.floor((Date.now() - new Date(relevantMemory.date).getTime()) / (1000 * 60 * 60 * 24));
        const timePhrase = daysSince < 1
            ? 'earlier'
            : daysSince < 7
                ? 'the other day'
                : daysSince < 30
    ...
    selectPhraseType(types, context) {
    ...
    selectUnusedPhrase(phrases, topic) {
 ...
 */
export function getEmotionalMemory(userId, existingBond) {
 ...
 */
export function clearEmotionalMemory(userId) {
