...
export type ReflectionType =
  | 'feeling' // Reflect the emotion
  | 'experience' // Reflect what they're going through
  | 'meaning' // Reflect why it matters to them
  | 'validation' // Validate their response is normal
  | 'need' // Reflect what they might need
  | 'strength'; // Reflect the strength in what they're doing
...
interface SessionReflectionState {
  reflectionsUsed: string[];
  lastReflectionType: ReflectionType | null;
  turnCount: number;
}

const sessionStates = new Map<string, SessionReflectionState>();

function getSessionState(sessionId: string): SessionReflectionState {
...
export function generateReflection(context: ReflectionContext): Reflection | null {
  ...
  // Don't over-reflect
  if (state.turnCount <= 1) return null; // Let them speak first
  if (state.reflectionsUsed.length > 5) return null; // Don't overdo it
 ...
 */
export function formatReflectionGuidance(context: ReflectionContext): string | null {
  const reflection = generateReflection(context);
  if (!reflection) return null;

  const typeExplanations: Record<ReflectionType, string> = {
...
// Export for testing
export function clearReflectionStates(): void {
  sessionStates.clear();
}
