 ...
 */
export function recordConversation(userId, durationMinutes, topics = []) {
 ...
 */
export function recordGoalAchieved(userId) {
    const stats = getStats(userId);
    stats.goalsAchieved++;
    statsStore.set(userId, stats);
}
 ...
 */
export function recordBreakthrough(userId) {
    const stats = getStats(userId);
    stats.breakthroughs++;
    statsStore.set(userId, stats);
}
 ...
 */
export function recordLaugh(userId) {
    const stats = getStats(userId);
    stats.laughMoments++;
    statsStore.set(userId, stats);
}
...
export function checkMilestones(userId) {
    ...
    const daysTogether = Math.floor((Date.now() - stats.firstConversation.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export function acknowledgeMilestone(userId, type, value) {
    const stats = getStats(userId);
 ...
 */
export function formatMilestoneForPrompt(milestone) {
    const lines = [
