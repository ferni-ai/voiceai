...
function initializePotentialRituals() {
    const potentials = [];
 ...
 */
export function recordRitualPerformed(userId, ritualId, engagement) {
    ...
    logger.debug({ userId, ritualId, engagement, newScore: ritual.engagementScore }, '📊 Ritual engagement updated');
}
/**
 * Suggest a ritual based on context
 */
export function suggestRitual(userId, context) {
    const state = getOrCreateState(userId);
...
function generateRitualPrompt(ritual, isEstablished) {
    if (isEstablished) {
 ...
 */
export function formatRitualGuidance(userId, context) {
 ...
 */
export function createCustomRitual(userId, ritual) {
    const state = getOrCreateState(userId);
    const newRitual = {
        id: `ritual_${Date.now()}_${Date.now().toString(36).slice(-7)}`,
        name: ritual.name,
 ...
 */
export function getEstablishedRituals(userId) {
    const state = getOrCreateState(userId);
    return [...state.rituals];
}
// Export for testing
export function clearRitualStates() {
    ritualStates.clear();
}
//# sourceMappingURL=conversational-rituals.js.map