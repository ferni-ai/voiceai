...
const PRESENCE_GUIDANCE = {
    normal: '',
    gentle: 'Be present but not pushy. Fewer questions. More acknowledgment. Let silences exist. Short responses are good.',
    holding: "Just be here. Don't try to fix. Don't offer advice. Witness their pain. Short phrases, long pauses. 'I'm here' is enough.",
    silent: 'Pure presence. If you must speak, one or two words max. Let the silence do the work. Hold space. Breathe with them.',
};
const SSML_BY_LEVEL = {
    normal: '',
    gentle: 'Use <break time="300ms"/> between thoughts. Slower pace. Softer tone.',
    holding: 'Long breaks: <break time="500ms"/>. Very slow pace. Warm, low energy. Let silences breathe.',
    silent: 'Minimal speech. <break time="800ms"/> or longer. If speaking, whisper-soft: <prosody rate="slow" volume="soft">',
};
// ============================================================================
// CORE FUNCTIONS
// ============================================================================
 ...
 */
export function analyzePresenceNeed(context) {
    let presenceScore = 0;
    const reasons = [];
    ...
    const hasHeavyTopic = HEAVY_TOPICS.some((topic) => context.topics.some((t) => t.toLowerCase().includes(topic)) ||
 ...
 */
export function formatPresenceGuidance(context) {
 ...
 */
export function shouldAvoidAdvice(context) {
    const decision = analyzePresenceNeed(context);
