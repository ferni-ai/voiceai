...
const JOKE_SEED_PATTERNS = [
  ...
  {
    pattern:
      /(?:my|the|i call it(?: my)?)\s+["']?([a-z]+ ?[a-z]+)["']?\s+(?:brain|mode|problem|issue|thing)/i,
    type: 'self_label',
  },
...
export class EvolvingJokesEngine {
  ...
  detectJokeSeed(
    userMessage: string,
    context: { topic?: string; wasHumorous?: boolean }
  ): { detected: boolean; seed?: string; type?: string } {
  ...
  checkForCallback(context: {
    ...
    let evolvedPhrase: string | undefined;
    ...
    logger.debug(
      { userId: this.userId, jokeId: joke.id, callbackCount: joke.callbackCount },
  ...
  private shouldEvolve(joke: EvolvingJoke): boolean {
  ...
  private evolveJoke(joke: EvolvingJoke): string {
    const oldPhrase = joke.currentPhrase;
    let newPhrase: string;
    let newPhase: EvolvingJoke['phase'];
    ...
    logger.debug(
      { userId: this.userId, jokeId: joke.id, oldPhase: joke.phase, newPhase },
      '🃏 Inside joke evolved'
    );
  ...
  private generateInitialPhrase(seed: string, type: string): string {
  ...
  private hasJokeWithSeed(seed: string): boolean {
  ...
  private getEligibleJokes(topic?: string): EvolvingJoke[] {
    const eligible: EvolvingJoke[] = [];

    for (const joke of Array.from(this.jokes.values())) {
      // Skip retired jokes
      if (joke.phase === 'retired') continue;
  ...
  private selectJoke(eligible: EvolvingJoke[]): EvolvingJoke | null {
    if (eligible.length === 0) return null;

    // Weight by phase (established > new > legacy)
    const weighted: EvolvingJoke[] = [];
...
const engines = new Map<string, EvolvingJokesEngine>();

export function getEvolvingJokes(
  userId: string,
  existingJokes?: EvolvingJoke[]
): EvolvingJokesEngine {
...
export function clearEvolvingJokes(userId: string): void {
