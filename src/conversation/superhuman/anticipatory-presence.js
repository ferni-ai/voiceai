...
export class AnticipatoryPresenceEngine {
     ...
     */
    recordTopic(topic, previousTopic) {
     ...
     */
    recordEmotionalTrigger(trigger, need) {
    ...
    getAnticipation(context) {
        ...
        const temporalAnticipation = this.checkTemporalPatterns(context.hour, context.dayOfWeek, context.detectedMood);
    ...
    getThinkingOfYouMoment(context) {
        // Not too early
        if (context.turnCount < 3 || context.sessionCount < 5)
            return null;
        // 10% chance
        if (!seededChance(`${Date.now()}:1`, 0.1))
            return null;
        // Check if we have topic associations
        if (context.currentTopic) {
            const association = this.profile.topicAssociations.find((a) => a.topic === context.currentTopic && a.strength > 0.5);
            if (association) {
    ...
    checkTemporalPatterns(hour, dayOfWeek, mood) {
        ...
        for (const pattern of this.profile.temporalPatterns) {
            ...
            if (pattern.pattern.toLowerCase().includes('late night') &&
                (hour >= 22 || hour <= 4) &&
            ...
            if (pattern.pattern.toLowerCase().includes('friday') &&
                dayOfWeek === 5 &&
    ...
    checkTopicAnticipation(topic) {
        ...
        for (const association of this.profile.topicAssociations) {
            if (association.topic === topic &&
                association.strength > 0.6 &&
                recentTopics.includes(association.associatedTopic)) {
                const phrase = this.selectPhrase(ANTICIPATION_PHRASES.topic_anticipation).replace('{topic}', topic);
                return {
                    shouldAnticipate: true,
                    phrase,
    ...
    checkMoodAnticipation(mood) {
        // Check if we can predict needs based on mood patterns
        const trigger = this.profile.emotionalTriggers.find((t) => t.trigger.toLowerCase().includes(mood.toLowerCase()));
        if (trigger && trigger.frequency >= 3) {
            const moodPhrases = ANTICIPATION_PHRASES.mood_prediction[mood.toLowerCase()] || ANTICIPATION_PHRASES.mood_prediction.reflective;
            return {
                shouldAnticipate: true,
    ...
    // PATTERN UPDATING
    // ==========================================================================
    updateTemporalPatterns(hour, dayOfWeek, mood) {
        const timeOfDay = this.getTimeOfDay(hour);
        const day = DAYS[dayOfWeek];
    ...
    updateTopicAssociation(fromTopic, toTopic) {
        const existing = this.profile.topicAssociations.find((a) => a.topic === fromTopic && a.associatedTopic === toTopic);
        if (existing) {
    ...
    updateEnergyPatterns(hour, energyLevel) {
        const timeOfDay = this.getTimeOfDay(hour);
