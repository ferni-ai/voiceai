...
export class MetaRelationshipEngine {
    ...
    personaId;
    milestones = [];
    currentStage = 'new_acquaintance';
    lastMetaCommentTurn = 0;
    sessionCount = 0;
    constructor(userId, existing, personaId = 'ferni') {
        this.userId = userId;
    ...
    updateStage(stage) {
        if (stage !== this.currentStage) {
            logger.debug({ userId: this.userId, from: this.currentStage, to: stage }, '🌟 Relationship stage updated');
    ...
    checkForMetaComment(context) {
        ...
        if (context.wasVulnerable && this.sessionCount > 5 && seededChance(`${Date.now()}:221`, 0.15)) {
            this.lastMetaCommentTurn = context.turnCount;
            const phrase = getMetaRelationshipPhrase(content, 'trust_observation') ||
        ...
        if (this.sessionCount > 15 &&
            this.currentStage !== 'new_acquaintance' &&
            seededChance(`${Date.now()}:237`, 0.05)) {
            this.lastMetaCommentTurn = context.turnCount;
            const phrase = getMetaRelationshipPhrase(content, 'growth_together') ||
        ...
        if (this.currentStage === 'old_friend' && seededChance(`${Date.now()}:251`, 0.03)) {
            this.lastMetaCommentTurn = context.turnCount;
            const phrase = getMetaRelationshipPhrase(content, 'connection') ||
    ...
    checkMilestoneComment(context) {
        ...
        const milestone = this.sessionCount;
        if (milestone in sessionMilestones) {
            // Only mention once per milestone
            if (!this.milestones.some((m) => m.description === `${this.sessionCount} conversations mentioned`)) {
                this.recordMilestone('session_milestone', `${this.sessionCount} conversations mentioned`);
                return {
                    shouldComment: true,
                    type: 'milestone_acknowledgment',
                    phrase: sessionMilestones[milestone],
                };
            }
        }
        return { shouldComment: false };
...
export class SomaticPresenceEngine {
     ...
     */
    checkForSomaticCue(context, turnCount) {
...
//# sourceMappingURL=meta-relationship.js.map