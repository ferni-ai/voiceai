...
import { getBetterThanHumanContentSync, getDelightPhrase, getProtectivePhrase, getVulnerabilityPhrase, } from './content-loader.js';
const logger = createLogger({ module: 'SpontaneousDelight' });
// ============================================================================
// DELIGHT PHRASES
// ============================================================================
const DELIGHT_PHRASES = {
    appreciation: [
...
export class SpontaneousDelightEngine {
    ...
    delightHistory = [];
    lastDelightTurn = 0;
    constructor(userId, personaId = 'ferni') {
        this.userId = userId;
    ...
    checkForDelight(context) {
        ...
        logger.debug({ userId: this.userId, type, turn: context.turnCount }, '✨ Spontaneous delight emitted');
        return {
            shouldEmit: true,
            type,
            phrase,
    ...
    selectDelightType(context) {
        ...
        // Strong relationship
        if (context.relationshipStage === 'trusted_advisor' ||
        ...
        if (filtered.length === 0)
            return candidates[0]; // Fall back if all used
        return seededPick(`${Date.now()}:289`, filtered) ?? filtered[0];
    }
    getProbability(type, context) {
...
export class VisibleVulnerabilityEngine {
     ...
     */
    checkForVulnerability(context, turnCount) {
        ...
        // Try persona-specific content first
        const content = getBetterThanHumanContentSync(this.personaId);
        let phrase = getVulnerabilityPhrase(content, type);
        // Fall back to hardcoded phrases
        if (!phrase) {
        ...
        logger.debug({ userId: this.userId, type, turn: turnCount }, '💔 Visible vulnerability expressed');
        return {
            shouldExpress: true,
            type,
            phrase,
    ...
    determineVulnerabilityType(context) {
        // Outside expertise
...
export class ProtectiveInstinctsEngine {
     ...
     */
    getProtectiveResponse(type, severity, relationshipStage) {
        // Try persona-specific content first
        const content = getBetterThanHumanContentSync(this.personaId);
        let phrase = getProtectivePhrase(content, type);
        // Fall back to hardcoded phrases
        if (!phrase) {
        ...
        const shouldInterrupt = severity > 0.7 ||
...
// ============================================================================
const delightEngines = new Map();
const vulnerabilityEngines = new Map();
const protectiveEngines = new Map();
export function getSpontaneousDelight(userId) {
...
export function getVisibleVulnerability(userId) {
...
export function getProtectiveInstincts(userId) {
...
export function clearDelightEngines(userId) {
...
//# sourceMappingURL=spontaneous-delight.js.map