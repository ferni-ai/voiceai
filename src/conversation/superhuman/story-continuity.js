 ...
 */
export function extractPerson(userId, message) {
 ...
 */
export function getOrCreatePerson(userId, partial) {
    const people = peopleStore.get(userId) || [];
    // Check if this person already exists
    const existing = people.find((p) => (partial.name && p.name.toLowerCase() === partial.name.toLowerCase()) ||
    ...
    log.info({ userId, personId: person.id, name: person.name, relationship: person.relationship }, "👥 New person in user's life tracked");
    return person;
}
/**
 * Add a detail about a person
 */
export function addPersonDetail(userId, personId, detail) {
    const people = peopleStore.get(userId);
    if (!people)
        return;
 ...
 */
export function updateStoryline(userId, personId, storylineSummary, update) {
    const people = peopleStore.get(userId);
    if (!people)
        return;
    const person = people.find((p) => p.id === personId);
    ...
    let storyline = person.activeStorylines.find((s) => s.summary.toLowerCase().includes(storylineSummary.toLowerCase().slice(0, 30)));
    if (storyline) {
 ...
 */
export function findPeopleToAskAbout(userId, context) {
    const people = peopleStore.get(userId);
    ...
    const candidates = people
        ...
        if (p.activeStorylines.length === 0)
            return false;
        ...
        const storyline = person.activeStorylines.sort((a, b) => b.lastUpdate.getTime() - a.lastUpdate.getTime())[0];
 ...
 */
function generateFollowUpQuestion(person, storyline) {
 ...
 */
export function formatFollowUpForPrompt(followUp) {
    return [
...
//# sourceMappingURL=story-continuity.js.map