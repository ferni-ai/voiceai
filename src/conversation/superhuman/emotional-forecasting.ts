...
const POST_CONVERSATION_FORECASTS: Record<string, EmotionalForecast> = {
  heavy_grief: {
    ...
    acknowledgment:
      "You might feel exhausted tonight after sharing all that. That's normal—grief takes energy.",
    supportSuggestions: [
  ...
  big_decision: {
    ...
    acknowledgment:
      "Tomorrow you might wake up second-guessing this. That doesn't mean it was wrong.",
    supportSuggestions: [
  ...
  confrontation_planned: {
    ...
    acknowledgment:
      "You're probably going to feel anxious before that conversation. That's your body preparing, not a bad sign.",
    supportSuggestions: [
  ...
  good_news_shared: {
    ...
    acknowledgment:
      "If your brain tries to convince you something will go wrong, that's just fear of hope. The good thing is still real.",
    supportSuggestions: [
...
const DAY_OF_WEEK_FORECASTS: Record<number, Partial<EmotionalForecast>> = {
  0: {
    // Sunday
...
const TIME_OF_DAY_FORECASTS: Record<string, Partial<EmotionalForecast>> = {
  late_night: {
    predictedEmotion: 'spiraling or raw',
    reason: 'Late night thoughts hit different',
    acknowledgment:
      "Late night feelings are real, but they're also amplified. Things often look different in daylight.",
  },
  early_morning: {
 ...
 */
export function generateForecast(context: ForecastContext): EmotionalForecast | null {
  const forecasts: EmotionalForecast[] = [];

  // Check for heavy topics
  const heavyTopics = ['grief', 'death', 'trauma', 'abuse', 'loss', 'divorce', 'breakup'];
  const hasHeavyTopic = context.topics.some((t) =>
  ...
  logger.debug(
    { predictedEmotion: bestForecast.predictedEmotion, confidence: bestForecast.confidence },
 ...
 */
export function formatForecastGuidance(context: ForecastContext): string | null {
  const forecast = generateForecast(context);
  if (!forecast || forecast.confidence < 0.5) return null;
 ...
 */
export function getForecastAcknowledgment(context: ForecastContext): string | null {
  const forecast = generateForecast(context);
  if (!forecast || forecast.confidence < 0.6) return null;
 ...
 */
export function shouldMentionForecast(context: ForecastContext): boolean {
  const forecast = generateForecast(context);
  if (!forecast) return false;

  // High confidence forecasts after heavy conversations should be mentioned
  if (forecast.confidence >= 0.7 && context.hadHeavySharing) return true;

  // Decision forecasts should be mentioned
  if (context.madeDecision && forecast.confidence >= 0.6) return true;
