 ...
 */
import { seededIndex, seededPick } from '../utils/rng.js';
 ...
 */
export function detectChallengeOpportunity(context) {
    ...
    const softLanding = seededPick(`${Date.now()}:3`, templates.softLandings) ?? templates.softLandings[0];
 ...
 */
export function formatChallengeGuidance(context) {
 ...
 */
export function isGoodTimeToChallenge(context) {
    // Not in first few turns
    if (context.turnCount < 5)
        return false;
    // Not when highly emotional
    if (['sad', 'anxious', 'devastated', 'angry'].includes(context.emotion))
        return false;
    // Not with strangers
 ...
 */
export function getSoftChallenge(type) {
...
//# sourceMappingURL=gentle-challenges.js.map