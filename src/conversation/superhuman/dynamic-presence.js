 ...
 */
function extractSpecificReference(message) {
    ...
    const actionPattern = message.match(/\b(decided|realized|discovered|found out|noticed|remembered|forgot)\b/i);
    if (actionPattern) {
...
function detectTopic(message) {
    ...
    const topicPatterns = {
        ...
        relationship: /\b(relationship|dating|partner|love|breakup|marriage|divorce|boyfriend|girlfriend)\b/,
 ...
 */
function generateSpecificPresence(context) {
    ...
    return (seededPick(`${context.sessionId || Date.now()}:fallback`, EMBODIED_PRESENCE.simplePresence) ??
        EMBODIED_PRESENCE.simplePresence[0]);
}
/**
 * Generate presence through physicality (without "sitting with")
 */
function generatePhysicalPresence(context) {
 ...
 */
function generateNoticingPresence(context) {
 ...
 */
function generateCuriosityPresence(context) {
 ...
 */
function generateTopicPresence(context) {
    const topic = context.topic || (context.lastUserMessage ? detectTopic(context.lastUserMessage) : 'general');
 ...
 */
export function generatePresenceExpression(context) {
    ...
    let selectedStyle = 'through_physicality';
 ...
 */
export function generateMinimalPresence() {
    const minimal = [
        '<break time="400ms"/>',
        '<break time="300ms"/>Mm.',
        '<break time="400ms"/>Yeah.',
 ...
 */
export function isSittingWithCliche(phrase) {
    const lower = phrase.toLowerCase();
    return (lower.includes('sitting with') ||
 ...
 */
export function rewriteSittingWithPhrase(originalPhrase, context) {
...
//# sourceMappingURL=dynamic-presence.js.map