 ...
 */
export function extractQuote(userId, message, context) {
    ...
    if (message.length > 150) {
        ...
        if (sentences && sentences.length > 0) {
            ...
            quote =
                sentences.find((s) => WISDOM_PATTERNS.some((p) => p.test(s)) ||
                    VULNERABLE_PATTERNS.some((p) => p.test(s)) ||
                    GOAL_PATTERNS.some((p) => p.test(s)) ||
                    FUNNY_PATTERNS.some((p) => p.test(s))) || sentences[0];
        }
        else {
...
export function saveQuote(quote) {
    ...
    const isDupe = existing.some((q) => q.quote.toLowerCase().includes(quote.quote.toLowerCase().slice(0, 50)) &&
 ...
 */
export function captureQuote(userId, message, context) {
 ...
 */
export function findRelevantQuote(context) {
    ...
    const eligibleQuotes = quotes.filter((q) => {
        if (q.surfacedCount >= 3)
            return false; // Max 3 times ever
        if (q.lastSurfaced) {
            const daysSince = (Date.now() - q.lastSurfaced.getTime()) / (1000 * 60 * 60 * 24);
            if (daysSince < 7)
                return false; // Wait at least a week between surfacing
        }
        return true;
    });
    ...
    const scored = eligibleQuotes.map((quote) => {
        ...
        if (context.currentTopic &&
            quote.topics.some((t) => t.toLowerCase().includes(context.currentTopic.toLowerCase()) ||
...
function generateQuoteIntro(quote, reason) {
    ...
    const options = intros[reason] || intros['topic match'];
    return seededPick(`${Date.now()}:377`, options) ?? options[0];
}
/**
 * Mark a quote as surfaced (call after using it)
 */
export function markQuoteSurfaced(userId, quoteId) {
 ...
 */
export function formatQuoteForPrompt(suggestion) {
    return [
...
//# sourceMappingURL=quote-memory.js.map