...
export class TemporalEmotionalEngine {
     ...
     */
    getComparativeObservation(currentEmotion, currentTopic) {
        if (this.profile.sessionEmotions.length < 3)
            return null;
        // Only 10% chance
        ...
        if (currentTopic) {
            const topicSessions = this.profile.sessionEmotions.filter((s) => s.topics.includes(currentTopic));
            if (topicSessions.length >= 2) {
                const concernRate = topicSessions.filter((s) => s.concernsDetected).length / topicSessions.length;
                if (concernRate > 0.5) {
    ...
    // ANALYSIS
    // ==========================================================================
    checkEnergyComparison(currentEnergy) {
    ...
    checkMoodShift() {
        ...
        const daysSinceShift = Math.floor((Date.now() - new Date(recentShift.to).getTime()) / (1000 * 60 * 60 * 24));
        // Only mention if shift was recent-ish (within 14 days)
        if (daysSinceShift > 14) {
            return { shouldMention: false };
        }
        const phrases = TEMPORAL_PHRASES.mood_shift[recentShift.direction === 'positive' ? 'improving' : 'declining'];
        return {
            shouldMention: true,
            type: 'mood_shift',
    ...
    checkGrowthObservation() {
        ...
        if (recentAvgPositivity - earlyAvgPositivity > 0.15 ||
     ...
     */
    checkOpennessComparison(currentOpenness) {
        if (this.profile.sessionEmotions.length < 2) {
            return { shouldMention: false };
        }
        const lastSession = this.profile.sessionEmotions[this.profile.sessionEmotions.length - 1];
    ...
    inferOpenness(snapshot) {
        ...
        const hasPersonalTopic = snapshot.topics.some((t) => personalTopics.some((p) => t.toLowerCase().includes(p)));
        if (hasPersonalTopic) {
            openness += 0.2;
        }
