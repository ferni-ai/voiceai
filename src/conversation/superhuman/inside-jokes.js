 ...
 */
export function detectJokeMoment(userId, message, context) {
    ...
    if (message.length > 200) {
        ...
        const funnySentence = sentences.find((s) => HUMOR_INDICATORS.some((p) => p.test(s)) || NICKNAME_PATTERNS.some((p) => p.test(s)));
        funnyPart = funnySentence || sentences[0];
    }
    // Determine related topics
    const relatedTopics = (context.topics || []).filter((t) => JOKE_TOPICS.some((jt) => t.toLowerCase().includes(jt)));
...
export function saveJoke(joke) {
    ...
    const isDupe = existing.some((j) => j.shorthand === joke.shorthand ||
 ...
 */
export function captureJoke(userId, message, context) {
 ...
 */
export function findRelevantJoke(userId, context) {
    ...
    const eligibleJokes = jokes.filter((j) => {
        if (j.timesReferenced >= 5)
            return false; // Max 5 times
        if (j.lastReferenced) {
            const daysSince = (Date.now() - j.lastReferenced.getTime()) / (1000 * 60 * 60 * 24);
            if (daysSince < 3)
                return false; // Wait at least 3 days
        }
        // Don't use supportive jokes in light moods
        if (j.mood === 'supportive' && context.currentMood === 'light')
            return false;
        return true;
    });
    ...
    const scored = eligibleJokes.map((joke) => {
        ...
        // Topic match bonus
        if (context.currentTopics?.some((t) => joke.relatedTopics.some((jt) => t.toLowerCase().includes(jt) || jt.includes(t.toLowerCase())))) {
            score += 30;
        }
 ...
 */
function generateNaturalUsage(joke) {
    if (joke.shorthand) {
        const usages = [
 ...
 */
export function markJokeReferenced(userId, jokeId) {
    const jokes = jokeStore.get(userId);
    if (!jokes)
        return;
 ...
 */
export function formatJokeForPrompt(reference) {
    return [
