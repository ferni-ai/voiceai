...
const PERSON_PATTERNS = [
    {
        pattern: /my (mom|mother|dad|father|brother|sister|wife|husband|partner|friend|boss|coworker|colleague)/i,
        relationship: true,
    },
...
const UNRESOLVED_PATTERNS = [
    ...
    { pattern: /i (need|have|want) to (.+)/i, category: 'goal' },
    { pattern: /i'?m (not sure|deciding|thinking about) (.+)/i, category: 'decision' },
    { pattern: /i (don'?t know|wonder) (if|whether|what|how) (.+)/i, category: 'question' },
    { pattern: /(this|that|it) (is|has been) (.+)/i, category: 'situation' },
    { pattern: /i'?ve been (struggling|dealing) with (.+)/i, category: 'problem' },
];
...
export class CuriosityEngine {
    ...
    curiosityHistory = [];
    turnCount = 0;
    lastCuriosityTurn = -10;
    // Config - "Better Than Human" relationship building
    // Reduced intervals to keep conversations moving forward
    MIN_CURIOSITY_INTERVAL = 3; // Was 5 - show genuine interest more often
    MAX_FOLLOW_UPS_PER_THREAD = 4; // Was 3 - follow threads more deeply
    MAX_CURIOSITY_PER_SESSION = 10; // Was 6 - more engaged conversations
    constructor() {
     ...
     */
    processMessage(message, turnCount) {
     ...
     */
    getCuriosityPrompt(turnCount, recentTopics) {
        this.turnCount = turnCount;
     ...
     */
    recordCuriosityOutcome(wasWellReceived) {
    ...
    // PRIVATE METHODS
    // ============================================================================
    extractLifeDetails(message, turn) {
        // Extract people
        for (const { pattern, relationship, extractName } of PERSON_PATTERNS) {
            ...
            if (match) {
                ...
                // Don't duplicate
                const exists = this.lifeDetails.find((d) => d.category === 'person' && d.content.toLowerCase() === detail.content.toLowerCase());
                if (!exists) {
    ...
    findBestCuriosityPrompt(recentTopics) {
        const candidates = [];
        // Check unresolved threads
        for (const thread of this.threads.values()) {
            if (thread.resolved)
                continue;
            if (thread.followUpCount >= this.MAX_FOLLOW_UPS_PER_THREAD)
                continue;
            // Don't ask about very recent threads
            if (this.turnCount - thread.lastReferencedTurn < 3)
                continue;
            ...
            if (thread.importance === 'high')
                confidence += 0.15;
            if (this.turnCount - thread.introducedTurn > 10)
                confidence += 0.1; // Older = more natural
            if (thread.followUpCount === 0)
                confidence += 0.1; // Never asked
            candidates.push({
                question,
                type: 'follow_up',
                threadId: thread.id,
...
const curiosityEngineRegistry = createSessionRegistry((userId) => new CuriosityEngine(), {
    name: 'CuriosityEngine',
    cleanup: (engine) => engine.reset(),
    verbose: false,
});
registerGlobalRegistry(curiosityEngineRegistry);
export function getCuriosityEngine(userId) {
    return curiosityEngineRegistry.get(userId);
}
export function resetCuriosityEngine(userId) {
    const engine = curiosityEngineRegistry.get(userId);
    engine.reset();
}
export function clearCuriosityEngine(userId) {
    curiosityEngineRegistry.reset(userId);
}
export function getActiveCuriosityEngineCount() {
