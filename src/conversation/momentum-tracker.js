...
// ============================================================================
function calculateMomentumScore(signals, profile) {
    if (signals.length === 0)
        return 0.5;
    ...
    for (const signal of recent) {
        ...
        if (signal.responseLatencyMs !== undefined) {
            if (signal.responseLatencyMs < 2000)
                latencyFactor = 0.05;
            else if (signal.responseLatencyMs > 8000)
                latencyFactor = -0.05;
        }
...
function detectMomentumState(score, signals, profile) {
    if (signals.length < 2)
        return 'building';
    const recent = signals.slice(-3);
    const avgEmotionalIntensity = recent.reduce((sum, s) => sum + s.emotionalIntensity, 0) / recent.length;
    ...
    const stalledTurns = recent.filter((s) => s.wordCount < 10 && !s.questionAsked && !s.selfDisclosure).length;
...
function detectConversationPhase(turnCount, signals) {
    if (turnCount <= 3)
        return 'opening';
    ...
    if (hasClosingSignal && turnCount > 5)
        return 'closing';
    if (turnCount > 10)
        return 'deep';
    return 'exploring';
...
function generateSuggestions(state, phase, velocity, score) {
    const suggestions = [];
    switch (state) {
        case 'building':
            suggestions.push({
...
export class ConversationMomentumTracker {
    ...
    constructor(personaId = 'default', sessionId = 'default') {
        ...
        log.debug({ personaId, profileType: personaId in PERSONA_PROFILES ? 'custom' : 'default' }, 'Momentum tracker initialized');
    }
    /**
    ...
    recordSignal(signal) {
        ...
        if (this.currentState === oldState) {
            ...
            log.debug({ oldState, newState: this.currentState, turn: fullSignal.turn }, 'Momentum state changed');
            // Record transition for metrics
            recordMomentumTransition(this.sessionId, this.personaId, oldState, this.currentState, fullSignal.turn);
        }
        // Track peaks
     ...
     */
    isGoodTimeForTangent() {
        // Good times: cruising, early building, after peak
        if (this.currentState === 'intimate')
            return false; // Don't interrupt
        if (this.currentState === 'peaking')
            return false; // Let user have the moment
        if (this.currentState === 'stalled')
            return true; // Tangent might help
        if (this.currentState === 'cruising' && this.turnsInCurrentState > 2)
            return true;
        if (this.currentState === 'building' && this.topicDepth > 3)
            return true;
        return false;
    }
    /**
    ...
    shouldSlowDown() {
        ...
        if (this.currentState === 'intimate')
            return true;
        if (this.lastPeakTurn && this.signals.length - this.lastPeakTurn < 2)
            return true;
        const recent = this.signals.slice(-2);
        const slowResponses = recent.filter((s) => s.responseLatencyMs && s.responseLatencyMs > 5000).length;
...
export function getMomentumTracker(sessionId, personaId) {
...
export function resetMomentumTracker(sessionId) {
    trackers.delete(sessionId);
}
export function resetAllMomentumTrackers() {
    trackers.clear();
}
//# sourceMappingURL=momentum-tracker.js.map