...
import { analyzeLinguistic, analyzeBehavioral, analyzeProsody, analyzeBreathing, analyzeTemporal, } from './analyzers.js';
...
export class ConcernDetectionEngine {
    ...
    turnCount = 0;
    engagementHistory = [];
    responseLengthHistory = [];
    lastProsodySignals = null;
    lastBreathingSignals = null;
    userBaseline = { ...DEFAULT_USER_BASELINE };
    constructor() {
    ...
    analyze(userMessage, context) {
        ...
        const behavioralResult = analyzeBehavioral(userMessage, {
    ...
    computeState() {
        ...
        if (crisisSignal) {
            return {
                ...
                responseGuidance: 'This person may be in crisis. Prioritize safety. Gently ask if they are safe and provide crisis resources.',
            };
        }
        ...
        if (score < 0.15)
            ...
            level = 'elevated'; // Don't auto-escalate to crisis from score alone
    ...
    determineApproach(level, primaryConcern, escalating) {
        if (level === 'crisis') {
            return {
                approach: 'safety_check',
                guidance: 'This person may be in crisis. Prioritize safety above all else. Ask directly if they are safe.',
            };
        }
        if (level === 'elevated' || (level === 'moderate' && escalating)) {
            if (primaryConcern === 'hopelessness') {
                return {
                    approach: 'hold_space',
                    guidance: "This person is expressing hopelessness. Don't try to fix or reframe. Be present. Acknowledge the pain. Only after validation, gently explore what small thing might help.",
                };
            }
            return {
        ...
        if (level === 'moderate') {
            if (primaryConcern === 'loneliness') {
                return {
                    ...
                    guidance: "This person may be feeling alone. Your presence matters more than your words. Don't rush to solutions.",
                };
            }
            if (primaryConcern === 'exhaustion') {
                return {
                    approach: 'slow_down',
                    guidance: 'This person seems exhausted. Keep your responses shorter. Offer to slow down or take a break.',
                };
            }
            return {
                approach: 'check_in',
        ...
        if (level === 'mild') {
            return {
                approach: 'gentle_presence',
                guidance: 'Mild concern signals present. Stay attuned. No action needed yet, but watch for escalation.',
            };
        }
        return {
            approach: 'normal',
    ...
    // HELPERS
    // ==========================================================================
    addSignal(source, type, confidence, indicator) {
        const existing = this.signals.find((s) => s.source === source && s.type === type && s.indicator === indicator);
        if (existing) {
