 ...
 */
export function analyzeLinguistic(text, addSignal, existingSignals) {
    ...
    if (absolutistCount >= 2) {
        ...
        addSignal('linguistic', existingType, 0.3 + absolutistCount * 0.1, 'Absolutist language spiral');
    }
}
/**
 * Analyze behavioral patterns in user interaction
 */
export function analyzeBehavioral(text, context, state, addSignal) {
    ...
    if (context.previousTopics && context.currentTopic) {
        ...
        const wasDiscussingHeavy = context.previousTopics.some((t) => heavyTopics.some((h) => t.toLowerCase().includes(h)));
 ...
 */
export function analyzeProsody(prosody, addSignal) {
 ...
 */
export function analyzeBreathing(breathing, addSignal) {
 ...
 */
export function analyzeTemporal(temporal, existingSignals, addSignal) {
    ...
    if (temporal.historicalVulnerability && temporal.historicalVulnerability > 0.5) {
        addSignal('temporal', 'anxiety', temporal.historicalVulnerability * 0.5, 'Historical vulnerability at this time');
    }
