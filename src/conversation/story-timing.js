...
export class StoryTimingEngine {
    storiesTold = new Map(); // storyId -> turnTold
    lastStoryTurn = -10;
    storyOutcomes = new Map(); // storyId -> wasWellReceived
    // Timing parameters
    minTurnsBeforeFirstStory = 4;
    minTurnsBetweenStories = 6;
    maxStoriesPerSession = 4;
    rapportThreshold = 5; // turns needed for "deep" stories
    constructor() {
     ...
     */
    evaluateStoryTiming(persona, context, candidateStory) {
     ...
     */
    findBestStory(persona, context) {
        if (!persona.stories || persona.stories.length === 0) {
            return null;
        }
        // Filter out already-told stories this session
        const availableStories = persona.stories.filter((s) => !context.storiesToldThisSession.includes(s.id));
     ...
     */
    recordStoryOutcome(storyId, wasWellReceived) {
    ...
    scoreStoryFit(story, context) {
        let score = 0.5; // Base score
        // Topic relevance
        if (context.currentTopic || context.recentTopics) {
            const topics = [context.currentTopic, ...(context.recentTopics || [])].filter(Boolean);
            const triggerMatch = story.triggers.some((trigger) => topics.some((topic) => topic?.toLowerCase().includes(trigger.toLowerCase()) ||
    ...
    checkEmotionalFit(story, context) {
    ...
    generateTransition(story, context) {
        const transitions = [
...
// ============================================================================
let instance = null;
export function getStoryTimingEngine() {
    if (!instance) {
        instance = new StoryTimingEngine();
    }
    return instance;
}
export function resetStoryTimingEngine() {
    if (instance) {
        instance.reset();
    }
    instance = null;
...
//# sourceMappingURL=story-timing.js.map