...
const TOPIC_RESISTANCE_SIGNALS = [
...
export class ConversationalRepairEngine {
     ...
     */
    recordTurn(speaker, message, turnCount) {
     ...
     */
    analyze(userMessage, previousAgentMessage) {
        const signals = [];
        ...
        const primaryMiscue = signals.reduce((best, current) => current.confidence > best.confidence ? current : best);
        // Update tracking
...
const conversationalRepairRegistry = createSessionRegistry((sessionId) => new ConversationalRepairEngine(), { name: 'ConversationalRepair', cleanup: (engine) => engine.reset(), verbose: false });
registerGlobalRegistry(conversationalRepairRegistry);
export function getConversationalRepairEngine(sessionId) {
    return conversationalRepairRegistry.get(sessionId);
}
export function resetConversationalRepairEngine(sessionId) {
    const engine = conversationalRepairRegistry.get(sessionId);
    engine.reset();
}
export function clearConversationalRepairEngine(sessionId) {
    conversationalRepairRegistry.reset(sessionId);
}
export function getActiveConversationalRepairCount() {
    return conversationalRepairRegistry.getActiveCount();
}
export default ConversationalRepairEngine;
//# sourceMappingURL=conversational-repair.js.map