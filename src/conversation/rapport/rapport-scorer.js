 ...
 */
export function getRapportScorer(sessionId) {
 ...
 */
export function resetRapportScorer(sessionId) {
 ...
 */
export function getActiveRapportScorerCount() {
    return scorers.size;
}
// ============================================================================
...
export class RapportScorer {
     ...
     */
    updateTurnBalanceSignal(obs, now) {
     ...
     */
    updateInterruptionSignal(obs, now) {
     ...
     */
    updateEngagementSignal(obs, now) {
        let rawValue = 0.3; // Base score
        // Response length
        if (obs.responseLength === 'medium')
            rawValue += 0.2;
        if (obs.responseLength === 'long')
            rawValue += 0.3;
        // High engagement behaviors
        if (obs.userAskedQuestion)
            rawValue += 0.15;
        if (obs.userElaborated)
            rawValue += 0.15;
        if (obs.userIntroducedTopic)
            rawValue += 0.1;
        if (obs.userShowedEmotion)
            rawValue += 0.1;
     ...
     */
    updateEmotionalAlignmentSignal(obs, now) {
     ...
     */
    updateFlowContinuitySignal(obs, now) {
     ...
     * Update trust signal
     */
    updateTrustSignal(obs, now) {
        let rawValue = 0.5; // Neutral baseline
        // Positive trust signals
        if (obs.userDisclosed)
            rawValue += 0.15;
        if (obs.userShowedVulnerability)
            rawValue += 0.2;
        if (obs.userAskedForHelp)
            rawValue += 0.15;
        // Negative signals
        if (obs.userExpressedSkepticism)
            rawValue -= 0.2;
     ...
     */
    updateSignal(name, newValue, now) {
        const signal = this.currentSignals.get(name);
        if (!signal)
            return;
        // EMA smoothing
        const smoothedValue = RAPPORT_CONFIG.EMA_ALPHA * newValue + (1 - RAPPORT_CONFIG.EMA_ALPHA) * signal.value;
     ...
     */
    activateRepairStrategy(strategy) {
...
//# sourceMappingURL=rapport-scorer.js.map