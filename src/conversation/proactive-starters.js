...
function daysSinceLastConversation(lastDate) {
    const now = new Date();
 ...
 */
export function generateProactiveOpener(persona, context) {
 ...
 */
export async function generateProactiveOpenerAsync(persona, context) {
    const openerType = determineOpenerType(context);
    const llmContext = {
        contentType: 'proactive_starter',
        personaId: persona.id,
...
function determineOpenerType(context) {
    ...
    if (context.pendingIntentions && context.pendingIntentions.length > 0) {
        ...
        const dueIntention = context.pendingIntentions.find((i) => {
            if (!i.targetTime) {
                ...
                const hoursSinceStated = (now.getTime() - new Date(i.statedAt).getTime()) / (1000 * 60 * 60);
...
// ============================================================================
function generateFirstMeetingOpener(persona, _context) {
    const time = getTimeOfDay();
    const { name } = persona;
...
function generateReturningRecentOpener(persona, context) {
...
function generateReturningFamiliarOpener(persona, context) {
...
function generateReconnectOpener(persona, context) {
    const { userName } = context;
    const daysSince = context.lastConversationDate
        ? daysSinceLastConversation(context.lastConversationDate)
        : 30;
...
function generateMemoryCallbackOpener(persona, context) {
    const userName = context.userName || '';
    const nameStr = userName ? `${userName}, ` : '';
    let greeting;
    let followUp;
...
function generateThreadContinuityOpener(persona, context) {
    const userName = context.userName || '';
    const nameStr = userName ? `${userName}, ` : '';
...
function generateTimeAwareOpener(persona, context) {
    const time = getTimeOfDay();
...
function generateSeasonalOpener(persona, context) {
...
function generateCalendarAwareOpener(persona, context) {
    const userName = context.userName || '';
    const nameStr = userName ? `${userName}, ` : '';
 ...
 */
function generateIntentionFollowupOpener(persona, context) {
    const userName = context.userName || '';
    const nameStr = userName ? `${userName}` : '';
 ...
 */
export function buildOpenerContext(profile, isReturningUser) {
    if (!profile) {
...
//# sourceMappingURL=proactive-starters.js.map