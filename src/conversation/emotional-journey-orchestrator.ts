...
export type EmotionalMomentType =
  | 'warm_welcome' // Smile
  | 'delightful_surprise' // Laugh
  | 'vulnerability_invitation' // Opening to tears
  | 'protective_embrace' // Safety in tears
  | 'breakthrough_insight' // Joy of discovery
  | 'celebration_of_effort' // Pride
  | 'meaningful_farewell'; // Bittersweet warmth
...
function detectJourneyPhase(ctx: EmotionalContext): JourneyPhase {
...
function isBreakthroughMoment(ctx: EmotionalContext): boolean {
  const breakthroughTopics = ['realized', 'understand', 'clicked', 'see now', 'makes sense'];
  return ctx.topicsTouched.some((t) =>
    breakthroughTopics.some((bt) => t.toLowerCase().includes(bt))
  );
}

function isCelebrationMoment(ctx: EmotionalContext): boolean {
  const positiveEmotions = ['happy', 'excited', 'proud', 'grateful', 'hopeful', 'relieved'];
  return (
    positiveEmotions.includes(ctx.userEmotion.toLowerCase()) &&
...
function selectEmotionalMoment(
  phase: JourneyPhase,
  ctx: EmotionalContext
): EmotionalMomentType | null {
  switch (phase) {
    case 'arrival':
...
function coordinateSystems(
  phase: JourneyPhase,
  ctx: EmotionalContext
): { activate: string[]; suppress: string[] } {
...
function selectCoachingMode(
  phase: JourneyPhase,
  ctx: EmotionalContext
): JourneyDecision['coachingMode'] {
...
function generateGuidance(
  phase: JourneyPhase,
  moment: EmotionalMomentType | null,
  coachingMode: JourneyDecision['coachingMode'],
  ctx: EmotionalContext
): string {
  const parts: string[] = [];
 ...
 */
export function orchestrateEmotionalJourney(ctx: EmotionalContext): JourneyDecision {
...
function buildReasoning(
  ...
  if (ctx.distressLevel > 0.3) parts.push(`Distress: ${ctx.distressLevel.toFixed(2)}`);
  if (ctx.resistanceDetected) parts.push(`Resistance detected`);
  if (ctx.vulnerabilityShared) parts.push(`Vulnerability shared`);
  if (moment) parts.push(`Emotional moment: ${moment}`);
...
export function buildEmotionalContext(params: {
  ...
  voiceEmotion?: { arousal?: number; valence?: number; speechRate?: number };
  resistanceDetected?: boolean;
  vulnerabilityShared?: boolean;
  wasAdviceGiven?: boolean;
  topicsTouched?: string[];
