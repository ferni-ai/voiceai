...
import { applyDeepHumanization, getMoodTracker, } from '../deep-humanization/index.js';
import { getEmotionalArcTracker } from '../emotional-arc.js';
import { getHumanizationOrchestrator, humanizationAnalytics, humanizationConfig, } from '../humanization/index.js';
...
import { detectAdviceGiving, detectBreakthrough, detectDisengagement, detectEvidence, detectHighEngagement, } from '../utils/detection.js';
import { detectUserEnergy, humanizeVocals } from '../vocal-humanization.js';
import { applySsmlEnhancements, createDeterministicTrigger, getComfortLevel, getTimeOfDay, mapRelationshipStage, shouldAddUncertainty, stripSsml, } from './utils.js';
...
export class PostLlmProcessor {
    ...
    listening = getActiveListeningEngine();
    memory = getConversationalMemory();
    questions = getQuestionPatternEngine();
    emotional = getEmotionalArcTracker();
    dynamics = getResponseDynamicsEngine();
    silencePresence = getSilencePresenceEngine();
    // Tracking
    lastResponseHadCallback = false;
    lastSessionInsight = null;
    lastBetterThanHumanInsight = null;
    constructor(personaId, sessionId, userId, sessionCount = 0, sessionStartTime) {
    ...
    async humanizeResponseAsync(rawResponse, context) {
        ...
        const advResult = await this.applyAdvancedHumanization(enhancedText, enhancedSsml, context, signals);
    ...
    applySilencePresence(text, ssml, context) {
        const topicWeight = classifyTopicWeight(context.userMessage, context.userEmotion);
        ...
        if (silenceDecision.useSilence) {
            const { text: withSilence, ssml: ssmlWithSilence } = this.silencePresence.applyToResponse(text, silenceDecision);
            log.debug({ reason: silenceDecision.reason, duration: silenceDecision.duration }, 'Applied silence presence to response');
            return {
                text: withSilence,
    ...
    applyBetterThanHuman(text, ssml, context) {
        const features = [];
        try {
            if (!this.userId) {
                return { text, ssml, features };
            }
            const betterThanHuman = getBetterThanHuman(this.userId, this.sessionId, this.personaId, this.sessionCount);
            const bthContext = {
                userMessage: context.userMessage,
                turnCount: context.turnNumber,
    ...
    async applyAdvancedHumanization(text, ssml, context, signals) {
        const features = [];
        try {
            const sessionId = context.sessionData?.sessionId ||
                `humanizer-${this.personaId}`;
            if (!humanizationConfig.isEnabled('selfCorrection') &&
            ...
            const orchestratorContext = {
                ...
                responseComplexity: (RUST_COUNTING_AVAILABLE ? countWordsRust(text) : text.split(/\s+/).length) > 50
                    ? 0.7
                    : 0.3,
            ...
            if (orchestratorResult.appliedHumanizations.length > 0) {
                ...
                for (const humanization of orchestratorResult.appliedHumanizations) {
                    humanizationAnalytics.recordApplied(sessionId, humanization.type, { placement: humanization.placement }, {
                        turnCount: context.turnNumber,
...
//# sourceMappingURL=post-llm.js.map