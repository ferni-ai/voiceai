...
export class PreLlmProcessor {
    ...
    constructor(personaId, sessionId) {
    ...
    processUserMessage(context) {
        ...
        if (context.userEmotion && context.wasPersonalSharing) {
            actions.acknowledgment = this.listening.generateEmotionalEcho(context.userEmotion, context.userMessage, 'medium');
        }
        return actions;
    }
    /**
     ...
     */
    generateContextGuidance(context, sessionInsight) {
        const guidance = [];
        ...
        const needsEmotionalSupport = context.wasPersonalSharing ||
            (context.userEmotion &&
     ...
     */
    addSessionIntelligenceGuidance(guidance, insight) {
        if (!insight)
            return;
        ...
        if (insight.predictions.voiceState.acknowledgment &&
        ...
        if (insight.responseGuidance.approach !== 'normal') {
            const avoidStr = insight.responseGuidance.avoid.length > 0
...
//# sourceMappingURL=pre-llm.js.map