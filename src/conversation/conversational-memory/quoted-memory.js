...
const QUOTABLE_PATTERNS = [
    ...
    // Self-descriptions
    {
        pattern: /i('m| am) (?:the (?:kind|type|sort) of person (?:who|that)|someone who) ([^.!?]{10,80})/i,
    ...
    // Core beliefs
    {
        pattern: /(?:what matters (?:to me )?is|i (?:truly |really )?believe|my (?:philosophy|motto) is) ([^.!?]{10,80})/i,
    ...
    {
        pattern: /my (?:mom|dad|partner|wife|husband|boss|friend|brother|sister|family) (?:always|never) ([^.!?]{10,60})/i,
...
export class QuotedMemoryEngine {
     ...
     */
    extractQuotedMemories(text, turn, context) {
        for (const { pattern, contextDescription, emotionalWeight } of QUOTABLE_PATTERNS) {
            ...
            if (match && match[0]) {
                ...
                const isVulnerable = emotionalWeight === 'heavy' ||
                    context.wasPersonal ||
                    context.emotion === 'sad' ||
     ...
     */
    getCallback(currentTurn) {
        // Find unused memories from earlier in the conversation
        const available = this.quotedMemories.filter((m) => !m.usedThisSession && currentTurn - m.turn >= 3);
        ...
        void humanizationSignalEmitter.memoryCallback(memory.phrase, memory.context, timeAgo, memory.emotionalWeight);
        return {
            phrase,
    ...
    import(memories) {
        ...
        this.trimToMax();
        log.debug({ importedCount: imported.length, totalCount: this.quotedMemories.length }, 'Imported quoted memories from previous session');
    }
    /**
...
//# sourceMappingURL=quoted-memory.js.map