...
export class SurfacingEngine {
     ...
     */
    getOpeningSuggestion(memories) {
        const now = new Date();
        // Priority 1: Events that should have just happened
        const recentEvents = memories.filter((m) => {
            if (m.type !== 'event' || !m.expectedFollowUpAt)
                return false;
            const hoursSinceExpected = (now.getTime() - m.expectedFollowUpAt.getTime()) / (1000 * 60 * 60);
        ...
        const importantPeople = memories.filter((m) => m.type === 'person' && m.emotionalWeight !== 'light' && m.surfaceCount === 0);
     ...
     */
    getTimeBasedSuggestions(memories, now) {
        const suggestions = [];
        for (const memory of memories) {
            if (memory.type !== 'event' || !memory.expectedFollowUpAt)
                continue;
            if (memory.surfaceCount > 1)
                continue;
            const hoursSinceExpected = (now.getTime() - memory.expectedFollowUpAt.getTime()) / (1000 * 60 * 60);
            if (hoursSinceExpected > 1 && hoursSinceExpected < 72) {
                const phrases = hoursSinceExpected < 24
                    ? [
     ...
     */
    getTopicBasedSuggestions(memories, currentTopic) {
        const suggestions = [];
        ...
        for (const memory of memories) {
            ...
            const contentMatch = memory.content.toLowerCase().includes(topicLower) ||
     ...
     */
    getPatternBasedSuggestions(patterns, context) {
        const suggestions = [];
        ...
        for (const pattern of patterns) {
            ...
            if (pattern.type === 'temporal' && context.currentDayOfWeek !== undefined) {
                const dayMatches = pattern.evidence.some((e) => e.toLowerCase().includes(dayNames[context.currentDayOfWeek] || ''));
