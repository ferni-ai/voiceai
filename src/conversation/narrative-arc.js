...
export class NarrativeArcTracker {
    ...
    analyzeUtterance(context) {
        ...
        const { intervention, guidance } = this.determineIntervention(structure, climaxApproaching, hasReachedCore);
    ...
    getNarrativeSummary() {
        ...
        if (weights.length >= 3) {
            ...
            const variance = weights.reduce((sum, w) => sum + (w - (firstAvg + secondAvg) / 2) ** 2, 0) / weights.length;
            if (variance > 0.2)
                emotionalArc = 'volatile';
            else if (secondAvg > firstAvg + 0.15)
                emotionalArc = 'increasing';
            else if (secondAvg < firstAvg - 0.15)
                emotionalArc = 'decreasing';
        }
        return {
    ...
    detectStructure(text) {
        ...
        if (this.points.length >= 3) {
            ...
            const avgConnectedness = recentConnectedness.reduce((a, b) => a + b, 0) / recentConnectedness.length;
            if (avgConnectedness < 0.3) {
                // Low connectedness could be meandering or digressing
                if (digressionScore > 0)
                    return 'digressing';
                return 'meandering';
            }
        }
        // Determine structure
        if (buildupScore >= 2)
            return 'building_to_point';
        if (circularScore >= 2)
            return 'circular';
        if (digressionScore >= 1)
            return 'digressing';
        if (exploratoryScore >= 2)
            return 'exploratory';
        return 'direct';
    }
    detectClimaxApproaching(text) {
        let climaxScore = 0;
    ...
    determineIntervention(structure, climaxApproaching, hasReachedCore) {
        if (hasReachedCore) {
            return {
        ...
        switch (structure) {
            ...
            case 'circular':
                return {
                    ...
                    guidance: 'User keeps returning to same concern. Reflect it back: "I notice you keep coming back to..."',
                };
            case 'meandering':
                return {
            ...
            case 'digressing':
                return {
                    ...
                    guidance: 'User digressed - this might be avoidance or might be important. Briefly acknowledge, then: "Would you like to explore that, or come back to...?"',
                };
            case 'exploratory':
                return {
                    intervention: 'wait',
    ...
    getContentOverlap(content1, content2) {
    ...
    inferStructureFromPoint(point) {
        if (point.emotionalWeight > 0.7)
            return 'building_to_point';
        if (point.connectedness > 0.7)
            return 'direct';
        if (point.connectedness < 0.3)
            return 'meandering';
        return 'direct';
    }
}
 ...
 */
const narrativeArcRegistry = createSessionRegistry((sessionId) => new NarrativeArcTracker(), {
    name: 'NarrativeArc',
...
export function getNarrativeArcTracker(sessionId) {
    return narrativeArcRegistry.get(sessionId);
}
export function resetNarrativeArcTracker(sessionId) {
    narrativeArcRegistry.reset(sessionId);
}
export function resetAllNarrativeArcTrackers() {
    narrativeArcRegistry.resetAll();
}
export function hasNarrativeArcTracker(sessionId) {
    return narrativeArcRegistry.has(sessionId);
}
export function getActiveNarrativeArcCount() {
    return narrativeArcRegistry.getActiveCount();
}
export default NarrativeArcTracker;
//# sourceMappingURL=narrative-arc.js.map