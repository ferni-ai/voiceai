...
import { detectEmotionalContent as sharedDetectEmotionalContent, detectHeavyContent as sharedDetectHeavyContent, detectUserEnergy as sharedDetectUserEnergy, } from './utils/detection.js';
 ...
 */
export function enforceContractions(text) {
    let result = text;
    // Sort by length (longest first) to avoid partial replacements
    const sortedContractions = Object.entries(CONTRACTION_MAP).sort(([a], [b]) => b.length - a.length);
 ...
 */
export function generateVocalProfile(context) {
 ...
 */
export function addPitchVariation(text, context) {
    let result = text;
    // Don't add pitch if already has prosody tags
    if (/<prosody/.test(text))
        return text;
 ...
 */
function addEmphasisPitch(text) {
    let result = text;
 ...
 */
export function addIntakeBreath(text, context) {
 ...
 */
export function applyEmotionBleeding(text, context) {
    let result = text;
 ...
 */
function applyEmotionEffect(text, effect, pattern) {
    switch (effect) {
        case 'sympathetic':
 ...
 */
export function addMidSentenceReactions(text, context) {
    const rng = context.rng ??
        (context.randomSeed
 ...
 */
export function humanizeVocals(text, context) {
    ...
    log.debug({ energy: userEnergy, features: appliedFeatures.length, profile }, 'Applied vocal humanization');
    return {
        ssml: result,
        appliedFeatures,
...
//# sourceMappingURL=vocal-humanization.js.map