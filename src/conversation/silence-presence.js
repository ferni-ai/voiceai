...
export class SilencePresenceEngine {
    ...
    MIN_SILENCE_INTERVAL = 30000; // 30 seconds
    // Max silences per conversation
    MAX_SILENCES_PER_CONV = 5;
    constructor() {
    ...
    decideSilence(context) {
        const { userMessage, userEmotion, turnCount, wasPersonalSharing, conversationDepth, topicWeight, randomSeed, } = context;
        // Default: no silence
        const noSilence = {
            useSilence: false,
            reason: 'presence',
     ...
     */
    detectSilenceReason(userMessage, context) {
     ...
     */
    buildSilenceSsml(reason, duration, config) {
        const parts = [];
     ...
     */
    applyToResponse(response, decision) {
...
// ============================================================================
let instance = null;
export function getSilencePresenceEngine() {
    if (!instance) {
        instance = new SilencePresenceEngine();
    }
    return instance;
}
export function resetSilencePresenceEngine() {
    if (instance) {
        instance.reset();
    }
    instance = null;
...
//# sourceMappingURL=silence-presence.js.map