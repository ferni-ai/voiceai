...
// ============================================================================
const COGNITIVE_QUESTION_TEMPLATES = {
    analytical: [
...
const PERSONA_QUESTION_SIGNATURES = {
    ferni: {
        prefix: undefined,
        suffix: undefined,
        style: 'narrative',
 ...
 */
export function generateCognitiveQuestion(context) {
    ...
    if (context.recentQuestions && context.recentQuestions.length > 0) {
        questionPool = questionPool.filter((q) => !context.recentQuestions.some((recent) => recent.toLowerCase().includes(q.text.toLowerCase().slice(0, 20))));
    }
...
function formatQuestion(question, signature) {
    if (!signature)
        return question;
    let { text } = question;
    if (signature.prefix) {
 ...
 */
export function getPersonaFavoriteQuestions(personaId) {
...
//# sourceMappingURL=cognitive-questions.js.map