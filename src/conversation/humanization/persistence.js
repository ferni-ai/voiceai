...
function getUserHumanizationPath(userId) {
 ...
 */
export async function saveVoicePrint(userId, voicePrint) {
    try {
 ...
 */
export async function saveCrossSessionMemory(userId, memory) {
    try {
        const db = await getFirestore();
        if (!db) {
            const existing = memoryCache.get(userId) || createEmptyBundle(userId);
 ...
 */
export async function saveComfortState(userId, state) {
    try {
        const db = await getFirestore();
        if (!db) {
            const existing = memoryCache.get(userId) || createEmptyBundle(userId);
 ...
 */
export async function saveAllHumanizationData(userId) {
    const saved = [];
    const failed = [];
 ...
 */
export async function loadVoicePrint(userId) {
    try {
        const db = await getFirestore();
        if (!db) {
        ...
        const data = doc.data();
        const voicePrint = JSON.parse(data.data);
 ...
 */
export async function loadCrossSessionMemory(userId) {
    try {
        const db = await getFirestore();
        if (!db) {
            const cached = memoryCache.get(userId);
        ...
        const data = doc.data();
        const memory = JSON.parse(data.data);
 ...
 */
export async function loadComfortState(userId) {
    try {
        const db = await getFirestore();
        if (!db) {
            const cached = memoryCache.get(userId);
 ...
 */
export async function loadAllHumanizationData(userId) {
    try {
 ...
 */
export async function initializeFromPersistence(userId, sessionId) {
 ...
 */
export async function persistOnSessionEnd(userId, sessionId) {
    ...
    const relationshipStage = comfortLevel >= 0.8
        ? 'trusted_advisor'
        : comfortLevel >= 0.6
            ? 'friend'
            : comfortLevel >= 0.35
    ...
    logger.info({ userId, sessionId, saved: result.saved, failed: result.failed }, '💾 Session humanization state persisted');
    return {
...
// HELPERS
// ============================================================================
function createEmptyBundle(userId) {
    return {
        userId,
        lastSynced: new Date(),
        version: 1,
 ...
 */
export async function clearHumanizationData(userId) {
    try {
        const db = await getFirestore();
        if (!db) {
