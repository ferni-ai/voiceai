...
// ============================================================================
const PERSONA_DISFLUENCY_PREFERENCES = {
    ferni: {
 ...
 */
function detectContexts(context) {
    const contexts = ['any'];
    // Question complexity
    ...
    // Thinking/processing
    if (context.responseText.includes('...') ||
 ...
 */
function isSimpleResponse(context, threshold) {
    // Very short
    if (context.responseWordCount < threshold)
        return true;
    // Greetings
...
export class DisfluencyEngine {
    ...
    shouldApply(context) {
        ...
        if (this.config.skipSimpleResponses &&
    ...
    // PRIVATE HELPERS
    // ==========================================================================
    chooseDisfluencyType(context, personaPrefs, contexts) {
        // Filter to enabled and preferred types
        const availableTypes = this.config.enabledTypes.filter((t) => personaPrefs.preferredTypes.includes(t));
    ...
    choosePattern(type, personaPrefs, _context) {
        const patternConfig = DISFLUENCY_PATTERNS[type];
        // Use persona-specific patterns when available
        if (type === 'filled_pause' && personaPrefs.filledPauseStyle.length > 0) {
            const pattern = seededPick(`${Date.now()}:filled_pause`, personaPrefs.filledPauseStyle) ??
        ...
        if (type === 'discourse_marker' && personaPrefs.discourseMarkers.length > 0) {
            const pattern = seededPick(`${Date.now()}:discourse_marker`, personaPrefs.discourseMarkers) ??
...
export function resetAllDisfluencyEngines() {
    engines.clear();
}
export default DisfluencyEngine;
//# sourceMappingURL=disfluency-injection.js.map