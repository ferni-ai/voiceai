 ...
 */
export function getVoicePatternEngine(sessionId, userId, persistedData) {
    const key = sessionId;
    if (!engines.has(key)) {
 ...
 */
export function resetVoicePatternEngine(sessionId) {
    const engine = engines.get(sessionId);
    if (engine) {
        log.debug({ sessionId, observationCount: engine.observations.length }, 'Resetting voice pattern engine');
    }
 ...
 */
export function getActiveVoicePatternEngineCount() {
    return engines.size;
}
// ============================================================================
 ...
 */
export function getCurrentTimeOfDay(timestamp = Date.now()) {
 ...
 */
export function recordVoiceObservation(sessionId, observation) {
    const engine = engines.get(sessionId);
    if (!engine) {
 ...
 */
function emaUpdate(current, newValue, alpha = VOICE_PATTERN_CONFIG.EMA_ALPHA) {
    return alpha * newValue + (1 - alpha) * current;
}
/**
 * Bayesian update for probability estimate
 */
function bayesianUpdate(prior, observed, priorStrength = VOICE_PATTERN_CONFIG.BAYESIAN_PRIOR_STRENGTH) {
 ...
 */
function calculateConfidence(sessionCount, observations) {
 ...
 */
export function getVoicePatterns(sessionId) {
    const engine = engines.get(sessionId);
    if (!engine) {
        return null;
    }
 ...
 */
export function getRecommendedAgentWpm(sessionId) {
 ...
 */
export function getRecommendedTurnGap(sessionId) {
    const patterns = getVoicePatterns(sessionId);
    if (!patterns || patterns.confidence < 0.4) {
 ...
 */
function createDefaultPatternData(userId) {
    const defaultTodPattern = {
 ...
 */
export async function loadVoicePatterns(userId) {
    try {
        ...
        const data = doc.data();
        log.debug({ userId, sessionCount: data.sessionCount, confidence: data.confidence }, '📖 Voice patterns loaded from Firestore');
        return data;
    }
    catch (error) {
 ...
 */
export async function saveVoicePatterns(patterns) {
    try {
        const db = await getFirestore();
        if (!db) {
        ...
        await db.doc(path).set(removeUndefined({
            ...patterns,
 ...
 */
export async function initializeVoicePatterns(sessionId, userId) {
    // Load persisted data
 ...
 */
export async function persistVoicePatterns(sessionId) {
    const patterns = getVoicePatterns(sessionId);
    if (!patterns) {
...
//# sourceMappingURL=voice-pattern-learning.js.map