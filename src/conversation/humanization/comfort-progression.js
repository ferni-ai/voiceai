...
// COMFORT-GATED BEHAVIORS
// ============================================================================
export const COMFORT_GATED_BEHAVIORS = [
    // Level 0.3+ (basic)
    {
        name: 'gentle_humor',
        minComfort: 0.3,
...
export class ComfortProgressionEngine {
     ...
     */
    recordEvent(event, turnCount) {
        let delta = 0;
        if (event in COMFORT_BUILDING_EVENTS) {
            delta = COMFORT_BUILDING_EVENTS[event];
            // Update evidence tracking
            this.updateEvidence(event);
        }
        else if (event in COMFORT_REDUCING_EVENTS) {
            delta = COMFORT_REDUCING_EVENTS[event];
        }
        // Apply delta
     ...
     */
    recordIndicator(indicator, turnCount) {
        if (!this.state.indicators[indicator]) {
            ...
            this.state.category = this.calculateCategory(this.state.level);
            logger.debug({ indicator, turnCount, newLevel: this.state.level.toFixed(3) }, '🎯 Comfort indicator recorded');
        }
    }
    /**
    ...
    isBehaviorUnlocked(behaviorName) {
        ...
        if (!behavior)
            return true; // Unknown behaviors are allowed
        return this.state.level >= behavior.minComfort;
    }
    /**
     * Get all currently unlocked behaviors
     */
    getUnlockedBehaviors() {
        return COMFORT_GATED_BEHAVIORS.filter((b) => this.state.level >= b.minComfort);
    }
    /**
     * Get the next behaviors that could be unlocked
     */
    getUpcomingBehaviors() {
        return COMFORT_GATED_BEHAVIORS.filter((b) => this.state.level < b.minComfort && this.state.level >= b.minComfort - 0.15).sort((a, b) => a.minComfort - b.minComfort);
    }
    /**
...
export function getComfortProgressionEngine(sessionId) {
    if (!engines.has(sessionId)) {
        engines.set(sessionId, new ComfortProgressionEngine());
    }
    return engines.get(sessionId);
}
export function resetComfortProgressionEngine(sessionId, preserveBaseLevel = false) {
...
export function resetAllComfortProgressionEngines() {
    engines.clear();
}
export default ComfortProgressionEngine;
//# sourceMappingURL=comfort-progression.js.map