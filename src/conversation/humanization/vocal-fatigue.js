...
export class VocalFatigueEngine {
     ...
     */
    update(context) {
        const { turnCount, topicWeight, userEmotion, userEnergy, responseWordCount, wasEmotionalSupport, } = context;
     ...
     */
    getFatigueCategory() {
        if (this.state.fatigueLevel < 0.15)
            return 'none';
        if (this.state.fatigueLevel < 0.3)
            return 'subtle';
        if (this.state.fatigueLevel < 0.5)
            return 'moderate';
        return 'pronounced';
    }
    /**
    ...
    calculateBaseFatigue() {
        let fatigue = 0;
        // Time-based fatigue
        const minutesPastOnset = Math.max(0, this.state.sessionMinutes - this.config.fatigueOnsetMinutes);
    ...
    chooseFatigueExpression() {
        const category = this.getFatigueCategory();
        const expressions = FATIGUE_EXPRESSIONS[category] ||
            FATIGUE_EXPRESSIONS.subtle;
        return seededPick(`${Date.now()}:491`, expressions) ?? expressions[0];
    }
    getFatigueExpressionSsml() {
        const category = this.getFatigueCategory();
        const ssmlExpressions = FATIGUE_SSML[category] || FATIGUE_SSML.subtle;
...
export function getVocalFatigueEngine(sessionId) {
    if (!engines.has(sessionId)) {
        engines.set(sessionId, new VocalFatigueEngine());
    }
    return engines.get(sessionId);
}
export function resetVocalFatigueEngine(sessionId) {
...
export function resetAllVocalFatigueEngines() {
    engines.clear();
}
export default VocalFatigueEngine;
//# sourceMappingURL=vocal-fatigue.js.map