...
const DEFAULT_SELF_CORRECTION_PATTERNS: PersonaHumanizationProfile['selfCorrectionPatterns'] = {
  restart: [
...
const PERSONA_PATTERNS: Record<
  string,
  Partial<PersonaHumanizationProfile['selfCorrectionPatterns']>
> = {
  ferni: {
    restart: [
 ...
 */
function detectComplexity(text: string): number {
  ...
  if (sentenceCount > 4) complexity += 0.1;
  ...
  for (const pattern of complexPatterns) {
    if (pattern.test(text)) complexity += 0.05;
  }

  // Multiple clauses (commas, semicolons)
  const clauseCount = (text.match(/[,;:—]/g) || []).length;
  if (clauseCount > 3) complexity += 0.1;
  if (clauseCount > 6) complexity += 0.1;
 ...
 */
function detectEmotionalContent(text: string): boolean {
 ...
 */
function detectAdviceGiving(text: string): boolean {
...
export class SelfCorrectionEngine {
  private state: SelfCorrectionState;
   ...
   */
  apply(response: string, correction: SelfCorrectionResult): { text: string; ssml: string } {
    let text = response;
    let ssml = response;
  ...
  private chooseCorrectionType(context: HumanizationContext): SelfCorrectionType {
  ...
  private getPatternsForPersona(
    personaId: string
  ): PersonaHumanizationProfile['selfCorrectionPatterns'] {
  ...
  private generateSsml(pattern: string, type: SelfCorrectionType): string {
  ...
  private findMidSentencePoint(text: string): number {
  ...
  private findKeyPointMarker(text: string): number {
...
const engines = new Map<string, SelfCorrectionEngine>();

export function getSelfCorrectionEngine(sessionId: string): SelfCorrectionEngine {
...
export function resetSelfCorrectionEngine(sessionId: string): void {
...
export function resetAllSelfCorrectionEngines(): void {
  engines.clear();
}
