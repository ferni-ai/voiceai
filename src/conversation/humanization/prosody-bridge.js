 ...
 */
export function prosodyToVoiceSnapshot(prosody, emotion) {
    return {
 ...
 */
function normalizeEnergy(energyDb) {
 ...
 */
export function prosodyToBreathPattern(prosody) {
    ...
    const depth = prosody.energyVariance > 10 ? 'deep' : prosody.energyVariance > 5 ? 'normal' : 'shallow';
    return {
 ...
 */
export function processProsodyForHumanization(sessionId, userId, voiceEmotion) {
 ...
 */
export function getVoiceStateInsight(sessionId, userId, currentProsody, currentEmotion) {
    const voicePrint = getVoicePrintEngine(userId);
 ...
 */
export function getCrossSessionInsight(sessionId, userId, currentProsody, currentEmotion) {
    const snapshot = prosodyToVoiceSnapshot(currentProsody, currentEmotion);
 ...
 * - Etc.
 */
export function inferAmbientFromProsody(prosody) {
    ...
    const noisiness = Math.max(0, Math.min(1, (1 - speakingRatio) * 0.6 + energyLevel * 0.25 + voiceQualityNoisiness * 0.15));
    ...
    return {
        ...
        features: {
            energyLevel,
            frequencyProfile: sounds[0].sound === 'traffic' || sounds[0].sound === 'crowd' ? 'broadband' : 'normal',
            periodicityScore,
            noisiness,
        },
    };
 ...
 */
export function initProsodyBridge(sessionId, userId) {
 ...
 */
export function cleanupProsodyBridge(sessionId) {
 ...
 */
export function getBridgeState(sessionId) {
    return bridgeStates.get(sessionId) || null;
}
//# sourceMappingURL=prosody-bridge.js.map