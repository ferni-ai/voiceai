...
// ============================================================================
const EMOTION_DEVIATION_TEMPLATES = {
    happy: {
...
export class VoicePrintEngine {
    ...
    constructor(userId, existingPrint) {
        if (existingPrint) {
    ...
    detectState(currentSnapshot) {
        ...
        const tempoDeviation = (currentSnapshot.speechRate - baseline.avgWordsPerMinute) / baseline.avgWordsPerMinute;
        ...
        const suggestedAcknowledgments = this.generateAcknowledgments(emotionMatch.emotion, insights, emotionMatch.confidence);
        return {
            currentState: {
                emotion: emotionMatch.emotion,
                confidence: emotionMatch.confidence,
                deviationFromBaseline: Math.max(Math.abs(pitchDeviation), Math.abs(tempoDeviation), Math.abs(energyDeviation)),
            },
     ...
     */
    compareToSessionStart(currentSnapshot) {
        if (!this.sessionStartSnapshot) {
    ...
    learnEmotionalSignature(snapshot) {
        ...
        const deviation = {
            ...
            tempoChange: (snapshot.speechRate - this.voicePrint.baseline.avgWordsPerMinute) /
                this.voicePrint.baseline.avgWordsPerMinute,
            energyChange: (snapshot.energyMean - this.voicePrint.baseline.avgEnergy) /
    ...
    deriveEmotionFromSnapshot(snapshot) {
        ...
        if (arousal < 0.3)
            return 'tired';
        if (valence < -0.1 && snapshot.strain > 0.3)
            return 'stressed';
        if (arousal < 0.5 && valence > 0)
            return 'calm';
        return null;
    }
    matchEmotionalSignature(deviation) {
    ...
    calculateSignatureMatch(actual, expected) {
    ...
    generateInsights(pitchDeviation, tempoDeviation, energyDeviation) {
        const insights = [];
        if (Math.abs(pitchDeviation) > 0.1) {
            insights.push(pitchDeviation > 0
        ...
        if (Math.abs(tempoDeviation) > 0.15) {
            insights.push(tempoDeviation > 0
        ...
        if (Math.abs(energyDeviation) > 0.15) {
            insights.push(energyDeviation > 0
    ...
    generateAcknowledgments(emotion, insights, confidence) {
        if (confidence < 0.5)
...
export function resetAllVoicePrintEngines() {
    engines.clear();
}
export default VoicePrintEngine;
//# sourceMappingURL=voice-print.js.map