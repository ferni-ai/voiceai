...
export class PhoneticMirroringEngine {
    ...
    mirror(response) {
            ...
            for (const marker of this.profile.regionalMarkers) {
                ...
                if (pattern && seededChance(`${Date.now()}:2`, this.config.mirroringStrength)) {
                    ...
                    for (const alt of pattern.alternatives) {
                        const before = result;
                        result = result.replace(new RegExp(`\\b${alt}\\b`, 'gi'), marker === 'yall' ? "y'all" : marker);
                        if (result !== before) {
        ...
        if (this.config.mirrorTagQuestions &&
    ...
    rebuildProfile() {
        ...
        if (topFiller) {
            profile.fillerPreference = topFiller[0];
        }
        ...
        if (profile.detectedReductions.length >= 2)
            profile.confidence += 0.1;
        if (profile.regionalMarkers.length >= 1)
            profile.confidence += 0.1;
        if (profile.usesTagQuestions)
...
export function getPhoneticMirroringEngine(sessionId) {
    if (!engines.has(sessionId)) {
        engines.set(sessionId, new PhoneticMirroringEngine());
    }
    return engines.get(sessionId);
}
export function resetPhoneticMirroringEngine(sessionId) {
...
export function resetAllPhoneticMirroringEngines() {
    engines.clear();
}
export default PhoneticMirroringEngine;
//# sourceMappingURL=phonetic-mirroring.js.map