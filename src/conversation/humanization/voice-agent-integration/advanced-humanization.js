 ...
 */
export function getAdvancedGuidance(sessionId) {
 ...
 */
export function getAdvancedModifications(sessionId) {
    const state = getSession(sessionId);
    if (!state || !state.isActive || !state.advancedHumanization.enabled) {
        return null;
    }
 ...
 */
export function recordAdvice(sessionId) {
    const state = getSession(sessionId);
    if (!state || !state.isActive || !state.advancedHumanization.enabled) {
        return;
    }
 ...
 */
export function shouldStopAdvice(sessionId) {
    const state = getSession(sessionId);
    if (!state || !state.isActive || !state.advancedHumanization.enabled) {
        return false;
    }
 ...
 */
export function getAdvancedSystemPromptAdditions(sessionId) {
