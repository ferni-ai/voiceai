...
export class SessionDynamicsEngine {
    ...
    update(context) {
        ...
        if (newPhase !== this.state.phase) {
            logger.debug({ fromPhase: this.state.phase, toPhase: newPhase, turnCount }, '📈 Phase transition');
     ...
     */
    isBehaviorAppropriate(behavior) {
        const phaseBehavior = PHASE_BEHAVIORS[this.state.phase];
        return phaseBehavior.specialBehaviors.some((b) => b.toLowerCase().includes(behavior.toLowerCase()));
    }
    /**
    ...
    calculatePhaseProgress(turnCount, phase) {
    ...
    updateEnergy(userEnergy, topicWeight) {
...
export function getSessionDynamicsEngine(sessionId) {
    if (!engines.has(sessionId)) {
        engines.set(sessionId, new SessionDynamicsEngine());
    }
    return engines.get(sessionId);
}
export function resetSessionDynamicsEngine(sessionId) {
...
export function resetAllSessionDynamicsEngines() {
    engines.clear();
}
export default SessionDynamicsEngine;
//# sourceMappingURL=session-dynamics.js.map