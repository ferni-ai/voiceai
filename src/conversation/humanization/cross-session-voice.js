...
export class CrossSessionVoiceEngine {
    ...
    sessionMoments = [];
    sessionTurnCount = 0;
    constructor(userId, existingMemory) {
        if (existingMemory) {
        ...
        logger.debug({ userId, sessions: this.memory.totalSessions }, 'CrossSessionVoiceEngine initialized');
    }
    /**
     * Start a new session
     */
    startSession(sessionId, startingVoice) {
     ...
     */
    recordMoment(turn, description, voiceState, emotion) {
     ...
     */
    generateAcknowledgment(currentVoice) {
     ...
     */
    markAcknowledged(changeId) {
        const change = this.memory.significantChanges.find((c) => `${c.type}-${c.detected.getTime()}` === changeId);
        if (change) {
    ...
    updatePatterns() {
        if (this.memory.sessionSnapshots.length < 3)
            return;
        const recent = this.memory.sessionSnapshots.slice(0, 5);
    ...
    createAcknowledgment(type, category, priority) {
    ...
    createAcknowledgmentForChange(change) {
        let templates;
        let category;
        switch (change.type) {
            case 'energy':
...
export function getCrossSessionVoiceEngine(userId, existingMemory) {
    if (!engines.has(userId)) {
        engines.set(userId, new CrossSessionVoiceEngine(userId, existingMemory));
    }
    return engines.get(userId);
}
export function resetCrossSessionVoiceEngine(userId) {
...
export function resetAllCrossSessionVoiceEngines() {
    engines.clear();
}
export default CrossSessionVoiceEngine;
//# sourceMappingURL=cross-session-voice.js.map