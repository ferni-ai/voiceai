...
export class MicroAffirmationEngine {
    turnCount = 0;
    lastAffirmationTurn = -10;
    consecutiveAffirmations = 0;
    sessionAffirmationCount = 0;
    recentTypes = [];
    // Config
    config = {
     ...
     */
    decide(userMessage, turnCount, forceContext) {
     ...
     */
    getAffirmationOfType(type, context = 'general') {
        const candidates = ALL_AFFIRMATIONS.filter((a) => a.type === type && (a.contexts.includes(context) || a.contexts.includes('general')));
    ...
    calculateAffirmProbability(turnsSinceLast) {
    ...
    selectAffirmation(context, message) {
        // Filter by context
        const contextMatches = ALL_AFFIRMATIONS.filter((a) => a.contexts.includes(context) || a.contexts.includes('general'));
...
const microAffirmationRegistry = createSessionRegistry((sessionId) => new MicroAffirmationEngine(), { name: 'MicroAffirmation', cleanup: (engine) => engine.reset(), verbose: false });
registerGlobalRegistry(microAffirmationRegistry);
export function getMicroAffirmationEngine(sessionId) {
    return microAffirmationRegistry.get(sessionId);
}
export function resetMicroAffirmationEngine(sessionId) {
    const engine = microAffirmationRegistry.get(sessionId);
    engine.reset();
}
export function clearMicroAffirmationEngine(sessionId) {
    microAffirmationRegistry.reset(sessionId);
}
export function getActiveMicroAffirmationCount() {
    return microAffirmationRegistry.getActiveCount();
}
export default MicroAffirmationEngine;
//# sourceMappingURL=micro-affirmations.js.map