...
export class CircuitBreaker {
    ...
    recordFailure() {
        ...
        if (this.state === 'half-open') {
            ...
            this.state = 'open';
            log.warn({ name: this.config.name, failures: this.failures }, '⛔ Circuit opened (too many failures)');
        }
    }
    /**
...
export class LazyLoader {
    ...
    loader;
    name;
    constructor(loader, name = 'unknown') {
        this.loader = loader;
        this.name = name;
    }
    /**
...
const detectionCache = new LRUCache(200, 30000); // 30s TTL
/**
 * Get cached detection result
 */
export function getCachedDetection(key) {
    return detectionCache.get(key);
}
/**
 * Set cached detection result
 */
export function setCachedDetection(key, value) {
    detectionCache.set(key, value);
}
/**
 * Get or compute cached detection
 */
export function getOrComputeDetection(key, compute) {
    return detectionCache.getOrCompute(key, compute);
}
/**
 ...
 */
export function getOrComputeDetectionWithHit(key, compute) {
    const cached = getCachedDetection(key);
    if (cached !== undefined) {
 ...
 */
export function getCircuitBreaker(system, config) {
    if (!circuitBreakers.has(system)) {
        circuitBreakers.set(system, new CircuitBreaker({
            name: system,
            failureThreshold: 3,
 ...
 */
export function resetPerformanceOptimizations() {
    clearDetectionCache();
