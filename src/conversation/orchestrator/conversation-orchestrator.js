...
// Shared utilities
import { analyzeMessage, detectBreakthrough, detectDisengagement, detectEvidence, detectHesitation, detectHighEngagement, } from '../utils/detection.js';
// Phase 2: Intelligence systems
import { getMoodTracker } from '../deep-humanization/index.js';
...
import { applyAdvancedHumanization as applyAdvancedHumanizationHelper, applyComposableEffects as applyComposableEffectsHelper, applyContentDeliveryPacing, applyDeepHumanization as applyDeepHumanizationHelper, applyPriorityActions as applyPriorityActionsHelper, applySilencePresence as applySilencePresenceHelper, applySpeechNaturalization as applySpeechNaturalizationHelper, applyVocalHumanization as applyVocalHumanizationHelper, applyVocabularyMirroring as applyVocabularyMirroringHelper, generateAdditions as generateAdditionsHelper, } from './humanization-helpers.js';
...
export class ConversationOrchestrator {
    ...
    personaId = null;
    sessionId;
    constructor(sessionId = 'default', config = {}) {
    ...
    async orchestrate(input) {
        ...
        try {
            ...
            const humanizationResult = await this.runHumanizationPhase(input, analysisResult, intelligenceResult);
            timing.humanization = Date.now() - humanizationStart;
            // Phase 4: Output
            const outputStart = Date.now();
            const output = this.runOutputPhase(input, analysisResult, intelligenceResult, humanizationResult, timing);
    ...
    runAnalysisPhase(input) {
        ...
        const analysisResult = getOrComputeDetectionWithHit(cacheKey, () => analyzeMessage(input.userMessage, input.userEmotion));
        ...
        const signals = {
            hasEvidence: getOrComputeDetectionWithHit(`${signalCachePrefix}evidence`, () => detectEvidence(input.userMessage)).value,
            isBreakthrough: getOrComputeDetectionWithHit(`${signalCachePrefix}breakthrough`, () => detectBreakthrough(input.userMessage)).value,
            hasHesitation: getOrComputeDetectionWithHit(`${signalCachePrefix}hesitation`, () => detectHesitation(input.userMessage)).value,
            isDisengaged: getOrComputeDetectionWithHit(`${signalCachePrefix}disengaged`, () => detectDisengagement(input.userMessage)).value,
            isHighlyEngaged: getOrComputeDetectionWithHit(`${signalCachePrefix}engaged`, () => detectHighEngagement(input.userMessage)).value,
    ...
    getConversationDepth(turnNumber) {
        if (turnNumber > 8)
            return 'deep';
        if (turnNumber > 4)
            return 'medium';
    ...
    // PHASE 2: INTELLIGENCE
    // ==========================================================================
    async runIntelligencePhase(input, analysis) {
        ...
        const guidance = this.buildGuidance(analysis, sessionInsight, superhumanInsight, emotionalGuidance);
        return {
            sessionInsight,
            superhumanInsight,
            mood,
    ...
    async getSessionInsight(input, analysis) {
    ...
    async getSuperhumanInsight(input, analysis) {
        ...
        const result = await breaker.execute(async () => {
            const { value, timedOut } = await withTimeout(async () => {
                const orchestrator = getBetterThanHuman(input.userId, input.sessionId, input.personaId, input.sessionCount || 0);
                const context = {
                    userMessage: input.userMessage,
                    turnCount: input.turnNumber,
    ...
    async getMoodState(input, analysis) {
    ...
    buildGuidance(analysis, sessionInsight, superhumanInsight, emotionalGuidance) {
        const guidance = {
            approach: 'normal',
            pacing: 'normal',
        ...
        if (sessionInsight) {
            if (sessionInsight.concern.level === 'elevated' ||
            ...
            if (sessionInsight.responseModifications.length > 0) {
                guidance.priorityActions.push(...sessionInsight.responseModifications.slice(0, 2).map((m) => ({
                    type: m.type,
                    content: m.content || '',
                    placement: 'prefix',
                    priority: 0.8,
        ...
        if (superhumanInsight && superhumanInsight.prioritizedActions.length > 0) {
            guidance.priorityActions.push(...superhumanInsight.prioritizedActions.slice(0, 2).map((a) => ({
                type: a.type,
                content: a.content,
                placement: a.placement === 'interrupt' ? 'prefix' : a.placement,
    ...
    // PHASE 3: HUMANIZATION
    // ==========================================================================
    async runHumanizationPhase(input, analysis, intelligence) {
        ...
        if (this.config.features.vocalHumanization) {
            ...
            appliedFeatures.push(...vocalResult.appliedFeatures.map((f) => ({ name: f, source: 'vocal' })));
        }
        ...
        if (this.config.features.composableEffects) {
            const effectsResult = await applyComposableEffectsHelper(text, ssml, input, analysis, intelligence);
        ...
        if (this.config.features.advancedHumanization) {
            const advResult = applyAdvancedHumanizationHelper(text, ssml, input, analysis, this.getComfortLevel.bind(this));
            text = advResult.text;
            ssml = advResult.ssml;
        ...
        const actionResult = applyPriorityActionsHelper(text, ssml, intelligence.guidance.priorityActions);
     ...
     */
    shouldTrigger(sessionId, turnNumber, feature, probability) {
    ...
    // PHASE 4: OUTPUT
    // ==========================================================================
    runOutputPhase(input, analysis, intelligence, humanization, timing) {
    ...
    applySsmlEnhancements(ssml, guidance) {
        if (!guidance)
            return ssml;
        let result = ssml;
        // Add opening break
 ...
 */
const orchestratorRegistry = createSessionRegistry((sessionId) => new ConversationOrchestrator(sessionId), {
    name: 'ConversationOrchestrator',
    cleanup: (orchestrator) => orchestrator.resetSession(),
    verbose: false,
});
 ...
 */
export function getConversationOrchestrator(sessionId, config) {
