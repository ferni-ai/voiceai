...
// Import all engines
import { getSubtextDetectionEngine, resetSubtextDetectionEngine, } from './subtext-detection.js';
import { getEmotionalAftercareEngine, resetEmotionalAftercareEngine, } from './emotional-aftercare.js';
import { getConversationalRepairEngine, resetConversationalRepairEngine, } from './conversational-repair.js';
import { getHopeInjectionEngine, resetHopeInjectionEngine, } from './hope-injection.js';
import { getCuriosityEngine } from './curiosity-engine.js';
import { getEnergyRegulationEngine, resetEnergyRegulationEngine, } from './energy-regulation.js';
import { getMicroAffirmationEngine, resetMicroAffirmationEngine, } from './micro-affirmations.js';
import { getTemporalContextEngine, } from './temporal-context.js';
import { getRelationshipEventsEngine, } from './relationship-events.js';
import { getParadoxicalInterventionEngine, resetParadoxicalInterventionEngine, } from './paradoxical-intervention.js';
...
export class AdvancedHumanizationOrchestrator {
    ...
    processTurn(context) {
        ...
        // Detect energy state
        const energyState = energyEngine.detectEnergy(context.userMessage, context.turnCount, context.prosodyHints);
        // Detect resistance to advice
        const resistance = paradoxicalEngine.detectResistance(context.userMessage, context.turnCount, context.wasAdviceGiven || false);
        ...
        const aftercareState = aftercareEngine.processTurn(context.userMessage, context.turnCount, context.detectedEmotion);
        ...
        // Curiosity prompt
        const curiosityPrompt = curiosityEngine.getCuriosityPrompt(context.turnCount, context.recentTopics);
        // Milestone opportunity
        const milestone = relationshipEngine.checkMilestoneOpportunity(context.turnCount);
        // ========================================================================
        // COMPUTE META GUIDANCE
        // ========================================================================
        const stopDirectAdvice = paradoxical.stopDirectAdvice ||
            (resistance.detected && resistance.count >= 2) ||
            aftercareState.priority === 'urgent';
        const toneGuidance = this.computeToneGuidance(aftercareGuidance, energyGuidance, temporalState, subtext);
        const lengthGuidance = this.computeLengthGuidance(aftercareState, energyState, subtext, repair);
        const priorityActions = this.computePriorityActions(subtext, aftercareState, repair, resistance, milestone);
        // ========================================================================
     ...
     */
    recordAdviceOutcome(wasAccepted) {
        const paradoxicalEngine = getParadoxicalInterventionEngine(this.sessionId);
        paradoxicalEngine.recordAdviceResponse(this.turnCount, this.lastAgentMessage || '', wasAccepted ? 'accepted' : 'rejected');
    }
    /**
     * Record a relationship milestone manually
     */
    recordMilestone(type, context) {
        const relationshipEngine = getRelationshipEventsEngine(this.userId);
    ...
    // PRIVATE HELPERS
    // ============================================================================
    computeToneGuidance(aftercare, energy, temporal, subtext) {
        const parts = [];
    ...
    computeLengthGuidance(aftercare, energy, subtext, repair) {
        // Shorter when:
        ...
        if (aftercare.emotionalDebt > 0.7)
            return 'shorter'; // Raised from 0.6
        if (energy.level < 0.2 && energy.valence < 0)
            return 'shorter'; // Raised from 0.3, added valence check
        if (repair.shouldRepair)
            return 'shorter';
        if (subtext.type === 'testing_waters')
            return 'shorter';
        // Longer when high engagement
        if (energy.level > 0.6 && energy.valence > 0.1)
            return 'longer'; // Lowered threshold to be more generous
        return 'normal';
    }
    computePriorityActions(subtext, aftercare, repair, resistance, milestone) {
        const actions = [];
 ...
 */
export function getAdvancedHumanization(sessionId, userId) {
 ...
 */
export function resetAdvancedHumanization(sessionId, userId) {
    const key = `${sessionId}:${userId}`;
 ...
 */
export function clearAdvancedHumanization(sessionId, userId) {
    const key = `${sessionId}:${userId}`;
    orchestrators.delete(key);
}
export default AdvancedHumanizationOrchestrator;
//# sourceMappingURL=advanced-humanization.js.map