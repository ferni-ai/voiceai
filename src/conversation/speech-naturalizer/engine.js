 ...
 */
import { chance, createSeededRandom, createSystemRandom } from '../utils/rng.js';
import { requestThinkingPhrase, wasPhraseUsedThisTurn } from '../thinking-phrase-coordinator.js';
import { getPatternsForPersona, HEDGES_BY_STRENGTH, TYPE_SPECIFIC_THINKING, } from './patterns.js';
function getRng(options, salt) {
    if (options?.rng)
        return options.rng;
    if (options?.randomSeed)
        return createSeededRandom(`${options.randomSeed}:${salt}`);
...
export class SpeechNaturalizer {
     ...
     */
    naturalize(text, personaId, context = {}) {
     ...
     */
    generateRepair(originalStatement, correctedStatement, personaId, options) {
        const patterns = getPatternsForPersona(personaId);
        const rng = getRng(options, `speech-repair:${personaId}:${originalStatement}:${correctedStatement}`);
     ...
     */
    getThinkingPhrase(personaId, type = 'processing', options) {
        ...
        if (options?.sessionId && options?.turnNumber !== undefined) {
            ...
            const result = requestThinkingPhrase(options.sessionId, options.turnNumber, 'speech-naturalizer', personaId, { isQuestion: type === 'processing' });
     ...
     */
    getHedge(personaId, strength = 'medium', options) {
        const patterns = getPatternsForPersona(personaId);
        ...
        const strengthHedge = HEDGES_BY_STRENGTH[strength][rng.nextInt(HEDGES_BY_STRENGTH[strength].length)];
        return chance(rng, 0.6) ? personalHedge : strengthHedge;
    }
    /**
     * Wrap text with uncertainty markers
     */
    addUncertainty(text, personaId, level, options) {
        const hedge = this.getHedge(personaId, level === 'high' ? 'strong' : level === 'medium' ? 'medium' : 'soft', options);
        if (level === 'high') {
    ...
    // PRIVATE HELPERS
    // ============================================================================
    addOpeningFiller(text, patterns, rng) {
    ...
    addThinkingPhrase(text, patterns, context, rng) {
