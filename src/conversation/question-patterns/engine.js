...
export class QuestionPatternEngine {
     ...
     */
    generateFollowUp(intent, context) {
     ...
     */
    async generateQuestionAsync(context) {
        const type = this.selectQuestionType(context);
        const llmContext = {
    ...
    selectQuestionType(context) {
        ...
        return (seededPick(`${baseSeed}:pick`, candidates) ?? candidates[0]);
    }
    buildQuestion(type, context) {
        const personaStyle = context.personaId ? PERSONA_QUESTION_STYLES[context.personaId] : null;
        const baseSeed = context.randomSeed ??
