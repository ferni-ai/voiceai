...
export function detectContentType(text) {
    ...
    for (const pattern of INSTRUCTION_PATTERNS) {
        if (pattern.test(text))
            scores.instructions += 2;
    }
    // Find highest score
    let maxScore = 0;
    let detectedType = 'conversational';
...
export function analyzeContent(text, options) {
    ...
    const complexityMultiplier = complexity === 'complex' ? 1.3 : complexity === 'moderate' ? 1.15 : 1;
 ...
 */
function segmentContent(text, type) {
    const segments = [];
    ...
    for (let i = 0; i < sentences.length; i++) {
        ...
        const isTransition = /^(also|and|plus|additionally|furthermore|however|but|on the other hand)/i.test(sentence);
 ...
 */
function detectImportantContent(text) {
 ...
 */
function createSegment(text, type, importance, contentType) {
 ...
 */
export function applyDeliveryPacing(text, options) {
 ...
 */
export function shouldApplyDeliveryPacing(text) {
 ...
 */
export function addSignposting(text, options) {
    ...
    if (addTransitions) {
        ...
        if (sentences.length > 2) {
            ...
            if (secondSentence && !/^(also|and|plus|but|however|additionally)/i.test(secondSentence)) {
                const transition = seededPick(`${Date.now()}:2`, SIGNPOSTING.transitions) ?? SIGNPOSTING.transitions[0];
 ...
 */
export function getSummaryIntro(contentType) {
    const intros = {
        web_result: [
...
//# sourceMappingURL=content-delivery-pacing.js.map