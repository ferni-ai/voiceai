...
export class EmotionalAftercareEngine {
    ...
    turnCount = 0;
    // Config
    DEBT_DECAY_RATE = 0.08; // Per turn
    HIGH_DEBT_THRESHOLD = 0.6;
    URGENT_DEBT_THRESHOLD = 0.85;
    MAX_EVENTS = 10;
    MINIMUM_GROUNDING_TURNS = 3;
    constructor() {
     ...
     */
    processTurn(userMessage, turnCount, detectedEmotion) {
     ...
     */
    shouldSuggestClosing(isNearEnd) {
    ...
    // PRIVATE METHODS
    // ============================================================================
    detectIntensity(message, emotion) {
        ...
        if (emotion) {
            ...
            if (highEmotions.includes(emotion.toLowerCase()))
                return 'high';
            if (moderateEmotions.includes(emotion.toLowerCase()))
                return 'moderate';
        }
        return 'low';
    }
    inferContentType(message, emotion) {
        const lower = message.toLowerCase();
        if (/(died|death|passed away|lost|grief|mourning)/.test(lower))
            return 'grief';
        if (/(trauma|abuse|assault|ptsd|flashback)/.test(lower))
            return 'trauma';
        if (/(scared|terrified|afraid|fear|panic)/.test(lower))
            return 'fear';
        if (/(angry|furious|rage|hate|pissed)/.test(lower))
            return 'anger';
        if (/(shame|embarrassed|humiliated|ashamed)/.test(lower))
            return 'shame';
        if (/(overwhelmed|too much|can'?t cope)/.test(lower))
            return 'overwhelm';
        if (/(realized|breakthrough|finally|figured out)/.test(lower))
            return 'breakthrough';
        if (emotion) {
            const emotionMap = {
                grief: 'grief',
    ...
    recordEmotionalEvent(intensity, contentType) {
        const event = {
            turn: this.turnCount,
            timestamp: Date.now(),
            intensity,
    ...
    detectRecoverySignals(message) {
        return RECOVERY_SIGNALS.some((p) => p.test(message));
    }
    updateEmotionalDebt(currentIntensity) {
    ...
    updatePriority() {
        const debt = this.state.emotionalDebt;
        const hasUnaddressed = this.state.recentEvents.some((e) => !e.addressedInAftercare && (e.intensity === 'high' || e.intensity === 'overwhelming'));
...
const emotionalAftercareRegistry = createSessionRegistry((sessionId) => new EmotionalAftercareEngine(), { name: 'EmotionalAftercare', cleanup: (engine) => engine.reset(), verbose: false });
registerGlobalRegistry(emotionalAftercareRegistry);
export function getEmotionalAftercareEngine(sessionId) {
    return emotionalAftercareRegistry.get(sessionId);
}
export function resetEmotionalAftercareEngine(sessionId) {
    const engine = emotionalAftercareRegistry.get(sessionId);
    engine.reset();
}
export function clearEmotionalAftercareEngine(sessionId) {
    emotionalAftercareRegistry.reset(sessionId);
}
export function getActiveEmotionalAftercareCount() {
    return emotionalAftercareRegistry.getActiveCount();
}
export default EmotionalAftercareEngine;
//# sourceMappingURL=emotional-aftercare.js.map