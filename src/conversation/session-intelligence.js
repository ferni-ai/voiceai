...
import { getConcernDetectionEngine, } from './concern-detection.js';
import { getPredictiveAnticipationEngine, } from './predictive-anticipation.js';
import { getProactiveMemoryEngine, } from './proactive-memory.js';
...
export class SessionIntelligenceOrchestrator {
    ...
    analyze(context) {
        ...
        const responseModifications = this.generateModifications(concern, memorySuggestions, predictions);
     ...
     */
    applyModifications(response, insight) {
        let result = response;
    ...
    // PRIVATE METHODS
    // ==========================================================================
    generateModifications(concern, memorySuggestions, predictions) {
        const mods = [];
        ...
        if (predictions.need.confidence > 0.7 &&
    ...
    generateGuidance(concern, predictions) {
        // Default guidance
        let approach = 'normal';
        let pacing = 'normal';
        let energy = 'normal';
    ...
    generateOpening(memorySuggestions, predictions) {
    ...
    emitSignals(concern, predictions) {
        // Emit concern signal
        if (concern.level !== 'none') {
            void humanizationSignalEmitter.concernDetected(concern.level, concern.primaryConcern || 'unknown', concern.recommendedApproach, concern.score);
        }
        // Emit voice state signal
        if (predictions.voiceState.confidence > 0.6 && predictions.voiceState.state !== 'normal') {
            void humanizationSignalEmitter.voiceStateDetected(predictions.voiceState.state, predictions.voiceState.confidence);
        }
        // Emit need prediction signal
        if (predictions.need.confidence > 0.6 && predictions.need.primaryNeed !== 'unknown') {
            void humanizationSignalEmitter.needPredicted(predictions.need.primaryNeed, predictions.need.confidence);
        }
        // Emit emotional trajectory signal
        if (predictions.emotional.trajectory !== 'stable') {
            void humanizationSignalEmitter.emotionalTrajectory(predictions.emotional.trajectory, predictions.emotional.confidence);
        }
    }
    calculateConfidence(concern, predictions) {
        ...
        return (concern.score * weights.concern +
...
const sessionIntelligenceRegistry = createSessionRegistry((key) => {
...
export function getSessionIntelligence(sessionId, userId) {
    const key = `${sessionId}:${userId || 'anonymous'}`;
    return sessionIntelligenceRegistry.get(key);
}
export function resetSessionIntelligence(sessionId, userId) {
    const key = `${sessionId}:${userId || 'anonymous'}`;
    const orchestrator = sessionIntelligenceRegistry.get(key);
    orchestrator.reset();
}
export function clearSessionIntelligence(sessionId, userId) {
    const key = `${sessionId}:${userId || 'anonymous'}`;
    sessionIntelligenceRegistry.reset(key);
}
export function getActiveSessionIntelligenceCount() {
