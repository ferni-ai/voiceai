...
export type ResistanceType =
  | 'yes_but' // Classic "yes but" pattern
  | 'already_tried' // "I've already tried that"
  | 'wont_work' // "That won't work for me"
  | 'different' // "My situation is different"
  | 'deflection' // Changes subject when advice given
  | 'passive' // Agrees but clearly won't act
  | 'arguing' // Actively argues against suggestions
  | 'overwhelmed'; // Too overwhelmed to receive input
...
export type InterventionType =
  | 'paradoxical_question' // "What if you didn't change anything?"
  | 'meta_observation' // "I notice you've dismissed three suggestions..."
  | 'explore_resistance' // "What's making this hard to consider?"
  | 'validate_first' // Validate instead of advise
  | 'ask_permission' // "Would you like suggestions or just listening?"
  | 'reverse_angle' // Approach from opposite direction
  | 'normalize_inaction' // "Maybe now isn't the time to fix this"
  | 'gentle_curiosity'; // Pure curiosity without agenda
...
export class ParadoxicalInterventionEngine {
  private resistanceHistory: Array<{
    type: ResistanceType;
    turn: number;
    confidence: number;
  }> = [];

  private adviceHistory: AdviceHistory[] = [];
  private turnCount = 0;
  private consecutiveResistances = 0;
  private lastInterventionTurn = -20;
  ...
  detectResistance(
    userMessage: string,
    turnCount: number,
    wasAdviceJustGiven = false
  ): ResistanceDetection {
    this.turnCount = turnCount;

    // Check patterns
    const detections: Array<{ type: ResistanceType; confidence: number; evidence: string }> = [];
   ...
   */
  recordAdviceResponse(
    turn: number,
    adviceGiven: string,
    response: AdviceHistory['response']
  ): void {
  ...
  decide(resistance: ResistanceDetection): InterventionDecision {
    ...
    const rejectedCount = recentAdvice.filter(
      (a) => a.response === 'rejected' || a.response === 'deflected'
    ).length;
    ...
    const shouldIntervene =
      (resistance.confidence >= 0.75 && this.consecutiveResistances >= 2) ||
  ...
  getIntervention(type: InterventionType): string {
    const phraseMap: Record<InterventionType, string[]> = {
  ...
  getStats(): {
    ...
    const typeBreakdown = {} as Record<ResistanceType, number>;
    for (const r of this.resistanceHistory) {
      typeBreakdown[r.type] = (typeBreakdown[r.type] || 0) + 1;
    }

    const rejectedCount = this.adviceHistory.filter(
      (a) => a.response === 'rejected' || a.response === 'deflected'
    ).length;
    ...
    return {
      ...
      adviceRejectionRate:
        this.adviceHistory.length > 0 ? rejectedCount / this.adviceHistory.length : 0,
      typeBreakdown,
    };
  }
