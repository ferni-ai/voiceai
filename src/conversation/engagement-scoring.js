...
export class EngagementScorer {
    ...
    recordResponse(text, options) {
        ...
        for (const pattern of ENGAGEMENT_PHRASES) {
            ...
            if (matches)
                engagementPhrases += matches.length;
        }
        for (const pattern of DISENGAGEMENT_PHRASES) {
            const matches = text.match(pattern);
            if (matches)
                disengagementPhrases += matches.length;
        }
    ...
    computeEngagement() {
        ...
        const avgLatency = this.observations.reduce((sum, o) => sum + o.responseLatencyMs, 0) / this.observations.length;
        if (avgLatency < 2000)
            score += 0.15;
            ...
            score -= 0.15;
        // Length factor
        const avgLength = this.observations.reduce((sum, o) => sum + o.wordCount, 0) / this.observations.length;
        if (avgLength > 20)
            score += 0.1;
        else if (avgLength < 5)
            score -= 0.15;
        // Question rate factor
        if (signals.questionRate > 0.3)
            score += 0.15;
        else if (signals.questionRate > 0.1)
            score += 0.05;
        ...
        if (signals.backchannelRate > 0.5)
            score -= 0.2;
        else if (signals.backchannelRate > 0.3)
            score -= 0.1;
        ...
        if (totalEngagement > totalDisengagement * 2)
            score += 0.1;
        if (totalDisengagement > totalEngagement * 2)
            score -= 0.15;
        // Clamp score
    ...
    calculateSignals() {
        ...
        const firstAvgLatency = firstHalfLatency.reduce((sum, o) => sum + o.responseLatencyMs, 0) / firstHalfLatency.length;
        const secondAvgLatency = secondHalfLatency.reduce((sum, o) => sum + o.responseLatencyMs, 0) / secondHalfLatency.length;
        let latencyTrend = 'stable';
        if (secondAvgLatency < firstAvgLatency * 0.7)
            latencyTrend = 'faster';
        else if (secondAvgLatency > firstAvgLatency * 1.5)
            latencyTrend = 'slower';
        // Length trend
        const firstAvgLength = firstHalfLatency.reduce((sum, o) => sum + o.wordCount, 0) / firstHalfLatency.length;
        const secondAvgLength = secondHalfLatency.reduce((sum, o) => sum + o.wordCount, 0) / secondHalfLatency.length;
        let lengthTrend = 'stable';
        if (secondAvgLength > firstAvgLength * 1.3)
            lengthTrend = 'longer';
        else if (secondAvgLength < firstAvgLength * 0.7)
            lengthTrend = 'shorter';
        // Question rate
        ...
        const backchannelResponses = recent.filter((o) => o.wordCount <= 3 && o.disengagementPhrases > 0).length;
    ...
    calculateHalfScore(observations) {
        if (observations.length === 0)
            return 0.5;
        let score = 0.5;
        const avgLength = observations.reduce((sum, o) => sum + o.wordCount, 0) / observations.length;
        const avgLatency = observations.reduce((sum, o) => sum + o.responseLatencyMs, 0) / observations.length;
        ...
        if (questionCount > 0)
            score += 0.1;
        if (engagementCount > disengagementCount)
            score += 0.1;
        if (disengagementCount > engagementCount)
            score -= 0.1;
        return Math.max(0, Math.min(1, score));
    }
    determineAction(level, declining, signals) {
        if (level === 'distracted') {
            if (signals.backchannelRate > 0.5) {
                return {
                    action: 'check_in',
                    guidance: 'User seems to be just nodding along. Check in: "Are you still with me?" or "What\'s on your mind?"',
                };
            }
            return {
                action: 'shift_topic',
                guidance: "User seems distracted. Try a different approach or ask what they'd like to talk about.",
            };
        }
        if (level === 'low') {
            if (declining) {
                return {
                    action: 'energize',
                    guidance: 'Engagement is dropping. Try asking a direct question or introducing something new.',
                };
            }
            return {
                action: 'check_in',
    ...
    checkTopicContinuity(text, currentTopic) {
...
const engagementScorerRegistry = createSessionRegistry((sessionId) => new EngagementScorer(), { name: 'EngagementScorer', cleanup: (scorer) => scorer.reset(), verbose: false });
registerGlobalRegistry(engagementScorerRegistry);
export function getEngagementScorer(sessionId) {
    return engagementScorerRegistry.get(sessionId);
}
export function resetEngagementScorer(sessionId) {
    engagementScorerRegistry.reset(sessionId);
}
export function resetAllEngagementScorers() {
    engagementScorerRegistry.resetAll();
}
export function getActiveEngagementScorerCount() {
    return engagementScorerRegistry.getActiveCount();
}
export default EngagementScorer;
//# sourceMappingURL=engagement-scoring.js.map