...
import { generateMoodSignal, generateSpontaneousThought, generatePhysicalPresence, generateBreathSound, generateExcitementInterruption, generateLiveReaction, generatePlayfulness, generateFirstTurnNotice, canFire, recordFired, resetGenerators, } from './generators/index.js';
import { detectDisengagement, detectHighEngagement, detectBreakthrough, detectEvidence, classifyTopicWeight, detectUserEnergy, } from '../utils/detection.js';
 ...
 */
export async function applyDeepHumanization(response, context) {
    ...
    for (const { fn, type } of generators) {
        if (injections.length >= maxEffects)
            break;
        // Check cooldowns
        if (!canFire(type, context.turnCount))
            continue;
        try {
 ...
 */
function applyInjections(response, injections) {
    let result = response;
 ...
 */
export function resetDeepHumanization(personaId) {
 ...
 */
export function resetAllDeepHumanization() {
