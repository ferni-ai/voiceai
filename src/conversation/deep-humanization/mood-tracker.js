...
export class MoodTracker {
     ...
     */
    initializeSession(sessionId, userId, personaId) {
    ...
    calculateSummary() {
        ...
        if (this.snapshots.length >= 4) {
            ...
            const secondHalfAvg = this.snapshots.slice(mid).reduce((sum, s) => sum + s.energy, 0) / (this.snapshots.length - mid);
...
export function getMoodTracker(personaId) {
 ...
 */
export function initializeMoodTracker(personaId, sessionId, userId) {
 ...
 */
export async function resetMoodTrackerWithPersistence(personaId) {
 ...
 */
export function resetMoodTracker(personaId) {
    const tracker = trackers.get(personaId);
    if (tracker) {
        tracker.reset();
    }
 ...
 */
export async function resetAllMoodTrackers(persist = false) {
    if (persist) {
 ...
 */
export async function getRecentTrajectories(userId, limit = 10) {
    try {
        const db = await getFirestoreDb();
        if (!db)
            return [];
