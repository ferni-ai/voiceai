...
class MayaGamificationStore {
    ...
    GAMIFICATION_SUBCOLLECTION = 'maya_gamification';
    LEADERBOARD_COLLECTION = 'maya_leaderboards';
     ...
     */
    async getBadgesByRarity(userId, rarity) {
        if (!(await this.ensureInitialized()) || !this.db) {
            return [];
        }
        try {
     ...
     */
    async startChallenge(userId, challenge) {
        const newChallenge = {
            ...challenge,
     ...
     */
    async updateChallenge(userId, challengeId, updates) {
        if (!(await this.ensureInitialized()) || !this.db) {
            return null;
        }
        try {
     ...
     */
    async saveBehaviorTool(userId, tool) {
        const newTool = {
            ...tool,
     ...
     */
    async getBehaviorToolsByType(userId, toolType) {
        if (!(await this.ensureInitialized()) || !this.db) {
            return [];
        }
        try {
     ...
     */
    async setLeaderboardPreferences(userId, preferences) {
        const profile = await this.getProfile(userId);
        await this.updateProfile(userId, {
            preferences: {
                ...profile.preferences,
     ...
     */
    async importUserData(userId, data, options = {}) {
...
export async function initializeMayaGamificationStore() {
