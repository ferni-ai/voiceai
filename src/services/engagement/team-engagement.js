...
export { getIntelligentBanter, buildBanterContext, detectTimeOfDay, 
// LLM-driven banter (new!)
getLLMDrivenBanter, buildLLMSoftOpenInstructions, buildLLMArrivingInstructions, } from '../team-engagement/intelligent-banter.js';
// ============================================================================
// PERSONA EVOLUTION EVENTS - Stories of growth and change
// ============================================================================
export const PERSONA_EVOLUTION_STORIES = [
    // Ferni's evolution
    {
        ...
        description: "One of Ferni's kids graduated this week. 'Eight kids, and every graduation still gets me.'",
        occurredAt: new Date(),
    ...
    },
    {
        ...
        description: "I was thinking about Tanaka-san today. It's the anniversary of when I lost him. Funny how grief works—it gets softer but never goes away.",
        occurredAt: new Date(),
        sharedWithUser: false,
    ...
    // Alex's evolution
    {
        ...
        description: "My mom taught me her scallion pancake recipe this weekend. Finally! She's been 'not ready' to share it for 30 years.",
        occurredAt: new Date(),
        sharedWithUser: false,
    ...
    },
    {
        ...
        description: "I got a new plant. Still deciding on the name. 'New Guy' is getting old. Any suggestions?",
        occurredAt: new Date(),
        sharedWithUser: false,
    ...
    // Maya's evolution
    {
        ...
        description: 'I did it. The half marathon. Me—who used to think running was punishment. Compound and Interest were unimpressed, but Daniel cried.',
        occurredAt: new Date(),
        sharedWithUser: false,
    ...
    },
    {
        ...
        description: "My grandmother asked me again: 'Apo, are you taking care of yourself?' She's 84 and still looking out for me.",
        occurredAt: new Date(),
        sharedWithUser: false,
        unlockCondition: { type: 'relationship_stage', value: 'trusted_advisor' },
    },
    // Jordan's evolution
    {
        ...
        description: 'Destiny—my mentee—got into nursing school! Three years of planning, and she did it. I may have happy-cried.',
        occurredAt: new Date(),
        sharedWithUser: false,
    ...
    },
    {
        ...
        description: 'Compass and I found a new hiking trail this weekend. She was so happy she rolled in something unidentifiable. Worth it.',
        occurredAt: new Date(),
        sharedWithUser: false,
        unlockCondition: { type: 'relationship_stage', value: 'getting_to_know' },
    },
    // Nayan's evolution
    {
        ...
        description: 'I dreamt of Kailash last night. The mountain where everything became clear. Some places stay with you.',
        occurredAt: new Date(),
        sharedWithUser: false,
    ...
    // Peter's evolution
    {
        ...
        description: "Forty years of pattern-watching and I still find new ones. Today's discovery: people's spending predicts their mood three days out.",
        occurredAt: new Date(),
        sharedWithUser: false,
    ...
    },
    {
        ...
        description: "56 years with Carolyn. She still laughs at my pattern observations at dinner. 'Peter, you're doing it again.'",
        occurredAt: new Date(),
        sharedWithUser: false,
        unlockCondition: { type: 'relationship_stage', value: 'trusted_advisor' },
    },
...
export class TeamEngagementService {
    huddles = new Map();
    sharedEvolutions = new Map(); // userId -> eventIds shared
    userAnniversaries = new Map();
    // Persistence store
     ...
     */
    async generateTeamHuddle(userId, profile, type = 'weekly') {
        ...
        for (const personaId of selectedPersonas) {
            const personaComments = scripts.personaComments[personaId];
            if (personaComments) {
                // Get all comment arrays for this persona
                const allCommentArrays = Object.values(personaComments).filter((arr) => Array.isArray(arr) && arr.length > 0);
        ...
        getLogger().info({ userId, type, participants: huddle.participants.length }, '👥 Team huddle generated');
        return { intro, comments, outro };
    }
    /**
     * Select which personas should comment in a huddle
     */
    selectPersonasForHuddle(available, profile) {
     ...
     */
    async getUnlockedEvolutions(userId, profile, personaId) {
        // Load from persistence if not in memory
        ...
        for (const event of PERSONA_EVOLUTION_STORIES) {
            if (sharedIds.includes(event.id))
                continue;
            if (personaId && event.personaId !== personaId)
                continue;
     ...
     */
    checkUnlockCondition(event, profile) {
        ...
        switch (type) {
            case 'relationship_stage': {
                ...
                const requiredIndex = stageOrder.indexOf(value);
            ...
            case 'conversation_count': {
                if (!profile)
                    return false;
                return (profile.totalConversations || 0) >= value;
            }
            case 'time_based': {
            ...
            case 'topic_discussed': {
                if (!profile)
                    return false;
                return profile.preferredTopics?.includes(value) || false;
            }
            default:
                return false;
        }
     ...
     */
    markEvolutionShared(userId, eventId) {
        const sharedIds = this.sharedEvolutions.get(userId) || [];
    ...
    checkFerniday(profile) {
        ...
        if (created.getMonth() === today.getMonth() &&
            ...
            return {
                ...
                date: today,
                acknowledged: false,
                celebrationType: yearsTogetherValue >= 2 ? 'big' : yearsTogetherValue >= 1 ? 'medium' : 'small',
            };
        }
        return null;
    }
     ...
     */
    getCrossPersonaReference(fromPersonaId, context) {
        const references = CROSS_PERSONA_REFERENCES[fromPersonaId];
        if (!references)
            return null;
        ...
        const comments = references[randomPersona];
     ...
     */
    getSeasonalResponse(event, personaId) {
...
// SINGLETON
// ============================================================================
let teamEngagementService = null;
export function getTeamEngagementService() {
...
export function resetTeamEngagementService() {
    teamEngagementService = null;
}
/**
 * Shutdown team engagement service (call on app shutdown)
 */
export async function shutdownTeamEngagementService() {
    if (teamEngagementService) {
...
//# sourceMappingURL=team-engagement.js.map