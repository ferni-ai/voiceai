...
const MESSAGE_TEMPLATES = {
    habit_reminder: (data) => `🌱 Hey! Time for your "${data.habitName}" habit. Remember: small steps, big changes! — Maya`,
    streak_at_risk: (data) => `🔥 Your ${data.streakDays}-day streak on "${data.habitName}" needs you today! Don't let it slip - you've got this! — Maya`,
    streak_celebration: (data) => `🎉 ${data.streakDays} days strong on "${data.habitName}"! You're building something real. Keep showing up! — Maya`,
    challenge_day: (data) => `💪 Day ${data.dayNumber} of your ${data.challengeType} challenge! Today's action: ${data.action}. Let's go! — Maya`,
    weekly_reflection: () => `📝 It's reflection time! How did your week go? I'd love to hear about your wins (and learn from any struggles). — Maya`,
    proactive_checkin: (data) => data.customMessage ||
        `👋 Hey! Haven't heard from you in a bit. Everything okay? I'm here when you're ready. — Maya`,
    milestone_celebration: (data) => `🏆 WOW! ${data.milestone}! This is huge! Take a moment to really feel this achievement. So proud of you! — Maya`,
    comeback_welcome: (data) => `🌟 Welcome back! Life happens - what matters is you're here now. Ready to pick up where we left off? ${data.daysSinceActive ? `(It's been ${data.daysSinceActive} days - no judgment!)` : ''} — Maya`,
    mood_checkin: () => `💚 Quick mood check: How are you feeling today (1-10)? And energy level? This helps me understand your patterns! — Maya`,
};
// ============================================================================
// MAYA NOTIFICATION SERVICE
// ============================================================================
...
class MayaNotificationService extends EventEmitter {
    ...
    async getPreferences(userId) {
        ...
        const prefs = store.getUserPreference(userId, 'mayaNotificationPrefs');
        if (prefs) {
    ...
    async scheduleNotification(request) {
        ...
        const deliveryAddress = prefs.preferredMethod === 'sms' ? profile?.contactInfo?.phone : profile?.contactInfo?.email;
        if (!deliveryAddress) {
     ...
     */
    async scheduleDailyHabitReminders(userId) {
        const store = getProductivityStore();
        const habits = store.getUserEnhancedHabits(userId);
        const prefs = await this.getPreferences(userId);
        if (!prefs.enabled || habits.length === 0)
            return;
        const now = new Date();
     ...
     */
    async scheduleStreakAlert(userId, habitId, habitName, currentStreak) {
        const now = new Date();
     ...
     */
    async sendStreakCelebration(userId, habitName, streakDays) {
     ...
     */
    async scheduleChallengePrompt(userId, challengeType, dayNumber, action) {
        const prefs = await this.getPreferences(userId);
     ...
     */
    async scheduleWeeklyReflection(userId) {
        const now = new Date();
     ...
     */
    async sendSilenceCheckin(userId, daysSinceActive) {
        const now = new Date();
     ...
     */
    async sendMilestoneCelebration(userId, milestone) {
        const now = new Date();
        await this.scheduleNotification({
            userId,
    ...
    // ============================================================================
    startProactiveCheckLoop() {
        // Check every hour for proactive opportunities using managed interval
        registerInterval(MAYA_PROACTIVE_CHECK_INTERVAL, () => {
    ...
    async checkUserForProactiveOpportunities(userId) {
        const prefs = await this.getPreferences(userId);
        if (!prefs.enabled)
            return;
        const store = getProductivityStore();
    ...
    adjustForQuietHours(prefs, time) {
     ...
     */
    async adjustForCalendarBusy(userId, scheduledTime) {
        try {
            ...
            getLogger().debug({ error, userId }, 'Calendar busy check failed, proceeding with original time');
            return null;
        }
    }
}
...
export function getMayaNotificationService() {
    if (!serviceInstance) {
        serviceInstance = new MayaNotificationService();
    }
    return serviceInstance;
}
export async function initializeMayaNotificationService() {
    const service = getMayaNotificationService();
    await service.initialize();
    return service;
}
export async function shutdownEngagementNotificationService() {
    if (serviceInstance) {
...
//# sourceMappingURL=engagement-notification-service.js.map