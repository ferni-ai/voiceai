...
export class SlackNotificationService {
     ...
     */
    async notifyRollout(featureId, status, details) {
        const typeMap = {
     ...
     */
    async notifyDeployment(service, status, details) {
        const typeMap = {
     ...
     */
    async notifyIncident(title, status, details) {
        return this.notify({
    ...
    async notifyCrisisAlert(details) {
        const title = details.severity === 'emergency'
    ...
    // INTERNAL METHODS
    // ============================================================================
    buildMessage(context, emoji, color) {
        const blocks = [
            {
                type: 'section',
                text: {
    ...
    getWebhookForType(type) {
        if (type.startsWith('rollout_'))
            return this.webhooks.rollouts;
        if (type.startsWith('deployment_'))
            return this.webhooks.deployments;
        if (type.startsWith('incident_') || type.startsWith('health_'))
            return this.webhooks.alerts;
        if (type === 'crisis_alert')
            return this.webhooks.safety;
...
export function getSlackNotifications() {
    if (!slackService) {
...
// ============================================================================
export async function notifySlack(context) {
    return getSlackNotifications().notify(context);
}
export async function notifyRollout(featureId, status, details) {
    return getSlackNotifications().notifyRollout(featureId, status, details);
}
export async function notifyDeployment(service, status, details) {
    return getSlackNotifications().notifyDeployment(service, status, details);
}
export async function notifyIncident(title, status, details) {
    return getSlackNotifications().notifyIncident(title, status, details);
}
export async function notifyCrisisAlert(details) {
 ...
 */
export async function notifyDDoSAlert(details) {
    const severityMap = {
        low: 'warning',
        medium: 'error',
        high: 'error',
...
//# sourceMappingURL=slack-notifications.js.map