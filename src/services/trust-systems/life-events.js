 ...
 */
export function detectLifeEvents(userId, text, context) {
    const results = [];
    const signals = [];
 ...
 */
function extractEventDescription(text, eventType) {
 ...
 */
function determineImportance(type, sentiment, text) {
    // High importance events
    const highTypes = ['health', 'milestone', 'deadline'];
    if (type && highTypes.includes(type))
        return 'high';
    // High importance sentiments
    if (sentiment === 'nervous' || sentiment === 'dreading')
        return 'high';
    if (sentiment === 'excited')
        return 'medium';
    // Check for importance keywords
    if (/\b(important|big|major|crucial|critical)\b/i.test(text))
        return 'high';
...
function extractTags(text) {
    ...
    if (/\bwork\b/i.test(text))
        tags.push('work');
    if (/\b(family|mom|dad|parent|sibling|brother|sister)\b/i.test(text))
        tags.push('family');
    if (/\b(friend|buddy|pal)\b/i.test(text))
        tags.push('friends');
    if (/\b(health|doctor|medical)\b/i.test(text))
        tags.push('health');
    if (/\b(money|financial|pay|bill)\b/i.test(text))
        tags.push('financial');
    return tags;
}
/**
 ...
 */
function calculateConfidence(dateInfo, eventType, description) {
 ...
 */
export function getEventsNeedingReminders(userId) {
    const events = userEvents.get(userId) || [];
    const now = new Date();
    const tomorrow = new Date(now);
    tomorrow.setDate(tomorrow.getDate() + 1);
    return events.filter((e) => {
        if (!e.followUp.beforeReminder)
            return false;
        if (e.followUp.reminderSent)
            return false;
        if (e.date < now)
            return false;
 ...
 */
export function getEventsNeedingFollowUp(userId) {
    const events = userEvents.get(userId) || [];
    const now = new Date();
    return events.filter((e) => {
        if (!e.followUp.afterCheckIn)
            return false;
        if (e.followUp.checkInSent)
            return false;
        if (e.followUp.outcome)
            return false; // Already know outcome
 ...
 */
export function recordEventOutcome(userId, eventId, outcome) {
    const events = userEvents.get(userId);
    if (!events)
        return;
 ...
 * Mark reminder sent
 */
export function markReminderSent(userId, eventId) {
    const events = userEvents.get(userId);
    if (!events)
        return;
    const event = events.find((e) => e.id === eventId);
 ...
 * Mark check-in sent
 */
export function markCheckInSent(userId, eventId) {
    const events = userEvents.get(userId);
    if (!events)
        return;
    const event = events.find((e) => e.id === eventId);
 ...
 */
export function generateReminderMessage(event) {
    const messages = {
        excited: [
 ...
 */
export function generateFollowUpMessage(event) {
    const messages = {
        deadline: [
