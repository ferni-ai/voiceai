...
export function generateStarters(context) {
    ...
    if (context.callbackMoments && context.relationshipStage !== 'new') {
        ...
        if (callback) {
            ...
            if (callbackStarter)
                starters.push(callbackStarter);
        }
    }
    // 6. Gentle check-in if last session had emotional content
    if (context.lastEmotionalState &&
    ...
    log.debug({ userId: context.userId, starterCount: starters.length }, '💬 Generated conversation starters');
    return starters;
}
/**
 * Generate time-sensitive starter
 */
function generateTimeSensitiveStarter(event, context) {
 ...
 */
function generateFollowUpStarter(followUp, context) {
 ...
 */
function generateCelebrationStarter(win, context) {
 ...
 */
function generateGrowthStarter(pattern, context) {
 ...
 */
function generateCallbackStarter(moment, context) {
    ...
    if (moment.lastUsed) {
        ...
        if (daysSinceLastUse < 14)
            return null; // Wait 2 weeks between uses
    }
 ...
 */
function generateGentleCheckIn(context) {
 ...
 */
function generateRandomWarmth(context) {
 ...
 */
function selectBestCallback(moments, context) {
 ...
 */
export function getBestStarter(context) {
 ...
 */
export function markStarterUsed(userId, starterId, reception) {
