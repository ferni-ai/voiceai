...
import { generateRandomWarmth, generateThinkingOfYouMoments, markMomentSent, } from './thinking-of-you.js';
import { generateCelebration, getUncelebratedWins, } from './small-wins.js';
import { generateGrowthReflection, getUnreflectedGrowth, } from './growth-reflection.js';
import { cleanForFirestore } from '../../utils/firestore-utils.js';
import { routeToPersona, formatSmsMessage, formatPushNotification, formatVoiceMessage, } from '../outreach/persona-outreach-formatter.js';
 ...
 */
export function queueThinkingOfYou(userId, moment) {
 ...
 */
export function queueCelebration(userId, celebration) {
 ...
 */
export function queueGrowthReflection(userId, reflection) {
    ...
    const item = {
        ...
        scheduledFor: reflection.timing === 'now' ? new Date() : new Date(Date.now() + 24 * 60 * 60 * 1000),
        personaId,
        metadata: {
 ...
 */
export async function executeOutreach(item, method = 'sms') {
    ...
    try {
        ...
        if (sent) {
            ...
            outreachQueue.set(item.userId, queue.filter((i) => i.id !== item.id));
            // Mark as sent in thinking-of-you system
            if (item.type === 'thinking_of_you') {
                markMomentSent(item.userId, item.id);
            }
            log.info({ userId: item.userId, itemId: item.id, type: item.type, method }, '💌 Outreach sent');
            return {
                success: true,
                itemId: item.id,
                method,
 ...
 */
async function sendMessage(item, method) {
    try {
        ...
        const personaId = item.personaId ||
            routeToPersona(item.type, {
                topic: item.metadata?.triggerContext,
            });
        // Build format context from profile and item metadata
        const formatContext = {
            userName: profile.firstName || profile.name,
            topic: item.metadata?.triggerContext,
            habit: item.metadata?.habit,
            appointment: item.metadata?.appointment,
        };
        switch (method) {
            case 'sms': {
            ...
            case 'push': {
                ...
                const { sendPushNotification, hasPushEnabled } = await import('../outreach/delivery/push-notifications.js');
 ...
 */
export function generateOutreachOpportunities(userId) {
 ...
 */
export async function processUserOutreach(userId) {
 ...
 */
export function disableOutreach(userId) {
    const current = userPreferences.get(userId) || DEFAULT_PREFERENCES;
 ...
 */
export function enableOutreach(userId) {
    const current = userPreferences.get(userId) || DEFAULT_PREFERENCES;
...
//# sourceMappingURL=outreach-integration.js.map