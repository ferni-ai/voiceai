 ...
 */
export function detectSignificantShare(userId, userMessage, context) {
    ...
    const shouldTrack = isHeavy || upcomingEvent || isEmotionallySignificant || peopleMentioned.length > 0;
    if (!shouldTrack)
        return null;
    ...
    log.debug({ userId, topic: share.topic, followUpType, weight: share.emotionalWeight }, '💭 Significant share detected');
    return share;
}
/**
 ...
 */
function extractPeopleMentioned(message) {
    const people = [];
 ...
 */
export function generateThinkingOfYouMoments(userId) {
    const profile = profiles.get(userId);
    if (!profile || profile.significantShares.length === 0)
        return [];
    const moments = [];
    const now = new Date();
    for (const share of profile.significantShares) {
        // Skip if we've recently reached out
        if (profile.lastNoAgendaOutreach &&
            now.getTime() - profile.lastNoAgendaOutreach.getTime() < 24 * 60 * 60 * 1000) {
            continue;
        }
        // Skip if we've already generated a moment for this share
        const existingMoment = profile.pendingMoments.find((m) => m.trigger.context?.includes(share.id));
        if (existingMoment)
            continue;
 ...
 */
function createMomentFromShare(share, userId) {
    const now = new Date();
 ...
 */
function createSupportMessage(share) {
    const options = [
 ...
 */
function createCelebrateMessage(share) {
    const options = [
 ...
 */
function createRememberMessage(share) {
    const options = [
 ...
 */
function createCheckInMessage(share) {
    const options = [
 ...
 */
export function generateRandomWarmth(userId) {
    const profile = profiles.get(userId);
    if (!profile)
        return null;
 ...
 */
export function getDueMoments(userId) {
    const profile = profiles.get(userId);
    if (!profile)
        return [];
    const now = new Date();
 ...
 */
export function markMomentSent(userId, momentId) {
    const profile = profiles.get(userId);
    if (!profile)
        return;
 ...
 */
export function recordOutreachResponse(userId, momentId, responded) {
    const profile = profiles.get(userId);
    if (!profile)
        return;
 ...
 */
export function exportThinkingOfYouProfile(userId) {
    return profiles.get(userId) || null;
}
/**
 * Import profile from persistence
 */
export function importThinkingOfYouProfile(profile) {
    profiles.set(profile.userId, profile);
    log.debug({ userId: profile.userId, shareCount: profile.significantShares.length }, 'Imported thinking of you profile');
}
// ============================================================================
// EXPORTS
// ============================================================================
