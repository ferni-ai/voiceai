...
import { getActiveBoundaries, getProbingDepth } from './boundary-memory.js';
import { getAvoidedTopics } from './reading-between-lines.js';
import { getUnreflectedGrowth, } from './growth-reflection.js';
import { getSharedMoments, getCallbackTraits } from './inside-jokes.js';
import { getPendingIntentions, getUncelebratedWins, } from './small-wins.js';
 ...
 */
export function buildHandoffContext(userId, sourcePersonaId, targetPersonaId) {
    const criticalWarnings = [];
    const sensitiveAreas = [];
    const rapportBuilders = [];
    ...
    const relevantMoments = moments.filter((m) => m.triggers.some((t) => targetDomains.some((d) => t.includes(d) || d.includes(t))));
    ...
    const contextSummary = buildContextSummary(targetPersonaId, criticalWarnings, sensitiveAreas, rapportBuilders, pendingFollowUps, probingDepth);
    log.info({
        userId,
        sourcePersonaId,
        targetPersonaId,
 ...
 */
function buildContextSummary(targetPersonaId, criticalWarnings, sensitiveAreas, rapportBuilders, pendingFollowUps, probingDepth) {
    const lines = [];
 ...
 */
export function getHandoffWarnings(userId) {
    const warnings = [];
    // Absolute boundaries
    const boundaries = getActiveBoundaries(userId);
 ...
 */
export function formatHandoffForLLM(context) {
    return context.contextSummary;
}
/**
 * Create a brief handoff note for the receiving persona
 */
export function createHandoffNote(context) {
    const parts = [];
