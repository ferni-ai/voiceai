 ...
 */
export function recordEmotionalSnapshot(userId, snapshot) {
 ...
 */
function calculateValence(emotion, intensity) {
 ...
 */
function calculateArousal(emotion, intensity) {
 ...
 * Update daily summary
 */
function updateDailySummary(timeline, snapshot) {
    ...
    for (const [emotion, count] of Object.entries(summary.emotionDistribution)) {
        if (count > maxCount) {
            maxCount = count;
            summary.dominantEmotion = emotion;
        }
    }
}
/**
 ...
 */
function updateBaseline(timeline) {
 ...
 */
function detectPeaksValleys(timeline) {
    ...
    for (let i = 1; i < summaries.length - 1; i++) {
        ...
        if (curr > prev && curr > next && curr > baseline + threshold) {
            const existingPeak = timeline.peaks.find((p) => p.date.getTime() === summaries[i].date.getTime());
            if (!existingPeak) {
                timeline.peaks.push({
                    type: 'peak',
        ...
        if (curr < prev && curr < next && curr < baseline - threshold) {
            const existingValley = timeline.peaks.find((p) => p.date.getTime() === summaries[i].date.getTime());
            if (!existingValley) {
                timeline.peaks.push({
                    type: 'valley',
                    date: summaries[i].date,
...
function updateTrends(timeline) {
    ...
    for (const period of periods) {
        ...
        if (periodSummaries.length < 3)
            continue;
        const valences = periodSummaries.map((s) => s.avgValence);
        const startValence = valences.slice(0, Math.ceil(valences.length / 3)).reduce((a, b) => a + b, 0) /
            Math.ceil(valences.length / 3);
        const endValence = valences.slice(-Math.ceil(valences.length / 3)).reduce((a, b) => a + b, 0) /
            Math.ceil(valences.length / 3);
 ...
 */
function detectDayOfWeekPattern(timeline) {
    const dayValences = [[], [], [], [], [], [], []];
    for (const summary of timeline.dailySummaries) {
        const day = summary.date.getDay();
        dayValences[day].push(summary.avgValence);
    }
    const dayAvgs = dayValences.map((vals) => vals.length > 0 ? vals.reduce((a, b) => a + b, 0) / vals.length : null);
    // Find significant differences
    const nonNullAvgs = dayAvgs.filter((v) => v !== null);
    if (nonNullAvgs.length < 5)
        return null;
 ...
 */
function detectTriggerPatterns(timeline) {
    const patterns = [];
    ...
    for (const snapshot of timeline.snapshots) {
        ...
        if (snapshot.valence > 0.3)
            entry.positive++;
        else if (snapshot.valence < -0.3)
            entry.negative++;
 ...
 */
function detectGrowthPattern(timeline) {
    if (timeline.dailySummaries.length < 30)
        return null;
 ...
 */
function getOrCreateTimeline(userId) {
 ...
 */
export function getCurrentMoodContext(userId) {
 ...
 */
export function getRecentPeaksValleys(userId, limit = 5) {
    const timeline = timelines.get(userId);
    if (!timeline)
        return [];
 ...
 */
export function getInsightfulPatterns(userId) {
    const timeline = timelines.get(userId);
    if (!timeline)
        return [];
 ...
 */
export function exportTimelineData(userId, period = 'month') {
    const timeline = timelines.get(userId);
    if (!timeline)
        return null;
    const periodDays = {
 ...
 */
export function generateTimelineSummary(userId) {
    const timeline = timelines.get(userId);
    if (!timeline || timeline.dailySummaries.length < 7)
        return null;
    const sections = [];
...
//# sourceMappingURL=sentiment-timeline.js.map