...
export type ThinkingType =
  | 'mulling' // Still processing what they said
  | 'connecting' // Connected it to something else
  | 'realizing' // Had a realization about it
  | 'questioning' // Raised new questions
  | 'remembering' // Just came to mind
  | 'concerned'; // Worried about them
...
export function recordThinkingMoment(params: {
  ...
  const existing = userRecords.find(
    (r) =>
  ...
  if (existing) {
    ...
    if (userQuote) existing.userQuote = userQuote;
  ...
  log.info(
    { userId, personaId, topic, thinkingType, emotionalWeight },
    '💭 Recorded thinking moment'
  );
 ...
 */
export function getThinkingMomentToSurface(
  userId: string,
  personaId: string,
  ...
  const candidates = userRecords.filter(
    (r) =>
      !r.surfacedAt &&
  ...
  candidates.sort((a, b) => {
    ...
    if (aSessionScore !== bSessionScore) return bSessionScore - aSessionScore;
 ...
 */
export function markThinkingSurfaced(recordId: string): void {
...
function generateThinkingPhrase(record: ThinkingRecord): GeneratedPhrase {
...
function generateMullingPhrase(topic: string, quote: string | undefined, sessions: number): string {
  const phrases = quote
    ? [
  ...
  if (sessions > 3) {
    phrases.push(
      `I know it's been a while, but I still think about what you shared about ${topic}.`
    );
  }
...
function generateConnectingPhrase(topic: string, sessions: number): string {
  const phrases = [
  ...
  if (sessions === 1) {
    phrases.unshift(
      `Even since yesterday, I've been connecting what you said about ${topic} to other things.`
    );
  }
...
function generateRealizingPhrase(topic: string, _sessions: number): string {
  const phrases = [
...
function generateQuestioningPhrase(topic: string, _sessions: number): string {
  const phrases = [
...
function generateRememberingPhrase(
  topic: string,
  ...
  if (sessions > 5) {
    phrases.push(
      `It's been a while since we talked about ${topic}, but I still think about it sometimes.`
    );
  }
...
function generateConcernedPhrase(topic: string, sessions: number): string {
  const phrases = [
  ...
  if (sessions === 1) {
    phrases.unshift(
      `I couldn't stop thinking about ${topic}. Just wanted to make sure you're okay.`
    );
  }
...
function extractMeaningfulQuote(text: string): string | undefined {
...
export function loadThinkingRecords(userId: string, records: ThinkingRecord[]): void {
  // Hydrate dates
...
export function getThinkingRecordsForPersistence(userId: string): ThinkingRecord[] {
  return thinkingRecords.get(userId) || [];
}

export function getAllUnsurfacedThinking(userId: string): ThinkingRecord[] {
