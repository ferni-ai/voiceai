 ...
 */
export function detectSmallWin(userId, userMessage, context) {
 ...
 */
function checkIntentionCompletion(profile, lower, original) {
    for (const intention of profile.pendingIntentions) {
        ...
        const explicitComplete = lower.includes('did it') ||
            lower.includes('i did') ||
            lower.includes('finally') ||
            lower.includes('actually');
        if (matches || explicitComplete) {
            intention.status = 'completed';
            const win = createWin(profile, 'followed_through', original, `Following through on: ${intention.intention}`);
            intention.linkedWinId = win.id;
            log.info({ userId: profile.userId, intention: intention.intention }, '🎯 Intention completed!');
            return win;
        }
    }
    return null;
 ...
 */
function createWin(profile, type, message, whatMadeItHard) {
    const win = {
    ...
    log.debug({ userId: profile.userId, type, description: message.slice(0, 50) }, '🏆 Small win detected');
    return win;
}
// ============================================================================
// INTENTION TRACKING
 ...
 */
export function detectIntention(userId, userMessage) {
    const profile = getOrCreateProfile(userId);
 ...
 */
function extractCompletionKeywords(intention) {
    const keywords = [];
    const words = intention.toLowerCase().split(/\s+/);
    // Get meaningful words
    const meaningful = words.filter((w) => w.length > 3 && !['that', 'this', 'with', 'them', 'about', 'just', 'really'].includes(w));
 ...
 */
function parseTargetTime(timeRef) {
    const now = new Date();
 ...
 */
export function generateCelebration(userId, win) {
 ...
 */
function createCelebration(win, preference) {
    const celebrations = {
        followed_through: {
            enthusiastic: [
...
function getIntensity(type) {
    const bigWins = ['courage_moment', 'hard_conversation', 'boundary_held'];
    const mediumWins = ['followed_through', 'self_care', 'asked_for_help'];
    if (bigWins.includes(type))
        return 'big';
    if (mediumWins.includes(type))
        return 'medium';
    return 'small';
}
/**
 ...
 */
export function recordCelebrationResponse(userId, winId, response) {
    const profile = profiles.get(userId);
    if (!profile)
        return;
 ...
 */
export function getPendingIntentions(userId) {
    const profile = profiles.get(userId);
 ...
 */
export function getOverdueIntentions(userId, options) {
    const profile = profiles.get(userId);
    if (!profile)
        return [];
    ...
    return profile.pendingIntentions.filter((intention) => {
        ...
        if (includeNoTarget) {
            const daysSinceStated = (now.getTime() - intention.statedAt.getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
export function generateIntentionFollowUp(intention) {
    const templates = {
        curious: [
 ...
 */
export function getIntentionToFollowUp(userId) {
 ...
 */
export function markIntentionAbandoned(userId, intentionId) {
    const profile = profiles.get(userId);
    if (!profile)
        return;
 ...
 */
export function markIntentionStruggled(userId, intentionId) {
    const profile = profiles.get(userId);
    if (!profile)
        return null;
    const intention = profile.pendingIntentions.find((i) => i.id === intentionId);
 ...
 */
export function getUncelebratedWins(userId) {
    const profile = profiles.get(userId);
 ...
 */
export function recordKnownDifficulty(userId, difficulty) {
    const profile = getOrCreateProfile(userId);
 ...
 */
export function exportSmallWinsProfile(userId) {
    return profiles.get(userId) || null;
}
/**
 * Import profile from persistence
 */
export function importSmallWinsProfile(profile) {
    profiles.set(profile.userId, profile);
    log.debug({ userId: profile.userId, winCount: profile.wins.length }, 'Imported small wins profile');
}
// ============================================================================
// EXPORTS
// ============================================================================
...
//# sourceMappingURL=small-wins.js.map