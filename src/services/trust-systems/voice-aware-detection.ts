 ...
 */
function detectVoiceTextContradiction(
  lowerMessage: string,
  voiceEmotion: VoiceEmotionSignal,
  context: VoiceAwareContext
): UnsaidSignal | null {
  ...
  const voiceSaysNotFine =
    CONTRADICTING_VOICE_EMOTIONS.includes(voiceEmotion.primary) && voiceEmotion.intensity > 0.5;

  if (textSaysFine && voiceSaysNotFine) {
    // THIS IS THE MOMENT - voice reveals what words hide
    const isHighConfidence =
      HIGH_CONFIDENCE_VOICE_EMOTIONS.includes(voiceEmotion.primary) ||
 ...
 */
function detectVoiceOnlySignals(
  lowerMessage: string,
  voiceEmotion: VoiceEmotionSignal
): UnsaidSignal | null {
  ...
  if (
    voiceEmotion.characteristics?.pitchVariance !== undefined &&
    ...
    if (hasEmotionalTopic) {
      return {
        ...
        phrase:
          "You're talking about something really significant, but your voice is very calm. That's okay - but I'm here if there's more underneath.",
        context: {
          userMessage: lowerMessage,
        },
      };
 ...
 */
function generateVoiceAwarePhrase(
  voiceEmotion: VoiceEmotionSignal,
  detectionType: 'contradiction' | 'voice_only'
): string {
 ...
 */
function deduplicateSignals(signals: UnsaidSignal[]): UnsaidSignal[] {
  const byType = new Map<string, UnsaidSignal>();
