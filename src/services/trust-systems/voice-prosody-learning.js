...
function deserializeBaseline(b) {
    return {
        ...b,
 ...
 * Flush persistence
 */
export async function flushVoiceProsodyPersistence() {
 ...
 */
export async function shutdownVoiceProsody() {
 ...
 */
export function recordVoiceSample(userId, characteristics, context) {
    // Store sample
    const samples = voiceSamples.get(userId) || [];
    samples.push({
        timestamp: new Date(),
    ...
    if (context?.detectedEmotion || context?.userConfirmedEmotion) {
        updateEmotionalProfile(userId, context.userConfirmedEmotion || context.detectedEmotion, characteristics);
    }
 ...
 */
function updateBaseline(userId, samples) {
    if (samples.length < MIN_SAMPLES_FOR_BASELINE)
        return;
    // Use only "neutral" samples for baseline (no strong emotion)
    const neutralSamples = samples.filter((s) => !s.detectedEmotion || s.detectedEmotion === 'neutral');
 ...
 */
function calculateBaseline(userId, samples) {
    ...
    const confidence = Math.min(1, samples.length / 50) * 0.7 + (1 - calculateVarianceScore(chars)) * 0.3;
 ...
 */
function updateEmotionalProfile(userId, emotion, characteristics) {
    const baseline = userBaselines.get(userId);
    if (!baseline)
        return;
 ...
 */
export function analyzeDeviation(userId, currentCharacteristics) {
    const baseline = userBaselines.get(userId);
    ...
    const possibleMeaning = generateInterpretation(significantFactors, direction, baseline.emotionalProfiles);
    return {
 ...
 */
function interpretFactor(factor, increased) {
    ...
    if (!interp)
        return `${factor} is ${increased ? 'higher' : 'lower'} than usual`;
    return increased ? interp.up : interp.down;
}
/**
 * Determine overall direction of deviation
 */
function determineDirection(factors, baseline, current) {
    if (factors.length === 0)
        return 'normal';
    ...
    if (elevatedSignals >= 2)
        return 'elevated';
    if (elevatedSignals <= 1 && factors.length >= 2)
        return 'subdued';
    return 'normal';
}
/**
 ...
 */
function generateInterpretation(factors, direction, emotionalProfiles) {
    if (factors.length === 0) {
 ...
 */
export function getVoiceEvolution(userId, period) {
    const samples = voiceSamples.get(userId);
 ...
 */
export function getFamiliarityScore(userId) {
    const baseline = userBaselines.get(userId);
    const samples = voiceSamples.get(userId);
    if (!baseline || !samples) {
        return {
 ...
 */
export function getEmotionDetectionBoost(userId) {
...
function calculateVarianceScore(chars) {
    // Lower variance = more consistent baseline = higher score
    const keys = ['pitchMean', 'energyMean', 'speakingRate'];
 ...
 */
export function detectVoiceMention(text) {
    const patterns = {
    ...
    for (const [type, pattern] of Object.entries(patterns)) {
        if (pattern.test(text)) {
            return { mentioned: true, type: type };
        }
    }
 ...
 */
export function generateVoiceContext(userId) {
    const familiarity = getFamiliarityScore(userId);
    ...
    const samples = voiceSamples.get(userId) || [];
    const recentSamples = samples.filter((s) => Date.now() - s.timestamp.getTime() < 60 * 60 * 1000 // Last hour
    );
    if (recentSamples.length > 0 && baseline.confidence >= BASELINE_CONFIDENCE_THRESHOLD) {
        ...
        if (deviation && deviation.deviates && deviation.confidence > 0.5) {
            ...
            if (deviation.direction === 'subdued') {
                sections.push(`💡 HINT: They sound quieter/slower than usual. Check in gently without asking "are you okay?" directly.`);
            }
            else if (deviation.direction === 'elevated') {
                sections.push(`💡 HINT: They sound more energized than usual. Could be excitement OR stress - listen for which.`);
            }
...
//# sourceMappingURL=voice-prosody-learning.js.map