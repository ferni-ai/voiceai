 ...
 */
export function recordLinguisticPatterns(userId, message, context) {
 ...
 */
function extractEmotionVocabulary(message, profile, detectedEmotion) {
 ...
 */
function extractSignaturePhrases(message, profile) {
    ...
    for (let len = minLength; len <= maxLength && len <= words.length; len++) {
        for (let i = 0; i <= words.length - len; i++) {
            ...
            const occurrences = profile.recentMessages.filter((m) => m.message.toLowerCase().includes(phrase)).length;
            if (occurrences >= 2) {
 ...
 */
function updateSpeechPatterns(message, profile) {
    // Sentence length
    const sentences = message.split(/[.!?]+/).filter((s) => s.trim().length > 0);
    // 🦀 Rust-accelerated word counting for each sentence
    const avgWords = sentences.reduce((sum, s) => sum + (RUST_COUNTING_AVAILABLE ? countWordsRust(s) : s.split(/\s+/).length), 0) / (sentences.length || 1);
 ...
 */
export function adaptResponseStyle(response, userId) {
 ...
 */
export function getTheirWordFor(userId, emotion) {
    const profile = profiles.get(userId);
    if (!profile)
        return null;
 ...
 */
export function isWordAvoided(userId, word) {
    const profile = profiles.get(userId);
    if (!profile)
        return false;
    return profile.avoidedWords.some((aw) => aw.word.toLowerCase() === word.toLowerCase() && aw.confidenceAvoided > 0.6);
}
// ============================================================================
// CONTEXT BUILDING
// ============================================================================
 ...
 */
export function buildLinguisticContext(userId) {
    const profile = profiles.get(userId);
    if (!profile)
        return '';
    const sections = ['[LINGUISTIC MIRRORING]'];
    // Emotion vocabulary
 ...
 */
export function getLinguisticProfile(userId) {
    return profiles.get(userId) || null;
}
/**
 * Save profile to Firestore.
 */
export async function saveLinguisticProfile(userId) {
    const profile = profiles.get(userId);
    if (!profile)
        return;
    try {
 ...
 */
export async function loadLinguisticProfile(userId) {
    try {
