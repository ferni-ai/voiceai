 ...
 */
function getProfile(userId, personaId) {
...
export function recordPersonaGrowth(params) {
    const { userId, personaId, growthType, topic, beforeThinking, afterThinking, userContribution, triggerQuote, significance = 'moderate', relationshipStage = 'familiar', } = params;
    const profile = getProfile(userId, personaId);
    // Check if we already have growth about this topic
    const existing = profile.growthRecords.find((r) => r.topic.toLowerCase() === topic.toLowerCase() && !r.sharedAt);
    if (existing) {
        ...
        if (significance === 'major' ||
            ...
            if (triggerQuote)
                existing.triggerQuote = triggerQuote;
 ...
 */
export function getGrowthMomentToShare(userId, personaId, currentTopic) {
    const profile = profiles.get(`${userId}:${personaId}`);
    if (!profile)
        return null;
    // Filter to unshared records
    const candidates = profile.growthRecords.filter((r) => !r.sharedAt && !profile.sharedTopics.includes(r.topic.toLowerCase()));
 ...
 */
export function markGrowthShared(recordId) {
...
function generateSharingPhrase(record) {
...
function generatePerspectiveShiftPhrase(topic, _afterThinking, quote) {
    const phrases = quote
        ? [
...
function generateLearnedFromUserPhrase(topic, _afterThinking, quote) {
    const phrases = quote
        ? [
...
function generateInfluencedThinkingPhrase(topic, _afterThinking) {
    const phrases = [
...
function generateReconsideredPhrase(topic, _afterThinking, quote) {
    const phrases = quote
        ? [
...
function generateExpandedViewPhrase(topic, _afterThinking) {
    const phrases = [
...
function generateSoftenedStancePhrase(topic, _afterThinking) {
    const phrases = [
...
function generateGrewTogetherPhrase(topic) {
    const phrases = [
...
// ============================================================================
function getPersonaDefaultThinking(personaId, topic) {
    const defaults = {
        ferni: {
    ...
    return (personaDefaults[topicKey] ||
...
// PERSISTENCE
// ============================================================================
export function loadPersonaGrowthProfile(userId, personaId, data) {
    const key = `${userId}:${personaId}`;
    // Hydrate dates
    const hydrated = {
        ...data,
    ...
    log.debug({ userId, personaId, recordCount: hydrated.growthRecords.length }, '🌱 Loaded growth profile');
}
export function getPersonaGrowthForPersistence(userId, personaId) {
    return profiles.get(`${userId}:${personaId}`) || null;
}
export function getAllGrowthProfiles(userId) {
    const result = [];
...
//# sourceMappingURL=persona-growth.js.map