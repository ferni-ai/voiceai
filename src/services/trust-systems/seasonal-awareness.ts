...
export interface PersonalDate {
  id: string;
  date: { month: number; day: number }; // Recurring annually
  name: string;
  type: AnniversaryType;
  description?: string;
  yearsAgo?: number; // For milestone tracking
  approach: 'celebrate' | 'acknowledge' | 'gentle' | 'avoid';
  daysBeforeToMention?: number; // How early to bring it up
}
...
export interface SADIndicator {
  ...
  moodScore: number;
  energyScore: number;
  correlation?: number; // How much daylight seems to affect this user
}
...
const UNIVERSAL_HOLIDAYS: Array<{
  name: string;
  month: number;
  day?: number;
  weekday?: { week: number; day: number }; // e.g., 4th Thursday
  type: HolidayType;
}> = [
...
const seasonalProfiles = new Map<string, SeasonalProfile>();
 ...
 */
function getDaysIntoSeason(date: Date = new Date()): number {
  const month = date.getMonth() + 1;
  const day = date.getDate();

  // Approximate season start dates (northern)
  const seasonStarts: Record<Season, { month: number; day: number }> = {
 ...
 */
function getOrCreateProfile(userId: string): SeasonalProfile {
 ...
 */
export function recordSeasonalData(
  userId: string,
  data: {
    mood: number;
 ...
 */
export function updateHolidayPreference(
  userId: string,
  holiday: string,
  preference: Partial<HolidayPreference>
): void {
  const profile = getOrCreateProfile(userId);
 ...
 */
function detectSeasonalPatterns(profile: SeasonalProfile): void {
  // Need data from multiple seasons
  if (profile.seasonPatterns.length < 2) return;
 ...
 */
export function detectSADPatterns(userId: string): {
  likely: boolean;
  correlation: number;
 ...
 */
export function buildSeasonalContext(userId: string): SeasonalContext {
  const profile = getOrCreateProfile(userId);
  const now = new Date();
 ...
 * Get upcoming holidays
 */
function getUpcomingHolidays(profile: SeasonalProfile, from: Date): UpcomingHoliday[] {
...
function getNextOccurrence(
  holiday: { month: number; day?: number; weekday?: { week: number; day: number } },
  from: Date
): Date {
 ...
 */
function getNthWeekday(year: number, month: number, week: number, weekday: number): Date {
 ...
 * Get upcoming personal dates
 */
function getUpcomingPersonalDates(profile: SeasonalProfile, from: Date): UpcomingPersonalDate[] {
  const upcoming: UpcomingPersonalDate[] = [];
  const lookAhead = 30;
  ...
  for (const pd of profile.personalDates) {
    ...
    const daysUntil = Math.floor(
      (nextOccurrence.getTime() - from.getTime()) / (24 * 60 * 60 * 1000)
    );
 ...
 */
function getSuggestedApproach(pref?: HolidayPreference): string {
  if (!pref) return 'Ask how they feel about it';

  if (pref.avoidMentioning) return 'Avoid bringing it up';
 ...
 */
function getPersonalDateApproach(pd: PersonalDate): string {
  switch (pd.approach) {
    case 'celebrate':
 ...
 */
export function getSeasonalProfile(userId: string): SeasonalProfile | null {
 ...
 */
export function generateSeasonalContextForLLM(userId: string): string | null {
  ...
  if (context.upcomingHolidays.length > 0) {
    const notable = context.upcomingHolidays.filter(
      (h) => h.daysUntil <= 7 && h.userSentiment !== 'unknown'
    );
    if (notable.length > 0) {
      sections.push('');
