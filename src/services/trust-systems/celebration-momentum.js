...
// ============================================================================
const momentumProfiles = new Map();
 ...
 * Flush persistence
 */
export async function flushCelebrationMomentumPersistence() {
 ...
 */
export async function shutdownCelebrationMomentum() {
 ...
 * Record a new win
 */
export function recordWin(userId, win) {
 ...
 */
function updateStreaks(profile, win) {
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    // Daily streak
    let dailyStreak = profile.streaks.find((s) => s.type === 'daily' && s.active);
    if (dailyStreak) {
        const lastWinDay = new Date(dailyStreak.lastWinDate.getFullYear(), dailyStreak.lastWinDate.getMonth(), dailyStreak.lastWinDate.getDate());
        const daysSinceLastWin = Math.floor((today.getTime() - lastWinDay.getTime()) / (24 * 60 * 60 * 1000));
    ...
    const typeStreak = profile.streaks.find((s) => s.type === 'type_specific' && s.winType === win.type && s.active);
    if (typeStreak) {
        typeStreak.count++;
        ...
        const prevStreak = profile.streaks.find((s) => s.type === 'type_specific' && s.winType === win.type && !s.active);
        if (prevStreak) {
            // Start fresh
            profile.streaks.push({
...
function updateThemes(profile) {
    ...
    profile.themes = Array.from(typeCounts.entries())
        ...
        let trend = 'stable';
        if (newPercentage > prevPercentage + 5)
            trend = 'increasing';
        else if (newPercentage < prevPercentage - 5)
            trend = 'decreasing';
        return {
            type,
            count,
 ...
 */
function calculateMomentum(profile) {
    const now = Date.now();
 ...
 */
function detectSpecialStates(profile) {
    ...
    const priorWins = profile.wins.filter((w) => w.detectedAt >= threeWeeksAgo && w.detectedAt < twoWeeksAgo).length;
    profile.comebackDetected = priorWins === 0 && recentWins >= 2;
    // Breakthrough - multiple wins of a difficult type
    const hardWinsRecent = profile.wins.filter((w) => w.detectedAt >= twoWeeksAgo && w.difficulty === 'hard').length;
 ...
 */
export function generateCelebrations(userId) {
    const profile = momentumProfiles.get(userId);
    if (!profile)
        return [];
    const suggestions = [];
 ...
 */
function getThemeMessage(type, count) {
    const messages = {
 ...
 * Generate SSML
 */
function generateSSML(text, mood) {
    const prosody = {
 ...
 */
export function getMomentumProfile(userId) {
    return momentumProfiles.get(userId) || null;
}
/**
 * Get active streaks
 */
export function getActiveStreaks(userId) {
    const profile = momentumProfiles.get(userId);
    if (!profile)
        return [];
 ...
 */
export function getMomentumSummary(userId) {
    const profile = momentumProfiles.get(userId);
    if (!profile || profile.totalWins === 0)
        return null;
    const parts = [];
    // Momentum state
 ...
 */
export function markCelebrationShown(userId, streakId) {
    const profile = momentumProfiles.get(userId);
    if (!profile)
        return;
    if (streakId) {
...
//# sourceMappingURL=celebration-momentum.js.map