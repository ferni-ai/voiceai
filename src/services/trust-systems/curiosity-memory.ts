...
export type MentionType =
  | 'person' // Friend, family, colleague
  | 'place' // Locations mentioned
  | 'event' // Upcoming or past events
  | 'activity' // Hobbies, interests, actions
  | 'goal' // Aspirations, intentions
  | 'object' // Things (new car, book they're reading)
  | 'media'; // Movies, shows, music they mentioned
...
const profiles = new Map<string, CuriosityProfile>();

// Track what we've followed up on this session
const followedUpThisSession = new Set<string>();
...
export function recordPassingMention(params: {
  ...
  const existing = profile.mentions.find(
    (m) => normalizeName(m.name) === normalizedName && m.type === type && !m.followedUpAt
  );
  ...
  if (existing) {
    ...
    if (originalQuote) existing.originalQuote = originalQuote;
    if (emotionalContext) existing.emotionalContext = emotionalContext;
    if (expectedDate) existing.expectedDate = expectedDate;
    if (relatedTopics) {
  ...
  if (unfollowed.length > 50) {
    ...
    for (const m of toRemove) {
      ...
      if (idx >= 0) profile.mentions.splice(idx, 1);
    }
  }
 ...
 */
export function getFollowUpOpportunity(
  userId: string,
  currentTopic?: string
  ...
  const candidates = profile.mentions.filter((m) => {
    ...
    if (followedUpThisSession.has(m.id)) return false;

    // Time check: at least 1 day old, not more than 6 months
    const age = now - new Date(m.mentionedAt).getTime();
    if (age < 24 * 60 * 60 * 1000) return false; // Too recent
    if (age > 180 * 24 * 60 * 60 * 1000) return false; // Too old
  ...
  const scored = candidates.map((m) => {
    ...
    // Priority bonus
    if (m.followUpPriority === 'high') score += 30;
    if (m.followUpPriority === 'medium') score += 15;
 ...
 */
export function markFollowedUp(mentionId: string): void {
...
export function detectPassingMentions(params: {
  ...
  const mentions: ReturnType<typeof detectPassingMentions> = [];
    ...
    while ((match = pattern.exec(userText)) !== null) {
      ...
      if (name && name.length > 1 && !isCommonWord(name)) {
        const exists = mentions.some(
          (m) => m.type === 'person' && normalizeName(m.name) === normalizeName(name)
        );
        if (!exists) {
          mentions.push({
            type: 'person',
  ...
  const eventPatterns = [
    ...
    { pattern: /(\w+(?:'s)? wedding)/gi, type: 'event' as const },
    {
      pattern: /(?:the |a )(?:concert|show|party|reunion|conference) (?:for |at |with )?(\w+)?/gi,
      type: 'event' as const,
    },
    { pattern: /deadline (?:for |on )?(\w+(?:\s+\w+)?)/gi, type: 'event' as const },
  ];
  ...
  const unique = mentions.filter(
    (m, i, arr) =>
      arr.findIndex((x) => x.type === m.type && normalizeName(x.name) === normalizeName(m.name)) ===
      i
  );
...
function generateFollowUpPhrase(mention: PassingMention): GeneratedPhrase {
  const { type, name, context, emotionalContext, timeSensitive, expectedDate, mentionedAt } =
    mention;

  let phrase: string;
  let urgency: 'immediate' | 'soon' | 'whenever' = 'whenever';

  const weeksAgo = Math.floor(
    (Date.now() - new Date(mentionedAt).getTime()) / (7 * 24 * 60 * 60 * 1000)
  );
    ...
    case 'event':
      if (timeSensitive && expectedDate) {
        const daysUntil = Math.ceil(
          (new Date(expectedDate).getTime() - Date.now()) / (24 * 60 * 60 * 1000)
        );
        if (daysUntil > 0 && daysUntil < 14) {
...
function generatePersonFollowUp(
  name: string,
  timeRef: string,
  emotional?: 'positive' | 'negative' | 'neutral' | 'mixed'
): string {
  const phrases =
    emotional === 'negative' || emotional === 'mixed'
      ? [
...
function generateActivityFollowUp(name: string, timeRef: string): string {
  const phrases = [
...
function generateGoalFollowUp(name: string, timeRef: string, context: string): string {
  const phrases = [
...
function generatePlaceFollowUp(name: string, timeRef: string, context: string): string {
...
function calculateFollowUpPriority(params: {
  ...
  expectedDate?: Date;
  context: string;
}): { priority: 'high' | 'medium' | 'low'; reason: string; timeSensitive: boolean } {
...
function mapEmotion(emotion?: string): 'positive' | 'negative' | 'neutral' | 'mixed' | undefined {
  if (!emotion) return undefined;
  const e = emotion.toLowerCase();
  if (['happy', 'excited', 'joyful', 'grateful'].includes(e)) return 'positive';
  if (['sad', 'angry', 'anxious', 'worried', 'frustrated'].includes(e)) return 'negative';
  if (e === 'mixed' || e === 'conflicted') return 'mixed';
  return 'neutral';
}

function extractDateFromContext(text: string): Date | undefined {
...
function isLikelyPlace(name: string): boolean {
  ...
  if (exclude.includes(lower)) return false;
...
export function loadCuriosityProfile(userId: string, data: CuriosityProfile): void {
  // Hydrate dates
  const hydrated: CuriosityProfile = {
    ...data,
...
export function getCuriosityProfileForPersistence(userId: string): CuriosityProfile | null {
  return profiles.get(userId) || null;
}

export function getAllUnfollowedMentions(userId: string): PassingMention[] {
  const profile = profiles.get(userId);
  return profile?.mentions.filter((m) => !m.followedUpAt) || [];
}

export function getMentionsByType(userId: string, type: MentionType): PassingMention[] {
  const profile = profiles.get(userId);
 ...
 */
export function clearUserMentions(userId: string): void {
