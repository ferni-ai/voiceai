...
const EMAIL_SUBJECTS = {
    thinking_of_you: (name) => `Hey ${name}, just thinking of you`,
    celebration: (name) => `${name}, something caught my eye ✨`,
    growth_reflection: (name) => `${name}, I noticed something`,
};
// ============================================================================
// DELIVERY FUNCTIONS
// ============================================================================
 ...
 */
export async function deliverPush(item, pushToken) {
 ...
 */
export async function deliverEmail(item, email, userName, personaId) {
    ...
    const subject = subjectTemplates[item.type]?.(name, firstName) || `Hey ${name}, ${firstName} here`;
    const payload = {
        to: email,
        subject,
        text: item.message,
 ...
 * Deliver via SMS
 */
export async function deliverSms(item, phone) {
 ...
 */
export async function deliverVoice(item, phone, personaId = 'ferni') {
    try {
        ...
        if (result.success) {
            log.info({ userId: item.userId, phone: maskPhone(phone), callSid: result.callSid, personaId }, '📞 Voice call initiated');
            return {
                success: true,
                channel: 'voice',
 ...
 */
export async function deliverToUser(item, config) {
    ...
    for (const channel of config.enabledChannels) {
        if (channel === config.preferredChannel)
            continue;
 ...
 */
async function tryChannel(item, config, channel) {
    switch (channel) {
        case 'push':
            if (!config.pushToken) {
                return {
...
function generateEmailHtml(item, name, personaId = 'ferni') {
...
function truncateForSms(message, maxLength = 160) {
...
//# sourceMappingURL=notification-delivery.js.map