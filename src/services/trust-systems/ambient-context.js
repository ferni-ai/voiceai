...
function calculateConfidence(features, signals) {
 ...
 */
export function getAmbientResponse(context) {
    const adjustments = {
        keepShort: context.distractionLevel > 0.3,
        avoidSensitiveTopics: context.privacyConcern,
        speakClearly: context.environment === 'noisy',
        offerPause: context.signals.some((s) => ['baby_crying', 'phone_ringing', 'door_knock'].includes(s.type)),
    };
 ...
 */
export function recordAmbientSignal(signalType, confidence) {
    return {
        type: signalType,
        confidence,
        timestamp: new Date(),
 ...
 */
export function buildAmbientContext(context) {
    if (!context || context.environment === 'quiet')
        return '';
    const sections = ['[AMBIENT CONTEXT]'];
