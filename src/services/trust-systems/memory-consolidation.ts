...
// ============================================================================
// PROFILE TYPES (Simplified for consolidation - avoids circular deps)
// ============================================================================
...
interface SimplifiedGrowthPattern {
  id: string;
  type: string;
  before: { pattern: string; firstSeen: Date };
  after: { pattern: string; firstSeen: Date };
  significance: string;
}

interface SimplifiedSharedMoment {
  id: string;
  type: string;
  content: string;
  firstMentioned: Date;
...
interface SimplifiedProfile<T> {
  boundaries?: T[];
  patterns?: SimplifiedGrowthPattern[];
  sharedMoments?: SimplifiedSharedMoment[];
  wins?: SimplifiedSmallWin[];
  avoidedTopics?: Array<{ topic: string; count: number }>;
}
...
const consolidatedProfiles = new Map<string, ConsolidatedProfile>();
 ...
 */
async function consolidateTrustProfilesImpl(
  userId: string,
  profiles: {
    boundaries?: { boundaries?: SimplifiedBoundary[] };
    growth?: { patterns?: SimplifiedGrowthPattern[]; historicalResponses?: unknown[] };
  ...
  try {
    ...
    if (profiles.boundaries?.boundaries) {
      const boundaryResult = consolidateBoundaries(
        profiles.boundaries.boundaries,
        cfg,
        consolidated
      );
    ...
    if (profiles.insideJokes?.sharedMoments) {
      const momentsResult = consolidateSharedMoments(
        profiles.insideJokes.sharedMoments,
        cfg,
        consolidated
      );
    ...
    if (profiles.unsaid?.avoidedTopics) {
      const unsaidResult = consolidateUnsaidSignals(
        profiles.unsaid.avoidedTopics,
        profiles.unsaid.patterns || [],
        cfg,
        consolidated
      );
 ...
 */
function consolidateBoundaries(
  boundaries: SimplifiedBoundary[],
  config: ConsolidationConfig,
  consolidated: ConsolidatedProfile
): { archived: number; merged: number; themes: string[] } {
  let archived = 0;
  let merged = 0;
  const themes: string[] = [];
  ...
  const topicGroups = new Map<string, SimplifiedBoundary[]>();
 ...
 */
function consolidateGrowth(
  patterns: SimplifiedGrowthPattern[],
  config: ConsolidationConfig,
  consolidated: ConsolidatedProfile
): { archived: number; milestones: string[] } {
  let archived = 0;
 ...
 */
function consolidateSharedMoments(
  moments: SimplifiedSharedMoment[],
  config: ConsolidationConfig,
  consolidated: ConsolidatedProfile
): { archived: number; highlights: string[] } {
  let archived = 0;
...
function consolidateSmallWins(
  ...
  for (const [type, groupWins] of winsByType) {
    const oldWins = groupWins.filter(
      (w: SimplifiedSmallWin) => now - w.detectedAt.getTime() > maxAge
    );
 ...
 */
function consolidateUnsaidSignals(
  avoidedTopics: Array<{ topic: string; timesAvoided?: number }>,
  patterns: unknown[],
  config: ConsolidationConfig,
  consolidated: ConsolidatedProfile
): { archived: number; patternsIdentified: number } {
  let archived = 0;
 ...
 */
function createEmptyProfile(): ConsolidatedProfile {
  return {
    archive: [],
    themes: {
      boundaries: [],
 ...
 */
export async function consolidateTrustProfiles(
  userId: string,
 ...
 */
export function getConsolidatedProfile(userId: string): ConsolidatedProfile | null {
 ...
 */
export function getThemesForContext(userId: string): {
  boundaries: string[];
  growth: string[];
  relationship: string[];
  wins: string[];
...
export async function runScheduledConsolidation(
  userIds: string[],
  profileLoader: (userId: string) => Promise<Record<string, unknown>>,
  config?: Partial<ConsolidationConfig>
): Promise<{
  processed: number;
  succeeded: number;
