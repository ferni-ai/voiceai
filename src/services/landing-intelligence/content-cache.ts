...
export async function getCachedMemoryStories(): Promise<CachedMemoryStory[]> {
  try {
    ...
    return snapshot.docs.map(
      (doc: FirebaseFirestore.QueryDocumentSnapshot) => doc.data() as CachedMemoryStory
    );
  } catch (error) {
...
export async function getCachedLateNightScenarios(): Promise<CachedLateNightScenario[]> {
  try {
    ...
    return snapshot.docs.map(
      (doc: FirebaseFirestore.QueryDocumentSnapshot) => doc.data() as CachedLateNightScenario
    );
  } catch (error) {
