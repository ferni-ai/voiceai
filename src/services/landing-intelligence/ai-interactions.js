...
// ============================================================================
import { getGeminiClient, isGeminiConfigured, getDefaultModel, } from '../../config/gemini-config.js';
// ============================================================================
 ...
 */
export async function sendDemoChatMessage(visitorId, message, persona = 'ferni') {
    // Check rate limit
...
async function generateChatResponse(messages, persona) {
 ...
 */
export async function generatePersonaPreview(request) {
 ...
 */
export async function answerSmartFAQ(request) {
    const genAI = await getGenAI();
    if (!genAI) {
...
function getFallbackFAQ(question) {
    const lowerQ = question.toLowerCase();
    if (lowerQ.includes('price') || lowerQ.includes('cost') || lowerQ.includes('free')) {
        return {
            question,
            answer: "Ferni is free forever - really free. If you believe in what we're building, you can chip in as a Founding Member ($10/mo) for unlimited access to all 6 specialists. Founding Patrons ($20/mo) get premium features and Nayan, our sage mentor.",
            relatedQuestions: ["What's included in the free tier?", 'Can I cancel anytime?'],
            confidence: 0.95,
        };
    }
    if (lowerQ.includes('therapy') ||
...
function getDefaultHero(request) {
    // Time-based defaults
 ...
 */
export async function generateSocialProof(count = 3) {
    const genAI = await getGenAI();
    if (!genAI) {
 ...
 */
export async function generateHoverPreview(elementType, context) {
    const genAI = await getGenAI();
    if (!genAI) {
        const fallbacks = {
 ...
 */
export async function generateSentimentReactiveCopy(request) {
...
//# sourceMappingURL=ai-interactions.js.map