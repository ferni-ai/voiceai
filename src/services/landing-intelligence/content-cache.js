...
const FALLBACK_SOCIAL_PROOF = [
    {
        text: 'Last night at 3am, someone asked me about building a morning routine. We talked for 47 minutes.',
        type: 'presence',
    },
    {
        text: "I reminded someone about a breakthrough they had 4 months ago. They'd forgotten. I hadn't.",
        type: 'memory',
    },
    {
        text: "Someone said 'I'm fine' but I heard the pause. We talked about what was really going on.",
        type: 'understanding',
    },
    {
        text: "A user mentioned their sister's birthday is coming up. I remembered they were trying to reconnect.",
        type: 'memory',
    },
    {
        text: "It's 2am somewhere. Right now I'm talking to 47 people who couldn't sleep.",
        type: 'presence',
    },
];
// ═══════════════════════════════════════════════════════════════════════════
 ...
 */
export async function getCachedHero(timeBlock, visitorType) {
...
function createFallbackHero(cacheKey) {
    ...
    return {
        ...
        context: {
            timeBlock: cacheKey.split('-')[0],
            visitorType: cacheKey.split('-')[1],
        },
 ...
 */
export async function getCachedSocialProof(count = 5) {
    try {
        const db = getDb();
        if (!db) {
 ...
 */
export async function getCachedMemoryStories() {
    try {
        const db = getDb();
        if (!db)
            return [];
 ...
 */
export async function getCachedLateNightScenarios() {
    try {
        const db = getDb();
        if (!db)
            return [];
 ...
 */
export async function getCachedUseCaseQuotes(category) {
    try {
        const db = getDb();
        if (!db)
            return [];
 ...
 */
export async function generateAndCacheHeroes() {
 ...
 */
export async function generateAndCacheSocialProof() {
    const db = getDb();
    if (!db)
        return 0;
    try {
        ...
        if (snippets && snippets.length > 0) {
            const cached = {
                ...
                messages: snippets.map((snippet, i) => ({
                    ...
                    type: ['memory', 'presence', 'understanding', 'moment'][i % 4],
                })),
                generatedAt: new Date(),
...
// HELPERS
// ═══════════════════════════════════════════════════════════════════════════
function timeBlockToHour(timeBlock) {
    switch (timeBlock) {
        case 'lateNight':
            return 3;
        case 'earlyMorning':
 ...
 */
export function getCacheControlHeader(contentType) {
