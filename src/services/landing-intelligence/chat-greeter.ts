...
function selectPrebuiltGreeting(context: ChatGreetingContext): string {
...
export async function generateChatGreeting(context: ChatGreetingContext): Promise<string> {
  // For most cases, use prebuilt greetings (faster)
  const shouldUseAI =
    context.intent?.confidence && context.intent.confidence > 0.7 && context.timeOnPage > 30;
...
export function getGreetingTiming(context: ChatGreetingContext): GreetingTiming {
