...
const LAYOUT_PRESETS: Record<string, Partial<LayoutOptimization>> = {
...
function selectPreset(context: LayoutContext): string | null {
...
export async function getOptimalSectionOrder(context: LayoutContext): Promise<LayoutOptimization> {
  // Try preset first
  ...
  if (presetKey) {
    ...
    return {
      order: (preset.order as string[]) || DEFAULT_ORDER,
  ...
  const result = await generateJSON<LayoutOptimization>(prompt, {
    timeout: 4000,
...
export function optimizeForMobile(layout: LayoutOptimization): LayoutOptimization {
  ...
  const mobileOrder = layout.order.filter(
    (section) =>
      prioritySections.includes(section) || layout.emphasis.some((e) => e.section === section)
  );

  // Hide more sections on mobile
  const mobileHide = layout.order.filter(
    (section) => !mobileOrder.includes(section) && !layout.hide.includes(section)
  );
