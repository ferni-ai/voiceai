...
const TIME_CONTENT: Record<TimeMode, TimeAwareContent> = {
  'late-night': {
    mode: 'late-night',
    hero: {
      tagline: "Can't sleep?",
      ...
      subhead:
        "No judgment. No advice unless you want it. Just someone who's here, right now, when you need it most.",
    },
  ...
  evening: {
    ...
    hero: {
      ...
      subhead:
        "No matter what happened today, you don't have to carry it alone. Talk through it, let it go, or just exist together.",
    },
...
export function getTimeAwareContent(hour?: number): TimeAwareContent {
...
interface SpecialOccasion {
  name: string;
  check: () => boolean;
  content: Partial<TimeAwareContent>;
}
...
export function getTimeAwareContentWithOccasions(hour?: number): TimeAwareContent {
...
export function getTimeAwareClasses(content: TimeAwareContent): string[] {
