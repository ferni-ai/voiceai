...
// METRICS COLLECTION
// ============================================================================
export async function collectLandingMetrics(period = 'week') {
    ...
    try {
        ...
        for (const session of sessions) {
            ...
            if (session.endTime && session.startTime) {
                const duration = (session.endTime.toDate?.() || new Date(session.endTime)).getTime() -
            ...
            if (session.timePerSection) {
                for (const [section, time] of Object.entries(session.timePerSection)) {
                    ...
                    sectionTimes[section].push(time);
                }
            }
            // Hour distribution
...
async function getPersonaInsights(persona, metrics) {
...
[
  {
    ...
    const insights = await generateJSON(prompt, { timeout: 10000 });
    if (!insights)
        return [];
...
// ============================================================================
async function generateExperimentSuggestions(metrics, insights) {
...
[
  {
    ...
    const experiments = await generateJSON(prompt, {
        timeout: 15000,
    });
    if (!experiments)
        return [];
...
// REPORT GENERATION
// ============================================================================
export async function generateOptimizationReport(period = 'week') {
    ...
    // Generate summary
    const summary = await generateText(`Summarize this landing page optimization report in 2-3 sentences for the team:
Visitors: ${metrics.visitors}, Conversion: ${metrics.conversionRate.toFixed(1)}%
Top insights: ${allInsights
        .slice(0, 3)
        .map((i) => i.title)
        .join(', ')}
Suggested experiments: ${experiments.length}`, { maxTokens: 150 });
    const report = {
        id: `report_${Date.now()}`,
...
export async function runAutomatedOptimization(config = DEFAULT_CONFIG) {
    const report = await generateOptimizationReport('week');
    const actionsExecuted = [];
    // Handle high-confidence alerts
    const alerts = report.insights.filter((i) => i.type === 'alert' && i.confidence >= config.minConfidenceForAction);
...
// SCHEDULED JOBS
// ============================================================================
export async function dailyOptimizationCheck() {
...
//# sourceMappingURL=optimization-agent.js.map