...
export function updateSessionState(sessionId, update) {
    ...
    if (update.gaveAdvice)
        session.metacognitive.adviceGivenCount++;
    if (update.askedQuestion)
        session.metacognitive.questionsAskedCount++;
    if (update.toldStory)
        session.metacognitive.storiesToldCount++;
    if (update.changedTopic)
        session.metacognitive.topicChangesCount++;
    if (update.providedEmotionalSupport)
        session.metacognitive.emotionalSupportMoments++;
 ...
 */
export function getPhysicalStateComment(sessionId, personaId) {
    const session = getOrCreateSession(sessionId);
    const now = Date.now();
    ...
    if (personaId === 'ferni') {
        ...
        if (!session.physicalState.weatherMentioned && lastType !== 'weather' && Math.random() < 0.25) {
            ...
            comments.push('<break time="200ms"/>Just checked the weather. <break time="150ms"/>Wyoming habit. <break time="200ms"/>Can\'t shake it.');
            comments.push('<break time="200ms"/>Looking outside for a second... <break time="150ms"/>weather\'s changing. <break time="200ms"/>Anyway—');
        }
        ...
        if (!session.physicalState.musicMentioned && lastType !== 'music' && Math.random() < 0.2) {
            ...
            comments.push('<break time="200ms"/>Bon Iver. <break time="200ms"/>Helps me think. <break time="150ms"/>Go on.');
            comments.push('<break time="200ms"/>Let me put some music on... <break time="150ms"/>there. <break time="200ms"/>Better.');
        }
        ...
        if (session.physicalState.coffeeStatus === 'cold' &&
            lastType !== 'coffee' &&
            Math.random() < 0.3) {
            comments.push("Hold on, my coffee's gone cold. <break time=\"300ms\"/>That's what happens when the conversation's good.");
            session.physicalState.lastPhysicalType = 'coffee';
        }
        // Notebook mention (less frequent)
        if (!session.physicalState.notebookMentioned &&
    ...
    if (timeSinceStretch > 25 * 60 * 1000) {
        // 25 minutes
        if (personaId === 'ferni') {
            comments.push('<break time="200ms"/>Sitting too long. My back knows it. <break time="150ms"/>Where were we?');
            comments.push('Need to move. <break time="200ms"/>Okay. Better.');
        }
        if (personaId === 'alex-chen') {
            comments.push('<break time="200ms"/>I should stand up. <break time="150ms"/>But this is important.');
        }
    ...
    if (sessionMinutes > 30 && Math.random() < 0.3) {
        comments.push('We\'ve been at this for a while. <break time="200ms"/>Good conversation, though.');
    }
 ...
 */
export function getMetacognitiveComment(sessionId, personaId) {
    const session = getOrCreateSession(sessionId);
    ...
    if (meta.adviceGivenCount > 5 && meta.questionsAskedCount < 3) {
        if (personaId === 'ferni') {
            comments.push('I notice I\'ve been giving a lot of advice. <break time="250ms"/>Let me ask instead: what do YOU think you should do?');
            comments.push('Wait. <break time="200ms"/>I\'ve been talking too much. <break time="200ms"/>What\'s YOUR take on all this?');
        }
        if (personaId === 'nayan-patel') {
            comments.push('Hmm. <break time="300ms"/>I\'m noticing I keep offering perspectives. <break time="250ms"/>Perhaps the question is more important than any answer I could give.');
        }
    }
    // Too many questions awareness
    if (meta.questionsAskedCount > 6 && meta.adviceGivenCount < 2) {
        if (personaId === 'ferni') {
            comments.push('I\'ve been asking a lot of questions. <break time="200ms"/>Let me actually share something useful.');
        }
    }
    // Story overload
    if (meta.storiesToldCount > 3) {
        comments.push('I\'ve told a few stories today. <break time="200ms"/>Enough about me. <break time="150ms"/>What about you?');
    }
    // Topic jumping awareness
    if (meta.topicChangesCount > 4) {
        comments.push('We\'ve covered a lot of ground. <break time="200ms"/>What feels most important to dig into?');
    }
    // Heavy emotional support
    if (meta.emotionalSupportMoments > 3) {
        if (personaId === 'ferni') {
            comments.push('This has been a heavy conversation. <break time="300ms"/>How are YOU feeling right now? <break time="200ms"/>Honestly.');
        }
    }
    if (comments.length > 0) {
 ...
 */
export function getSelfCorrectionPhrase(personaId) {
    const patterns = SELF_CORRECTION_PATTERNS[personaId] ||
 ...
 */
export function getTemporalAnchor(sessionId, lastConversationDate, personaId) {
    const session = getOrCreateSession(sessionId);
    const now = Date.now();
    ...
    if (lastConversationDate) {
        ...
        if (daysSince === 1) {
            phrases.push('It\'s only been a day, <break time="150ms"/>but I was thinking about what we discussed.');
        }
        else if (daysSince >= 3 && daysSince <= 7) {
            phrases.push(`It's been ${daysSince} days. <break time=\"200ms\"/>How have things evolved since we talked?`);
        }
        else if (daysSince > 7 && daysSince <= 14) {
            phrases.push('It\'s been over a week. <break time="200ms"/>A lot can change. <break time="150ms"/>Fill me in.');
        }
        else if (daysSince > 14) {
            phrases.push('It\'s been a while. <break time="250ms"/>I\'ve been thinking about you. <break time="200ms"/>What\'s new?');
        }
    }
    ...
    if (sessionMinutes > 45) {
        phrases.push('We\'ve been talking for almost an hour. <break time="200ms"/>Time flies with good conversation.');
    }
...
//# sourceMappingURL=embodied-awareness.js.map