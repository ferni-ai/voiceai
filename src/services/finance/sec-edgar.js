...
export async function getCIKByTicker(ticker) {
    ...
    try {
        const response = await secCircuitBreaker.execute(() => rateLimitedFetch(`${SEC_BASE_URL}/submissions/CIK${normalizedTicker}.json`));
        if (!response.ok) {
 ...
 */
export async function getCompanyFilings(tickerOrCik, options = {}) {
    ...
    try {
        ...
        const response = await secCircuitBreaker.execute(() => rateLimitedFetch(`${SEC_BASE_URL}/submissions/CIK${cik.padStart(10, '0')}.json`));
        if (!response.ok) {
 ...
 */
export async function getInsiderTransactions(tickerOrCik, options = {}) {
 ...
 */
export async function getInsiderTradingSummary(tickerOrCik, daysBack = 90) {
 ...
 */
export async function getInstitutionalHolders(tickerOrCik, limit = 10) {
    try {
        ...
        const holdings = filingsResult.data.map((f) => ({
...
export async function searchFilings(query, options = {}) {
    ...
    try {
        ...
        const response = await secCircuitBreaker.execute(() => rateLimitedFetch(`${SEC_EFTS_URL}?${params.toString()}`));
        if (!response.ok) {
            return { success: false, error: `SEC search error: ${response.status}` };
        }
        const data = (await response.json());
        const filings = data.hits.hits.slice(0, limit).map((hit) => ({
 ...
 * "Better than Human" - Institutional-grade research for individuals
 */
export async function generateSECInsight(ticker) {
    try {
        ...
        if (filingsResult.success && filingsResult.data && filingsResult.data.length > 0) {
            ...
            const daysAgo = Math.floor((Date.now() - new Date(recentFiling.filingDate).getTime()) / (1000 * 60 * 60 * 24));
            if (daysAgo <= 7) {
                insights.push(`${ticker} filed an 8-K (${recentFiling.description}) ${daysAgo} days ago - worth checking`);
            }
        }
        ...
        if (insiderResult.success && insiderResult.data) {
            ...
            if (sentiment === 'bullish' && netShares > 0) {
                insights.push(`Insiders at ${ticker} are buying - ${uniqueInsiders} insiders net bought ${netShares.toLocaleString()} shares recently`);
            }
            else if (sentiment === 'bearish' && netShares < 0) {
                insights.push(`Heads up: Insiders at ${ticker} are selling - net ${Math.abs(netShares).toLocaleString()} shares sold`);
            }
        }
