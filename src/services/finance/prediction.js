...
import { getStoredAccessToken, getAccountBalances, getTransactions, analyzeSpending, } from '../../tools/domains/finance/plaid.js';
const log = createLogger({ module: 'FinancePrediction' });
const userFinancialState = new Map();
// ============================================================================
// BILL DETECTION
// ============================================================================
/**
 ...
 */
export async function detectBills(userId) {
    ...
    try {
        ...
        const transactions = await getTransactions(accessToken, startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0], 500);
        ...
        for (const [merchant, txns] of Object.entries(merchantGroups)) {
            ...
            for (let i = 1; i < txns.length; i++) {
                const days = Math.round((txns[i].date.getTime() - txns[i - 1].date.getTime()) / (1000 * 60 * 60 * 24));
                intervals.push(days);
            }
            // Determine frequency
...
function calculateRecurringConfidence(intervals, expected) {
 ...
 */
export async function detectIncome(userId) {
    const accessToken = getStoredAccessToken(userId);
    if (!accessToken)
        return [];
    try {
 ...
 */
export async function predictCashFlow(userId, daysOut = 14) {
    const accessToken = getStoredAccessToken(userId);
    if (!accessToken)
        return null;
    try {
...
export async function detectAnomalies(userId) {
    ...
    try {
        ...
        const avgTxnSize = recentAnalysis.totalSpending / (recentTxns.filter((t) => t.amount > 0).length || 1);
        ...
        state.baselineSpending = Object.fromEntries(Object.entries(historicalAnalysis.byCategory).map(([k, v]) => [k, v.total]));
 ...
 */
export async function detectSubscriptionCreep(userId) {
    const accessToken = getStoredAccessToken(userId);
    if (!accessToken)
        return null;
    try {
        ...
        const merchantGroups = {};
        for (const txn of transactions) {
            if (txn.amount <= 0 || txn.amount > 200)
                continue; // Skip income and large purchases
            const key = (txn.merchant_name || txn.name).toLowerCase();
            if (!merchantGroups[key]) {
                merchantGroups[key] = [];
            }
        ...
        const changePercent = previousMonthly > 0 ? ((totalMonthly - previousMonthly) / previousMonthly) * 100 : 0;
        // Suggest potential savings (streaming services, duplicates)
        const unusedSuggestions = [];
        const streamingServices = subscriptions.filter((s) => /netflix|hulu|disney|hbo|paramount|peacock|spotify|apple music|youtube/i.test(s.name));
        if (streamingServices.length > 3) {
            unusedSuggestions.push(`You have ${streamingServices.length} streaming services - consider rotating them seasonally`);
        }
        ...
        log.debug({ userId, subCount: subscriptions.length, total: totalMonthly }, 'Subscription creep analyzed');
        return result;
    }
    catch (error) {
 ...
 */
export function createSavingsGoal(userId, name, targetAmount, targetDate, currentAmount = 0) {
    const state = getOrCreateState(userId);
    const goal = {
        id: `goal_${Date.now()}`,
        name,
 ...
 */
export function updateGoalProgress(userId, goalId, newAmount) {
...
function calculateGoalProgress(goal) {
    ...
    const monthsRemaining = Math.max(0.1, (goal.targetDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24 * 30));
    ...
    const monthsToComplete = goal.monthlyContribution > 0 ? remaining / goal.monthlyContribution : Infinity;
 ...
 */
export async function generateFinancialInsight(userId) {
    ...
    if (cashFlow && cashFlow.warnings.length > 0) {
        ...
        return {
            ...
            suggestion: warning.type === 'overdraft_risk'
    ...
    if (state?.anomalies && state.anomalies.length > 0) {
        ...
        return {
            ...
            suggestion: anomaly.type === 'category_increase'
...
export function generateSuperhumanMoment(userId) {
    ...
    if (upcomingBills.length > 0) {
        ...
        moments.push(`${bill.name} ($${bill.amount}) hits in ${Math.ceil((bill.dueDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24))} days - just wanted you to know.`);
    }
    // Spending pattern awareness
    if (state.anomalies.length > 0) {
        const anomaly = state.anomalies[0];
        if (anomaly.type === 'category_increase' && anomaly.category) {
            moments.push(`You've spent ${Math.round(anomaly.percentAboveNormal)}% more on ${anomaly.category} this month - stress spending?`);
        }
    }
    // Goal progress
    for (const goal of state.goals) {
        ...
        if (progress.percentComplete >= 90 && progress.percentComplete < 100) {
            moments.push(`You're ${Math.round(progress.percentComplete)}% to your ${goal.name} goal - so close!`);
        }
    }
