...
export class ProfilePersonalizer {
     ...
     */
    enhancePromptWithPersonalization(basePrompt, profile) {
        ...
        if (profile.financialSituation) {
            ...
            if (profile.financialSituation.hasDebt) {
                enhanced += '\n⚠️ Has debt';
                if (profile.financialSituation.debtTypes &&
     ...
     */
    tailorAdviceToFinancialSituation(genericAdvice, profile) {
        ...
        if (financialSituation.hasEmergencyFund) {
            tailored = tailored.replace(/build an emergency fund/gi, 'keep maintaining that emergency fund you have');
            tailored = tailored.replace(/start saving for emergencies/gi, 'continue building on that emergency fund');
        }
        else {
        ...
        if (financialSituation.investmentAccounts.length > 0) {
            ...
            if (accountTypes.length > 0) {
                ...
                tailored = tailored.replace(/open (a|an) (retirement |investment )?account/gi, `maximize your ${accountTypes[0]}`);
     ...
     */
    applyPreferenceFilters(response, profile) {
     ...
     */
    getPersonalizationContext(profile) {
        return {
     ...
     */
    hasMinimalPersonalizationData(profile) {
     ...
     */
    buildPersonalizedGreeting(profile, isReturning) {
        ...
        if (profile.lastConversationSummary) {
            const daysSince = Math.floor((Date.now() - new Date(profile.lastContact).getTime()) / (1000 * 60 * 60 * 24));
            if (daysSince < 1) {
 ...
 */
export function getPersonalizer() {
    if (!personalizer) {
 ...
 * Reset for testing
 */
export function resetPersonalizer() {
    personalizer = null;
}
