...
const ERROR_HUMANIZATIONS: Array<{
  pattern: RegExp;
  humanize: (match: RegExpMatchArray, error: Error) => HumanizedError;
}> = [
  // Connection issues
  {
    ...
    humanize: () => ({
      userMessage:
        "I had a brief moment where I couldn't connect - like when your phone drops a bar for a second. I'm here now though!",
  ...
  },
  {
    ...
    humanize: () => ({
      userMessage:
        "Things got a bit slow on my end for a moment - like when you're waiting for a page to load. I'm back up to speed now!",
  ...
  // Session issues
  {
    ...
    humanize: () => ({
      userMessage:
        "Looks like we got disconnected for a moment. These things happen sometimes with voice calls. I'm back though!",
