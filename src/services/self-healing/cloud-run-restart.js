 ...
 */
async function getServiceInfo(serviceName, region, projectId) {
 ...
 */
async function triggerNewRevision(serviceName, region, projectId, reason) {
    const token = await getAccessToken();
    // Get current service
 ...
 */
async function waitForRevisionReady(serviceName, revisionName, region, projectId, timeoutMs = 120000) {
    const token = await getAccessToken();
...
export async function restartService(options) {
    const { serviceName, region = 'us-central1', projectId = process.env.GOOGLE_CLOUD_PROJECT || process.env.GCP_PROJECT || '', reason, notify = true, force = false, } = options;
    const startTime = Date.now();
    // Check cooldown
 ...
 */
export function canRestart(serviceName) {
    const lastRestart = restartCooldowns.get(serviceName);
 ...
 */
export function getCooldownRemaining(serviceName) {
    const lastRestart = restartCooldowns.get(serviceName);
    if (!lastRestart)
        return 0;
 ...
 */
export function clearCooldown(serviceName) {
 ...
 */
export async function handleCriticalFailure(serviceName, error, context = {}) {
 ...
 */
export function setupAutoRestart(serviceName) {
    // Only in Cloud Run
    if (!process.env.K_SERVICE) {
...
//# sourceMappingURL=cloud-run-restart.js.map