...
import { getCachedHealthStatus, isCapabilityHealthy, type HealthStatus } from './health-monitors.js';
...
const RECOVERY_PHRASES = {
  ...
  calendar: [
    "Your calendar is accessible again.",
    "I can see your calendar now - all sorted!",
  ],
  smartHome: [
...
function categorizeService(serviceName: string): keyof typeof DEGRADATION_PHRASES {
  ...
  if (lower.includes('home') || lower.includes('hue') || lower.includes('lifx') || lower.includes('smart')) return 'smartHome';
  if (lower.includes('firestore') || lower.includes('memory') || lower.includes('context')) return 'memory';
...
export function getHealthContext(): HealthContext {
  ...
  state.recentRecoveries = state.recentRecoveries.filter(
    r => now - r.time < 300000
  );
...
export function handleConversationError(error: Error): string {
  ...
  log.warn(
    { error: error.message, severity: humanized.severity },
 ...
 */
export function checkCapability(capability: 'music' | 'weather' | 'calendar' | 'smartHome' | 'memory'): {
  available: boolean;
  message?: string;
} {
  const context = getHealthContext();
...
export function onConversationStart(): { 
  ...
  if (context.overallHealth === 'critical') {
    openingHealthMessage = "Just a heads up - I'm experiencing some technical difficulties, but I'm still here for you.";
