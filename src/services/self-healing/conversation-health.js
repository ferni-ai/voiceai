...
import { getCachedHealthStatus, isCapabilityHealthy, } from './health-monitors.js';
...
const CONTEXT_INJECTIONS = {
    music: 'Note: Spotify/music service is currently unavailable. If user asks about music, acknowledge this warmly and suggest trying again later.',
    weather: 'Note: Weather service is temporarily unavailable. Acknowledge this if user asks about weather.',
    calendar: 'Note: Calendar access is having issues. If user asks about schedule, acknowledge the temporary issue.',
    smartHome: 'Note: Smart home integration is slow/unavailable. Acknowledge this if user tries to control devices.',
    memory: 'Note: Some memory functions are temporarily limited. Be especially present and attentive.',
    ai: 'Note: AI processing is running slower than usual. Responses may take a moment longer.',
    voice: 'Note: Voice connection quality may be affected. Speak clearly and be patient with any audio issues.',
};
const state = {
    lastCheck: 0,
    lastMention: 0,
...
function categorizeService(serviceName) {
    ...
    if (lower.includes('weather') || lower.includes('open-meteo'))
        return 'weather';
    if (lower.includes('calendar') || lower.includes('google-calendar'))
        return 'calendar';
 ...
 */
export function configureHealthAwareness(newConfig) {
...
export function getHealthContext() {
    ...
    // Check capabilities
    if (!isCapabilityHealthy('audio'))
        affectedCapabilities.push('voice quality');
    if (!isCapabilityHealthy('tools'))
        affectedCapabilities.push('external services');
    if (!isCapabilityHealthy('memory'))
        affectedCapabilities.push('memory');
    ...
    if (degradedServices.size > 0) {
        ...
        for (const cat of categories) {
            const injection = CONTEXT_INJECTIONS[cat];
            if (injection)
    ...
    const shouldMentionHealth = config.proactiveCommunication &&
 ...
 */
export function getProactiveHealthMessage() {
    ...
    for (const cat of categories) {
        if (cat !== 'general') {
            const phrases = DEGRADATION_PHRASES[cat];
            if (phrases) {
 ...
 */
export function handleConversationError(error) {
...
export function checkCapability(capability) {
    ...
    const affected = context.degradedServices.some((svc) => relevantServices.some((rel) => svc.toLowerCase().includes(rel)));
    if (affected) {
 ...
 */
export function withHealthAwareness(capability, operation) {
    return (async () => {
 ...
 */
export function onConversationPause() {
...
//# sourceMappingURL=conversation-health.js.map