 ...
 */
export async function communicateRecovery(session, error, context, sessionId) {
    try {
        ...
        log.info({ phase: context.phase, errorType: context.errorType, autoRecovered: context.autoRecovered }, 'Communicated recovery to user');
        return true;
    }
    catch (sayError) {
 ...
 */
export function createRecoveryAwareSession(session, options = {}) {
    return {
        ...session,
         ...
         */
        async notifyRecovery(error, context = {}) {
            const fullContext = {
 ...
 */
export function getRecoveryPhrase(category) {
