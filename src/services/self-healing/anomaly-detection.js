...
// ============================================================================
function getOrCreateWindow(metricName) {
...
function updateWindowStats(window) {
...
// ANOMALY DETECTION
// ============================================================================
function isInCooldown(window) {
    if (!window.lastAnomalyTime)
        return false;
    return Date.now() - window.lastAnomalyTime < config.anomalyCooldownMs;
}
function createAnomaly(window, value, zScore, type) {
    const isHigherBetter = window.name.includes('success_rate');
    const isSpike = type === 'spike' || (type !== 'drop' && zScore > 0);
    // Determine severity based on z-score
    const severity = Math.abs(zScore) > 4 ? 'critical' : 'warning';
 ...
 */
export function recordAndDetect(metricName, value, options) {
 ...
 */
export function configureAnomalyDetection(newConfig) {
 ...
 */
export function getAllMetricStats() {
...
export function getAnomalyHistory(windowMinutes = 60) {
 ...
 */
export function getMetricAnomalies(metricName, limit = 50) {
    return recentAnomalies.filter((a) => a.metricName === metricName).slice(-limit);
}
/**
 * Clear all state (for testing)
 */
export function resetAnomalyDetection() {
    metricWindows.clear();
 ...
 */
export function createPredictiveMonitor(serviceName) {
    let recentRequests = 0;
    let recentFailures = 0;
    let lastReset = Date.now();
    const WINDOW_MS = 60000; // 1 minute window
    return {
        onRequest: (durationMs, success) => {
 ...
 */
export function createAlertingAnomalyHandler() {
    return (anomaly) => {
        if (anomaly.severity === 'critical') {
            // Use circuit alerting to send notifications
            handleCircuitStateChange(`anomaly:${anomaly.metricName}`, 'closed', 'open', {
                failures: 1,
