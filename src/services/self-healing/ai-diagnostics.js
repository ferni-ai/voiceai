 ...
 */
export async function analyzeFailure(errorLogs, context) {
    ...
    for (const { pattern, diagnosis } of KNOWN_PATTERNS) {
        if (pattern.test(combinedLogs)) {
            log.debug({ pattern: pattern.toString(), stage: context.stage }, 'Matched known failure pattern');
            return {
                ...diagnosis,
                metadata: {
 ...
 */
async function analyzeWithGemini(errorLogs, context) {
 ...
 */
export function quickDiagnose(errorMessage) {
    for (const { pattern, diagnosis } of KNOWN_PATTERNS) {
