...
// STORAGE
// ============================================================================
const userSeasonalProfiles = new Map();
// ============================================================================
// SEASON DEFINITIONS
// ============================================================================
...
function getSeasonDateRange(period, referenceDate) {
 ...
 */
export async function predictSeasonalMood(userId) {
    const now = new Date();
    ...
    const supportStrategies = generateSupportStrategies(period, historicalPattern, upcomingSignificantDate);
    // Generate message
    const { message, suggestion } = generateSeasonalMessage(period, predictedMood, severity, historicalPattern, upcomingSignificantDate);
    // Calculate confidence
    const confidence = calculateConfidence(profile.moodHistory.length, historicalPattern?.avgMoodScore !== undefined);
    // Should surface if moderate+ severity with decent confidence
    const shouldSurface = (severity !== 'mild' && confidence >= 0.5) || upcomingSignificantDate !== null;
    return {
        userId,
        period,
        periodStart,
        ...
        try {
            ...
            if (snapshots && snapshots.length > 0) {
                for (const snapshot of snapshots) {
                    ...
                    const overallScore = (((dims.mood || 0.5) +
                        (dims.energy || 0.5) +
...
async function analyzeHistoricalPattern(profile, period, now) {
    // Check cache
...
function findUpcomingSignificantDate(profile, now) {
...
function generateStrategiesFromThemes(themes) {
    const strategies = [];
...
function generateSupportStrategies(period, historicalPattern, significantDate) {
    const strategies = [];
...
function generateSeasonalMessage(period, predictedMood, severity, historicalPattern, significantDate) {
    let message = '';
    let suggestion = '';
    const periodNames = {
...
function calculateConfidence(historyLength, hasPattern) {
    let confidence = 0.3;
    if (historyLength >= 365)
        confidence += 0.3;
    else if (historyLength >= 180)
        confidence += 0.2;
 ...
 */
export function recordMoodEntry(userId, score, themes = [], notes) {
    let profile = userSeasonalProfiles.get(userId);
    if (!profile) {
        profile = {
            userId,
 ...
 */
export function addSignificantDate(userId, date, // MM-DD format
type, description, associatedMood = 50) {
    let profile = userSeasonalProfiles.get(userId);
    if (!profile) {
        profile = {
            userId,
 ...
 */
export function clearSeasonalData(userId) {
...
//# sourceMappingURL=seasonal-mood.js.map