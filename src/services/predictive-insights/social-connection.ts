...
interface TrackedPerson {
  ...
  avgDaysBetweenMentions?: number;
  importance: number; // 0-1, based on mention frequency and sentiment
}
...
const userSocialGraphs = new Map<string, Map<string, TrackedPerson>>();
...
export async function checkSocialConnections(
  ...
  for (const [personId, person] of socialGraph) {
    ...
    const daysSinceLastMention = Math.floor(
      (now - lastMention.timestamp.getTime()) / (24 * 60 * 60 * 1000)
    );
...
function createAlert(
  ...
  usualFrequency: number,
  gapRatio: number
): SocialConnectionAlert {
  // Determine severity
  let severity: ConnectionSeverity = 'minor';
  ...
  // Generate message
  const { message, suggestion } = generateSocialMessage(
    person.name,
...
function calculateUsualFrequency(person: TrackedPerson): number {
...
function generateSocialMessage(
  ...
  const frequencyDesc =
    usualFrequency <= 7 ? 'weekly' : usualFrequency <= 14 ? 'every week or two' : 'regularly';
 ...
 */
export function getTrackedPeople(userId: string): Array<{
  name: string;
  relationshipType: RelationshipType;
  mentionCount: number;
  importance: number;
 ...
 */
export function clearSocialData(userId: string): void {
