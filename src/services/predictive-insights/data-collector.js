 ...
 */
export async function processConversationSignals(signals) {
...
// SIGNAL PROCESSORS
// ============================================================================
function processEnergySignals(userId, signals) {
    ...
    if (signals.mood === 'positive')
        factors.push('positive_mood');
    if (signals.mood === 'negative')
        factors.push('negative_mood');
    if (signals.sessionDuration && signals.sessionDuration > 30)
        factors.push('long_session');
    if (signals.userInitiated)
        factors.push('user_initiated');
    recordEnergyObservation(userId, energyLevel, factors);
}
function processMoodSignals(userId, signals) {
    const score = signals.moodScore ?? (signals.mood === 'positive' ? 75 : signals.mood === 'negative' ? 35 : 50);
    const themes = signals.themes || signals.topics || [];
    recordMoodEntry(userId, score, themes);
}
function processPeopleSignals(userId, people) {
    for (const person of people) {
        // Map acquaintance and close_friend to friend for relationship health tracking
        const rawType = person.relationshipType;
        const relationshipType = rawType === 'acquaintance' || rawType === 'close_friend' ? 'friend' : rawType;
        // Record for relationship health tracking
        recordRelationshipMention(userId, person.name, relationshipType, person.sentiment, person.pronouns || { we: 0, i: 0, they: 0 }, [], // topics
        person.sentiment > 0 ? 'positive' : person.sentiment < 0 ? 'negative' : 'neutral');
        // Also record for social connection tracking
        recordPersonMention(userId, person.name, relationshipType, person.context || '', person.sentiment);
    }
}
function processDecisionSignals(userId, decisions) {
    for (const decision of decisions) {
        if (decision.resolved) {
            ...
            // Still mulling over
            recordDecisionMention(userId, decision.topic, decision.category, decision.sentiment, [], // themes
            decision.options || [], decision.concerns || []);
        }
    }
}
function processGoalSignals(userId, goals) {
...
function processHabitSignals(userId, habits) {
    for (const habit of habits) {
        recordHabitCompletion(userId, habit.habitId, habit.completed, habit.duration);
    }
}
function processSignificantDates(userId, dates) {
    for (const date of dates) {
        // Estimate mood impact
        const associatedMood = date.emotionalWeight !== undefined
 ...
 */
export function extractSignalsFromTranscript(userId, sessionId, transcript, metadata) {
    const signals = {
        userId,
        sessionId,
        timestamp: new Date(),
...
function extractMood(transcript) {
...
function extractPeopleMentioned(transcript) {
    const people = [];
    ...
    for (const { pattern, type } of relationshipPatterns) {
        ...
        if (matches) {
            for (const match of matches) {
                ...
                // Simple sentiment
                const sentiment = extractMood(context) === 'positive'
                    ? 0.5
...
function getContextAround(text, target, chars) {
 ...
 */
export async function onSessionEnd(userId, sessionId, transcript, metadata) {
 ...
 */
export function onConversationEvent(userId, event) {
    try {
        switch (event.type) {
            case 'mood_detected': {
                const moodScore = event.data.score;
                const themes = event.data.themes || [];
