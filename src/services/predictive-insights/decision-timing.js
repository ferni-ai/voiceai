...
// STORAGE
// ============================================================================
const userDecisionProfiles = new Map();
// ============================================================================
 ...
 */
export async function assessDecisionReadiness(userId) {
...
function assessSingleDecision(userId, decision, profile) {
    ...
    const incubationDays = Math.floor((Date.now() - decision.firstMentioned.getTime()) / (24 * 60 * 60 * 1000));
    ...
    const { isReady, readinessScore } = calculateReadiness(incubationDays, mentionCount, sentimentStability, historicalPattern, mentions);
    // Generate message
    const { message, suggestion } = generateDecisionMessage(topic, isReady, incubationDays, mentionCount, historicalPattern);
    // Calculate confidence
    const confidence = calculateConfidence(mentions.length, profile.historicalDecisions.length, sentimentStability);
    // Should surface if decision seems ready or has been pending too long
    const shouldSurface = (isReady && confidence >= 0.5) || (incubationDays > 30 && mentionCount >= 5);
    return {
        userId,
        decisionId: id,
        topic,
...
// ANALYSIS FUNCTIONS
// ============================================================================
function calculateSentimentStability(mentions) {
...
function getHistoricalPattern(profile) {
    ...
    if (successful.length > 0) {
        ...
        const avgSuccessIncubation = successful.reduce((sum, d) => {
            const resolved = d.resolvedAt || d.mentions[d.mentions.length - 1]?.timestamp;
            if (!resolved)
                return sum;
        ...
        const avgSuccessMentions = successful.reduce((sum, d) => sum + d.mentions.length, 0) / successful.length;
    ...
    return {
        ...
        optimalMentionCount: Math.round(mentionCounts.reduce((a, b) => a + b, 0) / mentionCounts.length),
        bestOutcomeConditions: bestConditions,
    };
}
function calculateReadiness(incubationDays, mentionCount, sentimentStability, historicalPattern, mentions) {
    let score = 0;
    ...
    if (historicalPattern) {
        ...
        if (incubationDays >= 7)
            score += 30;
        else if (incubationDays >= 3)
            score += 20;
        else if (incubationDays >= 1)
            score += 10;
    }
...
function generateDecisionMessage(topic, isReady, incubationDays, mentionCount, pattern) {
    let message = '';
    let suggestion = '';
    if (isReady) {
...
function calculateConfidence(mentionCount, historicalCount, stability) {
    let confidence = 0.3;
    if (mentionCount >= 5)
        confidence += 0.2;
 ...
 */
export function recordDecisionMention(userId, topic, category, sentiment, themes = [], consideredOptions = [], expressedConcerns = []) {
 ...
 */
export function resolveDecision(userId, topic, outcome) {
    const profile = userDecisionProfiles.get(userId);
    if (!profile)
        return;
    const decisionId = topic.toLowerCase().replace(/\s+/g, '_').slice(0, 50);
 ...
 */
export function getActiveDecisions(userId) {
    const profile = userDecisionProfiles.get(userId);
    if (!profile)
        return [];
 ...
 */
export function clearDecisionData(userId) {
...
//# sourceMappingURL=decision-timing.js.map