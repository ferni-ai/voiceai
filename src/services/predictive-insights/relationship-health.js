 ...
 */
export async function assessRelationshipHealth(userId) {
...
function assessSingleRelationship(userId, relationship) {
    ...
    // Calculate severity
    const { severity, shouldSurface } = determineSeverity(sentimentTrend, languageShift, daysSincePositiveMention, relationship.relationshipType);
    // Skip if not worth surfacing
    if (!shouldSurface) {
        return null;
    }
    // Generate message
    const { message, suggestion } = generateRelationshipMessage(personName, sentimentTrend, languageShift, daysSincePositiveMention, severity);
...
// ANALYSIS FUNCTIONS
// ============================================================================
function avgSentiment(mentions) {
    if (mentions.length === 0)
        return 0;
    return mentions.reduce((sum, m) => sum + m.sentiment, 0) / mentions.length;
}
function calculateSentimentTrend(recent, older) {
...
function detectLanguageShift(recent, older, relationship) {
    if (older.length < 5)
        return undefined;
...
function calculateWeRatio(mentions) {
    let totalWe = 0;
    let totalI = 0;
...
function calculateDaysSincePositive(mentions) {
    const now = Date.now();
...
function determineSeverity(trend, languageShift, daysSincePositive, relationshipType) {
    let score = 0;
    // Sentiment trend
    if (trend === 'declining')
        score += 2;
    if (trend === 'volatile')
        score += 1;
    // Language shift
    if (languageShift && languageShift.significance > 0.4)
        score += 2;
    else if (languageShift)
        score += 1;
    // Days since positive
    if (daysSincePositive > 30)
        score += 2;
    else if (daysSincePositive > 14)
        score += 1;
    // Weight by relationship type
    const typeMultiplier = relationshipType === 'partner' ? 1.5 : relationshipType === 'family' ? 1.2 : 1.0;
...
function generateRelationshipMessage(personName, trend, languageShift, daysSincePositive, severity) {
    let message = '';
    let suggestion = '';
...
function calculateConfidence(mentionCount, hasSignal) {
    let confidence = 0.3;
    if (mentionCount >= 20)
        confidence += 0.3;
 ...
 */
export function recordRelationshipMention(userId, personName, relationshipType, sentiment, pronouns, topics = [], emotionalTone = 'neutral') {
 ...
 */
export function getTrackedRelationships(userId) {
