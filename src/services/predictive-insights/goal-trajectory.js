 ...
 */
export async function projectGoalTrajectory(userId) {
...
function projectSingleGoal(userId, goal) {
    const now = new Date();
    ...
    // Calculate days off
    const daysOff = Math.round((goal.deadline.getTime() - projectedCompletion.getTime()) / (24 * 60 * 60 * 1000));
    ...
    const courseCorrection = !onTrack
        ...
        : undefined;
    // Generate message
    const { message, suggestion } = generateGoalMessage(goal, currentProgress, projectedCompletion, daysOff, onTrack, courseCorrection);
    ...
    const shouldSurface = (!onTrack && Math.abs(daysOff) > 7) ||
...
function projectCompletionDate(goal, progressRate, now) {
    if (progressRate <= 0) {
...
function generateCourseCorrection(goal, daysOff, currentRate) {
...
function generateGoalMessage(goal, currentProgress, projectedCompletion, daysOff, onTrack, courseCorrection) {
    let message = '';
    let suggestion = '';
...
function calculateConfidence(historyLength, progressRate) {
    let confidence = 0.3;
    if (historyLength >= 14)
        confidence += 0.3;
...
// DATA LOADING
// ============================================================================
async function loadGoalsFromSources(userId) {
    try {
    ...
    try {
        ...
        const goalsMap = userGoals.get(userId) || new Map();
 ...
 */
export function recordGoalProgress(userId, goalId, currentValue) {
    const goals = userGoals.get(userId);
    if (!goals)
        return;
 ...
 */
export function addGoalToTrack(userId, goal) {
 ...
 */
export function clearGoalData(userId) {
...
//# sourceMappingURL=goal-trajectory.js.map