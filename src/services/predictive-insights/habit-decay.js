 ...
 */
export async function detectHabitDecay(userId) {
...
function analyzeHabitDecay(userId, habit) {
    const now = Date.now();
    // Need enough history
    ...
    const baselineCutoff = new Date(now - (ANALYSIS_WINDOWS.recent + ANALYSIS_WINDOWS.baseline) * 24 * 60 * 60 * 1000);
    const recentCompletions = habit.completions.filter((c) => c.date >= recentCutoff && c.completed);
    const baselineCompletions = habit.completions.filter((c) => c.date >= baselineCutoff && c.date < recentCutoff && c.completed);
    ...
    const decayRate = baselineFrequency > 0
    ...
    const { message, suggestion, interventions } = generateDecayInsight(habit, recentFrequency, baselineFrequency, decayRate);
...
// ANALYSIS FUNCTIONS
// ============================================================================
function projectAbandonment(currentFrequency, baselineFrequency, decayRate) {
...
function generateDecayInsight(habit, currentFreq, baselineFreq, decayRate) {
    const freqDescription = (freq) => {
        if (freq >= 6.5)
            return 'daily';
        if (freq >= 4.5)
            return 'about 5x/week';
        if (freq >= 3.5)
            return 'about 4x/week';
        if (freq >= 2.5)
            return '3x/week';
        if (freq >= 1.5)
            return 'twice a week';
...
function generateInterventions(habit, decayRate) {
    const interventions = [];
...
function calculateConfidence(completionCount, decayRate, baselineFrequency) {
    let confidence = 0.3;
    // More data = more confidence
    if (completionCount >= 90)
        confidence += 0.25;
    else if (completionCount >= 45)
        confidence += 0.15;
    else if (completionCount >= 21)
        confidence += 0.1;
    // Higher baseline = more reliable pattern
    if (baselineFrequency >= 5)
        confidence += 0.15;
    else if (baselineFrequency >= 3)
        confidence += 0.1;
...
// DATA LOADING
// ============================================================================
async function loadHabitsFromSources(userId) {
    try {
    ...
    try {
        ...
        if (streaks && streaks.length > 0) {
            const habitsMap = userHabits.get(userId) || new Map();
...
function mapRitualCategory(category) {
    if (!category)
        return 'other';
 ...
 */
export function recordHabitCompletion(userId, habitId, completed, duration, notes) {
 ...
 */
export function addHabitToTrack(userId, habitId, name, category, targetFrequency = 7) {
    let habits = userHabits.get(userId);
    if (!habits) {
        habits = new Map();
        userHabits.set(userId, habits);
 ...
 */
export function getTrackedHabits(userId) {
    const habits = userHabits.get(userId);
    if (!habits)
        return [];
...
//# sourceMappingURL=habit-decay.js.map