 ...
 */
export async function predictEnergy(userId) {
    const now = new Date();
    const factors = [];
    try {
        ...
        const shouldSurface = confidence >= 0.5 &&
...
// HELPER FUNCTIONS
// ============================================================================
function calculateTimeBasedScore(date) {
    const hour = date.getHours();
    return BASE_HOURLY_ENERGY[hour] || 2.5;
}
function getTimeExplanation(hour) {
    if (hour >= 9 && hour <= 11)
        return 'Mid-morning is typically a peak focus time';
    if (hour >= 14 && hour <= 16)
        return 'Early afternoon often brings renewed energy';
    if (hour >= 13 && hour <= 14)
        return 'Post-lunch energy dip is natural';
    if (hour >= 21 || hour <= 5)
        return 'Late night/early morning - energy naturally low';
    if (hour >= 6 && hour <= 8)
        return 'Morning ramp-up period';
    return 'Standard energy period';
}
function getDayExplanation(day) {
    const explanations = {
...
async function getCalendarDensityFactor(userId, date) {
    try {
...
function getPersonalPattern(history, now) {
...
async function getRecentStressFactor(userId) {
    try {
        ...
        const stressLevel = (((profile.current.dimensions.mood || 0.5) + (profile.current.dimensions.energy || 0.5)) /
            2) *
            100;
...
function findOptimalWindow(now, history, currentScore) {
    ...
    for (let hoursAhead = 0; hoursAhead < 24; hoursAhead++) {
        ...
        if (historical.length >= 3) {
            const avgHistorical = historical.reduce((sum, e) => sum + ENERGY_SCORES[e.level], 0) / historical.length;
    ...
    for (let i = 0; i < candidates.length - 2; i++) {
        const windowScore = (candidates[i].score + candidates[i + 1].score + candidates[i + 2].score) / 3;
...
function calculateConfidence(historyLength, factors) {
...
function generateInsight(level, windowStart, windowEnd, factors) {
 ...
 */
export function recordEnergyObservation(userId, level, factors = []) {
    const now = new Date();
    const entry = {
        timestamp: now,
        level,
 ...
 */
export function clearEnergyHistory(userId) {
...
//# sourceMappingURL=energy-prediction.js.map