 ...
 */
export function analyzeVoiceBiomarkers(input) {
    const { pitchVariability = 0.5, speechRate = 150, pauseFrequency = 0.1, strain = 0, nasalResonance = 0.3, breathiness = 0.2, tremor = 0, } = input;
    // Fatigue detection
    ...
    const fatigueLevel = Math.min((fatigueFromPitch + fatigueFromSpeed + fatigueFromPauses + tremor * 0.3) / 1.5, 1);
    // Hydration estimate
    ...
    const illnessRisk = Math.min(nasalResonance * 0.4 + strain * 0.3 + fatigueLevel * 0.2 + tremor * 0.1, 1);
    // Medication change indicator
    // Significant deviation from baseline patterns suggests change
    const medicationChangeIndicator = Math.abs(pitchVariability - 0.5) > 0.3 || Math.abs(speechRate - 150) > 40;
 ...
 */
export function calculateStressTrajectory(readings) {
 ...
 */
export async function storeBiomarkerReading(userId, sessionId, biomarkers, input) {
    ...
    try {
        ...
        void onVoiceBiomarkerChange(userId, docRef.id, {
 ...
 */
export async function loadBiomarkerReadings(userId, daysBack = 30) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
 ...
 */
export async function getBiomarkerTrends(userId) {
 ...
 */
export async function buildVoiceBiomarkersContext(userId, currentBiomarkers) {
    ...
    if (currentBiomarkers && currentBiomarkers.confidence > 0.5) {
        ...
        if (fatigueLevel > 0.6) {
            sections.push(`[VOICE BIOMARKER] User's voice shows signs of fatigue (${Math.round(fatigueLevel * 100)}% confidence). ` +
                `Consider: shorter responses, more warmth, avoid complex topics.`);
        }
        if (hydrationEstimate < 0.4) {
            sections.push(`[VOICE BIOMARKER] Voice patterns suggest dehydration. ` +
                `If appropriate, gently encourage water/self-care.`);
        }
        if (illnessRisk > 0.5) {
            sections.push(`[VOICE BIOMARKER] Voice patterns suggest possible illness (${Math.round(illnessRisk * 100)}% risk). ` +
    ...
    if (readings.length >= 3) {
        ...
        if (trajectory === 'rising') {
            sections.push(`[VOICE TREND] Stress/fatigue has been rising over recent conversations. ` +
                `This person may be heading toward burnout. Proactively check in.`);
        }
        else if (trajectory === 'falling') {
            sections.push(`[VOICE TREND] Stress/fatigue has been decreasing. They're recovering. ` +
    ...
    for (const trend of trends) {
        if (trend.direction !== 'stable' && Math.abs(trend.percentChange) > 15) {
            if (trend.metric === 'fatigue' && trend.direction === 'declining') {
                sections.push(`[WELLNESS ALERT] Fatigue levels up ${Math.abs(Math.round(trend.percentChange))}% vs baseline. ` +
...
function getEmotionFromBiomarkers(biomarkers) {
    if (biomarkers.fatigueLevel > 0.7)
        return 'exhausted';
    if (biomarkers.fatigueLevel > 0.5)
        return 'tired';
    if (biomarkers.stressTrajectory === 'rising')
        return 'stressed';
    if (biomarkers.illnessRisk > 0.5)
        return 'unwell';
    if (biomarkers.stressTrajectory === 'falling')
        return 'relaxing';
    return 'neutral';
}
/** Convert pitch variability to category */
function getPitchCategory(variability) {
    if (variability < 0.3)
        return 'low';
    if (variability < 0.6)
        return 'normal';
...
function getPaceCategory(speechRate) {
    if (speechRate < 120)
        return 'slow';
    if (speechRate < 160)
        return 'normal';
...
function getEnergyCategory(fatigueLevel) {
    if (fatigueLevel > 0.6)
        return 'low';
    if (fatigueLevel > 0.3)
        return 'moderate';
...
function generateBiomarkerInsights(biomarkers) {
    const insights = [];
    if (biomarkers.fatigueLevel > 0.7) {
...
//# sourceMappingURL=voice-biomarkers.js.map