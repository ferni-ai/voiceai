 ...
 */
export function detectContradiction(message, recentEmotions, topic) {
 ...
 */
function calculateConfidence(markers, emotion1InMessage, emotion2InMessage) {
 ...
 */
function inferTopicFromMessage(message, examples) {
    const lowerMessage = message.toLowerCase();
 ...
 */
export async function recordContradiction(userId, detection, validationUsed, userResponse) {
    ...
    try {
        ...
        const existingPattern = profile.frequentPatterns.find((p) => p.emotions.sort().join('-') === patternKey);
        if (existingPattern) {
        ...
        void onEmotionalPatternChange(userId, `contradiction_${Date.now()}`, {
 ...
 */
export async function loadContradictionProfile(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
 ...
 */
export function buildContradictionContext(detection) {
 ...
 */
export async function buildContradictionAwarenessContext(userId) {
    const profile = await loadContradictionProfile(userId);
    if (!profile || profile.history.length === 0)
        return '';
    const sections = ['[CONTRADICTION AWARENESS]'];
 ...
 */
export function getValidationPhrase(emotion1, emotion2) {
    for (const pattern of CONTRADICTION_PATTERNS) {
        const [e1, e2] = pattern.emotions;
 ...
 */
export function areCommonlyCoexisting(emotion1, emotion2) {
    for (const pattern of CONTRADICTION_PATTERNS) {
        const [e1, e2] = pattern.emotions;
        if ((emotion1 === e1 && emotion2 === e2) || (emotion1 === e2 && emotion2 === e1)) {
            return true;
...
//# sourceMappingURL=contradiction-comfort.js.map