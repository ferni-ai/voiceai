 ...
 */
export async function findOptimalMilestoneWindows(
  userId: string,
  options: {
    daysAhead?: number;
  ...
  try {
    ...
    for (let i = 0; i < daysAhead; i++) {
      ...
      // Check for afternoon block
        ...
        !hasConflict(dayOverview, ACCEPTABLE_FOCUS_HOURS.start, ACCEPTABLE_FOCUS_HOURS.end)
      ) {
        windows.push({
          date,
...
export async function suggestTimeBlocks(
  ...
  try {
    ...
    const purposes: Array<'planning' | 'execution' | 'review'> = [
      'planning',
      'execution',
      'review',
    ];
    ...
    for (const window of windows) {
      if (accumulatedHours >= result.totalHoursNeeded) break;
    ...
    if (ratio >= 1.2) result.feasibility = 'easy';
    else if (ratio >= 1.0) result.feasibility = 'moderate';
    else if (ratio >= 0.7) result.feasibility = 'challenging';
    else result.feasibility = 'unlikely';
 ...
 */
export async function detectMilestoneConflicts(
  userId: string,
  milestones: SimpleMilestone[]
): Promise<MilestoneConflict[]> {
 ...
 */
export async function getCapacityForNewMilestone(
  userId: string,
  activeMilestones: SimpleMilestone[]
): Promise<CapacityAssessment> {
  log.info(
    { userId, activeMilestoneCount: activeMilestones.length },
    'Assessing milestone capacity'
  );
  ...
  try {
    ...
    if (calendarLoad) {
      ...
      assessment.calendarLoadPercent = Math.round(
        (calendarLoad.weeklyMeetingHours / weeklyCapacityHours) * 100
      );
    }
    ...
    log.info(
      { userId, currentLoad: assessment.currentLoad, canTakeNew: assessment.canTakeNewMilestone },
      'Capacity assessed'
    );
    ...
    if (windows.length > 0) {
      ...
      if (idealWindows.length > 0) {
        ...
        for (const window of idealWindows.slice(0, 3)) {
          ...
          lines.push(
            `- ${dateStr} ${window.startHour}:00-${window.endHour}:00 (${window.quality})`
          );
        }
      }
    }
...
function calculateMeetingHours(dayOverview: DayOverview): number {
  if (!dayOverview.events || dayOverview.events.length === 0) return 0;
...
function hasConflict(dayOverview: DayOverview, startHour: number, endHour: number): boolean {
  if (!dayOverview.events || dayOverview.events.length === 0) return false;
