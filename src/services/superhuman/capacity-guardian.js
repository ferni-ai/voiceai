...
import { getCalendarLoadFactors, getCalendarBurnoutRiskFactors, getCalendarLoadSummary, } from '../calendar/calendar-load-service.js';
...
export function detectEnergyLevel(transcript, voiceSignals) {
    const indicators = [];
...
export function detectOvercommitment(transcript) {
...
export async function recordEnergyReading(userId, reading) {
    const now = new Date();
    const fullReading = {
        ...reading,
    ...
    try {
        ...
        void onCapacityStateChange(userId, fullReading.id, {
            level: fullReading.energyLevel === 'high'
                ? 'thriving'
        ...
        log.debug({ userId, level: reading.energyLevel, score: reading.energyScore }, '⚡ Energy recorded');
    }
    catch (error) {
        log.warn({ error: String(error), userId }, 'Failed to record energy');
    }
}
export async function loadEnergyHistory(userId, days = 14) {
...
// BURNOUT ASSESSMENT
// ============================================================================
export async function assessBurnoutRisk(userId) {
    ...
    if (readings.length >= 5) {
        ...
        const olderAvg = readings.length > 5
    ...
    const highEnergyCount = readings.filter((r) => r.energyLevel === 'high' || r.energyLevel === 'good').length;
    ...
    try {
        ...
        if (hasCrisis && lastCrisis) {
            const daysSince = Math.floor((Date.now() - new Date(lastCrisis.timestamp).getTime()) / (1000 * 60 * 60 * 24));
...
// CONTEXT BUILDING
// ============================================================================
export async function buildCapacityContext(userId) {
    ...
    const sections = ['[CAPACITY GUARDIAN - Better Than Human Energy Tracking]'];
    ...
    if (readings.length >= 3) {
        const avgScore = Math.round(readings.slice(0, 3).reduce((sum, r) => sum + r.energyScore, 0) / 3);
...
//# sourceMappingURL=capacity-guardian.js.map