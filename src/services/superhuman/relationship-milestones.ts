...
export type MilestoneType =
  | 'duration' // Time-based (30 days, 6 months, 1 year)
  | 'conversations' // Conversation count
  | 'trust' // Trust-building moments
  | 'breakthrough' // Significant realizations
  | 'growth' // Observable growth
  | 'support' // Times Ferni was there
  | 'celebration'; // Shared celebrations
...
export interface RelationshipMilestone {
  ...
  // When
  achievedAt: number;
  acknowledged: boolean; // Has Ferni mentioned it?

  // Context
  context?: string; // What was happening when this was reached
  userSentiment?: string; // How user seemed at the time
}
...
export interface RelationshipSummary {
  ...
  nextMilestone?: { type: string; description: string; progressPercent: number };
}
...
export async function checkAndRecordMilestones(
  ...
  for (const milestone of DURATION_MILESTONES) {
    if (daysSinceFirst >= milestone.days) {
      const exists = existingMilestones.some(
        (m) => m.type === 'duration' && m.title === milestone.title
      );
      if (!exists) {
  ...
  for (const milestone of CONVERSATION_MILESTONES) {
    if (stats.totalConversations >= milestone.count) {
      const exists = existingMilestones.some(
        (m) => m.type === 'conversations' && m.title === milestone.title
      );
      if (!exists) {
        const newMilestone: RelationshipMilestone = {
          id: `milestone_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
  ...
  if (newMilestones.length > 0) {
    log.info(
      { userId, milestones: newMilestones.map((m) => m.title) },
...
export async function recordSpecialMilestone(
  userId: string,
  milestone: {
    type: MilestoneType;
    title: string;
...
const milestoneCache = new Map<string, RelationshipMilestone[]>();

async function loadMilestones(userId: string): Promise<RelationshipMilestone[]> {
  ...
  try {
    ...
    const milestones = snapshot.docs.map((doc) => doc.data() as RelationshipMilestone);
...
async function saveMilestone(milestone: RelationshipMilestone): Promise<void> {
  const db = getFirestoreDb();
  if (db) {
    await db
      .collection('bogle_users')
...
export async function buildRelationshipSummary(
  ...
  let trustLevel: RelationshipSummary['trustLevel'] = 'new';
  ...
  // Find next milestone
  let nextMilestone: RelationshipSummary['nextMilestone'];
...
export async function buildMilestoneContext(
  ...
  const sections: string[] = ['[RELATIONSHIP MILESTONES - Better Than Human Journey Awareness]'];
  ...
  if (summary.totalDays > 90) {
    sections.push('\n**Journey Reflection:**');
    sections.push(
      `"Remember our first conversation? That was ${summary.totalDays} days ago. Look how far you've come."`
    );
  }
