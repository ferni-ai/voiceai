...
import { getWeekOverview, findFreeTimeSlots, createEvent, } from '../calendar/calendar-service.js';
 ...
 */
export async function validateCommitmentFeasibility(userId, commitment) {
    try {
        ...
        // Generate suggestion
        let suggestion = null;
        let alternativeCommitment = null;
 ...
 */
export async function findTimeForCommitment(userId, commitment) {
 ...
 */
export async function createCalendarBlocksForCommitment(userId, commitment, slots) {
    const eventIds = [];
    let totalMinutes = 0;
    for (const slot of slots) {
        ...
        try {
            ...
            if (event) {
                ...
                log.debug({ userId, eventId: event.id, commitment: commitment.text }, 'Created calendar block for commitment');
            }
        }
        catch (error) {
 ...
 */
export async function checkCommitmentConflicts(userId, newEvent, commitments) {
    const conflicts = [];
    // Only check commitments that have calendar blocks
    const commitmentsWithBlocks = commitments.filter((c) => c.calendarEventIds && c.calendarEventIds.length > 0);
    for (const commitment of commitmentsWithBlocks) {
        if (!commitment.calendarEventIds)
            continue;
 ...
 */
export async function onCalendarChange(userId, change, commitments) {
    // Fetch commitments if not provided
    let userCommitments = commitments || [];
    if (!commitments || commitments.length === 0) {
        try {
            const { loadUserCommitments } = await import('./commitment-keeper.js');
            userCommitments = await loadUserCommitments(userId);
            log.debug({ userId, count: userCommitments.length }, 'Fetched commitments for conflict check');
        }
        catch (error) {
            log.warn({ error: String(error), userId }, 'Could not fetch commitments, skipping conflict check');
            return [];
        }
    }
    if (change.type === 'deleted') {
        // Check if deleted event was a commitment block
        const affectedCommitments = userCommitments.filter((c) => c.calendarEventIds?.includes(change.event.id || ''));
 ...
 */
function emitCommitmentConflictEvent(userId, conflicts) {
    try {
        ...
        if (highPriorityConflicts.length > 0) {
            ...
            storeCommitmentAlert(userId, highPriorityConflicts).catch((err) => log.warn({ error: String(err) }, 'Failed to store commitment alert'));
        }
    }
    catch (error) {
 ...
 */
async function storeCommitmentAlert(userId, conflicts) {
    try {
...
function parseCommitmentRequirements(commitment) {
    ...
    if ('frequency' in commitment && commitment.frequency) {
        ...
        if (commitment.frequency.period === 'day')
            period = 'day';
        else if (commitment.frequency.period === 'month')
            period = 'month';
        else
            period = 'week';
    }
    ...
    // Check for weekends
    const includesWeekends = text.includes('weekend') || text.includes('saturday') || text.includes('sunday');
    return {
...
function extractActivityName(text) {
    ...
    if (lower.includes('gym'))
        return 'Gym';
    if (lower.includes('run') || lower.includes('running'))
        return 'Run';
 ...
 */
export async function buildCommitmentCalendarContext(userId, commitment) {
...
//# sourceMappingURL=commitment-calendar-integration.js.map