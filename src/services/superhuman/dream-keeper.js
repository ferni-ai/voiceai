...
export function detectDream(transcript) {
...
export async function loadUserDreams(userId) {
    ...
    try {
        ...
        const snapshot = await db
            ...
            .limit(30)
            .get();
        const dreams = snapshot.docs.map((doc) => doc.data());
...
export async function recordDreamMention(userId, detected) {
    const dreams = await loadUserDreams(userId);
    // Find existing dream of same type with similar content
    const existing = dreams.find((d) => d.type === detected.type &&
...
function generateDreamTitle(type, statement) {
    const prefixes = {
...
// DORMANCY TRACKING
// ============================================================================
export async function findDormantDreams(userId) {
    const dreams = await loadUserDreams(userId);
    const reminders = [];
    const now = Date.now();
    const dayMs = 24 * 60 * 60 * 1000;
    for (const dream of dreams) {
        if (dream.status !== 'alive' && dream.status !== 'dormant')
            continue;
        ...
        if (daysSinceLastMention > 30 && dream.status === 'alive') {
            ...
            try {
                ...
                log.debug({ error: String(outreachError) }, 'Dream dormancy outreach failed (non-blocking)');
            }
        }
...
// CONTEXT BUILDING
// ============================================================================
export async function buildDreamContext(userId) {
    const dreams = await loadUserDreams(userId);
    const reminders = await findDormantDreams(userId);
    if (dreams.length === 0) {
        return '';
    }
    const sections = ['[DREAM KEEPER - Better Than Human Aspiration Memory]'];
...
//# sourceMappingURL=dream-keeper.js.map