 ...
 */
export function classifyEvent(event, eventHistory) {
    ...
    if (eventHistory) {
        ...
        if (relevantHistory) {
            if (relevantHistory.averageAnxiety > 0.7)
                difficulty = 'high';
            else if (relevantHistory.averageAnxiety > 0.4)
                difficulty = 'medium';
            else
                difficulty = 'low';
        }
    ...
    if (difficulty === 'unknown') {
        ...
        if (highDifficulty.includes(matchedType))
            difficulty = 'high';
        else if (mediumDifficulty.includes(matchedType))
            difficulty = 'medium';
        else
            difficulty = 'low';
    }
 ...
 */
export async function loadEventHistory(userId) {
 ...
 */
export async function recordEventOutcome(userId, eventType, outcome, reflection, helpfulPrep, wouldDoDifferently) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export async function getPrepRecommendations(userId, event) {
 ...
 */
export async function buildCalendarPrepContext(userId, upcomingEvents) {
    ...
    const eventsNeedingPrep = [];
    for (const event of upcomingEvents) {
        const hoursUntil = (event.startTime - now) / (1000 * 60 * 60);
        if (hoursUntil < 0 || hoursUntil > 48)
            continue; // Skip past events and far future
        const { type, difficulty } = classifyEvent(event, history);
...
//# sourceMappingURL=calendar-prep-coaching.js.map