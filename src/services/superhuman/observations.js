...
const LINGUISTIC_PATTERNS = {
    ...
    patterns: [
        ...
        },
        {
            ...
            surfacingPhrase: 'You say "I guess" a lot. Almost like you\'re asking permission to have opinions.',
            minOccurrences: 4,
        },
        {
            pattern: /\bmaybe\b.*\?\s*$/gim,
            description: 'Ends statements as questions',
            surfacingPhrase: "You phrase things as questions a lot. You're allowed to just... state things.",
            minOccurrences: 5,
        },
        {
...
export class SuperhumanObservationsEngine {
            ...
            for (const pattern of config.patterns) {
                ...
                if (matches) {
                    ...
                    if (currentCount >= pattern.minOccurrences && !this.observations.has(key)) {
                        ...
                        logger.debug({ userId: this.userId, pattern: key, count: currentCount }, '🔍 New superhuman observation detected');
                    }
     ...
     */
    recordObservation(type, observation, surfacingPhrase) {
    ...
    checkForSurfacing(context) {
        ...
        const eligible = Array.from(this.observations.values()).filter((obs) => !obs.surfaced && obs.confidence > 0.6 && obs.evidenceCount >= 4);
        if (eligible.length === 0) {
            return { shouldSurface: false };
        }
        // Probability based on relationship depth
        const probability = context.relationshipStage === 'old_friend'
            ? 0.08
        ...
        logger.debug({ userId: this.userId, observation: selected.observation }, '🔍 Superhuman observation surfaced');
        return {
            shouldSurface: true,
    ...
    getRelevantObservation(topic) {
        ...
        if (!types)
            return null;
        const candidates = Array.from(this.observations.values()).filter((obs) => !obs.surfaced && types.includes(obs.type) && obs.confidence > 0.65);
