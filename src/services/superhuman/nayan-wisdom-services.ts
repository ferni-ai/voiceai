...
export async function getEnoughStatements(userId: string, domain?: string): Promise<EnoughStatement[]> {
  const db = getFirestoreDb();
  if (!db) return [];
...
export async function buildNayanWisdomContext(userId: string): Promise<string> {
  const [paradoxes, enoughStatements, incubating, patterns, legacyStatements, cyclicalPatterns, chapters] =
    await Promise.all([
