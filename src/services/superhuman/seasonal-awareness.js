...
import { onSeasonalPatternDetected, } from '../outreach/superhuman-outreach-bridge.js';
...
export function getDaysUntilSeasonChange(date = new Date()) {
...
export function detectSeasonalPattern(transcript) {
...
export async function loadSeasonalObservations(userId) {
...
export async function loadPersonalDates(userId) {
...
export async function recordSeasonalObservation(userId, detected) {
    ...
    const newObs = {
        ...
        sentiment: detected.type === 'sad' || detected.type === 'holiday_stress' ? 'negative' : 'neutral',
        observationCount: 1,
    ...
    void onSeasonalPatternChange(userId, newObs.id, {
        pattern: newObs.type,
    ...
    if (newObs.observationCount >= 2) {
        try {
            ...
            log.debug({ error: String(outreachError) }, 'Seasonal pattern outreach failed (non-blocking)');
        }
    }
    return newObs;
}
export async function recordPersonalDate(userId, date) {
    const newDate = {
        ...date,
    ...
    dates.push(newDate);
    dateCache.set(userId, dates);
    log.info({ userId, name: newDate.name, month: newDate.month, day: newDate.day }, '📅 Personal date recorded');
    return newDate;
}
// ============================================================================
// UPCOMING DATE DETECTION
// ============================================================================
export async function findUpcomingDates(userId, daysAhead = 14) {
...
// CONTEXT BUILDING
// ============================================================================
export async function buildSeasonalContext(userId) {
    const now = new Date();
    const currentSeason = getCurrentSeason(now);
    const currentMonth = now.getMonth() + 1;
    ...
    const relevantPatterns = observations.filter((o) => o.month === currentMonth || o.season === currentSeason);
    const sections = ['[SEASONAL AWARENESS - Better Than Human Cycle Memory]'];
    ...
    if (relevantPatterns.length > 0) {
        ...
        for (const pattern of relevantPatterns.slice(0, 3)) {
            const sentimentEmoji = pattern.sentiment === 'negative' ? '⚠️' : pattern.sentiment === 'positive' ? '✨' : '📌';
            sections.push(`${sentimentEmoji} ${pattern.type}: "${pattern.observation.slice(0, 60)}..." (seen ${pattern.observationCount}x)`);
        }
    }
    // Upcoming dates
    if (upcomingDates.length > 0) {
        ...
        for (const ud of upcomingDates.slice(0, 4)) {
            const emoji = ud.date.sentiment === 'negative' ? '💙' : ud.date.sentiment === 'positive' ? '🎉' : '📅';
            sections.push(`${emoji} ${ud.date.name} in ${ud.daysUntil} day${ud.daysUntil === 1 ? '' : 's'}`);
        }
    }
