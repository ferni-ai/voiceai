...
export type CelebrationType =
  | 'personal_achievement' // Their own wins
  | 'personal_milestone' // Their own life milestones
  | 'for_others' // Celebrating someone else
  | 'community' // Group/social celebrations
  | 'holiday' // Seasonal/cultural celebrations
  | 'spontaneous' // Impromptu joy moments
  | 'quiet' // Internal acknowledgment, not a party
  | 'recovery'; // Celebrating after hard times
...
export interface CelebrationBalanceProfile {
  ...
  preferences: {
    ...
    energyCapacity: 'low' | 'medium' | 'high'; // How much celebration energy they have
    soloVsSocial: 'solo' | 'social' | 'mixed';
  };
  /** Detected patterns */
  patterns: {
    bestMonthsForCelebration: number[]; // 1-12
    avoidMonths: number[]; // Months where energy is typically low
...
const SIZE_ENERGY_MAP: Record<CelebrationSize, number> = {
  micro: 1,
  small: 2,
  medium: 4,
  large: 6,
...
async function loadCelebrationProfile(userId: string): Promise<CelebrationBalanceProfile | null> {
...
export async function recordCelebration(
  ...
  const celebration: RecordedCelebration = {
    id,
    description,
    type,
    size,
...
export async function getCelebrationBalance(userId: string): Promise<CelebrationBalance> {
  ...
  const daysSinceLastCelebration = Math.floor(
    (now.getTime() - new Date(lastCelebration.date).getTime()) / (1000 * 60 * 60 * 24)
  );
  ...
  // Determine state
  let state: CelebrationBalance['state'] = 'balanced';
  ...
  if (daysSinceLastCelebration >= CELEBRATION_DROUGHT_DAYS) {
    ...
    state = 'self_focused';
    recommendations.push(
      "Your self-celebration is strong! Consider: is there someone else's win you could celebrate?"
    );
  }
 ...
 */
export async function getCelebrationSuggestions(userId: string): Promise<{
  needsJoy: boolean;
  needsRest: boolean;
  suggestions: string[];
}> {
 ...
 */
export async function shouldPromptForCelebration(userId: string): Promise<{
 ...
 */
export async function buildCelebrationBalanceContext(userId: string): Promise<string> {
  const balance = await getCelebrationBalance(userId);
  ...
  // State summary
  const stateDescriptions: Record<CelebrationBalance['state'], string> = {
...
function createDefaultProfile(userId: string): CelebrationBalanceProfile {
  return {
    userId,
    celebrations: [],
    preferences: {
...
function updatePatterns(profile: CelebrationBalanceProfile, celebration: RecordedCelebration): void {
  ...
  if (!profile.patterns.preferredDayOfWeek.includes(dayOfWeek)) {
    ...
    const sameDayCelebrations = profile.celebrations.filter(
      (c) => new Date(c.date).getDay() === dayOfWeek
    ).length;
