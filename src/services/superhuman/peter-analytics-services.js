...
const log = createLogger({ module: 'superhuman:peter-analytics' });
// ============================================================================
// BLIND SPOT SERVICE
// ============================================================================
export async function recordBlindSpot(userId, blindSpot) {
...
export async function getBlindSpots(userId, domain) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
...
// ============================================================================
export async function recordCounterfactual(userId, counterfactual) {
    const db = getFirestoreDb();
    if (!db) {
    ...
    try {
        await db.collection('bogle_users').doc(userId).collection('counterfactuals').add(counterfactual);
...
export async function getCounterfactuals(userId) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
...
// ============================================================================
export async function recordPatternPrediction(userId, prediction) {
    const db = getFirestoreDb();
    if (!db) {
    ...
    try {
        await db.collection('bogle_users').doc(userId).collection('pattern_predictions').add(prediction);
...
export async function getPatternPredictions(userId, domain) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
...
// ============================================================================
export async function recordDecisionScore(userId, score) {
    const db = getFirestoreDb();
    if (!db) {
...
export async function getDecisionScores(userId, domain) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
...
// CORRELATION SERVICE
// ============================================================================
export async function recordCorrelation(userId, correlation) {
    const db = getFirestoreDb();
    if (!db) {
    ...
    try {
        ...
        log.info({ userId, factor1: correlation.factor1, factor2: correlation.factor2 }, 'Correlation recorded');
    }
    catch (error) {
...
// ANOMALY SERVICE
// ============================================================================
export async function recordAnomaly(userId, anomaly) {
    const db = getFirestoreDb();
    if (!db) {
...
// ============================================================================
export async function recordInsight(userId, insight) {
    const db = getFirestoreDb();
    if (!db) {
...
export async function getInsights(userId, domain) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
...
// CONTEXT BUILDER
// ============================================================================
export async function buildPeterAnalyticsContext(userId) {
    ...
    const lines = ['[PETER ANALYTICS MEMORY - Better Than Human]'];
    // Active blind spots
...
//# sourceMappingURL=peter-analytics-services.js.map