...
import { getMostMentioned, getRecentlyMentioned, getPeopleByImpact, getTopSupporters, } from '../../semantic-intelligence/relationship-graph.js';
...
async function fetchRelationshipData(userId) {
    const insights = [];
    try {
        ...
        for (const person of allMentioned) {
            ...
            if (!isRecent && person.mentionCount >= 3) {
                ...
                const daysSince = person.lastMentioned
                    ? Math.floor((Date.now() - new Date(person.lastMentioned).getTime()) / (24 * 60 * 60 * 1000))
                    : 30;
                if (daysSince >= 14) {
                    insights.push({
                        person: person.name,
...
// GENERATOR
// ============================================================================
async function generateRelationshipInsights(userId, context) {
    const insights = [];
    try {
...
function buildRelationshipInsight(data, userId) {
    ...
    return {
        ...
        headline: data.insightType === 'support_system'
            ? 'Your support system'
...
// REGISTRATION
// ============================================================================
const relationshipNetworkGenerator = {
    category: 'relationship_network',
