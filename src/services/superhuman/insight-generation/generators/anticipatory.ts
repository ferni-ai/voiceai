...
async function fetchAnticipatoryData(userId: string): Promise<AnticipatoryData[]> {
  const insights: AnticipatoryData[] = [];
  ...
  try {
    ...
    for (const pattern of ANNUAL_PATTERNS) {
      ...
      if (daysUntil > 0 && daysUntil <= 14) {
        insights.push({
          type: pattern.type as AnticipatoryData['type'],
          event: pattern.event,
...
async function generateAnticipatoryInsights(
  userId: string,
...
function buildAnticipatoryInsight(data: AnticipatoryData, userId: string): GeneratedInsight | null {
  ...
  return {
    ...
    triggerTopics: [data.event, data.trigger, data.season].filter(Boolean) as string[],
    confidence: 0.75,
    dataPoints: 3,
...
const anticipatoryGenerator: InsightGenerator = {
  category: 'anticipatory',
