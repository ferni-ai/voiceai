...
async function getTopicSilences(userId: string): Promise<TopicSilenceData[]> {
  const silences: TopicSilenceData[] = [];

  try {
    // Get all commitments and filter for avoidance types
    const allCommitments = await getAllCommitments(userId);
    const avoidances = allCommitments.filter(
      (c) => c.type === 'avoid' && !c.fulfilled && !c.violated
    );
...
async function generateUnspokenInsights(
  userId: string,
...
function buildUnspokenInsight(data: TopicSilenceData, userId: string): GeneratedInsight | null {
  ...
  let priority: GeneratedInsight['priority'];
  ...
  return {
    ...
    tone,
    triggerTopics: [data.topic, data.relatedPerson].filter(Boolean) as string[],
...
function formatTimeAgo(days: number): string {
  ...
  if (days < 21) return 'a couple weeks ago';
  if (days < 30) return 'about three weeks ago';
...
const unspokenAwarenessGenerator: InsightGenerator = {
  category: 'unspoken_awareness',
