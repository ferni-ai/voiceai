...
interface FirstTimeData {
  type: 'anniversary' | 'recent_first' | 'growth_since' | 'depth_increase' | 'new_language';
  topic: string;
  firstDate: Date;
  daysSince: number;
...
): Promise<FirstTimeData[]> {
  const moments: FirstTimeData[] = [];
...
function extractNewWord(text: string): string {
...
async function generateFirstTimeInsights(
  userId: string,
  context: InsightGeneratorContext
...
function buildFirstTimeInsight(data: FirstTimeData, userId: string): GeneratedInsight | null {
  ...
  const depthPriority: Record<string, GeneratedInsight['priority']> = {
    surface: 'low',
    moderate: 'medium',
    deep: 'high',
    profound: 'critical',
  ...
  return {
    ...
    tone: 'celebratory',
    triggerTopics: [data.topic, data.newWord].filter(Boolean) as string[],
...
async function hasEnoughData(_userId: string): Promise<boolean> {
...
const firstTimeCelebrationGenerator: InsightGenerator = {
  category: 'first_time_celebration',
