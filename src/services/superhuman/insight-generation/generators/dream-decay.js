...
async function fetchDreamData(userId) {
    const dreams = [];
    try {
...
function categorizeDream(dream) {
    const lower = dream.toLowerCase();
    if (lower.match(/learn|study|course|skill|language|piano|guitar|code|paint/))
        return 'skill';
    if (lower.match(/travel|visit|trip|explore|country|city/))
        return 'travel';
    if (lower.match(/write|book|novel|blog|create|art|music/))
        return 'creative';
    if (lower.match(/business|start|launch|company|career|job/))
        return 'career';
    if (lower.match(/health|fitness|marathon|weight|meditation|yoga/))
        return 'health';
    if (lower.match(/family|kids|marriage|home|house/))
        return 'life';
    return 'personal';
}
// ============================================================================
// GENERATOR
// ============================================================================
async function generateDreamInsights(userId, _context) {
    const insights = [];
    try {
...
function buildDreamInsight(dream, userId) {
...
//# sourceMappingURL=dream-decay.js.map