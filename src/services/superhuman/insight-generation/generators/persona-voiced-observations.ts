...
// ============================================================================
// DATA FETCHING (Lazy imports to avoid circular deps)
// ============================================================================
...
interface UserData {
  profile?: {
    ...
    humanMemory?: {
      growthArc?: { challenges?: Array<{ challenge: string; status: string }> };
    };
    personalJourney?: {
      seasonal?: { timeAnchors?: Array<{ description: string; topics?: string[] }> };
    };
  };
  commitments?: CommitmentData[];
  dreams?: DreamData[];
  capacityData?: { recentMentions?: Array<{ type: string; count: number }> };
  networkData?: NetworkPersonData[];
}
...
async function fetchUserData(userId: string): Promise<UserData> {
  ...
  try {
    ...
    if (profileDoc.exists) {
      data.profile = profileDoc.data() as UserData['profile'];
    }
...
function generatePersonaOpening(persona: string): string {
...
const personaVoicedGenerator: InsightGenerator = {
  ...
  async generate(userId: string, context: InsightGeneratorContext): Promise<GeneratedInsight[]> {
