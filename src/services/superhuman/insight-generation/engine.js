 ...
 */
export function registerInsightGenerator(generator) {
 ...
 */
export function getRegisteredGenerators() {
...
function getCacheKey(userId, category) {
 ...
 */
export async function generateAllInsights(userId, context = {}) {
    ...
    const generatorPromises = Array.from(generators.values()).map(async (generator) => {
        try {
            ...
            if (!hasData) {
                log.debug({ userId, category: generator.category }, 'Skipping generator - insufficient data');
                return [];
            }
    ...
    for (const insights of allInsights) {
        for (const insight of insights) {
            // Dedupe by headline
            if (seenHeadlines.has(insight.headline))
                continue;
 ...
 */
export async function generateCategoryInsights(userId, category, context = {}) {
 ...
 */
export function queryCachedInsights(userId, options = {}) {
    ...
    if (options.triggerTopic) {
        ...
        insights = insights.filter((i) => i.triggerTopics?.some((t) => t.toLowerCase().includes(topic)) ?? false);
    }
    if (options.triggerEmotion) {
        insights = insights.filter((i) => i.triggerEmotions?.includes(options.triggerEmotion) ?? false);
    }
...
export async function getInsightsToSurface(userId, context) {
    ...
    if (context.currentPerson) {
        ...
        insights = insights.filter((i) => !i.triggerPerson || i.triggerPerson.toLowerCase() === personLower);
    }
 ...
 */
export function markInsightSurfaced(userId, insightId, reaction) {
    const cached = insightCache.get(getCacheKey(userId));
    if (!cached)
        return;
