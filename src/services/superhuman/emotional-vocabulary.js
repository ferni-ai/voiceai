...
// VAGUE WORD MAPPINGS
// ============================================================================
const VAGUE_WORD_MAPPINGS = [
    {
        vagueWord: 'bad',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['disappointed', 'sad', 'guilty', 'ashamed', 'frustrated', 'anxious'].includes(e.word)),
        clarifyingQuestion: 'When you say "bad," is it more like disappointed, sad, guilty, or something else?',
    },
    {
        vagueWord: 'fine',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['content', 'neutral', 'numb', 'resigned', 'disconnected'].includes(e.word)),
        clarifyingQuestion: 'What does "fine" feel like right now? Content, neutral, or maybe holding something back?',
    },
    {
        vagueWord: 'stressed',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['overwhelmed', 'anxious', 'frustrated', 'exhausted', 'worried'].includes(e.word)),
        clarifyingQuestion: 'Is the stress more like overwhelm, anxiety, frustration, or exhaustion?',
    },
    {
        vagueWord: 'good',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['content', 'happy', 'grateful', 'proud', 'relieved', 'hopeful'].includes(e.word)),
        clarifyingQuestion: 'What kind of good? Content, proud, grateful, or something else?',
    },
    {
        vagueWord: 'upset',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['hurt', 'angry', 'disappointed', 'sad', 'frustrated'].includes(e.word)),
        clarifyingQuestion: 'Is upset leaning more toward hurt, angry, disappointed, or sad?',
    },
    {
        vagueWord: 'okay',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['neutral', 'content', 'resigned', 'numb'].includes(e.word)),
        clarifyingQuestion: 'Is okay meaning "actually fine" or "getting by"?',
    },
    {
        vagueWord: 'weird',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['confused', 'uneasy', 'disconnected', 'conflicted', 'ambivalent'].includes(e.word)),
        clarifyingQuestion: 'Weird in what way? Confused, uneasy, or hard to put your finger on?',
    },
    {
        vagueWord: 'down',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['sad', 'melancholic', 'disappointed', 'empty', 'lonely'].includes(e.word)),
        clarifyingQuestion: 'Is down more like sad, empty, lonely, or disappointed?',
    },
    {
        vagueWord: 'off',
        possibleMeanings: EMOTION_DICTIONARY.filter((e) => ['uneasy', 'disconnected', 'anxious', 'confused'].includes(e.word)),
 ...
 */
export function detectVagueEmotions(text) {
 ...
 */
export function suggestPreciseEmotions(category, intensityHint) {
 ...
 */
export async function recordEmotionUsage(userId, emotionWord, context) {
    ...
    try {
        ...
        const impact = positiveCategories.includes(emotion.category)
            ...
            : negativeCategories.includes(emotion.category)
                ? 'negative'
                : 'mixed';
        void onEmotionalPatternChange(userId, docRef.id, {
 ...
 */
export async function loadEmotionHistory(userId, daysBack = 90) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
 ...
 */
export async function analyzeVocabularyProfile(userId) {
    const history = await loadEmotionHistory(userId, 90);
    // Count emotion usage
    const frequentEmotions = new Map();
    ...
    for (const category of underusedCategories) {
        const suggestions = EMOTION_DICTIONARY.filter((e) => e.category === category && e.intensity === 'medium')
            .slice(0, 2)
            .map((e) => e.word);
 ...
 */
export function buildVagueEmotionContext(detectedVague) {
    ...
    for (const vague of detectedVague.slice(0, 2)) {
        ...
        sections.push(`   Possible meanings: ${vague.possibleMeanings
            .slice(0, 4)
            .map((e) => e.word)
            .join(', ')}`);
        sections.push('');
 ...
 */
export async function buildVocabularyContext(userId) {
    ...
    if (profile.underusedCategories.length > 2) {
        ...
        sections.push(`This person rarely names ${profile.underusedCategories.slice(0, 2).join(' or ')}-related emotions. ` +
...
//# sourceMappingURL=emotional-vocabulary.js.map