...
const RECEPTIVITY_INTERPRETATIONS = [
...
export function detectReceptivity(voiceAnalysis) {
    ...
    if (hour >= 9 && hour <= 11)
        ...
        timeModifier = -0.1; // Late night/early morning
    const score = Math.max(0, Math.min(1, baseReceptivity + toneModifier + timeModifier));
    // Generate interpretation
    const interpretation = RECEPTIVITY_INTERPRETATIONS.find((i) => score >= i.min && score < i.max)?.text ||
 ...
 */
export async function recordTimingLearning(userId, data) {
 ...
 * Update optimal windows based on patterns.
 */
function updateOptimalWindows(profile) {
    const windows = [];
 ...
 */
export async function queueTopicForRightMoment(userId, topic, options) {
    let profile = profiles.get(userId);
    if (!profile) {
        profile = createEmptyProfile(userId);
        profiles.set(userId, profile);
 ...
 */
export function getTopicsForNow(userId, currentConditions) {
    ...
    for (const qt of profile.queuedTopics) {
        ...
        if (qt.surfacedAt)
            continue;
        if (qt.expiresAt && now > qt.expiresAt)
            continue;
        const conditions = qt.idealConditions;
        // Check energy
        if (conditions.minEnergy && currentConditions.energy < conditions.minEnergy)
            continue;
        // Check calendar pressure
        if (conditions.maxCalendarPressure && currentConditions.calendarPressure) {
            const pressureRank = { light: 0, moderate: 1, heavy: 2 };
            if (pressureRank[currentConditions.calendarPressure] >
                pressureRank[conditions.maxCalendarPressure]) {
                continue;
            }
        }
        // Check avoided times
        if (conditions.avoidDaysOfWeek?.includes(currentDay))
            continue;
        if (conditions.avoidHoursOfDay?.includes(currentHour))
            continue;
        // Check required mood
        if (conditions.requiredMood && currentConditions.mood) {
            if (!conditions.requiredMood.includes(currentConditions.mood))
                continue;
        }
 ...
 */
export function markTopicSurfaced(userId, topic, wasEffective) {
    const profile = profiles.get(userId);
    if (!profile)
        return;
 ...
 */
export function isGoodTimeFor(userId, conversationType) {
    const profile = profiles.get(userId);
    const now = new Date();
    const currentHour = now.getHours();
    const currentDay = now.getDay();
    ...
    const windows = profile.optimalWindows[conversationType === 'deep'
        ? 'deepConversations'
    ...
    const matchingWindow = windows.find((w) => w.dayOfWeek === currentDay && currentHour >= w.startHour && currentHour < w.endHour);
    if (matchingWindow) {
        return {
            isGood: true,
 ...
 */
export function buildTimingContext(userId, currentReceptivity) {
    const profile = profiles.get(userId);
    const sections = ['[PERFECT TIMING INTELLIGENCE]'];
    // Current receptivity
    ...
    if (waitingTopics.length > 0) {
        ...
        for (const topic of waitingTopics.slice(0, 3)) {
            const daysWaiting = Math.floor((Date.now() - new Date(topic.queuedAt).getTime()) / (24 * 60 * 60 * 1000));
    ...
    if (profile.optimalWindows.deepConversations.length > 0) {
        ...
        sections.push(`Best time for deep conversations: ${dayNames[best.dayOfWeek]} ${best.startHour}:00-${best.endHour}:00`);
    }
    sections.push('');
...
// PERSISTENCE
// ============================================================================
async function saveTimingProfile(userId, profile) {
    ...
    try {
        ...
        if (profile.optimalWindows) {
            ...
            if (profile.optimalWindows.deepConversations?.length) {
                void onCoachingInsightChange(userId, `timing_deep`, {
...
export async function loadTimingProfile(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
...
//# sourceMappingURL=perfect-timing.js.map