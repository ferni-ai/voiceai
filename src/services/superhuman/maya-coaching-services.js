...
// HABIT DNA SERVICE
// ============================================================================
export async function recordHabitDNA(userId, habitName, event) {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            // Compute aggregates
            const timesStarted = events.filter((e) => e.event === 'started' || e.event === 'restarted').length;
            ...
            const successEvents = events.filter((e) => e.event === 'maintained' || e.event === 'mastered');
            const bestTime = getMostCommon(successEvents.map((e) => e.timeOfDay).filter(Boolean));
            const bestMood = getMostCommon(successEvents.map((e) => e.emotionalState).filter(Boolean));
            await docRef.update({
                events,
                timesStarted,
                timesBroke,
                currentStreak: event.event === 'broke' ? 0 : (existing.currentStreak || 0) + (event.event === 'maintained' ? 1 : 0),
            ...
            await docRef.set({
                ...
                currentStreak: 0,
                longestStreak: 0,
                commonTriggers: event.triggerOrBarrier && (event.event === 'maintained' || event.event === 'mastered')
                    ? [event.triggerOrBarrier]
                    : [],
                commonBarriers: event.triggerOrBarrier && (event.event === 'struggled' || event.event === 'broke')
                    ? [event.triggerOrBarrier]
                    : [],
                optimalConditions: {},
            });
...
export async function getHabitDNA(userId, habitName) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
...
// ============================================================================
export async function recordFrictionPoint(userId, habitName, friction) {
    const db = getFirestoreDb();
    if (!db) {
...
export async function getFrictionPoints(userId, habitName) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
        ...
        if (habitName) {
            query = query.where('habitName', '==', habitName);
        }
...
// ============================================================================
export async function recordTendencySignal(userId, signal) {
    const db = getFirestoreDb();
    if (!db) {
...
export async function getTendencyProfile(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
        ...
        const primaryTendency = Object.entries(scores).sort((a, b) => b[1] - a[1])[0][0];
...
// ============================================================================
export async function recordKeystoneObservation(userId, observation) {
    const db = getFirestoreDb();
    if (!db) {
...
export async function getKeystoneHabits(userId) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
        ...
        for (const obs of observations) {
            if (!byHabit[obs.primaryHabit])
                byHabit[obs.primaryHabit] = [];
...
// ============================================================================
export async function recordIdentityStatement(userId, statement) {
    const db = getFirestoreDb();
    if (!db) {
    ...
    try {
        ...
        if (!existing.empty) {
            ...
            if (confidenceOrder.indexOf(statement.confidence) > confidenceOrder.indexOf(existingData.confidence)) {
...
export async function getIdentityEvolution(userId) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
...
// ============================================================================
export async function recordSetbackPattern(userId, pattern) {
    const db = getFirestoreDb();
    if (!db) {
...
export async function getSetbackPatterns(userId, habitName) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
...
// ============================================================================
export async function recordHabitAutopsy(userId, autopsy) {
    const db = getFirestoreDb();
    if (!db) {
...
export async function getHabitAutopsies(userId) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
 ...
 */
export async function buildMayaCoachingContext(userId) {
    const [frictionPoints, tendencyProfile, keystones, identityStatements, setbackPatterns, autopsies] = await Promise.all([
    ...
    const lines = ['[MAYA COACHING MEMORY - Better Than Human]'];
    // Tendency profile
    ...
    const coreIdentity = identityStatements.filter((s) => s.confidence === 'core' || s.confidence === 'established');
...
//# sourceMappingURL=maya-coaching-services.js.map