...
export function extractPerson(transcript) {
 ...
 */
export function extractNames(transcript) {
    const results = [];
    const seen = new Set();
    ...
    const relationshipNameRegex = /\bmy\s+(mom|mother|dad|father|sister|brother|wife|husband|partner|boyfriend|girlfriend|friend|boss|coworker|colleague|mentor|cousin|aunt|uncle)\s+([A-Z][a-z]+)/gi;
    let match;
...
export async function loadNetwork(userId) {
    ...
    try {
        ...
        const snapshot = await db
            ...
            .limit(50)
            .get();
        const network = snapshot.docs.map((doc) => doc.data());
...
export async function savePerson(person) {
    ...
    void onRelationshipNetworkChange(person.userId, person.id, {
        person: person.name,
        relationship: person.type,
        connectionStrength: person.mentionCount > 10 ? 'strong' : person.mentionCount > 5 ? 'moderate' : 'weak',
...
export async function recordMention(userId, extracted) {
    ...
    const existing = network.find((p) => p.name.toLowerCase() === extracted.name.toLowerCase() ||
    ...
    log.info({ userId, personName: newPerson.name, type: newPerson.type }, '👤 New person in network');
    return newPerson;
}
// ============================================================================
// CONNECTION OPPORTUNITIES
// ============================================================================
export async function findConnectionOpportunities(userId) {
    const network = await loadNetwork(userId);
    const opportunities = [];
    const now = Date.now();
    ...
    for (const person of network) {
        ...
        if ((person.sentiment === 'positive' || person.sentiment === 'very_positive') &&
        ...
        if ((person.type === 'ex' || person.sentiment === 'complicated') &&
...
// CONTEXT BUILDING
// ============================================================================
export async function buildNetworkContext(userId) {
    const network = await loadNetwork(userId);
    const opportunities = await findConnectionOpportunities(userId);
    if (network.length === 0) {
        return '';
    }
    const sections = ['[RELATIONSHIP NETWORK - Better Than Human People Memory]'];
    ...
    if (keyPeople.length > 0) {
        ...
        for (const person of keyPeople) {
            ...
            sections.push(`• ${person.name} (${person.type}) ${sentimentEmoji[person.sentiment]} - last mentioned ${daysSince}d ago`);
        }
    }
    // Connection opportunities
 ...
 */
export async function checkAndTriggerReconnectionOutreach(userId) {
    const opportunities = await findConnectionOpportunities(userId);
    ...
    if (highUrgency.length > 0) {
        try {
            const { onReconnectionOpportunity } = await import('../conversation-thread/group-outreach-triggers.js');
            ...
            for (const opportunity of highUrgency.slice(0, 2)) {
                ...
                if (result) {
                    triggeredCount++;
                    log.info({ userId, personName: opportunity.personName }, '🎭 Triggered group outreach for reconnection');
                }
            }
        }
        catch (error) {
            log.debug({ error: String(error), userId }, 'Failed to trigger reconnection outreach (non-fatal)');
        }
    }
...
//# sourceMappingURL=relationship-network.js.map