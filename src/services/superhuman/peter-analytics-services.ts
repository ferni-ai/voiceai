...
export async function recordCounterfactual(
  ...
  try {
    const docRef = await db.collection('bogle_users').doc(userId).collection('counterfactuals').add(counterfactual);
...
export async function recordPatternPrediction(
  ...
  try {
    const docRef = await db.collection('bogle_users').doc(userId).collection('pattern_predictions').add(prediction);
