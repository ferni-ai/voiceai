...
import { onWisdomInsightChange, onPerspectiveShiftChange, } from '../data-layer/hooks/wisdom-hooks.js';
const log = createLogger({ module: 'FutureSelf' });
// ============================================================================
// LETTER TEMPLATES
// ============================================================================
const OPTIMISTIC_TEMPLATES = {
    '3_months': [
        `Dear present-you,
 ...
 */
export async function generateFutureSelfLetter(userId, timeframe, context) {
...
function extractPositivePatterns(context) {
    const patterns = [];
    ...
    if (context.values?.length) {
        ...
        const aligned = statedValues.filter((sv) => demonstratedValues.some((dv) => dv.value === sv.value));
        if (aligned.length > 0) {
            patterns.push({
...
function extractConcerningPatterns(context) {
    const patterns = [];
    // From burnout risk
...
function generateOptimisticLetter(timeframe, patterns, context) {
...
function generateCautionaryLetter(timeframe, patterns, context) {
...
function generateKeyInsights(positive, concerning, context) {
    const insights = [];
...
// PERSISTENCE
// ============================================================================
async function saveLetter(userId, letter) {
    ...
    try {
        ...
        void onWisdomInsightChange(userId, `future_self_${letter.id}`, {
        ...
        if (letter.optimisticPath || letter.cautionaryPath) {
            void onPerspectiveShiftChange(userId, `trajectory_${letter.id}`, {
 ...
 */
export async function getRecentLetter(userId, timeframe) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
 ...
 */
export function buildFutureSelfContext(letter) {
    if (!letter)
        return '';
    const sections = ['[FUTURE SELF LETTER AVAILABLE]'];
...
//# sourceMappingURL=future-self.js.map