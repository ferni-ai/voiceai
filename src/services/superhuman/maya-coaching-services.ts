...
export async function recordHabitDNA(
  ...
  try {
    ...
    if (doc.exists) {
      ...
      const bestTime = getMostCommon(successEvents.map((e) => e.timeOfDay).filter(Boolean) as string[]);
      const bestMood = getMostCommon(successEvents.map((e) => e.emotionalState).filter(Boolean) as string[]);
...
export async function getTendencyProfile(userId: string): Promise<TendencyProfile | null> {
  ...
  try {
    ...
    const primaryTendency = Object.entries(scores).sort((a, b) => b[1] - a[1])[0][0] as TendencyProfile['primaryTendency'];
...
export async function recordSetbackPattern(
  userId: string,
  pattern: SetbackPattern
): Promise<void> {
  const db = getFirestoreDb();
  if (!db) {
...
export async function getSetbackPatterns(userId: string, habitName?: string): Promise<SetbackPattern[]> {
  const db = getFirestoreDb();
  if (!db) return [];
...
export async function buildMayaCoachingContext(userId: string): Promise<string> {
  const [frictionPoints, tendencyProfile, keystones, identityStatements, setbackPatterns, autopsies] =
    await Promise.all([
