...
async function loadSeasonalProfile(userId: string): Promise<SeasonalPlanningProfile | null> {
 ...
 */
export async function getRelevantCulturalDates(
  userId: string,
  startDate: string,
  endDate: string
): Promise<CulturalDate[]> {
 ...
 */
export async function updatePersonalPatterns(
  userId: string,
  patterns: Partial<PersonalSeasonalPattern>
): Promise<void> {
  const profile = (await loadSeasonalProfile(userId)) || createDefaultProfile(userId);
 ...
 */
export async function suggestOptimalTiming(
  userId: string,
  eventType: string,
...
export async function checkDateConflicts(
  ...
  const personalConflicts: string[] = [];
 ...
 */
export async function buildSeasonalPlanningContext(
  userId: string,
  eventType?: string,
  targetDate?: string
): Promise<string> {
  if (!eventType && !targetDate) return '';
...
function createDefaultProfile(userId: string): SeasonalPlanningProfile {
  return {
    userId,
...
function createDefaultPersonalPatterns(): PersonalSeasonalPattern {
  return {
