...
import { onPatternInsightChange, onBehavioralPatternChange, } from '../data-layer/hooks/better-than-human-hooks.js';
 ...
 */
export function recordTopicEnergy(userId, data) {
...
function updateTopicList(list, topic, energyDelta, data) {
...
function updateFadingTopics(profile, currentTopic) {
    ...
    profile.fadingTopics = profile.fadingTopics.filter((f) => {
        const recentMentions = profile.topicHistory.filter((h) => h.topic === f.topic && new Date(h.timestamp) > thirtyDaysAgo).length;
 ...
 */
export function recordWordVoiceMismatch(userId, data) {
    let profile = profiles.get(userId);
    if (!profile) {
        profile = createEmptyProfile(userId);
        profiles.set(userId, profile);
 ...
 */
export function recordCyclicalPattern(userId, data) {
    let profile = profiles.get(userId);
    if (!profile) {
        profile = createEmptyProfile(userId);
        profiles.set(userId, profile);
 ...
 */
export function getPatternToSurface(userId) {
    ...
    const recentMismatches = profile.wordVoiceMismatches.filter((m) => !m.surfacedToUser && Date.now() - new Date(m.timestamp).getTime() < 7 * 24 * 60 * 60 * 1000);
 ...
 */
export function markInsightSurfaced(userId, insightType, identifier, // topic or patternId
userReaction) {
    const profile = profiles.get(userId);
    if (!profile)
        return;
    if (insightType === 'faded_energizer') {
        ...
        for (const m of profile.wordVoiceMismatches) {
            if (m.topic === identifier)
                m.surfacedToUser = true;
        }
    }
}
// ============================================================================
 ...
 */
export function buildPatternMirrorContext(userId) {
    const profile = profiles.get(userId);
    if (!profile)
        return '';
    const sections = ['[PATTERN MIRROR - Unspoken Insights]'];
...
// PERSISTENCE
// ============================================================================
export async function savePatternProfile(userId) {
    const profile = profiles.get(userId);
    if (!profile)
        return;
    ...
    try {
        ...
        for (const pattern of profile.cyclicalPatterns.filter((p) => p.confidence >= 0.6)) {
            void onPatternInsightChange(userId, pattern.id, {
        ...
        for (const topic of [...profile.energizingTopics, ...profile.drainingTopics].filter((t) => t.mentionCount >= 3)) {
            void onBehavioralPatternChange(userId, `topic_${topic.topic.replace(/\s+/g, '_')}`, {
...
export async function loadPatternProfile(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
...
export function getPatternProfile(userId) {
...
//# sourceMappingURL=pattern-mirror.js.map