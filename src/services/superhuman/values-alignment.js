...
import { onValuesConflictDetected, } from '../outreach/superhuman-outreach-bridge.js';
...
// VALUE DETECTION
// ============================================================================
export function detectValue(transcript) {
...
export function detectConflict(transcript, userValues) {
...
export async function loadUserValues(userId) {
...
export async function saveValue(value) {
    const db = getFirestoreDb();
    if (db) {
        await db
            .collection('bogle_users')
    ...
    indexValuesAlignment(value.userId, {
        id: value.id,
...
export async function recordValueMention(userId, detected) {
...
// CONTEXT BUILDING
// ============================================================================
export async function buildValuesContext(userId) {
    const values = await loadUserValues(userId);
    if (values.length === 0) {
        return '';
    }
    const sections = ['[VALUES ALIGNMENT - Better Than Human Value Tracking]'];
    ...
    if (conflictingValues.length > 0) {
        ...
        for (const value of conflictingValues.slice(0, 3)) {
            ...
            sections.push(`• ${value.category}: Conflict detected ${daysAgo} days ago. Worth exploring gently.`);
        }
    }
...
//# sourceMappingURL=values-alignment.js.map