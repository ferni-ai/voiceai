...
async function loadGuestIntelligence(userId) {
...
async function saveGuestIntelligence(userId, profile) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export async function getGuestProfile(userId, guestName) {
    ...
    for (const guest of Object.values(intelligence.guests)) {
        if (guest.name.toLowerCase() === normalizedName ||
 ...
 */
export async function upsertGuestProfile(userId, guestName, updates) {
    ...
    if (existing) {
        ...
        const filterStrings = (arr, fallback) => (arr?.filter((s) => s !== undefined) ?? fallback);
        const updated = {
        ...
        const toStringArray = (arr) => arr?.filter((s) => s !== undefined) ?? [];
        const newGuest = {
            name: normalizedName,
 ...
 */
export async function recordGuestDietary(userId, guestName, dietary) {
    await upsertGuestProfile(userId, guestName, { dietary });
}
/**
 * Record accessibility needs for a guest
 */
export async function recordGuestAccessibility(userId, guestName, accessibility) {
    await upsertGuestProfile(userId, guestName, { accessibility });
}
/**
 * Record a relationship between two guests
 */
export async function recordGuestRelationship(userId, person1, person2, type, strength, context) {
    const intelligence = (await loadGuestIntelligence(userId)) || createDefaultProfile(userId);
    // Check if relationship exists (either direction)
    const existingIdx = intelligence.relationships.findIndex((r) => (r.person1.toLowerCase() === person1.toLowerCase() &&
 ...
 */
export async function upsertGuestGroup(userId, groupName, members, type, notes) {
    const intelligence = (await loadGuestIntelligence(userId)) || createDefaultProfile(userId);
    const existingIdx = intelligence.groups.findIndex((g) => g.name.toLowerCase() === groupName.toLowerCase());
    const group = { name: groupName, members, type, notes };
    if (existingIdx >= 0) {
 ...
 */
export async function getSeatingRecommendations(userId, guestList) {
    const intelligence = await loadGuestIntelligence(userId);
    if (!intelligence)
        return [];
    ...
    for (const guestName of guestList) {
        ...
        for (const rel of intelligence.relationships) {
            const isInvolved = rel.person1.toLowerCase() === guestName.toLowerCase() ||
                rel.person2.toLowerCase() === guestName.toLowerCase();
            if (!isInvolved)
                continue;
            const otherPerson = rel.person1.toLowerCase() === guestName.toLowerCase() ? rel.person2 : rel.person1;
            // Only include if other person is also on guest list
            if (!guestList.some((g) => g.toLowerCase() === otherPerson.toLowerCase()))
                continue;
        ...
        for (const group of intelligence.groups) {
            if (group.members.some((m) => m.toLowerCase() === guestName.toLowerCase())) {
                const otherMembers = group.members.filter((m) => m.toLowerCase() !== guestName.toLowerCase() &&
 ...
 */
export async function getGuestListDietary(userId, guestList) {
    const intelligence = await loadGuestIntelligence(userId);
    if (!intelligence) {
    ...
    for (const guestName of guestList) {
        ...
        if (restrictions.includes('vegetarian'))
            result.vegetarian.push(guestName);
        if (restrictions.includes('vegan'))
            result.vegan.push(guestName);
        ...
        const otherRestrictions = restrictions.filter((r) => !['vegetarian', 'vegan', 'gluten-free', 'gluten free', 'celiac'].includes(r));
 ...
 */
export async function predictAttendance(userId, guestList) {
    const intelligence = await loadGuestIntelligence(userId);
    if (!intelligence) {
        return {
 ...
 */
export async function getGuestListSummary(userId, guestList) {
    ...
    if (dietary.vegetarian.length > 0)
        dietaryItems.push(`${dietary.vegetarian.length} vegetarian`);
    if (dietary.vegan.length > 0)
        dietaryItems.push(`${dietary.vegan.length} vegan`);
    if (dietary.glutenFree.length > 0)
        dietaryItems.push(`${dietary.glutenFree.length} gluten-free`);
    if (dietary.allergies.length > 0)
        dietaryItems.push(`${dietary.allergies.length} with allergies`);
 ...
 */
export async function buildGuestIntelligenceContext(userId, currentGuestList) {
...
//# sourceMappingURL=guest-intelligence.js.map