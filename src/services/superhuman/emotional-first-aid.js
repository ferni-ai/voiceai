...
export function detectCrisis(transcript) {
...
export function getFirstAidResponse(level) {
    const protocol = PROTOCOL_BY_LEVEL[level];
    const technique = protocol.techniques[0]; // Start with first technique
    const followUps = {
        safety: "I'm not going anywhere. Do you have someone you can call right now? A crisis line is 988 if you need it.",
        containing: "You're doing hard work right now. I'm proud of you for staying with me. What do you need right now?",
        stabilizing: 'That was intense. You came through it. <pause> What would feel supportive right now?',
        calming: 'Better? <pause> Panic passes. It always does. You did the hard part - you stayed present.',
...
// ============================================================================
export function getVoiceInstructions(response) {
...
// CONTEXT BUILDING
// ============================================================================
export function buildFirstAidContext(crisis) {
    const response = getFirstAidResponse(crisis.severity);
    const sections = ['[⚠️ EMOTIONAL FIRST AID ACTIVATED]'];
...
//# sourceMappingURL=emotional-first-aid.js.map