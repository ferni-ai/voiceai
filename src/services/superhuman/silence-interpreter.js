 ...
 */
export function analyzeSilence(durationMs, context) {
...
function buildSilenceResponse(type, duration, voiceMarkers, confidence) {
 ...
 */
export async function recordSilenceOutcome(userId, analysis, outcome) {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            void onEmotionalPatternChange(userId, `silence_${Date.now()}`, {
 ...
 */
export async function loadSilenceProfile(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
 ...
 */
export async function updateBaselineTolerance(userId) {
    ...
    const comfortableSilences = profile.silenceHistory.filter((s) => s.wasHelpful !== false && s.type !== 'uncomfortable');
    if (comfortableSilences.length < 5)
        return;
    const avgDuration = comfortableSilences.reduce((sum, s) => sum + s.duration, 0) / comfortableSilences.length;
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export function buildSilenceGuidance(analysis) {
    if (!analysis)
        return '';
    const guidance = {
        processing: 'SILENCE DETECTED: Processing type. Give them time to think. Do NOT fill this silence.',
        emotional: 'SILENCE DETECTED: Emotional type. Words may be hard right now. Simply acknowledge presence.',
        uncomfortable: 'SILENCE DETECTED: Something unspoken. Gently create space for sharing without pressure.',
        invitational: 'SILENCE DETECTED: They want you to go deeper. Follow their lead.',
        exhausted: 'SILENCE DETECTED: They need rest, not more words. Offer to just be present quietly.',
        contemplative: "SILENCE DETECTED: Contemplative moment. Honor it. Don't rush. Let the moment breathe.",
    };
    const sections = [
        '[SILENCE INTERPRETER]',
 ...
 */
export async function buildSilenceContext(userId) {
    const profile = await loadSilenceProfile(userId);
    if (!profile)
        return '';
    const sections = ['[SILENCE AWARENESS]'];
    // User's typical tolerance
    sections.push(`This user's comfortable pause threshold: ${(profile.baselinePauseTolerance / 1000).toFixed(1)}s`);
    // Recent patterns
    const recentEmotional = profile.silenceHistory.filter((s) => s.type === 'emotional' &&
        Date.now() - new Date(s.timestamp).getTime() < 7 * 24 * 60 * 60 * 1000).length;
...
//# sourceMappingURL=silence-interpreter.js.map