...
export interface EventLearning {
  ...
  budgetLearnings: {
    plannedTotal: number;
    actualTotal: number;
    surpriseCosts: Array<{ item: string; amount: number }>;
  ...
  /** Timing learnings */
  timingLearnings: {
    leadTimeUsed: string; // e.g., "3 months"
...
export interface PostEventLearningProfile {
  ...
  accumulatedWisdom: Array<{
    ...
    eventType: string;
    wisdom: string;
    sourceCount: number; // How many events contributed to this
  }>;
  lastUpdated: string;
}
...
const FOLLOW_UP_SCHEDULE: FollowUpPrompt[] = [
  {
    stage: 'immediate',
    daysAfterEvent: 1,
    questions: [
...
async function loadLearningProfile(userId: string): Promise<PostEventLearningProfile | null> {
 ...
 */
export async function scheduleEventFollowUps(
  userId: string,
  eventId: string,
  eventName: string,
  eventDate: string,
...
export async function recordLearning(
  ...
  eventId: string,
  stage: 'immediate' | 'one_week' | 'one_month',
  learning: Partial<EventLearning>
): Promise<void> {
  const profile = (await loadLearningProfile(userId)) || createDefaultProfile(userId);
 ...
 */
export async function getApplicableLearnings(
  userId: string,
  eventType: string
): Promise<AppliedWisdom[]> {
  const profile = await loadLearningProfile(userId);
  if (!profile) return [];

  const applicable: AppliedWisdom[] = [];

  // Get wisdom for this event type
  const relevantWisdom = profile.accumulatedWisdom.filter(
    (w) => w.eventType === eventType || w.eventType === 'all'
  );
  ...
  const similarEvents = profile.learnings.filter(
    (l) => l.eventType === eventType && l.overallSatisfaction >= 7
  );
 ...
 */
export async function buildPostEventLearningContext(
  userId: string,
  eventType?: string
): Promise<string> {
  const lines: string[] = [];
...
function createDefaultProfile(userId: string): PostEventLearningProfile {
  return {
    userId,
    learnings: [],
    pendingFollowUps: [],
...
async function distillWisdom(
  ...
  for (const worked of learning.whatWorked.filter((w) => w.importance === 'major')) {
    const existingWisdom = profile.accumulatedWisdom.find(
      (w) => w.category === worked.category && w.eventType === learning.eventType
    );
