...
import { onBlindSpotChange, } from '../../data-layer/hooks/coaching-hooks.js';
...
// CACHE
// ============================================================================
const blindSpotCache = new Map();
const gapCache = new Map();
const valuesCache = new Map();
const distortionCache = new Map();
// ============================================================================
 ...
 */
export async function recordBlindSpotEvidence(userId, evidence) {
    const blindSpots = await loadBlindSpots(userId);
    const now = new Date();
    // Find or create blind spot
    let blindSpot = blindSpots.find((b) => b.pattern.toLowerCase() === evidence.pattern.toLowerCase());
    if (blindSpot) {
 ...
 */
export async function getBlindSpots(userId) {
    const blindSpots = await loadBlindSpots(userId);
    return blindSpots.filter((b) => b.occurrences >= CONFIG.MIN_EVIDENCE_FOR_BLIND_SPOT && b.confidence >= 0.5);
}
/**
 * Get unsurfaced blind spots to potentially mention.
 */
export async function getUnsurfacedBlindSpots(userId) {
 ...
 */
export async function markBlindSpotSurfaced(userId, blindSpotId, response) {
    const blindSpots = await loadBlindSpots(userId);
    ...
    if (blindSpot) {
        ...
        if (response)
            blindSpot.userResponse = response;
        await saveBlindSpot(userId, blindSpot);
    }
}
// ============================================================================
 ...
 */
export async function recordSelfPerception(userId, statement, context) {
    ...
    const existing = gaps.find((g) => g.selfStatement.toLowerCase().includes(statement.toLowerCase().slice(0, 50)));
    if (existing) {
 ...
 */
export async function recordBehavior(userId, behavior, context) {
    const gaps = await loadGaps(userId);
    const now = new Date();
    // Find contradicting self-perceptions
    for (const gap of gaps) {
        if (gap.gapSeverity > 0)
 ...
 */
export async function getGaps(userId) {
    const gaps = await loadGaps(userId);
 ...
 */
export async function recordStatedValue(userId, value, context) {
 ...
 */
export async function recordValueBehavior(userId, value, behavior, aligned) {
 ...
 */
export async function getValuesAlignment(userId) {
 ...
 */
export async function getMisalignedValues(userId) {
 ...
 */
export async function recordDistortions(userId, text) {
 ...
 */
export async function getDistortionProfile(userId) {
 ...
 */
export async function formatSelfAwarenessContext(userId) {
...
// PERSISTENCE
// ============================================================================
async function loadBlindSpots(userId) {
...
async function saveBlindSpot(userId, blindSpot) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
        ...
        void onBlindSpotChange(userId, blindSpot.id, {
...
async function loadGaps(userId) {
...
async function saveGap(userId, gap) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
...
async function loadValuesAlignment(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
        ...
        return {
            ...data,
            statedValues: (data.statedValues ?? []).map((v) => ({
                ...v,
...
async function saveValuesAlignment(userId, alignment) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
...
async function loadDistortionProfile(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
...
async function saveDistortionProfile(userId, profile) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
        const distortionsObj = {};
...
// CACHE MANAGEMENT
// ============================================================================
export function clearSelfAwarenessCache(userId) {
    if (userId) {
