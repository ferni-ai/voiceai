 ...
 */
export async function recordSnapshot(userId, data) {
    const now = new Date();
    const snapshot = {
        timestamp: now,
...
async function computePatterns(userId) {
    ...
    // Topic counters
    const hourlyTopics = new Map();
    const dailyTopics = new Map();
    const seasonalThemes = new Map();
    // Process snapshots
    for (const snap of snapshots) {
        ...
        if (snap.topic) {
            if (!hourlyTopics.has(hour))
                hourlyTopics.set(hour, new Map());
        ...
        if (snap.topic) {
            if (!dailyTopics.has(day))
                dailyTopics.set(day, new Map());
        ...
        if (snap.topic) {
            if (!seasonalThemes.has(season))
                seasonalThemes.set(season, new Map());
 ...
 */
export async function getHourlyPattern(userId, hour) {
 ...
 */
export async function getDayPattern(userId, day) {
    await ensurePatterns(userId);
    const patterns = patternCache.get(userId);
    if (!patterns)
        return null;
 ...
 */
export async function getSeasonalPattern(userId, season) {
    await ensurePatterns(userId);
    const patterns = patternCache.get(userId);
    if (!patterns)
        return null;
 ...
 */
export async function getTemporalContext(userId) {
    const now = new Date();
 ...
 */
export async function detectAnomaly(userId, current) {
 ...
 */
export async function formatTemporalContext(userId) {
...
function computeReceptivity(hour, pattern) {
...
function computeStressLevel(pattern) {
...
function generateSeasonalWarnings(pattern) {
    const warnings = [];
...
async function ensurePatterns(userId) {
...
async function saveSnapshot(userId, snapshot) {
    ...
    try {
        ...
        if (snapshot.dominantEmotion || snapshot.energyLevel) {
            ...
            void onSeasonalPatternChange(userId, `temporal_${id}`, {
...
// CACHE MANAGEMENT
// ============================================================================
export function clearTemporalCache(userId) {
    if (userId) {
 ...
 */
export async function getPatternPrediction(userId) {
    const now = new Date();
    const hour = now.getHours();
    const day = now.getDay();
    const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    try {
        ...
        const energyValues = [];
        if (hourPattern)
            energyValues.push(hourPattern.energyLevel);
        if (dayPattern)
            energyValues.push(dayPattern.energyLevel);
        if (seasonPattern)
            energyValues.push(seasonPattern.energyBaseline);
...
//# sourceMappingURL=temporal-patterns.js.map