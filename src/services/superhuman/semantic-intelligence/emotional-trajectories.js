 ...
 */
export async function recordEmotionalWaypoint(userId, waypoint) {
    ...
    if (matchingArc) {
        ...
        log.debug({ userId, arcId: matchingArc.id, theme: matchingArc.theme }, '📈 Waypoint added to existing arc');
        return matchingArc;
    }
 ...
 */
async function findMatchingArc(arcs, waypoint, waypointEmbedding) {
    // Only match with active (non-resolved) arcs
    const activeArcs = arcs.filter((arc) => arc.phase !== 'resolved' &&
    ...
    for (const arc of activeArcs) {
        ...
        if (waypoint.emotion.toLowerCase() === arc.theme.toLowerCase() ||
 ...
 */
function addWaypointToArc(arc, waypoint) {
    // Add waypoint
 ...
 */
function createNewArc(userId, waypoint) {
    const now = Date.now();
 ...
 */
function updateArcNarrative(arc) {
 ...
 */
function detectTurningPoint(arc, waypoint) {
    const waypoints = arc.waypoints;
    ...
    if (Math.abs(recentValence - olderValence) > 0.4) {
        arc.turningPoint = {
            timestamp: waypoint.timestamp,
            description: recentValence > olderValence
 ...
 */
export async function getActiveArcs(userId) {
    const arcs = arcCache.get(userId) || (await loadArcs(userId));
    return arcs.filter((arc) => arc.phase !== 'resolved' &&
        Date.now() - arc.lastUpdated < CONFIG.ARC_TIMEOUT_DAYS * 24 * 60 * 60 * 1000);
}
/**
 * Get arcs relevant to current emotional context.
 */
export async function getRelevantArcs(userId, currentEmotion, currentTopic) {
    ...
    const scored = activeArcs.map((arc) => {
        ...
        if (currentEmotion) {
            ...
            const emotionMatch = arc.waypoints.some((w) => w.emotion.toLowerCase() === currentEmotion.toLowerCase());
            if (emotionMatch)
                relevance += 0.3;
        }
        if (currentTopic) {
            ...
            // Check triggers
            const triggerMatch = arc.waypoints.some((w) => w.trigger?.toLowerCase().includes(currentTopic.toLowerCase()));
            if (triggerMatch)
                relevance += 0.3;
        }
        // Boost for intensity
 ...
 */
export async function buildEmotionalTrajectoryContext(userId, currentContext) {
...
// PERSISTENCE
// ============================================================================
async function loadArcs(userId) {
    ...
    try {
        const snapshot = await db
            ...
            .get();
        const arcs = snapshot.docs.map((doc) => doc.data());
        arcCache.set(userId, arcs);
        return arcs;
    }
    catch (error) {
...
async function saveArc(userId, arc) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
        ...
        void onEmotionalPatternChange(userId, `arc_${arc.id}`, {
 ...
 */
export async function getEmotionalContext(userId) {
    try {
        const activeArcs = await getActiveArcs(userId);
        ...
        const avgIntensity = rankedArcs.reduce((sum, { arc }) => {
...
//# sourceMappingURL=emotional-trajectories.js.map