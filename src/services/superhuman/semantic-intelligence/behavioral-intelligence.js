 ...
 */
export async function recordPotentialSabotage(userId, instance) {
 ...
 */
export async function getSabotagePatterns(userId) {
    const patterns = await loadPatterns(userId);
    return patterns
 ...
 */
export async function getUnsurfacedPatterns(userId) {
 ...
 */
export async function updateBaseline(userId, data) {
 ...
 */
export async function checkBaselineDeviation(userId, current) {
 ...
 */
export async function recordTrigger(userId, trigger) {
    const triggers = await loadTriggers(userId);
    const now = new Date();
    // Find existing trigger
    let existing = triggers.find((t) => t.triggerPattern.toLowerCase() === trigger.pattern.toLowerCase());
    if (existing) {
 ...
 * Get known triggers.
 */
export async function getTriggers(userId) {
    const triggers = await loadTriggers(userId);
 ...
 */
export async function checkForTriggers(userId, text) {
 ...
 */
export async function formatBehavioralContext(userId, currentContext) {
...
// HELPERS
// ============================================================================
function calculateFrequency(instances) {
...
function calculateSensitivity(instances) {
...
async function savePattern(userId, pattern) {
    ...
    try {
        ...
        void onBehavioralPatternChange(userId, pattern.id, {
...
async function loadBaseline(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
...
async function saveBaseline(userId, baseline) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
...
async function saveTrigger(userId, trigger) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
        ...
        void onPatternInsightChange(userId, trigger.id, {
            ...
            significance: trigger.sensitivity >= 0.8
                ? 'critical'
...
// CACHE MANAGEMENT
// ============================================================================
export function clearBehavioralCache(userId) {
    if (userId) {
