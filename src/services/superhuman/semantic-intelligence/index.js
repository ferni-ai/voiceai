...
import { correlationMining, recordObservation, buildCorrelationContext, getRelevantCorrelations, } from './correlation-mining.js';
import { emotionalTrajectories, recordEmotionalWaypoint, buildEmotionalTrajectoryContext, getActiveArcs, } from './emotional-trajectories.js';
import { relationalSemantics, recordPersonMention, buildRelationalContext, getImpactfulRelationships, } from './relational-semantics.js';
import { counterfactualMemory, recordDecisionPoint, buildCounterfactualContext, getPendingFollowUps, } from './counterfactual-memory.js';
import { growthFingerprint, recordConversationData, buildGrowthContext, getGrowthFingerprint, } from './growth-fingerprint.js';
import { crossSessionThreading, recordMoment, buildThreadingContext, getUnconsciousConnections, } from './cross-session-threading.js';
// V3.2+ imports
...
const metricsStore = new LRUCache({
 ...
 */
export function getMetricsSummary(name) {
    const samples = metricsStore.get(name);
...
export { processSemanticIntelligence, recordAgentAdvice, recordAdviceOutcome, detectAdviceOutcome, warmupSemanticIntelligence, } from './integration.js';
// Advice detection (for response tracking)
export { detectAdvice, trackAdviceInResponse, } from './advice-detector.js';
// Enhanced person extraction (V3.1)
export { extractPersons, getPrimaryPerson, getPrimaryPersonName, } from './person-extractor.js';
// Semantic advice matching (V3.1)
export { findMatchingAdvice, precomputeAdviceEmbeddings, clearAdviceEmbeddingCache, } from './advice-matcher.js';
// LLM-powered detection (Gemini Flash 2.0)
export { detectAdviceWithLLM, extractPersonsWithLLM, detectAdviceOutcomeWithLLM, detectAdviceHybrid, extractPersonsHybrid, clearLLMDetectorCache, resetLLMDetectorClient, getLLMDetectorStats, } from './llm-detector.js';
// ============================================================================
// V3.2 PROACTIVE INTELLIGENCE
// ============================================================================
// Insight Broker - proactive surfacing of insights
export { insightBroker, createInsight, getInsightsToSurface, markInsightSurfaced, dismissInsight, getPendingInsightCount, formatInsightsForPrompt, generateCorrelationInsight, generateTrajectoryInsight, generateCounterfactualInsight, generateGrowthInsight, generateThreadingInsight, clearInsightCache, } from './insight-broker.js';
// Open Loops - track things needing follow-up
export { openLoops, createOpenLoop, getLoopsReadyForFollowUp, markFollowedUp, resolveLoop, dismissLoop, getAllOpenLoops, getLoopsByType, detectOpenLoops, processUserTextForLoops, clearLoopCache, } from './open-loops.js';
// Ferni Commitments - track Ferni's promises
export { ferniCommitments, createCommitment, fulfillCommitment, checkAvoidanceViolation, getPendingCommitments, getRememberedThings, getAvoidanceTopics, getAllCommitments, detectCommitmentsInResponse, trackCommitmentsInResponse, formatCommitmentsForContext, clearCommitmentCache, } from './ferni-commitments.js';
// ============================================================================
// V3.3 RELATIONAL NETWORK
// ============================================================================
export { relationshipGraph, upsertPerson, findPersonByName, getAllPeople, getPeopleByRelationship, getPeopleByImpact, getMostMentioned, getRecentlyMentioned, recordConnection, getConnectionsForPerson, getConflicts, updateSupportScore, getTopSupporters, getGraphSummary, formatGraphForContext, clearGraphCache, } from './relationship-graph.js';
// ============================================================================
// V3.4 TEMPORAL INTELLIGENCE
// ============================================================================
export { temporalPatterns, recordSnapshot as recordTemporalSnapshot, getHourlyPattern, getDayPattern, getSeasonalPattern, getTemporalContext, detectAnomaly as detectTemporalAnomaly, formatTemporalContext, clearTemporalCache, } from './temporal-patterns.js';
// ============================================================================
// V3.5 BEHAVIORAL INTELLIGENCE
// ============================================================================
export { behavioralIntelligence, recordPotentialSabotage, getSabotagePatterns, getUnsurfacedPatterns, markPatternSurfaced, updateBaseline, getBaseline, checkBaselineDeviation, recordTrigger, getTriggers, checkForTriggers, formatBehavioralContext, clearBehavioralCache, } from './behavioral-intelligence.js';
// ============================================================================
// V3.6 COACHING INTELLIGENCE
// ============================================================================
export { coachingIntelligence, recordAdviceOutcome as recordCoachingOutcome, getEffectivenessProfile, getBestApproach, detectLearningStyleFromText, updateLearningStyle, getLearningStyle, recordDeflection, recordPushback, getResistancePattern, isTopicSensitive, getCoachingRecommendations, formatCoachingContext, clearCoachingCache, } from './coaching-intelligence.js';
// ============================================================================
// V3.7 SELF-AWARENESS INTELLIGENCE
// ============================================================================
export { selfAwareness, recordBlindSpotEvidence, getBlindSpots, getUnsurfacedBlindSpots, markBlindSpotSurfaced, recordSelfPerception, recordBehavior, getGaps, recordStatedValue, recordValueBehavior, getValuesAlignment, getMisalignedValues, detectDistortions, recordDistortions, getDistortionProfile, formatSelfAwarenessContext, clearSelfAwarenessCache, } from './self-awareness.js';
// ============================================================================
// UNIFIED RECORDING
// ============================================================================
/**
 ...
 */
export async function recordSemanticData(userId, data) {
    const { content, topics, emotion, emotionIntensity, emotionValence, personMentioned, personRelationship, personSentiment, adviceGiven, adviceContext, cognitivePattern, significance = 'medium', } = data;
    ...
    try {
        ...
        if (emotion) {
            correlationPromises.push(recordObservation(userId, {
                domain: 'emotion',
                pattern: emotion,
        ...
        if (emotion && emotionIntensity !== undefined) {
            correlationPromises.push(recordEmotionalWaypoint(userId, {
                emotion,
        ...
        if (personMentioned) {
            correlationPromises.push(recordPersonMention(userId, {
        ...
        if (adviceGiven) {
            correlationPromises.push(recordDecisionPoint(userId, {
                advice: adviceGiven,
                context: adviceContext || content.slice(0, 200),
            }).then(() => { }));
        }
        // 5. Growth Fingerprint - record conversation data
        correlationPromises.push(recordConversationData(userId, {
            topics,
            emotion,
            messageText: content,
            cognitivePattern,
        ...
        if (significance !== 'low') {
            correlationPromises.push(recordMoment(userId, {
                content,
                emotion,
                topic: topics?.[0],
                significance,
        ...
        log.debug({ userId, hasEmotion: !!emotion, hasPerson: !!personMentioned }, '🧠 Semantic data recorded');
    }
    catch (error) {
 ...
 */
export async function buildSemanticIntelligenceContext(userId, currentContext) {
    ...
    try {
        ...
        const [
        // V3.0 Core
        correlationCtx, trajectoryCtx, relationalCtx, counterfactualCtx, growthCtx, threadingCtx, 
        // V3.2 Proactive Intelligence
        proactiveInsights, openLoopsCtx, commitmentsCtx, 
        // V3.3 Relational Network
        graphCtx, 
        // V3.4 Temporal Intelligence
        temporalCtx, 
        ...
        coachingCtx, 
        // V3.7 Self-Awareness
        selfAwarenessCtx,] = await Promise.all([
            ...
            Promise.all([getPendingCommitments(userId), getAvoidanceTopics(userId)]).then(([commitments, avoidance]) => formatCommitmentsForContext(commitments, avoidance)),
            // V3.3 Relational Network
        ...
        // V3.0 Core Context
        if (correlationCtx)
            context.activeCorrelations = [correlationCtx];
        if (trajectoryCtx)
            context.emotionalArcs = [trajectoryCtx];
        if (relationalCtx)
            context.relationalInsights = [relationalCtx];
        if (counterfactualCtx)
            context.relevantPatterns = [counterfactualCtx];
        if (growthCtx)
            context.growthContext = growthCtx;
        if (threadingCtx)
            context.hiddenConnections = [threadingCtx];
        // V3.2+ New Capabilities
 ...
 */
export function formatSemanticIntelligenceContext(context) {
    const sections = [];
    // V3.0 Core sections
 ...
 */
export async function getSemanticIntelligenceSummary(userId) {
