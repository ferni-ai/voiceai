...
async function callLLM(prompt, cacheKey, parseResponse, timeout = getShortLLMTimeout()) {
 ...
 */
export async function detectAdviceWithLLM(text) {
    ...
    const result = await callLLM(prompt, cacheKey, parseAdviceResponse);
    if (result.success && result.data) {
        log.debug({ cached: result.cached, latencyMs: result.latencyMs, advice: result.data.containsAdvice }, '🎯 LLM advice detection');
        return result.data;
    }
...
function parsePersonResponse(text) {
    try {
        const jsonMatch = text.match(/\{[\s\S]*\}/);
        if (!jsonMatch)
            return null;
 ...
 */
export async function extractPersonsWithLLM(text) {
    ...
    // BUGFIX: Parameter order was reversed (prompt, cacheKey) not (cacheKey, prompt)
    const result = await callLLM(prompt, cacheKey, parsePersonResponse);
    if (result.success && result.data) {
        log.debug({ cached: result.cached, latencyMs: result.latencyMs, count: result.data.persons.length }, '👥 LLM person extraction');
...
function parseOutcomeResponse(text) {
    try {
        const jsonMatch = text.match(/\{[\s\S]*\}/);
        if (!jsonMatch)
            return null;
 ...
 */
export async function detectAdviceOutcomeWithLLM(userMessage, previousAdvice) {
    ...
    const prompt = OUTCOME_DETECTION_PROMPT.replace('{advice}', previousAdvice.slice(0, 200)).replace('{message}', userMessage.slice(0, 300));
    // BUGFIX: Parameter order was reversed (prompt, cacheKey) not (cacheKey, prompt)
    const result = await callLLM(prompt, cacheKey, parseOutcomeResponse);
    if (result.success && result.data) {
        log.debug({
 ...
 */
export async function detectAdviceHybrid(text) {
 ...
 */
export async function extractPersonsHybrid(text) {
    // Try regex first
 ...
 */
export function clearLLMDetectorCache() {
    cache.clear();
 ...
 */
export function resetLLMDetectorClient() {
    geminiClient = null;
}
/**
 * Get cache stats for monitoring.
 */
export function getLLMDetectorStats() {
    return {
        cacheSize: cache.size,
        circuitOpen: !llmCircuitBreaker.canRequest(),
    };
}
//# sourceMappingURL=llm-detector.js.map