...
export async function recordMoment(
  ...
  const threadMoment: ThreadMoment = {
    sessionId,
    timestamp,
    content,
    embedding,
 ...
 */
async function discoverThreadConnections(
  userId: string,
  newMoment: ThreadMoment,
  embedding: number[]
): Promise<SemanticThread | null> {
  ...
  const pastMoments = buffer.filter(
    (m) =>
      m.sessionId !== newMoment.sessionId && // Different session
      m.embedding // Has embedding
  );

  // Find similar past moments
  const similarMoments: Array<{ moment: ThreadMoment; similarity: number }> = [];

  for (const past of pastMoments.slice(-CONFIG.THREAD_DISCOVERY_DEPTH)) {
    if (!past.embedding) continue;
 ...
 */
function addMomentToThread(thread: SemanticThread, moment: ThreadMoment): void {
...
async function createThread(
  userId: string,
  currentMoment: ThreadMoment,
  connections: Array<{ moment: ThreadMoment; similarity: number }>,
  embedding: number[]
 ...
 */
async function generateTheme(contents: string[]): Promise<string> {
 ...
 */
function updateConnectionInsight(thread: SemanticThread): void {
 ...
 */
export async function getRelevantThreads(
  userId: string,
  context: {
 ...
 */
export async function markThreadSurfaced(
  userId: string,
  threadId: string,
  reaction?: 'resonated' | 'surprised' | 'dismissed' | 'explored'
): Promise<void> {
  const threads = await getThreads(userId);
 ...
 */
export async function buildThreadingContext(
  userId: string,
  currentContext?: {
    content?: string;
    topic?: string;
  ...
  sections.push(
    'Surface unconscious connections gently: "I notice something..." or "There\'s a thread here..."'
  );
...
async function loadThreads(userId: string): Promise<SemanticThread[]> {
  ...
  try {
    ...
    const threads = snapshot.docs.map((doc) => doc.data() as SemanticThread);
    threadCache.set(userId, threads);
    return threads;
  } catch (error) {
...
async function saveThread(userId: string, thread: SemanticThread): Promise<void> {
  const db = getFirestoreDb();
  if (!db) return;
