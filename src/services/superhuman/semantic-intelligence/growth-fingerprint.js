...
import { onGrowthEdgeChange, onCoachingInsightChange, } from '../../data-layer/hooks/coaching-hooks.js';
 ...
 */
export async function recordConversationData(userId, data) {
 ...
 */
function analyzeLanguage(metrics, text) {
 ...
 */
async function checkAndCreateSnapshot(userId) {
 ...
 */
async function createSnapshot(userId, fingerprint, pending) {
...
function buildSnapshot(pending) {
    ...
    const totalCognitive = problemSolvingSignals +
 ...
 */
function calculateGrowthMetrics(snapshots) {
    ...
    const languageMaturation = {
        questionToStatementShift: first.languagePatterns.questionRatio - last.languagePatterns.questionRatio,
        certaintyGrowth: last.languagePatterns.certaintyLevel - first.languagePatterns.certaintyLevel,
        futureOrientationGrowth: last.languagePatterns.futureOrientation - first.languagePatterns.futureOrientation,
    };
    // Cognitive growth
    const cognitiveGrowth = {
        problemSolvingImprovement: last.cognitivePatterns.problemSolvingRatio - first.cognitivePatterns.problemSolvingRatio,
        growthMindsetProgress: last.cognitivePatterns.growthMindsetSignals - first.cognitivePatterns.growthMindsetSignals,
        selfCompassionGrowth: last.cognitivePatterns.selfCompassionLevel - first.cognitivePatterns.selfCompassionLevel,
    };
    return {
 ...
 */
function calculateInitialGrowth(snapshot) {
    return {
 ...
 */
function detectSignificantShifts(fingerprint) {
    ...
    for (let i = 1; i < snapshots.length; i++) {
        ...
        if (emotionalShift >= CONFIG.SIGNIFICANT_SHIFT_THRESHOLD) {
            shifts.push({
                ...
                description: curr.emotionalRange > prev.emotionalRange
        ...
        const cognitiveShift = curr.cognitivePatterns.problemSolvingRatio - prev.cognitivePatterns.problemSolvingRatio;
        if (Math.abs(cognitiveShift) >= CONFIG.SIGNIFICANT_SHIFT_THRESHOLD) {
            shifts.push({
                timestamp: curr.timestamp,
                description: cognitiveShift > 0
        ...
        const futureShift = curr.languagePatterns.futureOrientation - prev.languagePatterns.futureOrientation;
        if (Math.abs(futureShift) >= CONFIG.SIGNIFICANT_SHIFT_THRESHOLD) {
            shifts.push({
                timestamp: curr.timestamp,
                description: futureShift > 0
 ...
 */
function generateGrowthNarrative(fingerprint) {
...
export async function getGrowthComparison(userId, weeksAgo) {
    ...
    if (Math.abs(emotionalChange) > 0.05) {
        changes.push(emotionalChange > 0
    ...
    const cognitiveChange = now.cognitivePatterns.problemSolvingRatio - then.cognitivePatterns.problemSolvingRatio;
    if (Math.abs(cognitiveChange) > 0.1) {
        changes.push(cognitiveChange > 0
 ...
 */
export async function buildGrowthContext(userId) {
    const fingerprint = await getGrowthFingerprint(userId);
...
// PERSISTENCE
// ============================================================================
async function loadFingerprint(userId) {
...
async function saveFingerprint(userId, fingerprint) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
        ...
        if (fingerprint.growth) {
            void onGrowthEdgeChange(userId, 'growth_fingerprint', {
                area: 'overall',
        ...
        if (fingerprint.significantShifts?.length) {
            for (const shift of fingerprint.significantShifts.slice(0, 3)) {
                void onCoachingInsightChange(userId, `shift_${shift.domain}`, {
 ...
 */
export function clearGrowthCache(userId) {
    if (userId) {
 ...
 */
export async function forceCreateSnapshot(userId) {
    const fingerprint = fingerprintCache.get(userId) || (await loadFingerprint(userId));
    const pending = pendingData.get(userId);
    if (pending) {
        await createSnapshot(userId, fingerprint, pending);
...
//# sourceMappingURL=growth-fingerprint.js.map