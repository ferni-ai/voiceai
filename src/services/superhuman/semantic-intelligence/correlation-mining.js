...
async function detectCorrelations(userId, newObs) {
    ...
    for (const otherObs of recentObs) {
        ...
        const existingCorr = findExistingCorrelation(correlations, newObs.domain, newObs.pattern, otherObs.domain, otherObs.pattern);
        if (existingCorr) {
 ...
 */
function findExistingCorrelation(correlations, domainA, patternA, domainB, patternB) {
    return correlations.find((c) => (c.domainA.type === domainA &&
 ...
 */
function updateCorrelation(correlation, obsA, obsB, similarity) {
    // Add co-occurrence
    const coOccurrence = {
 ...
 */
function createCorrelation(userId, obsA, obsB, similarity) {
    const now = Date.now();
    return {
 ...
 */
function inferInsightType(correlation) {
    ...
    if ((domainA.type === 'sleep' && domainB.type === 'energy') ||
 ...
 */
export async function getRelevantCorrelations(userId, context) {
    const correlations = correlationCache.get(userId) || (await loadCorrelations(userId));
    // Filter by confidence
    const confident = correlations.filter((c) => c.confidence >= 0.5 && c.strength >= CONFIG.MIN_STRENGTH);
    ...
    const scored = confident.map((corr) => {
        ...
        if (context.currentTopics) {
            for (const topic of context.currentTopics) {
                if (corr.domainA.pattern.toLowerCase().includes(topic.toLowerCase()) ||
 ...
 */
export async function buildCorrelationContext(userId, context) {
...
// PERSISTENCE
// ============================================================================
async function loadCorrelations(userId) {
    ...
    try {
        const snapshot = await db
            ...
            .get();
        const correlations = snapshot.docs.map((doc) => doc.data());
        correlationCache.set(userId, correlations);
        return correlations;
    }
    catch (error) {
...
async function saveCorrelations(userId, correlations) {
    ...
    try {
        ...
        const toSave = correlations.filter((c) => c.observationCount >= CONFIG.MIN_COOCCURRENCES || c.confidence >= 0.3);
        ...
        for (const corr of toSave.filter((c) => c.confidence >= 0.5)) {
            void onCorrelationInsightChange(userId, corr.id, {
                ...
                examples: corr.coOccurrences
                    ?.slice(0, 3)
                    .map((co) => co.contextSnippet || `${corr.domainA.pattern} + ${corr.domainB.pattern}`) || [],
 ...
 */
export function clearCorrelationCache(userId) {
    if (userId) {
...
//# sourceMappingURL=correlation-mining.js.map