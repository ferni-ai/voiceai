...
export type PersonRelationship =
  ...
  | 'extended_family' // aunt, uncle, cousin, grandparent
  | 'friend' // friend, buddy, bestie
  | 'romantic' // girlfriend, boyfriend, partner, ex
  | 'coworker' // colleague, coworker, boss, manager
  | 'professional' // doctor, therapist, lawyer, teacher
  | 'acquaintance' // neighbor, roommate
  | 'pet' // dog, cat, pet
  | 'other';
 ...
 */
const NAME_CONTEXT_PATTERNS: Array<{
  pattern: RegExp;
  nameGroup: number;
  confidence: number;
}> = [
 ...
 */
export function getPrimaryPerson(text: string): ExtractedPerson | null {
 ...
 */
export function getPrimaryPersonName(text: string): string | undefined {
...
function isLikelyName(word: string): boolean {
  ...
  if (/^[A-Z]+$/.test(word) && word.length > 2) return false;
 ...
 */
function inferRelationship(text: string, name: string): PersonRelationship | undefined {
  ...
  if (/\b(friend|buddy|pal)\b/.test(context)) return 'friend';
  if (/\b(boss|manager|coworker|colleague)\b/.test(context)) return 'coworker';
  if (/\b(wife|husband|spouse|partner|married)\b/.test(context)) return 'spouse';
  if (/\b(girlfriend|boyfriend|dating|date)\b/.test(context)) return 'romantic';
  if (/\b(sister|brother|sibling)\b/.test(context)) return 'sibling';
  if (/\b(doctor|therapist|lawyer)\b/.test(context)) return 'professional';
 ...
 */
function extractContextSnippet(text: string, matchIndex: number, matchLength: number): string {
