 ...
 */
export async function recordPersonMention(userId, mention) {
...
function findNodeByName(nodes, name) {
    ...
    return nodes.find((n) => n.name.toLowerCase() === nameLower || n.aliases.some((a) => a.toLowerCase() === nameLower));
}
/**
...
async function createNode(userId, data) {
    ...
    const topicAssociations = topics
        ? topics.map((topic) => ({
            topic,
            frequency: 1,
            sentiment,
 ...
 */
function updateEmotionalSignature(signature, emotion, context) {
    // Update primary emotions
    const existing = signature.primaryEmotions.find((e) => e.emotion.toLowerCase() === emotion.toLowerCase());
    if (existing) {
        existing.frequency++;
    }
 ...
 * Update topic associations.
 */
function updateTopicAssociations(associations, topics, sentiment) {
 ...
 */
function updateSupportLevel(node) {
...
function updateInsights(node) {
    ...
    if (node.valenceVariance > 0.4) {
        insights.push(`Your feelings about ${node.name} tend to fluctuate - sometimes positive, sometimes difficult`);
    }
 ...
 */
export async function getRelationalGraph(userId) {
    const nodes = nodeCache.get(userId) || (await loadNodes(userId));
 ...
 */
export async function getPersonInsights(userId, personName) {
    const nodes = nodeCache.get(userId) || (await loadNodes(userId));
 ...
 */
export async function getPeopleByContext(userId, context) {
    const nodes = nodeCache.get(userId) || (await loadNodes(userId));
    const { emotion, topic } = context;
    return nodes.filter((node) => {
        if (emotion) {
            const hasEmotion = node.emotionalSignature.primaryEmotions.some((e) => e.emotion.toLowerCase() === emotion.toLowerCase());
            if (hasEmotion)
                return true;
        }
        if (topic) {
            const hasTopic = node.topicAssociations.some((t) => t.topic.toLowerCase().includes(topic.toLowerCase()));
            if (hasTopic)
                return true;
        }
        return false;
 ...
 */
export async function getImpactfulRelationships(userId) {
    const nodes = nodeCache.get(userId) || (await loadNodes(userId));
 ...
 */
export async function buildRelationalContext(userId, currentContext) {
    const nodes = nodeCache.get(userId) || (await loadNodes(userId));
 ...
 */
export async function recordConnection(userId, connection) {
    const { person1, person2, connectionType = 'unknown', sentiment = 0 } = connection;
    const edges = edgeCache.get(userId) || (await loadEdges(userId));
    // Find or create edge
    let edge = edges.find((e) => (e.fromPerson === person1 && e.toPerson === person2) ||
...
async function saveNode(userId, node) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
...
async function saveEdges(userId, edges) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export function clearRelationalCache(userId) {
    if (userId) {
...
//# sourceMappingURL=relational-semantics.js.map