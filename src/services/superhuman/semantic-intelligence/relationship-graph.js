...
export async function findPersonByName(userId, name) {
    ...
    const exactMatch = nodes.find((n) => n.name.toLowerCase() === lowerName || n.aliases.includes(lowerName));
    if (exactMatch)
        return exactMatch;
 ...
 */
export async function getAllPeople(userId) {
    return loadNodes(userId);
}
/**
 * Get people by relationship type.
 */
export async function getPeopleByRelationship(userId, relationship) {
    const nodes = await loadNodes(userId);
 ...
 */
export async function getPeopleByImpact(userId) {
    const nodes = await loadNodes(userId);
    return {
        energizing: nodes
 ...
 */
export async function getMostMentioned(userId, limit = 5) {
    const nodes = await loadNodes(userId);
 ...
 */
export async function getRecentlyMentioned(userId, days = 7) {
    const nodes = await loadNodes(userId);
...
export async function recordConnection(userId, connection) {
    ...
    let existing = connections.find((c) => (c.personA === nodeA.id && c.personB === nodeB.id) ||
 ...
 */
export async function getConnectionsForPerson(userId, personName) {
 ...
 */
export async function updateSupportScore(userId, personName, domain, wasSupport) {
 ...
 * Get top supporters.
 */
export async function getTopSupporters(userId, limit = 5) {
    const nodes = await loadNodes(userId);
    return nodes
 ...
 */
export async function getGraphSummary(userId) {
    const nodes = await loadNodes(userId);
    const connections = await loadConnections(userId);
 ...
 */
export async function formatGraphForContext(userId, currentPerson) {
    const nodes = await loadNodes(userId);
    if (nodes.length === 0)
        return '';
    const impact = await getPeopleByImpact(userId);
...
async function saveNode(userId, node) {
    ...
    try {
        ...
        void onRelationshipNetworkChange(userId, node.id, {
            person: node.name,
            relationship: node.type || 'unknown',
            connectionStrength: node.interactionCount && node.interactionCount > 10 ? 'core' : 'moderate',
...
// CACHE MANAGEMENT
// ============================================================================
export function clearGraphCache(userId) {
    if (userId) {
