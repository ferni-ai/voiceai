...
const loopCache = new Map<string, OpenLoop[]>();
 ...
 */
export async function processUserTextForLoops(
  userId: string,
  userText: string,
  context: {
    emotion?: string;
...
function generateFollowUpText(loop: OpenLoop): string | null {
  switch (loop.type) {
    case 'intention':
...
async function loadOpenLoops(userId: string): Promise<OpenLoop[]> {
  // Check cache
  const cached = loopCache.get(userId);
...
export function clearLoopCache(userId?: string): void {
  if (userId) {
