 ...
 */
export async function findMatchingAdvice(
  userText: string,
  pastAdvice: PastAdvice[]
): Promise<AdviceMatch | null> {
 ...
 */
function topicMatch(userText: string, pastAdvice: PastAdvice[]): AdviceMatch[] {
  const matches: AdviceMatch[] = [];
...
const adviceEmbeddingCache = new Map<string, number[]>();
 ...
 */
export function clearAdviceEmbeddingCache(): void {
