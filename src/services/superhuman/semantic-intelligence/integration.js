...
import { recordObservation } from './correlation-mining.js';
import { recordEmotionalWaypoint } from './emotional-trajectories.js';
import { recordPersonMention } from './relational-semantics.js';
import { recordDecisionPoint } from './counterfactual-memory.js';
import { recordConversationData } from './growth-fingerprint.js';
import { recordMoment } from './cross-session-threading.js';
...
import { extractPersons } from './person-extractor.js';
import { findMatchingAdvice, precomputeAdviceEmbeddings, } from './advice-matcher.js';
...
import { processUserTextForLoops } from './open-loops.js';
import { trackCommitmentsInResponse } from './ferni-commitments.js';
// V3.3 Relational Network
import { upsertPerson } from './relationship-graph.js';
// V3.4 Temporal Intelligence
import { recordSnapshot as recordTemporalSnapshot } from './temporal-patterns.js';
// V3.5 Behavioral Intelligence
import { recordPotentialSabotage, updateBaseline, } from './behavioral-intelligence.js';
// V3.6 Coaching Intelligence
import { updateLearningStyle, } from './coaching-intelligence.js';
// V3.7 Self-Awareness
import { recordDistortions, recordSelfPerception } from './self-awareness.js';
import { cleanForFirestore } from '../../../utils/firestore-utils.js';
// V4.0 Superhuman Outreach Intelligence - ALL SIGNAL GENERATORS
import { accumulateSignal, 
// Core signals
signalFromCapacity, signalFromOpenLoop, signalFromValuesConflict, signalFromVoiceDistress, 
// BTH V1 signals
signalFromLifeChapter, signalFromContradiction, signalFromVagueEmotion, } from '../../conversation-thread/superhuman-outreach-intelligence.js';
import { detectEnergyLevel, detectOvercommitment } from '../capacity-guardian.js';
import { detectConflict, loadUserValues } from '../values-alignment.js';
 ...
 */
export async function processSemanticIntelligence(data) {
    ...
    await Promise.allSettled([
        ...
        recordCorrelationData(data).catch((e) => log.warn({ error: String(e), userId }, 'Correlation mining error')),
        // 2. Feed emotional trajectories with emotion data
        recordEmotionalTrajectoryData(data).catch((e) => log.warn({ error: String(e), userId }, 'Emotional trajectory error')),
        // 3. Feed relational semantics with person mentions
        recordRelationalData(data).catch((e) => log.warn({ error: String(e), userId }, 'Relational semantics error')),
        // 4. Feed growth fingerprint with linguistic patterns
        recordGrowthData(data).catch((e) => log.warn({ error: String(e), userId }, 'Growth fingerprint error')),
        // 5. Feed cross-session threading with topics
        recordThreadingData(data).catch((e) => log.warn({ error: String(e), userId }, 'Cross-session threading error')),
        // 6. Detect advice outcome (counterfactual memory)
        detectAdviceOutcome(userId, userText).catch((e) => log.warn({ error: String(e), userId }, 'Advice outcome detection error')),
        // V3.2 Proactive Intelligence
        ...
        recordRelationshipGraphData(data).catch((e) => log.warn({ error: String(e), userId }, 'Relationship graph error')),
        // V3.4 Temporal Intelligence
        ...
        updateLearningStyle(userId, userText).catch((e) => log.warn({ error: String(e), userId }, 'Learning style error')),
        // V3.7 Self-Awareness
        // 13. Record cognitive distortions
        recordDistortions(userId, userText).catch((e) => log.warn({ error: String(e), userId }, 'Distortion detection error')),
        // 14. Record self-perception statements
        recordSelfPerception(userId, userText, userText.slice(0, 200)).catch((e) => log.warn({ error: String(e), userId }, 'Self-perception error')),
        // V4.0 Superhuman Outreach Intelligence
        // Accumulate signals for intelligent group outreach decisions
        accumulateOutreachSignals(userId, data).catch((e) => log.warn({ error: String(e), userId }, 'Outreach signal accumulation error')),
    ]);
    log.debug({ userId, topic, turnNumber: data.turnNumber }, '🧠 Semantic intelligence V4.0 turn processed');
}
/**
...
function getEmotionValence(emotion) {
    ...
    if (positive.some((e) => lower.includes(e)))
        return 1;
    if (negative.some((e) => lower.includes(e)))
        return -1;
    return 0;
}
// ============================================================================
 ...
 */
async function accumulateOutreachSignals(userId, data) {
    const { userText, textEmotion, textEmotionIntensity, voiceEmotion, voiceEmotionConfidence, speechRate, energy, breathiness, } = data;
    const signalsAccumulated = [];
    // =========================================================================
    ...
    try {
        ...
        if (userValues.length > 0) {
            ...
            if (conflict) {
                const statedValue = userValues.find((v) => v.id === conflict.valueId);
                if (statedValue) {
                    accumulateSignal(userId, signalFromValuesConflict({
    ...
    if (voiceEmotion && voiceEmotionConfidence && voiceEmotionConfidence > 0.5) {
        const isDistressEmotion = ['distressed', 'anxious', 'fearful', 'crying', 'panicked'].includes(voiceEmotion.toLowerCase());
        const hasHighArousal = (energy || 0) > 0.7;
        if (isDistressEmotion ||
    ...
    try {
        ...
        if (contradiction && contradiction.detected && contradiction.emotions.length >= 2) {
            const signal = signalFromContradiction({
                emotions: [contradiction.emotions[0], contradiction.emotions[1]],
    ...
    log.debug({ userId, signalCount: signalsAccumulated.length, signals: signalsAccumulated }, '🧠 Outreach signals accumulated (core + text-based)');
}
/**
 * Record data for relationship graph (V3.3)
 */
async function recordRelationshipGraphData(data) {
 ...
 */
async function recordCorrelationData(data) {
    const { userId, userText, topic, textEmotion, textEmotionIntensity } = data;
 ...
 */
async function recordEmotionalTrajectoryData(data) {
    const { userId, textEmotion, textEmotionIntensity, voiceEmotion, voiceEmotionConfidence, topic, userText, } = data;
    // Determine primary emotion (prefer voice if confident)
    const emotion = voiceEmotionConfidence && voiceEmotionConfidence > 0.6 ? voiceEmotion : textEmotion;
 ...
 */
async function recordRelationalData(data) {
    ...
    if (enhancedPersons.length === 0 || maxRegexConfidence < 0.6) {
        try {
            ...
            if (llmPersons.length > 0) {
                log.debug({ regexCount: enhancedPersons.length, llmCount: llmPersons.length }, '🤖 LLM hybrid extraction found additional persons');
    ...
    const legacyMentions = extractPersonMentions(userText, textEmotion ?? 'neutral', textEmotionIntensity ?? 0.5);
 ...
 */
async function recordGrowthData(data) {
 ...
 */
async function recordThreadingData(data) {
    ...
    const isSignificant = (textEmotionIntensity && textEmotionIntensity > 0.6) ||
        topic !== undefined ||
        userText.length > 100;
    if (!isSignificant) {
        return;
 ...
 */
export async function recordAgentAdvice(advice) {
 ...
 */
export async function trackFerniCommitments(userId, responseText, context) {
    if (!userId || userId === 'anonymous') {
        return;
    }
    try {
 ...
 */
export async function recordAdviceOutcome(userId, adviceId, outcome) {
    if (!userId || userId === 'anonymous') {
        return;
    }
    try {
 ...
 */
export async function detectAdviceOutcome(userId, userText) {
    if (!userId || userId === 'anonymous') {
        return;
    }
    ...
    try {
        ...
        const pastAdviceList = pendingAdvice.map((a) => ({
            id: a.id,
            adviceText: a.advice,
 ...
 */
function detectEmotionalCatalyst(text) {
    const catalysts = [
 ...
 */
function extractLinguisticMarkers(text) {
    const markers = [];
 ...
 */
export async function warmupSemanticIntelligence(userId) {
    if (!userId || userId === 'anonymous') {
        return;
    }
    try {
