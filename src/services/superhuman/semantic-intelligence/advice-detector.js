...
import { recordAgentAdvice } from './integration.js';
 ...
 */
export function detectAdvice(responseText) {
 ...
 * Extract the sentence containing the advice.
 */
function extractAdviceSentence(text, matchIndex) {
    // Find sentence boundaries
    const beforeMatch = text.slice(0, matchIndex);
    const afterMatch = text.slice(matchIndex);
    // Find start of sentence
    const lastSentenceEnd = Math.max(beforeMatch.lastIndexOf('.'), beforeMatch.lastIndexOf('!'), beforeMatch.lastIndexOf('?'), beforeMatch.lastIndexOf('\n'));
 ...
 */
export async function trackAdviceInResponse(responseText, context) {
    ...
    if (detection.confidence < 0.6) {
        log.debug({ userId, confidence: detection.confidence, pattern: detection.matchedPattern }, 'Low-confidence advice skipped');
        return;
    }
...
//# sourceMappingURL=advice-detector.js.map