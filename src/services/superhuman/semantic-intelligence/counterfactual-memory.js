 ...
 */
export async function recordDecisionPoint(userId, decision) {
 ...
 */
export async function recordFollowUp(userId, followUp) {
 ...
 */
export async function recordOutcome(userId, outcome) {
 ...
 */
function generateLesson(decision) {
...
async function updatePatterns(userId, newDecision) {
    ...
    if (similarDecisions.length >= CONFIG.MIN_POINTS_FOR_PATTERN - 1) {
        ...
        let pattern = patterns.find((p) => p.decisionPoints.includes(newDecision.id) ||
 ...
 */
function extractPatternDescription(decisions) {
...
function generatePatternInsight(pattern) {
    ...
    const followedTotal = followedOutcomes.positive + followedOutcomes.negative + followedOutcomes.neutral;
    const ignoredTotal = ignoredOutcomes.positive + ignoredOutcomes.negative + ignoredOutcomes.neutral;
...
export async function getPendingFollowUps(userId) {
    ...
    return decisions.filter((d) => d.pathTaken === 'unknown' &&
 ...
 */
export async function getRelevantPatterns(userId, context) {
    const patterns = patternCache.get(userId) || (await loadPatterns(userId));
 ...
 */
export async function findSimilarPastDecisions(userId, currentSituation) {
    const decisions = decisionCache.get(userId) || (await loadDecisionPoints(userId));
    let situationEmbedding = null;
    try {
 ...
 */
export async function buildCounterfactualContext(userId, currentContext) {
...
// PERSISTENCE
// ============================================================================
async function loadDecisionPoints(userId) {
    ...
    try {
        const snapshot = await db
            ...
            .get();
        const decisions = snapshot.docs.map((doc) => doc.data());
        decisionCache.set(userId, decisions);
        return decisions;
    }
    catch (error) {
...
async function saveDecisionPoint(userId, decision) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
        ...
        if (decision.outcome) {
            void onLifeLessonChange(userId, `decision_${decision.id}`, {
 ...
 */
export function clearCounterfactualCache(userId) {
    if (userId) {
...
//# sourceMappingURL=counterfactual-memory.js.map