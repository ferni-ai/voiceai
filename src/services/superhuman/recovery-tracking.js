 ...
 */
export async function startRecoveryTracking(userId, eventType, intensity, context) {
    ...
    try {
        ...
        void onRecoveryMilestoneChange(userId, docRef.id, {
 ...
 */
export async function markRecovered(userId, eventId, helpfulActions) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export async function loadRecoveryHistory(userId, daysBack = 180) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
 ...
 */
export async function getActiveRecoveryEvents(userId) {
 ...
 */
export async function buildRecoveryProfile(userId) {
 ...
 */
export async function getCheckInRecommendation(userId, eventType, eventTimestamp) {
    ...
    const expectedRecovery = recoveryData.sampleSize > 0 ? recoveryData.avgHours : DEFAULT_RECOVERY_TIMES[eventType];
 ...
 */
export async function buildRecoveryContext(userId) {
    ...
    for (const event of activeEvents.slice(0, 3)) {
        ...
        if (hoursSince < 2) {
            // Very recent
            sections.push(`⏳ Recent ${RECOVERY_DESCRIPTIONS[event.eventType]} (${hoursSince.toFixed(0)}h ago)\n` +
            ...
            sections.push(`🛑 Still recovering from ${RECOVERY_DESCRIPTIONS[event.eventType]} (${hoursSince.toFixed(0)}h ago)\n` +
                `   ${recommendation.message}\n` +
                `   Wait approximately ${recommendation.recommendedWaitHours} more hours.`);
        }
        else {
            sections.push(`✅ May be ready to talk about ${RECOVERY_DESCRIPTIONS[event.eventType]} (${hoursSince.toFixed(0)}h ago)\n` +
                `   ${recommendation.message}`);
        }
        sections.push('');
    }
...
//# sourceMappingURL=recovery-tracking.js.map