 ...
 */
export async function recordSharedMoment(userId, type, essence, context, triggerKeywords, callbackPhrase, resonance = 0.7) {
    ...
    try {
        ...
        void onInsideJokeChange(userId, docRef.id, {
 ...
 * Load shared moments.
 */
export async function loadSharedMoments(userId) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
 ...
 */
export async function recordMomentReference(userId, momentId) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export function findCallbackOpportunities(text, moments, currentMood = 'neutral') {
 ...
 */
export function detectPotentialMoment(text, context) {
    const textLower = text.toLowerCase();
 ...
 */
export function identifyRunningGags(moments) {
    return moments
 ...
 */
export async function buildInsideJokeContext(userId, currentText) {
    ...
    sections.push('These shared memories make your relationship feel real and lasting. ' +
 ...
 */
export function suggestCallback(opportunity) {
...
//# sourceMappingURL=inside-joke-memory.js.map