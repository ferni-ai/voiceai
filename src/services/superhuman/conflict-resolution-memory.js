...
export async function recordConflict(userId, conflict) {
    ...
    try {
        ...
        void onConflictMemoryChange(userId, docRef.id, {
        ...
        log.debug({ userId, withPerson: conflict.withPerson, type: conflict.conflictType }, 'Recorded conflict');
        return docRef.id;
    }
    catch (error) {
 ...
 */
export async function updateConflictResolution(userId, conflictId, updates) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export async function loadConflictHistory(userId, withPerson) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
 ...
 */
export function analyzeConflictPattern(conflicts) {
    ...
    const ineffectiveCounts = new Map();
    for (const c of conflicts) {
    ...
    const averageCooldown = cooldowns.length > 0 ? cooldowns.reduce((a, b) => a + b, 0) / cooldowns.length : 2;
 ...
 */
export async function getAllConflictPatterns(userId) {
 ...
 */
export async function getConflictRecommendations(userId, withPerson) {
    ...
    return {
        ...
        cooldownAdvice: pattern.averageCooldown > 12
 ...
 */
export async function buildConflictResolutionContext(userId, mentionedPerson) {
    const sections = [];
    // Get patterns
    ...
    if (mentionedPerson) {
        const personPattern = patterns.find((p) => p.person.toLowerCase() === mentionedPerson.toLowerCase());
        if (personPattern) {
...
//# sourceMappingURL=conflict-resolution-memory.js.map