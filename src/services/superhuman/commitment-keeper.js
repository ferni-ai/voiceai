...
import { validateCommitmentFeasibility, createCalendarBlocksForCommitment, buildCommitmentCalendarContext, } from './commitment-calendar-integration.js';
...
import { onCommitmentMade } from '../outreach/superhuman-outreach-bridge.js';
...
// DETECTION
// ============================================================================
export function detectCommitment(transcript, userId, context) {
...
export async function saveCommitment(commitment, options) {
    ...
    try {
        ...
        void onCommitmentKeeperChange(commitment.userId, id, {
        ...
        if (shouldCreateBlocks && feasibility?.suggestedSlots?.length) {
            try {
                const blocks = await createCalendarBlocksForCommitment(commitment.userId, fullCommitment, feasibility.suggestedSlots);
        ...
        if (commitment.targetDate) {
            try {
                await syncCommitmentToCalendar(commitment.userId, id, fullCommitment.summary, new Date(commitment.targetDate), {
        ...
        log.info({ userId: commitment.userId, commitmentId: id, type: fullCommitment.type }, 'Commitment saved');
...
export async function loadUserCommitments(userId) {
    ...
    try {
        ...
        const snapshot = await db
            ...
            .limit(50)
            .get();
        const commitments = snapshot.docs.map((doc) => doc.data());
...
export async function updateCommitmentStatus(userId, commitmentId, status, reaction) {
    try {
        const db = getFirestoreDb();
        if (db) {
            await db
        ...
        if (idx >= 0) {
            ...
            if (reaction)
                userCommitments[idx].userReactionToFollowUp = reaction;
 ...
 */
async function triggerCommitmentCelebration(userId, commitment) {
    try {
        const { onCommitmentMilestone } = await import('../conversation-thread/group-outreach-triggers.js');
...
export function generateFollowUp(commitment) {
    const now = Date.now();
...
export async function getFollowUpsForUser(userId) {
...
// CONTEXT INJECTION
// ============================================================================
export async function buildCommitmentContextForLLM(userId) {
    const commitments = await loadUserCommitments(userId);
    const followUps = await getFollowUpsForUser(userId);
    if (commitments.length === 0) {
        return '';
    }
    const sections = ['[COMMITMENT KEEPER - Better Than Human Accountability]'];
    ...
    if (active.length > 0) {
        ...
        for (const c of active.slice(0, 5)) {
            ...
            const calendarNote = c.feasibilityScore !== undefined && c.feasibilityScore < 50
    ...
    const atRiskCommitments = active.filter((c) => c.feasibilityScore !== undefined && c.feasibilityScore < 40);
    if (atRiskCommitments.length > 0) {
        sections.push('\n**⚠️ Calendar Conflicts:**');
        for (const c of atRiskCommitments) {
            sections.push(`• "${c.summary}" may be hard to fit in - consider helping them rescope or find time`);
        }
    }
 ...
 */
const ACTION_TO_COMMITMENT_KEYWORDS = {
 ...
 */
export async function findMatchingCommitment(userId, actionType, target, withinDays = 14) {
    try {
        const commitments = await loadUserCommitments(userId);
        if (commitments.length === 0)
            return null;
        ...
        for (const commitment of commitments) {
            // Skip if too old
            if (commitment.createdAt < cutoffTime)
                continue;
            // Skip if not active
            if (commitment.status !== 'active')
                continue;
        ...
        if (bestMatch) {
            log.debug({ userId, actionType, target, commitmentId: bestMatch.id, score: bestScore }, '🔗 Found matching commitment for action');
        }
        return bestMatch;
    }
    catch (error) {
...
//# sourceMappingURL=commitment-keeper.js.map