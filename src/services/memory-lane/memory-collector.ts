...
const SOURCE_TO_MEMORY_TYPE: Record<MemorySourceType, MemoryType> = {
...
const SOURCE_TO_DEFAULT_TONE: Record<MemorySourceType, EmotionalTone> = {
 ...
 */
export async function collectFromCommitments(userId: string): Promise<MemoryHighlight[]> {
 ...
 */
export async function collectFromDreams(userId: string): Promise<MemoryHighlight[]> {
  const memories: MemoryHighlight[] = [];
  ...
  try {
    ...
    const notable = dreams.filter(
      (d) => d.status === 'achieved' || (d.progressNotes && d.progressNotes.length > 0)
    );
 ...
 */
export async function collectFromSharedMoments(userId: string): Promise<MemoryHighlight[]> {
  const memories: MemoryHighlight[] = [];
 ...
 */
export async function collectFromMilestones(userId: string): Promise<MemoryHighlight[]> {
  const memories: MemoryHighlight[] = [];
 ...
 */
export async function collectFromCelebrations(userId: string): Promise<MemoryHighlight[]> {
  const memories: MemoryHighlight[] = [];
  ...
  try {
    ...
    if (!profile) return memories;

    // Filter to significant wins (hard difficulty or breakthrough type)
    const notable = profile.wins.filter(
      (w) => w.difficulty === 'hard' || w.type === 'breakthrough' || w.type === 'courage_moment'
    );
...
function createMemoryFromCommitment(
  userId: string,
  commitment: {
    id: string;
    summary: string;
...
function createMemoryFromDream(
  userId: string,
  dream: {
    id: string;
    title: string;
...
function createMemoryFromSharedMoment(
  userId: string,
  moment: {
    id: string;
    essence: string;
...
function createMemoryFromMilestone(
  userId: string,
  milestone: {
    id: string;
    title: string;
 ...
 */
export async function saveMemory(memory: MemoryHighlight): Promise<boolean> {
  const db = getFirestoreDb();
  if (!db) {
 ...
 */
export async function loadOnThisDayMemories(userId: string): Promise<MemoryHighlight[]> {
 ...
 */
export async function collectAllMemories(userId: string): Promise<{
  collected: number;
  saved: number;
  errors: string[];
}> {
  const errors: string[] = [];
  const allMemories: MemoryHighlight[] = [];
  ...
  log.info(
    { userId, collected: allMemories.length, saved, errors: errors.length },
 ...
 */
export async function processCollectionInput(
