 ...
 */
export function scoreMemory(memory, context, weights = WEIGHTS) {
    let score = 0;
 ...
 */
function calculateRecencyScore(memory, context) {
 ...
 */
function calculateAnniversaryScore(memory, context) {
 ...
 */
function calculateTopicRelevance(memory, currentTopic) {
    ...
    if (memory.topicTags.some((tag) => normalizedTopic.includes(tag.toLowerCase()) ||
 ...
 */
export async function getHighlights(userId, options = {}) {
    const { types, emotionalTones, personaId, topicTags, fromDate, toDate, isOnThisDay, excludeRecentlySurfaced = true, surfaceCooldownDays = DEFAULT_SURFACING_COOLDOWN_DAYS, excludeDismissed = true, limit = 20, cursor, sortBy = 'score', sortOrder = 'desc', } = options;
    // Load all memories
 ...
 */
export async function getOnThisDayHighlights(userId, options = {}) {
...
function filterMemories(memories, filters) {
    ...
    return memories.filter((memory) => {
        ...
        if (filters.topicTags && filters.topicTags.length > 0) {
            const hasMatch = filters.topicTags.some((tag) => memory.topicTags.some((mt) => mt.toLowerCase().includes(tag.toLowerCase())));
            if (!hasMatch)
                return false;
        }
        ...
        if (filters.excludeRecentlySurfaced && memory.lastSurfacedAt) {
            const daysSinceSurfaced = (now.getTime() - new Date(memory.lastSurfacedAt).getTime()) / (24 * 60 * 60 * 1000);
 ...
 */
export async function markMemorySurfaced(userId, memoryId, context) {
 ...
 */
export async function recordReaction(userId, memoryId, reaction, context) {
    ...
    try {
        ...
        await db
            ...
            .doc(memoryId)
            .update({
            reactions: (await import('firebase-admin/firestore')).FieldValue.arrayUnion(cleanForFirestore(reactionRecord)),
            updatedAt: now,
        });
...
//# sourceMappingURL=highlight-scorer.js.map