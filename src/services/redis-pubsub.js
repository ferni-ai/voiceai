...
class RedisPubSubService {
    ...
    async initialize() {
        if (this.initialized)
            return true;
        if (this.initPromise)
            return this.initPromise.then(() => this.initialized);
 ...
 */
export async function shutdownRedisPubSub() {
    if (pubsubInstance) {
 ...
 */
export async function publishSessionEvent(event, data) {
    const pubsub = getRedisPubSub();
 ...
 */
export async function publishCacheInvalidation(cacheType, keys) {
    const pubsub = getRedisPubSub();
 ...
 */
export async function publishInsightsUpdate(userId, insightType, data) {
    const pubsub = getRedisPubSub();
 ...
 */
export async function subscribeToSessionEvents(handler) {
    const pubsub = getRedisPubSub();
 ...
 */
export async function subscribeToInsightsUpdates(handler) {
    const pubsub = getRedisPubSub();
