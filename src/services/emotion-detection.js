...
function detectEnergyLevel(text) {
    ...
    if (exclamations >= 2)
        highScore++;
    if (exclamations === 0 && text.length > 50)
        lowScore++;
    // Determine level
    if (highScore > lowScore && highScore >= 2)
        return 'high';
    if (lowScore > highScore && lowScore >= 2)
        return 'low';
    return 'medium';
}
/**
 ...
 */
export function isUserDistressed(text) {
    const result = detectEmotion(text);
    return (result.primary === 'distressed' ||
 ...
 */
export function isUserExcited(text) {
    const result = detectEmotion(text);
 ...
 */
export function getResponseStyle(emotion) {
 ...
 */
export function analyzeConversationEmotion(messages) {
    ...
    const positiveEmotions = ['excited', 'happy', 'grateful'];
    ...
    const averageEnergy = avgEnergy < 0.7 ? 'low' : avgEnergy > 1.3 ? 'high' : 'medium';
