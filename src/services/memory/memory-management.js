...
const phoneToUserCache = new Map();
 ...
 */
export async function loadPhoneCache() {
    if (phoneCacheLoaded)
 ...
 */
export async function savePhoneMapping(phone, userId) {
 ...
 */
export function getCachedPhoneMapping(phone) {
 ...
 */
export async function findDuplicateProfiles(profile, store) {
 ...
 */
export async function consolidateProfiles(primaryProfile, profilesToMerge, store) {
    const memoryStore = store || getDefaultStore();
    const result = {
    ...
    for (const mergeProfile of profilesToMerge) {
        try {
            ...
            for (const moment of moments) {
                ...
                const isDupe = primaryProfile.keyMoments.some((m) => m.summary === moment.summary &&
            ...
            if (mergeProfile.personaMemories) {
                ...
                for (const [persona, memories] of Object.entries(mergeProfile.personaMemories)) {
                    ...
                    primaryProfile.personaMemories[key].push(...memories);
                }
            }
            ...
            getLogger().info({ primary: primaryProfile.id, merged: mergeProfile.id }, '🔄 Merged duplicate profile');
        }
        catch (error) {
 ...
 */
export function compareVoiceSketches(sketch1, sketch2) {
 ...
 */
export async function findProfilesByVoice(voiceSketch, store, minSimilarity = 0.75) {
    const memoryStore = store || getDefaultStore();
    const matches = [];
    try {
 ...
 */
export function generateVoiceRecognitionGreeting(similarity, userName) {
    if (similarity < 0.75)
        return null;
 ...
 */
export async function getProactiveMemories(profile, currentTopic, turnCount) {
    const memories = [];
    const now = new Date();
    ...
    for (const goal of profile.goals) {
        ...
        if (goal.targetDate) {
            const daysUntil = Math.floor((new Date(goal.targetDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
    ...
    for (const moment of recentMoments) {
        const daysSince = Math.floor((now.getTime() - new Date(moment.timestamp).getTime()) / (1000 * 60 * 60 * 24));
        // Only mention if it's been a while and relevant
        if (daysSince >= 7 && daysSince <= 90) {
            // Check topic relevance
            const isTopicRelevant = currentTopic &&
                moment.topics.some((t) => t.toLowerCase().includes(currentTopic.toLowerCase()) ||
    ...
    if (profile.emotionalPatterns.length >= 5) {
        ...
        const anxiousCount = recentPatterns.filter((p) => p.emotion === 'anxiety' || p.emotion === 'fear' || p.emotion === 'worry').length;
        if (anxiousCount >= 3) {
            memories.push({
                type: 'pattern',
                priority: 'medium',
 ...
 */
export function shouldSurfaceMemory(memory, turnCount, lastMemorySurfacedTurn) {
    // Don't surface memories in first few turns (let conversation warm up)
    if (turnCount < 3)
        return false;
    // High priority memories can surface earlier
    if (memory.priority === 'high' && turnCount >= 3)
        return true;
    // Don't surface memories too frequently
    if (lastMemorySurfacedTurn && turnCount - lastMemorySurfacedTurn < 5)
        return false;
 ...
 */
export async function pruneMemorySystem(config, store) {
    ...
    try {
        ...
        for (const profile of profiles) {
            ...
            const sortedSummaries = [...summaries].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
            const toRemove = [];
        ...
        try {
            ...
            if (vectorStore.isInitialized) {
                ...
                getLogger().info({ documentCount: stats.documentCount }, 'Vector store stats (pruning requires usage tracking)');
            }
        }
        catch (error) {
        ...
        getLogger().info({ ...result, dryRun: cfg.dryRun }, cfg.dryRun ? '🧹 Memory pruning dry run complete' : '🧹 Memory pruning complete');
        return result;
    }
    catch (error) {
