 ...
 */
export async function saveUserBaseline(baseline) {
    // Always update in-memory cache
    userBaselines.set(baseline.userId, baseline);
    // Persist to Firestore
    const db = getFirestore();
    if (!db) {
        log.debug({ userId: baseline.userId }, 'Firestore unavailable - baseline cached in memory only');
        return;
    }
    try {
        await db.collection(BASELINES_COLLECTION).doc(baseline.userId).set(cleanForFirestore(baseline));
        log.debug({ userId: baseline.userId, sessionCount: baseline.sessionCount }, 'Baseline saved to Firestore');
    }
    catch (error) {
 ...
 */
export function createInitialBaseline(userId) {
    return {
        userId,
        updatedAt: Date.now(),
        sessionCount: 0,
 ...
 */
export function initSessionObservations(sessionId) {
 ...
 */
export function recordTurnObservations(sessionId, observations) {
 ...
 */
export function getSessionObservations(sessionId) {
 ...
 */
export function detectDeviations(userId, sessionId) {
 ...
 */
export async function updateBaselineFromSession(userId, sessionId) {
    const session = sessionObservations.get(sessionId);
    if (!session || session.turnCount < 5) {
        log.debug({ userId, sessionId, turns: session?.turnCount }, 'Insufficient data for baseline update');
        return;
    }
    ...
    if (session.responseLatencies.length > 0) {
        const avgLatency = session.responseLatencies.reduce((a, b) => a + b, 0) / session.responseLatencies.length;
    ...
    if (session.responseLengths.length > 0) {
        const avgLength = session.responseLengths.reduce((a, b) => a + b, 0) / session.responseLengths.length;
 ...
 */
export function cleanupSessionObservations(sessionId) {
 ...
 */
export async function deleteUserBaseline(userId) {
    // Remove from cache
 ...
 */
export async function loadBaselinesForUsers(userIds) {
 ...
 */
export async function getAllBaselineUserIds(limit = 1000) {
    const db = getFirestore();
    if (!db)
...
//# sourceMappingURL=human-listening-memory.js.map