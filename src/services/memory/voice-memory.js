 ...
 */
function calculateSpectralRolloff(magnitudes, sampleRate, percentage = 0.85) {
    let totalEnergy = 0;
 ...
 */
function calculateMagnitudeSpectrum(samples) {
 ...
 */
function isSpeechFrame(samples, threshold = 0.01) {
...
export class VoiceSketchBuilder {
     ...
     */
    processAudioChunk(samples, timestampMs) {
        // Process in frames
    ...
    build() {
        ...
        const confidence = Math.min(1.0, Math.sqrt(this.pitchSamples.length / 100) * Math.sqrt(this.totalDurationMs / 30000));
        return {
...
export class VoiceMemoryService {
     ...
     */
    getBuilder(sessionId, sampleRate = 16000) {
     ...
     */
    processAudio(sessionId, samples, timestampMs, sampleRate = 16000) {
     ...
     */
    findBestMatch(currentSketch, candidates) {
     ...
     */
    async searchVoices(currentSketch, store) {
...
//# sourceMappingURL=voice-memory.js.map