...
import { getMealData, getRecipesForDiet, getQuickRecipes, suggestRecipesByIngredients, } from '../stores/meal-store.js';
...
export class MealPlanner {
     ...
     */
    async suggestWithIngredients(ingredients) {
     ...
     */
    async getQuickMeals(maxMinutes = 30) {
        return getQuickRecipes(this.userId, maxMinutes);
    }
    /**
     * Generate reason for suggestion
     */
    generateSuggestionReason(recipe, data) {
    ...
    scoreRecipe(recipe, data) {
        ...
        if (recipe.lastCookedAt) {
            const daysSince = Math.floor((Date.now() - new Date(recipe.lastCookedAt).getTime()) / (1000 * 60 * 60 * 24));
            if (daysSince < 7)
                score -= (7 - daysSince) * 3;
        }
     ...
     */
    pickRecipe(recipes, mealType, usedIds) {
     ...
     */
    generateCombinedShoppingList(recipes) {
...
export function getMealPlanner(userId) {
...
export function resetMealPlanner(userId) {
