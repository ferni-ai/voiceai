...
async function checkRedisRateLimit(key, maxRequests, windowMs) {
    ...
    try {
        const now = Date.now();
        const result = (await redisClient.eval(RATE_LIMIT_SCRIPT, 1, `ratelimit:${key}`, maxRequests, windowMs, now));
        return {
            allowed: result[0] === 1,
            remaining: result[1],
            resetAt: result[2],
...
export const rateLimiter = {
     ...
     */
    checkSync(key, maxRequests, windowMs) {
        return checkMemoryRateLimit(key, maxRequests, windowMs);
    },
    /**
...
if (isProduction && !redisAvailable) {
    log.warn('SECURITY: Using in-memory rate limiting in production. ' +
