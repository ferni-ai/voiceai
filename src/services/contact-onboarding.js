 ...
 */
import { getUserContactInfo, setUserContactInfo, } from './outreach/user-contact.js';
 ...
 */
function isThirdPartyPhoneReference(text) {
    ...
    const relationshipWords = 'mom|mother|dad|father|parent|brother|sister|friend|wife|husband|spouse|boss|doctor|dentist|therapist|coworker|colleague|partner|girlfriend|boyfriend|aunt|uncle|grandma|grandpa|grandmother|grandfather';
    const thirdPartyPatterns = [
        ...
        new RegExp(`(get\\s+a\\s+hold\\s+of|get\\s+in\\s+touch\\s+with)\\s+(my\\s+)?(${relationshipWords})`, 'i'),
 ...
 */
export async function completeOnboarding(userId, contactInfo) {
    ...
    onboardingStateStore.set(userId, state);
    getLogger().info({ userId, hasPhone: !!merged.phone, hasEmail: !!merged.email }, '✅ Contact onboarding complete');
    return state;
}
/**
 * Mark that we've asked for contact info (to avoid repeated prompts)
 */
export async function markAskedForContact(userId) {
    const state = await getOnboardingState(userId);
 ...
 */
export async function shouldPromptForContact(userId, turnCount) {
    const state = await getOnboardingState(userId);
 ...
 */
export function getContactPrompt(context) {
    const prompts = {
 ...
 */
export function getPhoneAskOpener(relationshipStage) {
    const openers = {
 ...
 */
export function getConfirmationMessage(contactInfo) {
 ...
 */
export async function processMessageForOnboarding(userId, message, turnCount) {
...
//# sourceMappingURL=contact-onboarding.js.map