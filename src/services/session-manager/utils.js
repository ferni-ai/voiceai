 ...
 */
export async function withTimeout(promise, timeoutMs, operationName, sessionId) {
    return Promise.race([
        promise,
        new Promise((resolve) => {
            setTimeout(() => {
                // Info level since timeout is expected behavior with graceful fallback
                getLogger().info({ sessionId, operation: operationName, timeoutMs }, 'Operation timed out, using fallback');
                resolve(null);
            }, timeoutMs);
        }),
    ]);
 ...
 */
export function generateFallbackSummary(turns, topicsDiscussed, durationMinutes) {
 ...
 */
export function calculateSpeakingPace(wpm) {
    if (wpm < 120)
        return 'slow';
    if (wpm > 180)
        return 'fast';
 ...
 */
export function blendWPM(sessionWPM, profileWPM) {
    if (!profileWPM)
        return sessionWPM;
 ...
 */
export function paceToWPM(pace) {
