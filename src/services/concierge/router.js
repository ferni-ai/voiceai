...
export class ConciergeRouter {
     ...
     */
    checkPermissions(domain, channel) {
     ...
     */
    async routeRequest(description, requirements, options) {
        try {
            ...
            log.info({ requestId: request.id, domain, type, targets: request.targets.length }, 'Concierge request created');
            return {
                success: true,
...
export function createConciergeRouter(options) {
