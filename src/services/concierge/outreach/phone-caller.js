...
export class PhoneCaller {
     ...
     */
    async executeTwilioCall(target, domain, requirements, script, timeout) {
     ...
     */
    async simulateCall(target, domain, requirements) {
     ...
     */
    generateMockResult(target, domain, requirements) {
        const contactName = ['Sarah', 'Mike', 'Jennifer', 'David', 'Lisa'][Math.floor(Math.random() * 5)];
        switch (domain) {
            case 'hotel': {
                const pricePerNight = Math.floor(Math.random() * 200) + 100;
                const nights = requirements.dateRange
                    ? Math.ceil((requirements.dateRange.end.getTime() - requirements.dateRange.start.getTime()) /
                        (1000 * 60 * 60 * 24))
                    : 1;
...
// Factory function
export function createPhoneCaller(options) {
    return new PhoneCaller(options);
}
//# sourceMappingURL=phone-caller.js.map