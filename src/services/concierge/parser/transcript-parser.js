 ...
 */
function extractPrices(transcript) {
    const prices = [];
    const seen = new Set();
...
function extractAvailability(transcript) {
    ...
    const confidence = positiveScore + negativeScore > 0
 ...
 */
function extractWaitlist(transcript) {
 ...
 */
function extractTimes(transcript) {
    const times = [];
 ...
 */
function extractContactName(transcript) {
 ...
 */
function parseHotelTranscript(transcript) {
    ...
    if (/free\s+cancellation/i.test(transcript)) {
        const match = transcript.match(/free\s+cancellation\s+(?:until|up\s+to|within)\s+(.+?)(?:\.|,|$)/i);
 ...
 */
function parseRestaurantTranscript(transcript) {
 ...
 */
function parseHealthcareTranscript(transcript) {
    const { available } = extractAvailability(transcript);
    ...
    if (contactName) {
        ...
        if (/dr\.?/i.test(transcript.substring(0, transcript.indexOf(contactName) + contactName.length + 20))) {
 ...
 */
function parseServiceTranscript(transcript) {
    const prices = extractPrices(transcript);
    const confirmation = extractConfirmation(transcript);
    const data = {};
    if (prices.length > 0) {
 ...
 */
export function parseTranscript(transcript, domain, businessName) {
 ...
 */
function generateSummary(domain, businessName, data) {
    switch (domain) {
        case 'hotel':
 ...
 */
export function parseMultipleTranscripts(transcripts, domain) {
    return transcripts.map(({ transcript, businessName }) => parseTranscript(transcript, domain, businessName));
}
