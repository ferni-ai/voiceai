...
const PROGRESS_MESSAGES = {
    discovery_started: (domain, _count) => `${DOMAIN_EMOJI[domain]} Looking for options now! I'll text you when I have results.`,
    outreach_started: (domain, count) => `${DOMAIN_EMOJI[domain]} Calling ${count} ${domain}s for you. Back in a few minutes!`,
    call_started: (domain, _count) => `${DOMAIN_EMOJI[domain]} On a call now, gathering info...`,
};
/**
 * Format results for SMS (short, scannable)
 */
function formatSmsResults(request) {
 ...
 */
function formatPushResults(request) {
    const { domain, results, recommendation } = request;
    const emoji = DOMAIN_EMOJI[domain];
 ...
 */
function formatEmailResults(request) {
    const { domain, results, recommendation } = request;
    const emoji = DOMAIN_EMOJI[domain];
 ...
 */
async function sendEmailNotification(email, subject, body, userId, requestId) {
    try {
        const { sendEmail: sendViaDelivery, isEmailDeliveryAvailable } = await import('../../outreach/delivery/email-delivery.js');
 ...
 */
export async function notifyRequestComplete(request) {
    log.info({ requestId: request.id, userId: request.userId, domain: request.domain }, 'Sending completion notification');
    ...
    const channels = [prefs.preferredChannel];
    if (prefs.phone && !channels.includes('sms'))
        channels.push('sms');
    if (prefs.pushToken && !channels.includes('push'))
        channels.push('push');
    if (prefs.email && !channels.includes('email'))
        channels.push('email');
 ...
 */
export async function notifyProgress(userId, eventType, domain, count) {
 ...
 */
export function handleConciergeEvent(event, request) {
    switch (event.type) {
...
//# sourceMappingURL=result-notifier.js.map