 ...
 */
export const CHANGE_TALK_PATTERNS = {
    desire: [
        /i (?:really )?want to/gi,
        /i wish i could/gi,
        /i'd like to/gi,
 ...
 */
export function detectChangeTalk(text, topic) {
    const instances = [];
    const lowerText = text.toLowerCase();
    for (const [type, patterns] of Object.entries(CHANGE_TALK_PATTERNS)) {
        for (const pattern of patterns) {
            ...
            if (match) {
                ...
                let strength = 0.5;
                if (type === 'commitment' || type === 'taking_steps')
                    strength = 0.9;
                else if (type === 'need')
                    strength = 0.7;
                else if (type === 'desire' || type === 'reasons')
                    strength = 0.6;
                // Boost if emphatic
 ...
 */
export function getStrongestChangeTalk(instances) {
 ...
 */
export function detectSustainTalk(text) {
    const patterns = [];
...
// ============================================================================
const changeTalkHistory = new Map();
 ...
 * Flush persistence
 */
export async function flushMotivationalInterviewingPersistence() {
 ...
 */
export async function shutdownMotivationalInterviewing() {
 ...
 */
export function recordChangeTalk(userId, instances) {
    const history = changeTalkHistory.get(userId) || [];
 ...
 */
export function getChangeTalkHistory(userId) {
    return changeTalkHistory.get(userId) || [];
}
/**
 * Get topics with the most change talk.
 */
export function getTopChangeTalkTopics(userId, limit = 3) {
    const history = changeTalkHistory.get(userId) || [];
    const topicCounts = {};
 ...
 */
export function analyzeAmbivalence(userId) {
 ...
 */
export function buildMIContext(userId, userText, topic) {
...
//# sourceMappingURL=motivational-interviewing.js.map