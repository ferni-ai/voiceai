...
const userValues = new Map<string, ACTValue[]>();
const committedActions = new Map<string, CommittedAction[]>();
 ...
 */
export const VALUES_QUESTIONS: Record<ValueDomain, string[]> = {
  relationships: [
 ...
 */
export const VALUE_EXAMPLES: Record<ValueDomain, string[]> = {
 ...
 */
export function detectValuesInSpeech(
  text: string,
  ...
  const indirectIndicators: Record<string, { value: string; domain: ValueDomain }> = {
 ...
 */
function inferDomain(value: string): ValueDomain {
...
export function recordValue(
  ...
  const existingIndex = existing.findIndex(
    (v) => v.value.toLowerCase() === value.toLowerCase() && v.domain === domain
  );
 ...
 */
export function getUserValues(userId: string): ACTValue[] {
 ...
 */
export function getValuesByDomain(userId: string, domain: ValueDomain): ACTValue[] {
 ...
 */
export function getTopValues(userId: string, limit = 5): ACTValue[] {
  const all = getUserValues(userId);
  return all
 ...
 */
export function getPendingActions(userId: string): CommittedAction[] {
 ...
 */
export function getValuesQuestion(domain: ValueDomain): string {
 ...
 */
export function getValueExamples(domain: ValueDomain, count = 3): string[] {
 ...
 */
export function generateValuesPrompt(
  userId: string,
  context?: { currentTopic?: string; emotion?: string }
): string {
  const values = getUserValues(userId);
