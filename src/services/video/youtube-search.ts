...
export async function searchVideos(
  ...
  try {
    const data = await youtubeCircuitBreaker.execute(async () => {
      ...
      return (await response.json()) as YouTubeApiSearchResponse;
    });
...
export async function getVideoDetails(videoId: string): Promise<YouTubeVideoDetails> {
  ...
  try {
    const data = await youtubeCircuitBreaker.execute(async () => {
      ...
      return (await response.json()) as YouTubeApiVideoResponse;
    });
...
export async function isYouTubeApiAvailable(): Promise<boolean> {
  ...
  try {
    const response = await fetch(
      `${YOUTUBE_API_BASE}/search?part=snippet&q=test&type=video&maxResults=1&key=${apiKey}`,
