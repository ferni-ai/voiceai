...
import { getDayOverview, getWeekOverview } from '../calendar/calendar-service.js';
 ...
 */
export async function getHabitCalendarInsights(userId, habit, completionHistory) {
    try {
        ...
        const { heavyDayCompletions, lightDayCompletions, missedOnHeavyDays } = await analyzeCompletionCorrelation(userId, habit, completionHistory, weekOverview);
        // Calculate completion rates
        const completionRateOnHeavyDays = heavyDayCompletions.total > 0 ? heavyDayCompletions.completed / heavyDayCompletions.total : 0;
        const completionRateOnLightDays = lightDayCompletions.total > 0 ? lightDayCompletions.completed / lightDayCompletions.total : 1;
        // Determine correlation strength
        let correlation = 'none';
        const rateDifference = completionRateOnLightDays - completionRateOnHeavyDays;
        if (rateDifference > 0.5)
            correlation = 'strong';
        else if (rateDifference > 0.3)
            correlation = 'moderate';
        else if (rateDifference > 0.1)
            correlation = 'weak';
        // Generate adaptation suggestion
        const suggestedAdaptation = generateAdaptationSuggestion(habit, correlation, completionRateOnHeavyDays);
 ...
 */
export async function getTomorrowHabitRecommendations(userId, habits) {
    ...
    try {
        ...
        for (const habit of habits) {
            const recommendation = generateHabitRecommendation(habit, tomorrowOverview, isHeavyDay, hasBackToBack);
            if (recommendation) {
 ...
 */
async function getCelebrationContext(userId, habit) {
    try {
 ...
 */
export async function buildHabitCalendarContext(userId, habits) {
    const sections = ['[HABIT-CALENDAR CORRELATION - Better Than Human Pattern Detection]'];
    try {
...
// HELPER FUNCTIONS
// ============================================================================
async function analyzeCompletionCorrelation(userId, habit, completionHistory, weekOverview) {
    ...
    for (const dayOverview of weekOverview.days) {
        ...
        if (isHeavyDay) {
            ...
            if (wasCompleted)
                heavyDayCompletions.completed++;
            else
                missedOnHeavyDays = true;
        }
        else {
            lightDayCompletions.total++;
            if (wasCompleted)
                lightDayCompletions.completed++;
        }
    }
    return { heavyDayCompletions, lightDayCompletions, missedOnHeavyDays };
}
function generateAdaptationSuggestion(habit, correlation, heavyDayRate) {
...
function generateHabitRecommendation(habit, dayOverview, isHeavyDay, hasBackToBack) {
    const duration = habit.duration || 30;
 ...
 */
export async function getHabitCalendarContextForBuilder(userId) {
    try {
...
//# sourceMappingURL=habit-calendar-integration.js.map