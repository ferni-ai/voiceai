 ...
 */
export function verifyPhoneHash(phoneNumber, storedHash) {
 ...
 */
export function verifyEmailHash(email, storedHash) {
 ...
 */
export async function decryptSensitive(encryptedData) {
    // Remove prefix
 ...
 */
export async function decryptVoiceSketch(encryptedSketch) {
    const decrypted = await decryptSensitive(encryptedSketch);
 ...
 */
const tokenVaultCache = new Map();
const reverseTokenCache = new Map(); // value -> token for dedup
/**
 * Save token to Firestore
 */
async function saveTokenToFirestore(token, value, category) {
 ...
 */
async function loadTokenFromFirestore(token) {
    const db = getFirestore();
    if (!db)
        return null;
    try {
 ...
 */
async function findExistingToken(value, category) {
    const db = getFirestore();
    if (!db)
        return null;
    try {
 ...
 */
export function tokenize(value, category) {
    // Check cache first
 ...
 */
export async function tokenizeAsync(value, category) {
    // Check cache first
    const cacheKey = `${category}:${value}`;
    if (reverseTokenCache.has(cacheKey)) {
        return reverseTokenCache.get(cacheKey);
 ...
 */
export async function detokenizeAsync(token) {
    // Check cache first
 ...
 */
export function stripPII(obj, fieldsToStrip = [
    'phone',
    'email',
    'name',
    'address',
    ...
    for (const [key, value] of Object.entries(obj)) {
        if (fieldsToStrip.includes(key)) {
            ...
            result[key] = stripPII(value, fieldsToStrip);
        }
        else {
            result[key] = value;
        }
...
//# sourceMappingURL=privacy-crypto.js.map