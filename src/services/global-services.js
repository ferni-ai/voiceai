...
import { validateAndLog } from './deployment/startup-validation.js';
 ...
 */
export async function initializeServices(indexPersona = true) {
    ...
    try {
        ...
        const [{ initializeProductivityStore }, { initializeBackgroundTasks }, { initializeCollectiveLearning }, { initializeMemoryManagement },] = await Promise.all([
 ...
 */
export function getGlobalServicesSync() {
    return globalServices;
}
/**
 * Reset global services (for testing)
 */
export async function resetGlobalServices() {
    ...
    try {
        const { shutdownEngagementNotificationService } = await import('./engagement/engagement-notification-service.js');
    ...
    try {
        const { shutdownReadingBetweenLines } = await import('./trust-systems/reading-between-lines.js');
    ...
    try {
        const { shutdownMotivationalInterviewing } = await import('./therapeutic-frameworks/motivational-interviewing.js');
    ...
    try {
        const { shutdownCommunicationMirroringPersistence } = await import('../intelligence/communication-mirroring.js');
 ...
 */
export function getStartupCapabilities() {
    return startupCapabilities;
}
/**
 * Mark persona as indexed (for external tracking)
 */
export function markPersonaIndexed() {
    personaIndexed = true;
}
/**
 * Check if persona has been indexed
 */
export function isPersonaIndexed() {
