...
export const SEASONAL_EVENTS: Record<string, Omit<SeasonalEvent, 'id' | 'startDate' | 'endDate' | 'userCelebrated'>> = {
  new_year: {
    name: 'New Year',
    type: 'holiday',
    personaResponses: {
...
export class TeamEngagementService {
  private huddles: Map<string, TeamHuddle[]> = new Map();
  private sharedEvolutions: Map<string, string[]> = new Map(); // userId -> eventIds shared
  private userAnniversaries: Map<string, UserAnniversary[]> = new Map();
  ...
  generateTeamHuddle(
    ...
    const availablePersonas = ['alex-chen', 'maya-santos', 'jordan-taylor', 'peter-john', 'nayan-patel'];
  ...
  getCrossPersonaReference(fromPersonaId: string, context?: string): string | null {
    const references = CROSS_PERSONA_REFERENCES[fromPersonaId as keyof typeof CROSS_PERSONA_REFERENCES];
