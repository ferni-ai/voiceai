...
// STORAGE
// ============================================================================
const warningHistories = new Map();
// ============================================================================
// WARNING PATTERNS
// ============================================================================
        ...
        signals: [
            ...
            },
            {
                ...
                detector: (_profile, context) => {
                    ...
                    return (context?.recentMessages?.some((m) => avoidanceWords.some((w) => m.toLowerCase().includes(w))) ?? false);
                },
            },
        ],
        threshold: 0.5,
        ...
        signals: [
            ...
            },
            {
                ...
                detector: (_profile, context) => {
                    ...
                    return (context?.recentMessages?.some((m) => doubts.some((d) => m.toLowerCase().includes(d))) ??
                        false);
                },
            },
            {
            ...
            },
            {
                ...
                detector: (_profile, context) => {
                    ...
                    return (context?.recentMessages?.some((m) => indicators.some((i) => m.toLowerCase().includes(i))) ?? false);
                },
            },
        ],
        threshold: 0.5,
 ...
 */
export function checkWarnings(profile, context) {
    const warnings = [];
    ...
    for (const pattern of WARNING_PATTERNS) {
        ...
        if (triggerScore >= pattern.threshold) {
            const severity = triggerScore >= pattern.severeThreshold
                ? 'urgent'
            ...
            log.warn({ userId: profile.userId, type: pattern.type, severity, score: triggerScore }, 'Early warning detected');
        }
    }
    return warnings;
}
 ...
 */
export function getActiveWarnings(userId) {
 ...
 */
export function getWarningContextInjection(userId) {
    const active = getActiveWarnings(userId);
    if (active.length === 0)
        return '';
    // Get most severe warning
    const mostSevere = active.sort((a, b) => severityLevel(b.severity) - severityLevel(a.severity))[0];
 ...
 */
export function recordWarningFeedback(userId, warningId, wasAccurate) {
    const history = warningHistories.get(userId);
    if (!history)
        return;
...
function severityLevel(severity) {
    const levels = { watch: 1, concern: 2, urgent: 3 };
    return levels[severity];
}
function selectRecommendations(all, severity) {
