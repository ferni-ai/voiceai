 ...
 */
export async function persistSnapshot(userId, snapshot) {
 ...
 */
export async function loadSnapshots(userId, days = 30) {
    // Check cache first
    const cached = snapshotCache.get(userId);
    if (cached && cached.expiresAt > Date.now()) {
        return cached.snapshots.filter((s) => s.timestamp >= new Date(Date.now() - days * 24 * 60 * 60 * 1000));
    }
    const db = getFirestoreDb();
    if (!db) {
        return [];
 ...
 */
export async function exportWellbeingData(userId) {
    const db = getFirestoreDb();
    if (!db) {
        return null;
    }
 ...
 */
export async function deleteWellbeingData(userId) {
    const db = getFirestoreDb();
    if (!db) {
        return false;
    }
