...
import { getActiveWorkflows, startExecution, completeExecution, failExecution, updateExecutionAction, } from '../stores/workflow-store.js';
import { executeAction as executeRealAction } from './action-executor.js';
...
export class WorkflowEngine {
    ...
    async checkTimeTriggers() {
        ...
        for (const workflow of workflows) {
            ...
            if (!this.cronMatchesNow(trigger.schedule, now))
                continue;
     ...
     */
    async handlePhraseTrigger(phrase) {
        const workflows = await getActiveWorkflows(this.userId);
        const normalizedPhrase = phrase.toLowerCase().trim();
        for (const workflow of workflows) {
            if (workflow.trigger.type !== 'phrase')
                continue;
            const trigger = workflow.trigger;
    ...
    async handleEventTrigger(eventType, eventData) {
        ...
        for (const workflow of workflows) {
            if (workflow.trigger.type !== 'event')
                continue;
            const trigger = workflow.trigger;
            if (trigger.eventName !== eventType)
                continue;
     ...
     */
    async handleCalendarTrigger(eventType, calendarEvent) {
        const workflows = await getActiveWorkflows(this.userId);
        for (const workflow of workflows) {
            if (workflow.trigger.type !== 'calendar')
                continue;
            const trigger = workflow.trigger;
            if (!('triggerOn' in trigger) || trigger.triggerOn !== eventType)
                continue;
     ...
     */
    async handleHabitTrigger(habitEvent, habitData) {
     ...
     */
    async handleLocationTrigger(action, location) {
        const workflows = await getActiveWorkflows(this.userId);
        for (const workflow of workflows) {
            if (workflow.trigger.type !== 'location')
                continue;
            const trigger = workflow.trigger;
            if (!('triggerOn' in trigger))
                continue;
            // Check trigger action
            const triggerOn = trigger.triggerOn;
            if (triggerOn !== 'both' && triggerOn !== action)
                continue;
            ...
            if ('locationName' in trigger && trigger.locationName) {
                ...
                const distance = this.haversineDistance(trigger.latitude ?? 0, trigger.longitude ?? 0, location.latitude, location.longitude);
     ...
     * Execute a workflow
     */
    async executeWorkflow(workflow, triggeredBy, eventData) {
        ...
        try {
            ...
            for (let i = 0; i < workflow.actions.length; i++) {
                ...
                try {
                    ...
                    log.error({ workflowId: workflow.id, actionId: action.id, error: String(error) }, 'Action execution failed');
                    await updateExecutionAction(this.userId, execution.id, action.id, {
                        status: 'failed',
                        completedAt: new Date().toISOString(),
                        error: String(error),
...
export function getWorkflowEngine(userId) {
...
export function resetWorkflowEngine(userId) {
