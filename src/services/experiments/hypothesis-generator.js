...
import { getWebExperiments } from './web-experiments.js';
 ...
 */
function extractHeadlineFeatures(headline) {
...
function detectTone(text) {
    if (text.includes('?'))
        return 'questioning';
    if (/finally|someone|understand|feel/i.test(text))
        return 'warm';
    if (/six|brilliant|never|always|better/i.test(text))
        return 'bold';
    return 'direct';
}
/**
 ...
 */
function validateVariantContent(content) {
    ...
    if (typeof c.headline === 'string')
        textsToCheck.push(c.headline);
    if (typeof c.tagline === 'string')
        textsToCheck.push(c.tagline);
    if (typeof c.text === 'string')
        textsToCheck.push(c.text);
    if (typeof c.cta === 'string')
        textsToCheck.push(c.cta);
    // Validate each text
...
export async function analyzeWinningPatterns() {
    ...
    const completed = experiments.filter((exp) => exp.status === 'completed' && exp.winner && exp.winnerConfidence && exp.winnerConfidence >= 95);
 ...
 */
function analyzeHeadlinePatterns(experiments) {
    const patterns = [];
    const featureWins = {};
    for (const exp of experiments) {
        ...
        const winningContent = HERO_HEADLINE_VARIANTS[winnerId];
        if (!winningContent)
            continue;
 ...
 */
function analyzeCTAPatterns(experiments) {
    const patterns = [];
    const featureWins = {};
    for (const exp of experiments) {
        const winnerId = exp.winner;
        if (!winnerId)
            continue;
        const winningContent = HERO_CTA_VARIANTS[winnerId];
        if (!winningContent)
            continue;
 ...
 */
export async function generateHypotheses(patterns) {
    const hypotheses = [];
 ...
 */
function generateHeadlineHypothesis(patterns) {
 ...
 */
function generateHeadlineVariants(pattern) {
    const variants = [];
 ...
 */
function generateCTAHypothesis(patterns) {
    const topPattern = patterns.sort((a, b) => b.confidence - a.confidence)[0];
    if (!topPattern)
        return null;
 ...
 */
export async function saveHypothesis(hypothesis) {
 ...
 */
export async function getHypotheses(status) {
    const db = getFirestore();
 ...
 */
export async function updateHypothesisStatus(hypothesisId, status) {
    const db = getFirestore();
    await db
        .collection('generated_hypotheses')
        .doc(hypothesisId)
