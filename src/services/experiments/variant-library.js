...
export const HERO_HEADLINE_VARIANTS = {
    ...
    emotional_question: {
        ...
        subhead: 'Six AI specialists who listen without judgment, remember everything, and help you grow. Available whenever you need them.',
    },
    team_focus: {
        tagline: 'Better than human.',
        headline: 'Six brilliant minds. One conversation.',
        subhead: 'A life coach, mentor, researcher, strategist, habit expert, and planner—all working together for you.',
    },
    memory_focus: {
        tagline: 'Beyond human limitations.',
        headline: 'Someone who never forgets.',
        subhead: 'That thing you mentioned six months ago? We remember. Every detail, every context, every nuance of your story.',
    },
    presence_focus: {
        tagline: 'Always here for you.',
        headline: 'Never too busy. Never too tired.',
        subhead: "Your best friend has their own problems. Your therapist has other patients. We're fully present, every time.",
    },
};
// ============================================================================
// CTA VARIANTS
...
// ============================================================================
export const TEAM_SHOWCASE_VARIANTS = {
    control: {
        order: 'ferni-first',
        style: 'cards',
        showQuotes: true,
 ...
 */
export function getVariant(experimentId, variantId) {
 ...
 */
export function getControlVariant(experimentId) {
    return getVariant(experimentId, 'control');
}
/**
 * Get all variants for an experiment
 */
export function getAllVariants(experimentId) {
    const experiment = EXPERIMENTS.find((e) => e.id === experimentId);
    if (!experiment) {
        return null;
    }
 ...
 */
export function getVariantIds(experimentId) {
    const experiment = EXPERIMENTS.find((e) => e.id === experimentId);
    if (!experiment) {
        return [];
    }
 ...
 */
export async function getCurrentDefault(experimentId) {
 ...
 */
export async function setCurrentDefault(experimentId, variantId) {
    const db = getFirestore();
    await db
 ...
 */
export async function addCustomVariant(experimentId, variantId, content) {
    const db = getFirestore();
    await db
        .collection('variant_library')
        .doc(experimentId)
 ...
 */
export async function getCustomVariants(experimentId) {
    const db = getFirestore();
 ...
 */
export function getVariantsForFrontend() {
    const result = {};
    for (const experiment of EXPERIMENTS) {
        result[experiment.id] = experiment.variants;
    }
    return result;
}
/**
