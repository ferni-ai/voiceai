...
import { analyzeExperiment, completeWebExperiment, getRunningWebExperiments, } from './web-experiments.js';
 ...
 */
export async function runOptimizationLoop(config = {}) {
 ...
 */
async function optimizeExperiment(experiment, config) {
 ...
 */
function detectWinner(experiment, analysis, config) {
    const alerts = [];
    const now = new Date();
 ...
 */
function checkSampleRatioMismatch(experiment, analysis, threshold) {
 ...
 */
async function shipWinner(experimentId, winnerId, confidence) {
    ...
    await db.collection('experiment_shipments').add(removeUndefined({
        experimentId,
        winnerId,
        confidence,
 ...
 */
async function markForReview(experimentId, decision) {
    const db = getFirestore();
    await db
 ...
 */
async function updateVariantLibraryDefault(experimentId, winnerId) {
    const db = getFirestore();
    await db
