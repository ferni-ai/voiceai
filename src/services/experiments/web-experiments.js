...
const assignmentsCache = new Map(); // experimentId -> userId -> variantId
 ...
 */
export async function initWebExperiments() {
    ...
    try {
        ...
        const runningExperiments = Array.from(experimentsCache.values()).filter((e) => e.status === 'running');
 ...
 */
export async function assignVariant(experimentId, userId, context) {
 ...
 */
function passesTargeting(experiment, userId, context) {
 ...
 */
function hashAssignment(experimentId, userId, variants) {
 ...
 */
async function getExistingAssignment(experimentId, userId) {
    // Check cache first
 ...
 */
async function persistAssignment(experimentId, userId, variantId) {
    try {
        const db = getDb();
        await db
            .collection('web_experiments')
 ...
 */
export async function trackExposure(experimentId, variantId, userId, metadata) {
    await trackEvent({
        experimentId,
        variantId,
        userId,
 ...
 */
export async function trackConversion(experimentId, variantId, userId, goalId, value, metadata) {
    await trackEvent({
        experimentId,
        variantId,
        userId,
...
export async function analyzeExperiment(experimentId) {
    ...
    const { winner, confidence, isSignificant } = calculateSignificance(variantStats, experiment.minimumSamples);
 ...
 */
function calculateSignificance(variants, minimumSamples) {
    ...
    if (best && secondBest && best.exposures > 0 && secondBest.exposures > 0) {
        const pooledRate = (best.conversions + secondBest.conversions) / (best.exposures + secondBest.exposures);
        const se = Math.sqrt(pooledRate * (1 - pooledRate) * (1 / best.exposures + 1 / secondBest.exposures));
        if (se > 0) {
...
export async function createWebExperiment(config) {
    ...
    const docRef = await db.collection('web_experiments').add(removeUndefined({
        ...experiment,
 ...
 * Start an experiment
 */
export async function startWebExperiment(experimentId) {
    const db = getDb();
    await db
        .collection('web_experiments')
        .doc(experimentId)
 ...
 * Pause an experiment
 */
export async function pauseWebExperiment(experimentId) {
    const db = getDb();
    await db
        .collection('web_experiments')
        .doc(experimentId)
 ...
 */
export async function completeWebExperiment(experimentId, winner, confidence) {
    const db = getDb();
    await db
        .collection('web_experiments')
        .doc(experimentId)
 ...
 */
export async function getWebExperiments() {
    await initWebExperiments();
 ...
 */
export async function getWebExperiment(experimentId) {
    await initWebExperiments();
 ...
 */
export async function getRunningWebExperiments() {
    await initWebExperiments();
