 ...
 */
export async function checkTrustSystemsHealth() {
...
export async function checkOutreachHealth() {
    const start = Date.now();
    try {
        const { generateTextMessage, selectPersonaForOutreach, getTimingProfile } = await import('../outreach/index.js');
        ...
        const message = generateTextMessage(persona || 'ferni', {
            userId: 'health-check',
            userName: 'Test',
...
export async function checkPersistenceHealth() {
    ...
    try {
        ...
        // Write test
        await services.store.saveProfile(testProfile);
        // Read test
 ...
 */
export async function checkCognitiveIntelligenceHealth() {
    const start = Date.now();
    try {
        const { detectDistortions, getGentleResponse, getUserDistortionStats } = await import('../cognitive-intelligence/index.js');
 ...
 */
export async function checkTherapeuticFrameworksHealth() {
    const start = Date.now();
    try {
        const { detectChangeTalk, buildMIContext } = await import('../therapeutic-frameworks/motivational-interviewing.js');
 ...
 */
export async function checkSessionManagementHealth() {
    const start = Date.now();
    try {
 ...
 */
export async function checkWellbeingTrackingHealth() {
    const start = Date.now();
    try {
