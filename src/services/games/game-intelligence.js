 ...
 */
export function recordGuess(gameMemory, item, guessTimeMs, correct, genre, decade) {
    const timing = {
        item,
        guessTimeMs,
        correct,
 ...
 */
function updateAffinity(gameMemory, type, category, guessTimeMs, correct) {
    ...
    if (existing) {
        ...
        if (correct) {
            ...
            existing.avgGuessTimeMs = Math.round((existing.avgGuessTimeMs * (existing.correctGuesses - 1) + guessTimeMs) /
                existing.correctGuesses);
        }
 ...
 */
export function getTopAffinities(gameMemory, type, limit = 3) {
    const affinities = type === 'genre' ? gameMemory.genreAffinities : gameMemory.decadeAffinities;
    if (!affinities)
        return [];
 ...
 */
export function getWeakAreas(gameMemory, type, limit = 3) {
    const affinities = type === 'genre' ? gameMemory.genreAffinities : gameMemory.decadeAffinities;
    if (!affinities)
        return [];
    return Object.values(affinities)
 ...
 */
export function analyzeDifficulty(gameMemory, recentResults, currentRound) {
...
function getHarderMessage(round) {
    const messages = [
 ...
 */
export function checkMilestones(gameMemory, gameType, result, guessTimeMs) {
...
function getMilestoneSoundEffect(type) {
    switch (type) {
        case 'fastest_guess':
        case 'streak_ten':
            return 'fanfare';
 ...
 */
export function analyzeMusicalPersonality(gameMemory) {
    const insights = [];
    const traits = [];
    ...
    if (gameMemory.desertIslandPicks && gameMemory.desertIslandPicks.length > 0) {
        ...
        const emotionalCount = gameMemory.desertIslandPicks.filter((song) => emotionalKeywords.some((kw) => song.toLowerCase().includes(kw))).length;
        if (emotionalCount >= 2) {
            traits.push({
                trait: 'nostalgic',
    ...
    if (genreAffinities.length >= 4 && decadeAffinities.length >= 3) {
        const allHaveDecentScore = [...genreAffinities, ...decadeAffinities].every((a) => a.affinityScore >= 40);
...
function getTraitInsight(trait) {
    const insightMap = {
        nostalgic: [
 ...
 */
export function getPersonalityComment(gameMemory) {
 ...
 */
export function storeConversationHint(gameMemory, topic, relatedArtists, relatedGenres) {
 ...
 */
export function getConversationCallback(gameMemory) {
    const hints = gameMemory.conversationMusicHints || [];
    if (hints.length === 0)
        return null;
    // Check for recent hints (within last hour)
    const recentHints = hints.filter((h) => Date.now() - new Date(h.mentionedAt).getTime() < 60 * 60 * 1000);
 ...
 */
export function getSongSelectionContext(gameMemory) {
 ...
 */
export function getMusicalDNAMessage(gameMemory) {
    const genreAffinities = Object.values(gameMemory.genreAffinities || {});
    const decadeAffinities = Object.values(gameMemory.decadeAffinities || {});
    ...
    if (topGenre && topGenre.affinityScore > 60) {
        messages.push(`Your ${topGenre.category} knowledge is incredible! ${Math.round(topGenre.successRate * 100)}% accuracy.`);
    }
    // Top decade insight
    const topDecade = decadeAffinities.sort((a, b) => b.affinityScore - a.affinityScore)[0];
    if (topDecade && topDecade.affinityScore > 60) {
        messages.push(`${topDecade.category} music? You guess those in an average of ${(topDecade.avgGuessTimeMs / 1000).toFixed(1)} seconds!`);
    }
    // Weak area challenge
    ...
    if (weakGenre && weakGenre.affinityScore < 40 && weakGenre.totalAttempts >= 3) {
        messages.push(`${weakGenre.category} trips you up though - only ${Math.round(weakGenre.successRate * 100)}% there. Want to practice?`);
    }
...
//# sourceMappingURL=game-intelligence.js.map