...
const VALUES_DECK: ValueCard[] = [
  // Relationships
  { id: 'family', name: 'Family', description: 'Being close to family, supporting loved ones', category: 'relationships' },
  { id: 'friendship', name: 'Friendship', description: 'Deep, meaningful friendships', category: 'relationships' },
  { id: 'love', name: 'Love', description: 'Romantic love and partnership', category: 'relationships' },
  { id: 'belonging', name: 'Belonging', description: 'Being part of a community', category: 'relationships' },
  { id: 'loyalty', name: 'Loyalty', description: 'Standing by those you care about', category: 'relationships' },

  // Achievement
  { id: 'success', name: 'Success', description: 'Achieving your goals', category: 'achievement' },
  { id: 'excellence', name: 'Excellence', description: 'Being the best at what you do', category: 'achievement' },
  { id: 'recognition', name: 'Recognition', description: 'Being acknowledged for your work', category: 'achievement' },
  { id: 'wealth', name: 'Wealth', description: 'Financial security and abundance', category: 'achievement' },
  { id: 'influence', name: 'Influence', description: 'Having impact and power', category: 'achievement' },

  // Growth
  { id: 'learning', name: 'Learning', description: 'Continuous growth and knowledge', category: 'growth' },
  { id: 'wisdom', name: 'Wisdom', description: 'Deep understanding of life', category: 'growth' },
  { id: 'creativity', name: 'Creativity', description: 'Self-expression and making things', category: 'growth' },
  { id: 'curiosity', name: 'Curiosity', description: 'Exploring and discovering', category: 'growth' },
  ...
  { id: 'balance', name: 'Balance', description: 'Harmony between life areas', category: 'wellbeing' },
  { id: 'security', name: 'Security', description: 'Safety and stability', category: 'wellbeing' },
  { id: 'simplicity', name: 'Simplicity', description: 'An uncluttered, simple life', category: 'wellbeing' },

  // Meaning
  { id: 'purpose', name: 'Purpose', description: 'Living a meaningful life', category: 'meaning' },
  { id: 'contribution', name: 'Contribution', description: 'Making a difference', category: 'meaning' },
  { id: 'spirituality', name: 'Spirituality', description: 'Connection to something greater', category: 'meaning' },
  { id: 'integrity', name: 'Integrity', description: 'Living by your principles', category: 'meaning' },
  { id: 'justice', name: 'Justice', description: 'Fairness and equality', category: 'meaning' },

  // Pleasure
  { id: 'adventure', name: 'Adventure', description: 'Excitement and new experiences', category: 'pleasure' },
  { id: 'fun', name: 'Fun', description: 'Playfulness and enjoyment', category: 'pleasure' },
  { id: 'freedom', name: 'Freedom', description: 'Independence and autonomy', category: 'pleasure' },
  { id: 'beauty', name: 'Beauty', description: 'Appreciating aesthetics', category: 'pleasure' },
  { id: 'pleasure', name: 'Pleasure', description: 'Enjoying life\'s pleasures', category: 'pleasure' },
];
...
export function createInitialState(): ValuesCardSortState {
...
function getCardMessage(card: ValueCard, remaining: number): string {
  return `**${card.name}**: ${card.description}\n\nIs this important to you? (${remaining} cards left)`;
}

function getNarrowingMessage(count: number): string {
...
function getComparisonMessage(a: ValueCard, b: ValueCard): string {
  return `**${a.name}** or **${b.name}**?\n\nWhich one is MORE important to you right now?`;
}

function getTopFiveMessage(topFive: ValueCard[]): string {
...
function getFinalReflection(topFive: ValueCard[]): string {
...
function parseImportantResponse(input: string): boolean | null {
  const lower = input.toLowerCase().trim();

  // Important variations
  if (lower.includes('important') || lower === 'yes' || lower === 'y' ||
      lower === 'definitely' || lower === 'very' || lower === 'absolutely' ||
      lower === 'for sure' || lower === '1' || lower === 'important') {
    return true;
  }

  // Not as important variations
  if (lower.includes('not') || lower === 'no' || lower === 'n' ||
      lower === 'nah' || lower === 'skip' || lower === 'pass' ||
      lower === 'less' || lower === '2' || lower === 'not as') {
    return false;
  }
...
function parseChoiceResponse(input: string, optionA: ValueCard, optionB: ValueCard): ValueCard | null {
  const lower = input.toLowerCase().trim();

  // Check for option A
  if (lower.includes(optionA.name.toLowerCase()) ||
      lower === 'a' || lower === '1' || lower === 'first' ||
...
export function processInput(state: ValuesCardSortState, input: string): ValuesCardSortResult {
  const lower = input.toLowerCase().trim();
  ...
  if (state.phase === 'intro') {
    if (lower === 'yes' || lower === 'start' || lower === 'ready' || lower === "let's go" || lower === 'ok') {
      const firstCard = state.deck[0];
      const newState: ValuesCardSortState = {
        ...state,
        phase: 'sorting',
  ...
  if (state.phase === 'sorting' && state.currentCard) {
    ...
    const newImportant = isImportant
      ...
      : state.importantPile;
    const newNotAs = !isImportant
      ? [...state.notAsPile, state.currentCard]
      : state.notAsPile;
  ...
  if (state.phase === 'narrowing' && state.comparisonPair) {
    ...
    const remainingPool = state.importantPile.filter(
      v => v.id !== optionA.id && v.id !== optionB.id
    );
 ...
 */
export function describeStateForVoice(state: ValuesCardSortState): string {
  if (state.phase === 'intro') {
 ...
 */
export function getStartResult(state: ValuesCardSortState): ValuesCardSortResult {
  return {
    message: getIntroMessage(),
    gameOver: false,
    newState: state,
 ...
 */
export function getTopFiveValues(state: ValuesCardSortState): ValueCard[] {
