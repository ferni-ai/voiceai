...
function getRandomTimeframe(): HeadlineTimeframe {
  const timeframes: HeadlineTimeframe[] = ['today', 'this_week', 'this_year', 'future'];
  return timeframes[Math.floor(Math.random() * timeframes.length)];
}

function getRandomTone(): HeadlineTone {
  const tones: HeadlineTone[] = ['triumphant', 'honest', 'humorous', 'hopeful'];
...
function isValidHeadline(input: string): boolean {
...
function parseTimeframeChoice(input: string): HeadlineTimeframe | null {
  const lower = input.toLowerCase().trim();

  if (lower.includes('today') || lower === '1') return 'today';
  if (lower.includes('week') || lower === '2') return 'this_week';
  if (lower.includes('month') || lower === '3') return 'this_month';
  if (lower.includes('year') || lower === '4') return 'this_year';
  if (lower.includes('past') || lower.includes('memory') || lower === '5') return 'past';
  if (lower.includes('future') || lower.includes('5 year') || lower === '6') return 'future';
  if (lower.includes('dream') || lower.includes('wild') || lower === '7') return 'dream';
...
export function processInput(state: HeadlineWriterState, input: string): HeadlineWriterResult {
  const lower = input.toLowerCase().trim();
  ...
  if (state.phase === 'prompt') {
    ...
    if (timeframeChoice) {
      const newState: HeadlineWriterState = {
        ...state,
    ...
    if (isValidHeadline(input)) {
      const currentHeadline: Partial<Headline> = {
        text: input.trim(),
 ...
 */
export function describeStateForVoice(state: HeadlineWriterState): string {
 ...
 */
export function getStartResult(state: HeadlineWriterState): HeadlineWriterResult {
  return {
    message: getPromptMessage(state),
    gameOver: false,
    newState: { ...state, phase: 'writing' },
 ...
 */
export function getSessionHeadlines(state: HeadlineWriterState): Headline[] {
