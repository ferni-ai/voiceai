 ...
 */
async function searchSongDirect(query) {
    try {
        ...
        log.error({ error: errorMessage, query, isNetworkError, isRateLimitError }, '🎮 Failed to search song');
 ...
 */
export async function searchSong(query, useFallback = true) {
    try {
 ...
 */
export async function searchSongWithWord(word) {
    try {
        ...
        const matchingTrack = results.results.find((t) => t.previewUrl && t.trackName.toLowerCase().includes(word.toLowerCase()));
        if (!matchingTrack) {
            // Fall back to first track with preview
            const firstWithPreview = results.results.find((t) => t.previewUrl);
            if (!firstWithPreview) {
 ...
 */
export async function getRandomGameSongs(count = 10, options) {
 ...
 */
export async function searchSongForMood(mood) {
 ...
 */
export async function playGameTrack(track, waitForStart = true) {
 ...
 */
export function restoreNormalVolume() {
    const player = getMusicPlayer();
 ...
 */
export async function fadeOutGameTrack(delayMs = 500) {
    const player = getMusicPlayer();
 ...
 */
export function duckForUserGuess() {
    const player = getMusicPlayer();
    if (player.isPlaying()) {
        player.duck();
 ...
 */
export function unduckAfterGuess() {
    const player = getMusicPlayer();
    player.unduck();
 ...
 */
export async function preloadNextRoundSongs(count = 3) {
 ...
 */
export async function getPreloadedOrSearch(query) {
...
//# sourceMappingURL=game-music.js.map