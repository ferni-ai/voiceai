 ...
 */
function getAnswer(state, question) {
    ...
    if (q.includes('alive') || q.includes('living') || q.includes('live')) {
        if (state.category === 'person' || state.category === 'animal')
            return 'yes';
        return 'no';
    }
    ...
    if (q.includes('famous') || q.includes('well-known') || q.includes('celebrity')) {
        if (state.category === 'person' || state.category === 'place')
            return 'yes';
        return 'maybe';
    }
    // Can you find it indoors/outdoors?
    if (q.includes('indoor') || q.includes('inside') || q.includes('house') || q.includes('home')) {
        if (secret.includes('library') || secret.includes('coffee'))
            return 'yes';
        if (state.category === 'thing')
            return 'maybe';
        return 'no';
    }
    // Is it big/large?
    if (q.includes('big') || q.includes('large') || q.includes('tall')) {
        if (secret.includes('elephant') || secret.includes('tower') || secret.includes('canyon'))
            return 'yes';
        if (secret.includes('butterfly') || secret.includes('hummingbird'))
            return 'no';
        return 'maybe';
    }
    // Is it small?
    if (q.includes('small') || q.includes('little') || q.includes('tiny')) {
        if (secret.includes('butterfly') || secret.includes('hummingbird') || secret.includes('candle'))
            return 'yes';
        if (secret.includes('elephant') || secret.includes('tower'))
            return 'no';
        return 'maybe';
    }
 ...
 */
function isGuess(input) {
    const lower = input.toLowerCase();
    return (lower.includes('is it ') ||
 ...
 */
function checkGuess(state, guess) {
...
function getAnswerMessage(answer, questionsLeft) {
    const answerPhrases = {
...
function getLoseMessage(state) {
    return `Out of questions! I was thinking of ${state.secretThing}. Want to play again?`;
}
function getWrongGuessMessage(state, questionsLeft) {
    if (questionsLeft <= 0) {
...
//# sourceMappingURL=twenty-questions.js.map