 ...
 */
export function createEmptyGameMemory() {
    return {
        gameStats: {},
        recentGames: [],
        favoriteGames: [],
 ...
 */
export function getGameMemory(userProfile) {
 ...
 */
export function saveGameSession(gameMemory, session, highlights) {
    const record = {
 ...
 */
export function saveSongGuessed(gameMemory, songName) {
 ...
 */
export function saveDesertIslandPicks(gameMemory, picks) {
 ...
 */
export function getGameHistoryIntro(gameMemory, gameType, personaId) {
    const stats = gameMemory.gameStats[gameType];
    if (!stats || stats.gamesPlayed === 0) {
        return null; // Never played before
    }
    const daysSinceLast = Math.floor((Date.now() - new Date(stats.lastPlayed).getTime()) / (1000 * 60 * 60 * 24));
    ...
    if (daysSinceLast > 7) {
        return `It's been ${daysSinceLast} days since we played this! ${gameIntros[Math.floor(Math.random() * gameIntros.length)]}`;
    }
 ...
 */
export function getSuggestedGame(gameMemory) {
    ...
    if (gameMemory.favoriteGames.length > 0) {
        ...
        if (stats) {
            const daysSince = Math.floor((Date.now() - new Date(stats.lastPlayed).getTime()) / (1000 * 60 * 60 * 24));
            if (daysSince > 3) {
                return {
                    gameType: favorite,
 ...
 */
export function shouldAutoEndGame() {
    if (lastGameActivity === 0)
        return false;
 ...
 */
export function resetGameActivity() {
    lastGameActivity = 0;
}
/**
 ...
 */
export function detectTopicChange(userMessage, activeGameType) {
    if (!activeGameType)
        return false;
    ...
    if (hasOffTopic && !hasGameKeyword)
        return true;
    // Very long messages that don't seem like game answers
    if (message.length > 100 && !hasGameKeyword)
        return true;
    return false;
}
