 ...
 */
function detectChoice(input, state) {
    ...
    if (lower === 'a' || lower === '1' || lower === 'left')
        return 'A';
    if (lower === 'b' || lower === '2' || lower === 'right')
        return 'B';
    return null;
}
/**
...
function getChoiceResponseMessage(choice, state, followUp) {
...
export function processInput(state, input) {
    ...
    const currentDilemmaFull = DILEMMAS.find((d) => d.optionA === state.currentDilemma.optionA && d.optionB === state.currentDilemma.optionB);
    // Record choice
    const newHistory = [
        ...state.choiceHistory,
        {
