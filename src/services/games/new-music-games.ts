...
export function setGameMemoryForNewGames(memory: GameMemory | null): void {
...
// ============================================================================
// LYRIC DATABASE - Famous song lyrics with completions
// ============================================================================

interface LyricChallenge {
  prompt: string; // The lyric to complete
  completion: string; // What they should say
  fullLyric: string; // The complete line
  songName: string;
  artistName: string;
...
const LYRIC_DATABASE: LyricChallenge[] = [
...
export class FinishTheLyricGame implements IGameImplementation {
  // Game state
  private challenges: LyricChallenge[] = [];
  ...
  async initialize(config?: Record<string, unknown>): Promise<{
    ...
    const initialState: FinishTheLyricData = {
    ...
    const welcomeMessage =
      `${getRandomPhrase('gameStart')} 🎤 Let's play Finish the Lyric! ` +
  ...
  async setupNextRound(
    ...
    const nextState: FinishTheLyricData = {
  ...
  private fuzzyMatch(input: string, expected: string): boolean {
    ...
    if (input.length < 2 || expected.length < 2) return false;
...
// ============================================================================
// DECADE CHALLENGE GAME
// ============================================================================

// Songs by decade for the challenge
interface DecadeSong {
  name: string;
  artist: string;
  releaseYear: number;
  decade: string;
  searchTerm: string; // For iTunes search
  previewUrl?: string;
}

const DECADE_SONGS: DecadeSong[] = [
  // 1960s
  {
...
export class DecadeChallengeGame implements IGameImplementation {
  // Game state
  private songs: DecadeSong[] = [];
  ...
  async initialize(config?: Record<string, unknown>): Promise<{
    ...
    const initialState: DecadeChallengeData = {
      currentSong: {
        name: firstSong.name,
    ...
    const welcomeMessage =
      `${getRandomPhrase('gameStart')} 🕰️ Time for the Decade Challenge! ` +
  ...
  async setupNextRound(
    ...
    const data = gameData as unknown as DecadeChallengeData;

    const nextState: DecadeChallengeData = {
      currentSong: {
        name: nextSong.name,
  ...
  private parseDecadeFromInput(input: string): string | null {
    const normalized = input.toLowerCase().replace(/[^a-z0-9]/g, '');

    // Direct matches
    for (const decade of DECADES) {
      if (normalized.includes(decade.replace('s', ''))) return decade;
    }
  ...
  private isDecadeClose(guessed: string | null, correct: string): boolean {
