...
export function createInitialState(
  ...
  const userSymbol: TicTacToePlayer = userGoesFirst ? 'X' : 'O';
  const aiSymbol: TicTacToePlayer = userGoesFirst ? 'O' : 'X';
 ...
 */
export function isValidMove(state: TicTacToeState, position: number): boolean {
...
export function makeMove(
  ...
  const newCells = [...state.board.cells] as TicTacToeBoard['cells'];
 ...
 * Check for a winner
 */
export function checkWinner(board: TicTacToeBoard): TicTacToePlayer | null {
  for (const [a, b, c] of WINNING_COMBINATIONS) {
    if (board.cells[a] && board.cells[a] === board.cells[b] && board.cells[a] === board.cells[c]) {
      return board.cells[a] as TicTacToePlayer;
    }
  }
  return null;
}
 ...
 * Get empty positions
 */
export function getEmptyPositions(board: TicTacToeBoard): number[] {
  return board.cells
 ...
 */
export function getAIMove(state: TicTacToeState): number {
 ...
 */
function getOptimalMove(state: TicTacToeState): number {
  const emptyPositions = getEmptyPositions(state.board);
...
function minimax(
  ...
  if (isBoardFull(state.board)) return 0;
 ...
 */
export function describeBoardForVoice(state: TicTacToeState): string {
  const { cells } = state.board;

  // Describe each row
  const describeCell = (cell: TicTacToeCell): string => {
 ...
 */
export function describeMoveForVoice(position: number, player: TicTacToePlayer): string {
 ...
 */
export function describeAvailablePositions(state: TicTacToeState): string {
  const empty = getEmptyPositions(state.board);

  if (empty.length === 0) return 'No positions available';
  if (empty.length === 9) return 'All positions are open';

  const positions = empty.map((i) => POSITION_NAMES[i]);

  if (positions.length === 1) return `Only ${positions[0]} is open`;
  if (positions.length === 2) return `${positions[0]} and ${positions[1]} are open`;
...
export function getGameMessage(
  state: TicTacToeState,
  event: 'start' | 'user_move' | 'ai_move' | 'invalid_move' | 'user_wins' | 'ai_wins' | 'draw',
  position?: number
): string {
 ...
 */
export function processUserMove(state: TicTacToeState, input: string): TicTacToeResult {
