...
export function generateMusicInsights(gameMemory: GameMemory | null | undefined): MusicInsights {
  ...
  if (!gameMemory || gameMemory.totalGamesPlayed === 0) {
    return {
      ...
      personaStats: [],
      coachingMessage: "Let's play some music games! I'll learn your taste and share insights about your musical personality.",
      generatedAt: now,
    };
  }
  ...
  const decadeStrengths = generateAffinityDisplays(getTopAffinities(gameMemory, 'decade', 2), 'strength');
...
function generateMemorableMoments(gameMemory: GameMemory): MemorableMoment[] {
  ...
  if (gameMemory.bestStreak && gameMemory.bestStreak >= 3) {
    moments.push({
      ...
      icon: '🔥',
      coachingNote: gameMemory.bestStreak >= 7 
        ? 'Incredible focus!'
...
function calculateJourneyStats(gameMemory: GameMemory): JourneyStats {
  ...
  const gamesThisWeek = recentGames.filter(
...
function calculatePersonaStats(recentGames: GameSessionRecord[]): PersonaPlayStats[] {
  ...
  for (const game of recentGames) {
    ...
    if (!personaCounts[game.personaId].lastPlayed || 
...
function generateCoachingMessage(
  ...
  if (personality && personality.traits.length > 0) {
    messages.push(
      `You're a ${personality.label}. ${personality.description}`
    );
  }
...
export function getConversationalInsight(gameMemory: GameMemory | null | undefined): string | null {
  ...
  if (traits.length > 0) {
    ...
    const traitInsights: Record<string, string> = {
      ...
      thoughtful: "You take your time guessing - but you're almost always right. Quality over speed.",
    };
...
export function getGameSuggestion(gameMemory: GameMemory | null | undefined): string | null {
  ...
  const allGames = ['name-that-tune', 'one-word-song', 'desert-island-discs', 'this-or-that', 'mood-dj-challenge'];
  ...
  if (favorite) {
    const daysSince = stats[favorite]?.lastPlayed
      ? Math.floor((Date.now() - new Date(stats[favorite].lastPlayed).getTime()) / (1000 * 60 * 60 * 24))
      : 0;
