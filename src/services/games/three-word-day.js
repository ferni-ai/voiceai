...
const EXPLORATION_PROMPTS = [
    (word) => `Let's start with "${word}". What made that word come to mind?`,
    (word) => `Interesting. And "${word}"—is there a story behind that one?`,
    (word) => `Finally, "${word}". How does this word connect to the others?`,
];
...
// GAME CREATION
// ============================================================================
export function createInitialState(promptType = 'day', customPrompt) {
    return {
        promptType,
        customPrompt,
        words: [],
...
function analyzeWordTriad(words) {
    ...
    if (categories.length >= 2 && new Set(categories).size === 1) {
        ...
        if (cat === 'relational')
            return 'People seem important in your world right now.';
        if (cat === 'action')
            return "You're in motion. Lots of doing energy here.";
    }
    // Mixed categories
...
function generateInsight(words) {
...
function parseThreeWords(input) {
    // Clean up input
...
export function describeStateForVoice(state) {
    ...
    return `Three Word Day. Your words: ${state.words.join(', ')}. Currently exploring word ${state.explorationPhase + 1} of 3.`;
}
/**
...
//# sourceMappingURL=three-word-day.js.map