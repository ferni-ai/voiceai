...
const STORY_PROMPTS: StoryPrompt[] = [
  ...
  {
    opening:
      'The old map had been hidden in the library for decades. When Alex finally unfolded it,',
    genre: 'adventure',
    setting: 'mysterious library',
  },
  {
    opening:
      'The compass always pointed north, except today. Today it pointed straight down into the earth, where',
    genre: 'adventure',
    setting: 'underground mystery',
  },
  {
    opening:
      'Nobody had climbed that mountain in fifty years. They said the last expedition found something up there—something that',
    genre: 'adventure',
  ...
  {
    opening:
      'The letter arrived without a return address. Inside was a single photograph of a door that',
    genre: 'mystery',
    setting: 'mysterious correspondence',
  },
  {
    opening:
      'Everyone in town knew the clock tower had been broken for years. So when it suddenly chimed at midnight,',
    genre: 'mystery',
    setting: 'small town secrets',
  },
  {
    opening:
      'The detective had seen many strange cases, but nothing like this. The room was completely empty except for',
    genre: 'mystery',
  ...
  {
    opening:
      'In a world where dreams could be harvested like crops, one farmer discovered that their latest harvest contained',
    genre: 'fantasy',
    setting: 'dream world',
  },
  {
  ...
  {
    opening:
      'They said the forest moved at night—that the trees walked to new locations. And one morning,',
    genre: 'fantasy',
  ...
  {
    opening:
      'The AI had been dormant for a century. When it finally woke up, it asked only one question:',
    genre: 'scifi',
    setting: 'future technology',
  },
  {
    opening:
      'Space travel had become routine, boring even. But this mission was different because the distress signal came from',
    genre: 'scifi',
    setting: 'space mission',
  },
  {
    opening:
      'Time machines were invented, then immediately banned. But someone had used one anyway, and now',
    genre: 'scifi',
    setting: 'time travel',
  },
  ...
  {
    opening:
      'The coffee shop was always empty at this hour, which is why they were surprised to see',
    genre: 'slice-of-life',
    setting: 'coffee shop',
  },
  {
    opening:
      'Moving to a new city is hard. But finding this particular apartment changed everything because',
    genre: 'slice-of-life',
    setting: 'new beginnings',
  },
  {
    opening:
      'Every Sunday, grandmother made the same recipe. But this time, she added something different—',
    genre: 'slice-of-life',
...
export function createInitialState(genre?: StoryBuilderState['genre']): StoryBuilderState {
 ...
 */
function isValidContribution(text: string): boolean {
  // Must have some content
  if (text.trim().length < 5) return false;

  // Check for stop commands (not valid contribution)
  const lower = text.toLowerCase();
  if (lower === 'stop' || lower === 'quit' || lower === 'end') return false;
 ...
 */
function generateAIContinuation(state: StoryBuilderState, userPart: string): string {
  const lower = userPart.toLowerCase();

  // Determine what type of continuation makes sense
  let continuationType: keyof typeof STORY_CONTINUATIONS = 'transition';
 ...
 */
function shouldTransitionChapter(state: StoryBuilderState): boolean {
...
function getStartMessage(state: StoryBuilderState): string {
  const genreNames: Record<StoryBuilderState['genre'], string> = {
...
function getContinuationMessage(aiPart: string, fullStory: string[]): string {
...
function getChapterTransitionMessage(chapter: number): string {
  return `\n📖 Chapter ${chapter}\n\n`;
}

function getInvalidContributionMessage(): string {
  return 'Add a bit more to the story! What happens next?';
}

function getEndGameMessage(state: StoryBuilderState): string {
 ...
 */
export function processInput(state: StoryBuilderState, input: string): StoryBuilderResult {
  ...
  if (
    ...
    // Add a story ending
    const finalState: StoryBuilderState = {
      ...state,
 ...
 */
export function describeStateForVoice(state: StoryBuilderState): string {
 ...
 */
export function getStartResult(state: StoryBuilderState): StoryBuilderResult {
  return {
 ...
 */
export function getFullStory(state: StoryBuilderState): string {
