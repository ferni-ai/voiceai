...
import { searchSong, searchSongWithWord, searchSongForMood, getRandomGameSongs, playGameTrack, stopGameTrack, fadeOutGameTrack, isMusicAvailable, } from './game-music.js';
import { getSongSelectionContext, getMusicalDNAMessage, } from './game-intelligence.js';
import { FinishTheLyricGame, DecadeChallengeGame, setGameMemoryForNewGames, } from './new-music-games.js';
const log = getLogger();
// ============================================================================
// GAME FACTORY
// ============================================================================
 ...
 */
export function setGameMemoryForGames(memory) {
    sharedGameMemory = memory;
    setGameMemoryForNewGames(memory);
}
export function getMusicGameImplementation(gameType, personaId) {
    switch (gameType) {
        case 'name-that-tune':
...
class NameThatTuneGame {
    ...
    async evaluateAnswer(answer, gameData, round) {
        ...
        const isCorrect = data.acceptableAnswers.some((acceptable) => normalized.includes(acceptable.toLowerCase()));
        if (isCorrect) {
    ...
    async getWelcomeMessage(rounds, firstSong) {
        const intros = {
            hype: [
...
class OneWordSongGame {
    ...
    async evaluateAnswer(answer, gameData) {
        const data = gameData;
...
class DesertIslandDiscsGame {
    ...
    async evaluateAnswer(answer, gameData, round) {
        ...
        if (isLastPick) {
            ...
            if (sharedGameMemory) {
                sharedGameMemory.desertIslandPicks = data.pickedSongs.map((s) => s.artist ? `${s.name} by ${s.artist}` : s.name);
    ...
    getPickFeedback(songName, pickNumber) {
        const feedbacks = {
...
class ThisOrThatGame {
    ...
    async getWelcomeMessage(firstMatchup) {
...
//# sourceMappingURL=music-games.js.map