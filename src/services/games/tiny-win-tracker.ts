...
export function createInitialState(): TinyWinTrackerState {
  return {
    phase: 'prompt',
    wins: [],
    sessionWinCount: 0,
...
function categorizeWin(text: string): TinyWin['category'] {
...
function getCelebration(): string {
...
export function processInput(state: TinyWinTrackerState, input: string): TinyWinTrackerResult {
  const lower = input.toLowerCase().trim();
 ...
 */
export function describeStateForVoice(state: TinyWinTrackerState): string {
 ...
 */
export function getStartResult(state: TinyWinTrackerState): TinyWinTrackerResult {
  const prompt = OPENING_PROMPTS[Math.floor(Math.random() * OPENING_PROMPTS.length)];
 ...
 */
export function getSessionWins(state: TinyWinTrackerState): TinyWin[] {
  return state.wins;
}
