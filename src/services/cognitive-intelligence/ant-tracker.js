...
function serializePattern(pattern) {
    return {
        ...
        timeOfDayPatterns: Object.fromEntries(Array.from(pattern.timeOfDayPatterns.entries()).map(([k, v]) => [k, v])),
        dayOfWeekPatterns: Object.fromEntries(Array.from(pattern.dayOfWeekPatterns.entries()).map(([k, v]) => [k, v])),
        topicTriggers: Object.fromEntries(Array.from(pattern.topicTriggers.entries()).map(([k, v]) => [k, v])),
...
function deserializePattern(data) {
    return {
        userId: data.userId,
        distortionFrequency: new Map(Object.entries(data.distortionFrequency)),
        topDistortions: data.topDistortions,
        timeOfDayPatterns: new Map(Object.entries(data.timeOfDayPatterns)),
        dayOfWeekPatterns: new Map(Object.entries(data.dayOfWeekPatterns)),
        topicTriggers: new Map(Object.entries(data.topicTriggers)),
        distortionTrend: data.distortionTrend,
        reframingSuccess: data.reframingSuccess,
        firstRecorded: data.firstRecorded?.toDate?.() ||
            new Date(data.firstRecorded),
...
const antEntries = new Map(); // userId -> entries
// ============================================================================
 ...
 */
async function loadPatternFromFirestore(userId) {
    ...
    try {
        ...
        if (!doc.exists)
            return null;
        const pattern = deserializePattern(doc.data());
 ...
 */
async function savePatternToFirestore(pattern) {
    const db = getFirestore();
    if (!db)
        return;
    try {
 ...
 */
async function loadEntriesFromFirestore(userId, limit = 500) {
    const db = getFirestore();
    if (!db)
        return [];
    try {
 ...
 */
export async function loadUserANTData(userId) {
    // Check cache first
 ...
 */
export function recordANT(userId, detection, options) {
    const entry = {
    ...
    if (pattern)
        void savePatternToFirestore(pattern);
 ...
 */
export function recordReframeResponse(userId, entryId, accepted) {
 ...
 */
export function getANTPatterns(userId) {
 ...
 */
export function generateWeeklyReport(userId) {
    const entries = antEntries.get(userId) || [];
    const pattern = getOrCreatePattern(userId);
    const now = new Date();
    ...
    const lastWeekEntries = entries.filter((e) => e.timestamp >= twoWeeksAgo && e.timestamp < weekAgo);
    // Calculate stats
 ...
 */
export function getANTContextInjection(userId) {
    const pattern = getOrCreatePattern(userId);
    const insights = getInsights(userId);
...
// INTERNAL HELPERS
// ============================================================================
function getOrCreatePattern(userId) {
...
function updatePatterns(userId, entry) {
    const pattern = getOrCreatePattern(userId);
    // Update frequency
...
function calculateTrend(userId) {
    ...
    const previousCount = entries.filter((e) => e.timestamp.getTime() >= fourWeeksAgo && e.timestamp.getTime() < twoWeeksAgo).length;
    if (previousCount === 0)
        return 'stable';
...
function estimateIntensity(detection) {
    // Higher confidence + more pattern history = higher intensity
    let intensity = detection.confidence;
    // Boost for repeated patterns
    if (detection.patternCount > 5)
        intensity += 0.1;
    if (detection.patternCount > 10)
        intensity += 0.1;
    return Math.min(intensity, 1);
}
function analyzeTemporalPatterns(pattern, entries) {
    if (entries.length < 10)
        return null;
    // Find peak time
    let maxTimeCount = 0;
    ...
    if (peakTimePercentage < 30)
        return null; // Not a strong pattern
    return {
        type: 'temporal',
...
function analyzeTopicPatterns(pattern) {
    if (pattern.topicTriggers.size < 3)
        return null;
    // Find most triggering topic
    let maxCount = 0;
    let triggerTopic = '';
    let triggerDistortions = [];
...
function analyzeProgress(entries) {
    ...
    if (successRate > 0.6) {
        ...
        return {
            ...
            confidence: 0.7,
            actionable: "Try noticing the thought without trying to change it first. Just 'there it is again.'",
        };
    }
    return null;
}
function analyzeDistortionPatterns(pattern) {
    if (pattern.topDistortions.length < 2)
        return null;
...
function generateCelebration(percentageChange, reframingSuccess) {
    const celebrations = [
...
function generateFocusArea(topDistortion, pattern) {
    if (!topDistortion) {
        return 'This week, try to notice when a negative thought pops up, without judging it.';
    }
    const focusAreas = {
        catastrophizing: "This week, when you notice catastrophizing, ask 'What's most likely to happen?'",
...
function formatDistortion(type) {
    const names = {
 ...
 */
export async function clearOldANTData(daysToKeep = RETENTION_DAYS) {
    const db = getFirestore();
    if (!db) {
    ...
    try {
        ...
        for (const userDoc of usersSnapshot) {
            ...
            while (true) {
                ...
                if (entriesSnapshot.size < FIRESTORE_BATCH_LIMIT)
                    break;
            }
        }
 ...
 */
export async function getAllUsersWithANTData() {
    const db = getFirestore();
    // Include in-memory users
    const users = new Set(userPatterns.keys());
    if (db) {
        try {
 ...
 */
export async function deleteUserANTData(userId) {
    // Clear from cache
...
//# sourceMappingURL=ant-tracker.js.map