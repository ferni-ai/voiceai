...
function updateUserHistory(userId, detection) {
 ...
 */
export function detectDistortions(userId, message, context) {
    const detections = [];
    const history = getUserHistory(userId);
    for (const [distortionType, pattern] of Object.entries(DISTORTION_PATTERNS)) {
        const type = distortionType;
        ...
        for (const regex of pattern.patterns) {
            ...
            if (match) {
                ...
                if (confidence >= 0.5) {
                    ...
                    log.debug({ userId, type, confidence, trigger: match[0] }, 'Cognitive distortion detected');
 ...
 */
export function getGentleResponse(detection) {
 ...
 */
export function getUserDistortionStats(userId) {
    const history = getUserHistory(userId);
    const topDistortions = Array.from(history.distortionCounts.entries())
        .map(([type, count]) => ({ type, count }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 5);
    const totalDetections = Array.from(history.distortionCounts.values()).reduce((sum, count) => sum + count, 0);
 ...
 */
export function getANTProfile(userId) {
 ...
 */
export function isCommonDistortion(userId, type) {
    const history = getUserHistory(userId);
 ...
 */
export function getDistortionContextInjection(detections) {
...
// HELPERS
// ============================================================================
function calculateConfidence(message, pattern, context) {
    let confidence = 0.6; // Base confidence for regex match
    // Boost for keywords present
    const keywordsFound = pattern.keywords.filter((kw) => message.toLowerCase().includes(kw.toLowerCase())).length;
...
function formatDistortionName(type) {
    return type
        .split('_')
...
//# sourceMappingURL=distortion-detector.js.map