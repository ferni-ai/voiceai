...
function detectTopicCategory(topic, reason) {
...
// ============================================================================
function detectRelationshipStage(context) {
    ...
    if (context.isFirstTimeUser || sessions <= 1)
        return 'new';
    if (sessions <= 5)
        return 'building';
    if (sessions <= 20)
...
// ============================================================================
function generateSoftOpenBanter(fromPersonaId, toPersonaId, context) {
    ...
    if (relationshipProfile.warmth === 'intimate' &&
        !isRepeatHandoff &&
...
function generateArrivingBanter(toPersonaId, fromPersonaId, context) {
    const handoffCount = context.handoffCountThisSession || 0;
    const timeProfile = TIME_ENERGY[context.timeOfDay || 'afternoon'];
    const relationshipStage = detectRelationshipStage(context);
    const topicCategory = detectTopicCategory(context.currentTopic, context.handoffReason);
...
// HELPER FUNCTIONS
// ============================================================================
function getMinimalHandoff(toPersonaId) {
...
function getMinimalArrival(toPersonaId, topicCategory) {
    const questions = {
        'alex-chen': {
...
function getTopicQuestion(personaId, topicCategory) {
    const questions = {
        'alex-chen': {
...
function getReasonAcknowledgment(personaId, reason) {
...
function getPersonaFirstName(personaId) {
    const names = {
        ferni: 'Ferni',
        'alex-chen': 'Alex',
        'maya-santos': 'Maya',
 ...
 * });
 */
export function getIntelligentBanter(fromPersonaId, toPersonaId, context = {}) {
    ...
    return {
        softOpenBanter: getHandoffBanter(fromPersonaId, toPersonaId) ||
 ...
 */
export function buildLLMSoftOpenInstructions(fromPersonaId, toPersonaId, context) {
    ...
    const brevity = (context.handoffCountThisSession || 0) > 1
    ...
    // Relationship depth
    const relationshipHint = context.relationshipStage === 'deep'
 ...
 */
export function buildLLMArrivingInstructions(toPersonaId, fromPersonaId, context) {
    const toProfile = PERSONA_PROFILES[toPersonaId] || PERSONA_PROFILES.ferni;
    const fromProfile = PERSONA_PROFILES[fromPersonaId] || PERSONA_PROFILES.ferni;
    // Build contextual hints
    const contextHints = [];
 ...
 */
export function getLLMDrivenBanter(fromPersonaId, toPersonaId, context = {}) {
    const enrichedContext = {
        ...context,
        timeOfDay: context.timeOfDay || detectTimeOfDay(),
    };
...
//# sourceMappingURL=intelligent-banter.js.map