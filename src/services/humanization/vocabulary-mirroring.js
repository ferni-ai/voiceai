 ...
 */
export function analyzeVocabulary(userId, context) {
 ...
 */
function updateStyleIndicators(profile, message) {
    ...
    // Intensity
    const intensifierCount = (lower.match(/\b(super|literally|totally|absolutely|extremely|so|really|very)\b/gi) || []).length;
    // 🦀 Rust-accelerated word counting
    ...
    // Filler frequency
    const fillerCount = (lower.match(/\b(basically|honestly|actually|you know|i mean|like)\b/gi) || []).length;
 ...
 */
export function getMirrorOpportunities(userId, responseContext, maxOpportunities = 2) {
    const profile = getOrCreateProfile(userId);
    const opportunities = [];
    ...
    const candidates = profile.items
        ...
        if (item.mirrored && item.mirrorCount > 2)
            return false;
        // Need some frequency
        if (item.frequency < 2)
            return false;
 ...
 */
function buildMirrorSuggestion(item, style) {
 ...
 */
export function generateMirrorPhrase(word, category, _context) {
    switch (category) {
        case 'emotional':
            return [
 ...
 */
export function markWordMirrored(userId, word, landed) {
    const profile = getOrCreateProfile(userId);
    ...
    if (item) {
        ...
        logger.debug({ userId, word, mirrorCount: item.mirrorCount, landed }, 'Marked word as mirrored');
    }
}
/**
 * Check if a word should be mirrored
 */
export function shouldMirrorWord(userId, word) {
    const profile = getOrCreateProfile(userId);
    const item = profile.items.find((i) => i.word.toLowerCase() === word.toLowerCase());
    if (!item) {
 ...
 */
export function getTopVocabulary(userId, category, limit = 10) {
    const profile = userProfiles.get(userId);
    if (!profile)
        return [];
 ...
 */
export function getVocabSummary(userId) {
    const profile = userProfiles.get(userId);
    if (!profile) {
        return {
            topWords: [],
...
//# sourceMappingURL=vocabulary-mirroring.js.map