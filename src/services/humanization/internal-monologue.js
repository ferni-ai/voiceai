...
// ============================================================================
const activeMonologues = new Map();
/**
 * Get or create the monologue stream for a session
 */
function getMonologueStream(sessionId) {
 ...
 */
export function processForThoughts(sessionId, context, bundleRuntime) {
    ...
    for (const trigger of MEMORY_TRIGGERS) {
        if (trigger.pattern.test(context.userMessage)) {
            ...
            if (memory) {
                newThoughts.push(createThought({
                    type: 'memory_stirred',
    ...
    for (const pattern of CONCERN_PATTERNS) {
        if (pattern.pattern.test(context.userMessage)) {
            ...
            if (activeStream.some((t) => t.type === 'concern_forming' && t.topic === pattern.concern)) {
                ...
                const existing = activeStream.find((t) => t.type === 'concern_forming' && t.topic === pattern.concern);
                if (existing) {
 ...
 */
export function decideSurfacing(sessionId, context) {
    const stream = getMonologueStream(sessionId);
    if (stream.length === 0) {
 ...
 */
export function markThoughtSurfaced(sessionId, thoughtId) {
    const stream = getMonologueStream(sessionId);
 ...
 */
export function getInternalStateSummary(sessionId) {
    const stream = getMonologueStream(sessionId);
    if (stream.length === 0) {
        return {
...
function getMemoryContent(memoryType, bundleRuntime) {
    if (!bundleRuntime)
        return null;
    switch (memoryType) {
        ...
        case 'japan_experience':
            return (bundleRuntime.getRandomSensoryMemory?.()?.memory ||
        ...
        case 'tanaka_san':
            return (bundleRuntime.getMentorQuote?.()?.quote ||
...
function getConcernExpression(concernType) {
    switch (concernType) {
...
function getVulnerabilityContent(bundleRuntime) {
    if (!bundleRuntime) {
    ...
    if (admission)
        return `Can I tell you something? ${admission}`;
...
function generateSpontaneousThought(context, bundleRuntime) {
    // Only generate if we have sufficient relationship
    if (context.relationshipStage === 'stranger')
        return null;
    const types = ['tangent_impulse', 'appreciation', 'question_forming'];
 ...
 */
export function clearMonologue(sessionId) {
    activeMonologues.delete(sessionId);
}
/**
 * Clear all monologues (for testing)
 */
export function clearAllMonologues() {
...
//# sourceMappingURL=internal-monologue.js.map