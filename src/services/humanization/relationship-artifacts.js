 ...
 */
export function analyzeTurnForArtifacts(context, existingArtifacts) {
    ...
    if (context.isBreakthrough ||
        ...
        if (ferniResponse.toLowerCase().includes('i ') &&
 ...
 */
export function getBestCallback(artifacts, currentContext) {
    const { topic, turn } = currentContext;
    const candidates = [];
    // Score breakthroughs
    for (const bt of artifacts.breakthroughs) {
        // Skip if referenced too recently
        if (bt.lastReferenced && turn - bt.lastReferenced < 10)
            continue;
        let score = 0;
        // Topic match boost
    ...
    for (const ref of artifacts.insideReferences) {
        ...
        if (ref.lastUsed && turn - ref.lastUsed < 5)
            continue;
        let score = 0;
        // User used it back = confirmed it landed
        if (ref.userUsedItBack)
            score += 2;
        // Usage count sweet spot (2-5 uses is the sweet spot)
        if (ref.timesUsed >= 2 && ref.timesUsed <= 5)
            score += 2;
        if (score > 0) {
            candidates.push({
                type: 'reference',
 ...
 */
export function getVocabularyToMirror(artifacts, maxWords = 2) {
    // Prioritize:
    ...
    return artifacts.userVocabulary
        ...
        if (a.category === 'emotional')
            scoreA += 3;
        if (b.category === 'emotional')
            scoreB += 3;
        if (a.category === 'unique')
            scoreA += 2;
        if (b.category === 'unique')
            scoreB += 2;
 ...
 */
function generateCallbackPhrase(userMessage, topic) {
    const phrases = [
...
// HELPER FUNCTIONS
// ============================================================================
function determineReaction(message, intensity) {
    const lower = message.toLowerCase();
    if (intensity && intensity > 0.8) {
        if (lower.includes('cry') || lower.includes('tear'))
            return 'tears';
        return 'emotional';
    }
    if (/oh\s*(my\s*god|wow)|holy|damn/i.test(lower))
        return 'aha';
    if (/relief|finally|weight.*off/i.test(lower))
        return 'relief';
    return 'quiet';
}
function extractBreakthroughContent(message) {
...
function extractFerniShare(response) {
    // Extract the personal sharing portion
    const sentences = response.split(/[.!?]+/);
    const personalSentence = sentences.find((s) => s.toLowerCase().includes('i ') &&
        (s.includes('remember') || s.includes('feel') || s.includes('think')));
    return personalSentence?.trim().slice(0, 100) || '';
}
function extractReferenceName(match) {
...
function extractKeyFragment(message) {
 ...
 */
export function getOrCreateArtifacts(userId, personaId) {
 ...
 */
export function recordBreakthrough(userId, personaId, breakthrough) {
    ...
    if (artifacts.breakthroughs.length > 50) {
        artifacts.breakthroughs.sort((a, b) => {
            ...
            const scoreA = (a.userReaction === 'tears' ? 3 : a.userReaction === 'emotional' ? 2 : 1) +
                (a.timesReferenced === 0 ? 1 : 0);
            const scoreB = (b.userReaction === 'tears' ? 3 : b.userReaction === 'emotional' ? 2 : 1) +
    ...
    logger.info({ userId, personaId, breakthroughId: bt.id, topic: bt.topic }, 'Recorded relationship breakthrough');
    return bt;
}
/**
 * Record an inside reference
 */
export function recordInsideReference(userId, personaId, reference) {
    const artifacts = getOrCreateArtifacts(userId, personaId);
    const ref = {
        ...reference,
    ...
    if (artifacts.insideReferences.length > 30) {
        ...
        artifacts.insideReferences.sort((a, b) => {
            if (a.userUsedItBack && !b.userUsedItBack)
                return -1;
            if (b.userUsedItBack && !a.userUsedItBack)
                return 1;
 ...
 */
export function updateVocabulary(userId, personaId, word, category, context) {
    const artifacts = getOrCreateArtifacts(userId, personaId);
    const existing = artifacts.userVocabulary.find((v) => v.word.toLowerCase() === word.toLowerCase());
    if (existing) {
        existing.frequency++;
 ...
 */
export function markCallbackUsed(userId, personaId, type, artifactId, turn) {
    const artifacts = getOrCreateArtifacts(userId, personaId);
 ...
 */
export function markVocabularyMirrored(userId, personaId, word) {
    const artifacts = getOrCreateArtifacts(userId, personaId);
 ...
 */
export function markUserUsedReferenceBack(userId, personaId, referenceId) {
    const artifacts = getOrCreateArtifacts(userId, personaId);
 ...
 */
export function incrementTurns(userId, personaId) {
    const artifacts = getOrCreateArtifacts(userId, personaId);
 ...
 */
export function getArtifactsSummary(userId, personaId) {
    const artifacts = getOrCreateArtifacts(userId, personaId);
    return {
...
function describeCommunicationStyle(rhythm) {
    const parts = [];
...
//# sourceMappingURL=relationship-artifacts.js.map