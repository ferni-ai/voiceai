 ...
 */
export function getArcBehaviorRecommendation(currentTurn, previousPhase, context) {
    const arcTracker = getEmotionalArcTracker();
    const arc = arcTracker.getArc();
    const currentPhase = (arcTracker.getCurrentPhase?.() || determinePhaseFromArc(arc, currentTurn));
    ...
    const behaviors = {
        ...
        surfaceVulnerability: personality.innerWorldActive && (currentPhase === 'peak' || currentPhase === 'release'),
 ...
 */
function determinePhaseFromArc(arc, turn) {
 ...
 */
export function getPhasePersonality(phase) {
 ...
 */
export function shouldSurfaceInnerWorld(phase, emotionalIntensity, relationshipStage) {
    ...
    const innerWorldPhases = ['peak', 'release', 'building'];
    if (!innerWorldPhases.includes(phase) && !personality.innerWorldActive)
        return false;
    // HUMANIZATION FIX: Lower the intensity threshold
    // Emotional moments at 0.35+ can benefit from personal connection
    if (emotionalIntensity < 0.35)
        return false;
    ...
    const relationshipBonus = relationshipStage === 'acquaintance'
        ? 0.05
 ...
 */
export function areStoriesAppropriate(phase, context) {
    const personality = PHASE_PERSONALITIES[phase];
    if (!personality.storiesAppropriate)
        return false;
 ...
 */
export function getRecommendedResponseLength(phase, userMessageLength) {
    const personality = PHASE_PERSONALITIES[phase];
    const bases = {
...
//# sourceMappingURL=arc-aware-selector.js.map