 ...
 */
export function getUnlockedStories(context) {
    const results = [];
 ...
 */
export function getBestStoryForMoment(context) {
 ...
 */
function evaluateStoryUnlock(storyId, requirements, context) {
    let fitScore = 0;
    let isUnlocked = true;
    const reasons = [];
    // =========================================================================
    ...
    if (requirements.prerequisiteStories?.length) {
        const missingPrereqs = requirements.prerequisiteStories.filter((p) => !context.storiesTold.includes(p));
 ...
 */
export function isStoryUnlocked(storyId, context) {
    const requirements = FERNI_STORY_UNLOCKS[storyId];
    if (!requirements)
        return true; // Unknown stories are always "unlocked"
    const evaluation = evaluateStoryUnlock(storyId, requirements, context);
 ...
 */
export function getStoriesByDepth(depth) {
 ...
 */
export function getStoriesForTopic(topic, context) {
    const contextWithTopic = { ...context, currentTopic: topic };
    return getUnlockedStories(contextWithTopic).filter((r) => r.story.requirements.topicTriggers.some((t) => topic.toLowerCase().includes(t)));
}
/**
 * Get the introduction for a story
 */
export function getStoryIntroduction(storyId, hasBeenToldBefore) {
    const requirements = FERNI_STORY_UNLOCKS[storyId];
    if (!requirements) {
 ...
 */
export function registerStoryUnlock(storyId, requirements) {
...
// ============================================================================
const storyProgression = new Map();
/**
 * Get stories that naturally follow from a told story
 */
export function getFollowUpStories(storyId, context) {
    // Find stories that have this story as a prerequisite
    const followUps = Object.entries(FERNI_STORY_UNLOCKS)
        .filter(([, req]) => req.prerequisiteStories?.includes(storyId) && !context.storiesTold.includes(storyId))
        .map(([id]) => id);
 ...
 */
export function recordStoryTold(sessionId, storyId) {
 ...
 */
export function getStoriesToldThisSession(sessionId) {
    return storyProgression.get(sessionId) || [];
}
/**
 * Clear session story tracking
 */
export function clearStoryProgression(sessionId) {
...
//# sourceMappingURL=story-unlocks.js.map