...
async function initializeFirestore() {
    try {
        ...
        getLogger().warn({ error }, 'Firestore not available for outreach analytics, using in-memory only');
 ...
 */
async function saveUserAnalytics(userId, analytics) {
    const firestore = await getFirestore();
    if (firestore) {
        try {
            await firestore
 ...
 */
export function logOutreachEvent(event) {
    ...
    getLogger().debug({ eventId: id, userId: event.userId, trigger: event.trigger, status: event.status }, '📊 Outreach event logged');
    return id;
}
/**
 * Update event status (e.g., when user responds)
 */
export function updateEventStatus(eventId, status, responseTime, responseType) {
 ...
 */
export function logResponse(userId, trigger, responseType) {
    ...
    if (recentEvents.length > 0) {
        ...
        getLogger().info({ userId, trigger, responseTimeMinutes: Math.round(responseTime / 60000) }, '📊 User responded to outreach');
    }
}
// ============================================================================
// USER ANALYTICS
...
export function getGlobalAnalytics() {
    ...
    const responseRateByHour = new Array(24).fill(0);
    ...
    const responseRateByDay = new Array(7).fill(0);
 ...
 */
export async function getOptimizationRecommendations(userId) {
...
//# sourceMappingURL=outreach-analytics.js.map