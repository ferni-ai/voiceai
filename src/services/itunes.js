 ...
 */
export async function searchItunes(query, limit = 5) {
    ...
    if (!itunesCircuitBreaker.canRequest()) {
        log.warn({ query, circuitState: 'OPEN' }, '🎵 [DIAG] iTunes circuit breaker is OPEN - skipping request! Music search will fail.');
        return { resultCount: 0, results: [] };
    }
    log.info({ query, circuitState: 'CLOSED' }, '🎵 [DIAG] iTunes circuit breaker allows request');
    if (DEBUG_ITUNES)
        log.debug('searchItunes called', { query, limit, url });
    log.info({ query, limit }, 'iTunes API searching');
    try {
        const data = await itunesCircuitBreaker.execute(async () => {
            if (DEBUG_ITUNES)
                log.debug('Making fetch request', { url });
            ...
            getLogger().info({ status: response.status, ok: response.ok }, '🎵 [iTunes API] Response received');
            if (!response.ok) {
 ...
 */
export async function findTrack(query) {
    // 🐛 FIX: Check circuit breaker BEFORE calling searchItunes
    // If circuit is open, return a more informative error message
    if (!itunesCircuitBreaker.canRequest()) {
        log.warn({ query, circuitState: 'OPEN' }, '🎵 [DIAG] iTunes circuit breaker OPEN - returning service unavailable message');
        return {
            found: false,
...
export async function searchByMood(mood) {
    ...
    const moodKey = Object.keys(moodQueries).find((k) => mood.toLowerCase().includes(k)) || 'relaxing';
 ...
 */
export async function isItunesAvailable() {
    try {
