 ...
 */
export function selectBestSource(intent, config, options) {
    ...
    if (roomSpecified && config.sonos.available) {
        const roomMatch = config.sonos.rooms.some((r) => r.toLowerCase().includes(roomSpecified.toLowerCase()) ||
 ...
 */
export function extractRoomFromQuery(query) {
    const lowerQuery = query.toLowerCase();
 ...
 */
export function getBestSource(intent, options = {}) {
    const { spotifyLinked = false, spotifyPremium = false, spotifyDeviceReady = false, appleMusicAvailable = false, sonosLinked = false, sonosRooms = [], } = options;
 ...
 */
export function formatPlaybackResult(result, room) {
    if (!result.success) {
 ...
 */
export function logMusicPlay(userId, track, intent, success) {
    log.info({
        userId,
