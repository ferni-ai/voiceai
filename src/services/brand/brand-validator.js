 ...
 */
export async function validateBrandCompliance(content, options = {}) {
 ...
 */
function checkBannedPhrases(content, context) {
    const violations = [];
 ...
 */
function checkAvoidedWords(content, context) {
    const violations = [];
    for (const wordRule of context.voice.wordsToAvoid) {
        ...
        if (match) {
            violations.push({
                ...
                suggestion: wordRule.useInstead === "(just don't mention)" ||
 ...
 */
function checkPersonaAntiPatterns(content, personaId) {
    const violations = [];
 ...
 */
function analyzeTone(content, context) {
    const violations = [];
 ...
 */
function checkCorporateLanguage(content) {
    const violations = [];
 ...
 */
function checkSaccharineLanguage(content) {
    const violations = [];
 ...
 */
export function autoFixViolations(content) {
    let fixed = content;
    const changes = [];
    const avoided = getWordsToAvoid();
    for (const rule of avoided) {
 ...
 */
export function getQuickScore(content) {
