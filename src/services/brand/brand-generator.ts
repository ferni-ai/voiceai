...
export async function generateBrandContent(
  ...
  log.info(
    { type: request.type, persona: personaId, context: request.context.emotion },
 ...
 * Generate a CTA
 */
function generateCTA(context: ContextType): string {
  const ctas: Record<ContextType, string[]> = {
 ...
 */
function generateNotification(persona: PersonaVoice): string {
  const notifications = [
    'Thinking of you.',
 ...
 */
function generateToast(context: ContextType): string {
  const toasts: Record<ContextType, string> = {
 ...
 */
function getMaxTokens(type: GenerationRequest['type']): number {
  const maxTokens: Record<GenerationRequest['type'], number> = {
    headline: 50,
    cta: 20,
    toast: 30,
    notification: 100,
...
export async function generateExperimentVariants(
  experimentType: 'headline' | 'cta' | 'notification',
  context: ContextType = 'marketing',
  llmClient?: LLMClient
): Promise<{ variantId: string; content: string; score: number }[]> {
