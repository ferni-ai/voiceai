...
import { getWebExperiments } from '../experiments/web-experiments.js';
 ...
 */
export async function extractBrandLearnings() {
    const patterns = [];
    try {
 ...
 */
function analyzeWinningVariant(experiment) {
 ...
 */
function findDistinguishingFeatures(winner, control) {
    const features = [];
 ...
 */
function findMetaPatterns(patterns) {
    const metaPatterns = [];
    const patternCounts = {};
 ...
 */
export async function evolveBrandRules(learnings) {
    const changes = [];
    ...
    for (const learning of learnings) {
        ...
        if (learning.pattern.startsWith('[META]'))
            continue;
        const change = {
 ...
 */
export async function recordFailedApproach(approach, reason, experimentId) {
    const db = getFirestore();
    try {
        await db
            .collection('brand_learnings')
 ...
 */
export async function calculateBrandHealth() {
    const db = getFirestore();
    try {
        ...
        const validationSnap = await db
            ...
            .limit(1000)
            .get();
        const validations = validationSnap.docs.map((d) => d.data());
        ...
        const recentLearnings = learningsSnap.data()?.patterns || [];
 ...
 */
export async function logValidation(content, result, context) {
    const db = getFirestore();
    try {
        await db.collection('brand_validations').add(removeUndefined({
...
export async function runDailyEvolution() {
    ...
    log.info({ learnings: learnings.length, changes: changes.length }, 'Daily brand evolution complete');
    return { learnings, changes };
}
/**
 * Get recent rule changes
 */
export async function getRecentRuleChanges(days = 30) {
    const db = getFirestore();
    try {
