...
export async function loadBrandContext() {
    ...
    cacheTime = Date.now();
    log.info({ personas: Object.keys(context.personas).length, principles: context.voice.principles.length }, 'Brand context loaded');
    return context;
}
/**
 * Load brand learnings from experiments
 */
async function loadBrandLearnings() {
    try {
        ...
        return {
            winningPatterns: patternsSnap.data()?.patterns || [],
 ...
 */
export function clearBrandContextCache() {
    cachedContext = null;
    cacheTime = 0;
 ...
 */
export async function getBrandContextForPersona(personaId) {
 ...
 */
export function getWordsToAvoid() {
    return WORDS_TO_AVOID;
}
/**
 * Get tone config for a context
 */
export function getToneConfig(context) {
