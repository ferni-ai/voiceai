 ...
 */
export function adaptForChannel(content, toChannel, options = {}) {
 ...
 */
function shortenContent(content, maxLength, context) {
 ...
 */
function adjustFormality(content, level) {
    if (level === 'casual') {
 ...
 */
function addChannelFraming(content, channel, personaId, context) {
 ...
 */
function addEmailFraming(content, personaName, context) {
    // Add warm opening if not present
    const hasGreeting = content.toLowerCase().startsWith('hey') ||
    ...
    const hasClosing = framed.toLowerCase().includes('— ' + personaName.toLowerCase()) ||
 ...
 */
function addSmsFraming(content) {
 ...
 */
function addPushFraming(content, personaName) {
 ...
 */
export function generateForAllChannels(baseContent, options = {}) {
    const { persona = 'ferni', context = 'checkin', channels } = options;
    const targetChannels = channels || ['app', 'email', 'sms', 'push'];
    const result = {};
 ...
 */
export function fitsChannelConstraints(content, channel) {
    const config = CHANNEL_CONFIGS[channel];
    const issues = [];
    ...
    if (!config.constraints.allowEmoji &&
 ...
 */
export function checkVoiceConsistency(contents) {
    const issues = [];
    const channels = Object.keys(contents);
 ...
 */
function analyzeContentTone(content) {
    ...
    if (/\?/.test(content))
        energy += 0.5;
    if (/amazing|exciting|huge/i.test(content))
        energy += 1;
    if (/quiet|gentle|soft/i.test(content))
        energy -= 1;
    // Warmth indicators
    if (/you|your|we|our/i.test(content))
        warmth += 1;
    if (/thinking of|here for|care/i.test(content))
        warmth += 1;
    if (/must|should|required/i.test(content))
        warmth -= 1;
    return {
