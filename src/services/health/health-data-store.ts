...
export async function getHealthSummary(
  ...
  try {
    ...
    return doc.data() as HealthSummary;
  } catch (error) {
...
export async function getRecentHealthSummaries(
  ...
  try {
    ...
    return snapshot.docs.map((doc) => doc.data() as HealthSummary);
  } catch (error) {
 ...
 */
export async function getHealthPreferences(userId: string): Promise<HealthPreferences | null> {
  const db = getFirestoreDb();
  if (!db) return null;
  ...
  try {
    ...
    return doc.data() as HealthPreferences;
  } catch (error) {
 ...
 */
export async function handleHealthSync(request: HealthSyncRequest): Promise<HealthSyncResponse> {
  ...
  try {
    ...
    const filteredSummary: HealthSummary = {
      userId,
      date: today,
      syncedAt: timestamp,
    ...
    if (!preferences || preferences.shareSleep) {
      if (summary.sleepHours !== undefined) filteredSummary.sleepHours = summary.sleepHours;
      if (summary.sleepQuality !== undefined) filteredSummary.sleepQuality = summary.sleepQuality;
      if (summary.bedtime !== undefined) filteredSummary.bedtime = summary.bedtime;
      if (summary.wakeTime !== undefined) filteredSummary.wakeTime = summary.wakeTime;
    }

    if (!preferences || preferences.shareStress) {
      if (summary.hrvValue !== undefined) filteredSummary.hrvValue = summary.hrvValue;
      if (summary.hrvTrend !== undefined) filteredSummary.hrvTrend = summary.hrvTrend;
    ...
    if (preferences?.shareCycle) {
      if (summary.cycleDay !== undefined) filteredSummary.cycleDay = summary.cycleDay;
      if (summary.cyclePhase !== undefined) filteredSummary.cyclePhase = summary.cyclePhase;
    }
 ...
 */
export async function getHealthContextInjection(userId: string): Promise<string> {
