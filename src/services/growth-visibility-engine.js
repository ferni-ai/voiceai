...
export class GrowthVisibilityEngine {
    ...
    detectGrowth() {
        ...
        const daysBetween = Math.floor((newSnapshot.capturedAt.getTime() - oldSnapshot.capturedAt.getTime()) / (1000 * 60 * 60 * 24));
        ...
        for (const [topic, newComfort] of newSnapshot.topicComfort) {
            ...
            if (oldComfort && this.hasComfortImproved(oldComfort, newComfort)) {
                detectedInsights.push(this.createGrowthInsight('topic_comfort', {
                    area: topic,
        ...
        if (this.hasDepthIncreased(oldSnapshot.conversationDepth, newSnapshot.conversationDepth)) {
            detectedInsights.push(this.createGrowthInsight('depth_increase', {
        ...
        if (this.hasSelfAwarenessGrown(oldSnapshot.selfAwareness, newSnapshot.selfAwareness)) {
            detectedInsights.push(this.createGrowthInsight('self_awareness', {
                area: 'Self-awareness',
        ...
        this.insights = this.insights.filter((i) => !i.surfaced ||
     ...
     */
    hasComfortImproved(old, current) {
    ...
    formatTimespan(days) {
        if (days < 7)
            return 'this week';
        if (days < 14)
            return 'the past couple weeks';
        if (days < 30)
            return 'the past month';
        if (days < 60)
            return 'the past couple months';
        if (days < 90)
            return 'the past few months';
        if (days < 180)
            return 'the past half year';
        if (days < 365)
     ...
     */
    getSuggestedMoment(insight) {
        if (insight.type === 'topic_comfort')
            return 'after_related_topic';
        if (insight.timespan.durationDays >= 30)
            return 'milestone';
    ...
    getInsightToSurface(context) {
        ...
        if (context?.currentTopic) {
            const topicRelated = candidates.filter((i) => i.area.toLowerCase().includes(context.currentTopic.toLowerCase()));
            if (topicRelated.length > 0) {
                candidates = topicRelated;
            }
        }
        if (context?.sessionStart) {
            const sessionStartCandidates = candidates.filter((i) => i.type === 'depth_increase' || i.type === 'self_awareness');
     ...
     */
    recordReaction(insightId, reaction) {
    ...
    importFromProfile(profile) {
        ...
        if (profile.customData?.growthSnapshots) {
            this.snapshots = profile.customData.growthSnapshots;
        }
        if (profile.customData?.growthInsights) {
            this.insights = profile.customData.growthInsights;
        }
        log.debug({ snapshots: this.snapshots.length, insights: this.insights.length }, 'Imported growth data');
    }
    /**
    ...
    getStats() {
        ...
        for (const insight of this.insights) {
            ...
            if (insight.surfaced)
                surfaced++;
            if (insight.userReaction === 'resonated')
                resonated++;
        }
        return {
...
export function getGrowthVisibilityEngine(userId) {
    if (!engines.has(userId)) {
        engines.set(userId, new GrowthVisibilityEngine(userId));
    }
    return engines.get(userId);
}
export function resetGrowthVisibilityEngine(userId) {
...
//# sourceMappingURL=growth-visibility-engine.js.map