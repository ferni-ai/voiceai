...
import { scheduleText, scheduleEmail, scheduleCall, getUserContactInfo, setUserContactInfo, } from './outreach/user-contact.js';
 ...
 */
export async function initializeOutreachPersistence() {
    if (isInitialized)
        return;
 ...
 */
function persistUserData(userId) {
    if (!persistenceStore)
        return;
    const data = {
 ...
 */
export async function shutdownOutreachPersistence() {
    if (persistenceStore) {
 ...
 */
export async function clearUserOutreachData(userId) {
 ...
 */
export function clearAllOutreachData() {
 ...
 */
export function pruneStaleOutreachData(maxAgeDays = 30) {
 ...
 */
export function extractCommitments(userId, conversationText, conversationTime = new Date()) {
    const commitments = [];
    ...
    for (const pattern of commitmentPatterns) {
        ...
        while ((match = pattern.exec(lower)) !== null) {
            ...
            if (what && what.length > 3 && what.length < 100) {
                ...
                getLogger().info({ userId, what, when: targetTime.toISOString(), checkIn: checkInTime.toISOString() }, '📝 Commitment extracted');
            }
        }
    }
    // Store commitments
 ...
 */
function parseTimeReference(timeRef, fromDate) {
 ...
 */
function calculateCheckInTime(targetTime, timeRef) {
 ...
 */
export async function detectOutreachOpportunities(userId, agentId = AgentRole.COACH) {
    ...
    if (engagement && prefs.enabledTriggers.includes('reengagement')) {
        const daysSinceContact = (now.getTime() - engagement.lastInteraction.getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
export function detectEmotionalTriggers(userId, conversationText, agentId = AgentRole.COACH) {
    const prefs = getPreferences(userId);
    ...
    if (hasStruggle) {
        ...
        getLogger().info({ userId, trigger: 'emotional_support' }, '💙 Emotional support outreach scheduled');
        return opportunity;
    }
    return null;
}
 ...
 */
function adjustForPreferences(time, prefs) {
 ...
 */
export function canSendOutreach(userId) {
    const prefs = getPreferences(userId);
    ...
    if (weekCount >= prefs.maxPerWeek) {
        getLogger().debug({ userId, weekCount, max: prefs.maxPerWeek }, 'Weekly outreach limit reached');
        return false;
    }
    return true;
}
 ...
 */
function logOutreach(userId, trigger) {
    const log = sentOutreachLog.get(userId) || [];
 ...
 */
export function recordInteraction(userId, interactionTime = new Date(), respondedToOutreach = false, method) {
    const existing = engagementStore.get(userId);
    if (existing) {
        // Update average gap
        const gap = (interactionTime.getTime() - existing.lastInteraction.getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
export async function executeOutreach(opportunity) {
    ...
    if (suggestedTime > new Date()) {
        ...
        switch (method) {
            case 'email':
                result = await scheduleEmail(userId, '💬 Check-in from Ferni', message, suggestedTime, opportunity.agentId);
                break;
            case 'call':
        ...
        if (result.success) {
            ...
            getLogger().info({ userId, trigger, method, scheduledFor: suggestedTime }, '📤 Outreach scheduled');
        }
        return result;
    }
 ...
 */
export async function analyzeConversationForOutreach(userId, conversationText, agentId = AgentRole.COACH) {
    ...
    getLogger().info({ userId, commitments: commitments.length, opportunities: opportunities.length, scheduled }, '🧠 Conversation analyzed for outreach');
...
//# sourceMappingURL=outreach-intelligence.js.map