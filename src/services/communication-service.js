...
import { validateEmail, validatePhone, sanitizePlainText, sanitizeEmailForLog, sanitizePhoneForLog, } from '../utils/validation.js';
...
async function withRetry(fn, options = {}) {
    ...
    for (let attempt = 0; attempt < opts.maxRetries; attempt++) {
        try {
            ...
            getLogger().warn({ attempt: attempt + 1, maxRetries: opts.maxRetries, delay, error: lastError.message }, 'Retrying after failure');
...
export async function sendEmail(to, subject, body, isHtml = false) {
    ...
    return withRetry(async () => {
        ...
        getLogger().info({ to: sanitizeEmailForLog(to), subject: sanitizedSubject.slice(0, 50) }, 'Email sent successfully');
...
export async function sendSMS(to, message) {
    ...
    return withRetry(async () => {
        ...
        getLogger().info({ to: sanitizePhoneForLog(to), messageLength: sanitizedMessage.length }, 'SMS sent successfully');
 ...
 */
export async function sendReminder(to, reminderText, context) {
