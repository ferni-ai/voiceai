...
const sharedContent = new Map(); // personaId:userId -> Set of shared content
let persistenceStore = null;
let isInitialized = false;
/**
 * Initialize persistence for spontaneous sharing
 */
export async function initializeSpontaneousSharingPersistence() {
    if (isInitialized)
        return;
 ...
 */
async function loadSharedContent(userId) {
    if (!persistenceStore)
        return;
    try {
        const data = await persistenceStore.load(userId);
        if (!data?.sharedByPersona)
            return;
 ...
 */
function persistSharedContent(userId) {
    if (!persistenceStore)
        return;
    const sharedByPersona = {};
 ...
 */
export async function shutdownSpontaneousSharingPersistence() {
    if (persistenceStore) {
 ...
 */
async function ensureSharedContentLoaded(userId) {
...
function hasBeenShared(personaId, userId, content) {
    const key = `${personaId}:${userId}`;
    return sharedContent.get(key)?.has(content) ?? false;
}
function markAsShared(personaId, userId, content) {
    const key = `${personaId}:${userId}`;
 ...
 */
function meetsRelationshipGate(required, current) {
    const order = [
        'stranger',
        'acquaintance',
        'friend',
 ...
 */
export async function surfaceEnderingContradiction(context, userId) {
    ...
    const contradictions = quirks?.['endearing_contradictions'];
 ...
 */
export async function shareSimpleJoy(context, userId) {
    const behaviors = await loadPersonaBehaviors(context.personaId);
    if (!behaviors)
        return null;
    const quirks = behaviors['quirks'];
    const joys = quirks?.['things_that_make_me_unreasonably_happy'];
 ...
 */
export async function referencePetPeeve(context, userId) {
    if (!context.userMessage && !context.currentTopic)
        return null;
    const behaviors = await loadPersonaBehaviors(context.personaId);
    if (!behaviors)
        return null;
    const quirks = behaviors['quirks'];
    const peeves = quirks?.['things_that_make_me_unreasonably_annoyed'];
    if (!peeves || peeves.length === 0)
        return null;
    // Ensure shared content is loaded from persistence
    await ensureSharedContentLoaded(userId);
 ...
 */
export async function shareGrowthEdge(context, userId) {
    // Need friend level
    ...
    if (!behaviors)
        return null;
    const quirks = behaviors['quirks'];
    const edges = quirks?.['growth_edges'];
 ...
 */
export async function shareRelationshipMoment(context, userId) {
    // Need friend level
    if (!meetsRelationshipGate('friend', context.relationshipStage)) {
        return null;
    }
    ...
    if (!behaviors)
        return null;
    const quirks = behaviors['quirks'];
    const moments = quirks?.['relationship_moments'];
 ...
 */
export async function shareGuiltyPleasure(context, userId) {
    ...
    if (!behaviors)
        return null;
    const quirks = behaviors['quirks'];
    const pleasures = quirks?.['guilty_pleasures'];
    if (!pleasures || pleasures.length === 0)
        return null;
    // Low chance
    if (Math.random() > 0.1)
        return null;
 ...
 */
export async function trySpontaneousShare(context, userId) {
 ...
 */
export async function clearSharedContent(personaId, userId) {
    const key = `${personaId}:${userId}`;
...
//# sourceMappingURL=spontaneous-sharing.js.map