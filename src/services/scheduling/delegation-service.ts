...
export class DelegationService extends EventEmitter {
  ...
  updateDelegation(
    ...
    if (update.status) {
      ...
      if (update.status === 'accepted') delegation.acceptedAt = new Date();
      if (update.status === 'completed') delegation.completedAt = new Date();
    }

    if (update.outcome) delegation.outcome = update.outcome;
   ...
   */
  async getActiveDelegationsForPersona(userId: string, personaId: string): Promise<Delegation[]> {
   ...
   */
  async getPendingDelegationsForPersona(userId: string, personaId: string): Promise<Delegation[]> {
    return this.getUserDelegations(userId, {
      toPersona: personaId,
      status: 'delegated',
    });
