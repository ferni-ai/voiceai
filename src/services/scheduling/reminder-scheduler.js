...
import { removeCalendarSyncedItem, syncReminderToCalendar, syncScheduledCallToCalendar, syncScheduledEmailToCalendar, syncScheduledTextToCalendar, } from '../calendar/calendar-bridge.js';
...
const voiceMessageStore = new Map();
// ============================================================================
...
export async function createReminder(params) {
    ...
    try {
        ...
        if (db) {
            ...
            getLogger().info({ reminderId: reminder.id, userId: params.userId }, '📅 Reminder saved to Firestore');
        ...
        getLogger().warn({ error: String(error) }, 'Firestore not available for reminders, using in-memory');
    }
    getLogger().info({
    ...
    try {
        ...
        if (params.isDirectToContact && params.contactName) {
            switch (reminder.deliveryMethod) {
                case 'sms':
                case 'voice_message':
                    calendarResult = await syncScheduledTextToCalendar(params.userId, reminder.id, params.contactName, params.message, params.scheduledFor, params.createdBy);
                    break;
                case 'email':
                    calendarResult = await syncScheduledEmailToCalendar(params.userId, reminder.id, params.contactName, params.subject || params.message.substring(0, 50), params.scheduledFor, params.createdBy);
                    break;
                case 'call':
                    calendarResult = await syncScheduledCallToCalendar(params.userId, reminder.id, params.contactName, params.context || 'Scheduled call', params.scheduledFor, 15, // Default 15 min duration for calls
                    params.createdBy);
                    break;
                default:
                    calendarResult = await syncReminderToCalendar(params.userId, reminder.id, params.message, params.scheduledFor, params.createdBy);
            }
        }
        else {
        ...
        getLogger().warn({ error: String(calendarError), reminderId: reminder.id }, '⚠️ Failed to sync to calendar (item still created)');
    }
    return reminder;
}
/**
 ...
 */
export async function loadRemindersFromFirestore(userId) {
    try {
        const { getFirestoreDb } = await import('../superhuman/firestore-utils.js');
        const db = getFirestoreDb();
        if (!db)
...
export async function cancelReminder(reminderId) {
    ...
    if (reminder && reminder.status === 'pending') {
        ...
        try {
            ...
            getLogger().warn({ error: String(error), reminderId }, 'Failed to update reminder status in Firestore');
        }
        ...
        try {
            ...
            getLogger().warn({ error: String(calendarError), reminderId }, '⚠️ Failed to remove reminder from calendar');
        }
        return true;
    }
    return false;
 ...
 */
async function updateReminderStatus(reminderId, status, error) {
    const reminder = reminderStore.get(reminderId);
    if (reminder) {
        ...
        try {
            ...
            getLogger().warn({ error: String(firestoreError), reminderId }, 'Failed to update reminder status in Firestore');
        }
    }
}
// ============================================================================
...
export async function deliverReminder(reminder) {
    ...
    try {
        switch (reminder.deliveryMethod) {
            case 'sms': {
                const smsResult = await sendReminderSMS(reminder.deliveryAddress, messageToDeliver, reminder.context);
            ...
            case 'email': {
                ...
                const emailResult = await sendEmail(reminder.deliveryAddress, subject, `${messageToDeliver}\n\n${reminder.context ? `Context: ${reminder.context}\n\n` : ''}— ${firstName}`);
            ...
            case 'call': {
                ...
                const personaId = reminder.personaId || reminder.createdBy || 'ferni';
                const callResult = await callWithPersonaVoice(reminder.deliveryAddress, messageToDeliver, personaId, { fallbackToTwilioVoice: true });
            ...
            case 'voice_message': {
                ...
                if (audioBuffer) {
                    ...
                    if (GCS_BUCKET) {
                        try {
                            ...
                            if (Storage) {
                                ...
                                logger.info({ reminderId: reminder.id, audioUrl }, '🎤 Voice audio uploaded to GCS');
                            }
                        }
                        catch (gcsError) {
                    ...
                    await sendSMS(reminder.deliveryAddress, `🎤 Voice message from ${firstName}: "${reminder.message}"`);
        ...
        if (reminder.contactId && reminder.isDirectToContact) {
            try {
                const channel = reminder.deliveryMethod === 'sms' || reminder.deliveryMethod === 'voice_message'
                    ? 'sms'
                ...
                getLogger().debug({ contactId: reminder.contactId, channel }, '📊 Recorded outcome for timing ML');
            }
            catch (error) {
 ...
 */
export async function sendVoiceMessage(voiceMessageId, toPhone) {
...
export function startReminderScheduler(intervalMs = 60000) {
    ...
    cleanupCounter = 0;
    registerInterval('reminder-scheduler', () => {
        void (async () => {
 ...
 */
export function cleanupOldReminders(maxAgeMs = 7 * 24 * 60 * 60 * 1000) {
 ...
 */
export function parseNaturalTime(expression, timezone = 'America/New_York') {
    const now = new Date();
