 ...
 */
async function getActiveUsersForInsights(limit = 50) {
...
class ProactiveInsightsService {
    ...
    start() {
        ...
        registerInterval('proactive-insights-scanner', () => {
     ...
     */
    async runScanForUser(userId, scanType = 'daily') {
        ...
        getLogger().info({ userId, insightCount: insights.length, domainsScanned, patternsChecked }, '🔬 Peter completed proactive insight scan');
        return result;
    }
    /**
     * Scan spending domain for insights
     */
    async scanSpendingDomain(userId) {
        const insights = [];
     ...
     */
    async scanBehavioralDomain(userId) {
        const insights = [];
        ...
        const stressTriggers = triggers.filter((t) => ['stressed', 'anxious', 'overwhelmed', 'tired', 'exhausted'].includes(t.emotion.toLowerCase()));
     ...
     */
    async findCrossDomainCorrelations(userId) {
        const insights = [];
     ...
     */
    getPendingInsights(userId) {
        return this.insightQueue.get(userId) || [];
    }
    /**
     * Mark insights as delivered
     */
    markInsightsDelivered(userId, insightIds) {
    ...
    async runScheduledScans() {
        ...
        try {
            ...
            for (let i = 0; i < activeUsers.length; i += batchSize) {
                ...
                totalInsights += results.reduce((a, b) => a + b, 0);
...
// Singleton instance
let insightsServiceInstance = null;
export function getProactiveInsightsService() {
...
//# sourceMappingURL=proactive-insights-service.js.map