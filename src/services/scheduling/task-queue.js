...
export class TaskQueueService extends EventEmitter {
     ...
     */
    getTaskFromData(allData, taskId) {
     ...
     * Update task status
     */
    updateTaskStatus(task, status, result, error) {
        task.status = status;
        if (result !== undefined)
            task.result = result;
        if (error)
     ...
     */
    async triggerPendingAction(allData, actionId, triggerData) {
        let foundAction;
        let userId;
    ...
    startProcessor() {
        const processQueue = async () => {
            ...
            const now = new Date();
            const readyTasks = this.taskQueue.filter((t) => t.status === 'pending' && (!t.scheduledFor || t.scheduledFor <= now));
...
registerTaskHandler('notify_user', async (task) => {
    const { message, method } = task.parameters;
    try {
