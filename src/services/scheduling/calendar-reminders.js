...
const reminderSentLog = new Map(); // userId -> Set of reminderIds
 ...
 */
export async function flushCalendarPersistence() {
 ...
 */
export async function shutdownCalendarReminders() {
...
export async function upsertEvent(event) {
    ...
    getLogger().info({ userId: event.userId, eventId: event.id, title: event.title, startTime: event.startTime }, '📅 Calendar event saved');
    return event;
}
/**
 * Get default reminders based on event type
 */
function getDefaultReminders(event) {
    const reminders = [];
 ...
 */
export function getUpcomingEvents(userId, withinHours = 24) {
    const events = eventStore.get(userId) || [];
    const now = new Date();
 ...
 */
export function generateMorningDigest(userId) {
 ...
 */
function generatePreEventMessage(event, minutesBefore) {
    const { title } = event;
    let timeStr;
    if (minutesBefore >= 60) {
 ...
 */
function generateContextualMessage(event) {
    const title = event.title.toLowerCase();
            ...
            for (const reminder of event.reminders) {
                ...
                if (now >= reminderTime && now < event.startTime) {
                    ...
                    if (result.success) {
                        ...
                        remindersSent++;
                        getLogger().info({ userId, eventId: event.id, reminderType: reminder.type }, '⏰ Event reminder sent');
                    }
                }
            }
        }
 ...
 */
export async function syncFromGoogleCalendar(userId, accessToken) {
    try {
        const { getValidAccessToken, getEvents, isCalendarConfigured } = await import('../identity/google-calendar-oauth.js');
        ...
        getLogger().info({ userId, syncedCount, totalEvents: googleEvents.length }, '📅 Google Calendar synced');
        return syncedCount;
    }
    catch (error) {
 ...
 */
export function startCalendarReminders(intervalMs = 5 * 60 * 1000) {
    ...
    // Run immediately
    checkAndSendReminders().catch((err) => getLogger().error({ error: err }, 'Initial reminder check failed'));
    // Then run on interval
    registerInterval('calendar-reminders', () => {
        void (async () => {
            try {
    ...
    reminderCheckInterval = 1; // Marker
}
/**
 ...
 */
export function clearUserCalendarData(userId) {
 ...
 */
export function pruneOldCalendarData(olderThanDays = 30) {
...
//# sourceMappingURL=calendar-reminders.js.map