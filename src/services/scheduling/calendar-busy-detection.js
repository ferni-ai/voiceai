...
import { getEvents, getFreeBusy, getUserTokens, isCalendarConfigured, refreshAccessToken, } from '../identity/google-calendar-oauth.js';
import { addBusyPeriod, addNeverDuringRule, } from '../outreach/timing-intelligence.js';
const log = createLogger({ module: 'CalendarBusyDetection' });
// ============================================================================
// CACHE
// ============================================================================
const busyProfileCache = new Map();
 ...
 */
export async function getNextOutreachWindow(userId, minDurationMinutes = 15) {
    try {
        const profile = await getCalendarBusyProfile(userId);
 ...
 */
export async function getCalendarBusyProfile(userId) {
    // Check cache first
 ...
 */
export async function syncCalendarToOutreach(userId) {
...
function findNextFreeWindow(from, busySlots, minDurationMinutes) {
...
function detectRecurringPatterns(events) {
    const rules = [];
    const eventTimes = new Map(); // time -> day occurrences
 ...
 */
export function clearBusyProfileCache(userId) {
    busyProfileCache.delete(userId);
}
/**
 * Clear all cached profiles
 */
export function clearAllBusyProfileCaches() {
...
//# sourceMappingURL=calendar-busy-detection.js.map