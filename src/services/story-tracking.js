...
const storiesToldMap = new Map(); // userId:personaId -> Set of storyIds
// Story registry (loaded from persona bundles)
const storyRegistry = new Map();
...
async function flushToPersistence() {
    ...
    try {
        ...
        for (const userId of usersToFlush) {
            ...
            logger.debug({ userId, personaCount: Object.keys(perPersonaStories).length }, 'Persisted story history');
        }
    }
    catch (error) {
...
async function loadFromProfile(userId) {
    try {
        ...
        if (profile?.customData?.perPersonaStoriesTold) {
            const storedStories = profile.customData.perPersonaStoriesTold;
            ...
            logger.debug({ userId, personaCount: Object.keys(storedStories).length }, 'Loaded story history from profile');
        }
    }
    catch (error) {
 ...
 */
function getTrackingKey(userId, personaId) {
    return `${userId}:${personaId}`;
}
/**
 * Check if a story has been told to a user by a specific persona
 */
export async function hasStoryBeenTold(userId, personaId, storyId) {
    const key = getTrackingKey(userId, personaId);
 ...
 */
export async function markStoryTold(userId, personaId, storyId) {
    const key = getTrackingKey(userId, personaId);
    let told = storiesToldMap.get(key);
    if (!told) {
        await loadFromProfile(userId);
 ...
 */
export async function getStoriesTold(userId, personaId) {
    const key = getTrackingKey(userId, personaId);
    let told = storiesToldMap.get(key);
    if (!told) {
        await loadFromProfile(userId);
 ...
 */
export function canTellStory(story, relationshipStage) {
    const stageOrder = [
        'stranger',
        'acquaintance',
        'friend',
...
const storyGraphCache = new Map();
/**
 * Register a story graph for a persona (loaded from bundles)
 */
export function registerStoryGraph(personaId, graph) {
 ...
 */
function detectContextTriggers(userMood, currentTopic) {
    const triggers = [];
 ...
 */
export async function findStoryForContext(context, availableStories) {
    ...
    for (const story of availableStories) {
        ...
        if (story.personaId !== personaId)
            continue;
        // Must not have been told already
        if (await hasStoryBeenTold(userId, personaId, story.id))
            continue;
        // Must meet relationship gate
        if (!canTellStory(story, relationshipStage))
            continue;
    ...
    const graphRecommendations = new Map();
 ...
 */
export async function getContinuationStories(storyId, context, allStories) {
    ...
    for (const story of allStories) {
        ...
        if (!justTold.leadsTo?.includes(story.id))
            continue;
        // Must not have been told
        if (await hasStoryBeenTold(context.userId, context.personaId, story.id))
            continue;
        // Must meet relationship gate
        if (!canTellStory(story, context.relationshipStage))
            continue;
        results.push(story);
    }
    return results;
 ...
 */
export function registerStory(story) {
 ...
 */
export async function getStoryStats(userId, personaId) {
 ...
 */
export async function flushStoryPersistence() {
    if (persistenceTimer) {
        clearTimeout(persistenceTimer);
...
//# sourceMappingURL=story-tracking.js.map