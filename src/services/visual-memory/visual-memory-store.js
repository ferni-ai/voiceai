 ...
 */
export async function uploadVisualMemory(request) {
    ...
    try {
        ...
        log.info({ userId, memoryId, category, labelsCount: detectedLabels.length }, 'Visual memory stored');
 ...
 */
export async function getVisualMemory(userId, memoryId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
 ...
 */
export async function getRecentVisualMemories(userId, limit = 10) {
    const db = getFirestoreDb();
    if (!db)
        return [];
    try {
        ...
        try {
            const snapshot = await db
                ...
                .limit(limit)
                .get();
            return snapshot.docs.map((doc) => doc.data()).filter((m) => !m.deletedAt);
        }
        catch (err) {
 ...
 */
export async function deleteVisualMemory(userId, memoryId, reason) {
    const db = getFirestoreDb();
    if (!db)
        return false;
    try {
 ...
 */
export async function searchVisualMemories(request) {
...
function getMatchReason(memory, query) {
    if (!query)
        return undefined;
    const queryLower = query.toLowerCase();
    if (memory.userDescription?.toLowerCase().includes(queryLower)) {
 ...
 */
export async function getVisualPreferences(userId) {
    const db = getFirestoreDb();
    if (!db)
        return null;
    try {
 ...
 */
export async function updateVisualPreferences(userId, prefs) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export async function buildVisualContext(userId) {
    const prefs = await getVisualPreferences(userId);
    if (prefs && !prefs.enabled) {
        return {
 ...
 */
export async function getVisualContextInjection(userId) {
    ...
    if (context.recentVisuals) {
        ...
        for (const visual of context.recentVisuals.slice(0, 3)) {
            const daysAgo = Math.floor((Date.now() - new Date(visual.sharedAt).getTime()) / (1000 * 60 * 60 * 24));
...
//# sourceMappingURL=visual-memory-store.js.map