...
options = {}) {
    ...
    const { detectLabels = true, detectText = true, detectFaces = true, detectLandmarks = true, detectLogos = false, webDetection = false, safeSearch = true, imageProperties = false, } = options;
    try {
        log.debug('Analyzing image with Vision API');
        // Build feature requests
        const features = [];
        if (detectLabels)
            features.push({ type: 'LABEL_DETECTION', maxResults: 15 });
        if (detectText)
            features.push({ type: 'TEXT_DETECTION' });
        if (detectFaces)
            features.push({ type: 'FACE_DETECTION', maxResults: 10 });
        if (detectLandmarks)
            features.push({ type: 'LANDMARK_DETECTION', maxResults: 5 });
        if (detectLogos)
            features.push({ type: 'LOGO_DETECTION', maxResults: 5 });
        if (webDetection)
 ...
 */
export function generateImageDescription(analysis) {
    const parts = [];
 ...
 */
export function categorizeImage(analysis) {
...
//# sourceMappingURL=vision-analysis.js.map