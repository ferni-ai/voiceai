...
function serializeStreak(streak) {
    return {
        ...streak,
        lastCheckIn: streak.lastCheckIn.toISOString(),
    };
}
function deserializeStreak(data) {
    return {
        ...data,
...
// Persistence stores
let goalPersistence = null;
let streakPersistence = null;
function getGoalPersistence() {
    if (!goalPersistence) {
        goalPersistence = createPersistenceStore({
...
function getStreakPersistence() {
    if (!streakPersistence) {
        streakPersistence = createPersistenceStore({
            collection: 'goals_outreach',
            documentId: 'streaks',
            syncIntervalMs: 3000,
        });
 ...
 */
function persistStreaks(userId) {
 ...
 */
export async function recordCheckIn(checkIn) {
    ...
    if (completed) {
        ...
        const hoursSinceLastCheckIn = (timestamp.getTime() - streak.lastCheckIn.getTime()) / (1000 * 60 * 60);
    ...
    getLogger().info({ userId, habitName, streak: streak.currentStreak, completed }, '📊 Habit check-in recorded');
 ...
 */
function getOptimalSendTime(baseTime, prefs) {
    const time = new Date(baseTime);
    const hour = time.getHours();
    // Check if current time is within preferred time windows
    const isPreferredTime = (prefs.preferredTimes.morning && hour >= 7 && hour < 11) ||
 ...
 */
export async function checkAtRiskStreaks() {
    let alertsSent = 0;
    const now = new Date();
        ...
        for (const streak of streaks) {
            ...
            if (hoursSinceCheckIn >= riskThreshold && !streak.atRisk) {
                ...
                if (result.success) {
                    ...
                    alertsSent++;
                    getLogger().info({ userId, habitName: streak.habitName, streak: streak.currentStreak }, '⚠️ Streak at-risk alert sent');
                }
            }
        }
        streakStore.set(userId, streaks);
 ...
 */
function getStreakRiskMessage(streak) {
    const messages = [
...
export async function createGoal(params) {
    ...
    getLogger().info({ userId: params.userId, goalId: goal.id, title: goal.title }, '🎯 Goal created');
    return goal;
}
/**
 * Update goal progress and check for milestone celebrations
 */
export async function updateGoalProgress(userId, goalId, newProgress) {
    // Ensure user data is loaded from persistence
    await ensureUserLoaded(userId);
    const goals = goalStore.get(userId) || [];
    ...
    if (celebration && (await canReachUser(userId)) && canSendOutreach(userId)) {
        ...
        if (goal.progress >= 100) {
            ...
            const emailResult = await scheduleEmail(userId, `🎊 You Did It! "${goal.title}" Complete!`, `Congratulations on completing your goal "${goal.title}"!\n\n` +
                `This achievement represents real dedication and effort. ` +
                `Take a moment to celebrate what you've accomplished.\n\n` +
                `- Maya\n\nP.S. What's your next goal? I'd love to help you crush that one too! 🚀`, new Date(), 'Maya');
        ...
        for (const goal of goals) {
            ...
            for (const alertDay of alertDays) {
                if (daysUntilDeadline <= alertDay && daysUntilDeadline > alertDay - 1) {
                    ...
                    if (result.success) {
                        alertsSent++;
                        getLogger().info({ userId, goalId: goal.id, daysLeft: alertDay }, '⏰ Goal deadline alert sent');
                    }
                    break;
                }
            }
 ...
 */
function getDeadlineMessage(goal, daysLeft) {
    if (daysLeft === 1) {
 ...
 */
export async function sendMissedCheckInNudges(maxDaysSinceContact = 3) {
    let nudgesSent = 0;
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            const daysSinceContact = Math.floor((now.getTime() - lastEngagement.getTime()) / (1000 * 60 * 60 * 24));
            ...
            try {
                ...
                nudgesSent++;
                getLogger().info({ userId, daysSinceContact, goalCount: activeGoals.length }, '📬 Sent missed check-in nudge');
            }
            catch (error) {
 ...
 */
function generateMissedCheckInMessage(daysSinceContact, goal) {
    const messages = [
 ...
 */
export function startGoalMonitoring(intervalMs = 60 * 60 * 1000) {
    ...
    registerInterval(GOAL_MONITORING_INTERVAL, () => {
        void (async () => {
            try {
 ...
 */
export function stopGoalMonitoring() {
 ...
 */
export async function flushGoalOutreachPersistence() {
 ...
 */
export async function shutdownGoalOutreach() {
    stopGoalMonitoring();
...
//# sourceMappingURL=goal-outreach-integration.js.map