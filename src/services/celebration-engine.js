...
export class CelebrationEngine {
    ...
    personaId;
    celebrationHistory = [];
    lastCelebrationTurn = 0;
    celebrationCooldown = 3; // Minimum turns between celebrations
    constructor(userId, personaId) {
        this.userId = userId;
     ...
     */
    detectCelebration(userMessage, turnCount, context) {
        // Cooldown check
        ...
        if (GOAL_COMPLETION_PATTERNS.some((p) => p.test(userMessage))) {
            const goalMatch = context?.activeGoals?.find((g) => g.progress >= 90 || lowered.includes(g.title.toLowerCase()));
     ...
     */
    getCelebrationTemplates(type, intensity) {
        const templates = {
            goal_completed: {
     ...
     */
    buildCelebrationSSML(message, intensity) {
        const prosodyMap = {
     ...
     */
    getExpression(type, intensity) {
        if (intensity === 'ecstatic')
            return 'celebrating';
        if (intensity === 'enthusiastic')
            return 'excited';
        if (type === 'growth_recognized' || type === 'effort_recognized')
            return 'pride';
    ...
    celebrateRelationshipMilestone(milestone) {
        ...
        let intensity = 'warm';
        if (milestone.type === 'months' && milestone.value >= 12)
            intensity = 'enthusiastic';
        if (milestone.type === 'conversations' && milestone.value >= 100)
            intensity = 'enthusiastic';
     ...
     */
    recordReaction(triggerId, reaction) {
    ...
    getStats() {
        ...
        for (const record of this.celebrationHistory) {
            ...
            if (record.userReaction === 'positive')
                positiveReactions++;
        }
        return {
...
export function getCelebrationEngine(userId, personaId) {
...
export function resetCelebrationEngine(userId, personaId) {
    const key = `${userId}:${personaId}`;
...
//# sourceMappingURL=celebration-engine.js.map