...
const actionTypeRegistry: Map<ActionType, ActionTypeConfig> = new Map();

/**
 * Register an action type with its executor
 */
export function registerActionType(config: ActionTypeConfig): void {
 ...
 */
export function getActionTypeConfig(type: ActionType): ActionTypeConfig | undefined {
...
export class ActionEngine {
  ...
  private integrationHub = getIntegrationHub();

  // Default expiry times by priority (seconds)
  private readonly defaultExpiry: Record<ActionPriority, number> = {
   ...
   */
  async prepareAction<T extends ActionPayload>(params: {
    userId: string;
    sessionId?: string;
    type: ActionType;
    payload: T;
    ...
    if (typeConfig.prepare) {
      try {
        ...
        if (prepareResult.enrichedPayload) {
          enrichedPayload = prepareResult.enrichedPayload as T;
        }
      } catch (error) {
    ...
    log.info(
      { actionId: action.id, userId, type, expiresAt: expiresAt.toISOString() },
      'Action prepared and awaiting confirmation'
    );
  ...
  async confirmAction(
    ...
    if (modifiedPayload) {
      ...
      action.payload = { ...action.payload, ...modifiedPayload } as typeof action.payload;
    }
   ...
   */
  cancelAction(actionId: string): { success: boolean; error?: string } {
    const action = this.store.get(actionId);
   ...
   */
  private async executeAction(actionId: string): Promise<{
    success: boolean;
    result?: ActionResult;
    error?: string;
  }> {
    ...
    try {
      ...
      if (result.success) {
        ...
        log.info(
          { actionId, type: action.type, externalId: result.externalId },
   ...
   */
  async rollbackAction(actionId: string): Promise<{
    success: boolean;
    error?: string;
  }> {
    const action = this.store.get(actionId);
   ...
   */
  getRecentActions(userId: string, limit = 10): Action[] {
...
let actionEngineInstance: ActionEngine | null = null;
 ...
 */
export function formatActionForVoice(action: Action): string {
 ...
 */
export function actionNeedsAttention(action: Action): boolean {
  if (action.status !== 'pending_confirmation') {
    return false;
  }
