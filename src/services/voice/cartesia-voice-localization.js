 ...
 */
import { ACCENT_TO_DIALECT } from '../../config/voice-accents.js';
 ...
 */
const localizedVoiceCache = new Map();
/**
 ...
 */
async function saveToFirestore(cacheKey, voice) {
    try {
 ...
 */
export async function loadCacheFromFirestore() {
 ...
 */
async function deleteFromFirestore(cacheKey) {
    try {
        const db = await getFirestore();
        if (!db)
            return;
 ...
 */
async function callLocalizeApi(request) {
 ...
 */
export async function getLocalizedVoiceId(personaId, accent) {
 ...
 */
export function getLocalizedVoiceIdSync(personaId, accent) {
    if (accent === 'american') {
    ...
    log.warn({ personaId, accent }, '⚠️ Localized voice not cached, using original. Call preWarmLocalizedVoices() at startup.');
    return getVoiceIdForPersona(personaId);
}
/**
 ...
 */
export async function preWarmLocalizedVoices(personaIds = Object.keys(PERSONA_VOICE_META), accents = ['british', 'australian', 'indian']) {
    ...
    log.info({ total: results.length, success: successCount, failed: results.length - successCount }, '✅ Voice cache pre-warming complete');
}
/**
 * Check if a localized voice is cached.
 */
export function isVoiceCached(personaId, accent) {
    if (accent === 'american')
        return true;
    const cacheKey = `${personaId}:${accent}`;
 ...
 */
export async function clearLocalizationCache(clearFirestore = false) {
 ...
 */
export async function initializeLocalizationService(options = {}) {
...
//# sourceMappingURL=cartesia-voice-localization.js.map