...
import { getTwilioEnhancer, float32ToInt16Buffer, } from './twilio-audio-enhance.js';
 ...
 */
function mulawToLinear(mulawData) {
    const MULAW_BIAS = 33;
 ...
 */
function linearToMulaw(linearData) {
    const MULAW_BIAS = 33;
...
export class TwilioStreamBridge extends EventEmitter {
    ...
    constructor(config) {
        super();
        this.config = config;
        this.roomService = new RoomServiceClient(config.livekitUrl, config.livekitApiKey, config.livekitApiSecret);
    }
    /**
     ...
     */
    attachToServer(httpServer, path = '/stream') {
     ...
     */
    async handleStreamStart(ws, start) {
        ...
        if (isExperimentalEnabled('preSTTAudioProcessing')) {
            try {
                ...
                log.info({ callSid, usingRust: session.audioEnhancer.isUsingRust() }, '🎤 Twilio audio enhancement enabled');
            }
            catch (err) {
                log.warn({ error: String(err), callSid }, '⚠️ Failed to initialize audio enhancer, using simple upsampling');
            }
        }
     ...
     */
    async handleMedia(session, media) {
        ...
        if (session.audioEnhancer) {
            ...
            const int16Array = new Int16Array(linearBuffer.buffer, linearBuffer.byteOffset, linearBuffer.length / 2);
     ...
     */
    async transcribeAudio(pcmAudio) {
        if (!GOOGLE_API_KEY) {
     ...
     */
    setAgentSpeaking(callSid, isSpeaking) {
     ...
     */
    sendAudioToCaller(callSid, linearPcm16k) {
        const session = this.sessions.get(callSid);
...
export function getTwilioStreamBridge(config) {
...
//# sourceMappingURL=twilio-stream-bridge.js.map