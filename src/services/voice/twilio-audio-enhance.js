...
import { PreSTTProcessor, PreSTTPresets, } from '../../agents/shared/performance/pre-stt-transform.js';
 ...
 * ```
 */
export async function getTwilioEnhancer(config) {
    const { sessionId, enableAgc = true, enableNoiseSuppression = true, enableBandwidthExtension = true, enableHighpass = true, } = config;
    ...
    return {
        enhanceFrame: (samples8kHz, isSpeech = true) => {
 ...
 */
export function removeTwilioEnhancer(sessionId) {
 ...
 */
export function getActiveEnhancerCount() {
    return enhancers.size;
}
/**
 * Clear all enhancers (emergency cleanup).
 */
export function clearAllEnhancers() {
 ...
 */
export async function enhanceTwilioAudio(samples8kHz) {
    const processor = new PreSTTProcessor({
        ...PreSTTPresets.twilio,
        sessionId: 'one-shot',
    });
 ...
 */
export function float32ToInt16Buffer(samples) {
...
//# sourceMappingURL=twilio-audio-enhance.js.map