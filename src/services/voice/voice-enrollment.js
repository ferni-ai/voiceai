 ...
 */
export function startEnrollmentSession(userId, options) {
 ...
 */
export async function addEnrollmentSample(session, audio, context) {
    ...
    try {
        ...
        return {
            success: true,
            session,
            feedback: session.samples.length >= session.requiredSamples
 ...
 */
export async function completeEnrollment(session) {
    ...
    try {
        ...
        const threshold = computeAdaptiveThreshold(session.samples.map((s) => s.embedding), centroid);
        // Calculate quality score
        const qualityScore = calculateEnrollmentQuality(session.samples);
        // Collect device types
        const deviceTypes = Array.from(new Set(session.samples.map((s) => s.context?.deviceType).filter(Boolean)));
        const profile = {
            userId: session.userId,
 ...
 */
export async function verifyUser(audio, profile) {
 ...
 */
export async function identifySpeaker(audio, profiles, options) {
    const startTime = Date.now();
 ...
 */
export async function updateProfile(profile, newSamples) {
    // Add new samples
    ...
    // Recompute threshold
    const threshold = computeAdaptiveThreshold(keptSamples.map((s) => s.embedding), centroid);
    // Update quality
 ...
 */
export function needsReEnrollment(profile) {
    const reasons = [];
    // Check quality score
 ...
 */
function computeAdaptiveThreshold(embeddings, centroid) {
 ...
 */
async function checkSampleConsistency(existingSamples, newSample) {
 ...
 */
function calculateEnrollmentQuality(samples) {
    if (samples.length === 0) {
        return 0;
    }
    // Factors:
...
//# sourceMappingURL=voice-enrollment.js.map