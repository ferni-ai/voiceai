...
import { enhanceOutboundMessage } from '../outreach/outbound-ssml.js';
 ...
 */
export async function generatePersonaVoice(text, personaId = 'ferni', options) {
    if (!CARTESIA_API_KEY) {
    ...
    if (text !== cleanText) {
        getLogger().debug({ originalLength: text.length, cleanLength: cleanText.length, personaId }, '🎤 Stripped SSML tags for Cartesia');
    }
    ...
    try {
        const response = await cartesiaCircuitBreaker.execute(async () => fetch('https://api.cartesia.ai/tts/bytes', {
            method: 'POST',
            headers: {
 ...
 */
export async function generateAlexVoice(text) {
 ...
 */
export async function uploadAudioToGCS(audioBuffer, filename) {
    if (!GCS_BUCKET) {
 ...
 */
async function getHostedAudioUrl(message, audioBuffer, personaId = 'ferni') {
 ...
 */
export async function callWithPersonaVoice(toPhone, message, personaId = 'ferni', options) {
...
<Response>
  ...
  <Say voice="${twilioVoice}">${escapeXml(message)}</Say>
                    ...
                    return {
                        success: false,
                        message: 'Audio hosting (GCS) not configured for voice calls. Set GCS_VOICE_BUCKET env var.',
                    };
                }
            }
        }
 ...
 */
export async function callWithAlexVoice(toPhone, message, options) {
 ...
 */
export async function configureIncomingCallWebhook(webhookUrl) {
    if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_PHONE_NUMBER) {
        return { success: false, message: 'Twilio not configured' };
    }
    try {
        // First, get the phone number SID
        const listResponse = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/IncomingPhoneNumbers.json?PhoneNumber=${encodeURIComponent(TWILIO_PHONE_NUMBER)}`, {
            headers: {
                Authorization: `Basic ${Buffer.from(`${TWILIO_ACCOUNT_SID}:${TWILIO_AUTH_TOKEN}`).toString('base64')}`,
            },
        });
        ...
        const updateResponse = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/IncomingPhoneNumbers/${phoneNumberSid}.json`, {
            method: 'POST',
            headers: {
                Authorization: `Basic ${Buffer.from(`${TWILIO_ACCOUNT_SID}:${TWILIO_AUTH_TOKEN}`).toString('base64')}`,
                'Content-Type': 'application/x-www-form-urlencoded',
...
export { enhanceOutboundMessage, createWarmOpening, createWarmClosing, thoughtfulPause, warmWrap, } from '../outreach/outbound-ssml.js';
export default {
