...
import { compareVoiceSketches, } from '../memory/voice-memory.js';
// ============================================================================
 ...
 */
function getAdaptiveThresholds(userId, profile) {
    // Check cache
    ...
    if (profile?.customData?.voiceThresholds) {
        const stored = profile.customData.voiceThresholds;
 ...
 */
export async function recordVoiceMatchFeedback(userId, matchScore, wasCorrect, profile) {
    ...
    if (!wasCorrect) {
        if (matchScore >= thresholds.matchThreshold) {
            ...
            getLogger().info({ userId, newMatchThreshold: thresholds.matchThreshold }, 'Raised voice match threshold due to false positive');
        }
        else {
            ...
            getLogger().info({ userId, newMatchThreshold: thresholds.matchThreshold }, 'Lowered voice match threshold due to false negative');
        }
    }
    // Update cache
    ...
    if (profile) {
        try {
            ...
            getLogger().debug({ userId, samples: thresholds.samples, accuracy: thresholds.accuracy }, 'Persisted adaptive voice thresholds');
        }
        catch (error) {
 ...
 */
export async function identifyWithVoice(metadata, store, currentVoiceSketch) {
 ...
 */
async function handleKnownDevice(deviceResult, currentVoiceSketch, store) {
 ...
 */
async function handleNewDeviceWithVoice(deviceResult, currentVoiceSketch, store) {
    ...
    return {
        ...
        suggestedAction: 'ask_name',
        contextForAgent: "New user - no prior conversations and voice doesn't match any existing users.",
    };
}
/**
 * Search for voice matches among stored profiles
 */
async function searchByVoice(currentVoiceSketch, store, excludeUserId) {
 ...
 */
export function mergeVoiceSketch(existing, newSketch) {
    if (!existing) {
        return newSketch;
    }
    ...
    const weightedAvg = (existingVal, newVal) => {
    ...
    return {
        ...
        spectralCentroidMean: weightedAvg(existing.spectralCentroidMean, newSketch.spectralCentroidMean),
        spectralCentroidStdDev: weightedAvg(existing.spectralCentroidStdDev, newSketch.spectralCentroidStdDev),
 ...
 */
function buildContextForAgent(profile, scenario, voiceMatches) {
    ...
    switch (scenario) {
        ...
        case 'voice_mismatch': {
            ...
            if (possiblePerson) {
                return (`Device belongs to "${name}" but voice sounds like "${possiblePerson}". ` +
                    `Ask naturally: "I was expecting ${name} - is this ${possiblePerson}?"`);
            }
            return (`Device belongs to "${name}" but voice doesn't match. ` +
        ...
        case 'voice_match': {
            ...
            if (matchedName) {
                return (`Voice strongly matches "${matchedName}" (${Math.round(similarity * 100)}% confidence). ` +
        ...
        case 'voice_possible_match': {
            ...
            if (possibleName) {
                return (`Voice might be "${possibleName}" (${Math.round(possibleSimilarity * 100)}% confidence). ` +
...
//# sourceMappingURL=voice-identification.js.map