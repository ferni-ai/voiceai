...
import { identifySpeaker } from './voice-enrollment.js';
import { loadVoiceProfile } from './voice-profile-store.js';
 ...
 */
export async function createHousehold(deviceId, ownerId, name = 'My Home') {
    const household = {
 ...
 */
export async function getHousehold(deviceId) {
...
async function saveHousehold(household) {
    ...
    try {
        await db
            ...
            members: household.members.map((m) => removeUndefined({
                ...m,
 ...
 */
export async function addHouseholdMember(deviceId, userId, displayName, role = 'adult') {
    ...
    await saveHousehold(household);
    log.info({ userId, householdId: household.id, role, voiceEnrolled: hasVoiceProfile }, 'Member added to household');
    return member;
}
/**
 * Remove a member from a household.
 */
export async function removeHouseholdMember(deviceId, userId) {
 ...
 */
export async function getHouseholdMembers(deviceId) {
    const household = await getHousehold(deviceId);
 ...
 */
export async function updateMemberLastSeen(deviceId, userId) {
    const household = await getHousehold(deviceId);
    if (!household)
        return;
 ...
 */
export async function identifyHouseholdSpeaker(deviceId, audio) {
    const household = await getHousehold(deviceId);
 ...
 */
export function startHouseholdSession(deviceId, initialUserId = null) {
    const session = {
        deviceId,
 ...
 */
export function getActiveSession(deviceId) {
 ...
 */
export async function updateSessionSpeaker(deviceId, newUserId, confidence) {
 ...
 */
export function endHouseholdSession(deviceId) {
    const session = activeSessions.get(deviceId);
...
//# sourceMappingURL=voice-household.js.map