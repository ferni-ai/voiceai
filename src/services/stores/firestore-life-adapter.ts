...
import type { Firestore, DocumentReference, DocumentSnapshot } from '@google-cloud/firestore';
 ...
 */
export async function getLifeAutomationData<T>(
  userId: string,
  domain: LifeAutomationDomain
): Promise<T | null> {
 ...
 */
export async function saveLifeAutomationData<T extends object>(
  userId: string,
  domain: LifeAutomationDomain,
  data: T
 ...
 */
export async function deleteLifeAutomationData(
  userId: string,
  domain: LifeAutomationDomain
): Promise<FirestoreOperationResult<void>> {
  const db = getFirestoreDb();
 ...
 */
export async function batchSaveLifeAutomationData(
  userId: string,
  domains: Array<{ domain: LifeAutomationDomain; data: object }>
): Promise<FirestoreOperationResult<void>> {
  const db = getFirestoreDb();
  if (!db) {
 ...
 */
export async function getAllLifeAutomationData(
  userId: string
): Promise<Record<LifeAutomationDomain, unknown>> {
  const db = getFirestoreDb();
  const result: Record<LifeAutomationDomain, unknown> = {
    subscriptions: null,
    documents: null,
    meals: null,
    workflows: null,
  ...
  const domains: LifeAutomationDomain[] = ['subscriptions', 'documents', 'meals', 'workflows'];
 ...
 */
export async function migrateToFirestore<T extends object>(
  userId: string,
  domain: LifeAutomationDomain,
  inMemoryData: T
): Promise<FirestoreOperationResult<void>> {
 ...
 */
export async function hasLifeAutomationData(userId: string): Promise<boolean> {
  const db = getFirestoreDb();
  if (!db) {
    return false;
  }
