...
import { loadUserCommitments, assessBurnoutRisk, findDormantDreams, findUpcomingDates, } from './superhuman/index.js';
...
function getInsightsPersistence() {
    if (!persistenceStore) {
 ...
 */
async function loadInsightsFromPersistence(userId) {
 ...
 */
function persistInsights(userId) {
 ...
 */
async function persistInsightsImmediate(userId) {
    const insights = userInsightsCache.get(userId) || [];
    const store = getInsightsPersistence();
 ...
 */
export function addCrossPersonaInsight(userId, insight) {
    const insights = getInsightsForUser(userId);
    // Check for duplicate insights (same source, target, category within last hour)
    const oneHourAgo = Date.now() - 60 * 60 * 1000;
    const isDuplicate = insights.some((i) => i.source === insight.source &&
        i.target === insight.target &&
        i.category === insight.category &&
        i.createdAt > oneHourAgo);
    if (isDuplicate) {
        log.debug({ userId, source: insight.source, category: insight.category }, 'Skipping duplicate insight');
        // Return the existing insight
        const existing = insights.find((i) => i.source === insight.source &&
            i.target === insight.target &&
            i.category === insight.category &&
            i.createdAt > oneHourAgo);
        return existing;
 ...
 */
export function getInsightsForPersona(userId, personaId, options) {
    const insights = getInsightsForUser(userId);
    const now = Date.now();
    ...
    const filtered = insights.filter((insight) => insight.expiresAt > now &&
    ...
    return filtered.map((insight) => ({
        ...
        relevanceScore: insight.priority === 'critical'
            ? 1.0
 ...
 */
function getInsightsForPersonaRaw(userId, personaId) {
    const insights = getInsightsForUser(userId);
    const now = Date.now();
    const normalizedPersona = normalizePersonaId(personaId);
    return insights.filter((insight) => insight.expiresAt > now && (insight.target === normalizedPersona || insight.target === 'all'));
}
/**
 * Get proactive insights that should be surfaced
 */
export function getProactiveInsights(userId) {
    const insights = getInsightsForUser(userId);
    const now = Date.now();
 ...
 */
export function consumeInsight(userId, insightId) {
    const insights = getInsightsForUser(userId);
 ...
 */
export function clearExpiredInsights(userId) {
    const insights = getInsightsForUser(userId);
    const now = Date.now();
 ...
 */
export async function generateTeamStatus(userId) {
    const status = {
        habitHealth: {
            activeHabits: 0,
            totalStreakDays: 0,
    ...
    try {
        ...
        status.habitHealth.keystoneActive = activeHabits.some((h) => h.isKeystone && h.currentStreak >= 7);
        status.habitHealth.atRiskCount = activeHabits.filter((h) => h.longestStreak >= 7 && h.currentStreak <= 1).length;
        ...
        status.goalStatus.nearingCompletion = goals.filter((g) => g.currentAmount / g.targetAmount >= 0.8).length;
        ...
        if (budget) {
            status.financialHealth.budgetUsedPercent = Math.round((budget.spent / budget.monthlyLimit) * 100);
        }
        ...
        status.financialHealth.recentStressTriggers = triggers.filter((t) => stressEmotions.includes(t.emotion)).length;
        ...
        const goalsOnTrack = goals.filter((g) => {
            ...
            const daysTotal = Math.ceil((deadline.getTime() - new Date(g.createdAt || now).getTime()) / (1000 * 60 * 60 * 24));
            const daysElapsed = Math.ceil((now.getTime() - new Date(g.createdAt || now).getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export async function scanForCrossPersonaInsights(userId) {
    try {
        ...
        const overallHealthy = status.habitHealth.atRiskCount === 0 &&
 ...
 */
async function scanSuperhumanInsights(userId) {
    try {
        ...
        if (commitments && commitments.length > 0) {
            ...
            const overdueCommitments = activeCommitments.filter((c) => c.followUpAfter && c.followUpAfter < Date.now());
        ...
        // =========================================================================
        if (burnout) {
            const assessment = burnout;
            if (assessment.risk === 'high' || assessment.risk === 'critical') {
                const signalDescriptions = assessment.factors?.map((f) => f.factor).join(', ') || 'exhaustion patterns';
                addCrossPersonaInsight(userId, {
                    source: 'system',
                    target: 'all',
                    priority: 'critical',
 ...
 */
export async function buildInsightBriefingForHandoff(userId, targetPersonaId) {
    ...
    const proactiveDiscoveries = [];
    // Add high-priority proactive insights
    const proactiveInsights = getProactiveInsights(userId);
    for (const insight of proactiveInsights.filter((i) => i.priority === 'high' || i.priority === 'critical')) {
    ...
    if (teamStatus.goalStatus.nearingCompletion > 0) {
        proactiveDiscoveries.push(`${teamStatus.goalStatus.nearingCompletion} goal(s) close to completion - celebration opportunity`);
    }
 ...
 */
export function formatInsightBriefingForPrompt(briefing) {
    const lines = [];
    lines.push('[CROSS-TEAM BRIEFING]');
    // Team status summary
    lines.push('\n=== TEAM STATUS ===');
    const { teamStatus } = briefing;
    lines.push(`• Habits: ${teamStatus.habitHealth.activeHabits} active, ${teamStatus.habitHealth.totalStreakDays} streak days, ` +
        `keystone ${teamStatus.habitHealth.keystoneActive ? '✅' : '❌'}`);
    lines.push(`• Goals: ${teamStatus.goalStatus.activeGoals} active, ${teamStatus.goalStatus.nearingCompletion} near completion, ` +
        `$${teamStatus.goalStatus.totalSaved.toLocaleString()} saved`);
    lines.push(`• Budget: ${teamStatus.financialHealth.budgetUsedPercent}% used, ` +
...
function normalizePersonaId(personaId) {
    ...
    if (normalized.includes('peter'))
        return 'peter';
    if (normalized.includes('maya'))
        return 'maya';
    if (normalized.includes('jordan'))
        return 'jordan';
    if (normalized.includes('nayan'))
        return 'nayan';
    if (normalized.includes('ferni'))
        return 'ferni';
 ...
 */
export function recordInsight(userId, source, contentOrOptions) {
 ...
 */
export async function loadInsights(userId) {
    await loadInsightsFromPersistence(userId);
    clearExpiredInsights(userId);
    await scanForCrossPersonaInsights(userId);
}
 ...
 */
export function getInsightsToSurface(userId, personaId, _limit) {
    let insights;
    if (personaId) {
 ...
 */
export async function acknowledgeInsight(userId, insightId, _personaId) {
 ...
 */
export function buildInsightContext(userId, personaId, _options) {
    ...
    const lines = ['[CROSS-TEAM INSIGHTS]'];
...
//# sourceMappingURL=cross-persona-insights.js.map