 ...
 */
export async function canUseCapability(userId, sessionId, category, context) {
 ...
 */
export async function shouldHoldBack(userId, sessionId, category) {
    ...
    if (uniqueCategories.size >= 2 && !uniqueCategories.has(category)) {
        log.debug({ userId, sessionId, category, usedCategories: Array.from(uniqueCategories) }, '⏸️ Holding back - already showed multiple capabilities');
        return true;
    }
    // If this is a "heavy" capability and we've used any other heavy one, hold back
    const heavyCategories = ['pattern', 'growth', 'challenge', 'synthesis'];
    if (heavyCategories.includes(category)) {
        const usedHeavy = usedCapabilities.filter((c) => heavyCategories.includes(c));
        if (usedHeavy.length > 0) {
            log.debug({ userId, sessionId, category, usedHeavy }, '⏸️ Holding back - already used a heavy capability');
            return true;
        }
    }
    return false;
 ...
 */
export async function useCapability(userId, sessionId, category) {
 ...
 */
export async function isFirstRevelation(userId, type) {
    return !(await hasRevelation(userId, type));
}
/**
 * Get which revelation types are still available
 */
export async function getAvailableRevelations(userId, context) {
 ...
 */
function getThrottleRule(category) {
 ...
 */
export async function getThrottleState(userId, sessionId, context) {
    const { getSessionCapabilities } = await import('./storage.js');
    const allCategories = [
        'memory',
        'pattern',
