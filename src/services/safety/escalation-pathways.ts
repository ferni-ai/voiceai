...
export type EscalationLevel =
  ...
  | 'gentle_suggestion' // Mention therapy might help
  | 'warm_recommendation' // Actively recommend professional support
  | 'urgent_referral' // Crisis-level need for professional help
  | 'emergency'; // Life-threatening, need emergency services
...
export function determineEscalation(context: EscalationContext): EscalationDecision {
  const { sessionSignals, historicalSignals, isInTherapy, previouslyDeclined, userPreferences } =
    context;
  ...
  if (currentSessionCritical) {
    ...
    return {
      ...
      ferniStance:
        "I'm here with you, and I want to make sure you have everything you need right now.",
  ...
  if (currentSessionHigh) {
    ...
    return {
      ...
      ferniStance:
        "I care about you too much to not say this - what you're going through deserves professional support.",
  ...
  if (mediumSignalCount >= 3 || uniqueCrisisTypes >= 2) {
    ...
    return {
      ...
      ferniStance:
        "I've noticed some patterns in what you're sharing, and I think you deserve more support than just me.",
      nextStep: isInTherapy
...
function getEmergencyFraming(crisisType: CrisisType): string {
  const framings: Partial<Record<CrisisType, string>> = {
    suicidal_ideation:
      "What you're feeling is a crisis, and crises need crisis support. That's not weakness - that's wisdom.",
    domestic_abuse:
      'Your safety is what matters most right now. There are people who specialize in exactly this.',
    substance_crisis:
      "What you're describing is dangerous, and you need people who can help right now.",
    sexual_assault:
      'What happened to you matters. There are people trained to help with exactly this.',
  };
...
function getUrgentFraming(crisisType: CrisisType, isInTherapy?: boolean): string {
  if (isInTherapy) {
  ...
  const framings: Partial<Record<CrisisType, string>> = {
    suicidal_ideation:
      "Thoughts like this deserve professional support. Not because you're broken - because you matter.",
    self_harm:
      'The urge to hurt yourself is telling you something. A therapist can help you understand what.',
    substance_crisis:
      'Addiction is so hard to fight alone. There are people who specialize in this exact battle.',
    eating_disorder_crisis:
      "What you're describing with food deserves specialized support. This is treatable.",
    severe_distress:
      'When things feel this overwhelming, it helps to have someone trained to help you through.',
  };
...
function getWarmRecommendationFraming(
  isInTherapy?: boolean,
  preferences?: EscalationContext['userPreferences']
): string {
  if (isInTherapy) {
...
function getGentleSuggestionFraming(isInTherapy?: boolean): string {
  if (isInTherapy) {
...
function getPreviouslyDeclinedFraming(preferences?: EscalationContext['userPreferences']): string {
  if (preferences?.hasBadExperiences) {
...
function getProfessionalForCrisis(crisisType: CrisisType): ProfessionalType {
  const mapping: Record<CrisisType, ProfessionalType> = {
 ...
 */
export function getEscalationFollowUp(
  ...
  const declinedFollowUps: Record<EscalationLevel, string[]> = {
    emergency: [
 ...
 */
export function getTherapyFinderIntro(): string {
  const intros = [
 ...
 */
export function getTherapyFinderTips(): string[] {
  return [
 ...
 */
export function buildEscalationContext(decision: EscalationDecision): string | null {
  if (decision.level === 'none') return null;
