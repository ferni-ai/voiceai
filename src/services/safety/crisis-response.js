 ...
 */
export function generateCrisisResponse(context) {
 ...
 */
function formatResourceForVoice(resource) {
    const parts = [];
    if (resource.phone) {
 ...
 */
function generateCrisisSSML(text, severity) {
    let ssml = text;
    ...
    if (severity === 'critical') {
        ssml = ssml.replace(/(Will you call|Can we keep talking)/g, '<emphasis level="moderate">$1</emphasis>');
    }
    return ssml;
}
/**
 ...
 */
export function getCrisisResources(crisisType) {
 ...
 */
export function getSafetyCheckQuestion(crisisType) {
...
//# sourceMappingURL=crisis-response.js.map