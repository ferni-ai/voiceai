...
import { generateBiometricInsight as generateInsightFromSnapshot, generateSuperhumanMoment as generateSuperhumanMomentFromSnapshot, } from './insights.js';
 ...
 */
async function ensureUserBiometrics(userId) {
 ...
 */
export function getAuthorizationUrl(platform, userId, scopes) {
    ...
    switch (platform) {
        ...
        case 'googlefit':
            const googleScopes = scopes?.join(' ') ||
                [
        ...
        case 'oura':
            ...
            return (`https://cloud.ouraring.com/oauth/authorize?` +
                ...
                `state=${state}`);
        case 'whoop':
            const whoopScopes = scopes?.join(' ') || 'read:recovery read:sleep read:workout read:body_measurement';
 ...
 */
export async function generateTerraSession(userId, options) {
    if (!config.terra.devId || !config.terra.apiKey) {
        return {
            success: false,
    ...
    try {
        ...
        const response = await terraCircuitBreaker.execute(() => fetch('https://api.tryterra.co/v2/auth/generateWidgetSession', {
            method: 'POST',
            headers: {
 ...
 */
export async function handleTerraWebhook(webhookBody, signature) {
 ...
 */
async function processTerraDataWebhook(userId, dataType, data) {
    ...
    for (const entry of data) {
        const terraData = entry;
        if (dataType === 'sleep' && terraData.sleep_durations_data) {
            const sleepData = terraData.sleep_durations_data;
            const asleepData = sleepData.asleep;
            const durationHours = (asleepData?.duration_asleep_state_seconds || 0) / 3600;
            const qualityScore = sleepData.sleep_efficiency || 70;
        ...
        if (dataType === 'activity' && terraData.active_durations_data) {
            const activityData = terraData.active_durations_data;
            const existingActivity = snapshot.activity;
            snapshot.activity = {
                steps: activityData.steps || existingActivity?.steps || 0,
                activeMinutes: (activityData.activity_seconds || 0) / 60 || existingActivity?.activeMinutes || 0,
        ...
        if (dataType === 'body' && terraData.heart_rate_data) {
            const hrvData = terraData.heart_rate_data;
 ...
 */
export async function exchangeCodeForTokens(platform, code, userId) {
    try {
        let tokenUrl;
        let body;
        ...
        if (platform === 'fitbit') {
            ...
            const credentials = Buffer.from(`${platformConfig.clientId}:${platformConfig.clientSecret}`).toString('base64');
 ...
 */
export async function syncBiometrics(userId) {
...
async function refreshToken(userId) {
    ...
    try {
        let tokenUrl;
        let body;
        const platformConfig = config[user.platform];
        if (!platformConfig || !('clientId' in platformConfig)) {
...
async function fetchGoogleFitData(userId, accessToken) {
    ...
    // Fetch HRV data
    const hrvResponse = await fetch(`https://www.googleapis.com/fitness/v1/users/me/dataset:aggregate`, {
        method: 'POST',
        headers: {
    ...
    return {
        ...
        recovery: null,
        stressLevel,
        raw: hrvResponse.ok ? (await hrvResponse.json()) : undefined,
    };
}
async function fetchOuraData(userId, accessToken) {
    const today = new Date().toISOString().split('T')[0];
    // Fetch sleep data
    const sleepResponse = await fetch(`https://api.ouraring.com/v2/usercollection/daily_sleep?start_date=${today}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
    });
    // Fetch readiness (recovery) data
    const readinessResponse = await fetch(`https://api.ouraring.com/v2/usercollection/daily_readiness?start_date=${today}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
    });
    let sleepData = null;
    ...
    if (readinessResponse.ok) {
        ...
        if (readiness.data?.[0]) {
            ...
            recoveryData = {
                score: r.score,
                readiness: r.score >= 85 ? 'peak' : r.score >= 70 ? 'high' : r.score >= 50 ? 'moderate' : 'low',
                factors: {
...
async function fetchWhoopData(userId, accessToken) {
    // Fetch recovery data
    const recoveryResponse = await whoopCircuitBreaker.execute(() => fetch('https://api.prod.whoop.com/developer/v1/recovery?limit=1', {
        headers: { Authorization: `Bearer ${accessToken}` },
    }));
    let recoveryData = null;
    let hrvData = null;
 ...
 */
async function fetchTerraData(userId, terraUserId) {
    ...
    try {
        ...
        const response = await fetch(`https://api.tryterra.co/v2/daily?user_id=${terraUserId}&start_date=${today}&end_date=${today}`, {
            headers: {
                'dev-id': config.terra.devId,
                'x-api-key': config.terra.apiKey,
            },
        ...
        if (data.data?.[0]) {
            ...
            if (d.sleep_data) {
                sleepData = {
                    ...
                    deepSleepPercent: ((d.sleep_data.deep_sleep_duration_hours || 0) /
                        (d.sleep_data.sleep_duration_in_hours || 1)) *
                        100,
                    remSleepPercent: ((d.sleep_data.rem_sleep_duration_hours || 0) /
                        (d.sleep_data.sleep_duration_in_hours || 1)) *
                        100,
                    disturbances: 0,
                    bedtime: new Date(),
...
function createMockSnapshot(userId, platform) {
    return {
        userId,
        platform,
        timestamp: new Date(),
...
// STRESS CALCULATION
// ============================================================================
function calculateStressLevel(hrv) {
    if (!hrv)
        return 'moderate';
    // HRV deviation from baseline indicates stress
    if (hrv.deviationPercent <= -30)
        return 'elevated';
    if (hrv.deviationPercent <= -20)
        return 'high';
    if (hrv.deviationPercent <= -10)
        return 'moderate';
    return 'low';
}
...
function calculateStressFromOura(sleep, recovery) {
    if (!sleep && !recovery)
        return 'moderate';
    let stressScore = 50; // Neutral
    if (sleep) {
        if (sleep.qualityScore < 50)
            stressScore += 20;
        else if (sleep.qualityScore < 70)
            stressScore += 10;
        if (sleep.duration < 6)
            stressScore += 15;
    }
...
// EVENT DETECTION
// ============================================================================
function checkForEvents(userId, snapshot) {
    const user = userBiometrics.get(userId);
    if (!user || user.eventCallbacks.size === 0)
        return;
    const events = [];
    // Check HRV spike
 ...
 */
export function hasBiometricsConnected(userId) {
    return userBiometrics.has(userId);
}
/**
 * Check if user has biometrics connected (async - checks persistence)
 */
export async function hasBiometricsConnectedAsync(userId) {
    const user = await ensureUserBiometrics(userId);
    return user !== null;
}
/**
 ...
 */
export function getConnectedPlatform(userId) {
    return userBiometrics.get(userId)?.platform ?? null;
}
/**
 * Get connected platform (async - checks persistence)
 */
export async function getConnectedPlatformAsync(userId) {
    const user = await ensureUserBiometrics(userId);
 ...
 */
export function disconnectBiometrics(userId) {
    userBiometrics.delete(userId);
 ...
 */
export function generateBiometricInsight(userId) {
 ...
 */
export function generateSuperhumanMoment(userId) {
    const snapshot = getCurrentBiometrics(userId);
