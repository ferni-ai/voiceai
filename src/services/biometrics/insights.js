 ...
 */
export function generateBiometricInsight(snapshot) {
    if (!snapshot)
        return null;
    ...
    if (snapshot.stressLevel === 'elevated' || snapshot.stressLevel === 'high') {
        ...
        return {
            ...
            suggestion: 'Consider offering a grounding exercise or acknowledging they might be having a rough day.',
            confidence: 0.8,
        };
    }
 ...
 */
export function generateSuperhumanMoment(snapshot) {
    if (!snapshot)
        return null;
    const moments = [];
    // HRV correlation with stress
    if (snapshot.hrv && snapshot.hrv.deviationPercent <= -20) {
        moments.push(`Your HRV dropped ${Math.abs(snapshot.hrv.deviationPercent)}% - rough day? Let's take it easy.`);
    }
    ...
    if (snapshot.activity &&
        ...
        moments.push(`You've been sitting for ${snapshot.activity.hoursSinceActivity} hours during a stressful time - want a 2-minute stretch?`);
    }
    // Low recovery
