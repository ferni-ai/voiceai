...
import { teamCelebrationOutreach, fullTeamSupportOutreach, mayaJordanPlanningOutreach, peterFerniInsightOutreach, initiateTeamRoundtableCall, } from './group-outreach.js';
...
function checkTimingIntelligence(context) {
    ...
    if (context.lastOutreachAt) {
        const hoursSinceLastOutreach = (now.getTime() - context.lastOutreachAt.getTime()) / (1000 * 60 * 60);
 ...
 */
export async function processSuperhumanSignals(userId, signals, userContext) {
    if (signals.length === 0)
        return null;
    log.debug({ userId, signalCount: signals.length, signalTypes: signals.map((s) => s.type) }, '🧠 Processing superhuman signals');
    // =========================================================================
        ...
        if (!hasHighReceptivity && !hasOptimalEnergy) {
            log.debug({
                ...
                gatingSignals: signals
                    .filter((s) => ['receptivity_low', 'energy_wave_avoid', 'silence_processing'].includes(s.type))
                    .map((s) => s.type),
    ...
    const matchingRules = OUTREACH_RULES.filter((rule) => {
        ...
        const triggersMet = rule.triggers.operator === 'AND'
        ...
        if (rule.conditions) {
            // Relationship stage
            if (rule.conditions.relationshipStage &&
                !rule.conditions.relationshipStage.includes(userContext.relationshipStage)) {
                return false;
            }
            // Recent outreach
            if (rule.conditions.recentOutreach && userContext.lastOutreachAt) {
                const hoursSinceOutreach = (Date.now() - userContext.lastOutreachAt.getTime()) / (1000 * 60 * 60);
 ...
 */
async function executeOutreachAction(userId, action, context) {
    try {
        switch (action.type) {
...
export function signalFromCrisis(crisisData) {
    return {
        ...
        severity: crisisData.severity === 'severe'
            ? 'urgent'
...
export function signalFromCapacity(capacity) {
    if (capacity.level !== 'depleted' && capacity.level !== 'low')
        return null;
    return {
...
export function signalFromOpenLoop(loop) {
    if (loop.priority < 3)
        return null; // Only high priority loops
    return {
...
export function signalFromVoiceDistress(voice) {
    if (!voice.hasStrain && !voice.hasTremor && voice.arousal < 0.7)
        return null;
    return {
        type: 'voice_distress',
...
export function signalFromDreamReignition(dream) {
    if (!dream.mentionedAgain || dream.dormantDays < 30)
        return null;
    return {
...
export function signalFromReconnection(reconnect) {
    if (reconnect.daysSinceLastMention < 30 || reconnect.importance < 0.5)
        return null;
    return {
...
export function signalFromLifeChapter(chapter) {
    if (!chapter.isNewChapter || chapter.significance === 'minor')
        return null;
    return {
...
export function signalFromSeasonalDate(date) {
    ...
    if (date.daysUntil > threshold || date.daysUntil < 0)
        return null;
    return {
...
export function signalFromSilence(silence) {
    if (silence.silenceType !== 'processing' && silence.silenceType !== 'invitation')
        return null;
    return {
...
export function signalFromBlindSpot(pattern) {
    if (!pattern.surfaceable || pattern.confidence < 0.7)
        return null;
    return {
...
export function signalFromFutureTrajectory(trajectory) {
    if (trajectory.concerningPatterns.length === 0)
        return null;
    return {
...
export function signalFromVoiceBiomarkers(biomarkers) {
    if (!biomarkers.trends.concerning && biomarkers.stressLevel < 0.7)
        return null;
    return {
...
export function signalFromMoodPrediction(prediction) {
    ...
    if (!negativeMoods.includes(prediction.predictedMood.toLowerCase()) ||
...
export function signalFromConflict(conflict) {
    if (conflict.resolved || conflict.daysSinceConflict < 2)
        return null;
    return {
...
export function signalFromCalendarPrep(event) {
    ...
    if (event.hoursUntil > thresholds[event.difficulty])
        return null;
    return {
        type: 'calendar_prep_needed',
        severity: event.difficulty === 'high_stakes'
            ? 'high'
...
export function signalFromInsideJoke(opportunity) {
    if (!opportunity.canCallback || opportunity.relevanceScore < 0.6)
        return null;
    return {
...
export function signalFromSleep(sleep) {
    if (sleep.quality !== 'poor' && sleep.hoursSlept >= 6)
        return null;
    return {
...
export function signalFromCorrelation(correlation) {
    if (!correlation.actionable || correlation.strength < 0.6)
        return null;
    return {
...
export function signalFromCounterfactual(decision) {
    if (!decision.hasRegret || !decision.alternativeMentioned)
        return null;
    return {
 ...
 */
export function accumulateSignal(userId, signal) {
    const existing = sessionSignals.get(userId) || [];
    // Dedupe by type within 5 minutes
    const fiveMinutesAgo = Date.now() - 5 * 60 * 1000;
    const filtered = existing.filter((s) => s.timestamp.getTime() > fiveMinutesAgo || s.type !== signal.type);
    ...
    log.debug({ userId, signalType: signal.type, totalSignals: filtered.length }, '🧠 Signal accumulated');
}
/**
 * Get accumulated signals for a user (and optionally clear them).
 */
export function getAccumulatedSignals(userId, clear = false) {
 ...
 */
export async function processAccumulatedSignals(userId, userContext) {
    const signals = getAccumulatedSignals(userId, true);
    if (signals.length === 0) {
        return null;
    }
    log.info({ userId, signalCount: signals.length }, '🧠 Processing accumulated signals at session end');
 ...
 */
export async function integrateWithSemanticIntelligence(userId, turnData) {
...
//# sourceMappingURL=superhuman-outreach-intelligence.js.map