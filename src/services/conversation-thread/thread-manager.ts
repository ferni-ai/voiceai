...
import {
  saveThread as persistThread,
  loadThread as loadPersistedThread,
  loadActiveThread as loadPersistedActiveThread,
...
const activeThreads = new Map<string, ConversationThread>();
/** Thread lookup by ID */
const threadIndex = new Map<string, ConversationThread>();
 ...
 * Checks both in-memory cache and Firestore.
 */
export async function getActiveThread(userId: string): Promise<ConversationThread | null> {
  // Check in-memory cache first
  const cached = activeThreads.get(userId);
  if (cached && !isThreadStale(cached)) {
    return cached;
 ...
 */
function isThreadStale(thread: ConversationThread): boolean {
...
export async function addMessage(
  threadId: string,
  message: Omit<ThreadMessage, 'id' | 'threadId'>
 ...
 */
export async function updateThreadTopics(threadId: string, topics: string[]): Promise<void> {
  const thread = threadIndex.get(threadId);
  if (!thread) return;
 ...
 */
export async function updateEmotionalContext(
  threadId: string,
  current: string,
...
export async function buildAgentContext(
  ...
  const isNewToThread =
    !thread.ownershipHistory.some((h) => h.toAgentId === agentId) &&
  ...
  const previousOwner =
    lastTransfer && thread.currentOwnerId === agentId ? lastTransfer.fromAgentId : undefined;
...
function formatLLMContext(
  ...
  if (thread.channelsUsed.length > 1) {
    lines.push(
      `This conversation has spanned multiple channels: ${thread.channelsUsed.join(', ')}.`
    );
  ...
  if (thread.ownershipHistory.length > 0) {
    lines.push(
      `This conversation has involved: ${[...new Set([thread.ownershipHistory[0].fromAgentId, ...thread.ownershipHistory.map((h) => h.toAgentId)])].join(' → ')}`
    );
    if (options.previousOwner) {
      lines.push(
        `You're taking over from ${options.previousOwner}. Review their context carefully.`
      );
    }
    lines.push('');
  }
  ...
  if (thread.emotionalContext) {
    lines.push(
      `Current emotional state: ${thread.emotionalContext.current} (${thread.emotionalContext.trajectory})`
    );
    lines.push('');
  }
  ...
  if (messages.length > 0) {
    ...
    for (const msg of messages.slice(-10)) {
      ...
      lines.push(
        `${roleLabel} ${channelLabel}: ${msg.content.slice(0, 200)}${msg.content.length > 200 ? '...' : ''}`
      );
    }
    lines.push('');
  }
  ...
  if (options.previousOwner) {
    lines.push(
      `• ${options.previousOwner} handed this to you. Honor their work - don't start over.`
    );
  }
 ...
 */
export function getUserThreads(userId: string): ConversationThread[] {
 ...
 */
export function clearStaleThreads(): number {
  let cleared = 0;
