 ...
 */
export async function routeInbound(userId, channel, content, options) {
    ...
    if (bridgeContext && bridgeContext.isDirectResponse) {
        const agentId = bridgeContext.outreach.personaId;
        log.info({ userId, agentId, outreachId: bridgeContext.outreach.outreachId }, '↩️ Routing reply to original outreach agent');
        return {
            agentId,
            confidence: 0.95,
    ...
    if (activeThread) {
        log.info({ userId, agentId: activeThread.currentOwnerId, threadId: activeThread.id }, '🧵 Routing to active thread owner');
        return {
    ...
    if (intentRoute && intentRoute.confidence > 0.75) {
        log.info({ userId, agentId: intentRoute.agentId, confidence: intentRoute.confidence }, '🎯 Routing by intent detection');
        return {
 ...
 */
function detectIntentRoute(content) {
 ...
 */
export async function handleInboundSMS(userId, fromPhone, body) {
    // Route the message
 ...
 */
export async function handlePushTap(userId, notificationData) {
 ...
 */
export async function handleVoiceCallStart(userId, options) {
    ...
    if (options?.fromNotification && options?.outreachId) {
        ...
        if (bridgeContext) {
            return {
                agentId: bridgeContext.outreach.personaId,
                confidence: 0.95,
...
function detectSentiment(text) {
    const positive = /\b(thanks|thank|great|awesome|love|yes|sure|ok|okay|perfect|wonderful|amazing|good|happy|excited)\b/i;
    const negative = /\b(no|stop|don't|hate|bad|terrible|awful|annoyed|angry|frustrated|sad|upset|worried)\b/i;
