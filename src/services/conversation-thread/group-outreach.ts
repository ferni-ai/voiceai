...
import type { EngagementChannel, ConversationThread } from './types.js';
...
} from '../outreach/persona-voice-generator.js';
...
function getDisplayName(personaId: PersonaId): string {
  return PERSONA_DISPLAY_NAMES[personaId] || personaId;
}

function formatPersonaList(personas: PersonaId[]): string {
 ...
 */
export function generateGroupCallIntroductions(
  personas: PersonaId[],
  topic: string,
  leadPersona: PersonaId
): Map<PersonaId, string> {
  const introductions = new Map<PersonaId, string>();
  const teamNames = formatPersonaList(personas);

  // Lead persona opens
  introductions.set(
    leadPersona,
    `Hey there! It's ${getDisplayName(leadPersona)}. ` +
  ...
  for (const personaId of personas) {
    if (personaId === leadPersona) continue;
 ...
 */
export async function initiateGroupOutreach(
  options: GroupOutreachOptions
  ...
  if (options.personas.length < 2) {
    log.warn(
      { userId: options.userId, personas: options.personas },
      'Group outreach requires at least 2 personas'
    );
    return {
      success: false,
      outreachId,
  ...
  try {
    ...
    let roundtableConfig: RoundtableSetupConfig | undefined;
    if (options.preferredChannel === 'voice' || options.preferredChannel === 'call') {
      roundtableConfig = {
    ...
    log.error(
      { error: String(error), userId: options.userId },
      'Failed to initiate group outreach'
    );
    return {
      success: false,
      outreachId,
 ...
 */
export async function mayaJordanPlanningOutreach(
  userId: string,
  options: {
    eventName: string;
    eventDate?: Date;
 ...
 */
export async function peterFerniInsightOutreach(
  userId: string,
  options: {
    topic: string;
    insight: string;
 ...
 */
export async function teamCelebrationOutreach(
  userId: string,
  options: {
    achievement: string;
    preferredName?: string;
 ...
 */
export async function fullTeamSupportOutreach(
  userId: string,
  options: {
    situation: string;
    preferredName?: string;
 ...
 */
export async function initiateTeamRoundtableCall(
  userId: string,
  options: {
    personas: PersonaId[];
    topic: string;
...
function mapChannelToOutreach(channel: EngagementChannel): 'sms' | 'email' | 'call' | 'push' {
  switch (channel) {
    case 'sms':
      return 'sms';
    case 'email':
 ...
 */
async function queueGroupDelivery(
  options: GroupOutreachOptions,
  outreachId: string,
  message: string,
  roundtableConfig?: RoundtableSetupConfig
  ...
  try {
    ...
    if (!options.scheduledFor || options.scheduledFor <= new Date()) {
      ...
      log.debug(
        { outreachId, scheduledFor: options.scheduledFor },
        'Group outreach scheduled for later (not yet implemented)'
      );
    }
  } catch (error) {
    // Graceful degradation - log but don't fail
    log.warn(
      { error: String(error), outreachId },
      'Failed to deliver group outreach (delivery system may not be available)'
    );
  }
}
