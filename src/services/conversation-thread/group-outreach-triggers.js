...
import { initiateGroupOutreach, mayaJordanPlanningOutreach, peterFerniInsightOutreach, teamCelebrationOutreach, fullTeamSupportOutreach, initiateTeamRoundtableCall, } from './group-outreach.js';
 ...
 */
const GROUP_OUTREACH_TRIGGER_TYPES = new Set([
 ...
 */
export function shouldTriggerGroupOutreach(userId, triggerType, context) {
 ...
 */
export async function triggerGroupOutreach(userId, triggerType, context) {
 ...
 */
export async function onCommitmentMilestone(userId, milestone) {
    // Only celebrate significant milestones
    if ((milestone.streakDays || 0) < 7 && (milestone.completionRate || 0) < 0.5) {
        return null;
    }
    const achievement = milestone.streakDays && milestone.streakDays >= 7
 ...
 */
export async function onReconnectionOpportunity(userId, opportunity) {
 ...
 */
export async function onPredictivePattern(userId, pattern) {
 ...
 */
export async function onEmotionalDistress(userId, signal) {
...
//# sourceMappingURL=group-outreach-triggers.js.map