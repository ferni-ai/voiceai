 ...
 */
export const PERSONA_CAMEO_CONFIGS = {
    'peter-john': {
        introductions: [
            'Quick data point—',
 ...
 */
export function getCooldownForPriority(priority) {
    switch (priority) {
        case 'celebration':
 ...
 */
export function getPersonaCameoConfig(personaId) {
    return PERSONA_CAMEO_CONFIGS[personaId];
}
/**
 * Get a random introduction phrase for a persona
 */
export function getRandomIntroduction(personaId) {
 ...
 */
export function getRandomHandback(personaId) {
    const config = PERSONA_CAMEO_CONFIGS[personaId];
 ...
 */
export function isCooldownExpired(lastCameoEndTime, priority = 'normal', customCooldownMs) {
 ...
 * @param customCooldownMs - Optional custom cooldown (e.g., from user preferences)
 */
export function getRemainingCooldown(lastCameoEndTime, priority = 'normal', customCooldownMs) {
    const baseCooldown = getCooldownForPriority(priority);
    // Use custom cooldown if provided and greater than base (more restrictive wins)
    const cooldown = customCooldownMs ? Math.max(baseCooldown, customCooldownMs) : baseCooldown;
 ...
 */
export function getTotalTransitionTime() {
    return (CAMEO_TIMING.ARRIVAL_DELAY +
...
//# sourceMappingURL=cameo-timing.js.map