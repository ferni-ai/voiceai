 ...
 */
export function detectCameoOpportunity(context, sessionState) {
 ...
 */
function detectDirectPersonaMention(message) {
    const nameMappings = {
        peter: 'peter-john',
 ...
 */
function detectKeywordTrigger(message) {
    let bestMatch = null;
        ...
        if (matchCount > 0) {
            ...
            if (!bestMatch || confidence > bestMatch.confidence) {
                bestMatch = {
                    ...
                    triggerType: getTriggerTypeForPersona(personaId),
 ...
 */
function detectEmotionalTrigger(emotionalState, sessionState) {
 ...
 */
function detectConversationPattern(context, sessionState) {
    ...
    if (topicPersona && !shouldSkipPersonaForVariety(topicPersona, sessionState)) {
        ...
        const mentionCount = topicKeywords.filter((topic) => allText.includes(topic.toLowerCase())).length;
        if (mentionCount >= 3) {
            return {
                shouldCameo: true,
 ...
 */
function shouldSkipPersonaForVariety(personaId, sessionState) {
 ...
 */
function getTriggerTypeForPersona(personaId) {
    const typeMap = {
 ...
 */
export function generateCameoDetectionPrompt(context) {
    const history = context.conversationHistory
        .slice(-6)
 ...
 */
export function parseCameoDetectionResponse(response) {
    try {
        ...
        return {
            ...
            triggerType: getTriggerTypeForPersona(parsed.personaId),
        };
    }
    catch (error) {
...
//# sourceMappingURL=cameo-triggers.js.map