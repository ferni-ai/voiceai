...
import { getUserPreferences } from './cameo-analytics.js';
import { getCameoGreeting, getCameoHandback, getPersonaColor } from './cameo-content.js';
import { CAMEO_CONFIG, CAMEO_TIMING, getPersonaCameoConfig, getRemainingCooldown, isCooldownExpired, } from './cameo-timing.js';
const log = getLogger();
// Cache user preferences per session to avoid repeated lookups
const sessionPreferencesCache = new Map();
// ============================================================================
// EVENT EMITTER
// ============================================================================
/**
 ...
 */
export function getCameoSessionState(sessionId) {
 ...
 */
export async function executeCameo(request, options) {
    ...
    if (state.lastCameoEndTime) {
        ...
        if (timeSinceLastCameo < MIN_CAMEO_INTERVAL_MS) {
            log.debug({ timeSinceLastCameo, minInterval: MIN_CAMEO_INTERVAL_MS }, 'Cameo rate limited - too soon after previous cameo');
            return {
                success: false,
    ...
    if (userPrefs?.avoidPersonas.includes(request.personaId)) {
        log.debug({ personaId: request.personaId, userId }, 'Skipping cameo - user prefers to avoid this persona');
        return {
            success: false,
    ...
    const handlerCompletePromise = new Promise((resolve) => {
        ...
        const timeoutId = setTimeout(() => {
            log.warn({ cameoId, personaId: request.personaId }, '⚠️ Cameo handler timeout - proceeding without confirmation');
 ...
 */
export async function endCameo(sessionId, cameoId) {
    const state = getSessionState(sessionId);
    // Validate
    if (!state.isInCameo) {
        return {
 ...
 */
export async function cancelCameo(sessionId, reason) {
    const state = getSessionState(sessionId);
    if (!state.isInCameo) {
        return {
            success: false,
 ...
 */
export function isInCameo(sessionId) {
    const state = sessionStates.get(sessionId);
 ...
 */
export function getCurrentCameoPersona(sessionId) {
    const state = sessionStates.get(sessionId);
 ...
 */
export function hasPersonaCameoed(sessionId, personaId) {
    const state = sessionStates.get(sessionId);
 ...
 * Get cooldown status
 */
export function getCooldownStatus(sessionId, priority = 'normal') {
    const state = sessionStates.get(sessionId);
    if (!state) {
 ...
 */
export function getCameoStats(sessionId) {
    const state = sessionStates.get(sessionId);
    if (!state) {
        return {
            totalCameos: 0,
    ...
    const avgDuration = completedCameos.length > 0
...
//# sourceMappingURL=cameo-orchestrator.js.map