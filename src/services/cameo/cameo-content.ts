 ...
 */
const FIRST_TIME_INTRODUCTIONS: Record<CameoPersonaId, string[]> = {
  'peter-john': [
 ...
 */
const RETURNING_INTRODUCTIONS: Record<CameoPersonaId, string[]> = {
  'peter-john': [
 ...
 */
const TRIGGER_GREETINGS: Record<CameoTriggerType, Record<CameoPersonaId, string[]>> = {
  data_insight: {
    'peter-john': [
 ...
 */
const CONTEXTUAL_HANDBACKS: Record<CameoTriggerType, Record<CameoPersonaId, string[]>> = {
  data_insight: {
 ...
 */
export function getCameoGreeting(
 ...
 */
export function getCameoHandback(
  personaId: CameoPersonaId,
  options: {
    triggerType?: CameoTriggerType;
 ...
 */
export function buildCameoSpeech(
  personaId: CameoPersonaId,
  insight: string,
  options: {
    isFirstCameo: boolean;
...
): { greeting: string; insight: string; handback: string; fullSpeech: string } {
 ...
 */
export function getTriggerTopicsForPersona(personaId: CameoPersonaId): string[] {
 ...
 */
export function getBestPersonaForTopic(topic: string): CameoPersonaId | null {
  ...
  for (const [personaId, config] of Object.entries(PERSONA_CAMEO_CONFIGS)) {
    for (const triggerTopic of config.triggerTopics) {
      if (topicLower.includes(triggerTopic.toLowerCase())) {
        return personaId as CameoPersonaId;
      }
    }
  }
 ...
 */
export function isPersonaEnergetic(personaId: CameoPersonaId): boolean {
