 ...
 */
export function getPersonaRelationshipStage(profile, personaId) {
 ...
 */
export function getPersonaRelationshipData(profile, personaId) {
 ...
 */
export function calculatePersonaRelationshipStage(data, personaId) {
    const thresholds = {
        ...DEFAULT_THRESHOLDS,
    ...
    if (conversationCount >= thresholds.friendToTrustedAdvisor.conversations &&
    ...
    const deepMoments = keyMoments.filter((m) => m.type === 'vulnerability' || m.type === 'breakthrough').length;
    if (conversationCount >= thresholds.acquaintanceToFriend ||
 ...
 */
export function createDefaultRelationshipData() {
    return {
        conversationCount: 0,
        totalMinutes: 0,
        keyMoments: [],
 ...
 */
export function updatePersonaRelationshipData(existingData, update) {
 ...
 */
export function applyRelationshipUpdateToProfile(profile, personaId, update) {
    // Get existing data
    ...
    if (newStage !== oldStage) {
        getLogger().info({ personaId, from: oldStage, to: newStage }, '🎭 Relationship stage advanced!');
    }
 ...
 */
export function hasMinimumRelationship(profile, personaId, minimumStage) {
 ...
 */
export function getWarmthMultiplier(profile, personaId) {
 ...
 */
export function canTellStory(profile, personaId, storyId, requiredStage = 'acquaintance') {
 ...
 */
export function canShareVulnerability(profile, personaId, vulnerabilityType = 'medium') {
    const minimumStages = {
 ...
 */
export function getRelationshipTransitionPhrase(oldStage, newStage, transitionPhrases) {
 ...
 */
export function getMemoryCallbackPhrase(topic, callbackType, memoryCallbacks) {
    let phrases;
    switch (callbackType) {
        case 'hard_topic':
 ...
 */
export function shouldAnnounceTransition(oldStage, newStage) {
 ...
 */
export function logRelationshipSummary(profile, personaId) {
    const stage = getPersonaRelationshipStage(profile, personaId);
    const data = getPersonaRelationshipData(profile, personaId);
    getLogger().info({
        personaId,
...
//# sourceMappingURL=per-persona-relationship.js.map