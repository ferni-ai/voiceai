...
export async function createShareableContext(params) {
    ...
    if (containsPrivateTopics(params.summary)) {
        log.info({ fromUserId: params.fromUserId, type: params.type }, '🔒 Context not shared - contains private topics');
        return null;
    }
 ...
 */
export async function getPendingContexts(userId) {
    const db = getFirestore();
    if (!db)
        return [];
    try {
 ...
 */
export async function markContextsDelivered(ids) {
    const db = getFirestore();
    if (!db)
        return;
    try {
 ...
 */
export async function getEmotionalStateSummary(userId) {
 ...
 */
function containsPrivateTopics(text) {
...
//# sourceMappingURL=family-context-sharing.js.map