...
export class DailyRitualsService {
     ...
     */
    async getOrCreateProfileAsync(userId) {
     ...
     */
    async activateRitual(userId, ritualId) {
     ...
     */
    async recordCompletionAsync(userId, ritualId, data) {
        const profile = await this.getOrCreateProfileAsync(userId);
        ...
        const daysSinceLast = Math.floor((now.getTime() - lastCompletion.getTime()) / (1000 * 60 * 60 * 24));
        // Update streak
        if (daysSinceLast === 1) {
            ...
            if (streak.currentStreak > 0) {
                streak.streakHistory.push({
                    startDate: new Date(lastCompletion.getTime() - streak.currentStreak * 24 * 60 * 60 * 1000),
        ...
        getLogger().info({ userId, ritualId, streak: streak.currentStreak, isNewRecord }, '✅ Ritual completed');
        return {
     ...
     */
    recordCompletion(userId, ritualId, data) {
     ...
     */
    getStreakCelebration(ritualId, streak) {
        const prompts = RITUAL_PROMPTS[ritualId];
        if (!prompts || !('streakCelebrations' in prompts))
            return undefined;
        const celebrations = prompts.streakCelebrations;
        return celebrations[streak];
    }
    /**
     * Get ritual opening for a persona
     */
    getRitualOpening(ritualId) {
        const prompts = RITUAL_PROMPTS[ritualId];
        if (!prompts || !('openings' in prompts)) {
            return 'Time for your daily ritual.';
        }
        const openings = prompts.openings;
     ...
     */
    getWeatherTrends(userId, days = 7) {
        ...
        const firstAvg = firstHalf.reduce((sum, e) => sum + energyValues[e.weather.energy], 0) / firstHalf.length;
        const secondAvg = secondHalf.reduce((sum, e) => sum + energyValues[e.weather.energy], 0) / secondHalf.length;
        ...
        if (recent.length >= 7) {
            ...
            if (weekdayWeathers.length && weekendWeathers.length) {
                const weekdayEnergy = weekdayWeathers.reduce((sum, e) => sum + energyValues[e.weather.energy], 0) /
                    weekdayWeathers.length;
                const weekendEnergy = weekendWeathers.reduce((sum, e) => sum + energyValues[e.weather.energy], 0) /
     ...
     */
    shouldRemind(userId, ritualId) {
        const profile = this.userProfiles.get(userId);
        if (!profile || !profile.preferences.reminderEnabled)
            return false;
        const streak = profile.streaks[ritualId];
        if (!streak)
            return false;
    ...
    getDueRituals(userId) {
        ...
        return profile.activeRituals
            ...
            if (!ritual)
                return false;
            // Check frequency
            if (ritual.frequency === 'weekday' && isWeekend)
                return false;
            if (ritual.frequency === 'weekend' && !isWeekend)
                return false;
