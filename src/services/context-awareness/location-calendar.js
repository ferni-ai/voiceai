...
import { getUserTokens, refreshAccessToken, isCalendarConfigured, getEvents, generateAuthUrl, exchangeCodeForTokens, } from '../identity/google-calendar-oauth.js';
 ...
 */
export async function fetchUpcomingEvents(userId, hoursAhead = 24) {
...
function classifyEventType(title, description) {
    ...
    if (/meet|call|sync|standup|review|interview/.test(text))
        return 'meeting';
    if (/doctor|dentist|appointment|therapy|haircut/.test(text))
        return 'appointment';
    if (/flight|hotel|travel|trip|vacation/.test(text))
        return 'travel';
    if (/remind|todo|task/.test(text))
        return 'reminder';
    if (/birthday|dinner|party|date|gym|workout/.test(text))
        return 'personal';
    return 'other';
}
function estimateStressWeight(title, attendeeCount) {
    let weight = 0;
 ...
 */
export function updateLocation(userId, latitude, longitude, accuracy) {
    const state = getOrCreateState(userId);
 ...
 */
function classifyLocation(userId, lat, lng) {
 ...
 */
export function saveLocation(userId, name, type, latitude, longitude) {
    const state = getOrCreateState(userId);
 ...
 */
export async function getTravelTime(userId, destinationAddress) {
    const state = userStates.get(userId);
    if (!state?.currentLocation || !config.maps.apiKey)
        return null;
    try {
        ...
        return {
            ...
            trafficCondition: delay > 20 ? 'severe' : delay > 10 ? 'heavy' : delay > 5 ? 'moderate' : 'light',
 ...
 */
export async function generateAnticipationInsights(userId) {
    const state = userStates.get(userId);
    if (!state)
        return [];
    const insights = [];
...
export function generateSuperhumanMoment(userId) {
    ...
    for (const event of state.upcomingEvents) {
        ...
        if (minutesUntil > 0 && minutesUntil <= 60 && (event.stressWeight || 0) > 0.5) {
            moments.push(`You're heading to ${event.title} in ${Math.round(minutesUntil)} mins - want to rehearse that boundary conversation?`);
        }
    }
    // Extra time due to cancelled event
    const cancelledTime = state.upcomingEvents.find((e) => e.title.toLowerCase().includes('cancelled'));
    if (cancelledTime) {
    ...
    if (dayOfWeek === 1) {
        // Monday
        moments.push(`It's Monday - you sometimes seem stressed at the start of the week. How are you feeling?`);
    }
    else if (dayOfWeek === 5 && now.getHours() >= 16) {
        // Friday afternoon
        moments.push(`It's Friday afternoon - how was your week? Anything you want to process before the weekend?`);
    }
 ...
 */
export async function hasCalendarConnected(userId) {
...
//# sourceMappingURL=location-calendar.js.map