...
const loadedUsers = new Set(); // Track which users have been loaded from persistence
let localContactsLoaded = false;
/**
 * Ensure contacts are loaded from persistence for a user
 */
async function ensureContactsLoaded(userId) {
...
export function createContact(userId, data) {
    ...
    void onContactChange(userId, id, {
...
export function updateContact(contactId, updates) {
    ...
    // Index to semantic memory for "Better Than Human" search
    void onContactChange(contact.userId, contactId, {
        name: contact.displayName,
        relationship: contact.relationship || 'contact',
        notes: contact.notes,
        importantDates: contact.birthday
 ...
 */
export function getUserContactsSync(userId) {
 ...
 */
export async function getFavoriteContacts(userId) {
 ...
 * Get recent contacts
 */
export async function getRecentContacts(userId, limit = 10) {
    const contacts = await getUserContacts(userId);
    return contacts
 ...
 */
export async function getContactsByGroup(userId, group) {
    const contacts = await getUserContacts(userId);
 ...
 */
export async function findContact(userId, query) {
...
export async function findContactByPhone(userId, phone) {
    ...
    const contacts = await getUserContacts(userId);
    return (contacts.find((c) => c.phones.some((p) => normalizePhone(p.number) === normalized)) || null);
}
/**
 * Find contact by email
 */
export async function findContactByEmail(userId, email) {
 ...
 */
export async function importFromGoogle(userId, accessToken) {
    const stats = { imported: 0, skipped: 0, errors: 0 };
    try {
        // Fetch contacts from Google People API
        const response = await fetch(`${GOOGLE_PEOPLE_API}/people/me/connections?personFields=names,phoneNumbers,emailAddresses,organizations,addresses,birthdays,relations&pageSize=1000`, {
            headers: {
...
function mapGooglePhoneType(type) {
    if (!type)
        return 'mobile';
...
function mapGoogleEmailType(type) {
    if (!type)
        return 'personal';
    const t = type.toLowerCase();
    if (t.includes('home') || t.includes('personal'))
        return 'personal';
    if (t.includes('work'))
        return 'work';
    return 'other';
 ...
 */
export function importFromVCard(userId, vCardData) {
...
function parseVCard(userId, vcard) {
 ...
 */
export function importFromCSV(userId, csvData, mapping) {
    const stats = { imported: 0, errors: 0 };
 ...
 */
export function formatContactForSpeech(contact) {
 ...
 */
export function markContacted(contactId) {
    const contact = contactsStore.get(contactId);
    if (contact) {
 ...
 */
export function addNickname(contactId, nickname) {
    const contact = contactsStore.get(contactId);
    if (!contact)
        return null;
