 ...
 */
export function recordMention(userId, name, context, sentiment, topics = [], emotionalWeight = 0.5) {
...
export function extractNames(text) {
    ...
    const relationshipMentions = text.match(/\b(my\s+)?(mom|dad|mother|father|brother|sister|boss|therapist|partner|husband|wife)\b/gi);
            ...
            for (const [canonical, aliases] of Object.entries(RELATIONSHIP_ALIASES)) {
                if (aliases.includes(normalized) || normalized === canonical) {
                    results.push({
                        ...
                        context: text.slice(Math.max(0, text.indexOf(mention) - 30), Math.min(text.length, text.indexOf(mention) + mention.length + 30)),
                    });
                    break;
                }
            }
 ...
 */
function isLikelyRealName(word) {
 ...
 */
export function detectWithdrawal(userId) {
    ...
    for (const person of graph.people.values()) {
        ...
        if (!person.isConfirmedImportant && person.importance < 0.5)
            continue;
        const daysSinceLastMention = Math.floor((now.getTime() - person.lastMentioned.getTime()) / (1000 * 60 * 60 * 24));
        ...
        if (daysSinceLastMention > usualFrequencyDays * 2 && daysSinceLastMention > 7) {
            alerts.push({
                ...
                significance: person.importance > 0.8 || person.isConfirmedImportant
                    ? 'high'
...
function calculateUsualFrequency(mentions) {
 ...
 */
export function detectSentimentPatterns(userId) {
    const graph = userGraphs.get(userId);
    if (!graph)
        return [];
    const patterns = [];
    for (const person of graph.people.values()) {
        const personMentions = graph.mentions.filter((m) => m.personId === person.id);
        if (personMentions.length < 3)
            continue;
        const avgSentiment = personMentions.reduce((sum, m) => sum + m.sentiment, 0) / personMentions.length;
        if (avgSentiment > 0.3) {
            patterns.push({
                personId: person.id,
                personName: person.name,
 ...
 */
export function getUpcomingDates(userId, daysAhead = 7) {
    const graph = userGraphs.get(userId);
    if (!graph)
        return [];
    const dates = [];
 ...
 */
export function addImportantDate(userId, personName, date, // MM-DD format
type, label) {
    const graph = userGraphs.get(userId);
    if (!graph)
        return false;
 ...
 */
export function generateSocialInsights(userId) {
    const insights = [];
    ...
    for (const date of dates) {
        const dayText = date.daysUntil === 0
            ? 'Today'
            : date.daysUntil === 1
                ? 'Tomorrow'
...
export function generateSuperhumanMoment(userId) {
    ...
    for (const date of dates) {
        if (date.type === 'birthday') {
            moments.push(`Today's ${date.personName}'s birthday - want to talk about how you're feeling?`);
        }
        else if (date.type === 'memorial') {
            moments.push(`I noticed today might be significant regarding ${date.personName}. I'm here if you want to talk.`);
        }
    }
...
// HELPERS
// ============================================================================
function getOrCreateGraph(userId) {
...
function findPersonByName(graph, name) {
    const normalized = name.toLowerCase();
    for (const person of graph.people.values()) {
        if (person.name.toLowerCase() === normalized)
            return person;
        if (person.aliases.some((a) => a.toLowerCase() === normalized))
            return person;
    }
...
function inferRelationship(name) {
    const lower = name.toLowerCase();
    for (const [type, aliases] of Object.entries(RELATIONSHIP_ALIASES)) {
        if (aliases.includes(lower) || lower === type) {
            if (['mother', 'father', 'sibling', 'child'].includes(type))
                return 'family';
            if (type === 'partner')
                return 'partner';
            if (type === 'friend')
                return 'friend';
            if (['boss', 'therapist'].includes(type))
                return 'professional';
        }
    }
    return 'unknown';
...
function updateImportance(graph, personId) {
    ...
    // - User confirmation
    const recentMentions = graph.mentions.filter((m) => m.personId === personId && Date.now() - m.timestamp.getTime() < 30 * 24 * 60 * 60 * 1000 // Last 30 days
    );
    const frequencyScore = Math.min(1, recentMentions.length / 10);
    const recencyScore = recentMentions.length > 0
        ? Math.max(0, 1 -
            (Date.now() - recentMentions[recentMentions.length - 1].timestamp.getTime()) /
                (14 * 24 * 60 * 60 * 1000))
        : 0;
    const emotionalScore = recentMentions.length > 0
...
// PUBLIC API
// ============================================================================
export function getImportantPeople(userId) {
    const graph = userGraphs.get(userId);
    if (!graph)
        return [];
...
export function getPerson(userId, personId) {
    return userGraphs.get(userId)?.people.get(personId);
}
export function confirmImportantPerson(userId, personId) {
...
export function getMentionFrequency(userId, personName, days) {
    const graph = userGraphs.get(userId);
    if (!graph)
        return 0;
    const person = findPersonByName(graph, personName);
...
export function clearSocialGraph(userId) {
 ...
 */
export function getUserGraph(userId) {
 ...
 */
export async function persistGraphToFirestore(userId, graph) {
    try {
        ...
        const importantPeople = Array.from(graph.people.values()).filter((p) => p.importance >= 0.5 || p.isConfirmedImportant);
        for (const person of importantPeople.slice(0, 10)) {
            void onRelationshipNetworkChange(userId, `person_${person.id}`, {
                person: person.name,
                relationship: person.relationship,
                connectionStrength: person.importance >= 0.8
                    ? 'core'
        ...
        for (const pattern of graph.patterns.filter((p) => p.confidence >= 0.7)) {
            void onCorrelationInsightChange(userId, `rel_pattern_${pattern.personId}`, {
 ...
 */
export async function loadGraphFromFirestore(userId) {
    try {
        const { getFirestoreDb } = await import('../superhuman/firestore-utils.js');
        const db = getFirestoreDb();
        if (!db)
        ...
        graph.mentions = (data.mentions || []).map((m) => ({
            ...m,
 ...
 */
export function getAllPeople(userId) {
    const graph = userGraphs.get(userId);
    if (!graph)
        return [];
 ...
 */
export function getSocialInsights(userId) {
    const graph = userGraphs.get(userId);
    if (!graph)
        return [];
 ...
 */
export function clearAllSocialGraphs() {
 ...
 */
export function getCachedGraphCount() {
 ...
 */
export function pruneMentions(userId, retentionDays = 90) {
    const graph = userGraphs.get(userId);
    if (!graph)
        return 0;
 ...
 */
export function cleanupInactiveGraphs(inactiveDays = 7) {
    const cutoffDate = new Date();
