 ...
 */
export function checkCooldown(webhookId, cooldownSeconds) {
 ...
 */
function recordCooldown(webhookId) {
 ...
 */
function processPayloadTemplate(template, context) {
 ...
 * Execute a webhook
 */
export async function executeWebhook(userId, webhook, triggeredBy, voiceContext) {
    ...
    if (!cooldownStatus.allowed) {
        log.debug({ userId, webhookId: webhook.id, waitSeconds: cooldownStatus.waitSeconds }, 'Webhook in cooldown');
        return {
            success: false,
            webhookId: webhook.id,
            webhookName: webhook.name,
    ...
    while (attempts <= MAX_RETRIES) {
        try {
            ...
            if (attempts <= MAX_RETRIES) {
                log.debug({ userId, webhookId: webhook.id, attempt: attempts, statusCode: result.statusCode }, 'Retrying webhook after transient failure');
            ...
            if (!isTimeout && isRetryable(undefined, errorMessage) && attempts < MAX_RETRIES) {
                attempts++;
                log.debug({ userId, webhookId: webhook.id, attempt: attempts, error: errorMessage }, 'Retrying webhook after network error');
                await sleep(RETRY_DELAY_MS);
                continue;
            }
            incrementRateLimit(userId);
            await recordExecution(userId, webhook.id, false);
            log.error({ userId, webhookId: webhook.id, error: errorMessage, attempts }, isTimeout ? 'Webhook timed out' : 'Webhook execution failed');
            return {
                success: false,
                webhookId: webhook.id,
                webhookName: webhook.name,
    ...
    if (lastResult.success) {
        log.info({ userId, webhookId: webhook.id, statusCode: lastResult.statusCode }, 'Webhook executed successfully');
    }
    else {
        log.warn({ userId, webhookId: webhook.id, statusCode: lastResult.statusCode, attempts }, 'Webhook returned error status after retries');
    }
    return {
 ...
 */
export async function testWebhook(userId, webhook) {
    const startTime = Date.now();
    const timestamp = new Date().toISOString();
    const payloadContext = {
        userId,
    ...
    try {
        ...
        clearTimeout(timeoutId);
        const responseBody = await response.text();
        log.info({ userId, webhookId: webhook.id, statusCode: response.status }, 'Webhook test completed');
        return {
            success: response.ok,
            webhookId: webhook.id,
            webhookName: webhook.name,
 ...
 */
export function createExecutionLog(result, userId, triggeredBy) {
    return {
 ...
 */
export function clearRateLimitCache(userId) {
    userRateLimits.delete(userId);
}
/**
 * Clear cooldown cache for a webhook (for testing)
 */
export function clearCooldownCache(webhookId) {
    webhookCooldowns.delete(webhookId);
}
/**
 * Clear all caches (for testing)
 */
export function clearAllExecutorCaches() {
    userRateLimits.clear();
    webhookCooldowns.clear();
}
//# sourceMappingURL=webhook-executor.js.map