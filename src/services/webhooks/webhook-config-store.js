 ...
 */
async function loadUserWebhooks(userId) {
 ...
 * Update a webhook
 */
export async function updateWebhook(userId, input) {
    await loadUserWebhooks(userId);
    const cache = getUserWebhookCache(userId);
 ...
 */
export async function findWebhookByTrigger(userId, phrase) {
    await loadUserWebhooks(userId);
    const cache = getUserWebhookCache(userId);
 ...
 */
export async function recordExecution(userId, webhookId, success) {
    await loadUserWebhooks(userId);
    const cache = getUserWebhookCache(userId);
    const webhook = cache.get(webhookId);
    if (!webhook)
 ...
 */
export async function getWebhookStats(userId) {
    await loadUserWebhooks(userId);
    const cache = getUserWebhookCache(userId);
    const webhooks = Array.from(cache.values());
    ...
    const lastExecution = webhooks
        ...
        .sort((a, b) => (b.lastTriggeredAt || '').localeCompare(a.lastTriggeredAt || ''))[0]?.lastTriggeredAt;
    return {
 ...
 */
export async function createSiriToken(userId, input) {
    const firestore = await getFirestore();
 ...
 */
export async function validateSiriToken(userId, token) {
    const firestore = await getFirestore();
    if (!firestore)
        return null;
    const tokenHash = await hashToken(token);
    try {
        ...
        if (snapshot.empty)
            return null;
        const siriToken = snapshot.docs[0].data();
        // Update usage stats
        await firestore
            .collection(USERS_COLLECTION)
            .doc(userId)
 ...
 */
export async function listSiriTokens(userId) {
    const firestore = await getFirestore();
    if (!firestore)
        return [];
    try {
 ...
 * Delete a Siri token
 */
export async function deleteSiriToken(userId, tokenId) {
    const firestore = await getFirestore();
    const cache = getUserTokenCache(userId);
...
//# sourceMappingURL=webhook-config-store.js.map