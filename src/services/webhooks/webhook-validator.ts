...
import type { PlatformValidation, UrlValidationResult, WebhookPlatform } from './types.js';
...
const PLATFORM_PATTERNS: PlatformValidation[] = [
  {
    platform: 'ifttt',
 ...
 */
function isHomeAssistant(url: URL): boolean {
...
export function validateWebhookUrl(
  ...
  // Detect platform
  let detectedPlatform: WebhookPlatform | undefined;
 ...
 */
export function validateWebhookUrls(
  urls: string[],
  options: { allowPrivateIps?: boolean } = {}
): { valid: boolean; results: UrlValidationResult[] } {
 ...
 */
export function validateVoiceTriggers(triggers: string[]): {
  valid: boolean;
  error?: string;
  normalized: string[];
} {
 ...
 */
export function getPlatformInstructions(platform: WebhookPlatform): {
  name: string;
  setupUrl: string;
  instructions: string[];
} {
