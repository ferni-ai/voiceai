 ...
 */
export function getPersonalizedWisdom(userId, situation, contextFactors) {
    const prefs = userPreferences.get(userId);
    // Find relevant approaches
    const relevant = aggregatedWisdom.filter((w) => w.situation.category === situation.category &&
    ...
    const scored = relevant.map((approach) => {
        ...
        if (contextFactors) {
            const matchesWorksWell = contextFactors.filter((f) => approach.worksWellWhen.some((w) => w.toLowerCase().includes(f.toLowerCase()))).length;
            const matchesWorksLess = contextFactors.filter((f) => approach.worksLessWellWhen.some((w) => w.toLowerCase().includes(f.toLowerCase()))).length;
 ...
 */
export function contributeWisdom(contribution) {
    // Find matching approach pattern
    const pattern = aggregatedWisdom.find((w) => w.situation.category === contribution.situationType.category &&
    ...
    if (pattern) {
        ...
        log.debug({ pattern: pattern.id, helpful: contribution.wasHelpful }, 'Wisdom contribution recorded');
    }
}
/**
 * Record user preference.
 */
export function recordUserPreference(userId, approachId, wasHelpful) {
 ...
 */
export function getWisdomContextInjection(userId, situation) {
 ...
 */
export function getPopulationInsights(situation) {
    const relevant = aggregatedWisdom.filter((w) => w.situation.category === situation.category &&
        w.situation.subcategory === situation.subcategory);
...
// HELPERS
// ============================================================================
function generateReasoning(approach, fit, prefs) {
    const reasons = [];
 ...
 */
export function aggregatePopulationWisdom() {
