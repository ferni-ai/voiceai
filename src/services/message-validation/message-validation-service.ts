...
const draftCache = new Map<string, MessageDraft[]>();
...
export function analyzeMessage(
  ...
  if (
    contentLower.includes('hope you') ||
  ...
  const toneFreq = new Map<MessageTone, number>();
  for (const tone of tones) {
    toneFreq.set(tone, (toneFreq.get(tone) || 0) + 1);
  }

  let dominantTone: MessageTone = 'neutral';
  let maxFreq = 0;
 ...
 */
function calculateWaitHours(analysis: MessageAnalysis, timeOfDay?: Date): number {
 ...
 */
export async function getPendingDrafts(userId: string): Promise<MessageDraft[]> {
  await ensureUserLoaded(userId);
  const drafts = draftCache.get(userId) || [];
 ...
 */
export async function getDraft(userId: string, draftId: string): Promise<MessageDraft | null> {
  await ensureUserLoaded(userId);
  const drafts = draftCache.get(userId) || [];
 ...
 */
export async function getDraftsReadyForReview(userId: string): Promise<MessageDraft[]> {
  await ensureUserLoaded(userId);
  const drafts = draftCache.get(userId) || [];
  const now = new Date();
 ...
 * Discard a draft
 */
export async function discardDraft(userId: string, draftId: string): Promise<boolean> {
  const draft = await getDraft(userId, draftId);
 ...
 */
export async function isReadyForReview(userId: string, draftId: string): Promise<boolean> {
  const draft = await getDraft(userId, draftId);
  if (!draft) return false;
 ...
 */
export async function getTimeUntilReview(userId: string, draftId: string): Promise<number | null> {
  const draft = await getDraft(userId, draftId);
 ...
 */
export function formatAnalysisForSpeech(analysis: MessageAnalysis): string {
  let output = '';
...
async function persistDraft(draft: MessageDraft): Promise<void> {
  const firestore = await getFirestore();
  if (!firestore) return;
