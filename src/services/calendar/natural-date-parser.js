...
export function parseNaturalDate(input, options = {}) {
    const { referenceDate = new Date(), defaultTime = { hour: 9, minute: 0 }, prefer = 'future', } = options;
 ...
 */
function parseExactDateTime(input, reference, defaultTime, prefer) {
    ...
    const regex = new RegExp(`(${monthPattern})\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:\\s+(?:at\\s+)?(\\d{1,2})(?::(\\d{2}))?\\s*(am|pm)?)?`, 'i');
 ...
 */
function parseRelativeDay(input, reference, defaultTime, _prefer) {
 ...
 */
function parseRelativeTime(input, reference, _defaultTime, _prefer) {
 ...
 */
function parseNextWeekday(input, reference, defaultTime, prefer) {
    // Find day name
    let targetDay = -1;
 ...
 */
function parseDayOfMonth(input, reference, defaultTime, prefer) {
 ...
 */
function parseTimeOfDay(input, reference, defaultTime, prefer) {
    const timeMatch = extractTime(input, defaultTime);
 ...
 */
function extractTime(input, defaultTime) {
 ...
 */
function formatInterpretation(date, hasTime) {
    const options = {
        weekday: 'long',
        month: 'long',
        day: 'numeric',
 ...
 */
export function suggestClarification(parsed) {
    if (!parsed.ambiguous)
        return null;
 ...
 */
export function isValidForScheduling(date) {
    const now = new Date();
    if (date < now) {
 ...
 */
export function suggestTimes(vagueness, reference = new Date()) {
    const suggestions = [];
    switch (vagueness) {
        case 'morning':
