...
import { getValidAccessToken, getEvents as getGoogleEvents, createEvent as createGoogleEvent, updateEvent as updateGoogleEvent, deleteEvent as deleteGoogleEvent, getFreeBusy, isCalendarConfigured, } from '../identity/google-calendar-oauth.js';
import { getEventsForDay as getUnifiedEventsForDay, getEvents as getUnifiedEvents, createEvent as createUnifiedEvent, updateEvent as updateUnifiedEvent, deleteEvent as deleteUnifiedEvent, } from './unified-calendar-store.js';
const log = getLogger();
// ============================================================================
 ...
 */
function unifiedToCalendarEvent(unified, calendarId = 'ferni') {
    return {
        id: unified.id,
        title: unified.title,
 ...
 */
function toUnifiedCreateInput(input) {
 ...
 */
function toUnifiedUpdateInput(updates) {
    const result = {};
    ...
    if (updates.location !== undefined)
        result.location = updates.location;
    if (updates.startTime !== undefined)
        result.startTime = updates.startTime;
    if (updates.endTime !== undefined)
        result.endTime = updates.endTime;
    if (updates.attendees !== undefined)
        result.attendees = updates.attendees;
...
function calendarEventToGoogleEvent(event) {
 ...
 */
export async function getCalendarMode(userId) {
 ...
 */
export async function getEventsForDay(userId, date = new Date(), calendarId = DEFAULT_CALENDAR_ID) {
 ...
 * Uses Google Calendar if connected, otherwise local storage.
 */
export async function getEventsForWeek(userId, startDate, calendarId = DEFAULT_CALENDAR_ID) {
    const mode = await getCalendarMode(userId);
 ...
 * Uses Google Calendar if connected, otherwise local storage.
 */
export async function createEvent(userId, event, calendarId = DEFAULT_CALENDAR_ID) {
    const mode = await getCalendarMode(userId);
    if (mode === 'google') {
        const accessToken = await getValidAccessToken(userId);
        if (!accessToken) {
 ...
 * Uses Google Calendar if connected, otherwise local storage.
 */
export async function updateEvent(userId, eventId, updates, calendarId = DEFAULT_CALENDAR_ID) {
    ...
    if (mode === 'google') {
        ...
        try {
            const googleUpdates = {};
            if (updates.title)
                googleUpdates.summary = updates.title;
            if (updates.description)
                googleUpdates.description = updates.description;
            if (updates.location)
                googleUpdates.location = updates.location;
 ...
 * Uses Google Calendar if connected, otherwise local storage.
 */
export async function deleteEvent(userId, eventId, calendarId = DEFAULT_CALENDAR_ID) {
    // Check if this is a local/unified event (starts with evt_ or local_)
    if (eventId.startsWith('local_') || eventId.startsWith('evt_')) {
 ...
 */
export async function isTimeSlotAvailable(userId, startTime, endTime, calendarId = DEFAULT_CALENDAR_ID) {
    const accessToken = await getValidAccessToken(userId);
    if (!accessToken) {
        return false;
    }
 ...
 */
export async function findNextAvailableSlot(userId, options) {
    const { durationMinutes, startFrom = new Date(), withinDays = 7, preferredHours = { start: WORK_DAY_START_HOUR, end: WORK_DAY_END_HOUR }, } = options;
 ...
 */
export async function getDayOverview(userId, date = new Date()) {
 ...
 */
export async function getWeekOverview(userId, startDate) {
    const weekStart = startDate || getStartOfWeek(new Date());
    const days = [];
