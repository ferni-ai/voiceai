...
export async function getMeetingAttendeeContext(
  ...
  try {
    ...
    const interactions: StoredInteraction[] = interactionsSnapshot.docs.map(
      (doc) => doc.data() as StoredInteraction
    );
    ...
    const personNotes = noteDoc.exists ? (noteDoc.data() as StoredPersonNote) : null;
    ...
    let frequency: 'weekly' | 'monthly' | 'occasional' | 'rare' = 'rare';
    ...
    return {
      ...
      relationship: {
        type:
          (personNotes?.relationshipType as MeetingMemoryContext['relationship']['type']) ||
          'unknown',
        sentiment: inferSentiment(interactions),
        interactionCount: interactions.length,
        firstInteraction:
          interactions.length > 0 ? new Date(interactions[interactions.length - 1].date) : null,
      },
...
export async function enrichPreMeetingBriefing(
  ...
  const attendeeContexts: MeetingMemoryContext[] = [];
  ...
  for (const ctx of attendeeContexts) {
    ...
    if (ctx.patterns.lastMeetingDaysAgo && ctx.patterns.lastMeetingDaysAgo > 30) {
      suggestedItems.push(
        `Catch up on what's new (haven't met in ${ctx.patterns.lastMeetingDaysAgo} days)`
      );
    }
  }
 ...
 */
export async function recordMeetingInteraction(
  userId: string,
  interaction: {
    personEmail: string;
    personName?: string;
...
function createEmptyContext(email: string): MeetingMemoryContext {
  return {
    attendeeEmail: email,
...
function inferSentiment(
...
): 'positive' | 'neutral' | 'complex' | 'unknown' {
  if (interactions.length === 0) return 'unknown';

  // Simple heuristic based on sentiment tags in interactions
  const sentiments = interactions.map((i) => i.sentiment).filter(Boolean) as string[];
  ...
  if (negative > positive) return 'complex';
  return 'neutral';
}

function generateStandardTips(event: MinimalCalendarEvent, minutesUntil: number): string[] {
  const tips: string[] = [];
...
function assessMeetingPriority(
  event: MinimalCalendarEvent,
  contexts: MeetingMemoryContext[]
  ...
  const hasOpenCommitments = contexts.some(
    (c) => c.lastInteraction && c.lastInteraction.openItems.length > 0
  );
