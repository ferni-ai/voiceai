...
import { parseNaturalDate, suggestTimes, isValidForScheduling, } from './natural-date-parser.js';
import { isTimeSlotAvailable, getEventsForDay, createEvent, findFreeTimeSlots, } from './calendar-service.js';
 ...
 */
export async function parseEventRequest(userId, input, defaultDuration = 60 // minutes
) {
 ...
 */
export async function clarifyEventTime(pendingId, timeInput) {
    ...
    // Check availability
    const isAvailable = await isTimeSlotAvailable(pending.userId, pending.proposedStart, pending.proposedEnd);
    if (!isAvailable) {
 ...
 */
export async function confirmEvent(pendingId) {
    const pending = pendingEvents.get(pendingId);
    if (!pending) {
        return {
            success: false,
 ...
 */
export function cancelPendingEvent(pendingId) {
    const pending = pendingEvents.get(pendingId);
    if (!pending) {
        return {
            success: false,
 ...
 */
export function getPendingEvent(pendingId) {
    return pendingEvents.get(pendingId);
}
/**
 * Get all pending events for a user
 */
export function getUserPendingEvents(userId) {
 ...
 */
function extractEventDetails(input, defaultDuration) {
    ...
    if (lower.includes('quick') || lower.includes('brief'))
        duration = 30;
    if (lower.includes('lunch'))
        duration = 60;
    if (lower.includes('all day') || lower.includes('block'))
        duration = 480;
 ...
 */
function buildConflictPrompt(pending, conflicts) {
    const conflictEvent = conflicts[0];
    const conflictTime = conflictEvent
        ? new Date(conflictEvent.startTime).toLocaleTimeString('en-US', {
            hour: 'numeric',
 ...
 */
function buildAmbiguousTimePrompt(pending, parsed) {
 ...
 */
function buildConfirmationPrompt(pending) {
    if (!pending.proposedStart)
        return `Ready to schedule "${pending.title}"?`;
    const time = pending.proposedStart.toLocaleTimeString('en-US', {
        hour: 'numeric',
        minute: '2-digit',
    });
