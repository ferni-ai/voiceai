...
import { createEvent, deleteEvent, updateEvent, findFreeTimeSlots, getEventsForDay, } from './calendar-service.js';
const log = getLogger();
// ============================================================================
// CONFIGURATION
// ============================================================================
...
// ============================================================================
const externalToCalendarMap = new Map();
// ============================================================================
// CORE FUNCTIONS
// ============================================================================
/**
 ...
 */
export async function createCalendarSyncedItem(input) {
    const { userId, type, title, scheduledFor, durationMinutes = DEFAULT_DURATIONS[type], description, contactName, location, externalId, createdBy, checkConflicts = true, } = input;
    log.info({
        userId,
        type,
        title,
    ...
    try {
        ...
        if (checkConflicts) {
            ...
            if (conflicts.length > 0) {
                log.warn({ userId, conflicts: conflicts.length, type }, '⚠️ Calendar bridge: conflicts detected');
 ...
 */
export async function removeCalendarSyncedItem(userId, externalId) {
 ...
 */
export async function updateCalendarSyncedItem(userId, externalId, updates) {
    const calendarEventId = externalToCalendarMap.get(externalId);
    if (!calendarEventId) {
        log.debug({ userId, externalId }, 'Calendar bridge: no calendar event found for external ID');
        return false;
    ...
    try {
        ...
        if (updates.scheduledFor) {
            ...
            if (updates.durationMinutes) {
                updateInput.endTime = new Date(updates.scheduledFor.getTime() + updates.durationMinutes * 60000);
            }
        }
...
export async function detectConflicts(userId, startTime, endTime) {
    ...
    try {
        ...
        for (const event of dayEvents) {
            ...
            if ((proposedEnd > eventStart - gapMs && proposedEnd <= eventStart) ||
 ...
 */
export async function findNonConflictingSlot(userId, preferredTime, durationMinutes, options = {}) {
 ...
 */
export async function syncReminderToCalendar(userId, reminderId, message, scheduledFor, createdBy) {
 ...
 */
export async function syncScheduledTextToCalendar(userId, messageId, contactName, message, scheduledFor, createdBy) {
    return createCalendarSyncedItem({
        userId,
        type: 'scheduled_text',
 ...
 */
export async function syncScheduledEmailToCalendar(userId, emailId, contactName, subject, scheduledFor, createdBy) {
    return createCalendarSyncedItem({
        userId,
 ...
 */
export async function syncScheduledCallToCalendar(userId, callId, contactName, purpose, scheduledFor, durationMinutes = 15, createdBy) {
    return createCalendarSyncedItem({
        userId,
        type: 'scheduled_call',
        title: purpose,
 ...
 */
export async function syncAppointmentToCalendar(userId, appointmentId, title, scheduledFor, options = {}) {
    return createCalendarSyncedItem({
        userId,
        type: 'appointment',
        title,
 ...
 */
export async function syncTaskToCalendar(userId, taskId, taskTitle, scheduledFor, durationMinutes = 30, createdBy) {
    return createCalendarSyncedItem({
        userId,
        type: 'task',
        title: taskTitle,
 ...
 */
export async function syncMedicationToCalendar(userId, medicationId, medicationName, dosage, scheduledFor, options = {}) {
 ...
 */
export async function syncBillToCalendar(userId, billId, billName, amount, dueDate, options = {}) {
 ...
 */
export async function syncGoalToCalendar(userId, goalId, goalTitle, deadline, options = {}) {
 ...
 */
export async function syncMilestoneToCalendar(userId, milestoneId, milestoneName, eventDate, options = {}) {
    let description = options.description || '';
    if (options.category) {
 ...
 */
export async function syncCommitmentToCalendar(userId, commitmentId, commitmentTitle, targetDate, options = {}) {
    return createCalendarSyncedItem({
        userId,
        type: 'commitment',
 ...
 */
export async function syncTravelToCalendar(userId, travelId, destination, startDate, endDate, options = {}) {
 ...
 */
export async function syncHabitToCalendar(userId, habitId, habitName, scheduledFor, options = {}) {
    let description = '';
