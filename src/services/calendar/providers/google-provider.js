...
import { isOAuthConfigured, getValidAccessToken, getEvents as getGoogleEvents, createEvent as createGoogleEvent, updateEvent as updateGoogleEvent, deleteEvent as deleteGoogleEvent, deleteUserTokens, } from '../../identity/google-calendar-oauth.js';
const log = getLogger();
/**
 * Convert Google Calendar event to unified format
 */
function googleToUnified(event, userId) {
 ...
 */
function unifiedToGoogle(event) {
...
export class GoogleCalendarProvider {
     ...
     */
    async handleAuthCallback(userId, code) {
     ...
     */
    async fetchEvents(userId, startDate, endDate, calendarId = 'primary') {
    ...
    async createEvent(userId, event) {
        ...
        try {
            ...
            const created = await createGoogleEvent(accessToken, event.externalCalendarId || 'primary', googleEvent);
    ...
    async updateEvent(userId, event) {
        ...
        try {
            const googleEvent = unifiedToGoogle(event);
            await updateGoogleEvent(accessToken, event.externalCalendarId || 'primary', event.externalId, googleEvent);
            return true;
        }
        catch (error) {
