...
function getUserProvidersCollection(userId) {
...
function storedToProvider(stored) {
    return {
        ...stored,
 ...
 */
export async function getEventsForDay(userId, date) {
 ...
 */
export async function getEventsForWeek(userId, referenceDate = new Date()) {
 ...
 * Create a new event
 */
export async function createEvent(userId, input, source = 'ferni') {
    await loadUserData(userId);
    const now = new Date();
 ...
 */
export async function importExternalEvent(userId, provider, externalId, externalCalendarId, eventData, etag) {
    await loadUserData(userId);
 ...
 */
export async function importEventsFromProvider(userId, provider, events) {
    let imported = 0;
    for (const eventData of events) {
        try {
            await importExternalEvent(userId, provider, eventData.externalId || `${provider}_${Date.now()}_${imported}`, eventData.externalCalendarId || 'primary', {
                title: eventData.title,
                description: eventData.description,
                location: eventData.location,
                startTime: eventData.startTime,
 ...
 */
export async function getProviderConnections(userId) {
    await loadUserData(userId);
 ...
 */
export async function getProviderConnection(userId, provider) {
    await loadUserData(userId);
 ...
 */
export async function hasAnyProviderConnected(userId) {
 ...
 */
export async function updateProviderConnection(userId, connection) {
    await loadUserData(userId);
    const providers = providerCache.get(userId) || new Map();
 ...
 */
export async function removeProviderConnection(userId, provider) {
    await loadUserData(userId);
    const providers = providerCache.get(userId);
    if (providers) {
 ...
 */
export async function findFreeTimeSlots(userId, date, options = {}) {
 ...
 */
export async function isTimeSlotAvailable(userId, startTime, endTime) {
 ...
 * Get day overview
 */
export async function getDayOverview(userId, date) {
    const events = await getEventsForDay(userId, date);
 ...
 * Get week overview
 */
export async function getWeekOverview(userId, referenceDate = new Date()) {
    const startOfWeek = new Date(referenceDate);
    startOfWeek.setDate(referenceDate.getDate() - referenceDate.getDay());
    startOfWeek.setHours(0, 0, 0, 0);
    const days = [];
...
// PERSISTENCE HELPERS
// ============================================================================
async function persistEvent(userId, event) {
    const firestore = await getFirestore();
    if (!firestore)
        return;
    try {
        ...
        void onCalendarEventChange(userId, event.id, {
            title: event.title,
...
async function deletePersistedEvent(userId, eventId) {
    const firestore = await getFirestore();
    if (!firestore)
        return;
    try {
...
async function persistProviderConnection(userId, connection) {
    const firestore = await getFirestore();
    if (!firestore)
        return;
    try {
 ...
 */
export async function markEventsForSync(userId, eventIds) {
    await loadUserData(userId);
    const events = eventCache.get(userId);
    if (!events)
        return;
 ...
 */
export async function getEventsNeedingSync(userId) {
    await loadUserData(userId);
    const events = eventCache.get(userId) || new Map();
    return Array.from(events.values()).filter((e) => e.syncStatus === 'pending' || e.syncStatus === 'deleted');
}
const PREFERENCES_COLLECTION = 'calendar_preferences';
function getUserPreferencesCollection(userId) {
 ...
 */
export async function getCalendarPreference(userId, calendarId) {
 ...
 */
export async function setCalendarPreference(userId, preference) {
    const firestore = await getFirestore();
    if (!firestore) {
 ...
 */
export async function isCalendarEnabled(userId, calendarId) {
 ...
 */
export async function getEnabledCalendarIds(userId) {
    const preferences = await getCalendarPreferences(userId);
 ...
 */
export async function setCalendarPreferences(userId, preferences) {
    const firestore = await getFirestore();
    if (!firestore) {
        return;
    }
 ...
 */
export function clearPreferencesCache(userId) {
...
//# sourceMappingURL=unified-calendar-store.js.map