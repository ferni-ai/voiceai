...
import { getEvents, getEventsNeedingSync, importExternalEvent, updateEvent, getProviderConnections, updateProviderConnection, clearUserCache, } from './unified-calendar-store.js';
...
export class CalendarSyncEngine {
     ...
     */
    async syncProvider(userId, provider, options = {}) {
     ...
     */
    async pullFromProvider(userId, provider, adapter, startDate, endDate, opts) {
        const result = {
            created: 0,
            updated: 0,
            conflicts: [],
                ...
                if (existing) {
                    ...
                    if (conflict) {
                        ...
                        if (resolved === 'provider') {
                            ...
                            await importExternalEvent(userId, provider, providerEvent.externalId, providerEvent.externalCalendarId || 'primary', {
     ...
     */
    async pushToProvider(userId, provider, adapter, _startDate, _endDate) {
            ...
            for (const event of pendingEvents) {
                try {
                    if (event.syncStatus === 'deleted' && event.externalId) {
                        ...
                        const success = await adapter.deleteEvent(userId, event.externalId, event.externalCalendarId);
                        if (success) {
                            result.deleted++;
                        }
                    }
     ...
     */
    detectConflict(ferniEvent, providerEvent) {
        // If Ferni event was modified after last sync
        const ferniModified = ferniEvent.lastSyncAttempt && ferniEvent.updatedAt > ferniEvent.lastSyncAttempt;
        // Check for meaningful differences
        const timeDiff = ferniEvent.startTime.getTime() !== providerEvent.startTime.getTime() ||
 ...
 */
export async function syncAllProviders(userId, options) {
    return getSyncEngine().syncAll(userId, options);
}
/**
 * Sync a specific provider
 */
export async function syncProvider(userId, provider, options) {
