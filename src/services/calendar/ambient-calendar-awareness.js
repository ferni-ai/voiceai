...
import { getEventsForDay, isConnected, } from './calendar-service.js';
 ...
 */
export async function getAmbientCalendarContext(userId) {
 ...
 */
export function generateAmbientContextInjection(context) {
    ...
    if (context.nextMeeting.shouldWarnUser && context.nextMeeting.minutesUntil !== null) {
        ...
        if (mins <= 2) {
            lines.push(`⚠️ USER HAS "${title}" STARTING IN ${mins} MINUTE${mins !== 1 ? 'S' : ''}. ` +
                'Wrap up NOW or offer to continue after.');
        }
        else if (mins <= 5) {
            lines.push(`⏰ User has "${title}" in ${mins} minutes. ` +
                'Be concise. Offer to continue later if needed.');
        }
        else {
            lines.push(`📅 User has "${title}" in ${mins} minutes. ` + 'Keep this in mind for conversation pacing.');
        }
    }
    // Currently in a meeting
    if (context.currentlyInMeeting && context.currentMeeting) {
        lines.push(`📍 User is currently in "${context.currentMeeting.title}". ` +
    ...
    if (context.justEndedMeeting.event && context.justEndedMeeting.followUpPrompt) {
        ...
        if (isHighPriorityMeeting(context.justEndedMeeting.event)) {
            lines.push(`💬 User just finished "${title}" ${mins} minutes ago. ` +
    ...
    if (context.remainingMeetingsToday >= 4 && context.totalRemainingMeetingMinutes > 180) {
        lines.push(`📊 User has ${context.remainingMeetingsToday} more meetings today ` +
 ...
 */
export function generateAmbientSummaryForUser(context) {
    if (!context.isCalendarConnected) {
        return null;
    }
 ...
 */
function generateWrapUpSuggestion(event, minutesUntil) {
 ...
 */
function generateFollowUpPrompt(event) {
    const title = event.title.toLowerCase();
    if (title.includes('interview')) {
 ...
 */
function isHighPriorityMeeting(event) {
    const title = event.title.toLowerCase();
 ...
 */
export function shouldInterruptForCalendar(context) {
    // Only interrupt for imminent meetings
    return (context.nextMeeting.shouldWarnUser &&
...
//# sourceMappingURL=ambient-calendar-awareness.js.map