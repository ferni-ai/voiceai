...
import { getDayOverview, getWeekOverview, findFreeTimeSlots, createEvent, } from './calendar-service.js';
 ...
 */
export async function detectRecoveryNeeds(userId, settings = {}) {
 ...
 */
export async function autoBlockRecoveryTime(userId, afterMeetingStreak, // minutes of consecutive meetings
settings = {}) {
    const config = { ...DEFAULT_SETTINGS, ...settings };
    ...
    try {
        ...
        const recoveryDuration = Math.min(Math.max(config.minRecoveryMinutes, 30), nextSlot.durationMinutes);
        const eventInput = {
            title: '🧘 Recovery Time',
            description: 'Auto-blocked by Ferni after a long meeting streak. ' +
        ...
        if (event) {
            log.info({ userId, duration: recoveryDuration, startTime: nextSlot.start }, '🧘 Auto-blocked recovery time');
        }
        return event;
    }
    catch (error) {
 ...
 */
export async function findRecoveryOpportunities(userId, minDurationMinutes = 60) {
    const opportunities = [];
    try {
 ...
 */
export async function getRecoverySuggestions(userId) {
    ...
    for (const rec of recommendations.slice(0, 3)) {
        if (rec.type === 'block_time' && rec.suggestedAction.suggestedDuration) {
            suggestions.push(`Block ${rec.suggestedAction.suggestedDuration} minutes for yourself: ${rec.reason}`);
        }
...
export async function buildRecoveryContext(userId) {
    ...
    const sections = ['[RECOVERY PROTECTION - Better Than Human Time Protection]'];
...
//# sourceMappingURL=recovery-protection.js.map