...
export async function generateNudges(userId) {
    ...
    try {
        ...
        for (const contact of needsAttention) {
            const daysSince = Math.floor((now.getTime() - new Date(contact.lastInteraction).getTime()) / (1000 * 60 * 60 * 24));
            nudges.push({
                ...
                suggestedChannel: contact.preferredChannel || 'sms',
        ...
        for (const holiday of HOLIDAYS) {
            ...
            if (daysUntil >= 0 && daysUntil <= holiday.leadTimeDays) {
                ...
                const relevantGroups = groups.filter((g) => {
                    ...
                    if (holiday.occasion === 'christmas')
                        return prefs.christmas !== false;
                    if (holiday.occasion === 'thanksgiving')
                        return prefs.thanksgiving !== false;
                    if (holiday.occasion === 'new_year')
                        return prefs.newYear !== false;
                    return false;
                });
...
function createDateNudge(contact, date, daysUntil) {
    const now = new Date();
    ...
    return {
        ...
        daysUntilEvent: daysUntil,
        suggestedChannel: contact.preferredChannel || 'email',
        suggestedOccasion: occasion,
        expiresAt: new Date(now.getTime() + (daysUntil + 1) * 24 * 60 * 60 * 1000),
        context: {},
    };
}
function getDaysUntilDate(dateStr, now) {
...
function getDaysUntilHoliday(holiday, now) {
    const thisYear = now.getFullYear();
...
function getHolidayDate(holiday, now) {
    const thisYear = now.getFullYear();
    let targetDate = new Date(thisYear, holiday.month - 1, holiday.day);
    if (targetDate < now) {
        targetDate = new Date(thisYear + 1, holiday.month - 1, holiday.day);
 ...
 */
export async function buildNudgeContext(userId) {
 ...
 */
export function formatNudgeAsSuggestion(nudge) {
    switch (nudge.type) {
 ...
 */
export async function getOverdueFrequentContacts(userId) {
    const contacts = await getContacts(userId);
    const now = new Date();
    const overdue = [];
    for (const contact of contacts) {
    ...
    overdue.sort((a, b) => b.daysSinceLastContact / b.avgDaysBetweenContact -
...
//# sourceMappingURL=outreach-nudges.js.map