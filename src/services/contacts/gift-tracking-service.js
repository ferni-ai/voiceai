...
export function clearGiftCache() {
    giftCache.clear();
}
/**
 * Get Firestore collection reference for user's gifts
 */
async function getGiftsCollection(userId) {
 ...
 */
async function loadGiftsFromFirestore(userId) {
    try {
 ...
 */
async function saveGiftToFirestore(userId, gift) {
    try {
        const collection = await getGiftsCollection(userId);
        if (!collection) {
 ...
 */
export async function recordGift(userId, gift) {
    const key = getCacheKey(userId);
    ...
    void onGiftIdeaChange(userId, newGift.id, {
    ...
    try {
        await recordInteraction(userId, {
            ...
            topics: gift.tags,
            sentiment: gift.reaction === 'loved' || gift.reaction === 'liked'
                ? 'positive'
    ...
    log.info({ giftId: newGift.id, contactId: gift.contactId, direction: gift.direction }, 'Gift recorded');
    return newGift;
}
/**
 * Get gift history for a contact
 */
export async function getGiftHistory(userId, contactId) {
    const key = getCacheKey(userId);
    ...
    const favoriteCategories = Object.entries(tagCounts)
        ...
        .slice(0, 5)
        .map(([tag]) => tag);
    const avgSpending = given.length > 0 ? given.reduce((sum, g) => sum + (g.price || 0), 0) / given.length : 0;
 ...
 */
export async function getAllGifts(userId) {
    const key = getCacheKey(userId);
    // Check cache first
 ...
 */
export async function updateGiftReaction(userId, giftId, reaction) {
    const key = getCacheKey(userId);
    const gifts = giftCache.get(key) || [];
 ...
 */
export async function generateGiftSuggestions(userId, contactId, occasion, budget) {
    // Get contact info
    ...
    const pastGiftsContext = history.given.length > 0
 ...
 */
function getDefaultSuggestions(occasion, budget) {
    const defaults = {
        birthday: [
            {
 ...
 */
export async function getUpcomingGiftOccasions(userId, daysAhead = 30) {
    ...
    for (const contact of contacts) {
        ...
        for (const importantDate of contact.importantDates) {
            ...
            if (dateThisYear <= cutoff) {
                const daysUntil = Math.ceil((dateThisYear.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export async function getGiftAnalytics(userId) {
    ...
    // Top recipients
    const recipientCounts = given.reduce((acc, g) => {
    ...
    const popularCategories = Object.entries(tagCounts)
        ...
        .slice(0, 5);
    // Reaction breakdown
    const reactionBreakdown = given.reduce((acc, g) => {
...
//# sourceMappingURL=gift-tracking-service.js.map