 ...
 */
function getConfidenceLevel(attempts) {
    if (attempts >= 20)
        return 'high';
    if (attempts >= 10)
        return 'medium';
 ...
 */
function createInitialProfile(contactId, userId) {
    const timeSlots = {
 ...
 */
export async function getTimingProfile(userId, contactId) {
 ...
 * Save timing profile
 */
async function saveTimingProfile(profile) {
    ...
    try {
        ...
        void onCommunicationPreferenceChange(profile.userId, profile.contactId, {
...
function findBestSlot(timeSlots) {
    let best = 'morning';
    let bestSample = 0;
 ...
 */
function trackRecentOutcome(userId, contactId, sentAt, timeSlot, day) {
 ...
 */
export async function markContactResponded(userId, contactId, responseTime) {
    const key = `${userId}_${contactId}`;
    const outcomes = recentOutcomes.get(key);
    ...
    const recentOutcome = outcomes.find((o) => now.getTime() - o.sentAt.getTime() < OUTCOME_EXPIRY_MS);
    if (!recentOutcome) {
 ...
 */
export async function getTimingRecommendation(userId, contactId, contactName) {
    const profile = await getTimingProfile(userId, contactId);
    ...
    const expectedRate = betaMean(profile.timeSlots[selectedSlot].alpha, profile.timeSlots[selectedSlot].beta);
...
function selectTimeSlotThompson(timeSlots) {
    let best = 'morning';
    let bestSample = -1;
    for (const [slot, params] of Object.entries(timeSlots)) {
        const sample = sampleBeta(params.alpha, params.beta);
...
function selectDayThompson(dayPrefs) {
    let best = 'tuesday';
    let bestSample = -1;
    for (const [day, params] of Object.entries(dayPrefs)) {
        const sample = sampleBeta(params.alpha, params.beta);
...
function getNextOccurrence(day, slot) {
    const now = new Date();
 ...
 */
export async function getBatchTimingRecommendations(userId, contacts) {
 ...
 */
export async function groupByOptimalTime(userId, contacts) {
...
//# sourceMappingURL=optimal-timing.js.map