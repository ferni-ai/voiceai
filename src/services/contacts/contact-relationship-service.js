 ...
 */
export async function getContact(userId, identifier) {
    const contacts = await getContacts(userId);
    return (contacts.find((c) => c.id === identifier ||
        c.email?.toLowerCase() === identifier.toLowerCase() ||
        c.contactId === identifier) || null);
}
/**
...
export async function upsertContact(userId, contact) {
    ...
    const existingIndex = contacts.findIndex((c) => c.contactId === contact.contactId || c.id === contact.id);
    const now = new Date();
    let saved;
    if (existingIndex >= 0) {
        // Update existing
 ...
 */
async function detectStreak(userId, contactId, interactionType) {
    ...
    try {
        ...
        const snapshot = await firestore
            ...
            .get();
        if (snapshot.empty)
            return { isStreak: false, streakCount: 1 };
    ...
    if (contact) {
        ...
        if (interaction.topics) {
            for (const topic of interaction.topics) {
                const existingTopic = contact.topics.find((t) => t.topic.toLowerCase() === topic.toLowerCase());
                if (existingTopic) {
 ...
 */
export async function setFollowUp(userId, contactId, followUp) {
 ...
 */
export async function completeFollowUp(userId, contactId) {
    const contact = await getContact(userId, contactId);
    if (!contact || !contact.pendingFollowUp)
        return;
 ...
 */
export async function getRelationshipInsights(userId) {
    const contacts = await getContacts(userId);
    const insights = [];
    const now = new Date();
    for (const contact of contacts) {
        ...
        const daysSinceContact = Math.floor((now.getTime() - contact.lastInteraction.getTime()) / (1000 * 60 * 60 * 24));
...
export async function searchContacts(userId, query) {
    ...
    let matchingRelationshipType = null;
    ...
    const results = contacts.filter((contact) => {
        ...
        if (contact.phone?.includes(queryLower))
            return true;
        // Search notes (for aliases like "my mom")
        if (contact.notes?.toLowerCase().includes(queryLower))
            return true;
        // Search topics
        if (contact.topics.some((t) => t.topic.toLowerCase().includes(queryLower)))
            return true;
        // Search recent context
        if (contact.recentContext.some((c) => c.toLowerCase().includes(queryLower)))
            return true;
        return false;
    });
            ...
            for (const result of mainResults) {
                ...
                if (mainContact.phones?.[0]?.number || mainContact.emails?.[0]?.address) {
                    const converted = {
                        ...
                        relationship: mainContact.relationship || 'other',
                    ...
                    log.info({ userId, query, contactName: converted.name, phone: converted.phone }, '📇 Found contact in main contacts service (fallback)');
                }
            }
        }
        catch (fallbackErr) {
 ...
 */
export async function getContactContext(userId, contactId) {
    const contact = await getContact(userId, contactId);
    if (!contact)
        return null;
    const now = new Date();
 ...
 */
export async function getInteractionHistory(userId, contactId, options = {}) {
    const firestore = await getFirestore();
    if (!firestore)
        return [];
    try {
 ...
 */
export async function getInteractionStats(userId, contactId) {
    ...
    const monthsSpan = Math.max(1, Math.ceil((Date.now() - firstInteraction.getTime()) / (1000 * 60 * 60 * 24 * 30)));
    ...
    let suggestedNextInteraction = 'text';
    ...
    if (mostCommon?.[0] === 'text' && byType['call'] < byType['text'] / 3) {
        ...
        suggestedNextInteraction = mostCommon?.[0] || 'text';
    }
    return {
 ...
 */
export async function getTopicsToDiscuss(userId, contactId) {
    const contact = await getContact(userId, contactId);
    if (!contact || contact.topics.length === 0)
        return [];
    const now = new Date();
    return contact.topics
        ...
        .slice(0, 5)
        .map((topic) => {
        const daysSince = Math.floor((now.getTime() - topic.lastMentioned.getTime()) / (1000 * 60 * 60 * 24));
        let suggestion = '';
        if (daysSince > 30) {
...
async function persistContact(contact) {
    ...
    try {
        ...
        const cleanContact = Object.fromEntries(Object.entries(contact).filter(([_, v]) => v !== undefined));
        await firestore
            .collection(CONTACTS_COLLECTION)
            .doc(contact.id)
            .set(cleanForFirestore(cleanContact));
        // Index to semantic memory for contact awareness
        void onContactChange(contact.userId, contact.id, {
            name: contact.name,
...
async function persistInteraction(interaction) {
    const firestore = await getFirestore();
    if (!firestore)
        return;
    try {
        ...
        void onContactInteractionChange(interaction.userId, interaction.id, {
...
//# sourceMappingURL=contact-relationship-service.js.map