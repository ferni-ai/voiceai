 ...
 */
export async function addToReadingList(userId, book, options = {}) {
    ...
    try {
        ...
        log.error({ error: String(error), userId, bookId: book.bookId }, 'Failed to add to reading list');
 ...
 */
export async function updateReadingStatus(userId, entryId, update) {
    const collection = getCollection(userId);
    if (!collection) {
        return { success: false, error: 'Database not available' };
    }
    try {
        ...
        return {
            success: true,
            entry: { ...updatedData, id: docRef.id },
        };
    }
    catch (error) {
 ...
 * Mark a book as read.
 */
export async function markBookAsRead(userId, entryId, rating) {
 ...
 */
export async function removeFromReadingList(userId, entryId) {
    const collection = getCollection(userId);
    if (!collection) {
        return { success: false, error: 'Database not available' };
    }
 ...
 */
export async function getReadingList(userId, options = {}) {
    const collection = getCollection(userId);
    if (!collection) {
        return { success: false, error: 'Database not available' };
    }
 ...
 */
export async function getReadingListEntry(userId, bookId) {
    const collection = getCollection(userId);
    if (!collection) {
        return { success: false, error: 'Database not available' };
    }
...
export async function getReadingStats(userId) {
    ...
    try {
        ...
        const pagesRead = completed.reduce((sum, e) => sum + (e.pageCount || 0), 0) +
 ...
 */
function calculateReadingStreak(completed) {
 ...
 */
export function isReadingListStoreAvailable() {
    return getFirestoreDb() !== null;
}
//# sourceMappingURL=reading-list-store.js.map