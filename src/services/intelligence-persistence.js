...
import { getHumorCalibration, removeHumorCalibration, } from '../intelligence/humor-calibration.js';
import { getStoryPreference, removeStoryPreference, } from '../intelligence/story-preference.js';
import { getCommunicationMirroring, removeCommunicationMirroring, } from '../intelligence/communication-mirroring.js';
import { getEmotionalMemory, removeEmotionalMemory, } from '../intelligence/emotional-memory.js';
import { getVoicePaceAdapter, removeVoicePaceAdapter, } from '../intelligence/voice-pace-adapter.js';
import { getResponseQualityTracker, removeResponseQualityTracker, } from '../intelligence/response-quality-tracker.js';
import { getConversationPatternAnalyzer, removeConversationPatternAnalyzer, } from '../intelligence/conversation-pattern-analyzer.js';
import { getCrossSessionThreader, removeCrossSessionThreader, } from '../intelligence/cross-session-threader.js';
// 🌟 Better Than Human capabilities
import { getBetterThanHuman, getExistingBetterThanHumanForUser, } from '../conversation/superhuman/orchestrator.js';
 ...
 */
export function exportIntelligenceState(userId) {
    ...
    try {
        ...
        if (existingOrchestrator) {
            ...
            engineCount++;
            getLogger().debug({ userId, sessionCount: bthState.sessionCount }, 'Exported Better Than Human state');
        }
        else {
 ...
 */
export function importIntelligenceState(userId, state) {
    const startTime = Date.now();
    let engineCount = 0;
    if (!state) {
    ...
    if (state.version > INTELLIGENCE_STATE_VERSION) {
        getLogger().warn({ userId, stateVersion: state.version, currentVersion: INTELLIGENCE_STATE_VERSION }, 'Intelligence state from newer version, some data may not load');
    }
    try {
    ...
    try {
        if (state.betterThanHuman) {
            const bthOrchestrator = getBetterThanHuman(userId, `import-${Date.now()}`, 'ferni', state.betterThanHuman.sessionCount || 0);
            bthOrchestrator.import(state.betterThanHuman);
            engineCount++;
            getLogger().debug({ userId, sessionCount: state.betterThanHuman.sessionCount }, 'Imported Better Than Human state');
        }
    }
    catch (error) {
 ...
 */
export function applyIntelligenceToProfile(profile, userId) {
    const state = exportIntelligenceState(userId);
    // Store in customData for persistence
    if (!profile.customData) {
        profile.customData = {};
    }
    profile.customData.intelligenceState = state;
    ...
    if (state.emotional?.moments) {
        profile.customData.emotionalMoments = state.emotional.moments;
    }
 ...
 */
export function loadIntelligenceFromProfile(userId, profile) {
    const customData = profile.customData;
    if (customData?.intelligenceState) {
        importIntelligenceState(userId, customData.intelligenceState);
        return;
    }
    ...
    if (customData?.emotionalMoments) {
        legacyState.emotional = {
            moments: customData.emotionalMoments,
    ...
    if (profile.openThreads || profile.promisedFollowUps) {
        legacyState.threads = {
            ...
            promisedFollowUps: (profile.promisedFollowUps || []),
        };
    }
 ...
 */
export function cleanupIntelligenceEngines(userId) {
    try {
...
// AUTO-SAVE MANAGER
// ============================================================================
function getAutoSaveIntervalName(userId) {
    return `intelligence-auto-save-${userId}`;
}
const autoSaveRegistry = new Map();
/**
 * Start auto-saving intelligence state for a user
 */
export function startAutoSave(userId, saveCallback, config = {}) {
    ...
    stopAutoSave(userId);
    registerInterval(getAutoSaveIntervalName(userId), () => {
        void (async () => {
            try {
 ...
 */
export function stopAutoSave(userId) {
    ...
    if (entry) {
        ...
        getLogger().warn({ userId, intervalName }, '⚠️ Auto-save interval existed but was not in registry - cleaned up orphan');
    }
    else {
...
//# sourceMappingURL=intelligence-persistence.js.map