...
// ============================================================================
const RITUAL_REMINDER_MESSAGES = {
    ferni: [
...
class PushNotificationsBackendService {
    ...
    async initialize() {
        ...
        this.subscriptionStore = createPersistenceStore({
            collection: 'push_subscriptions',
            syncIntervalMs: 10000, // Sync every 10 seconds
            maxPendingChanges: 50,
        });
        this.scheduledStore = createPersistenceStore({
     ...
     */
    async rehydrateScheduledNotifications() {
        try {
            ...
            if (data?.notifications) {
                ...
                getLogger().info({ count: this.scheduledNotifications.size }, 'Rehydrated scheduled notifications from persistence');
            }
        }
        catch (error) {
    ...
    async registerSubscription(subscription) {
        ...
        if (!exists) {
            ...
            getLogger().info({ userId: subscription.userId, platform: subscription.platform }, 'Push subscription registered and persisted');
        }
    }
    /**
    ...
    async sendNotification(userId, payload) {
        ...
        for (const sub of userSubs) {
            try {
                ...
                getLogger().warn({ error, userId, endpoint: sub.endpoint }, 'Failed to send push notification');
     ...
     */
    async scheduleNotification(userId, payload, scheduledFor) {
     ...
     */
    async sendRitualReminder(userId, personaId, ritualName) {
     ...
     */
    async sendStreakMilestone(userId, streak, ritualName) {
     ...
     */
    async sendPredictionResult(userId, predictionTitle, accuracy) {
     ...
     */
    async sendTeamHuddleInvite(userId, topic, personaIds) {
     ...
     */
    async sendFerniCheckin(userId, message) {
        return this.sendNotification(userId, {
            title: 'Ferni Here',
            body: message,
            type: 'ferni_checkin',
    ...
    async sendNativePush(subscription, payload) {
        // Use the FCM push notification service from outreach/delivery
        try {
            const { sendPushNotification, isPushNotificationsAvailable, registerPushToken } = await import('./outreach/delivery/push-notifications.js');
            ...
            if (!success) {
                getLogger().warn({ userId: subscription.userId, errors: result.map((r) => r.error) }, 'Native push delivery failed');
            }
            else {
                getLogger().info({ userId: subscription.userId, platform: subscription.platform }, '📱 Native push notification sent via FCM');
            }
        }
        catch (error) {
    ...
    getStats() {
        ...
        return {
            subscriptions: Array.from(this.subscriptions.values()).reduce((sum, arr) => sum + arr.length, 0),
 ...
 */
export async function resetPushNotificationsService() {
    if (instance) {
 ...
 */
export async function shutdownPushNotificationsService() {
    if (instance) {
