...
const userLinkedIn = new Map<string, LinkedInUserData>();
 ...
 */
async function persistTokens(userId: string, data: LinkedInUserData): Promise<void> {
  try {
    const store = await getStore();
    const profile = await store.getOrCreateProfile(userId);

    const tokensToSave: PersistedLinkedInTokens = {
    ...
    } as typeof profile & { linkedInTokens: PersistedLinkedInTokens };
 ...
 */
async function loadTokens(userId: string): Promise<LinkedInTokens | null> {
  try {
    const store = await getStore();
    const profile = await store.getOrCreateProfile(userId);

    const persisted = (profile as { linkedInTokens?: PersistedLinkedInTokens }).linkedInTokens;
 ...
 */
export function hasLinkedInConnected(userId: string): boolean {
 ...
 */
export async function initializeLinkedIn(userId: string): Promise<boolean> {
...
export async function disconnectLinkedIn(userId: string): Promise<void> {
  ...
  try {
    ...
    delete (updatedProfile as { linkedInTokens?: unknown }).linkedInTokens;
 ...
 */
async function refreshLinkedInToken(userId: string, tokens: LinkedInTokens): Promise<boolean> {
 ...
 */
export async function syncLinkedInData(userId: string): Promise<void> {
  const userData = userLinkedIn.get(userId);
  if (!userData) return;
  ...
  try {
    const response = await linkedInCircuitBreaker.execute(async () => {
      const res = await fetch(
        'https://api.linkedin.com/v2/positions?projection=(elements*(id,title,company,timePeriod,isCurrent,locationName))',
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
            'X-Restli-Protocol-Version': '2.0.0',
 ...
 */
function calculateUpcomingMilestones(positions: LinkedInPosition[]): CareerMilestone[] {
  const milestones: CareerMilestone[] = [];
  const now = new Date();
  const thirtyDaysFromNow = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);

  for (const position of positions) {
    if (!position.startDate.year) continue;
 ...
 */
export function getLinkedInProfile(userId: string): LinkedInProfile | null {
 ...
 */
export function getUpcomingMilestones(userId: string): CareerMilestone[] {
 ...
 */
export function getCurrentPosition(userId: string): LinkedInPosition | null {
 ...
 */
export function generateLinkedInInsight(userId: string): LinkedInInsight | null {
  const userData = userLinkedIn.get(userId);
  ...
  if (soonMilestone) {
    const daysUntil = Math.ceil(
      (soonMilestone.date.getTime() - Date.now()) / (24 * 60 * 60 * 1000)
    );
    return {
      type: 'milestone',
