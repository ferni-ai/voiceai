...
const recentOperations: IndexingOperation[] = [];
const recentErrors: IndexingError[] = [];
 ...
 */
export function trackIndexingOperation(
  entityType: EntityType,
  userId: string,
  operation: 'create' | 'update' | 'delete',
  durationMs: number,
 ...
 */
export function trackIndexingError(entityType: EntityType, userId: string, error: string): void {
 ...
 */
export async function getSemanticStoreMetrics(): Promise<SemanticStoreMetrics> {
  const vectorStore = getFirestoreVectorStore();

  let totalDocuments = 0;
  const documentsByDomain: Record<string, number> = {};
  const documentsByEntity: Record<string, number> = {};
 ...
 */
export async function getSemanticStoreDiagnostics(): Promise<SemanticStoreDiagnostics> {
  const vectorStore = getFirestoreVectorStore();

  // Check vector store status
  let vectorStoreStatus: 'connected' | 'disconnected' | 'error' = 'disconnected';
  try {
  ...
  const now = Date.now();
  const last5Min = recentOperations.filter(
    (op) => new Date(op.timestamp).getTime() > now - 5 * 60 * 1000
  ).length;
  const lastHour = recentOperations.filter(
    (op) => new Date(op.timestamp).getTime() > now - 60 * 60 * 1000
  ).length;
  ...
  if (recentErrors.length > 5) {
    recommendations.push(
      `High error rate detected (${recentErrors.length} recent errors). Check embedding service and Firestore connectivity.`
    );
  }
  ...
  const avgDuration =
    recentOperations.reduce((sum, op) => sum + op.durationMs, 0) /
    Math.max(recentOperations.length, 1);
  if (avgDuration > 500) {
    recommendations.push(
      `Average indexing duration is ${Math.round(avgDuration)}ms. Consider batching operations.`
    );
  }
 ...
 */
export async function isSemanticStoreHealthy(): Promise<boolean> {
  try {
 ...
 */
export async function getSemanticStoreHealth(): Promise<{
...
export {
  recentOperations as _recentOperations, // For testing
