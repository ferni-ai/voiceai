...
export class WriteAheadLog {
  ...
  private stats = {
    ...
    totalFlushed: 0,
    totalFailed: 0,
    flushLatencies: [] as number[],
    lastFlushTime: 0,
  };

  // Firestore batch writer (injected to avoid circular deps)
  private batchWriter:
    | ((entries: WALEntry[]) => Promise<{ success: boolean; failedIds: string[] }>)
    | null = null;
   ...
   */
  configureBatchWriter(
    writer: (entries: WALEntry[]) => Promise<{ success: boolean; failedIds: string[] }>
  ): void {
  ...
  write(
    ...
    docId: string,
    data?: Record<string, unknown>,
    options?: { userId?: string; priority?: WALEntry['priority'] }
  ): string {
  ...
  async flush(): Promise<{ flushed: number; failed: number }> {
    ...
    try {
          ...
          for (const entry of entries) {
            if (result.failedIds.includes(entry.id)) {
              if (entry.retryCount < this.config.maxRetries) {
                ...
                log.warn(
                  { entryId: entry.id, collection: entry.collection, docId: entry.docId },
      ...
      log.info(
        { flushed, failed, latencyMs: latency, queueRemaining: this.queue.length },
        'WAL flush complete'
      );
    } catch (error) {
   ...
   */
  private async defaultBatchWriter(
    entries: WALEntry[]
  ...
  getStats(): WALStats {
    return {
      ...
      avgFlushLatencyMs:
        this.stats.flushLatencies.length > 0
  ...
  private startPeriodicFlush(): void {
    if (hasInterval(WAL_FLUSH_INTERVAL)) return;
 ...
 */
export function getWriteAheadLog(config?: Partial<WALConfig>): WriteAheadLog {
  if (!walInstance) {
 ...
 */
export async function initializeWriteAheadLog(): Promise<WriteAheadLog> {
 ...
 */
export async function shutdownWriteAheadLog(): Promise<void> {
  if (walInstance) {
 ...
 */
export async function flushWrites(): Promise<{ flushed: number; failed: number }> {
 ...
 * Get WAL statistics
 */
export function getWALStats(): WALStats {
