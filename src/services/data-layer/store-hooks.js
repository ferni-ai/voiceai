 ...
 */
export function getIndexingMetrics() {
    return {
 ...
 */
async function enforceMaxPerUser(vectorStore, userId, entityType) {
    ...
    try {
        ...
        const toDelete = existingDocs
            .sort((a, b) => {
            const aTime = a.document.metadata?.indexedAt || '';
            const bTime = b.document.metadata?.indexedAt || '';
        ...
        for (const docId of toDelete) {
            if (docId) {
                ...
                log.debug({ userId, entityType, docId }, '🧹 Removed old document to enforce maxPerUser limit');
            }
        }
        if (toDelete.length > 0) {
            log.info({ userId, entityType, removed: toDelete.length, limit: maxPerUser }, '📊 maxPerUser limit enforced');
        }
    }
    catch (error) {
 ...
 */
export function onStoreChange(event) {
 ...
 */
async function processChange(key) {
    ...
    try {
        ...
        if (event.changeType === 'delete') {
            try {
                ...
                trackIndexingOperation(event.entityType, event.userId, 'delete', durationMs, true);
        ...
        const { shouldIndex: doIndex, reason } = shouldIndex(event.entityType, event.metadata || {});
        if (!doIndex) {
        ...
        await enforceMaxPerUser(vectorStore, event.userId, event.entityType);
        // Generate embedding
        ...
        const durationMs = Date.now() - startTime;
        indexedCount++;
        trackIndexingOperation(event.entityType, event.userId, event.changeType, durationMs, true);
        log.debug({ key, changeType: event.changeType, durationMs }, '🔄 Store change indexed');
    }
    catch (error) {
        const durationMs = Date.now() - startTime;
        errorCount++;
        trackIndexingError(event.entityType, event.userId, String(error));
        trackIndexingOperation(event.entityType, event.userId, event.changeType, durationMs, false);
 ...
 */
export function onHabitChange(userId, habitId, habit, changeType = 'update') {
    onStoreChange({
 ...
 */
export function onSavingsGoalChange(userId, goalId, goal, changeType = 'update') {
    onStoreChange({
        storeType: 'financial',
        changeType,
        userId,
 ...
 */
export function onMilestoneChange(userId, milestoneId, milestone, changeType = 'update') {
    onStoreChange({
        storeType: 'life-data',
        changeType,
        userId,
 ...
 */
export function onBudgetChange(userId, budgetId, budget, changeType = 'update') {
    onStoreChange({
        storeType: 'financial',
        changeType,
        userId,
 ...
 */
export function onTaskChange(userId, taskId, task, changeType = 'update') {
 ...
 */
export function onSubscriptionChange(userId, subId, sub, changeType = 'update') {
 ...
 */
export function onSpendingTriggerChange(userId, triggerId, trigger, changeType = 'update') {
    onStoreChange({
        storeType: 'financial',
        changeType,
        userId,
 ...
 */
export function onRoutineChange(userId, routineId, routine, changeType = 'update') {
    onStoreChange({
        storeType: 'productivity',
        changeType,
        userId,
 ...
 */
export function onLifeGoalChange(userId, goalId, goal, changeType = 'update') {
    onStoreChange({
        storeType: 'life-data',
        changeType,
        userId,
...
//# sourceMappingURL=store-hooks.js.map