 ...
 */
export async function createFeedbackPrompt(input) {
 ...
 */
export async function recordFeedbackReaction(input) {
    const { feedbackId, userId, reaction } = input;
    if (!feedbackId)
        return { ok: false, reason: 'feedbackId required' };
    if (!userId)
        return { ok: false, reason: 'userId required' };
    try {
        const db = getFirestore();
 ...
 */
export async function markFeedbackSkipped(feedbackId, userId) {
 ...
 */
export async function getSessionFeedback(userId, sessionId) {
    try {
        const db = getFirestore();
        const snapshot = await db
            .collection('profiles')
 ...
 */
export async function getPersonaFeedback(userId, personaId, limit = 100) {
    try {
        const db = getFirestore();
        const snapshot = await db
            .collection('profiles')
 ...
 */
export async function calculateUserFeedbackStats(userId) {
    try {
        ...
        for (const fb of feedback) {
            ...
            if (personaStats) {
                ...
                if (fb.reaction === 'resonated')
                    personaStats.resonated++;
                if (fb.reaction === 'helpful')
                    personaStats.helpful++;
                if (fb.reaction === 'too_much')
                    personaStats.tooMuch++;
                if (fb.reaction === 'off_track')
                    personaStats.offTrack++;
            }
            // By trigger
        ...
        for (const trigger of Object.keys(stats.byTrigger)) {
            const triggerData = stats.byTrigger[trigger];
            const triggerFeedback = feedback.filter((f) => f.trigger === trigger);
            const positiveCount = triggerFeedback.filter((f) => f.reaction === 'resonated' || f.reaction === 'helpful').length;
...
//# sourceMappingURL=conversation-feedback-store.js.map