...
import type { ConversationFeedback, FeedbackInsights, UserFeedbackStats } from './types.js';
...
export async function generateFeedbackInsights(
  ...
  try {
    ...
    if (!stats || stats.totalPrompts < MIN_FEEDBACK_FOR_INSIGHTS) {
      log.debug(
        { userId, totalPrompts: stats?.totalPrompts },
        'Insufficient feedback for insights'
      );
      return null;
    }
...
function analyzeTopics(
  ...
  for (const fb of feedback) {
    ...
    if (!fb.reaction || fb.reaction === 'skipped') continue;
...
function analyzeTimeOfDay(
  ...
  const timeScores: Record<string, { positive: number; total: number }> = {
  ...
  if (bestPeriod && bestRate > 0.5) {
    return bestPeriod as 'morning' | 'afternoon' | 'evening' | 'night';
  }
