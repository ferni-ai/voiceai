...
export type ProcessingMode =
  | 'verbal' // Thinks out loud
  | 'internal' // Needs quiet to process
  | 'collaborative'; // Thinks through dialogue

export type FeedbackPreference =
  | 'direct' // Tell me straight
  | 'gentle' // Soften it
  | 'curious'; // Ask me questions instead
...
const styleProfiles = new Map<string, CoachingStyleProfile>();

function getOrCreateProfile(userId: string): CoachingStyleProfile {
...
const STYLE_INDICATORS: Record<CoachingStyle, { patterns: RegExp[]; keywords: string[] }> = {
  analytical: {
    patterns: [
 ...
 */
export function detectStyleSignals(userId: string, userMessage: string): StyleSignal[] {
    ...
    for (const pattern of indicators.patterns) {
      if (pattern.test(lower)) {
        signals.push({
          ...
          indicatedStyle: style as CoachingStyle,
          source: 'behavioral',
        });
      }
    }
 ...
 */
function updateStyleAssessment(profile: CoachingStyleProfile): void {
  const styleCounts: Record<CoachingStyle, number> = {
    analytical: 0,
    emotional: 0,
    action: 0,
    reflective: 0,
  ...
  if (sorted.length > 0) {
    profile.primaryStyle = sorted[0][0] as CoachingStyle;
    profile.secondaryStyle = sorted[1]?.[0] as CoachingStyle;
 ...
 */
export function setExplicitStylePreference(
  userId: string,
  style: CoachingStyle,
  preferences?: Partial<CoachingStyleProfile['preferences']>
): void {
  const profile = getOrCreateProfile(userId);
...
const STYLE_GUIDANCE: Record<CoachingStyle, StyleGuidance> = {
  analytical: {
...
export function getStyleGuidance(
  userId: string
): StyleGuidance & { style: CoachingStyle; confidence: number } {
  const profile = getOrCreateProfile(userId);
 ...
 */
export function buildStyleContext(userId: string): string | null {
  const profile = styleProfiles.get(userId);
  if (!profile || profile.confidence < 0.4) return null;
...
export function getStyleProfile(userId: string): CoachingStyleProfile {
  return getOrCreateProfile(userId);
}

export function getPreferredStyle(userId: string): CoachingStyle {
...
export function exportStyleProfile(userId: string): CoachingStyleProfile | null {
  return styleProfiles.get(userId) || null;
}

export function importStyleProfile(profile: CoachingStyleProfile): void {
