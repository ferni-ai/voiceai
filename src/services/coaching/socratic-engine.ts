...
export type SocraticQuestionType =
  ...
  | 'origin_exploring' // "Where does that belief come from?"
  | 'alternative_generating'; // "What's another way to see this?"
...
const QUESTION_TEMPLATES: Record<SocraticQuestionType, string[]> = {
  clarifying: [
...
const QUESTION_TRIGGERS: Record<SocraticQuestionType, RegExp[]> = {
  clarifying: [
    /i feel (like|that)/i,
    /it('s| is) just/i,
    /always|never/i,
...
export function selectQuestionType(
  userMessage: string,
  context?: Partial<SocraticContext>
  ...
  for (const [type, patterns] of Object.entries(QUESTION_TRIGGERS)) {
    for (const pattern of patterns) {
      if (pattern.test(lower)) {
        ...
        if (context?.previousQuestions?.slice(-2).includes(type as SocraticQuestionType)) {
          continue;
        }
        return type as SocraticQuestionType;
      }
    }
  }

  // Default progression if no clear trigger
  const defaultProgression: SocraticQuestionType[] = [
    'clarifying',
    'assumption_probing',
    'perspective_taking',
    'implication_exploring',
 ...
 */
export function generateSocraticQuestion(context: SocraticContext): SocraticQuestion {
...
export function generateQuestionSequence(topic: string, depth: number = 3): SocraticQuestion[] {
  ...
  const idealSequence: SocraticQuestionType[] = [
    'clarifying',
    'assumption_probing',
    'perspective_taking',
    'implication_exploring',
...
export function generateSocraticResponse(
  ...
  const validationOptions = validations[tone as keyof typeof validations] || validations.neutral;
 ...
 */
export function buildSocraticContext(
  userMessage: string,
  previousQuestions?: SocraticQuestionType[]
): string {
