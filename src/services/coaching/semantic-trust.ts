...
export type TrustSignalType =
  | 'boundary' // Don't want to talk about something
  | 'permission' // Seeking permission to share
  | 'growth' // Noticing personal change
  | 'sensitive' // Touching on heavy/personal topic
  | 'rapport' // Building on shared history
  | 'deflection' // Changing subject, avoiding
  | 'vulnerability' // Opening up, being vulnerable
  | 'false_fine' // Saying fine when not fine
  | 'none';
 ...
 */
export function detectTrustSignals(message: string): TrustSignal[] {
  const signals: TrustSignal[] = [];

  for (const [signalType, patterns] of Object.entries(TRUST_PATTERNS)) {
    if (signalType === 'none') continue;
    ...
    if (score >= 0.4) {
      ...
      signals.push({
        type: signalType as TrustSignalType,
  ...
  if (signals.length > 0) {
    log.debug(
      { signalCount: signals.length, topSignal: signals[0]?.type },
 ...
 */
export function detectPrimaryTrustSignal(message: string): TrustSignal {
