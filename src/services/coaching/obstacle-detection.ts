...
export type ObstacleType =
  ...
  | 'energy' // Too tired/burned out
  | 'fear' // Afraid of outcome
  | 'perfectionism' // Waiting for perfect conditions
  | 'overwhelm' // Too big/complex
  | 'unclear' // Don't know how
  | 'motivation' // Lost the "why"
  | 'external' // Outside factors blocking
  | 'self_doubt' // Don't believe they can
  | 'competing_priorities' // Other things taking precedence
  | 'emotional' // Emotional block
...
const obstacleProfiles = new Map<string, ObstacleProfile>();

function getOrCreateProfile(userId: string): ObstacleProfile {
...
const OBSTACLE_PATTERNS: Record<ObstacleType, RegExp[]> = {
  time: [
 ...
 */
export function detectObstacle(
  userId: string,
  userMessage: string,
  context?: {
    goalId?: string;
  ...
  for (const [type, patterns] of Object.entries(OBSTACLE_PATTERNS)) {
    for (const pattern of patterns) {
      if (pattern.test(lower)) {
        return createObstacle(userId, type as ObstacleType, userMessage, context);
      }
    }
  }
...
function updatePatterns(profile: ObstacleProfile, type: ObstacleType, context?: string): void {
...
const OBSTACLE_SUPPORT: Record<ObstacleType, ObstacleSupport> = {
  time: {
    type: 'time',
  ...
  perfectionism: {
    type: 'perfectionism',
    acknowledgment:
      "The desire to do things well isn't bad. But waiting for perfect means waiting forever.",
    questions: [
  ...
  self_doubt: {
    type: 'self_doubt',
    acknowledgment:
      "Self-doubt is that voice that says you're not enough. It's loud, but it's not truth.",
    questions: [
 ...
 */
export function getObstacleSupport(type: ObstacleType): ObstacleSupport {
 ...
 */
export function generateObstacleResponse(obstacle: Obstacle): {
  acknowledgment: string;
  question: string;
  ssml: string;
} {
 ...
 */
export function markObstacleAddressed(
  userId: string,
  obstacleId: string,
  supportOffered: string
): void {
 ...
 */
export function markObstacleResolved(
  userId: string,
  obstacleId: string,
  whatHelped?: string
): void {
 ...
 */
export function getActiveObstacles(userId: string): Obstacle[] {
  const profile = obstacleProfiles.get(userId);
 ...
 */
export function getObstaclePatterns(userId: string): ObstaclePattern[] {
  const profile = obstacleProfiles.get(userId);
 ...
 */
export function getMostCommonObstacle(userId: string): ObstacleType | null {
 ...
 */
export function buildObstacleContext(userId: string): string | null {
  const patterns = getObstaclePatterns(userId);
  if (patterns.length === 0) return null;

  const top = patterns.sort((a, b) => b.frequency - a.frequency).slice(0, 3);

  const lines: string[] = ['[🚧 OBSTACLE PATTERNS]'];
...
export function exportObstacleProfile(userId: string): ObstacleProfile | null {
  return obstacleProfiles.get(userId) || null;
}

export function importObstacleProfile(profile: ObstacleProfile): void {
  profile.obstacles.forEach((o) => {
    o.detectedAt = new Date(o.detectedAt);
    if (o.resolvedAt) o.resolvedAt = new Date(o.resolvedAt);
  });
