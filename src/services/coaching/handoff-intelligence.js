...
export function getOrCreateExperience(userId) {
 ...
 */
export function detectHandoffOpportunity(userId, userMessage, currentPersona = 'ferni') {
    ...
    if (candidates.length > 0 && candidates[0].confidence >= 0.5) {
        ...
        const badHistory = experience.handoffHistory.filter((h) => h.to === candidates[0].personaId && !h.successful);
 ...
 * Record a handoff
 */
export function recordHandoff(userId, from, to, topic, successful) {
    const experience = getOrCreateExperience(userId);
 ...
 */
export function getUnmetTeamMembers(userId) {
    const experience = getOrCreateExperience(userId);
    const allPersonas = [
        'maya-santos',
        'alex-chen',
 ...
 */
export function generateTeamIntroduction(personaId, context) {
 ...
 */
export function buildHandoffContext(userId, currentPersona) {
    const experience = getOrCreateExperience(userId);
    const unmet = getUnmetTeamMembers(userId);
    const lines = ['[🤝 TEAM COORDINATION]'];
    // Who they've met
...
// PERSISTENCE
// ============================================================================
export function exportTeamExperience(userId) {
    return userExperiences.get(userId) || null;
}
export function importTeamExperience(experience) {
...
//# sourceMappingURL=handoff-intelligence.js.map