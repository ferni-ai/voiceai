...
export function detectCalendarIntent(message) {
    ...
    log.debug({ type: best.type, confidence: best.score, reason: best.reason }, '📅 Calendar intent detected');
    return {
        type: best.type,
 ...
 */
function extractCalendarInfo(message, _intentType) {
    const info = {};
    // Time references
    const timeMatch = message.match(/\b(today|tomorrow|tonight|this\s*(morning|afternoon|evening|week|weekend)|next\s*(week|monday|tuesday|wednesday|thursday|friday|saturday|sunday)|monday|tuesday|wednesday|thursday|friday|saturday|sunday|at\s*\d{1,2}(:\d{2})?(\s*(am|pm))?)/i);
    if (timeMatch) {
        info.timeReference = timeMatch[0];
    }
    // Event type
    const eventMatch = message.match(/\b(meeting|appointment|call|event|lunch|dinner|breakfast|interview|presentation|review|standup|1:1|one-on-one)/i);
    if (eventMatch) {
    ...
    // Action verbs
    const actionMatch = message.match(/\b(schedule|add|create|cancel|remove|delete|move|push|reschedule|remind|book)/i);
    if (actionMatch) {
