 ...
 */
export function recordSession(userId, context) {
    const profile = getOrCreateProfile(userId);
    const now = new Date();
    // Calculate gap since last session
    const daysSinceLast = Math.floor((now.getTime() - profile.lastSessionDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    log.debug({ userId, daysSinceLast, avgGap: profile.averageSessionGap.toFixed(1) }, 'Session recorded');
}
// ============================================================================
// NUDGE DETERMINATION
// ============================================================================
 ...
 */
export function shouldSendNudge(userId) {
    ...
    if (recentNudges.length > 0) {
        ...
        const daysSinceNudge = Math.floor((now.getTime() - lastNudge.date.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
function determineNudgeType(profile, daysSinceLast) {
    ...
    if (lastContext?.emotionalState === 'sad' ||
 ...
 */
export function generateNudge(userId) {
    const check = shouldSendNudge(userId);
    if (!check.shouldNudge || !check.nudgeType)
        return null;
    const profile = engagementProfiles.get(userId);
 ...
 */
export function getUsersNeedingNudges() {
    const needsNudge = [];
 ...
 */
export function optOutOfReengagement(userId) {
    const profile = getOrCreateProfile(userId);
 ...
 */
export function optInToReengagement(userId) {
    const profile = getOrCreateProfile(userId);
...
// PERSISTENCE
// ============================================================================
export function exportEngagementProfile(userId) {
    return engagementProfiles.get(userId) || null;
}
export function importEngagementProfile(profile) {
