...
export type HolidayType =
  | 'major_secular' // New Year, Thanksgiving (US)
  | 'religious' // Christmas, Hanukkah, Eid, Diwali
  | 'cultural' // Lunar New Year, etc.
  | 'personal'; // Birthdays, anniversaries

export interface Holiday {
  name: string;
  date: Date | null; // null for variable dates
  type: HolidayType;
...
export interface UserSeasonalProfile {
  ...
  avoidHolidays: string[]; // Don't assume they celebrate
}
...
const SEASONAL_THEMES: Record<Season, string[]> = {
  spring: [
    'renewal',
    'new beginnings',
    'growth',
...
const SEASONAL_AWARENESS: Record<Season, string> = {
  spring: 'Spring energy can feel like pressure to "bloom" - not everyone feels ready.',
  summer:
    'Summer has a lot of "should" energy - should be happy, should be social, should be outside.',
...
const userProfiles = new Map<string, UserSeasonalProfile>();

function getOrCreateProfile(userId: string): UserSeasonalProfile {
 ...
 */
export function getDayLength(date: Date = new Date()): 'short' | 'medium' | 'long' {
  const month = date.getMonth();

  // Northern hemisphere approximation
  if (month >= 10 || month <= 1) return 'short';
  if (month >= 4 && month <= 7) return 'long';
  return 'medium';
}
 ...
 */
export function getUpcomingHolidays(userId: string, date: Date = new Date()): Holiday[] {
 ...
 */
export function getSeasonalContext(userId: string): SeasonalContext {
  const now = new Date();
...
function getTimeOfYearDescription(date: Date): string {
  const month = date.getMonth();
  const descriptions = [
 ...
 */
export function recordDifficultTime(userId: string, period: string, reason?: string): void {
  const profile = getOrCreateProfile(userId);
 ...
 */
export function isCurrentlyDifficultTime(userId: string): {
  isDifficult: boolean;
  period?: string;
  reason?: string;
} {
 ...
 */
export function buildSeasonalContext(userId: string): string | null {
  const context = getSeasonalContext(userId);
  const difficultTime = isCurrentlyDifficultTime(userId);

  const lines: string[] = ['[🍂 SEASONAL AWARENESS]'];
 ...
 */
export function setHolidayPreferences(
  userId: string,
  preferences: {
    celebrate?: string[];
    avoid?: string[];
...
export function exportSeasonalProfile(userId: string): UserSeasonalProfile | null {
  return userProfiles.get(userId) || null;
}

export function importSeasonalProfile(profile: UserSeasonalProfile): void {
