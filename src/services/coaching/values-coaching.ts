...
export type ValueDomain =
  ...
  | 'work' // Career, contribution, achievement
  | 'health' // Physical, mental, wellbeing
  | 'growth' // Learning, personal development
  | 'leisure' // Fun, play, creativity
  | 'spirituality' // Meaning, purpose, transcendence
  | 'community' // Service, citizenship, belonging
...
export interface Value {
  ...
  currentAlignment: number; // 1-10, how well living this value
  examples: string[]; // Specific behaviors that express this value
}
...
export interface ValuesProfile {
  ...
  topValues: string[]; // Top 5 value IDs
...
const COMMON_VALUES: Record<ValueDomain, Array<{ name: string; description: string }>> = {
  relationships: [
...
const valuesProfiles = new Map<string, ValuesProfile>();

function getOrCreateProfile(userId: string): ValuesProfile {
...
const EXPLORATION_PROMPTS: string[] = [
 ...
 */
export function getValuesExplorationPrompt(userId: string): {
  prompt: string;
  context: string;
  ssml: string;
} {
 ...
 */
export function recordValuesExplorationResponse(
  userId: string,
  prompt: string,
  response: string
): void {
...
export function identifyValue(
  ...
  const existing = profile.identifiedValues.find(
  ...
  const libraryValue = COMMON_VALUES[domain]?.find(
    (v) => v.name.toLowerCase() === valueName.toLowerCase()
  );
 ...
 */
function updateTopValues(profile: ValuesProfile): void {
 ...
 */
export function suggestValuesFromConversation(
  userMessage: string
): Array<{ value: string; domain: ValueDomain; confidence: number }> {
  const suggestions: Array<{ value: string; domain: ValueDomain; confidence: number }> = [];
  const lower = userMessage.toLowerCase();

  // Simple keyword matching - in production use NLP
  const keywords: Record<string, { value: string; domain: ValueDomain }> = {
 ...
 */
export function recordValueAlignment(
  userId: string,
  valueName: string,
  alignmentScore: number,
  context?: string
  ...
  const profile = getOrCreateProfile(userId);
  const value = profile.identifiedValues.find(
    (v) => v.name.toLowerCase() === valueName.toLowerCase()
  );
 ...
 */
export function getLowAlignmentValues(userId: string): Value[] {
 ...
 */
export function generateValuesCheck(
  userId: string,
  decision: string
): {
  questions: string[];
  ...
  if (topValues.length > 0) {
    questions.push(
      `Looking at this through your values - you've said ${topValues[0]?.name} matters a lot to you. How does this decision relate to that?`
    );
    ...
    questions.push(
      "Before deciding, what matters most to you here? What would the 'right' choice honor?"
    );
  }

  questions.push(
    'If you chose based on what you truly value (not fear or obligation), what would you choose?'
  );
 ...
 */
export function buildValuesContext(userId: string): string | null {
  const profile = valuesProfiles.get(userId);
  if (!profile || profile.identifiedValues.length === 0) return null;

  const lines: string[] = ['[💎 VALUES CONTEXT]'];
  ...
  if (topValues.length > 0) {
    ...
    for (const value of topValues) {
      const alignment =
        value.currentAlignment >= 7 ? '✓ living it' : value.currentAlignment <= 4 ? '⚠️ gap' : '';
...
export function exportValuesProfile(userId: string): ValuesProfile | null {
  return valuesProfiles.get(userId) || null;
}

export function importValuesProfile(profile: ValuesProfile): void {
