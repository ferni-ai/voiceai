 ...
 */
function calculateSemanticScore(message, personaId) {
 ...
 */
export function detectHandoffSemanticly(userId, userMessage, currentPersona = 'ferni') {
    ...
    const badHistory = experience.handoffHistory.filter((h) => h.to === best.personaId && !h.successful);
    ...
    log.debug({ userId, personaId: best.personaId, score: best.score, reason: best.reason }, '🎯 Semantic handoff match');
    // Track detection for analytics
    recordDetection('handoff', userMessage, best.personaId, best.score, [] // Could add matched pattern details here
    );
    return {
        shouldHandoff: true,
        candidate: {
 ...
 */
export function detectHandoffEnhanced(userId, userMessage, currentPersona = 'ferni') {
    // Try semantic first
...
//# sourceMappingURL=semantic-handoff.js.map