 ...
 */
export function shareContext(userId, context) {
    ...
    log.info({ userId, topic: context.topic, from: context.sharedBy }, '🔗 Context shared across personas');
}
/**
 * Record an interaction with a persona
 */
export function recordPersonaInteraction(userId, interaction) {
    const userContext = getOrCreateContext(userId);
 ...
 */
export function updateCurrentSituation(userId, situation, updatedBy) {
    const userContext = getOrCreateContext(userId);
 ...
 */
export function addCrossPersonaItem(userId, item, originPersona, relevantPersonas) {
    const userContext = getOrCreateContext(userId);
    ...
    log.info({ userId, item: item.slice(0, 50), origin: originPersona }, '📌 Cross-persona item added');
}
// ============================================================================
// CONTEXT RETRIEVAL
// ============================================================================
 ...
 */
export function getContextForPersona(userId, personaId) {
    ...
    const relevantContexts = userContext.sharedContexts.filter((sc) => sc.relevantFor.includes(personaId) || sc.relevantFor.length === 0 // Empty means all personas
    );
    // Get recent interactions with other personas
    const recentTeamInteractions = [];
 ...
 */
export function getHandoffSummary(userId, fromPersona, toPersona) {
    const userContext = userContexts.get(userId);
    if (!userContext) {
        return {
    ...
    if (latestFromInteraction) {
        keyPoints.push(`Last talked with ${fromPersona} about: ${latestFromInteraction.topics.join(', ')}`);
        if (latestFromInteraction.openItems) {
            openItems.push(...latestFromInteraction.openItems);
        }
    }
    // Add relevant cross-persona items
    const crossItems = userContext.crossPersonaItems.filter((item) => item.status === 'active' && item.relevantPersonas.includes(toPersona));
 ...
 */
export function buildCrossPersonaContext(userId, currentPersona) {
...
// PERSISTENCE
// ============================================================================
export function exportTeamContext(userId) {
    return userContexts.get(userId) || null;
}
export function importTeamContext(context) {
    // Convert Map from serialized format
    if (!(context.personaInteractions instanceof Map)) {
        const entries = Object.entries(context.personaInteractions);
        context.personaInteractions = new Map(entries);
    }
    // Convert dates
    context.sharedContexts.forEach((sc) => {
        sc.sharedAt = new Date(sc.sharedAt);
        if (sc.expiresAt)
            sc.expiresAt = new Date(sc.expiresAt);
    });
...
//# sourceMappingURL=cross-persona-context.js.map