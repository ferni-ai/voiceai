 ...
 */
export function detectActionOpportunity(userMessage, context) {
 ...
 */
export function generateActionSuggestions(topic, context) {
    const suggestions = [];
 ...
 */
export function createAction(userId, actionData) {
 ...
 * Complete an action
 */
export function completeAction(userId, actionId) {
 ...
 * Skip an action
 */
export function skipAction(userId, actionId, reason) {
    const profile = actionProfiles.get(userId);
    if (!profile)
        return false;
    const action = profile.actions.find((a) => a.id === actionId);
 ...
 */
export function deferAction(userId, actionId, newDate) {
    const profile = actionProfiles.get(userId);
    if (!profile)
        return false;
    const action = profile.actions.find((a) => a.id === actionId);
...
function updateCompletionRate(profile) {
 ...
 */
export function getActionsNeedingFollowUp(userId) {
    const profile = actionProfiles.get(userId);
    if (!profile)
        return [];
    const now = new Date();
    return profile.actions.filter((a) => a.status === 'pending' && a.followUpScheduled && a.followUpScheduled <= now && !a.followUpSent);
}
/**
 * Get action stats
 */
 ...
 */
export function generateActionFollowUp(action) {
    const templates = {
        curious: [
    ...
    const daysSinceCreated = Math.floor((Date.now() - action.createdAt.getTime()) / (1000 * 60 * 60 * 24));
    const tone = daysSinceCreated > 3 ? 'supportive' : 'curious';
 ...
 */
export function getActionToFollowUp(userId) {
 ...
 */
export function buildActionContext(userId) {
...
// PERSISTENCE
// ============================================================================
export function exportActionProfile(userId) {
    return actionProfiles.get(userId) || null;
}
export function importActionProfile(profile) {
    profile.actions.forEach((a) => {
        ...
        if (a.followUpScheduled)
            a.followUpScheduled = new Date(a.followUpScheduled);
    });
    actionProfiles.set(profile.userId, profile);
    log.debug({ userId: profile.userId, actionCount: profile.actions.length }, 'Imported action profile');
}
// ============================================================================
// EXPORTS
// ============================================================================
...
//# sourceMappingURL=action-planning.js.map