...
export interface ProgressSummary {
  ...
  topics: {
    ...
    deepDiveTopics: string[]; // Topics discussed multiple times
  };
...
export interface ProgressProfile {
  ...
  streaks: {
    ...
    currentGoalStreak: number; // Days with goal progress
    weeklyConsistency: number[]; // Last 12 weeks
  };
  ...
  trends: {
    ...
    progressRate: number; // Goals completed per month
  };
}
...
const progressProfiles = new Map<string, ProgressProfile>();

function getOrCreateProfile(userId: string): ProgressProfile {
 ...
 */
export function recordProgressSession(
  userId: string,
  sessionData: {
 ...
 */
export function recordHighlight(userId: string, highlight: Omit<GrowthHighlight, 'date'>): void {
  const profile = getOrCreateProfile(userId);
...
export function generateProgressSummary(
  ...
  const summary: ProgressSummary = {
    ...
    growth: {
      ...
      goalsCompleted: 0,
      milestonesReached: 0,
      breakthroughMoments: profile.highlights.filter(
        (h) => h.type === 'breakthrough' && h.date >= periodStart
      ).length,
      habitsFormed: 0,
    },
    emotional: {
 ...
 */
export function generateProgressReflection(userId: string): {
  title: string;
  reflection: string;
  highlights: GrowthHighlight[];
  ssml: string;
  ...
  const profile = progressProfiles.get(userId);
  if (!profile || profile.highlights.length === 0) return null;
...
export function getStreakInfo(userId: string): {
  ...
  let message: string;
  if (
    profile.streaks.currentSessionStreak === profile.streaks.longestSessionStreak &&
 ...
 */
export function buildProgressContext(userId: string): string | null {
  const profile = progressProfiles.get(userId);
  if (!profile) return null;
...
export function exportProgressProfile(userId: string): ProgressProfile | null {
  return progressProfiles.get(userId) || null;
}

export function importProgressProfile(profile: ProgressProfile): void {
