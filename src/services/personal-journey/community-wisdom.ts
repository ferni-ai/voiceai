 ...
 */
const JOURNEY_WISDOM: Record<
  string,
  {
    triggers: string[]; // Keywords that indicate this journey
    nuggets: string[];
    challenges: string[];
    whatHelps: string[];
  }
...
const UNIVERSAL_PATTERNS: Array<{
...
}> = [
  {
    ...
    message:
      "Every big decision feels uncertain. <break time='200ms'/> Certainty is rare, and waiting for it means waiting forever.",
  },
  {
    pattern: 'comparing yourself to others',
    prevalence: 'universal',
    message:
      "Everyone compares. <break time='200ms'/> And everyone else's journey looks cleaner from the outside.",
  },
  {
  ...
  },
  {
    ...
    prevalence: 'very_common',
    message:
      "Most people want to help and don't know how. <break time='200ms'/> Asking is a gift to them too.",
  },
  {
  ...
  },
  {
    ...
    prevalence: 'very_common',
    message:
      "Progress is invisible from the inside. <break time='200ms'/> You've come further than you think.",
  },
  {
    pattern: 'struggling with change',
    prevalence: 'universal',
    message:
      "Humans aren't wired to love change. <break time='200ms'/> Resistance is natural, not weak.",
  },
];
 ...
 */
export function findRelevantWisdom(text: string, recentTopics?: string[]): JourneyMoment | null {
 ...
 */
export function getUniversalInsight(text: string): JourneyMoment | null {
  const lower = text.toLowerCase();
 ...
 */
export function getWhatHelps(journeyType: string): string[] {
 ...
 */
export function getCommonChallenges(journeyType: string): string[] {
  const wisdom = JOURNEY_WISDOM[journeyType];
 ...
 */
export function detectJourneyType(text: string): string | null {
  const lower = text.toLowerCase();
 ...
 */
export function getComfortMessage(journeyType: string): string | null {
  const wisdom = JOURNEY_WISDOM[journeyType];
 ...
 */
export function getAvailableJourneyTypes(): string[] {
