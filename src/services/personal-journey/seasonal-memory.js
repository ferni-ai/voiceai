 ...
 */
function isSameTimeOfYear(date1, date2) {
 ...
 */
function createEmptySeasonalMemory(userId) {
    return {
        userId,
        updatedAt: new Date(),
        seasonalSnapshots: [],
 ...
 */
export function getSeasonalMemory(userId) {
 ...
 */
export function initializeSeasonalMemory(userId, persistedData) {
    if (persistedData) {
        const memory = {
 ...
 */
export function captureSeasonalSnapshot(userId, data) {
 ...
 */
export function addTimeAnchoredMemory(userId, data) {
    const memory = getSeasonalMemory(userId);
    const now = new Date();
    const anchor = {
        id: generateId(),
 ...
 */
export function detectAnnualPatterns(userId) {
    const memory = getSeasonalMemory(userId);
    const patterns = [];
    ...
    for (const [season, snapshots] of bySeasonMap.entries()) {
        ...
        for (const [state, count] of stateFrequency.entries()) {
            if (count >= 2 && count / snapshots.length >= 0.5) {
                ...
                const existingPattern = memory.annualPatterns.find((p) => p.timeOfYear === season && p.pattern.includes(state));
                if (!existingPattern) {
                    const pattern = {
                        id: generateId(),
                        timeOfYear: season,
        ...
        for (const [theme, count] of themeFrequency.entries()) {
            if (count >= 2 && count / snapshots.length >= 0.5) {
                const existingPattern = memory.annualPatterns.find((p) => p.timeOfYear === season && p.pattern.includes(theme));
                if (!existingPattern) {
                    const pattern = {
                        id: generateId(),
                        timeOfYear: season,
 ...
 */
export function getRelevantTimeMemories(userId) {
    const memory = getSeasonalMemory(userId);
    const moments = [];
    const now = new Date();
    ...
    const lastYearSnapshots = memory.seasonalSnapshots.filter((s) => s.season === currentSeason && s.year === currentYear - 1);
    for (const snapshot of lastYearSnapshots) {
        ...
        if (snapshot.struggles && snapshot.struggles.length > 0) {
            ...
            const template = SEASONAL_MESSAGES.seasonalStruggle[Math.floor(Math.random() * SEASONAL_MESSAGES.seasonalStruggle.length)];
        ...
        if (snapshot.wins && snapshot.wins.length > 0) {
            ...
            const template = SEASONAL_MESSAGES.seasonalWin[Math.floor(Math.random() * SEASONAL_MESSAGES.seasonalWin.length)];
        ...
        if (snapshot.activeThemes.length > 0) {
            ...
            const template = SEASONAL_MESSAGES.lastYearSameTime[Math.floor(Math.random() * SEASONAL_MESSAGES.lastYearSameTime.length)];
    ...
    const relevantAnchors = memory.timeAnchors.filter((a) => a.canReference &&
    ...
    const relevantPatterns = memory.annualPatterns.filter((p) => p.timeOfYear === currentSeason && p.confidence >= 0.5);
    for (const pattern of relevantPatterns) {
        const template = SEASONAL_MESSAGES.seasonalPattern[Math.floor(Math.random() * SEASONAL_MESSAGES.seasonalPattern.length)];
 ...
 */
export function markMemoryReferenced(userId, memoryId) {
    const memory = getSeasonalMemory(userId);
 ...
 */
export function getSeasonalGreetingContext(userId) {
    const memory = getSeasonalMemory(userId);
    const currentSeason = getCurrentSeason();
    ...
    if (Math.random() < 0.1) {
        const lastYear = memory.seasonalSnapshots.find((s) => s.season === currentSeason && s.year === currentYear - 1);
    ...
    if (Math.random() < 0.05) {
        const pattern = memory.annualPatterns.find((p) => p.timeOfYear === currentSeason && p.confidence >= 0.6);
        if (pattern) {
            return {
                hasSeasonalInsight: true,
 ...
 */
export function getSeasonalMemoryForPersistence(userId) {
    return seasonalCache.get(userId) || null;
}
/**
 * Clear cache
 */
export function clearSeasonalCache(userId) {
 ...
 */
export function shouldCaptureSnapshot(userId) {
    const memory = getSeasonalMemory(userId);
    const now = new Date();
    const currentSeason = getCurrentSeason();
    const currentYear = now.getFullYear();
    // Check if we already have this season's snapshot
    const hasCurrentSnapshot = memory.seasonalSnapshots.some((s) => s.season === currentSeason && s.year === currentYear);
    ...
    const isEndOfSeason = (month === 3 && day >= 15) ||
        (month === 6 && day >= 15) ||
        (month === 9 && day >= 15) ||
        (month === 12 && day >= 15);
    return isEndOfSeason;
}
//# sourceMappingURL=seasonal-memory.js.map