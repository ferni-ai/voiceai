 ...
 */
function detectTransitionFromText(text) {
    const lower = text.toLowerCase();
 ...
 */
function calculateThemeConfidence(theme, recentTopics, recentEmotions) {
    let matches = 0;
    const keywords = CHAPTER_THEMES[theme] || [];
 ...
 */
function createEmptyChapters(userId) {
    return {
        userId,
        updatedAt: new Date(),
        pastChapters: [],
 ...
 */
export function getChapters(userId) {
 ...
 */
export function initializeChapters(userId, persistedData) {
    if (persistedData) {
        const chapters = {
 ...
 */
export function updateChapterDetection(userId, data) {
    ...
    if (chapters.transitionSignals.detectedAt) {
        const daysSinceDetection = (now.getTime() - chapters.transitionSignals.detectedAt.getTime()) / (24 * 60 * 60 * 1000);
 ...
 */
export function recordChapterChallenge(userId, challenge) {
    const chapters = getChapters(userId);
    if (chapters.currentChapter) {
 ...
 */
export function recordChapterGrowth(userId, growth) {
    const chapters = getChapters(userId);
    if (chapters.currentChapter) {
 ...
 */
export function getChapterMoments(userId) {
    const chapters = getChapters(userId);
    const moments = [];
    ...
    if (chapters.transitionSignals.isInTransition && chapters.transitionSignals.confidence > 0.5) {
        ...
        if (type === 'beginning') {
            const template = CHAPTER_MESSAGES.transitionBeginning[Math.floor(Math.random() * CHAPTER_MESSAGES.transitionBeginning.length)];
            moments.push({
            ...
            const template = CHAPTER_MESSAGES.transitionEnding[Math.floor(Math.random() * CHAPTER_MESSAGES.transitionEnding.length)];
            moments.push({
    ...
    if (chapters.currentChapter) {
        ...
        const daysInChapter = Math.floor((new Date().getTime() - chapter.startedApprox.getTime()) / (24 * 60 * 60 * 1000));
        // Only surface after being in chapter for at least 2 weeks
        if (daysInChapter >= 14) {
            const template = CHAPTER_MESSAGES.themeRecognition[Math.floor(Math.random() * CHAPTER_MESSAGES.themeRecognition.length)];
        ...
        if (chapter.growth.length >= 2 && daysInChapter >= 30) {
            const template = CHAPTER_MESSAGES.growthReflection[Math.floor(Math.random() * CHAPTER_MESSAGES.growthReflection.length)];
            moments.push({
 ...
 */
export function getChapterGreetingContext(userId) {
    const chapters = getChapters(userId);
    ...
    if (chapters.transitionSignals.isInTransition && Math.random() < 0.2) {
        if (chapters.transitionSignals.transitionType === 'beginning') {
            ...
            return {
                hasChapterInsight: true,
                insight: "Something's wrapping up for you, isn't it? <break time='200ms'/> I'm here for the transition.",
                insightType: 'transition',
            };
        }
    }
    ...
    if (chapters.currentChapter &&
        ...
        return {
            hasChapterInsight: true,
            insight: "You're different than when this chapter started. <break time='200ms'/> In a good way.",
            insightType: 'growth',
        };
    }
 ...
 */
export function getCurrentChapterSummary(userId) {
    const chapters = getChapters(userId);
    if (!chapters.currentChapter) {
    ...
    return {
        ...
        daysInChapter: Math.floor((new Date().getTime() - chapters.currentChapter.startedApprox.getTime()) /
            (24 * 60 * 60 * 1000)),
 ...
 */
export function getChaptersForPersistence(userId) {
    return chapterCache.get(userId) || null;
}
/**
 * Clear cache
 */
export function clearChapterCache(userId) {
    chapterCache.delete(userId);
    log.debug('Cleared chapter cache', { userId });
}
//# sourceMappingURL=chapter-detector.js.map