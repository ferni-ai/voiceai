...
import type { LifeChapters, PersonalJourneyData, SeasonalMemory, UserRhythm } from './types.js';
...
} from './journey-orchestrator.js';
import {
  clearRhythmCache,
  getRhythmForPersistence,
  initializeRhythm,
  recordSession,
...
import {
  ...
  clearSeasonalCache,
  getSeasonalMemoryForPersistence,
 ...
 */
export async function initPersonalJourney(
  userId: string,
 ...
 */
export function recordJourneySession(userId: string): void {
  try {
    recordSession(userId);
 ...
 */
export function cleanupPersonalJourney(userId: string): void {
  try {
 ...
 */
export function getPersonalJourneyForPersistence(userId: string): Partial<PersonalJourneyData> {
  try {
 ...
 */
function extractJourneyDataFromProfile(profile?: UserProfile | null): Partial<PersonalJourneyData> {
  if (!profile) {
    return {};
  }
  ...
  const result: Partial<PersonalJourneyData> = {};

  if (journeyData.rhythm) {
    result.rhythm = reconstructRhythmDates(journeyData.rhythm as UserRhythm);
  }

  if (journeyData.seasonal) {
    result.seasonal = reconstructSeasonalDates(journeyData.seasonal as SeasonalMemory);
  }

  if (journeyData.chapters) {
    result.chapters = reconstructChapterDates(journeyData.chapters as LifeChapters);
  }
 ...
 */
function bootstrapFromExistingProfile(profile: UserProfile): Partial<PersonalJourneyData> {
  const result: Partial<PersonalJourneyData> = {};
 ...
 */
function reconstructRhythmDates(data: UserRhythm): UserRhythm {
  return {
    ...data,
 ...
 */
function reconstructSeasonalDates(data: SeasonalMemory): SeasonalMemory {
  return {
    ...data,
    updatedAt: new Date(data.updatedAt),
 ...
 */
function reconstructChapterDates(data: LifeChapters): LifeChapters {
  return {
    ...data,
    updatedAt: new Date(data.updatedAt),
 ...
 */
export async function updateJourneyFromConversation(
  userId: string,
  data: {
    topics: string[];
    emotions: string[];
  ...
  try {
    ...
    const { updateChapterDetection, recordChapterChallenge, recordChapterGrowth } =
      await import('./chapter-detector.js');
 ...
 */
export async function captureSeasonalSnapshotIfNeeded(
  userId: string,
  data: {
