...
import { getChapterGreetingContext, getChapterMoments, getCurrentChapterSummary, } from './chapter-detector.js';
import { acknowledgeMilestone, getRhythm, getRhythmGreetingContext, getRhythmStats, getUnacknowledgedMilestones, } from './rhythm-awareness.js';
import { getRelevantTimeMemories, getSeasonalGreetingContext, markMemoryReferenced, } from './seasonal-memory.js';
 ...
 */
const RELATIONSHIP_STAGES = ['new', 'building', 'established', 'deep'];
/**
...
const deliveryHistoryCache = new Map();
// ============================================================================
// HELPER FUNCTIONS
// ============================================================================
/**
 ...
 */
function getDeliveryHistory(userId) {
 ...
 */
export function initializeDeliveryHistory(userId, persistedDeliveries) {
 ...
 */
function determineRelationshipStage(userId) {
 ...
 */
function isInCooldown(userId, momentType) {
 ...
 */
function hasRequiredRelationshipStage(currentStage, requiredStage) {
    if (!requiredStage)
        return true;
 ...
 */
export function gatherAllMoments(userId) {
    const moments = [];
    // 1. Rhythm milestones
    try {
 ...
 */
export function filterMoments(userId, moments) {
    ...
    return moments.filter((moment) => {
        ...
        if (!hasRequiredRelationshipStage(relationshipStage, moment.requiresRelationshipStage)) {
        ...
        const recentSimilar = history.deliveries.find((d) => d.contentHash === contentHash &&
 ...
 */
export function prioritizeMoments(moments, context) {
    const scoredMoments = moments.map((moment) => {
        ...
        if (context.userText) {
            ...
            const momentTopics = moment.context?.keyTopics || [];
        ...
        if (moment.type === 'rhythm_milestone') {
            const milestoneType = moment.context?.milestoneType;
 ...
 */
export function selectMomentForTurn(userId, context) {
 ...
 */
export function recordSuppression(userId, reason) {
 ...
 */
export function recordDelivery(userId, moment, reaction) {
    const history = getDeliveryHistory(userId);
    const now = new Date();
    const record = {
        momentId: moment.id,
 ...
 */
export function getJourneyGreetingContext(userId) {
    const candidates = [];
    ...
    if (rhythmContext.hasRhythmInsight && rhythmContext.insight) {
        candidates.push({
            ...
            source: 'rhythm',
            priority: rhythmContext.insightType === 'comeback'
                ? 8
 ...
 */
function getMomentTypeFromSource(source) {
    switch (source) {
        case 'rhythm':
 ...
 */
export function getJourneySnapshot(userId) {
    ...
    return {
        ...
        stats: {
            ...
            daysKnown: Math.floor((Date.now() - rhythm.sessions.firstSession.getTime()) / (24 * 60 * 60 * 1000)),
 ...
 */
export function getDeliveryHistoryForPersistence(userId) {
    const history = getDeliveryHistory(userId);
 ...
 */
export function clearAllJourneyCaches(userId) {
