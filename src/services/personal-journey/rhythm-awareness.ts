 ...
 */
const CONVERSATION_MILESTONES: Record<number, { type: RhythmMilestoneType; messages: string[] }> = {
  1: {
 ...
 */
const STREAK_MILESTONES: Record<number, { type: RhythmMilestoneType; messages: string[] }> = {
  3: {
    type: 'streak_3',
    messages: [
 ...
 */
const TIME_MILESTONES: Record<number, { type: RhythmMilestoneType; messages: string[] }> = {
  7: {
    type: 'one_week',
    messages: [
...
const rhythmCache = new Map<string, UserRhythm>();
 ...
 */
function createEmptyRhythm(userId: string): UserRhythm {
  const now = new Date();
  return {
    userId,
    updatedAt: now,
 ...
 */
export function getRhythm(userId: string): UserRhythm {
 ...
 */
export function initializeRhythm(userId: string, persistedData?: Partial<UserRhythm>): void {
  if (persistedData) {
    const rhythm = {
 ...
 */
export function recordSession(userId: string, timestamp: Date = new Date()): RhythmMilestone[] {
  const rhythm = getRhythm(userId);
  const previousSession = rhythm.sessions.lastSession;
  const achievedMilestones: RhythmMilestone[] = [];

  // Calculate gap since last session
  const gapDays = Math.floor(
    (timestamp.getTime() - previousSession.getTime()) / (24 * 60 * 60 * 1000)
  );
 ...
 */
function updateTimePreferences(rhythm: UserRhythm, timestamp: Date): void {
  ...
  if (maxCount === morningCount) rhythm.timePreferences.mostActiveTimeOfDay = 'morning';
  else if (maxCount === afternoonCount) rhythm.timePreferences.mostActiveTimeOfDay = 'afternoon';
  else if (maxCount === eveningCount) rhythm.timePreferences.mostActiveTimeOfDay = 'evening';
  else rhythm.timePreferences.mostActiveTimeOfDay = 'night';
 ...
 * Update consistency metrics
 */
function updateConsistency(rhythm: UserRhythm, latestGapDays: number): void {
  // Update average gap
  const totalSessions = rhythm.sessions.totalCount;
  if (totalSessions > 1) {
    const daysSinceStart = Math.floor(
      (rhythm.sessions.lastSession.getTime() - rhythm.sessions.firstSession.getTime()) /
        (24 * 60 * 60 * 1000)
    );
 ...
 */
function checkConversationMilestones(rhythm: UserRhythm): RhythmMilestone[] {
  const milestones: RhythmMilestone[] = [];
 ...
 */
function checkStreakMilestones(rhythm: UserRhythm): RhythmMilestone[] {
  const milestones: RhythmMilestone[] = [];
 ...
 */
function checkTimeMilestones(rhythm: UserRhythm): RhythmMilestone[] {
  const milestones: RhythmMilestone[] = [];
  const daysSinceFirst = Math.floor(
    (new Date().getTime() - rhythm.sessions.firstSession.getTime()) / (24 * 60 * 60 * 1000)
  );
 ...
 */
export function getMilestoneMessage(type: RhythmMilestoneType): string | null {
 ...
 */
export function getUnacknowledgedMilestones(userId: string): JourneyMoment[] {
  const rhythm = getRhythm(userId);
  ...
  for (const milestone of rhythm.rhythmMilestones) {
    if (!milestone.acknowledged) {
      ...
      if (message) {
        ...
        let priority = 5;
        if (milestone.type.includes('100') || milestone.type === 'one_year') priority = 9;
        else if (milestone.type.includes('50') || milestone.type === 'six_months') priority = 8;
        else if (milestone.type.includes('30') || milestone.type === 'three_months') priority = 7;
        else if (milestone.type.includes('10') || milestone.type === 'one_month') priority = 6;
        ...
        moments.push({
          ...
          context: {
            ...
            daysKnown: Math.floor(
              (new Date().getTime() - rhythm.sessions.firstSession.getTime()) /
                (24 * 60 * 60 * 1000)
            ),
          },
          source: 'rhythm-awareness',
          requiresRelationshipStage:
            milestone.type.includes('100') || milestone.type === 'one_year'
              ? 'established'
              : 'building',
        });
      }
 ...
 */
export function acknowledgeMilestone(userId: string, milestoneType: RhythmMilestoneType): void {
  const rhythm = getRhythm(userId);
 ...
 */
export function getRhythmGreetingContext(userId: string): {
 ...
 */
export function getRhythmForPersistence(userId: string): UserRhythm | null {
 ...
 */
export function clearRhythmCache(userId: string): void {
 ...
 */
export function getRhythmStats(userId: string): {
