...
const LIFE_TABLE_MALE: Record<number, number> = {
  0: 76.1,
  5: 71.5,
  10: 66.5,
  15: 61.6,
...
const LIFE_TABLE_FEMALE: Record<number, number> = {
  0: 81.2,
  5: 76.5,
  10: 71.5,
  15: 66.6,
 ...
 */
export function calculateLifeExpectancy(input: LifeExpectancyInput): LifeExpectancyResult {
  const now = new Date();
  const currentAge = Math.floor(
    (now.getTime() - input.birthDate.getTime()) / (1000 * 60 * 60 * 24 * 365.25)
  );
  ...
  for (const age of ages) {
    if (age <= currentAge) closestAge = age;
    else break;
  }
  ...
  if (input.healthFactors) {
    ...
    if (bmi) {
      if (bmi < 18.5) expectedYearsRemaining += HEALTH_ADJUSTMENTS.bmiUnderweight;
      else if (bmi < 25) expectedYearsRemaining += HEALTH_ADJUSTMENTS.bmiNormal;
      else if (bmi < 30) expectedYearsRemaining += HEALTH_ADJUSTMENTS.bmiOverweight;
      else expectedYearsRemaining += HEALTH_ADJUSTMENTS.bmiObese;
    }
  ...
  const survivalProbabilities: { age: number; probability: number }[] = [];
...
function generateWisdomContext(currentAge: number, yearsRemaining: number): string {
  if (currentAge < 30) {
 ...
 */
export function generateMortalityPerspective(
  situation: string,
  ...
  if (additionalContext?.parentAge && additionalContext?.visitFrequency) {
    ...
    const visitsPerYear =
      additionalContext.visitFrequency === 'weekly'
        ? 52
    ...
    return {
      ...
      wisdom:
        'Time with loved ones is not infinite. The number is knowable. The question is: how do you want to spend those visits?',
  ...
  if (situation.toLowerCase().includes('health') || situation.toLowerCase().includes('exercise')) {
    return {
      ...
      wisdom:
        'Your body is the only home you will live in your entire life. How are you maintaining it?',
...
function calculateParentExpectancy(parentAge: number): number {
  ...
  let closestAge = ages[0];
  for (const age of ages) {
    if (age <= parentAge) closestAge = age;
    else break;
  }
...
export function calculateRemainingInstances(
  ...
  eventType:
    ...
    | 'sunrise'
    | 'conversation'
): { count: number; wisdom: string } {
  const { expectedYearsRemaining } = lifeExpectancy;

  const calculations: Record<string, { count: number; wisdom: string }> = {
    christmas: {
    ...
    fullMoon: {
      ...
      wisdom:
        'The moon has risen for 4.5 billion years. You will see it only a few hundred more times.',
    },
    tuesday: {
    ...
    sunrise: {
      ...
      wisdom:
        'You have seen perhaps 10,000 sunrises. You may see 10,000 more. Have you really SEEN any of them?',
    },
    conversation: {
 ...
 */
export function generateSuperhumanMortalityMoment(
  lifeExpectancy: LifeExpectancyResult,
  currentTopic?: string
): string | null {
