...
const sessionStates = new Map<string, SessionEvalState>();

function getSessionState(sessionId: string): SessionEvalState {
 ...
 */
export function recordUserMessage(sessionId: string, userMessage: string): void {
  try {
    const state = getSessionState(sessionId);
    state.lastUserMessage = userMessage;
  } catch (error) {
 ...
 */
export function getLastEvaluation(sessionId: string): ResponseEvaluation | null {
 ...
 */
export function getSessionEvalStats(sessionId: string): {
  turnCount: number;
  evaluationsThisSession: number;
