...
export interface PersonaVoiceFingerprint {
  ...
  sentenceProfile: {
    ...
    questionFrequency: 'low' | 'medium' | 'high'; // How often they ask questions
    storyFrequency: 'low' | 'medium' | 'high'; // How often they tell stories
  };
  ...
  emotionalTone: {
    ...
    analyticalVsEmotional: number; // -1 analytical to +1 emotional
    energy: number; // 0-1
  };
...
export interface ResponseEvaluation {
  ...
  voiceConsistency: {
    ...
    voiceDriftScore: number; // 0 = no drift, 1 = complete drift
  };
...
export interface TestScenarioResult {
  ...
  /** Detailed scoring */
  scores: {
    includeScore: number; // % of shouldInclude items present
    avoidScore: number; // % of shouldAvoid items absent
    overallScore: number;
  };
