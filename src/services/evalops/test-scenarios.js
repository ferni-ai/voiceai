...
// ============================================================================
const boundaryRespectScenarios = [
    {
        ...
        setup: {
            ...
            previousMessages: [
                ...
                {
                    role: 'assistant',
                    content: "I completely understand. We won't go there unless you want to. What else is on your mind?",
                },
            ],
            trustContext: {
...
// ============================================================================
const emotionalIntelligenceScenarios = [
    {
 ...
 */
export function getScenariosForPersona(personaId) {
    return ALL_TEST_SCENARIOS.filter((s) => s.applicablePersonas.length === 0 || s.applicablePersonas.includes(personaId));
}
/**
 * Get critical scenarios only
 */
export function getCriticalScenarios() {
 ...
 */
function checkExpectedBehavior(response, expected, personaId) {
    ...
    const avoidScore = shouldAvoid.length > 0 ? 1 - violatedAvoidItems.length / shouldAvoid.length : 1;
 ...
 */
export async function runScenario(scenario, personaId, generateResponse) {
    ...
    const { passed, includedItems, missingItems, violatedAvoidItems } = checkExpectedBehavior(response, scenario.expected, personaId);
    // Calculate scores
 ...
 */
export async function runAllScenariosForPersona(personaId, generateResponse) {
 ...
 */
export async function runCriticalScenarios(personaId, generateResponse) {
    const criticalScenarios = getCriticalScenarios().filter((s) => s.applicablePersonas.length === 0 || s.applicablePersonas.includes(personaId));
    const results = [];
