...
import { getPersonaFingerprint, analyzeSignaturePhraseUsage, detectAntiPatterns, calculateVoiceDrift, } from './persona-fingerprints.js';
const log = getLogger();
// ============================================================================
// EVALUATION PROMPT TEMPLATE
// ============================================================================
function buildEvaluationPrompt(userMessage, aiResponse, context) {
 ...
 */
async function callEvaluatorLLM(prompt, config) {
 ...
 */
function parseEvaluationResponse(response, context, aiResponse, userMessage, startTime) {
    try {
        ...
        return {
            ...
            metadata: {
                ...
                contextProvided: Object.keys(context).filter((k) => context[k] !== undefined),
            },
        };
    }
    catch (error) {
 ...
 */
function createHeuristicEvaluation(context, aiResponse, userMessage, startTime) {
    const { fingerprint } = context;
    ...
    const overallScore = Math.round(personaVoice * 0.3 + authenticity * 0.2 + helpfulness * 0.25 + 70 * 0.25 // Default for dimensions we can't measure heuristically
    );
    return {
        id: `eval_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        timestamp: new Date(),
        ...
        feedback: {
            strengths: signaturesUsed.length > 0 ? [`Used signature phrases: ${signaturesUsed.join(', ')}`] : [],
            improvements: antiPatterns.length > 0 ? [`Contains anti-patterns: ${antiPatterns.join(', ')}`] : [],
 ...
 */
function getMockEvaluationResponse() {
 ...
 */
export async function evaluateResponse(userMessage, aiResponse, context, config = {}) {
    const startTime = Date.now();
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    log.debug({ personaId: context.personaId, turnNumber: context.turnNumber }, 'Starting response evaluation');
    ...
    // Parse response
    const evaluation = parseEvaluationResponse(llmResponse, context, aiResponse, userMessage, startTime);
    log.info({
        personaId: context.personaId,
 ...
 */
export function evaluateVoiceConsistency(aiResponse, personaId) {
 ...
 */
export function shouldSampleConversation(turnNumber, config = DEFAULT_SAMPLING_CONFIG, metadata) {
    // Always evaluate if special conditions met
    if (metadata?.userReportedIssue && config.alwaysEvaluateIf.userReportedIssue)
        return true;
    if (metadata?.isLongConversation && config.alwaysEvaluateIf.longConversation)
        return true;
    if ((metadata?.emotionalIntensity || 0) > 0.7 && config.alwaysEvaluateIf.emotionalIntensity)
        return true;
    if (metadata?.isNewUser && config.alwaysEvaluateIf.newUser)
        return true;
 ...
 */
export async function evaluateBatch(items, config) {
    const results = [];
    // Process sequentially to avoid rate limits
    // Could be parallelized with appropriate throttling
    for (const item of items) {
        try {
            const evaluation = await evaluateResponse(item.userMessage, item.aiResponse, item.context, config);
