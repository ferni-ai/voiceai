...
${fingerprint.signaturePhrases.slice(0, 15).map(p => `- "${p}"`).join('\n')}

## PHRASES THIS PERSONA SHOULD AVOID (Anti-patterns)
${fingerprint.antiPatterns.slice(0, 15).map(p => `- "${p}"`).join('\n')}

## CONVERSATION CONTEXT
${context.conversationHistory.length > 0 ? 
  context.conversationHistory.slice(-4).map(m => `${m.role.toUpperCase()}: ${m.content}`).join('\n\n') :
  ...
  try {
    ...
    if (config.model.startsWith('claude')) {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          model: config.model === 'claude-3-5-sonnet' ? 'claude-3-5-sonnet-20241022' : 'claude-3-opus-20240229',
  ...
  try {
    ...
    return {
      ...
      metadata: {
        ...
        contextProvided: Object.keys(context).filter(k => context[k as keyof EvaluationContext] !== undefined),
      },
    };
  } catch (error) {
...
function createHeuristicEvaluation(
  ...
  const overallScore = Math.round(
    ...
    (authenticity * 0.2) + 
    (helpfulness * 0.25) + 
    (70 * 0.25) // Default for dimensions we can't measure heuristically
  );
...
export function shouldSampleConversation(
  ...
  if ((metadata?.emotionalIntensity || 0) > 0.7 && config.alwaysEvaluateIf.emotionalIntensity) return true;
