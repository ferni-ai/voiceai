...
// ============================================================================
export const jordanFingerprint = {
 ...
 */
export const personaFingerprints = {
 ...
 */
export function getPersonaFingerprint(personaId) {
    return personaFingerprints[personaId];
}
/**
 * Get all persona IDs with fingerprints
 */
export function getFingerprrintedPersonas() {
 ...
 */
export function analyzeSignaturePhraseUsage(response, fingerprint) {
    const lower = response.toLowerCase();
    const used = fingerprint.signaturePhrases.filter((phrase) => lower.includes(phrase.toLowerCase()));
    return {
        used,
 ...
 */
export function detectAntiPatterns(response, fingerprint) {
    const lower = response.toLowerCase();
    const detected = fingerprint.antiPatterns.filter((pattern) => lower.includes(pattern.toLowerCase()));
    return {
        detected,
 ...
 */
export function calculateVoiceDrift(response, fingerprint) {
    ...
    const driftWords = words.filter((word) => fingerprint.vocabularyProfile.driftIndicators.some((d) => word.includes(d.toLowerCase())));
 ...
 */
export function getVoiceConsistencyScore(response, fingerprint) {
    const drift = calculateVoiceDrift(response, fingerprint);
    return Math.round((1 - drift) * 100);
}
//# sourceMappingURL=persona-fingerprints.js.map