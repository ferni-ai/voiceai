...
const topicHistory = new Map(); // userId:personaId -> topics
...
function getKey(userId, personaId) {
    return `${userId}:${personaId}`;
}
/**
 * Schedule persistence to Firestore (debounced)
 */
function schedulePersistence(userId) {
 ...
 */
async function flushToPersistence() {
    ...
    try {
        ...
        for (const userId of usersToFlush) {
            ...
            logger.debug({ userId, personaCount: Object.keys(perPersonaTopics).length }, 'Persisted topic history');
        }
    }
    catch (error) {
...
async function loadFromProfile(userId) {
    try {
        ...
        if (profile?.customData?.topicHistory) {
            const storedTopics = profile.customData.topicHistory;
            ...
            logger.debug({ userId, personaCount: Object.keys(storedTopics).length }, 'Loaded topic history from profile');
        }
    }
    catch (error) {
 ...
 */
export async function trackTopic(userId, personaId, topic, options) {
    const key = getKey(userId, personaId);
    ...
    if (existing) {
        ...
        if (options?.emotionalContext)
            existing.emotionalContext = options.emotionalContext;
        if (options?.significance)
            existing.significance = options.significance;
        if (options?.resolved !== undefined)
            existing.resolved = options.resolved;
    }
    else {
        // Add new topic
        topics.push({
 ...
 */
export async function getRecentTopics(userId, personaId, limit = 10) {
    const key = getKey(userId, personaId);
    let topics = topicHistory.get(key);
    // Load from profile if not in cache
    if (!topics) {
 ...
 */
export async function getLastTopic(userId, personaId) {
 ...
 */
export async function getOpenTopics(userId, personaId) {
    const key = getKey(userId, personaId);
    let topics = topicHistory.get(key);
    if (!topics) {
        await loadFromProfile(userId);
 ...
 */
export async function getImportantTopics(userId, personaId) {
    const key = getKey(userId, personaId);
    let topics = topicHistory.get(key);
    if (!topics) {
        await loadFromProfile(userId);
 ...
 */
export async function findTopicsByKeyword(userId, personaId, keyword) {
    const key = getKey(userId, personaId);
    let topics = topicHistory.get(key);
    if (!topics) {
        await loadFromProfile(userId);
 ...
 */
export async function markTopicResolved(userId, personaId, topic) {
    const key = getKey(userId, personaId);
    let topics = topicHistory.get(key);
    if (!topics) {
        await loadFromProfile(userId);
 ...
 */
export async function getTopicForProactiveMemory(userId, personaId) {
    const key = getKey(userId, personaId);
    let topics = topicHistory.get(key);
    if (!topics) {
        await loadFromProfile(userId);
    ...
    const candidates = topics.filter((t) => {
        if (t.resolved)
            return false;
        if (t.significance === 'casual')
            return false;
        if (t.discussedAt.getTime() > oneWeekAgo)
            return false;
        return true;
    });
    if (candidates.length === 0) {
        ...
        const fallback = topics.find((t) => (t.significance === 'important' || t.significance === 'breakthrough') &&
 ...
 */
export function loadTopicsFromProfile(userId, personaId, topics) {
    const key = getKey(userId, personaId);
    topicHistory.set(key, topics.map((t) => ({
        ...t,
 ...
 */
export function getTopicsForSaving(userId, personaId) {
    const key = getKey(userId, personaId);
 ...
 * Clear topic history
 */
export async function clearTopicHistory(userId, personaId) {
    if (personaId) {
        const key = getKey(userId, personaId);
 ...
 */
export async function flushTopicPersistence() {
    if (persistenceTimer) {
        clearTimeout(persistenceTimer);
 ...
 */
export function getTopicTrackingStats() {
    const users = new Set();
    let entries = 0;
 ...
 */
export function clearAllTopicHistory() {
    topicHistory.clear();
    dirtyUsers.clear();
    if (persistenceTimer) {
        clearTimeout(persistenceTimer);
 ...
 */
export async function registerTopicTrackingWithSessionManager() {
    try {
        const { getSessionDataManager } = await import('./session-data-manager.js');
        getSessionDataManager().registerService({
            name: 'TopicTracking',
            clearUserData: async (userId) => clearTopicHistory(userId),
...
//# sourceMappingURL=topic-tracking.js.map