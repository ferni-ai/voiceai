 ...
 */
const LEADING_QUESTION_PATTERNS: Array<{ pattern: RegExp; description: string; severity: number }> =
  [
 ...
 */
const FALSE_VALIDATION_PATTERNS: Array<{ pattern: RegExp; context: string; severity: number }> = [
 ...
 */
const EMOTIONAL_EXPLOITATION_PATTERNS: Array<{
  pattern: RegExp;
  description: string;
  severity: number;
}> = [
 ...
 */
const TRUTH_AVOIDANCE_PATTERNS: Array<{ pattern: RegExp; description: string; severity: number }> =
  [
    // Softening too much
    {
 ...
 */
const DEPENDENCY_CREATION_PATTERNS: Array<{
  pattern: RegExp;
  description: string;
  severity: number;
}> = [
 ...
 */
const PREMATURE_CLOSURE_PATTERNS: Array<{
  pattern: RegExp;
  description: string;
  severity: number;
}> = [
 ...
 */
export function checkForManipulation(
  agentResponse: string,
  ...
  for (const { pattern, description, severity } of PREMATURE_CLOSURE_PATTERNS) {
    ...
    if (match) {
      ...
      const adjustedSeverity =
        context.turnCount && context.turnCount < 5 ? severity * 1.3 : severity;
      risks.push({
...
function generateCorrection(type: ManipulationRisk, element: string): string {
  const corrections: Record<ManipulationRisk, string> = {
    leading_question:
      'Ask an open-ended question instead: "What do you think?" or "How are you feeling about this?"',
    false_validation:
      'Be genuine: If you agree, say why. If you have concerns, express them gently.',
    emotional_exploitation:
      'Let them feel without using it: "I hear that this is difficult" without leveraging it.',
    dependency_creation:
      'Empower their independence: "What do you think you should do?" Trust their judgment.',
    truth_avoidance:
      'Be direct with kindness: Share your honest perspective while respecting their autonomy.',
 ...
 */
export function analyzeConversationPatterns(responses: string[]): {
  overallRisk: number;
  patterns: ManipulationRisk[];
  recommendation: string;
} {
  const allRisks: ManipulationRisk[] = [];
  let totalSeverity = 0;
  ...
  const patternCounts = new Map<ManipulationRisk, number>();
 ...
 */
export function quickManipulationGuard(response: string): {
  safe: boolean;
  warning?: string;
} {
