...
const userValuesProfiles = new Map();
// ============================================================================
// VALUE EXTRACTION
// ============================================================================
/**
 ...
 */
export function extractValues(userId, userMessage) {
 ...
 */
function updateUserValue(userId, category, keyword, confidence) {
 ...
 */
export function analyzeValuesAlignment(userId, userMessage, context = {}) {
...
// HELPERS
// ============================================================================
function determineSignificance(value, action, profile) {
    // High-stakes values
    ...
    if (isHighStakes && (isHighConfidence || isPattern))
        return 'major';
    if (isHighStakes || isPattern)
        return 'significant';
    if (isHighConfidence)
        return 'moderate';
    return 'minor';
}
function detectShortVsLongTermConflict(message, values) {
...
function generateReflectionQuestion(conflict) {
    const questions = {
        family: [
 ...
 */
export function getUserValuesProfile(userId) {
    return userValuesProfiles.get(userId) || null;
}
/**
 * Get user's top stated values
 */
export function getTopValues(userId, count = 3) {
    const profile = userValuesProfiles.get(userId);
    if (!profile)
        return [];
 ...
 */
export function setUserValues(userId, values) {
    let profile = userValuesProfiles.get(userId);
    if (!profile) {
        profile = {
            userId,
    ...
    for (const value of values) {
        ...
        for (const [category, keywords] of Object.entries(VALUE_CATEGORIES)) {
            if (keywords.some((k) => value.toLowerCase().includes(k)) ||
 ...
 * Clear user data
 */
export function clearUserValuesData(userId) {
...
//# sourceMappingURL=values-surfacing.js.map