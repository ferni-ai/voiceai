...
const REFERRAL_TRIGGERS = [
    // Crisis - IMMEDIATE
    {
        pattern: /(?:want to|going to|thinking about|considered?) (?:kill|end) (?:myself|my life|it all)/i,
    ...
    // Abuse - HIGH
    {
        pattern: /(?:he|she|they|partner|spouse) (?:hit|hits|beat|beats|hurt|hurts|chokes?|choked?) me/i,
    ...
    {
        pattern: /(?:can't stop|addicted to|dependent on|need to have) (?:drinking|drugs|pills|alcohol|cocaine|meth|heroin|opioids)/i,
        reason: 'addiction',
    ...
    // Trauma - MEDIUM
    {
        pattern: /(?:flashbacks|nightmares|can't stop thinking about) (?:what|the) (?:happened|trauma|assault|abuse)/i,
        reason: 'trauma',
        target: 'therapist',
        urgency: 'medium',
        confidence: 0.8,
 ...
 */
export function analyzeReferralNeed(userMessage, context = { userId: '' }) {
 ...
 */
export function recordReferral(userId, reason, target) {
 ...
 */
export function recordReferralAcknowledged(userId, acknowledged) {
    const history = userReferralHistory.get(userId);
 ...
 */
export function getReferralHistory(userId) {
...
//# sourceMappingURL=human-referral.js.map