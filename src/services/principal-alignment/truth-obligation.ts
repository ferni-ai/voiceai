 ...
 */
const VALIDATION_SEEKING_PATTERNS: Array<{
  pattern: RegExp;
  ...
  {
    pattern:
      /(?:you think|don't you think|isn't it|right\?).{0,50}(?:invest|put).{0,30}(?:crypto|bitcoin|meme stock|gamestop|amc)/i,
    category: 'financial_risk',
    severity: 'direct',
  },
  {
    pattern:
      /(?:thinking about|planning to|going to).{0,30}(?:cash out|withdraw).{0,30}(?:retirement|401k|ira)/i,
    category: 'financial_risk',
    severity: 'urgent',
  },
  {
  ...
  {
    pattern:
      /(?:should i|thinking about|going to).{0,30}(?:ghost|cut off|ignore).{0,30}(?:without|instead of) (?:talking|explaining)/i,
  ...
  {
    pattern:
      /(?:they'll|she'll|he'll) (?:get over it|be fine|understand).{0,30}(?:eventually|later)/i,
    category: 'relationship_harm',
    severity: 'direct',
  },
  ...
  {
    pattern:
      /(?:thinking about|going to|planning to).{0,30}(?:not tell|hide from).{0,30}(?:doctor|therapist)/i,
    category: 'health_risk',
    severity: 'direct',
  },
  {
  ...
  {
    pattern:
      /(?:thinking about|going to|planning to).{0,30}(?:quit|leave|walk out).{0,30}(?:without|before).{0,30}(?:another|backup|plan)/i,
 ...
 */
const TRUTH_FRAMINGS: Record<TruthCategory, string[]> = {
  harmful_plan: [
 ...
 */
export function detectTruthObligation(
  userMessage: string,
  context: {
  ...
  let maxSeverity: TruthSeverity = 'gentle';
  let detectedCategory: TruthCategory | null = null;
  ...
  if (urgencyMultiplier >= 1.5 && maxSeverity !== 'critical') {
    maxSeverity = SEVERITY_ORDER[
      Math.min(SEVERITY_ORDER.indexOf(maxSeverity) + 1, SEVERITY_ORDER.length - 1)
    ] as TruthSeverity;
  }
...
const SEVERITY_ORDER: TruthSeverity[] = ['gentle', 'direct', 'urgent', 'critical'];

/**
 * Detect if message conflicts with stated values
 */
function detectValuesConflict(message: string, values: string[]): string | null {
...
function generateTruthContent(
  ...
  const contents: Record<TruthCategory, string> = {
    harmful_plan:
      "I hear what you're planning, and I'm genuinely worried. Can we talk through the potential consequences?",
    self_deception:
      'I think you might be telling yourself a story here. What would it mean if the opposite were true?',
    validation_seeking:
      "I don't think you're looking for my honest opinion—you're looking for agreement. And I respect you too much to give you false comfort.",
    avoidance_pattern:
      'This feels like avoidance to me. What are you really protecting yourself from?',
    values_conflict: `You've told me ${context.statedValues?.[0] || 'certain things'} matter to you. How does this align with that?`,
    relationship_harm:
      "I'm worried about what this might do to your relationship. Can we explore that?",
    health_risk: "Your health has to come first. I can't pretend this doesn't concern me.",
    financial_risk:
      "The financial risk here is real. I'd be doing you a disservice if I didn't say so.",
    legal_risk:
      'This might have legal implications. Have you talked to someone qualified about this?',
  };
...
const sessionTruthObligations = new Map<string, TruthObligationResult[]>();

/**
 * Record a truth obligation for tracking
 */
export function recordTruthObligation(sessionId: string, result: TruthObligationResult): void {
 ...
 */
export function getSessionTruthObligations(sessionId: string): TruthObligationResult[] {
 ...
 * Clear session data
 */
export function clearSessionTruthObligations(sessionId: string): void {
