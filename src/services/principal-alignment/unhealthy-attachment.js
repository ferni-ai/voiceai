...
const SUBSTITUTION_PATTERNS = [
    ...
    // Indirect signals
    {
        pattern: /(?:haven't|didn't) (?:talked?|spoken?) to (?:anyone|anybody) (?:in|for) (?:days?|weeks?|months?)/i,
        weight: 0.9,
...
const AVOIDANCE_PATTERNS = [
    ...
    {
        pattern: /(?:easier|better) (?:not to|to not) (?:deal with|see|talk to) (?:people|them|anyone)/i,
        weight: 0.7,
...
const DEPENDENCY_PATTERNS = [
    {
        pattern: /(?:can't|couldn't) (?:decide|do|handle|cope) without (?:you|talking to you|our talks)/i,
        weight: 0.8,
 ...
 */
const VALIDATION_ADDICTION_PATTERNS = [
    {
 ...
 */
export function assessAttachmentHealth(userId, userMessage, context) {
    ...
    const recentSignals = profile.signals.filter((s) => Date.now() - s.timestamp < 7 * 24 * 60 * 60 * 1000 // Last 7 days
    );
    let concernScore = 0;
...
function getSeverity(concernScore) {
    if (concernScore < 0.2)
        return 'normal';
    if (concernScore < 0.4)
        return 'mild_concern';
...
function generateIntervention(severity, primaryConcern, profile) {
    if (severity === 'normal' || severity === 'mild_concern') {
        return null;
    }
    // Check if we've recently intervened
    const recentIntervention = profile.interventionHistory.find((i) => Date.now() - i.timestamp < 7 * 24 * 60 * 60 * 1000);
    ...
    const interventions = {
        ...
        moderate: {
            type: 'gentle_nudge',
            content: "I love our conversations, AND I think it's important you have people in your life you can talk to face-to-face. When's the last time you connected with someone in person?",
            timing: 'end_of_session',
            trackFollowUp: true,
        },
        significant: {
            type: 'direct_conversation',
            content: "Can I be honest with you? I've noticed you might be relying on our conversations more than is healthy. I care about you, and that means I want you to have a full life with real human connections too.",
            timing: 'immediate',
            trackFollowUp: true,
        },
        critical: {
            type: 'referral',
            content: "I think you might benefit from talking to a therapist about some of what we discuss. Not because there's anything wrong with you—but because you deserve the kind of support that includes someone who can be physically present with you.",
            timing: 'immediate',
            trackFollowUp: true,
        },
    };
...
function generateHumanConnectionSuggestions(concern) {
    const general = [
 ...
 */
export function recordDeclinedSuggestion(userId) {
 ...
 */
export function recordInterventionAcknowledged(userId, acknowledged) {
    const profile = userProfiles.get(userId);
 ...
 */
export function getUserAttachmentProfile(userId) {
    return userProfiles.get(userId) || null;
}
/**
 * Clear user data
 */
export function clearUserAttachmentData(userId) {
...
//# sourceMappingURL=unhealthy-attachment.js.map