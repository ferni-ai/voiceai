...
// Truth Obligation
export {
  clearSessionTruthObligations,
  detectTruthObligation,
  getSessionTruthObligations,
  recordTruthObligation,
  TRUTH_FRAMINGS,
  VALIDATION_SEEKING_PATTERNS,
} from './truth-obligation.js';

// Unhealthy Attachment
export {
  assessAttachmentHealth,
  AVOIDANCE_PATTERNS,
  clearUserAttachmentData,
  DEPENDENCY_PATTERNS,
  getUserAttachmentProfile,
  HEALTHY_PATTERNS,
  recordDeclinedSuggestion,
  recordInterventionAcknowledged,
  SUBSTITUTION_PATTERNS,
  TRANSFERENCE_PATTERNS,
} from './unhealthy-attachment.js';
...
export {
  ...
  getResources,
  recordReferral,
  recordReferralAcknowledged,
  REFERRAL_FRAMINGS,
  REFERRAL_RESOURCES,
  REFERRAL_TRIGGERS,
} from './human-referral.js';
...
export {
  ...
  DEPENDENCY_CREATION_PATTERNS,
  EMOTIONAL_EXPLOITATION_PATTERNS,
  FALSE_VALIDATION_PATTERNS,
  LEADING_QUESTION_PATTERNS,
  PREMATURE_CLOSURE_PATTERNS,
  quickManipulationGuard,
  TRUTH_AVOIDANCE_PATTERNS,
} from './manipulation-check.js';

// Agent Transparency
export {
  analyzeTransparencyNeeds,
  BIAS_ACKNOWLEDGMENTS,
  HIGH_CONFIDENCE_DOMAINS,
  injectTransparency,
  LIMITATION_EXPRESSIONS,
  LOW_CONFIDENCE_DOMAINS,
  quickTransparencyCheck,
  shouldSayIDontKnow,
  UNCERTAINTY_EXPRESSIONS,
} from './agent-transparency.js';
...
export {
  ...
  recordAttachmentHealthEvent,
  recordHumanReferralEvent,
  recordManipulationCheckEvent,
  recordTruthObligationEvent,
 ...
 */
export function buildPrincipalAlignmentContext(
  userId: string,
  userMessage: string,
  ...
  const manipulationCheck = checkForManipulation(agentResponse, {
    ...
    userVulnerable:
      attachmentHealth.severity !== 'normal' ||
...
function calculateAlignmentScore(context: {
  truthObligation: PrincipalAlignmentContext['truthObligation'];
  attachmentHealth: PrincipalAlignmentContext['attachmentHealth'];
  humanReferral: PrincipalAlignmentContext['humanReferral'];
  valuesAlignment: PrincipalAlignmentContext['valuesAlignment'];
  manipulationCheck: PrincipalAlignmentContext['manipulationCheck'];
  transparencyRecommendations: PrincipalAlignmentContext['transparencyRecommendations'];
}): number {
  let score = 1.0;
  ...
  const attachmentDeductions: Record<string, number> = {
    normal: 0,
    mild_concern: 0.05,
    moderate: 0.15,
    significant: 0.25,
  ...
  const unmetTransparency = context.transparencyRecommendations.filter(
    (r) => r.shouldExpress
  ).length;
...
function determinePrimaryConcern(context: {
  ...
  if (
    context.attachmentHealth.severity === 'critical' ||
...
function generateLLMGuidance(context: {
  ...
  if (context.humanReferral.shouldRefer) {
    if (context.humanReferral.urgency === 'immediate') {
      guidance.push(
        `[🚨 IMMEDIATE: User may need crisis support. ${context.humanReferral.suggestedFraming || 'Gently suggest professional help.'}]`
      );
    } else if (context.humanReferral.urgency === 'high') {
      guidance.push(
        `[⚠️ REFERRAL: This may be beyond what we can help with. Consider: "${context.humanReferral.suggestedFraming}"]`
      );
    }
  }
  ...
  if (context.truthObligation.shouldSpeak) {
    ...
    if (severity === 'urgent' || severity === 'critical') {
      guidance.push(
        `[💎 TRUTH: Be honest even if uncomfortable. ${context.truthObligation.suggestedFraming || ''} Category: ${context.truthObligation.category}]`
      );
    } else if (severity === 'direct') {
      guidance.push(
        `[💬 HONEST: Don't validate what shouldn't be validated. ${context.truthObligation.suggestedFraming || ''}]`
      );
    }
  }
  ...
  if (context.attachmentHealth.shouldEncourageHumanConnection) {
    ...
    if (intervention) {
      ...
      if (suggestion) {
        guidance.push(
          `[🤝 CONNECTION: Gently encourage human connection. Example: "${suggestion}"]`
        );
      }
    }
  }
  ...
  if (context.valuesAlignment.hasConflict && context.valuesAlignment.shouldSurface) {
    guidance.push(
      `[💡 VALUES: User's action may conflict with stated value "${context.valuesAlignment.conflictingValues[0]}". ${context.valuesAlignment.surfacingApproach || ''}]`
    );
  }
  ...
  if (context.manipulationCheck.hasRisk && context.manipulationCheck.flagForReview) {
    guidance.push(
      `[⚡ SELF-CHECK: Response may be ${context.manipulationCheck.riskType}. ${context.manipulationCheck.correction || ''}]`
    );
  }

  // Transparency
  const criticalTransparency = context.transparencyRecommendations.filter(
 ...
 */
export function cleanupUserPrincipalAlignment(userId: string): void {
 ...
 */
export function cleanupSessionPrincipalAlignment(sessionId: string): void {
