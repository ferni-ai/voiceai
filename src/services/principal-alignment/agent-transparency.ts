...
const UNCERTAINTY_EXPRESSIONS: Record<string, string[]> = {
  high_uncertainty: [
...
const LIMITATION_EXPRESSIONS: Record<string, string[]> = {
  scope: [
...
const BIAS_ACKNOWLEDGMENTS: Record<string, string[]> = {
  perspective: [
 ...
 */
export function analyzeTransparencyNeeds(
  ...
  const recommendations: TransparencyRecommendation[] = [];
  ...
  if (context.userExpectingExpertise) {
    ...
    if (!hasExpertise) {
      recommendations.push({
        ...
        type: 'limitation',
        suggestedPhrasing:
          "I should be upfront—I'm not an expert in this area. What I can offer is a thoughtful perspective and some questions to consider.",
...
function detectDomain(userMessage: string, topic?: string): string | null {
...
function checkForOverconfidence(
  response: string,
...
function checkForPotentialBias(
  response: string,
  userMessage: string
): {
 ...
 */
export function injectTransparency(
  response: string,
  ...
  const hasTransparency =
    /(?:I'm not sure|I could be wrong|This is outside|I might be biased)/i.test(response);
  if (hasTransparency) {
    return response;
  }
 ...
 */
export function quickTransparencyCheck(
  response: string,
  userMessage: string
): {
  ...
  if (domain && ['medical', 'legal', 'financial_specific'].includes(domain)) {
    ...
    const hasHedging = /(?:I'm not|I can't|outside my|you should talk to|speak with a)/i.test(
      response
    );
    if (!hasHedging) {
      return {
 ...
 */
export function shouldSayIDontKnow(
  userMessage: string,
  context: {
