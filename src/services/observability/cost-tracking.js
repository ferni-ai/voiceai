...
// RECORDING
// ============================================================================
export function recordLLMCost(model, inputTokens, outputTokens, userId, sessionId) {
...
export function recordTTSCost(provider, characters, userId, sessionId) {
...
export function recordSTTCost(provider, durationSeconds, userId, sessionId) {
...
export function recordEmbeddingCost(provider, tokens, userId, sessionId) {
...
export function getSnapshot() {
    ...
    // By service
    const byService = (service) => costEvents.filter((e) => e.service === service).reduce((sum, e) => sum + e.estimatedCost, 0);
    // By model/provider
    const costByModel = {};
    ...
    const charactersSpoken = costEvents
        ...
        .reduce((sum, e) => sum + e.units, 0);
    const minutesTranscribed = costEvents.filter((e) => e.service === 'stt').reduce((sum, e) => sum + e.units, 0) / 60;
    // Unique sessions
