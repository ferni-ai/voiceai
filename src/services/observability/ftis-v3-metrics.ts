 ...
 *
 * @module services/observability/ftis-v3-metrics
 */
...
export class FTISMetricsCollector {
   ...
   */
  private computeECE(outcomes: ClassificationOutcome[], nBins: number = 10): { ece: number; bins: CalibrationBin[] } {
    ...
    for (let i = 0; i < nBins; i++) {
      ...
      if (inBin.length > 0) {
        const avgConfidence = inBin.reduce((sum, o) => sum + o.effectiveConfidence, 0) / inBin.length;
  ...
  getSummary(): FTISMetricsSummary {
    ...
    return {
      ...
      classificationAccuracy: outcomesWithLabels.length > 0 ? correctCount / outcomesWithLabels.length : 0,

      totalOpenIntents: actualOpenIntents.length,
      detectedOpenIntents: detectedOpenIntents.length,
      openIntentRecall: actualOpenIntents.length > 0 ? detectedOpenIntents.length / actualOpenIntents.length : 0,
      openIntentPrecision:
        predictedOpenIntents.length > 0 ? truePositiveOpenIntent.length / predictedOpenIntents.length : 0,
  ...
  toPrometheus(): string {
    ...
    metric('classification_accuracy', 'Classification accuracy ratio', 'gauge', summary.classificationAccuracy);
    metric('total_classifications', 'Total classifications in window', 'counter', summary.totalClassifications);
    metric('open_intent_recall', 'Open intent detection recall', 'gauge', summary.openIntentRecall);
    metric('open_intent_precision', 'Open intent detection precision', 'gauge', summary.openIntentPrecision);
    metric('expected_calibration_error', 'Expected Calibration Error (ECE)', 'gauge', summary.expectedCalibrationError);
    metric('false_positive_rate', 'Tool call false positive rate', 'gauge', summary.falsePositiveRate);
    ...
    for (const [category, data] of summary.categoryAccuracy) {
      lines.push(
        `ftis_category_accuracy{category="${category}"} ${data.accuracy}`
      );
    }
