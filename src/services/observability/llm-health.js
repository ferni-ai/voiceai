...
class LLMHealthService {
    calls = [];
    MAX_CALLS = 10000;
    rateLimitQuota = 10000; // Tokens per minute default
    ...
    recordCall(call) {
        ...
        if (call.contextWindowUsed / call.contextWindowMax > 0.9) {
            log.warn({ utilization: `${((call.contextWindowUsed / call.contextWindowMax) * 100).toFixed(1)}%` }, '⚠️ High context window utilization');
        }
    }
    /**
     * Record a simple completion (convenience method)
     */
    recordCompletion(model, promptTokens, completionTokens, latencyMs, options = {}) {
        this.recordCall({
            model,
            promptTokens,
            completionTokens,
     ...
     * Record an error
     */
    recordError(model, errorType, latencyMs) {
        this.recordCall({
            model,
            promptTokens: 0,
            completionTokens: 0,
     ...
     */
    setRateLimitQuota(tokensPerMinute) {
        this.rateLimitQuota = tokensPerMinute;
    }
    /**
     * Update rate limit tracking
     */
    updateRateLimitTracking(tokens) {
        const now = Date.now();
    ...
    getSnapshot(windowMinutes = 60) {
        ...
        const truncations = windowCalls.filter((c) => c.contextWindowUsed >= c.contextWindowMax * 0.95).length;
        // Rate limits
