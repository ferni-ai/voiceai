 ...
 */
import { llmHealthMetrics } from './llm-health.js';
import { connectionHealthMetrics } from './connection-health.js';
import { uxQualityMetrics } from './ux-quality.js';
import { memoryMetrics } from './memory-health.js';
import { costMetrics } from './cost-tracking.js';
import { errorMetrics } from './error-recovery.js';
import { personaMetrics } from './persona-health.js';
import { getAggregateMetrics, } from '../../tools/semantic-router/integration/metrics.js';
...
// HEALTH CALCULATIONS
// ============================================================================
function calculateLLMHealth(snapshot) {
    let score = 100;
    // Error rate impact
    ...
    if (snapshot.p95LatencyMs >= 5000)
        score -= 30;
    else if (snapshot.p95LatencyMs >= 2000)
        score -= 15;
    // Context utilization
    if (snapshot.maxContextUtilization >= 85)
        score -= 10;
    // Fallback rate
    if (snapshot.fallbackRate > 10)
        score -= 10;
    return Math.max(0, score);
}
function calculateConnectionHealth(snapshot) {
    let score = 100;
    // Availability
    if (snapshot.availabilityPercent < 95)
        score -= 30;
    // Reconnections
    if (snapshot.reconnectionCount >= 5)
        score -= 20;
    // Packet loss
    if (snapshot.packetLossPercent >= 2)
        score -= 20;
    // Data channel
    if (snapshot.deliveryRate < 95)
        score -= 15;
    // Connection state
...
function calculateUXHealth(snapshot) {
    let score = 100;
    // Completion rate
    if (snapshot.completionRate < 70)
        score -= 25;
    // Interruption rate
    if (snapshot.interruptionRate > 30)
        score -= 15;
    // Timeout rate
    if (snapshot.timeoutRate > 10)
        score -= 20;
    // Error end rate
    if (snapshot.errorEndRate > 5)
        score -= 20;
    // Quality score
    if (snapshot.avgQualityScore < 70)
        score -= 15;
    return Math.max(0, score);
}
function calculateMemoryHealth(snapshot) {
    let score = 100;
    // Latency
    if (snapshot.avgSearchLatencyMs > 200)
        score -= 20;
    // Relevance
    if (snapshot.avgRelevanceScore < 0.6)
        score -= 25;
    // Error rate
    if (snapshot.embeddingErrorRate > 0.05)
        score -= 20;
    // Cache hit rate
    if (snapshot.cacheHitRate < 0.5)
        score -= 10;
    // Empty results
    if (snapshot.emptyResultSearches > 5)
        score -= 15;
    return Math.max(0, score);
}
function calculateCostHealth(snapshot) {
    let score = 100;
    // High projected costs
    if (snapshot.projectedMonthlyCost > 1000)
        score -= 30;
    else if (snapshot.projectedMonthlyCost > 500)
        score -= 15;
    // High hourly costs
    if (snapshot.costLastHour > 10)
        score -= 25;
    return Math.max(0, score);
}
function calculateErrorHealth(snapshot) {
    // Use the built-in health score
    return snapshot.errorHealthScore;
}
function calculatePersonaHealth(snapshot) {
    let score = 100;
    // Bundle load time
    if (snapshot.avgLoadTimeMs > 500)
        score -= 15;
    // Load failures
    if (snapshot.loadSuccessRate < 0.95)
        score -= 20;
    // Voice quality
    if (snapshot.voiceQualityRate < 0.8)
        score -= 20;
    // Knowledge query success
    if (snapshot.knowledgeQuerySuccessRate < 0.9)
        score -= 15;
    // Unhealthy personas
    if (snapshot.unhealthyPersonas.length > 0)
        score -= 15;
    return Math.max(0, score);
}
function calculateSemanticRoutingHealth(snapshot) {
    let score = 100;
    // No data is fine - just means routing hasn't been used yet
    if (snapshot.totalRoutes === 0)
        return 100;
...
// ALERT GENERATION
// ============================================================================
function generateAlerts(llm, connection, ux, memory, cost, errors, persona) {
    const newAlerts = [];
    const now = Date.now();
    // LLM alerts
    if (llm.errorRate >= 10) {
...
function getSnapshot(windowMinutes = 60) {
    ...
    const overallHealth = Math.round(llmHealth * 0.18 +
