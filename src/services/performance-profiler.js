...
class PerformanceProfiler {
     ...
     */
    startTrace(traceId, sessionId, metadata = {}) {
        const trace = {
            traceId,
            sessionId,
     ...
     */
    getInterval(traceId, startMark, endMark) {
        const trace = this.traces.get(traceId) || this.completedTraces.find((t) => t.traceId === traceId);
        if (!trace)
            return null;
     ...
     */
    generateReport(periodMs = 300000) {
    ...
    getOptimizationSuggestions() {
        ...
        if (report.timeToFirstToken.p90 > 100) {
            suggestions.push(`High TTFT (p90: ${report.timeToFirstToken.p90.toFixed(0)}ms) - Consider:` +
        ...
        if (report.llmLatency.p90 > 200) {
            suggestions.push(`High LLM latency (p90: ${report.llmLatency.p90.toFixed(0)}ms) - Consider:` +
                '\n  - Reducing max_tokens' +
        ...
        if (report.ttsLatency.p90 > 100) {
            suggestions.push(`High TTS latency (p90: ${report.ttsLatency.p90.toFixed(0)}ms) - Consider:` +
        ...
        if (report.sttLatency.p90 > 150) {
            suggestions.push(`High STT latency (p90: ${report.sttLatency.p90.toFixed(0)}ms) - Consider:` +
        ...
        if (report.totalResponseTime.p90 > 400) {
            suggestions.push(`High total response time (p90: ${report.totalResponseTime.p90.toFixed(0)}ms) - Review:` +
 ...
 */
export async function withTiming(traceId, markName, fn) {
 ...
 */
export function Timed(markName) {
