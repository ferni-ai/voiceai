...
class DataExportService {
  ...
  private async exportProfile(userId: string) {
    try {
      ...
      return {
        ...
        familyMembers: profile.familyMembers?.map((f: AnyRecord) => ({
          relationship: f.relationship,
          name: f.name,
          mentionedTopics: f.mentionedTopics,
        })),
        lifeEvents: profile.lifeEvents?.map((e: AnyRecord) => ({
          id: e.id,
          type: e.type,
          title: e.title,
          status: e.status,
        ...
        goals: profile.goals?.map((g: AnyRecord) => ({
          id: g.id,
          name: g.name,
          type: g.type,
          status: g.status,
  ...
  private async exportContacts(userId: string) {
    try {
      ...
      return contacts.map((c: AnyRecord) => ({
        id: c.id,
      ...
      return {
        boundaries: {
          ...
          types: boundaries.reduce(
            (acc: Record<string, number>, b: AnyRecord) => {
        ...
        growth: {
          ...
          types: [...new Set(growth.map((g: AnyRecord) => g.type))],
          examples: growth.slice(0, 10).map((g: AnyRecord) => ({
            type: g.type,
        ...
        sharedMoments: {
          ...
          runningGags: sharedMoments.filter((m: AnyRecord) => m.type === 'running_gag').length,
          phrases: sharedMoments.filter((m: AnyRecord) => m.type === 'phrase').length,
          stories: sharedMoments.filter((m: AnyRecord) => m.type === 'story').length,
        },
        celebrations: {
          winsRecognized: wins.length,
          winTypes: [...new Set(wins.map((w: AnyRecord) => w.type))],
    ...
    try {
      ...
      return {
        profile: data.profile
          ...
          : null,
        snapshots: data.snapshots.map((s: AnyRecord) => ({
          id: s.id,
  ...
  private async exportHabits(userId: string) {
    try {
      ...
      return {
        habits: (data.habits || []).map((h: AnyRecord) => ({
          id: h.id,
          name: h.name,
        ...
        habitLogs: (data.habitLogs || []).slice(-1000).map((l: AnyRecord) => ({
          id: l.id,
          habitId: l.habitId,
          date: l.date,
        ...
        enhancedHabits: (data.enhancedHabits || []).map((h: AnyRecord) => ({
          id: h.id,
          name: h.name,
          domain: h.domain,
        ...
        habitStacks: (data.habitStacks || []).map((s: AnyRecord) => ({
          id: s.id,
          name: s.name,
  ...
  private async exportProductivity(userId: string) {
    try {
      ...
      return {
        tasks: (data.tasks || []).map((t: AnyRecord) => ({
          id: t.id,
          title: t.title,
        ...
        notes: (data.notes || []).map((n: AnyRecord) => ({
          id: n.id,
          title: n.title,
          type: n.type,
          content: n.content,
        ...
        journalEntries: (data.journalEntries || []).map((j: AnyRecord) => ({
          id: j.id,
          date: j.date,
        ...
        bills: (data.bills || []).map((b: AnyRecord) => ({
          id: b.id,
          name: b.name,
          payee: b.payee,
          amount: b.amount,
        ...
        routines: (data.routines || []).map((r: AnyRecord) => ({
          id: r.id,
          name: r.name,
          type: r.type,
          steps: r.steps,
        ...
        shoppingLists: (data.shoppingLists || []).map((s: AnyRecord) => ({
          id: s.id,
          name: s.name,
          items: s.items,
