...
let cachedBypassConfig = {
    value: null,
    parsed: false,
};
export function parseBypassConfig() {
    ...
    if (bypassValue) {
        ...
        if (normalized === 'all' || normalized === 'true') {
            log.info({ bypassValue, memberCount: TEAM_MEMBERS.length }, '🔓 BYPASS_TEAM_UNLOCKS=all - unlocking ALL team members (logged once)');
        }
        else {
 ...
 */
export function isMemberBypassed(memberId) {
 ...
 */
export function calculateStreaks(profile) {
    ...
    log.debug({ currentStreak, longestStreak, totalDates: uniqueDates.length }, 'Calculated conversation streaks');
...
export const TEAM_MEMBERS = [
    {
        ...
        description: 'Your main point of contact. Asks the questions that unlock insight, celebrates every win, and connects you to the right specialist.',
    ...
    },
    {
        ...
        displayName: 'Maya',
        role: 'Habits Coach',
        description: 'Helps you build habits that stick through systems, not willpower. Start embarrassingly small. One habit at a time.',
        unlocksAt: 'getting-started',
        introductionMessage: 'I want you to meet someone special. Maya is incredible at helping people build habits that actually stick. She has this philosophy: start embarrassingly small. I think you two would really hit it off.',
        teaserMessage: "I have a friend who's amazing at habits... once we get to know each other a bit more, I'll introduce you.",
    },
    {
        memberId: 'peter-john',
        displayName: 'Peter',
        role: 'The Quant',
        description: 'Spots patterns nobody else sees across your spending, habits, and calendar. Turns data into insights that actually change behavior.',
        unlocksAt: 'building-trust',
        introductionMessage: "You're ready for this. Peter is our data whiz - he sees patterns that most people miss. He's going to show you things about yourself that will blow your mind.",
        teaserMessage: 'Peter can show you some incredible patterns in your life, but I need to know you better first. Trust is important for the kind of insights he provides.',
    },
    {
        memberId: 'alex-chen',
        displayName: 'Alex',
        role: 'Chief of Staff',
        description: 'Your communication coach. Manages calendar, email, and helps you navigate difficult conversations with confidence.',
        unlocksAt: 'established',
        introductionMessage: "Alex is going to change how you communicate. They're brilliant at helping people say what they mean and mean what they say. You've earned this introduction.",
        teaserMessage: "There's someone on my team who can transform how you communicate... but that's a deeper level of trust. Keep talking to me.",
    },
    {
        memberId: 'jordan-taylor',
        displayName: 'Jordan',
        role: 'Lifetime Planner',
        description: 'Turns vague dreams into lived experiences. From vacations to life transitions, helps you design every chapter intentionally.',
        unlocksAt: 'established',
        introductionMessage: "Jordan is special. They help people turn dreams into actual plans. Not someday - this year. I've been waiting to connect you two.",
        teaserMessage: 'I know someone who can help you plan your whole life... but we need to build more trust first.',
    },
    {
        memberId: 'nayan-patel',
        displayName: 'Nayan',
        role: 'The Sage',
        description: 'Lifetime advisor. Combines patience, simplicity, and wit. Helps you see that small, consistent actions create extraordinary results.',
        unlocksAt: 'deep-partnership',
        introductionMessage: "I've been waiting for this moment. Nayan is... different. He's the wisest person I know. He doesn't give advice often, but when he does, it changes lives. You've earned the right to meet him.",
        teaserMessage: 'There is someone I deeply respect... a sage. But Nayan only speaks to those who have proven their commitment to growth. Keep showing up.',
        premium: true,
    },
];
// ============================================================================
...
export function calculateRelationshipStage(metrics) {
    ...
    for (let i = STAGE_ORDER.length - 1; i >= 0; i--) {
        ...
        const meetsStreak = metrics.currentStreak >= threshold.minStreak || metrics.longestStreak >= threshold.minStreak;
 ...
 */
function stageAtOrBeyond(current, target) {
    return STAGE_ORDER.indexOf(current) >= STAGE_ORDER.indexOf(target);
}
/**
 * Get unlock status for a team member.
 */
export function getTeamMemberUnlockStatus(member, stage, tier, metrics) {
...
function calculateProgress(metrics, threshold) {
    if (!metrics)
        return 0;
    ...
    const daysProgress = threshold.minDays === 0 ? 1 : Math.min(1, metrics.daysSinceFirstMeeting / threshold.minDays);
    const streakProgress = threshold.minStreak === 0
        ? 1
        : Math.min(1, Math.max(metrics.currentStreak ?? 0, metrics.longestStreak ?? 0) / threshold.minStreak);
 ...
 */
export function getTeamUnlockState(profile, tier = 'free') {
    ...
    if (bypass instanceof Set) {
        ...
        const bypassedMembers = TEAM_MEMBERS.filter((m) => bypass.has(m.memberId)).map((m) => m.memberId);
    ...
    const metrics = profile
        ? {
            ...
            daysSinceFirstMeeting: profile.firstContact
                ? Math.floor((Date.now() - new Date(profile.firstContact).getTime()) / (1000 * 60 * 60 * 24))
                : 0,
 ...
 */
export function isTeamMemberAvailable(memberId, profile, tier = 'free') {
 ...
 */
export function isFullTeamUnlocked(profile, tier = 'free') {
 ...
 */
export function detectNewUnlock(previousMembers, currentMembers) {
 ...
 */
export function getUnlockIntroduction(memberId) {
    const member = TEAM_MEMBERS.find((m) => m.memberId === memberId);
 ...
 */
export function getLockedTeaser(memberId) {
    const member = TEAM_MEMBERS.find((m) => m.memberId === memberId);
...
export const UNLOCK_TEASERS = {
    ...
    topicTeaser: {
        ...
        communication: "Alex could transform that conversation for you... a bit more trust and I'll connect you.",
 ...
 */
export function getContextualUnlockTeaser(state, topic) {
    ...
    if (topic) {
        ...
        if (nextMember.memberId === 'jordan-taylor' &&
 ...
 */
export function getTeamDisplayData(state) {
    const metrics = {
...
//# sourceMappingURL=team-unlocks.js.map