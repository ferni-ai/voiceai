...
const userIntentions = new Map<string, ImplementationIntention[]>();
...
function assessSpecificity(when: string, then: string): 'vague' | 'moderate' | 'specific' {
  ...
  const specificityScore =
    (hasSpecificTrigger ? 1 : 0) +
    (hasSpecificAction ? 1 : 0) +
    (hasTime ? 1 : 0) +
    (hasPlace ? 1 : 0);

  if (specificityScore >= 3) return 'specific';
  if (specificityScore >= 1) return 'moderate';
  return 'vague';
}
 ...
 */
export function strengthenIntention(intention: ImplementationIntention): string[] {
...
const userCommitments = new Map<string, CommitmentDevice[]>();

/**
 * Create a commitment device.
 */
export function createCommitmentDevice(
  userId: string,
  commitment: string,
 ...
 */
export function suggestCommitmentDevice(
  goal: string,
  userContext?: { hasAccountabilityPartner?: boolean; valuesIdentity?: string[] }
 ...
 */
export function getLossFramedQuestions(topic: string): string[] {
  return [
...
const userBundles = new Map<string, TemptationBundle[]>();
 ...
 */
export function suggestTemptationBundles(
  shoulds: string[],
  wants: string[]
): TemptationBundleSuggestion[] {
  const suggestions: TemptationBundleSuggestion[] = [];

  // Common bundles that work well
  const classicBundles: Record<string, string[]> = {
 ...
 */
export function getTemptationBundles(userId: string): TemptationBundle[] {
 ...
 */
export function auditFriction(goal: string, currentBarriers?: string[]): FrictionAudit {
  const barriers: FrictionBarrier[] = [];
 ...
 */
export function getFrictionQuestions(goal: string): string[] {
  return [
...
export function generateSocialProof(context: {
  ...
  if (context.demographic) {
    proofs.push(
      `Other ${context.demographic} I've worked with have found that the first week is the hardest—after that it gets easier.`
    );
  }

  // Progress-based
  proofs.push(
    `The fact that you're even talking about this puts you ahead of most people who never take the first step.`
  );
 ...
 */
export function buildBehavioralEconomicsContext(
  userId: string,
  context: {
    goal?: string;
    barrier?: string;
