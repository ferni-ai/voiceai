...
interface PersistedNudgeData {
  commitments: CommitmentDevice[];
  intentions: ImplementationIntention[];
  lastUpdated: string;
}

let nudgePersistence: PersistenceStore<PersistedNudgeData> | null = null;

function getPersistence(): PersistenceStore<PersistedNudgeData> {
  if (!nudgePersistence) {
    nudgePersistence = createPersistenceStore<PersistedNudgeData>({
 ...
 */
export async function loadUserNudgeData(userId: string): Promise<void> {
  try {
 ...
 */
function saveUserNudgeData(userId: string): void {
  try {
    const persistence = getPersistence();
    const data: PersistedNudgeData = {
 ...
 */
export async function flushUserNudgeData(userId: string): Promise<void> {
  const persistence = getPersistence();
 ...
 */
export async function clearUserNudgeData(userId: string): Promise<void> {
...
export type NudgeType =
  | 'default_setting' // Make good choice the default
  | 'social_proof' // Show what others do
  | 'commitment_device' // Pre-commitment to future action
  | 'implementation_intention' // If-then planning
  | 'loss_framing' // Frame as avoiding loss
  | 'gain_framing' // Frame as gaining benefit
  | 'temporal_reframing' // Future vs present focus
  | 'identity_alignment' // Connect to who they want to be
  | 'friction_reduction' // Make it easier
  | 'friction_addition' // Make bad choice harder
  | 'salience' // Make relevant info prominent
...
export interface Nudge {
  ...
  effectiveness: number; // Estimated 0-1 based on research
}
...
export interface ImplementationIntention {
  ...
  userId: string;
  goal: string;
  situation: string; // When/where/how
  behavior: string; // What to do
  formula: string; // "If [situation], then [behavior]"
  createdAt: Date;
...
const commitments = new Map<string, CommitmentDevice[]>();
const intentions = new Map<string, ImplementationIntention[]>();
 ...
 */
export function selectNudges(context: NudgeContext): Nudge[] {
  const nudges: Nudge[] = [];
 ...
 */
function createNudge(type: NudgeType, context: NudgeContext): Nudge {
 ...
 */
export function recordIntentionOutcome(
  userId: string,
  intentionId: string,
  succeeded: boolean
): void {
  const userIntentions = intentions.get(userId);
  if (!userIntentions) return;
...
export function updateCommitmentStatus(
  userId: string,
  commitmentId: string,
  status: CommitmentDevice['status']
): void {
  const userCommitments = commitments.get(userId);
  if (!userCommitments) return;
 ...
 */
export function getActiveCommitments(userId: string): CommitmentDevice[] {
 ...
 */
export function getImplementationIntentions(userId: string): ImplementationIntention[] {
 ...
 */
export function getNudgeContextInjection(context: NudgeContext): string {
 ...
 */
export function generateIntentionPrompt(goal: string): string {
