 ...
 */
export function resetWeeklyCounters() {
 ...
 */
export function cleanupDeadTriggers() {
    const engine = getOutreachDecisionEngine();
    const now = new Date();
    let cleanedUp = 0;
    try {
 ...
 */
export function pruneOutreachHistory() {
    const engine = getOutreachDecisionEngine();
...
export async function syncAllCalendars() {
    ...
    try {
        ...
        const { isCalendarConfigured, getAllCalendarUsers } = await import('../identity/google-calendar-oauth.js');
        ...
        const realUserIds = userIds.filter((id) => !id.startsWith('cal-test-') && !id.startsWith('test-'));
 ...
 */
export function startMaintenanceScheduler(intervalMs = 60 * 60 * 1000) {
    // Run once on startup
    void runMaintenance();
    // Then schedule periodic runs using managed interval
    registerInterval(OUTREACH_MAINTENANCE_INTERVAL, () => {
        void runMaintenance();
    }, intervalMs);
 ...
 */
export function updateMaintenanceConfig(updates) {
