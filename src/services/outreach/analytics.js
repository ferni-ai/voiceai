 ...
 */
export function recordOutreachEvent(decision, channel) {
    const existing = (outreachEvents.get(decision.trigger.userId) || []).find((e) => e.id === decision.trigger.id);
    if (existing) {
...
export function calculatePeriodAnalytics(periodDays) {
    ...
    outreachEvents.forEach((events, userId) => {
        ...
        for (const e of sentEvents) {
            totalSent++;
            const hasResponse = responseMap.has(e.id) && responseMap.get(e.id)?.responseType !== 'no_response';
            if (hasResponse)
                totalResponded++;
            // Channel
            if (!byChannel[e.channel])
                byChannel[e.channel] = { sent: 0, responded: 0, rate: 0 };
            byChannel[e.channel].sent++;
            if (hasResponse)
                byChannel[e.channel].responded++;
            // Trigger
            if (!byTrigger[e.triggerType])
                byTrigger[e.triggerType] = { sent: 0, responded: 0, rate: 0 };
            byTrigger[e.triggerType].sent++;
            if (hasResponse)
                byTrigger[e.triggerType].responded++;
            // Persona
            if (!byPersona[e.personaId])
                byPersona[e.personaId] = { sent: 0, responded: 0, rate: 0 };
            byPersona[e.personaId].sent++;
            if (hasResponse)
                byPersona[e.personaId].responded++;
        }
    });
 ...
 */
export function calculateUserAnalytics(userId) {
    // Check cache
    ...
    const respondedEvents = sentEvents.filter((e) => responseMap.has(e.id) && responseMap.get(e.id)?.responseType !== 'no_response');
...
export function calculateGlobalAnalytics() {
    ...
    outreachEvents.forEach((events, userId) => {
        ...
        sentEvents.forEach((e) => {
            ...
            if (hasResponse)
                channelPerformance[e.channel].responded++;
            // Trigger stats
            ...
            if (hasResponse)
                triggerPerformance[e.triggerType].responded++;
            // Persona stats
            ...
            if (hasResponse)
                personaPerformance[e.personaId].responded++;
            // Time slot stats
            const hour = e.timestamp.getHours();
            ...
            if (hasResponse)
                timeSlotPerformance[slot].responded++;
        });
    });
    // Calculate rates
...
export function getRecommendations(userId) {
    ...
    const suggestedChannel = hasEnoughUserData
        ...
        : Object.entries(globalStats.channelPerformance).sort(([, a], [, b]) => b.rate - a.rate)[0]?.[0] || 'sms';
    const suggestedTime = hasEnoughUserData && userStats.bestTimeSlots.length > 0
    ...
    const suggestedTriggers = hasEnoughUserData && userStats.effectiveTriggers.length > 0
        ? userStats.effectiveTriggers.slice(0, 3).map((t) => t.type)
        : ['commitment_check', 'celebration', 'thinking_of_you'];
    const suggestedPersona = hasEnoughUserData
        ? userStats.preferredPersona
        : Object.entries(globalStats.personaPerformance).sort(([, a], [, b]) => b.rate - a.rate)[0]?.[0] || 'ferni';
    return {
        suggestedChannel,
        suggestedTime,
        suggestedTriggers,
...
export function predictResponseLikelihood(params) {
    ...
    const triggerRate = userStats.effectiveTriggers.find((t) => t.type === triggerType)?.rate ||
 ...
 */
export function exportAnalyticsData(userId) {
    const events = [];
    const responses = [];
    const userAnalytics = [];
    if (userId) {
 ...
 */
export function pruneOldAnalyticsData(maxAgeDays = 90) {
 ...
 */
export function clearUserAnalyticsData(userId) {
