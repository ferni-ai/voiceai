...
const configCache = new Map<string, OutreachVoiceConfig>();
 ...
 */
function getOutreachVoicePath(personaId: string): string {
 ...
 */
export function loadOutreachVoiceConfig(personaId: string): OutreachVoiceConfig | null {
  const canonical = getCanonicalPersonaId(personaId);
  ...
  try {
    ...
    const config = JSON.parse(content) as OutreachVoiceConfig;
 ...
 */
export function getOutreachVoiceConfig(personaId: string): OutreachVoiceConfig {
  const config = loadOutreachVoiceConfig(personaId);
  if (config) return config;

  // Fallback to Ferni
  const ferniConfig = loadOutreachVoiceConfig('ferni');
  if (ferniConfig) return ferniConfig;
 ...
 */
function getDefaultConfig(personaId: string): OutreachVoiceConfig {
 ...
 */
function applyTemplate(template: string, context: FormatContext): string {
  let result = template;
 ...
 */
function maybeAddEmoji(config: OutreachVoiceConfig, currentEmojiCount: number): string {
 ...
 */
export function formatVoicemailMessage(
  personaId: string,
  message: string,
  context: FormatContext = {}
): FormattedOutreach {
 ...
 */
export function personaSpecializesIn(personaId: string, topic: string): boolean {
  const canonical = getCanonicalPersonaId(personaId);
  const specialties = PERSONA_SPECIALTIES[canonical] || [];
  const lowerTopic = topic.toLowerCase();

  return specialties.some(
    (specialty) => lowerTopic.includes(specialty) || specialty.includes(lowerTopic)
  );
}
