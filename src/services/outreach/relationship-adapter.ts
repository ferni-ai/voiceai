...
export interface SignificantMoment {
  ...
  description: string;
  canReference: boolean; // Can we bring this up?
}
...
const STAGE_TONE_SETTINGS: Record<RelationshipStage, ToneAdjustment> = {
  new: {
    formality: 'formal',
    emojiLevel: 'minimal',
    humorAllowed: false,
...
const relationshipStore = new Map<string, RelationshipProfile>();
 ...
 */
export function getRelationshipProfile(userId: string): RelationshipProfile {
 ...
 */
export function calculateStage(profile: RelationshipProfile): RelationshipStage {
  ...
  if (
    metrics.totalConversations >= STAGE_THRESHOLDS.deep.minConversations &&
  ...
  if (
    metrics.totalConversations >= STAGE_THRESHOLDS.established.minConversations &&
  ...
  if (
    metrics.totalConversations >= STAGE_THRESHOLDS.building.minConversations &&
 ...
 */
export function updateStage(userId: string): RelationshipStage {
 ...
 */
export function addSharedReference(userId: string, reference: string): void {
  const profile = getRelationshipProfile(userId);
...
export function updateCommunicationStyle(
  userId: string,
  style: Partial<RelationshipProfile['communicationStyle']>
): void {
  const profile = getRelationshipProfile(userId);
 ...
 */
export function getToneAdjustment(userId: string): ToneAdjustment {
  const profile = getRelationshipProfile(userId);
 ...
 */
export function getMessageAdjustment(
  userId: string,
  userName: string,
  ...
  const adjustment: MessageAdjustment = {
...
function generateClosing(stage: RelationshipStage): string {
  switch (stage) {
    case 'new':
      return 'Take care';
    case 'building':
...
function determineNameUsage(
  profile: RelationshipProfile,
  tone: ToneAdjustment,
  preferredName?: string
): MessageAdjustment['nameUsage'] {
 ...
 */
export function getRandomReference(userId: string): string | null {
  const profile = getRelationshipProfile(userId);
 ...
 */
export function getReferenceableMoment(
  userId: string,
  ...
  const moments = profile.memory.significantMoments.filter((m) => {
    if (!m.canReference) return false;
