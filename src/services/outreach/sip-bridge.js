 ...
 */
export function initializeSIPBridge(bridgeConfig) {
    config = bridgeConfig;
 ...
 */
export function isSIPBridgeAvailable() {
 ...
 */
export async function initiateConversationalCall(options) {
...
<Response>
  ...
  <Hangup/>
    ...
    const contextHeader = Buffer.from(JSON.stringify({
        personaId,
        context,
        isOutbound: true,
 ...
 */
export function generateVoicemailTwiML(personaId, message) {
...
function getPersonaVoice(personaId) {
...
function getVoicemailMessage(personaId, originalMessage) {
    ...
    const shortMessage = originalMessage.length > 100 ? `${originalMessage.substring(0, 100)}...` : originalMessage;
 ...
 */
export function handleCallStatus(callSid, status, details) {
 ...
 */
export function handleMachineDetection(callSid, answeredBy) {
    const session = activeSessions.get(callSid);
    if (!session) {
 ...
 */
export function markSessionInConversation(callSid) {
    const session = activeSessions.get(callSid);
    if (session) {
...
//# sourceMappingURL=sip-bridge.js.map