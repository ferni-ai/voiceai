...
import { getPendingOutreach, updateOutreachStatus, incrementRetry, } from './scheduled-multi-outreach.js';
 ...
 */
async function executeScheduledOutreach(outreach) {
    const { userId, personaId, target, id } = outreach;
    const outreachId = `sched_exec_${id}`;
    log.info({ userId, contact: target.resolvedContactName, channel: target.channel }, '⏰ Executing scheduled outreach');
    // Mark as executing
    ...
    try {
        ...
        if (channelUsed === 'call' || channelUsed === 'conversation') {
            ...
            if (channelUsed === 'conversation') {
                ...
                const result = await callWithPersonaVoice(target.resolvedPhone, target.message || target.purpose, personaId, { fallbackToTwilioVoice: true });
        ...
        if (success) {
            log.info({ userId, contact: target.resolvedContactName, channel: channelUsed }, '✅ Scheduled outreach completed');
        }
        else {
            log.warn({ userId, contact: target.resolvedContactName, error: errorMsg }, '⚠️ Scheduled outreach failed');
        }
    }
    catch (error) {
        const errorMsg = error instanceof Error ? error.message : 'Unknown error';
        log.error({ userId, contact: target.resolvedContactName, error: errorMsg }, '❌ Scheduled outreach error');
 ...
 */
async function processUserOutreach(userId) {
    if (config.dryRun) {
 ...
 */
export function startScheduledOutreachExecutor(overrides) {
    if (isRunning) {
    ...
    isRunning = true;
    log.info({ pollIntervalMs: config.pollIntervalMs, dryRun: config.dryRun }, '🚀 Starting scheduled outreach executor');
    registerInterval('scheduled-outreach-executor', poll, config.pollIntervalMs);
}
/**
 * Stop the scheduled outreach executor
 */
export function stopScheduledOutreachExecutor() {
    if (!isRunning)
        return;
    isRunning = false;
 ...
 */
export function registerUserForScheduledOutreach(userId) {
 ...
 */
export function unregisterUserFromScheduledOutreach(userId) {
...
//# sourceMappingURL=scheduled-outreach-executor.js.map