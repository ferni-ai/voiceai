 ...
 */
export function initializeSMSDelivery(deliveryConfig) {
 ...
 */
export function isSMSDeliveryAvailable() {
 ...
 */
export function formatSMSMessage(body, options = {}) {
 ...
 */
export async function shortenLinks(body, userId, outreachId) {
...
export async function sendSMS(message) {
    ...
    try {
        ...
        const brandCheck = validateSmsContent(message.body, message.personaId);
        if (!brandCheck.isValid) {
            log.warn({ userId: message.userId, issues: brandCheck.issues }, '⚠️ SMS content has brand issues');
        }
 ...
 */
export async function sendSMSWithRetry(message, retryCount = 0) {
...
export async function sendBulkSMS(messages) {
    ...
    for (let i = 0; i < messages.length; i += batchSize) {
        ...
        const batchResults = await Promise.all(batch.map(async (msg) => sendSMS(msg).then((r) => [msg.outreachId, r])));
 ...
 */
export function handleSMSStatus(messageSid, status, errorCode, errorMessage) {
    ...
    if (status === 'delivered') {
        ...
        log.warn({ messageSid, userId: record.userId, errorCode, errorMessage }, '❌ SMS delivery failed');
    }
 ...
 * Get delivery record
 */
export function getDeliveryRecord(messageSid) {
    return deliveryRecords.get(messageSid);
}
/**
 * Get all delivery records for a user
 */
export function getUserDeliveryRecords(userId) {
 ...
 */
export function cancelPendingRetry(outreachId) {
 ...
 */
export function clearOldRecords(maxAgeHours = RECORD_TTL_HOURS) {
...
function enforceMaxSize() {
    ...
    const sorted = Array.from(deliveryRecords.entries()).sort(([, a], [, b]) => b.sentAt.getTime() - a.sentAt.getTime());
    const toRemove = sorted.slice(MAX_DELIVERY_RECORDS);
    for (const [sid] of toRemove) {
        deliveryRecords.delete(sid);
    }
    log.info({ removed: toRemove.length, remaining: deliveryRecords.size }, 'Enforced max delivery records limit');
    return toRemove.length;
}
/**
 * Run periodic record cleanup (TTL + size limit)
 */
function runRecordCleanup() {
    const ttlCleared = clearOldRecords();
    const sizeCleared = enforceMaxSize();
    if (ttlCleared > 0 || sizeCleared > 0) {
        log.debug({ ttlCleared, sizeCleared, remaining: deliveryRecords.size }, 'SMS delivery record cleanup complete');
    }
}
/**
 * Shutdown SMS delivery
 */
export function shutdownSMSDelivery() {
...
//# sourceMappingURL=sms-delivery.js.map