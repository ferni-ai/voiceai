 ...
 */
export function initializePushNotifications(pushConfig) {
    config = pushConfig;
 ...
 */
export function registerPushToken(userId, token, platform, userAgent) {
 ...
 */
export function hasPushEnabled(userId) {
    const tokens = userTokens.get(userId);
 ...
 */
async function sendToToken(notification, token, style) {
    ...
    const response = await fcmCircuitBreaker.execute(() => fetch(`https://fcm.googleapis.com/v1/projects/${config.firebaseProjectId}/messages:send`, {
        method: 'POST',
        headers: {
...
export async function sendBulkPushNotifications(notifications) {
    ...
    for (let i = 0; i < notifications.length; i += concurrencyLimit) {
        ...
        const batchResults = await Promise.all(batch.map(async (n) => sendPushNotification(n).then((r) => [n.userId, r])));
 ...
 */
export function generatePersonaNotification(personaId, type, context) {
    const templates = {
        ferni: {
            commitment: {
 ...
 */
export function handlePushInteraction(messageId, interaction, actionId) {
 ...
 */
export function getUserDeliveryRecords(userId, limit = 50) {
 ...
 */
export function clearInactiveTokens(maxAgeDays = 90) {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - maxAgeDays);
    let cleared = 0;
