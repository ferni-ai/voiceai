 ...
 */
export function initializeEmailDelivery(deliveryConfig) {
 ...
 */
export function generatePersonaEmailHTML(personaId, options) {
 ...
 */
export function generatePlainText(personaId, body, userName) {
    const personaName = getPersonaDisplayName(personaId);
    const style = PERSONA_STYLES[personaId] || PERSONA_STYLES.ferni;
    let text = '';
    if (userName) {
...
export async function sendEmail(message) {
    ...
    try {
        ...
        const brandCheck = validateEmailContent(message.subject, message.body, message.personaId);
        if (!brandCheck.isValid) {
            log.warn({ userId: message.userId, issues: brandCheck.issues }, '⚠️ Email content has brand issues');
        }
 ...
 * Send via SendGrid
 */
async function sendViaSendGrid(message, html, text) {
    const personaName = getPersonaDisplayName(message.personaId);
    const response = await sendGridCircuitBreaker.execute(() => fetch('https://api.sendgrid.com/v3/mail/send', {
        method: 'POST',
        headers: {
 ...
 * Send via Resend
 */
async function sendViaResend(message, html, text) {
    const personaName = getPersonaDisplayName(message.personaId);
    const response = await resendCircuitBreaker.execute(() => fetch('https://api.resend.com/emails', {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${config.apiKey}`,
            'Content-Type': 'application/json',
 ...
 */
export async function sendEmailWithRetry(message, retryCount = 0) {
 ...
 */
export function handleEmailEvent(messageId, event, details) {
    ...
    switch (event) {
        ...
        case 'bounced':
            ...
            log.warn({ messageId, userId: record.userId, reason: details?.bounceReason }, '⚠️ Email bounced');
            break;
        case 'failed':
 ...
 * Get delivery record
 */
export function getDeliveryRecord(messageId) {
 ...
 */
export function clearOldRecords(maxAgeDays = 90) {
 ...
 */
export function shutdownEmailDelivery() {
...
//# sourceMappingURL=email-delivery.js.map