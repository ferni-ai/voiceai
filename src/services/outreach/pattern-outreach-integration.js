 ...
 */
function wasRecentlyScheduled(userId, pattern) {
 ...
 */
function markAsScheduled(userId, pattern) {
    const key = `${userId}:${pattern}`;
 ...
 */
export async function schedulePatternOutreach(pattern, ctx) {
    ...
    if (wasRecentlyScheduled(ctx.userId, pattern.pattern)) {
        log.debug({ userId: ctx.userId, pattern: pattern.pattern }, 'Pattern already scheduled recently, skipping duplicate');
        return;
    }
 ...
 */
export function schedulePatternOutreachAsync(pattern, ctx) {
 ...
 */
function calculateScheduledTime(schedule) {
    const now = new Date();
 ...
 */
export async function scheduleSundayAnxietyFollowUp(userId, sessionId, anxietyLevel) {
    ...
    const trigger = {
        ...
        context: {
            ...
            metadata: {
                ...
                suggestedMessage: "Good morning! I know Mondays can feel heavy. How are you doing? I'm here if you want to talk through anything.",
                source: 'pattern_detection',
            },
        },
    };
    await publishOutreachTrigger(trigger);
    log.info({ userId, scheduledFor: mondayMorning.toISOString() }, '📅 Monday morning check-in scheduled');
}
/**
 * Schedule work stress evening check-in
 * Called when user is stressed about work during the day
 */
export async function scheduleWorkStressFollowUp(userId, sessionId, stressTopics) {
    ...
    const trigger = {
        ...
        context: {
            ...
            metadata: {
                ...
                suggestedMessage: "Hey, I was thinking about what you shared earlier about work. How are you feeling now that the day's winding down?",
                source: 'pattern_detection',
            },
        },
    };
 ...
 */
export async function scheduleRelationshipCheckIn(userId, sessionId, relationshipType) {
 ...
 */
function getNextWeekdayAt(targetDay, hour) {
    const now = new Date();
...
//# sourceMappingURL=pattern-outreach-integration.js.map