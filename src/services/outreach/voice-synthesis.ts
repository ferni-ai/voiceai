...
let config: VoiceSynthesisConfig | null = null;
 ...
 */
export function initializeVoiceSynthesis(synthesisConfig: VoiceSynthesisConfig): void {
 ...
 */
export async function cleanupOldVoiceMessages(maxAgeDays = 7): Promise<number> {
  if (!gcsClient || !config?.gcsBucketName) {
    return 0;
  }
