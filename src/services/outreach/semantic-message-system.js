 ...
 */
function getTransition(context) {
 ...
 */
function getMainMessage(context) {
 ...
 */
function getPersonalTouch(context) {
    ...
    if (memory?.lastContactDate) {
        const daysSince = Math.floor((Date.now() - memory.lastContactDate.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export function generateSemanticMessage(context) {
...
export function inferRelationshipStage(relationship) {
    ...
    if (deepRelationships.some((r) => lower.includes(r)))
        return 'deep';
    if (establishedRelationships.some((r) => lower.includes(r)))
        return 'established';
    if (buildingRelationships.some((r) => lower.includes(r)))
        return 'building';
    return 'new';
}
/**
 ...
 */
export function quickGenerate(request, contactName, relationship, senderName, isVoicemail = false) {
...
//# sourceMappingURL=semantic-message-system.js.map