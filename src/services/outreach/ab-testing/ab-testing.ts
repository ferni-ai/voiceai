...
export interface ABTest {
  ...
  controlVariantId: string; // Which variant is the control

  // Targeting
  triggerTypes?: string[]; // Which outreach types to test on
  userSegment?: 'all' | 'new' | 'established' | 'deep';
  sampleSize?: number; // Optional max participants
...
export interface VariantResult {
  variantId: string;
  participants: number;
  conversions: number; // Based on primary metric
  conversionRate: number;
  confidence?: number; // Probability this is the best
}
...
const assignments = new Map<string, TestAssignment[]>(); // userId -> assignments
const variantResults = new Map<string, Map<string, VariantResult>>(); // testId -> variantId -> result
...
export function createTest(config: Omit<ABTest, 'id' | 'createdAt' | 'status'>): ABTest {
  ...
  const resultMap = new Map<string, VariantResult>();
 ...
 */
export function completeTest(testId: string): ABTestResults | null {
  const test = tests.get(testId);
  if (!test) return null;
 ...
 * Get all tests
 */
export function getAllTests(status?: TestStatus): ABTest[] {
 ...
 */
function selectVariantByWeight(variants: ABTestVariant[], hash: number): ABTestVariant {
  let cumulative = 0;
 ...
 */
function getTotalParticipants(testId: string): number {
  const results = variantResults.get(testId);
  if (!results) return 0;
...
export function recordConversion(
  testId: string,
  userId: string,
  metadata?: { responseTimeMs?: number; channel?: string }
): boolean {
  const userAssignments = assignments.get(userId);
 ...
 */
function checkAutoComplete(testId: string): void {
  const test = tests.get(testId);
 ...
 */
export function calculateTestResults(testId: string): ABTestResults {
  const test = tests.get(testId);
  const results = variantResults.get(testId);
  ...
  const variantResultsArray: VariantResult[] = Array.from(results.values());
  ...
  const { isSignificant, confidence } = calculateSignificance(
    variantResultsArray,
    test.controlVariantId,
 ...
 */
export function getUserAssignments(userId: string): TestAssignment[] {
 ...
 */
export function isUserInTest(userId: string, testId: string): boolean {
  const userAssignments = assignments.get(userId);
 ...
 */
export function clearOldTests(maxAgeDays = 90): number {
