...
// ============================================================================
function breakTag(time) {
    return `<break time="${time}"/>`;
}
function speedTag(ratio) {
...
const CALL_TYPE_EMOTION_OVERRIDES = {
    celebration: 'excited',
    support: 'sympathetic',
};
// ============================================================================
 ...
 * - Closing warmth
 */
export function enhanceOutboundMessage(message, options = {}) {
    const { personaId = 'ferni', callType = 'introduction', relationshipStage = 'new', addOpeningWarmth = true, } = options;
    if (!message?.trim()) {
        return message;
    }
    ...
    for (const closing of meaningfulClosings) {
        if (enhanced.toLowerCase().includes(closing.toLowerCase())) {
            enhanced = enhanced.replace(new RegExp(`(${closing}[^.!?]*)([.!?])`, 'gi'), (match, text, punct) => `${speedTag(slowdownSpeed)}${text}${punct}`);
        }
    }
    ...
    for (const phrase of supportivePhrases) {
        if (enhanced.toLowerCase().includes(phrase.toLowerCase())) {
            enhanced = enhanced.replace(new RegExp(phrase, 'gi'), (match) => `${emotionTag('sympathetic')}${match}`);
        }
    }
 ...
 */
export function createWarmOpening(personaId, recipientName, referrerName) {
 ...
 */
export function createWarmClosing(recipientName, occasion) {
    if (occasion) {
 ...
 */
export function emphasize(text, speed, personaId) {
...
//# sourceMappingURL=outbound-ssml.js.map