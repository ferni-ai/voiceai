...
const callResultStore = new Map();
const followUpActionsStore = new Map();
/**
 * Store a call result in Firestore (with in-memory fallback)
 */
async function storeCallResult(result) {
    try {
        // Try Firestore first
        ...
        if (db) {
            ...
            void onCallResultChange(result.userId, result.callId, {
                ...
                outcome: result.outcome.status === 'completed'
                    ? 'answered'
 ...
 */
export async function getCallResult(callId, userId) {
    try {
        const { getFirestoreDb } = await import('../superhuman/firestore-utils.js').catch(() => ({
            getFirestoreDb: null,
        }));
 ...
 */
async function notifyOriginalSession(originalSessionId, callId, outcome) {
    try {
        log.info({ originalSessionId, callId, status: outcome.status }, 'Storing call result notification for session');
 ...
 */
async function storeNotification(sessionId, callId, outcome) {
    try {
        const { getFirestoreDb } = await import('../superhuman/firestore-utils.js').catch(() => ({
            getFirestoreDb: null,
        }));
 ...
 */
async function injectToActiveSession(sessionId, outcome, request) {
    try {
        ...
        log.info({ sessionId, callId: outcome.callId, contactName }, 'Injected call result into active session');
        return true;
    }
    catch (error) {
        // Log but don't fail - this is a nice-to-have, not critical
        log.debug({ error: String(error), sessionId }, 'Could not inject to active session (user may have disconnected)');
        return false;
    }
}
// ============================================================================
 ...
 */
async function createFollowUpActions(userId, callId, outcome, request) {
    const actions = [];
 ...
 */
export async function captureCallResult(callId, outcome, request) {
    log.info({
        callId,
        status: outcome.status,
        objectiveAchieved: outcome.objectiveAchieved,
    ...
    try {
        ...
        const injectedToActive = await injectToActiveSession(request.originalSessionId, outcome, request);
        ...
        log.info({ callId, userId: request.userId }, 'Call result captured successfully (push + email + calendar + unified)');
    }
    catch (error) {
 ...
 */
async function sendCallResultPushNotification(request, outcome, callId) {
    try {
        const { sendPushNotification, isPushNotificationsAvailable } = await import('./delivery/push-notifications.js');
 ...
 */
async function sendCallResultEmail(request, outcome, callId) {
    try {
        // Get user's email
        ...
        const { subject, body } = buildCallResultEmailContent(contactName, outcome, request.purpose, completedAt);
        await sendEmail({
            to: email,
            subject,
            body,
 ...
 */
function buildCallResultEmailContent(contactName, outcome, purpose, completedAt) {
 ...
 */
async function createCallbackCalendarEvent(request, outcome, callId) {
    ...
    try {
        ...
        log.info({ callId, userId: request.userId, contactName, scheduledFor: callbackDate.toISOString() }, 'Created calendar event for callback');
    }
    catch (error) {
 ...
 */
function parseCallbackTimeHint(hint) {
    const now = new Date();
 ...
 */
async function storeInUnifiedResults(request, outcome, callId) {
    try {
 ...
 */
export async function getRecentCallResults(userId, limit = 10) {
    try {
        const { getFirestoreDb } = await import('../superhuman/firestore-utils.js').catch(() => ({
            getFirestoreDb: null,
        }));
...
//# sourceMappingURL=call-result-capture.js.map