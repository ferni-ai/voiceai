...
import { addCommitment, addLifeEvent, addStruggle, addWin, updateEmotionalState, } from './context-aggregator.js';
...
const MILESTONE_PATTERNS = [
    ...
    {
        pattern: /(\d+)\s+(?:days?|weeks?)\s+of\s+(?:meditation|exercise|working out|journaling|reading)/i,
        type: 'streak',
    },
    // Goal progress
    ...
    // Counts/milestones
    {
        pattern: /(?:this is|that's)\s+(?:my |the )\s*(\d+)(?:st|nd|rd|th)\s+(?:time|day|week|session)/i,
        type: 'count',
    },
    {
 ...
 */
export async function processExtractionResults(userId, extraction) {
    ...
    for (const commitment of extraction.commitments || []) {
        if (commitment.confidence >= 0.7) {
            try {
                ...
                log.info({ userId, commitment: commitment.what }, 'Commitment extracted and trigger created');
            }
            catch (error) {
        ...
        if (pattern.confidence >= 0.7) {
            try {
                ...
                if (pattern.type === 'day_of_week' && pattern.dayOfWeek !== undefined) {
                    ...
                    triggersCreated.push(triggerId);
                    log.info({ userId, pattern: pattern.pattern, triggerDate }, '📊 Day pattern detected - preemptive support scheduled');
                }
            }
            catch (error) {
 ...
 */
export async function extractAndProcess(userId, message) {
...
function extractTrigger(message, emotionPattern) {
