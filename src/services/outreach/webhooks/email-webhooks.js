...
// STATE
// ============================================================================
let sendgridWebhookKey = null;
let resendWebhookSecret = null;
const eventHandlers = [];
 ...
 */
export function onEmailEvent(handler) {
 ...
 */
export function validateSendGridSignature(payload, signature, timestamp) {
 ...
 */
export function validateResendSignature(payload, signature, webhookId, timestamp) {
 ...
 */
export async function handleSendGridWebhook(events, signature, timestamp, rawPayload) {
 ...
 */
export async function handleResendWebhook(event, signature, webhookId, timestamp, rawPayload) {
    // Validate signature in production
 ...
 */
function parseResendEvent(event) {
    const eventMap = {
...
async function processEmailEvent(event) {
    ...
    if (event.event !== 'complained' && event.event !== 'unsubscribed') {
        handleEmailEvent(event.messageId, event.event, {
 ...
 */
export function generateOpenTrackingPixel(messageId, baseUrl) {
    return `${baseUrl}/api/outreach/webhooks/email/open?mid=${encodeURIComponent(messageId)}`;
}
/**
 * Generate click tracking URL
 */
export function generateClickTrackingUrl(messageId, originalUrl, baseUrl) {
 ...
 */
export async function handleOpenTracking(messageId, userAgent, ip) {
 ...
 */
export async function handleClickTracking(messageId, encodedUrl, userAgent, ip) {
 ...
 * Clear old events
 */
export function clearOldEvents(maxAgeHours = 48) {
...
//# sourceMappingURL=email-webhooks.js.map