...
export type OnboardingMilestone =
  ...
  | 'first_followup' // Day 2 check-in
  | 'first_topic_explored' // Explored a topic in depth
  | 'first_week' // Day 7
  | 'second_week' // Day 14
  | 'first_month'; // Day 30

export type CheckInType =
  | 'welcome_followup' // Day 1: "How are you settling in?"
  | 'next_day_check' // Day 2: "How did yesterday go?"
  | 'topic_deepdive' // Day 3-4: Follow up on their main concern
  | 'first_week_reflection' // Day 7: "First week! How's it going?"
  | 'momentum_check' // Day 10: Keep the momentum going
  | 'two_week_celebration' // Day 14: Milestone celebration
  | 'habit_nudge' // Gentle nudge if going quiet
  | 'win_celebration'; // Celebrate early wins
...
interface CheckInTemplate {
  type: CheckInType;
  dayRange: [number, number]; // [minDay, maxDay] from signup
  priority: 'low' | 'medium' | 'high';
  condition?: (state: OnboardingState) => boolean;
  messageTemplates: Array<(state: OnboardingState) => string>;
  persona: 'ferni' | 'contextual'; // 'contextual' = use their preferred persona
}

const CHECK_IN_TEMPLATES: CheckInTemplate[] = [
  {
    ...
    messageTemplates: [
      (state) =>
        `Hey ${state.name || 'there'}! Just wanted to check in after our first chat. How are you feeling about everything? No pressure to respond - just thinking of you. 🌱`,
      (state) =>
        `${state.name || 'Hey'}! Yesterday was a great start. I've been thinking about what you shared. How are you doing today?`,
    ],
    persona: 'ferni',
  },
  {
  ...
  },
  {
    ...
    messageTemplates: [
      (state) =>
        `I've been reflecting on what you said about ${state.primaryConcern || 'your situation'}. Would love to dig deeper when you have a moment. No rush though.`,
      (state) =>
        `Hey ${state.name || 'friend'} - that thing you mentioned is still on my mind. I have some thoughts if you want to explore together.`,
    ],
    persona: 'contextual',
  },
  {
    ...
    messageTemplates: [
      (state) =>
        `${state.name || 'Hey'}! It's been about a week since we started talking. How are you feeling about things? I'd love to hear what's on your mind. 🌟`,
      () =>
      ...
      (state) =>
        `Week one in the books! ${state.name || 'Friend'}, I'm genuinely curious - how are you really doing? No right answer, just honest.`,
    ],
    persona: 'ferni',
  },
  {
    ...
    messageTemplates: [
      ...
      (state) =>
        `${state.name || 'Hey'} - we're building something here. What's been on your mind lately?`,
    ],
    persona: 'contextual',
  },
  {
    ...
    messageTemplates: [
      (state) =>
        `Two weeks, ${state.name || 'friend'}! 🎉 That's not nothing. Most people never start, but you did. I'm proud of you. How does it feel?`,
      () =>
        `Two weeks of showing up for yourself. I wanted to mark the moment - you're doing the work, and it matters. What's next? 💚`,
      (state) =>
        `Hey ${state.name || 'there'} - it's been two weeks since we started this journey together. I've really enjoyed getting to know you. Here's to many more conversations.`,
    ],
    persona: 'ferni',
  },
  {
    ...
    messageTemplates: [
      (state) =>
        `Hey ${state.name || 'friend'} - just a gentle nudge. I'm here whenever you're ready. No pressure, but I've been thinking about you. 💚`,
      () =>
  ...
  },
  {
    ...
    messageTemplates: [
      (state) =>
        `${state.name || 'Hey'}! I noticed you dove deep into something today. That takes courage. How do you feel about it?`,
      () =>
...
export function recordConversation(
  userId: string,
  context?: { primaryConcern?: string; persona?: string }
): void {
  ...
  state.daysSinceSignup = Math.floor(
    (Date.now() - state.signupDate.getTime()) / (24 * 60 * 60 * 1000)
  );
 ...
 */
export function recordTopicExplored(userId: string, topic: string): void {
  const state = onboardingStates.get(userId);
  if (!state) return;
 ...
 */
export function recordCheckInSent(
  userId: string,
  type: CheckInType,
 ...
 */
export function recordCheckInResponse(userId: string): void {
  const state = onboardingStates.get(userId);
 ...
 */
function calculateEngagementLevel(state: OnboardingState): EngagementLevel {
...
export async function getPendingCheckIns(userId: string): Promise<ScheduledCheckIn[]> {
  ...
  for (const template of CHECK_IN_TEMPLATES) {
    ...
    if (alreadySentTypes.has(template.type)) continue;

    // Check day range
    const [minDay, maxDay] = template.dayRange;
    if (state.daysSinceSignup < minDay || state.daysSinceSignup > maxDay) continue;

    // Check condition
    if (template.condition && !template.condition(state)) continue;

    // Don't schedule habit nudge if engagement is good
    if (template.type === 'habit_nudge' && state.engagementLevel === 'high') continue;

    // Generate message
    const messageTemplate =
      template.messageTemplates[Math.floor(Math.random() * template.messageTemplates.length)];
    const message = messageTemplate(state);

    // Determine persona
    const persona =
      template.persona === 'contextual' ? state.preferredPersona || 'ferni' : template.persona;
 ...
 */
export function isInOnboardingPeriod(userId: string): boolean {
  const state = onboardingStates.get(userId);
