...
import { checkCallCompliance, generateComplianceScript, } from '../../tools/domains/telephony/compliance.js';
...
class OnBehalfCallOrchestrator extends EventEmitter {
    ...
    // =========================================================================
    async createLiveKitRoom(callId, request, script) {
        ...
        try {
            ...
            const roomService = new RoomServiceClient(this.config.livekitUrl, this.config.livekitApiKey, this.config.livekitApiSecret);
    ...
    async spawnOnBehalfAgent(roomName, callId, request, script) {
        log.info({ roomName, callId, contactName: request.resolvedContact?.name }, '🤖 Dispatching on-behalf agent to room');
        try {
            ...
            const agentDispatch = new AgentDispatchClient(this.config.livekitUrl, this.config.livekitApiKey, this.config.livekitApiSecret);
            ...
            log.warn({ error: String(error), callId }, '⚠️ AgentDispatchClient failed, falling back to event emission');
     ...
     */
    async initiateLiveKitSipCall(callId, request, roomName) {
        const contact = request.resolvedContact;
        // Normalize phone number to E.164
        const cleanPhone = contact.phone.replace(/\D/g, '');
        const e164Phone = cleanPhone.startsWith('1') ? `+${cleanPhone}` : `+1${cleanPhone}`;
        log.info({ callId, to: this.maskPhone(e164Phone), roomName, sipTrunkId: this.config.sipTrunkId }, '📞 Initiating outbound call via LiveKit SIP');
        try {
            const { SipClient } = await import('livekit-server-sdk');
            const sipClient = new SipClient(this.config.livekitUrl, this.config.livekitApiKey, this.config.livekitApiSecret);
            // Create SIP participant - this makes LiveKit call out to the phone number
            const sipParticipant = await sipClient.createSipParticipant(this.config.sipTrunkId, e164Phone, roomName, {
            ...
            log.info({ callId, sipParticipantId: sipParticipant.participantId, roomName }, '✅ LiveKit SIP call initiated');
            // Track the call
    ...
    // =========================================================================
    async initiateTwilioCall(callId, request, roomName) {
        const contact = request.resolvedContact;
        ...
        log.debug({ callId, to: this.maskPhone(e164Phone), roomName }, 'Initiating Twilio call (fallback)');
        const response = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${this.config.twilioAccountSid}/Calls.json`, {
            method: 'POST',
            headers: {
    ...
    generateSipBridgeTwiml(roomName) {
...
<Response>
  <Connect>
     ...
     */
    async handleStatusCallback(callId, status, twilioData) {
     ...
     */
    async handleMachineDetection(callId, machineResult) {
        const call = activeCallsStore.get(callId);
        if (!call) {
            return null;
        }
...
<Response>
  <Say voice="Polly.Joanna">${this.escapeXml(voicemailMessage)}</Say>
     ...
     */
    async generateVoicemailMessageAsync(call) {
     ...
     */
    generateVoicemailMessageFallback(call) {
        const contact = call.request.resolvedContact;
    ...
    async notifyOriginalSession(call) {
        try {
...
export function getOnBehalfCallOrchestrator(config) {
...
//# sourceMappingURL=on-behalf-call-orchestrator.js.map