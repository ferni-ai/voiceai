...
import { publishOutreachTrigger, publishThinkingOfYouTrigger, publishEmotionalSupportTrigger, publishCommitmentTrigger, } from './trigger-publisher.js';
import { initiateGroupOutreach, teamCelebrationOutreach, fullTeamSupportOutreach, peterFerniInsightOutreach, } from '../conversation-thread/group-outreach.js';
 ...
 */
export async function onDreamBecameDormant(userId, dream) {
    if (dream.dormantDays < 30)
        return; // Wait at least 30 days
    const scheduledFor = new Date();
    scheduledFor.setDate(scheduledFor.getDate() + 7); // Follow up in a week
    try {
        await publishThinkingOfYouTrigger(userId, `Gentle nudge about dream: "${dream.title}" - hasn't been mentioned in ${dream.dormantDays} days`, {
            personaId: 'ferni',
            metadata: { dreamId: dream.id, dreamTitle: dream.title, dormantDays: dream.dormantDays },
        });
        log.info({ userId, dream: dream.title, dormantDays: dream.dormantDays }, '💭 Dream revival outreach scheduled');
    }
    catch (error) {
 ...
 */
export async function onDreamProgress(userId, dream) {
    // Celebrate milestones
    const milestones = [25, 50, 75, 100];
    const milestone = milestones.find((m) => dream.progressPercent >= m && dream.progressPercent < m + 10);
    if (!milestone)
        return;
    try {
 ...
 */
export async function onBurnoutRiskElevated(userId, assessment) {
    ...
    try {
        ...
        log.info({ userId, risk: assessment.risk, score: assessment.riskScore }, '🔥 Burnout support outreach triggered');
    }
    catch (error) {
 ...
 */
export async function onCapacityRecovered(userId, recovery) {
    try {
        await publishOutreachTrigger({
            userId,
            type: 'celebration',
 ...
 */
export async function onValuesConflictDetected(userId, conflict) {
    if (conflict.severity === 'low')
        return;
    const scheduledFor = new Date();
    scheduledFor.setHours(scheduledFor.getHours() + 24); // Give them a day
    try {
        await publishThinkingOfYouTrigger(userId, `Values reflection: ${conflict.values.join(' vs ')} in "${conflict.situation}"`, {
 ...
 */
export async function onValuesAligned(userId, alignment) {
    if (alignment.alignmentScore < 0.8)
        return; // Only celebrate strong alignment
    try {
        await publishOutreachTrigger({
            userId,
 ...
 */
export async function onCommitmentMade(userId, commitment) {
 ...
 */
export async function onCommitmentAtRisk(userId, commitment) {
    try {
        await publishOutreachTrigger({
            userId,
        ...
        log.info({ userId, commitment: commitment.summary, daysOverdue: commitment.daysOverdue }, '⚠️ Commitment risk outreach triggered');
    }
    catch (error) {
 ...
 */
export async function onImportantDateApproaching(userId, date) {
    if (date.daysUntil > 7)
        return; // Only within a week
    try {
        await publishOutreachTrigger({
            userId,
        ...
        log.info({ userId, date: date.name, daysUntil: date.daysUntil }, '📅 Important date reminder scheduled');
    }
    catch (error) {
 ...
 */
export async function onSeasonalPatternDetected(userId, pattern) {
    if (pattern.historicalOccurrences < 2)
        return; // Need at least 2 data points
    try {
        await publishThinkingOfYouTrigger(userId, `Seasonal awareness: "${pattern.pattern}" tends to happen in ${pattern.season}`, {
            personaId: 'ferni',
            metadata: pattern,
        });
        log.info({ userId, pattern: pattern.pattern, season: pattern.season }, '🍂 Seasonal awareness outreach scheduled');
    }
    catch (error) {
 ...
 */
export async function onLifeChapterTransition(userId, transition) {
    if (transition.significance === 'minor')
        return;
    const scheduledFor = new Date();
    scheduledFor.setDate(scheduledFor.getDate() + 3); // Give them time to settle
    try {
        await publishThinkingOfYouTrigger(userId, `Life chapter transition: Moving from "${transition.fromChapter}" to "${transition.toChapter}"`, {
            personaId: 'nayan',
            metadata: transition,
        });
        log.info({ userId, from: transition.fromChapter, to: transition.toChapter }, '📖 Life chapter transition outreach scheduled');
    }
    catch (error) {
 ...
 */
export async function onStrugglePredicted(userId, prediction) {
    ...
    try {
        ...
        log.info({ userId, prediction: prediction.type, confidence: prediction.confidence }, '🔮 Predictive outreach scheduled');
    }
    catch (error) {
 ...
 */
export async function onCrisisSignalsDetected(userId, crisis) {
    if (crisis.severity === 'low')
        return;
    try {
        await publishEmotionalSupportTrigger(userId, 'crisis', crisis.severity === 'critical' ? 1.0 : crisis.severity === 'high' ? 0.8 : 0.6, {
            personaId: 'ferni',
 ...
 */
export async function onRelationshipHealthDecline(userId, relationship) {
    if (relationship.daysSinceContact < 14)
        return; // Give it at least 2 weeks
    try {
        await publishThinkingOfYouTrigger(userId, `Relationship check-in: Haven't heard about ${relationship.personName} in a while`, {
            personaId: 'ferni',
 ...
 */
export async function onMoodPatternDetected(userId, pattern) {
    try {
        await publishThinkingOfYouTrigger(userId, `Mood pattern awareness: ${pattern.type} dip around ${pattern.lowPoint}`, {
            personaId: 'ferni',
            metadata: pattern,
        });
 ...
 */
export async function onPersistentLowEnergy(userId, energy) {
    if (energy.consecutiveDays < 3)
        return;
    try {
 ...
 */
export async function onRecoveryCheckIn(userId, recovery) {
    const daysToMilestone = recovery.nextMilestone - recovery.daysSober;
    if (daysToMilestone > 7 && daysToMilestone !== 1)
        return; // Only near milestones
    try {
        await publishOutreachTrigger({
            userId,
        ...
        log.info({ userId, days: recovery.daysSober, type: recovery.type }, '🎖️ Recovery milestone outreach scheduled');
    }
    catch (error) {
 ...
 */
export async function onNeedsTeamSupport(userId, situation) {
    try {
        let result;
        ...
        if (result.success) {
            log.info({ userId, type: situation.type, personas: result.personas }, '👥 Team support outreach initiated');
        }
        else {
 ...
 */
export async function onNeedsMultiplePerspectives(userId, insight) {
    try {
 ...
 */
export async function onCommitmentNeedsTeamSupport(userId, commitment) {
    ...
    try {
        ...
        if (result.success) {
            log.info({ userId, commitment: commitment.summary, daysOverdue: commitment.daysOverdue }, '👥 Team support for commitment initiated');
        }
    }
    catch (error) {
 ...
 */
export async function onNeedsTeamRoundtable(userId, roundtable) {
    try {
...
//# sourceMappingURL=superhuman-outreach-bridge.js.map