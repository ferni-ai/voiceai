 ...
 */
function getLocalHour(utcTime, timezone) {
    try {
 ...
 * Returns 0=Sun, 6=Sat
 */
function getLocalDayOfWeek(utcTime, timezone) {
    try {
        const formatter = new Intl.DateTimeFormat('en-US', {
            weekday: 'short',
            timeZone: timezone,
 ...
 */
function toUserLocalTime(utcTime, timezone) {
    return {
 ...
 */
function isWithinQuietHours(utcTime, timezone, quietHoursStart, // "22:00"
quietHoursEnd // "07:00"
) {
 ...
 */
async function loadTimingProfileFromFirestore(userId) {
    try {
 ...
 */
function persistTimingProfile(userId, profile) {
 ...
 */
function serializeTimingProfile(profile) {
    return {
        ...profile,
        engagementPatterns: {
            ...profile.engagementPatterns,
            responseRateByHour: Object.fromEntries(profile.engagementPatterns.responseRateByHour),
            responseRateByDay: Object.fromEntries(profile.engagementPatterns.responseRateByDay),
        },
    };
}
/**
 ...
 */
function deserializeTimingProfile(data) {
    if (!data.engagementPatterns)
        return data;
    return {
        ...data,
        engagementPatterns: {
            ...data.engagementPatterns,
            responseRateByHour: data.engagementPatterns.responseRateByHour instanceof Map
                ? data.engagementPatterns.responseRateByHour
                : new Map(Object.entries(data.engagementPatterns.responseRateByHour || {}).map(([k, v]) => [
                    parseInt(k),
                    v,
                ])),
            responseRateByDay: data.engagementPatterns.responseRateByDay instanceof Map
                ? data.engagementPatterns.responseRateByDay
                : new Map(Object.entries(data.engagementPatterns.responseRateByDay || {}).map(([k, v]) => [
                    parseInt(k),
                    v,
                ])),
        },
 ...
 */
async function loadTimezoneFromUserProfile(userId) {
    try {
        ...
        if (userProfile?.contactInfo?.timezone) {
            ...
            if (timingProfile) {
                ...
                log.debug({ userId, timezone: userProfile.contactInfo.timezone }, 'Loaded timezone from user profile');
            }
        }
    }
    catch (error) {
 ...
 */
export function updateTimingPreferences(userId, preferences) {
 ...
 */
export function addNeverDuringRule(userId, rule) {
    const profile = getTimingProfile(userId);
 ...
 * Add a busy period
 */
export function addBusyPeriod(userId, period) {
    const profile = getTimingProfile(userId);
 ...
 */
export function addRecurringEvent(userId, event) {
    const profile = getTimingProfile(userId);
 ...
 * Set work schedule
 */
export function setWorkSchedule(userId, schedule) {
    const profile = getTimingProfile(userId);
 ...
 * Set sleep pattern
 */
export function setSleepPattern(userId, pattern) {
    const profile = getTimingProfile(userId);
 ...
 */
export function recordInteraction(userId, data) {
    ...
    log.debug({ userId, channel: data.channel, gotResponse: data.gotResponse }, 'Recorded interaction');
}
/**
 * Update timing profile based on new interaction
 */
function updateProfileFromInteraction(userId, record) {
    const profile = getTimingProfile(userId);
    // Update totals
    ...
    profile.engagementPatterns.preferredHours = getTopHours(profile.engagementPatterns.responseRateByHour);
    // Update preferred days
    profile.engagementPatterns.preferredDays = getTopDays(profile.engagementPatterns.responseRateByDay);
...
function getTopHours(rateMap, count = 8) {
    if (rateMap.size === 0) {
...
function getTopDays(rateMap, count = 5) {
    if (rateMap.size === 0) {
 ...
 */
export function calculateOptimalTime(userId, context) {
    const profile = getTimingProfile(userId);
    const now = new Date();
    ...
    if (isWithinQuietHours(now, userTimezone, profile.preferences.quietHoursStart, profile.preferences.quietHoursEnd)) {
 ...
 */
export function isGoodTimeForOutreach(userId, time, context) {
    const profile = getTimingProfile(userId);
...
// SCORING LOGIC
// ============================================================================
function scoreTime(time, profile, context) {
...
function isTimeBlocked(time, profile, context) {
    const hour = time.getHours();
    const day = time.getDay();
...
function isInQuietHours(time, startStr, endStr) {
    const hour = time.getHours();
...
function isInTimePeriod(time, period) {
    const hour = time.getHours();
    switch (period) {
        case 'morning':
...
// HELPER FUNCTIONS
// ============================================================================
function getPriorityWindow(priority) {
    switch (priority) {
        case 'urgent':
...
function generateCandidateTimes(from, windowMs, profile) {
    const candidates = [];
...
function getFirstAvailableTime(from, profile) {
...
function generateReasoning(time, score, profile) {
    const hour = time.getHours();
    const day = time.getDay();
 ...
 * ```
 */
export async function checkCalendarBeforeOutreach(userId, priority) {
    try {
        // Dynamic import to avoid circular dependencies
        const { isUserBusy, getCalendarBusyProfile } = await import('../scheduling/calendar-busy-detection.js');
 ...
 */
export async function calculateOptimalTimeWithCalendar(userId, context) {
...
// CLEANUP
// ============================================================================
export function clearUserTimingData(userId) {
