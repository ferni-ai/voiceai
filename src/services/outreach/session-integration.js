...
import { recordTimingInteraction, updateEmotionalState as updateContextEmotionalState, updateUserContext, } from './index.js';
import { publishCommitmentTrigger, publishEmotionalSupportTrigger, publishMilestoneTrigger, publishOutreachTrigger, } from './trigger-publisher.js';
import { checkStreaksAtRisk, checkMilestonesToCelebrate, publishStreakProtectionAlert, publishMilestoneCelebration, } from './maya-habit-outreach.js';
...
function extractCommitments(text) {
    ...
    for (const pattern of COMMITMENT_PATTERNS) {
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            if (!what || what.length < 5 || what.length > 200)
                continue;
            ...
            const cleanedWhat = what
                ...
                .trim();
            if (seenCommitments.has(cleanedWhat.toLowerCase()))
                continue;
...
function mapToContextEmotionalState(state) {
    switch (state) {
        case 'excited':
            return 'thriving';
        case 'happy':
...
function extractWinsAndStruggles(text) {
    const wins = [];
    const struggles = [];
 ...
 */
export async function analyzeSessionForOutreach(data) {
    ...
    for (const commitment of commitments) {
        try {
            const result = await publishCommitmentTrigger(userId, commitment.what, commitment.checkInDate, { sessionId: data.sessionId, personaId });
            if (result.success)
                triggersCreated++;
        }
        catch (error) {
    ...
    if (emotionalState &&
        ...
        try {
            ...
            const result = await publishEmotionalSupportTrigger(userId, emotionalState, 0.7, // Intensity above threshold since we matched the emotion
    ...
    try {
        ...
        await recordPredictiveSignals(userId, `session-${Date.now()}`, transcript, {
            ...
            satisfactionSignal: satisfaction === 'positive'
                ? 'positive'
    ...
    try {
        ...
        if (trustResult.triggersCreated > 0) {
            log.info({ userId, trustTriggers: trustResult.triggersCreated, types: trustResult.triggerTypes }, '🧠 Trust-based outreach triggers created');
        }
    }
    catch (error) {
    ...
    log.info({ userId, commitmentsFound: commitments.length, triggersCreated }, 'Session analyzed for outreach');
    return {
 ...
 */
export function analyzeMessageForContext(userId, message, role) {
    if (role !== 'user' || !userId || userId === 'anonymous')
        return;
 ...
 */
async function analyzeMayaHabitSession(userId, sessionId) {
    const result = {
        triggersCreated: 0,
        streaksAtRisk: 0,
        milestonesFound: 0,
    ...
    try {
        ...
        for (const milestone of milestones.slice(0, 2)) {
            const sent = await publishMilestoneCelebration(userId, milestone.habitId, milestone.habitName, milestone.days);
            if (sent) {
...
// EXPORT
// ============================================================================
export { analyzeMayaHabitSession, detectEmotionalState, extractCommitments, extractWinsAndStruggles, };
