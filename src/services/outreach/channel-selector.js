...
// STORAGE
// ============================================================================
const channelProfileStore = new Map();
// ============================================================================
// CHANNEL SELECTOR SERVICE
// ============================================================================
 ...
 */
export function getChannelProfile(userId) {
 ...
 */
async function loadChannelProfileFromFirestore(userId) {
    try {
 ...
 */
function persistChannelProfile(userId, profile) {
 ...
 */
export function updateChannelPreferences(userId, preferences) {
 ...
 */
export function updateContactAvailability(userId, availability) {
    const profile = getChannelProfile(userId);
 ...
 */
export function updateRelationshipStage(userId, stage) {
    const profile = getChannelProfile(userId);
 ...
 */
export function recordOutreachOutcome(userId, data) {
    const profile = getChannelProfile(userId);
    // Update totals
    ...
    if (data.userSatisfaction) {
        const satisfactionDelta = data.userSatisfaction === 'positive' ? 0.1 : data.userSatisfaction === 'negative' ? -0.1 : 0;
        const currentSatisfaction = profile.learning.satisfactionScores[data.channel];
        profile.learning.satisfactionScores[data.channel] = Math.max(0, Math.min(1, currentSatisfaction + satisfactionDelta));
    }
    channelProfileStore.set(userId, profile);
    log.debug({
        userId,
 ...
 */
export function selectChannel(userId, context) {
    const profile = getChannelProfile(userId);
...
function getAvailableChannels(profile) {
...
function scoreChannel(channel, profile, context, _timingProfile) {
...
function getTimeAppropriateness(channel, timeOfDay, isWorkHours) {
...
function generateChannelReason(channel, context) {
    const reasons = [];
    // Content type reason
    const idealChannels = CONTENT_CHANNEL_MAP[context.contentType];
 ...
 */
export function getContentTypeFromTrigger(triggerType) {
    const mapping = {
 ...
 * Check if work hours
 */
export function isWorkHours(date = new Date()) {
    const hour = date.getHours();
 ...
 */
export const CHANNEL_SEQUENCES = {
 ...
 */
export function getRecommendedSequence(triggerType, priority) {
...
// CLEANUP
// ============================================================================
export function clearUserChannelData(userId) {
