 ...
 */
export async function checkForMemoryBasedOutreach(
  userId: string,
  sessionId: string
): Promise<SuperhumanOutreachTrigger[]> {
  const triggers: SuperhumanOutreachTrigger[] = [];
  ...
  try {
    ...
    for (const memory of memories) {
      if (!memory.expectedFollowUpAt) continue;
      if (memory.surfaced) continue; // Already addressed
      ...
      if (daysSinceDue >= 0 && daysSinceDue <= 3) {
        ...
        let priority: SuperhumanOutreachTrigger['priority'] = 'medium';
    ...
    logger.debug(
      { userId, triggerCount: triggers.length },
      '🧠 Checked for memory-based outreach triggers'
    );
    return triggers;
  } catch (error) {
 ...
 */
export function convertToOutreachTrigger(
  trigger: SuperhumanOutreachTrigger
): Omit<OutreachTrigger, 'id' | 'createdAt'> {
  const priorityMap: Record<
    SuperhumanOutreachTrigger['priority'],
  ...
  const typeMap: Record<SuperhumanOutreachTrigger['type'], string> = {
  ...
  return {
    type: typeMap[trigger.type] as OutreachTrigger['type'],
 ...
 */
export async function syncMemoriesToOutreachContext(
  userId: string,
  sessionId: string
): Promise<void> {
  try {
      ...
      switch (memory.type) {
        ...
        case 'struggle':
          ...
          if (!context.progress.currentStruggles.some((s) => s.description === memory.content)) {
            ...
            logger.debug(
              { userId, struggle: memory.content },
              'Synced struggle to outreach context'
            );
          }
          break;
      }
 ...
 */
export async function processConcernForOutreach(
  userId: string,
  concernLevel: 'none' | 'mild' | 'moderate' | 'elevated' | 'crisis',
  concernType: string
): Promise<SuperhumanOutreachTrigger | null> {
 ...
 */
export async function scheduleSuperhunmanOutreach(
  triggers: SuperhumanOutreachTrigger[]
): Promise<void> {
  ...
  for (const trigger of triggers) {
    try {
      ...
      logger.debug(
        { userId: trigger.userId, type: trigger.type },
        '🧠 Superhuman outreach trigger evaluated'
      );
    } catch (error) {
