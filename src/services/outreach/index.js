...
// Import sub-modules
import { getOutreachDecisionEngine, startOutreachDecisionEngine, stopOutreachDecisionEngine, } from './decision-engine.js';
import { generateCallOpening, generateEmailMessage, generateOutreach, generateTextMessage, generateVoicemailMessage, getPersonaOutreachVoice, personaOutreachVoices, selectPersonaForOutreach, } from './persona-voice-generator.js';
import { getThinkingOfYouEngine, ThinkingOfYouEngine, } from './thinking-of-you.js';
import { getConversationalCallService, isConversationalCallsConfigured, makeConversationalCall, } from './conversational-calls.js';
import { addBusyPeriod, addNeverDuringRule, addRecurringEvent, calculateOptimalTime, calculateOptimalTimeWithCalendar, checkCalendarBeforeOutreach, clearUserTimingData, getTimingProfile, isGoodTimeForOutreach, recordInteraction as recordTimingInteraction, setSleepPattern, setWorkSchedule, updateTimingPreferences, } from './timing-intelligence.js';
import { addCommitment, addInsideJoke, addLifeEvent, addOngoingEvent, addOpenLoop, addSignificantMoment, addStruggle, addWin, clearUserContext, getContextForOutreach, getFollowUpItems, getUserContext, needsSupport, pruneOldData, recordConversation, resolveOpenLoop, resolveStruggle, updateCommitmentStatus, updateEmotionalState, updatePersonalInfo, } from './context-aggregator.js';
import { CHANNEL_SEQUENCES, clearUserChannelData, getChannelProfile, getContentTypeFromTrigger, getRecommendedSequence, getTimeOfDay, isWorkHours, recordOutreachOutcome, selectChannel, updateChannelPreferences, updateRelationshipStage as updateChannelRelationshipStage, updateContactAvailability, } from './channel-selector.js';
import { adaptMessage, addNickname, addInsideJoke as addRelationshipInsideJoke, addSharedReference, calculateStage, canDoAction, clearRelationshipData, getMessageAdjustment, getRandomReference, getReferenceableMoment, getRelationshipProfile, getToneAdjustment, recordConversation as recordRelationshipConversation, recordSignificantMoment, updateCommunicationStyle, updateStage, } from './relationship-adapter.js';
// Session Integration
import { analyzeMessageForContext, analyzeSessionForOutreach, detectEmotionalState, extractCommitments, extractWinsAndStruggles, } from './session-integration.js';
// Maintenance
import { cleanupDeadTriggers, getMaintenanceStats, pruneOutreachHistory, resetUserData, resetWeeklyCounters, startMaintenanceScheduler, stopMaintenanceScheduler, updateMaintenanceConfig, } from './maintenance.js';
// Analytics & Learning
import { getBetterThanHumanTelemetry } from '../analytics/better-than-human-telemetry.js';
import { analytics, calculateGlobalAnalytics, calculatePeriodAnalytics, calculateUserAnalytics, clearUserAnalyticsData, exportAnalyticsData, getRecommendations, predictResponseLikelihood, pruneOldAnalyticsData, recordOutreachEvent, recordResponseEvent, } from './analytics.js';
// Voice Synthesis
import { cleanupOldVoiceMessages, generateCallGreeting, generateVoicemail, generateVoiceMessage, initializeVoiceSynthesis, isVoiceSynthesisAvailable, voiceSynthesis, } from './voice-synthesis.js';
// SIP Bridge (Twilio → LiveKit)
import { createOutboundCallRoom, endCall, generateCallConnectTwiML, generateVoicemailTwiML, getActiveSessions, getCallSession, handleCallStatus, handleMachineDetection, initializeSIPBridge, initiateConversationalCall, isSIPBridgeAvailable, markSessionInConversation, sipBridge, } from './sip-bridge.js';
// Firestore Persistence
import { cleanupOldTriggers, deleteAllUserOutreachData, deleteOutreachProfile, deleteTrigger, deleteUserContext, deleteUserHistory, getOutreachStats, initializeFirestore, isFirestoreAvailable, loadAllPendingTriggers, loadContext, loadHistory, loadOutreachProfile, loadPendingTriggers, saveContext, saveOutreachProfile, saveToHistory, saveTrigger, updateTriggerStatus, } from './firestore-persistence.js';
// Delivery Services
import { calculateDeliveryStats, deliveryTracker, emailDelivery, initializeDeliveryServices, pushNotifications, queueDelivery, sendEmail, sendEmailWithRetry, sendPushNotification, sendSMS, sendSMSWithRetry, shutdownDeliveryServices, smsDelivery, } from './delivery/index.js';
// Webhook Handlers
import { emailWebhooks, initializeWebhooks, onInboundMessage, twilioWebhooks, } from './webhooks/index.js';
// A/B Testing
import { abTesting, calculateTestResults, completeTest, createChannelTest, createMessageTest, createPersonaTest, createTest, createTimingTest, getAllTests, getTest, getVariantForUser, pauseTest, recordConversion, startTest, } from './ab-testing/index.js';
// ============================================================================
// INITIALIZATION
// ============================================================================
 ...
 */
export async function initializeOutreachSystem() {
    if (initialized) {
 ...
 */
export function shutdownOutreachSystem() {
 ...
 */
async function handleOutreachDelivery(decision) {
    ...
    try {
        ...
        if (delivered) {
            ...
            const toOutreachType = (t) => {
                if (t === 'thinking_of_you')
                    return 'thinking_of_you';
                if (t === 'commitment_check')
                    return 'commitment_check';
                if (t === 'celebration' || t === 'streak_celebration')
                    return 'celebration';
 ...
 * ```
 */
export function triggerOutreach(trigger) {
    const engine = getOutreachDecisionEngine();
 ...
 */
export function updateOutreachPreferences(userId, preferences) {
    const engine = getOutreachDecisionEngine();
 ...
 */
export function updateUserContext(userId, context) {
    const engine = getOutreachDecisionEngine();
 ...
 */
export function registerUserForOutreach(userId, _relationshipStartDate) {
 ...
 */
export function getPendingOutreach(userId) {
    const engine = getOutreachDecisionEngine();
 ...
 */
export function cancelOutreach(triggerId) {
    const engine = getOutreachDecisionEngine();
 ...
 */
export function getOutreachHistory(userId, limit = 20) {
    const engine = getOutreachDecisionEngine();
 ...
 */
export async function triggerThinkingOfYou(userId, _trigger, reason) {
...
// ============================================================================
// Decision Engine
export { getOutreachDecisionEngine, startOutreachDecisionEngine, stopOutreachDecisionEngine, };
...
export { getConversationalCallService, isConversationalCallsConfigured, makeConversationalCall, };
// Timing Intelligence
...
// 🌱 Maya Habit Outreach
export { checkStreaksAtRisk, checkMilestonesToCelebrate, checkSetbackRecoveryNeeded, generateWeeklyReviewData, publishStreakProtectionAlert, publishMilestoneCelebration, publishWeeklyReviewTrigger, publishSetbackRecoveryTrigger, MAYA_STREAK_PROTECTION_MESSAGES, MAYA_MILESTONE_MESSAGES, MAYA_WEEKLY_REVIEW_MESSAGES, MAYA_SETBACK_MESSAGES, MAYA_HABIT_OUTREACH_CONFIG, } from './maya-habit-outreach.js';
// 🧠 Trust-Based Outreach Bridge ("Better than Human")
export { evaluateTrustBasedOutreach, handleConcernDetection, shouldAvoidOutreachTopic, runTrustBasedOutreachBatch, } from './trust-outreach-bridge.js';
// 🌊 Life Rhythm Prediction Outreach
export { evaluateLifeRhythmOutreach, triggerLifeRhythmOutreach, generateLifeRhythmMessage, runDailyLifeRhythmOutreach, } from './life-rhythm-outreach.js';
// 🔔 Pattern-Based Proactive Outreach (Better Than Human)
export { schedulePatternOutreach, schedulePatternOutreachAsync, scheduleSundayAnxietyFollowUp, scheduleWorkStressFollowUp, scheduleRelationshipCheckIn, PATTERN_OUTREACH_MAP, } from './pattern-outreach-integration.js';
// ============================================================================
// DEFAULT EXPORT
// ============================================================================
export default {
