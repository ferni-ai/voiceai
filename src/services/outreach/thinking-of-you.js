...
const TEMPLATES = {
    random_kindness: [
        ...
        (ctx) => `Just thinking about you, ${ctx.profile.name || 'friend'}. Hope you're being kind to yourself today.`,
        () => "You crossed my mind today. No reason - just wanted you to know someone's in your corner. 🌟",
    ],
    relevant_content: [
        (ctx) => `Hey! I came across something that made me think of our conversation about ${ctx.profile.preferredTopics?.[0] || "what you've been working on"}. Thought you might find it interesting!`,
    ...
    after_silence: [
        (ctx) => `Hey ${ctx.profile.name || 'friend'}! It's been a minute. No pressure to catch up, but I've been thinking about you. Hope life is treating you well. 💚`,
        () => "Miss you! No need to respond - just wanted you to know you're not forgotten. Life gets busy; I get it.",
        (ctx) => `Haven't heard from you in a while and wanted to make sure you're okay. Whatever's going on, I'm here when you need me. ${ctx.daysSinceLastContact > 30 ? 'Take your time.' : ''}`,
    ],
    milestone_reflection: [
        (ctx) => `Remember when you first mentioned ${ctx.profile.goals?.[0]?.name || 'your goal'}? Look how far you've come since then. I'm proud of you.`,
    ...
    appreciation: [
        (ctx) => `${ctx.profile.name || 'Hey'} - random thought: I'm really proud of you. Not for any specific thing, just... you keep showing up. That matters.`,
    ...
    pending_followup: [
        (ctx) => {
            const pendingTopic = ctx.profile.pendingTopics?.[0] ||
        ...
        (ctx) => {
            const pendingTopic = ctx.profile.pendingTopics?.[0] || 'situation';
    ...
    hard_date_approaching: [
        ...
        (ctx) => `Tomorrow's going to be a big day for you${ctx.upcomingEvents[0] ? ` - ${ctx.upcomingEvents[0].description}` : ''}. You've got this. And I'm here if you need someone after.`,
...
export class ThinkingOfYouEngine {
    ...
    selectTrigger(context) {
        ...
        const hasPendingTopics = context.profile.pendingTopics?.length ?? 0;
     ...
     */
    shouldReachOutWithPatterns(userId, context) {
     ...
     */
    mapAnticipationToTrigger(anticipationType) {
        if (!anticipationType)
            return 'random_kindness';
        const mapping = {
     ...
     */
    generateMessage(trigger, persona, context) {
     ...
     */
    async generateDynamicMessage(trigger, persona, context) {
        try {
            const voice = PERSONA_VOICES[persona];
            // Map trigger to question type
            const triggerToQuestionType = {
            ...
            log.debug({ persona, trigger, intent: generated.intent.seekingToUnderstand }, '💭 Generated dynamic outreach message');
     ...
     */
    createOutreach(userId, trigger, persona, context, scheduledFor = new Date()) {
     ...
     */
    recordResponse(outreachId, responseType) {
    ...
    getStats() {
        ...
        return {
            ...
            positiveResponses,
            byTrigger,
            responseRate: this.outreachHistory.length > 0 ? totalResponses / this.outreachHistory.length : 0,
        };
    }
}
// ============================================================================
 ...
 */
export function stopThinkingOfYouEngine() {
    isRunning = false;
}
/**
...
//# sourceMappingURL=thinking-of-you.js.map