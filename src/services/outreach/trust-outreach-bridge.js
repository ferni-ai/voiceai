...
import { getDueMoments, generateThinkingOfYouMoments, generateRandomWarmth, markMomentSent, } from '../trust-systems/thinking-of-you.js';
import { getUncelebratedWins, generateCelebration, getOverdueIntentions, generateIntentionFollowUp, } from '../trust-systems/small-wins.js';
import { getUnreflectedGrowth, generateGrowthReflection, } from '../trust-systems/growth-reflection.js';
import { detectUnsaidSignals, getAvoidedTopics, } from '../trust-systems/reading-between-lines.js';
import { getProactiveRememberWhen } from '../trust-systems/our-songs.js';
import { checkBoundary, getActiveBoundaries, } from '../trust-systems/boundary-memory.js';
 ...
 */
export async function evaluateTrustBasedOutreach(userId, sessionId) {
    const result = {
        triggersCreated: 0,
        triggerTypes: [],
        skipped: [],
...
// ============================================================================
async function processThinkingOfYouMoments(userId, result) {
    ...
    for (const moment of dueMoments) {
        ...
        const hasBoundaryIssue = boundaries.some((b) => underlying.toLowerCase().includes(b.topic.toLowerCase()));
        if (hasBoundaryIssue) {
            result.skipped.push({
...
// ============================================================================
async function processUncelebratedWins(userId, result) {
    ...
    for (const win of uncelebrated) {
        ...
        const priority = celebration.intensity === 'big' ? 'high' : 'medium';
        const published = await publishOutreachTrigger({
            userId,
            type: 'celebration',
            priority,
...
// ============================================================================
async function processUnreflectedGrowth(userId, result) {
...
// ============================================================================
async function processOverdueIntentions(userId, result) {
    ...
    for (const intention of overdue) {
        ...
        const boundaries = getActiveBoundaries(userId);
        const wouldViolateBoundary = boundaries.some((b) => intention.intention.toLowerCase().includes(b.topic.toLowerCase()));
        ...
        const daysOverdue = Math.floor((Date.now() - new Date(targetTime).getTime()) / (1000 * 60 * 60 * 24));
...
// OUR SONGS MEMORIES
// ============================================================================
async function processOurSongsMemories(userId, result) {
...
// ============================================================================
async function processLifeRhythmPredictions(userId, result) {
 ...
 */
export async function handleConcernDetection(context) {
    ...
    const significantSignal = unsaidSignals.find((s) => s.type === 'minimizing_pain' || s.type === 'false_closure' || s.type === 'emotional_mismatch');
    // Calculate priority
    const priority = concernLevel === 'crisis' ? 'urgent' : concernLevel === 'elevated' ? 'high' : 'medium';
 ...
 */
export function shouldAvoidOutreachTopic(userId, topic) {
    ...
    const isAvoided = avoidedTopics.some((avoidedTopic) => topic.toLowerCase().includes(avoidedTopic.toLowerCase()));
    if (isAvoided) {
        return {
            avoid: true,
 ...
 */
function mapTriggerType(superhumanType) {
    const mapping = {
 ...
 */
export async function runTrustBasedOutreachBatch(userIds) {
    let processed = 0;
    let totalTriggers = 0;
    const byType = {};
...
//# sourceMappingURL=trust-outreach-bridge.js.map