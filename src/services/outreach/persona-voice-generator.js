...
// ============================================================================
export const personaOutreachVoices = {
    ferni: {
        personaId: 'ferni',
        displayName: 'Ferni',
        tone: {
 ...
 */
export function getPersonaOutreachVoice(personaId) {
 ...
 */
function selectPhrase(phrases) {
    return phrases[Math.floor(Math.random() * phrases.length)];
}
/**
 * Get appropriate emoji based on persona and tone
 */
function getEmoji(voice, tone) {
    if (voice.textStyle.emojiUse === 'none')
        return '';
    const { preferredEmoji } = voice.textStyle;
    if (preferredEmoji.length === 0)
        return '';
 ...
 */
function adjustForRelationship(message, voice, stage, context) {
 ...
 */
export function generateTextMessage(personaId, context, tone) {
 ...
 */
export function generateEmailMessage(personaId, context, tone) {
    const voice = getPersonaOutreachVoice(personaId);
    const name = context.preferredName || context.userName;
    let subject = '';
    let body = '';
 ...
 */
function generateVoicemailOpening(name, displayName, stage) {
    switch (stage) {
        case 'new':
 ...
 */
function generatePersonalContext(context) {
    const { recentTopics, recentWins, currentStruggles, lastConversationSummary, upcomingEvents } = context.context;
 ...
 */
function generateVoicemailClose(displayName, stage, tone) {
 ...
 */
function generateVoicemailMainMessage(context, voice, hasPersonalContext) {
 ...
 */
function assembleVoicemail(components) {
 ...
 */
export function generateVoicemailMessage(personaId, context, tone) {
    const voice = getPersonaOutreachVoice(personaId);
    const name = context.preferredName || context.userName;
 ...
 */
export function generateWarmIntroductionVoicemail(personaId, name, stage = 'new') {
    const voice = getPersonaOutreachVoice(personaId);
    switch (stage) {
        case 'new':
            return (`Hey ${name}, this is ${voice.displayName}. ` +
                ...
                `Take care.`);
        case 'building':
            return (`Hey ${name}! It's ${voice.displayName}. ` +
 ...
 */
export function generateCallOpening(personaId, context) {
    const voice = getPersonaOutreachVoice(personaId);
    const name = context.preferredName || context.userName;
 ...
 */
export function generateOutreach(personaId, context, channel, tone = 'casual') {
    const voice = getPersonaOutreachVoice(personaId);
    let message = '';
    let subject;
 ...
 */
export function selectPersonaForOutreach(triggerType, lastPersonaId, wasRecentConversation) {
...
//# sourceMappingURL=persona-voice-generator.js.map