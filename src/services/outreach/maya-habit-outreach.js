 ...
 */
export async function checkStreaksAtRisk(userId) {
    try {
 ...
 */
export async function publishStreakProtectionAlert(context) {
    ...
    try {
        ...
        if (result.success) {
            log.info({ userId, habitName, streakDays, triggerId: result.triggerId }, 'Streak protection alert published');
        }
 ...
 */
export async function checkMilestonesToCelebrate(userId) {
    try {
        const store = getProductivityStore();
        await store.loadUserData(userId);
        const habits = store.getUserHabits(userId).filter((h) => h.isActive);
 ...
 */
export async function publishMilestoneCelebration(userId, habitId, habitName, days) {
    try {
        const result = await publishOutreachTrigger({
            userId,
 ...
 */
export async function generateWeeklyReviewData(userId) {
    try {
        const store = getProductivityStore();
        await store.loadUserData(userId);
        const habits = store.getUserHabits(userId).filter((h) => h.isActive);
        ...
        // This week's logs
        const thisWeekLogs = logs.filter((l) => l.completed && new Date(l.date) >= weekAgo && new Date(l.date) <= today);
        // Last week's logs (for comparison)
        const lastWeekLogs = logs.filter((l) => l.completed && new Date(l.date) >= twoWeeksAgo && new Date(l.date) < weekAgo);
        // Calculate stats
 ...
 */
export async function publishWeeklyReviewTrigger(userId) {
    try {
        ...
        if (result.success) {
            log.info({ userId, completionRate: reviewData.completionRate, triggerId: result.triggerId }, 'Weekly review trigger published');
        }
        return result.success;
    }
    catch (error) {
 ...
 */
export async function checkSetbackRecoveryNeeded(userId) {
    try {
        const store = getProductivityStore();
        await store.loadUserData(userId);
        const habits = store.getUserHabits(userId).filter((h) => h.isActive);
 ...
 */
export async function publishSetbackRecoveryTrigger(userId, habitId, habitName, daysMissed, previousStreak) {
    try {
        const result = await publishOutreachTrigger({
            userId,
            type: 'habit_check',
        ...
        if (result.success) {
            log.info({ userId, habitName, daysMissed, previousStreak, triggerId: result.triggerId }, 'Setback recovery trigger published');
        }
        return result.success;
    }
    catch (error) {
 ...
 */
export const MAYA_STREAK_PROTECTION_MESSAGES = [
 ...
 */
export const MAYA_MILESTONE_MESSAGES = {
    7: [
 ...
 */
export const MAYA_SETBACK_MESSAGES = [
...
//# sourceMappingURL=maya-habit-outreach.js.map