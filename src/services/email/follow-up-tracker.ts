...
export class FollowUpTracker {
  ...
  private contactPatterns: Map<string, ContactResponsePattern> = new Map();
  ...
  recordResponse(
    followUpId: string,
    responseEmailId: string,
    responseReceivedAt: Date = new Date()
  ): FollowUp | null {
  ...
  closeFollowUp(
    followUpId: string,
    reason: 'manually_closed' | 'no_longer_needed'
  ): FollowUp | null {
    const followUp = this.followUps.get(followUpId);
    if (!followUp) {
      return null;
   ...
   */
  getLowResponseContacts(threshold: number = 0.3): ContactResponsePattern[] {
...
const instances: Map<string, FollowUpTracker> = new Map();
...
export function resetFollowUpTracker(userId: string): void {
