...
export class EmailIntelligence {
    ...
    scoreEmail(email) {
        ...
        const priorityScore = Math.round(signals.senderReputation * WEIGHTS.sender +
        ...
        // Determine urgency
        const urgencyScore = Math.round((signals.subjectUrgency * 0.5 + signals.timeRelevance * 0.3 + signals.senderReputation * 0.2));
        // Suggested action
     ...
     */
    getTopPriorityEmails(scores, limit = 5) {
        return scores
    ...
    scorePersonalConnection(email, profile) {
        let score = 40;
        if (profile.isPersonal)
            score += 30;
        if (profile.responseRate > 0.5)
            score += 20;
        if (profile.isVip)
            score += 30;
    ...
    // CLASSIFICATION
    // ==========================================================================
    determinePriority(score, email, profile) {
    ...
    suggestAction(priority, urgencyScore, category) {
    ...
    estimateResponseTime(priority, urgencyScore) {
     ...
     */
    updateSenderProfile(email, update) {
    ...
    createDefaultProfile(email) {
        ...
        const isAutomatic = ['noreply', 'no-reply', 'donotreply', 'mailer'].some((n) => email.includes(n));
     ...
     */
    getAutoArchiveCandidates(scores) {
        return scores.filter((s) => s.suggestedAction === 'archive' ||
     ...
     */
    getUnsubscribeCandidates(scores) {
        return scores.filter((s) => s.category === 'newsletters' ||
...
export function getEmailIntelligence(userId) {
...
export function resetEmailIntelligence(userId) {
    instances.delete(userId);
}
//# sourceMappingURL=email-intelligence.js.map