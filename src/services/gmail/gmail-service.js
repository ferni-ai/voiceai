 ...
 */
function parseEmailAddress(fullAddress) {
 ...
 */
export async function isGmailConfigured(userId) {
 ...
 * Get inbox messages
 */
export async function getInboxMessages(userId, options = {}) {
    ...
    try {
        // Get message IDs
        const listResponse = await gmailRequest(accessToken, endpoint);
        if (!listResponse.messages || listResponse.messages.length === 0) {
            return [];
        }
        // Fetch full message details (batch to avoid too many requests)
        const messages = await Promise.all(listResponse.messages.slice(0, maxResults).map(async (m) => {
            const msg = await gmailRequest(accessToken, `/messages/${m.id}?format=metadata&metadataHeaders=From&metadataHeaders=To&metadataHeaders=Subject&metadataHeaders=Date`);
 ...
 * Get unread messages
 */
export async function getUnreadMessages(userId, maxResults = 10) {
 ...
 */
export async function getImportantUnread(userId, maxResults = 5) {
    return getInboxMessages(userId, {
        maxResults,
 ...
 */
export async function getInboxStats(userId) {
    const accessToken = await getValidAccessToken(userId);
    if (!accessToken) {
        return null;
    }
    try {
        // Get unread count
        const unreadResponse = await gmailRequest(accessToken, '/messages?maxResults=100&labelIds=INBOX,UNREAD');
        // Get important unread
        const importantResponse = await gmailRequest(accessToken, '/messages?maxResults=100&labelIds=INBOX,UNREAD,IMPORTANT');
        // Get today's messages
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const todayQuery = `after:${Math.floor(today.getTime() / 1000)}`;
        const todayResponse = await gmailRequest(accessToken, `/messages?maxResults=100&labelIds=INBOX&q=${encodeURIComponent(todayQuery)}`);
        return {
 ...
 */
export async function getEmailThread(userId, threadId) {
    const accessToken = await getValidAccessToken(userId);
    if (!accessToken) {
        return null;
    }
    try {
        const thread = await gmailRequest(accessToken, `/threads/${threadId}?format=metadata&metadataHeaders=From&metadataHeaders=To&metadataHeaders=Subject&metadataHeaders=Date`);
 ...
 */
export async function triageUnreadEmails(userId) {
 ...
 */
export function formatInboxSummaryForSpeech(stats, urgent) {
    let summary = '';
 ...
 */
function createRawEmail(from, options) {
 ...
 */
export async function createDraft(userId, options) {
    const accessToken = await getValidAccessToken(userId);
    if (!accessToken) {
    ...
    try {
        ...
        const draft = await gmailRequest(accessToken, '/drafts', {
            method: 'POST',
            body: JSON.stringify({
                message: {
                    raw,
...
export async function sendEmail(userId, options) {
    ...
    try {
        ...
        const raw = createRawEmail(fromEmail, options);
        const result = await gmailRequest(accessToken, '/messages/send', {
            method: 'POST',
            body: JSON.stringify({
                raw,
                threadId: options.threadId,
 ...
 */
export async function replyToThread(userId, threadId, body, options = {}) {
    const accessToken = await getValidAccessToken(userId);
    if (!accessToken) {
        return { success: false, error: 'Not authenticated' };
    }
 ...
 */
export async function updateDraft(userId, draftId, options, sendNow = false) {
    const accessToken = await getValidAccessToken(userId);
    if (!accessToken) {
        return { success: false, error: 'Not authenticated' };
    }
    try {
        ...
        if (sendNow) {
            // Send the draft
            const result = await gmailRequest(accessToken, `/drafts/${draftId}/send`, {
                method: 'POST',
            });
 ...
 * List user's drafts
 */
export async function listDrafts(userId, maxResults = 10) {
    const accessToken = await getValidAccessToken(userId);
    if (!accessToken)
        return [];
    try {
        const result = await gmailRequest(accessToken, `/drafts?maxResults=${maxResults}`);
 ...
 */
export async function generateEmailSuggestions(userId) {
