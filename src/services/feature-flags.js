 ...
 */
export async function setFlag(flagId, config) {
 ...
 */
export async function setUserOverride(flagId, userId, enabled) {
    const config = getFlag(flagId);
 ...
 */
export async function removeUserOverride(flagId, userId) {
    const config = getFlag(flagId);
 ...
 */
export async function setRolloutPercentage(flagId, percentage) {
 ...
 */
export async function enableAllTrustFlags() {
    for (const flagId of Object.keys(TRUST_FLAGS)) {
        await setFlag(flagId, { enabled: true, percentage: 100 });
    }
 ...
 */
export async function disableAllTrustFlags() {
    for (const flagId of Object.keys(TRUST_FLAGS)) {
        await setFlag(flagId, { enabled: false, percentage: 0 });
    }
 ...
 */
export function withFlag(flagId, userId, callback, fallback) {
 ...
 */
export async function withFlagAsync(flagId, userId, callback, fallback) {
    if (isEnabled(flagId, userId)) {
        return callback();
    }
    return fallback;
 ...
 */
export function getSimpleUtilitiesConfig() {
    return {
        timers: true,
        tips: true,
        timezone: true,
...
export function getFeatureFlags() {
    return {
        ...
        getFlag: (flagId) => getFlag(flagId),
        createFlag: async (flag) => setFlag((flag.id || ''), {
