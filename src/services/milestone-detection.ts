...
function checkAnniversary(
  personaId: string,
  firstInteraction: Date | undefined
): Milestone | null {
 ...
 */
export function checkHabitStreak(
  personaId: string,
  streakDays: number
): Milestone | null {
...
export function getMilestoneCelebrationPhrase(milestone: Milestone): string {
  const templates: Record<MilestoneType, string[]> = {
    first_meeting: [
      ...
      "Hey! First time meeting - exciting!",
    ],
    conversation_count: [
    ...
    time_spent: [
      `We've spent real time together. That matters.`,
    ],
    streak: [
    ...
    breakthrough: [
      `That's a breakthrough. Seriously. Remember this moment.`,
    ],
    goal_achieved: [
      `You did it! Goal achieved!`,
    ],
    habit_formed: [
      `It's a habit now. You ARE a person who does this.`,
    ],
    vulnerability_shared: [
