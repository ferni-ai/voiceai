...
async function initializeFirestore() {
    try {
        ...
        getLogger().warn({ error }, 'Firestore not available for cross-agent awareness, using in-memory only');
 ...
 */
export async function recordConversationForTeam(userId, summary) {
    const log = getLogger();
    // Update cache
    ...
    if (firestore) {
        try {
            ...
            if (doc.exists) {
                const data = doc.data();
                teamHistory = (data?.conversations || []).map((c) => ({
                    ...c,
 ...
 */
export async function getTeamContext(userId, currentAgentId) {
    const log = getLogger();
    // Try cache first
    ...
    if (!summaries) {
        ...
        if (firestore) {
            try {
                ...
                if (doc.exists) {
                    const data = doc.data();
                    summaries = (data?.conversations || []).map((c) => ({
                        ...c,
    ...
    notes = notes || [];
    const relevantNotes = notes.filter((n) => (n.toAgent === currentAgentId || n.toAgent === '*') && !n.acknowledged);
    return {
 ...
 */
async function loadTeamNotes(userId) {
    const firestore = await getFirestore();
    if (!firestore)
        return [];
    try {
 ...
 */
export async function addTeamNote(userId, note) {
    const log = getLogger();
    const fullNote = {
        ...note,
        timestamp: new Date(),
 ...
 */
export async function acknowledgeTeamNote(userId, noteTimestamp, _agentId // Reserved for audit logging
) {
    // Update cache
    ...
    if (firestore) {
        try {
            ...
            if (doc.exists) {
                const data = doc.data();
                const firestoreNotes = (data?.notes || []).map((n) => {
 ...
 */
export function formatCrossAgentContextForPrompt(context, _currentAgentId // Reserved for agent-specific filtering
) {
    ...
    if (context.recentTeamInteractions.length > 0) {
        ...
        for (const interaction of context.recentTeamInteractions) {
            ...
            const daysAgo = Math.floor((Date.now() - new Date(interaction.timestamp).getTime()) / (1000 * 60 * 60 * 24));
        ...
        parts.push('');
        parts.push(`[TIP: You can naturally reference what ${context.recentTeamInteractions[0]?.agentName || 'a teammate'} discussed - e.g., "${context.recentTeamInteractions[0]?.agentName || 'Alex'} mentioned you were working on..."]`);
    }
    // Team notes
 ...
 */
export function generateTeamReferencePhrases(context, _currentAgentId // Reserved for agent-specific phrase generation
) {
    const phrases = [];
    for (const interaction of context.recentTeamInteractions.slice(0, 3)) {
        ...
        if (interaction.topics.includes('stress') || interaction.topics.includes('anxiety')) {
            phrases.push(`${agentName} mentioned you've been under some pressure lately. How are you holding up?`);
        }
        if (interaction.topics.includes('goals') || interaction.topics.includes('planning')) {
            phrases.push(`I heard from ${agentName} you're working on some goals. Want to talk about how that's going?`);
        }
        if (interaction.topics.includes('finances') || interaction.topics.includes('budget')) {
            phrases.push(`${agentName} and I were chatting - sounds like you've been thinking about finances. How's that sitting with you?`);
        }
        // Emotional tone references
        if (interaction.emotionalTone === 'struggling') {
            phrases.push(`I know things have been tough - ${agentName} filled me in. I'm here if you want to talk.`);
        }
        ...
        if (interaction.followUpNeeded && interaction.keyMoments.length > 0) {
            phrases.push(`${agentName} mentioned something I wanted to follow up on: ${interaction.keyMoments[0]}`);
        }
    }
    return phrases;
}
 ...
 */
export function analyzeConversationForTeam(agentId, conversationText, userEmotions) {
 ...
 */
export async function recordSessionForTeam(userId, agentId, conversationSummary, userEmotions) {
 ...
 */
export async function initializeCrossAgentAwareness(userId) {
    const log = getLogger();
    ...
    if (firestore) {
        try {
            ...
            if (convDoc.exists) {
                ...
                log.debug({ userId, count: conversations.length }, 'Loaded team conversations from Firestore');
            }
            // Process notes
            if (notesDoc.exists) {
 ...
 */
export function clearCrossAgentCaches() {
    conversationCache.clear();
    teamNotesCache.clear();
}
//# sourceMappingURL=cross-agent-awareness.js.map