 ...
 */
export function detectCrisisSignals(transcript) {
    const signals = {
        severity: 0,
    ...
    if (/\b(hurt|harm|abuse|hit|beat|molest|inappropriate).*(child|kid|son|daughter|baby)/i.test(text)) {
 ...
 */
export function classifyEscalation(signals, conversationContext) {
...
// HELPER FUNCTIONS
// ============================================================================
function getImmediateCrisisReason(signals) {
    if (signals.medicalEmergency)
        return 'Medical emergency detected - immediate professional help needed';
    if (signals.dangerToOthers)
        return 'Safety concern for others - professional intervention needed';
    if (signals.suicidalIdeation)
        return 'Active crisis detected - trained crisis counselors available 24/7';
    if (signals.childSafetyConcern)
        return 'Child safety concern - professional intervention needed';
    return 'Immediate crisis support needed';
}
function getTherapyReason(signals) {
    if (signals.traumaIndicators) {
...
function extractSafetyFlags(signals) {
    return {
 ...
 */
export function classifyWithContext(currentTranscript, conversationHistory, userProfile) {
    ...
    if (userProfile) {
        ...
        if ((decision.type === 'crisis_support' || decision.type === 'crisis_immediate') &&
...
//# sourceMappingURL=escalation-classifier.js.map