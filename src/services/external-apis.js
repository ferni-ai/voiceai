...
import { getAlphaVantageClient, getGoogleApisClient, getWikipediaClient, getYahooFinanceClient, } from './self-healing/index.js';
...
export async function getStockQuote(symbol) {
    ...
    const { data, error, status } = await yahooClient.get(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&range=1d`);
    ...
    if (error) {
        logger.warn({ error: error.message, code: error.code }, `Yahoo Finance error for ${symbol}, trying Alpha Vantage`);
    }
    return getStockQuoteAlphaVantage(symbol);
}
/**
 ...
 */
async function getStockQuoteAlphaVantage(symbol) {
    ...
    const { data, error } = await alphaClient.get(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=${apiKey}`);
...
export async function getMarketOverview() {
    ...
    const promises = indices.map(async (symbol) => {
        const { data } = await yahooClient.get(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&range=1d`);
...
export async function getWeather(location) {
    ...
    const { data: geoData, error: geoError } = await googleClient.get(geoUrl);
    ...
    const { data: weatherData, error: weatherError } = await googleClient.get(weatherUrl);
    if (weatherError) {
...
function getWeatherFallback(location) {
...
export async function getHistoricalEvent() {
    ...
    const { data, error } = await wikipediaClient.get(`https://en.wikipedia.org/api/rest_v1/feed/onthisday/events/${month}/${day}`);
    if (error) {
    ...
    if (data?.events?.length) {
        ...
        const relevantEvents = events.filter((e) => relevantKeywords.some((keyword) => e.text?.toLowerCase().includes(keyword)));
