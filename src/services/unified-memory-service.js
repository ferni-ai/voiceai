...
import { createLinksForNewMemory, getUserMemories, reinforceMemory as reinforceMemoryInStorage, runLifecycleMaintenance, saveMemory, } from '../memory/lifecycle-integration.js';
import { getMemoryConsolidator, } from '../memory/memory-consolidator.js';
import { getMemoryDecayManager } from '../memory/memory-decay.js';
 ...
 */
class TimingEngine {
    surfacingHistory = new Map(); // userId -> timestamps
    cooldownMs = 60_000; // 1 minute between surfaces
    maxSurfacesPerSession = 3;
    /**
     * Decide if now is a good time to surface this memory
     */
    decide(userId, context) {
...
class FeedbackCollector {
    ...
    record(feedback) {
        ...
        log.debug({ userId: feedback.userId, action: feedback.action, memoryId: feedback.memoryId }, 'Memory feedback recorded');
    }
    /**
...
export class UnifiedMemoryService {
    ...
    pendingSurfacingEvents = new Map(); // memoryId -> eventId
    constructor() {
    ...
    async recall(context) {
        ...
        try {
            ...
            const avgStrength = memory.primaryMemories.length > 0
                ? memory.primaryMemories.reduce((sum, m) => sum +
                    (m.connectionStrength === 'strong'
                        ? 0.9
                        : m.connectionStrength === 'moderate'
                            ? 0.6
                            : 0.3), 0) / memory.primaryMemories.length
                : 0;
            ...
            const phrasing = topMemory
                ...
                : { style: 'natural_weave', personaVoice: true };
            ...
            if (timing.shouldSurface && memory.primaryMemories.length > 0) {
                ...
                for (const mem of memory.primaryMemories.slice(0, learnings.maxProactivePerSession)) {
                    this.recordPendingSurfacing(context.userId, mem.item.id, mem.item.type, mem.item.topics ?? [], mem.item.emotionalWeight, {
                        surfacingMethod: 'query_response',
                        conversationPhase: conversationPhase === 'exploring'
                            ? 'mid'
            ...
            const associatedMemories = await this.safeGetAssociatedMemories(context.userId, memory.primaryMemories.slice(0, 3) // Top 3 primary memories as seeds
            );
            ...
            return {
                ...
                activePatterns: [],
                formattedContext: '',
                timing: { shouldSurface: false, reason: 'low_confidence', confidence: 0 },
     ...
     */
    async safeGetAssociatedMemories(userId, primaryMemories) {
        if (primaryMemories.length === 0) {
            return [];
        }
        try {
            const result = await this.getAssociatedMemoriesFromPrimary(userId, primaryMemories);
            log.debug({ userId, associatedCount: result.length }, 'Spreading activation found associated memories');
            return result;
        }
        catch (activationError) {
            log.debug({ error: String(activationError), userId }, 'Spreading activation failed (non-critical)');
            return [];
        }
    }
    /**
     ...
     */
    async getAssociatedMemoriesFromPrimary(userId, primaryMemories) {
        ...
        for (const result of activationResults.slice(0, 5)) {
            ...
            if (memory) {
                associated.push({
                    ...
                    linkTypes: result.pathTypes, // LinkType[] is compatible with string[]
                });
            }
        }
        return associated;
    ...
    async write(input) {
        ...
        try {
            ...
            try {
                ...
                if (protection) {
                    isProtected = true;
                    log.debug({ userId, memoryId, protectionLevel: protection.protectionLevel }, 'Memory auto-protected');
                }
            }
            catch (protectionError) {
                log.warn({ error: String(protectionError) }, 'Protection analysis failed');
            }
            log.debug({ userId, type, importance, memoryId, linksCreated, isProtected }, 'Memory written');
     ...
     */
    async recordLearning(userId, memoryId, userResponse, context = {}) {
        ...
        const reaction = this.learningEngine.inferReaction(userResponse, context.changedTopic ?? false, context.expressedGratitude ?? false, context.expressedDiscomfort ?? false);
        // Record reaction
        ...
        if (reaction === 'engaged' || reaction === 'grateful') {
            await this.learningEngine.reinforceMemory(userId, memoryId, reaction === 'grateful' ? 1.0 : 0.7);
        }
        ...
        this.feedbackCollector.record({
            memoryId,
            userId,
            action: reaction === 'engaged' || reaction === 'grateful'
                ? 'engaged'
                : reaction === 'negative'
                    ? 'dismissed'
                    : 'ignored',
    ...
    async consolidateMemories(userId) {
        ...
        try {
            ...
            log.info({ userId, processed: result.memoriesProcessed, consolidated: result.consolidated.length }, 'Memory consolidation complete');
            return result;
        }
        catch (error) {
    ...
    async applyDecay(userId) {
        try {
            ...
            const memoriesProtected = memories.length - pruneResult.archived.length - pruneResult.strengthDistribution.weak;
            log.info({ userId, analyzed: memories.length, archived: pruneResult.archived.length }, 'Memory decay applied');
            return {
                memoriesDecayed: pruneResult.strengthDistribution.weak + pruneResult.strengthDistribution.moderate,
     ...
     */
    async reinforceMemory(userId, memoryId, boostFactor = 1.5) {
        try {
     ...
     */
    async getAssociatedMemories(userId, memoryId, depth = 2) {
        try {
            const results = await this.memoryGraph.spreadActivation(userId, [memoryId], {
                maxDepth: depth,
            });
            log.debug({ userId, memoryId, resultCount: results.length }, 'Graph spreading activation complete');
            return results;
        }
        catch (error) {
     ...
     */
    async createMemoryLinks(userId, newMemoryId, newMemoryContent, newMemoryTopics = []) {
        try {
     ...
     */
    async saveMemoryDirect(userId, memory) {
        try {
     ...
     */
    recordPendingSurfacing(userId, memoryId, memoryType, memoryTopics, emotionalWeight, context) {
        // Map input type to valid MemoryItem types
        const typeMap = {
            fact: 'summary',
            emotion: 'moment',
    ...
    mapEmotionalState(emotion) {
        ...
        if (positiveEmotions.some((e) => lower.includes(e)))
            return 'positive';
        if (vulnerableEmotions.some((e) => lower.includes(e)))
            return 'vulnerable';
        if (negativeEmotions.some((e) => lower.includes(e)))
            return 'negative';
        return 'neutral';
    }
}
 ...
 */
export function getUnifiedMemoryService() {
    if (!instance) {
 ...
 */
export function resetUnifiedMemoryService() {
    instance = null;
}
// ============================================================================
 ...
 */
export async function saveMemoryDirect(userId, memory) {
