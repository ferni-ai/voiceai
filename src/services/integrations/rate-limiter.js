...
export class RateLimiter {
     ...
     */
    async checkLimit(userId, integrationId) {
        ...
        if (currentCount >= config.requests) {
            ...
            log.warn({ userId, integrationId, currentCount, limit: config.requests }, 'Rate limit exceeded');
            return {
                allowed: false,
                remaining: 0,
                resetAt,
     ...
     */
    async recordRequest(userId, integrationId) {
        const key = this.getKey(userId, integrationId);
    ...
    getState(userId, integrationId) {
        ...
        return {
            ...
            retryAfter: timestamps.length >= config.requests
                ? new Date(timestamps[0] + config.windowMs)
                : undefined,
        };
    }
 ...
 */
export function hasGlobalRateLimit(integrationId) {
    const integration = INTEGRATIONS[integrationId];
    if (!integration)
        return false;
 ...
 */
export function getRateLimitConfig(integrationId) {
    const integration = INTEGRATIONS[integrationId];
 ...
 */
export function formatRateLimit(config) {
