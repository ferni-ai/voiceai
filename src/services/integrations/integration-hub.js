...
export class IntegrationHub {
     ...
     */
    getConnection(userId, integrationId) {
     ...
     */
    isConnected(userId, integrationId) {
     ...
     */
    async isConnectedAsync(userId, integrationId) {
    ...
    setConnection(connection) {
        ...
        log.debug({ userId: connection.userId, integrationId: connection.integrationId, status: connection.status }, 'Connection updated');
    }
    /**
     * Remove a connection
     */
    removeConnection(userId, integrationId) {
        const key = `${userId}:${integrationId}`;
     ...
     */
    async getAuthorizationUrl(userId, integrationId, redirectPath) {
     ...
     */
    async refreshTokens(userId, integrationId) {
        const connection = this.getConnection(userId, integrationId);
        if (!connection?.tokens?.refreshToken) {
            return false;
        }
        const newTokens = await this.oauthManager.refreshTokens(integrationId, connection.tokens.refreshToken);
        if (newTokens) {
     ...
     */
    async request(userId, integrationId, config) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                log.warn({ userId, integrationId, statusCode: response.status, error: errorText }, 'API request failed');
                return {
                    success: false,
                    error: errorText,
...
// SINGLETON INSTANCE
// ============================================================================
let integrationHubInstance = null;
...
export function resetIntegrationHub() {
