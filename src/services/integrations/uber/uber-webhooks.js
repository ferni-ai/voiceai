 ...
 */
export function registerUberWebhookHandlers(newHandlers) {
 ...
 */
async function processUberWebhook(event) {
    const { event_type, event_id, meta } = event;
    log.debug({ eventId: event_id, eventType: event_type, resourceId: meta.resource_id }, 'Processing Uber webhook');
    try {
        switch (event_type) {
            ...
            case 'requests.receipt_ready': {
                if (handlers.onReceiptReady) {
                    ...
                    await handlers.onReceiptReady(meta.user_id, meta.resource_id, {});
                }
                return {
                    success: true,
 ...
 */
export function initializeUberWebhooks() {
 ...
 * POST /webhooks/uber
 */
export async function handleUberWebhook(req, res) {
    try {
 ...
 */
export async function defaultStatusChangeHandler(userId, requestId, status) {
...
//# sourceMappingURL=uber-webhooks.js.map