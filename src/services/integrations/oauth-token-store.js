 ...
 */
export async function updateOAuthTokens(userId, provider, tokens) {
    const db = getFirestoreDb();
    if (!db) {
 ...
 */
export async function deleteOAuthTokens(userId, provider) {
    const db = getFirestoreDb();
    if (!db) {
 ...
 */
export async function getConnectedIntegrations(userId) {
    const db = getFirestoreDb();
    if (!db) {
        return [];
    }
 ...
 */
export async function markConnectionError(userId, provider, error) {
    const db = getFirestoreDb();
    if (!db)
        return;
    try {
 ...
 */
export async function needsRefresh(userId, provider, bufferMinutes = 5) {
    const db = getFirestoreDb();
    if (!db)
        return false;
    try {
