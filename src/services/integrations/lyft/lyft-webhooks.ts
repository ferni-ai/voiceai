...
const handlers: LyftWebhookHandlers = {};

/**
 * Register handlers for Lyft webhook events
 */
export function registerLyftWebhookHandlers(newHandlers: LyftWebhookHandlers): void {
 ...
 */
async function processLyftWebhook(
  event: LyftWebhookEvent
    ...
    switch (eventType) {
      ...
      case 'ride.receipt_ready': {
        if (handlers.onReceiptReady && user_id) {
          await handlers.onReceiptReady(user_id, ride_id, {} as LyftRideReceipt);
        }
        return {
          success: true,
 ...
 */
export function initializeLyftWebhooks(): void {
  ...
  webhookRouter.registerHandler({
    ...
    handler: async (event) => {
      const lyftEvent = event.payload as LyftWebhookEvent;
 ...
 * POST /webhooks/lyft
 */
export async function handleLyftWebhook(req: Request, res: Response): Promise<void> {
  try {
    const signature = req.headers['x-lyft-signature'] as string;
    ...
    const event = req.body as LyftWebhookEvent;
...
export async function defaultStatusChangeHandler(
  ...
  const statusMessages: Record<LyftRideStatus, string> = {
