...
import { clearAllSessions as clearAll, getActiveSessionCount as getActiveCount, getActiveSessionIds as getActiveIds, getSessionServices as getSession, initializeAccess, } from './session-manager/access.js';
import { initializeCleanup, startSessionCleanup as startCleanup, stopSessionCleanup as stopCleanup, } from './session-manager/cleanup.js';
...
buildMemoryIndex, clearCurrentSessionMomentsGetter, getConversationPrimingMemories, getHistoryTracker, 
// Session priming for cross-session continuity
getSessionPrimer, indexConversationSummary, removeHistoryTracker, ragLookup as semanticRagLookup, semanticSearch, setCurrentSessionMomentsGetter, summarizeConversation, } from '../memory/index.js';
// Intelligence imports
import { analyzeMessage, 
// Superhuman Memory - "Better than Human" proactive intelligence
buildSuperhumanContext, getCommunicationMirroring, getConversationPatternAnalyzer, getCrossSessionThreader, getEmotionalMemory, getEmotionDetector, getFinancialJourneyTracker, 
// Human-Level Interaction Engines
getHumorCalibration, getLearningEngine, getProactiveInsightEngine, 
// Advanced Intelligence Engines
getResponseQualityTracker, getStateMachine, getStoryPreference, getTopicTracker, getVoicePaceAdapter, markSuperhumanInsightDelivered as markSuperhumanInsightDeliveredFn, recordVoicePattern, removeCommunicationMirroring, removeConversationPatternAnalyzer, removeCrossSessionThreader, removeEmotionalMemory, removeFinancialJourneyTracker, removeHumorCalibration, removeProactiveInsightEngine, removeResponseQualityTracker, removeStoryPreference, removeVoicePaceAdapter, resetIntelligence, resetLearningEngine, UserLearningEngine, } from '../intelligence/index.js';
// Context imports
import { getContextManager, removeContextManager } from '../context/index.js';
// Speech imports - using session-scoped WPM tracking
import { buildSpeechContext, getSessionWPMTracker, tagAdvice, tagStory, tagSupportResponse, tagTextWithSsmlAdaptive, tagWrapUp, } from '../speech/index.js';
// Local imports
...
import { applyIntelligenceToProfile, cleanupIntelligenceEngines, loadIntelligenceFromProfile, startAutoSave, stopAutoSave, } from './intelligence-persistence.js';
...
export async function createSessionServices(sessionIdOrOptions, userId, isReturningUser, personaSpeech, personaEnergy, personaId) {
    ...
    let lastUserEmotion = undefined;
    // Track humanizing state updates during session
    const humanizingStateUpdates = [];
    // ============================================================================
    ...
    if (validatedUserId) {
        ...
        const { getProfileWithCache, cacheProfile, invalidateProfile } = await import('./data-layer/profile-cache.js');
        ...
        if (!userProfile) {
            ...
            getLogger().info({ userId: validatedUserId, name: userName || '(none)' }, '🆕 Created new user profile with name from onboarding');
        }
            ...
            getLogger().info({ userId: validatedUserId, name: userName }, '✨ Updated existing profile with name from onboarding');
        }
    ...
    const initialAgent = (personaId || 'ferni');
    ...
    const crossSessionThreader = getCrossSessionThreader(engineKey, existingThreads, existingFollowUps);
        ...
        try {
            ...
            try {
                ...
                if (primingMemories.length > 0) {
                    getLogger().info({ userId: validatedUserId, memoriesLoaded: primingMemories.length }, '🧠 Memory index warmed with salient memories for session priming');
                }
            }
            catch (memoryWarmupError) {
                getLogger().debug({ error: String(memoryWarmupError) }, 'Memory warmup failed (non-blocking) - continuing with empty memories');
            }
            // Generate priming context with actual memories from vector store
            const primingResult = await primer.generatePrimingContext(userProfile, primingMemories, // Now populated with real memories!
            recentSummaries);
            sessionPriming = {
                suggestedOpener: primingResult.suggestedOpener,
                openThreads: primingResult.openThreads.map((t) => ({
                    topic: t.topic,
                    urgency: t.priority,
                })),
                pendingFollowUps: primingResult.pendingFollowUps.map((f) => ({
                    topic: f.commitment,
            ...
            getLogger().debug({ error: String(primingError) }, 'Failed to generate session priming (non-blocking)');
        }
    }
    // ============================================================================
        ...
        try {
            ...
            if (superhumanContext.insights.length > 0) {
                getLogger().info({
                    ...
                    hasGrowthCelebration: superhumanContext.insights.some((i) => i.type === 'growth_celebration'),
            ...
            getLogger().debug({ error: String(superhumanError) }, 'Failed to generate superhuman memory context (non-blocking)');
        }
    }
    ...
    // ============================================================================
    if (validatedUserId) {
        const autoSaveCallback = async (uid) => {
    ...
    if (validatedUserId) {
        try {
            realtimeConversationId = await realtimeMemory.startConversation(validatedUserId, personaId || 'ferni');
            getLogger().info({ userId: validatedUserId, conversationId: realtimeConversationId }, '🔴 REALTIME: Conversation started - every turn will be persisted immediately');
        }
        catch (error) {
            getLogger().warn({ error: String(error), userId: validatedUserId }, 'Failed to start realtime conversation (falling back to session-end persistence)');
        }
    }
    // ============================================================================
    ...
    if (validatedUserId) {
        try {
            ...
            if (!existingSession) {
                ...
                getLogger().info({ sessionId, userId: validatedUserId }, '👥 Household session started for voice identification');
            }
        }
        catch (error) {
            getLogger().warn({ error: String(error) }, 'Failed to start household session (voice identification unavailable)');
        }
    }
    // ============================================================================
    ...
    const services = {
        ...
        addTurn: (role, content, durationMs) => {
            const turn = {
                role,
                content,
                timestamp: new Date(),
            ...
            if (role === 'user') {
                ...
                if (durationMs) {
                    ...
                    if (validatedUserId) {
                        ...
                        const pace = paceRatio < 0.85
                            ? 'slower_than_usual'
                            : paceRatio > 1.15
                                ? 'faster_than_usual'
                                : 'normal';
                        // Simple energy heuristic based on pace and message length
                        const energy = pace === 'slower_than_usual' && content.length < 50
                            ? 'lower_than_usual'
            ...
            if (turnCount <= 5 || turnCount % 10 === 0) {
                getLogger().debug({ sessionId, role, turnCount, contentPreview: content.slice(0, 50) }, `📝 Turn added (total: ${turnCount})`);
            }
            ...
            if (validatedUserId && realtimeConversationId) {
                ...
                realtimeMemory
                    ...
                    getLogger().warn({ error: String(err), sessionId }, 'Failed to persist turn in realtime (data in RAM, will save at session end)');
                });
            }
        },
        // ========================================================================
        ...
        getEnhancedPromptContext: () => {
            const sections = [];
            const baseContext = contextManager.buildPromptContext(stateMachine.getState(), stateMachine.getGuidance(), getEmotionDetector().detect(''));
        ...
        // SPEECH METHODS
        // ========================================================================
        getSpeechContext: (text, userEmotion) => {
            const state = stateMachine.getState();
        ...
        tagWithSsml: (text) => {
            ...
            const storyPatterns = /\b(i remember|back in|when i was|years ago|one time|there was|let me tell you|i'll never forget|i once|my father|at vanguard|in 1974|in 1975|in 2008)\b/i;
        ...
        // ========================================================================
        trackResponseQuality: (response, userReaction) => {
        ...
        recordResponseSignal: async (params) => {
            ...
            const { agentResponse, userResponse, topic, conversationPhase, emotion, storyId, questionAsked, } = params;
            // Record the full quality signal
            const signal = responseQualityTracker.recordSignal(agentResponse, userResponse, topic, conversationPhase, emotion);
            ...
            if (signal.engagementScore >= 0.8) {
                ...
                if (storyId && personaId) {
                    try {
                        ...
                        if (communityInsights) {
                            communityInsights.recordStoryUsage(storyId, personaId, {
                                topic,
                ...
                if (questionAsked && personaId) {
                    try {
                        ...
                        const communityInsights = getCommunityInsights();
                        if (communityInsights) {
                            communityInsights.recordBreakthroughQuestion(questionAsked, personaId, topic, conversationPhase, // context string
        ...
        getProactiveInsights: async () => {
            ...
            if (userId) {
                try {
                    const { getInsightsToSurface, markInsightSurfaced } = await import('./superhuman/semantic-intelligence/insight-broker.js');
                    const semanticInsights = await getInsightsToSurface(userId, {
                        isSessionStart: true,
                        hourOfDay: new Date().getHours(),
                    });
                    // Count high priority semantic insights
                    const semanticHighPriority = semanticInsights.filter((i) => i.priority === 'high' || i.priority === 'critical');
         ...
         */
        markInsightDelivered: (insightId) => {
        ...
        refreshSuperhumanContext: (options) => {
            ...
            try {
                ...
                getLogger().debug({ insights: services.superhumanContext?.insights.length || 0 }, '🧠 Refreshed superhuman context');
            }
            catch (error) {
         ...
         */
        markSuperhumanInsightDelivered: (insightId) => {
        ...
        updateHumanizingState: (update) => {
            ...
            getLogger().debug({ sessionId, updateCount: humanizingStateUpdates.length }, '🎭 Humanizing state update recorded');
        },
        // ========================================================================
        // LIFECYCLE METHODS
        // ========================================================================
            ...
            if (profileToSave && validatedUserId) {
                try {
                    ...
                    const updated = updateProfileFromSession(profileToSave, {
                        ...
                        energyLevel: state.distressLevel < 0.3 ? 'high' : state.distressLevel < 0.6 ? 'medium' : 'low',
                    ...
                    if (!updated.lastConversationSummary) {
                        ...
                        getLogger().info({ userId: validatedUserId, summary: updated.lastConversationSummary }, '🔒 SAFEGUARD: Set minimum lastConversationSummary in saveProfile()');
                    }
                    await global.store.saveProfile(updated);
                    services.userProfile = updated;
                    getLogger().info({ userId: validatedUserId, lastSummary: updated.lastConversationSummary?.slice(0, 50) }, '✅ Saved profile for user');
                }
                catch (error) {
                    // FIX BUG #session-7: Log errors instead of silent failure
                    getLogger().error({ userId: validatedUserId, error: String(error) }, 'Failed to save user profile');
        ...
        endSession: async () => {
            ...
            if (validatedUserId && userProfile) {
                try {
                    ...
                    if (turns.length === 0) {
                        getLogger().warn({ sessionId, userId: validatedUserId }, '⚠️ No conversation turns to summarize - this means addTurn() was never called');
                    }
                    let summary = null;
                    if (turns.length > 0) {
                        getLogger().info({ sessionId, turnCount: turns.length, userId: validatedUserId }, '📝 Starting conversation summarization');
                        ...
                        try {
                            ...
                            summary = await withTimeout(summarizeWithLLM(sessionId, turns, llmCaller), SUMMARIZE_TIMEOUT_MS, 'summarizeWithLLM', sessionId);
                            if (summary) {
                                getLogger().info({ sessionId, keyPoints: summary.keyPoints?.length || 0 }, '✅ LLM summarization succeeded');
                            }
                        }
                        catch (llmError) {
                            // LLM failed, fall through to extraction
                            getLogger().warn({ sessionId, error: String(llmError) }, '⚠️ LLM summarization failed, trying extraction fallback');
                        }
                        // Fall back to extraction-based summarization
                        if (!summary) {
                            try {
                                summary = await withTimeout(summarizeConversation(sessionId, turns), SUMMARIZE_TIMEOUT_MS, 'summarizeConversation', sessionId);
                                if (summary) {
                                    getLogger().info({ sessionId, keyPoints: summary.keyPoints?.length || 0 }, '✅ Extraction summarization succeeded');
                                }
                            }
                            catch (extractError) {
                                getLogger().warn({ sessionId, error: String(extractError) }, '⚠️ Extraction summarization also failed');
                            }
                        }
                        if (!summary) {
                            getLogger().warn({ sessionId, turnCount: turns.length }, '❌ All summarization methods failed - will use fallback');
                        }
                        else {
                    ...
                    if (humanizingStateUpdates.length > 0) {
                        try {
                            const { getHumanizingState, mergeHumanizingStateUpdate, applyHumanizingStateToProfile, logHumanizingStateSummary, } = await import('./humanizing-state.js');
                            ...
                            getLogger().warn({ error: String(humanizingError) }, 'Failed to persist humanizing state (non-fatal)');
                        }
                    }
                    ...
                    try {
                        ...
                        updatedProfile.customData.meetingCounts = meetingCounts;
                        updatedProfile.customData.lastTopicsPerPersona =
                            lastTopicsPerPersona;
                        getLogger().debug({ meetingCounts: Object.keys(meetingCounts).length }, 'Persisted handoff state to profile');
                    }
                    catch (handoffPersistError) {
                        getLogger().warn({ error: String(handoffPersistError) }, 'Failed to persist handoff state (non-fatal)');
                    }
                    ...
                    try {
                        ...
                        if (openThreadCount > 0 || pendingFollowUps > 0) {
                            updatedProfile.customData.openThreads =
                                threadData.threads;
                            updatedProfile.customData.promisedFollowUps =
                                threadData.followUps;
                            getLogger().info({ openThreads: openThreadCount, pendingFollowUps }, 'Persisted cross-session threads to profile');
                        }
                    }
                    catch (threadPersistError) {
                        getLogger().warn({ error: String(threadPersistError) }, 'Failed to persist cross-session threads (non-fatal)');
                    }
                    ...
                    try {
                        ...
                        if (moments.length > 0) {
                            ...
                            updatedProfile.customData.emotionalMoments =
                                recentMoments;
                            getLogger().info({ momentCount: recentMoments.length }, 'Persisted emotional memory to profile');
                        }
                    }
                    catch (emotionalMemoryError) {
                        getLogger().warn({ error: String(emotionalMemoryError) }, 'Failed to persist emotional memory (non-fatal)');
                    }
                    ...
                    try {
                        ...
                        getLogger().warn({ error: String(intelligenceError), userId: validatedUserId }, 'Failed to apply intelligence state (non-fatal)');
                    }
                    // ================================================================
                    ...
                    // ================================================================
                    try {
                        const { getPersonalJourneyForPersistence, updateJourneyFromConversation } = await import('./personal-journey/session-integration.js');
                        ...
                        if (summary) {
                            await updateJourneyFromConversation(validatedUserId, {
                                ...
                                wins: summary.keyPoints?.filter((kp) => /achieved|completed|succeeded|won|accomplished/i.test(kp)),
                                struggles: summary.keyPoints?.filter((kp) => /struggled|difficult|hard|worried|anxious|stressed/i.test(kp)),
                            });
                        }
                        ...
                        const { captureSeasonalSnapshotIfNeeded } = await import('./personal-journey/session-integration.js');
                        if (summary) {
                        ...
                        getLogger().warn({ error: String(journeyError), userId: validatedUserId }, 'Failed to persist personal journey data (non-fatal)');
                    }
                    // ================================================================
                    ...
                    // ================================================================
                    try {
                        const { extractHumanSignals, mergeSignalsIntoMemory } = await import('../memory/human-signal-extractor.js');
                        if (turns.length > 0) {
                            ...
                            if (totalSignals > 0) {
                                updatedProfile.humanMemory = mergeSignalsIntoMemory(updatedProfile.humanMemory, signals);
                                getLogger().info({
                                    userId: validatedUserId,
                                    totalSignals,
                        ...
                        getLogger().warn({ error: String(humanMemoryError), userId: validatedUserId }, 'Failed to extract human memory signals (non-fatal)');
                    }
                    // ================================================================
                    ...
                    try {
                        ...
                        getLogger().debug({ error: String(outreachError) }, 'Outreach analysis skipped (non-fatal)');
                    }
                    getLogger().info({
                        userId: validatedUserId,
            ...
            if (validatedUserId && realtimeConversationId) {
                try {
                    ...
                    realtimeMemory
                        ...
                        .catch((err) => {
                        getLogger().warn({ error: String(err), conversationId: realtimeConversationId }, 'Async summarization failed (turns are still persisted)');
                    });
                    getLogger().info({ userId: validatedUserId, conversationId: realtimeConversationId }, '🔴 REALTIME: Conversation ended, async summarization triggered');
                }
                catch (error) {
                    getLogger().warn({ error: String(error), userId: validatedUserId }, 'Failed to end realtime conversation (non-blocking)');
                }
            }
