 ...
 */
export async function analyzeVoiceEmotion(audioBuffer, sessionId) {
...
function parseGeminiResponse(text) {
    try {
        ...
        const primary = (parsed.primary || 'neutral');
        const secondary = (parsed.secondary || []);
        // Build scores object from primary/secondary
        const scores = { [primary]: parsed.confidence || 0.7 };
 ...
 */
export async function startEmotionStream(sessionId, onEmotion) {
    if (!isGeminiConfigured()) {
    ...
    registerInterval(intervalName, async () => {
        if (stopped || isProcessing || audioBuffer.length === 0)
            return;
        isProcessing = true;
        try {
 ...
 */
export function getEmotionTimeline(sessionId) {
    ...
    const dominantEmotion = [...emotionCounts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] || 'neutral';
    return {
        sessionId,
        points,
        trajectory,
 ...
 */
export function detectSuppression(result) {
 ...
 */
export function distinguishNegativeEmotion(result) {
 ...
 */
export function generateEmotionInsight(result) {
    // Check for suppression
    const suppression = detectSuppression(result);
    if (suppression.isSuppressing && suppression.suppressed) {
        return (`User appears to be suppressing ${suppression.suppressed} while displaying ${suppression.displayed}. ` +
    ...
    if (result.valence < -0.2) {
        ...
        if (distinction.emotion !== 'other') {
            ...
            return (`Voice analysis indicates ${distinction.emotion} (${indicatorText}). ` +
...
// ============================================================================
function calculateValence(scores) {
    let positive = 0;
    let negative = 0;
...
function calculateArousal(scores) {
    const highArousal = ['anger', 'excitement', 'fear', 'ecstasy', 'anxiety'];
    const lowArousal = ['calmness', 'tiredness', 'sadness', 'contentment'];
    let high = 0;
    let low = 0;
    for (const [emotion, score] of Object.entries(scores)) {
        if (highArousal.includes(emotion))
            high += score;
        if (lowArousal.includes(emotion))
            low += score;
    }
    const total = high + low || 1;
    return high / total;
}
function estimateSuppression(scores, valence) {
...
function updateSessionTimeline(sessionId, result) {
    if (!sessionStates.has(sessionId)) {
        sessionStates.set(sessionId, { timeline: [] });
    }
    const state = sessionStates.get(sessionId);
...
function calculateTrajectory(points) {
    if (points.length < 3)
        return 'stable';
    ...
    const variance = recent.reduce((sum, p) => sum + Math.abs(p.valence - recentAvg), 0) / recent.length;
    if (variance > 0.4)
        return 'volatile';
    if (diff > 0.15)
        return 'improving';
