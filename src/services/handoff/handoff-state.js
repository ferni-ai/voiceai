 ...
 */
export function createHandoffState(initialAgent = 'ferni') {
    return {
 ...
 */
export function recordHandoff(state, from, to, reason) {
    const now = Date.now();
 ...
 */
export function getLastHandoff(state) {
 ...
 */
export function markPersonaMet(state, personaId) {
 ...
 */
export function setLastTopicForPersona(state, personaId, topic) {
    state.perPersonaLastTopic.set(personaId, topic);
}
/**
 * Update user context for mood detection in handoffs
 */
export function updateUserContextForHandoff(state, context) {
 ...
 */
export function getHandoffContext(state) {
    return state.conversationContext;
}
/**
 * Format handoff context for the new agent
 */
export function formatHandoffContextForAgent(state) {
 ...
 */
export function getHandoffAnalytics(state) {
    const history = [...state.handoffHistory];
    const handoffsByAgent = {};
    const timeByAgent = {};
    ...
    for (let i = 0; i < history.length; i++) {
        ...
        if (record.duration) {
            if (!timeByAgent[record.from])
                timeByAgent[record.from] = [];
...
//# sourceMappingURL=handoff-state.js.map