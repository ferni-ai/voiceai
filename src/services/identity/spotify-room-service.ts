...
import {
  ...
  listRooms,
  updateRoom,
} from './spotify-room-config-store.js';
import type {
  CreateRoomInput,
  ListRoomsResult,
  PlayInRoomResult,
 ...
 */
export async function discoverDevices(): Promise<RoomResult<SpotifyDevice[]>> {
 ...
 */
export async function getUnassignedDevices(userId: string): Promise<RoomResult<SpotifyDevice[]>> {
 ...
 */
export async function playInRoom(
  userId: string,
  roomName: string,
  options: {
    query?: string;
...
async function playOnDevices(
  ...
  try {
    ...
    if (volume !== undefined) {
      await fetch(
        `${SPOTIFY_API_BASE}/me/player/volume?volume_percent=${volume}&device_id=${availableDeviceId}`,
        {
          method: 'PUT',
 ...
 */
async function transferToDevices(
  deviceIds: string[],
  roomName: string,
  volume?: number
): Promise<TransferRoomResult> {
  ...
  let fromRoom: string | undefined;
  try {
    const currentResponse = await fetch(`${SPOTIFY_API_BASE}/me/player`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (currentResponse.ok) {
      const current = (await currentResponse.json()) as { device?: { name: string } };
 ...
 */
export async function setRoomVolume(
  userId: string,
  roomName: string,
  volume: number
  ...
  try {
    ...
    await fetch(
      `${SPOTIFY_API_BASE}/me/player/volume?volume_percent=${volume}&device_id=${roomDeviceIds[0]}`,
      {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      }
 ...
 */
export async function getRoomsForVoice(userId: string): Promise<ListRoomsResult> {
  ...
  let playingDeviceId: string | null = null;
  try {
    const token = await getSpotifyAccessToken();
    if (token) {
      ...
      if (response.ok) {
        const data = (await response.json()) as { device?: { id: string }; is_playing?: boolean };
