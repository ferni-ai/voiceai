...
export async function requestPin(userId) {
    ...
    try {
        ...
        if (firestore) {
            await firestore
                .collection(ECOBEE_PENDING_AUTH_COLLECTION)
                .doc(userId)
                .set(cleanForFirestore(pending));
        }
 ...
 */
export async function checkAuthorization(userId) {
    if (!ECOBEE_API_KEY) {
        return { success: false, error: 'Ecobee API key not configured' };
    }
    // Get pending auth
 ...
 */
export async function getPendingAuth(userId) {
    let pending = pendingAuth.get(userId);
    if (!pending) {
        const firestore = await getFirestore();
        if (firestore) {
...
export async function getValidAccessToken(userId) {
    ...
    if (areTokensExpired(tokens)) {
        ...
        try {
            ...
            log.warn({ userId, error: String(error) }, 'Failed to refresh Ecobee token - user needs to re-authenticate');
            return null;
        }
    }
 ...
 */
export async function isEcobeeConfigured(userId) {
 ...
 */
export async function getAllEcobeeUsers() {
    const userIds = [];
    // Add cached users
...
//# sourceMappingURL=ecobee-auth.js.map