...
export async function getSpotifyAccessToken(forceRefresh = false) {
    ...
    if (!cachedTokens) {
        getLogger().warn('🎵 No Spotify tokens available.\n' +
            '   To set up Spotify:\n' +
...
export async function ensureTokenFresh() {
    ...
    getLogger().info({ valid: status.valid, minutesRemaining: status.minutesRemaining }, '🎵 Token expiring soon - proactively refreshing');
...
export function startAutoRefresh() {
    ...
    registerInterval(SPOTIFY_AUTO_REFRESH_INTERVAL, () => {
 ...
 */
export function storeSpotifyTokens(accessToken, refreshToken, expiresIn) {
    const tokens = {
 ...
 */
export function recordSpotifyError(error) {
    lastError = error;
 ...
 */
export function getSpotifyHealthStatus() {
 ...
 */
export function logSpotifyDiagnostics() {
    ...
    if (!status.configured) {
        if (!status.hasClientId || !status.hasClientSecret) {
            getLogger().warn('🎵 Missing Spotify credentials. Add to .env:\n' +
    ...
    if (status.circuitBreakerOpen) {
        ...
        getLogger().warn(`🎵 Circuit breaker OPEN after ${stats.totalFailures} failures. Will retry in 1 minute.`);
    }
}
/**
 * Reset the circuit breaker manually (for testing/recovery)
 */
export function resetSpotifyCircuitBreaker() {
