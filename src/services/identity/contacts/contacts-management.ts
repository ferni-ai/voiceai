 ...
 *
 * @module identity/contacts/contacts-management
 *
 * PERSISTENCE:
...
export function importFromCSV(
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      const csvFirstName = columnMap.firstName >= 0 ? values[columnMap.firstName] : '';
      const csvLastName = columnMap.lastName >= 0 ? values[columnMap.lastName] : '';
      const csvPhone = columnMap.phone >= 0 ? values[columnMap.phone] : '';
      const csvEmail = columnMap.email >= 0 ? values[columnMap.email] : '';
      const csvCompany = columnMap.company >= 0 ? values[columnMap.company] : '';

      const displayName = name || `${csvFirstName} ${csvLastName}`.trim();
