 ...
 */
export async function getRoomConfig(userId) {
    try {
 ...
 */
export async function updateRoom(userId, input) {
    try {
        const config = await loadUserConfig(userId);
        ...
        if (input.name &&
            config.rooms.some((r) => r.id !== input.id && r.name.toLowerCase() === input.name.toLowerCase())) {
            return { success: false, error: `Room "${input.name}" already exists` };
        }
 ...
 */
export async function setDefaultRoom(userId, roomId) {
    try {
        const config = await loadUserConfig(userId);
 ...
 * Create a room group
 */
export async function createRoomGroup(userId, input) {
    try {
        const config = await loadUserConfig(userId);
        // Check for duplicate name
 ...
 * Delete a room group
 */
export async function deleteRoomGroup(userId, groupId) {
    try {
        const config = await loadUserConfig(userId);
 ...
 */
export async function findRoomByName(userId, roomName) {
    try {
        const config = await loadUserConfig(userId);
        ...
        if (!room) {
            room = config.rooms.find((r) => r.name.toLowerCase().includes(normalizedName) ||
 ...
 */
export async function findRoomGroupByName(userId, groupName) {
    try {
        const config = await loadUserConfig(userId);
        ...
        // Fuzzy match if no exact match
        if (!group) {
            group = config.roomGroups.find((g) => g.name.toLowerCase().includes(normalizedName) ||
 ...
 */
export async function getDevicesForRoom(userId, roomOrGroupName) {
    try {
        const config = await loadUserConfig(userId);
        ...
        // Try as a room first
        const room = config.rooms.find((r) => r.name.toLowerCase() === normalizedName ||
        ...
        // Try as a group
        const group = config.roomGroups.find((g) => g.name.toLowerCase() === normalizedName ||
            g.name.toLowerCase().includes(normalizedName) ||
            normalizedName.includes(g.name.toLowerCase()));
        if (group) {
            const deviceIds = group.roomIds.flatMap((roomId) => config.rooms.find((r) => r.id === roomId)?.deviceIds ?? []);
 ...
 */
export async function getDefaultRoom(userId) {
    try {
        const config = await loadUserConfig(userId);
...
//# sourceMappingURL=spotify-room-config-store.js.map