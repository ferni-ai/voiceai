...
import { authenticateNaturally, generateContextForLLM, getNaturalGreeting, } from './natural-auth.js';
// ============================================================================
 ...
 */
export function normalizePhoneNumber(phone, defaultCountry = 'US') {
 ...
 * 3. New user
 */
export async function identifyByPhone(phoneNumber) {
    ...
    try {
        ...
        getLogger().debug({ error: String(error), phone: normalized }, 'Error checking sponsored identities (continuing with regular lookup)');
    }
    // Priority 2: Check linked phone numbers (web users)
    // Import memory management for persistent phone cache
    const { getCachedPhoneMapping, savePhoneMapping } = await import('../memory/memory-management.js');
 ...
 */
export async function identifyByWebAuth(authId, authProvider = 'default') {
    const store = getStore();
 ...
 */
export async function identifyFromMetadata(metadata) {
            ...
            if (deviceProfile && deviceProfile.totalConversations > 0) {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        getLogger().warn({ firebaseUid: `${firebaseUid.slice(0, 8)}...`, error: result.error }, '⚠️ AUTO-MIGRATION: Migration failed, continuing with new profile');
                    }
                }
                catch (migrationError) {
                    getLogger().error({ error: String(migrationError), firebaseUid: `${firebaseUid.slice(0, 8)}...` }, '❌ AUTO-MIGRATION: Error during migration (continuing with new profile)');
                }
            }
        }
        return {
    ...
    return {
        ...
        isReturning: false,
        profile: null,
        source: { type: 'anonymous', identifier: sessionId },
        linkedIdentifiers: [],
    };
}
// ============================================================================
 ...
 */
export async function linkPhoneToProfile(userId, phoneNumber) {
    const normalized = normalizePhoneNumber(phoneNumber);
    const store = getStore();
    ...
    const links = profile.linkedIdentifiers;
 ...
 */
export async function linkWebAuthToPhone(phoneNumber, authId, authProvider = 'default') {
    const normalized = normalizePhoneNumber(phoneNumber);
    const store = getStore();
 ...
 */
async function findProfileByPhone(normalizedPhone) {
    const store = getStore();
    ...
    try {
        ...
        for (const profile of profiles) {
            const linked = profile.linkedIdentifiers || [];
 ...
 */
function getLinkedIdentifiers(profile) {
    const links = profile.linkedIdentifiers || [];
 ...
 */
export function areUsersSame(userId1, userId2) {
    if (userId1 === userId2)
        return true;
 ...
 */
export async function identifyWithNaturalAuth(metadata, voiceSketch) {
    ...
    const store = getStore();
    const profile = authContext.userId !== 'unknown' ? await store.getProfile(authContext.userId) : null;
    return {
        identification: {
            ...identification,
