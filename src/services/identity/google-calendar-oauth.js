...
const loadedTokenUsers = new Set();
// ============================================================================
 ...
 */
export function markTokenAsFailed(userId) {
 ...
 */
export function clearFailedTokenStatus(userId) {
    failedTokenCache.delete(userId);
}
/**
...
export async function storeUserTokens(userId, tokens) {
    ...
    if (firestore) {
        try {
            ...
            getLogger().info({ userId, hasRefreshToken: !!tokens.refresh_token }, 'Stored Google tokens in Firestore');
        }
        catch (err) {
        ...
        getLogger().info({ userId, hasRefreshToken: !!tokens.refresh_token }, 'Stored Google tokens (in-memory only)');
    }
}
/**
 ...
 */
export function getUserTokensSync(userId) {
...
export async function refreshAccessToken(refreshToken) {
    ...
    if (!response.ok) {
        ...
        const isPermanentError = error.includes('invalid_grant') ||
 ...
 */
export async function getEvents(accessToken, calendarId, timeMin, timeMax, maxResults = 50) {
    const rateLimiter = getRateLimiter('google-calendar');
    if (!rateLimiter.tryAcquire()) {
        getLogger().warn('Google Calendar API rate limited');
        return [];
 ...
 */
export async function getFreeBusy(accessToken, calendarIds, timeMin, timeMax) {
 ...
 */
export async function createAppointmentEvent(userId, options) {
    const accessToken = await getValidAccessToken(userId);
    if (!accessToken) {
        getLogger().warn({ userId }, 'No valid access token for calendar');
        return null;
    }
    const { title, description, location, startTime, durationMinutes = 60, calendarId = 'primary', reminders = [
 ...
 */
export async function isCalendarConfigured(userId) {
 ...
 */
export function isCalendarConfiguredSync(userId) {
 ...
 */
export async function getAllCalendarUsers() {
    const userIds = [];
