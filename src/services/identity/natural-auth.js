...
import { identifyWithVoice, mergeVoiceSketch, } from '../voice/voice-identification.js';
// ============================================================================
 ...
 */
export async function verifyIdentity(userId, userResponse) {
    const store = getDefaultStore();
 ...
 */
export async function enrollVoice(userId, voiceSketch) {
    const store = getDefaultStore();
    const profile = await store.getProfile(userId);
    if (!profile) {
 ...
 */
export async function updateVoiceSignature(userId, voiceSketch) {
 ...
 */
export async function linkIdentifier(userId, identifier) {
    const store = getDefaultStore();
    const profile = await store.getProfile(userId);
    if (!profile) {
...
// CONTEXT BUILDERS
// ============================================================================
function buildAuthContext(voiceResult, requireVerification) {
    ...
    else if (voiceResult.identificationMethod === 'voice' &&
...
function buildWarmGreeting(name) {
...
function buildSecurityContext(profile) {
    const questions = [];
    const answers = [];
    ...
    if (profile.familyMembers?.length) {
        const spouse = profile.familyMembers.find((m) => m.relationship.toLowerCase().includes('spouse') ||
        ...
        const children = profile.familyMembers.filter((m) => m.relationship.toLowerCase().includes('son') ||
        ...
        if (children.length > 0) {
            ...
            for (const child of children) {
                if (child.name)
                    answers.push(child.name);
            }
        }
        const pets = profile.familyMembers.filter((m) => m.relationship.toLowerCase().includes('pet') ||
...
function getVerificationQuestion(profile) {
    if (!profile)
        return undefined;
    const securityContext = buildSecurityContext(profile);
 ...
 */
export function getNaturalGreeting(authContext) {
 ...
 */
export function generateContextForLLM(authContext) {
    const lines = [];
    ...
    switch (authContext.action) {
        ...
        case 'greet_casually':
            ...
            break;
        case 'confirm_gently':
            lines.push(`Ask gently if this is ${authContext.userName}. Your voice recognition suggests it might be them.`);
            break;
        case 'ask_naturally':
            lines.push(`This appears to be a new user. Ask their name naturally in conversation.`);
            break;
        case 'verify_security':
            lines.push(`Before proceeding with sensitive operations, verify identity: "${authContext.verificationQuestion}"`);
            break;
        case 'enroll_voice':
            lines.push(`Known user but no voice signature yet. Greet normally - voice will be enrolled automatically.`);
            break;
    }
    return lines.join('\n');
}
...
//# sourceMappingURL=natural-auth.js.map