 ...
 */
function getPersonaVoiceDna(personaId) {
    if (!personaId)
...
let latencyCount = 0;
function recordMetric(contentType, outcome, latencyMs) {
 ...
 */
export function resetContentMetrics() {
...
function getCacheKey(contentType, context) {
    const parts = [
        contentType,
 ...
 */
export function registerContentGenerator(contentType, config) {
    ...
    const generationPromise = (async () => {
        try {
            ...
            if (result) {
                ...
                cleanCache();
                log.debug({ contentType: context.contentType, content: cleaned.slice(0, 50), latencyMs }, '📝 LLM content generated');
 ...
 */
export function getContentSync(context) {
    const generator = generators.get(context.contentType);
    if (!generator)
        return null;
    const cacheKey = getCacheKey(context.contentType, context);
 ...
 */
export function getContentWithFallback(context) {
    const generator = generators.get(context.contentType);
    // Try cache first
 ...
 */
export async function prewarmContent(contexts) {
...
//# sourceMappingURL=llm-dynamic-content.js.map