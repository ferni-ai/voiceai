...
import { getFavorites, getGroups, getHouseholds, playFavorite, setPlaybackState, setGroupVolume, getCurrentTrack, setTokenRefreshCallback, getSonosCircuitBreakerStatus, SonosApiError, } from './sonos.js';
 ...
 */
export function createTokenRefreshHandler(userId) {
    setTokenRefreshCallback(async (newCredentials) => {
 ...
 */
export function isSonosAvailable() {
...
const lastUsedRoomCache = new Map();
/**
 * Get the user's last used room
 */
export function getLastUsedRoom(userId) {
    return lastUsedRoomCache.get(userId);
}
/**
 * Set the user's last used room
 */
export function setLastUsedRoom(userId, room) {
 ...
 */
export function matchRoomName(query, groups) {
    ...
    if (exactMatch)
        return exactMatch;
    // Try substring match
    const substringMatch = groups.find((g) => g.name.toLowerCase().includes(normalizedQuery) ||
    ...
    for (const [canonical, aliasList] of Object.entries(aliases)) {
        if (normalizedQuery.includes(canonical) ||
            ...
            const aliasMatch = groups.find((g) => {
                ...
                return (groupLower.includes(canonical) ||
 ...
 */
export async function getAvailableRooms(credentials) {
    try {
 ...
 */
export async function searchFavorites(credentials, query) {
    try {
        const households = await getHouseholds(credentials);
        const allFavorites = [];
        for (const household of households) {
            const favorites = await getFavorites(credentials, household.id);
            const matches = favorites.filter((f) => f.name.toLowerCase().includes(query.toLowerCase()) ||
 ...
 */
export async function playSonosFavorite(credentials, userId, favoriteName, roomName) {
    ...
    try {
        ...
        for (const household of households) {
            ...
            const favorite = favorites.find((f) => f.name.toLowerCase().includes(favoriteName.toLowerCase()) ||
 ...
 */
export async function playSonosMusic(credentials, userId, query, roomName) {
    ...
    try {
        ...
        for (const household of households) {
            ...
            if (smartMatches.length > 0 && smartMatches[0].score >= 10) {
                ...
                log.info({ favorite: bestMatch.name, room: targetGroup.name, query, score: bestMatch.score }, '🔊 Playing smart-matched Sonos favorite');
 ...
 */
function scoreFavorite(favorite, query) {
    ...
    const wordOverlap = queryWords.filter((qw) => favWords.some((fw) => fw.includes(qw) || qw.includes(fw))).length;
 ...
 */
function findSmartMatchingFavorites(query, favorites) {
    // Score all favorites
 ...
 */
function findVibeMatchingFavorites(query, favorites) {
    return findSmartMatchingFavorites(query, favorites);
}
/**
 * Get suggestions when no matches found
 */
function getSuggestions(query, favorites) {
    const queryLower = query.toLowerCase();
 ...
 */
export async function pauseSonos(credentials, userId, roomName) {
    try {
 ...
 */
export async function resumeSonos(credentials, userId, roomName) {
    try {
        const room = await resolveRoom(credentials, userId, roomName);
        if (!room.success)
            return room;
 ...
 */
export async function setSonosVolume(credentials, userId, volume, roomName) {
    try {
        const room = await resolveRoom(credentials, userId, roomName);
        if (!room.success)
            return room;
 ...
 */
export async function getSonosNowPlaying(credentials, userId, roomName) {
    try {
        const room = await resolveRoom(credentials, userId, roomName);
        if (!room.success)
            return room;
 ...
 */
async function resolveRoom(credentials, userId, roomName) {
    // Register token refresh callback
    createTokenRefreshHandler(userId);
    // Check circuit breaker
    if (!isSonosAvailable()) {
