...
function recordFailure() {
    ...
    if (circuitBreaker.failures >= CIRCUIT_BREAKER_THRESHOLD) {
        ...
        log.warn({ failures: circuitBreaker.failures, nextRetryMs: CIRCUIT_BREAKER_HALF_OPEN_MS }, '🔌 Sonos circuit breaker OPEN - too many failures');
    }
}
/**
 ...
 */
export function resetSonosCircuitBreaker() {
    circuitBreaker.failures = 0;
    circuitBreaker.isOpen = false;
    circuitBreaker.lastFailure = 0;
    circuitBreaker.nextRetry = 0;
 ...
 */
async function sonosRequest(credentials, endpoint, method = 'GET', body, isRetry = false) {
    // Check circuit breaker
    if (isCircuitOpen()) {
        throw new SonosApiError('Sonos service temporarily unavailable (circuit breaker open)', 503, endpoint);
    }
        ...
        if (!response.ok) {
            ...
            if (response.status === 401 && !isRetry) {
                ...
                if (refreshed) {
                    ...
                    return sonosRequest(refreshed, endpoint, method, body, true);
                }
                // Refresh failed
                recordFailure();
                throw new SonosApiError('Sonos authentication failed - please reconnect your account', 401, endpoint);
            }
 ...
 */
async function attemptTokenRefresh(credentials) {
 ...
 */
export async function getHouseholds(credentials) {
    const response = await sonosRequest(credentials, '/households');
    return response.households;
}
/**
 * Get all player groups in a household
 */
export async function getGroups(credentials, householdId) {
    const response = await sonosRequest(credentials, `/households/${householdId}/groups`);
    // Get volume for each group
    const groupsWithVolume = await Promise.all(response.groups.map(async (group) => {
        try {
 ...
 */
export async function getPlayers(credentials, householdId) {
    const response = await sonosRequest(credentials, `/households/${householdId}/groups`);
 ...
 * Play/pause a group
 */
export async function setPlaybackState(credentials, groupId, action) {
 ...
 * Skip to next track
 */
export async function skipToNext(credentials, groupId) {
    await sonosRequest(credentials, `/groups/${groupId}/playback/skipToNextTrack`, 'POST');
}
/**
 * Skip to previous track
 */
export async function skipToPrevious(credentials, groupId) {
    await sonosRequest(credentials, `/groups/${groupId}/playback/skipToPreviousTrack`, 'POST');
}
/**
 * Get current playback status
 */
export async function getPlaybackStatus(credentials, groupId) {
    return sonosRequest(credentials, `/groups/${groupId}/playback`);
}
/**
 * Get current track info
 */
export async function getCurrentTrack(credentials, groupId) {
    try {
        const response = await sonosRequest(credentials, `/groups/${groupId}/playbackMetadata`);
        if (!response.currentItem?.track)
            return null;
 ...
 * Get group volume
 */
export async function getGroupVolume(credentials, groupId) {
    return sonosRequest(credentials, `/groups/${groupId}/groupVolume`);
}
/**
 * Set group volume (0-100)
 */
export async function setGroupVolume(credentials, groupId, volume) {
 ...
 */
export async function setGroupMute(credentials, groupId, muted) {
 ...
 */
export async function adjustVolume(credentials, groupId, delta) {
 ...
 */
export async function getFavorites(credentials, householdId) {
    const response = await sonosRequest(credentials, `/households/${householdId}/favorites`);
 ...
 * Play a favorite
 */
export async function playFavorite(credentials, groupId, favoriteId) {
 ...
 */
export async function setAllGroupsVolume(credentials, volume) {
 ...
 */
export async function playVibeMusic(credentials, vibe, targetRoom) {
    const vibeKeywords = {
    ...
    for (const household of households) {
        ...
        const matchingFavorite = favorites.find((fav) => keywords.some((keyword) => fav.name.toLowerCase().includes(keyword) ||
