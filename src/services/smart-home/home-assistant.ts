...
export async function getEntities(): Promise<HAEntity[]> {
  ...
  try {
    const entities = await haRequest<HAEntity[]>('/states');
    return entities;
  } catch (error) {
 ...
 * Get entity by ID
 */
export async function getEntity(entityId: string): Promise<HAEntity | null> {
  if (!isHomeAssistantConfigured()) return null;

  try {
    const entity = await haRequest<HAEntity>(`/states/${entityId}`);
    return entity;
  } catch {
    return null;
  }
 ...
 */
export async function callService(call: HAServiceCall): Promise<boolean> {
  if (!isHomeAssistantConfigured()) {
    log.warn('Home Assistant not configured');
    return false;
  }
 ...
 * Turn off a light
 */
export async function turnOffLight(entityId: string): Promise<string> {
  const entity = await getEntity(entityId);
  const name = entity?.attributes?.friendly_name || entityId;
 ...
 * Toggle a light
 */
export async function toggleLight(entityId: string): Promise<string> {
  const entity = await getEntity(entityId);
  const name = entity?.attributes?.friendly_name || entityId;
...
export async function setThermostat(
  entityId: string,
  temperature: number,
  hvacMode?: 'heat' | 'cool' | 'heat_cool' | 'off'
): Promise<string> {
  const entity = await getEntity(entityId);
  const name = entity?.attributes?.friendly_name || entityId;
 ...
 * Lock/unlock a lock
 */
export async function setLock(entityId: string, lock: boolean): Promise<string> {
  const entity = await getEntity(entityId);
  const name = entity?.attributes?.friendly_name || entityId;
 ...
 * Activate a scene
 */
export async function activateScene(sceneId: string): Promise<string> {
 ...
 */
export async function setSwitch(entityId: string, on: boolean): Promise<string> {
  const entity = await getEntity(entityId);
  const name = entity?.attributes?.friendly_name || entityId;
 ...
 */
export async function findEntitiesByName(query: string, domain?: string): Promise<HAEntity[]> {
 ...
 */
export async function getRoomStatus(roomName: string): Promise<string> {
