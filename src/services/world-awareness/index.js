...
import { getFinancialNews, getGeneralNews, getTechNews, } from '../../tools/domains/information/news.js';
...
// ============================================================================
function generateWeatherHook(weatherText) {
...
async function fetchSportsData(favoriteTeams) {
        ...
        for (const team of favoriteTeams.slice(0, 5)) {
            ...
            try {
                ...
                if (score && !score.includes("couldn't find")) {
                    ...
                    if ((scoreText.includes('in progress') || scoreText.includes('live')) &&
 ...
 */
export async function warmWorldCache(userId, interests = {}) {
    const fullInterests = {
 ...
 */
export function getWorldSnapshot(userId) {
...
export function updateUserInterests(userId, interests) {
    ...
    if (interests.favoriteTeams) {
        const newTeams = interests.favoriteTeams.filter((t) => !cache.interests.favoriteTeams.includes(t));
 ...
 */
export function getConversationStarter(userId) {
 ...
 */
export function detectTeamMention(text) {
    const teamPatterns = [
        // NFL
...
// BACKGROUND REFRESH
// ============================================================================
async function refreshStaleData(userId, stale) {
    const cache = userCaches.get(userId);
    ...
    try {
        ...
        if (stale.weatherStale && cache.interests.location) {
            promises.push(fetchWeatherData(cache.interests.location).then((weather) => {
                if (weather)
                    cache.weather = weather;
            }));
        }
        if (stale.newsStale) {
            promises.push(fetchNewsData().then((news) => {
                if (news)
                    cache.news = news;
            }));
        }
        if (stale.sportsStale && cache.interests.favoriteTeams.length > 0) {
            promises.push(fetchSportsData(cache.interests.favoriteTeams).then((sports) => {
                if (sports)
                    cache.sports = sports;
            }));
        }
