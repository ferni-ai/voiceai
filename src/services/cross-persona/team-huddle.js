 ...
 */
export function getObservations(userId) {
 ...
 */
export function clearObservations(userId) {
 ...
 */
function detectConnections(observations) {
    const connections = [];
    // Group by persona
 ...
 */
function findConnection(obs1, obs2) {
    // Same topic connection
    const sharedTopics = (obs1.relatedTopics || []).filter((t) => (obs2.relatedTopics || []).includes(t));
 ...
 */
function detectCausalPattern(obs1, obs2) {
    ...
    for (const pattern of patterns) {
        const obs1Match = obs1.domain === pattern.domain1 &&
            pattern.keywords1.some((kw) => obs1.content.toLowerCase().includes(kw));
        const obs2Match = obs2.domain === pattern.domain2 &&
        ...
        // Check reverse
        const obs1MatchRev = obs1.domain === pattern.domain2 &&
            pattern.keywords2.some((kw) => obs1.content.toLowerCase().includes(kw));
        const obs2MatchRev = obs2.domain === pattern.domain1 &&
 ...
 */
function synthesizeInsights(observations, connections) {
 ...
 */
function generateRecommendations(observations, connections) {
    const recommendations = [];
    // High-confidence concerns → Proactive mention
    const highConfidenceConcerns = observations.filter((o) => o.observationType === 'concern' && o.confidence > 0.7);
    ...
    const opportunities = observations.filter((o) => o.observationType === 'opportunity' && o.confidence > 0.6);
    for (const opp of opportunities.slice(0, 2)) {
        if (opp.personaId !== 'ferni') {
            recommendations.push({
                ...
                reason: opp.content,
                priority: 'low',
                suggestedApproach: opp.suggestedAction ||
 ...
 */
function assessUserState(observations, connections) {
    // Count positive vs negative observations
    const concerns = observations.filter((o) => o.observationType === 'concern');
    const opportunities = observations.filter((o) => o.observationType === 'opportunity' || o.observationType === 'milestone');
    // Calculate wellbeing estimate
    const totalObs = observations.length || 1;
    const wellbeing = Math.max(0.2, Math.min(0.9, 0.5 + ((opportunities.length - concerns.length) / totalObs) * 0.3));
    ...
    const recentObs = observations.filter((o) => Date.now() - o.detectedAt.getTime() < 3 * 24 * 60 * 60 * 1000);
 ...
 */
export async function generateTeamHuddle(userId) {
 ...
 */
export function formatTeamHuddleForLLM(huddle) {
    const lines = [];
...
//# sourceMappingURL=team-huddle.js.map