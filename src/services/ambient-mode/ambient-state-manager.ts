 ...
 */
export async function storeAmbientState(state: AmbientState): Promise<void> {
 ...
 */
export async function getAmbientState(userId: string): Promise<AmbientState | null> {
  const db = getFirestoreDb();
  if (!db) return null;
  ...
  try {
    ...
    return doc.data() as AmbientState;
  } catch (error) {
 ...
 */
export async function getAmbientPreferences(userId: string): Promise<AmbientPreferences | null> {
  const db = getFirestoreDb();
  if (!db) return null;
  ...
  try {
    ...
    return doc.data() as AmbientPreferences;
  } catch (error) {
 ...
 */
export async function updateAmbientPreferences(
  userId: string,
  prefs: Partial<AmbientPreferences>
): Promise<void> {
  const db = getFirestoreDb();
  if (!db) return;
...
const lastNudgeTimes = new Map<string, Map<NudgeType, number>>();

/**
 * Evaluate and generate nudge based on ambient state
 */
export async function evaluateNudge(state: AmbientState): Promise<AmbientNudge | null> {
  ...
  if (prefs && !prefs.allowPushNudges) return null;

  // Don't nudge during DND
  if (doNotDisturbEnabled) return null;
  ...
  if (withCooldowns.length === 0) return null;
...
function recordNudgeSent(userId: string, nudgeType: NudgeType): void {
...
function getMorningMessage(state: AmbientState): string {
  const messages = [
...
function getEveningMessage(state: AmbientState): string {
  const messages = [
    'How was your day?',
...
function getCommuteMessage(state: AmbientState): string {
  const messages = [
 ...
 */
export async function handleAmbientSync(request: AmbientSyncRequest): Promise<AmbientSyncResponse> {
 ...
 */
export async function buildAmbientContext(userId: string): Promise<AmbientContext> {
  ...
  // Time awareness
  const timeDescriptions: Record<AmbientState['timeOfDay'], string> = {
 ...
 */
export async function getAmbientContextInjection(userId: string): Promise<string> {
  ...
  if (context.environmentAwareness) lines.push(`- ${context.environmentAwareness}`);
