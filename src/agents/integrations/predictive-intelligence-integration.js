...
import { processTurnForSuperhumanLearning, processSessionStart as superhumanSessionStart, processSessionEnd as superhumanSessionEnd, markSuperhumanDirty, } from '../../intelligence/predictive/index.js';
// 🧬 EMBEDDING INTELLIGENCE: Vector-powered predictions
import { embeddingPersistence, conversationTrajectory, getEmbeddingPredictiveContext, } from '../../intelligence/predictive/embeddings/index.js';
// Import the three pattern systems
import { recordObservation, } from '../../services/superhuman/predictive-coaching.js';
import { getSuperhumanObservations, } from '../../services/superhuman/observations.js';
import { detectPatternsInTranscript, } from '../../intelligence/coaching-patterns.js';
 ...
 */
export function configurePredictiveIntelligence(newConfig) {
 ...
 */
export function initializePredictiveIntelligence(sessionId, userId, existingObservations, sessionData) {
    // Get or create the superhuman observations engine
    const engine = getSuperhumanObservations(userId, existingObservations);
 ...
 */
export function cleanupPredictiveIntelligence(sessionId, userId, sessionSummary) {
 ...
 */
export async function processForPredictiveIntelligence(observation) {
    const { userId, sessionId, message, topic, emotion, emotionIntensity, dayOfWeek, hourOfDay, turnCount, sessionCount, relationshipStage, } = observation;
    let patternsRecorded = 0;
    let superhumanObservationDetected = false;
    let surfacingContent;
    try {
        // ========================================================================
        ...
        if (config.useAsyncEvents) {
            ...
            AsyncEvents.emit('prediction:observation', {
            ...
            AsyncEvents.emit('conversation:turn', { message, topic, emotion, dayOfWeek, hourOfDay }, { userId, sessionId });
        }
        else {
            ...
            for (const pattern of coachingPatterns) {
                ...
                observationPromises.push(recordObservation(userId, {
                    type: predictiveType,
            ...
            const hasSignificantEmotion = emotion !== undefined && emotionIntensity !== undefined && emotionIntensity > 0.5;
        ...
        if (engine) {
            ...
            if (surfacing.shouldSurface && surfacing.phrase) {
                ...
                log.info({ userId, pattern: surfacing.observation?.observation }, '🔍 Superhuman observation ready to surface');
            }
        }
        log.debug({ userId, patternsRecorded, superhumanObservationDetected }, '🔮 Predictive intelligence processed turn');
    }
    catch (error) {
 ...
 */
function mapToPredictiveType(coachingType) {
    const mapping = {
 ...
 */
export async function getPredictiveContextForTurn(userId, sessionId) {
    const sections = [];
    try {
        // Get predictions from predictive coaching
        const { generatePredictions, getDayPatterns } = await import('../../services/superhuman/predictive-coaching.js');
        ...
        try {
            const { getSuperhumanPredictiveContext } = await import('../../intelligence/predictive/index.js');
...
//# sourceMappingURL=predictive-intelligence-integration.js.map