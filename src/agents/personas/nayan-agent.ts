...
// Superhuman Wisdom Tools - "Better Than Human" capabilities (merged into wisdom/ domain)
import { wisdomIntelligenceTools } from '../../tools/domains/wisdom/intelligence.js';
...
async function buildSuperhumanWisdomTools(userId: string): Promise<ToolSet> {
  const toolDefs = wisdomIntelligenceTools;
  const ctx: ToolContext = {
    agentId: 'nayan-patel',
    agentDisplayName: 'Nayan',
    userId,
