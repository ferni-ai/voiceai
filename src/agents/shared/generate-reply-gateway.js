...
import { markLLMRequestSent, markLLMFirstToken, markLLMComplete, markAudioStarted, } from './e2e-latency-tracker.js';
const log = getLogger();
/**
...
function extractGeminiErrorDetails(error) {
    ...
    if (errorStr.includes('waitForPlayout') &&
    ...
    if (errorObj.response && typeof errorObj.response === 'object') {
        ...
        if (response.data && typeof response.data === 'object') {
            ...
            if (data.error && typeof data.error === 'object') {
                ...
                if (innerError.code)
                    details.errorCode = innerError.code;
            }
        }
    }
    return details;
 ...
 */
export function registerSessionForReconnection(sessionId, session) {
    sessionObjects.set(sessionId, session);
}
/**
 * Unregister a session (on cleanup).
 */
export function unregisterSessionForReconnection(sessionId) {
 ...
 */
async function handleGeminiDeath(sessionId) {
    ...
    if (attempts.count >= MAX_RECONNECTION_ATTEMPTS) {
        log.warn({ sessionId, attempts: attempts.count }, '💀 [GATEWAY] Max reconnection attempts reached - giving up');
        return false;
    }
    if (now - attempts.lastAttempt < RECONNECTION_COOLDOWN_MS && attempts.count > 0) {
        log.debug({ sessionId, cooldownRemainingMs: RECONNECTION_COOLDOWN_MS - (now - attempts.lastAttempt) }, '💀 [GATEWAY] Reconnection on cooldown');
        return false;
    }
    // Update attempt tracking
    reconnectionAttempts.set(sessionId, { count: attempts.count + 1, lastAttempt: now });
    log.warn({ sessionId, attempt: attempts.count + 1 }, '💀 [GATEWAY] Gemini dead - attempting reconnection...');
    ...
    try {
        ...
        log.error({ sessionId, error: String(err), attempt: attempts.count + 1 }, '💀 [GATEWAY] Gemini reconnection failed');
        return false;
    }
}
/**
 ...
 */
async function triggerGracefulExit(sessionId) {
    ...
    try {
        ...
        log.error({ error: String(err), sessionId }, '👋 [GATEWAY] Graceful exit failed - user may be stuck');
    }
}
// ============================================================================
 ...
 */
export function getGatewayStats(sessionId) {
    const state = sessionStates.get(sessionId);
    return (state?.stats ?? {
        totalCalls: 0,
        successfulCalls: 0,
 ...
 */
export function clearPendingLowPriorityResponse(sessionId) {
    const state = sessionStates.get(sessionId);
 ...
 */
export function markSessionReady(sessionId) {
 ...
 */
export function markSessionNotReady(sessionId, reason) {
    const state = getSessionState(sessionId);
    state.isReady = false;
 ...
 */
export async function waitForSessionReady(sessionId, timeoutMs = 20000) {
    const state = getSessionState(sessionId);
    if (state.isReady) {
        return true;
    }
 ...
 * ```
 */
export async function generateReply(session, sessionId, options) {
    const { instructions, allowInterruptions = true, context = 'unknown', priority = 'normal', waitForPlayout = true, fallbackMessage, timeoutMs = 4000, // Reduced from 6000 for human-like response latency
     } = options;
    ...
    if (timeSinceLastCall < debounceMs && priority !== 'high') {
        ...
        log.debug({ sessionId, context, timeSinceLastCall, debounceMs, priority }, '⏸️ [GATEWAY] Debouncing rapid call');
        return {
            success: false,
            usedFallback: false,
            debounced: true,
    ...
    // -------------------------------------------------------------------------
    if (!state.isReady) {
        log.debug({ sessionId, context, isReady: state.isReady }, '⏳ [GATEWAY] Session not ready - queueing or rejecting');
    ...
    if (state.consecutiveFailures >= 3) {
        ...
        log.info({ sessionId, context, timeSinceOpenMs: timeSinceOpen }, '🔄 [GATEWAY] Circuit breaker HALF-OPEN - allowing test call');
    }
    // -------------------------------------------------------------------------
    // SAFEGUARD 3: Concurrent call limit
    // -------------------------------------------------------------------------
    if (state.pendingCallCount >= 2) {
        log.warn({ sessionId, context, pendingCalls: state.pendingCallCount }, '🚧 [GATEWAY] Too many pending calls - rejecting');
        return {
            success: false,
            usedFallback: false,
    ...
    if (state.hasActiveLowPriorityResponse && priority !== 'low') {
        ...
        const timeSinceLowPriority = state.lowPriorityResponseStartedAt
            ? Date.now() - state.lowPriorityResponseStartedAt
            : 0;
        log.debug({ sessionId, context, priority, timeSinceLowPriority }, '🛑 [GATEWAY] Interrupting active low-priority response before new request');
        try {
            session.interrupt();
            ...
            log.debug({ error: String(interruptErr) }, 'Low-priority interrupt failed (non-critical, continuing anyway)');
        }
        // Clear the flag
        state.hasActiveLowPriorityResponse = false;
        state.lowPriorityResponseStartedAt = undefined;
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => {
            timeoutId = setTimeout(() => {
                ...
                if (speechStarted) {
                    log.debug({ sessionId, context, timeoutMs }, '⏸️ [GATEWAY] Timeout reached but agent is speaking - allowing playout to complete');
                ...
                try {
                    session.interrupt();
                    log.debug({ sessionId, context, timeoutMs }, '🛑 [GATEWAY] Session interrupted on timeout (no speech started)');
                }
                catch (interruptErr) {
        ...
        const replyPromise = (async () => {
            ...
            if (waitForPlayout) {
                ...
                handle.waitForPlayout().catch((err) => {
                    log.debug({ error: err.message, context }, '🔇 [GATEWAY] Swallowed dangling playout rejection');
                });
                await handle.waitForPlayout();
            }
        })();
        ...
        replyPromise.catch((err) => {
            log.debug({ error: err.message, context }, '🔇 [GATEWAY] Swallowed dangling generateReply rejection');
        });
        try {
        ...
        if (wasCircuitBreakerOpen) {
            log.info({ sessionId, context, latencyMs }, '✅ [GATEWAY] Circuit breaker CLOSED - recovered!');
        }
        else {
        ...
        if (errorDetails.errorType === 'session_draining') {
            log.debug({ sessionId, context, error: errorMessage, latencyMs }, '🔄 [GATEWAY] Session draining (post-handoff) - skipping gracefully');
        ...
        if (errorDetails.isGeminiDead && state.consecutiveFailures >= 2 && priority !== 'low') {
            log.warn({ sessionId, context, consecutiveFailures: state.consecutiveFailures }, '💀 [GATEWAY] Gemini appears dead - attempting proactive reconnection');
        ...
        if (state.circuitBreakerOpenedAt !== undefined && state.consecutiveFailures >= 3) {
            ...
            log.warn({ sessionId, context, error: errorMessage, latencyMs }, '⚡ [GATEWAY] Half-open test call failed - circuit breaker re-OPEN');
        }
        else {
            log.warn({
                sessionId,
 ...
 */
export async function prewarmSession(session, sessionId) {
    const startTime = Date.now();
    ...
    if (SKIP_PREWARM_GENERATEREPLY) {
        ...
        log.info({ sessionId, durationMs: Date.now() - startTime }, '🔥 [GATEWAY] Prewarm complete (QUICK MODE - lazy connection)');
        return true;
    }
    ...
    try {
        ...
        if (cancelledSessions.has(sessionId)) {
            log.warn({ sessionId, durationMs: Date.now() - startTime }, '⚠️ [GATEWAY] Prewarm completed but session was cancelled - not marking ready');
            return false;
        }
        // SUCCESS - Gemini connection is now established
        markSessionReady(sessionId);
        log.info({ sessionId, durationMs: Date.now() - startTime }, '🔥 [GATEWAY] FULL prewarm complete - Gemini connection established');
        return true;
    }
    catch (error) {
 ...
 */
export function prewarmSessionAsync(session, sessionId) {
...
//# sourceMappingURL=generate-reply-gateway.js.map