 ...
 */
export function recordAction(sessionId, toolId, args, success, resultSummary) {
    if (!sessionId)
        return;
    const record = {
        toolId,
 ...
 */
export function wasActionExecuted(sessionId, toolId, filter) {
 ...
 */
export function wasHighImpactActionExecuted(sessionId, query) {
    const history = sessionHistory.get(sessionId);
    if (!history || history.length === 0) {
        return {
            executed: false,
    ...
    for (let i = history.length - 1; i >= 0; i--) {
        ...
        if (query.contact) {
            ...
            const recordContact = record.args.contact?.toLowerCase() || '';
            const recordRecipient = record.args.recipient?.toLowerCase() || '';
 ...
 */
export function getActionSummary(sessionId) {
    const history = sessionHistory.get(sessionId) || [];
    const byTool = {};
 ...
 */
export function getHumanReadableSummary(sessionId) {
 ...
 */
function generateActionDescription(toolId, args) {
