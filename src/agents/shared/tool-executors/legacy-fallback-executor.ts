...
const log = createLogger({ module: 'LegacyFallbackExecutor' });

/**
 * Execute legacy tool routing for tools not yet in modular executors.
 * Returns the result if handled, or null if no match.
 */
export async function executeLegacyFallback(
  fn: string,
  ...
  if (fnLower === 'getcalendartoday') {
    ...
    return executeLegacyFallback('getschedule', { date: 'today' }, ctx);
  }
  ...
  if (fnLower === 'searchapplemusic') {
    ...
    // Route to modular music executor (playmusic)
    const { routeToToolModular } = await import('./index.js');
    const modularResult = await routeToToolModular('playmusic', { query: query || '' }, ctx);
    if (modularResult !== null) return modularResult;
    return null;
  }
  ...
  if (fnLower === 'getappleweather') {
    ...
    return executeLegacyFallback('getweather', { location: location || 'current' }, ctx);
  }
  ...
  if (fnLower === 'getnews') {
    ...
    // Route to searchNews
    return executeLegacyFallback('searchnews', { query: topic || category || 'top stories' }, ctx);
  }
