...
export async function withResilience(fn, options) {
    const { maxRetries, baseDelay, maxDelay = 30000, operationName, onRetry, onBeforeRetry, } = options;
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        try {
            ...
            if (operationName) {
                process.stderr.write(`[lightweight-resilience] ${operationName} failed (attempt ${attempt}/${maxRetries + 1}), ` +
            ...
            if (onBeforeRetry) {
                try {
                    ...
                    process.stderr.write(`[lightweight-resilience] ${operationName} cleanup warning: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}\n`);
                }
            }
            // Wait before retry
...
//# sourceMappingURL=lightweight-resilience.js.map