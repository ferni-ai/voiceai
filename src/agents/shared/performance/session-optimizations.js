 ...
 */
export async function prewarmUserEmbeddings(userId, config = {}) {
    ...
    try {
        ...
        for (const doc of factsSnapshot.docs) {
            const data = doc.data();
            const fact = (data.fact || data.content);
        ...
        if (textsToWarm.length > 0) {
            ...
            log.info({ userId, warmedCount: uniqueTexts.length, durationMs }, '🔥 Pre-warmed user embeddings');
 ...
 */
export async function executeToolsParallel(toolCalls, executor) {
    ...
    if (parallel.length > 0) {
        ...
        const parallelResults = await Promise.allSettled(parallel.map(async (call) => {
            const start = Date.now();
        ...
        log.debug({ parallelCount: parallel.length, durationMs: Date.now() - parallelStart }, '⚡ Executed tools in parallel');
    }
 ...
 */
function getSessionMemoryCache(sessionId) {
 ...
 */
export function getCachedMemoryResult(sessionId, query) {
 ...
 */
export function cacheMemoryResult(sessionId, query, result) {
    const cache = getSessionMemoryCache(sessionId);
    const normalizedQuery = query.toLowerCase().trim();
 ...
 */
export function clearSessionMemoryCache(sessionId) {
...
const sessionPrefetchState = new Map();
 ...
 */
export function startSpeculativePrefetch(sessionId, partialText, prefetchFn) {
    // Skip if too short
 ...
 */
export function getSpeculativePrefetch(sessionId, finalText) {
    const state = sessionPrefetchState.get(sessionId);
    if (!state)
        return null;
    // Check if stale
    ...
    const isRelated = finalLower.startsWith(prefetchLower) || finalLower.includes(prefetchLower.slice(0, 20));
    if (!isRelated) {
    ...
    if (state.prefetchedContext) {
        ...
        const result = state.prefetchedContext;
        sessionPrefetchState.delete(sessionId);
        return result;
    }
    return null;
 ...
 */
export function clearSpeculativePrefetch(sessionId) {
    sessionPrefetchState.delete(sessionId);
}
// ============================================================================
 ...
 */
export async function optimizeSessionStart(sessionId, userId) {
    const startTime = Date.now();
