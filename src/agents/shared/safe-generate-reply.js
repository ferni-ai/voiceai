...
// ============================================================================
function checkCircuitBreaker(session, fallbackMessage, context, sessionId) {
    if (!failureTracker.shouldSkip()) {
        return null;
    }
    logger.debug({ context, failures: failureTracker.getFailureCount() }, `⚡ Circuit breaker OPEN - ${context} paused (protective measure, auto-resets soon)`);
    if (fallbackMessage) {
        try {
...
// ============================================================================
function checkMutualExclusion(context) {
    if (generateReplyInProgress) {
        logger.warn({ context, currentContext }, '🔒 MUTEX: Skipping generateReply - another call in progress');
        return {
            success: false,
            usedFallback: false,
...
function checkContextSize(instructions) {
    ...
    if (critical) {
        logger.error({ size, estimatedTokens, maxAllowed: CRITICAL_CONTEXT_CHARS }, '🚨 CRITICAL: Context size exceeds safe limits - likely to cause Gemini failure');
    }
    else if (warning) {
        logger.warn({ size, estimatedTokens, recommended: MAX_CONTEXT_CHARS }, '⚠️ Context size exceeds recommended limit - may cause slow responses');
    }
 ...
 */
function checkConnectionHealth(session) {
    try {
...
async function executeWithTimeout(session, instructions, allowInterruptions, waitForPlayout, timeoutMs, sessionId, context, fallbackMessage, priority = 'normal') {
    ...
    replyPromise.catch((err) => {
        ...
        logger.debug({ error: err instanceof Error ? err.message : String(err) }, '🔇 Swallowed dangling generateReply rejection (expected after pre-emptive timeout)');
    });
...
// ============================================================================
function speakFallback(session, fallbackMessage, context, errorMessage, sessionId) {
    try {
        // Use coordinated speech if sessionId available
        if (sessionId) {
            coordinatedSay(sessionId, fallbackMessage, { allowInterruptions: true });
 ...
 * ```
 */
export async function safeGenerateReply(session, options) {
    const { instructions, allowInterruptions = true, fallbackMessage, waitForPlayout = true, timeoutMs = SAFE_TIMEOUT_MS, context = 'unknown', sessionId, bypassCircuitBreaker = false, bypassSessionClosingCheck = false, priority = 'normal', } = options;
    ...
    try {
        const result = await executeWithTimeout(session, instructions, allowInterruptions, waitForPlayout, timeoutMs, sessionId, context, fallbackMessage, priority // FIX: Pass through priority so low-priority requests don't affect circuit breaker
        );
        ...
        if (result.success) {
            ...
            logger.debug({ context, error: result.error, instructionChars: instructions.length }, '🔄 generateReply failed, used fallback');
            return {
                success: false,
                usedFallback: true,
                error: result.error,
 ...
 */
export async function safeSay(session, text, options) {
    const { allowInterruptions = true, timeoutMs = 5000, context = 'say', sessionId } = options ?? {};
    try {
        const sayPromise = new Promise((resolve) => {
            // Use coordinated speech if sessionId available
            if (sessionId) {
 ...
 */
export function canGenerateReply() {
    if (failureTracker.shouldSkip()) {
 ...
 */
export function forceUnlockMutex() {
    if (generateReplyInProgress) {
 ...
 * ```
 */
export function behavioralInstruction(situation, instruction) {
 ...
 */
export function stageDirection(context) {
 ...
 * ```
 */
export async function generateReplyWithContext(session, options) {
...
//# sourceMappingURL=safe-generate-reply.js.map