 ...
 * - "valence === 'positive'"
 */
function parseCustomCondition(condition) {
    // Handle "false" for manually triggered tasks
    if (condition === 'false') {
        return () => false;
    }
    return (analysis, _userText) => {
        try {
 ...
 */
function parseKeywordRegex(pattern) {
 ...
 */
function loadTaskFile(filename) {
    const log = getLogger();
 ...
 */
export function loadTaskWisdom() {
    const files = [
        'micro-tasks.json',
        'support-tasks.json',
        'life-events.json',
...
// CACHED SINGLETON
// ============================================================================
let cachedWisdom = null;
/**
 * Get task wisdom (cached after first load)
 */
export function getTaskWisdom() {
    if (!cachedWisdom) {
 ...
 */
export function reloadTaskWisdom() {
    cachedWisdom = loadTaskWisdom();
    return cachedWisdom;
}
/**
 ...
 */
export function getTaskWisdomByCategory(category) {
    return getTaskWisdom().filter((t) => t.category === category);
}
/**
 * Get task wisdom by ID
 */
export function getTaskWisdomById(id) {
