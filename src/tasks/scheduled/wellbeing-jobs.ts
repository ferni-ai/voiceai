...
interface ANTReport {
  ...
  topDistortions: Array<{ type: string; count: number }>;
  peakTime: string;
  peakDay: string;
 ...
 */
async function generateANTReport(userId: string): Promise<ANTReport | null> {
  try {
    const { getANTPatterns, generateWeeklyInsights } =
    ...
    const trend: 'improving' | 'stable' | 'increasing' =
      pattern.distortionTrend === 'decreasing'
        ? 'improving'
 ...
 */
export async function runWeeklyANTReports(): Promise<{
  processed: number;
  reports: number;
  errors: number;
}> {
  ...
  try {
    ...
    const { getAllUsersWithANTData, clearOldANTData } =
      await import('../../services/cognitive-intelligence/index.js');
 ...
 */
export async function runDailyWarningChecks(): Promise<{
  processed: number;
  warnings: number;
  urgent: number;
      ...
      try {
        ...
        if (warnings.length > 0) {
          ...
          const actionableWarnings = warnings.filter(
            (w) => w.severity === 'concern' || w.severity === 'urgent'
          );

          if (actionableWarnings.length > 0) {
            await triggerWarningOutreach(
              profile.userId,
 ...
 */
export async function runWisdomAggregation(): Promise<{
  ...
  try {
    const { aggregatePopulationWisdom, discoverNewPatterns } =
 ...
 */
export async function runCheckInNudges(): Promise<{
  eligible: number;
  nudged: number;
}> {
  ...
  try {
    const { getUsersNeedingCheckIn, getLastCheckInTime } =
 ...
 */
export async function runThinkingOfYouOutreach(): Promise<{
  ...
  try {
    ...
    const { getDueItems, canSendOutreach, executeOutreach, queueThinkingOfYou } =
      await import('../../services/trust-systems/outreach-integration.js');

    const { getDueMoments, generateThinkingOfYouMoments } =
      ...
      try {
        ...
        if (dueItems.length > 0) {
          ...
          if (result.success) {
            stats.outreachSent++;
            log.info(
              { userId, type: item.type, method: result.method },
              '💌 BETTER-THAN-HUMAN: Sent "thinking of you" outreach'
            );
          } else {
            ...
            if (method === 'push') {
              ...
              if (smsResult.success) {
                stats.outreachSent++;
                log.info(
                  { userId, type: item.type, method: 'sms' },
                  '📱 BETTER-THAN-HUMAN: Sent via SMS fallback'
                );
              }
            }
          }
...
async function storeANTReport(userId: string, report: ANTReport): Promise<void> {
...
async function triggerANTInsightOutreach(userId: string, report: ANTReport): Promise<void> {
  try {
    ...
    const trend =
      report.trend === 'improving' ? 'getting better at noticing' : 'noticed some patterns in';
...
async function triggerWarningOutreach(
  userId: string,
...
async function triggerCheckInNudge(userId: string, daysSince: number): Promise<void> {
  try {
    const { triggerOutreach } = await import('../../services/outreach/index.js');
 ...
 */
export function getJobConfigs(): Record<string, JobConfig> {
  return { ...JOBS };
}
