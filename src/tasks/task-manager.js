...
import { getContextualTransition, getTransition, TASK_TRANSITIONS, } from './transitions.js';
import { getTaskWisdom } from './wisdom/index.js';
// ============================================================================
 ...
 */
export const TASK_WISDOM = getTaskWisdom();
// ============================================================================
// TASK MANAGER
// ============================================================================
export class TaskManager {
     ...
     */
    processUserTurn(analysis, userText, context) {
        const contextParts = [];
        ...
        for (const [taskId, activeTask] of this.activeTasks) {
            ...
            if (this.isTaskComplete(activeTask, analysis, userText)) {
                ...
                if (activeTask.wisdom.transitions?.exit) {
                    const exitPhrase = activeTask.wisdom.transitions.exit[Math.floor(Math.random() * activeTask.wisdom.transitions.exit.length)];
                    contextParts.push(`[TRANSITION] Consider saying: "${exitPhrase}"`);
                }
            }
        }
        // 3. Build context from active tasks (sorted by priority)
        const sortedTasks = Array.from(this.activeTasks.values()).sort((a, b) => b.wisdom.priority - a.wisdom.priority);
     ...
     */
    getSmartEntryTransition(wisdom, analysis) {
        ...
        if (transitionKey && transitionKey in TASK_TRANSITIONS) {
            return getTransition(transitionKey);
        }
     ...
     */
    getTargetMoodForCategory(category) {
        switch (category) {
            case 'support':
                return 'support';
            case 'micro':
     ...
     */
    shouldTrigger(wisdom, analysis, userText) {
     ...
     * Activate a task
     */
    activateTask(wisdom, initialDistress) {
        // Deactivate lower priority tasks in same category if this is higher priority
        for (const [taskId, activeTask] of this.activeTasks) {
            if (activeTask.wisdom.category === wisdom.category &&
     ...
     */
    isTaskComplete(activeTask, analysis, userText) {
 ...
 */
export function getTaskManager(options) {
