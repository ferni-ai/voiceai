 ...
 */
export function isValidTransitionKey(key) {
    return (key in TOPIC_ENTRY_TRANSITIONS ||
        key in TOPIC_EXIT_TRANSITIONS ||
        key in EMOTIONAL_TRANSITIONS ||
        key in TASK_TRANSITIONS);
}
export function getTransition(category) {
    // Check each object for the category
    if (category in TOPIC_ENTRY_TRANSITIONS) {
        const phrases = TOPIC_ENTRY_TRANSITIONS[category];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    if (category in TOPIC_EXIT_TRANSITIONS) {
        const phrases = TOPIC_EXIT_TRANSITIONS[category];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    if (category in EMOTIONAL_TRANSITIONS) {
        const phrases = EMOTIONAL_TRANSITIONS[category];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    if (category in TASK_TRANSITIONS) {
        const phrases = TASK_TRANSITIONS[category];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    return 'Anyway...';
}
 ...
 */
export function wrapWithTransitions(message, options) {
    let result = message;
