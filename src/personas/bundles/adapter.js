...
import { registerBundleBackchannels, registerBundleCelebrations, registerBundleEntrances, registerBundleGoodbyes, registerBundleStorytelling, } from '../theatrical.js';
// ============================================================================
 ...
 */
function extractCatchphrases(catchphrases) {
    if (!catchphrases)
        return [];
    // Legacy array format
 ...
 */
function extractThinkingSounds(thinkingSounds) {
 ...
 */
export async function bundleToPersonaConfig(bundle) {
    ...
    const knowledgeDomains = manifest.knowledge?.domains ?? [];
    ...
    if (behaviors.backchannels) {
        ...
        const backchannelRecord = {};
        if (behaviors.backchannels.neutral)
            backchannelRecord.neutral = behaviors.backchannels.neutral;
        if (behaviors.backchannels.empathetic)
            backchannelRecord.empathetic = behaviors.backchannels.empathetic;
        if (behaviors.backchannels.engaged)
            backchannelRecord.engaged = behaviors.backchannels.engaged;
    ...
    if (behaviors.celebrations) {
        ...
        registerBundleCelebrations(manifest.identity.id, behaviors.celebrations);
    }
    ...
    if (cognitiveSource) {
        getLogger().debug(`Registering bundle cognitive profile for ${manifest.identity.id} (source: ${manifest.cognitive ? 'manifest' : 'behaviors'})`);
        try {
            const { registerBundleCognitiveProfile, convertBundleCognitive } = await import('../cognitive-profiles.js');
            const cognitiveProfile = convertBundleCognitive(cognitiveSource);
            registerBundleCognitiveProfile(manifest.identity.id, cognitiveProfile);
        }
        catch (error) {
            getLogger().warn({ error, personaId: manifest.identity.id }, 'Failed to register cognitive profile from bundle');
        }
    }
    ...
    const personalityConfig = {
        ...
        humorStyle: ['dry-wit', 'self-deprecating', 'observational'],
    ...
    const knowledge = {
        ...
        outOfScopeTopics: [],
        outOfScopeResponse: "That's a bit outside my expertise. Let me connect you with someone better suited to help.",
    };
    // Convert stories
    ...
    if (behaviors.pet_peeves) {
        if (Array.isArray(behaviors.pet_peeves)) {
            ...
            const wrapper = behaviors.pet_peeves;
            if (Array.isArray(wrapper.pet_peeves)) {
                petPeevesArray = wrapper.pet_peeves;
            }
        }
    }
    const petPeeves = petPeevesArray.map((pp) => ({
 ...
 */
async function buildCommunicationConfig(behaviors, manifest) {
 ...
 */
function generateSystemPrompt(manifest, behaviors) {
    // Defensive defaults
 ...
 */
function convertStoryType(category) {
    const typeMap = {
        personal: 'personal',
