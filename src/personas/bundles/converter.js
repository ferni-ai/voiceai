 ...
 */
export function generateManifest(persona) {
    ...
    return {
        ...
        voice: {
            ...
            voice_id: persona.voice.voiceId || `\${env:${persona.id.toUpperCase().replace(/-/g, '_')}_VOICE_ID}`,
            default_rate: speechChars?.baseSpeedMultiplier && speechChars.baseSpeedMultiplier < 0.8
                ? 'slow'
                : 'medium',
        },
 ...
 */
function estimateTokenCount(persona) {
    let tokens = 0;
    // Stories
 ...
 */
function convertStories(persona) {
    const stories = [];
    const indexRefs = [];
    for (const story of persona.stories || []) {
 ...
 */
function convertBehaviors(persona) {
    return {
 ...
 */
export async function convertLegacyToBundle(persona, outputDir) {
    ...
    for (const story of stories) {
        await writeFile(join(bundlePath, 'content/stories', `${story.id}.json`), JSON.stringify(story, null, 2));
    }
    // Convert and write behaviors
    const behaviors = convertBehaviors(persona);
    await writeFile(join(bundlePath, 'content/behaviors/catchphrases.json'), JSON.stringify(behaviors.catchphrases, null, 2));
    await writeFile(join(bundlePath, 'content/behaviors/pet-peeves.json'), JSON.stringify(behaviors.pet_peeves, null, 2));
    if (behaviors.backchannels) {
        await writeFile(join(bundlePath, 'content/behaviors/backchannels.json'), JSON.stringify(behaviors.backchannels, null, 2));
    }
 ...
 */
function generateBiography(persona) {
    const lines = [];
