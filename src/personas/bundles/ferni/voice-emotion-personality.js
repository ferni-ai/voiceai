 ...
 */
export function getVoiceEmotionAdjustment(voiceContext) {
 ...
 */
export function isThemePreferredForVoice(theme, adjustment) {
    // If no preferences, all themes are ok
    if (adjustment.preferredThemes.length === 0)
        return true;
 ...
 */
export function shouldAvoidThemeForVoice(theme, adjustment) {
    return adjustment.avoidThemes.includes(theme);
}
/**
 * Convert VoiceEmotionResult to our internal context format
 */
export function fromVoiceEmotionResult(result) {
    if (!result) {
        return {};
    }
    return {
...
//# sourceMappingURL=voice-emotion-personality.js.map