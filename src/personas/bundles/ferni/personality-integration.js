...
import { composeExpression, } from '../../shared/better-than-human-personality.js';
import { assemblePersonalityContext, } from '../../shared/personality-context-assembler.js';
import { detectNoticing, shouldThrottleNoticing, recordNoticing, } from '../../shared/realtime-noticing.js';
import { recordResonanceEvent, recordUserTopicMention, detectEngagement, prewarmResonanceCache, } from '../../shared/personality-resonance-store.js';
import { getBestExpression, prewarmCache, requestEmotionalExpressions, getStats as getLLMStats, markExpressionEngagement, loadPersistedExpressions, } from './llm-expression-generator.js';
// NEW: Telemetry & Transparency
import { startTiming, recordTelemetry, } from './personality-telemetry.js';
...
import { memoryPersonalityBridge } from './memory-personality-bridge.js';
// NEW: Cross-Persona Learning
import { crossPersonaLearning } from '../../shared/cross-persona-learning.js';
// NEW: Voice Pace Integration
import { getPacePersonalityAdjustment, applyPaceToExpression, fromVoicePaceData, } from './voice-pace-personality.js';
 ...
 */
function mapNoticingToBehaviorEventType(noticingType) {
    const eventMap = {
 ...
 */
export function noticingToBehaviorEvent(noticing) {
 ...
 */
export async function processTurnPersonality(input) {
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    let previousEngagement = 'neutral';
    if (input.userId && input.previousExpression) {
        ...
        const exprId = input.previousExpression.id ||
        ...
        if (previousEngagement === 'positive') {
            crossPersonaLearning.learnFromExpression(input.previousExpression.content, 'ferni', input.previousExpression.theme, {
    ...
    if (input.userId && !noticing) {
        ...
        const memoryCallback = memoryPersonalityBridge.getBest(input.userId, timing, input.textEmotion?.primary);
        if (memoryCallback) {
    ...
    if (!expression && !(noticing && noticing.type !== 'breakthrough_moment')) {
        ...
        const crossPersonaPatterns = crossPersonaLearning.getBestPatternsForPersona('ferni', theme, {
            emotionalState: input.textEmotion?.primary,
            relationshipStage: input.relationshipStage,
            momentum: input.momentum,
        }, 1);
        if (crossPersonaPatterns.length > 0 && Math.random() < 0.3) {
            ...
            const bestExpr = getBestExpression(input.userId || 'anonymous', input.sessionId, theme, llmContext);
            if (bestExpr) {
                expression = {
    ...
    if (expression) {
        ...
        if (paceAdjustment.preferredTiming && expression.timing !== 'immediate') {
            expression.timing = paceAdjustment.preferredTiming;
        }
        log.debug({
 ...
 */
function mapInputMomentumToExpressionMomentum(momentum) {
    switch (momentum) {
        case 'opening':
            return 'opening';
        case 'building':
 ...
 */
function pickExpressionTheme(context, input) {
    // Vulnerability for heavy topics with strong relationship
    if (input.isHeavyTopic &&
...
function determineInjectionPoint(expression, noticing) {
 ...
 */
export function applyPersonalityToResponse(rawResponse, personalityResult) {
 ...
 */
export function cleanupPersonalitySession(sessionId) {
 ...
 */
export async function prewarmPersonalitySession(userId, context) {
...
recordResonanceEvent, detectEngagement, };
...
//# sourceMappingURL=personality-integration.js.map