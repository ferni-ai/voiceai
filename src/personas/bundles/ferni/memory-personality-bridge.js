 ...
 */
export function createCallbackFromInsight(insight) {
    const typeMap = {
        date_reminder: 'date',
 ...
 */
export function createDateCallback(dateType, dateName, daysUntil) {
    let naturalPhrase;
    let tone = 'warm';
    let priority = 'medium';
    if (daysUntil === 0) {
 ...
 */
export function createAbsenceCallback(topic, lastMentioned) {
 ...
 */
export function createGrowthCallback(area, progress) {
    return {
        type: 'growth',
 ...
 */
export function createComfortCallback(pattern, context) {
    return {
        type: 'comfort',
 ...
 */
export function addCallback(userId, callback) {
    const existing = userCallbacks.get(userId) || [];
    existing.push(callback);
    userCallbacks.set(userId, existing);
    log.debug({ userId, type: callback.type, priority: callback.priority }, '📝 Memory callback added');
}
/**
 * Get the best callback for current context
 */
export function getBestCallback(userId, currentTiming, currentMood) {
    ...
    const available = callbacks.filter((cb) => {
        if (cb.delivered)
            return false;
        if (cb.sourceId && delivered.has(cb.sourceId))
            return false;
        if (cb.timing !== 'anytime' && cb.timing !== currentTiming)
            return false;
        return true;
    });
 ...
 */
export function markDelivered(userId, callback) {
    ...
    log.debug({ userId, type: callback.type, content: callback.content }, '✅ Memory callback delivered');
 ...
 */
export function clearCallbacks(userId) {
 ...
 */
export function callbackToExpression(callback) {
 ...
 */
export function getMemoryPersonalityContext(userId) {
    const callbacks = userCallbacks.get(userId) || [];
    return {
...
//# sourceMappingURL=memory-personality-bridge.js.map