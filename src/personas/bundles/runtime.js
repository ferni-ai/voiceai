...
export class BundleRuntimeEngine {
    ...
    state;
    // Cached content
    voiceExpressions = null;
    situationalResponses = null;
    relationshipStages = null;
    memoryPatterns = null;
    personaModes = null;
    storyGraph = null;
    microExpressions = null;
    contextualNuances = null;
    ...
    cachedSortedStages = null;
    constructor(bundle, initialState) {
        this.bundle = bundle;
        this.state = {
    ...
    static CONTENT_LOAD_TIMEOUT_MS = 5000; // 5 seconds
    /**
    ...
    async initialize() {
        ...
        const [voiceExpressions, situationalResponses, relationshipStages, memoryPatterns, personaModes, storyGraph, microExpressions, contextualNuances, conflictHandling,] = await Promise.all([
     ...
     */
    updateTimeContext(userTimezoneOffset) {
        let now = new Date();
     ...
     */
    getSortedStages() {
        if (!this.relationshipStages)
            return [];
        // Use cached version if available
        if (this.cachedSortedStages)
            return this.cachedSortedStages;
        // Sort once and cache
        this.cachedSortedStages = Object.entries(this.relationshipStages.stages).sort(([, a], [, b]) => b.turn_threshold - a.turn_threshold);
        return this.cachedSortedStages;
    }
    getCurrentRelationshipStage() {
        if (!this.relationshipStages)
            return null;
    ...
    applyProgressionTrigger(triggerName) {
        if (!this.relationshipStages?.progression_triggers)
            return;
     ...
     */
    detectAndSetMode(userText, emotionalSignal) {
    ...
    getModeTransitionPhrase(fromMode, toMode) {
        if (!this.personaModes?.mode_transitions)
            return null;
    ...
    // VOICE EXPRESSIONS
    // ============================================================================
    getVoiceExpression(emotionType) {
        if (!this.voiceExpressions?.emotional_expressions)
            return null;
     ...
     */
    getRandomExpressionPhrase(emotionType) {
        const expression = this.getVoiceExpression(emotionType);
        if (!expression || !expression.phrases.length)
            return null;
     ...
     */
    getSituationalResponse(category, situation) {
        if (!this.situationalResponses)
            return null;
    ...
    // MEMORY PATTERNS
    // ============================================================================
    getNameUsagePhrase(context) {
        if (!this.memoryPatterns?.name_usage?.patterns)
            return null;
    ...
    getCallbackPhrase(type) {
        if (!this.memoryPatterns?.reference_patterns)
            return null;
        const pattern = this.memoryPatterns.reference_patterns[type];
        if (!pattern || !pattern.phrases.length)
            return null;
        return pattern.phrases[Math.floor(Math.random() * pattern.phrases.length)];
    }
    getDetailCallback(category) {
        if (!this.memoryPatterns?.detail_callbacks)
            return null;
        const callback = this.memoryPatterns.detail_callbacks[category];
        if (!callback || !callback.patterns.length)
            return null;
    ...
    shouldTellStory(currentTurn) {
        ...
        if (this.storyGraph?.story_timing_rules) {
            ...
            if (rules.minimum_turns_before_first_story &&
            ...
            if (rules.minimum_turns_between_stories &&
            ...
            if (rules.max_stories_per_session &&
        ...
        const timingContext = {
            ...
            emotionalArc: arc,
            userEngagement,
            userPacing: arc.conversationTemperature > 0.7 ? 'rushed' : 'normal',
        };
        ...
        const minimalPersonaConfig = { stories: [] };
    ...
    getRecommendedStories(context) {
        if (!this.storyGraph?.context_triggers)
            return [];
        const trigger = this.storyGraph.context_triggers[context];
        if (!trigger)
            return [];
        // Filter out already told stories
        return trigger.recommended_stories.filter((id) => !this.state.storiesToldThisSession.includes(id));
    }
    /**
     ...
     */
    async getRecommendedStoriesWithEvolution(context, limit = 3) {
        ...
        try {
            ...
            const evolutionRecs = evolution.getRecommendedStories(personaId, {
                topic: context.topic,
            ...
            // Filter out already-told stories
            const filteredRecs = evolutionRecs.filter((rec) => !this.state.storiesToldThisSession.includes(rec.storyId));
     ...
     */
    async recordStoryUsage(storyId, turn, userReaction) {
        const personaId = this.getPersonaId();
        ...
        if (userReaction) {
            try {
                ...
                insights.recordStoryUsage(storyId, personaId, {
                ...
                }, reactionMap[userReaction.engagement], userReaction.engagement === 'positive'
                    ? 0.9
    ...
    // MICRO-EXPRESSIONS
    // ============================================================================
    getListeningSound(emotion) {
        if (!this.microExpressions?.listening_sounds?.with_emotion)
            return null;
        const emotionSounds = this.microExpressions.listening_sounds.with_emotion[emotion];
        if (!emotionSounds || !emotionSounds.sounds.length)
            return null;
    ...
    getVocalTexture(type, variant) {
        if (!this.microExpressions?.vocal_textures)
            return null;
    ...
    getPacingVariation(context) {
        if (!this.microExpressions?.pacing_variations)
            return null;
    ...
    getDayOfWeekAcknowledgment() {
        if (!this.contextualNuances?.day_of_week)
            return null;
    ...
    // CONFLICT HANDLING
    // ============================================================================
    detectPushback(userText) {
        if (!this.conflictHandling?.user_pushback)
            return null;
        const lowerText = userText.toLowerCase();
    ...
    getPushBackPhrase(context) {
        if (!this.conflictHandling?.persona_disagreement?.how_to_push_back)
            return null;
    ...
    getRepairPhrase(type) {
        if (!this.conflictHandling?.repair_after_conflict)
            return null;
        const repair = this.conflictHandling.repair_after_conflict[type];
        if (!repair || !repair.phrases?.length)
            return null;
     ...
     */
    async loadInnerWorld() {
        try {
            const fs = await import('node:fs/promises');
            const path = await import('node:path');
            // Try to load inner-world.json
            const innerWorldPath = path.join(this.bundle.bundlePath, 'content', 'identity', 'inner-world.json');
            try {
            ...
            const sensoryWorldPath = path.join(this.bundle.bundlePath, 'content', 'identity', 'sensory-world.json');
            try {
     ...
     */
    getSelfTalk(context) {
        if (!this.innerWorld?.inner_voice)
            return null;
        switch (context) {
            case 'struggling':
     ...
     */
    getRandomSelfTalk() {
        if (!this.innerWorld?.inner_voice?.self_talk_patterns?.length)
            return null;
     ...
     */
    getContradiction() {
        if (!this.innerWorld?.contradictions?.belief_vs_behavior?.length)
            return null;
     ...
     */
    getSensoryMemory(trigger) {
        if (!this.innerWorld?.embodied_memories?.sense_memories?.length)
            return null;
        const lowerTrigger = trigger.toLowerCase();
        // Look for a matching memory
        const memory = this.innerWorld.embodied_memories.sense_memories.find((m) => lowerTrigger.includes(m.trigger.toLowerCase()));
        return memory ?? null;
    }
    /**
     ...
     */
    detectEmotionalFlashpoint(userText) {
        if (!this.innerWorld?.emotional_flashpoints)
            return null;
        const lowerText = userText.toLowerCase();
        ...
        const matchesTrigger = (text, trigger) => {
     ...
     */
    getRegret() {
        if (!this.innerWorld?.unfinished_business?.regrets?.length)
            return null;
     ...
     */
    getSecretFear() {
        if (!this.innerWorld?.secret_self?.secret_fears?.length)
            return null;
     ...
     */
    getGuiltyAdmission() {
        if (!this.innerWorld?.secret_self?.guilty_admissions?.length)
            return null;
     ...
     */
    getSignaturePhrase() {
        if (!this.sensoryWorld?.voice_fingerprint?.phrases_that_are_theirs?.length)
            return null;
     ...
     */
    getUniqueWord() {
        if (!this.sensoryWorld?.voice_fingerprint?.words_only_they_use?.length)
            return null;
     ...
     */
    getVerbalTic() {
        if (!this.sensoryWorld?.voice_fingerprint?.verbal_tics?.length)
            return null;
     ...
     */
    getMentorQuote() {
        if (!this.sensoryWorld?.relationship_history?.mentors_who_shaped_them?.length)
            return null;
    ...
    getMusicForMood(mood) {
        if (!this.sensoryWorld?.sensory_preferences?.music_for_different_moods)
            return null;
     ...
     */
    getGrowthEdge() {
        if (!this.sensoryWorld?.growth_edges?.actively_working_on?.length)
            return null;
     ...
     */
    getPhysicalPresence() {
        if (!this.sensoryWorld?.physical_presence)
            return null;
     ...
     */
    getDailyRhythms() {
        if (!this.sensoryWorld?.daily_rhythms)
            return null;
     ...
     */
    getHabit(sessionId) {
        if (!this.quirks?.habits?.length)
            return null;
        if (sessionId) {
            const tracker = getSessionVarietyTracker();
            const expressions = this.quirks.habits.map((h, i) => ({
                id: `quirk-habit-${i}`,
                theme: tracker.detectTheme(h) || 'physical_habits',
                content: h,
            }));
     ...
     * Uses variety tracking when sessionId provided to prevent repetition
     */
    getGuiltyPleasure(sessionId) {
        if (!this.quirks?.guilty_pleasures?.length)
            return null;
        if (sessionId) {
            const tracker = getSessionVarietyTracker();
            const expressions = this.quirks.guilty_pleasures.map((p, i) => ({
                id: `quirk-pleasure-${i}`,
                theme: tracker.detectTheme(p) || 'quirky_interests',
                content: p,
            }));
            const selected = tracker.selectWithVariety(sessionId, expressions);
            return selected?.content || null;
     ...
     * Uses variety tracking when sessionId provided to prevent repetition
     */
    getStrongOpinion(sessionId) {
        if (!this.quirks?.strong_opinions?.length)
            return null;
        if (sessionId) {
            const tracker = getSessionVarietyTracker();
            const expressions = this.quirks.strong_opinions.map((o, i) => ({
                id: `quirk-opinion-${i}`,
                theme: tracker.detectTheme(o) || 'philosophical',
                content: o,
            }));
            const selected = tracker.selectWithVariety(sessionId, expressions);
            return selected?.content || null;
     ...
     * Uses variety tracking when sessionId provided to prevent repetition
     */
    getWeakness(sessionId) {
        if (!this.quirks?.not_good_at?.length)
            return null;
        if (sessionId) {
            const tracker = getSessionVarietyTracker();
            const expressions = this.quirks.not_good_at.map((w, i) => ({
                id: `quirk-weakness-${i}`,
                theme: tracker.detectTheme(w) || 'vulnerability',
                content: w,
            }));
            const selected = tracker.selectWithVariety(sessionId, expressions);
            return selected?.content || null;
     ...
     * Uses variety tracking when sessionId provided to prevent repetition
     */
    getCaughtDoing(sessionId) {
        if (!this.quirks?.caught_doing?.length)
            return null;
        if (sessionId) {
            const tracker = getSessionVarietyTracker();
            const expressions = this.quirks.caught_doing.map((c, i) => ({
                id: `quirk-caught-${i}`,
                theme: tracker.detectTheme(c) || 'quirky_interests',
                content: c,
            }));
            const selected = tracker.selectWithVariety(sessionId, expressions);
            return selected?.content || null;
 ...
 */
const globalRuntimeEngines = new Map();
/**
...
export class SessionBundleRuntimeManager {
     ...
     */
    async getOrCreateRuntime(bundle, initialState) {
        ...
        getLogger().debug({ sessionId: this.sessionId, personaId }, 'Created session-scoped bundle runtime');
        return engine;
    }
    /**
     * Get an existing runtime for a persona (if it exists)
     */
    getRuntime(personaId) {
        const key = `${this.sessionId}:${personaId}`;
 ...
 */
export async function createBundleRuntime(bundle, initialState) {
    const personaId = bundle.manifest.identity.id;
