 ...
 */
export async function loadAPIRegisteredServers(
  publisherId: string,
  personaId?: string
  ...
  try {
    ...
    if (personaId) {
      ...
      for (const doc of personaSnapshot.docs) {
        const server = await convertApiServerToBundleServer(doc.data() as DeveloperMCPServer);
    ...
    for (const doc of globalSnapshot.docs) {
      const data = doc.data() as DeveloperMCPServer;
    ...
    log.info(
      { publisherId, personaId, serverCount: servers.length },
 ...
 */
async function convertApiServerToBundleServer(
  apiServer: DeveloperMCPServer
): Promise<BundleMCPServer | null> {
  try {
    const bundleServer: BundleMCPServer = {
      id: apiServer.id,
      name: apiServer.name,
...
export async function loadMCPConfig(
  ...
  const allServers: BundleMCPServer[] = [];
