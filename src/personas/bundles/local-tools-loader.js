 ...
 */
export async function loadLocalTool(filePath) {
    try {
 ...
 */
export async function loadLocalTools(toolsDir) {
    const tools = [];
    try {
 ...
 */
export async function loadBundleLocalTools(bundlePath) {
 ...
 */
function executePromptTool(tool, params) {
 ...
 */
async function executeWebhookTool(tool, params, context) {
    if (!tool.webhook) {
...
function getBundlePathFromToolFilePath(filePath) {
    if (!filePath)
        return null;
...
async function executeScriptTool(tool, params, context) {
    if (!tool.script) {
    ...
    const mod = (await import(pathToFileURL(normalizedScript).toString()));
    const runFn = typeof mod.run === 'function'
        ...
        : typeof mod.default === 'function'
            ...
            : null;
    if (!runFn) {
        throw new Error('Script module must export a run(params, context) function (or default export)');
    }
...
async function executeMcpTool(tool, params) {
    ...
    const { connectToMCPServer, findServer, getMCPConfig, callMCPTool } = await import('./mcp-loader.js');
 ...
 */
export async function executeLocalTool(context) {
 ...
 */
export async function getLocalTools(bundlePath, forceReload = false) {
 ...
 */
export function clearLocalToolCache(bundlePath) {
    if (bundlePath) {
 ...
 * Find a tool by ID
 */
export async function findLocalTool(bundlePath, toolId) {
 ...
 */
export function localToolToFunctionDefinition(tool) {
    return {
        name: tool.name,
 ...
 */
export async function getLocalToolDefinitions(bundlePath) {
    const tools = await getLocalTools(bundlePath);
...
//# sourceMappingURL=local-tools-loader.js.map