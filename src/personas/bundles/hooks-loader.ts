...
export async function loadHooks(bundlePath: string): Promise<BundleAgentHooks | null> {
  ...
  try {
    ...
    const hooks = JSON.parse(content) as BundleAgentHooks;
...
async function executeShellHook(
  ...
  try {
    ...
    const err = error as Error & { code?: number; killed?: boolean; signal?: string };
    ...
    if (isTimeout) {
      return {
        success: true,
        prompt:
          "I took a moment to check something, but it's taking longer than expected. Let's continue our conversation.",
      };
    }
...
export async function executeHooksForEvent(
  hooks: BundleAgentHooks,
  event: HookEvent,
  baseContext: Omit<HookExecutionContext, 'event' | 'hook'>
): Promise<HookExecutionResult[]> {
...
const hooksCache = new Map<string, BundleAgentHooks | null>();
 ...
 */
export function clearHooksCache(bundlePath?: string): void {
  if (bundlePath) {
 ...
 */
export function hasHook(hooks: BundleAgentHooks | null, event: HookEvent): boolean {
 ...
 */
export function getHookPrompt(
  hooks: BundleAgentHooks | null,
  event: HookEvent,
  data?: Record<string, unknown>
): string | null {
