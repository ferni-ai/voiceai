...
function evictOldestEntries() {
    if (bundleCache.size <= MAX_BUNDLE_CACHE_SIZE)
        return;
    // Sort by lastAccessed, remove oldest
    const entries = Array.from(bundleCache.entries()).sort(([, a], [, b]) => a.lastAccessed - b.lastAccessed);
 ...
 */
export async function loadBundle(bundlePath, options = {}) {
    ...
    const rawManifest = await loadJsonFile(manifestPath);
    ...
    let voiceExpressionsCache = null;
    let situationalResponsesCache = null;
    let relationshipStagesCache = null;
    let memoryPatternsCache = null;
    let personaModesCache = null;
    let storyGraphCache = null;
    let microExpressionsCache = null;
    let contextualNuancesCache = null;
    let conflictHandlingCache = null;
    let promptAssemblyCache = null;
    // Reload callbacks
    const reloadCallbacks = new Set();
    ...
    async function loadStoryIndex() {
        ...
        if (await fileExists(indexPath)) {
            const rawIndex = await loadJsonFile(indexPath);
            ...
            if ('stories_index' in rawIndex && rawIndex.stories_index?.stories) {
            ...
            else if ('story_index' in rawIndex &&
                rawIndex.story_index?.stories) {
                storyIndex = {
                    stories: rawIndex.story_index.stories,
                };
            }
                ...
                getLogger().warn({ indexPath, keys: Object.keys(rawIndex) }, 'Story index has unexpected format - expected "stories", "stories_index.stories", or "story_index.stories"');
                return null;
            }
            return storyIndex;
        }
    ...
    async function loadKnowledgeIndex() {
        ...
        if (await fileExists(indexPath)) {
            knowledgeIndex = await loadJsonFile(indexPath);
            return knowledgeIndex;
        }
        return null;
    }
    ...
    const bundle = {
        ...
        async getStory(id) {
            ...
            if (storyFile.endsWith('.json')) {
                story = await loadJsonFile(storyPath);
            }
            else {
         ...
         */
        async getStoriesByTrigger(trigger) {
            const index = await loadStoryIndex();
            if (!index)
                return [];
            ...
            if (!normalizedTrigger)
                return [];
            const matching = index.stories.filter((s) => s.triggers.some((t) => {
        ...
        async getKnowledge(topic) {
            ...
            if (knowledgeFile.endsWith('.json')) {
                knowledge = await loadJsonFile(knowledgePath);
            }
            else {
        ...
        async getBehaviors() {
            ...
            if (await fileExists(thinkingSoundsPath)) {
                const data = await loadJsonFile(thinkingSoundsPath);
            ...
            if (await fileExists(silenceFillersPath)) {
                behaviors.silence_fillers =
                    await loadJsonFile(silenceFillersPath);
            }
            ...
            if (await fileExists(celebrationsPath)) {
                behaviors.celebrations =
                    await loadJsonFile(celebrationsPath);
            }
            ...
            if (await fileExists(goodbyesPath)) {
                const goodbyesData = await loadJsonFile(goodbyesPath);
                ...
                if (goodbyesData && typeof goodbyesData === 'object' && 'goodbyes' in goodbyesData) {
                    behaviors.goodbyes = goodbyesData.goodbyes;
                }
                else {
            ...
            if (await fileExists(storytellingPath)) {
                behaviors.storytelling =
                    await loadJsonFile(storytellingPath);
            }
            // ========================================================================
            ...
            if (await fileExists(vulnerabilityPath)) {
                behaviors.vulnerability =
                    await loadJsonFile(vulnerabilityPath);
            }
            ...
            if (await fileExists(culturalMomentsPath)) {
                behaviors.cultural_moments =
                    await loadJsonFile(culturalMomentsPath);
            }
            ...
            if (await fileExists(sensoryMomentsPath)) {
                behaviors.sensory_moments =
                    await loadJsonFile(sensoryMomentsPath);
            }
            ...
            if (await fileExists(conflictHandlingPath)) {
                behaviors.conflict_handling =
                    await loadJsonFile(conflictHandlingPath);
            }
            ...
            if (await fileExists(relationshipTransitionsPath)) {
                behaviors.relationship_transitions = await loadJsonFile(relationshipTransitionsPath);
            }
            // ========================================================================
            ...
            if (await fileExists(emotionalIntelligencePath)) {
                behaviors.emotional_intelligence =
                    await loadJsonFile(emotionalIntelligencePath);
            }
            ...
            if (await fileExists(physicalPresencePath)) {
                behaviors.physical_presence =
                    await loadJsonFile(physicalPresencePath);
            }
            ...
            if (await fileExists(lateNightPresencePath)) {
                behaviors.late_night_presence =
                    await loadJsonFile(lateNightPresencePath);
            }
            ...
            if (await fileExists(superhumanInsightsPath)) {
                behaviors.superhuman_insights =
                    await loadJsonFile(superhumanInsightsPath);
            }
            ...
            if (await fileExists(trustPhrasesPath)) {
                behaviors.trust_phrases =
                    await loadJsonFile(trustPhrasesPath);
            }
            ...
            if (await fileExists(iNoticePowerPath)) {
                behaviors.i_notice_power =
                    await loadJsonFile(iNoticePowerPath);
            }
            ...
            if (await fileExists(quirksPath)) {
                behaviors.quirks = await loadJsonFile(quirksPath);
            }
            ...
            if (await fileExists(thinkingOfYouPath)) {
                behaviors.thinking_of_you =
                    await loadJsonFile(thinkingOfYouPath);
            }
            ...
            if (await fileExists(selfDoubtPath)) {
                behaviors.self_doubt = await loadJsonFile(selfDoubtPath);
            }
            ...
            if (await fileExists(secretModesPath)) {
                behaviors.secret_modes =
                    await loadJsonFile(secretModesPath);
            }
            ...
            if (await fileExists(secretFearsPath)) {
                behaviors.secret_fears =
                    await loadJsonFile(secretFearsPath);
            }
            ...
            if (await fileExists(anticipationPath)) {
                behaviors.anticipation =
                    await loadJsonFile(anticipationPath);
            }
            ...
            if (await fileExists(mortalityAwarenessPath)) {
                behaviors.mortality_awareness =
                    await loadJsonFile(mortalityAwarenessPath);
            }
            ...
            if (await fileExists(silenceResponsesPath)) {
                behaviors.silence_responses =
                    await loadJsonFile(silenceResponsesPath);
            }
            // ========================================================================
            ...
            if (await fileExists(lovableMomentsPath)) {
                behaviors.lovable_moments =
                    await loadJsonFile(lovableMomentsPath);
            }
            ...
            if (await fileExists(delightfulSurprisesPath)) {
                behaviors.delightful_surprises =
                    await loadJsonFile(delightfulSurprisesPath);
            }
            ...
            if (await fileExists(verbalPersonalityPath)) {
                behaviors.verbal_personality =
                    await loadJsonFile(verbalPersonalityPath);
            }
            ...
            if (await fileExists(noticingPatternsPath)) {
                behaviors.noticing_patterns =
                    await loadJsonFile(noticingPatternsPath);
            }
            ...
            if (await fileExists(liveReactionsPath)) {
                behaviors.live_reactions =
                    await loadJsonFile(liveReactionsPath);
            }
            // ========================================================================
            ...
            if (await fileExists(methodologyPath)) {
                behaviors.methodology = await loadJsonFile(methodologyPath);
            }
        ...
        async getVoiceExpressions() {
            if (voiceExpressionsCache)
                return voiceExpressionsCache;
            const voicePath = join(bundlePath, 'content/voice/expressions.json');
            if (await fileExists(voicePath)) {
                voiceExpressionsCache = await loadJsonFile(voicePath);
                return voiceExpressionsCache;
            }
            return null;
        },
        async getSituationalResponses() {
            if (situationalResponsesCache)
            ...
            if (await fileExists(responsesPath)) {
                situationalResponsesCache = await loadJsonFile(responsesPath);
                return situationalResponsesCache;
            }
            return null;
        },
        async getRelationshipStages() {
            if (relationshipStagesCache)
            ...
            if (await fileExists(stagesPath)) {
                relationshipStagesCache = await loadJsonFile(stagesPath);
                return relationshipStagesCache;
            }
            return null;
        },
        async getMemoryPatterns() {
            if (memoryPatternsCache)
            ...
            if (await fileExists(patternsPath)) {
                memoryPatternsCache = await loadJsonFile(patternsPath);
                return memoryPatternsCache;
            }
            return null;
        },
        async getPersonaModes() {
            if (personaModesCache)
            ...
            if (await fileExists(modesPath)) {
                personaModesCache = await loadJsonFile(modesPath);
                return personaModesCache;
            }
            return null;
        },
        async getStoryGraph() {
            ...
            if (await fileExists(graphPath)) {
                storyGraphCache = await loadJsonFile(graphPath);
                return storyGraphCache;
            }
            return null;
        },
        async getMicroExpressions() {
            if (microExpressionsCache)
            ...
            if (await fileExists(microPath)) {
                microExpressionsCache = await loadJsonFile(microPath);
                return microExpressionsCache;
            }
            return null;
        },
        async getContextualNuances() {
            if (contextualNuancesCache)
            ...
            if (await fileExists(nuancesPath)) {
                contextualNuancesCache = await loadJsonFile(nuancesPath);
                return contextualNuancesCache;
            }
            return null;
        },
        async getConflictHandling() {
            if (conflictHandlingCache)
            ...
            if (await fileExists(conflictPath)) {
                conflictHandlingCache = await loadJsonFile(conflictPath);
                return conflictHandlingCache;
            }
            return null;
        },
        async getPromptAssembly() {
            if (promptAssemblyCache)
            ...
            if (await fileExists(assemblyPath)) {
                promptAssemblyCache = await loadJsonFile(assemblyPath);
                return promptAssemblyCache;
            }
            return null;
        },
    ...
    if (preloadContent) {
        ...
        getLogger().info({ bundleId: manifest.identity.id, stories: storyCache.size }, 'Bundle preloaded');
    }
 ...
 */
export async function loadBundleById(bundleId, options = {}) {
 ...
 */
export function getBundleSearchPaths() {
    const paths = [];
 ...
 */
export function clearBundleCache(bundleId) {
    if (bundleId) {
 ...
 */
export function getCachedBundles() {
