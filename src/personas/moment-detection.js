...
import { detectMomentsUnified, extractMemorableMoments as extractMemorableMomentsUnified, } from './unified-moment-detection.js';
const log = getLogger();
// ============================================================================
// DETECTION PATTERNS
// ============================================================================
 ...
 */
function calculateSignificance(matchCount, context) {
 ...
 */
function detectBreakthrough(context) {
    ...
    return {
        ...
        tags: ['insight', 'growth', topic || 'general'].filter(Boolean),
    };
}
/**
 * Detect vulnerability moments
 */
function detectVulnerability(context) {
    ...
    if (!matched)
        return null;
    const type = hasSharedVulnerabilityBefore
        ? 'trust_demonstration'
        : 'first_vulnerability';
    return {
        type,
        ...
        topic,
        significance: calculateSignificance(matches.length, context),
        tags: ['vulnerability', 'emotional', topic || 'personal'].filter(Boolean),
    };
}
/**
 * Detect laughter moments
 */
function detectLaughter(context) {
    const { userMessage, topic } = context;
    ...
    const isGenuine = matches.length >= 2 ||
 ...
 */
function detectCelebration(context) {
    const { userMessage, topic } = context;
    ...
    return {
        ...
        topic,
        significance: calculateSignificance(matches.length, context),
        tags: ['win', 'achievement', 'celebration', topic || 'milestone'].filter(Boolean),
    };
}
/**
...
function detectCrisis(context) {
    ...
    return {
        ...
        tags: ['crisis', 'support', 'presence', topic || 'emotional'].filter(Boolean),
    };
}
/**
 * Detect trust demonstration moments
 */
function detectTrust(context) {
    const { userMessage, topic } = context;
    ...
    return {
        ...
        topic,
        significance: calculateSignificance(matches.length, context),
        tags: ['trust', 'sensitive', topic || 'personal'].filter(Boolean),
    };
}
/**
 * Detect growth recognition moments
 */
function detectGrowth(context) {
    const { userMessage, topic } = context;
    ...
    return {
        ...
        topic,
        significance: calculateSignificance(matches.length, context),
        tags: ['growth', 'progress', 'awareness', topic || 'self'].filter(Boolean),
    };
}
// ============================================================================
// MAIN DETECTION API
 ...
 */
export function detectPrimaryMoment(context) {
 ...
 */
function convertUnifiedMoment(unified) {
    return {
        type: unified.type,
 ...
 */
export function detectMomentsUnifiedWrapper(context) {
...
//# sourceMappingURL=moment-detection.js.map