 ...
 */
export function getCognitiveGuidance(personaId, context) {
 ...
 */
export function buildCognitivePromptInjection(personaId, context) {
    const profile = getCognitiveProfile(personaId);
    if (!profile)
        return '';
    const engine = getCognitiveEngine(personaId, profile);
 ...
 */
export function detectQuestionComplexity(message) {
 ...
 */
export function detectUserExpertise(messages, topic) {
    ...
    if (expertScore >= 2 && noviceScore === 0)
        return 'expert';
    if (noviceScore >= 2 && expertScore === 0)
        return 'novice';
    if (expertScore > noviceScore)
        return 'intermediate';
    if (noviceScore > expertScore)
        return 'novice';
    return 'unknown';
}
export default {
...
//# sourceMappingURL=cognitive-index.js.map