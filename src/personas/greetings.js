 ...
 */
function getGreetingTemplates(style, personaName) {
 ...
 */
export function generateVoiceRecognitionGreeting(persona, scenario, options) {
    const templates = getGreetingTemplates(persona.communication.greetingStyle, persona.identity.selfReference);
    let templateList;
    switch (scenario) {
 ...
 */
function pickUnusedTemplate(templateList, usedGreetings) {
...
export function generateStaticGreeting(persona, options) {
 ...
 */
export async function generateDynamicGreeting(persona, options) {
...
Generate ONE greeting (2-4 sentences max). Be warm and natural.`;
    try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${getDefaultModel()}:generateContent?key=${apiKey}`, {
            method: 'POST',
        ...
        if (!text || text.length < 10)
            return null;
        getLogger().info({ source: 'gemini', length: text.length, persona: persona.id }, 'Dynamic greeting generated');
        return text;
    }
    catch (error) {
 ...
 */
function generateMemoryBasedGreeting(persona, userName, memories) {
...
// ============================================================================
import { generateProactiveOpener } from '../conversation/proactive-starters.js';
...
import { findEventsToAcknowledge, generateEventAcknowledgment, } from './shared/life-events.js';
 ...
 */
export async function generateGreeting(persona, options) {
    ...
    if (options?.lifeEvents && options.lifeEvents.length > 0) {
        ...
        if (eventsToAcknowledge.length > 0) {
            ...
            if (eventAck) {
                getLogger().info({ persona: persona.id, eventType: event.type }, '🎂 Including life event acknowledgment in greeting');
    ...
    if (options?.conversationCount && isMilestoneConversation(options.conversationCount)) {
        ...
        if (milestoneMsg) {
            getLogger().info({ persona: persona.id, conversationCount: options.conversationCount }, '🎉 Including milestone message in greeting');
 ...
 */
async function generateGreetingWithoutLifeEvents(persona, options) {
    // Static is now last resort only
    const staticGreeting = generateStaticGreeting(persona, options);
    ...
    if (options?.isReturningUser && Math.random() < 0.6) {
        try {
            ...
            if (opener) {
                ...
                getLogger().info({ persona: persona.id, type: opener.type, reason: opener.reason }, '🚀 Using PROACTIVE opener - context-aware greeting');
                return greeting;
            }
        }
        catch (err) {
    ...
    if (options?.isReturningUser && options.userName && Math.random() < 0.5) {
        try {
            ...
            if (userId) {
                ...
                const { isPersonalJourneyFeatureEnabled, isUserInPersonalJourneyRollout } = await import('../config/feature-flags.js');
                if (isPersonalJourneyFeatureEnabled('greetingEnhancement') &&
                    ...
                    if (journeyEnhancement.hasEnhancement && journeyEnhancement.content) {
                        ...
                        const baseGreeting = pickUnusedTemplate(getGreetingTemplates(persona.communication.greetingStyle, persona.identity.selfReference).returningUser, options.usedGreetings).replace(/{name}/g, options.userName);
                        const enhancedGreeting = `${baseGreeting} <break time="300ms"/> ${journeyEnhancement.content}`;
                        getLogger().info({ persona: persona.id, type: journeyEnhancement.type }, '🌟 Using PERSONAL JOURNEY greeting enhancement');
            ...
            getLogger().debug({ error: String(err) }, 'Personal journey greeting enhancement failed, continuing...');
        }
    }
    ...
    if (options?.isReturningUser && options?.personaMemories && options.personaMemories.length > 0) {
        if (Math.random() < 0.6) {
            const memoryGreeting = generateMemoryBasedGreeting(persona, options.userName, options.personaMemories);
            if (memoryGreeting) {
                getLogger().info({ persona: persona.id, type: 'memory-based' }, '🧠 Using memory-enhanced greeting');
    ...
    if (Math.random() < 0.5) {
        try {
            ...
            if (dynamicGreeting) {
                getLogger().info({ persona: persona.id, dynamicLength: dynamicGreeting.length }, 'Using dynamic Gemini greeting');
    ...
    try {
        const compositional = await generateCompositionalGreeting(options?.bundleRuntime ?? null, persona, {
            userName: options?.userName,
            isReturningUser: options?.isReturningUser,
            relationshipStage: options?.relationshipStage,
        });
        if (compositional) {
            getLogger().info({ persona: persona.id, type: 'compositional' }, '🧩 Using COMPOSITIONAL greeting - built from atoms');
 ...
 */
export function generateRandomGreeting(persona, options) {
