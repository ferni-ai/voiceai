...
const IMPERFECTION_PATTERNS: Record<string, ImperfectionSet> = {
  ...
  'maya-santos': {
    ...
    midThoughtNotices: [
      "Oh! Hey— I was hoping you'd come by. ",
      "Wait— oh good, it's you! ",
    ],
    corrections: [
 ...
 */
function generatePhysicalIntro(
  personaId: string,
  ctx: AliveIntroContext
): AliveIntroResult | null {
...
function generateInnerWorldIntro(
  ...
  const needsClosing = !bleedMoment.includes("What's up") && !bleedMoment.includes("What's going on");
...
export async function generateAliveIntro(
  ...
  generators.push({
    name: 'inner_world',
    weight: ctx.relationshipStage === 'trusted_advisor' ? 30 : ctx.relationshipStage === 'friend' ? 20 : 10,
