...
export class CognitiveIntelligenceEngine {
     ...
     */
    getAttentionCuePrompt(focus, context) {
        const focusPrompts = {
     ...
     */
    getBlindSpotDescription(blindSpot) {
        const descriptions = {
    ...
    checkForBiases(context) {
        ...
        for (const bias of primaryBiases) {
            ...
            const triggered = bias.triggers.some((trigger) => context.currentTopic.toLowerCase().includes(trigger.toLowerCase()));
            if (triggered && Math.random() < biasIntensity) {
                if (selfAwareness) {
                    // Self-aware personas can catch themselves
                    const phrase = this.profile.biases.biasRecognitionPhrases[Math.floor(Math.random() * this.profile.biases.biasRecognitionPhrases.length)];
    ...
    selectPhrases(context) {
        ...
        if (Math.random() < 0.3 && this.profile.signatureThinkingPhrases.length > 0) {
            phrases.push(this.profile.signatureThinkingPhrases[Math.floor(Math.random() * this.profile.signatureThinkingPhrases.length)]);
        }
        return phrases;
    }
    /**
     ...
     */
    confidenceMatchesLevel(confidence, level) {
        switch (level) {
            case 'very_confident':
    ...
    calculateAverageExpertise() {
        ...
        for (const entry of this.conversationHistory) {
            const level = entry.userExpertise;
 ...
 */
export function getCognitiveEngine(personaId, profile) {
 ...
 */
export function removeCognitiveEngine(personaId) {
    engines.delete(personaId);
}
/**
 * Reset all cognitive engines
 */
export function resetAllCognitiveEngines() {
    for (const engine of engines.values()) {
        engine.reset();
    }
}
export default CognitiveIntelligenceEngine;
//# sourceMappingURL=cognitive-intelligence.js.map