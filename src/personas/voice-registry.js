...
import { toCanonical, isKnownId, DISPLAY_NAMES, ALL_CANONICAL_IDS, ALIAS_TO_CANONICAL, } from './persona-ids.js';
 ...
 */
export async function initializeVoiceRegistry() {
    if (initialized) {
 ...
 */
export function getVoiceId(personaId) {
 ...
 * NOTE: Alias resolution is delegated to persona-ids.ts (SINGLE SOURCE OF TRUTH).
 */
export function getVoiceEntry(personaId) {
    if (!initialized) {
        initializeFallbacks();
    }
    const normalized = personaId.toLowerCase().trim();
 ...
 */
export function getCanonicalPersonaId(personaId) {
    const normalized = personaId.toLowerCase().trim();
 ...
 */
export function isKnownPersona(personaId) {
 ...
 */
export function getAliasesForPersona(canonicalId) {
    const aliases = [];
 ...
 */
export function getFrontendPersonaId(personaId) {
 ...
 */
export function isVoiceRegistryInitialized() {
    return initialized;
}
/**
