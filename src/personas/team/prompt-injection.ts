...
import type { TeamConfig, TeamContext, TeamMember } from './types.js';
import { DEFAULT_TEAM_CONFIG } from './team-config.js';

/**
 * Generate the teammates section for a system prompt
 */
export function generateTeammatesSection(
  currentRole: string,
 ...
 */
export function generateHandoffSection(
  currentRole: string,
  team: TeamConfig = DEFAULT_TEAM_CONFIG
): string {
 ...
 */
export function generateCoordinationSection(
  currentRole: string,
  team: TeamConfig = DEFAULT_TEAM_CONFIG
): string {
  ...
  if (relevantRouting.length === 0) return '';
 ...
 */
export function injectTeamContext(systemPrompt: string, context: TeamContext): string {
