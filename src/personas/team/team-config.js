...
export const DEFAULT_TEAM_MEMBERS = [
    {
        ...
        characterId: 'ferni',
        displayName: 'Ferni',
        roleDescription: 'Your main point of contact. Asks the questions that unlock insight, celebrates every win, and connects you to the right specialist when you need them.',
        active: true,
    },
    ...
    },
    {
        ...
        displayName: 'Nayan',
        roleDescription: 'Your lifetime coach. Combines patience, simplicity, and wit to help you see that small, consistent actions over decades create extraordinary results.',
        active: true,
    },
];
/**
 ...
 */
export const DEFAULT_HANDOFF_TEMPLATES = [
 ...
 * @deprecated Prefer using getTeamConfig() which generates from bundles.
 */
export const DEFAULT_TEAM_COORDINATION = {
    teammateReferences: [
 ...
 */
export function getTeamMemberByRole(roleId, team = DEFAULT_TEAM_CONFIG) {
    return team.members.find((m) => m.roleId === roleId);
}
/**
 * Get a team member by character ID
 */
export function getTeamMemberByCharacter(characterId, team = DEFAULT_TEAM_CONFIG) {
    return team.members.find((m) => m.characterId === characterId);
}
/**
 * Get handoff templates from one role to another
 */
export function getHandoffTemplates(fromRole, toRole, team = DEFAULT_TEAM_CONFIG) {
    const template = team.handoffTemplates?.find((t) => t.fromRole === fromRole && t.toRole === toRole);
    return template?.phrases || [];
}
/**
 * Get a random handoff phrase
 */
export function getRandomHandoffPhrase(fromRole, toRole, team = DEFAULT_TEAM_CONFIG) {
 ...
 */
export function getCoordinatorId(team = DEFAULT_TEAM_CONFIG) {
    return team.coordinatorId;
}
/**
 * Check if team mode is enabled
 */
export function isTeamEnabled(team = DEFAULT_TEAM_CONFIG) {
    return team.enabled;
}
// ============================================================================
 ...
 */
export async function generateTeamConfigFromBundles(forceRefresh = false) {
    const now = Date.now();
    ...
    members.sort((a, b) => {
        if (a.characterId === coordinatorId)
            return -1;
        if (b.characterId === coordinatorId)
            return 1;
        return 0;
    });
 ...
 */
async function generateHandoffTemplatesFromBundles(bundles, coordinatorId) {
    const templates = [];
 ...
 */
function generateTaskRoutingFromBundles(bundles) {
    const routing = [];
    for (const bundle of bundles) {
        const { manifest } = bundle;
 ...
 */
export async function getHandoffTriggersForPersona(personaId) {
    const result = await discoverAndLoadBundles();
    for (const bundle of result.bundles) {
        const { manifest } = bundle;
 ...
 */
export async function getAllHandoffTriggers() {
    const result = await discoverAndLoadBundles();
