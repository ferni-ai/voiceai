...
import { applyVoiceAdjustmentToEntrance, getVoiceEmotionEntranceAdjustment, shouldVoiceOverrideMood, } from './voice-emotion-entrances.js';
...
const loadedEntranceConfigs = new Map();
/**
 ...
 */
async function loadEntranceConfigFromBundle(personaId) {
    // Check cache first
    ...
    try {
        ...
        const hasContent = config.acknowledgments.length > 0 ||
 ...
 */
async function getEntranceConfig(personaId) {
...
export function clearEntranceConfigCache() {
...
const HARDCODED_ENTRANCE_CONFIGS = {
    'jordan-taylor': {
 ...
 */
function generateCalmEntrance(config, ctx) {
 ...
 */
function generateExcitedEntrance(config, ctx) {
    const entrance = config.matchedExcitement[Math.floor(Math.random() * config.matchedExcitement.length)];
    return {
        entrance,
 ...
 */
function generateCaughtMomentEntrance(config, ctx, caughtDoing) {
 ...
 */
function generateSelfAwareEntrance(config, ctx) {
 ...
 */
function generateRelationshipEntrance(config, ctx) {
    // Map relationship stage to greeting category
    const relationshipGreetings = {
        acquaintance: [
 ...
 */
function generateTimeAppropriateEntrance(config, ctx) {
 ...
 */
function generateMemoryCallbackEntrance(config, ctx) {
    if (!ctx.lastTopicWithAgent)
        return null;
    const template = config.memoryCallbacks[Math.floor(Math.random() * config.memoryCallbacks.length)];
 ...
 */
function getPersonaGroundedFollowUp(personaId) {
    ...
    if (cognitiveDiff?.questioning?.questionStarters &&
 ...
 */
export async function generateAliveEntrance(runtime, personaId, options = {}) {
    ...
    if ((ctx.timeOfDay === 'late_night' || ctx.timeOfDay === 'early_morning') &&
        Math.random() < 0.4) {
    ...
    if (ctx.lastTopicWithAgent && Math.random() < 0.5) {
        ...
        if (memoryEntrance) {
            getLogger().info({ personaId, topic: ctx.lastTopicWithAgent, style: 'memory_callback' }, '🧠 BETTER THAN HUMAN: Memory callback entrance - referencing past conversation');
    ...
    if (ctx.relationshipStage && ctx.relationshipStage !== 'stranger' && Math.random() < 0.4) {
        ...
        if (relationshipEntrance) {
            getLogger().info({ personaId, relationshipStage: ctx.relationshipStage, style: 'relationship_based' }, '💝 ENTRANCE: Relationship-aware greeting based on connection depth');
 ...
 */
export async function getAliveEntrance(personaId, runtime, userMood) {
    const result = await generateAliveEntrance(runtime || null, personaId, { userMood });
    if (result) {
        getLogger().info({ personaId, style: result.style, components: result.components }, '✨ Generated alive entrance');
 ...
 */
export async function getAliveEntranceForHandoff(personaId, options = {}) {
    try {
 ...
 */
export function detectUserMoodFromContext(lastUserMessage, lastEmotionAnalysis) {
    ...
    if (lastEmotionAnalysis) {
        ...
        if (['sadness', 'fear', 'anxiety'].includes(primary))
            return 'sad';
        if (['anger', 'frustration'].includes(primary))
            return 'stressed';
        if (['joy', 'excitement', 'happiness'].includes(primary))
            return 'excited';
        if (['confusion', 'uncertainty'].includes(primary))
            return 'confused';
    }
...
//# sourceMappingURL=alive-entrances.js.map