 ...
 */
export function generatePerspective(personaId, situation, topic, details = {}) {
 ...
 */
export function generateCollaborativePerspectives(situation, topic, personaIds = ['ferni'] // Default to Ferni only - callers must pass unlocked IDs
) {
 ...
 */
function findEmergentInsights(perspectives) {
    const insights = [];
...
function noticeMatchesMiss(notice, miss) {
    const noticeKeywords = notice.toLowerCase().split(/\s+/);
    const missKeywords = miss.toLowerCase().split(/\s+/);
    const overlap = noticeKeywords.filter((kw) => missKeywords.some((mk) => mk.includes(kw) || kw.includes(mk)));
    return overlap.length >= 1;
}
/**
 * Recommend which persona should handle based on cognitive fit
 */
function recommendCognitiveHandoff(situation, topic, perspectives) {
    const situationLower = situation.toLowerCase();
    const topicLower = topic.toLowerCase();
    // Emotional situations → Maya
    if (situationLower.includes('stress') ||
    ...
    if (topicLower.includes('meaning') ||
    ...
    if (topicLower.includes('organize') ||
 ...
 */
export function generateTeamCommentary(personaId, topic, context, unlockedMemberIds) {
    const commentary = [];
...
function generateHandoffCommentary(from, to, toName, topic) {
...
function generateReflectionCommentary(style, otherStyle, otherName) {
    const reflections = {
...
function generateCollaborationCommentary(style, otherStyle, otherName) {
...
//# sourceMappingURL=collaborative-cognition.js.map