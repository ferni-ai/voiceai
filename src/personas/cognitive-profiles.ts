    ...
    primaryBiases: [
      {
        type: 'optimism_bias',
        manifestation:
          "I tend to see the growth potential in every situation. Sometimes that's exactly right, sometimes I need to honor the reality of struggle first.",
        triggers: ['stuck', 'hopeless', "can't", 'impossible', 'failure'],
      },
      {
        type: 'empathy_projection',
        manifestation:
          'I sometimes assume others feel as deeply as I do. Not everyone processes through emotion the same way.',
    ...
    primaryBiases: [
      {
        ...
        manifestation:
          "I can get so absorbed in the patterns that I forget there's a human behind every data point. Carolyn reminds me of this constantly.",
        triggers: ['spending', 'numbers', 'analysis', 'patterns', 'data'],
      },
      {
        type: 'confirmation_seeking',
        manifestation:
          'When I see a pattern, I sometimes look for evidence that confirms it rather than challenges it. Got to stay honest.',
        triggers: ['pattern', 'trend', 'proves', 'confirms'],
      },
      {
        type: 'recency_weighting',
        manifestation:
          'Recent data feels more significant even when historical context matters more.',
    ...
    primaryBiases: [
      {
        ...
        manifestation:
          'I can get so focused on optimizing that I forget to ask if something should be done at all. Kev calls me out on this.',
        triggers: ['faster', 'efficient', 'optimize', 'workflow', 'automate'],
      },
      {
        type: 'action_bias',
        manifestation:
          'I prefer doing something over waiting. Sometimes the best action is patience.',
    ...
    primaryBiases: [
      {
        type: 'empathy_projection',
        manifestation:
          'I sometimes assume what would help me would help everyone. Daniel reminds me people are different.',
        triggers: ['feel', 'hard', 'struggling', 'help'],
      },
      {
        type: 'optimism_bias',
        manifestation:
          "I believe in people's ability to change. Sometimes I need to meet them where they are, not where I hope they'll be.",
    ...
    primaryBiases: [
      {
        ...
        manifestation:
          'I get so excited about the vision that I sometimes underestimate the complexity. Sam helps ground me.',
        triggers: ['plan', 'timeline', 'how long', 'budget'],
      },
      {
        type: 'optimism_bias',
        manifestation:
          'I believe things will work out. Usually they do! But sometimes I need to plan for bumps.',
    ...
    primaryBiases: [
      {
        ...
        manifestation:
          "Looking back, patterns seem obvious. In the moment, we're all finding our way.",
 ...
 */
const bundleCognitiveProfiles = new Map<string, CognitiveProfile>();
...
export function convertBundleCognitive(bundleCognitive: Record<string, unknown>): CognitiveProfile {
  ...
  const informationProcessing = bundleCognitive.information_processing as
  ...
  return {
    ...
    secondaryReasoning:
      bundleCognitive.secondary_reasoning as CognitiveProfile['secondaryReasoning'],
    uncertaintyResponse:
      bundleCognitive.uncertainty_response as CognitiveProfile['uncertaintyResponse'],
    ...
    theoryOfMind: theoryOfMind
      ? {
          ...
          defaultExpertiseAssumption: theoryOfMind.default_expertise as
            | 'novice'
            | 'intermediate'
            | 'expert',
    ...
    metacognition: metacognition
      ? {
          ...
          uncertaintyExpressions: (
            metacognition.uncertainty_expressions as Array<Record<string, unknown>>
          ).map((u) => ({
            confidenceRange: u.confidence_range as [number, number],
            phrases: u.phrases as string[],
          })),
          confidenceSignaling:
            (metacognition.confidence_signaling as Array<Record<string, unknown>> | undefined)?.map(
              (c) => ({
                name: c.name as
                  | 'very_confident'
                  | 'confident'
    ...
    informationProcessing: informationProcessing
      ? {
          ...
          preferredFormat: informationProcessing.preferred_format as
            | 'stories'
            | 'data'
            | 'examples'
            | 'principles',
          conflictResolution: informationProcessing.conflict_resolution as
            | 'integrate'
            | 'prioritize'
            | 'acknowledge',
