...
import { toCanonical, isKnownId as isKnownIdFromSource, DISPLAY_NAMES, } from './persona-ids.js';
// ============================================================================
...
export function resolveAgentId(input) {
    ...
    if (Object.values(AgentRole).includes(input)) {
        return ROLE_TO_PERSONA[input];
    }
    // Otherwise, resolve string alias to canonical ID
    return getPersonaId(input);
}
/**
 ...
 */
export function getAgentRoleForPersona(id) {
...
const roleToPersonaCache = new Map();
/** Initialize the role cache */
function initializeRoleCache() {
    if (roleToPersonaCache.size > 0)
        return;
...
export function getDisplayName(id) {
    ...
    return (PERSONA_REGISTRY[personaId]?.displayName || DISPLAY_NAMES[personaId] || id);
}
/**
 * Get the short name from any ID format.
 */
export function getShortName(id) {
    const personaId = getPersonaId(id);
 ...
 */
export function getPersonaMetadata(id) {
    const personaId = getPersonaId(id);
 ...
 */
export function getHandoffToolName(id) {
    const personaId = getPersonaId(id);
 ...
 */
export function isTeamMember(id) {
    const personaId = getPersonaId(id);
 ...
 * but also handles AgentRole enum values.
 */
export function isKnownPersonaId(id) {
