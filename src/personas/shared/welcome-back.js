 ...
 */
function getTimeBucket(daysSinceContact) {
 ...
 */
export function generateWelcomeBack(profile) {
    ...
    const milestoneMessage = RETURNING_USER_RECOGNITION.milestones[conversationCount];
    if (milestoneMessage) {
 ...
 */
export function getTimeBasedGreeting(daysSinceContact, name) {
 ...
 */
export function isMilestoneConversation(conversationCount) {
    return [5, 10, 25, 50, 100].includes(conversationCount);
}
/**
 * Get milestone message if applicable
 */
export function getMilestoneMessage(conversationCount) {
    return (RETURNING_USER_RECOGNITION.milestones[conversationCount] || null);
}
