 ...
 */
export function extractPattern(expression) {
 ...
 */
export function adaptPatternToPersona(pattern, targetPersonaId) {
 ...
 */
export function learnFromExpression(expression, sourcePersona, theme, context, engagement) {
 ...
 */
export function recordPatternEngagement(patternId, engagement) {
 ...
 */
export function getBestPatternsForPersona(personaId, theme, context, limit = 3) {
    const results = [];
    for (const pattern of learnedPatterns.values()) {
        // Skip low-scoring patterns
        if (pattern.engagement.score < 0.5)
            continue;
 ...
 */
export function getPatternsFromPersona(sourcePersonaId) {
 ...
 */
export function prunePatterns(maxAgeMs = 30 * 24 * 60 * 60 * 1000) {
...
//# sourceMappingURL=cross-persona-learning.js.map