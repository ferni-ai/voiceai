...
function detectEmotionalTrajectory(input) {
    ...
    const positiveCount = recentEmotions.filter((e) => positiveEmotions.includes(e.toLowerCase())).length;
    const negativeCount = recentEmotions.filter((e) => negativeEmotions.includes(e.toLowerCase())).length;
    ...
    if (currentPositive && negativeCount > positiveCount)
        return 'rising';
    // Falling: more negative now than before
    if (currentNegative && positiveCount > negativeCount)
        return 'falling';
...
function detectSpeechPace(input) {
    if (!input.speechRateWPM)
        return 'normal';
    if (input.speechRateWPM > 180)
        return 'fast';
    if (input.speechRateWPM < 100)
        return 'slow';
...
function detectEnergyLevel(input) {
...
function detectTopicShift(input) {
    if (!input.currentTopics || input.currentTopics.length === 0)
        return false;
    if (!input.lastTopics || input.lastTopics.length === 0)
        return false;
    // Check if any current topic overlaps with last topics
    const hasOverlap = input.currentTopics.some((ct) => input.lastTopics.some((lt) => ct.toLowerCase().includes(lt.toLowerCase()) || lt.toLowerCase().includes(ct.toLowerCase())));
    return !hasOverlap;
}
function detectUserSharingType(transcript) {
    const lower = transcript.toLowerCase();
    // Win patterns
    if (lower.match(/i (did|finished|completed|achieved|got|made|passed)/i) ||
        lower.match(/finally|succeeded|accomplished/i)) {
        return 'win';
    }
    // Struggle patterns
    if (lower.match(/i('m| am) (struggling|having trouble|stuck|overwhelmed)/i) ||
...
function isPersonalSharing(transcript, distressLevel) {
    const lower = transcript.toLowerCase();
...
function isHeavyTopic(transcript, distressLevel) {
    const lower = transcript.toLowerCase();
    const heavyPatterns = [
    ...
    if (heavyPatterns.some((p) => p.test(lower)))
        return true;
    // High distress indicates heavy topic
    if (distressLevel && distressLevel > 0.7)
        return true;
    return false;
}
// ============================================================================
 ...
 */
export function assemblePersonalityContext(input) {
...
function mapRelationshipStage(input) {
...
//# sourceMappingURL=personality-context-assembler.js.map