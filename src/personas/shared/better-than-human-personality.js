 ...
 */
export function composeRealtimeNoticing(ctx, blocks) {
...
// ============================================================================
export function composeTemporalExpression(ctx, blocks) {
    // Late night presence - special mode for all personas
    if (ctx.timeOfDay === 'late_night') {
        const lateNightPhrase = blocks.temporalPhrases?.late_night?.[0] || randomFrom(SHARED_SENSORY_FRAGMENTS.late_night);
        return {
    ...
    if (ctx.timeOfDay === 'dawn') {
        const dawnPhrase = blocks.temporalPhrases?.dawn?.[0] || randomFrom(SHARED_SENSORY_FRAGMENTS.morning);
        return {
    ...
    if (ctx.isWeekend && ctx.timeOfDay === 'evening') {
        const eveningPhrase = blocks.temporalPhrases?.evening?.[0] || randomFrom(SHARED_SENSORY_FRAGMENTS.evening);
        return {
...
// ============================================================================
export function composeConnectionCallback(ctx, blocks) {
...
// ============================================================================
export function composePassionExpression(ctx, blocks) {
    if (!blocks.passions || blocks.passions.length === 0) {
        return null;
    }
    // Find relevant passion based on current topic
    const relevantPassion = blocks.passions.find((p) => ctx.currentTopic?.toLowerCase().includes(p.topic.toLowerCase()) ||
...
// ============================================================================
export function composeQuirkyExpression(ctx, blocks) {
...
// ============================================================================
export function composeLocationExpression(ctx, blocks) {
...
// ============================================================================
export function composeVulnerabilityExpression(ctx, blocks) {
 ...
 */
export function composeExpression(ctx) {
 ...
 */
function composeOrganicExpression(ctx, blocks) {
...
function composeWarmDrinkExpression(ctx, blocks) {
...
function composeFamilyExpression(ctx, blocks) {
...
// DECISION LOGIC
// ============================================================================
function shouldDoRealtimeNoticing(ctx) {
    return (ctx.pauseBeforeUserSpoke > 1500 || ctx.voiceEnergyLevel === 'subdued' || ctx.topicShiftDetected);
}
function shouldDoConnectionCallback(ctx) {
    return (ctx.relationshipStage !== 'stranger' &&
        ctx.conversationsTotal >= 3 &&
        ctx.conversationMomentum !== 'intimate' &&
        Math.random() < 0.2);
}
function shouldDoTemporalExpression(ctx) {
    return (ctx.timeOfDay === 'late_night' ||
        ctx.timeOfDay === 'dawn' ||
        (ctx.isWeekend && ctx.timeOfDay === 'evening'));
}
function shouldDoPassionExpression(ctx) {
    return ctx.conversationMomentum === 'cruising' && ctx.turnCount > 3 && Math.random() < 0.25;
}
function isRightMomentForPersonality(ctx) {
...
function selectThemeForContext(ctx, blocks) {
    const themes = [];
...
function selectLocationForContext(ctx, locations) {
...
function findNaturalConnection(topic, ctx, blocks) {
...
//# sourceMappingURL=better-than-human-personality.js.map