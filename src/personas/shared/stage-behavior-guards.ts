 ...
 */
const DEFAULT_STAGE_BEHAVIORS: Record<RelationshipStage, string[]> = {
 ...
 */
const BEHAVIOR_MAPPING: Record<GuardableBehavior, string[]> = {
...
const MINIMUM_THRESHOLDS: Record<
  GuardableBehavior,
  { minSessions?: number; minTurns?: number; stage?: RelationshipStage }
> = {
...
function isAtOrBeyondStage(current: RelationshipStage, required: RelationshipStage): boolean {
...
export function checkBehavior(
  ...
  const hasMatchingBehavior =
    allowedBehaviors.includes('*') || behaviorNames.some((name) => allowedBehaviors.includes(name));
  ...
  log.debug(
    { behavior, stage, allowed: true, intensity: intensityMultiplier },
 ...
 */
function calculateIntensity(behavior: GuardableBehavior, context: BehaviorContext): number {
  ...
  const stageIntensity: Record<RelationshipStage, number> = {
    stranger: 0.5,
    acquaintance: 0.7,
    friend: 0.85,
 ...
 */
function getSuggestion(behavior: GuardableBehavior, reason: string): string {
  const suggestions: Record<GuardableBehavior, Record<string, string>> = {
    personal_story: {
...
export function checkBehaviors(
  behaviors: GuardableBehavior[],
  context: BehaviorContext
): Map<GuardableBehavior, BehaviorGuardResult> {
  const results = new Map<GuardableBehavior, BehaviorGuardResult>();
 ...
 */
export function getAllowedBehaviorsForContext(context: BehaviorContext): GuardableBehavior[] {
  const allBehaviors: GuardableBehavior[] = [
    'personal_story',
    'direct_advice',
    'humor',
    'vulnerability_sharing',
 ...
 */
export function generateBehaviorConstraints(context: BehaviorContext): string {
  ...
  for (const behavior of allowed) {
    ...
    if (result.intensityMultiplier < 1) {
      lines.push(
        `• ${humanName} (gentle - ${Math.round(result.intensityMultiplier * 100)}% intensity)`
      );
    } else {
