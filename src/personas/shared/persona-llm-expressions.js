...
// VOICE DNA LOADING
// ============================================================================
const voiceDnaCache = new Map();
/**
 * Load voice DNA for a persona from their voice-guidance.md file
 */
async function loadVoiceDna(personaId) {
    // Check cache
 ...
 */
export async function generatePersonaExpressions(personaId, themes, context) {
    // Rate limiting
    const now = Date.now();
    ...
    try {
        ...
        const expressions = parsed.map((item, i) => ({
 ...
 */
function buildPrompt(config, themes, context, voiceDna) {
...
// CACHE MANAGEMENT
// ============================================================================
function addToCache(personaId, expressions) {
 ...
 */
export function getPersonaExpression(personaId, themeId, context) {
 ...
 */
export async function prewarmPersonaExpressions(personaId, context) {
    const config = PERSONA_CONFIGS[personaId];
    if (!config)
        return;
 ...
 */
export function clearPersonaCache(personaId) {
 ...
 */
export function hasPersonaExpressionSupport(personaId) {
    return personaId in PERSONA_CONFIGS;
}
//# sourceMappingURL=persona-llm-expressions.js.map