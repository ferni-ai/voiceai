...
const HUMANIZATION_PROMPTS: Record<string, string> = {
...
const GENERATION_TEMPLATES: Record<HumanizationType, Record<string, string>> = {
  subtext: {
...
// Cache
const humanizationCache = new Map<string, GeneratedHumanization[]>();
const MAX_CACHE = 20;

/**
 * Generate a humanization response using LLM
 */
export async function generateHumanizationLLM(
  context: HumanizationContext
): Promise<GeneratedHumanization | null> {
...
Return ONLY the response text with SSML. No JSON, no explanation.`;
    ...
    const result: GeneratedHumanization = {
      type,
      subtype,
...
function addToCache(personaId: string, humanization: GeneratedHumanization): void {
...
function getCachedHumanization(
  personaId: string,
  type: HumanizationType,
  subtype: string
): GeneratedHumanization | null {
 ...
 */
export function clearHumanizationCache(personaId: string): void {
