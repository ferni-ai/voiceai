...
// EXPORT MAP
// ============================================================================
export const personaCognitiveQuirks = {
    ferni: ferniQuirks,
 ...
 */
export function getCognitiveQuirks(personaId) {
    return personaCognitiveQuirks[personaId];
}
/**
 * Get a random activated quirk based on context
 */
export function getActiveQuirk(personaId, contextText) {
    const quirks = getCognitiveQuirks(personaId);
    if (!quirks)
        return null;
    const contextLower = contextText.toLowerCase();
    // Find quirks whose triggers match the context
    const matchingQuirks = quirks.quirks.filter((quirk) => quirk.triggers.some((trigger) => contextLower.includes(trigger.toLowerCase())));
    if (matchingQuirks.length === 0)
        return null;
...
export function getTransitionPhrase(personaId) {
    const quirks = getCognitiveQuirks(personaId);
    if (!quirks || quirks.transitionPhrases.length === 0)
        return null;
