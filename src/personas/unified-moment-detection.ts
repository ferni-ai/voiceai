...
export type MomentCategory =
  | 'emotional' // Vulnerability, celebration, crisis
  | 'cognitive' // Breakthrough, realization, insight
  | 'relational' // Trust demonstration, sharing, opening up
  | 'behavioral' // Laughter, engagement, deflection
  | 'life_event'; // Major life events mentioned
...
const MEMORABLE_DETAIL_PATTERNS = [
  {
    pattern:
      /my (mom|dad|mother|father|sister|brother|wife|husband|son|daughter|partner|kids?|children)/i,
    category: 'family',
  },
 ...
 */
export function detectMomentsUnified(context: MomentDetectionContext): MomentDetectionResult {
  const { userMessage, sessionNumber, hasSharedVulnerabilityBefore, topic } = context;
  const messageLower = userMessage.toLowerCase();

  const moments: UnifiedMoment[] = [];
  ...
  for (const patterns of Object.values(BREAKTHROUGH_PATTERNS)) {
    ...
    if (moments.some((m) => m.type === 'breakthrough')) break;
  }
...
function extractTriggerPhrase(message: string, pattern: RegExp): string | undefined {
  const match = message.match(pattern);
  return match ? match[0] : undefined;
}

function extractMemorableDetail(match: RegExpMatchArray, category: string): string | null {
  switch (category) {
    case 'family':
...
function generateAcknowledgment(moment: UnifiedMoment): string {
  switch (moment.type) {
    case 'breakthrough':
 ...
 */
export function detectMoments(context: MomentDetectionContext): UnifiedMoment[] {
