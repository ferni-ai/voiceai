 ...
 */
import { playAmbientMusic as playAmbient, stopAmbientMusic as stopAmbient, } from '../audio/ambient-music.js';
...
import { loadSilenceResponses } from '../services/persona-content-loader.js';
...
import { generatePresenceExpression, isSittingWithCliche, } from '../conversation/superhuman/dynamic-presence.js';
const log = getLogger();
// ============================================================================
// DYNAMIC CONTENT CACHE
// ============================================================================
/** Cache for loaded silence responses per persona */
const silenceContentCache = new Map();
/** Track recently used phrases to avoid repetition */
const recentlyUsedPhrases = new Map();
const MAX_RECENT_PHRASES = 10;
/** Track phrase usage for recency */
function trackPhraseUsage(sessionId, phrase) {
    if (!sessionId)
        return;
 ...
 */
async function getSilenceContent(personaId) {
 ...
 */
function _randomFromDynamic(arr, fallback) {
...
// ============================================================================
const TOPIC_SPECIFIC_RESPONSES = {
    retirement: [
 ...
 */
function _detectTopicCategory(context) {
    const textToSearch = [
...
function interpolateTemplate(template, context) {
    let result = template;
    // Replace {topic}
    const topic = context.wasDiscussingTopic ||
 ...
 */
function getTimeBasedReference(hour) {
 ...
 */
function generateSmartFallback(context, invitesReply) {
    ...
    let contextRef;
    const hasContext = context.wasDiscussingTopic || (context.topicsDiscussed && context.topicsDiscussed.length > 0);
 ...
 */
function getSmartFallbackWithRecency(context, invitesReply) {
    const sessionId = context.sessionId || 'default';
 ...
 */
export function getMeaningfulSilenceResponse(persona, context) {
    const { silenceDurationSeconds, turnCount, topicsDiscussed, recentEmotionalTone, wasDiscussingTopic, memorableMoments, currentHour = new Date().getHours(), isWeekend = [0, 6].includes(new Date().getDay()), silenceResponseCount = 0, isGameActive = false, activeGameType, isMusicPlaying = false, sessionId = 'default-silence-session', } = context;
    ...
    if (silenceDurationSeconds < 40) {
        ...
        const hasAmbientTracks = !!(process.env.AMBIENT_TRACK_1 ||
        ...
        if (Math.random() < 0.3 && musicMasterEnabled && ambientEnabled && hasAmbientTracks) {
            ...
            const djOffer = getSpontaneousMusicOffer(getCanonicalPersonaId(persona.id), {
                ...
                recentMood: context.recentEmotionalTone === 'heavy'
                    ? 'stressed'
        ...
        if (Math.random() < 0.1 && musicMasterEnabled) {
            try {
                ...
                if (sessionHistory && sessionHistory.length > 0) {
                    const conversationStarter = getMusicConversationStarter(getCanonicalPersonaId(persona.id), {
 ...
 */
function silenceContextToQuestionContext(context, persona, sessionId) {
    ...
    else if (context.turnCount > 20)
        relationshipStage = 'established';
    else if (context.turnCount > 5)
        relationshipStage = 'building';
    // Infer silence reason
    let silenceReason = 'unknown';
    if (context.recentEmotionalTone === 'heavy')
        silenceReason = 'emotional';
    else if (context.isGameActive)
        silenceReason = 'thinking';
    else if (context.silenceDurationSeconds > 30)
        silenceReason = 'processing';
    return {
 ...
 */
async function getDynamicThoughtfulQuestion(context, persona, sessionId, options) {
    try {
        ...
        try {
            const questionContext = silenceContextToQuestionContext(context, persona, sessionId);
            let questionType = 'silence_break';
            if (context.recentEmotionalTone === 'heavy') {
 ...
 */
function getThoughtfulQuestionSync(context, persona) {
 ...
 */
function getGameSilenceResponse(gameType) {
    const responses = {
        'name-that-tune': [
 ...
 */
function getGameSuggestion(personaId) {
 ...
 */
function selectBestTopicForCallback(topics) {
 ...
 */
function getRelevantQuestions(topics) {
 ...
 */
function getPersonaObservations(persona) {
 ...
 */
function findRelevantStoryForSilence(stories, topic) {
 ...
 */
function getTimeAwareResponse(hour, _isWeekend) {
 ...
 * Uses canonical ID resolution for consistent persona matching
 */
function getPersonaMicroStory(persona) {
    const canonicalId = getCanonicalPersonaId(persona.id);
    const storyMap = {
 ...
 * Uses canonical ID resolution for consistent persona matching
 */
function getPersonaHumor(persona) {
    const canonicalId = getCanonicalPersonaId(persona.id);
 ...
 */
function getThinkingOutLoudMoment(context, _persona) {
 ...
 */
export async function getMicroStoryAsync(persona) {
    const canonicalId = getCanonicalPersonaId(persona.id);
 ...
 */
export async function getThoughtfulQuestionAsync(persona, topics) {
    const canonicalId = getCanonicalPersonaId(persona.id);
    const dynamicContent = await getSilenceContent(canonicalId);
 ...
 */
export async function getMusicOfferingAsync(persona) {
    const canonicalId = getCanonicalPersonaId(persona.id);
    const dynamicContent = await getSilenceContent(canonicalId);
 ...
 * Let the LLM handle weekend context naturally instead of injecting static phrases.
 */
export async function getTimeAwareResponseAsync(persona, hour, _isWeekend) {
    const canonicalId = getCanonicalPersonaId(persona.id);
    const dynamicContent = await getSilenceContent(canonicalId);
    // REMOVED: Weekend awareness injection (30% random)
    // This was causing robotic "consistent quotes about the weekend"
...
export class SilenceHandler {
    ...
    getNextResponse() {
        ...
        const shouldRespond = (silenceDuration >= 10 && !this.responsesSent.includes('comfortable_presence')) ||
            (silenceDuration >= 20 &&
...
const MEMORABLE_PATTERNS = [
    // Family mentions
    {
        pattern: /\b(?:my|our)\s+(mom|mother|dad|father|wife|husband|spouse|son|daughter|kid|child|brother|sister|grandma|grandfather|grandmother|grandpa|baby)\b/i,
        category: 'family',
    },
        ...
        if (match) {
            ...
            switch (category) {
                ...
                case 'fear': {
                    ...
                    const fearMatch = message.match(/(?:worried|scared|afraid|anxious|nervous)\s+(?:about|that)\s+(.{10,50})/i);
                    if (fearMatch) {
                ...
                case 'dream': {
                    const dreamMatch = message.match(/(?:dream|hope|wish|want)\s+(?:to|is|was)\s+(.{10,50})/i);
                    if (dreamMatch) {
 ...
 */
export function mergeMemorableMoments(existing, newMoments) {
 ...
 */
export async function playAmbientMusicDuringSilence() {
    return playAmbient();
}
/**
 ...
 */
export async function getMeaningfulSilenceResponseAsync(persona, context, sessionId) {
 ...
 */
export function buildLLMSilenceInstructions(persona, context) {
 ...
 */
export async function buildLLMSilenceInstructionsAsync(persona, context) {
    const canonicalId = getCanonicalPersonaId(persona.id);
    const dynamicContent = await getSilenceContent(canonicalId);
 ...
 */
function buildLLMSilenceInstructionsInternal(persona, context, dynamicContent) {
    const { silenceDurationSeconds, lastUserMessage, recentEmotionalTone, wasDiscussingTopic, topicsDiscussed, turnCount, userName, currentHour = new Date().getHours(), isMusicPlaying, } = context;
    // Build context hints
    const contextHints = [];
    if (lastUserMessage) {
    ...
    if (isMusicPlaying) {
        contextHints.push('MUSIC IS CURRENTLY PLAYING - You already started music. Do NOT say you cannot play music. ' +
    ...
    // Time of day awareness
    const timeHint = currentHour >= 22 || currentHour < 6
    ...
    const hasTopicsToReference = (topicsDiscussed && topicsDiscussed.length > 0) || wasDiscussingTopic || lastUserMessage;
    ...
    if (silenceDurationSeconds < firstThreshold || recentEmotionalTone === 'heavy') {
        ...
        responseGuidance =
            llmGuidance?.presence?.instruction_template?.replace('{duration}', String(Math.round(silenceDurationSeconds))) || '[TYPE: soft_presence] → [MAX: 5 words OR silence]';
        invitesReply = false;
    }
        ...
        responseGuidance =
            llmGuidance?.memory_callback?.instruction_template?.replace('{duration}', String(Math.round(silenceDurationSeconds))) || '[TYPE: memory_reference] → [REF: something they shared] [NO: questions]';
        invitesReply = false;
    }
        ...
        responseType = 'comfortable_presence';
        responseGuidance =
            llmGuidance?.presence?.instruction_template?.replace('{duration}', String(Math.round(silenceDurationSeconds))) || '[TYPE: presence] → [MAX: 8 words] [TONE: warm]';
        invitesReply = false;
    }
        ...
        responseGuidance =
            llmGuidance?.thoughtful_question?.instruction_template?.replace('{duration}', String(Math.round(silenceDurationSeconds))) || '[TYPE: curious_question] → [ABOUT: specific detail they shared] [NOT: generic]';
        invitesReply = true;
    }
    else {
        ...
        responseGuidance =
            llmGuidance?.check_in?.instruction_template?.replace('{duration}', String(Math.round(silenceDurationSeconds))) || '[TYPE: check_in] → [STYLE: curious not concerned] [MAX: 10 words]';
        invitesReply = true;
    }
    ...
    if (context.memorableMoments && context.memorableMoments.length > 0) {
        conversationContext.push(`Key moments shared: ${context.memorableMoments.slice(0, 3).join('; ')}`);
    }
    if (wasDiscussingTopic) {
 ...
 * }
 */
export function getLLMSilenceInstructions(persona, context) {
 ...
 * }
 */
export async function getLLMSilenceInstructionsAsync(persona, context) {
 ...
 */
export async function preloadSilenceContent(personaId) {
    await getSilenceContent(personaId);
}
/**
 * Clear the silence content cache (for testing)
 */
export function clearSilenceContentCache() {
...
//# sourceMappingURL=meaningful-silence.js.map