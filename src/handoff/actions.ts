...
import type { HandoffContext, PendingHandoff } from './types.js';
import {
  getHandoffState,
  isHandoffAllowed,
  isHandoffInProgress,
...
export function startHandoff(
  ...
  if (!isHandoffAllowed(sessionId)) {
    log.warn(
      ...
      '⏸️ [HANDOFF] Not allowed (rate limited or in progress)'
    );
    return false;
  }
 ...
 */
export function abortHandoff(sessionId: string, reason: string): void {
 ...
 */
export function processNextQueuedHandoff(sessionId: string): boolean {
 ...
 */
export function setupHandoffTimeout(
  sessionId: string,
  onTimeout: () => void,
 ...
 */
export function setupProgressHeartbeat(
  sessionId: string,
  onHeartbeat: () => void,
 ...
 */
export function cleanupHandoffSession(sessionId: string): void {
