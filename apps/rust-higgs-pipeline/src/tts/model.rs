...
use crate::tts::config::{HiggsAudioConfig, TextConfig};
...
impl HiggsAudioModel {
    ...
    /// Slide the KV cache window: keep only the most recent `window_size` entries.
    ///
    /// This is used for long-form generation to prevent unbounded memory growth.
    /// Drops the oldest `seq_len - window_size` entries from each layer's cache.
    ///
    /// **Critical**: `cache.offset` is NOT reset — rotary embeddings use absolute
    /// positions, and the retained K/V tensors already have correct position
    /// information baked in from when they were originally computed.
    pub fn slide_caches(&mut self, window_size: usize) -> candle_core::Result<()> {
        for cache in self.caches.iter_mut() {
            if let (Some(k), Some(v)) = (&cache.k, &cache.v) {
                let seq_len = k.dim(2)?; // [batch, heads, seq, dim]
                if seq_len > window_size {
                    let drop = seq_len - window_size;
                    cache.k = Some(k.narrow(2, drop, window_size)?);
                    cache.v = Some(v.narrow(2, drop, window_size)?);
                    // DO NOT reset offset — rotary embeddings use absolute position
                }
            }
        }
        Ok(())
...
mod tests {
    ...
    #[test]
    fn kv_cache_slide_trims_to_window() {
        let device = Device::Cpu;
        // Simulate a cache with 10 steps: shape [1, 4, 10, 64]
        let mut cache = KvCache::new();
        let k = Tensor::zeros((1, 4, 10, 64), DType::F32, &device).unwrap();
        let v = Tensor::zeros((1, 4, 10, 64), DType::F32, &device).unwrap();
        cache.k = Some(k);
        cache.v = Some(v);
        cache.offset = 10;

        // Slide to window_size=6 — should drop the oldest 4 entries
        let mut caches = vec![cache];
        // Inline slide logic (mirrors HiggsAudioModel::slide_caches)
        for c in caches.iter_mut() {
            if let (Some(k), Some(v)) = (&c.k, &c.v) {
                let seq_len = k.dim(2).unwrap();
                if seq_len > 6 {
                    let drop = seq_len - 6;
                    c.k = Some(k.narrow(2, drop, 6).unwrap());
                    c.v = Some(v.narrow(2, drop, 6).unwrap());
                }
            }
        }

        let c = &caches[0];
        assert_eq!(c.k.as_ref().unwrap().dim(2).unwrap(), 6);
        assert_eq!(c.v.as_ref().unwrap().dim(2).unwrap(), 6);
        // Offset must NOT be reset — rotary uses absolute position
        assert_eq!(c.offset, 10);
    }

    #[test]
    fn kv_cache_slide_noop_when_within_window() {
        let device = Device::Cpu;
        let mut cache = KvCache::new();
        ...
        // Window = 8, cache only has 5 — no-op
        let mut caches = vec![cache];
        for c in caches.iter_mut() {
            if let (Some(k), Some(v)) = (&c.k, &c.v) {
                let seq_len = k.dim(2).unwrap();
                if seq_len > 8 {
                    let drop = seq_len - 8;
                    c.k = Some(k.narrow(2, drop, 8).unwrap());
                    c.v = Some(v.narrow(2, drop, 8).unwrap());
                }
            }
        }

        assert_eq!(caches[0].k.as_ref().unwrap().dim(2).unwrap(), 5);
        assert_eq!(caches[0].offset, 5);
    }

    #[test]
    fn kv_cache_slide_empty_cache_is_safe() {
        // Empty cache (None) — slide should not panic
        ...
        for c in caches.iter_mut() {
            if let (Some(k), Some(v)) = (&c.k, &c.v) {
                ...
                if seq_len > 4 {
                    let drop = seq_len - 4;
                    c.k = Some(k.narrow(2, drop, 4).unwrap());
                    c.v = Some(v.narrow(2, drop, 4).unwrap());
                }
            }
        }

        assert!(caches[0].k.is_none());
        assert!(caches[0].v.is_none());
