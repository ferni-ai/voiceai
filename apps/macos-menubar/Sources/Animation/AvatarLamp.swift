...
struct AvatarLamp: ViewModifier {
    let isActive: Bool
    ...
    func triggerTilt(direction: TiltDirection) {
...
extension View {
    ...
    func avatarLamp(isActive: Bool) -> some View {
...
class LampController: ObservableObject {
    var onNod: (() -> Void)?
    var onTilt: ((AvatarLamp.TiltDirection) -> Void)?
    var onBounce: (() -> Void)?
    var onPerkUp: (() -> Void)?

    func triggerAnimation(_ animation: LampAnimation) {
        switch animation {
        case .none:
            break
        case .nod:
