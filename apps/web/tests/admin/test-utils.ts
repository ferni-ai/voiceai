 ...
 */
export function isAccessible(element: Element | null, options: {
  role?: string;
  label?: string;
  labelledBy?: string;
  describedBy?: string;
...
export function usesDesignTokens(html: string): { valid: boolean; violations: string[] } {
  ...
  if (pxMatches.length > 10) {
    violations.push(`Many hardcoded pixel values (${pxMatches.length}), consider using spacing tokens`);
  }
 ...
 */
export function simulateKeyPress(element: Element, key: string, options: KeyboardEventInit = {}): void {
