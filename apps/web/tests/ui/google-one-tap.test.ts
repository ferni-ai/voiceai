...
describe('Google One-Tap Service', () => {
  ...
  describe('Cooldown Management', () => {
    it('isInCooldown returns false when no dismissal recorded', async () => {
      const { isInCooldown } = await import('../../src/services/google-one-tap.service.js');
    ...
    it('clearCooldown removes cooldown data', async () => {
      const { clearCooldown, isInCooldown } = await import(
        '../../src/services/google-one-tap.service.js'
      );
  ...
  describe('cancelOneTap', () => {
    it('calls google cancel when prompt is showing', async () => {
      ...
      // Eligibility requires MIN_SESSION_MS (5s) of session time, so advance past
      // it - with only 1000ms shouldShowOneTap() bails on "session too short",
      // isPromptShowing stays false and cancelOneTap() is a no-op.
  ...
  describe('Integration with Firebase Auth', () => {
    it('does not show prompt when user is already linked', async () => {
      ...
      const { getAuthState } = await import('../../src/services/firebase-auth.service.js');
      vi.mocked(getAuthState).mockReturnValue(mockLinkedAuthState);

      const { showOneTapIfEligible } = await import('../../src/services/google-one-tap.service.js');
  ...
  describe('Event Dispatch', () => {
    it('dispatches ferni:one-tap-success on successful credential', async () => {
      ...
      const { linkWithGoogleCredential } = await import('../../src/services/firebase-auth.service.js');
...
describe('Storage Keys', () => {
  it('uses correct storage keys', async () => {
    const { clearCooldown } = await import('../../src/services/google-one-tap.service.js');
