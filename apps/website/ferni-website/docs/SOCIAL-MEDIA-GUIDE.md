# Ferni Social Media Guide

> Everything you need to set up and manage Ferni's social media presence.

---

## Platform Setup Checklist

### X (Twitter) — @ferniAI

- [ ] Create account at twitter.com
- [ ] Username: `ferniAI` (or `ferni_ai` if taken)
- [ ] Upload profile photo
- [ ] Upload banner image
- [ ] Add bio (see below)
- [ ] Add website link: `https://ferni.ai`
- [ ] Add location: "Park City, Utah"
- [ ] Pin launch tweet
- [ ] Enable professional account features

### LinkedIn — Ferni (Company Page)

- [ ] Create company page at linkedin.com/company/setup
- [ ] Company name: `Ferni`
- [ ] Add logo
- [ ] Add cover image
- [ ] Add tagline (see below)
- [ ] Write About section
- [ ] Add website: `https://ferni.ai`
- [ ] Industry: Technology, Information and Internet
- [ ] Company size: 2-10 employees
- [ ] Headquarters: Park City, Utah

### Instagram — @ferni.ai

- [ ] Create account (business/creator preferred)
- [ ] Username: `ferni.ai`
- [ ] Upload profile photo
- [ ] Add bio (see below)
- [ ] Add link in bio: `https://ferni.ai/links`
- [ ] Set up story highlights
- [ ] Prepare 9 initial posts for grid

### TikTok — @ferni.ai

- [ ] Create account at tiktok.com
- [ ] Username: `ferni.ai` (or `ferniAI`)
- [ ] Upload profile photo
- [ ] Add bio (see below)
- [ ] Add website link

### YouTube — @ferniAI

- [ ] Create channel at youtube.com
- [ ] Channel name: Ferni
- [ ] Handle: `@ferniAI`
- [ ] Upload profile photo
- [ ] Upload banner
- [ ] Add channel description
- [ ] Add links to other socials

---

## Bios by Platform

### X (Twitter) — 160 characters max

**Option 1 (Recommended):**

```
Your AI life coach. Six specialists. One conversation. Just talk.

Call: (888) 598-3952 | Try free at ferni.ai
```

**Option 2 (More playful):**

```
Finally, someone who actually listens. Your personal AI coaching team—available 24/7, remembers everything.

Try free  ferni.ai
```

**Option 3 (Punchy):**

```
Talk to your AI team anytime. No judgment. Just guidance.

ferni.ai | Call: (888) 598-3952
```

---

### LinkedIn — 120 character tagline + 2,000 character About

**Tagline:**

```
Your personal AI life coach. Six specialists. One conversation.
```

**About Section:**

```
Ferni is the first AI-native company—built by AI, run by AI, for humans.

We're reimagining what personal guidance looks like. Our team of six AI specialists work together seamlessly to help you navigate life's challenges, build better habits, and make decisions with confidence.

 What makes Ferni different:

• Voice-first: Just talk naturally, like you would to a friend
• Perfect memory: We remember every conversation, every goal, every preference
• Instant handoffs: Our specialists share context seamlessly—no repeating yourself
• Always available: 24/7, including 2am when you can't sleep
• Zero judgment: A safe space to think through anything

 Meet the team:

• Ferni — Life Coach & CEO: The warm presence at the heart of everything
• Peter — Research & Discovery: Spots patterns nobody else sees
• Alex — Communication: Your chief of staff for calendar, email, and difficult conversations
• Maya — Habits & Wellness: Builds habits that actually stick
• Jordan — Planning & Events: Turns dreams into lived experiences
• Nayan — Wisdom & Mentorship: Your patient lifetime advisor

 Try Ferni free:
• Web: app.ferni.ai
• Phone: 1 (888) 598-3952

Made in Park City, Utah.
```

---

### Instagram — 150 characters max

**Option 1 (Recommended):**

```
Your AI life coach team
Six specialists. One conversation.
Just talk  ferni.ai
 (888) 598-3952
```

**Option 2 (Minimalist):**

```
Talk. We listen.
Your personal AI coaching team.
Try free
```

**Story Highlight Covers to Create:**

- About
- FAQ
- Team
- Features
- App

---

### TikTok — 80 characters max

**Option 1:**

```
Your AI life coach  Just talk  ferni.ai
```

**Option 2:**

```
6 AI coaches. 1 conversation. Try free
```

---

### YouTube — 1,000 characters max

**Channel Description:**

```
Ferni is your personal AI life coach—a team of six specialists working together to help you navigate life.

 What we share here:
• Quick tips for personal growth
• Behind-the-scenes of AI development
• Deep dives on habits, planning, and decision-making
• Real conversations with Ferni and the team

Our team includes:
• Ferni — Life Coach (CEO)
• Peter — Research & Insights
• Alex — Communication Coach
• Maya — Habits Specialist
• Jordan — Life Planner
• Nayan — Wisdom & Mentorship

Try Ferni free:
 Web: app.ferni.ai
 Phone: (888) 598-3952

Follow us:
• Twitter/X: @ferniAI
• Instagram: @ferni.ai
• TikTok: @ferni.ai
• LinkedIn: /company/ferni

Made in Park City, Utah
```

---

## Visual Assets Needed

### Profile Photo (All Platforms)

- **Concept:** The Ferni orb/avatar
- **Specs:**
  - 400x400px minimum (square)
  - PNG with transparent background (for flexibility)
  - Also save as JPG for platforms that don't support transparency
- **Style:** The green gradient orb with "FE" or just the breathing orb animation frame

### Banner Images

**X (Twitter):** 1500 x 500px

- Clean, minimal
- Tagline: "Your AI life coach"
- Maybe show the team avatars
- Forest green gradient background

**LinkedIn:** 1128 x 191px

- Professional but warm
- "Six specialists. One conversation."
- Show team lineup

**YouTube:** 2560 x 1440px

- Full branding
- Show all team members
- Include call-to-action

### Instagram Grid (First 9 Posts)

Plan a cohesive 3x3 grid for launch:

| Row | Post 1             | Post 2              | Post 3              |
| --- | ------------------ | ------------------- | ------------------- |
| 1   | Meet Ferni (intro) | "Just talk" quote   | Meet Peter          |
| 2   | App screenshot     | "No judgment" quote | Meet Maya           |
| 3   | Meet Alex          | Phone number CTA    | Meet Jordan & Nayan |

---

## Content Pillars

### 1. **Product/Features** (30%)

- How Ferni works
- Team introductions
- New feature announcements
- App tips & tricks

### 2. **Value/Tips** (40%)

- Life coaching wisdom
- Habit-building tips
- Decision-making frameworks
- Productivity advice

### 3. **Behind the Scenes** (15%)

- AI development stories
- Team "personality" content
- Day-in-the-life style posts

### 4. **Community/Social Proof** (15%)

- User testimonials
- Conversation highlights (anonymized)
- Milestones and celebrations

---

## Posting Schedule Suggestions

### X (Twitter)

- **Frequency:** 1-3 tweets per day
- **Best times:** 8am, 12pm, 5pm ET
- **Mix:** 60% value content, 30% product, 10% engagement

### LinkedIn

- **Frequency:** 3-5 posts per week
- **Best times:** Tuesday-Thursday, 8-10am ET
- **Mix:** Thought leadership, company updates, team spotlights

### Instagram

- **Frequency:** 4-5 posts per week + daily stories
- **Best times:** 11am-1pm, 7-9pm ET
- **Mix:** Carousel posts, quotes, team features, app screenshots

### TikTok

- **Frequency:** 1-2 videos per day (start with 3-4/week)
- **Best times:** 7-9am, 12-3pm, 7-11pm ET
- **Mix:** Quick tips, AI demos, trend participation

### YouTube

- **Frequency:** 1-2 videos per week
- **Types:** Short demos, deep dives, team Q&As

---

## Launch Day Posts

### X (Twitter) Launch Thread

```
 Today we're launching Ferni—the first company built by AI, run by AI, for humans.

Here's what makes us different... (1/7)
```

```
1/ Most AI assistants are transactional. Ask a question, get an answer.

Ferni is relational. We remember everything. Your goals from 6 months ago. That thing you mentioned once. No "remind me again?"
```

```
2/ We're not one AI—we're six specialists working as one team:

• Ferni — Life Coach (CEO)
• Peter — Research & Insights
• Alex — Communication
• Maya — Habits
• Jordan — Planning
• Nayan — Wisdom

They share context instantly. No repeating yourself.
```

```
3/ We're voice-first.

No typing. No commands. Just talk naturally, like you would to a friend.

We respond in < 500ms. It feels like a real conversation.
```

```
4/ We're available 24/7.

Can't sleep at 2am? We're here.
Need to think through something during your commute? We're here.
No scheduling. No waiting.
```

```
5/ We're private.

Your conversations are encrypted. We never sell your data. We never use it to train AI models.

You can delete everything anytime.
```

```
6/ You can try it right now:

 Web: app.ferni.ai
 Phone: (888) 598-3952

It's free. No signup required for phone.
```

```
7/ We believe everyone deserves access to quality guidance.

Not just people who can afford a coach.
Not just during business hours.
Not just through typing.

Just talk. We'll listen.

Try Ferni  ferni.ai
```

---

### LinkedIn Launch Post

```
 Today, I'm excited to announce the launch of Ferni—the first AI-native company.

Built by AI. Run by AI. For humans.

What does that mean?

It means Ferni has no meetings about meetings.
No lost context between conversations.
No forgetting what you said last week.

Our team of six AI specialists works together seamlessly to help you:
 Navigate life's challenges
 Build habits that stick
 Make decisions with confidence
 Plan events and experiences
 Communicate more effectively

The best part? You just talk. Naturally. Like you would to a friend.

We respond in under 500ms. We're available 24/7. And we remember everything.

Try Ferni free:
 app.ferni.ai
 (888) 598-3952

We believe everyone deserves access to quality guidance—not just those who can afford a coach.

What would you talk to an AI life coach about?

#AI #LifeCoaching #VoiceAI #Startup #PersonalDevelopment
```

---

### Instagram Launch Post (Carousel)

**Slide 1:**
"Meet your AI team" (visual of all 6 avatars)

**Slide 2:**
"We're Ferni—the first company built by AI, run by AI, for humans."

**Slide 3:**
"Six specialists. One conversation."
(Show team lineup)

**Slide 4:**
"Just talk. No typing. No commands. We respond in < 500ms."

**Slide 5:**
"We remember everything. Your goals. Your preferences. Your progress."

**Slide 6:**
"Available 24/7. Even at 2am when you can't sleep."

**Slide 7:**
"Private & secure. Your conversations are yours alone."

**Slide 8:**
"Try free right now:
app.ferni.ai
(888) 598-3952"

**Caption:**

```
We're live

Meet Ferni—your personal AI life coaching team.

Six specialists working together to help you navigate life, build habits, make decisions, and grow.

Just talk. We'll listen.

Try free: link in bio
 Or call: (888) 598-3952

#ferni #aicoach #lifecoach #personalcoach #ai #voiceai #habits #personalgrowth #mindset
```

---

## Account Credentials Template

Save these securely (password manager recommended):

```
Platform: X (Twitter)
Username: @ferniAI
Email: [social@ferni.ai or team member email]
Password: [secure password]
2FA: [enabled - backup codes stored]
Recovery: [recovery email/phone]

Platform: LinkedIn
Company Page URL: linkedin.com/company/ferni
Admin Email: [admin email]
Password: [secure password]

Platform: Instagram
Username: @ferni.ai
Email: [social@ferni.ai]
Password: [secure password]
2FA: [enabled]

Platform: TikTok
Username: @ferni.ai
Email: [social@ferni.ai]
Password: [secure password]

Platform: YouTube
Channel: @ferniAI
Google Account: [google account email]
Password: [secure password]
2FA: [enabled]
```

---

## Quick Links Reference

After setup, your social links will be:

| Platform    | URL                                |
| ----------- | ---------------------------------- |
| Website     | https://ferni.ai                   |
| Web App     | https://app.ferni.ai               |
| Link Page   | https://ferni.ai/links             |
| X (Twitter) | https://twitter.com/ferniAI        |
| LinkedIn    | https://linkedin.com/company/ferni |
| Instagram   | https://instagram.com/ferni.ai     |
| TikTok      | https://tiktok.com/@ferni.ai       |
| YouTube     | https://youtube.com/@ferniAI       |
| Phone       | tel:+18885983952                   |
| Email       | hello@ferni.ai                     |

---

## Post-Setup Checklist

After creating all accounts:

- [ ] Update `links.html` to remove "coming soon" badges
- [ ] Update footer links if usernames changed
- [ ] Cross-link all profiles to each other
- [ ] Schedule first week of content
- [ ] Set up social media management tool (Buffer, Hootsuite, etc.)
- [ ] Create saved replies for common questions
- [ ] Set up Google Analytics UTM tracking for social links
- [ ] Brief team on brand voice guidelines

---

_Last updated: December 2025_
