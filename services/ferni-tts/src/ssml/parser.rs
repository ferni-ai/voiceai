    ...
    pub fn parse(&self, input: &str) -> Result<SsmlDocument> {
        ...
        loop {
            match reader.read_event() {
                ...
                Err(e) => {
                    return Err(Error::ssml_parse(format!("XML error: {}", e), reader.error_position()));
                }
                _ => continue,
            }
        }
            ...
            match reader.read_event() {
                ...
                Ok(Event::Text(e)) => {
                    let text = e.unescape().map_err(|err| {
                        Error::ssml_parse(format!("Text decode error: {}", err), reader.error_position())
                    })?;
                ...
                Ok(Event::Eof) => {
                    return Err(Error::ssml_parse("Unexpected end of document", reader.error_position()));
                }
                Err(e) => {
                    return Err(Error::ssml_parse(format!("XML error: {}", e), reader.error_position()));
                }
